...
static XmTraversalNode TraverseControl(XmTraversalNode cur_node, XmTraversalDirection action);
static XmTraversalNode NextControl(XmTraversalNode ctl_node);
static XmTraversalNode PrevControl(XmTraversalNode ctl_node);
static Boolean InitializeCurrent(XmTravGraph list, Widget wid, Boolean renew_list_if_needed);
static XmTraversalNode GetNextNearestNode(XmGraphNode graph, XRectangle *rect, XmDirection layout);
static XmTraversalNode TraverseTab(XmTraversalNode cur_node, XmTraversalDirection action);
static XmTraversalNode AllocListEntry(XmTravGraph list);
static Boolean GetChildList(Widget composite, Widget **widget_list, Cardinal *num_in_list);
static void GetNodeList(Widget wid,
                        XRectangle *parent_rect,
                        XmTravGraph trav_list,
                        int tab_parent,
                        int control_parent);
static XmTraversalNode GetNodeFromGraph(XmGraphNode graph, Widget wid);
static XmTraversalNode GetNodeOfWidget(XmTravGraph trav_list, Widget wid);
static void LinkNodeList(XmTravGraph list);
static Boolean NodesOverlap(XmTraversalNode nodeA, XmTraversalNode nodeB, Boolean horizontal);
...
static Boolean NodeDominates(XmTraversalNode node_1,
                             ...
                             Boolean horizontal,
                             XmDirection layout);
static void Sort(XmTraversalNode *nodes, size_t n_mem, Boolean horizontal, XmDirection layout);
...
static void SortTabGraph(XmGraphNode graph, Boolean exclusive, XmDirection layout);
static void SortControlGraph(XmGraphNode graph, Boolean exclusive, XmDirection layout);
static void SortNodeList(XmTravGraph trav_list);
static Boolean SetInitialNode(XmGraphNode graph, XmTraversalNode init_node);
static void SetInitialWidgets(XmTravGraph trav_list);
static void GetRectRelativeToShell(Widget wid, XRectangle *rect);
static int SearchTabList(XmTravGraph graph, Widget wid);
static void DeleteFromTabList(XmTravGraph graph, int indx);
static Boolean LastControl(Widget w, XmTraversalDirection dir, XmTravGraph graph);
static XmTraversalDirection LocalDirection(Widget w, XmTraversalDirection direction);
...
XmNavigability _XmGetNavigability(Widget wid)
{
  if (XtIsRectObj(wid) && !wid->core.being_destroyed) {
    XmBaseClassExt *er;
    if ((er = _XmGetBaseClassExtPtr(XtClass(wid), XmQmotif)) && (*er) &&
        ((*er)->version >= XmBaseClassExtVersion) && (*er)->widgetNavigable)
    {
      ...
      if (XmIsPrimitive(wid))
        ...
        wc = NULL;
      if (wc && (er = _XmGetBaseClassExtPtr(wc, XmQmotif)) && (*er) &&
          ((*er)->version >= XmBaseClassExtVersion) && (*er)->widgetNavigable)
      {
        return (*((*er)->widgetNavigable))(wid);
      }
...
Boolean _XmIsViewable(Widget wid)
{
...
Widget _XmIsScrollableClipWidget(Widget child, Boolean scrollable, XRectangle *visRect)
{
  Widget wid = XtParent(child);
  XmScrolledWindowWidget sw;
  if (wid && XmIsClipWindow(wid) && ((sw = (XmScrolledWindowWidget)XtParent(wid)) != NULL) &&
      XmIsScrolledWindow(sw) && ((Widget)sw->swindow.ClipWindow == wid) &&
...
Boolean _XmGetEffectiveView(Widget wid, XRectangle *visRect)
{
  ...
  for (prev = wid; ((wid = XtParent(wid)) != NULL) && !XtIsShell(wid); prev = wid) {
    if (!_XmIsViewable(wid)) {
      _XmClearRect(visRect);
      return FALSE;
    }
    if (_XmIsScrollableClipWidget(prev, True, visRect)) {
      ...
      if (acceptClipping) {
        ...
        if (!_XmIntersectionOf(visRect, &widRect, &tmpRect) || (visRect->width != tmpRect.width) ||
...
XmNavigationType _XmGetNavigationType(Widget widget)
{
...
Widget _XmGetActiveTabGroup(Widget wid)
{
...
static Boolean NodeIsTraversable(XmTraversalNode node)
{
  return (node && node->any.widget && (node->any.type != XmTAB_GRAPH_NODE) &&
          (node->any.type != XmCONTROL_GRAPH_NODE) && XmIsTraversable(node->any.widget));
}
...
static XmTraversalNode NextControl(XmTraversalNode ctl_node)
{
  ...
  do {
    if ((ptr > ctl_node) && ((ptr < next) || (next == NULL)))
      next = ptr;
    if (ptr < min)
      min = ptr;
    ptr = ptr->any.next;
...
static XmTraversalNode PrevControl(XmTraversalNode ctl_node)
{
  ...
  do {
    if ((ptr < ctl_node) && ((ptr > prev) || (prev == NULL)))
      prev = ptr;
    if (ptr > max)
      max = ptr;
    ptr = ptr->any.prev;
...
static Boolean InitializeCurrent(XmTravGraph list, Widget wid, Boolean renew_list_if_needed)
{
  ...
  if (!(cur_node = GetNodeOfWidget(list, wid))) {
    ...
    while ((wid = XtParent(wid)) && !XtIsShell(wid) && !(cur_node = GetNodeOfWidget(list, wid)))
      /*EMPTY*/;
  }
  if (cur_node)
...
Widget _XmTraverseAway(XmTravGraph list, Widget wid, Boolean wid_is_control)
{
  ...
  if ((list->current->any.widget != wid) && (list->current->any.type == XmTAB_GRAPH_NODE)) {
    ...
    if ((nearest_node = GetNextNearestNode(
             (XmGraphNode)list->current, &wid_rect, GetLayout(list->current->any.widget))) != NULL)
    {
      list->current = nearest_node;
    }
  }
  if ((list->current->any.widget == wid) || !NodeIsTraversable(list->current)) {
...
static XmTraversalNode GetNextNearestNode(XmGraphNode graph, XRectangle *rect, XmDirection layout)
{
  ...
  if ((node = graph->sub_head) != NULL) {
    ...
    node_list = (XmTraversalNode *)XmStackAlloc(num_nodes * sizeof(XmTraversalNode), storage);
...
Widget _XmTraverse(XmTravGraph list,
                   ...
                   Widget reference_wid)
{
  ...
  *local_dir = action;
  if ((action == XmTRAVERSE_CURRENT) && reference_wid) {
    ...
    if ((wid_nav == XmTAB_NAVIGABLE) || (wid_nav == XmCONTROL_NAVIGABLE)) {
  ...
  if (action == XmTRAVERSE_CURRENT) {
    ...
    if ((list->current->any.type == XmTAB_NODE) || (list->current->any.type == XmCONTROL_NODE)) {
  ...
  switch (action) {
    ...
    case XmTRAVERSE_GLOBALLY_BACKWARD:
      ...
      if (traverse_control)
        ...
        *local_dir = (action == XmTRAVERSE_GLOBALLY_FORWARD ? XmTRAVERSE_NEXT_TAB_GROUP :
  ...
  if (traverse_control)
    ...
    if (!rtnNode &&
        ((action == XmTRAVERSE_GLOBALLY_FORWARD) || (action == XmTRAVERSE_GLOBALLY_BACKWARD)))
    {
...
static XmTraversalNode TraverseTab(XmTraversalNode cur_node, XmTraversalDirection action)
{
  ...
  do {
    switch (action) {
      ...
      case XmTRAVERSE_CURRENT:
      default:
        if ((new_tab->any.type == XmTAB_GRAPH_NODE) && (new_tab->graph.sub_head)) {
        ...
        /* new_tab is not a tab graph with a non-null subgraph. */
          ...
          while (((new_tab = (XmTraversalNode)new_tab->any.tab_parent.link) != NULL) &&
                 ((action != XmTRAVERSE_CURRENT) || (new_tab != cur_node)) && !(new_tab->any.next))
          {
            top_node = new_tab;
          }
      ...
      case XmTRAVERSE_PREV_TAB_GROUP:
         ...
         */
        if ((new_tab->any.type == XmTAB_GRAPH_NODE) && (new_tab->graph.sub_tail)) {
          ...
          XmTraversalNode top_node = new_tab;
          while ((new_tab = (XmTraversalNode)new_tab->any.tab_parent.link) && !(new_tab->any.prev))
          {
            top_node = new_tab;
          }
          if (!new_tab)
...
void _XmFreeTravGraph(XmTravGraph trav_list)
{
...
void _XmTravGraphRemove(XmTravGraph tgraph, Widget wid)
{
  XmTraversalNode node;
...
void _XmTravGraphAdd(XmTravGraph tgraph, Widget wid)
{
  if (tgraph->num_entries && !GetNodeOfWidget(tgraph, wid))
    _XmFreeTravGraph(tgraph);
}

void _XmTravGraphUpdate(XmTravGraph tgraph, Widget wid) /* unused */
{
  _XmFreeTravGraph(tgraph);
}

Boolean _XmNewTravGraph(XmTravGraph trav_list, Widget top_wid, Widget init_current)
{
  ...
  if ((trav_list->num_entries + XmTRAV_LIST_ALLOC_INCREMENT) < trav_list->num_alloc) {
...
static Boolean GetChildList(Widget composite, Widget **widget_list, Cardinal *num_in_list)
{
  ...
  if (XmIsManager(composite) &&
      (mext = (XmManagerClassExt *)_XmGetClassExtensionPtr(
           (XmGenericClassExt *)&(
               ((XmManagerWidgetClass)XtClass(composite))->manager_class.extension),
           NULLQUARK)) &&
      *mext && (*mext)->traversal_children)
  {
    return (*((*mext)->traversal_children))(composite, widget_list, num_in_list);
  }
  return FALSE;
}

static void GetNodeList(
    Widget wid, XRectangle *parent_rect, XmTravGraph trav_list, int tab_parent, int control_parent)
{
  ...
  if (wid->core.being_destroyed || (!(node_type = _XmGetNavigability(wid)) && !XtIsShell(wid)))
    return;
  ...
  list_entry->any.nav_type = (list_entry_offset ? _XmGetNavigationType(wid) : XmSTICKY_TAB_GROUP);
  if (node_type == XmCONTROL_NAVIGABLE) {
  ...
  else if (((node_type == XmNOT_NAVIGABLE) && list_entry_offset) || !XtIsComposite(wid)) {
    ...
    if (!(free_child_list = GetChildList(wid, &trav_children, &num_trav_children))) {
                        ...
                        XmDirection layout)
{
  ...
  Comparator compare = (horizontal ? HorizNodeComparator(layout) : VertNodeComparator(layout));
...
static void Sort(XmTraversalNode *list, size_t n_mem, Boolean horizontal, XmDirection layout)
{
      ...
      if (new_row) {
        ...
        if (horizontal) {
          might_overlap = (rows[row].max_hint > node->any.rect.y) &&
                          (rows[row].min_hint < (node->any.rect.y + node->any.rect.height));
        }
        else {
          might_overlap = (rows[row].max_hint > node->any.rect.x) &&
                          (rows[row].min_hint < (node->any.rect.x + node->any.rect.width));
        }
      }
      /* Append this node if it overlaps the end of the row. */
      if (NodesOverlap(node, rows[row].items[row_len - 1], horizontal)) {
        TruncateRow(rows + row, row_len, list, &first_free, n_mem, horizontal, layout);
  ...
  first_free = 0;
  if ((horizontal && XmDirectionMatchPartial(layout, XmLEFT_TO_RIGHT, XmHORIZONTAL_MASK)) ||
      (!horizontal && XmDirectionMatchPartial(layout, XmTOP_TO_BOTTOM, XmVERTICAL_MASK)))
  {
    /* Nodes appear in the desired order. */
    for (row = 0; row < num_rows; row++) {
      assert(rows[row].num_items && rows[row].items);
      memcpy(list + first_free, rows[row].items, sizeof(XmTraversalNode) * rows[row].num_items);
...
static void SortTabGraph(XmGraphNode graph, Boolean exclusive, XmDirection layout)
{
  ...
  if (head) {
    ...
    if (num_nodes > 1) {
      if (exclusive)
        qsort(node_list, num_nodes, sizeof(XmTraversalNode), CompareExclusive);
      else {
        ...
        horizontal = XmDirectionMatchPartial(layout, XmPRECEDENCE_HORIZ_MASK, XmPRECEDENCE_MASK);
        reverse = (horizontal ?
        ...
        if (reverse) {
          ...
          for (idx = 0; idx < ((num_nodes - 1) / 2); idx++) {
            memcpy((char *)&tmp, (char *)(node_list + idx + 1), sizeof(XmTraversalNode));
            memcpy((char *)(node_list + idx + 1),
                   (char *)(node_list + num_nodes - idx - 1),
                   sizeof(XmTraversalNode));
            memcpy(
                (char *)(node_list + num_nodes - idx - 1), (char *)&tmp, sizeof(XmTraversalNode));
          }
          /* Rotate the initial element into the proper position. */
          if (num_nodes > 2) {
            memcpy((char *)&tmp, (char *)(node_list + num_nodes - 1), sizeof(XmTraversalNode));
            memmove((char *)(node_list + 2),
                    (char *)(node_list + 1),
                    (num_nodes - 2) * sizeof(XmTraversalNode));
            memcpy((char *)(node_list + 1), (char *)&tmp, sizeof(XmTraversalNode));
          }
        }
      }
    }
...
static void SortControlGraph(XmGraphNode graph, Boolean exclusive, XmDirection layout)
{
  ...
  if (head) {
    ...
    if (!XmDirectionMatchPartial(layout, XmPRECEDENCE_HORIZ_MASK, XmPRECEDENCE_MASK)) {
...
static void SortNodeList(XmTravGraph trav_list)
{
  ...
  while (cnt++ < trav_list->num_entries) {
    if (ptr->any.type == XmTAB_GRAPH_NODE) {
      SortTabGraph((XmGraphNode)ptr, !!(trav_list->exclusive), GetLayout(ptr->any.widget));
    }
    else if (ptr->any.type == XmCONTROL_GRAPH_NODE) {
      SortControlGraph((XmGraphNode)ptr, !!(trav_list->exclusive), GetLayout(ptr->any.widget));
    }
    ++ptr;
  }
  _XmProcessUnlock();
...
Boolean _XmSetInitialOfTabGraph(XmTravGraph trav_graph, Widget tab_group, Widget init_focus)
{
  XmTraversalNode tab_node = GetNodeOfWidget(trav_graph, tab_group);
  XmGraphNode control_graph_node;
  if (tab_node &&
      ((tab_node->any.type == XmTAB_GRAPH_NODE) || (tab_node->any.type == XmCONTROL_GRAPH_NODE)))
  {
    if (SetInitialNode((XmGraphNode)tab_node,
                       GetNodeFromGraph((XmGraphNode)tab_node, init_focus)) ||
        ((control_graph_node = (XmGraphNode)GetNodeFromGraph((XmGraphNode)tab_node, tab_group)) &&
         SetInitialNode(control_graph_node, GetNodeFromGraph(control_graph_node, init_focus)) &&
...
static void SetInitialWidgets(XmTravGraph trav_list)
{
  ...
  unsigned cnt = 0;
  while (cnt < trav_list->num_entries) {
    if (((ptr->any.type == XmTAB_GRAPH_NODE) || (ptr->any.type == XmCONTROL_GRAPH_NODE)) &&
        ptr->graph.sub_head)
    {
      if (ptr->any.widget && XmIsManager(ptr->any.widget) &&
          (init_focus = ((XmManagerWidget)ptr->any.widget)->manager.initial_focus) &&
...
void _XmTabListAdd(XmTravGraph graph, Widget wid)
{
...
void _XmTabListDelete(XmTravGraph graph, Widget wid)
{
...
static Boolean LastControl(Widget w, XmTraversalDirection dir, XmTravGraph graph)
{
  ...
  } while (new_ctl && !NodeIsTraversable(new_ctl) && (new_ctl != cur_node));
  return False;
}
