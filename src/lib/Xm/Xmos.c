      ...
      if (loopCount < 2) {
        ...
        do {
           ...
           */
                 ...
                 ((dirNameLen == 2) && (dirName[0] == '.') && (dirName[1] == '.')));
        if (dirName == NULL)
                        ...
                        unsigned int *pNumAlloc)   /* Cannot be NULL. */
{
  ...
  if (!*nextPatternPtr) {
    ...
    _XmOSGetDirEntries(
        qualifiedDir, pattern, typeMask, FALSE, TRUE, pEntries, pNumEntries, pNumAlloc);
  }
  else {
    ++nextPatternPtr; /* Move past '/' character.*/
    localEntries = NULL;
    _XmOSGetDirEntries(qualifiedDir,
                       pattern,
                       XmFILE_DIRECTORY,
                       TRUE,
                       TRUE,
    ...
    while (entryIndex < localNumEntries) {
      _XmOSBuildFileList(
          localEntries[entryIndex], nextPatternPtr, typeMask, pEntries, pNumEntries, pNumAlloc);
...
if (!pwd)
  pwd = getcwd(buf, MAX_DIR_PATH_LEN) return pwd;
#endif
/*
 * buf must be of length MAX_DIR_PATH_LEN
 */
 ...
 *
 ************************************************************/
void _XmOSFindPathParts(String path, String *filenameRtn, String *suffixRtn)
{
...
XmDirection _XmOSGetCharDirection(XtPointer character, /* unused */
                                  XmTextType type,
                                  XmStringTag locale) /* unused */
{
  /* ??? This is a temporary stub implementation. */
...
static XmOSMethodEntry method_table[] = {
    {XmMCharDirection, (XtPointer)_XmOSGetCharDirection, NULL, NULL},
    {XmMInitialCharsDirection, (XtPointer)_XmOSGetInitialCharsDirection, NULL, NULL},
    {NULL, NULL, NULL, NULL}};
    ...
    if (method_id == method_table[i].method_id) {
      if (*method == NULL || (method_table[i].method != NULL && *method != method_table[i].method))
      {
        *method = method_table[i].method;
        if (os_data)
          *os_data = method_table[i].os_data;
 ...
 */
int _XmOSKeySymToCharacter(KeySym keysym, char *locale, char *buffer)
{
  /*
