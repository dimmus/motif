...
#include <ctype.h>
#include <stdio.h>
static void ButtonEventHandler(Widget w, XtPointer data, XEvent *event, Boolean *cont);
static void PopupMenuEventHandler(Widget wid, XtPointer ignored, XEvent *e, Boolean *cont);
static void AddHandlersToPostFromWidget(Widget popup, Widget widget);
static Widget FindBestMatchWidget(Widget, XEvent *);
static Widget FindPopupMenu(Widget toplevel, Widget target, XEvent *event, int level);
static void EventNoop(Widget reportingWidget, XtPointer data, XEvent *event, Boolean *cont);
static Boolean ProcessKey(XmRowColumnWidget rowcol, XEvent *event);
static Boolean CheckKey(XmRowColumnWidget rowcol, XEvent *event);
static int MatchInKeyboardList(XmRowColumnWidget rowcol, XKeyEvent *event, int startIndex);
...
static void PopupMenuEventHandler(Widget wid, XtPointer poster, XEvent *e, Boolean *cont)
{
  Boolean seen;
  Widget postWidget;
  Widget popup;
  ...
  if (e->type != ButtonPress && e->type != ButtonRelease && e->type != KeyPress &&
      e->type != KeyRelease)
    return;
  ...
  if (e->type == KeyPress || e->type == KeyRelease) {
    ...
    mst->RC_ButtonEventStatus.verified = _XmMatchBtnEvent(
        e, RC_PostEventType(popupW), RC_PostButton(popupW), RC_PostModifiers(popupW));
  }
  /* Add the handler to remove the grab if an error occurs */
  if (mst->RC_ButtonEventStatus.verified) {
    if (!popupW->core.being_destroyed && !popupW->row_column.popup_workproc)
      popupW->row_column.popup_workproc = XtAppAddWorkProc(
          XtWidgetToApplicationContext((Widget)popupW), _XmRC_PostTimeOut, (XtPointer)popupW);
 ...
 ***************************************************************/
static Widget FindBestMatchWidget(Widget wid, XEvent *event)
{
  Widget target;
  Widget possible;
    ...
    if (XtIsComposite(target)) {
      ...
      for (i = 0; i < ctarget->composite.num_children; i++) {
        ...
        if (XtIsManaged(possible)) {
          ...
          if (delx >= 0 && delx <= XtWidth(possible) && dely >= 0 && dely <= XtHeight(possible)) {
            target = possible;
            appx = delx;
            appy = dely;
            found = 1;
...
static Widget MenuMatches(Widget menu, int level, XEvent *event)
{
  Boolean found = False;
  if (XtIsShell(menu) && ((CompositeRec *)menu)->composite.num_children == 1) {
    menu = ((CompositeRec *)menu)->composite.children[0];
    if (XmIsRowColumn(menu) && IsPopup(menu) &&
        (((level == 0) && (RC_PopupEnabled(menu) == XmPOPUP_AUTOMATIC)) ||
         ...
         an appropriate popup enabled value */
    {
      if (event->type == KeyPress || event->type == KeyRelease) {
        found = MatchInKeyboardList((XmRowColumnWidget)menu, (XKeyEvent *)event, 0) != -1;
      }
      else {
        found = _XmMatchBtnEvent(
            event, RC_PostEventType(menu), RC_PostButton(menu), RC_PostModifiers(menu));
      }
    }
  }
  if (found)
...
/*Argsused*/
Boolean _XmRC_PostTimeOut(XtPointer wid)
{
...
static void ButtonEventHandler(Widget w, XtPointer data, XEvent *event, Boolean *cont)
{
  ...
  if ((mst->RC_ButtonEventStatus.time == xbutton_event->time) &&
    ...
    if (!RC_TornOff(popup) ||
        !_XmMatchBtnEvent(
            event, RC_PostEventType(popup), RC_PostButton(popup), RC_PostModifiers(popup)))
      return;
  ...
  if (mst->RC_ButtonEventStatus.verified) {
    ...
    mst->RC_ButtonEventStatus.waiting_to_be_managed = TRUE;
    if (!popup->core.being_destroyed && !popup->row_column.popup_workproc)
      popup->row_column.popup_workproc = XtAppAddWorkProc(
          XtWidgetToApplicationContext((Widget)popup), _XmRC_PostTimeOut, (XtPointer)popup);
     ...
     * BEFORE application's event handler.
     */
    RC_CascadeBtn(popup) = XtWindowToWidget(XtDisplay(popup), xbutton_event->window);
  }
}
...
static void AddHandlersToPostFromWidget(Widget popup, Widget widget)
{
  ...
  if ((RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC ||
       RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC_RECURSIVE))
    XtAddEventHandler(
        widget, KeyPressMask | KeyReleaseMask, False, PopupMenuEventHandler, (XtPointer)popup);
  else
   ...
   */
  XtAddEventHandler(widget, ButtonReleaseMask, False, EventNoop, NULL);
  /*
...
void _XmRC_RemoveHandlersFromPostFromWidget(Widget popup, Widget widget)
{
  if (RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC ||
      RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC_RECURSIVE)
    ...
    XtRemoveEventHandler(
        widget, ButtonPressMask | ButtonReleaseMask, False, ButtonEventHandler, (XtPointer)popup);
  XtRemoveEventHandler(
      widget, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)popup);
  XtRemoveEventHandler(widget, ButtonReleaseMask, False, EventNoop, NULL);
 ...
 */
void _XmRC_AddPopupEventHandlers(XmRowColumnWidget pane)
{
  int i;
  /* to myself for gadgets */
  XtAddEventHandler((Widget)pane,
                    KeyPressMask | KeyReleaseMask,
                    False,
                    _XmRC_KeyboardInputHandler,
                    (XtPointer)pane);
  ...
  XtAddEventHandler(
      XtParent(pane), KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, pane);
 ...
 */
void _XmRC_RemovePopupEventHandlers(XmRowColumnWidget pane)
{
  int i;
  /* Remove it from us */
  XtRemoveEventHandler((Widget)pane,
                       KeyPressMask | KeyReleaseMask,
                       False,
                       _XmRC_KeyboardInputHandler,
                       (XtPointer)pane);
  /* Remove it from our shell parent */
  XtRemoveEventHandler(XtParent(pane),
                       KeyPressMask | KeyReleaseMask,
                       False,
                       _XmRC_KeyboardInputHandler,
                       (XtPointer)pane);
  ...
  for (i = 0; i < pane->row_column.postFromCount; i++) {
    _XmRC_RemoveHandlersFromPostFromWidget((Widget)pane, pane->row_column.postFromList[i]);
  }
}
 ...
 */
void _XmAllowAcceleratedInsensitiveUnmanagedMenuItems(Widget wid, Boolean allowed)
{
  _XmGetMenuState((Widget)wid)->RC_allowAcceleratedInsensitiveUnmanagedMenuItems = (Boolean)
      allowed;
}

static void EventNoop(Widget reportingWidget, /* unused */
 ...
 * of other widgets.
 */
void _XmRC_KeyboardInputHandler(Widget reportingWidget,
                                XtPointer data,
                                XEvent *event,
                                Boolean *cont) /* unused */
{
  ...
  else if ((IsPopup(topLevel) || IsPulldown(topLevel)) && !XmIsMenuShell(topLevelShell) &&
      ...
      if (XtIsSensitive(child) && XtIsManaged(child)) {
        if (XmIsCascadeButtonGadget(child)) {
          if (CBG_Submenu(child)) {
            ...
            found = ProcessKey(
                (XmRowColumnWidget)((XmCascadeButtonGadget)child)->cascade_button.submenu, event);
          ...
          if (CB_Submenu(child)) {
            ...
            found = ProcessKey(
                (XmRowColumnWidget)((XmCascadeButtonWidget)child)->cascade_button.submenu, event);
            if (!found)
...
static Boolean CheckKey(XmRowColumnWidget rowcol, XEvent *event)
{
  ...
  while ((menu_index = MatchInKeyboardList(rowcol, (XKeyEvent *)event, menu_index)) != -1) {
    ...
    if (XmIsRowColumn(entry->component)) {
      ...
      if (!XtIsSensitive(entry->component) ||
          ((RC_Type(entry->component) != XmMENU_POPUP) &&
           (RC_Type(entry->component) != XmMENU_PULLDOWN) && (!XtIsManaged(entry->component))))
      {
        menu_index++;
        continue;
      }
             ...
             (!XtIsSensitive(entry->component) || !XtIsManaged(entry->component)))
    {
     ...
     */
    if (entry->isMnemonic) {
      if (XmeTraitGet((XtPointer)XtClass(entry->component), XmQTmenuSavvy) != NULL) {
        if (IsBar(XtParent(entry->component)) && !RC_PopupPosted(XtParent(entry->component)) &&
            ((XmManagerWidget)XtParent(entry->component))->manager.active_child == NULL)
        {
          menu_index++;
          continue;
        }
        else if (IsPopup(XtParent(entry->component)) || IsPulldown(XtParent(entry->component))) {
          ...
          if ((!shell->shell.popped_up) ||
              (shell->composite.children[0] != XtParent(entry->component)))
          {
            menu_index++;
            continue;
          }
    ...
    if (XmIsPrimitive(entry->component)) {
      ...
      (*(prim->primitive_class.arm_and_activate))(entry->component, event, NULL, NULL);
    }
    else if (XmIsGadget(entry->component)) {
      XmGadgetClassRec *gadget;
      gadget = (XmGadgetClassRec *)XtClass(entry->component);
      (*(gadget->gadget_class.arm_and_activate))(entry->component, event, NULL, NULL);
    }
    else if (XmIsRowColumn(entry->component)) {
      XmRowColumnClassRec *rc;
      rc = (XmRowColumnClassRec *)XtClass(entry->component);
      (*(rc->row_column_class.armAndActivate))(entry->component, event, NULL, NULL);
    }
    /* used the key */
...
static int MatchInKeyboardList(XmRowColumnWidget rowcol, XKeyEvent *event, int startIndex)
{
  ...
  for (i = startIndex; i < count; i++) {
    ...
    if (klist[i].key != NoSymbol) {
      if (_XmMatchKeyEvent((XEvent *)event,
                           klist[i].eventType,
                           klist[i].key,
                           klist[i].isMnemonic ?
                               klist[i].modifiers | (event->state & (ShiftMask | LockMask)) :
                               klist[i].modifiers))
      {
        return (i);
      }
 ...
 *
 ************************************************************************/
void XmAddToPostFromList(Widget m, Widget widget)
{
  ...
  if (!XmIsRowColumn(menu) || !(IsPopup(menu) || IsPulldown(menu)) || !widget) {
    _XmAppUnlock(app);
    return;
  }
  if (OnPostFromList(menu, widget) == -1) {
    ...
    /* See if we have an associated table.  If not,  create
         one. */
    if ((list = (PopupList)_XmGetHashEntry(popup_table, (XmHashKey)widget)) == NULL) {
      /* Resize if the table has many more entries than slots */
      if (_XmHashTableCount(popup_table) > (2 * _XmHashTableSize(popup_table)))
...
void XmRemoveFromPostFromList(Widget m, Widget widget)
{
  XmRowColumnWidget menu = (XmRowColumnWidget)m;
  Arg args[1];
  _XmWidgetToAppContext(m);
...
static void RemoveTable(Widget w,
