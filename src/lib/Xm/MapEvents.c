...
#include <X11/Xutil.h>
#include <Xm/XmP.h>
    typedef String(*XmEventParseProc)(
        String str, unsigned int closure, unsigned long *detail, Boolean *status);
 ...
 *************************************<->***********************************/
/* ARGSUSED */
static String ParseKeySym(String str, unsigned int closure, unsigned long *detail, Boolean *status)
{
  char keySymName[100];
  char *start = str;
                        ...
                        Boolean *status)
{
  ...
  str++;
  /* Save the detail */
  return ((*(table[index].parseProc))(str, table[index].closure, detail, status));
}
 ...
 *
 *************************************<->***********************************/
int _XmMapKeyEvents(String str, int **eventTypes, KeySym **keysyms, Modifiers **modifiers)
{
  Boolean status = TRUE;
  int count = 0;
  char *ptr = str;
...
Boolean _XmMatchBtnEvent(XEvent *event, int eventType, unsigned int button, Modifiers modifiers)
{
  ...
  if (((eventType == XmIGNORE_EVENTTYPE) || (event->type == eventType)) &&
      (event->xbutton.button == button) && ((modifiers == AnyModifier) || (state == modifiers)))
    return (TRUE);
  else
    return (FALSE);
}
...
Boolean _XmMatchKeyEvent(XEvent *event, int eventType, unsigned int key, Modifiers modifiers)
{
  ...
  if ((event->type == eventType) && (event->xkey.keycode == key) && (state == mods))
    return (TRUE);
  else
    return (FALSE);
}
