...
static void SetAndDisplayPixmap(XmToggleButtonWidget tb, XEvent *event, Region region);
static void Help(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void ToggleButtonCallback(XmToggleButtonWidget data,
                                 unsigned int reason,
                                 unsigned int value,
                                 XEvent *event);
...
static void Arm(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void Select(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void Disarm(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void ArmAndActivate(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void BtnDown(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void BtnUp(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void GetGC(XmToggleButtonWidget tw);
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static void Destroy(Widget wid);
static void DrawToggle(XmToggleButtonWidget w);
...
static void ComputeSpace(XmToggleButtonWidget tb);
...
static Boolean SetValues(
    Widget current, Widget request, Widget new_w, ArgList args, Cardinal *num_args);
static void DrawToggleShadow(XmToggleButtonWidget tb);
static void DrawToggleLabel(XmToggleButtonWidget tb);
static void DrawEtchedInMenu(XmToggleButtonWidget tb);
static void SetToggleSize(XmToggleButtonWidget newtb);
static void NextState(unsigned char *state);
static void DrawBox(XmToggleButtonWidget w,
                    GC top_gc,
                    GC bot_gc,
                    GC fillgc,
                    int x,
...
static XtResource resources[] = {{XmNindicatorSize,
                                  XmCIndicatorSize,
                                 ...
                                 {XmNselectColor,
                                  ...
                                  XmRSelectColor,
                                  sizeof(Pixel),
                                  Offset(toggle.select_color),
                                  XmRCallProc,
                                  (XtPointer)DefaultSelectColor},
                                 ...
                                 {XmNhighlightThickness,
                                  ...
                                  XmRHorizontalDimension,
                                  sizeof(Dimension),
                                  Offset(primitive.highlight_thickness),
                                  XmRCallProc,
                                 ...
                                 {XmNindeterminatePixmap,
                                  ...
                                  XmRDynamicPixmap,
                                  sizeof(Pixmap),
                                  Offset(toggle.indeterminate_pixmap),
                                  XmRImmediate,
                                  (XtPointer)XmUNSPECIFIED_PIXMAP},
                                 {XmNindeterminateInsensitivePixmap,
                                  ...
                                  XmRDynamicPixmap,
                                  sizeof(Pixmap),
                                  Offset(toggle.indeterminate_insensitive_pixmap),
                                  XmRImmediate,
                                  (XtPointer)XmUNSPECIFIED_PIXMAP},
                                 {XmNunselectColor,
                                  XmCUnselectColor,
                                  XmRPixel,
                                  sizeof(Pixel),
                                  Offset(toggle.unselect_color),
                                  XmRImmediate,
                                  (XtPointer)XmINVALID_PIXEL},
...
externaldef(xmtogglebuttonclassrec) XmToggleButtonClassRec xmToggleButtonClassRec = {
    {
 ...
 *
 ***********************************************************************/
static void redisplayPixmap(XmToggleButtonWidget tb, XEvent *event, Region region)
{
  Pixmap todo;
  ...
  w = XtWidth(tb) - x - offset - tb->label.margin_right - tb->label.margin_width;
  ASSIGN_MAX(w, 0);
  h = XtHeight(tb) - y - offset - tb->label.margin_bottom - tb->label.margin_height;
  ASSIGN_MAX(h, 0);
...
static void HandlePixmap(
    XmToggleButtonWidget tb, Pixmap pix, Pixmap insen_pix, XEvent *event, Region region)
{
...
static void ActionDraw(XmToggleButtonWidget w, XEvent *event, Boolean leave)
{
  if (w->toggle.Armed) {
...
static void Leave(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (Lab_IsMenupane(w)) {
        ...
        (/* !ActiveTearOff || */ event->xcrossing.mode == NotifyNormal))
    {
      ...
      if ((etched_in) &&
          ((w->toggle.ind_on) || (!(w->toggle.ind_on) && !(w->toggle.fill_on_select))))
      {
        DrawEtchedInMenu(w);
        if (w->toggle.ind_on)
          DrawToggle(w);
...
static void BtnDown(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (event && (event->type == ButtonPress)) {
    ...
    Boolean etched_in = dpy->display.enable_etched_in_menu;
    if ((etched_in) &&
        ((tb->toggle.ind_on) || (!(tb->toggle.ind_on) && !(tb->toggle.fill_on_select))))
    {
      DrawEtchedInMenu(tb);
      if (tb->toggle.ind_on)
        DrawToggle(tb);
 ...
 *
 ************************************************************************/
static void GetUnselectGC(XmToggleButtonWidget tw)
{
  XGCValues values;
  XtGCMask valueMask;
...
static void GetGC(XmToggleButtonWidget tw)
{
  ...
  valueMask |= GCLineWidth, values.line_width = 1;
  tw->toggle.indeterminate_GC = XtAllocateGC(
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if ((new_w->toggle.ind_type == XmINVALID_TYPE) ||
      !XmRepTypeValidValue(XmRID_INDICATOR_TYPE, new_w->toggle.ind_type, (Widget)new_w))
  {
    Boolean radio = FALSE;
  ...
  if (Lab_IsPixmap(new_w) && (!IsNull(PixmapOff(new_w)) || !IsNull(PixmapInsenOff(new_w)) ||
  ...
  if (new_w->toggle.ind_on) {
    ...
    boxSize = ((int)new_w->label.TextRect.height + (int)new_w->label.margin_top +
  ...
  if (request->core.width == 0) {
    new_w->core.width = new_w->label.TextRect.width + 2 * new_w->label.margin_width +
                        new_w->label.margin_right + new_w->label.margin_left +
    ...
    if ((new_w->label._acc_text != NULL) && (new_w->toggle.ind_on)) {
      if (LayoutIsRtoLP(new_w)) {
        ...
        new_w->label.acc_TextRect.x = new_w->core.width - new_w->primitive.highlight_thickness -
                                      new_w->primitive.shadow_thickness -
                                      new_w->label.margin_width - new_w->label.margin_right +
                                      LABEL_ACC_PAD;
      }
    }
  }
  if (request->core.height == 0)
    new_w->core.height = MAX(new_w->toggle.indicator_dim,
                         ...
                         2 * (new_w->primitive.highlight_thickness +
                              new_w->primitive.shadow_thickness);
  new_w->label.TextRect.y =
      (short)new_w->primitive.highlight_thickness + new_w->primitive.shadow_thickness +
      new_w->label.margin_height + new_w->label.margin_top +
      ((new_w->core.height - new_w->label.margin_top - new_w->label.margin_bottom -
        (2 * (new_w->label.margin_height + new_w->primitive.highlight_thickness +
              new_w->primitive.shadow_thickness)) -
        new_w->label.TextRect.height) /
       2);
  ...
  if (new_w->toggle.select_color == XmDEFAULT_SELECT_COLOR) {
    ...
    _XmSelectColorDefault(
        (Widget)new_w, XtOffsetOf(XmToggleButtonRec, toggle.select_color), &value);
...
static void DrawToggle(XmToggleButtonWidget w)
{
  ...
  if (Lab_IsPixmap(w) || XmStringEmpty(w->label._label))
    ...
    edge = MIN((int)w->toggle.indicator_dim,
               MAX(0,
                   ((int)w->core.height -
                    2 * (w->primitive.highlight_thickness + w->primitive.shadow_thickness +
  ...
  if ((DefaultDepthOfScreen(XtScreen(w)) > 1) &&
    ...
    margin = 1;
  if (LayoutIsRtoLP(w))
    x = (int)w->core.width - w->primitive.highlight_thickness - w->primitive.shadow_thickness -
        w->label.margin_width - w->toggle.indicator_dim;
  else
    ...
    switch (w->toggle.visual_set) {
      ...
      case XmINDETERMINATE:
        ...
        if (Lab_IsMenupane(w) && etched_in && w->toggle.Armed)
          ...
          fill_gc = (w->toggle.fill_on_select ? w->toggle.indeterminate_GC :
                                                w->toggle.background_gc);
...
static void ComputeSpace(XmToggleButtonWidget tb)
{
  ...
  needed_width = tb->label.TextRect.width + tb->label.margin_left + tb->label.margin_right +
                 (2 * (tb->primitive.shadow_thickness + tb->primitive.highlight_thickness +
                       tb->label.margin_width));
  needed_height = tb->label.TextRect.height + tb->label.margin_top + tb->label.margin_bottom +
                  (2 * (tb->primitive.shadow_thickness + tb->primitive.highlight_thickness +
                        tb->label.margin_height));
  if (needed_height > tb->core.height)
    if (tb->toggle.ind_on)
      tb->label.TextRect.y = tb->primitive.shadow_thickness + tb->primitive.highlight_thickness +
                             tb->label.margin_height + tb->label.margin_top +
                             ((tb->core.height - tb->label.margin_top - tb->label.margin_bottom -
                               (2 * (tb->label.margin_height + tb->primitive.highlight_thickness +
                                     tb->primitive.shadow_thickness)) -
                               tb->label.TextRect.height) /
                              2);
  if (LayoutIsRtoLP(tb)) {
    if ((needed_width > tb->core.width) ||
        ((tb->label.alignment == XmALIGNMENT_BEGINNING) && (needed_width < tb->core.width)) ||
        ((tb->label.alignment == XmALIGNMENT_CENTER) && (needed_width < tb->core.width) &&
        ...
        (needed_width == tb->core.width))
    {
      if (tb->toggle.ind_on)
        tb->label.TextRect.x = tb->core.width -
                               (tb->primitive.shadow_thickness +
                                tb->primitive.highlight_thickness + tb->label.margin_width +
                                tb->label.margin_right + tb->label.TextRect.width);
    }
  }
  else {
    if ((needed_width > tb->core.width) ||
    ...
    {
      if (tb->toggle.ind_on)
        tb->label.TextRect.x = tb->primitive.shadow_thickness + tb->primitive.highlight_thickness +
                               tb->label.margin_width + tb->label.margin_left;
    }
  }
}
 ...
 ************************************************************************/
/*ARGSUSED*/
static Boolean SetValuesPrehook(Widget cw, /* unused */
                                Widget rw, /* unused */
                                Widget nw,
                                ArgList args,       /* unused */
                                Cardinal *num_args) /* unused */
    ...
    Widget current, Widget request, Widget new_w, ArgList args, Cardinal *num_args)
{
   ...
   * mucked with it. */
  if (Lab_IsPixmap(newcbox) && (!IsNull(PixmapOff(newcbox)) || !IsNull(PixmapInsenOff(newcbox)) ||
  ...
  if ((newcbox->label._label != curcbox->label._label) ||
      (PixmapOff(newcbox) != PixmapOff(curcbox)) || (newcbox->label.font != curcbox->label.font) ||
      ...
      (Lab_IsPixmapAndText(newcbox) != Lab_IsPixmapAndText(curcbox)))
  {
    ...
    if (Lab_IsPixmap(newcbox) && ((PixmapOn(newcbox) != PixmapOn(curcbox)) ||
                                  (PixmapInsenOn(newcbox) != PixmapInsenOn(curcbox)) ||
                                  (PixmapInd(newcbox) != PixmapInd(curcbox)) ||
                                  (PixmapInsenInd(newcbox) != PixmapInsenInd(curcbox))))
    {
        ...
        (newcbox->toggle.indicator_dim == curcbox->toggle.indicator_dim))
    {
          ...
          (newcbox->toggle.ind_on != curcbox->toggle.ind_on))
      {
        if (Lab_IsPixmap(new_w) || Lab_IsPixmapAndText(new_w)) {
          ...
          Dimension height;
          int line_count;
          height = XmStringHeight(newcbox->label.font, newcbox->label._label);
    ...
    if (newcbox->toggle.ind_on) {
      ...
      if (newcbox->toggle.indicator_dim != curcbox->toggle.indicator_dim) {
        ...
        int boxSize = ((int)newcbox->label.TextRect.height + (int)newcbox->label.margin_top +
                       (int)newcbox->label.margin_bottom);
        top_delta = bottom_delta = (maxIndicatorSize - boxSize) / 2;
      }
      /*  End fix CR 5568 */
      if (LayoutIsRtoLP(newcbox))
        right_delta = (newcbox->toggle.indicator_dim + newcbox->toggle.spacing -
                       newcbox->label.margin_right);
      else
        left_delta = (newcbox->toggle.indicator_dim + newcbox->toggle.spacing -
    ...
    if (new_w->core.width == 0) {
      newcbox->core.width = newcbox->label.TextRect.width + newcbox->label.margin_left +
                            newcbox->label.margin_right +
    ...
    if (new_w->core.height == 0) {
      newcbox->core.height = MAX(newcbox->toggle.indicator_dim,
                                 newcbox->label.TextRect.height +
                                     2 * newcbox->label.margin_height + newcbox->label.margin_top +
                                     newcbox->label.margin_bottom) +
                             2 * (newcbox->primitive.highlight_thickness +
                                  newcbox->primitive.shadow_thickness);
      ...
      (newcbox->toggle.select_color != curcbox->toggle.select_color))
  {
    ...
    if (newcbox->toggle.select_color == XmDEFAULT_SELECT_COLOR) {
      ...
      XrmValue value;
      DefaultSelectColor(
          (Widget)newcbox, XtOffsetOf(XmToggleButtonRec, toggle.select_color), &value);
      assert(value.size == sizeof(Pixel));
  ...
  if ((curcbox->toggle.ind_type != newcbox->toggle.ind_type) &&
      (!XmRepTypeValidValue(XmRID_INDICATOR_TYPE, newcbox->toggle.ind_type, (Widget)newcbox)))
  {
 ...
 *
 ****************************************************************/
void XmToggleButtonSetState(Widget w, Boolean bnewstate, Boolean notify)
{
  XmToggleButtonWidget tw = (XmToggleButtonWidget)w;
  XmMenuSystemTrait menuSTrait;
  ...
  if (tw->toggle.set != newstate) {
    ...
    if (notify) {
      ...
      /* CR 8904: Notify value_changed before entry so that state is */
      /* 	reported correctly even if the entry callback resets it. */
      menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(tw)),
                                                  XmQTmenuSystem);
      if (menuSTrait != NULL) {
        XmToggleButtonCallbackStruct call_value;
        call_value.reason = XmCR_VALUE_CHANGED;
...
Widget XmVaCreateToggleButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmToggleButtonWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedToggleButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmToggleButtonWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
static void DrawToggleLabel(XmToggleButtonWidget tb)
{
  ...
  switch (tb->toggle.visual_set) {
    ...
    case XmINDETERMINATE: {
      ...
      XChangeGC(XtDisplay((Widget)tb),
                tb->toggle.indeterminate_GC,
...
static void DrawEtchedInMenu(XmToggleButtonWidget tb)
{
  ...
  XFillRectangle(XtDisplay(tb),
                 XtWindow(tb),
                 tb->toggle.Armed ? tb->toggle.arm_GC : tb->toggle.background_gc,
                 fx,
                 fy,
                 fw,
                 fh);
  if (tb->toggle.Armed) {
    XmGetColors(
        XtScreen(tb), tb->core.colormap, tb->core.background_pixel, NULL, NULL, NULL, &select_pix);
...
static void SetToggleSize(XmToggleButtonWidget newtb)
{
  ...
  if (XtIsSensitive((Widget)newtb)) {
    if (!IsNull(PixmapOn(newtb))) {
      XmeGetPixmapData(
          XtScreen(newtb), PixmapOn(newtb), NULL, NULL, NULL, NULL, NULL, NULL, &tmpW, &tmpH);
      ASSIGN_MAX(maxW, tmpW);
      ASSIGN_MAX(maxH, tmpH);
    }
    if (!IsNull(PixmapOff(newtb))) {
      XmeGetPixmapData(
          XtScreen(newtb), PixmapOff(newtb), NULL, NULL, NULL, NULL, NULL, NULL, &tmpW, &tmpH);
      ASSIGN_MAX(maxW, tmpW);
      ASSIGN_MAX(maxH, tmpH);
    }
    if (!IsNull(PixmapInd(newtb))) {
      XmeGetPixmapData(
          XtScreen(newtb), PixmapInd(newtb), NULL, NULL, NULL, NULL, NULL, NULL, &tmpW, &tmpH);
      ASSIGN_MAX(maxW, tmpW);
      ASSIGN_MAX(maxH, tmpH);
    }
  }
    ...
    if (!IsNull(PixmapInsenOn(newtb))) {
      XmeGetPixmapData(
          XtScreen(newtb), PixmapInsenOn(newtb), NULL, NULL, NULL, NULL, NULL, NULL, &tmpW, &tmpH);
      ASSIGN_MAX(maxW, tmpW);
      ASSIGN_MAX(maxH, tmpH);
    }
    if (!IsNull(PixmapInsenOff(newtb))) {
      XmeGetPixmapData(XtScreen(newtb),
                       PixmapInsenOff(newtb),
                       NULL,
                       NULL,
                       NULL,
                       NULL,
