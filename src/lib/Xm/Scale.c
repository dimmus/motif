...
#define SCALE_DEFAULT_MAJOR_SIZE (100 + (2 * sw->scale.highlight_thickness))
#define SCALE_DEFAULT_MINOR_SIZE (15 + (2 * sw->scale.highlight_thickness))
#define SLIDER_SIZE(sca) ((sca->scale.sliding_mode == XmTHERMOMETER) ? 0 : sca->scale.slider_size)
/* this one is context dependent, args and n are used */
#define SET(name, val) \
  { \
    XtSetArg(args[n], (name), (val)); \
    n++; \
  }
#define LeadXTic(sb, sca) \
  (sb->scrollBar.slider_area_x + (Dimension)(((float)SLIDER_SIZE(sca) / 2.0) + 0.5))
#define LeadYTic(sb, sca) \
  (sb->scrollBar.slider_area_y + (Dimension)(((float)SLIDER_SIZE(sca) / 2.0) + 0.5))
#define TrailXTic(sb, sca) \
  (sb->core.width - (sb->scrollBar.slider_area_x + sb->scrollBar.slider_area_width - \
                     (Dimension)(((float)SLIDER_SIZE(sca) / 2.0) + 0.5)))
#define TrailYTic(sb, sca) \
  (sb->core.height - (sb->scrollBar.slider_area_y + sb->scrollBar.slider_area_height - \
                      (Dimension)(((float)SLIDER_SIZE(sca) / 2.0) + 0.5)))
/********    Static Function Declarations    ********/
static void ScaleGetTitleString(Widget wid, int resource, XtArgVal *value);
static void ClassInitialize(void);
static void ClassPartInitialize(WidgetClass wc);
static void ProcessingDirectionDefault(XmScaleWidget widget, int offset, XrmValue *value);
static void SliderVisualDefault(XmScaleWidget widget, int offset, XrmValue *value);
static void SliderMarkDefault(XmScaleWidget widget, int offset, XrmValue *value);
static void EditableDefault(XmScaleWidget widget, int offset, XrmValue *value);
static void ValidateInitialState(XmScaleWidget req, XmScaleWidget new_w);
static Widget CreateScaleTitle(XmScaleWidget new_w);
static Widget CreateScaleScrollBar(XmScaleWidget new_w);
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static void GetForegroundGC(XmScaleWidget sw);
static void Redisplay(Widget wid, XEvent *event, Region region);
static void CalcScrollBarData(
    XmScaleWidget sw, int *value, int *slider_size, int *increment, int *page);
static void Resize(Widget wid);
static void ValidateInputs(XmScaleWidget cur, XmScaleWidget new_w);
...
static Cardinal NumManaged(XmScaleWidget sw, Widget *first_man, Widget *last_man);
static Dimension MajorLeadPad(XmScaleWidget sw);
static Dimension MajorTrailPad(XmScaleWidget sw);
static Dimension ScrollWidth(XmScaleWidget sw);
static Dimension ScrollHeight(XmScaleWidget sw);
static void GetScaleSize(XmScaleWidget sw, Dimension *w, Dimension *h);
static void LayoutHorizontalLabels(XmScaleWidget sw,
                                   XRectangle *scrollBox,
                                   XRectangle *labelBox,
                                   Widget instigator);
static void LayoutHorizontalScale(XmScaleWidget sw, XtWidgetGeometry *desired, Widget instigator);
static void LayoutVerticalLabels(XmScaleWidget sw,
                                 XRectangle *scrollBox,
                                 XRectangle *labelBox,
                                 Widget instigator);
...
static void StartDrag(Widget w, XtPointer data, XEvent *event, Boolean *cont);
static void DragConvertCallback(Widget w, XtPointer client_data, XmConvertCallbackStruct *cs);
...
static XtResource resources[] = {
    ...
    {XmNvalue,
     ...
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmScaleRec, scale.value),
     XmRImmediate,
    ...
    {XmNmaximum,
     ...
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmScaleRec, scale.maximum),
     XmRImmediate,
     (XtPointer)100},
    {XmNminimum,
     XmCMinimum,
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmScaleRec, scale.minimum),
     XmRImmediate,
     (XtPointer)0},
    {XmNorientation,
     XmCOrientation,
     ...
     XtOffsetOf(XmScaleRec, scale.orientation),
     XmRImmediate,
    ...
    {XmNtitleString,
     ...
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(XmScaleRec, scale.title),
     XmRImmediate,
     (XtPointer)NULL},
    {"pri.vate",
     "Pri.vate",
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmScaleRec, scale.last_value),
     XmRImmediate,
     (XtPointer)False},
    {XmNfontList,
     XmCFontList,
     XmRFontList,
     sizeof(XmFontList),
     XtOffsetOf(XmScaleRec, scale.font_list),
     XmRCallProc,
    ...
    {XmNshowValue,
     ...
     XmRShowValue,
     sizeof(XtEnum),
     XtOffsetOf(XmScaleRec, scale.show_value),
     XmRImmediate,
     (XtPointer)XmNONE},
    {XmNdecimalPoints,
     XmCDecimalPoints,
     XmRShort,
     sizeof(short),
     XtOffsetOf(XmScaleRec, scale.decimal_points),
     XmRImmediate,
     (XtPointer)0},
    {XmNscaleWidth,
     XmCScaleWidth,
     XmRHorizontalDimension,
     sizeof(Dimension),
     XtOffsetOf(XmScaleRec, scale.scale_width),
     XmRImmediate,
     (XtPointer)0},
    {XmNscaleHeight,
     XmCScaleHeight,
     ...
     sizeof(Dimension),
     XtOffsetOf(XmScaleRec, scale.scale_height),
     XmRImmediate,
     (XtPointer)0},
    {XmNhighlightThickness,
     ...
     XmRHorizontalDimension,
     sizeof(Dimension),
     XtOffsetOf(XmScaleRec, scale.highlight_thickness),
     XmRCallProc,
     (XtPointer)_XmSetThickness},
    {XmNhighlightOnEnter,
     XmCHighlightOnEnter,
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmScaleRec, scale.highlight_on_enter),
     XmRImmediate,
     (XtPointer)False},
    ...
    {XmNscaleMultiple,
     ...
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmScaleRec, scale.scale_multiple),
     XmRImmediate,
     (XtPointer)0},
    {XmNslidingMode,
     XmCSlidingMode,
     XmRSlidingMode,
     sizeof(XtEnum),
     XtOffsetOf(XmScaleRec, scale.sliding_mode),
     XmRImmediate,
     (XtPointer)XmSLIDER},
    {XmNeditable,
     XmCEditable,
     XmRBoolean,
     sizeof(XtEnum),
     XtOffsetOf(XmScaleRec, scale.editable),
     XmRCallProc,
...
static XmSyntheticResource syn_resources[] = {{XmNtitleString,
                                               sizeof(XmString),
                                               XtOffsetOf(XmScaleRec, scale.title),
                                               ScaleGetTitleString,
                                               (XmImportProc)NULL},
...
externaldef(xmscalewidgetclass) WidgetClass xmScaleWidgetClass = (WidgetClass)&xmScaleClassRec;
...
/****************************************************************/
static void ScaleGetTitleString(Widget wid,
                                int resource, /* unused */
                                XtArgVal *value)
{
 ...
 *
 *********************************************************************/
static void ProcessingDirectionDefault(XmScaleWidget widget,
                                       int offset, /* unused */
                                       XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void SliderVisualDefault(XmScaleWidget widget,
                                int offset, /* unused */
                                XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void SliderMarkDefault(XmScaleWidget widget,
                              int offset, /* unused */
                              XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void EditableDefault(XmScaleWidget widget,
                            int offset, /* unused */
                            XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void ValidateInitialState(XmScaleWidget req, /* unused */
                                 XmScaleWidget new_w)
{
  ...
  if (!XmRepTypeValidValue(XmRID_ORIENTATION, new_w->scale.orientation, (Widget)new_w)) {
    new_w->scale.orientation = XmVERTICAL;
  }
  if (!XmRepTypeValidValue(XmRID_SHOW_VALUE, new_w->scale.show_value, (Widget)new_w)) {
  ...
  if (new_w->scale.scale_multiple > (new_w->scale.maximum - new_w->scale.minimum)) {
    XmeWarning((Widget)new_w, MESSAGE7);
    new_w->scale.scale_multiple = (new_w->scale.maximum - new_w->scale.minimum) / 10;
  }
...
static Widget CreateScaleTitle(XmScaleWidget new_w)
{
  ...
  n++;
  XtSetArg(args[n], XmNfontList, new_w->scale.font_list);
  n++;
  title = (XmLabelGadget)XmCreateLabelGadget((Widget)new_w, "Title", args, n);
...
static void GetForegroundGC(XmScaleWidget sw)
{
  ...
  sw->scale.foreground_GC = XtAllocateGC(
      (Widget)sw, 0, valueMask, &values, GCClipMask, GCClipXOrigin | GCClipYOrigin);
}
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if (new_w->scale.font_list == NULL)
    new_w->scale.font_list = XmeGetDefaultRenderTable((Widget)new_w, XmLABEL_FONTLIST);
...
static void ValidateInputs(XmScaleWidget cur, XmScaleWidget new_w)
{
  ...
  if (!XmRepTypeValidValue(XmRID_SLIDING_MODE, new_w->scale.sliding_mode, (Widget)new_w)) {
...
static void HandleTitle(XmScaleWidget cur,
                        ...
                        XmScaleWidget new_w)
{
  ...
  if (n)
    XtSetValues(new_w->composite.children[0], args, n);
  if (new_w->scale.title != cur->scale.title) {
...
static void HandleScrollBar(XmScaleWidget cur,
                            XmScaleWidget req, /* unused */
                            XmScaleWidget new_w)
{
  Arg args[30];
...
#define DIFF(x) ((new_w->x) != (cur->x))
  ...
  if (XtIsRealized((Widget)new_w) &&
      (DIFF(scale.font_list) || DIFF(scale.highlight_thickness) || DIFF(scale.scale_height) ||
       DIFF(scale.scale_width) || DIFF(scale.orientation) || DIFF(manager.unit_type) ||
...
the new_w field equal to cur */
       ...
       (DIFF(scale.show_value) &&
        ((new_w->scale.show_value == XmNONE) || (cur->scale.show_value == XmNONE)))))
  {
  ...
  if (XtIsRealized((Widget)new_w) &&
      (DIFF(scale.sliding_mode) ||
       ...
       (DIFF(scale.show_value) && (new_w->scale.show_value != XmNONE) &&
  ...
  if (XtIsRealized((Widget)new_w) &&
      (DIFF(scale.decimal_points) || DIFF(scale.value) || DIFF(scale.minimum) ||
       DIFF(scale.maximum) || DIFF(scale.processing_direction) || DIFF(scale.show_value)))
  {
    ShowValue(new_w);
  }
  /*  See if the GC needs to be regenerated  */
  if (DIFF(manager.foreground) || DIFF(core.background_pixel) || DIFF(core.background_pixmap)) {
    XtReleaseGC((Widget)new_w, new_w->scale.foreground_GC);
    GetForegroundGC(new_w);
    redisplay = True;
  }
                                        ...
                                        XtWidgetGeometry *reply) /* unused */
{
  ...
  if (sw->scale.state_flags & FROM_SET_VALUE)
    return XtGeometryYes;
...
static void ChangeManaged(Widget wid)
{
  ...
  if (!XtIsRealized((Widget)sw)) {
    ...
    if (XtWidth(sw))
      desired.width = XtWidth(sw);
    if (XtHeight(sw))
      desired.height = XtHeight(sw);
  }
  desired.request_mode = (CWWidth | CWHeight);
  _XmMakeGeometryRequest((Widget)sw, &desired);
...
static void GetScaleSize(XmScaleWidget sw, Dimension *w, Dimension *h)
{
  ...
  if (sw->scale.orientation == XmHORIZONTAL) {
    if (!*w) {
      *w = MAX(TitleWidth(sw), MajorLeadPad(sw) + ScrollWidth(sw) + MajorTrailPad(sw));
    }
    if (!*h) {
      *h = MaxLabelHeight(sw) + ValueTroughHeight(sw) + ScrollHeight(sw) + TitleHeight(sw);
      if (sw->scale.show_value)
        *h += SCALE_VALUE_MARGIN;
    }
  }
  else /* sw->scale.orientation == XmVERTICAL */ {
    if (!*w) {
      *w = MaxLabelWidth(sw) + ValueTroughWidth(sw) + ScrollWidth(sw) + TitleWidth(sw);
      if (sw->scale.show_value)
        *w += SCALE_VALUE_MARGIN;
    }
    if (!*h) {
      *h = MAX(TitleHeight(sw), MajorLeadPad(sw) + ScrollHeight(sw) + MajorTrailPad(sw));
    }
  }
...
static Dimension ValueTroughHeight(XmScaleWidget sw)
{
  ...
  int ret_val = 0;
  if (sw->scale.show_value) {
    XmRenderTableGetDefaultFontExtents(sw->scale.font_list, &ret_val, NULL, NULL);
  }
...
static Dimension ValueTroughAscent(XmScaleWidget sw)
{
  ...
  int ret_val = 0;
  if (sw->scale.show_value) {
    XmRenderTableGetDefaultFontExtents(sw->scale.font_list, NULL, &ret_val, NULL);
  }
  return (Dimension)ret_val;
#else
  char buff[15];
...
static Dimension ValueTroughDescent(XmScaleWidget sw)
{
  ...
  int ret_val = 0;
  if (sw->scale.show_value) {
    XmRenderTableGetDefaultFontExtents(sw->scale.font_list, NULL, NULL, &ret_val);
  }
  return (Dimension)ret_val;
#else
  char buff[15];
...
static Dimension ValueTroughWidth(XmScaleWidget sw)
{
  ...
  (void)ascent;
  (void)descent;
  (void)overall_return; /* unused but required by XTextExtents */
#if USE_XFT
#  define GET_MAX(tmp, max_or_min_value) \
    { \
      XmString tmp_str; \
...
\
      ...
      (void)direction; \
      (void)ascent; \
      (void)descent; /* unused but required by XTextExtents */ \
...
static Dimension MajorLeadPad(XmScaleWidget sw)
{
  ...
  if (num_managed > 3) {
    if (sw->scale.orientation == XmHORIZONTAL)
      tmp1 = (TotalWidth(first_tic) / 2) - LeadXTic(sb, sw);
    else
      tmp1 = (TotalHeight(first_tic) / 2) - LeadYTic(sb, sw);
  }
     ...
     */
    if (sw->scale.orientation == XmHORIZONTAL)
      tmp1 = ((int)TotalWidth(first_tic) - (int)ScrollWidth(sw)) / 2;
    else
      tmp1 = ((int)TotalHeight(first_tic) - (int)ScrollHeight(sw)) / 2;
  }
  ...
  tmp2 -= (sb->primitive.highlight_thickness + sb->primitive.shadow_thickness);
  ASSIGN_MAX(tmp1, 0);
  ASSIGN_MAX(tmp2, 0);
...
static Dimension MajorTrailPad(XmScaleWidget sw)
{
  ...
  if (num_managed > 3) {
    if (sw->scale.orientation == XmHORIZONTAL)
      tmp1 = ((int)TotalWidth(last_tic) / 2) - (int)TrailXTic(sb, sw);
    else
      tmp1 = ((int)TotalHeight(last_tic) / 2) - (int)TrailYTic(sb, sw);
  }
  else if (num_managed == 3) {
    /*
     * This is a potential non-terminal recursion.
  ...
  if (sw->scale.orientation == XmVERTICAL) {
    ...
    if (!(tmp = sw->scale.scale_width)) {
      if (sw->core.width != 0) {
        ...
        if (num_managed > 3)
          tmp = (int)sw->core.width - (MajorLeadPad(sw) + MajorTrailPad(sw));
        else {
          /* Magic to handle excessively wide values */
          int tmp1, tmp2;
          XmScrollBarWidget sb = (XmScrollBarWidget)sw->composite.children[1];
          tmp1 = ((int)ValueTroughWidth(sw) / 2) - (int)LeadXTic(sb, sw);
          tmp2 = ((int)ValueTroughWidth(sw) / 2) - (int)TrailXTic(sb, sw);
          ASSIGN_MAX(tmp1, 0);
          ASSIGN_MAX(tmp2, 0);
  ...
  if (sw->scale.orientation == XmHORIZONTAL) {
    ...
    if (!(tmp = sw->scale.scale_height)) {
      if (sw->core.height != 0) {
        ...
        /* Have to catch an indirect recursion here */
        if (num_managed > 3)
          tmp = (int)sw->core.height - (MajorLeadPad(sw) + MajorTrailPad(sw));
        else {
          int tmp1, tmp2;
          XmScrollBarWidget sb = (XmScrollBarWidget)sw->composite.children[1];
          tmp1 = ((int)ValueTroughHeight(sw) / 2) - (int)LeadYTic(sb, sw);
          tmp2 = ((int)ValueTroughHeight(sw) / 2) - (int)TrailYTic(sb, sw);
          ASSIGN_MAX(tmp1, 0);
          ASSIGN_MAX(tmp2, 0);
                                   ...
                                   Widget instigator)
{
  ...
  if (num_managed > 3) {
    ...
    for (i = 2, tmp = first_tic_dim; i < sw->composite.num_children; i++) {
      ...
      y = y1 - TotalHeight(w);
      if (instigator != w)
        XmeConfigureObject(w, x, y, w->core.width, w->core.height, w->core.border_width);
      else {
        w->core.x = x;
        w->core.y = y;
      }
    ...
    y = y1 - TotalHeight(w);
    if (XtIsManaged(w) && !((Object)w)->object.being_destroyed) {
      tmp = (sb->scrollBar.slider_area_width - TotalWidth(w)) / 2;
      x = scrollBox->x + sb->scrollBar.slider_area_x + (int)tmp;
      if (instigator != w)
        XmeConfigureObject(w, x, y, w->core.width, w->core.height, w->core.border_width);
      else {
        w->core.x = x;
...
static void LayoutHorizontalScale(XmScaleWidget sw, XtWidgetGeometry *desired, Widget instigator)
{
  ...
  if (diff_w >= 0) {
    ...
    XmScrollBarWidget sb = (XmScrollBarWidget)(sw->composite.children[1]);
    sb_min = (2 * sb->primitive.highlight_thickness) + (4 * sb->primitive.shadow_thickness) +
             SLIDER_SIZE(sw);
    lp = MajorLeadPad(sw);
    tp = MajorTrailPad(sw);
  ...
  else if (instigator != sw->composite.children[0])
    XmeConfigureObject(sw->composite.children[0],
                       0,
                       titleBox.y,
                       (sw->composite.children[0])->core.width,
  ...
  if (instigator != sw->composite.children[1])
    XmeConfigureObject(
        sw->composite.children[1], scrollBox.x, scrollBox.y, scrollBox.width, scrollBox.height, 0);
  else {
                                 ...
                                 Widget instigator)
{
  ...
  if (num_managed > 3) {
    ...
    if (XtIsManaged(w) && !((Object)w)->object.being_destroyed) {
      x = x1 - TotalWidth(w);
      tmp = (sb->scrollBar.slider_area_height - TotalHeight(w)) / 2;
      y = scrollBox->y + sb->scrollBar.slider_area_y + (int)tmp;
      if (instigator != w)
        XmeConfigureObject(w, x, y, w->core.width, w->core.height, w->core.border_width);
      else {
        w->core.x = x;
...
static void ShowValue(XmScaleWidget sw)
{
  ...
  (void)direction;
  (void)descent;
  (void)width_return; /* unused but required by XTextExtents */
...
#else
  XTextExtents(
      sw->scale.font_struct, buffer, strlen(buffer), &direction, &height, &descent, &width_return);
  (void)direction;
  (void)descent; /* unused but required by XTextExtents */
  ...
  if (sw->scale.orientation == XmHORIZONTAL) {
#if USE_XFT
    x = scrollbar->core.x + scrollbar->scrollBar.slider_x +
        ((sw->scale.sliding_mode) ? scrollbar->scrollBar.slider_width : 0) -
    ...
    if (sw->scale.show_value == XmNEAR_BORDER) {
      if (LayoutIsRtoLM(sw))
        x = scrollbar->core.x + scrollbar->core.width + MaxLabelWidth(sw);
      else
#if USE_XFT
        x = scrollbar->core.x - MaxLabelWidth(sw) - sw->scale.show_value_width -
            SCALE_VALUE_MARGIN;
#else
        x = scrollbar->core.x - MaxLabelWidth(sw) - width_return.rbearing - SCALE_VALUE_MARGIN;
#endif
    }
    else { /* NEAR_SLIDER or NONE */
      if (LayoutIsRtoLM(sw))
...
#if USE_XFT
    y = scrollbar->core.y + scrollbar->scrollBar.slider_y + SLIDER_SIZE(sw) / 2 - height / 2;
#else
    y = scrollbar->core.y + scrollbar->scrollBar.slider_y + SLIDER_SIZE(sw) +
        ((height - SLIDER_SIZE(sw)) / 2) - 3;
#endif
  }
#if USE_XFT
...
#else
  XDrawImageString(
      XtDisplay(sw), XtWindow(sw), sw->scale.foreground_GC, x, y, buffer, strlen(buffer));
#endif
}
 ...
 *********************************************************************/
static void CalcScrollBarData(
    XmScaleWidget sw, int *pvalue, int *pslider_size, int *pincrement, int *ppage)
{
  int slider_size, increment, page, value;
  ...
  size = scrollbar_size - 2 * (sw->scale.highlight_thickness + sw->manager.shadow_thickness);
  ...
  increment = (int)((tmp / (float)(sw->scale.maximum - sw->scale.minimum)) + 0.5);
 ...
 *
 *********************************************************************/
static void ValueChanged(Widget wid,
                         XtPointer closure, /* unused */
                         XtPointer call_data)
{
  XmScaleWidget sw = (XmScaleWidget)XtParent(wid);
  XmScrollBarCallbackStruct *scroll_callback = (XmScrollBarCallbackStruct *)call_data;
  XmScaleCallbackStruct scale_callback;
  float sb_value;
  XmScrollBarWidget sb = (XmScrollBarWidget)(sw->composite.children[1]);
  sb_value = (float)scroll_callback->value / (float)(SCROLLBAR_MAX - sb->scrollBar.slider_size);
  ...
  if (scale_callback.reason == XmCR_DRAG)
    XtCallCallbackList((Widget)sw, sw->scale.drag_callback, &scale_callback);
  else /* value changed and to_top and to_bottom */ {
    scale_callback.reason = XmCR_VALUE_CHANGED;
    XtCallCallbackList((Widget)sw, sw->scale.value_changed_callback, &scale_callback);
  }
}
                      ...
                      Boolean *cont) /* unused */
{
  ...
  if ((!sw->scale.show_value) || (event->xbutton.button != Button2) ||
      ((event->xbutton.x < sw->scale.show_value_x) ||
       (event->xbutton.y < sw->scale.show_value_y) ||
       (event->xbutton.x > sw->scale.show_value_x + sw->scale.show_value_width) ||
       (event->xbutton.y > sw->scale.show_value_y + sw->scale.show_value_height)))
    return;
  drag_icon = XmeGetTextualDragIcon(w);
  n = 0;
  XtSetArg(args[n], XmNcursorBackground, sw->core.background_pixel);
  n++;
  XtSetArg(args[n], XmNcursorForeground, sw->manager.foreground);
  n++;
                                ...
                                XmConvertCallbackStruct *cs)
{
  ...
  if (cs->target == XA_STRING || cs->target == atoms[XmAUTF8_STRING]) {
  ...
  if (cs->target == atoms[XmACOMPOUND_TEXT]) {
    strlist = tmpstring;
    tp.value = NULL;
    XmbTextListToTextProperty(XtDisplay(w), &strlist, 1, XCompoundTextStyle, &tp);
...
Widget XmVaCreateScale(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmScaleWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedScale(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmScaleWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
