...
static void HighlightSash(Widget sash);
static void UnhighlightSash(Widget sash);
...
static XtResource resources[] = {
    {XmNborderWidth,
     ...
     sizeof(Dimension),
     XtOffsetOf(struct _XmSashRec, core.border_width),
     XmRImmediate,
     (XtPointer)0},
    {XmNcallback,
     XmCCallback,
     ...
     XtOffsetOf(struct _XmSashRec, sash.sash_action),
     XmRPointer,
     NULL},
    {XmNnavigationType,
     XmCNavigationType,
...
static void HighlightSash(Widget sash)
{
  ...
  XFillRectangle(XtDisplay(sash),
                 XtWindow(sash),
                 ((XmSashWidget)sash)->primitive.highlight_GC,
                 x,
                 y,
                 sash->core.width - (2 * x),
                 sash->core.height - (2 * y));
}
...
static void Realize(register Widget w, XtValueMask *p_valueMask, XSetWindowAttributes *attributes)
{
  ...
  if (0L == SashCursor) {
     ...
     */
    SashCursor = ((XmDisplayInfo *)(dd->display.displayInfo))->SashCursor = XCreateFontCursor(
        XtDisplay(w), XC_crosshair);
...
/*ARGSUSED*/
static void SashDisplayDestroyCallback(Widget w,
