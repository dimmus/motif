...
static void ArmTimeout(XtPointer closure, XtIntervalId *id);
...
static void InitializePrehook(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static void InitializePosthook(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
...
static void DrawPushButton(XmDrawnButtonWidget db, Boolean armed);
 ...
 *
 *************************************<->***********************************/
static XtActionsRec actionsList[] = {{"Arm", Arm},
                                     {"Activate", Activate},
...
externaldef(xmdrawnbuttonwidgetclass)
                    ...
                    Cardinal *num_params) /* unused */
{
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(db)), XmQTmenuSystem);
  if (menuSTrait == NULL)
    return;
  if (event && (event->type == ButtonPress))
 ...
 *
 ************************************************************/
static void InitializePrehook(Widget rw, /* unused */
                              Widget nw,
                              ArgList args,       /* unused */
 ...
 *
 ************************************************************/
static void InitializePosthook(Widget rw, /* unused */
                               Widget nw,
                               ArgList args,       /* unused */
                               Cardinal *num_args) /* unused */
{
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if (!XmRepTypeValidValue(XmRID_SHADOW_TYPE, new_w->drawnbutton.shadow_type, (Widget)new_w)) {
...
Widget XmVaCreateDrawnButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmDrawnButtonWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedDrawnButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmDrawnButtonWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
