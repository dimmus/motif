...
static void Activate(Widget wid, XEvent *event, String *params, Cardinal *num_params);
...
static void ActivateCommonG(XmArrowButtonGadget ag, XEvent *event, Mask event_mask);
...
static void DrawArrowG(XmArrowButtonGadget ag, GC top_gc, GC bottom_gc, GC center_gc);
...
externaldef(xmarrowbuttongadgetclass)
...
static Pixmap GetHighlightPixmapDefault(XmArrowButtonGadget ag)
{
  ...
  if (ag->arrowbutton.highlight_color == ag->arrowbutton.background)
    result = XmGetPixmapByDepth(XtScreen(ag),
                                XmS50_foreground,
                       ...
                       Cardinal *num_args) /* unused */
{
   ...
   */
  if (!XmRepTypeValidValue(XmRID_ARROW_DIRECTION, new_w->arrowbutton.direction, (Widget)new_w)) {
  ...
  new_w->arrowbutton.bottom_shadow_GC = _XmGetPixmapBasedGC(
      XtParent(nw),
...
static void GetBackgroundGC(XmArrowButtonGadget ag)
{
  ...
  XtGCMask valueMask;
  XmManagerWidget mw = (XmManagerWidget)XtParent(ag);
  ag->arrowbutton.fill_bg_box = ((mw->core.background_pixel != ag->arrowbutton.background) &&
...
static void GetArrowGC(XmArrowButtonGadget ag)
{
  ...
  ag->arrowbutton.arrow_GC = XtAllocateGC(
      (Widget)mw, 0, valueMask, &values, GCClipMask, unusedMask);
  valueMask |= GCFillStyle | GCStipple;
  values.fill_style = FillOpaqueStippled;
  values.stipple = _XmGetInsensitiveStippleBitmap((Widget)ag);
  ag->arrowbutton.insensitive_GC = XtAllocateGC(
      (Widget)mw, 0, valueMask, &values, GCClipMask, unusedMask);
}
...
static void Redisplay(Widget w, XEvent *event, Region region)
{
  ...
  background_x_offset = (aw->rectangle.x + aw->gadget.highlight_thickness +
                         aw->gadget.shadow_thickness);
  background_y_offset = (aw->rectangle.y + aw->gadget.highlight_thickness +
                         aw->gadget.shadow_thickness);
  ...
  if ((iwidth > 0) && (iheight > 0)) {
    ...
    if (aw->arrowbutton.selected && XtIsSensitive(w))
      ...
      DrawArrowG(aw,
                 aw->arrowbutton.top_shadow_GC,
                 aw->arrowbutton.bottom_shadow_GC,
                 (XtIsSensitive(w) ? aw->arrowbutton.arrow_GC : aw->arrowbutton.insensitive_GC));
  }
...
static void InitNewColorBehavior(XmArrowButtonGadget ag)
{
  ...
  if (ag->arrowbutton.background == INVALID_PIXEL) {
    _XmBackgroundColorDefault(
        (Widget)ag, XtOffsetOf(struct _XmArrowButtonGadgetRec, arrowbutton.background), &value);
  ...
  if (ag->arrowbutton.foreground == INVALID_PIXEL) {
    _XmForegroundColorDefault(
        (Widget)ag, XtOffsetOf(struct _XmArrowButtonGadgetRec, arrowbutton.foreground), &value);
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (current->arrowbutton.top_shadow_color != new_w->arrowbutton.top_shadow_color ||
      current->arrowbutton.top_shadow_pixmap != new_w->arrowbutton.top_shadow_pixmap)
  {
  ...
  if (current->arrowbutton.bottom_shadow_color != new_w->arrowbutton.bottom_shadow_color ||
      current->arrowbutton.bottom_shadow_pixmap != new_w->arrowbutton.bottom_shadow_pixmap)
  {
  ...
  if (current->arrowbutton.highlight_color != new_w->arrowbutton.highlight_color ||
      current->arrowbutton.highlight_pixmap != new_w->arrowbutton.highlight_pixmap)
  {
...
static void HighlightBorder(Widget w)
{
  ...
  XmeDrawHighlight(XtDisplay((Widget)ag),
                   XtWindow((Widget)ag),
...
static Boolean HandleRedraw(Widget kid, Widget cur_parent, Widget new_parent, Mask visual_flag)
{
  ...
  if (visual_flag & (VisualTopShadowColor | VisualTopShadowPixmap)) {
    ...
    if (ag->arrowbutton.top_shadow_color == curmw->manager.top_shadow_color)
      ag->arrowbutton.top_shadow_color = mw->manager.top_shadow_color;
    if ((ag->arrowbutton.top_shadow_pixmap == curmw->manager.top_shadow_pixmap) &&
  ...
  if (visual_flag & (VisualBottomShadowColor | VisualBottomShadowPixmap)) {
    ...
    if (ag->arrowbutton.bottom_shadow_color == curmw->manager.bottom_shadow_color)
      ag->arrowbutton.bottom_shadow_color = mw->manager.bottom_shadow_color;
    if ((ag->arrowbutton.bottom_shadow_pixmap == curmw->manager.bottom_shadow_pixmap) &&
...
static void InputDispatch(Widget wid, XEvent *event, Mask event_mask)
{
  ...
  if ((event_mask & XmARM_EVENT) ||
      ((ag->arrowbutton.multiClick == XmMULTICLICK_KEEP) && (event_mask & XmMULTI_ARM_EVENT)))
    Arm(ag, event);
                     ...
                     Cardinal *num_params) /* unused */
{
  ...
  if ((buttonEvent->type == ButtonPress || buttonEvent->type == ButtonRelease) &&
...
Widget XmVaCreateArrowButtonGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmArrowButtonGadgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedArrowButtonGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmArrowButtonGadgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
static void ActivateCommonG(XmArrowButtonGadget ag, XEvent *event, Mask event_mask)
{
  if (event->type == ButtonRelease) {
    ...
    (*(((XmArrowButtonGadgetClassRec *)(ag->object.widget_class))->gadget_class.arm_and_activate))(
...
static void DrawArrowG(XmArrowButtonGadget ag, GC top_gc, GC bottom_gc, GC center_gc)
{
  ...
  XmeDrawArrow(XtDisplay((Widget)ag),
               XtWindow((Widget)ag),
               top_gc,
               bottom_gc,
               center_gc,
