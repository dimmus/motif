...
static int add_sp(String name, XPointer value, VaArgList slp, VaArgList plp, VaArgList vlp);
static int add_p(String name, XPointer value, VaArgList slp, VaArgList plp, VaArgList vlp);
static int add_fs(String name, XPointer value, VaArgList slp, VaArgList plp, VaArgList vlp);
static int add_bgpxmp(String name, XPointer value, VaArgList slp, VaArgList plp, VaArgList vlp);
static XIMStyle check_style(XIMStyles *styles, XIMStyle preedit_style, XIMStyle status_style);
static int ImGetGeo(Widget vw, XmImXICInfo this_icp);
static void ImSetGeo(Widget vw, XmImXICInfo this_icp);
static void ImGeoReq(Widget vw);
static XFontSet extract_fontset(XmFontList fl);
static XmImDisplayInfo get_xim_info(Widget w);
static XtPointer *get_im_info_ptr(Widget w, Boolean create);
...
static XmImXICInfo recreate_xic_info(XIC xic,
                                     ...
                                     XmImDisplayInfo xim_info,
                                     XmImShellInfo im_info);
static void set_values(Widget w, ArgList args, Cardinal num_args, XmInputPolicy policy);
static XmImXICInfo get_current_xic(XmImDisplayInfo xim_info, Widget widget);
static void set_current_xic(XmImXICInfo xic_info, XmImDisplayInfo xim_info, Widget widget);
static void unset_current_xic(XmImXICInfo xic_info,
                              XmImShellInfo im_info,
                              XmImDisplayInfo xim_info,
                              Widget widget);
static Cardinal add_ref(XmImRefInfo refs, Widget widget);
static Cardinal remove_ref(XmImRefInfo refs, Widget widget);
static XVaNestedList VaCopy(VaArgList list);
static void VaSetArg(VaArgList list, char *name, XPointer value);
static int ImPreeditStartCallback(XIC xic, XPointer client_data, XPointer call_data);
static void ImPreeditDoneCallback(XIC xic, XPointer client_data, XPointer call_data);
static void ImPreeditDrawCallback(XIC xic, XPointer client_data, XPointer call_data);
static void ImPreeditCaretCallback(XIC xic, XPointer client_data, XPointer call_data);
...
void XmImRegister(Widget w, unsigned int reserved) /* unused */
{
  Widget p;
  XmImShellInfo im_info;
  XmImDisplayInfo xim_info;
  ...
  if (((xim_info = get_xim_info(p)) == NULL) || (xim_info->xim == NULL)) {
    _XmAppUnlock(app);
    return;
  }
  ...
  switch (input_policy) {
    case XmPER_SHELL:
      ...
      break;
    case XmPER_WIDGET:
      set_current_xic(create_xic_info(p, xim_info, im_info, input_policy), xim_info, w);
      break;
...
void XmImSetFocusValues(Widget w, ArgList args, Cardinal num_args)
{
  ...
  if (wind != XtWindow(w)) {
    ...
    if (fl)
      fs = extract_fontset(fl);
    if (fs)
      list = XVaCreateNestedList(0, XNBackground, bg, XNForeground, fg, XNFontSet, fs, NULL);
    else
      list = XVaCreateNestedList(0, XNBackground, bg, XNForeground, fg, NULL);
    ...
    if (xic_info->input_style & XIMPreeditCallbacks) {
      ...
      if (input_policy == XmPER_SHELL && wind)
        move_preedit_string(xic_info, XtWindowToWidget(XtDisplay(w), wind), w);
    }
  }
...
void XmImCloseXIM(Widget w)
{
  ...
  while (xim_info->shell_refs.refs != NULL) {
    ...
    assert((xim_info->shell_refs.refs == NULL) || (xim_info->shell_refs.refs[0] != shell));
  }
  shell = w;
    ...
    Widget w, XKeyPressedEvent *event, char *buf, int nbytes, KeySym *keysym, int *status)
{
  ...
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
  if ((icp = get_current_xic(get_xim_info(w), w)) == NULL || icp->xic == NULL) {
    if (status)
      *status = XLookupBoth;
  ...
  ret_val = XmbLookupString(icp->xic, event, buf, nbytes, keysym, status);
  _XmAppUnlock(app);
  return ret_val;
}

XIC XmImGetXIC(Widget w, XmInputPolicy input_policy, ArgList args, Cardinal num_args)
{
  XmImDisplayInfo xim_info;
  XmImShellInfo im_info;
...
void XmImFreeXIC(Widget w, XIC context)
{
  ...
  while (--index >= 0)
    unset_current_xic(xic_info, im_info, xim_info, xic_info->widget_refs.refs[index]);
  _XmAppUnlock(app);
}
...
void _XmImFreeShellData(Widget widget, XtPointer *data)
{
  XmImShellInfo im_info;
  XmImDisplayInfo xim_info;
  XmImXICInfo xic_info;
  Widget reference;
  if ((data == NULL) || (im_info = (XmImShellInfo)*data) == NULL)
    return;
  ...
  while (im_info->iclist != NULL) {
    ...
    assert((xic_info != im_info->iclist) || (reference != xic_info->widget_refs.refs[0]));
  }
  assert(im_info->shell_xic == NULL);
  /* Delete the dummy widget. */
  if (im_info->im_widget != NULL && !widget->core.being_destroyed) {
...
void _XmImRealize(Widget vw)
{
  ...
  if ((xim_info == NULL) || (im_info == NULL) || (im_info->iclist == NULL))
    return;
  ...
  if (ve && ve->vendor.im_height == 0) {
    ...
    if (!resize)
      shell->shell.allow_shell_resize = True;
    ImGeoReq(vw);
    if (!resize)
      shell->shell.allow_shell_resize = False;
  }
  else
    ImSetGeo(vw, NULL);
                                     ...
                                     XmImShellInfo im_info)
{
  ...
  for (xic_info = im_info->iclist; xic_info != NULL; xic_info = xic_info->next) {
    if (xic_info->xic == xic)
      return xic_info;
  }
  ...
  for (index = 0; index < xim_info->shell_refs.num_refs; index++)
    if (shell != xim_info->shell_refs.refs[index]) {
      XmImShellInfo tmp_info = get_im_info(xim_info->shell_refs.refs[index], False);
      assert(tmp_info != NULL);
      for (xic_info = tmp_info->iclist; xic_info != NULL; xic_info = xic_info->next) {
        if (xic_info->xic == xic)
          return xic_info;
      }
    }
                                   ...
                                   XmInputPolicy input_policy)
{
  ...
  if (cp != NULL) {
    ...
    while ((style == 0) && (cp < cpend)) {
      ...
      if (XmeNamesAreEqual(cp, OVERTHESPOT))
        style = check_style(
            styles, XIMPreeditPosition, XIMStatusArea | XIMStatusNothing | XIMStatusNone);
      else if (XmeNamesAreEqual(cp, OFFTHESPOT))
        style = check_style(
            styles, XIMPreeditArea, XIMStatusArea | XIMStatusNothing | XIMStatusNone);
      else if (XmeNamesAreEqual(cp, ROOT))
        style = check_style(styles, XIMPreeditNothing, XIMStatusNothing | XIMStatusNone);
      else if (XmeNamesAreEqual(cp, ONTHESPOT))
        style = check_style(
            styles, XIMPreeditCallbacks, XIMStatusArea | XIMStatusNothing | XIMStatusNone);
      cp = tp + 1;
    }
  }
  if (style == 0) {
  ...
  if ((im_info->im_widget == NULL) &&
    ...
    im_info->im_widget = XtVaCreateWidget(
        "xmim_wrapper", coreWidgetClass, shell, XmNwidth, 10, XmNheight, 10, NULL);
...
static void set_values(Widget w, ArgList args, Cardinal num_args, XmInputPolicy input_policy)
{
  ...
  for (i = num_args; i > 0; i--, argp++) {
    ...
    for (rlp = XmImResList, j = XtNumber(XmImResList); j != 0; j--, rlp++) {
      if (rlp->xrmname == name) {
        flags |= (*rlp->proc)(
            rlp->xstring, (XPointer)argp->value, &status_vlist, &preedit_vlist, &xic_vlist);
        break;
      }
    }
    _XmProcessUnlock();
  ...
  if (icp->xic == NULL) {
    ...
    if (va_vlist) {
      ...
      if (icp->xic == NULL) {
        ...
        icp->xic = XCreateIC(xim_info->xim,
                             XNInputStyle,
                             icp->input_style,
                             XNClientWindow,
                             XtWindow(p),
    ...
    if ((ret != NULL) && unrecognized) {
       ...
       * should ignore it.
       */
      va_slist = XVaCreateNestedList(0, XNBackground, &status_bg, XNForeground, &status_fg, NULL);
      va_plist = XVaCreateNestedList(
          0, XNBackground, &preedit_bg, XNForeground, &preedit_fg, NULL);
...
static void ImPreeditDrawCallback(XIC xic, XPointer client_data, XPointer call_data)
{
  ...
  PreeditBuffer pb;
  XIMText *text;
  XIMPreeditDrawCallbackStruct *data = (XIMPreeditDrawCallbackStruct *)call_data;
  ...
  if (data->text) {             /* text field is non-NULL */
    ...
    if (from || to || ml) {
      ...
      bzero((char *)pb->text + pb->length * sizeof(wchar_t), sizeof(wchar_t));
      bzero((char *)pb->feedback + pb->length * sizeof(XIMFeedback), sizeof(XIMFeedback));
      XtFree((char *)wchar);
    }
  }
    ...
    bzero((char *)pb->text + pb->length * sizeof(wchar_t), data->chg_length * sizeof(wchar_t));
...
static void ImPreeditCaretCallback(XIC xic, XPointer client_data, XPointer call_data)
{
  ...
  XmImXICInfo icp;
  PreeditBuffer pb;
  XIMPreeditCaretCallbackStruct *data = (XIMPreeditCaretCallbackStruct *)call_data;
  Widget real = NULL;
  assert(client_data);
  /* update the preedit buffer */
  if ((im_info = get_im_info(w, False)) == NULL)
...
static int NameToSwitch(String name)
{
...
static int add_sp(
    String name, XPointer value, VaArgList slp, VaArgList plp, VaArgList vlp) /* unused */
{
...
static int ImGetGeo(Widget vw, XmImXICInfo this_icp)
{
  ...
  for (icp = this_icp ? this_icp : im_info->iclist; icp != NULL; icp = icp->next) {
    if (icp->xic) {
      ...
      if (icp->input_style & XIMPreeditArea) {
        ...
        if ((int)rp->height > height)
          height = rp->height;
        icp->preedit_width = MIN((int)rp->width, (int)(vw->core.width - icp->status_width));
...
static void ImSetGeo(Widget vw, XmImXICInfo this_icp)
{
  ...
  for (icp = this_icp ? this_icp : im_info->iclist; icp != NULL; icp = icp->next) {
    ...
    if (use_slist && use_plist)
      ...
      XSetICValues(icp->xic, XNStatusAttributes, va_slist, NULL);
    else if (use_plist)
      XSetICValues(icp->xic, XNPreeditAttributes, va_plist, NULL);
    if (this_icp)
      break;
  }
  XFree(va_slist);
...
static XmImDisplayInfo get_xim_info(Widget widget)
{
  ...
  if (XGetIMValues(xim_info->xim, XNQueryInputStyle, &xim_info->styles, NULL) != NULL) {
    XCloseIM(xim_info->xim);
    xim_info->xim = NULL;
    XmeWarning((Widget)widget, MSG1);
    return xim_info;
...
{
  XmImXICInfo xic_info;
  if ((xim_info == NULL) || (xim_info->current_xics == (XContext)0))
    return NULL;
  if (XFindContext(
          XtDisplay(widget), (XID)widget, xim_info->current_xics, (XPointer *)&xic_info) != 0)
    return NULL;
  else
    return xic_info;
}
...
static void set_current_xic(XmImXICInfo xic_info, XmImDisplayInfo xim_info, Widget widget)
{
  ...
  (void)XSaveContext(XtDisplay(widget), (XID)widget, xim_info->current_xics, (XPointer)xic_info);
}
