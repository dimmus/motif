...
#include <stdlib.h>
    ...
    QueryAnyPolicy(XmGeoMatrix geoSpec, XtWidgetGeometry *parentRequestRtn);
static XtGeometryResult QueryGrowPolicy(XmGeoMatrix geoSpec, XtWidgetGeometry *parentRequestRtn);
static XtGeometryResult QueryNonePolicy(XmGeoMatrix geoSpec, XtWidgetGeometry *parentRequestRtn);
...
static void _XmGeoCalcFill(Dimension fillSpace,
                           Dimension margin,
...
static int boxWidthCompare(XmConst void *boxPtr1, XmConst void *boxPtr2);
...
/****************************************************************/
XtGeometryResult _XmHandleQueryGeometry(Widget widget,
                                        ...
                                        XmGeoCreateProc createMatrix)
{
  ...
  if (resize_policy == XmRESIZE_NONE) {
    ...
    _XmGeoArrangeBoxes(geoSpec, (Position)0, (Position)0, &width, &height);
...
/****************************************************************/
XtGeometryResult _XmHandleGeometryManager(Widget wid,
                                          Widget instigator,
                                          XtWidgetGeometry *desired,
                                          ...
                                          XmGeoCreateProc createMatrix)
{
  ...
  if (!cachePtr) {
    ...
    geoSpec = *cachePtr;
    if (geoSpec) {
      if ((geoSpec->composite == wid) && (geoSpec->instigator == instigator) &&
          _XmGeometryEqual(instigator, geoSpec->in_layout, desired))
      {
        ...
        if (GMode(desired) & XtCWQueryOnly) {
        ...
        else { /* If we get here, we should have already verified that
          ...
          if (geoSpec->parent_request.request_mode) {
            ...
            XtMakeGeometryRequest(wid, &geoSpec->parent_request, NULL);
          }
      ...
      else { /* Cached geometry is different than current request, so clear
  ...
  if (geoSpec->no_geo_request && (*geoSpec->no_geo_request)(geoSpec)) {
    _XmGeoMatrixFree(geoSpec);
    return (XtGeometryNo);
  }
  ...
  if (queryResult == XtGeometryYes) {
    if (_XmGeoReplyYes(instigator, desired, geoSpec->in_layout)) {
      ...
      else { /* Don't need almost reply and this is not a query, so do it!
              */
    ...
    else { /* If allowed and not an exception then reply Almost, since
            *   desired geometry is different than geometry in this layout.
            */
      if (allowed && (!geoSpec->almost_except || !(*(geoSpec->almost_except))(geoSpec))) {
        geoSpec->parent_request = parentRequest;
...
static XtGeometryResult QueryAnyPolicy(XmGeoMatrix geoSpec, XtWidgetGeometry *parentRequestRtn)
{
  ...
  layoutW = 0;
  layoutH = 0;
  _XmGeoArrangeBoxes(geoSpec, (Position)0, (Position)0, &layoutW, &layoutH);
  /*	Load request.
   */
  ...
  if ((layoutW == XtWidth(wid)) && (layoutH == XtHeight(wid))) {
    ...
    queryResult = XtMakeGeometryRequest(wid, parentRequestRtn, &parentResponse);
    if (queryResult == XtGeometryAlmost) {
      if ((parentResponse.request_mode & (CWWidth | CWHeight)) != (CWWidth | CWHeight)) {
        queryResult = XtGeometryNo;
      }
      else { /* The protocol guarantees an XtGeometryYes reply for
    ...
    if (queryResult == XtGeometryAlmost) {
      if ((parentResponse.request_mode & (CWWidth | CWHeight)) != (CWWidth | CWHeight)) {
        ...
        *parentRequestRtn = parentResponse;
        ...
        almostW = parentResponse.width;
        almostH = parentResponse.height;
        if ((almostW < XtWidth(wid)) || (almostH < XtHeight(wid))) {
          queryResult = XtGeometryNo;
        }
        else {
          if ((almostW != layoutW) || (almostH != layoutH)) {
  ...
  if (((w == XtWidth(wid)) && (h == XtHeight(wid)))) {
    ...
    if (policy != XmRESIZE_NONE) {
      ...
      if (parentResult == XtGeometryAlmost) {
        if ((policy == XmRESIZE_GROW) && ((r_w < XtWidth(wid)) || (r_h < XtHeight(wid)))) {
 ...
 ****************/
XmGeoMatrix _XmGeoMatrixAlloc(unsigned int numRows,  /* Number of rows of widgets to layout.*/
                              ...
                              unsigned int extSize)  /* Extension record size (bytes).*/
{
   ...
   */
  geoSpecPtr->layouts = (XmGeoMajorLayout)(((char *)geoSpecPtr) + matrixRecSize);
  geoSpecPtr->boxes = (XmKidGeometry)(((char *)geoSpecPtr) + matrixRecSize + layoutSize);
  if (extSize) {
    geoSpecPtr->extension = (XtPointer)(((char *)geoSpecPtr) + matrixRecSize + layoutSize +
                                        boxesSize);
  }
  return (geoSpecPtr);
}
 ...
 ****************/
Boolean _XmGeoSetupKid(XmKidGeometry geo, /* Must be non-NULL.*/
                       Widget kidWid)
{
  /****************/
...
void _XmGeoMatrixGet(XmGeoMatrix geoSpec, int geoType) /* XmGET_PREFERRED_SIZE or */
{
  ...
  while (!(layoutPtr->row.end)) {
    boxPtr = rowPtr;
    while (boxPtr->kid) {
      _XmGeoLoadValues(boxPtr->kid, geoType, instigator, request, &(boxPtr->box));
  ...
  if (!geoSpec->set_except || !(*geoSpec->set_except)(geoSpec)) {
    ...
    while (!(layoutPtr->end)) {
      if (layoutPtr->fix_up) {
         ...
         */
        (*(layoutPtr->fix_up))(geoSpec, XmGEO_PRE_SET, (XmGeoMajorLayout)layoutPtr, rowPtr);
        fixUps = TRUE;
      }
    ...
    if (fixUps) {
      ...
      while (!(layoutPtr->end)) {
        if (layoutPtr->fix_up) {
          (*(layoutPtr->fix_up))(geoSpec, XmGEO_POST_SET, (XmGeoMajorLayout)layoutPtr, rowPtr);
        }
        rowPtr += layoutPtr->box_count + 1; /* Skip to next row.*/
        ++layoutPtr;
      }
...
void _XmGeoAdjustBoxes(XmGeoMatrix geoSpec)
{
  ...
  while (!(layoutPtr->end)) {
    ...
    while ((rowPtr++)->kid) /* Go to next row of boxes. */ { /*EMPTY*/
    }
    ++layoutPtr; /* Go to next row layout record.*/
  }
  return;
  ...
  while (!(layoutPtr->end)) {
    ...
    if (numBoxes > 1 && layoutPtr->space_between > 0) {
      ...
      if (numBoxes - 1 > (UINT_MAX / layoutPtr->space_between)) {
        ...
        layoutPtr->fill_width = (endSpaceW << 1) + ((numBoxes - 1) * layoutPtr->space_between);
      }
    }
    else {
...
static Dimension _XmGeoStretchVertical(XmGeoMatrix geoSpec, Dimension actualH, Dimension desiredH)
{
  ...
  if (fillOffset < 0) { /* Must shrink stretchable boxes.
                         */
    while (!layoutPtr->end) {
      if (layoutPtr->stretch_height && (layoutPtr->max_box_height > layoutPtr->min_height)) {
        stretchableSpace += layoutPtr->max_box_height - layoutPtr->min_height;
      }
      ++layoutPtr;
    }
  ...
  else { /* Must grow stretchable boxes.
          */
    while (!layoutPtr->end) {
      if (layoutPtr->stretch_height) {
  ...
  if (!stretchableSpace) { /* No stretchable boxes, so return with current height.
                            */
    return (actualH);
  }
  deltaY = 0;
    ...
    if (layoutPtr->stretch_height) {
      if (fillOffset < 0) {
        if (layoutPtr->max_box_height > layoutPtr->min_height) {
          deltaH = (((int)(layoutPtr->max_box_height - layoutPtr->min_height)) * fillOffset) /
                   stretchableSpace;
        }
        else {
          deltaH = 0;
         ...
         */
        while (rowPtr->kid) {
          int boxCorrection = layoutPtr->max_box_height - rowPtr->box.height;
        ...
        deltaH = (layoutPtr->max_box_height * fillOffset) / stretchableSpace;
        while (rowPtr->kid) {
...
static Dimension _XmGeoFillVertical(XmGeoMatrix geoSpec, Dimension actualH, Dimension desiredH)
{
  ...
  if (!totalSpecSpace) { /* Zero spacing specified, so just return as is.
                          */
    return (actualH);
  }
   ...
   *   spacing.
   */
  fillAmount = (desiredH - actualH) + geoSpec->fill_minor + currentFirstSpace + currentLastSpace;
  newFirstSpace = (Dimension)((fillAmount * (unsigned long)firstSpecSpace) / totalSpecSpace);
  newLastSpace = (Dimension)((fillAmount * (unsigned long)lastSpecSpace) / totalSpecSpace);
  ...
  for (;;) {
    ...
    deltaY += (int)(((((unsigned long)layoutPtr->space_above) * fillAmount) / totalSpecSpace) -
 ...
 ****************/
static void _XmGeoCalcFill(Dimension fillSpace, /* Fill space, including margins.*/
 ...
 ****************/
void _XmGeoArrangeBoxes(XmGeoMatrix geoSpec, /* Array of box lists (rows).*/
                        ...
                        Dimension *pH)       /* Initial value is minimum height.*/
{
  ...
  Dimension initY;
  /****************/
  if (geoSpec->arrange_boxes && (geoSpec->arrange_boxes != _XmGeoArrangeBoxes)) {
...
******************************************************************/
  ...
  while (!(layoutPtr->end)) {
     ...
     */
    y = _XmGeoArrangeList(rowPtr, layoutPtr, x, y, actualW, marginW, marginH);
    rowPtr += layoutPtr->box_count + 1; /* Skip to next row.*/
    ++layoutPtr;
  ...
  if (layoutPtr->space_above < marginH) { /* Fill out to the minimum margin if previous spacing is
                                           * less than margin.
                                           */
  ...
  actualH = y - initY;
  if (*pH && (actualH != *pH)) {
    if (geoSpec->stretch_boxes) { /* Has stretchable boxes, so grow or shrink using stretch.
                                   */
      actualH = _XmGeoStretchVertical(geoSpec, actualH, *pH);
    }
    else {
      if (actualH < *pH) { /* Layout is smaller than specified height, so fill vertically.
                            */
...
{
  /****************/
  return ((*((XmKidGeometry *)boxPtr1))->box.width > (*((XmKidGeometry *)boxPtr2))->box.width);
}
                              ...
                              int amtOffset)
{
  ...
  qsort((void *)sortedBoxes, (size_t)numBoxes, sizeof(XmKidGeometry), boxWidthCompare);
  ...
  while (Index < numBoxes) {
     ...
     */
    if (amtOffset >= ((int)(boxWidth - ((sortedBoxes[Index]->box.width) * (numBoxes - Index))))) {
      break;
    }
                                 ...
                                 int amtOffset)
{
  ...
  if (boxWidth >= numBoxes) {
    deltaX = 0;
    while (rowPtr->kid) {
      deltaW = (amtOffset * (int)(rowPtr->box.width + (rowPtr->box.border_width << 1))) /
               ((int)boxWidth);
                        ...
                        Dimension betweenSpace)
{
  ...
  endSpace = (layoutPtr->space_end < marginW) ? marginW : layoutPtr->space_end;
  switch (layoutPtr->fill_mode) {
    ...
    case XmGEO_PACK: { /* For a packed layout, just layout with extra space
                        *   at the end of the row.
                        */
      break;
    }
    case XmGEO_EXPAND:
    default: { /* FitBoxesProportional will fill if amtOffset < 0,
                *    as it is here.
                */
  ...
  while (rowPtr->kid) {
    rowPtr->box.x = rowX;
    rowX += rowPtr->box.width + (rowPtr->box.border_width << 1) + betweenSpace;
    ++rowPtr;
  }
                                 ...
                                 Dimension marginW)
{
  ...
  while (boxPtr->kid) {
    ...
    if ((boxMaxX > maxX) && numBoxes) { /* Wrap the line.  Also adjust preceding segment according
                                         * to fill policy.
                                         */
      endX = rowX - betweenSpace;
      SegmentFill(
          rowBegin, numBoxes, layoutPtr, x, width, marginW, endX, maxX, endSpace, betweenSpace);
      numBoxes = 0;
      rowX = x + endSpace;
      y += rowH;
      rowBegin = boxPtr;
    ...
    if (boxMaxX > maxX) { /* Since it wasn't wrapped, there must be only one box in this
    ...
    boxPtr->box.y = y;
    boxH = boxPtr->box.height + (boxPtr->box.border_width << 1);
    if (boxH != rowH) { /* If box height is not the same as the maximum box height
                                  ...
                                  Dimension marginH) /* unused */
{
  ...
  endsOfBoxes = (layoutPtr->space_end < marginW) ? marginW : layoutPtr->space_end;
  ...
  { /* Wrapping is required, so fill routines and other policy decisions
     ...
     */
    y = _XmGeoLayoutWrap(
        rowBoxes, layoutPtr, x, y, endsOfBoxes, betweenBoxes, maxX, width, marginW);
  }
  else {
    if (sumW > width) {
      switch (layoutPtr->fit_mode) {
        case XmGEO_AVERAGING: {
          FitBoxesAveraging(rowBoxes, numBoxes, boxWidth, amtOffset);
          break;
        }
      ...
      if (sumW < width) {
        switch (layoutPtr->fill_mode) {
          case XmGEO_CENTER: {
             ...
             *   layout when passed to the simple layout routine.
             */
            totalFill = (marginW << 1) + layoutPtr->fill_width + width - sumW;
            _XmGeoCalcFill(totalFill,
                           marginW,
                           numBoxes,
                           layoutPtr->space_end,
    ...
    y = _XmGeoLayoutSimple(rowBoxes, layoutPtr, x, y, maxX, endsOfBoxes, betweenBoxes);
  }
  return (y);
}
 ...
 ****************/
void _XmSeparatorFix(XmGeoMatrix geoSpec,
                     int action,
 ...
 ****************/
void _XmMenuBarFix(XmGeoMatrix geoSpec,
                   int action,
                   XmGeoMajorLayout layoutPtr, /* unused */
                   XmKidGeometry rowPtr)
{
...
/****************************************************************/
void _XmGeoLoadValues(Widget wid,
                      int geoType,
                      Widget instigator,
                      XtWidgetGeometry *request,
                      XtWidgetGeometry *geoResult)
{
  ...
  /****************/
  if (wid == instigator) { /* If this widget is making the request, then use the request info.
                            */
    geoSource = request;
  }
  else {
  ...
  geoResult->height = IsHeight(geoSource) ? geoSource->height : XtHeight(wid);
  geoResult->border_width = IsBorder(geoSource) ? geoSource->border_width : XtBorderWidth(wid);
 ...
 ****************/
XmKidGeometry _XmGetKidGeo(Widget wid,                 /* Widget w/ children. */
  ...
  for (i = 0; i < c->composite.num_children; i++) {
    ...
    if (XtIsManaged(kidWid)) {
      if (kidWid == help) { /* Save to put help widget at the end of the widget list.*/
        ...
        geo[j].kid = kidWid;
        _XmGeoLoadValues(kidWid, geo_type, instigator, request, &(geo[j].box));
...
void _XmSetKidGeo(XmKidGeometry kg, Widget instigator)
{
  ...
  for (i = 0; kg[i].kid != NULL; i++) {
    ...
    b = &(kg[i].box);
    if (w != instigator) {
      XmeConfigureObject(w, b->x, b->y, b->width, b->height, b->border_width);
    }
    else {
      XtX(w) = b->x;
      XtY(w) = b->y;
 ...
 ****************/
Boolean _XmGeoReplyYes(Widget wid, /* unused */
                       XtWidgetGeometry *desired,
                       XtWidgetGeometry *response)
{
  ...
  if (IsWidth(desired)) {
    if (!IsWidth(response) || (desired->width != response->width)) {
      return (FALSE);
    }
  }
  if (IsHeight(desired)) {
    if (!IsHeight(response) || (desired->height != response->height)) {
      return (FALSE);
    }
  }
  if (IsBorder(desired)) {
    if (!IsBorder(response) || (desired->border_width != response->border_width)) {
      return (FALSE);
    }
  }
  if (IsX(desired)) {
    if (!IsX(response) || (desired->x != response->x)) {
      return (FALSE);
    }
  }
  if (IsY(desired)) {
    if (!IsY(response) || (desired->y != response->y)) {
      return (FALSE);
    }
  }
  return (TRUE);
 ...
 ****************/
{
  ...
  printf("%sw: %X, m: 0x%X, x: %d, y: %d, w: %d, h: %d, b: %d\n",
         hdr,
         box->kid,
         box->box.request_mode,
         box->box.x,
         box->box.y,
         box->box.width,
         box->box.height,
         box->box.border_width);
  return;
}
/****************************************************************/
void PrintList(char *hdr, XmKidGeometry listPtr)
