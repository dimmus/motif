...
XmPrimitiveClassExtRec _XmTextPrimClassExtRec = {NULL,
                                                 NULLQUARK,
                                                 1L,
...
XmGadgetClassExtRec _XmLabelGadClassExtRec = {
    NULL, NULLQUARK, 1L, sizeof(XmGadgetClassExtRec), NULL, NULL};
XmGadgetClassExtRec _XmPushBGadClassExtRec = {NULL,
                                              NULLQUARK,
                                              1L,
                                              sizeof(XmGadgetClassExtRec),
                                              XmInheritBaselineProc,
                                              XmInheritDisplayRectProc};
XmGadgetClassExtRec _XmToggleBGadClassExtRec = {NULL,
                                                NULLQUARK,
                                                1L,
                                                sizeof(XmGadgetClassExtRec),
                                                XmInheritBaselineProc,
...
externaldef(worldobjectclass) WidgetClass xmWorldClass = (WidgetClass)&xmWorldClassRec;
...
String _XmGetDefaultBackgroundColorSpec(Screen *screen)
{
  ...
  classes[1] = NULLQUARK;
  if (XrmQGetResource(XtScreenDatabase(screen), names, classes, &rep, &db_value)) {
    if (rep == XrmPermStringToQuark(XmRString))
      default_background_color_spec = db_value.addr;
  }
  else
    default_background_color_spec = XmDEFAULT_BACKGROUND;
...
void _XmGetDefaultThresholdsForScreen(Screen *screen)
{
  ...
  if (XrmQGetResource(XtScreenDatabase(screen), names, classes, &rep, &db_value)) {
    ...
    if (XtConvertAndStore(&widget, XmRString, &db_value, XmRInt, &to_value)) {
      ...
      if ((default_light_threshold_spec < 0) || (default_light_threshold_spec > 100))
  ...
  if (XrmQGetResource(XtScreenDatabase(screen), names, classes, &rep, &db_value)) {
    ...
    if (XtConvertAndStore(&widget, XmRString, &db_value, XmRInt, &to_value)) {
      ...
      if ((default_dark_threshold_spec < 0) || (default_dark_threshold_spec > 100))
...
void _XmInitializeScrollBars(Widget w)
{
  ...
  if (ScrollBarVisible(sw->swindow.WorkWindow)) {
    ...
    sw->swindow.vExtent = (sw->swindow.ClipWindow->core.height > 0) ?
                              sw->swindow.ClipWindow->core.height :
                              1;
    sw->swindow.hExtent = (sw->swindow.ClipWindow->core.width > 0) ?
                              sw->swindow.ClipWindow->core.width :
                              1;
  ...
  if (sw->swindow.vScrollBar) {
    ...
    if (sw->swindow.WorkWindow) {
      if ((inc = ((sw->swindow.WorkWindow->core.height) / 10)) < 1)
        inc = 1;
      XtSetArg(vSBArgs[i], XmNincrement, (XtArgVal)inc);
      i++;
    }
    if ((inc = (sw->swindow.AreaHeight - (sw->swindow.AreaHeight / 10))) < 1)
      inc = sw->swindow.AreaHeight;
    XtSetArg(vSBArgs[i], XmNpageIncrement, (XtArgVal)inc);
    i++;
    XtSetArg(vSBArgs[i], XmNminimum, (XtArgVal)(sw->swindow.vmin));
    i++;
    XtSetArg(vSBArgs[i], XmNmaximum, (XtArgVal)(sw->swindow.vmax));
    i++;
    XtSetArg(vSBArgs[i], XmNvalue, (XtArgVal)sw->swindow.vOrigin);
    i++;
    XtSetArg(vSBArgs[i], XmNsliderSize, (XtArgVal)(sw->swindow.vExtent));
    i++;
  ...
  if (sw->swindow.hScrollBar) {
    ...
    if (sw->swindow.WorkWindow) {
      if ((inc = ((sw->swindow.WorkWindow->core.width) / 10)) < 1)
        inc = 1;
      XtSetArg(hSBArgs[i], XmNincrement, (XtArgVal)inc);
      i++;
    }
    if ((inc = (sw->swindow.AreaWidth - (sw->swindow.AreaWidth / 10))) < 1)
      inc = sw->swindow.AreaWidth;
    XtSetArg(hSBArgs[i], XmNpageIncrement, (XtArgVal)inc);
    i++;
    XtSetArg(hSBArgs[i], XmNminimum, (XtArgVal)(sw->swindow.hmin));
    i++;
    XtSetArg(hSBArgs[i], XmNmaximum, (XtArgVal)(sw->swindow.hmax));
    i++;
    XtSetArg(hSBArgs[i], XmNvalue, (XtArgVal)sw->swindow.hOrigin);
    i++;
    XtSetArg(hSBArgs[i], XmNsliderSize, (XtArgVal)(sw->swindow.hExtent));
    i++;
...
void _XmClearBGCompatibility(Widget pbg)
{
...
void _XmBulletinBoardSetDefaultShadow(Widget button)
{
  ...
  argc = 0;
  XtSetArg(argv[argc], XmNshadowThickness, &shadowTh);
  ++argc;
  XtSetArg(argv[argc], XmNdefaultButtonShadowThickness, &dbShadowTh);
  ++argc;
...
static void SetMonochromeColors(XmColorData *colors)
{
  ...
  if (background == BlackPixelOfScreen(screen)) {
    ...
    colors->foreground.red = colors->foreground.green = colors->foreground.blue = XmMAX_SHORT;
    colors->bottom_shadow.pixel = WhitePixelOfScreen(screen);
    colors->bottom_shadow.red = colors->bottom_shadow.green = colors->bottom_shadow.blue =
        XmMAX_SHORT;
    colors->select.pixel = WhitePixelOfScreen(screen);
    colors->select.red = colors->select.green = colors->select.blue = XmMAX_SHORT;
    colors->top_shadow.pixel = BlackPixelOfScreen(screen);
    colors->top_shadow.red = colors->top_shadow.green = colors->top_shadow.blue = 0;
  }
  else if (background == WhitePixelOfScreen(screen)) {
    colors->foreground.pixel = BlackPixelOfScreen(screen);
    colors->foreground.red = colors->foreground.green = colors->foreground.blue = 0;
    colors->top_shadow.pixel = WhitePixelOfScreen(screen);
    colors->top_shadow.red = colors->top_shadow.green = colors->top_shadow.blue = XmMAX_SHORT;
    colors->bottom_shadow.pixel = BlackPixelOfScreen(screen);
    colors->bottom_shadow.red = colors->bottom_shadow.green = colors->bottom_shadow.blue = 0;
    colors->select.pixel = BlackPixelOfScreen(screen);
    colors->select.red = colors->select.green = colors->select.blue = 0;
  }
  colors->allocated |= (XmFOREGROUND | XmTOP_SHADOW | XmBOTTOM_SHADOW | XmSELECT);
}
...
XmColorData *_XmGetColors(Screen *screen, Colormap color_map, Pixel background)
{
  ...
  if (_XmSearchColorCache(
          (XmLOOK_AT_SCREEN | XmLOOK_AT_CMAP | XmLOOK_AT_BACKGROUND), &new_colors, &old_colors))
  {
    /*
...
XmColorData *_XmGetDefaultColors(Screen *screen, Colormap color_map)
{
  ...
  if (DefaultDepthOfScreen(screen) == 1) {
    ...
    if (!XtCallConverter(
            DisplayOfScreen(screen), XtCvtStringToPixel, args, num_args, &fromVal, &toVal, NULL))
      background = WhitePixelOfScreen(screen);
  }
  else {
    /*  Parse out a color for the default background  */
    if (XParseColor(DisplayOfScreen(screen),
                    color_map,
                    _XmGetDefaultBackgroundColorSpec(screen),
                    &color_def))
    {
      if (XAllocColor(DisplayOfScreen(screen), color_map, &color_def)) {
...
static int _XmBrightness(XColor *color)
{
   ...
   */
  luminosity = (int)((XmRED_LUMINOSITY * (float)red) + (XmGREEN_LUMINOSITY * (float)green) +
  ...
  brightness = ((intensity * XmINTENSITY_FACTOR) + (light * XmLIGHT_FACTOR) +
  ...
  switch (which) {
    case XmBACKGROUND:
      if (!(cd->allocated & which) &&
          (XAllocColor(cd->screen->display, cd->color_map, &(cd->background)) == 0))
      {
        if (_XmBrightness(&(cd->background)) < XmFOREGROUND_THRESHOLD)
          cd->background.pixel = _XmBlackPixel(cd->screen, cd->color_map, cd->background);
        else
          cd->background.pixel = _XmWhitePixel(cd->screen, cd->color_map, cd->background);
        XQueryColor(cd->screen->display, cd->color_map, &(cd->background));
      }
    ...
    case XmFOREGROUND:
      if (!(cd->allocated & which) &&
          (XAllocColor(cd->screen->display, cd->color_map, &(cd->foreground)) == 0))
      {
        if (_XmBrightness(&(cd->background)) < XmFOREGROUND_THRESHOLD)
          cd->foreground.pixel = _XmWhitePixel(cd->screen, cd->color_map, cd->foreground);
        else
          cd->foreground.pixel = _XmBlackPixel(cd->screen, cd->color_map, cd->foreground);
        XQueryColor(cd->screen->display, cd->color_map, &(cd->foreground));
      }
    ...
    case XmTOP_SHADOW:
      if (!(cd->allocated & which) &&
          (XAllocColor(cd->screen->display, cd->color_map, &(cd->top_shadow)) == 0))
      {
        if (_XmBrightness(&(cd->background)) > XmCOLOR_LITE_THRESHOLD)
          cd->top_shadow.pixel = _XmBlackPixel(cd->screen, cd->color_map, cd->top_shadow);
        else
          cd->top_shadow.pixel = _XmWhitePixel(cd->screen, cd->color_map, cd->top_shadow);
        XQueryColor(cd->screen->display, cd->color_map, &(cd->top_shadow));
      }
    ...
    case XmBOTTOM_SHADOW:
      if (!(cd->allocated & which) &&
          (XAllocColor(cd->screen->display, cd->color_map, &(cd->bottom_shadow)) == 0))
      {
        if (_XmBrightness(&(cd->background)) < XmCOLOR_DARK_THRESHOLD)
          cd->bottom_shadow.pixel = _XmWhitePixel(cd->screen, cd->color_map, cd->bottom_shadow);
        else
          cd->bottom_shadow.pixel = _XmBlackPixel(cd->screen, cd->color_map, cd->bottom_shadow);
        XQueryColor(cd->screen->display, cd->color_map, &(cd->bottom_shadow));
      }
    ...
    case XmSELECT:
      if (!(cd->allocated & which) &&
          (XAllocColor(cd->screen->display, cd->color_map, &(cd->select)) == 0))
      {
        if (_XmBrightness(&(cd->background)) < XmFOREGROUND_THRESHOLD)
          cd->select.pixel = _XmWhitePixel(cd->screen, cd->color_map, cd->select);
        else
          cd->select.pixel = _XmBlackPixel(cd->screen, cd->color_map, cd->select);
        XQueryColor(cd->screen->display, cd->color_map, &(cd->select));
      }
      p = cd->select.pixel;
      cd->allocated |= which;
      break;
...
{
  return _XmReadImageAndHotSpotFromFile(_XmGetDefaultDisplay(), filename, hot_x, hot_y);
}
...
externaldef(xmprimbaseclassextrec) XmBaseClassExtRec _XmPrimbaseClassExtRec = {0};
externaldef(xmprimclassextrec) XmPrimitiveClassExtRec _XmPrimClassExtRec = {0};
externaldef(xmdrawnbprimclassextrec) XmPrimitiveClassExtRec _XmDrawnBPrimClassExtRec = {0};
externaldef(xmtogglebprimclassextrec) XmPrimitiveClassExtRec _XmToggleBPrimClassExtRec = {0};
externaldef(xmpushbprimclassextrec) XmPrimitiveClassExtRec _XmPushBPrimClassExtRec = {0};
externaldef(xmcascadebprimclassextrec) XmPrimitiveClassExtRec _XmCascadeBPrimClassExtRec = {0};
externaldef(xmtearoffbprimclassextrec) XmPrimitiveClassExtRec _XmTearOffBPrimClassExtRec = {0};
 ...
 *
 ************************************************************************/
Cardinal _XmFilterResources(XtResource *resources,
 ...
 ************************************************************************/
/*ARGSUSED*/
XtGeometryResult _XmRootGeometryManager(Widget w,
...
WidgetClass _XmGetActualClass(Display *display, WidgetClass w_class)
{
   ...
   * class
   */
  if (XFindContext(display, (Window)w_class, actualClassContext, (char **)&actualClass)) {
    return w_class;
  }
  else
    return actualClass;
 ...
 ************************************************************************/
/* ARGSUSED */
void _XmSetActualClass(Display *display, WidgetClass w_class, WidgetClass actualClass)
{
  ...
  XtRemoveCallback((Widget)dd, XtNdestroyCallback, DisplayDestroyCallback, (XtPointer)previous);
  /*
   * Save class data.
   * Delete old context if one exists.
   */
  if (XFindContext(display, (Window)w_class, actualClassContext, (char **)&oldActualClass)) {
    XSaveContext(display, (Window)w_class, actualClassContext, (char *)actualClass);
  }
  else if (oldActualClass != actualClass) {
    XDeleteContext(display, (Window)w_class, actualClassContext);
    XSaveContext(display, (Window)w_class, actualClassContext, (char *)actualClass);
  }
  XtAddCallback((Widget)dd, XtNdestroyCallback, DisplayDestroyCallback, (XtPointer)w_class);
}
 ...
 ************************************************************************/
/* ARGSUSED */
XmDesktopObject _XmGetWorldObject(Widget shell, ArgList args, Cardinal *num_args)
{
  ...
  if (XFindContext(display, (Window)NULL, worldObjectContext, (char **)&worldObject)) {
    ...
    worldObject = (XmDesktopObject)XtCreateWidget(
        "world", worldClass, appShell, args, num_args ? *num_args : 0);
    ext = _XmGetWidgetExtData(worldObject->ext.logicalParent, worldObject->ext.extensionType);
    if (ext != NULL) {
...
#endif
    XSaveContext(display, (Window)NULL, worldObjectContext, (char *)worldObject);
  }
  return worldObject;
}
 ...
 ************************************************************************/
/*ARGSUSED*/
void _XmVirtKeysHandler(Widget widget, XtPointer client_data, XEvent *event, Boolean *dontSwallow)
{
 ...
 *
 ****************************************************************/
Boolean _XmStringSourceFindString(Widget w,
 ...
 ****************************************************************/
/*ARGSUSED*/
Boolean _XmGrabTheFocus(Widget w, XEvent *event) /* unused */
{
...
void _XmProcessTraversal(Widget w, XmTraversalDirection dir, Boolean check) /* unused */
{
...
/*ARGSUSED*/
XmString _XmStringCreateExternal(XmRenderTable rendertable, /* unused */
                                 _XmString cs)
{
 ...
 */
Dimension _XmStringHeight(XmRenderTable rendertable, _XmString string)
{
 ...
 */
void _XmStringExtent(XmRenderTable rendertable,
                     _XmString string,
                     Dimension *width,
                     Dimension *height)
{
 ...
 */
Dimension _XmStringWidth(XmRenderTable rendertable, _XmString string)
{
  if (!string || !rendertable)
    return (0);
...
void _XmStringDraw(Display *d,
                   ...
                   XRectangle *clip)
{
  XmStringDraw(d, w, rendertable, string, gc, x, y, width, align, lay_dir, clip);
}

void _XmStringDrawImage(Display *d,
                        Window w,
                        XmRenderTable rendertable,
                        _XmString string,
                        GC gc,
                        ...
                        XRectangle *clip)
{
  XmStringDrawImage(d, w, rendertable, string, gc, x, y, width, align, lay_dir, clip);
}

void _XmStringDrawUnderline(Display *d,
                            Window w,
                            XmRenderTable f,
                            _XmString s,
                            GC gc,
                            ...
                            _XmString u)
{
  XmStringDrawUnderline(d, w, f, s, gc, x, y, width, align, lay_dir, clip, u);
}

void _XmStringDrawMnemonic(Display *d,
                           Window w,
                           XmRenderTable rendertable,
                           _XmString string,
                           GC gc,
                           ...
                           XmStringTag tag)
{
  ...
  XmStringDrawUnderline(
      d, w, rendertable, string, gc, x, y, width, align, lay_dir, clip, underline);
...
{
  if (string == (_XmString)NULL)
    return ((_XmString)NULL);
  else
    return ((_XmString)XmStringCopy(string));
}
...
Dimension _XmStringBaseline(XmRenderTable rendertable, _XmString string)
{
  if (!string || !rendertable)
    return (0);
...
Pixmap _XmGetPixmap(
    Screen *screen, char *image_name, int depth, Pixel foreground, Pixel background)
{
  return (XmGetPixmapByDepth(screen, image_name, foreground, background, depth));
}

Boolean _XmInstallPixmap(
    Pixmap pixmap, Screen *screen, char *image_name, Pixel foreground, Pixel background)
{
  return _XmCachePixmap(pixmap, screen, image_name, foreground, background, 0, 0, 0);
}
