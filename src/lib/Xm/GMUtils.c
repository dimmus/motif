  ...
  *replyWidth = *replyHeight = 0;
  for (i = 0; i < manager->composite.num_children; i++) {
    ...
    if (XtIsManaged(child)) {
      ...
      if (right > (int)*replyWidth)
        *replyWidth = right;
      if (bottom > (int)*replyHeight)
        *replyHeight = bottom;
    }
  }
 ...
 ****************/
Boolean _XmGMDoLayout(XmManagerWidget manager,
                      Dimension margin_width,
                      Dimension margin_height,
                      int resize_policy,
                      Boolean queryonly)
{
  ...
  _XmGMCalcSize(manager, margin_width, margin_height, &request.width, &request.height);
  /* no change, just accept it */
  if ((XtWidth(manager) == request.width) && (XtHeight(manager) == request.height))
    return (True);
  /* the current manager sizes are bigger than the new ones and
       we don't want to strink, just accept the layout as is, since
       it already fits */
  if ((resize_policy == XmRESIZE_GROW || resize_policy == XmRESIZE_NONE) &&
      ((XtWidth(manager) >= request.width) && (XtHeight(manager) >= request.height)))
    return (True);
  ...
  _XmProcessUnlock();
  /* now the request */
  switch (XtMakeGeometryRequest((Widget)manager, &request, &reply)) {
    case XtGeometryYes:
      if (!queryonly)
    ...
    case XtGeometryAlmost:
      ...
      if ((reply.width < request.width) || (reply.height < request.height)) {
        ...
        if (!queryonly) {
          (void)XtMakeResizeRequest((Widget)manager, reply.width, reply.height, NULL, NULL);
          (*resize)((Widget)manager);
        }
        return (True);
      }
...
void _XmGMEnforceMargin(XmManagerWidget manager,
                        Dimension margin_width,
                        Dimension margin_height,
                        Boolean setvalue)
{
  ...
  for (i = 0; i < manager->composite.num_children; i++) {
    ...
    if (XtIsManaged(child)) {
      if ((margin_width != 0) && (XtX(child) < (int)margin_width)) {
        do_move = True;
        newx = margin_width;
      }
      else
        newx = XtX(child);
      if ((margin_height != 0) && (XtY(child) < (int)margin_height)) {
        do_move = True;
        newy = margin_height;
      }
      else
      ...
      if (do_move) {
        if (setvalue) {
          ...
          XmeConfigureObject(
              child, newx, newy, child->core.width, child->core.height, child->core.border_width);
      }
    }
  }
}
 ...
 ****************/
XtGeometryResult _XmGMHandleQueryGeometry(Widget widget,
                                          ...
                                          int resize_policy)
{
  ...
  if (resize_policy == XmRESIZE_NONE) {
    ...
    _XmGMCalcSize((XmManagerWidget)widget, margin_width, margin_height, &width, &height);
    if ((resize_policy == XmRESIZE_GROW) &&
 ...
 *
 ****************/
XtGeometryResult XmeReplyToQueryGeometry(Widget widget,
                                         XtWidgetGeometry *intended,
                                         XtWidgetGeometry *desired)
{
  ...
  if ((IsWidth(intended)) && (intended->width == desired->width) && (IsHeight(intended)) &&
...
Boolean _XmGMOverlap(XmManagerWidget manager, Widget w)
{
  ...
  for (i = 0; i < manager->composite.num_children; i++) {
    ...
    Dimension right2 = XtX(kid) + 2 * XtBorderWidth(kid) + XtWidth(kid);
    Dimension bottom2 = XtY(kid) + 2 * XtBorderWidth(kid) + XtHeight(kid);
    if (w != kid &&
 ...
 ****************/
XtGeometryResult _XmGMHandleGeometryManager(Widget parent,
                                            Widget w,
                                            XtWidgetGeometry *request,
                                            XtWidgetGeometry *reply,
                                            Dimension margin_width,
                                            ...
                                            int allow_overlap)
{
  ...
  if (!IsY(request))
    localReply.y = XtY(w);
  if (!IsWidth(request))
    localReply.width = XtWidth(w);
  if (!IsHeight(request))
    localReply.height = XtHeight(w);
  if (!IsBorder(request))
    localReply.border_width = XtBorderWidth(w);
  /*  check for x less than margin width
   */
  if (IsX(request) && (margin_width != 0) && (request->x < (int)margin_width)) {
    localReply.x = (Position)margin_width;
    returnCode = XtGeometryAlmost;
  }
  /*  check for y less than margin height  */
  if (IsY(request) && (margin_height != 0) && (request->y < (int)margin_height)) {
  ...
  if (!allow_overlap && _XmGMOverlap(manager, w)) {
    ...
    geoFlag = _XmGMDoLayout(manager, margin_width, margin_height, resize_policy, queryonly);
