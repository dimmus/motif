...
#define EVENTS \
  ((unsigned int)(ButtonPressMask | ButtonReleaseMask | EnterWindowMask | LeaveWindowMask))
...
static void Arm(XmCascadeButtonGadget cb);
static void ArmAndPost(XmCascadeButtonGadget cb, XEvent *event);
static void ArmAndActivate(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void Disarm(XmCascadeButtonGadget cb, Boolean unpost);
static void PostTimeout(XtPointer closure, XtIntervalId *id);
static void DelayedArm(XmCascadeButtonGadget cb, XEvent *event);
static void CheckDisarm(XmCascadeButtonGadget cb, XEvent *event);
static void StartDrag(XmCascadeButtonGadget cb, XEvent *event);
static void Select(XmCascadeButtonGadget cb, XEvent *event, Boolean doCascade);
static void DoSelect(XmCascadeButtonGadget cb, XEvent *event);
static void KeySelect(XmCascadeButtonGadget cb, XEvent *event);
static void MenuBarSelect(Widget wid, XEvent *event);
static void MenuBarEnter(Widget wid, XEvent *event);
static void MenuBarLeave(Widget wid);
static void size_cascade(XmCascadeButtonGadget cascadebtn);
static void setup_cascade(XmCascadeButtonGadget cascadebtn,
                          Boolean adjustWidth,
...
static void GetArmGC(XmCascadeButtonGadget cb);
...
static Cardinal GetCascadeBGClassSecResData(WidgetClass w_class,
                                            XmSecondaryResourceData **data_rtn);
static XtPointer GetCascadeBGClassSecResBase(Widget widget, XtPointer client_data);
...
static XtResource cache_resources[] = {
    {XmNcascadePixmap,
     ...
     XmRDynamicPixmap,
     sizeof(Pixmap),
     XtOffsetOf(struct _XmCascadeButtonGCacheObjRec, cascade_button_cache.cascade_pixmap),
     XmRImmediate,
    ...
    {XmNmappingDelay,
     ...
     XmRInt,
     sizeof(int),
     XtOffsetOf(struct _XmCascadeButtonGCacheObjRec, cascade_button_cache.map_delay),
     XmRImmediate,
...
externaldef(xmcascadebuttongadgetclassrec)
 ...
 */
externaldef(xmcascadebuttongadgetclass)
...
static void InitializePosthook(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
   ...
   */
  _XmProcessLock();
  LabG_Cache(cbw) = (XmLabelGCacheObjPart *)_XmCachePart(
      LabG_ClassCachePart(cbw), (XtPointer)LabG_Cache(cbw), sizeof(XmLabelGCacheObjPart));
  CBG_Cache(cbw) = (XmCascadeButtonGCacheObjPart *)_XmCachePart(
      CBG_ClassCachePart(cbw), (XtPointer)CBG_Cache(cbw), sizeof(XmCascadeButtonGCacheObjPart));
  /*
...
int _XmArrowPixmapCacheCompare(XtPointer A, XtPointer B)
{
  ...
  if ((arrowpix_rec->height == arrowpix_cache_rec->height) &&
      ...
      (arrowpix_rec->top_shadow_color == arrowpix_cache_rec->top_shadow_color) &&
      (arrowpix_rec->bottom_shadow_color == arrowpix_cache_rec->bottom_shadow_color) &&
 ...
 *******************************************************************/
void _XmArrowPixmapCacheDelete(XtPointer data)
{
...
static void DrawShadow(XmCascadeButtonGadget cb)
{
  ...
  Boolean etched_in;
  etched_in = dpy->display.enable_etched_in_menu && (!(LabG_MenuType(cb) == XmMENU_OPTION));
  if (CBG_IsArmed(cb) || (LabG_MenuType(cb) == XmMENU_OPTION)) {
    if (XtIsRealized(XtParent(cb))) {
      XmeDrawShadows(XtDisplay(cb),
    ...
    if ((LabG_MenuType(cb) == XmMENU_OPTION) && (CBG_CascadePixmap(cb) == XmUNSPECIFIED_PIXMAP)) {
      ...
      if (LayoutIsRtoLG(cb)) {
        ...
        XmeDrawShadows(XtDisplay(cb),
                       ...
                       LabG_TopShadowGC(cb),
                       LabG_BottomShadowGC(cb),
                       cb->rectangle.x + CBG_Cascade_x(cb) + offset_x - G_ShadowThickness(cb),
                       cb->rectangle.y + CBG_Cascade_y(cb) + offset_y - G_ShadowThickness(cb),
                       width + (2 * G_ShadowThickness(cb)),
      ...
      Pixmap pixmap;
      int depth;
      pixmap = CBG_IsArmed(cb) && (CBG_ArmedPixmap(cb) != XmUNSPECIFIED_PIXMAP) ?
                   CBG_ArmedPixmap(cb) :
      ...
      if (depth == XtParent(cb)->core.depth)
        XCopyArea(XtDisplay(cb),
                  ...
                  0,
                  0,
                  CBG_Cascade_width(cb),
                  CBG_Cascade_height(cb),
                  cb->rectangle.x + CBG_Cascade_x(cb),
  ...
  if (CBG_HasCascade(cascadebtn)) {
    if (LayoutIsRtoLG(cascadebtn))
      ...
      CBG_Cascade_x(cascadebtn) = cascadebtn->rectangle.width -
                                  ...
                                  cascadebtn->gadget.shadow_thickness -
                                  LabG_MarginWidth(cascadebtn) - CBG_Cascade_width(cascadebtn);
    buffer = cascadebtn->gadget.highlight_thickness + cascadebtn->gadget.shadow_thickness +
             LabG_MarginHeight(cascadebtn);
    CBG_Cascade_y(cascadebtn) = buffer + ((cascadebtn->rectangle.height - 2 * buffer) -
...
static void InputDispatch(Widget wid, XEvent *event, Mask event_mask)
{
  ...
  if (event_mask & XmARM_EVENT) {
    ...
    if ((LabG_MenuType(cb) == XmMENU_PULLDOWN) || (LabG_MenuType(cb) == XmMENU_POPUP) ||
    ...
    if (LabG_IsMenupane(cb) && (((XmManagerWidget)XtParent(cb))->manager.active_child == wid) &&
        CBG_Submenu(cb))
    {
      XmMenuShellWidget mshell = (XmMenuShellWidget)XtParent(CBG_Submenu(cb));
      if ((mshell->composite.children[0] == CBG_Submenu(cb)) && (XmIsMenuShell(mshell)) &&
...
static void ArmAndPost(XmCascadeButtonGadget cb, XEvent *event)
{
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(cb)), XmQTmenuSystem);
  if (menuSTrait == NULL)
    return;
  if (!CBG_IsArmed(cb)) {
    if ((LabG_MenuType(cb) == XmMENU_OPTION) &&
  ...
  switch (LabG_MenuType(cb)) {
    ...
    case XmMENU_BAR: {
      ...
      if (myShell && XmIsMenuShell(myShell) && /* not torn ?! */
          (myShell->shell.popped_up) && (myShell->composite.children[0] == CBG_Submenu(cb)) &&
 ...
 */
static void PostTimeout(XtPointer closure, XtIntervalId *id) /* unused */
{
...
static void DelayedArm(XmCascadeButtonGadget cb, XEvent *event)
{
  if ((!CBG_IsArmed(cb)) && (((XmMenuShellWidget)XtParent(XtParent(cb)))->shell.popped_up) &&
    ...
    if ((CBG_IsArmed(cb)) && (CBG_Submenu(cb))) {
      ...
      if (submenushell->shell.popped_up) {
        if ((entEvent->x_root >= submenushell->core.x) &&
            (entEvent->x_root < submenushell->core.x + submenushell->core.width +
                                    (submenushell->core.border_width << 1)) &&
            (entEvent->y_root >= submenushell->core.y) &&
            (entEvent->y_root < submenushell->core.y + submenushell->core.height +
                                    (submenushell->core.border_width << 1)))
...
static void KeySelect(XmCascadeButtonGadget cb, XEvent *event)
{
  ...
  if (!_XmGetInDragMode((Widget)cb) && RC_IsArmed(parent)) {
    if (LabG_MenuType(cb) == XmMENU_BAR) {
      menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(cb)),
                                                  XmQTmenuSystem);
      if (menuSTrait != NULL)
        menuSTrait->menuBarCleanup((Widget)parent);
    }
    ...
    if (validButton) {
      ...
      if (!CBG_Submenu(cb)) {
        ...
        if (RC_BeingArmed(XtParent(cb))) {
          ...
          _XmGrabPointer(
              XtParent(cb), True, EVENTS, GrabModeAsync, GrabModeAsync, None, cursor, _time);
 ...
 */
void _XmCreateArrowPixmaps(Widget wid)
{
  XmCascadeButtonWidget cb = (XmCascadeButtonWidget)wid;
  XmArrowPixmap cpart;
  ...
  if (XmIsGadget(cb)) {
    ...
    if (etched_in) {
      ...
      armGC = XtGetGC((Widget)wid, GCForeground | GCBackground | GCGraphicsExposures, &values);
    }
   ...
   */
  _XmProcessLock();
  unarmed_arrow = (XmArrowPixmap *)_XmCachePart(
      &ArrowPixmapCache, (XtPointer)&cpart, sizeof(XmArrowPixmap));
  /*
   * Create or get an existing arrow cache record for the armed arrow
   */
  cpart.top_shadow_color = bsc;
  cpart.bottom_shadow_color = tsc;
  armed_arrow = (XmArrowPixmap *)_XmCachePart(
      &ArrowPixmapCache, (XtPointer)&cpart, sizeof(XmArrowPixmap));
  _XmProcessUnlock();
      ...
      (armed_arrow->pixmap == XmUNSPECIFIED_PIXMAP))
  {
    ...
    values.graphics_exposures = False;
    gc = XtGetGC((Widget)cb, GCForeground | GCBackground | GCGraphicsExposures, &values);
    /* armed arrow */
    if (armed_arrow->pixmap == XmUNSPECIFIED_PIXMAP) {
      ...
      XmeDrawArrow(XtDisplay((Widget)cb),
                   pixmap,
                   bsGC,
                   tsGC,
                   gc,
...
static void size_cascade(XmCascadeButtonGadget cascadebtn)
{
  ...
  if (CBG_CascadePixmap(cascadebtn) != XmUNSPECIFIED_PIXMAP) {
    XGetGeometry(XtDisplay(cascadebtn),
                 CBG_CascadePixmap(cascadebtn),
                 &rootwin,
                 &x,
                 &y,
                 &width,
    ...
    if (LabG_MenuType(cascadebtn) == XmMENU_OPTION) {
      CBG_Cascade_width(cascadebtn) = CBG_Cascade_height(
          cascadebtn) = MAX(LabG_TextRect(cascadebtn).height,
                          ...
                          Boolean adjustHeight)
{
  ...
  if (CBG_HasCascade(cascadebtn)) {
    ...
    delta = CBG_Cascade_height(cascadebtn) +
            2 * (LabG_MarginHeight(cascadebtn) + cascadebtn->gadget.shadow_thickness +
                 cascadebtn->gadget.highlight_thickness);
    ...
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  /* assign if changed! */
  if (!_XmCascadeBCacheCompare((XtPointer)CBG_Cache(new_w), (XtPointer)CBG_Cache(current))) {
    _XmCacheDelete((XtPointer)CBG_Cache(current)); /* delete the old one */
    CBG_Cache(new_w) = (XmCascadeButtonGCacheObjPart *)_XmCachePart(
        CBG_ClassCachePart(new_w),
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  XmMenuSystemTrait menuSTrait;
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(cw)), XmQTmenuSystem);
  if ((CBG_Submenu(new_w)) &&
      ((!XmIsRowColumn(CBG_Submenu(new_w))) || (RC_Type(CBG_Submenu(new_w)) != XmMENU_PULLDOWN)))
  {
  ...
  else if (LabG_MenuType(new_w) == XmMENU_PULLDOWN || LabG_MenuType(new_w) == XmMENU_POPUP ||
           LabG_MenuType(new_w) == XmMENU_OPTION)
  {
     ...
     */
    if ((CBG_CascadePixmap(old) != CBG_CascadePixmap(new_w)) ||
        (LabG_LabelType(old) != LabG_LabelType(new_w)) || (CBG_Submenu(old) != CBG_Submenu(new_w)))
    {
    ...
    /* make sure that other changes did not scrunch our pixmap */
      ...
      if ((new_w->gadget.highlight_thickness != old->gadget.highlight_thickness) ||
          (new_w->gadget.shadow_thickness != old->gadget.shadow_thickness) ||
  ...
  new_w->gadget.event_mask = XmARM_EVENT | XmACTIVATE_EVENT | XmFOCUS_IN_EVENT |
                             XmFOCUS_OUT_EVENT | XmENTER_EVENT | XmLEAVE_EVENT | XmHELP_EVENT |
                             XmBDRAG_EVENT;
  return (flag);
}
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(nw)), XmQTmenuSystem);
  if (!(LabG_MenuType(new_w) == XmMENU_BAR || LabG_MenuType(new_w) == XmMENU_PULLDOWN ||
        LabG_MenuType(new_w) == XmMENU_POPUP || LabG_MenuType(new_w) == XmMENU_OPTION))
  {
  ...
  if (LabG_MenuType(new_w) == XmMENU_PULLDOWN || LabG_MenuType(new_w) == XmMENU_POPUP ||
      LabG_MenuType(new_w) == XmMENU_OPTION)
  {
...
Widget XmVaCreateCascadeButtonGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmCascadeButtonGadgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedCascadeButtonGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmCascadeButtonGadgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
 ...
 */
void XmCascadeButtonGadgetHighlight(Widget wid, Boolean highlight)
{
  XmCascadeButtonGadget cb = (XmCascadeButtonGadget)wid;
                                            ...
                                            XmSecondaryResourceData **data_rtn)
{
  ...
  arrayCount = _XmSecondaryResourceData(
      bcePtr, data_rtn, client_data, resource_name, resource_class, GetCascadeBGClassSecResBase);
  _XmProcessUnlock();
  return (arrayCount);
}
 ...
 */
static XtPointer GetCascadeBGClassSecResBase(Widget widget, XtPointer client_data) /* unused */
{
