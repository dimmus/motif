...
static void SetValuesAlmost(Widget cw,
                            Widget nw,
...
static XtResource resources[] = {
    ...
    {XmNshadowThickness,
     ...
     sizeof(Dimension),
     XtOffsetOf(XmLabelGadgetRec, gadget.shadow_thickness),
     XmRImmediate,
     (XtPointer)0},
    {XmNlabelPixmap,
     XmCLabelPixmap,
     XmRDynamicPixmap,
     sizeof(Pixmap),
     XtOffsetOf(XmLabelGadgetRec, label.pixmap),
     XmRImmediate,
    ...
    {XmNlabelInsensitivePixmap,
     ...
     XmRDynamicPixmap,
     sizeof(Pixmap),
     XtOffsetOf(XmLabelGadgetRec, label.pixmap_insen),
     XmRImmediate,
     (XtPointer)XmUNSPECIFIED_PIXMAP},
    {XmNlabelString,
     XmCXmString,
    ...
    {"pri.vate",
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmLabelGadgetRec, label.check_set_render_table),
     XmRImmediate,
     (XtPointer)False},
    {XmNfontList,
     XmCFontList,
     XmRFontList,
     sizeof(XmFontList),
     XtOffsetOf(XmLabelGadgetRec, label.font),
     XmRCallProc,
    ...
    {XmNmnemonic,
     ...
     XmRKeySym,
     sizeof(KeySym),
     XtOffsetOf(XmLabelGadgetRec, label.mnemonic),
     XmRImmediate,
    ...
    {XmNtraversalOn,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmGadgetRec, gadget.traversal_on),
     XmRImmediate,
     (XtPointer)False},
    {XmNhighlightThickness,
     ...
     XmRHorizontalDimension,
     sizeof(Dimension),
     XtOffsetOf(XmGadgetRec, gadget.highlight_thickness),
     XmRImmediate,
     (XtPointer)0},
    {XmNpixmapPlacement,
     XmCPixmapPlacement,
     ...
     XtOffsetOf(XmLabelGadgetRec, label.pixmap_placement),
     XmRImmediate,
     (XtPointer)XmPIXMAP_LEFT},
...
static XtResource label_pixmap_resource[] = {{XmNlabelPixmap,
                                              XmCLabelPixmap,
                                              XmRDynamicPixmap,
                                              sizeof(Pixmap),
                                              XtOffsetOf(XmLabelGadgetRec, label.pixmap),
...
externaldef(xmlabelgcacheobjclassrec) XmLabelGCacheObjClassRec xmLabelGCacheObjClassRec = {
    {
...
externaldef(xmlabelgadgetclassrec) XmLabelGadgetClassRec xmLabelGadgetClassRec = {
    {
...
externaldef(xmlabelgadgetclass)
    WidgetClass xmLabelGadgetClass = (WidgetClass)&xmLabelGadgetClassRec;

static Pixmap GetTopShadowPixmapDefault(Widget widget)
{
  ...
  if (LabG_TopShadowColor(lg) == LabG_Background(lg))
    result = XmGetPixmapByDepth(XtScreen(lg),
                                XmS50_foreground,
...
static Pixmap GetLabelHighlightPixmapDefault(Widget widget)
{
  XmLabelGadget lg = (XmLabelGadget)widget;
  XmManagerWidget mw = (XmManagerWidget)XtParent(lg);
  Pixmap result = XmUNSPECIFIED_PIXMAP;
 ...
 *
 *******************************************************************/
int _XmLabelCacheCompare(XtPointer A, XtPointer B)
{
  ...
  if ((label_inst->label_type == label_cache_inst->label_type) &&
      ...
      (label_inst->bottom_shadow_color == label_cache_inst->bottom_shadow_color) &&
      (label_inst->bottom_shadow_pixmap == label_cache_inst->bottom_shadow_pixmap) &&
...
void _XmLabelGCloneMenuSavvy(WidgetClass wc, XmMenuSavvyTrait mst)
{
...
static void InitializePosthook(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  LabG_Cache(lw) = (XmLabelGCacheObjPart *)_XmCachePart(
      LabG_ClassCachePart(lw), (XtPointer)LabG_Cache(lw), sizeof(XmLabelGCacheObjPart));
  /*
    ...
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  /* assign if changed! */
  _XmProcessLock();
  if (!_XmLabelCacheCompare((XtPointer)LabG_Cache(new_w), (XtPointer)LabG_Cache(current))) {
    /* delete the old one */
    _XmCacheDelete((XtPointer)LabG_Cache(current));
    LabG_Cache(new_w) = (XmLabelGCacheObjPart *)_XmCachePart(
        LabG_ClassCachePart(new_w), (XtPointer)LabG_Cache(new_w), sizeof(XmLabelGCacheObjPart));
  }
  else
...
static void BorderHighlight(Widget w)
{
  ...
  XmeDrawHighlight(XtDisplay((Widget)lg),
                   XtWindow((Widget)lg),
                   LabG_HighlightGC(lg),
                   lg->rectangle.x,
                   lg->rectangle.y,
 ...
 *
 ************************************************************************/
void _XmLabelSetBackgroundGC(XmLabelGadget lw)
{
  XGCValues values;
  XtGCMask valueMask;
  XmManagerWidget mw;
  ...
  if (mw->core.background_pixmap != XmUNSPECIFIED_PIXMAP) {
    int depth;
    XmeGetPixmapData(XtScreen((Widget)lw),
 ...
 *
 *************************************************************************/
void _XmCalcLabelGDimensions(Widget wid)
{
  ...
  if (LabG__acceleratorText(newlw) != NULL) {
    ...
    if (!XmStringEmpty(LabG__acceleratorText(newlw))) {
      XmStringExtent(LabG_Font(newlw), LabG__acceleratorText(newlw), &dw, &dh);
...
void _XmLabelGCalcTextRect(Widget wid)
{
  ...
  if (LabG_IsPixmap(wid)) {
    ...
    {
      LabG_TextRect(wid).height = LabG_PixmapRect(wid).height + LabG_StringRect(wid).height +
                                  LabG_PixmapTextPadding(wid);
    ...
    {
      LabG_TextRect(wid).width = LabG_PixmapRect(wid).width + LabG_StringRect(wid).width +
                                 LabG_PixmapTextPadding(wid);
    ...
    else if ((LabG_PixmapPlacement(wid) == XmPIXMAP_RIGHT && LayoutIsRtoLG(wid)) ||
             (LabG_PixmapPlacement(wid) == XmPIXMAP_LEFT && !LayoutIsRtoLG(wid)))
    {
      LabG_PixmapRect(wid).x = 0;
      LabG_StringRect(wid).x = LabG_PixmapRect(wid).width + LabG_PixmapTextPadding(wid);
    }
    else if ((LabG_PixmapPlacement(wid) == XmPIXMAP_LEFT && LayoutIsRtoLG(wid)) ||
             (LabG_PixmapPlacement(wid) == XmPIXMAP_RIGHT && !LayoutIsRtoLG(wid)))
    {
    ...
    if (LabG_PixmapPlacement(wid) == XmPIXMAP_RIGHT || LabG_PixmapPlacement(wid) == XmPIXMAP_LEFT)
    {
      ...
      else if ((LabG_Alignment(wid) == XmALIGNMENT_END && !LayoutIsRtoLG(wid)) ||
               (LabG_Alignment(wid) == XmALIGNMENT_BEGINNING && LayoutIsRtoLG(wid)))
      {
...
static void Resize(Widget wid)
{
  ...
  if (newlw->rectangle.width == 0)
    newlw->rectangle.width = LabG_TextRect(newlw).width + LabG_MarginLeft(newlw) +
                             LabG_MarginRight(newlw) +
                             (2 * (LabG_MarginWidth(newlw) + newlw->gadget.highlight_thickness +
                                   newlw->gadget.shadow_thickness));
  leftx = newlw->gadget.highlight_thickness + newlw->gadget.shadow_thickness +
          LabG_MarginWidth(newlw) + LabG_MarginLeft(newlw);
  rightx = newlw->rectangle.width - newlw->gadget.highlight_thickness -
           newlw->gadget.shadow_thickness - LabG_MarginWidth(newlw) - LabG_MarginRight(newlw);
  ...
  if (newlw->rectangle.height == 0)
    newlw->rectangle.height = MAX(LabG_TextRect(newlw).height, LabG_AccTextRect(newlw).height) +
                              LabG_MarginTop(newlw) + LabG_MarginBottom(newlw) +
                              (2 * (LabG_MarginHeight(newlw) + newlw->gadget.highlight_thickness +
                                    newlw->gadget.shadow_thickness));
  LabG_TextRect(newlw).y =
      (short)(newlw->gadget.highlight_thickness + newlw->gadget.shadow_thickness +
              LabG_MarginHeight(newlw) + LabG_MarginTop(newlw) +
              ((int)(newlw->rectangle.height - LabG_MarginTop(newlw) - LabG_MarginBottom(newlw) -
                     (2 * (LabG_MarginHeight(newlw) + newlw->gadget.highlight_thickness +
                           newlw->gadget.shadow_thickness)) -
                     LabG_TextRect(newlw).height) /
               2));
  if (LabG__acceleratorText(newlw) != NULL) {
    ...
    if (LayoutIsRtoLG(newlw))
      ...
      LabG_AccTextRect(newlw).x = (newlw->rectangle.width - newlw->gadget.highlight_thickness -
                                   newlw->gadget.shadow_thickness - LabG_MarginWidth(newlw) -
                                   LabG_MarginRight(newlw) + LABELG_ACC_PAD);
    LabG_AccTextRect(newlw).y =
        newlw->gadget.highlight_thickness + newlw->gadget.shadow_thickness +
        LabG_MarginHeight(newlw) + LabG_MarginTop(newlw) +
        ((int)(newlw->rectangle.height - LabG_MarginTop(newlw) - LabG_MarginBottom(newlw) -
               (2 * (LabG_MarginHeight(newlw) + newlw->gadget.highlight_thickness +
                     newlw->gadget.shadow_thickness)) -
    ...
    if (LabG_IsText(newlw)) {
      ...
      base_accText = XmStringBaseline(LabG_Font(newlw), LabG__acceleratorText(newlw));
...
static void Initialize(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (!XmRepTypeValidValue(XmRID_STRING_DIRECTION, LabG_StringDirection(new_w), new_w))
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass((Widget)XtParent(new_w)),
                                              XmQTmenuSystem);
  ...
  if (LabG__label(new_w) == NULL) {
    /* reserved */
    LabG__label(new_w) = XmeGetLocalizedString(
        (char *)NULL, (Widget)lw, XmNlabelString, XrmQuarkToString(lw->object.xrm_name));
  }
  ...
  if ((LabG_MenuType(new_w) == XmMENU_POPUP) || (LabG_MenuType(new_w) == XmMENU_PULLDOWN) ||
  ...
  LabG_HighlightGC(lw) = _XmGetPixmapBasedGC(
      XtParent(new_w), LabG_HighlightColor(lw), LabG_Background(lw), LabG_HighlightPixmap(lw));
  LabG_TopShadowGC(lw) = _XmGetPixmapBasedGC(
      XtParent(new_w), LabG_TopShadowColor(lw), LabG_Background(lw), LabG_TopShadowPixmap(lw));
  ...
  if ((XtClass(lw) == xmLabelGadgetClass) &&
      ((LabG_MenuType(new_w) == XmMENU_POPUP) || (LabG_MenuType(new_w) == XmMENU_PULLDOWN) ||
  ...
  lw->gadget.event_mask = XmHELP_EVENT | XmFOCUS_IN_EVENT | XmFOCUS_OUT_EVENT | XmENTER_EVENT |
                          XmLEAVE_EVENT | XmBDRAG_EVENT;
}
...
static void DealWithColors(XmLabelGadget lw)
{
   ...
   */
  if ((LabG_Background(lw) == INVALID_PIXEL || LabG_Background(lw) == mw->core.background_pixel) &&
      (LabG_Foreground(lw) == INVALID_PIXEL || LabG_Foreground(lw) == mw->manager.foreground) &&
...
static void InitNewColorBehavior(XmLabelGadget lw)
{
  ...
  if (LabG_Background(lw) == INVALID_PIXEL) {
    _XmBackgroundColorDefault(
        (Widget)lw, XtOffsetOf(struct _XmLabelGCacheObjRec, label_cache.background), &value);
    memcpy((char *)&LabG_Background(lw), value.addr, value.size);
  }
  if (LabG_Foreground(lw) == INVALID_PIXEL) {
    _XmForegroundColorDefault(
        (Widget)lw, XtOffsetOf(struct _XmLabelGCacheObjRec, label_cache.foreground), &value);
    memcpy((char *)&LabG_Foreground(lw), value.addr, value.size);
  }
  if (LabG_TopShadowColor(lw) == INVALID_PIXEL) {
    _XmTopShadowColorDefault(
        (Widget)lw, XtOffsetOf(struct _XmLabelGCacheObjRec, label_cache.top_shadow_color), &value);
  ...
  if (LabG_BottomShadowColor(lw) == INVALID_PIXEL) {
    _XmBottomShadowColorDefault(
        (Widget)lw,
        XtOffsetOf(struct _XmLabelGCacheObjRec, label_cache.bottom_shadow_color),
        &value);
  ...
  if (LabG_HighlightColor(lw) == INVALID_PIXEL) {
    _XmHighlightColorDefault(
        (Widget)lw, XtOffsetOf(struct _XmLabelGCacheObjRec, label_cache.highlight_color), &value);
                                      ...
                                      XtWidgetGeometry *reply)
{
  ...
  reply->width = LabG_TextRect(lg).width +
                 (2 * (LabG_MarginWidth(lg) + lg->gadget.highlight_thickness +
                       lg->gadget.shadow_thickness)) +
                 LabG_MarginLeft(lg) + LabG_MarginRight(lg);
  if (reply->width == 0)
    reply->width = 1;
  reply->height = MAX(LabG_TextRect(lg).height, LabG_AccTextRect(lg).height) +
                  (2 * (LabG_MarginHeight(lg) + lg->gadget.highlight_thickness +
                        lg->gadget.shadow_thickness)) +
                  LabG_MarginTop(lg) + LabG_MarginBottom(lg);
  if (reply->height == 0)
    reply->height = 1;
...
static void LabelDrawBackground(Widget wid,
                                XEvent *event, /* unused */
...
void _XmRedisplayLabG(Widget wid, XEvent *event, Region region, LRectangle *background_box)
{
  XmLabelGadget lw = (XmLabelGadget)wid;
  GC gc;
  GC clipgc = NULL;
  ...
  {
    clip_rect.x = lw->rectangle.x + lw->gadget.highlight_thickness + lw->gadget.shadow_thickness +
                  LabG_MarginLeft(lw);
    clip_rect.y = lw->rectangle.y + lw->gadget.highlight_thickness + lw->gadget.shadow_thickness +
                  LabG_MarginTop(lw);
  ...
  if ((LabG_IsText(lw) || LabG_IsPixmapAndText(lw)) && (LabG__label(lw) != NULL)) {
    ...
    if (LabG_Mnemonic(lw) != XK_VoidSymbol) {
      ...
      if (XtIsSensitive(wid)) {
        /*Draw normal text*/
        XmStringDrawUnderline(XtDisplay(lw),
                              XtWindow(lw),
                              LabG_Font(lw),
                              LabG__label(lw),
                              LabG_NormalGC(lw),
  ...
  if (LabG__acceleratorText(lw) != NULL) {
    ...
    if ((lw->rectangle.width) >=
        (2 * (lw->gadget.highlight_thickness + lw->gadget.shadow_thickness +
              LabG_MarginWidth(lw)) +
         LabG_MarginLeft(lw) + LabG_TextRect(lw).width + LabG_MarginRight(lw)))
    {
      if (XtIsSensitive(wid)) {
        /*Draw normal text*/
        XmStringDraw(XtDisplay(lw),
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  new_w->gadget.event_mask |= XmFOCUS_IN_EVENT | XmFOCUS_OUT_EVENT | XmENTER_EVENT |
                              XmLEAVE_EVENT | XmBDRAG_EVENT;
  if ((LabG_MenuType(new_w) == XmMENU_POPUP) || (LabG_MenuType(new_w) == XmMENU_PULLDOWN) ||
      (LabG_MenuType(new_w) == XmMENU_BAR))
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(new_w)), XmQTmenuSystem);
...
Boolean _XmLabelGCVTRedraw(Widget kid, Widget cur_parent, Widget new_parent, Mask visual_flag)
{
  ...
  if (visual_flag & (VisualTopShadowColor | VisualTopShadowPixmap)) {
    ...
    if (LabG_TopShadowPixmap(lw) == curmw->manager.top_shadow_pixmap &&
      ...
      LabG_TopShadowPixmap(lw) = mw->manager.top_shadow_pixmap;
    LabG_TopShadowGC(lw) = _XmGetPixmapBasedGC(
        XtParent(lw), LabG_TopShadowColor(lw), LabG_Background(lw), LabG_TopShadowPixmap(lw));
    redraw = True;
  }
  ...
  if (visual_flag & (VisualHighlightColor | VisualHighlightPixmap)) {
    ...
    if (LabG_HighlightPixmap(lw) == curmw->manager.highlight_pixmap &&
      ...
      LabG_HighlightPixmap(lw) = mw->manager.highlight_pixmap;
    LabG_HighlightGC(lw) = _XmGetPixmapBasedGC(
        XtParent(lw), LabG_HighlightColor(lw), LabG_Background(lw), LabG_HighlightPixmap(lw));
    redraw = True;
  }
  _XmProcessLock();
  LabG_Cache(lw) = (XmLabelGCacheObjPart *)_XmCachePart(
...
static void Help(Widget w, XEvent *event)
{
  XmLabelGadget lg = (XmLabelGadget)w;
  XmMenuSystemTrait menuSTrait;
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(lg)), XmQTmenuSystem);
 ...
 *
 ***********************************************************************/
static void GetMnemonicCharset(Widget wid,
                               int resource, /* unused */
                               XtArgVal *value)
{
  XmLabelGadget lw = (XmLabelGadget)wid;
...
Widget XmVaCreateLabelGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmLabelGadgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedLabelGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmLabelGadgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
static Cardinal GetLabelBGClassSecResData(WidgetClass w_class, XmSecondaryResourceData **data_rtn)
{
  ...
  arrayCount = _XmSecondaryResourceData(
      bcePtr, data_rtn, client_data, resource_name, resource_class, GetLabelClassResBase);
  _XmProcessUnlock();
  return arrayCount;
}
 ...
 *   - Not yet implemented.
 */
static XtPointer GetLabelClassResBase(Widget widget, XtPointer client_data) /* unused */
{
...
void _XmProcessDrag(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (XmIsCascadeButtonGadget(w) && CBG_IsArmed(w))
    return;
  ...
  if (!dpy->display.enable_unselectable_drag ||
      (dpy->display.enable_btn1_transfer == XmBUTTON2_ADJUST && event &&
       event->xany.type == ButtonPress && event->xbutton.button == 2))
    return;
  n = 0;
...
void _XmQualifyLabelLocalCache(XmLabelGCacheObjPart *_local_cache, XmLabelGadget w)
{
  _XmProcessLock();
  ClassCacheCopy(LabG_ClassCachePart(w))(
      LabG_Cache(w), _local_cache, sizeof(XmLabelGCacheObjPart));
  _XmProcessUnlock();
}
...
void _XmReCacheLabG_r(XmLabelGCacheObjPart *_local_cache, XmLabelGadget w)
{
  ...
  if (!_XmLabelCacheCompare(_local_cache, (XtPointer)LabG_Cache(w))) {
    ...
    LabG_Cache(w) = (XmLabelGCacheObjPart *)_XmCachePart(
        LabG_ClassCachePart(w), _local_cache, sizeof(XmLabelGCacheObjPart));
  }
  _XmProcessUnlock();
}
