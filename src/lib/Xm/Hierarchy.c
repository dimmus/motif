...
static unsigned char open_file_bits[] = {0x06,
                                         0x00,
                                         0xf9,
                                         0x01,
                                         0x01,
...
static unsigned char close_file_bits[] = {0x0e,
                                          0x00,
                                          0xf1,
                                          0x01,
                                          0xff,
 ...
 * Type Converters
 ******************/
static Boolean CvtStringToNodeState(
...
static XtResource resources[] = {
    ...
    {XmNautoClose,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmHierarchyRec, hierarchy.auto_close),
     XmRImmediate,
     (XtPointer)True},
    {XmNrefigureMode,
     XmCBoolean,
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmHierarchyRec, hierarchy.refigure_mode),
     XmRImmediate,
     (XtPointer)True},
    {XmNnodeStateCallback,
     ...
     XtOffsetOf(XmHierarchyRec, hierarchy.node_state_callback),
     XmRPointer,
     (XtPointer)NULL},
    {XmNverticalMargin,
     XmCDimension,
    ...
    {XmNopenFolderPixmap,
     ...
     sizeof(Pixmap),
     XtOffsetOf(XmHierarchyRec, hierarchy.open_folder),
     XmRImmediate,
    ...
    {XmNcloseFolderPixmap,
     ...
     XmRPrimForegroundPixmap,
     sizeof(Pixmap),
     XtOffsetOf(XmHierarchyRec, hierarchy.close_folder),
     XmRImmediate,
     (XtPointer)XmUNSPECIFIED_PIXMAP},
    {XmNnodeStateChangedCallback,
     XmCNodeStateChangedCallback,
     XmRCallback,
     sizeof(XtCallbackList),
     XtOffsetOf(XmHierarchyRec, hierarchy.node_state_changed_callback),
     XmRPointer,
     (XtPointer)NULL},
    {XmNnodeStateBegEndCallback,
     ...
     XmRCallback,
     sizeof(XtCallbackList),
     XtOffsetOf(XmHierarchyRec, hierarchy.node_state_beg_end_callback),
     XmRPointer,
     (XtPointer)NULL},
    {XmNnodeStateBeginEndCallback,
     ...
     XmRPointer,
     (XtPointer)NULL}};
static XmSyntheticResource get_resources[] = {{XmNhorizontalMargin,
                                               sizeof(Dimension),
                                               XtOffsetOf(XmHierarchyRec, hierarchy.h_margin),
...
static XtResource constraints[] = {{XmNnodeState,
                                    XmCNodeState,
                                    XmRXmHierarchyNodeState,
                                    sizeof(XmHierarchyNodeState),
                                    XtOffsetOf(XmHierarchyConstraintRec, hierarchy.state),
                                    XmRImmediate,
                                    (XtPointer)XmOpen},
                                   {XmNparentNode,
                                    XmCParentNode,
                                    XmRWidget,
                                    sizeof(Widget),
                                    XtOffsetOf(XmHierarchyConstraintRec, hierarchy.parent),
                                    XmRImmediate,
                                    (XtPointer)NULL},
                                   {XmNinsertBefore,
                                    XmCInsertBefore,
                                    XmRWidget,
                                    sizeof(Widget),
                                    XtOffsetOf(XmHierarchyConstraintRec, hierarchy.insert_before),
                                    XmRImmediate,
                                    (XtPointer)NULL},
                                   {XmNnodeOpenFolderPixmap,
                                    ...
                                    XmRPrimForegroundPixmap,
                                    sizeof(Pixmap),
                                    XtOffsetOf(XmHierarchyConstraintRec, hierarchy.open_folder),
                                    XmRImmediate,
                                    (XtPointer)XmUNSPECIFIED_PIXMAP},
                                   {XmNnodeCloseFolderPixmap,
                                    ...
                                    XmRPrimForegroundPixmap,
                                    sizeof(Pixmap),
                                    XtOffsetOf(XmHierarchyConstraintRec, hierarchy.close_folder),
                                    XmRImmediate,
                                    (XtPointer)XmUNSPECIFIED_PIXMAP}};
...
static void Initialize(Widget req, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  top_node = ((HierarchyConstraints)XtMalloc(sizeof(HierarchyConstraintRec)));
  ...
  XmHierarchy_def_open_folder(hw) = XCreateBitmapFromData(
      XtDisplay(set), root, (char *)open_file_bits, open_file_width, open_file_height);
  XmHierarchy_def_close_folder(hw) = XCreateBitmapFromData(
      XtDisplay(set), root, (char *)close_file_bits, close_file_width, close_file_height);
 ...
 *	Returns:       none
 */
static void SetChildValues(HierarchyConstraints node, Pixmap open_folder, Pixmap close_folder)
{
  Arg args[1];
  Pixmap which = None;
  if (XmHierarchyC_state(node) == XmOpen) {
    ...
    if ((XmUNSPECIFIED_PIXMAP != XmHierarchyC_close_folder(node)) &&
        XmHierarchyC_close_folder(node))
 ...
 *	Returns:       none
 */
static void RecursiveSetChildValues(HierarchyConstraints curr_node,
                                    Pixmap open_folder,
                                    Pixmap close_folder)
{
  ...
  if (XmHierarchyC_num_children(curr_node) == 0)
    return;
 ...
 */
/*ARGSUSED*/
static Boolean SetValues(Widget w, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
  XmHierarchyWidget setw = (XmHierarchyWidget)set;
  if (!LoadFolderImages(w, set))
    return (False);
  RecursiveSetChildValues(
      XmHierarchy_top_node(setw), XmHierarchy_open_folder(setw), XmHierarchy_close_folder(setw));
  return (True);
}
    ...
    Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  if (XmHierarchyC_state(new_node) != XmHierarchyC_state(old_node)) {
    XtCallCallbackList((Widget)hw, XmHierarchy_node_state_beg_end_callback(hw), (XtPointer)True);
    {
    ...
    XtCallCallbackList((Widget)hw, XmHierarchy_node_state_beg_end_callback(hw), (XtPointer)False);
  }
 ...
 *	Returns: A Boolean
 */
static Boolean AncestorClosed(HierarchyConstraints node)
{
 ...
 *	Returns:       none.
 */
static void MapNode(HierarchyConstraints node)
{
 ...
 *	Returns:       none.
 */
static void UnmapNode(HierarchyConstraints node)
{
  if (!(XmHierarchyC_status(node) & IS_MAPPED))
    return;
  if (XmHierarchyC_widget(node) != NULL) {
 ...
 *	Returns:       none
 */
static void BuildNodeTable(Widget w, HierarchyConstraints node, Cardinal *current_index)
{
  XmHierarchyWidget hw = (XmHierarchyWidget)w;
 ...
 *	Returns:       none.
 */
static void ResetOpenCloseButton(Widget w, HierarchyConstraints node)
{
  Arg args[5];
  Pixmap image;
  Cardinal num_args;
  ...
  switch (XmHierarchyC_state(node)) {
    ...
    case XmHidden:
    default:
      if ((XmUNSPECIFIED_PIXMAP != XmHierarchyC_open_folder(node)) &&
          XmHierarchyC_open_folder(node))
  ...
  if (XmHierarchyC_open_close_button(node) == NULL) {
    ...
    num_args++;
    XmHierarchyC_open_close_button(node) = XtCreateManagedWidget(
        "openAndClose", xmIconButtonWidgetClass, w, args, num_args);
    /*
...
static void ToggleNodeState(Widget w, XtPointer node_ptr, XtPointer call_data)
{
  ...
  node_data.widget = XmHierarchyC_widget(node);
  node_data.state = XmHierarchyC_state(node);
  XtCallCallbackList((Widget)hw, XmHierarchy_node_state_callback(hw), &node_data);
  XtCallCallbackList((Widget)hw, XmHierarchy_node_state_beg_end_callback(hw), (XtPointer)False);
}
 ...
 */
/*ARGSUSED*/
static Boolean CvtStringToNodeState(Display *dpy,
                                    XrmValuePtr args,
                                    Cardinal *num_args,
                                    XrmValuePtr fromVal,
                                    XrmValuePtr toVal,
 ...
 */
static Boolean LoadFolderImages(Widget old, Widget set)
{
  XmHierarchyWidget setw = (XmHierarchyWidget)set;
 ...
 *	Returns: none.
 */
static void DestroyFolderImages(Widget w)
{
  XmHierarchyWidget hw = (XmHierarchyWidget)w;
 ...
 *	Returns:       none.
 */
static void AddChild(HierarchyConstraints parent, HierarchyConstraints child)
{
  if (parent == NULL)
    return;
 ...
 *	Returns: none.
 */
static void AddChildToList(HierarchyConstraints **list,
                           Cardinal *num,
                           Cardinal *alloc,
                           HierarchyConstraints child)
{
  ...
  if (success == NO) {
    ...
    _XmWarningMsg(XmHierarchyC_widget(child),
                  XmNinsertBeforeNotSibling,
                  XmNinsertBeforeNotSiblingMsg,
                  params,
                  1);
  }
 ...
 */
static HierarchyConstraints GetNodeInfo(Widget w)
{
 ...
 *	Returns:       none
 */
static void BumpChildren(HierarchyConstraints *nlist, int i, int num)
{
  for (i++; i < num; i++)
    nlist[i - 1] = nlist[i];
}
 ...
 *	Returns: none
 */
void XmHierarchyOpenAllAncestors(Widget nw)
{
  Widget parent;
  HierarchyConstraints node;
  static Arg args[] = {{XmNnodeState, (XtArgVal)XmOpen}};
