...
#include <limits.h>
    ...
    static void
    AddWidget(XmTextSource source, XmTextWidget tw);
static char *_XmStringSourceGetChar(XmSourceData data, XmTextPosition position);
static int CountLines(XmTextSource source, XmTextPosition start, unsigned long length);
static void RemoveWidget(XmTextSource source, XmTextWidget tw);
static void _XmStringSourceReadString(XmTextSource source, int start, XmTextBlock block);
...
static XmTextPosition Scan(XmTextSource source,
                           ...
                           int count,
                           Boolean include);
static Boolean GetSelection(XmTextSource source, XmTextPosition *left, XmTextPosition *right);
 ...
 */
int _XmTextCharactersToBytes(char *bytes, char *characters, int num_chars, int max_char_size)
{
...
char *_XmStringSourceGetString(XmTextWidget tw,
                               XmTextPosition from,
                               XmTextPosition to,
                               Boolean want_wchar)
{
  ...
  if (!want_wchar) {
    ...
    for (pos = from; pos < to;) {
      ...
      return_val = mbstowcs(&wc_buf[destpos], block.ptr, (unsigned int)(ret_pos - pos));
      if (return_val > 0)
        destpos += return_val;
      pos = ret_pos;
    }
...
static void AddWidget(XmTextSource source, XmTextWidget tw)
{
  ...
  if (data->numwidgets == 1)
    ...
    memmove((void *)tw->text.highlight.list,
            ...
            (size_t)data->widgets[0]->text.highlight.number * sizeof(_XmHighlightRec));
  }
  if (data->hasselection && data->numwidgets == 1) {
    Time select_time = XtLastTimestampProcessed(XtDisplay((Widget)tw));
    if (!select_time)
      select_time = _XmValidTimestamp((Widget)tw);
    if (!XmePrimarySource((Widget)data->widgets[0], select_time)) {
      ...
      cb.event = NULL;
      XtCallCallbackList(
          (Widget)data->widgets[0], data->widgets[0]->text.gain_primary_callback, (XtPointer)&cb);
    }
  }
}
...
static int CountLines(XmTextSource source, XmTextPosition start, unsigned long length)
{
  ...
  switch ((int)tw->text.char_size) {
    ...
    case 2: {
      ...
      /* search up to gap */
      while (seg_length--) {
        if (*bits16_ptr++ == *(BITS16 *)(data->PSWC_NWLN))
          ++num_lines;
      }
      /* check to see if we need more data after the gap */
    ...
    default: {
      ...
      /* search up to gap */
      while (seg_length--) {
        if (*wchar_t_ptr++ == *(wchar_t *)(data->PSWC_NWLN))
          ++num_lines;
      }
      /* check to see if we need more data after the gap */
...
Boolean *_XmStringSourceGetPending(XmTextWidget tw)
{
  Boolean *pending;
...
void _XmStringSourceSetPending(XmTextWidget tw, Boolean *pending)
{
  XmSourceData data = tw->text.source->data;
  int i;
  if ((long)pending > 1)
    ...
    for (i = 0; i < data->numwidgets; i++)
      ((XmTextWidget)data->widgets[i])->text.pendingoff = (Boolean)(long)pending;
}
 ...
 */
/********************************<->***********************************/
void _XmStringSourceSetGappedBuffer(XmSourceData data,
                                    XmTextPosition position) /* starting position */
{
  XmTextWidget tw = (XmTextWidget)data->widgets[0];
  ...
  if (data->ptr + (position * char_size) < data->gap_start) {
    ...
    count = data->gap_start - (data->ptr + (position * char_size));
    ...
    count = (data->ptr + (position * char_size)) - data->gap_start;
...
static XmTextPosition ReadSource(XmTextSource source,
                                 XmTextPosition position,      /* starting position */
                                 XmTextPosition last_position, /* The last position we're
                                 ...
                                 XmTextBlock block)            /* RETURN: text read in */
{
  ...
  if (block->length > 0) {
    ...
    if ((int)tw->text.char_size == 1) {
      ...
      num_bytes = _XmTextCharactersToBytes(
          data->value, block->ptr, block->length / char_size, (int)tw->text.char_size);
...
void _XmTextValidate(XmTextPosition *start, XmTextPosition *end, int maxsize)
{
  if (*start < 0)
    *start = 0;
  if (*start > maxsize) {
...
Boolean _XmTextModifyVerify(XmTextWidget initiator,
                            XEvent *event,
                            XmTextPosition *start,
                            XmTextPosition *end,
                            ...
                            Boolean *freeBlock)
{
  ...
  *freeBlock = False;
  ...
  if (!initiator->text.modify_verify_callback && !initiator->text.wcs_modify_verify_callback) {
  ...
  if (initiator->text.modify_verify_callback) {
    ...
    if (block->length) {
      ...
      *freeBlock = True;
      (void)memcpy((void *)newblock->ptr, (void *)block->ptr, block->length);
    ...
    tvcb.text = newblock;
    XtCallCallbackList(
        (Widget)initiator, initiator->text.modify_verify_callback, (XtPointer)&tvcb);
      ...
      if (tvcb.text != newblock || tvcb.text->ptr != newblock->ptr) {
        ...
        *freeBlock = False;
        if (newblock->length) {
          ...
          *freeBlock = True;
          (void)memcpy((void *)newblock->ptr, (void *)tvcb.text->ptr, tvcb.text->length);
        }
        else
          newblock->ptr = NULL;
      }
      newblock->format = tvcb.text->format;
      block_num_chars = _XmTextCountCharacters(newblock->ptr, newblock->length);
      delta = block_num_chars - (*end - *start);
      if (delta > 0 && data->length + delta > data->maxallowed && (!UnderVerifyPreedit(initiator)))
      {
        if (newblock->ptr && newblock->ptr != block->ptr)
          XtFree(newblock->ptr);
        *freeBlock = False;
  ...
  if (initiator->text.wcs_modify_verify_callback) {
    ...
    XtCallCallbackList(
        (Widget)initiator, initiator->text.wcs_modify_verify_callback, (XtPointer)&wcs_tvcb);
    if (!wcs_tvcb.doit) {
      if (newblock->ptr && newblock->ptr != block->ptr)
        XtFree(newblock->ptr);
      *freeBlock = False;
      if (wcs_newblock.wcsptr)
                            ...
                            Boolean call_callbacks) /* unused */
{
  ...
  if (data->length + delta >= data->maxlength) {
    ...
    if (gap_end_offset != (old_maxlength * char_size))
      memmove(
          data->gap_end, data->ptr + gap_end_offset, (char_size * old_maxlength) - gap_end_offset);
  ...
  for (i = 0; i < data->numwidgets; i++) {
    ...
    _XmTextUpdateLineTable((Widget)data->widgets[i], *start, *end, block, True);
    if (data->hasselection)
      _XmTextSetHighlight((Widget)data->widgets[i], data->left, data->right, XmHIGHLIGHT_SELECTED);
...
#define Look(data, position, dir) \
  ((dir == XmsdLeft) ? \
       ((position) ? _XmStringSourceGetChar(data, position - 1) : NULL) : \
       ((position == data->length) ? NULL : _XmStringSourceGetChar(data, position)))
                          ...
                          XmTextPosition *last_char)
{
  ...
  while (position >= 0 && position <= data->length) {
    ...
    (void)_XmTextCharactersToBytes(mb_char, c, 1, (int)data->widgets[0]->text.char_size);
    if (*mb_char == '\n') {
      /* DELTA: Look now returns a pointer */
    ...
    case XmSELECT_WORD:
      if (tw->text.char_size == 1) {
        ...
        for (i = 0; i < count; i++) {
          whiteSpace = -1;
          num_bytes = _XmTextCharactersToBytes(
              mb_char, Look(data, position, dir), 1, (int)tw->text.char_size);
          start_is_mb = (num_bytes < 2 ? False : True);
          while (position >= 0 && position <= data->length) {
            num_bytes = _XmTextCharactersToBytes(
                mb_char, Look(data, position, dir), 1, (int)tw->text.char_size);
            cur_is_mb = (num_bytes < 2 ? False : True);
            if (!cur_is_mb && isspace((unsigned char)*mb_char)) {
              if (whiteSpace < 0)
                whiteSpace = position;
            }
            else if ((sType == XmSELECT_WORD) && (start_is_mb ^ cur_is_mb)) {
              if (whiteSpace < 0)
                whiteSpace = position;
              break;
            }
      ...
      for (i = 0; i < count; i++) {
        while (position >= 0 && position <= data->length) {
          ...
          if ((int)tw->text.char_size == 1) {
            ...
            if ((bits16_ptr == NULL) || (*bits16_ptr == *(BITS16 *)data->PSWC_NWLN))
              break;
          }
          else { /* MB_CUR_MAX == 3 or 4 or more */
            wchar_t_ptr = (wchar_t *)Look(data, position, dir);
            if ((wchar_t_ptr == NULL) || (*wchar_t_ptr == *(wchar_t *)data->PSWC_NWLN))
              break;
          }
          if (((dir == XmsdRight) && (position == data->length)) ||
              ((dir == XmsdLeft) && (position == 0)))
                         ...
                         Time set_time)
{
  ...
  if (!XtIsRealized((Widget)data->widgets[0]) || (left > right && !data->hasselection))
    return;
  if (left < 0)
    left = right = 0;
  for (i = 0; i < data->numwidgets; i++) {
  ...
  if (data->numwidgets > 0) {
    ...
    if (!set_time)
      set_time = _XmValidTimestamp(widget);
    if (left <= right) {
      ...
      if (data->hasselection && data->left < data->right) {
        for (i = 0; i < data->numwidgets; i++) {
          _XmTextSetHighlight(
              (Widget)data->widgets[i], data->left, data->right, XmHIGHLIGHT_SELECTED);
        }
      }
      if (left == right)
...
/* Public routines. */
void _XmTextValueChanged(XmTextWidget initiator, XEvent *event)
{
  XmAnyCallbackStruct cb;
  cb.reason = XmCR_VALUE_CHANGED;
  cb.event = event;
  if (initiator->text.value_changed_callback)
    XtCallCallbackList((Widget)initiator, initiator->text.value_changed_callback, (XtPointer)&cb);
}

XmTextSource _XmStringSourceCreate(char *value, Boolean is_wchar)
{
  XmTextSource source;
  XmSourceData data;
  int num_chars;
  ...
  data = source->data = (XmSourceData)XtMalloc((unsigned)sizeof(XmSourceDataRec));
  ...
  if (is_wchar) {
    ...
    if (ret_value < 0)
      ...
      data->length = _XmTextBytesToCharacters(
          data->ptr, tmp_value, num_chars, False, max_char_size);
    XtFree(tmp_value);
  }
  else {
    if (value)
  ...
  _XmTextBytesToCharacters(data->PSWC_NWLN, &newline, 1, False, max_char_size);
  data->numwidgets = 0;
...
char *_XmStringSourceGetValue(XmTextSource source, Boolean want_wchar)
{
  XmSourceData data = source->data;
  XmTextWidget tw = (XmTextWidget)data->widgets[0];
  XmTextBlockRec block;
...
void _XmStringSourceSetValue(XmTextWidget tw, char *value)
{
  ...
  (*source->SetSelection)(source, 1, 0, XtLastTimestampProcessed(XtDisplay(tw)));
  block.format = XmFMT_8_BIT;
  ...
  data->maxallowed = INT_MAX;
  _XmTextSetHighlight((Widget)tw, 0, tw->text.last_position, XmHIGHLIGHT_NORMAL);
  if (_XmTextModifyVerify(tw, NULL, &fromPos, &toPos, NULL, &block, &newblock, &freeBlock)) {
