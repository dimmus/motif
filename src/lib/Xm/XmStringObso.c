    ...
    XmStringLtoRCreate(char *text, XmStringTag tag)
{
  ...
  while (!done) /* loop thu local copy */ { /* looking for \n */
    end = start;
    while ((*end != '\0') && (*end != '\n'))
      end++;
    if (*end == '\0')
...
{
  XmString result;
  result = XmStringConcatAndFree(XmStringDirectionCreate(direction), XmStringCreate(text, tag));
  if (separator)
...
Boolean XmStringByteCompare(XmString a1, XmString b1)
{
  unsigned char *a;
  unsigned char *b;
  ...
  *text = NULL; /* pre-condition result */
  ...
  while (!done) {
    if (XmStringGetNextSegment(context, &t, &c, &d, &s)) {
      if (c && ((d == XmSTRING_DIRECTION_L_TO_R) || (d == XmSTRING_DIRECTION_UNSET)) &&
          (((is_local || is_default) &&
            ((c == XmFONTLIST_DEFAULT_TAG) || (strcmp(c, XmFONTLIST_DEFAULT_TAG) == 0) ||
 ...
 */
Boolean _XmStringGetSegment(_XmStringContext context,
                            ...
                            Boolean *pop_after)
{
  ...
  if (tag_count)
    *tag_count = 0;
  if (direction)
    *direction = _XmStrContDir(context); /* for BC */
  if (separator)
    *separator = False;
  if (tabs)
    *tabs = 0;
  ...
  if (!(context && text && tag && type && rendition_tags && tag_count && direction && separator &&
        tabs && char_count && push_before && pop_after))
    return False;
                                ...
                                Boolean *separator)
{
  ...
  result = _XmStringGetSegment(context,
                               True,
                               True,
                               (XtPointer *)text,
                               tag,
                               ...
                               Boolean *separator)
{
  ...
  Boolean ret_val;
  _XmProcessLock();
  ret_val = _XmStringGetNextSegment(
      (_XmStringContext)context, tag, direction, text, &char_count, separator);
  _XmProcessUnlock();
  return ret_val;
}
