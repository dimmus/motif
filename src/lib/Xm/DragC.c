...
static void CopyRefForeground(Widget widget, int offset, XrmValue *value);
static void GetRefBackground(Widget widget, int offset, XrmValue *value);
static void DragContextInitialize(Widget req, Widget new_w, ArgList args, Cardinal *numArgs);
static Boolean DragContextSetValues(
    Widget old, Widget ref, Widget new_w, Arg *args, Cardinal *numArgs);
static void DragContextDestroy(Widget w);
static void DragContextClassInitialize(void);
static void DragContextClassPartInitialize(WidgetClass);
static Window GetClientWindow(Display *dpy, Window win, Atom atom);
static void ValidateDragOver(XmDragContext dc, unsigned char oldStyle, unsigned char newStyle);
static XmDragReceiverInfo FindReceiverInfo(XmDragContext dc, Window win);
static void GetDestinationInfo(XmDragContext dc, Window root, Window win);
static void GetScreenInfo(XmDragContext dc);
static void SendDragMessage(XmDragContext dc, Window destination, unsigned char messageType);
static void GenerateClientCallback(XmDragContext dc, unsigned char reason);
static void DropLoseIncrSelection(Widget w, Atom *selection, XtPointer clientData);
static void DropLoseSelection(Widget w, Atom *selection);
static void DragDropFinish(XmDragContext dc);
static Boolean DropConvertIncrCallback(Widget w,
                                       Atom *selection,
                                       Atom *target,
                                       Atom *typeRtn,
                                       XtPointer *valueRtn,
...
static Boolean DropConvertCallback(Widget w,
                                   Atom *selection,
                                   Atom *target,
                                   Atom *typeRtn,
                                   XtPointer *valueRtn,
                                   unsigned long *lengthRtn,
                                   int *formatRtn);
static void DragStartProto(XmDragContext dc);
static void NewScreen(XmDragContext dc, Window newRoot);
static void LocalNotifyHandler(Widget w, XtPointer client, XtPointer call);
static void ExternalNotifyHandler(Widget w, XtPointer client, XtPointer call);
static void InitiatorMsgHandler(Widget w,
                                XtPointer clientData,
                                XEvent *event,
                                Boolean *dontSwallow);
static void SiteEnteredWithLocalSource(Widget w, XtPointer client, XtPointer call);
static void SiteLeftWithLocalSource(Widget w, XtPointer client, XtPointer call);
static void OperationChanged(Widget w, XtPointer client, XtPointer call);
static void SiteMotionWithLocalSource(Widget w, XtPointer client, XtPointer call);
static void DropStartConfirmed(Widget w, XtPointer client, XtPointer call);
static Widget GetShell(Widget w);
static void InitDropSiteManager(XmDragContext dc);
static void TopWindowsReceived(Widget w,
                               ...
                               int *format);
static void DragStart(XmDragContext dc, Widget src, XEvent *event);
static void DragStartWithTracking(XmDragContext dc);
static void UpdateMotionBuffer(XmDragContext dc, MotionBuffer mb, XEvent *event);
static void DragMotionProto(XmDragContext dc, Window root, Window subWindow);
static void ProcessMotionBuffer(XmDragContext dc, MotionBuffer mb);
static void DragMotion(Widget w, XEvent *event, String *params, Cardinal *numParams);
static void DragKey(Widget w, XEvent *event, String *params, Cardinal *numParams);
static void DropStartTimeout(XtPointer clientData, XtIntervalId *id);
static void DropFinishTimeout(XtPointer clientData, XtIntervalId *id);
static void FinishAction(XmDragContext dc, XEvent *ev);
static void CheckModifiers(XmDragContext dc, unsigned int state);
static void IgnoreButtons(Widget w, XEvent *event, String *params, Cardinal *numParams);
static void CancelDrag(Widget w, XEvent *event, String *params, Cardinal *numParams);
static void HelpDrag(Widget w, XEvent *event, String *params, Cardinal *numParams);
static void FinishDrag(Widget w, XEvent *event, String *params, Cardinal *numParams);
static void InitiatorMainLoop(XtPointer clientData, XtIntervalId *id);
static void DragCancel(XmDragContext dc);
static void CalculateDragOperation(XmDragContext dc);
static void cancelDrag(Widget w, XtPointer client, XtPointer call);
static void noMoreShell(Widget w, XtPointer client, XtPointer call);
...
static XtResource dragContextResources[] = {
    ...
    },
    {
        ...
        XmCConvertProc,
        XmRFunction,
        sizeof(XmConvertSelectionRec),
        Offset(drag.convertProc),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
    ...
    },
    {
        ...
        Offset(drag.dragOperations),
        XmRImmediate,
        (XtPointer)(XmDROP_COPY | XmDROP_MOVE),
    },
    {
        XmNsourceCursorIcon,
        XmCSourceCursorIcon,
    ...
    },
    {
        ...
        XmRWidget,
        sizeof(Widget),
        Offset(drag.operationCursorIcon),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
    ...
    },
    {
        ...
        XmRPixel,
        sizeof(Pixel),
        Offset(drag.cursorForeground),
        XmRCallProc,
        (XtPointer)GetRefForeground,
    },
    {
        XmNvalidCursorForeground,
        XmCValidCursorForeground,
        XmRPixel,
        sizeof(Pixel),
        Offset(drag.validCursorForeground),
        XmRCallProc,
        (XtPointer)CopyRefForeground,
    },
    {
        XmNinvalidCursorForeground,
        XmCInvalidCursorForeground,
        XmRPixel,
        sizeof(Pixel),
        Offset(drag.invalidCursorForeground),
        XmRCallProc,
        (XtPointer)CopyRefForeground,
    },
    {
        XmNnoneCursorForeground,
        XmCNoneCursorForeground,
        XmRPixel,
        sizeof(Pixel),
        Offset(drag.noneCursorForeground),
        XmRCallProc,
        (XtPointer)CopyRefForeground,
    },
    {
    ...
    },
    {
        ...
        XmRCallback,
        sizeof(XtCallbackList),
        Offset(drag.topLevelEnterCallback),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRCallback,
        sizeof(XtCallbackList),
        Offset(drag.dragMotionCallback),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRCallback,
        sizeof(XtCallbackList),
        Offset(drag.topLevelLeaveCallback),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
    ...
    },
    {
        ...
        XmRCallback,
        sizeof(XtCallbackList),
        Offset(drag.dragDropFinishCallback),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRCallback,
        sizeof(XtCallbackList),
        Offset(drag.dropFinishCallback),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRCallback,
        sizeof(XtCallbackList),
        Offset(drag.operationChangedCallback),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRBlendModel,
        sizeof(unsigned char),
        Offset(drag.blendModel),
        XmRImmediate,
        (XtPointer)XmBLEND_ALL,
    },
    {
        XmNsourceIsExternal,
        XmCSourceIsExternal,
...
externaldef(dragContextclass) WidgetClass xmDragContextClass = (WidgetClass)&xmDragContextClassRec;
...
static void DragContextInitialize(Widget req, Widget new_w, ArgList args, Cardinal *numArgs)
{
  ...
  dc->drag.dragFinishTime = dc->drag.dropFinishTime = 0;
  ...
  if (dc->drag.exportTargets) {
    ...
    dc->drag.exportTargets = (Atom *)_XmAllocAndCopy(dc->drag.exportTargets, size);
  }
  dc->core.x = dc->core.y = 0;
  dc->core.width = dc->core.height = 16;
  /* pixel values in drag context refer to the source widget's colormap */
  if (dc->drag.sourceWidget) {
    Widget sw = dc->drag.sourceWidget;
    dc->core.colormap = XmIsGadget(sw) ? XtParent(sw)->core.colormap : sw->core.colormap;
  }
  XtRealizeWidget((Widget)dc);
  dc->drag.currReceiverInfo = dc->drag.receiverInfos = NULL;
  dc->drag.numReceiverInfos = dc->drag.maxReceiverInfos = 0;
    ...
    Widget old, Widget ref, Widget new_w, Arg *args, Cardinal *numArgs)
{
  ...
  if (oldDC->drag.exportTargets != newDC->drag.exportTargets) {
    ...
    if (newDC->drag.exportTargets) {
      ...
      newDC->drag.exportTargets = (Atom *)_XmAllocAndCopy(newDC->drag.exportTargets, size);
    }
  }
  if (oldDC->drag.operationCursorIcon != newDC->drag.operationCursorIcon ||
      ...
      oldDC->drag.invalidCursorForeground != newDC->drag.invalidCursorForeground ||
      oldDC->drag.validCursorForeground != newDC->drag.validCursorForeground)
  {
...
static void DragContextClassPartInitialize(WidgetClass wc)
{
...
XmDragReceiverInfo _XmAllocReceiverInfo(XmDragContext dc)
{
  Cardinal offset = 0;
  if (dc->drag.currReceiverInfo) {
    offset = (Cardinal)(dc->drag.currReceiverInfo - dc->drag.receiverInfos);
  }
...
static void GetDestinationInfo(XmDragContext dc, Window root, Window win)
{
  ...
  currReceiverInfo = dc->drag.currReceiverInfo = FindReceiverInfo(dc, win);
  /*
   * check for bootstrap case
   */
  if ((dc->drag.trackingMode == XmDRAG_TRACK_MOTION) && (XtWindow(dc->drag.srcShell) == win) &&
      (!currReceiverInfo || (currReceiverInfo->frame == currReceiverInfo->window)))
  {
    ...
    (void)XTranslateCoordinates(
        XtDisplayOfObject((Widget)dc), win, currRoot, rel_x, rel_y, &root_x, &root_y, &win);
    if (currReceiverInfo)
  ...
  if (currReceiverInfo == NULL) {
    ...
    currReceiverInfo = dc->drag.currReceiverInfo = _XmAllocReceiverInfo(dc);
    currReceiverInfo->frame = win;
  ...
  if (currReceiverInfo != dc->drag.rootReceiverInfo /* is it the root ? */) {
    if (!currReceiverInfo->shell) {
      ...
      if (_XmDropSiteShell(dc->drag.currReceiverInfo->shell))
        currReceiverInfo->dragProtocolStyle = xmDisplay->display.dragReceiverProtocolStyle;
      else
  ...
  if (dc->drag.trackingMode != XmDRAG_TRACK_WM_QUERY_PENDING) {
    dc->drag.activeProtocolStyle = _XmGetActiveProtocolStyle((Widget)dc);
...
static void GetScreenInfo(XmDragContext dc)
{
  ...
  if (dc->drag.numReceiverInfos == 0) {
    dc->drag.rootReceiverInfo = rootInfo = _XmAllocReceiverInfo(dc);
  }
  else {
    dc->drag.rootReceiverInfo = rootInfo = dc->drag.receiverInfos;
  }
  rootInfo->frame = None;
...
static void SendDragMessage(XmDragContext dc, Window destination, unsigned char messageType)
{
  ...
  if (dc->drag.activeProtocolStyle == XmDRAG_NONE ||
      (dc->drag.activeProtocolStyle == XmDRAG_DROP_ONLY && reason != XmCR_DROP_START))
    return;
  switch (callbackRec.any.reason = reason) {
    ...
    case XmCR_TOP_LEVEL_LEAVE: {
      XmTopLevelLeaveCallback callback = (XmTopLevelLeaveCallback)&callbackRec;
      callback->timeStamp = dc->drag.lastChangeTime;
      callback->window = dc->drag.srcWindow;
    } break;
    case XmCR_DRAG_MOTION: {
      XmDragMotionCallback callback = (XmDragMotionCallback)&callbackRec;
      callback->timeStamp = dc->drag.lastChangeTime;
    ...
    case XmCR_DROP_START: {
      XmDropStartCallback callback = (XmDropStartCallback)&callbackRec;
   ...
   * to the wire
   */
  if ((!dc->drag.currReceiverInfo->shell) && (!dc->drag.sourceIsExternal /* sanity check */) &&
       ...
       dc->drag.activeProtocolStyle == XmDRAG_XDND || reason == XmCR_DROP_START))
  {
    ...
    if (dc->drag.activeProtocolStyle == XmDRAG_XDND && dc->drag.iccHandle != xdndSelection) {
    ...
    if ((reason == XmCR_TOP_LEVEL_ENTER) || (reason == XmCR_TOP_LEVEL_LEAVE) ||
        (reason == XmCR_DROP_START))
    {
      ...
      topLevelData.xOrigin = (Position)(dc->drag.currReceiverInfo->xOrigin);
      topLevelData.yOrigin = (Position)(dc->drag.currReceiverInfo->yOrigin);
      topLevelData.width = (Dimension)(dc->drag.currReceiverInfo->width);
      topLevelData.height = (Dimension)(dc->drag.currReceiverInfo->height);
    ...
    else if ((reason == XmCR_DRAG_MOTION) || (reason == XmCR_OPERATION_CHANGED)) {
    ...
    _XmDSMUpdate(dsm, (XtPointer)data, (XtPointer)&callbackRec);
  }
}
...
static void GenerateClientCallback(XmDragContext dc, unsigned char reason)
{
  ...
  switch (callbackRec.any.reason = reason) {
    ...
    case XmCR_DROP_SITE_LEAVE: {
      XmDropSiteLeaveCallback callback = (XmDropSiteLeaveCallback)&callbackRec;
      callback->timeStamp = dc->drag.lastChangeTime;
    } break;
    default:
      break;
...
static void DragDropFinish(XmDragContext dc)
{
  ...
  if (dc->drag.blendModel != XmBLEND_NONE && dc->drag.dragDropCancelEffect == False) {
    _XmDragOverFinish((Widget)dc->drag.curDragOver, dc->drag.dragDropCompletionStatus);
  }
  ...
  XtRemoveEventHandler(
      dc->drag.srcShell, FocusChangeMask, True, InitiatorMsgHandler, (XtPointer)dc);
                                       ...
                                       XtRequestId *requestID)
{
  enum {
    XmATRANSFER_SUCCESS,
    XmATRANSFER_FAILURE,
    XmA_MOTIF_CANCEL_DROP_EFFECT,
    XmA_MOTIF_DROP,
    XmATARGETS,
    NUM_ATOMS
  };

  static char *atom_names[] = {XmSTRANSFER_SUCCESS,
                               XmSTRANSFER_FAILURE,
  ...
  XInternAtoms(XtDisplayOfObject((Widget)dc), atom_names, XtNumber(atom_names), False, atoms);
      ...
      (*target == atoms[XmATRANSFER_FAILURE]))
  {
    ...
    *maxLengthRtn = 0;
    ...
    returnVal = (Boolean)((*(dc->drag.convertProc.sel_incr))((Widget)dc,
                                                             &motifDrop,
                                                             target,
                                                             typeRtn,
                                                             valueRtn,
                                   ...
                                   int *formatRtn)
{
      ...
      (*target == atoms[XmATRANSFER_FAILURE]))
  {
    ...
    *valueRtn = NULL;
    ...
    Atom motifDrop = atoms[XmA_MOTIF_DROP];
    returnVal = (Boolean)((*(dc->drag.convertProc.sel))(
        (Widget)dc, &motifDrop, target, typeRtn, valueRtn, lengthRtn, formatRtn));
  }
  /* add code to handle TARGET target automatically if not there yet */
  if ((!returnVal) && (*target == atoms[XmATARGETS])) {
    int len = sizeof(Atom) * dc->drag.numExportTargets;
...
static void DragStartProto(XmDragContext dc)
{
  ...
  GetDestinationInfo(dc, RootWindowOfScreen(XtScreen(dc)), XtWindow(dc->drag.srcShell));
...
static void NewScreen(XmDragContext dc, Window newRoot)
{
  ...
  i++;
  XtSetArg(args[i], XmNhotX, dc->core.x);
  i++;
  XtSetArg(args[i], XmNhotY, dc->core.y);
  i++;
  XtSetArg(args[i], XmNbackgroundPixmap, None);
  i++;
  XtSetArg(args[i], XmNscreen, dc->drag.currScreen);
  i++;
  XtSetArg(args[i], XmNdepth, DefaultDepthOfScreen(dc->drag.currScreen));
  i++;
  XtSetArg(args[i], XmNcolormap, DefaultColormapOfScreen(dc->drag.currScreen));
  i++;
  XtSetArg(args[i], XmNvisual, DefaultVisualOfScreen(dc->drag.currScreen));
  i++;
  /*
   ...
   * Isn't that handy?
   */
  dc->drag.curDragOver = (XmDragOverShellWidget)XtCreatePopupShell(
      "dragOver", xmDragOverShellWidgetClass, (Widget)dc, args, i);
  if (dc->drag.currScreen == XtScreen(dc->drag.srcShell))
    _XmDragOverSetInitialPosition((Widget)dc->drag.curDragOver, dc->drag.startX, dc->drag.startY);
  if (old != NULL) {
  ...
  if (dc->drag.trackingMode == XmDRAG_TRACK_MOTION) {
    ...
    if (XGrabPointer(XtDisplayOfObject((Widget)dc->drag.curDragOver),
      ...
      Warning(MESSAGE4);
    XAllowEvents(
        XtDisplayOfObject((Widget)dc->drag.srcShell), SyncPointer, dc->drag.lastChangeTime);
  }
}
...
static void LocalNotifyHandler(Widget w, XtPointer client, XtPointer call)
{
  ...
  switch (((XmAnyICCCallback)call)->reason) {
    ...
    case XmCR_DROP_SITE_LEAVE:
       ...
       */
      {
        XmDropSiteLeaveCallbackStruct *dcb = (XmDropSiteLeaveCallbackStruct *)call;
        ...
        SiteLeftWithLocalSource((Widget)dsm, (XtPointer)dc, (XtPointer)&new_call);
        break;
      }
    case XmCR_DRAG_MOTION:
                                ...
                                Boolean *dontSwallow)
{
  ...
  (void)dontSwallow;
  if (!dc ||
      !_XmICCEventToICCCallback((XClientMessageEvent *)event, &callbackRec, XmICC_RECEIVER_EVENT))
    return;
...
{
  XmDragContext dc = (XmDragContext)client;
  XmDropSiteEnterPendingCallbackStruct *cb = (XmDropSiteEnterPendingCallbackStruct *)call;
  dc->drag.inDropSite = False;
  if (dc->drag.siteLeaveCallback) {
    XtCallCallbackList(
        (Widget)dc, dc->drag.siteLeaveCallback, (XmDropSiteLeaveCallbackStruct *)cb);
  }
  CalculateDragOperation(dc);
  /*
...
static void DropStartConfirmed(Widget w, XtPointer client, XtPointer call)
{
  ...
  dc->drag.dragTimerId = XtAppAddTimeOut(
      appContext, XtAppGetSelectionTimeout(appContext) * 10, DropFinishTimeout, (XtPointer)dc);
...
static void InitDropSiteManager(XmDragContext dc)
{
  ...
  XtSetArg(args[i], XmNclientData, dc);
  i++;
  if (dc->drag.sourceIsExternal) {
    XtSetArg(args[i], XmNnotifyProc, ExternalNotifyHandler);
    i++;
  }
  else {
    XtSetArg(args[i], XmNnotifyProc, LocalNotifyHandler);
    i++;
  }
                               ...
                               int *format)
{
  ...
  if ((*length != 0) && (*format == 32) && (*type == XA_WINDOW)) {
    ...
    dc->drag.receiverInfos = (XmDragReceiverInfo)XtCalloc(dc->drag.maxReceiverInfos,
                                                          sizeof(XmDragReceiverInfoStruct));
    clientWindows = (Window *)value;
    if (startInfo) {
      memcpy((char *)dc->drag.receiverInfos, (char *)startInfo, sizeof(XmDragReceiverInfoStruct));
      dc->drag.rootReceiverInfo = dc->drag.receiverInfos;
    ...
    for (i = 1; i < dc->drag.numReceiverInfos; i++) {
      ...
      currInfo->shell = XtWindowToWidget(XtDisplay(dc), currInfo->window);
      if (currInfo->shell == NULL) {
        XSelectInput(XtDisplay(dc), currInfo->window, EnterWindowMask | LeaveWindowMask);
      }
    }
    /*
     ...
     * we're confined to
     */
    dc->drag.currReceiverInfo = FindReceiverInfo(dc, XtWindow(dc->drag.srcShell));
...
static void DragStart(XmDragContext dc, Widget src, XEvent *event)
{
  ...
  dc->drag.crossingTime = dc->drag.dragStartTime = dc->drag.lastChangeTime = event->xbutton.time;
  ...
  dc->drag.activeProtocolStyle = activeProtocolStyle = _XmGetActiveProtocolStyle((Widget)dc);
  ...
  XtInsertEventHandler(
      dc->drag.srcShell, FocusChangeMask, True, InitiatorMsgHandler, (XtPointer)dc, XtListHead);
  mask = _XmDRAG_EVENT_MASK(dc);
  /*
   ...
   */
  {
    ...
    XSelectInput(XtDisplay(dc), dc->drag.currWmRoot, xwa.your_event_mask | ButtonMotionMask);
  }
  /* End fixing OSF CR 5556 */
  if (dc->drag.trackingMode == XmDRAG_TRACK_WM_QUERY_PENDING) {
    enum { XmA_MOTIF_WM_QUERY, XmA_MOTIF_WM_ALL_CLIENTS, NUM_ATOMS };

    static char *atom_names[] = {XmS_MOTIF_WM_QUERY, XmS_MOTIF_WM_ALL_CLIENTS};
    Atom atoms[XtNumber(atom_names)];
    assert(XtNumber(atom_names) == NUM_ATOMS);
    XInternAtoms(XtDisplay(dc), atom_names, XtNumber(atom_names), False, atoms);
    XtGetSelectionValue((Widget)dd,
                        ...
                        dc->drag.dragStartTime);
    XAllowEvents(
        XtDisplayOfObject((Widget)dc->drag.srcShell), SyncPointer, dc->drag.dragStartTime);
  }
  else
    DragStartWithTracking(dc);
...
static void DragStartWithTracking(XmDragContext dc)
{
  ...
  XAllowEvents(XtDisplayOfObject((Widget)dc->drag.srcShell), SyncPointer, dc->drag.lastChangeTime);
}
  ...
  switch (event->type) {
    ...
    case EnterNotify:
          ...
          (dc->drag.trackingMode != XmDRAG_TRACK_MOTION))
      {
        ...
        if ((rInfo = FindReceiverInfo(dc, event->xcrossing.subwindow)) != NULL)
    ...
    if (dc->drag.currReceiverInfo->window) {
          ...
          (dc->drag.activeProtocolStyle != XmDRAG_DROP_ONLY))
      {
        ...
        if ((dc->drag.activeProtocolStyle == XmDRAG_DYNAMIC) &&
            (!dc->drag.currReceiverInfo->shell) && (dc->drag.inDropSite))
        {
...
static void ProcessMotionBuffer(XmDragContext dc, MotionBuffer mb)
{
  ...
  if (protoWindow == XtWindow(dc->drag.curDragOver)) {
    ...
    int dummyx, dummyy;
    (void)XTranslateCoordinates(
        XtDisplay(dc), currRoot, currRoot, dc->core.x, dc->core.y, &dummyx, &dummyy, &protoWindow);
  }
  /*
...
#define IsGrabEvent(ev) \
  ((ev->type == ButtonPress) || (ev->type == ButtonRelease) || (ev->type == KeyPress) || \
   (ev->type == KeyRelease))
...
static void FinishAction(XmDragContext dc, XEvent *ev)
{
  ...
  if (dc->drag.curDragOver) {
    ...
    XtSetArg(args[i], XmNhotY, dc->core.y);
    i++;
    if (currentActiveMode == XmCURSOR || (currentMode != XmDRAG_WINDOW && currentMode != XmWINDOW))
    {
      XtSetArg(args[i], XmNdragOverMode, XmWINDOW);
      i++;
    }
  ...
  if (dc->drag.currReceiverInfo) {
    if (dc->drag.currReceiverInfo->window) {
           ...
           (dc->drag.dragCompletionStatus == XmDROP_HELP)))
      {
         ...
         */
        dc->drag.dragTimerId = XtAppAddTimeOut(
            appContext, XtAppGetSelectionTimeout(appContext), DropStartTimeout, (XtPointer)dc);
        SendDragMessage(dc, dc->drag.currReceiverInfo->window, XmDROP_START);
      }
      else {
        SendDragMessage(dc, dc->drag.currReceiverInfo->window, XmTOP_LEVEL_LEAVE);
        GenerateClientCallback(dc, XmCR_TOP_LEVEL_LEAVE);
...
static void CheckModifiers(XmDragContext dc, unsigned int state)
{
  ...
  dc->drag.lastEventState = state;
  CalculateDragOperation(dc);
  if ((oldOperations != dc->drag.operations) || (oldOperation != dc->drag.operation)) {
    if ((dc->drag.currReceiverInfo->window) && (dc->drag.activeProtocolStyle != XmDRAG_NONE) &&
        (dc->drag.activeProtocolStyle != XmDRAG_DROP_ONLY))
    {
      SendDragMessage(dc, dc->drag.currReceiverInfo->window, XmOPERATION_CHANGED);
    }
    else {
...
static void InitiatorMainLoop(XtPointer clientData, XtIntervalId *id)
{
    ...
    if (_XmGetFocusPolicy(shell) == XmEXPLICIT) {
      ...
      if (focusWidget != (Widget)NULL) {
        if (XmIsPrimitive(focusWidget)) {
          if (((XmPrimitiveWidgetClass)XtClass(focusWidget))->primitive_class.border_unhighlight)
            (*(((XmPrimitiveWidgetClass)XtClass(focusWidget))
                   ->primitive_class.border_unhighlight))(focusWidget);
        }
        else if (XmIsGadget(focusWidget)) {
          if (((XmGadgetClass)XtClass(focusWidget))->gadget_class.border_unhighlight)
            (*(((XmGadgetClass)XtClass(focusWidget))->gadget_class.border_unhighlight))(
                focusWidget);
        }
      }
    }
  ...
  while ((*activeDC) && (XtAppGetExitFlag(appContext) == False)) {
    ...
    if (mask & XtIMXEvent) {
      ...
      if ((event.type == MotionNotify || event.type == LeaveNotify || event.type == EnterNotify) &&
   ...
   */
  if (contAction) {
    XtRemoveCallback(shell, XmNdestroyCallback, noMoreShell, (XtPointer)&contAction);
    if (_XmGetFocusPolicy(shell) == XmEXPLICIT) {
...
Widget XmDragStart(Widget w, XEvent *event, ArgList args, Cardinal numArgs)
{
  ...
  if (event->type != ButtonPress && event->type != ButtonRelease && event->type != KeyRelease &&
  ...
  cb.widget = w;
  cb.doit = True;
  XtCallCallbackList((Widget)dd, dd->display.dragStartCallback, (XtPointer)&cb);
  if (!cb.doit) {
    _XmAppUnlock(app);
    return NULL;
  }
  ...
  dc = (XmDragContext)XtCreateWidget(
      "dragContext", xmDragContextClass, (Widget)dd, mergedArgs, numArgs + 1);
...
Boolean XmTargetsAreCompatible(Display *dpy,
                               Atom *exportTargets,
...
{
  unsigned char initiator = XmDRAG_NONE, receiver = XmDRAG_NONE, active = XmDRAG_NONE;
  XmDragContext dc = (XmDragContext)w;
  XmDisplay xmDisplay = (XmDisplay)XtParent(dc);
...
static void CalculateDragOperation(XmDragContext dc)
{
  ...
  if ((dc->drag.lastEventState & ShiftMask) && (dc->drag.lastEventState & ControlMask)) {
    dc->drag.operations = dc->drag.operation = (unsigned char)(XmDROP_LINK &
                                                               dc->drag.dragOperations);
  }
  else if (dc->drag.lastEventState & ShiftMask) {
    dc->drag.operations = dc->drag.operation = (unsigned char)(XmDROP_MOVE &
                                                               dc->drag.dragOperations);
  }
  else if (dc->drag.lastEventState & ControlMask) {
    dc->drag.operations = dc->drag.operation = (unsigned char)(XmDROP_COPY &
                                                               dc->drag.dragOperations);
  }
