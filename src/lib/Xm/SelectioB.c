...
#define IsAutoButton(sb, w) \
  (w == SB_OkButton(sb) || w == SB_ApplyButton(sb) || w == SB_CancelButton(sb) || \
   w == SB_HelpButton(sb))
...
static void SelectionBoxCallback(Widget w, XtPointer client_data, XtPointer call_data);
static void ListCallback(Widget w, XtPointer client_data, XtPointer call_data);
...
static XtResource resources[] = {
    ...
    {XmNdialogType,
     ...
     XmRCallProc,
     (XtPointer)_XmDialogTypeDefault},
    {XmNchildPlacement,
     XmCChildPlacement,
     XmRChildPlacement,
     sizeof(unsigned char),
     ...
     XmRImmediate,
     (XtPointer)XmPLACE_ABOVE_SELECTION},
};
externaldef(xmselectionboxclassrec) XmSelectionBoxClassRec xmSelectionBoxClassRec = {
    {
...
externaldef(xmselectionboxwidgetclass)
...
static void ClassInitialize(void)
{
   ...
   */
  defaultTextAcceleratorsParsed = XtParseAcceleratorTable(defaultTextAccelerators);
  return;
}
...
static void ClassPartInitialize(WidgetClass w_class)
{
  ...
  XmSelectionBoxWidgetClass super = (XmSelectionBoxWidgetClass)wc->core_class.superclass;
  if (wc->selection_box_class.list_callback == XmInheritCallbackProc)
    wc->selection_box_class.list_callback = super->selection_box_class.list_callback;
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if (new_w->selection_box.text_accelerators == NULL) {
    new_w->selection_box.text_accelerators = defaultTextAcceleratorsParsed;
  }
  /*	Validate dialog type.
   */
  if (!XmRepTypeValidValue(XmRID_SELECTION_TYPE, new_w->selection_box.dialog_type, (Widget)new_w))
  {
    new_w->selection_box.dialog_type = XmIsDialogShell(XtParent(new_w)) ? XmDIALOG_SELECTION :
                                                                          XmDIALOG_WORK_AREA;
  }
  /*      Validate child placement.
   */
  if (!XmRepTypeValidValue(
          XmRID_CHILD_PLACEMENT, new_w->selection_box.child_placement, (Widget)new_w))
  {
      ...
      (new_w->selection_box.dialog_type != XmDIALOG_COMMAND))
  {
    if (new_w->selection_box.list_label_string == (XmString)XmUNSPECIFIED) {
      ...
      new_w->selection_box.list_label_string = (XmString)XmUNSPECIFIED;
    }
    else
      _XmSelectionBoxCreateListLabel(new_w);
  }
  ...
  if (new_w->selection_box.selection_label_string == (XmString)XmUNSPECIFIED) {
    new_w->selection_box.selection_label_string = NULL;
    _XmSelectionBoxCreateSelectionLabel(new_w);
    new_w->selection_box.selection_label_string = (XmString)XmUNSPECIFIED;
  }
  else {
    _XmSelectionBoxCreateSelectionLabel(new_w);
    new_w->selection_box.selection_label_string = NULL;
  ...
  if (new_w->selection_box.dialog_type != XmDIALOG_COMMAND) {
    ...
    if (new_w->selection_box.apply_label_string == (XmString)XmUNSPECIFIED) {
      ...
      new_w->selection_box.apply_label_string = (XmString)XmUNSPECIFIED;
    }
    else
      _XmSelectionBoxCreateApplyButton(new_w);
    ...
    _XmBulletinBoardSetDynDefaultButton((Widget)new_w, BB_DefaultButton(new_w));
  }
  else {
...
static void InsertChild(Widget child)
{
  ...
  _XmProcessLock();
  insert_child =
      ((XmBulletinBoardWidgetClass)xmBulletinBoardWidgetClass)->composite_class.insert_child;
  _XmProcessUnlock();
  (*insert_child)(child);
   ...
   */
  if (!sb->selection_box.adding_sel_widgets && !XtIsShell(child)) {
...
static void DeleteChild(Widget child)
{
  ...
  _XmProcessLock();
  delete_child =
      ((XmBulletinBoardWidgetClass)xmBulletinBoardWidgetClass)->composite_class.delete_child;
  _XmProcessUnlock();
  (*delete_child)(child);
  return;
}
 ...
 ****************/
static void _XmDialogTypeDefault(Widget widget,
                                 int offset, /* unused */
                                 XrmValue *value)
{
...
static XmImportOperator _XmSetSyntheticResForChild(Widget widget,   /* unused */
                                                   int offset,      /* unused */
...
{
  /****************/
  SB_ListLabel(sel) = _XmBB_CreateLabelG(
      (Widget)sel, sel->selection_box.list_label_string, "Items", XmItemsStringLoc);
  return;
}
...
void _XmSelectionBoxCreateSelectionLabel(XmSelectionBoxWidget sel)
{
  ...
  if (sel->selection_box.dialog_type == XmDIALOG_COMMAND)
    SB_SelectionLabel(sel) = _XmBB_CreateLabelG(
        (Widget)sel, sel->selection_box.selection_label_string, "Selection", XmPromptStringLoc);
  else
    SB_SelectionLabel(sel) = _XmBB_CreateLabelG(
        (Widget)sel, sel->selection_box.selection_label_string, "Selection", XmSelectionStringLoc);
}
...
void _XmSelectionBoxCreateList(XmSelectionBoxWidget sel)
{
  ...
  if (sel->selection_box.list_item_count != XmUNSPECIFIED) {
    XtSetArg(al[ac], XmNitemCount, sel->selection_box.list_item_count);
    ac++;
  }
  XtSetArg(al[ac], XmNvisibleItemCount, sel->selection_box.list_visible_item_count);
  ac++;
  sel->selection_box.list_selected_item_position = 0;
  XtSetArg(al[ac], XmNstringDirection, SB_StringDirection(sel));
  ac++;
  ...
  if (sel->selection_box.text_string != (XmString)XmUNSPECIFIED) {
    if (sel->selection_box.text_string &&
        XmListGetMatchPos(SB_List(sel), sel->selection_box.text_string, &position, &pos_count))
    {
      if (pos_count) {
  ...
  callbackProc =
      ((XmSelectionBoxWidgetClass)sel->core.widget_class)->selection_box_class.list_callback;
  if (callbackProc) {
    XtAddCallback(SB_List(sel), XmNsingleSelectionCallback, callbackProc, (XtPointer)sel);
    XtAddCallback(SB_List(sel), XmNbrowseSelectionCallback, callbackProc, (XtPointer)sel);
    XtAddCallback(SB_List(sel), XmNdefaultActionCallback, callbackProc, (XtPointer)sel);
  }
...
void _XmSelectionBoxCreateText(XmSelectionBoxWidget sel)
{
  ...
  /****************/
  XtSetArg(al[ac], XmNcolumns, sel->selection_box.text_columns);
  ac++;
  ...
  if ((sel->selection_box.text_string != (XmString)XmUNSPECIFIED)) {
    text_value = _XmStringGetTextConcat(sel->selection_box.text_string);
    XmTextFieldSetString(SB_Text(sel), text_value);
    if (text_value) {
      XmTextFieldSetInsertionPosition(SB_Text(sel), XmTextFieldGetLastPosition(SB_Text(sel)));
    }
...
void _XmSelectionBoxCreateSeparator(XmSelectionBoxWidget sel)
{
  ...
  ac++;
  SB_Separator(sel) = XmCreateSeparatorGadget((Widget)sel, "Separator", al, ac);
  return;
}
...
{
  /****************/
  SB_OkButton(sel) = _XmBB_CreateButtonG(
      (Widget)sel, sel->selection_box.ok_label_string, "OK", XmOkStringLoc);
  XtAddCallback(
      SB_OkButton(sel), XmNactivateCallback, SelectionBoxCallback, (XtPointer)XmDIALOG_OK_BUTTON);
  return;
}
...
{
  /****************/
  SB_ApplyButton(sel) = _XmBB_CreateButtonG(
      (Widget)sel, sel->selection_box.apply_label_string, "Apply", XmApplyStringLoc);
  ...
  XtAddCallback(SB_ApplyButton(sel),
                XmNactivateCallback,
...
{
  /****************/
  SB_CancelButton(sel) = _XmBB_CreateButtonG(
      (Widget)sel, sel->selection_box.cancel_label_string, "Cancel", XmCancelStringLoc);
  XtAddCallback(SB_CancelButton(sel),
                XmNactivateCallback,
                SelectionBoxCallback,
...
{
  /****************/
  SB_HelpButton(sel) = _XmBB_CreateButtonG(
      (Widget)sel, sel->selection_box.help_label_string, "Help", XmHelpStringLoc);
  /* Remove BulletinBoard Unmanage callback from apply and help buttons.
   */
  XtRemoveAllCallbacks(SB_HelpButton(sel), XmNactivateCallback);
  XtAddCallback(SB_HelpButton(sel),
                XmNactivateCallback,
                SelectionBoxCallback,
                (XtPointer)XmDIALOG_HELP_BUTTON);
  return;
}

/****************************************************************/
XmGeoMatrix _XmSelectionBoxGeoMatrixCreate(Widget wid,
                                           Widget instigator,
                                           XtWidgetGeometry *desired)
{
   ...
   */
  geoSpec = _XmGeoMatrixAlloc(XmSB_MAX_WIDGETS_VERT, sb->composite.num_children, 0);
  ...
  if (list && XtIsManaged((Widget)list) && _XmGeoSetupKid(boxPtr, XtParent(list))) {
    if (!listLabelBox) {
      layoutPtr->space_above = vspace;
      vspace = BB_MarginHeight(sb);
    }
...
static void SelectionBoxCallback(Widget w, XtPointer client_data, XtPointer call_data)
{
  ...
  temp.value = XmStringGenerate(text_value, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
  ...
  switch (which_button) {
    case XmDIALOG_OK_BUTTON:
      ...
      if (!match) {
        ...
        XtCallCallbackList((Widget)sel, sel->selection_box.no_match_callback, &temp);
      }
      else {
        temp.reason = XmCR_OK;
        XtCallCallbackList((Widget)sel, sel->selection_box.ok_callback, &temp);
      }
      break;
...
static void ListCallback(Widget w, XtPointer client_data, XtPointer call_data)
{
  enum { XmA_MOTIF_SELECTION_TEXT, XmA_MOTIF_COMPOUND_STRING, XmACOMPOUND_TEXT, NUM_ATOMS };

  static char *atom_names[] = {
      XmI_MOTIF_SELECTION_TEXT, XmS_MOTIF_COMPOUND_STRING, XmSCOMPOUND_TEXT};
  ...
  XmGadget dbutton = (XmGadget)BB_DynamicDefaultButton(sel);
  ...
  int format;
  Boolean success;
  textTrait = (XmAccessTextualTrait)XmeTraitGet((XtPointer)XtClass(SB_Text(sel)),
                                                XmQTaccessTextual);
  if (textTrait == NULL)
    return; /* Abort if text child is broken */
   ...
   */
  sel->selection_box.list_selected_item_position = callback->item_position;
  success = False;
  /*
   * Obtain item.
   *
  ...
  success = _XmConvertHandler(
      w, &PRIMARY, &atoms[XmA_MOTIF_SELECTION_TEXT], &type, &value, &size, &format);
  if (!success) {
    _XmConvertHandlerSetLocal();
    success = _XmConvertHandler(
        w, &PRIMARY, &atoms[XmA_MOTIF_COMPOUND_STRING], &type, &value, &size, &format);
  }
  if (success) {
   ...
   */
  if ((callback->reason == XmCR_DEFAULT_ACTION) && (callback->event->type != KeyPress) &&
      dbutton && XtIsManaged((Widget)dbutton) && XtIsSensitive((Widget)dbutton) &&
      XmIsGadget(dbutton))
  {
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  /*      Validate child placement.
   */
  if ((new_w->selection_box.child_placement != current->selection_box.child_placement) &&
      !XmRepTypeValidValue(
          XmRID_CHILD_PLACEMENT, new_w->selection_box.child_placement, (Widget)new_w))
  {
    new_w->selection_box.child_placement = current->selection_box.child_placement;
  }
  /*	Update label strings.
   */
  if (new_w->selection_box.selection_label_string != current->selection_box.selection_label_string)
  {
  ...
  if (new_w->selection_box.list_label_string != current->selection_box.list_label_string) {
    UpdateString(
        SB_ListLabel(new_w), new_w->selection_box.list_label_string, SB_StringDirection(new_w));
    new_w->selection_box.list_label_string = NULL;
  }
  if (new_w->selection_box.ok_label_string != current->selection_box.ok_label_string) {
    UpdateString(
        SB_OkButton(new_w), new_w->selection_box.ok_label_string, SB_StringDirection(new_w));
    new_w->selection_box.ok_label_string = NULL;
  }
  if (new_w->selection_box.apply_label_string != current->selection_box.apply_label_string) {
    UpdateString(
        SB_ApplyButton(new_w), new_w->selection_box.apply_label_string, SB_StringDirection(new_w));
    new_w->selection_box.apply_label_string = NULL;
  }
  if (new_w->selection_box.cancel_label_string != current->selection_box.cancel_label_string) {
    UpdateString(SB_CancelButton(new_w),
                 new_w->selection_box.cancel_label_string,
                 SB_StringDirection(new_w));
    new_w->selection_box.cancel_label_string = NULL;
  }
  if (new_w->selection_box.help_label_string != current->selection_box.help_label_string) {
    UpdateString(
        SB_HelpButton(new_w), new_w->selection_box.help_label_string, SB_StringDirection(new_w));
    new_w->selection_box.help_label_string = NULL;
  }
  /*	Update List widget.
   */
  ...
  if (new_w->selection_box.list_item_count != XmUNSPECIFIED_COUNT) {
    XtSetArg(al[ac], XmNitemCount, new_w->selection_box.list_item_count);
    ac++;
    new_w->selection_box.list_item_count = XmUNSPECIFIED_COUNT;
  }
  ...
  {
    XtSetArg(al[ac], XmNvisibleItemCount, new_w->selection_box.list_visible_item_count);
    ac++;
  }
  if (ac) {
    if (SB_List(new_w))
  ...
  if (new_w->selection_box.text_string != current->selection_box.text_string) {
    text_value = _XmStringGetTextConcat(new_w->selection_box.text_string);
    XtSetArg(al[ac], XmNvalue, text_value);
    ac++;
    new_w->selection_box.text_string = (XmString)XmUNSPECIFIED;
  }
  if (new_w->selection_box.text_columns != current->selection_box.text_columns) {
    XtSetArg(al[ac], XmNcolumns, new_w->selection_box.text_columns);
    ac++;
  }
  if (ac) {
    if (SB_Text(new_w))
  ...
  if (text_value) {
    if (SB_Text(new_w))
      XmTextFieldSetInsertionPosition(SB_Text(new_w), XmTextFieldGetLastPosition(SB_Text(new_w)));
    XtFree(text_value);
  }
  /*	Validate dialog type.
   */
  if (request->selection_box.dialog_type != current->selection_box.dialog_type) {
    XmeWarning((Widget)new_w, WARN_DIALOG_TYPE_CHANGE);
    new_w->selection_box.dialog_type = current->selection_box.dialog_type;
  }
...
/****************************************************************/
void _XmSelectionBoxGetSelectionLabelString(Widget wid,
...
/****************************************************************/
void _XmSelectionBoxGetListLabelString(Widget wid,
                                       int resource_offset, /* unused */
                                       XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxGetTextColumns(Widget wid,
                                   int resource_offset, /* unused */
                                   XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxGetTextString(Widget wid,
                                  int resource_offset, /* unused */
                                  XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
  ...
  if (SB_Text(sel)) {
    ...
    XtGetValues(SB_Text(sel), al, 1);
    text_string = XmStringGenerate(data, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
...
/****************************************************************/
void _XmSelectionBoxGetListItems(Widget wid,
                                 int resource_offset, /* unused */
                                 XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxGetListItemCount(Widget wid,
                                     int resource_offset, /* unused */
                                     XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxGetListVisibleItemCount(Widget wid,
                                            int resource_offset, /* unused */
                                            XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxGetOkLabelString(Widget wid,
                                     int resource_offset, /* unused */
                                     XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxGetApplyLabelString(Widget wid,
                                        int resource_offset, /* unused */
                                        XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxGetCancelLabelString(Widget wid,
                                         int resource_offset, /* unused */
                                         XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxGetHelpLabelString(Widget wid,
                                       int resource_offset, /* unused */
                                       XtArgVal *value)
{
  XmSelectionBoxWidget sel = (XmSelectionBoxWidget)wid;
...
/****************************************************************/
void _XmSelectionBoxUpOrDown(Widget wid,
                             XEvent *event, /* unused */
                             String *argv,
                             Cardinal *argc)
{
...
/****************************************************************/
void _XmSelectionBoxRestore(Widget wid,
                            XEvent *event,  /* unused */
                            String *argv,   /* unused */
                            Cardinal *argc) /* unused */
{
  ...
  if (list && SB_Text(sel)) {
    ac = 0;
    XtSetArg(al[ac], XmNselectedItems, &items);
    ++ac;
    XtSetArg(al[ac], XmNselectedItemCount, &count);
    ++ac;
...
Widget XmVaCreateSelectionBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmSelectionBoxWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedSelectionBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmSelectionBoxWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
Widget XmCreateSelectionDialog(Widget ds_p, String name, ArgList sb_args, Cardinal sb_n)
{
  ...
  w = XmeCreateClassDialog(xmSelectionBoxWidgetClass, ds_p, name, _sb_args, sb_n);
