...
static void SetMwmStuff(XmVendorShellExtObject ove, XmVendorShellExtObject nve);
static void AddGrab(XmVendorShellExtObject ve,
                    Widget shell,
                    Boolean exclusive,
                    Boolean springLoaded,
                    XmVendorShellExtObject origKid);
static void RemoveGrab(XmVendorShellExtObject ve, Boolean being_destroyed, Widget shell);
static void RemoveGrabCallback(Widget w, XtPointer client_data, XtPointer call_data);
static void AddToGrabList(Widget parent, Widget excludedKid, Widget origKid);
static void AddCousinsToGrabList(Widget parent, Widget excludedKid, Widget origKid);
static Boolean IsPopupShell(Widget shell);
static void PopupCallback(Widget shellParent, XtPointer closure, XtPointer callData);
static void PopdownCallback(Widget shellParent, XtPointer closure, XtPointer callData);
static Widget GetNextShell(Widget vw);
...
static void VendorExtInitialize(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void MotifWarningHandler(String name,
                                String type,
                                String s_class,
                                String message,
                                String *params,
...
static Boolean SetValuesPrehook(
    Widget old, Widget ref, Widget new_w, ArgList args, Cardinal *num_args);
...
static void SetTransientFor(Widget w, XtPointer closure, XtPointer call_data);
static void Resize(Widget w);
static void ChangeManaged(Widget wid);
static void UpdateCoreGeometry(VendorShellWidget vw, XmVendorShellExtObject vendorExt);
static void Realize(Widget wid, XtValueMask *vmask, XSetWindowAttributes *attr);
static XtGeometryResult GeometryManager(Widget wid,
                                        XtWidgetGeometry *request,
                                        XtWidgetGeometry *reply);
static XtGeometryResult RootGeometryManager(Widget wid,
                                            XtWidgetGeometry *request,
                                            XtWidgetGeometry *reply);
static void SetMwmHints(XmVendorShellExtObject ve);
static void SetMwmMenu(XmVendorShellExtObject ve);
static void VendorExtRealize(Widget w, XtPointer closure, XtPointer call_data);
static void AddDLEntry(XmVendorShellExtObject ve, Widget shell);
static void RemoveDLEntry(unsigned pos);
...
#ifdef DEBUG_GRABS
static void PrintModal(XmModalData modal);
static void PrintXmGrabs(Widget wid);
#endif /* DEBUG_GRABS */
...
__attribute__((constructor)) static void __VendorShellHack(void)
{
...
static XtResource resources[] = {
    ...
    },
    {
        ...
        sizeof(Dimension),
        XtOffsetOf(WidgetRec, core.width),
        XmRImmediate,
        (XtPointer)0,
    },
    {
        ...
        sizeof(Dimension),
        XtOffsetOf(WidgetRec, core.height),
        XmRImmediate,
        (XtPointer)0,
    },
    {
        ...
        XmRHorizontalDimension,
        sizeof(Dimension),
        XtOffsetOf(WidgetRec, core.border_width),
        XmRImmediate,
        (XtPointer)0,
    },
    ...
    {XmNbaseWidth,
     ...
     XmRHorizontalInt,
     (XtPointer)&default_unspecified_shell_int},
    {XmNbaseHeight,
     XmCBaseHeight,
     XmRVerticalInt,
     sizeof(int),
    ...
    },
    {
        ...
        XmRImmediate,
        (XtPointer)XtUnspecifiedWindowGroup,
    },
...
static XtResource subresources[] = {
    {
        ...
        XmRInt,
        sizeof(unsigned int),
        XtOffsetOf(XmToolTipConfigTraitRec, post_delay),
        XmRImmediate,
        (XtPointer)5000,
    },
    {
        ...
        XmRInt,
        sizeof(unsigned int),
        XtOffsetOf(XmToolTipConfigTraitRec, post_duration),
        XmRImmediate,
        (XtPointer)5000,
    },
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffsetOf(XmToolTipConfigTraitRec, enable),
        XmRImmediate,
        (XtPointer)False,
    },
};
...
static XmConst XmSpecifyLayoutDirectionTraitRec vsLDT = {0, /* version */
                                                         GetDirection};
/* Access Colors Trait record for vendor shell */
static XmConst XmAccessColorsTraitRec vsACT = {0, /* version */
                                               GetColors};
/* Unit Type Trait record for VendorShell */
static XmConst XmSpecUnitTypeTraitRec vsUTT = {0, /* version */
                                               GetUnitType};
 ...
 ************************************************************************/
/*ARGSUSED*/
static XtPointer BaseProc(Widget widget, XtPointer client_data) /* unused */
{
...
static Cardinal GetSecResData(WidgetClass w_class, XmSecondaryResourceData **secResDataRtn)
{
  ...
  _XmProcessLock();
  if ((bcePtr = _XmGetBaseClassExtPtr(w_class, XmQmotif)) && (bcePtr) && (*bcePtr) &&
      ((*bcePtr)->secondaryObjectClass))
  {
    ...
    arrayCount = _XmSecondaryResourceData(
        *bcePtr, secResDataRtn, client_data, resource_name, resource_class, BaseProc);
  }
  _XmProcessUnlock();
  return arrayCount;
}
...
****************************************************************************/
  ...
  if (((XmShellExtObjectClass)baseClassExtRec.secondaryObjectClass)
  ...
  if (((XmVendorShellExtObjectClass)baseClassExtRec.secondaryObjectClass)
          ->vendor_class.offset_handler == XmInheritProtocolHandler)
    ((XmVendorShellExtObjectClass)baseClassExtRec.secondaryObjectClass)
        ->vendor_class.offset_handler =
...
static void ClassPartInitialize(WidgetClass wc)
{
      ...
      ((*scePtr)->secondaryObjectClass != (*wcePtr)->secondaryObjectClass))
  {
    ...
    if (wceClass->desktop_class.insert_child == XtInheritInsertChild)
      wceClass->desktop_class.insert_child = sceClass->desktop_class.insert_child;
    if (wceClass->desktop_class.delete_child == XtInheritDeleteChild)
      wceClass->desktop_class.delete_child = sceClass->desktop_class.delete_child;
    if (wceClass->shell_class.structureNotifyHandler == XmInheritEventHandler)
      wceClass->shell_class.structureNotifyHandler = sceClass->shell_class.structureNotifyHandler;
    if (wceClass->vendor_class.offset_handler == XmInheritProtocolHandler)
      wceClass->vendor_class.offset_handler = sceClass->vendor_class.offset_handler;
  }
  compExtPtr = (CompositeClassExtensionRec **)&(compWc->composite_class.extension);
  _XmProcessUnlock();
  compExtPtr = (CompositeClassExtensionRec **)_XmGetClassExtensionPtr(
      (XmGenericClassExt *)compExtPtr, NULLQUARK);
  if (*compExtPtr == NULL) {
    CompositeClassExtensionRec **superExtPtr;
    _XmProcessLock();
    superExtPtr = (CompositeClassExtensionRec **)&(superWc->composite_class.extension);
    _XmProcessUnlock();
    superExtPtr = (CompositeClassExtensionRec **)_XmGetClassExtensionPtr(
        (XmGenericClassExt *)superExtPtr, NULLQUARK);
                    ...
                    XmVendorShellExtObject origKid)
{
  ...
  XtAddGrab((Widget)shell, exclusive, springLoaded);
#ifdef DEBUG_GRABS
...
static void RemoveGrab(XmVendorShellExtObject ve, Boolean being_destroyed, Widget shell)
{
  ...
  if (!being_destroyed) {
    ...
    XtRemoveCallback(shell, XmNdestroyCallback, RemoveGrabCallback, (XtPointer)ve);
  }
#ifdef DEBUG_GRABS
  ...
  for (i = 0, numRemoves = 0; i < numModals; i++) {
   ...
   */
  for (i = 0, incr = 0; (i + numRemoves) < numModals; i++) {
     ...
     */
    for (/*EMPTY*/; ((i + incr) < numModals); incr++) {
    ...
    if (incr && ((i + incr) < numModals)) {
      ...
      if (!(modals[i].wid->core.being_destroyed)) {
        XtAddGrab(modals[i].wid, modals[i].exclusive, modals[i].springLoaded);
#ifdef DEBUG_GRABS
  ...
  for (i = 0, currKid = children; i < numChildren; currKid++, i++) {
    if (*currKid != excludedKid) {
      if (!XmIsDisplay(parent)) {
        ShellWidget shell;
        shell = (ShellWidget)((XmDesktopObject)(*currKid))->ext.logicalParent;
        if (shell->shell.popped_up ||
            (XtIsRealized((Widget)shell) && !IsPopupShell((Widget)shell)))
          AddGrab((XmVendorShellExtObject)*currKid,
                  NULL,
                  (Boolean)False,
                  (Boolean)False,
                  (XmVendorShellExtObject)origKid);
 ...
 ************************************************************************/
/*ARGSUSED*/
static void PopupCallback(Widget shellParent, XtPointer closure, XtPointer callData) /* unused */
{
  ...
  if (grab_kind != XtGrabNone)
    AddGrab(ve, NULL, (grab_kind == XtGrabExclusive), False, ve);
  ve->vendor.grab_kind = grab_kind;
  if (grabCousins)
    AddCousinsToGrabList((Widget)ve->desktop.parent, (Widget)ve, (Widget)ve);
#ifdef DEBUG_GRABS
  printf("\n**** After popup: ****\n");
 ...
 ************************************************************************/
/*ARGSUSED*/
static void PopdownCallback(Widget shellParent, /* unused */
                            XtPointer closure,
                                             ...
                                             Cardinal *num_args) /* unused */
{
  ...
  if (vw->wm.transient) {
    if (XtIsSubclass((Widget)vw, transientShellWidgetClass)) {
      ...
      if (!(transientParent = tw->transient.transient_for)) {
        tw->transient.transient_for = transientParent = GetNextShell((Widget)vw);
      }
    }
    else {
  ...
  if (transientParent) {
    ...
    desktopParent = (XmDesktopObject)XmGetXmScreen(XtScreen((Widget)vw));
  }
  return desktopParent;
}
...
static void SecondaryObjectCreate(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (desktopParent) {
    ...
    _XmPushWidgetExtData(new_w, extData, ((XmVendorShellExtObject)newSec)->ext.extensionType);
    /*
 ...
 ************************************************************************/
/*ARGSUSED*/
static void VendorExtInitialize(Widget req,
                                Widget new_w,
                                ArgList args,       /* unused */
                                Cardinal *num_args) /* unused */
{
  enum { XmA_MOTIF_WM_OFFSET, XmA_MOTIF_WM_MESSAGES, XmAWM_DELETE_WINDOW, NUM_ATOMS };

  static char *atom_names[] = {_XA_MOTIF_WM_OFFSET, _XA_MOTIF_WM_MESSAGES, XmIWM_DELETE_WINDOW};
  ...
  if (handler) {
    XtInsertEventHandler(
        extParent, (EventMask)StructureNotifyMask, TRUE, handler, (XtPointer)new_w, XtListHead);
  }
#ifdef DEBUG
  else
    XtError("No structure notify handler for shell");
#endif /* DEBUG */
  ve->vendor.lastOffsetSerial = ve->vendor.lastMapRequest = 0;
  ve->vendor.xAtMap = ve->vendor.yAtMap = ve->vendor.xOffset = ve->vendor.yOffset = 0;
  _XmAddCallback((InternalCallbackList *)&(ve->vendor.realize_callback), VendorExtRealize, NULL);
  ...
  XInternAtoms(XtDisplay(extParent), atom_names, XtNumber(atom_names), FALSE, atoms);
  XmAddWMProtocols(extParent, &atoms[XmA_MOTIF_WM_MESSAGES], 1);
  XmAddProtocols(extParent, atoms[XmA_MOTIF_WM_MESSAGES], &atoms[XmA_MOTIF_WM_OFFSET], 1);
  _XmProcessLock();
  ...
  XmSetWMProtocolHooks(
      extParent, atoms[XmAWM_DELETE_WINDOW], NULL, NULL, delete_window_handler, (XtPointer)ve);
  ...
  if ((ve->vendor.focus_policy != XmEXPLICIT) && (ve->vendor.focus_policy != XmPOINTER)) {
    ve->vendor.focus_policy = XmEXPLICIT;
  }
  /* initialize input manager resources */
  ve->vendor.input_method_string = XtNewString(req_ve->vendor.input_method_string);
  ve->vendor.preedit_type_string = XtNewString(req_ve->vendor.preedit_type_string);
...
static void Initialize(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if ((extData = _XmGetWidgetExtData((Widget)vw, XmSHELL_EXTENSION)) != NULL) {
    VendorExtInitialize(extData->reqWidget, extData->widget, args, num_args);
  }
  ...
  if (!previousWarningHandler)
    previousWarningHandler = XtAppSetWarningMsgHandler(XtWidgetToApplicationContext(new_w),
                                                       MotifWarningHandler);
  _XmProcessUnlock();
}
    ...
    Widget old, Widget ref, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if ((nve->focus_policy != XmEXPLICIT) && (nve->focus_policy != XmPOINTER)) {
    nve->focus_policy = ove->focus_policy;
  }
  if (nve->focus_policy != ove->focus_policy) {
    _XmFocusModelChanged(nv->ext.logicalParent, NULL, (XtPointer)(unsigned long)nve->focus_policy);
  }
  SetMwmStuff(ov, nv);
  ...
  if (nve->button_font_list != ove->button_font_list) {
    ...
    if (!defaultFont) {
      ...
      if (!defaultFont)
        defaultFont = XmeGetDefaultRenderTable((Widget)new_w, XmBUTTON_FONTLIST);
    }
  ...
  if (nve->label_font_list != ove->label_font_list) {
    ...
    if (!defaultFont) {
      defaultFont = nve->default_font_list;
      if (!defaultFont)
        defaultFont = XmeGetDefaultRenderTable((Widget)new_w, XmLABEL_FONTLIST);
    }
  ...
  if (nve->text_font_list != ove->text_font_list) {
    ...
    if (!defaultFont) {
      defaultFont = nve->default_font_list;
      if (!defaultFont)
        defaultFont = XmeGetDefaultRenderTable((Widget)new_w, XmTEXT_FONTLIST);
    }
    ...
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  {
    VendorExtSetValues(extData->oldWidget, extData->reqWidget, extData->widget, args, num_args);
    vse = (XmVendorShellExtObject)extData->widget;
 ...
 ************************************************************************/
/*ARGSUSED*/
static Boolean SetValuesPosthook(Widget current, /* unused */
                                 Widget req,     /* unused */
                                 Widget new_w,
                                 ArgList args,       /* unused */
                                 Cardinal *num_args) /* unused */
 ...
 ************************************************************************/
/*ARGSUSED*/
static void GetValuesPrehook(Widget w,
                             ArgList args,       /* unused */
                             Cardinal *num_args) /* unused */
{
  XmWidgetExtData oldExtData, newExtData;
...
static void GetValuesHook(Widget w, ArgList args, Cardinal *num_args)
{
  ...
  if ((ext = _XmGetWidgetExtData(w, XmSHELL_EXTENSION)) != NULL) {
    _XmProcessLock();
    XtGetSubvalues(
        ext->widget, ec->core_class.resources, ec->core_class.num_resources, args, *num_args);
    _XmProcessUnlock();
...
/*ARGSUSED*/
static void GetValuesPosthook(Widget w,
                              ArgList args,       /* unused */
                              Cardinal *num_args) /* unused */
{
 ...
 */
/*ARGSUSED*/
static void PendingTransientDestroyed(Widget vw, XtPointer cl_data, XtPointer ca_data) /* unused */
{
...
*/
    _XmRemoveCallback((InternalCallbackList *)&(
                          ((XmVendorShellExtObject)ancestorExtObj)->vendor.realize_callback),
                      SetTransientFor,
                      (XtPointer)vw);
  }
  return;
 ...
 */
/*ARGSUSED*/
static void SetTransientFor(Widget w, XtPointer closure, XtPointer call_data) /* unused */
{
  ...
  XtSetArg(args[i], XtNwindowGroup, XtWindow(ancestor));
  i++;
  if (XtIsTransientShell((Widget)vw)) {
    ...
    XtSetArg(args[i], XtNtransientFor, ancestor);
    i++;
  }
...
*/
  _XmRemoveCallback(
      (InternalCallbackList *)&(((XmVendorShellExtObject)w)->vendor.realize_callback),
      SetTransientFor,
      (XtPointer)vw);
  XtRemoveCallback((Widget)vw, XmNdestroyCallback, PendingTransientDestroyed, (XtPointer)w);
  return;
}
    ...
    if (vendorExt->vendor.unit_type != XmPIXELS) {
      if (vw->wm.size_hints.flags & USPosition) {
        vw->core.x = (Position)XmCvtToHorizontalPixels(
            vw->core.screen, (int)vw->core.x, vendorExt->vendor.unit_type);
        vw->core.y = (Position)XmCvtToVerticalPixels(
            vw->core.screen, (int)vw->core.y, vendorExt->vendor.unit_type);
      }
      if (vw->wm.size_hints.flags & USSize) {
        vw->core.width = (Dimension)XmCvtToHorizontalPixels(
            vw->core.screen, (int)vw->core.width, vendorExt->vendor.unit_type);
        vw->core.height = (Dimension)XmCvtToVerticalPixels(
            vw->core.screen, (int)vw->core.height, vendorExt->vendor.unit_type);
      }
    }
  }
}
...
static void Realize(Widget wid, XtValueMask *vmask, XSetWindowAttributes *attr)
{
  ...
  if (vendorExt && XmIsShellExt(vendorExt->desktop.parent)) {
    Widget ancestor = ((XmExtObject)(vendorExt->desktop.parent))->ext.logicalParent;
    /* try to have WMShell do the work */
    if (XtIsRealized(ancestor))
      vw->wm.wm_hints.window_group = XtWindow(ancestor);
    else {
      XmWidgetExtData ancestorExtData = _XmGetWidgetExtData(ancestor, XmSHELL_EXTENSION);
      if (ancestorExtData && ancestorExtData->widget) {
        _XmAddCallback(
            (InternalCallbackList *)&(
                ((XmVendorShellExtObject)ancestorExtData->widget)->vendor.realize_callback),
            SetTransientFor,
            (XtPointer)vw);
                                        ...
                                        XtWidgetGeometry *reply) /* unused */
{
  ...
  if (res == XtGeometryYes) {
    ...
    if (!(request->request_mode & XtCWQueryOnly)) {
      ...
      if (request->request_mode & CWHeight)
        wid->core.height = shell->core.height - ve->vendor.im_height;
      if (request->request_mode & CWX)
        wid->core.x = 0;
      if (request->request_mode & CWY)
        wid->core.y = 0;
 ...
 ************************************************************************/
/*ARGSUSED*/
static XtGeometryResult RootGeometryManager(Widget w,
                                            XtWidgetGeometry *request,
                                            XtWidgetGeometry *reply)
{
...
#endif /* DEBUG */
  _XmProcessLock();
  scExtPtr = (ShellClassExtensionRec **)_XmGetClassExtensionPtr(
      (XmGenericClassExt *)&(swc->shell_class.extension), NULLQUARK);
  _XmProcessUnlock();
  if (request->request_mode & XtCWQueryOnly) {
...
static void SetMwmMenu(XmVendorShellExtObject ve)
{
  ...
  int status;
  mwm_menu_atom = XInternAtom(XtDisplay(shell), _XA_MWM_MENU, FALSE);
  text_prop.value = NULL;
  status = XmbTextListToTextProperty(
      XtDisplay(shell), &ve->vendor.mwm_menu, 1, XStdICCTextStyle, &text_prop);
  if (status == Success || status > 0) {
    XSetTextProperty(XtDisplay(shell), XtWindow(shell), &text_prop, mwm_menu_atom);
...
/*ARGSUSED*/
static void VendorExtRealize(Widget w,
...
static void Destroy(Widget wid)
{
  ...
  if (ext != NULL) {
    if ((ve = (XmVendorShellExtObject)ext->widget) != NULL) {
      ...
      _XmRemoveAllCallbacks((InternalCallbackList *)&(ve->vendor.realize_callback));
      _XmRemoveAllCallbacks((InternalCallbackList *)&(ve->vendor.focus_moved_callback));
      {
...
static void GetColors(Widget w, XmAccessColorData color_data)
{
  ...
  color_data->highlight_color = color_data->top_shadow_color = color_data->bottom_shadow_color =
      XmUNSPECIFIED_PIXEL;
}
 ...
 *
 ************************************************************************/
void _XmDefaultVisualResources(Widget widget)
{
...
static void PrintModal(XmModalData modal)
{
  printf("log_p: %s, grabber: %s, excl: %d, spring: %d, ve: 0x%x\n",
         modal->wid->core.name,
         (modal->grabber ? modal->grabber->ext.logicalParent->core.name : "NULL"),
         modal->exclusive,
         modal->springLoaded,
         modal->ve);
}
