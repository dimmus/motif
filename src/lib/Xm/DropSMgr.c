...
static void DropSiteManagerInitialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
...
static void DetectAllClippers(XmDropSiteManagerObject dsm, XmDSInfo parentInfo);
static Boolean InsertClipper(XmDropSiteManagerObject dsm, XmDSInfo parentInfo, XmDSInfo clipper);
static void DetectAndInsertAllClippers(XmDropSiteManagerObject dsm, XmDSInfo root);
static void RemoveClipper(XmDropSiteManagerObject dsm, XmDSInfo clipper);
static void RemoveAllClippers(XmDropSiteManagerObject dsm, XmDSInfo parentInfo);
static void DestroyDSInfo(XmDSInfo info, Boolean substructures);
static XmDSInfo CreateShellDSInfo(XmDropSiteManagerObject dsm, Widget widget);
static XmDSInfo CreateClipperDSInfo(XmDropSiteManagerObject dsm, Widget clipW);
static void InsertInfo(XmDropSiteManagerObject dsm, XtPointer info, XtPointer call_data);
static void RemoveInfo(XmDropSiteManagerObject dsm, XtPointer info);
static Boolean IntersectWithWidgetAncestors(Widget w, XmRegion r);
...
static Boolean PointInDS(XmDropSiteManagerObject dsm, XmDSInfo info, Position x, Position y);
static XmDSInfo PointToDSInfo(XmDropSiteManagerObject dsm, XmDSInfo info, Position x, Position y);
static void DoAnimation(XmDropSiteManagerObject dsm,
...
static void HandleEnter(XmDropSiteManagerObject dsm,
                        XmDragMotionClientData motionData,
                        XmDragMotionCallbackStruct *callback,
                        XmDSInfo info,
                        unsigned char style);
static void HandleMotion(XmDropSiteManagerObject dsm,
                         XmDragMotionClientData motionData,
                         XmDragMotionCallbackStruct *callback,
                         XmDSInfo info,
                         unsigned char style);
static void HandleLeave(XmDropSiteManagerObject dsm,
                        XmDragMotionClientData motionData,
                        XmDragMotionCallbackStruct *callback,
                        XmDSInfo info,
                        unsigned char style,
                        Boolean enterPending);
static void ProcessMotion(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer calldata);
static void ProcessDrop(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer cb);
static void ChangeOperation(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer calldata);
static void PutDSToStream(XmDropSiteManagerObject dsm,
                          XmDSInfo dsInfo,
                          Boolean last,
                          XtPointer dataPtr);
static void GetDSFromDSM(XmDropSiteManagerObject dsm,
                         ...
                         Boolean last,
                         XtPointer dataPtr);
static int GetTreeFromDSM(XmDropSiteManagerObject dsm, Widget shell, XtPointer dataPtr);
static XmDSInfo GetDSFromStream(XmDropSiteManagerObject dsm,
                                XtPointer dataPtr,
                                Boolean *close,
                                unsigned char *type);
...
static int CountDropSites(XmDSInfo info);
static void CreateInfo(XmDropSiteManagerObject dsm,
                       Widget widget,
                       ArgList args,
                       Cardinal argCount);
static void CopyVariantIntoFull(XmDropSiteManagerObject dsm,
                                XmDSInfo variant,
                                XmDSFullInfo full_info);
static void RetrieveInfo(XmDropSiteManagerObject dsm,
                         Widget widget,
                         ArgList args,
                         Cardinal argCount);
static void CopyFullIntoVariant(XmDSFullInfo full_info, XmDSInfo variant);
static void UpdateInfo(XmDropSiteManagerObject dsm,
                       Widget widget,
                       ArgList args,
                       Cardinal argCount);
static void StartUpdate(XmDropSiteManagerObject dsm, Widget refWidget);
static void EndUpdate(XmDropSiteManagerObject dsm, Widget refWidget);
static void DestroyInfo(XmDropSiteManagerObject dsm, Widget widget);
static void SyncDropSiteGeometry(XmDropSiteManagerObject dsm, XmDSInfo info);
static void SyncTree(XmDropSiteManagerObject dsm, Widget shell);
static void Update(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer callData);
static Boolean HasDropSiteDescendant(XmDropSiteManagerObject dsm, Widget widget);
static void DestroyCallback(Widget widget, XtPointer client_data, XtPointer call_data);
...
externaldef(xmdropsitemanagerclassrec) XmDropSiteManagerClassRec xmDropSiteManagerClassRec = {
    {
...
externaldef(xmdropsitemanagerobjectclass)
...
static Boolean CompareWidgets(XtPointer w1, XtPointer w2)
{
  return (w1 == w2);
}

static XmHashValue HashWidget(XtPointer w1)
{
...
static Boolean Coincident(XmDropSiteManagerObject dsm, Widget w, XmDSClipRect *r)
{
  ...
  if (!XtIsShell(w)) {
    ...
    XtTranslateCoords(XtParent(w), XtX(w), XtY(w), &(wR.x), &(wR.y));
  ...
  if (!(r->detected & XmDROP_SITE_RIGHT_EDGE) && ((r->x + r->width) == (wR.x + wR.width))) {
  ...
  if (!(r->detected & XmDROP_SITE_BOTTOM_EDGE) && ((r->y + r->height) == (wR.y + wR.height))) {
...
static void DetectImpliedClipper(XmDropSiteManagerObject dsm, XmDSInfo info)
{
  ...
  if ((GetDSType(info) == XmDROP_SITE_SIMPLE) && GetDSHasRegion(info)) {
    ...
    if ((rr.x != tr.x) || (rr.y != tr.y) || (rr.width != tr.width) || (rr.height != tr.height)) {
      XmDSInfo clipper;
      /*
...
static void DetectAllClippers(XmDropSiteManagerObject dsm, XmDSInfo parentInfo)
{
  ...
  for (i = 0; i < (int)GetDSNumChildren(parentInfo); i++) {
     ...
     */
    _XmRegionGetExtents(GetDSRegion(childInfo), (XRectangle *)(&extents));
    _XmProcessLock();
    _XmRegionUnion(GetDSRegion(childInfo), GetDSRegion(childInfo), tmpR);
    ...
    if ((clippedExtents.width < extents.width) || (clippedExtents.height < extents.height)) {
      /*
       * We've been clipped.  Find out who did it and add
       * them to the tree.
       */
      DetectAncestorClippers(dsm, XtParent(GetDSWidget(childInfo)), &clippedExtents, childInfo);
    }
    /*
...
static Boolean InsertClipper(XmDropSiteManagerObject dsm, XmDSInfo parentInfo, XmDSInfo clipper)
{
  ...
  if (IsDescendent(GetDSWidget(parentInfo), GetDSWidget(clipper))) {
    ...
    while (i < (int)GetDSNumChildren(parentInfo)) {
      childInfo = (XmDSInfo)GetDSChild(parentInfo, i);
      if (IsDescendent(GetDSWidget(clipper), GetDSWidget(childInfo))) {
        RemoveDSChild(parentInfo, childInfo);
        AddDSChild(clipper, childInfo, GetDSNumChildren(clipper));
      }
      else
        /*
  ...
  if (!GetDSLeaf(parentInfo)) {
    ...
    while (i < (int)GetDSNumChildren(parentInfo)) {
      ...
      /* Only increment i if the current child wasn't
           ...
           unintentionally */
      if (child == (XmDSInfo)GetDSChild(parentInfo, i))
        i++;
    }
  }
}
...
static void InsertInfo(XmDropSiteManagerObject dsm, XtPointer info, XtPointer call_data)
{
  ...
  while (!(parentInfo = (XmDSInfo)DSMWidgetToInfo(dsm, parent)) && !XtIsShell(parent)) {
  ...
  if (parentInfo == NULL) {
    ...
    if ((dsm->dropManager.treeUpdateProc) &&
        (!XtIsRealized(parent) || (_XmGetDragProtocolStyle(parent) == XmDRAG_DYNAMIC)))
    {
      /*
      ...
      (dsm->dropManager.treeUpdateProc)((Widget)dsm, NULL, (XtPointer)&outCB);
    }
  }
...
static void RemoveInfo(XmDropSiteManagerObject dsm, XtPointer info)
{
  ...
  if ((parentInfo != NULL) && (GetDSNumChildren(parentInfo) == 0) && (GetDSInternal(parentInfo))) {
...
static Boolean CalculateAncestorClip(XmDropSiteManagerObject dsm, XmDSInfo info, XmRegion r)
{
  ...
  if (GetDSRemote(info)) {
     ...
     * is left in r.
     */
    return (IntersectWithDSInfoAncestors((XmDSInfo)GetDSParent(info), r));
  }
  else {
    XRectangle parentR;
...
static XmDSInfo PointToDSInfo(XmDropSiteManagerObject dsm, XmDSInfo info, Position x, Position y)
{
  ...
  if (!GetDSLeaf(info)) {
    ...
    for (i = 0; i < GetDSNumChildren(info); i++) {
      ...
      if (managed && PointInDS(dsm, child, x, y) && GetDSActivity(child) != XmDROP_SITE_INACTIVE) {
        if (!GetDSLeaf(child)) {
          XmDSInfo descendant = PointToDSInfo(dsm, child, x, y);
          if (descendant != NULL)
            return (descendant);
        }
        if (!GetDSInternal(child))
                        ...
                        XtPointer callback)
{
  ...
  if (sourceIsExternal) {
     ...
     */
    animationData.window = XtWindow(GetDSWidget(dsm->dropManager.dsRoot));
    animationData.screen = XtScreen(GetDSWidget(dsm->dropManager.dsRoot));
  }
  else {
                          ...
                          XmDragProcCallbackStruct *callback)
{
  ...
  n = 0;
  XtSetArg(args[n], XmNexportTargets, &export_targets);
  n++;
  XtSetArg(args[n], XmNnumExportTargets, &num_export);
  n++;
  ...
  num_import = _XmIndexToTargets(shell, GetDSImportTargetsID(info), &import_targets);
  if ((callback->operation != XmDROP_NOOP) &&
      (XmTargetsAreCompatible(
          XtDisplay(dsm), export_targets, num_export, import_targets, num_import)))
                        ...
                        unsigned char style)
{
  ...
  if ((!GetDSRemote(info)) && (GetDSDragProc(info) != NULL)) {
    ...
    cbRec.x -= tmpX;
    cbRec.y -= tmpY;
    (*(GetDSDragProc(info)))(widget, NULL, (XtPointer)&cbRec);
  }
  if ((cbRec.animate) && (cbRec.dropSiteStatus == XmVALID_DROP_SITE))
  ...
  if (dsm->dropManager.notifyProc) {
    ...
    (*(dsm->dropManager.notifyProc))((Widget)dsm, dsm->dropManager.client_data, (XtPointer)&outCB);
  }
}
                         ...
                         unsigned char style)
{
  ...
  if (info != NULL) {
    ...
    if ((cbRec.animate) && (cbRec.dropSiteStatus != dsm->dropManager.curDropSiteStatus)) {
...
static void ProcessMotion(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer calldata)
{
  XmDragMotionCallbackStruct *callback = (XmDragMotionCallbackStruct *)calldata;
  XmDragMotionClientData motionData = (XmDragMotionClientData)clientData;
  ...
  if (dsRoot) {
    ...
    if (curDSInfo != newDSInfo) {
      if (curDSInfo) {
         ...
         */
        if (newDSInfo)
          HandleLeave(dsm, motionData, callback, curDSInfo, style, True);
        else
          HandleLeave(dsm, motionData, callback, curDSInfo, style, False);
      }
      ...
      if (newDSInfo)
        HandleEnter(dsm, motionData, callback, newDSInfo, style);
      return;
    }
  }
...
static void ProcessDrop(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer cb)
{
  ...
  XmDropStartCallbackStruct *callback = (XmDropStartCallbackStruct *)cb;
  XmDropFinishCallbackStruct *finish = (XmDropFinishCallbackStruct *)cb;
  XmDropProcCallbackStruct cbRec;
  Widget dragContext = XmGetDragContext((Widget)dsm, callback->timeStamp);
  XmDSInfo info = NULL;
  Widget widget = NULL;
  ...
  if ((info == NULL) || !XtIsManaged(widget) || /* CR 5215 */
      /* These are wierd conditions */
      (newRoot == NULL) || (GetDSRemote(info)) || (GetDSDropProc(info) == NULL))
  {
    /* we will do a failure drop transfer */
    Arg args[4];
    Cardinal i = 0;
    XtSetArg(args[i], XmNtransferStatus, XmTRANSFER_FAILURE);
    i++;
    XtSetArg(args[i], XmNnumDropTransfers, 0);
    i++;
  ...
  if (dsm->dropManager.notifyProc) {
    (*(dsm->dropManager.notifyProc))(
        (Widget)dsm, dsm->dropManager.client_data, (XtPointer)callback);
  }
...
static void ChangeOperation(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer calldata)
{
  XmOperationChangedCallbackStruct *callback = (XmOperationChangedCallbackStruct *)calldata;
  XmDragMotionClientData motionData = (XmDragMotionClientData)clientData;
  XmDragProcCallbackStruct cbRec;
  XmDSInfo info = (XmDSInfo)dsm->dropManager.curInfo;
  unsigned char style;
  ...
  if (info != NULL) {
    ProxyDragProc(dsm, NULL, &cbRec);
    if ((style == XmDRAG_DYNAMIC) && (!GetDSRemote(info)) && (GetDSDragProc(info) != NULL)) {
      Widget widget = GetDSWidget(info);
      Position tmpX, tmpY;
      /* Make the coordinates widget relative */
      XtTranslateCoords(widget, 0, 0, &tmpX, &tmpY);
  ...
  if (XmIsPrimitive(w)) {
    ...
    switch (iccInfo.header.animationStyle) {
      case XmDRAG_UNDER_HIGHLIGHT: {
        XmICCDropSiteHighlight info = (XmICCDropSiteHighlight)(&iccInfo);
        info->animation_data.highlightPixmap = XmUNSPECIFIED_PIXMAP;
        if (!GetDSHasRegion(dsInfo))
          info->animation_data.borderWidth = pw->core.border_width;
        info->animation_data.highlightThickness = pw->primitive.highlight_thickness;
        info->animation_data.highlightColor = pw->primitive.highlight_color;
        info->animation_data.highlightPixmap = pw->primitive.highlight_pixmap;
        info->animation_data.background = pw->core.background_pixel;
      } break;
      case XmDRAG_UNDER_SHADOW_IN:
      case XmDRAG_UNDER_SHADOW_OUT: {
        XmICCDropSiteShadow info = (XmICCDropSiteShadow)(&iccInfo);
        info->animation_data.topShadowPixmap = info->animation_data.bottomShadowPixmap =
            XmUNSPECIFIED_PIXMAP;
        if (!GetDSHasRegion(dsInfo))
          info->animation_data.borderWidth = pw->core.border_width;
        info->animation_data.highlightThickness = pw->primitive.highlight_thickness;
        info->animation_data.shadowThickness = pw->primitive.shadow_thickness;
        info->animation_data.foreground = pw->primitive.foreground;
        info->animation_data.topShadowColor = pw->primitive.top_shadow_color;
        info->animation_data.topShadowPixmap = pw->primitive.top_shadow_pixmap;
        info->animation_data.bottomShadowColor = pw->primitive.bottom_shadow_color;
        info->animation_data.bottomShadowPixmap = pw->primitive.bottom_shadow_pixmap;
      } break;
      case XmDRAG_UNDER_PIXMAP: {
        XmICCDropSitePixmap info = (XmICCDropSitePixmap)(&iccInfo);
        XmDSLocalPixmapStyle ps = (XmDSLocalPixmapStyle)GetDSLocalAnimationPart(dsInfo);
        info->animation_data.animationPixmapDepth = ps->animation_pixmap_depth;
        info->animation_data.animationPixmap = ps->animation_pixmap;
      ...
      case XmDRAG_UNDER_NONE: {
        XmICCDropSiteNone info = (XmICCDropSiteNone)(&iccInfo);
        if (!GetDSHasRegion(dsInfo))
          info->animation_data.borderWidth = pw->core.border_width;
        else
    ...
    switch (iccInfo.header.animationStyle) {
      case XmDRAG_UNDER_HIGHLIGHT: {
        ...
        if (!GetDSHasRegion(dsInfo)) {
          if (is_gadget)
            info->animation_data.borderWidth = w->core.border_width;
          else
        ...
        info->animation_data.highlightColor = mw->manager.highlight_color;
        info->animation_data.highlightPixmap = mw->manager.highlight_pixmap;
        info->animation_data.background = mw->core.background_pixel;
      } break;
      case XmDRAG_UNDER_SHADOW_IN:
      case XmDRAG_UNDER_SHADOW_OUT: {
        XmICCDropSiteShadow info = (XmICCDropSiteShadow)(&iccInfo);
        ...
        if (is_gadget) {
          if (!GetDSHasRegion(dsInfo))
            info->animation_data.borderWidth = w->core.border_width;
          info->animation_data.shadowThickness = g->gadget.shadow_thickness;
        }
        else {
          if (!GetDSHasRegion(dsInfo))
            info->animation_data.borderWidth = mw->core.border_width;
          info->animation_data.shadowThickness = mw->manager.shadow_thickness;
        }
        info->animation_data.highlightThickness = 0;
        info->animation_data.foreground = mw->manager.foreground;
        info->animation_data.topShadowColor = mw->manager.top_shadow_color;
        info->animation_data.topShadowPixmap = mw->manager.top_shadow_pixmap;
        info->animation_data.bottomShadowColor = mw->manager.bottom_shadow_color;
        info->animation_data.bottomShadowPixmap = mw->manager.bottom_shadow_pixmap;
      } break;
      case XmDRAG_UNDER_PIXMAP: {
        XmICCDropSitePixmap info = (XmICCDropSitePixmap)(&iccInfo);
        XmDSLocalPixmapStyle ps = (XmDSLocalPixmapStyle)GetDSLocalAnimationPart(dsInfo);
    ...
    switch (iccInfo.header.animationStyle) {
      case XmDRAG_UNDER_HIGHLIGHT: {
        ...
        n = 0;
        if (!GetDSHasRegion(dsInfo)) {
          XtSetArg(args[n], XmNborderWidth, &(info->animation_data.borderWidth));
          n++;
        }
        XtSetArg(args[n], XmNhighlightThickness, &(info->animation_data.highlightThickness));
        n++;
        XtSetArg(args[n], XmNbackground, &(info->animation_data.background));
        n++;
        XtSetArg(args[n], XmNhighlightColor, &(info->animation_data.highlightColor));
        n++;
        XtSetArg(args[n], XmNhighlightPixmap, &(info->animation_data.highlightPixmap));
        n++;
        XtGetValues(w, args, n);
      } break;
      case XmDRAG_UNDER_SHADOW_IN:
      case XmDRAG_UNDER_SHADOW_OUT: {
        ...
        XtSetArg(args[n], XmNhighlightThickness, &(info->animation_data.highlightThickness));
        n++;
        XtSetArg(args[n], XmNshadowThickness, &(info->animation_data.shadowThickness));
        n++;
        XtSetArg(args[n], XmNforeground, &(info->animation_data.foreground));
        n++;
        XtSetArg(args[n], XmNtopShadowColor, &(info->animation_data.topShadowColor));
        n++;
        XtSetArg(args[n], XmNbottomShadowColor, &(info->animation_data.bottomShadowColor));
        n++;
        XtSetArg(args[n], XmNtopShadowPixmap, &(info->animation_data.topShadowPixmap));
        n++;
        XtSetArg(args[n], XmNbottomShadowPixmap, &(info->animation_data.bottomShadowPixmap));
        n++;
        XtGetValues(w, args, n);
      } break;
      case XmDRAG_UNDER_PIXMAP: {
                                ...
                                unsigned char *type)
{
  ...
  switch (GetDSAnimationStyle(info)) {
    case XmDRAG_UNDER_HIGHLIGHT: {
      XmDSRemoteHighlightStyle hs = (XmDSRemoteHighlightStyle)GetDSRemoteAnimationPart(info);
      XmICCDropSiteHighlight hi = (XmICCDropSiteHighlight)(&iccInfo);
      hs->highlight_color = hi->animation_data.highlightColor;
      hs->highlight_pixmap = hi->animation_data.highlightPixmap;
      hs->background = hi->animation_data.background;
      hs->highlight_thickness = hi->animation_data.highlightThickness;
      hs->border_width = hi->animation_data.borderWidth;
    } break;
    case XmDRAG_UNDER_SHADOW_IN:
    case XmDRAG_UNDER_SHADOW_OUT: {
      XmDSRemoteShadowStyle ss = (XmDSRemoteShadowStyle)GetDSRemoteAnimationPart(info);
      XmICCDropSiteShadow si = (XmICCDropSiteShadow)(&iccInfo);
      ss->top_shadow_color = si->animation_data.topShadowColor;
      ss->top_shadow_pixmap = si->animation_data.topShadowPixmap;
      ss->bottom_shadow_color = si->animation_data.bottomShadowColor;
      ss->bottom_shadow_pixmap = si->animation_data.bottomShadowPixmap;
      ss->foreground = si->animation_data.foreground;
      ss->shadow_thickness = si->animation_data.shadowThickness;
      ss->highlight_thickness = si->animation_data.highlightThickness;
      ss->border_width = si->animation_data.borderWidth;
    } break;
    case XmDRAG_UNDER_PIXMAP: {
      XmDSRemotePixmapStyle ps = (XmDSRemotePixmapStyle)GetDSRemoteAnimationPart(info);
      XmICCDropSitePixmap pi = (XmICCDropSitePixmap)(&iccInfo);
      ps->animation_pixmap = pi->animation_data.animationPixmap;
      ps->animation_pixmap_depth = pi->animation_data.animationPixmapDepth;
      ...
      ps->shadow_thickness = pi->animation_data.shadowThickness;
      ps->highlight_thickness = pi->animation_data.highlightThickness;
      ps->border_width = pi->animation_data.borderWidth;
    } break;
    case XmDRAG_UNDER_NONE: {
      XmDSRemoteNoneStyle ns = (XmDSRemoteNoneStyle)GetDSRemoteAnimationPart(info);
      XmICCDropSiteNone ni = (XmICCDropSiteNone)(&iccInfo);
...
static void ChangeRoot(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer callData)
{
  ...
  if (callback->reason == XmCR_TOP_LEVEL_ENTER) {
     ...
     */
    dsm->dropManager.curDragContext = (Widget)XmGetDragContext((Widget)dsm, callback->timeStamp);
    if (newRoot) {
      ...
      dsm->dropManager.dsRoot = (XtPointer)ReadTree(dsm, dataPtr);
    }
    dsm->dropManager.rootX = cd->xOrigin;
    dsm->dropManager.rootY = cd->yOrigin;
    dsm->dropManager.rootW = cd->width;
    ...
    if (dsm->dropManager.curInfo != NULL) {
      ...
      unsigned char style = _XmGetActiveProtocolStyle(dsm->dropManager.curDragContext);
      ...
      HandleLeave(dsm, &cdRec, &cbRec, (XmDSInfo)dsm->dropManager.curInfo, style, False);
      dsm->dropManager.curInfo = NULL;
    }
...
static void CreateInfo(XmDropSiteManagerObject dsm, Widget widget, ArgList args, Cardinal argCount)
{
  ...
  if ((fullInfoRec.activity == XmDROP_SITE_ACTIVE) && (fullInfoRec.drop_proc == NULL)) {
      ...
      (fullInfoRec.animation_pixmap_depth == 0))
  {
    ...
    XmeGetPixmapData(XtScreen(widget),
  ...
  if ((fullInfoRec.type == XmDROP_SITE_COMPOSITE) &&
      ((fullInfoRec.rectangles != NULL) || (fullInfoRec.num_rectangles != 1)))
  {
  ...
  while (!XtIsShell(shell))
    shell = XtParent(shell);
  fullInfoRec.import_targets_ID = _XmTargetsToIndex(
      shell, fullInfoRec.import_targets, fullInfoRec.num_import_targets);
                                ...
                                XmDSFullInfo full_info)
{
  ...
  if (index) {
    num_targets = _XmIndexToTargets(shell, index, &targets);
  ...
  if (GetDSRemote(variant)) {
    switch (GetDSAnimationStyle(variant)) {
      case XmDRAG_UNDER_HIGHLIGHT: {
        XmDSRemoteHighlightStyle hs = (XmDSRemoteHighlightStyle)GetDSRemoteAnimationPart(variant);
        ...
        full_info->highlight_thickness = hs->highlight_thickness;
        full_info->border_width = hs->border_width;
      } break;
      case XmDRAG_UNDER_SHADOW_IN:
      case XmDRAG_UNDER_SHADOW_OUT: {
        XmDSRemoteShadowStyle ss = (XmDSRemoteShadowStyle)GetDSRemoteAnimationPart(variant);
        full_info->top_shadow_color = ss->top_shadow_color;
        full_info->top_shadow_pixmap = ss->top_shadow_pixmap;
        full_info->bottom_shadow_color = ss->bottom_shadow_color;
        full_info->bottom_shadow_pixmap = ss->bottom_shadow_pixmap;
      ...
      case XmDRAG_UNDER_PIXMAP: {
        XmDSRemotePixmapStyle ps = (XmDSRemotePixmapStyle)GetDSRemoteAnimationPart(variant);
        full_info->animation_pixmap = ps->animation_pixmap;
        full_info->animation_pixmap_depth = ps->animation_pixmap_depth;
        ...
        full_info->highlight_thickness = ps->highlight_thickness;
    ...
    switch (GetDSAnimationStyle(variant)) {
      ...
      case XmDRAG_UNDER_SHADOW_OUT:
        break;
      case XmDRAG_UNDER_PIXMAP: {
        XmDSLocalPixmapStyle ps = (XmDSLocalPixmapStyle)GetDSLocalAnimationPart(variant);
        full_info->animation_pixmap = ps->animation_pixmap;
        full_info->animation_pixmap_depth = ps->animation_pixmap_depth;
        full_info->animation_mask = ps->animation_mask;
      } break;
                         ...
                         Cardinal argCount)
{
  ...
  XtGetSubvalues((XtPointer)(&full_info_rec),
                 (XtResourceList)(_XmDSResources),
                 (Cardinal)(_XmNumDSResources),
                 (ArgList)(args),
                 (Cardinal)(argCount));
  freeRects = True;
...
static void CopyFullIntoVariant(XmDSFullInfo full_info, XmDSInfo variant)
{
  ...
  switch (full_info->animation_style) {
    ...
    case XmDRAG_UNDER_PIXMAP: {
      ...
      ps->animation_pixmap_depth = full_info->animation_pixmap_depth;
...
static void UpdateInfo(XmDropSiteManagerObject dsm, Widget widget, ArgList args, Cardinal argCount)
{
  XmDSFullInfoRec full_info_rec;
  ...
  /* Update the info */
  {
    ...
    XtSetSubvalues(full_info, _XmDSResources, _XmNumDSResources, args, argCount);
        ...
        (full_info->import_targets != old_import_targets))
    {
      ...
      while (!(XtIsShell(shell)))
        shell = XtParent(shell);
      full_info->import_targets_ID = _XmTargetsToIndex(
          shell, full_info->import_targets, full_info->num_import_targets);
    }
  }
  ...
  if ((full_info->rectangles != rects) || (full_info->num_rectangles != num_rects)) {
  ...
  {
    XmeGetPixmapData(XtScreen(GetDSWidget(info)),
  ...
  if (full_info->animation_style != GetDSAnimationStyle(info)) {
    ...
    if ((GetDSType(new_info) == XmDROP_SITE_COMPOSITE) && (GetDSNumChildren(new_info))) {
      XmDSInfo child;
      int i;
...
static void EndUpdate(XmDropSiteManagerObject dsm, Widget refWidget)
{
  ...
  if (!found) {
    ...
    dsupdate = (_XmDropSiteUpdateInfo)XtMalloc(sizeof(_XmDropSiteUpdateInfoRec));
    dsupdate->dsm = dsm;
  ...
  if (clean) {
    dsm->dropManager.updateTimeOutId = XtAppAddTimeOut(
        XtWidgetToApplicationContext(shell), 0, _XmIEndUpdate, dsm);
  }
}

void _XmIEndUpdate(XtPointer client_data, XtIntervalId *interval_id)
{
...
static void DestroyInfo(XmDropSiteManagerObject dsm, Widget widget)
{
  ...
  if (info == (XmDSInfo)(dsm->dropManager.curInfo)) {
    ...
    cdRec.dragOver =
        (Widget)(((XmDragContext)(dsm->dropManager.curDragContext))->drag.curDragOver);
    HandleLeave(dsm, &cdRec, &cbRec, (XmDSInfo)dsm->dropManager.curInfo, style, False);
    dsm->dropManager.curInfo = NULL;
  }
  while (info != NULL) {
...
static void SyncDropSiteGeometry(XmDropSiteManagerObject dsm, XmDSInfo info)
{
  ...
  if (!GetDSHasRegion(info)) {
    ...
    _XmRegionUnionRectWithRegion(&rect, GetDSRegion(info), GetDSRegion(info));
  }
}
...
void _XmSyncDropSiteTree(Widget shell)
{
  XmDropSiteManagerObject dsm = (XmDropSiteManagerObject)_XmGetDropSiteManagerObject(
      (XmDisplay)XmGetXmDisplay(XtDisplayOfObject(shell)));
  DSMSyncTree(dsm, shell);
}
...
void _XmDSMUpdate(XmDropSiteManagerObject dsm, XtPointer clientData, XtPointer callData)
{
  DSMUpdate(dsm, clientData, callData);
}

int _XmDSMGetTreeFromDSM(XmDropSiteManagerObject dsm, Widget shell, XtPointer dataPtr)
{
  return (DSMGetTreeFromDSM(dsm, shell, dataPtr));
}

Boolean _XmDropSiteShell(Widget widget)
{
  XmDropSiteManagerObject dsm = (XmDropSiteManagerObject)_XmGetDropSiteManagerObject(
      (XmDisplay)XmGetXmDisplay(XtDisplayOfObject(widget)));
...
Boolean _XmDropSiteWrapperCandidate(Widget widget)
{
  XmDropSiteManagerObject dsm = (XmDropSiteManagerObject)_XmGetDropSiteManagerObject(
      (XmDisplay)XmGetXmDisplay(XtDisplayOfObject(widget)));
  Widget shell;
...
Boolean XmDropSiteRegistered(Widget widget)
{
  XmDropSiteManagerObject dsm;
  XtPointer info;
  _XmWidgetToAppContext(widget);
  _XmAppLock(app);
  dsm = (XmDropSiteManagerObject)_XmGetDropSiteManagerObject(
      (XmDisplay)XmGetXmDisplay(XtDisplayOfObject(widget)));
...
void XmDropSiteStartUpdate(Widget refWidget)
{
  ...
  _XmAppLock(app);
  dsm = (XmDropSiteManagerObject)_XmGetDropSiteManagerObject(
      (XmDisplay)XmGetXmDisplay(XtDisplayOfObject(refWidget)));
...
void XmDropSiteUpdate(Widget enclosingWidget, ArgList args, Cardinal argCount)
{
  ...
  _XmAppLock(app);
  dsm = (XmDropSiteManagerObject)_XmGetDropSiteManagerObject(
      (XmDisplay)XmGetXmDisplay(XtDisplayOfObject(enclosingWidget)));
...
void XmDropSiteConfigureStackingOrder(Widget widget, Widget sibling, Cardinal stack_mode)
{
  ...
  if (sibling != NULL) {
    ...
    int i;
    if ((sib == NULL) || (((XmDSInfo)GetDSParent(sib)) != parent) ||
...
XmDropSiteVisuals XmDropSiteGetActiveVisuals(Widget widget)
{
  ...
  dsm = (XmDropSiteManagerObject)_XmGetDropSiteManagerObject(
      (XmDisplay)XmGetXmDisplay(XtDisplayOfObject(widget)));
  info = (XmDSInfo)dsm->dropManager.curInfo;
  dsv = (XmDropSiteVisuals)XtCalloc(1, sizeof(XmDropSiteVisualsRec));
  /* Update if dsm is dirty */
  _XmIEndUpdate((XtPointer)dsm, (XtIntervalId *)NULL);
  if (info == NULL) {
    XtFree((char *)dsv);
  ...
  if (!GetDSRemote(info)) {
    ...
    if (XmIsPrimitive(w)) {
      ...
      n = 0;
      XtSetArg(args[n], XmNbackground, &(dsv->background));
      n++;
      XtSetArg(args[n], XmNforeground, &(dsv->foreground));
      n++;
      XtSetArg(args[n], XmNtopShadowColor, &(dsv->topShadowColor));
      n++;
      XtSetArg(args[n], XmNtopShadowPixmap, &(dsv->topShadowPixmap));
      n++;
      XtSetArg(args[n], XmNbottomShadowColor, &(dsv->bottomShadowColor));
      n++;
      XtSetArg(args[n], XmNbottomShadowPixmap, &(dsv->bottomShadowPixmap));
      n++;
      XtSetArg(args[n], XmNshadowThickness, &(dsv->shadowThickness));
      n++;
      XtSetArg(args[n], XmNhighlightColor, &(dsv->highlightColor));
      n++;
      XtSetArg(args[n], XmNhighlightPixmap, &(dsv->highlightPixmap));
      n++;
      XtSetArg(args[n], XmNhighlightThickness, &(dsv->highlightThickness));
      n++;
      if (!GetDSHasRegion(info)) {
        XtSetArg(args[n], XmNborderWidth, &(dsv->borderWidth));
        n++;
      }
      else
        dsv->borderWidth = 0;
...
Widget _XmGetActiveDropSite(Widget widget)
{
  XmDropSiteManagerObject dsm = (XmDropSiteManagerObject)_XmGetDropSiteManagerObject(
      (XmDisplay)XmGetXmDisplay(XtDisplayOfObject(widget)));
  XmDSInfo info = (XmDSInfo)dsm->dropManager.curInfo;
 ...
 * state of the tree
 **********************************************************************/
static Boolean PrintHashEntry(XmHashKey key, XtPointer value, XtPointer data)
{
...
static void PrintTree(XmDSInfo rec, int level)
{
  int i;
