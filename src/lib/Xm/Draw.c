...
{
  XtAppContext app;
  if (!w || !shadow_thick || !width || !height)
    return;
  app = XtDisplayToApplicationContext(display);
  (void)app; /* unused but required for _XmAppLock */
  _XmAppLock(app);
                      ...
                      Dimension highlight_thickness)
{
  XRectangle rect[4];
  XtAppContext app;
  if (!d || !highlight_thickness || !width || !height)
    return;
  app = XtDisplayToApplicationContext(display);
  (void)app; /* unused but required for _XmAppLock */
  _XmAppLock(app);
                      ...
                      unsigned char separator_type)
{
  ...
  if (separator_type == XmSHADOW_ETCHED_IN_DASH || separator_type == XmSHADOW_ETCHED_OUT_DASH)
    ...
    shadow_dash_size = (orientation == XmHORIZONTAL) ? (width - 2 * margin) :
                                                       (height - 2 * margin);
