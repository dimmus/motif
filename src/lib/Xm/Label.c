...
static Boolean XmLabelGetBaselines(Widget wid, Dimension **baselines, int *line_count);
static void XmLabelMarginsProc(Widget w, XmBaselineMargins *margins_rec);
...
static XtActionsRec ActionsList[] = {
    {"Enter", Enter}, {"Leave", Leave}, {"Help", Help}, {"ProcessDrag", ProcessDrag}};
...
static XtResource resources[] = {{XmNshadowThickness,
                                  XmCShadowThickness,
                                  XmRHorizontalDimension,
                                  sizeof(Dimension),
                                  XtOffsetOf(XmLabelRec, primitive.shadow_thickness),
                                  XmRImmediate,
                                  (XtPointer)0},
                                 {XmNalignment,
                                  XmCAlignment,
                                  ...
                                  XtOffsetOf(XmLabelRec, label.alignment),
                                  XmRImmediate,
                                 ...
                                 {XmNlabelType,
                                  ...
                                  XmRLabelType,
                                  sizeof(unsigned char),
                                  XtOffsetOf(XmLabelRec, label.label_type),
                                  XmRImmediate,
                                  (XtPointer)XmSTRING},
                                 {XmNmarginWidth,
                                  XmCMarginWidth,
                                 ...
                                 {"pri.vate",
                                  ...
                                  XmRBoolean,
                                  sizeof(Boolean),
                                  XtOffsetOf(XmLabelRec, label.check_set_render_table),
                                  XmRImmediate,
                                  (XtPointer)False},
                                 {XmNfontList,
                                  XmCFontList,
                                  XmRFontList,
                                  sizeof(XmFontList),
                                  XtOffsetOf(XmLabelRec, label.font),
                                  XmRCallProc,
                                 ...
                                 {XmNlabelPixmap,
                                  ...
                                  XmRDynamicPixmap,
                                  sizeof(Pixmap),
                                  XtOffsetOf(XmLabelRec, label.pixmap),
                                  XmRImmediate,
                                 ...
                                 {XmNlabelInsensitivePixmap,
                                  ...
                                  XmRDynamicPixmap,
                                  sizeof(Pixmap),
                                  XtOffsetOf(XmLabelRec, label.pixmap_insen),
                                  XmRImmediate,
                                  (XtPointer)XmUNSPECIFIED_PIXMAP},
                                 {XmNlabelString,
                                  XmCXmString,
                                 ...
                                 {XmNmnemonic,
                                  ...
                                  XmRKeySym,
                                  sizeof(KeySym),
                                  XtOffsetOf(XmLabelRec, label.mnemonic),
                                  XmRImmediate,
                                 ...
                                 {XmNrecomputeSize,
                                  ...
                                  XmRBoolean,
                                  sizeof(Boolean),
                                  XtOffsetOf(XmLabelRec, label.recompute_size),
                                  XmRImmediate,
                                  (XtPointer)True},
                                 {XmNstringDirection,
                                  XmCStringDirection,
                                  XmRStringDirection,
                                  sizeof(unsigned char),
                                  XtOffsetOf(XmLabelRec, label.string_direction),
                                  XmRImmediate,
                                 ...
                                 {XmNpixmapPlacement,
                                  ...
                                  XtOffsetOf(XmLabelRec, label.pixmap_placement),
                                  XmRImmediate,
...
static void ClassInitialize(void)
{
  ...
  xmLabelClassRec.label_class.translations = (String)(XtParseTranslationTable(
      menu_traversal_events));
  /* Install menu savvy on just this class */
  XmeTraitSet((XtPointer)&xmLabelClassRec, XmQTmenuSavvy, (XtPointer)&MenuSavvyRecord);
}

void _XmLabelCloneMenuSavvy(WidgetClass wc, XmMenuSavvyTrait mst)
{
 ...
 *
 ************************************************************************/
void _XmCalcLabelDimensions(Widget wid)
{
...
void _XmLabelCalcTextRect(Widget wid)
{
  XmLabelWidget newlw = (XmLabelWidget)wid;
  XmLabelPart *lp = &(newlw->label);
  lp->TextRect.width = 0;
  ...
  if (Lab_IsPixmap(newlw)) {
    ...
    if (lp->pixmap_placement == XmPIXMAP_TOP || lp->pixmap_placement == XmPIXMAP_BOTTOM) {
      lp->TextRect.height = lp->PixmapRect.height + lp->StringRect.height +
                            lp->pixmap_text_padding;
    ...
    else if ((lp->pixmap_placement == XmPIXMAP_RIGHT && LayoutIsRtoLP(wid)) ||
             (lp->pixmap_placement == XmPIXMAP_LEFT && !LayoutIsRtoLP(wid)))
    {
      lp->PixmapRect.x = 0;
      lp->StringRect.x = lp->PixmapRect.width + lp->pixmap_text_padding;
    }
    else if ((lp->pixmap_placement == XmPIXMAP_LEFT && LayoutIsRtoLP(wid)) ||
             (lp->pixmap_placement == XmPIXMAP_RIGHT && !LayoutIsRtoLP(wid)))
    {
      lp->StringRect.x = 0;
    ...
    if (lp->pixmap_placement == XmPIXMAP_RIGHT || lp->pixmap_placement == XmPIXMAP_LEFT) {
      ...
      else if ((lp->alignment == XmALIGNMENT_END && !LayoutIsRtoLP(wid)) ||
               (lp->alignment == XmALIGNMENT_BEGINNING && LayoutIsRtoLP(wid)))
      {
...
static void Resize(Widget wid)
{
  ...
  if (newlw->core.width == 0)
    newlw->core.width = (Dimension)lp->TextRect.width + lp->margin_left + lp->margin_right +
                        (2 * (lp->margin_width + newlw->primitive.highlight_thickness +
                              newlw->primitive.shadow_thickness));
  leftx = (newlw->primitive.highlight_thickness + newlw->primitive.shadow_thickness +
           lp->margin_width + lp->margin_left);
  rightx = (newlw->core.width -
            (newlw->primitive.highlight_thickness + newlw->primitive.shadow_thickness +
  ...
  if (newlw->core.height == 0)
    newlw->core.height = (Dimension)MAX(lp->TextRect.height, lp->acc_TextRect.height) +
                         lp->margin_top + lp->margin_bottom +
                         (2 * (lp->margin_height + newlw->primitive.highlight_thickness +
                               newlw->primitive.shadow_thickness));
  lp->TextRect.y = (short)(newlw->primitive.highlight_thickness +
                           newlw->primitive.shadow_thickness + lp->margin_height + lp->margin_top +
                           ((int)(newlw->core.height - lp->margin_top - lp->margin_bottom -
                                  (2 * (lp->margin_height + newlw->primitive.highlight_thickness +
                                        newlw->primitive.shadow_thickness)) -
                                  lp->TextRect.height) /
                            2));
  if (lp->_acc_text != NULL) {
    ...
    if (LayoutIsRtoLP(newlw))
      lp->acc_TextRect.x = newlw->primitive.highlight_thickness +
                           newlw->primitive.shadow_thickness + newlw->label.margin_width;
    else
      lp->acc_TextRect.x = (short)newlw->core.width - newlw->primitive.highlight_thickness -
                           newlw->primitive.shadow_thickness - newlw->label.margin_width -
                           newlw->label.margin_right + LABEL_ACC_PAD;
    lp->acc_TextRect.y =
        (short)(newlw->primitive.highlight_thickness + newlw->primitive.shadow_thickness +
                lp->margin_height + lp->margin_top +
                ((int)(newlw->core.height - lp->margin_top - lp->margin_bottom -
                       (2 * (lp->margin_height + newlw->primitive.highlight_thickness +
                             newlw->primitive.shadow_thickness)) -
                       lp->acc_TextRect.height) /
                 2));
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if (!XmRepTypeValidValue(XmRID_STRING_DIRECTION, lw->label.string_direction, (Widget)lw)) {
  ...
  if ((lw->label.menu_type == XmMENU_POPUP) || (lw->label.menu_type == XmMENU_PULLDOWN) ||
  ...
  if ((XtClass(lw) == xmLabelWidgetClass) &&
      ((lw->label.menu_type == XmMENU_POPUP) || (lw->label.menu_type == XmMENU_PULLDOWN) ||
  ...
  if ((lw->label.menu_type == XmMENU_POPUP) || (lw->label.menu_type == XmMENU_PULLDOWN) ||
      (lw->label.menu_type == XmMENU_BAR) || (lw->label.menu_type == XmMENU_OPTION))
  {
    _XmProcessLock();
                                      ...
                                      XtWidgetGeometry *desired)
{
  ...
  if (lw->label.recompute_size == FALSE) {
    ...
    desired->width = (Dimension)lw->label.TextRect.width +
                     (2 * (lw->label.margin_width + lw->primitive.highlight_thickness +
                           lw->primitive.shadow_thickness)) +
                     lw->label.margin_left + lw->label.margin_right;
    if (desired->width == 0)
      desired->width = 1;
    desired->height = (Dimension)MAX(lw->label.TextRect.height, lw->label.acc_TextRect.height) +
                      (2 * (lw->label.margin_height + lw->primitive.highlight_thickness +
                            lw->primitive.shadow_thickness)) +
                      lw->label.margin_top + lw->label.margin_bottom;
    if (desired->height == 0)
      desired->height = 1;
  }
...
static void Redisplay(Widget wid, XEvent *event, Region region)
{
  ...
  marginal_width = lp->margin_left + lp->margin_right +
                   (2 * (lw->primitive.highlight_thickness + lw->primitive.shadow_thickness));
  ...
  {
    clip_rect.x = lw->primitive.highlight_thickness + lw->primitive.shadow_thickness +
                  lp->margin_left;
    clip_rect.y = lw->primitive.highlight_thickness + lw->primitive.shadow_thickness +
                  lp->margin_top;
  ...
  if (lp->_acc_text != NULL) {
     ...
     * room for everything
     */
    if (lw->core.width >= (Dimension)(2 * (lw->primitive.highlight_thickness +
                                           lw->primitive.shadow_thickness + lp->margin_width) +
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if ((new_w->label.menu_type == XmMENU_POPUP) || (new_w->label.menu_type == XmMENU_PULLDOWN) ||
  ...
  Boolean pixmap_size_changed = ((newlp->PixmapRect.width != curlp->PixmapRect.width) ||
                                 (newlp->PixmapRect.height != curlp->PixmapRect.height));
  if (((Lab_IsText(new_w) || Lab_IsPixmapAndText(new_w)) &&
       ((newstring) || (newlp->font != curlp->font))) ||
      ((Lab_IsPixmap(new_w) || Lab_IsPixmapAndText(new_w)) &&
       ((newlp->pixmap != curlp->pixmap) || (newlp->pixmap_insen != curlp->pixmap_insen) ||
        (XtIsSensitive(nw) != XtIsSensitive(cw))) &&
       pixmap_size_changed) ||
      (Lab_IsPixmapAndText(new_w) && (newlp->pixmap_placement != curlp->pixmap_placement)) ||
  ...
  if ((newlp->alignment != curlp->alignment) ||
      (XmPrim_layout_direction(new_w) != XmPrim_layout_direction(current)))
  {
  ...
  if ((newlp->margin_height != curlp->margin_height) ||
      (newlp->margin_width != curlp->margin_width) || (newlp->margin_left != curlp->margin_left) ||
      (newlp->margin_right != curlp->margin_right) || (newlp->margin_top != curlp->margin_top) ||
      ...
      (newlp->pixmap_text_padding != curlp->pixmap_text_padding))
  {
    ...
    if (!XmRepTypeValidValue(XmRID_STRING_DIRECTION, new_w->label.string_direction, (Widget)new_w))
    {
  ...
  if ((XtClass(new_w) == xmLabelWidgetClass) &&
      ((new_w->label.menu_type == XmMENU_POPUP) || (new_w->label.menu_type == XmMENU_PULLDOWN) ||
      ...
      new_w->core.tm.translations)
  {
    XtTranslations trans;
    if ((new_w->label.menu_type == XmMENU_POPUP) || (new_w->label.menu_type == XmMENU_PULLDOWN) ||
        (new_w->label.menu_type == XmMENU_BAR) || (new_w->label.menu_type == XmMENU_OPTION))
    {
      _XmProcessLock();
      trans = (XtTranslations)((XmLabelClassRec *)XtClass(new_w))->label_class.translations;
      _XmProcessUnlock();
      if (trans)
        XtOverrideTranslations((Widget)new_w, trans);
    }
    else {
      _XmProcessLock();
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(current)),
                                              XmQTmenuSystem);
 ...
 *
 ************************************************************************/
static void SetActivateCallbackState(Widget wid, XmActivateState state)
{
  XmLabelWidget w = (XmLabelWidget)wid;
  switch (state) {
...
static void Help(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  XmMenuSystemTrait menuSTrait;
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(lw)), XmQTmenuSystem);
 ...
 ***********************************************************************/
/*ARGSUSED*/
static void GetAccelerator(Widget wid,
                           int resource, /* unused */
                           XtArgVal *value)
{
  XmLabelWidget lw = (XmLabelWidget)wid;
 ...
 ***********************************************************************/
/*ARGSUSED*/
static void GetAcceleratorText(Widget wid,
                               int resource, /* unused */
                               XtArgVal *value)
{
  XmLabelWidget lw = (XmLabelWidget)wid;
...
Widget XmVaCreateLabel(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmLabelWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedLabel(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmLabelWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
static void ProcessDrag(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (XmIsCascadeButton(w) && CB_IsArmed(w))
    return;
  ...
  if (event && !(event->xbutton.state & ~((Button1Mask >> 1) << event->xbutton.button) &
...
/*ARGSUSED*/
void _XmLabelConvert(Widget w, XtPointer ignore, XmConvertCallbackStruct *cs)
{
  enum {
  ...
  if (cs->target == atoms[XmA_MOTIF_COMPOUND_STRING]) {
    ...
    format = 8;
    size = XmCvtXmStringToByteStream(label_string, (unsigned char **)&value);
  }
  else if (cs->target == atoms[XmACOMPOUND_TEXT] || cs->target == atoms[XmATEXT] ||
           cs->target == XA_STRING || cs->target == C_ENCODING)
  {
 ...
 **************************************************************************/
static XtPointer ConvertToEncoding(
    Widget w, char *str, Atom encoding, unsigned long *length, Boolean *flag)
{
  XtPointer rval = NULL;
  ...
  if (encoding == XA_STRING) {
    ...
    ret_status = XmbTextListToTextProperty(
        XtDisplay(w), (char **)&str, 1, (XICCEncodingStyle)XStringStyle, &tmp_prop);
    ...
    *flag = (ret_status == Success);
    ...
    ret_status = Xutf8TextListToTextProperty(
        XtDisplay(w), (char **)&str, 1, (XICCEncodingStyle)XUTF8StringStyle, &tmp_prop);
    if (ret_status == Success || ret_status > 0) {
      rval = (XtPointer)tmp_prop.value;
      *length = tmp_prop.nitems;
    }
...
static XtPointer LabelGetValue(Widget w, int type)
{
  XmString value;
  ...
  if (type == XmFORMAT_XmSTRING) {
    ...
    temp = (type == XmFORMAT_MBYTE) ?
               _XmStringUngenerate(value, NULL, XmMULTIBYTE_TEXT, XmMULTIBYTE_TEXT) :
               _XmStringUngenerate(value, NULL, XmWIDECHAR_TEXT, XmWIDECHAR_TEXT);
    XmStringFree(value);
    return temp;
  }
  else
...
static void LabelSetValue(Widget w, XtPointer value, int type)
{
  Arg args[1];
  Cardinal nargs;
  XmString temp;
...
/*ARGSUSED*/
static int LabelPreferredValue(Widget w) /* unused */
{
  return XmFORMAT_XmSTRING;
}

static char *GetLabelAccelerator(Widget w)
{
...
static KeySym GetLabelMnemonic(Widget w)
{
  if (XtClass(w) == xmLabelWidgetClass)
