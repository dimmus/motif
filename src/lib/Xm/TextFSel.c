...
static void TextFieldConvertCallback(Widget, XtPointer, XmConvertCallbackStruct *);
static void TextFieldDestinationCallback(Widget, XtPointer, XmDestinationCallbackStruct *);
...
static void DoStuff(Widget w, XtPointer ignore, XmSelectionCallbackStruct *ds);
...
static void TextFieldSecondaryWrapper(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
    ...
    if (total_value) {
      if (tf->text.max_char_size == 1) {
        ...
        replace_res = _XmTextFieldReplaceText(
            tf, (XEvent *)_insert_select->event, left, right, total_value, num_chars, True);
      }
        ...
        if (num_chars < 0)
          ...
          replace_res = _XmTextFieldReplaceText(
              tf, (XEvent *)_insert_select->event, left, right, (char *)wc_value, num_chars, True);
      ...
      if (left != right) {
        if (!dest_disjoint || !tf->text.add_mode) {
          _XmTextFieldStartSelection(
              tf, TextF_CursorPosition(tf), TextF_CursorPosition(tf), _insert_select->event->time);
        }
      }
    }
...
/* ARGSUSED */
Boolean _XmTextFieldConvert(Widget w,
                            Atom *selection,
                            Atom *target,
                            Atom *type,
                            XtPointer *value,
  ...
  if (*target == atoms[XmATARGETS]) {
    ...
    *format = 8;
    ...
    ret_status = XmbTextListToTextProperty(
        XtDisplay(w), &tmp_value, 1, (XICCEncodingStyle)XStringStyle, &tmp_prop);
    XtFree(tmp_value);
    ...
    *format = 8;
    ...
    ret_status = XmbTextListToTextProperty(
        XtDisplay(w), &tmp_value, 1, (XICCEncodingStyle)XCompoundTextStyle, &tmp_prop);
    XtFree(tmp_value);
    if (ret_status == Success || ret_status > 0) {
      *length = tmp_prop.nitems;
      *value = (XtPointer)tmp_prop.value;
    ...
    *format = 8;
    ...
    /* Convert to compound text */
    ret_status = XmbTextListToTextProperty(
        XtDisplay(w), &tmp_value, 1, (XICCEncodingStyle)XUTF8StringStyle, &tmp_prop);
    XtFree(tmp_value);
    if (ret_status == Success || ret_status > 0) {
      *length = tmp_prop.nitems;
      *value = (XtPointer)tmp_prop.value;
...
/* ARGSUSED */
void _XmTextFieldLoseSelection(Widget w, Atom *selection)
{
  XmTextFieldWidget tf = (XmTextFieldWidget)w;
...
Widget _XmTextFieldGetDropReciever(Widget w)
{
  Widget widget;
  XContext loc_context;
  _XmProcessLock();
...
static void DropTransferProc(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
  ...
  {
    if (insertPosLeft > tf->text.prim_pos_left && insertPosLeft < tf->text.prim_pos_right)
      insertPosLeft = tf->text.prim_pos_left;
    if (insertPosRight < tf->text.prim_pos_right && insertPosRight > tf->text.prim_pos_left)
...
static void DoStuff(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
  ...
  if (ds->type == atoms[XmANULL]) {
    ...
    if (_prim_select->num_chars > 0 && tf->text.selection_move) {
      ...
      _XmTextFieldStartSelection(tf,
                                 _prim_select->position,
                                 _prim_select->position + prim_char_length,
                                 _prim_select->time);
      tf->text.pending_off = False;
      _XmTextFieldSetCursorPosition(
          tf, NULL, _prim_select->position + prim_char_length, True, True);
...
static void HandleDrop(Widget w, XmDropProcCallbackStruct *cb, XmDestinationCallbackStruct *ds)
{
  ...
  if (cb->operation & XmDROP_MOVE && w == initiator && ((XmTextFieldWidget)w)->text.has_primary &&
...
/*ARGSUSED*/
static void TextFieldConvertCallback(Widget w,
...
/*ARGSUSED*/
static void TextFieldDestinationCallback(Widget w,
