...
#define IsButton(w) (((XtPointer)XmeTraitGet((XtPointer)XtClass((w)), XmQTactivatable) != NULL))
#define IsAutoButton(fsb, w) \
  (w == SB_OkButton(fsb) || w == SB_ApplyButton(fsb) || w == SB_CancelButton(fsb) || \
   w == SB_HelpButton(fsb))
...
static void ClassPartInitialize(WidgetClass fsc);
static void Initialize(Widget rw, Widget nw, ArgList args_in, Cardinal *num_args);
static void Destroy(Widget fsb);
...
static void FSBCreateFilterLabel(XmFileSelectionBoxWidget fsb);
static void FSBCreateDirListLabel(XmFileSelectionBoxWidget fsb);
static void FSBCreateDirList(XmFileSelectionBoxWidget fsb);
static void FSBCreateFilterText(XmFileSelectionBoxWidget fs);
static void FSBCreateDirText(XmFileSelectionBoxWidget fs);
static void FSBCreateDirTextLabel(XmFileSelectionBoxWidget fs);
static XmGeoMatrix FileSBGeoMatrixCreate(Widget wid, Widget instigator, XtWidgetGeometry *desired);
static Boolean FileSelectionBoxNoGeoRequest(XmGeoMatrix geoSpec);
static void ListLabelFix(XmGeoMatrix geoSpec,
                         int action,
                         XmGeoMajorLayout layoutPtr,
                         XmKidGeometry rowPtr);
static void ListFix(XmGeoMatrix geoSpec,
                    int action,
                    XmGeoMajorLayout layoutPtr,
                    XmKidGeometry rowPtr);
static void UpdateHorizPos(Widget wid);
static void FileSearchProc(Widget w, XtPointer sd);
static void QualifySearchDataProc(Widget w, XtPointer sd, XtPointer qsd);
static void FileSelectionBoxUpdate(XmFileSelectionBoxWidget fs,
                                   XmFileSelectionBoxCallbackStruct *searchData);
static void DirSearchProc(Widget w, XtPointer sd);
static void ListCallback(Widget wid, XtPointer client_data, XtPointer call_data);
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args_in, Cardinal *num_args);
static void FSBGetDirectory(Widget fs, int resource, XtArgVal *value);
static void FSBGetDirSpec(Widget fs, int resource, XtArgVal *value);
static void FSBGetNoMatchString(Widget fs, int resource, XtArgVal *value);
static void FSBGetPattern(Widget fs, int resource, XtArgVal *value);
static void FSBGetFilterLabelString(Widget fs, int resource_offset, XtArgVal *value);
static void FSBGetDirListLabelString(Widget fs, int resource_offset, XtArgVal *value);
static void FSBGetDirListItems(Widget fs, int resource_offset, XtArgVal *value);
static void FSBGetDirListItemCount(Widget fs, int resource_offset, XtArgVal *value);
static void FSBGetListItems(Widget fs, int resource_offset, XtArgVal *value);
static void FSBGetListItemCount(Widget fs, int resource_offset, XtArgVal *value);
static void FSBGetDirMask(Widget fs, int resource_offset, XtArgVal *value);
static Widget GetActiveText(XmFileSelectionBoxWidget fsb, XEvent *event);
static void FileSelectionBoxUpOrDown(Widget wid, XEvent *event, String *argv, Cardinal *argc);
static void FileSelectionBoxRestore(Widget wid, XEvent *event, String *argv, Cardinal *argc);
static void FileSelectionBoxFocusMoved(Widget wid, XtPointer client_data, XtPointer data);
static void FileSelectionPB(Widget wid, XtPointer which_button, XtPointer call_data);
static void FSBConvert(Widget wid, XtPointer client_data, XtPointer cb_struct);
static void FilterFix(XmGeoMatrix geoSpec,
                      int action,
                      XmGeoMajorLayout layoutPtr,
                      XmKidGeometry rowPtr);
...
static XtResource resources[] = {
    ...
    },                  /*   directory, because of XmNdirMask.   */
    {
        ...
        XmRXmString,
        sizeof(XmString),
        XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.pattern),
        XmRImmediate,
    ...
    {XmNdirListLabelString,
     ...
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.dir_list_label_string),
     XmRImmediate,
    ...
    {XmNdirListItemCount,
     ...
     XmRInt,
     sizeof(int),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.dir_list_item_count),
     XmRImmediate,
    ...
    {XmNfilterLabelString,
     ...
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.filter_label_string),
     XmRImmediate,
     (XtPointer)XmUNSPECIFIED},
    {XmNdirMask,
     XmCDirMask,
    ...
    {XmNnoMatchString,
     ...
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.no_match_string),
     XmRImmediate,
     (XtPointer)XmUNSPECIFIED},
    {XmNqualifySearchDataProc,
     ...
     XmRProc,
     sizeof(XtProc),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.qualify_search_data_proc),
     XmRImmediate,
    ...
    {XmNdirSearchProc,
     ...
     XmRProc,
     sizeof(XtProc),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.dir_search_proc),
     XmRImmediate,
    ...
    {XmNfileSearchProc,
     ...
     XmRProc,
     sizeof(XtProc),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.file_search_proc),
     XmRImmediate,
    ...
    {XmNfileTypeMask,
     ...
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.file_type_mask),
     XmRImmediate,
    ...
    {XmNlistUpdated,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.list_updated),
     XmRImmediate,
     (XtPointer)TRUE},
    {XmNdirectoryValid,
     XmCDirectoryValid,
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(struct _XmFileSelectionBoxRec, file_selection_box.directory_valid),
     XmRImmediate,
     (XtPointer)TRUE},
    ...
    {XmNautoUnmanage,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(struct _XmFileSelectionBoxRec, bulletin_board.auto_unmanage),
     XmRImmediate,
     (XtPointer)FALSE},
    ...
    {XmNapplyLabelString,
     ...
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(struct _XmFileSelectionBoxRec, selection_box.apply_label_string),
     XmRImmediate,
     (XtPointer)XmUNSPECIFIED},
    {XmNdialogType,
     XmCDialogType,
    ...
    {XmNpathMode,
     ...
     XmRPathMode,
     sizeof(XtEnum),
     XtOffsetOf(XmFileSelectionBoxRec, file_selection_box.path_mode),
     XmRImmediate,
    ...
    {XmNfileFilterStyle,
     ...
     XmRFileFilterStyle,
     sizeof(XtEnum),
     XtOffsetOf(XmFileSelectionBoxRec, file_selection_box.file_filter_style),
     XmRImmediate,
    ...
    {XmNdirTextLabelString,
     ...
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(XmFileSelectionBoxRec, file_selection_box.dir_text_label_string),
     XmRImmediate,
     (XtPointer)NULL},
};
...
externaldef(xmfileselectionboxclassrec) XmFileSelectionBoxClassRec xmFileSelectionBoxClassRec = {
    {
        /* core class record        */
...
externaldef(xmfileselectionboxwidgetclass)
                       ...
                       Cardinal *num_args) /* unused */
{
   ...
   */
  if (new_w->selection_box.list_label_string == (XmString)XmUNSPECIFIED) {
    local_xmstring = XmStringCreate(FILES_STRING, XmFONTLIST_DEFAULT_TAG);
    numArgs = 0;
    XtSetArg(args[numArgs], XmNlabelString, local_xmstring);
    ++numArgs;
    XtSetValues(SB_ListLabel(new_w), args, numArgs);
    XmStringFree(local_xmstring);
    new_w->selection_box.list_label_string = NULL;
  }
  if (new_w->selection_box.apply_label_string == (XmString)XmUNSPECIFIED) {
    local_xmstring = XmStringCreate(FILTER_APPLY_STRING, XmFONTLIST_DEFAULT_TAG);
    numArgs = 0;
    XtSetArg(args[numArgs], XmNlabelString, local_xmstring);
    ++numArgs;
  ...
  XtAddCallback(SB_ApplyButton(new_w),
                XmNactivateCallback,
                FileSelectionPB,
                (XtPointer)XmDIALOG_APPLY_BUTTON);
  XtAddCallback(
      SB_OkButton(new_w), XmNactivateCallback, FileSelectionPB, (XtPointer)XmDIALOG_OK_BUTTON);
  XtAddCallback(SB_CancelButton(new_w),
                XmNactivateCallback,
                FileSelectionPB,
                (XtPointer)XmDIALOG_CANCEL_BUTTON);
  XtAddCallback(
      SB_HelpButton(new_w), XmNactivateCallback, FileSelectionPB, (XtPointer)XmDIALOG_HELP_BUTTON);
  ...
  /* Add Convert callbacks to handle the FILE and FILENAME
       targets */
  XtAddCallback(FS_DirList(new_w), XmNconvertCallback, FSBConvert, (XtPointer)new_w);
  XtAddCallback(SB_List(new_w), XmNconvertCallback, FSBConvert, (XtPointer)new_w);
  ...
  if (XtClass(new_w) == xmFileSelectionBoxWidgetClass) {
    XtManageChildren(new_w->composite.children, new_w->composite.num_children);
  }
  return;
}
...
static void DeleteChild(Widget w)
{
  ...
  _XmProcessLock();
  delete_child =
      ((XmSelectionBoxWidgetClass)xmSelectionBoxWidgetClass)->composite_class.delete_child;
  _XmProcessUnlock();
  (*delete_child)(w);
  return;
}
...
static void FSBCreateFilterLabel(XmFileSelectionBoxWidget fsb)
{
  ...
  FS_FilterLabel(fsb) = _XmBB_CreateLabelG(
      (Widget)fsb, FS_FilterLabelString(fsb), "FilterLabel", XmFilterStringLoc);
  return;
}
...
static void FSBCreateDirListLabel(XmFileSelectionBoxWidget fsb)
{
  ...
  FS_DirListLabel(fsb) = _XmBB_CreateLabelG(
      (Widget)fsb, FS_DirListLabelString(fsb), "Dir", XmDirListStringLoc);
  return;
}
...
static void FSBCreateDirList(XmFileSelectionBoxWidget fsb)
{
  ...
  XtSetArg(al[ac], XmNvisibleItemCount, SB_ListVisibleItemCount(fsb));
  ac++;
  XtSetArg(al[ac], XmNstringDirection, SB_StringDirection(fsb));
  ac++;
  XtSetArg(al[ac], XmNselectionPolicy, XmBROWSE_SELECT);
  ac++;
  ...
  if (callbackProc) {
    XtAddCallback(FS_DirList(fsb), XmNsingleSelectionCallback, callbackProc, (XtPointer)fsb);
    XtAddCallback(FS_DirList(fsb), XmNbrowseSelectionCallback, callbackProc, (XtPointer)fsb);
    XtAddCallback(FS_DirList(fsb), XmNdefaultActionCallback, callbackProc, (XtPointer)fsb);
  }
...
static void FSBCreateFilterText(XmFileSelectionBoxWidget fs)
{
  ...
  argCount = 0;
  XtSetArg(arglist[argCount], XmNcolumns, SB_TextColumns(fs));
  argCount++;
  XtSetArg(arglist[argCount], XmNresizeWidth, FALSE);
  argCount++;
  XtSetArg(arglist[argCount], XmNvalue, stext_value);
  argCount++;
  XtSetArg(arglist[argCount], XmNnavigationType, XmSTICKY_TAB_GROUP);
  argCount++;
...
static void FSBCreateDirTextLabel(XmFileSelectionBoxWidget fs)
{
  FS_DirTextLabel(fs) = _XmBB_CreateLabelG(
      (Widget)fs, FS_DirTextLabelString(fs), "DirL", XmDirTextStringLoc);
}
...
static XmGeoMatrix FileSBGeoMatrixCreate(Widget wid, Widget instigator, XtWidgetGeometry *desired)
{
  ...
  geoSpec = _XmGeoMatrixAlloc(
      XmFSB_MAX_WIDGETS_VERT, fsb->composite.num_children, sizeof(FS_GeoExtensionRec));
                    ...
                    XmKidGeometry rowPtr)
{
  ...
  if (!fileListGeo->kid) { /* Only one list widget in this row, so do nothing.
                            */
    return;
  }
  extension = (FS_GeoExtension)geoSpec->extension;
  ...
  switch (action) {
    case XmGET_PREFERRED_SIZE: {
      if (FS_PathMode(geoSpec->composite) == XmPATH_MODE_FULL) {
        argc = 0;
        XtSetArg(argv[argc], XmNfontList, &listFonts);
        ++argc;
        XtGetValues((Widget)fileList, argv, argc);
        listPathWidth = XmStringWidth(listFonts, FS_Directory(geoSpec->composite));
        ...
        if (extension->filter_label &&
            (extension->filter_label->box.width > dirListGeo->box.width))
        {
        ...
        {
          fileListGeo->box.width = extension->file_list_label->box.width;
        }
        if (extension->filter_label && extension->filter_text &&
            (fileListGeo->box.height >=
             ((extension->filter_label->box.height + extension->filter_text->box.height) << 1)))
        {
          dirListGeo->box.height =
              (fileListGeo->box.height -= (extension->filter_label->box.height +
    ...
    case XmGEO_PRE_SET: {
      if (FS_PathMode(geoSpec->composite) == XmPATH_MODE_FULL) {
        if (fileListGeo->box.width > extension->prefer_width) {
           ...
           */
          extension->delta_width = fileListGeo->box.width - extension->prefer_width;
         ...
         */
        if (extension->file_list_label) {
          if (extension->file_list_label->box.width < fileListGeo->box.width) {
            extension->file_list_label->box.width = fileListGeo->box.width;
          ...
          if (extension->dir_list_label) {
            if (LayoutIsRtoLM(geoSpec->composite)) {
              ...
              if (listLabelsOffset > (int)layoutPtr->row.space_between) {
                extension->dir_list_label->box.width = (Dimension)listLabelsOffset -
        ...
        if (extension->filter_label && extension->filter_text && extension->file_list_label &&
            extension->dir_list_label)
        {
          Position dirListDelta = fileListGeo->box.y - extension->filter_text->box.y;
          extension->filter_label->box.width = extension->filter_text->box.width =
              extension->dir_list_label->box.width;
          extension->filter_label->box.x = extension->filter_text->box.x =
              extension->dir_list_label->box.x;
          extension->file_list_label->box.y = extension->filter_label->box.y;
    ...
    case XmGEO_POST_SET: {
      if (FS_PathMode(geoSpec->composite) == XmPATH_MODE_FULL) {
        if (extension->delta_width) { /* Undo the changes of PRE_SET, so subsequent re-layout
...
static void FileSearchProc(Widget w, XtPointer sd)
{
  ...
  XmFileSelectionBoxCallbackStruct *searchData = (XmFileSelectionBoxCallbackStruct *)sd;
  String dir;
  String pattern;
  Arg args[3];
  int Index;
  ...
  if (fileList && numFiles) {
      ...
      if (showDotFiles || ((fileList[Index])[dirLen] != '.')) {
        if (FS_PathMode(fs) == XmPATH_MODE_FULL)
          XmStringFileList[numItems++] = XmStringGenerate(
              fileList[Index], XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
        else
          XmStringFileList[numItems++] = XmStringGenerate(
              &(fileList[Index])[dirLen], XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
      }
      ++Index;
    }
    /* Update the list.
     */
    Index = 0;
    XtSetArg(args[Index], XmNitems, XmStringFileList);
    Index++;
    XtSetArg(args[Index], XmNitemCount, numItems);
    Index++;
...
static void QualifySearchDataProc(Widget w, XtPointer sd, XtPointer qsd)
{
  XmFileSelectionBoxWidget fs = (XmFileSelectionBoxWidget)w;
  XmFileSelectionBoxCallbackStruct *searchData = (XmFileSelectionBoxCallbackStruct *)sd;
  XmFileSelectionBoxCallbackStruct *qualifiedSearchData = (XmFileSelectionBoxCallbackStruct *)qsd;
  String valueString;
      ...
      (dirString && maskString && (maskString[0] != '/')))
  {
    ...
    _XmOSQualifyFileSpec(dirString, patternString, &qualifiedDir, &qualifiedPattern);
  }
  else {
    ...
    if (patternPartPtr != maskString) {
       ...
       */
      *(patternPartPtr - 1) = '\0';
      if (!*maskString || ((*maskString == '/') && !maskString[1])) {
        if (!*maskString) { /* The '/' that was replaced with '\0' above was the only
                             *    character in the directory specification (root
                             *    directory "/"), so simply restore it.
                             */
          dirPartPtr = "/";
        }
        else { /* The directory specification was "//" before the
                *   trailing '/' was deleted, so restore original.
                */
          dirPartPtr = "//";
        }
      }
      else { /* Is non-root directory specification, so its ok to have
  ...
  if (searchData->value) {
    ...
    if (FS_PathMode(fs) == XmPATH_MODE_FULL) {
      ...
      if ((fileStr == NULL) || (*fileStr == '\0') || (*fileStr == '/') ||
    ...
    qualifiedSearchData->value = XmStringGenerate(
        valueString, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
  }
  qualifiedSearchData->length = XmStringLength(qualifiedSearchData->value);
  qualifiedSearchData->mask = XmStringGenerate(
      qualifiedMask, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
  qualifiedSearchData->mask_length = XmStringLength(qualifiedSearchData->mask);
  qualifiedSearchData->dir = XmStringGenerate(
      qualifiedDir, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
  qualifiedSearchData->dir_length = XmStringLength(qualifiedSearchData->dir);
  qualifiedSearchData->pattern = XmStringGenerate(
      qualifiedPattern, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
  qualifiedSearchData->pattern_length = XmStringLength(qualifiedSearchData->pattern);
  XtFree(valueString);
                                   ...
                                   XmFileSelectionBoxCallbackStruct *searchData)
{
  ...
  (*FS_QualifySearchDataProc(fs))(
      (Widget)fs, (XtPointer)searchData, (XtPointer)&qualifiedSearchData);
  ...
  if (FS_DirectoryValid(fs)) {
    (*FS_FileSearchProc(fs))((Widget)fs, (XtPointer)&qualifiedSearchData);
  ...
  FS_StateFlags(fs) &= ~XmFS_IN_FILE_SEARCH;
  al = 0;
  XtSetArg(ac[al], XmNitemCount, &itemCount);
  ++al;
  ...
  if (itemCount) {
    ...
    if ((item = FS_NoMatchString(fs)) != NULL) {
      al = 0;
      XtSetArg(ac[al], XmNitems, &item);
      ++al;
      XtSetArg(ac[al], XmNitemCount, 1);
      ++al;
  ...
  if (FS_ListUpdated(fs)) {
    if (FS_PathMode(fs) == XmPATH_MODE_FULL) {
      if ((textValue = _XmStringGetTextConcat(FS_Directory(fs))) != NULL) {
        ...
        XmTextFieldSetInsertionPosition(SB_Text(fs), XmTextFieldGetLastPosition(SB_Text(fs)));
        XtFree(textValue);
      }
    }
    else {
...
static void DirSearchProc(Widget w, XtPointer sd)
{
   ...
   */
  if ((qualifiedDir = _XmStringGetTextConcat(searchData->dir)) == NULL) {
  ...
  if ((FS_StateFlags(fs) & XmFS_DIR_SEARCH_PROC) || (curDirModTime != FS_PrevDirModTime(fs)) ||
      !XmStringCompare(searchData->dir, FS_Directory(fs)))
  {
    ...
    dirList = NULL;
    _XmOSGetDirEntries(
        qualifiedDir, "*", XmFILE_DIRECTORY, FALSE, TRUE, &dirList, &numDirs, &numAlloc);
    if (!numDirs) {
    ...
    while (Index < numDirs) {
      ...
      if (showDotFiles || (Index == 1) || ((dirList[Index])[dirLen] != '.')) {
        if (FS_PathMode(fs) == XmPATH_MODE_FULL)
          XmStringDirList[numItems++] = XmStringGenerate(
              dirList[Index], XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
        else
          XmStringDirList[numItems++] = XmStringGenerate(
              &(dirList[Index])[dirLen], XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
      }
      ++Index;
    }
    /* Update the list.  */
    Index = 0;
    XtSetArg(args[Index], XmNitems, XmStringDirList);
    Index++;
    XtSetArg(args[Index], XmNitemCount, numItems);
    Index++;
    XtSetArg(args[Index], XmNtopItemPosition, 1);
    Index++;
  ...
  switch (callback->reason) {
    ...
    case XmCR_SINGLE_SELECT: {
      if (wid == FS_DirList(fsb)) {
        FS_DirListSelectedItemPosition(fsb) = callback->item_position;
        ...
        change_data.mask = XmStringGenerate(
            textValue, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
         ...
         */
        (*FS_QualifySearchDataProc(fsb))(
            (Widget)fsb, (XtPointer)&change_data, (XtPointer)&qualified_change_data);
        if (FS_PathMode(fsb) == XmPATH_MODE_FULL) {
        ...
        if ((textValue = _XmStringGetTextConcat(callback->item)) != NULL) {
          ...
          XmTextFieldSetInsertionPosition(SB_Text(fsb), XmTextFieldGetLastPosition(SB_Text(fsb)));
          XtFree(textValue);
        }
      }
      break;
    ...
    case XmCR_DEFAULT_ACTION: {
       ...
       */
      if ((callback->event->type != KeyPress) && dbutton && XtIsManaged((Widget)dbutton) &&
          XtIsSensitive((Widget)dbutton) && XmIsGadget(dbutton) &&
          (!(FS_StateFlags(fsb) & XmFS_NO_MATCH) || (wid == FS_DirList(fsb))))
      {
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (FS_DirListLabelString(current) != FS_DirListLabelString(new_w)) {
    n = 0;
    XtSetArg(args[n], XmNlabelString, FS_DirListLabelString(new_w));
    n++;
  ...
  if (FS_FilterLabelString(current) != FS_FilterLabelString(new_w)) {
    n = 0;
    XtSetArg(args[n], XmNlabelString, FS_FilterLabelString(new_w));
    n++;
    XtSetArg(args[n], XmNlabelType, XmSTRING);
    n++;
  ...
  n = 0;
  if (SB_ListVisibleItemCount(current) != SB_ListVisibleItemCount(new_w)) {
    XtSetArg(args[n], XmNvisibleItemCount, SB_ListVisibleItemCount(new_w));
    ++n;
  }
  if (FS_DirListItems(new_w)) {
    XtSetArg(args[n], XmNitems, FS_DirListItems(new_w));
    ++n;
    FS_DirListItems(new_w) = NULL;
  }
  if (FS_DirListItemCount(new_w) != XmUNSPECIFIED_COUNT) {
    XtSetArg(args[n], XmNitemCount, FS_DirListItemCount(new_w));
    ++n;
  ...
  if ((SB_TextColumns(new_w) != SB_TextColumns(current)) && FS_FilterText(new_w)) {
    n = 0;
    XtSetArg(args[n], XmNcolumns, SB_TextColumns(new_w));
    ++n;
...
static void FSBGetDirSpec(Widget fs, int resource, XtArgVal *value)
{
  if (FS_PathMode(fs) == XmPATH_MODE_RELATIVE) {
    ...
    *value = (XtArgVal)XmStringConcat(FS_Directory(fs), (XmString)filename);
...
/****************************************************************/
static void FSBGetDirectory(Widget fs,
                            int resource, /* unused */
                            XtArgVal *value)
/****************
...
/****************************************************************/
static void FSBGetNoMatchString(Widget fs,
                                int resource, /* unused */
                                XtArgVal *value)
/****************
 * This does get values hook magic to keep the
...
/****************************************************************/
static void FSBGetPattern(Widget fs,
                          int resource, /* unused */
                          XtArgVal *value)
/****************
 * This does get values hook magic to keep the
 ...
 ****************/
static void FSBGetFilterLabelString(Widget fs,
 ...
 * This does get values hook magic to keep the user happy.
 ****************/
static void FSBGetDirListLabelString(Widget fs,
                                     int resource_offset, /* unused */
                                     XtArgVal *value)
{
  XmString data;
 ...
 * This does get values hook magic to keep the user happy.
 ****************/
static void FSBGetDirListItems(Widget fs,
                               int resource_offset, /* unused */
                               XtArgVal *value)
{
  XmStringTable data;
 ...
 * This does get values hook magic to keep the user happy.
 ****************/
static void FSBGetDirListItemCount(Widget fs,
                                   int resource_offset, /* unused */
                                   XtArgVal *value)
{
  int data;
 ...
 * This does get values hook magic to keep the user happy.
 ****************/
static void FSBGetListItems(Widget fs,
                            int resource_offset, /* unused */
                            XtArgVal *value)
{
  XmStringTable data;
 ...
 * This does get values hook magic to keep the user happy.
 ****************/
static void FSBGetListItemCount(Widget fs,
                                int resource_offset, /* unused */
                                XtArgVal *value)
{
  int data;
 ...
 * user happy.
 ****************/
static void FSBGetDirMask(Widget fs,
                          int resource_offset, /* unused */
                          XtArgVal *value)
{
  String filterText;
  ...
  data = XmStringGenerate(filterText, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
  *value = (XtArgVal)data;
  XtFree(filterText);
  return;
}
...
static Widget GetActiveText(XmFileSelectionBoxWidget fsb, XEvent *event)
{
  ...
  if (_XmGetFocusPolicy((Widget)fsb) == XmEXPLICIT) {
    ...
    if (SB_Text(fsb) && (XtWindow(SB_Text(fsb)) == ((XKeyPressedEvent *)event)->window)) {
      activeChild = SB_Text(fsb);
    }
    else {
      if (FS_FilterText(fsb) &&
          (XtWindow(FS_FilterText(fsb)) == ((XKeyPressedEvent *)event)->window))
      {
        activeChild = FS_FilterText(fsb);
      }
      else {
        if (FS_DirText(fsb) && (XtWindow(FS_DirText(fsb)) == ((XKeyPressedEvent *)event)->window))
        {
...
static void FileSelectionBoxUpOrDown(Widget wid, XEvent *event, String *argv, Cardinal *argc)
{
  ...
  ac = 0;
  XtSetArg(av[ac], XmNitemCount, &count);
  ++ac;
  XtSetArg(av[ac], XmNtopItemPosition, &top);
  ++ac;
  XtSetArg(av[ac], XmNvisibleItemCount, &visible);
  ++ac;
  ...
  if (*position == 0) { /*  No selection, so select first item.
                         */
    XmListSelectPos(list, ++*position, True);
  }
  else {
    if (!key_pressed && (*position > 1)) { /*  up  */
      XmListDeselectPos(list, *position);
      XmListSelectPos(list, --*position, True);
    }
    else {
      if ((key_pressed == 1) && (*position < count)) { /*  down  */
        XmListDeselectPos(list, *position);
        XmListSelectPos(list, ++*position, True);
      }
      else {
        if (key_pressed == 2) { /*  home  */
          XmListDeselectPos(list, *position);
          *position = 1;
          XmListSelectPos(list, *position, True);
        }
        else {
          if (key_pressed == 3) { /*  end  */
            XmListDeselectPos(list, *position);
            *position = count;
            XmListSelectPos(list, *position, True);
          }
...
static void FileSelectionBoxFocusMoved(Widget wid, XtPointer client_data, XtPointer data)
{
  XmFocusMovedCallbackStruct *call_data = (XmFocusMovedCallbackStruct *)data;
  Widget ancestor;
  /****************/
  if (!call_data->cont) { /* Preceding callback routine wants focus-moved processing
                           *   to be discontinued.
                           */
    return;
  }
  ...
  else if (call_data->new_focus && ((call_data->new_focus == SB_Text(client_data)) ||
  ...
  if (call_data->cont) {
    ...
    while (ancestor && !XtIsShell(ancestor)) {
      if (XmIsBulletinBoard(ancestor)) {
        if (BB_DynamicDefaultButton(ancestor) && BB_DynamicDefaultButton(client_data)) {
          _XmBulletinBoardSetDynDefaultButton(ancestor, BB_DynamicDefaultButton(client_data));
        }
      }
...
static void FileSelectionPB(Widget wid, XtPointer which_button, XtPointer call_data)
{
  ...
  if (((long)which_button) == XmDIALOG_APPLY_BUTTON) {
    if (FS_FilterText(fs) && (text_value = XmTextFieldGetString(FS_FilterText(fs)))) {
      searchData.mask = XmStringGenerate(text_value, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
      searchData.mask_length = XmStringLength(searchData.mask);
      XtFree(text_value);
    }
    if (FS_DirText(fs) && (text_value = XmTextFieldGetString(FS_DirText(fs)))) {
      searchData.dir = XmStringGenerate(text_value, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
      searchData.dir_length = XmStringLength(searchData.dir);
      XtFree(text_value);
    }
    searchData.reason = XmCR_NONE;
  ...
  switch ((long)which_button) {
    case XmDIALOG_OK_BUTTON: {
      if (SB_MustMatch(fs)) {
        match = XmListItemExists(SB_List(fs), qualifiedSearchData.value);
      }
      if (!match) {
        qualifiedSearchData.reason = XmCR_NO_MATCH;
        XtCallCallbackList(
            ((Widget)fs), fs->selection_box.no_match_callback, &qualifiedSearchData);
      }
      else {
        qualifiedSearchData.reason = XmCR_OK;
        XtCallCallbackList(((Widget)fs), fs->selection_box.ok_callback, &qualifiedSearchData);
      }
      allowUnmanage = TRUE;
      break;
    }
    case XmDIALOG_APPLY_BUTTON: {
      ...
      XtCallCallbackList(((Widget)fs), fs->selection_box.apply_callback, &qualifiedSearchData);
      break;
    }
    case XmDIALOG_CANCEL_BUTTON: {
      qualifiedSearchData.reason = XmCR_CANCEL;
      XtCallCallbackList(((Widget)fs), fs->selection_box.cancel_callback, &qualifiedSearchData);
      allowUnmanage = TRUE;
      break;
    }
    case XmDIALOG_HELP_BUTTON: {
      if (fs->manager.help_callback) {
        ...
        XtCallCallbackList(((Widget)fs), fs->manager.help_callback, &qualifiedSearchData);
      }
      else {
  ...
  XmStringFree(qualifiedSearchData.mask);
  XmStringFree(qualifiedSearchData.value);
  if (allowUnmanage && fs->bulletin_board.shell && fs->bulletin_board.auto_unmanage) {
...
void XmFileSelectionDoSearch(Widget fs, XmString dirmask)
{
  ...
  if (dirmask) {
    ...
    if (textString) {
      searchData.mask = XmStringGenerate(textString, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
      searchData.mask_length = XmStringLength(searchData.mask);
      XtFree(textString);
    }
    else {
    ...
    if (FS_DirText(fs) && (textString = XmTextFieldGetString(FS_DirText(fs)))) {
      searchData.dir = XmStringGenerate(textString, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
      searchData.dir_length = XmStringLength(searchData.dir);
      XtFree(textString);
    }
  }
...
Widget XmVaCreateFileSelectionBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmFileSelectionBoxWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedFileSelectionBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmFileSelectionBoxWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
 ...
 ****************/
Widget XmCreateFileSelectionDialog(Widget parent, char *name, ArgList arglist, Cardinal argcount)
{
  return XmeCreateClassDialog(xmFileSelectionBoxWidgetClass, parent, name, arglist, argcount);
}
...
static void FSBConvert(Widget wid, XtPointer client_data, XtPointer cb_struct)
{
  ...
  static char *atom_names[] = {XmSTARGETS,
                               XmSFILE,
                               XmSFILE_NAME,
  ...
  if (FS_DirSearchProc(fsb) == DirSearchProc && FS_FileSearchProc(fsb) == FileSearchProc) {
    ...
    else if (cs->target == atoms[XmAFILE] || cs->target == atoms[XmAFILE_NAME]) {
