...
#define MAX_SELECTION_INCR(dpy) \
  (((65536 < XMaxRequestSize(dpy)) ? (65536 << 2) : (XMaxRequestSize(dpy) << 2)) - 100)
...
static Time ClipboardGetCurrentTime(Display *dpy);
static void ClipboardSetNextItemId(Display *display, long itemid);
static Boolean WeOwnSelection(Display *, ClipboardHeader header);
static void AssertClipboardSelection(Display *display,
                                     Window window,
                                     ClipboardHeader header,
                                     Time time);
static Boolean ClipboardConvertProc(
    Widget, Atom *, Atom *, Atom *, XtPointer *, unsigned long *, int *);
static Atom GetTypeFromTarget(Display *, Atom);
static Window InitializeSelection(Display *display,
                                  ClipboardHeader header,
                                  Window window,
                                  Time time);
...
static int GetWindowProperty(Display *display,
                             ...
                             int *format,
                             Boolean delete_flag);
static int ClipboardRetrieveItem(Display *display,
                                 itemId itemid,
                                 int add_length,
                                 int def_length,
                                 XtPointer *outpointer,
...
static void ClipboardReplaceItem(Display *display,
                                 itemId itemid,
                                 XtPointer pointer,
...
static void ClipboardDeleteFormat(Display *display, itemId formatitemid);
static void ClipboardDeleteFormats(Display *display, Window window, itemId dataitemid);
static void ClipboardDeleteItemLabel(Display *display, Window window, itemId dataitemid);
static unsigned long ClipboardIsMarkedForDelete(Display *display,
                                                ClipboardHeader header,
                                                itemId itemid);
static void ClipboardDeleteItem(Display *display,
                                ...
                                ClipboardHeader header,
                                itemId deleteid);
static void ClipboardDeleteMarked(Display *display, Window window, ClipboardHeader header);
static void ClipboardMarkItem(Display *display,
                              ClipboardHeader header,
                              itemId dataitemid,
                              unsigned long state);
static int ClipboardSendMessage(Display *display,
                                Window window,
...
static int ClipboardGetSelection(Display *display,
                                 ...
                                 unsigned long *size,
                                 int *format);
static int ClipboardRequestDataAndWait(Display *display,
                                       Window window,
                                       ClipboardFormatItem formatptr);
static itemId ClipboardGetNewItemId(Display *display);
static void ClipboardSetAccess(Display *display, Window window);
static int ClipboardLock(Display *display, Window window);
static int ClipboardUnlock(Display *display, Window window, Boolean all_levels);
static int ClipboardSearchForWindow(Display *display, Window parentwindow, Window window);
static int ClipboardWindowExists(Display *display, Window window);
static void ClipboardReceiveData(
 ...
 *
 ***********************************************************************/
static Boolean WeOwnSelection(Display *display, ClipboardHeader header)
{
                                     ...
                                     Time time)
{
  ...
  XtOwnSelection(widget, XmA_CLIPBOARD, time, ClipboardConvertProc, NULL, NULL);
  return;
}
                                      ...
                                      char *format)
{
  ...
  matchformat = ClipboardFindFormat(
      dpy, header, format, (itemId)NULL, 0, &maxname, &count, &matchformatlength);
  if (matchformat != 0) {
    dataok = 1;
...
static Boolean ClipboardConvertProc(Widget wid,
  ...
  *format = 8;
  ...
  if (*target == atoms[XmA_TARGETS]) {
    ...
    *ptr = atoms[XmA_TIMESTAMP];
    ...
    while (nextitem != NULL && n < count) {
      ...
      *ptr = nextitem->formatNameAtom;
      ...
      nextitem = ClipboardFindFormat(
          display, header, 0, (itemId)NULL, n + 1, &dummy, &ret_count, &dummy);
      if (nextitem != NULL)
        ptr++;
    }
    *value = (char *)save_ptr;
    ...
    *type = XA_INTEGER;
    ...
    if (XmClipboardInquireLength(display, window, format_name, size) != ClipboardSuccess) {
      rval = False;
      goto done;
    }
    if (*size == 0) {
                          ...
                          int format_length) /* Format length  8-16-32 */
{
  ...
  formatatom = ClipboardGetAtomFromFormat(display, format_name);
  rootwindow = RootWindow(display, 0);
  if (ClipboardGetLenFromFormat(display, format_name, &stored_len) == ClipboardSuccess) {
...
/*---------------------------------------------*/
static void CleanupHeader(Display *display)
{
  XDeleteProperty(
...
{
  enum {
    XmA_MOTIF_CLIP_MESSAGE,
    XmA_MOTIF_CLIP_DATA_REQUEST,
    XmA_MOTIF_CLIP_DATA_DELETE,
    NUM_ATOMS
  };

  static char *atom_names[] = {
      XmS_MOTIF_CLIP_MESSAGE, XmS_MOTIF_CLIP_DATA_REQUEST, XmS_MOTIF_CLIP_DATA_DELETE};
  ...
  (*callbackroutine)(widget, (long *)&formatitemid, &privateitemid, &reason);
...
static ClipboardHeader ClipboardOpen(Display *display, int add_length)
{
  ...
  if (ret_value != ClipboardSuccess) {
    ...
    root_clipboard_header->dataItemList = sizeof(ClipboardHeaderRec) / CONVERT_32_FACTOR;
  ...
  if (ret_value != ClipboardSuccess) {
    ...
    ClipboardReplaceItem(
        display, XM_NEXT_ID, int_ptr, sizeof(long), PropModeReplace, 32, False, XA_INTEGER);
  }
  else {
    XtFree((char *)int_ptr);
  }
...
{
  unsigned long headerlength;
  headerlength = sizeof(ClipboardHeaderRec) + (root_clipboard_header->currItems) * sizeof(itemId);
...
static void ClipboardDeleteFormat(Display *display, itemId formatitemid)
{
  ...
  if ((formatitem->cutByNameFlag == 0) || (formatitem->cancelledFlag != 0)) {
  ...
  ClipboardReplaceItem(display,
                       ...
                       XA_INTEGER);
  ClipboardReplaceItem(
      display, dataitemid, (XtPointer)dataitem, length, PropModeReplace, 32, True, XA_INTEGER);
}
...
static void ClipboardDeleteFormats(Display *display, Window window, itemId dataitemid)
{
  ...
  for (i = 0; i < datalist->formatCount; i++) {
    ...
    if (formatdata->cutByNameFlag == 1) {
      ...
      ClipboardSendMessage(display, window, formatdata, XM_DATA_DELETE_MESSAGE);
    }
                                ...
                                int messagetype)
{
  ...
  if (formatptr->windowId == window) {
    ...
    if (!ClipboardWindowExists(display, widgetwindow))
      return 0;
    /* send a client message to the window supplied by the user */
    XSendEvent(display, widgetwindow, True, event_mask, (XEvent *)&event_sent);
  }
  return 1;
}
...
static void ClipboardReceiveData(Widget dest, /* unused */
                                       ...
                                       ClipboardFormatItem formatptr)
{
  ...
  XSelectInput(display,
               rootwindow,
               PropertyChangeMask | StructureNotifyMask | rootattributes.your_event_mask);
  if (ClipboardSendMessage(display, window, formatptr, XM_DATA_REQUEST_MESSAGE) == 0) {
    /* put mask back the way it was */
    XSelectInput(display, rootwindow, rootattributes.your_event_mask);
    return 0;
  }
  cutbynameinfo.formatitemid = formatptr->thisFormatId;
  cutbynameinfo.window = window;
  dataisready = XCheckIfEvent(
      display, &event_return, ClipboardDataIsReady, (char *)&cutbynameinfo);
...
static void ClipboardTimeout(XtPointer client_data, XtIntervalId *timer) /* unused */
{
...
static void ClipboardSetAccess(Display *display, Window window)
{
  ...
  XChangeProperty(
      display, window, itematom, itematom, 8, PropModeReplace, (unsigned char *)"yes", 3);
}
...
static int ClipboardLock(Display *display, Window window)
{
  ...
  if (take_lock == True) {
    if (XGetSelectionOwner(display, atoms[XmA_MOTIF_CLIP_LOCK]) == None) {
      XSetSelectionOwner(
          display, atoms[XmA_MOTIF_CLIP_LOCK], window, ClipboardGetCurrentTime(display));
...
static int ClipboardSearchForWindow(Display *display, Window parentwindow, Window window)
{
  ...
  int found, i;
  Window *windowptr;
  if (XQueryTree(display, parentwindow, &rootwindow, &p_window, &children, &numchildren) == 0) {
    return (0);
  }
  found = 0;
  windowptr = children;
...
/*---------------------------------------------*/
/*---------------------------------------------*/
int XmClipboardBeginCopy(Display *display,  /* display id for application passing data */
...
{
  return (XmClipboardStartCopy(
      display, window, label, CurrentTime, widget, (XmCutPasteProc)callback, itemid));
}

/*---------------------------------------------*/
int XmClipboardStartCopy(Display *display, /* display id for application passing data */
                         Window window,    /* window to receive request for cut by name data */
                         XmString label,   /* label to be associated with data item   */
                         ...
                         long *itemid)            /* received id to identify this data item */
{
  ...
  if (label != NULL) {
    asn1strlen = XmCvtXmStringToByteStream(label, (unsigned char **)&asn1string);
    /* store the label */
    ClipboardReplaceItem(display,
                    ...
                    long *dataid)         /* Data id returned by clipboard */
{
  ...
  formatptr = ClipboardFindFormat(
      display, header, format, (itemId)itemid, 0, &maxname, &count, &formatlength);
  ...
  if (formatptr == 0) {
    ...
    *idptr = formatid;
    ...
    if (ClipboardGetLenFromFormat(display, format, &format_len) == ClipboardFail) {
    ...
    /* return the property on the root window for the item */
    ClipboardReplaceItem(
        display, itemid, (XtPointer)itemheader, itemlength, PropModeReplace, 32, True, XA_INTEGER);
...
int XmClipboardEndCopy(Display *display, Window window, long itemid)
{
  ...
  newitemoffset = header->dataItemList + (header->currItems * sizeof(itemId) / CONVERT_32_FACTOR);
...
/*---------------------------------------------*/
int XmClipboardLock(Display *display, Window window) /* identifies application owning lock */
{
  int ret_val;
  _XmDisplayToAppContext(display);
  _XmAppLock(app);
...
/*---------------------------------------------*/
int XmClipboardUnlock(Display *display,
...
/*---------------------------------------------*/
int XmClipboardStartRetrieve(Display *display, /* Display id of application wanting data */
                             Window window,
                             Time timestamp)
{
  ClipboardHeader header;
...
/*---------------------------------------------*/
int XmClipboardEndRetrieve(Display *display, /* Display id of application wanting data */
                           Window window)
{
  ClipboardHeader header;
  _XmDisplayToAppContext(display);
                        ...
                        long *private_id)         /* Private id provide by application */
{
  ...
  _XmDisplayToAppContext(display);
  _XmAppLock(app);
  ret_val = ClipboardRetrieve(
      display, window, format, buffer, length, outlength, private_id, &ignoretype);
  _XmAppUnlock(app);
  return ret_val;
}
                             ...
                             Atom *outtype)
{
  ...
  if (WeOwnSelection(display, header)) {
    ...
    /* find the matching format for the next paste item */
    matchformat = ClipboardFindFormat(
        display, header, format, (itemId)NULL, 0, &maxname, &count, &matchformatlength);
    if (matchformat != 0) {
      dataok = 1;
      ...
      if (matchformat->cutByNameFlag == 1) {
        ...
        dataok = ClipboardRequestDataAndWait(display, window, matchformat);
        if (dataok) {
      ...
      if (dataok) {
        ...
        if (header->incrementalCopyFrom) {
          ...
          if (loc_outlength == remaininglength) {
            ...
            matchformat->copiedLength = matchformat->copiedLength + loc_outlength;
          }
        }
    ...
    if (*outtype == None)
      *outtype = GetTypeFromTarget(display, XInternAtom(display, format, False));
  }
  else {
                              ...
                              &outformatsize))
    {
      ...
      if (header->incrementalCopyFrom) {
        ...
        if (loc_outlength == remaininglength) {
          ...
          header->foreignCopiedLength = header->foreignCopiedLength + loc_outlength;
        }
      }
    }
  }
                            ...
                            unsigned long *maxlength) /* receives max length of format names */
{
  ...
  if (WeOwnSelection(display, header)) {
    /* yes, find the next paste item, only looking for
           maxlength and count */
    alloc_to_free = (char *)ClipboardFindFormat(
        display, header, 0, (itemId)NULL, 0, &loc_maxlength, &loc_count, &loc_matchlength);
  }
  else {
    /* we don't own the selection, get the data from selection owner */
    if (!ClipboardGetSelection(display,
...
/*---------------------------------------------*/
int XmClipboardInquireFormat(Display *display, /* Display id of application inquiring */
                             Window window,
                             ...
                             unsigned long *outlength) /* Receives length copied to name buffer */
{
  ...
  header = ClipboardOpen(display, 0);
  /* check to see if we need to reclaim the selection */
  InitializeSelection(display, header, window, header->copyFromTimestamp);
  ptr = NULL;
  loc_outlength = 0;
  /* do we own the selection? */
  if (WeOwnSelection(display, header)) {
    ...
    matchformat = ClipboardFindFormat(
        display, header, 0, (itemId)NULL, n, &maxname, &count, &loc_matchlength);
    if (matchformat != 0) {
    ...
    unsigned long *length) /* Receives length of the data in that format */
{
  ...
  if (WeOwnSelection(display, header)) {
    ...
    matchformat = ClipboardFindFormat(
        display, header, format, (itemId)NULL, 0, &maxname, &count, &loc_matchlength);
    /* return the length */
    if (matchformat != 0) {
      if (CONVERT_32_FACTOR != 1) {
        ClipboardGetLenFromFormat(display, format, &format_len);
        loc_length = matchformat->itemLength * ((format_len == 32) ? CONVERT_32_FACTOR : 1);
      }
      else {
  ...
  *list = 0;
  ...
  for (i = 0; i < header->currItems; i++) {
    ...
    if (ClipboardIsMarkedForDelete(display, header, *id_ptr)) {
      ...
      matchformat = ClipboardFindFormat(
          display, header, format, *id_ptr, 0, &maxname, &dummy, &loc_matchlength);
    }
    if (matchformat != 0) {
                              ...
                              int format_length) /* Format length  8-16-32         */
{
  ...
  _XmDisplayToAppContext(display);
  _XmAppLock(app);
  if (format_length != 0 && format_length != 8 && format_length != 16 && format_length != 32) {
  ...
  if (format_name == 0 || strlen(format_name) == 0) {
    ...
    if (format_length != 0) {
      ...
      if (RegIfMatch(display, format_name, XmSTARGETS, XM_ATOM) ||
 ...
 *********************************************************************/
static Atom GetTypeFromTarget(Display *display, Atom target)
{
  enum {
    XmAATOM_PAIR,
    XmABACKGROUND,
    XmACHARACTER_POSITION,
    XmACLASS,
    XmACOLUMN_NUMBER,
    XmAFOREGROUND,
    XmAHOST_NAME,
  ...
  static char *atom_names[] = {XmIATOM_PAIR,
                               XmIBACKGROUND,
                               XmICHARACTER_POSITION,
                               XmICLASS,
                               XmICOLUMN_NUMBER,
                               XmIFOREGROUND,
                               XmIHOST_NAME,
  ...
  assert(XtNumber(atom_names) == NUM_ATOMS);
  XInternAtoms(display, atom_names, XtNumber(atom_names), False, atoms);
  if (target == atoms[XmATARGETS] || target == atoms[XmA_MOTIF_EXPORT_TARGETS] ||
  ...
  if (target == atoms[XmATIMESTAMP] || target == atoms[XmALIST_LENGTH] ||
      target == atoms[XmAPROCESS] || target == atoms[XmATASK] || target == atoms[XmALENGTH])
    return (XA_INTEGER);
  if (target == atoms[XmATEXT] || target == atoms[XmAODIF] || target == atoms[XmAOWNER_OS] ||
      target == atoms[XmAFILE_NAME] || target == atoms[XmAFILE] || target == atoms[XmAHOST_NAME] ||
      target == atoms[XmAUSER] || target == atoms[XmAPROCEDURE] || target == atoms[XmAMODULE] ||
      target == atoms[XmACLASS] || target == atoms[XmANAME])
  {
    int ret_status = 0;
  ...
  if (target == XA_PIXMAP)
    return (XA_DRAWABLE);
  if (target == atoms[XmAFOREGROUND] || target == atoms[XmABACKGROUND])
    return (atoms[XmAPIXEL]);
  if (target == atoms[XmACHARACTER_POSITION] || target == atoms[XmALINE_NUMBER] ||
      target == atoms[XmACOLUMN_NUMBER])
    return (atoms[XmASPAN]);
  if (target == atoms[XmACLIENT_WINDOW])
    return (XA_WINDOW);
  if (target == atoms[XmADELETE] || target == atoms[XmAINSERT_SELECTION] ||
      target == atoms[XmALINK_SELECTION] || target == atoms[XmAINSERT_PROPERTY])
    return (atoms[XmANone]);
  if (target == atoms[XmA_MOTIF_ENCODING_REGISTRY] || target == atoms[XmA_MOTIF_RENDER_TABLE])
    return (XA_STRING);
