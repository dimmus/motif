...
static void Layout(XmMainWindowWidget mw);
static void Resize(Widget wid);
static void GetSize(XmMainWindowWidget mw, Dimension *pwidth, Dimension *pheight);
...
static void CheckKids(XmMainWindowWidget mw);
 ...
 *									*
 ************************************************************************/
static XtResource resources[] = {{XmNcommandWindow,
                                  XmCCommandWindow,
                                  XmRWidget,
                                  sizeof(Widget),
...
externaldef(xmmainwindowwidgetclass)
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if (!XmRepTypeValidValue(XmRID_COMMAND_WINDOW_LOCATION, new_w->mwindow.CommandLoc, nw))
  ...
  new_w->mwindow.Sep1 = (XmSeparatorGadget)XtCreateManagedWidget(
      "Separator1", xmSeparatorGadgetClass, nw, loc_args, n);
  new_w->mwindow.Sep2 = (XmSeparatorGadget)XtCreateManagedWidget(
      "Separator2", xmSeparatorGadgetClass, nw, loc_args, n);
  new_w->mwindow.Sep3 = (XmSeparatorGadget)XtCreateManagedWidget(
      "Separator3", xmSeparatorGadgetClass, nw, loc_args, n);
...
static void InsertChild(Widget w)
{
  ...
  if (nc->child_type == (unsigned char)RESOURCE_DEFAULT) {
    ...
    if ((menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(w), XmQTmenuSystem)) !=
        NULL)
    {
...
static void Layout(XmMainWindowWidget mw)
{
  ...
  if (ExistManaged(mw->mwindow.MenuBar)) {
    ...
    if (mw->mwindow.ShowSep) {
      ...
      XmeConfigureObject((Widget)mw->mwindow.Sep1,
                         0,
                         mby + mbheight + (2 * bw),
                         mw->core.width,
  ...
  if (ExistManaged(mw->mwindow.CommandWindow)) {
    ...
    desired.request_mode = (CWWidth);
    if (XtQueryGeometry(mw->mwindow.CommandWindow, &desired, &preferred) != XtGeometryYes) {
      bw = preferred.border_width;
    ...
    if (mw->mwindow.CommandLoc == XmCOMMAND_BELOW_WORKSPACE) {
      ...
      swy = cwy;
      mw->swindow.HeightPad = sep2h + cwheight + mw->mwindow.margin_height;
      if (mw->mwindow.ShowSep)
  ...
  if (ExistManaged(mw->mwindow.Message)) {
    ...
    desired.request_mode = (CWWidth);
    if (XtQueryGeometry(mw->mwindow.Message, &desired, &preferred) != XtGeometryYes) {
      bw = preferred.border_width;
    ...
    mwy = sepy + sep3h;
    if (mw->mwindow.CommandLoc == XmCOMMAND_BELOW_WORKSPACE) {
      mw->swindow.HeightPad = sep2h + cwheight + sep3h + mwheight + mw->mwindow.margin_height;
      sep2y -= (sep3h + mwheight);
      cwy -= (sep3h + mwheight);
    }
    else
      mw->swindow.HeightPad = sep3h + mwheight + mw->mwindow.margin_height;
  ...
  if (ExistManaged(mw->mwindow.CommandWindow)) {
    XmeConfigureObject(mw->mwindow.CommandWindow, cwx, cwy, cwwidth, cwheight, bw);
    if (mw->mwindow.ShowSep) {
...
static void GetSize(XmMainWindowWidget mw, Dimension *pwidth, Dimension *pheight)
{
  ...
  XmScrollBarWidget hsb = mw->swindow.hScrollBar, vsb = mw->swindow.vScrollBar;
  Widget w;
  Dimension hsheight = 0, vmwidth = 0, ht = mw->manager.shadow_thickness * 2, hsbht = 0, vsbht = 0;
  ...
  if (ExistManaged(w)) {
    if ((mw->swindow.ScrollPolicy == XmAUTOMATIC) && !XtIsRealized((Widget)mw)) {
      newWidth = mw->swindow.AreaWidth + (w->core.border_width * 2) + hsbht + vmwidth + ht +
                 MyXpad;
      newHeight = mw->swindow.AreaHeight + (w->core.border_width * 2) + vsbht + hsheight + ht +
                  MyYpad;
    }
    else {
                                        ...
                                        XtWidgetGeometry *reply)
{
   ...
   *
   ****************/
  if (w != mw->mwindow.MenuBar && w != mw->mwindow.Message && w != mw->mwindow.CommandWindow &&
      w != (Widget)mw->mwindow.Sep1 && w != (Widget)mw->mwindow.Sep2 &&
      w != (Widget)mw->mwindow.Sep3)
  {
    ...
    if (res == XtGeometryYes) {
      ...
      if ((w == mw->swindow.WorkWindow) && (request->request_mode & CWWidth) && mb &&
          XtIsManaged(mb))
      {
        desired.x = mb->core.x;
        desired.y = mb->core.y;
        ...
        if (preferred.height != mb->core.height) {
          ...
          if (XtMakeGeometryRequest((Widget)mw, &parent_request, NULL) == XtGeometryYes) {
            if (!(request->request_mode & XtCWQueryOnly))
  ...
  if (request->request_mode & CWHeight)
    newHeight = mw->core.height - (w->core.height - (2 * w->core.border_width)) + request->height +
                2 * bw;
  else
...
static void ChangeManaged(Widget wid)
{
   ...
   *
   ****************/
  if ((mw->swindow.ScrollPolicy == XmAUTOMATIC) && (cw->composite.num_children > 1) &&
      (mw->swindow.WorkWindow != NULL))
  {
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if ((new_w->mwindow.CommandLoc != current->mwindow.CommandLoc) &&
      (XmRepTypeValidValue(
          XmRID_COMMAND_WINDOW_LOCATION, new_w->mwindow.CommandLoc, (Widget)new_w)))
  {
    XtWidgetProc resize;
    _XmProcessLock();
  ...
  if ((new_w->mwindow.MenuBar != current->mwindow.MenuBar) ||
      ...
      (new_w->swindow.WorkWindow != current->swindow.WorkWindow) || (flag))
  {
...
static void GetVertRects(Widget sw, XRectangle **vrect, Cardinal *num_vrect)
{
  ...
  *vrect = (XRectangle *)XtMalloc(sizeof(XRectangle) * (*num_vrect));
  ...
  (*vrect)[1].x = (*vrect)[0].x;
  (*vrect)[1].y = w->core.y + w->core.height - mw->swindow.vScrollBar->core.y;
  ...
  else if (ExistManaged(mw->mwindow.CommandWindow) &&
...
void XmMainWindowSetAreas(
    Widget w, Widget menu, Widget command, Widget hscroll, Widget vscroll, Widget wregion)
{
  Arg args[5];
  Cardinal n;
  n = 0;
...
Widget XmVaCreateMainWindow(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmMainWindowWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedMainWindow(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmMainWindowWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
