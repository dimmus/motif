...
static void CalcHelp(XmRowColumnWidget m,
                     Dimension *m_width,
                     Dimension *m_height,
                     Dimension b,
                     Position max_x,
                     ...
                     Dimension w,
                     Dimension h);
static void GetMaxValues(XmRowColumnWidget m,
                         Dimension *border,
                         Dimension *w,
                         Dimension *h,
                         int *items_per,
...
static void AdjustLast(XmRowColumnWidget m, int start_i, Dimension w, Dimension h);
static void SetAsking(XmRowColumnWidget m,
                      Dimension *m_width,
                      Dimension *m_height,
                      Dimension b,
                      Position max_x,
                      ...
                      Dimension w,
                      Dimension h);
static void FindLargestOption(XmRowColumnWidget submenu, Dimension *c_width, Dimension *c_height);
static void TopOrBottomAlignment(XmRowColumnWidget m,
                                 Dimension h,
                                 Dimension shadow,
                                 Dimension highlight,
                                 Dimension baseline,
                                 ...
                                 int start_i,
                                 int end_i);
static void BaselineAlignment(XmRowColumnWidget m,
                              Dimension h,
                              Dimension shadow,
                              Dimension highlight,
                              Dimension baseline,
                              ...
                              int start_i,
                              int end_i);
static void CenterAlignment(XmRowColumnWidget m, Dimension h, int start_i, int end_i);
static void ComputeTearOffHeight(XmRowColumnWidget m,
                                 Dimension *toc_b,
                                 Dimension *b,
                                 ...
                                 int *child_i,
                                 int r);
static void LayoutColumn(XmRowColumnWidget m, Dimension *m_width, Dimension *m_height);
...
static void GetMenuKidMargins(XmRowColumnWidget m,
                              Dimension *width,
                              Dimension *height,
                              Dimension *left,
                              Dimension *right,
                         ...
                         Dimension *text_height)
{
  ...
  Widget k;
  int i, n;
  *border = *w = *h = *baseline = *shadow = *highlight = *margin_top = *margin_height =
      *text_height = 0;
  /* skip the tearoff control */
  for (i = (RC_TearOffControl(m) && XtIsManaged(RC_TearOffControl(m))) ? 1 : 0, n = 0;
...
static void AdjustLast(XmRowColumnWidget m, int start_i, Dimension w, Dimension h)
{
  ...
  for (; kg[start_i].kid != NULL; start_i++) {
    b = &(kg[start_i].box);
    if (IsVertical(m)) {
      subtrahend = MGR_ShadowThickness(m) + RC_MarginW(m) + BX(b) + Double(BBorder(b));
      ...
      subtrahend = MGR_ShadowThickness(m) + RC_MarginH(m) + BY(b) + Double(BBorder(b));
      ...
      if (h > subtrahend) {
        ...
        m_top = ((h - subtrahend) > BHeight(b)) ? ((h - subtrahend) - BHeight(b)) : 0;
        BHeight(b) = h - subtrahend;
        if (m_top && (XmIsText(kg[start_i].kid) || XmIsTextField(kg[start_i].kid) ||
  ...
  if (IsVertical(m)) /* tell caller what he wants */ {
    if (Asking(*m_width))
      *m_width = x + w + b                                 /* right edge of last child */
                 + MGR_ShadowThickness(m) + RC_MarginW(m); /* plus margin on right */
    if (Asking(*m_height)) {
      ASSIGN_MAX(max_y, y);
    ...
    if (Asking(*m_width)) {
      ASSIGN_MAX(max_x, x);
      iwidth = (long)max_x - (long)(RC_Spacing(m)) + (long)(MGR_ShadowThickness(m)) +
               (long)(RC_MarginW(m));
      if (iwidth < 0)
        *m_width = 0;
      else
    ...
    if (Asking(*m_height))
      *m_height = y + h + b + MGR_ShadowThickness(m) + RC_MarginH(m);
  }
}
...
static void FindLargestOption(XmRowColumnWidget submenu, Dimension *c_width, Dimension *c_height)
{
  ...
  ForManagedChildren(submenu, i, child)
  {
    ...
    if (XmIsCascadeButton(*child)) {
      FindLargestOption((XmRowColumnWidget)CB_Submenu(*child), c_width, c_height);
    }
    else if (XmIsCascadeButtonGadget(*child)) {
      FindLargestOption((XmRowColumnWidget)CBG_Submenu(*child), c_width, c_height);
    }
    else {
...
void _XmRC_CheckAndSetOptionCascade(XmRowColumnWidget menu)
{
  Dimension width = 0;
  Dimension height = 0;
  int i;
  ...
  if ((cb = XmOptionButtonGadget((Widget)menu)) != NULL) {
    if (RC_OptionSubMenu(menu)) {
      FindLargestOption((XmRowColumnWidget)RC_OptionSubMenu(menu), &width, &height);
      if (LayoutIsRtoLG(cb))
        width += Double(G_HighlightThickness(cb)) + G_ShadowThickness(cb) + LabG_MarginLeft(cb) +
                 Double(MGR_ShadowThickness(RC_OptionSubMenu(menu))) - 2;
      else
        width += Double(G_HighlightThickness(cb)) + G_ShadowThickness(cb) + LabG_MarginRight(cb) +
                 Double(MGR_ShadowThickness(RC_OptionSubMenu(menu))) - 2;
      height += Double(G_HighlightThickness(cb)) + LabG_MarginTop(cb) + LabG_MarginBottom(cb);
      /* change cb if needed */
      if ((width != XtWidth(cb)) || (height != XtHeight(cb))) {
        ...
        unsigned char saved_unit_type = ((XmGadget)cb)->gadget.unit_type;
  ...
  *toc_b = *b = Double(RC_EntryBorder(m));
  if (RC_TearOffControl(m) && XtIsManaged(RC_TearOffControl(m))) {
    ...
    *toc_height = *toc_height >> r; /* r is 1 or 2 depending on the
    ...
    ASSIGN_MAX(*toc_height,
               2 + *toc_b + 2 * ((XmPrimitiveWidget)kg[0].kid)->primitive.shadow_thickness);
    /* Sync up the kid geo */
    /* Fix CR# 4778 */
 ...
 */
static void LayoutColumn(XmRowColumnWidget m,
                         Dimension *m_width,  /* if 0 then caller's asking */
                         Dimension *m_height) /* if 0 then caller's asking */
{
  XmRCKidGeometry kg = RC_Boxes(m);
  ...
  int items_per_column, kid_i, child_i, /* which child we are doing */
  ...
  for (; kg[child_i].kid != NULL; child_i++) {
    ...
    if (IsHelp(m, ((Widget)kg[child_i].kid)))
      CalcHelp(m, m_width, m_height, b, max_x, max_y, &x, &y, w, new_height);
...
#define SET_TEAR_OFF_BOX(toc_height) \
  if (toc_height) { \
    ...
    kg[0].box.height = toc_height; \
    kg[0].box.width = *m_width - Double(MGR_ShadowThickness(m) + RC_MarginW(m)) - toc_b; \
  }
 ...
 * it messes up the x, y for a following child.
 */
static void LayoutVerticalTight(XmRowColumnWidget m,
                                Dimension *m_width,  /* if 0 then caller's asking */
                                Dimension *m_height) /* if 0 then caller's asking */
{
  XmRCKidGeometry kg = RC_Boxes(m);
  ...
  for (; kg[child_i].kid != NULL; child_i++) {
    ...
    if (((y + h) > *m_height) && (!Asking(*m_height)) && (child_i)) { /* start new column */
      while (start_i < child_i)
        kg[start_i++].box.width = w;                                    /* set uniform width */
      x += w + Double(border) + MGR_ShadowThickness(m) + RC_MarginW(m); /* go right and */
      y = MGR_ShadowThickness(m) + RC_MarginH(m) + toc_height + toc_b;  /* back to top of menu */
...
static void LayoutHorizontaltight(XmRowColumnWidget m,
                                  Dimension *m_width,  /* if 0 then caller's asking */
                                  Dimension *m_height) /* if 0 then caller's asking */
{
  XmRCKidGeometry kg = RC_Boxes(m);
  ...
  for (; kg[child_i].kid != NULL; child_i++) {
    ...
    if (((x + w) > *m_width) && (!Asking(*m_width)) && (child_i)) { /* start a new row */
      if (AlignmentBaselineTop(m) || AlignmentBaselineBottom(m))
      ...
      start_i = child_i;
      x = MGR_ShadowThickness(m) + RC_MarginW(m); /* left edge of menu */
      ...
      if (XmIsGadget(kg[child_i].kid) || XmIsPrimitive(kg[child_i].kid)) {
        XmBaselineMargins textMargins;
        _XmRC_SetOrGetTextMargins(kg[child_i].kid, XmBASELINE_GET, &textMargins);
    ...
    if (kg[child_i].kid && XtIsWidget(kg[child_i].kid)) {
      ...
      ASSIGN_MAX(border, ((XmGadget)kg[child_i].kid)->rectangle.border_width);
    }
    if (XmIsGadget(kg[child_i].kid) || XmIsPrimitive(kg[child_i].kid)) {
      XmBaselineMargins textMargins;
      _XmRC_SetOrGetTextMargins(kg[child_i].kid, XmBASELINE_GET, &textMargins);
  ...
  if (new_height > h)
    SetAsking(m, m_width, m_height, Double(border), max_x, 0, x, y, w, new_height);
  else
    SetAsking(m, m_width, m_height, Double(border), max_x, 0, x, y, w, h);
  for (child_i = 0; kg[child_i].kid != NULL; child_i++) {
    bx = &(kg[child_i].box);
    ASSIGN_MIN(BWidth(bx),
               *m_width - Double(MGR_ShadowThickness(m)) - Double(RC_MarginW(m)) -
                   Double(BBorder(bx)));
    if (IsHelp(m, ((Widget)kg[child_i].kid)))
                                ...
                                Boolean calcMenuDimension)
{
  ...
  if (p) {
    ...
    if (LayoutIsRtoLG(cb))
      c_width += Double(G_HighlightThickness(cb)) + G_ShadowThickness(cb) + LabG_MarginLeft(cb) +
                 Double(MGR_ShadowThickness(p)) -
                 /* magic value */ 2;
    else
      c_width += Double(G_HighlightThickness(cb)) + G_ShadowThickness(cb) + LabG_MarginRight(cb) +
                 Double(MGR_ShadowThickness(p)) -
                 /* magic value */ 2;
    c_height += Double(G_HighlightThickness(cb)) + LabG_MarginTop(cb) + LabG_MarginBottom(cb);
    /* allow settings in cbg to be honored if greater than best size */
    if (instigator == (Widget)cb) {
      if ((request->request_mode & CWHeight) && (request->height > c_height)) {
        c_height = request->height;
      }
      if ((request->request_mode & CWWidth) && (request->width > c_width)) {
...
void _XmRCPreferredSize(XmRowColumnWidget m, Dimension *w, Dimension *h)
{
  Widget *q;
  int i;
  Dimension *baselines;
  ...
  Dimension y;
  if ((!IsOption(m)) &&
      ((PackColumn(m) && (IsVertical(m) || IsHorizontal(m))) || (PackTight(m) && IsHorizontal(m))))
  {
      ...
      if (XmIsGadget(rc_kid) || XmIsPrimitive(rc_kid)) {
        ...
        if (label_type == XmSTRING) {
          if (XmIsLabel(rc_kid) || XmIsLabelGadget(rc_kid)) {
            ...
            y = (textMargins.highlight + textMargins.shadow + textMargins.margin_height +
                 textMargins.margin_top +
                 ((RC_Boxes(m)[i].box.height - textMargins.margin_top - textMargins.margin_bottom -
                   (2 * (textMargins.margin_height + textMargins.shadow + textMargins.highlight)) -
                   textMargins.text_height) /
                  2));
          ...
          if (AlignmentBaselineTop(m) || AlignmentBaselineBottom(m)) {
            if (XmWidgetGetBaselines(rc_kid, &baselines, &line_count)) {
              if (AlignmentBaselineTop(m))
                ...
                RC_Boxes(m)[i].baseline = baselines[line_count - 1] - displayRect.y + y;
 ...
 */
void _XmRCAdaptToSize(XmRowColumnWidget m, Widget instigator, XtWidgetGeometry *request)
{
      ...
      ((PackColumn(m) && (IsVertical(m) || IsHorizontal(m))) || (PackTight(m) && IsHorizontal(m))))
  {
    for (i = 0; RC_Boxes(m)[i].kid != NULL; i++) {
      if (XmIsGadget(RC_Boxes(m)[i].kid) || XmIsPrimitive(RC_Boxes(m)[i].kid)) {
        ...
        if (label_type == XmSTRING && (AlignmentBaselineTop(m) || AlignmentBaselineBottom(m))) {
          Dimension *baselines;
          int line_count;
  ...
  if (!IsOption(m) || !instigator ||
      !((instigator == RC_Boxes(m)[0].kid) || (instigator == RC_Boxes(m)[1].kid)))
  {
  ...
  if ((instigator) && (instigator_w == XtWidth(instigator)) &&
      (instigator_h == XtHeight(instigator)) &&
...
void _XmRC_SetOrGetTextMargins(Widget wid, unsigned char op, XmBaselineMargins *textMargins)
{
  ...
  if (XmIsGadget(wid)) {
    ...
    if (*wcePtr && (*wcePtr)->version == XmGadgetClassExtVersion && (*wcePtr)->widget_margins)
 ...
 ****************/
XmRCKidGeometry _XmRCGetKidGeo(Widget wid,                 /* Widget w/ children. */
...
void _XmRCDoMarginAdjustment(XmRowColumnWidget m)
{
  ...
  ForManagedChildren(m, i, p)
  {
    if (XmIsLabelGadget(*p)) {
       ...
       */
      if (((*p)->core.widget_class == xmLabelGadgetClass) && (IsPulldown(m) || IsPopup(m)))
        continue;
      w = XtWidth(*p);
      h = XtHeight(*p);
      q = (XmLabelGadget)(*p);
       ...
       * don't do labels; i.e. only do buttons.
       */
      if (((*p)->core.widget_class == xmLabelWidgetClass) && (IsPulldown(m) || IsPopup(m)))
        continue;
      w = XtWidth(*p);
      h = XtHeight(*p);
      lw = (XmLabelWidget)(*p);
      if (IsVertical(m)) /* change horiz margins to */ { /* be uniform */
        ChangeMargin(Lab_MarginLeft(lw), m_l, w);
        ChangeMargin(Lab_MarginRight(lw), m_r, w);
        ChangeMargin(Lab_MarginWidth(lw), m_w, w);
        if (XtWidth(lw) != w) {
          XmeConfigureObject(
              (Widget)lw, lw->core.x, lw->core.y, w, lw->core.height, lw->core.border_width);
        }
      }
      if (!IsVertical(m) || PackColumn(m)) /* change vert margins */ {
        ...
        if (XtHeight(lw) != h) {
          XmeConfigureObject(
              (Widget)lw, lw->core.x, lw->core.y, lw->core.width, h, lw->core.border_width);
        }
