...
#define IsOneOfMany(ind_type) \
  (((ind_type) == XmONE_OF_MANY) || ((ind_type) == XmONE_OF_MANY_ROUND) || \
...
static void ToggleButtonCallback(XmToggleButtonGadget data,
                                 unsigned int reason,
                                 unsigned int value,
                                 XEvent *event);
static void Leave(XmToggleButtonGadget w, XEvent *event);
static void Enter(XmToggleButtonGadget w, XEvent *event);
static void Arm(Widget w, XEvent *event);
static void Select(XmToggleButtonGadget tb, XEvent *event);
static void Disarm(XmToggleButtonGadget tb, XEvent *event);
static void ArmAndActivate(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void BtnDown(XmToggleButtonGadget tb, XEvent *event);
static void BtnUp(XmToggleButtonGadget tb, XEvent *event);
static void GetGC(XmToggleButtonGadget tw);
static void GetUnselectGC(XmToggleButtonGadget tw);
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static void Destroy(Widget w);
static void DrawToggle(XmToggleButtonGadget w);
static void BorderHighlight(Widget wid);
static void BorderUnhighlight(Widget wid);
static void KeySelect(XmToggleButtonGadget tb, XEvent *event);
static void ComputeSpace(XmToggleButtonGadget tb);
...
static Boolean SetValues(
    Widget current, Widget request, Widget new_w, ArgList args, Cardinal *num_args);
static Cardinal GetToggleBGClassSecResData(WidgetClass w_class,
                                           XmSecondaryResourceData **data_rtn);
static XtPointer GetToggleBGClassSecResBase(Widget widget, XtPointer client_data);
static void DrawToggleLabel(XmToggleButtonGadget tb);
static void DrawEtchedInMenu(XmToggleButtonGadget tb);
static void DrawToggleShadow(XmToggleButtonGadget tb);
static void SetToggleSize(XmToggleButtonGadget newtbg);
static void NextState(unsigned char *state);
static void DrawBox(XmToggleButtonGadget w,
                    GC top_gc,
                    GC bot_gc,
                    GC fillgc,
                    int x,
...
static XtResource cache_resources[] = {{XmNindicatorSize,
                                        XmCIndicatorSize,
                                       ...
                                       {XmNindeterminateInsensitivePixmap,
                                        XmCIndeterminateInsensitivePixmap,
                                        XmRDynamicPixmap,
                                        sizeof(Pixmap),
...
static XmSyntheticResource cache_syn_resources[] = {{XmNspacing,
                                                     sizeof(Dimension),
                                                     CacheOffset(toggle_cache.spacing),
...
static void ClassPartInitialize(WidgetClass wc)
{
  ...
  XmeTraitSet((XtPointer)(WidgetClass)wc, XmQTcareParentVisual, (XtPointer)&ToggleBGCVT);
}
...
int _XmToggleBCacheCompare(XtPointer A, XtPointer B)
{
  ...
  if ((toggleB_inst->ind_type == toggleB_cache_inst->ind_type) &&
      ...
      (toggleB_inst->indeterminate_pixmap == toggleB_cache_inst->indeterminate_pixmap) &&
      (toggleB_inst->indeterminate_insensitive_pixmap ==
       toggleB_cache_inst->indeterminate_insensitive_pixmap) &&
      (toggleB_inst->indeterminate_GC == toggleB_cache_inst->indeterminate_GC) &&
      (toggleB_inst->indeterminate_box_GC == toggleB_cache_inst->indeterminate_box_GC) &&
...
static void InitializePosthook(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
   ...
   */
  _XmProcessLock();
  LabG_Cache(tbw) = (XmLabelGCacheObjPart *)_XmCachePart(
      LabG_ClassCachePart(tbw), (XtPointer)LabG_Cache(tbw), sizeof(XmLabelGCacheObjPart));
  TBG_Cache(tbw) = (XmToggleButtonGCacheObjPart *)_XmCachePart(
      TBG_ClassCachePart(tbw), (XtPointer)TBG_Cache(tbw), sizeof(XmToggleButtonGCacheObjPart));
  /*
                                 ...
                                 Mask visual_flag)
{
  ...
  _XmProcessLock();
  TBG_Cache(tw) = (XmToggleButtonGCacheObjPart *)_XmCachePart(
      TBG_ClassCachePart(tw), (XtPointer)TBG_Cache(tw), sizeof(XmToggleButtonGCacheObjPart));
  _XmProcessUnlock();
  return redraw;
}
...
static void InputDispatch(Widget wid, XEvent *event, Mask event_mask)
{
  ...
  /* END OSF Fix pir 2097 */
    ...
    (*(((XmToggleButtonGadgetClass)XtClass(tb))->gadget_class.border_highlight))((Widget)tb);
  else if (event_mask & XmFOCUS_OUT_EVENT)
    (*(((XmToggleButtonGadgetClass)XtClass(tb))->gadget_class.border_unhighlight))((Widget)tb);
 ...
 *
 ***********************************************************************/
static void redisplayPixmap(XmToggleButtonGadget tb, XEvent *event, Region region)
{
  Pixmap todo;
  ...
  w = tb->rectangle.width - x - offset - LabG_MarginRight(tb) - LabG_MarginWidth(tb);
  ASSIGN_MAX(w, 0);
  h = tb->rectangle.height - y - offset - LabG_MarginBottom(tb) - LabG_MarginHeight(tb);
  ASSIGN_MAX(h, 0);
  x += tb->rectangle.x;
  y += tb->rectangle.y;
...
static void HandlePixmap(
    XmToggleButtonGadget tb, Pixmap pix, Pixmap insen_pix, XEvent *event, Region region)
{
...
static void Help(XmToggleButtonGadget tb, XEvent *event)
{
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(tb)), XmQTmenuSystem);
...
static void ActionDraw(XmToggleButtonGadget w, XEvent *event, Boolean leave)
{
  if (TBG_Armed(w)) {
...
static void Select(XmToggleButtonGadget tb, XEvent *event)
{
  ...
  if (XmIsRowColumn(XtParent(tb))) {
    XtVaGetValues(XtParent(tb), XmNradioBehavior, &radio, XmNradioAlwaysOne, &always, NULL);
  }
  /* CR 8068: Verify that this is in fact a button event. */
  /* CR 9181: Consider clipping when testing visibility. */
  hit = (((event->xany.type == ButtonPress) || (event->xany.type == ButtonRelease)) &&
         _XmGetPointVisibility((Widget)tb, event->xbutton.x_root, event->xbutton.y_root));
  if (hit) {
    if (TBG_ToggleMode(tb) == XmTOGGLE_INDETERMINATE)
...
static void TBG_FixTearoff(XmToggleButtonGadget tb)
{
  if (XmMENU_PULLDOWN == LabG_MenuType(tb)) {
    ...
    if (mwid && XmIsRowColumn(mwid) && (XmMENU_OPTION == RC_Type(mwid)) &&
 ...
 ************************************************************************/
/*ARGSUSED*/
static void ArmAndActivate(Widget wid,
                           XEvent *event,
...
static void BtnUp(XmToggleButtonGadget tb, XEvent *event)
{
   ...
   */
  /* CR 9181: Consider clipping when testing visibility. */
  valid_event = (((event->xany.type == ButtonPress) || (event->xany.type == ButtonRelease)) &&
                 _XmGetPointVisibility((Widget)tb, event->xbutton.x_root, event->xbutton.y_root));
...
static void GetGC(XmToggleButtonGadget tw)
{
  ...
  valueMask |= GCGraphicsExposures, values.graphics_exposures = FALSE;
  valueMask |= GCStipple,
      values.stipple = XmGetPixmapByDepth(XtScreen((Widget)(tw)), XmS50_foreground, 1, 0, 1);
  valueMask |= GCLineWidth, values.line_width = 1;
  TBG_IndeterminateGC(tw) = XtAllocateGC(
      (Widget)tw, 0, valueMask, &values, GCForeground | GCBackground, 0);
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if ((TBG_IndType(new_w) == XmINVALID_TYPE) ||
      !XmRepTypeValidValue(XmRID_INDICATOR_TYPE, TBG_IndType(new_w), (Widget)new_w))
  {
    Boolean radio = FALSE;
    if (XmIsRowColumn(XtParent(new_w))) {
      XtVaGetValues(XtParent(new_w), XmNradioBehavior, &radio, NULL);
    }
    if (radio)
  ...
  if (LabG_IsPixmap(new_w) && (!IsNull(PixmapOff(new_w)) || !IsNull(PixmapInsenOff(new_w)) ||
  ...
  if (TBG_IndOn(new_w)) {
    ...
    boxSize = ((int)LabG_TextRect(new_w).height + (int)LabG_MarginTop(new_w) +
  ...
  if (request->rectangle.width == 0) {
    new_w->rectangle.width = LabG_TextRect(new_w).width + 2 * LabG_MarginHeight(new_w) +
    ...
    if ((LabG__acceleratorText(new_w) != NULL) && (TBG_IndOn(new_w))) {
      if (LayoutIsRtoLG(new_w)) {
        ...
        LabG_AccTextRect(new_w).x = new_w->rectangle.width - new_w->gadget.highlight_thickness -
                                    new_w->gadget.shadow_thickness - LabG_MarginHeight(new_w) -
  ...
  if (request->rectangle.height == 0)
    new_w->rectangle.height = MAX(TBG_IndicatorDim(new_w),
                              ...
                              2 * (new_w->gadget.highlight_thickness +
                                   new_w->gadget.shadow_thickness);
  LabG_TextRect(new_w).y = new_w->gadget.highlight_thickness + new_w->gadget.shadow_thickness +
                           LabG_MarginHeight(new_w) + LabG_MarginTop(new_w) +
                           ((new_w->rectangle.height - LabG_MarginTop(new_w) -
                             LabG_MarginBottom(new_w) -
                             (2 * (LabG_MarginHeight(new_w) + new_w->gadget.highlight_thickness +
                                   new_w->gadget.shadow_thickness)) -
                             LabG_TextRect(new_w).height) /
                            2);
  ...
  /* BEGIN OSF Fix pir 2097 */
  new_w->gadget.event_mask = XmARM_EVENT | XmACTIVATE_EVENT | XmMULTI_ARM_EVENT |
                             XmMULTI_ACTIVATE_EVENT | XmHELP_EVENT | XmFOCUS_IN_EVENT |
                             XmFOCUS_OUT_EVENT | XmENTER_EVENT | XmLEAVE_EVENT | XmBDRAG_EVENT;
  /* END OSF Fix pir 2097 */
  (*(resize))((Widget)new_w);
...
static void DrawToggle(XmToggleButtonGadget w)
{
  ...
  if ((DefaultDepthOfScreen(XtScreen(w)) > 1) && (LabG_TopShadowColor(w) != TBG_SelectColor(w)) &&
      (LabG_BottomShadowColor(w) != TBG_SelectColor(w)))
    margin = 0;
  else
    margin = 1;
  if (LayoutIsRtoLG(w))
    x = w->rectangle.x + w->rectangle.width - w->gadget.highlight_thickness -
        w->gadget.shadow_thickness - LabG_MarginHeight(w) - TBG_IndicatorDim(w);
  else
    x = w->rectangle.x + w->gadget.highlight_thickness + w->gadget.shadow_thickness +
        LabG_MarginHeight(w);
  if (LabG_IsPixmap(w) || XmStringEmpty(LabG__label(w)))
  ...
  if (TBG_Visible(w) || (IsOn(w) == XmSET)) {
    ...
    switch (TBG_VisualSet(w)) {
      ...
      case XmSET:
        ...
        glyph_gc = ((TBG_ReversedSelect(w) && DRAWBOX(normal_ind_on)) ? LabG_BackgroundGC(w) :
                                                                        LabG_NormalGC(w));
    ...
    switch (NormalizeIndType(w)) {
          ...
          case XmSET:
            ...
            if (!DRAWBOX(normal_ind_on) || ((edge - 2 * box_margin) >= MIN_GLYPH_SIZE)) {
              if (DRAWCHECK(normal_ind_on))
                XmeDrawIndicator(
                    dpy, drawable, glyph_gc, x, y, edge, edge, box_margin, normal_ind_on);
      ...
      case XmONE_OF_MANY_DIAMOND:
        XmeDrawDiamond(dpy,
                       ...
                       edge,
                       edge,
                       w->toggle.detail_shadow_thickness,
                       margin);
        break;
...
static void ComputeSpace(XmToggleButtonGadget tb)
{
  ...
  needed_width = LabG_TextRect(tb).width + LabG_MarginLeft(tb) + LabG_MarginRight(tb) +
                 (2 * (tb->gadget.shadow_thickness + tb->gadget.highlight_thickness +
                       LabG_MarginWidth(tb)));
  needed_height = LabG_TextRect(tb).height + LabG_MarginTop(tb) + LabG_MarginBottom(tb) +
                  (2 * (tb->gadget.shadow_thickness + tb->gadget.highlight_thickness +
                        LabG_MarginHeight(tb)));
  if (needed_height > tb->rectangle.height)
    if (TBG_IndOn(tb))
      LabG_TextRect(tb).y = tb->gadget.shadow_thickness + tb->gadget.highlight_thickness +
                            LabG_MarginHeight(tb) + LabG_MarginTop(tb) +
                            ((tb->rectangle.height - LabG_MarginTop(tb) - LabG_MarginBottom(tb) -
                              (2 * (LabG_MarginHeight(tb) + tb->gadget.highlight_thickness +
                                    tb->gadget.shadow_thickness)) -
                              LabG_TextRect(tb).height) /
                             2);
  if (LayoutIsRtoLG(tb)) {
    if ((needed_width > tb->rectangle.width) ||
        ((LabG_Alignment(tb) == XmALIGNMENT_BEGINNING) && (needed_width < tb->rectangle.width)) ||
        ((LabG_Alignment(tb) == XmALIGNMENT_CENTER) && (needed_width < tb->rectangle.width) &&
        ...
        (needed_width == tb->rectangle.width))
    {
      if (TBG_IndOn(tb))
        LabG_TextRect(tb).x = tb->rectangle.x + tb->rectangle.width -
                              (tb->gadget.shadow_thickness + tb->gadget.highlight_thickness +
                               LabG_MarginHeight(tb) + LabG_MarginRight(tb) +
    ...
    {
      if (TBG_IndOn(tb))
        LabG_TextRect(tb).x = tb->gadget.shadow_thickness + tb->gadget.highlight_thickness +
                              LabG_MarginHeight(tb) + LabG_MarginLeft(tb);
    }
  }
}
                                 ...
                                 Cardinal *num_args) /* unused */
{
  ...
  /* assign if changed! */
  if (!_XmToggleBCacheCompare((XtPointer)TBG_Cache(new_w), (XtPointer)TBG_Cache(current)))
  { /* delete the old one */
    _XmCacheDelete((XtPointer)TBG_Cache(current));
    TBG_Cache(new_w) = (XmToggleButtonGCacheObjPart *)_XmCachePart(
        TBG_ClassCachePart(new_w),
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  /* Have to reset the TextRect width because label's resize will have
     mucked with it. */
  if (LabG_IsPixmap(newcbox) && (!IsNull(PixmapOff(newcbox)) || !IsNull(PixmapInsenOff(newcbox)) ||
  ...
  if ((LabG__label(newcbox) != LabG__label(curcbox)) ||
      (PixmapOff(newcbox) != PixmapOff(curcbox)) || (LabG_Font(newcbox) != LabG_Font(curcbox)) ||
      (TBG_Spacing(newcbox) != TBG_Spacing(curcbox)) || (PixmapOn(newcbox) != PixmapOn(curcbox)) ||
      ...
      (LabG_IsPixmapAndText(newcbox) != LabG_IsPixmapAndText(curcbox)))
  {
    ...
    if (LabG_IsPixmap(newcbox) && ((PixmapOn(newcbox) != PixmapOn(curcbox)) ||
                                   (PixmapInsenOn(newcbox) != PixmapInsenOn(curcbox)) ||
                                   (PixmapInd(newcbox) != PixmapInd(curcbox)) ||
                                   (PixmapInsenInd(newcbox) != PixmapInsenInd(curcbox))))
    {
    ...
    if (!TBG_IndicatorSet(newcbox) && (TBG_IndicatorDim(newcbox) == TBG_IndicatorDim(curcbox))) {
      if ((LabG_IsPixmap(newcbox) != LabG_IsPixmap(curcbox)) ||
          ...
          (LabG_Font(newcbox) != LabG_Font(curcbox)) || (TBG_IndOn(newcbox) != TBG_IndOn(curcbox)))
      {
        if (LabG_IsPixmap(newcbox) || LabG_IsPixmapAndText(newcbox)) {
          ...
          Dimension height;
          int line_count;
          height = XmStringHeight(LabG_Font(newcbox), LabG__label(newcbox));
    ...
    if (TBG_IndOn(newcbox)) {
      ...
      if (TBG_IndicatorDim(newcbox) != TBG_IndicatorDim(curcbox)) {
        ...
        int boxSize = ((int)LabG_TextRect(newcbox).height + (int)LabG_MarginTop(newcbox) +
                       (int)LabG_MarginBottom(newcbox));
        top_delta = bottom_delta = (maxIndicatorSize - boxSize) / 2;
      }
      if (LayoutIsRtoLG(newcbox))
        right_delta = (TBG_IndicatorDim(newcbox) + TBG_Spacing(newcbox) -
    ...
    if (newcbox->rectangle.width == 0) {
      newcbox->rectangle.width = LabG_TextRect(newcbox).width + LabG_MarginLeft(newcbox) +
                                 LabG_MarginRight(newcbox) +
    ...
    if (newcbox->rectangle.height == 0) {
      newcbox->rectangle.height = MAX(TBG_IndicatorDim(newcbox),
                                      LabG_TextRect(newcbox).height +
                                          2 * LabG_MarginHeight(newcbox) +
  ...
  if ((TBG_IndType(curcbox) != TBG_IndType(newcbox)) &&
      (!XmRepTypeValidValue(XmRID_INDICATOR_TYPE, TBG_IndType(newcbox), (Widget)newcbox)))
  {
  ...
  /* End of fix br 1251 */
  /* BEGIN OSF Fix pir 2097 */
  newcbox->gadget.event_mask = XmARM_EVENT | XmACTIVATE_EVENT | XmMULTI_ARM_EVENT |
                               XmMULTI_ACTIVATE_EVENT | XmHELP_EVENT | XmFOCUS_IN_EVENT |
                               XmFOCUS_OUT_EVENT | XmENTER_EVENT | XmLEAVE_EVENT | XmBDRAG_EVENT;
  /* END OSF Fix pir 2097 */
  return (flag);
 ...
 *
 ****************************************************************/
void XmToggleButtonGadgetSetState(Widget w, Boolean bnewstate, Boolean notify)
{
  XmToggleButtonGadget tg = (XmToggleButtonGadget)w;
  XmMenuSystemTrait menuSTrait;
  ...
  if (TBG_Set(tg) != newstate) {
    ...
    if (notify) {
      ...
      /* CR 8904: Notify value_changed before entry so that state is */
      /*	reported correctly even if the entry callback resets it. */
      menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(tg)),
                                                  XmQTmenuSystem);
      if (menuSTrait != NULL) {
        XmToggleButtonCallbackStruct call_value;
        call_value.reason = XmCR_VALUE_CHANGED;
...
Widget XmVaCreateToggleButtonGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmToggleButtonGadgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedToggleButtonGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmToggleButtonGadgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
 ...
 */
/*ARGSUSED*/
static Cardinal GetToggleBGClassSecResData(WidgetClass w_class, /* unused */
                                           XmSecondaryResourceData **data_rtn)
{
  int arrayCount = 0;
  ...
  arrayCount = _XmSecondaryResourceData(
      bcePtr, data_rtn, client_data, resource_name, resource_class, GetToggleBGClassSecResBase);
  _XmProcessUnlock();
  return (arrayCount);
}
 ...
 */
/*ARGSUSED*/
static XtPointer GetToggleBGClassSecResBase(Widget widget, XtPointer client_data) /* unused */
{
...
static void DrawToggleLabel(XmToggleButtonGadget tb)
{
  ...
  switch (tb->toggle.visual_set) {
    ...
    case XmINDETERMINATE: {
      ...
      XChangeGC(
          XtDisplay((Widget)tb), TBG_IndeterminateGC(tb), GCForeground | GCBackground, &values);
...
static void SetToggleSize(XmToggleButtonGadget newtbg)
{
  ...
  if (XtIsSensitive((Widget)newtbg)) {
    if (!IsNull(PixmapOn(newtbg))) {
      XmeGetPixmapData(
          XtScreen(newtbg), PixmapOn(newtbg), NULL, NULL, NULL, NULL, NULL, NULL, &tmpW, &tmpH);
      ASSIGN_MAX(maxW, tmpW);
      ASSIGN_MAX(maxH, tmpH);
    }
    if (!IsNull(PixmapOff(newtbg))) {
      XmeGetPixmapData(
          XtScreen(newtbg), PixmapOff(newtbg), NULL, NULL, NULL, NULL, NULL, NULL, &tmpW, &tmpH);
      ASSIGN_MAX(maxW, tmpW);
      ASSIGN_MAX(maxH, tmpH);
    }
    if (!IsNull(PixmapInd(newtbg))) {
      XmeGetPixmapData(
          XtScreen(newtbg), PixmapInd(newtbg), NULL, NULL, NULL, NULL, NULL, NULL, &tmpW, &tmpH);
      ASSIGN_MAX(maxW, tmpW);
      ASSIGN_MAX(maxH, tmpH);
    }
  }
    ...
    if (!IsNull(PixmapInsenOn(newtbg))) {
      XmeGetPixmapData(XtScreen(newtbg),
                       PixmapInsenOn(newtbg),
                       NULL,
                       NULL,
                       NULL,
                       NULL,
...
static void DefaultSelectColor(Widget widget, int offset, XrmValue *value)
{
  ...
  if (enable_toggle_color) {
    ...
    if (IsOneOfMany(TBG_IndType(tb))) {
      ...
      XtVaGetValues(XtParent(widget), XmNradioBehavior, &force_highlight, NULL);
    }
  }
  if (force_highlight) {
