...
#define GET_OPERATION(flags) \
  ((unsigned char)((int)((flags) & _XM_DND_OPERATION_MASK) >> _XM_DND_OPERATION_SHIFT))
#define PUT_OPERATION(operation) \
  (((CARD16)(operation) << _XM_DND_OPERATION_SHIFT) & _XM_DND_OPERATION_MASK)
#define GET_SITE_STATUS(flags) \
  ((unsigned char)((int)((flags) & _XM_DND_SITE_STATUS_MASK) >> _XM_DND_SITE_STATUS_SHIFT))
#define PUT_SITE_STATUS(site_status) \
  (((CARD16)(site_status) << _XM_DND_SITE_STATUS_SHIFT) & _XM_DND_SITE_STATUS_MASK)
#define GET_MULTIOPS(flags) \
  ((unsigned char)((int)((flags) & _XM_DND_MULTIOPS_MASK) >> _XM_DND_MULTIOPS_SHIFT))
#define PUT_MULTIOPS(operation) \
  (((CARD16)(operation) << _XM_DND_MULTIOPS_SHIFT) & _XM_DND_MULTIOPS_MASK)
#define GET_COMPLETION(flags) \
  ((unsigned char)((int)((flags) & _XM_DND_COMPLETION_MASK) >> _XM_DND_COMPLETION_SHIFT))
#define PUT_COMPLETION(completion) \
  (((CARD16)(completion) << _XM_DND_COMPLETION_SHIFT) & _XM_DND_COMPLETION_MASK)
#define GET_TRAVERSAL_TYPE(flags) \
  ((unsigned char)((int)((flags) & _XM_TRAVERSAL_TYPE_MASK) >> _XM_TRAVERSAL_TYPE_SHIFT))
#define PUT_TRAVERSAL_TYPE(traversal_type) \
  (((CARD16)(traversal_type) << _XM_TRAVERSAL_TYPE_SHIFT) & _XM_TRAVERSAL_TYPE_MASK)
#define GET_DS_TYPE(flags) \
  ((unsigned char)((int)((flags) & _XM_DS_TYPE_MASK) >> _XM_DS_TYPE_SHIFT))
#define PUT_DS_TYPE(ds_type) (((CARD16)(ds_type) << _XM_DS_TYPE_SHIFT) & _XM_DS_TYPE_MASK)
#define GET_DS_ACTIVITY(flags) \
  ((unsigned char)((int)((flags) & _XM_DS_ACTIVITY_MASK) >> _XM_DS_ACTIVITY_SHIFT))
#define PUT_DS_ACTIVITY(ds_activity) \
  (((CARD16)(ds_activity) << _XM_DS_ACTIVITY_SHIFT) & _XM_DS_ACTIVITY_MASK)
#define GET_ANIMATION_STYLE(flags) \
  ((unsigned char)((int)((flags) & _XM_ANIMATION_STYLE_MASK) >> _XM_ANIMATION_STYLE_SHIFT))
#define PUT_ANIMATION_STYLE(animation_style) \
  (((CARD16)(animation_style) << _XM_ANIMATION_STYLE_SHIFT) & _XM_ANIMATION_STYLE_MASK)
 ...
 * later.
 */
typedef enum { XmICC_INITIATOR_EVENT, XmICC_RECEIVER_EVENT } XmICCEventType;

#define _XM_ICC_EVENT_TYPE_MASK ((BYTE)0x80)
#define _XM_ICC_EVENT_TYPE_SHIFT 7
#define GET_ICC_EVENT_TYPE(type) \
  ((XmICCEventType)((int)((type) & _XM_ICC_EVENT_TYPE_MASK) >> _XM_ICC_EVENT_TYPE_SHIFT))
#define PUT_ICC_EVENT_TYPE(type) \
  (((BYTE)(type) << _XM_ICC_EVENT_TYPE_SHIFT) & _XM_ICC_EVENT_TYPE_MASK)
...
extern unsigned char _XmReasonToMessageType(int reason);
extern unsigned int _XmMessageTypeToReason(unsigned char messageType);
extern void _XmICCCallbackToICCEvent(Display *display,
                                     Window window,
                                     XmICCCallback callback,
                                     XClientMessageEvent *cmev,
                                     XmICCEventType type);
extern void _XmSendICCCallback(Display *display,
                               Window window,
                               Window src_window,
                               int protocol_style,
                               XmICCCallback callback,
                               XmICCEventType type);
extern Boolean _XmICCEventToICCCallback(XClientMessageEvent *msgEv,
                                        XmICCCallback callback,
                                        XmICCEventType type);
extern CARD16 _XmReadDragBuffer(xmPropertyBuffer propBuf, BYTE which, BYTE *ptr, CARD32 size);
extern CARD16 _XmWriteDragBuffer(xmPropertyBuffer propBuf, BYTE which, BYTE *ptr, CARD32 size);
extern void _XmWriteInitiatorInfo(Widget dc);
extern void _XmReadInitiatorInfo(Widget dc);
extern Boolean _XmGetDragReceiverInfo(Display *display,
                                      Window window,
                                      XmDragReceiverInfoStruct *receiverInfoRtn);
extern Boolean _XmReadDSFromStream(XmDropSiteManagerObject dsm,
                                   XtPointer iccInfo,
                                   XmICCDropSiteInfo dropSiteInfoRtn);
extern void _XmWriteDSToStream(XmDropSiteManagerObject dsm,
                               XtPointer stream,
                               XmICCDropSiteInfo dropSiteInfo);
extern void _XmFreeDragReceiverInfo(XtPointer info);
extern void _XmClearDragReceiverInfo(Widget shell);
extern void _XmSetDragReceiverInfo(XmDisplay dd, Widget shell);
