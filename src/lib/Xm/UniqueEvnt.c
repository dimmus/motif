...
static Time ExtractTime(XEvent *event);
static Boolean ManipulateEvent(XEvent *event, int action);
static XmUniqueStamp GetUniqueStamp(XEvent *event);
static void UniqueStampDisplayDestroyCallback(Widget w,
...
static Boolean Later(unsigned long recorded, unsigned long new_l)
{
  long normalizedNew;
                                              ...
                                              XtPointer call_data)   /* unused */
{
  ...
  if ((XmDisplay)NULL != xmDisplay) {
    XmUniqueStamp uniqueStamp =
        (XmUniqueStamp)((XmDisplayInfo *)(xmDisplay->display.displayInfo))->UniqueStamp;
    if ((XmUniqueStamp)NULL != uniqueStamp) {
