...
#include <Xm/VendorSEP.h>
    ...
    static void
    ShellClassPartInitialize(WidgetClass w);
...
externaldef(xmShellExtobjectclass)
 ...
 *
 ************************************************************************/
static void ShellClassPartInitialize(WidgetClass w)
{
  XmShellExtObjectClass wc = (XmShellExtObjectClass)w;
  XmShellExtObjectClass sc = (XmShellExtObjectClass)wc->object_class.superclass;
  if (wc == (XmShellExtObjectClass)xmShellExtObjectClass)
    return;
  if (wc->shell_class.structureNotifyHandler == XmInheritEventHandler)
    wc->shell_class.structureNotifyHandler = sc->shell_class.structureNotifyHandler;
}
                                   ...
                                   Boolean *continue_to_dispatch)
{
  ...
  switch (event->type) {
    ...
    case UnmapNotify:
       ...
       */
      if ((vePPtr->xAtMap != w->core.x) || (vePPtr->yAtMap != w->core.y)) {
        if (xmScreen->screen.mwmPresent) {
          if (vePPtr->lastOffsetSerial &&
              (vePPtr->lastOffsetSerial >= vendorExt->shell.lastConfigureRequest) &&
    ...
    case ConfigureNotify:
       ...
       */
      if (event->xany.serial < shellExt->shell.lastConfigureRequest) {
        /*
