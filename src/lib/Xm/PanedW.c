...
#define MajorMargin(pw) \
  (Major(pw, (pw)->paned_window.margin_width, (pw)->paned_window.margin_height))
#define MinorMargin(pw) \
  (Major(pw, (pw)->paned_window.margin_height, (pw)->paned_window.margin_width))
#define MajorSize(pw) (Major(pw, (pw)->core.width, (pw)->core.height))
#define MinorSize(pw) (Major(pw, (pw)->core.height, (pw)->core.width))
#define MajorChildSize(pw, child) (Major(pw, (child)->core.width, (child)->core.height))
#define MinorChildSize(pw, child) (Major(pw, (child)->core.height, (child)->core.width))
...
#define MajorAssign(pw, x, v) \
  if (Horizontal(pw)) \
    (x).width = v; \
  else \
    (x).height = v
#define MinorAssign(pw, x, v) \
  if (Horizontal(pw)) \
    (x).height = v; \
  else \
    (x).width = v
...
static void RefigureLocationsAndCommit(XmPanedWindowWidget pw,
                                       ...
                                       Direction dir,
                                       Boolean rflag);
static void DrawTrackLines(XmPanedWindowWidget pw);
static void EraseTrackLines(XmPanedWindowWidget pw);
...
static void SashIndentDefault(Widget widget, int offset, XrmValue *value);
static void ClassPartInitialize(WidgetClass wc);
static void Initialize(Widget request, Widget new_w, ArgList args, Cardinal *num_args);
static void ConstraintInit(Widget request, Widget new_w, ArgList args, Cardinal *num_args);
...
static void GetFlipGC(XmPanedWindowWidget pw);
...
externaldef(xmpanedwindowwidgetclass)
...
static void Initialize(Widget request, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (!XmRepTypeValidValue(XmRID_ORIENTATION, pw->paned_window.orientation, (Widget)pw)) {
...
static void ResetDMajors(XmPanedWindowWidget pw)
{
  Widget *childP;
  int i;
  for (i = 0, childP = pw->paned_window.managed_children; i < pw->paned_window.pane_count;
       childP++, i++)
    ...
    register XmPanedWindowWidget pw, int c_index, Direction dir, Boolean rflag, Boolean sflag)
{
  ...
  if (dir == FirstPane && c_index != num_panes - 1)
    childP++;
  cdir = _dir;
  ...
  for (pass = 0; sizeused != MajorSize(pw) && pass < (9 * num_panes); pass++) {
    ...
    if ((!pane->skip_adjust || sflag) || cdir != _dir) {
      ...
      else if (sizeused - MajorSize(pw) < PaneDMajor(*childP) &&
    ...
    if (sash) /* IF THIS IS NOT NULL */ {
      ...
      if (separator) {
        sepPos = MajorChildPos(pw, *childP) + MajorChildSize(pw, *childP) +
                 ...
                 MajorChildSize(pw, separator) / 2 - separator->core.border_width;
      ...
      if (pw->paned_window.sash_indent < 0)
        minor_dim = MinorSize(pw) + pw->paned_window.sash_indent - MinorChildSize(pw, sash) -
      ...
      major_dim = MajorChildPos(pw, *childP) + MajorChildSize(pw, *childP) +
                  2 * (*childP)->core.border_width + pw->paned_window.spacing / 2 -
                  MajorChildSize(pw, sash) / 2 - sash->core.border_width;
      ...
      if (XtIsRealized(pane->sash))
        XConfigureWindow(
            XtDisplay(pane->sash), XtWindow(pane->sash), CWX | CWY | CWStackMode, &changes);
    }
    else {
      if (*childP != instigator) {
                                        ...
                                        XtWidgetGeometry *reply)
{
  ...
  if (PanePosIndex(w) == XmLAST_POSITION) {
    ...
    if (IsX(request) && request->x >= 0)
      w->core.x = request->x;
    if (IsY(request) && request->y >= 0)
      w->core.y = request->y;
  ...
  if (XmIsSash(w)) {
    if ((mask & CWX) && request->x >= 0)
      w->core.x = request->x;
    if ((mask & CWY) && request->y >= 0)
      w->core.y = request->y;
  ...
  if ((mask & Minor(pw, CWWidth, CWHeight))) {
     ...
     */
    childP = children;
    while ((num_panes < pw->paned_window.num_managed_children) && XtIsRectObj(*childP) &&
           IsPane(*childP))
    {
      childP++;
      num_panes++;
    ...
    if (MinorReq(pw, request) >= childMinor) {
      MinorAssign(
          pw, geo_desired, MinorReq(pw, request) + 2 * (childBorderWidth + MinorMargin(pw)));
  ...
  /* now the conclusion:
    ...
    */
  if ((!is_almost) &&
      (((mask & CWWidth) && (allowed.width == request->width)) || (!(mask & CWWidth))) &&
      (((mask & CWHeight) && (allowed.height == request->height)) || (!(mask & CWHeight))))
  {
...
static void ChangeManaged(Widget w)
{
   ...
   */
  for (childP = pw->composite.children, i = 0; i < num_children; childP++, i++) {
    if (!IsPane(*childP))
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (!XmRepTypeValidValue(XmRID_ORIENTATION, newpw->paned_window.orientation, (Widget)newpw)) {
  ...
  if (oldpw->paned_window.sash_indent != newpw->paned_window.sash_indent ||
      ...
      oldpw->paned_window.sash_shadow_thickness != newpw->paned_window.sash_shadow_thickness ||
      ...
      if (IsPane(*childP)) {
        ...
        if (sash) /* IF THIS IS NOT NULL */ {
          ...
          if (newpw->paned_window.sash_indent < 0)
            minor_dim = MinorSize(newpw) + newpw->paned_window.sash_indent -
          ...
          major_dim = MajorChildPos(newpw, *childP) + MajorChildSize(newpw, *childP) +
                      2 * (*childP)->core.border_width + newpw->paned_window.spacing / 2 -
                      MajorChildSize(newpw, sash) / 2 - sash->core.border_width;
  ...
  if (oldpw->paned_window.margin_width != newpw->paned_window.margin_width) {
    newpw->core.width = newpw->core.width + ((2 * newpw->paned_window.margin_width) -
                                             (2 * oldpw->paned_window.margin_width));
    returnFlag = True;
  }
  if ((oldpw->paned_window.spacing != newpw->paned_window.spacing ||
       oldpw->paned_window.margin_height != newpw->paned_window.margin_height ||
       (requestpw->paned_window.refiguremode && !(oldpw->paned_window.refiguremode))) &&
    ...
    Widget old, Widget request, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (PanePosIndex(old) != PanePosIndex(new_w)) {
    ...
    if ((PanePosIndex(new_w) < 0) || (PanePosIndex(new_w) >= i)) {
      ...
      if ((current.x != XtX(new_w)) || (current.y != XtY(new_w)) ||
          (current.width != XtWidth(new_w)) || (current.height != XtHeight(new_w)) ||
          (current.border_width != XtBorderWidth(new_w)))
    ...
    if ((new_pane->min == new_pane->max) && (new_pane->sash != NULL)) {
      ...
      if (new_pane->position != count) {
        if (new_pane->separator == NULL) {
          pw->paned_window.recursively_called = True;
          new_pane->separator = XtCreateWidget(
              "separator", xmSeparatorGadgetClass, (Widget)pw, NULL, 0);
          pw->paned_window.recursively_called = False;
          if (XtIsRealized(new_w))
            XtRealizeWidget(new_pane->separator);
        }
        ...
        if (new_pane->min != new_pane->max) {
          if (new_pane->sash == NULL) {
            ...
            XtSetArg(sashargs[nargs], XmNshadowThickness, pw->paned_window.sash_shadow_thickness),
                nargs++;
            XtSetArg(sashargs[nargs], XmNunitType, (XtArgVal)XmPIXELS), nargs++;
            assert(nargs <= XtNumber(sashargs));
            pw->paned_window.recursively_called = True;
            new_pane->sash = XtCreateManagedWidget(
                "sash", xmSashWidgetClass, (Widget)pw, sashargs, nargs);
            XtAddCallback(new_pane->sash, XmNcallback, HandleSash, (XtPointer)new_w);
            pw->paned_window.recursively_called = False;
            if (XtIsRealized(new_w))
              XtRealizeWidget(new_pane->sash);
          }
        }
      }
    }
...
Widget XmVaCreatePanedWindow(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmPanedWindowWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedPanedWindow(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmPanedWindowWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
