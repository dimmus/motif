...
XmFontListEntry XmFontListEntryCreate(char *tag, XmFontType type, XtPointer font)
{
  ...
  _XmProcessLock();
  if ((font == NULL) || (tag == NULL) || ((type != XmFONT_IS_FONTSET) && (type != XmFONT_IS_FONT)))
  {
    _XmProcessUnlock();
    return (NULL);
  }
  if ((tag != XmFONTLIST_DEFAULT_TAG) && (strcmp(tag, XmSTRING_DEFAULT_CHARSET) == 0))
  ...
  n++;
  XtSetArg(args[n], XmNloadModel, XmLOAD_IMMEDIATE);
  n++;
  XtSetArg(args[n], XmNfont, font);
  n++;
  ret_val = XmRenditionCreate(NULL, _XmStringCacheTag(derived_tag, XmSTRING_TAG_STRLEN), args, n);
  _XmProcessUnlock();
  return ret_val;
}
...
XmFontListEntry XmFontListEntryCreate_r(char *tag, XmFontType type, XtPointer font, Widget wid)
{
  ...
  XtSetArg(args[n], XmNfont, font);
  n++;
  ret_val = XmRenditionCreate(wid, _XmStringCacheTag(derived_tag, XmSTRING_TAG_STRLEN), args, n);
  _XmAppUnlock(app);
  return ret_val;
}
...
XtPointer XmFontListEntryGetFont(XmFontListEntry entry, XmFontType *typeReturn)
{
  ...
  n++;
  XtSetArg(args[n], XmNfont, &ret_val);
  n++;
#if USE_XFT
  XtSetArg(args[n], XmNxftFont, &ret_val2);
  n++;
#endif
...
XmFontListEntry XmFontListNextEntry(XmFontContext context)
{
  ...
  if (FLContextIndex(context) >= _XmRTCount(FLContextTable(context))) {
    FLContextError(context) = TRUE;
    _XmProcessUnlock();
    return (NULL);
  }
  entry = _XmRTRenditions(FLContextTable(context))[FLContextIndex(context)];
...
XmFontList XmFontListRemoveEntry(XmFontList old, XmFontListEntry entry)
{
  ...
  n++;
  XtSetArg(args[n], XmNfontType, &type1);
  n++;
  XtSetArg(args[n], XmNfont, &font1);
  n++;
  XmRenditionRetrieve(entry, args, n);
...
XmFontListEntry XmFontListEntryLoad(Display *display, char *fontName, XmFontType type, char *tag)
{
  ...
  _XmAppLock(app);
  n = 0;
  XtSetArg(args[n], XmNfontName, fontName);
  n++;
  XtSetArg(args[n], XmNfontType, type);
  n++;
  XtSetArg(args[n], XmNloadModel, XmLOAD_IMMEDIATE);
  n++;
  ret_val = _XmRenditionCreate(
      display, NULL, XmS, XmCFontList, _XmStringCacheTag(tag, XmSTRING_TAG_STRLEN), args, n, NULL);
  _XmAppUnlock(app);
  return ret_val;
}
...
XmFontList XmFontListCreate(XFontStruct *font, XmStringCharSet charset)
{
  ...
  n++;
  XtSetArg(args[n], XmNfont, (XtPointer)font);
  n++;
  XtSetArg(args[n], XmNloadModel, XmLOAD_IMMEDIATE);
  n++;
...
XmFontList XmFontListCreate_r(XFontStruct *font, XmStringCharSet charset, Widget wid)
{
  ...
  XtSetArg(args[n], XmNloadModel, XmLOAD_IMMEDIATE);
  n++;
  rends[0] = XmRenditionCreate(wid, _XmStringCacheTag(curcharset, XmSTRING_TAG_STRLEN), args, n);
  ret_val = XmRenderTableAddRenditions(NULL, rends, 1, XmDUPLICATE);
  XmRenditionFree(rends[0]);
  _XmAppUnlock(app);
  return (ret_val);
...
XFontStruct *_XmGetFirstFont(XmFontListEntry entry)
{
  ...
  if (font == (XtPointer)XmAS_IS) {
    ...
    char **font_name_list;
    if (XFontsOfFontSet((XFontSet)font, &font_struct_list, &font_name_list) &&
 ...
 */
Boolean XmeRenderTableGetDefaultFont(XmFontList fontlist, XFontStruct **font_struct)
{
 ...
 */
Boolean _XmFontListSearch(XmFontList fontlist,
                          ...
                          XFontStruct **font_struct)
{
  ...
  Boolean success;
  success = _XmRenderTableFindFallback(fontlist, charset, FALSE, indx, &entry);
...
Boolean XmFontListGetNextFont(XmFontContext context, XmStringCharSet *charset, XFontStruct **font)
{
  ...
  XmStringTag tag;
  _XmProcessLock();
  if ((context == NULL) || FLContextError(context) || (charset == NULL) || (font == NULL)) {
    _XmProcessUnlock();
    return (FALSE);
  }
...
void _Xm_dump_fontlist(XmFontList f)
{
  int i = 0;
...
void _Xm_dump_fontlist_cache(void)
{
  FontlistEntry *cache;
