...
static void CleanupMenuBar(Widget wid, XEvent *event, String *param, Cardinal *num_param);
static void PopdownGrandChildren(XmRowColumnWidget rowcol);
static void Cascading(Widget w, XEvent *event);
static void Popup(Widget cb, XEvent *event);
static void size_cascade(XmCascadeButtonWidget cascadebtn);
...
static void GetArmGC(XmCascadeButtonWidget cb);
static void GetBackgroundGC(XmCascadeButtonWidget cb);
static void Initialize(Widget w_req, Widget w_new, ArgList args, Cardinal *num_args);
 ...
 */
externaldef(xmcascadebuttonwidgetclass)
...
static void BorderUnhighlight(Widget wid)
{
  ...
  Boolean popdown;
  if (Lab_IsMenupane(cb) && (((XmManagerWidget)XtParent(cb))->manager.active_child == wid) &&
      CB_Submenu(cb))
  {
    XmMenuShellWidget mshell = (XmMenuShellWidget)XtParent(CB_Submenu(cb));
    if ((mshell->composite.children[0] == CB_Submenu(cb)) && (XmIsMenuShell(mshell)) &&
  ...
  if (CB_IsArmed(cb)) {
    if (XtIsRealized((Widget)cb)) {
      XmeDrawShadows(XtDisplay(cb),
                     ...
                     cb->primitive.highlight_thickness,
                     cb->core.width - 2 * cb->primitive.highlight_thickness,
                     cb->core.height - 2 * cb->primitive.highlight_thickness,
...
static void Redisplay(register Widget cb, XEvent *event, Region region)
{
  ...
  if (XtIsRealized(cb)) {
    ...
    if (etched_in) {
      if (CB_IsArmed(cb))
        XFillRectangle(
            XtDisplay(cb), XtWindow(cb), CB_ArmGC(cb), 0, 0, cb->core.width, cb->core.height);
      else
  ...
  switch (Lab_MenuType(cb)) {
    case XmMENU_BAR: {
      ...
      if (myShell && XmIsMenuShell(myShell) && /* not torn ?! */
          (myShell->shell.popped_up) && (myShell->composite.children[0] == CB_Submenu(cb)) &&
...
{
  XmCascadeButtonWidget cb = (XmCascadeButtonWidget)wid;
  if ((!CB_IsArmed(cb)) && (((XmMenuShellWidget)XtParent(XtParent(cb)))->shell.popped_up) &&
...
static void StartDrag(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  ...
  XmRowColumnWidget parent = (XmRowColumnWidget)XtParent(cb);
  XmMenuSystemTrait menuSTrait;
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass((Widget)parent), XmQTmenuSystem);
...
static void KeySelect(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  ...
  if (!_XmGetInDragMode((Widget)cb) &&
      (RC_IsArmed(XtParent(cb)) ||
       (RC_Type(XtParent(cb)) != XmMENU_BAR && !XmIsMenuShell(XtParent(XtParent(cb))))))
    (*(((XmCascadeButtonClassRec *)(cb->core.widget_class))->primitive_class.arm_and_activate))(
    ...
    if (validButton) {
      ...
      if (!CB_Submenu(cb)) {
        ...
        if (RC_BeingArmed(XtParent(cb))) {
          _XmGrabPointer(XtParent(cb),
                         True,
                         EVENTS,
                         GrabModeAsync,
                         GrabModeAsync,
                         None,
                         _XmGetMenuCursorByScreen(XtScreen(cb)),
                         _time);
...
static void CleanupMenuBar(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  ...
  XmRowColumnWidget parent = (XmRowColumnWidget)XtParent(cb);
  if (RC_IsArmed(parent)) {
    (*((XmRowColumnWidgetClass)XtClass(parent))->row_column_class.armAndActivate)(
        (Widget)parent, (XEvent *)NULL, (String *)NULL, (Cardinal *)NULL);
    _XmRecordEvent(event);
  }
}
...
void _XmCBHelp(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (RC_Type(parent) == XmMENU_BAR) {
    ...
    (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownDone))(
        XtParent(parent), event, params, num_params);
  }
  if (XmIsGadget(w))
...
static void PopdownGrandChildren(XmRowColumnWidget rowcol)
{
  ...
  if ((menuShell = (CompositeWidget)RC_PopupPosted(rowcol)) == NULL)
    return;
  if ((menuShell = (CompositeWidget)RC_PopupPosted(menuShell->composite.children[0])) != NULL) {
    (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(
        (Widget)menuShell, NULL, NULL, NULL);
  }
}
...
static void Cascading(Widget w, XEvent *event)
{
  ...
  if (XmIsCascadeButton(w)) {
    ...
    if (submenu) {
      ...
      if (XmIsMenuShell(ms) && ms->shell.popped_up &&
  ...
  if (*(excPP->pane)) {
    ...
    if (RC_TornOff(*(excPP->pane)) && !XmIsMenuShell(XtParent(*(excPP->pane)))) {
      ...
      if (RC_PopupPosted(XtParent(cb))) {
        XmRowColumnWidget postedPane = (XmRowColumnWidget)((CompositeWidget)RC_PopupPosted(
                                                               XtParent(cb)))
                                           ->composite.children[0];
        if ((Widget)postedPane != *(excPP->pane)) {
          _XmLowerTearOffObscuringPoppingDownPanes((Widget)postedPane, *(excPP->pane));
        }
      }
...
static void Popup(Widget cb, XEvent *event)
{
  ...
  if (submenu && (shell = (XmMenuShellWidget)XtParent(submenu)) && XmIsMenuShell(shell) &&
  ...
  if (XtIsManaged((Widget)parent)) {
    ...
    if (oldActiveChild && (oldActiveChild != (Widget)cb) &&
        XmIsCascadeButtonGadget(oldActiveChild) && CBG_Submenu(oldActiveChild) &&
        (((XmMenuShellWidget)XtParent(CBG_Submenu(oldActiveChild)))->shell.popped_up == False))
    {
       ...
       */
      if (!submenu || !popped_up || (RC_PopupPosted(parent) != (Widget)shell) ||
          (submenu && RC_CascadeBtn(submenu) && (RC_CascadeBtn(submenu) != cb) &&
           ((Widget)parent == XtParent(RC_CascadeBtn(submenu)))))
      {
         ...
         */
        if (NULL != oldActiveChild && oldActiveChild != cb &&
    ...
    *(excPP->pane) = NULL;
    ...
    if (submenu) {
      if (((ShellWidget)XtParent(submenu))->composite.num_children == 1) {
         ...
         */
        (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popupSharedMenupane))(
            cb, (Widget)submenu, event);
      }
...
static void position_cascade(XmCascadeButtonWidget cascadebtn)
{
  ...
  if (CB_HasCascade(cascadebtn)) {
    if (LayoutIsRtoLP(cascadebtn))
      ...
      CB_Cascade_x(cascadebtn) = XtWidth(cascadebtn) - cascadebtn->primitive.highlight_thickness -
                                 cascadebtn->primitive.shadow_thickness -
                                 Lab_MarginWidth(cascadebtn) - CB_Cascade_width(cascadebtn);
    buffer = cascadebtn->primitive.highlight_thickness + cascadebtn->primitive.shadow_thickness +
             Lab_MarginHeight(cascadebtn);
    CB_Cascade_y(cascadebtn) = buffer + ((XtHeight(cascadebtn) - 2 * buffer) -
                                         CB_Cascade_height(cascadebtn)) /
                                            2;
  }
  else {
                          ...
                          Boolean adjustHeight)
{
  ...
  if (CB_HasCascade(cascadebtn)) {
    ...
    delta = CB_Cascade_height(cascadebtn) +
            2 * (Lab_MarginHeight(cascadebtn) + cascadebtn->primitive.shadow_thickness +
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if ((CB_Submenu(new_w)) &&
      ((!XmIsRowColumn(CB_Submenu(new_w))) || (RC_Type(CB_Submenu(new_w)) != XmMENU_PULLDOWN)))
  {
  ...
  /* handle the cascade pixmap indicator */
    ...
    else if (((CB_CascadePixmap(new_w) == XmUNSPECIFIED_PIXMAP) &&
    ...
    /* make sure that other changes did not scrunch our pixmap */
      ...
      if ((new_w->primitive.highlight_thickness != old->primitive.highlight_thickness) ||
          (new_w->primitive.shadow_thickness != old->primitive.shadow_thickness) ||
          ...
          (Lab_MarginBottom(new_w) != Lab_MarginBottom(old)))
      {
        ...
        flag = TRUE;
               ...
               (new_w->core.width != old->core.width) || (new_w->core.height != old->core.height))
      {
 ...
 */
/*ARGSUSED*/
static void Initialize(Widget w_req,
                       Widget w_new,
                       ArgList args,       /* unused */
                       Cardinal *num_args) /* unused */
{
  ...
  if ((!XmIsRowColumn(parent)) &&
      ((Lab_MenuType(new_w) == XmMENU_PULLDOWN) || (Lab_MenuType(new_w) == XmMENU_POPUP) ||
...
Widget XmVaCreateCascadeButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmCascadeButtonWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedCascadeButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmCascadeButtonWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
 ...
 */
void XmCascadeButtonHighlight(Widget cb, Boolean highlight)
{
  XtAppContext app;
  if (NULL == cb)
    return;
