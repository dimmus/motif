...
static void GetDefaultThresholdsForScreen(Screen *screen);
...
static Pixel GetWhitePixel(Screen *screen, Colormap colormap, XColor whitecolor);
static void SetMonochromeColors(XmColorData *colors);
static int Brightness(XColor *color);
...
static void CalculateColorsRGB(
    XColor *bg_color, XColor *fg_color, XColor *sel_color, XColor *ts_color, XColor *bs_color);
static Pixel AccessColorData(XmColorData *cd, unsigned char which);
...
static XColor *GetDefaultBackgroundColor(Screen *screen, Colormap color_map)
{
  ...
  if (XrmQGetResource(XtScreenDatabase(screen), names, classes, &rep, &db_value)) {
    if (rep == XrmPermStringToQuark(XmRString)) {
      if (!XParseColor(DisplayOfScreen(screen), color_map, db_value.addr, &color))
        return NULL;
    }
    ...
    if (!XParseColor(DisplayOfScreen(screen), color_map, XmDEFAULT_BACKGROUND, &color))
      return NULL;
  }
  return &color;
}
...
static XmColorData *GetDefaultColors(Screen *screen, Colormap color_map)
{
  ...
  if (DefaultDepthOfScreen(screen) == 1) {
    ...
    if ((color_def = GetDefaultBackgroundColor(screen, color_map)) != NULL) {
      XmAllocColorProc aproc = _XmGetColorAllocationProc(screen);
      if (aproc == NULL)
        aproc = DEFAULT_ALLOCCOLOR_PROC;
      if ((*aproc)(DisplayOfScreen(screen), color_map, color_def)) {
   ...
   */
  default_set[default_set_count] = *GetColors(screen, color_map, background);
  default_set_count++;
  ...
  switch (which) {
    case XmBACKGROUND:
      if (!(cd->allocated & which) &&
          ((*aproc)(cd->screen->display, cd->color_map, &(cd->background)) == 0))
      {
        if (Brightness(&(cd->background)) < XmFOREGROUND_THRESHOLD)
          cd->background.pixel = GetBlackPixel(cd->screen, cd->color_map, cd->background);
        else
          cd->background.pixel = GetWhitePixel(cd->screen, cd->color_map, cd->background);
    ...
    case XmFOREGROUND:
      if (!(cd->allocated & which) &&
          ((*aproc)(cd->screen->display, cd->color_map, &(cd->foreground)) == 0))
      {
        if (Brightness(&(cd->background)) < XmFOREGROUND_THRESHOLD)
          cd->foreground.pixel = GetWhitePixel(cd->screen, cd->color_map, cd->foreground);
        else
          cd->foreground.pixel = GetBlackPixel(cd->screen, cd->color_map, cd->foreground);
    ...
    case XmTOP_SHADOW:
      if (!(cd->allocated & which) &&
          ((*aproc)(cd->screen->display, cd->color_map, &(cd->top_shadow)) == 0))
      {
        if (Brightness(&(cd->background)) > XmCOLOR_LITE_THRESHOLD)
          cd->top_shadow.pixel = GetBlackPixel(cd->screen, cd->color_map, cd->top_shadow);
        else
          cd->top_shadow.pixel = GetWhitePixel(cd->screen, cd->color_map, cd->top_shadow);
    ...
    case XmBOTTOM_SHADOW:
      if (!(cd->allocated & which) &&
          ((*aproc)(cd->screen->display, cd->color_map, &(cd->bottom_shadow)) == 0))
      {
        if (Brightness(&(cd->background)) < XmCOLOR_DARK_THRESHOLD)
          cd->bottom_shadow.pixel = GetWhitePixel(cd->screen, cd->color_map, cd->bottom_shadow);
        else
          cd->bottom_shadow.pixel = GetBlackPixel(cd->screen, cd->color_map, cd->bottom_shadow);
    ...
    case XmSELECT:
      if (!(cd->allocated & which) &&
          ((*aproc)(cd->screen->display, cd->color_map, &(cd->select)) == 0))
      {
        if (Brightness(&(cd->background)) < XmFOREGROUND_THRESHOLD)
          cd->select.pixel = GetWhitePixel(cd->screen, cd->color_map, cd->select);
        else
          cd->select.pixel = GetBlackPixel(cd->screen, cd->color_map, cd->select);
    ...
    XColor *bg_color, XColor *fg_color, XColor *sel_color, XColor *ts_color, XColor *bs_color)
{
  ...
  if (sel_color) {
    f = XmCOLOR_LO_SEL_FACTOR +
        (brightness * (XmCOLOR_HI_SEL_FACTOR - XmCOLOR_LO_SEL_FACTOR) / XmMAX_SHORT);
    color_value = bg_color->red;
  ...
  if (bs_color) {
    f = XmCOLOR_LO_BS_FACTOR +
        (brightness * (XmCOLOR_HI_BS_FACTOR - XmCOLOR_LO_BS_FACTOR) / XmMAX_SHORT);
    color_value = bg_color->red;
    color_value -= (color_value * f) / 100;
    bs_color->red = color_value;
    color_value = bg_color->green;
  ...
  if (ts_color) {
    f = XmCOLOR_LO_TS_FACTOR +
        (brightness * (XmCOLOR_HI_TS_FACTOR - XmCOLOR_LO_TS_FACTOR) / XmMAX_SHORT);
    color_value = bg_color->red;
 ...
 *********************************************************************/
/*ARGSUSED*/
void XmeGetDefaultPixel(Widget widget, int type, int offset, XrmValue *value)
{
  Screen *screen;
  Colormap color_map;
  ...
  if (!XtIsWidget(widget)) {
    ...
    if (type != XmBACKGROUND) {
      if ((XmIsLabelGadget(widget)) || (XmIsArrowButtonGadget(widget)) ||
...
Pixmap _XmConvertToBW(Widget w, Pixmap pm)
{
  XpmImage im;
  int i = 0;
