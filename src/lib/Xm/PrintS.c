...
static void PrintNotifyHandler(Widget w,
                               XtPointer client_data,
                               XEvent *event,
                               Boolean *continue_to_dispatch);
static void ResourcesUpdate(Widget w, XPContext pcontext, Dimension *pwidth, Dimension *pheight);
static void AttributesNotifyHandler(Widget w,
                                    XtPointer client_data,
                                    XEvent *event,
                                    Boolean *continue_to_dispatch);
static void SelectXpEvents(
    Widget widget, int *event_types, XtPointer *select_data, int count, XtPointer client_data);
...
static void FinishProc(Display *display,
                       ...
                       XPointer client_data);
static void PDMPhase2Handler(Widget w, XtPointer data, XEvent *event, Boolean *cont);
...
static XtResource resources[] = {
    {
        ...
        XmRBoolean,
        sizeof(Boolean),
        XtOffsetOf(XmPrintShellRec, shell.allow_shell_resize),
        XtRImmediate,
        (XtPointer)False,
    },
    {
    ...
    },
    {
        ...
        Offset(default_pixmap_resolution),
        XmRImmediate,
        (XtPointer)100,
    },
    {
...
#undef Offset
externaldef(xmprintshellclassrec) XmPrintShellClassRec xmPrintShellClassRec = {
    {
                               ...
                               Boolean *continue_to_dispatch)
{
  ...
  if (!print_shell->print.start_job_callback && !print_shell->print.page_setup_callback &&
      !print_shell->print.end_job_callback)
    return;
  pr_cbs.event = event;
  ...
  switch (xpev->detail) {
    case XPStartJobNotify:
      ...
      XtCallCallbackList(w, print_shell->print.start_job_callback, &pr_cbs);
    ...
    case XPEndPageNotify:
      ...
      if (!print_shell->print.last_page) {
        ...
        pr_cbs.last_page = False;
        XtCallCallbackList(w, print_shell->print.page_setup_callback, &pr_cbs);
    ...
    case XPEndJobNotify:
      ...
      XtCallCallbackList(w, print_shell->print.end_job_callback, &pr_cbs);
      break;
  }
}
...
static void ResourcesUpdate(Widget w, XPContext pcontext, Dimension *pwidth, Dimension *pheight)
{
  ...
  string_resolution = XpGetOneAttribute(
      XtDisplay(w), pcontext, XPDocAttr, "default-printer-resolution");
  ...
  if (!print_shell->print.print_resolution)
    ...
    print_shell->print.print_resolution = (254 * WidthOfScreen(XtScreen(w)) +
                                           5 * WidthMMOfScreen(XtScreen(w))) /
                                          (10 * WidthMMOfScreen(XtScreen(w)));
  /* get our size from the print server now */
  XpGetPageDimensions(XtDisplay(w), pcontext, pwidth, pheight, &imageable_area);
...
static void SelectXpEvents(
    Widget widget, int *event_types, XtPointer *select_data, int count, XtPointer client_data)
{
  XpSelectInput(XtDisplay(widget), XpGetContext(XtDisplay(widget)), XPPrintMask | XPAttributeMask);
}
...
static void GetFromTable(Display *pdpy, XPContext context, Widget *widget)
{
  XFindContext(pdpy, (XID)context, PrintContextToWidget, (XPointer *)widget);
}
...
static void AddToTable(XPContext context, Widget widget)
{
  XSaveContext(XtDisplay(widget), (XID)context, PrintContextToWidget, (XPointer)widget);
}
...
static void Initialize(Widget req_widget, Widget new_widget, ArgList args, Cardinal *num_args)
{
  ...
  XSaveContext(XtDisplay(new_widget),
               (XID)XtScreen(new_widget),
  ...
  if (!XpQueryExtension(XtDisplay(new_widget), &event_base_return, &error_base_return)) {
    return;
  }
  ...
  if (!pcontext || XpGetScreenOfContext(XtDisplay(new_widget), pcontext) != XtScreen(new_widget)) {
    return;
  }
  ...
  (void)XtSetEventDispatcher(
      XtDisplay(new_widget), event_base_return + XPPrintNotify, PrintDispatchEvent);
  (void)XtSetEventDispatcher(
      XtDisplay(new_widget), event_base_return + XPAttributeNotify, PrintDispatchEvent);
  /* go and get resolution and initial sizes */
  ResourcesUpdate(new_widget, pcontext, &(new_widget->core.width), &(new_widget->core.height));
}
...
static void Destroy(Widget w)
{
  ...
  XDeleteContext(XtDisplay(w), (XID)XtScreen(w), _XmPrintScreenToShellContext);
                    ...
                    Cardinal num_args)
{
  ...
  if (!video_widget)
    return NULL;
  XtGetApplicationNameAndClass(XtDisplay(video_widget), &video_name, &video_class);
  /* we want to be able to specify things like:
       ...
       named "print".
       */
  pappshell = XtVaAppCreateShell(video_name,
                                 video_class,
  ...
  print_shell = XtCreatePopupShell(
      print_shell_name, xmPrintShellWidgetClass, pappshell, args, num_args);
  /* Add callback to destroy application Shell parent. */
  XtAddCallback(print_shell, XmNdestroyCallback, _XmDestroyParentCallback, (XtPointer)NULL);
...
static void FilePipeCB(XtPointer client_data, int *source, XtInputId *id)
{
  ...
  close(cb->pipe);
  if (cb->finish_proc) {
    (*cb->finish_proc)(cb->display, XpGetContext(cb->display), status, cb->client_data);
  }
  XtFree((char *)cb);
}
                             ...
                             String application_class)
{
  ...
  if ((display = XtOpenDisplay(app_context,
                               display_name,
                               application_name,
                               application_class,
                               NULL,
  ...
  XpGetDocumentData(display, pcontext, PrintToFileProc, FinishProc, (XPointer)file_desc);
                     ...
                     XPointer client_data)
{
  ...
  XFlush(pdpy);
  XtGetApplicationNameAndClass(pdpy, &application_name, &application_class);
  display_name = XDisplayString(pdpy);
  pcontext = XpGetContext(pdpy);
  pid = fork();
  if (pid == 0) {
    ChildPrintToFile(
        display_name, pcontext, file, file_name, filedes[1], application_name, application_class);
  }
  else if (pid < 0) {
    fclose(file);
    return False;
  ...
  callback->input_id = XtAppAddInput(XtDisplayToApplicationContext(pdpy),
                                     callback->pipe,
...
static void PDMPhase2Handler(Widget w, XtPointer data, XEvent *event, Boolean *cont)
{
  enum { XmAPDM_REPLY, XmAPDM_EXIT_OK, XmAPDM_EXIT_ERROR, XmAPDM_EXIT_CANCEL, NUM_ATOMS };
  ...
  XInternAtoms(XtDisplay(print_shell), atom_names, XtNumber(atom_names), False, atoms);
  if (xevent->type == ClientMessage && xevent->message_type == atoms[XmAPDM_REPLY]) {
    /* check for the exit status of the PDM */
    if (xevent->data.l[0] == atoms[XmAPDM_EXIT_OK])
      pr_cbs.reason = XmCR_PDM_OK;
    else if (xevent->data.l[0] == atoms[XmAPDM_EXIT_CANCEL])
      pr_cbs.reason = XmCR_PDM_CANCEL;
    else if (xevent->data.l[0] == atoms[XmAPDM_EXIT_ERROR])
      pr_cbs.reason = XmCR_PDM_EXIT_ERROR;
    /* some error message might have been logged */
    XtCallCallbackList(w, print_shell->print.pdm_notification_callback, &pr_cbs);
  }
  /* remove me */
                             ...
                             int *format)
{
  ...
  static char *atom_names[] = {
      XmIPDM_START_OK, XmIPDM_START_ERROR, XmIPDM_START_VXAUTH, XmIPDM_START_PXAUTH};
  XmPrintShellCallbackStruct pr_cbs;
  PDMSelectData *pdm_select_data = (PDMSelectData *)client_data;
  Atom atoms[XtNumber(atom_names)];
  assert(XtNumber(atom_names) == NUM_ATOMS);
  XInternAtoms(
      XtDisplay(pdm_select_data->print_shell), atom_names, XtNumber(atom_names), False, atoms);
  ...
  if (!value) {
    ...
    else if (*(Atom *)value == atoms[XmAPDM_START_ERROR])
      pr_cbs.reason = XmCR_PDM_START_ERROR;
    else if (*(Atom *)value == atoms[XmAPDM_START_VXAUTH])
      pr_cbs.reason = XmCR_PDM_START_VXAUTH;
    else if (*(Atom *)value == atoms[XmAPDM_START_PXAUTH])
  ...
  XtCallCallbackList((Widget)pdm_select_data->print_shell,
                     pdm_select_data->print_shell->print.pdm_notification_callback,
                     &pr_cbs);
  XtFree((char *)pdm_select_data);
}

XtEnum XmPrintPopupPDM(Widget print_shell, Widget transient_for_video_shell)
{
  Atom pdm_selection;
  Atom type;
  unsigned char *value;
  ...
  else if (display_used == XtDisplay(transient_for_video_shell))
    widget_for_selection = transient_for_video_shell;
  else
    return XmPDM_NOTIFY_FAIL;
  XtSetSelectionParameters(
      widget_for_selection, pdm_selection, type, (XtPointer)value, length, format);
  XFree(value);
  ...
  PDM_START = XInternAtom(XtDisplay(widget_for_selection), XmIPDM_START, False);
...
#define REALLY_LONG_TIMEOUT (2 * 60 * 1000)
  ...
  XtGetSelectionValue(widget_for_selection,
                      ...
                      XtLastTimestampProcessed(XtDisplay(widget_for_selection)));
  ...
  pdm_select_data->transient_for_input_only_window = XCreateWindow(
      XtDisplay(transient_for_video_shell),
      XtWindow(transient_for_video_shell),
      0,
      0,
