...
static XmToolTipConfigTrait ToolTipGetData(Widget w)
{
  Widget top = w;
  ...
  if (ttp != NULL && ttp->label == NULL && !top->core.being_destroyed) {
    Widget shell;
    shell = XtVaCreateWidget(
        "TipShell", transientShellWidgetClass, top, XmNoverrideRedirect, True, NULL);
...
static void ToolTipUnpost(XtPointer client_data, XtIntervalId *id)
{
  XmToolTipConfigTrait ttp;
...
static void ToolTipPostFinish(Widget slide, XtPointer client_data, XtPointer call_data)
{
  XmToolTipConfigTrait ttp;
  ttp = (XmToolTipConfigTrait)client_data;
  if (ttp == NULL) {
  ...
  if (ttp->post_duration > 0) {
    ttp->duration_timer = XtAppAddTimeOut(XtWidgetToApplicationContext(ttp->label),
...
static void ToolTipPost(XtPointer client_data, XtIntervalId *id)
{
  ...
  XQueryPointer(XtDisplay(w), XtWindow(w), &root, &child, &rx, &ry, &x, &y, &key);
  if (ttp->duration_timer != (XtIntervalId)NULL) {
    XtRemoveTimeOut(ttp->duration_timer);
    ttp->duration_timer = (XtIntervalId)NULL;
  }
  if (XmIsPrimitive(w) || XmIsGadget(w)) {
    XtVaSetValues(ttp->label, XmNlabelString, XmGetToolTipString(w), NULL);
  }
  else {
    XmString string;
  ...
  XtVaSetValues(XtParent(ttp->label), XmNx, rx + 1, XmNy, ry + 1, XmNwidth, 1, XmNheight, 1, NULL);
  ttp->slider = XtVaCreateWidget("ToolTipSlider",
                                 ...
                                 geo.height,
                                 NULL);
  XtAddCallback(ttp->slider, XmNslideFinishCallback, (XtCallbackProc)ToolTipPostFinish, ttp);
...
void _XmToolTipEnter(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if ((XmIsPrimitive(wid) || XmIsGadget(wid)) && XmGetToolTipString(wid)) {
    if (ttp->enable && !ttp->timer) {
      ...
      ttp->timer = XtAppAddTimeOut(
          XtWidgetToApplicationContext(wid), delay, (XtTimerCallbackProc)ToolTipPost, wid);
    }
  }
}

void _XmToolTipLeave(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
...
Widget XmToolTipGetLabel(Widget wid)
{
...
void XmSetToolTipString(Widget w, XmString s)
{
  XmToolTipTrait ttp;
  ttp = (XmToolTipTrait)XmeTraitGet(w, XmQTtoolTip);
  if (!ttp) {
