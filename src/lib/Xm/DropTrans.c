...
static void SourceNotifiedCB(Widget w,
                             ...
                             int *format);
static void TerminateTransfer(XmDropTransferObject dt, Atom *selection);
static void ProcessTransferEntry(XmDropTransferObject dt, Cardinal which);
static void DropTransferSelectionCB(Widget w,
                                    XtPointer client_data,
                                    Atom *selection,
                                    Atom *type,
                                    XtPointer value,
                                    unsigned long *length,
                                    int *format);
static Widget StartDropTransfer(Widget refWidget, ArgList args, Cardinal argCount);
static void AddDropTransfer(Widget widget, XmDropTransferEntry transfers, Cardinal num_transfers);
...
static XtResource resources[] = {
    {XmNdropTransfers,
     XmCDropTransfers,
     XmRDropTransfers,
     sizeof(XmDropTransferEntry),
     XtOffsetOf(struct _XmDropTransferRec, dropTransfer.drop_transfers),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNnumDropTransfers,
     XmCNumDropTransfers,
     XmRCardinal,
     sizeof(Cardinal),
     XtOffsetOf(struct _XmDropTransferRec, dropTransfer.num_drop_transfers),
     XmRImmediate,
     (XtPointer)0},
    {XmNincremental,
     XmCIncremental,
    ...
    {XmNtransferStatus,
     ...
     XmRImmediate,
     (XtPointer)XmTRANSFER_SUCCESS},
};
/*  class record definition  */
externaldef(xmgadgetclassrec) XmDropTransferClassRec xmDropTransferClassRec = {
    {
...
externaldef(xmdroptransferobjectclass)
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  dtp->motif_drop_atom = XInternAtom(XtDisplayOfObject(nw), XmS_MOTIF_DROP, False);
...
static void Destroy(Widget w)
{
   ...
   */
  dc = (XmDragContext)XmGetDragContext((Widget)new_w, new_w->dropTransfer.timestamp);
  if (dc && dc->drag.sourceIsExternal)
    XtDestroyWidget((Widget)dc);
  for (i = 0; i < new_w->dropTransfer.num_drop_transfer_lists; i++) {
    XmDropTransferList currEntry = &(new_w->dropTransfer.drop_transfer_lists[i]);
...
{
  Atom status;
  XmDropTransferPart *dtp = (XmDropTransferPart *)&(dt->dropTransfer);
  XmDragContext dc = (XmDragContext)dtp->dragContext;
  if (dtp->transfer_status == XmTRANSFER_SUCCESS)
    status = XInternAtom(XtDisplayOfObject((Widget)dt), XmSTRANSFER_SUCCESS, False);
  else /* XmTRANSFER_FAILURE */
    status = XInternAtom(XtDisplayOfObject((Widget)dt), XmSTRANSFER_FAILURE, False);
  /*
...
static void ProcessTransferEntry(XmDropTransferObject dt, Cardinal which)
{
  ...
  if (dtp->incremental) {
    if (tl->num_transfers == 1) {
      XtGetSelectionValueIncremental(dc->drag.currReceiverInfo->shell,
                                     real_selection_atom,
                                     dtp->cur_targets[0],
      ...
      XtGetSelectionValuesIncremental(dc->drag.currReceiverInfo->shell,
                                      real_selection_atom,
                                      dtp->cur_targets,
                                      tl->num_transfers,
                                      DropTransferSelectionCB,
                                    ...
                                    int *format)
{
  XmDropTransferObject dt = (XmDropTransferObject)client_data;
  XmDropTransferPart *dtp = (XmDropTransferPart *)&(dt->dropTransfer);
  XmDropTransferList tl = &(dtp->drop_transfer_lists[dtp->cur_drop_transfer_list]);
  (*(dtp->transfer_callback))((Widget)dt,
                              tl->transfer_list[dtp->cur_xfer].client_data,
                              selection,
                              type,
                              value,
                              length,
  ...
  if (!(dtp->incremental) || ((dtp->incremental) && (value != NULL) && (*length == 0))) {
    dtp->cur_xfer++;
    if (dtp->cur_xfer == tl->num_transfers) {
      XtFree((char *)dtp->cur_targets);
      XtFree((char *)dtp->cur_client_data);
      if (++(dtp->cur_drop_transfer_list) < dtp->num_drop_transfer_lists) {
...
static void StartDropTimer(XtPointer clientData, XtIntervalId *id)
{
  ...
  if (dtp->num_drop_transfer_lists) {
    ...
    i = 0;
    XtSetArg(my_args[i], XmNiccHandle, &selection);
    i++;
...
static Widget StartDropTransfer(Widget refWidget, ArgList args, Cardinal argCount)
{
  ...
  dt->dropTransfer.timestamp = ((XmDragContext)refWidget)->drag.dragFinishTime;
  /*
  ...
  timer = XtAppAddTimeOut(
      XtWidgetToApplicationContext((Widget)dt), 0, StartDropTimer, (XtPointer)dt);
#ifdef CR1146
  if (isValidStartDropTimerId) {
    printf(
        "Motif Error in file %s at line %d: A DropTimer is already active,\n", __FILE__, __LINE__);
  }
  isValidStartDropTimerId = 1;
#endif
  XtAddCallback(refWidget, XmNdestroyCallback, DragContextDestroyCB, (XtPointer)timer);
  return ((Widget)dt);
}
...
Widget XmDropTransferStart(Widget refWidget, ArgList args, Cardinal argCount)
{
  ...
  _XmAppLock(app);
  n = 0;
  XtSetArg(lclArgs[n], XmNdropTransferClass, &wc);
  n++;
  XtGetValues(ddo, lclArgs, n);
  ret_val = (*(wc->dropTransfer_class.start_drop_transfer))(refWidget, args, argCount);
  _XmAppUnlock(app);
  return ret_val;
}
...
void XmDropTransferAdd(Widget widget, XmDropTransferEntry transfers, Cardinal num_transfers)
{
  ...
  ((*(wc->dropTransfer_class.add_drop_transfer))(widget, transfers, num_transfers));
  _XmAppUnlock(app);
}
