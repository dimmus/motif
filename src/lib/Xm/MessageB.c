...
#define IsAutoChild(mb, w) \
  ((w) == mb->message_box.symbol_wid || (w) == mb->message_box.message_wid || \
   (w) == mb->message_box.separator || (w) == mb->message_box.ok_button || \
   (w) == mb->bulletin_board.cancel_button || (w) == mb->message_box.help_button)
...
#define WARNING5 _XmMMsgMessageB_0004
                      ...
                      static XImage *
                      CreateDefaultImage(char *bits, unsigned int width, unsigned int height);
...
static void MessageCallback(Widget w, XtPointer closure, XtPointer call_data);
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static void SetUpSymbol(XmMessageBoxWidget w);
static void SetUpMessage(XmMessageBoxWidget w);
static void CreateWidgets(XmMessageBoxWidget w);
static Widget CreateDialog(Widget parent, char *name, ArgList al, int ac, unsigned char type);
static void GetMessageString(Widget wid, int resource_offset, XtArgVal *value);
static void GetSymbolPixmap(Widget wid, int resource_offset, XtArgVal *value);
static void GetOkLabelString(Widget wid, int resource_offset, XtArgVal *value);
static void GetCancelLabelString(Widget wid, int resource_offset, XtArgVal *value);
static void GetHelpLabelString(Widget wid, int resource_offset, XtArgVal *value);
...
externaldef(xmmessageboxwidgetclass)
...
static XmConst unsigned char errorBits[] = {
    0x00, 0x00, 0x00, 0xc0, 0x0f, 0x00, 0xf0, 0x3a, 0x00, 0x58, 0x55, 0x00, 0x2c, 0xa0, 0x00,
    0x56, 0x40, 0x01, 0xaa, 0x80, 0x02, 0x46, 0x81, 0x01, 0x8a, 0x82, 0x02, 0x06, 0x85, 0x01,
    0x0a, 0x8a, 0x02, 0x06, 0x94, 0x01, 0x0a, 0xe8, 0x02, 0x14, 0x50, 0x01, 0x28, 0xb0, 0x00,
    0xd0, 0x5f, 0x00, 0xa0, 0x2a, 0x00, 0x40, 0x15, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static XmConst unsigned char questionBits[] = {
    0xf0, 0x3f, 0x00, 0x58, 0x55, 0x00, 0xac, 0xaa, 0x00, 0xd6, 0x5f, 0x01, 0xea, 0xbf,
    0x02, 0xf6, 0x7f, 0x01, 0xea, 0xba, 0x02, 0xf6, 0x7d, 0x05, 0xea, 0xba, 0x0a, 0x56,
    0x7d, 0x15, 0xaa, 0xbe, 0x1e, 0x56, 0x5f, 0x01, 0xac, 0xaf, 0x02, 0x58, 0x57, 0x01,
    0xb0, 0xaf, 0x00, 0x60, 0x55, 0x01, 0xa0, 0xaa, 0x00, 0x60, 0x17, 0x00, 0xa0, 0x2f,
    0x00, 0x60, 0x17, 0x00, 0xb0, 0x2a, 0x00, 0x50, 0x55, 0x00};
static XmConst unsigned char warningBits[] = {
    0x00, 0x00, 0x18, 0x00, 0x2c, 0x00, 0x56, 0x00, 0x2a, 0x00, 0x56, 0x00, 0x2a, 0x00, 0x56,
    0x00, 0x2c, 0x00, 0x14, 0x00, 0x2c, 0x00, 0x14, 0x00, 0x2c, 0x00, 0x14, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x3c, 0x00, 0x14, 0x00, 0x2c, 0x00, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00};
static XmConst unsigned char workingBits[] = {
    0x00, 0x00, 0x00, 0xfe, 0xff, 0x0f, 0xaa, 0xaa, 0x0a, 0x44, 0x55, 0x06, 0xcc, 0x2a,
    0x02, 0x44, 0x55, 0x06, 0xcc, 0x2a, 0x02, 0x84, 0x15, 0x06, 0x8c, 0x2a, 0x02, 0x04,
    0x15, 0x06, 0x0c, 0x0a, 0x02, 0x04, 0x06, 0x06, 0x0c, 0x0b, 0x02, 0x84, 0x15, 0x06,
    0xcc, 0x2a, 0x02, 0x44, 0x55, 0x06, 0xcc, 0x2a, 0x02, 0x44, 0x55, 0x06, 0xcc, 0x2a,
    0x02, 0x44, 0x55, 0x06, 0xfe, 0xff, 0x0f, 0x56, 0x55, 0x05, 0x00, 0x00, 0x00};

/****************************************************************
 * Create a default images for symbol... used in ClassInitialize.
 ****************/
static XImage *CreateDefaultImage(char *bits, unsigned int width, unsigned int height)
{
  XImage *image;
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if (!XmRepTypeValidValue(XmRID_DIALOG_TYPE, new_w->message_box.dialog_type, (Widget)new_w)) {
    new_w->message_box.dialog_type = XmDIALOG_MESSAGE;
  }
  if (!XmRepTypeValidValue(XmRID_ALIGNMENT, new_w->message_box.message_alignment, (Widget)new_w)) {
    new_w->message_box.message_alignment = XmALIGNMENT_BEGINNING;
  }
  CreateWidgets(new_w);
  if (!XmRepTypeValidValue(
          XmRID_DEFAULT_BUTTON_TYPE, new_w->message_box.default_type, (Widget)new_w))
  {
...
static void Destroy(Widget wid)
{
  ...
  /****************/
  if ((d->message_box.symbol_pixmap != XmUNSPECIFIED_PIXMAP) && d->message_box.internal_pixmap) {
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (new_w->message_box.message_string) {
    XtSetArg(al[ac], XmNlabelString, new_w->message_box.message_string);
    ++ac;
    XtSetArg(al[ac], XmNstringDirection, BB_StringDirection(new_w));
    ++ac;
    new_w->message_box.message_string = NULL;
    need_layout = TRUE;
  }
  if (current->message_box.message_alignment != new_w->message_box.message_alignment) {
    if (!XmRepTypeValidValue(XmRID_ALIGNMENT, new_w->message_box.message_alignment, (Widget)new_w))
    {
      new_w->message_box.message_alignment = current->message_box.message_alignment;
    }
    else {
      XtSetArg(al[ac], XmNalignment, new_w->message_box.message_alignment);
      ++ac;
      need_layout = TRUE;
    }
  }
  ...
  if ((current->message_box.dialog_type != XmDIALOG_TEMPLATE) &&
      (current->bulletin_board.cancel_button != new_w->bulletin_board.cancel_button))
  {
    new_w->bulletin_board.cancel_button = current->bulletin_board.cancel_button;
    XmeWarning((Widget)new_w, WARNING5);
  }
  if ((current->message_box.dialog_type != new_w->message_box.dialog_type) &&
      !XmRepTypeValidValue(XmRID_DIALOG_TYPE, new_w->message_box.dialog_type, (Widget)new_w))
  {
    new_w->message_box.dialog_type = current->message_box.dialog_type;
  }
  if (new_w->message_box.symbol_pixmap != current->message_box.symbol_pixmap) {
    newPixmap = TRUE;
    new_w->message_box.internal_pixmap = FALSE;
    if ((current->message_box.symbol_pixmap != XmUNSPECIFIED_PIXMAP) &&
        current->message_box.internal_pixmap)
    {
      XmDestroyPixmap(current->core.screen, current->message_box.symbol_pixmap);
    }
  }
  else { /* If symbol pixmap is unchanged and a new dialog type is specified,
  ...
  if (newPixmap) {
    ...
    ac = 0;
    XtSetArg(al[ac], XmNlabelPixmap, new_w->message_box.symbol_pixmap);
    ++ac;
  ...
  if (new_w->message_box.ok_label_string) {
    if (new_w->message_box.ok_button) {
      ac = 0;
      XtSetArg(al[ac], XmNlabelString, new_w->message_box.ok_label_string);
      ++ac;
      XtSetArg(al[ac], XmNstringDirection, BB_StringDirection(new_w));
      ++ac;
  ...
  if (new_w->message_box.cancel_label_string) {
    if (new_w->bulletin_board.cancel_button) {
      ac = 0;
      XtSetArg(al[ac], XmNlabelString, new_w->message_box.cancel_label_string);
      ++ac;
      XtSetArg(al[ac], XmNstringDirection, BB_StringDirection(new_w));
      ++ac;
  ...
  if (new_w->message_box.help_label_string) {
    if (new_w->message_box.help_button) {
      ac = 0;
      XtSetArg(al[ac], XmNlabelString, new_w->message_box.help_label_string);
      ++ac;
      XtSetArg(al[ac], XmNstringDirection, BB_StringDirection(new_w));
      ++ac;
   ...
   */
  if (current->message_box.default_type != new_w->message_box.default_type) {
    if (!XmRepTypeValidValue(
            XmRID_DEFAULT_BUTTON_TYPE, new_w->message_box.default_type, (Widget)new_w))
    {
      new_w->message_box.default_type = current->message_box.default_type;
    }
    else {
      switch (new_w->message_box.default_type) {
        case XmDIALOG_CANCEL_BUTTON: {
      ...
      BB_DefaultButton(new_w) = defaultButton;
      _XmBulletinBoardSetDynDefaultButton((Widget)new_w, defaultButton);
      if ((current->manager.initial_focus == BB_DefaultButton(current)) &&
          (current->manager.initial_focus == new_w->manager.initial_focus))
      {
        new_w->manager.initial_focus = defaultButton;
   ...
   */
  if (need_layout && (XtClass(new_w) == xmMessageBoxWidgetClass)) {
...
static void SetUpSymbol(XmMessageBoxWidget w)
{
   ...
   */
  ac = 0;
  XtSetArg(al[ac], XmNwidth, 0);
  ac++;
  XtSetArg(al[ac], XmNheight, 0);
  ac++;
  XtSetArg(al[ac], XmNlabelType, XmPIXMAP);
  ac++;
  XtSetArg(al[ac], XmNlabelPixmap, w->message_box.symbol_pixmap);
  ac++;
...
static void SetUpMessage(XmMessageBoxWidget w)
{
  ...
  ac = 0;
  XtSetArg(al[ac], XmNalignment, w->message_box.message_alignment);
  ac++;
  ...
  if (w->message_box.message_string) {
    XtSetArg(al[ac], XmNlabelString, w->message_box.message_string);
    ac++;
    ...
    /* END OSF Fix CR 4847 */
    XtSetArg(al[ac], XmNlabelString, empty_string);
    ac++;
  }
  XtSetArg(al[ac], XmNstringDirection, BB_StringDirection(w));
  ac++;
  w->message_box.message_wid = XmCreateLabelGadget((Widget)w, "Message", al, ac);
  /* BEGIN OSF Fix CR 4847 */
...
/****************************************************************/
XmGeoMatrix _XmMessageBoxGeoMatrixCreate(Widget wid, Widget instigator, XtWidgetGeometry *desired)
{
...
static void CreateWidgets(XmMessageBoxWidget w)
{
  ...
  if (!(w->message_box.dialog_type == XmDIALOG_TEMPLATE && w->message_box.message_string == NULL))
    SetUpMessage(w);
  ...
  if (!(w->message_box.dialog_type == XmDIALOG_TEMPLATE &&
        w->message_box.ok_label_string == NULL && w->message_box.ok_callback == NULL))
  {
    w->message_box.ok_button = _XmBB_CreateButtonG(
        (Widget)w, w->message_box.ok_label_string, "OK", XmOkStringLoc);
    w->message_box.ok_label_string = NULL;
    XtAddCallback(w->message_box.ok_button, XmNactivateCallback, MessageCallback, (XtPointer)w);
  }
  ...
  if (!(w->message_box.dialog_type == XmDIALOG_TEMPLATE &&
        w->message_box.cancel_label_string == NULL && w->message_box.cancel_callback == NULL))
  {
    w->bulletin_board.cancel_button = _XmBB_CreateButtonG(
        (Widget)w, w->message_box.cancel_label_string, "Cancel", XmCancelStringLoc);
    w->message_box.cancel_label_string = NULL;
    XtAddCallback(
        w->bulletin_board.cancel_button, XmNactivateCallback, MessageCallback, (XtPointer)w);
  }
  /* "Help" button... if no label, use default localized "Help" string from
       message catalog, or "Help" if no message catalog, for label */
  if (!(w->message_box.dialog_type == XmDIALOG_TEMPLATE &&
        w->message_box.help_label_string == NULL && w->manager.help_callback == NULL))
  {
    w->message_box.help_button = _XmBB_CreateButtonG(
        (Widget)w, w->message_box.help_label_string, "Help", XmHelpStringLoc);
    ...
    XtAddCallback(w->message_box.help_button, XmNactivateCallback, MessageCallback, (XtPointer)w);
  }
                           ...
                           unsigned char type) /* type of dialog being created */
{
  ...
  w = XmeCreateClassDialog(xmMessageBoxWidgetClass, parent, name, argsNew, ac);
  XtFree((char *)argsNew);
  return w;
}
...
Widget XmVaCreateMessageBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmMessageBoxWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedMessageBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmMessageBoxWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
/****************************************************************/
static void GetMessageString(Widget wid,
...
/****************************************************************/
static void GetSymbolPixmap(Widget wid,
                            int resource_offset, /* unused */
                            XtArgVal *value)
{
  XmMessageBoxWidget w = (XmMessageBoxWidget)wid;
...
/****************************************************************/
static void GetOkLabelString(Widget wid,
                             int resource_offset, /* unused */
                             XtArgVal *value)
{
  XmMessageBoxWidget w = (XmMessageBoxWidget)wid;
...
/****************************************************************/
static void GetCancelLabelString(Widget wid,
                                 int resource_offset, /* unused */
                                 XtArgVal *value)
{
  XmMessageBoxWidget w = (XmMessageBoxWidget)wid;
...
/****************************************************************/
static void GetHelpLabelString(Widget wid,
                               int resource_offset, /* unused */
                               XtArgVal *value)
{
  XmMessageBoxWidget w = (XmMessageBoxWidget)wid;
...
static void GetMsgBoxPixmap(XmMessageBoxWidget mBox)
{
  ...
  if (fileName) {
    int depth;
    if (_XmGetBitmapConversionModel(XtScreen((Widget)mBox)) == XmMATCH_DEPTH)
    ...
    XmStringFree(s);
    tmpPix = _XmGetScaledPixmap(mBox->core.screen,
                                (Widget)mBox,
                                fileName,
                                &acc_color_rec,
                                depth,
