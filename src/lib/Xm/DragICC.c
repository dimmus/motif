...
static XmICCEventType GetMessageData(Display *display,
                                     xmICCMessageStruct *xmessage,
                                     XmICCCallbackStruct *callback);
static void SwapMessageData(xmICCMessageStruct *xmessage);
#ifdef DEBUG
 ...
 ***********************************************************************/
void _XmICCCallbackToICCEvent(Display *display,
                              Window window,
                              ...
                              XmICCEventType type)
{
  ...
  cmev->window = window;
  cmev->format = 8;
  cmev->message_type = XInternAtom(display, _Xm_MOTIF_DRAG_AND_DROP_MESSAGE, False);
  xmessage->any.byte_order = (BYTE)_XmByteOrderChar;
  xmessage->any.message_type = (BYTE)_XmReasonToMessageType(callback->any.reason);
...
static void _XmICCCallbackToXdndEvent(Display *display,
                                      Window window,
                                      Window src_window,
                                      XmICCCallback callback,
                                      ...
                                      XmICCEventType type)
{
  ...
  if (callback->any.reason != XmCR_TOP_LEVEL_ENTER) {
    switch (motion->operation) {
      case XmDROP_MOVE:
        ev->data.l[4] = xdndMove;
        break;
      case XmDROP_COPY:
        ev->data.l[4] = xdndCopy;
        break;
      case XmDROP_LINK:
        ev->data.l[4] = xdndLink;
        break;
    }
  }
 ...
 *
 ***********************************************************************/
void _XmSendICCCallback(Display *display,
                        Window window,
                        Window src_window,
                        int protocol_style,
                        XmICCCallback callback,
 ...
 */
static Boolean _XdndToMotifEvent(XClientMessageEvent *msgEv, XmICCCallbackStruct *callback)
{
  ...
  if ((Atom)msgEv->data.l[4] == xdndMove)
    op = XmDROP_MOVE;
  if ((Atom)msgEv->data.l[4] == xdndCopy)
    op = XmDROP_COPY;
  if ((Atom)msgEv->data.l[4] == xdndLink)
    op = XmDROP_LINK;
  /**
  ...
  if (msgEv->message_type == xdndEnter) {
    ...
    de->dropSiteStatus = (op != XmDROP_NOOP && (msgEv->data.l[1] & 1)) ? XmVALID_DROP_SITE :
                                                                         XmINVALID_DROP_SITE;
  }
                                     ...
                                     XmICCCallbackStruct *callback)
{
  ...
  callback->any.reason = (int)_XmMessageTypeToReason(message_type);
  callback->any.event = NULL;
  ...
  switch (message_type) {
    ...
    case XmCR_DROP_SITE_ENTER:
       ...
       */
      {
        ...
        cb->operations = (unsigned char)GET_MULTIOPS(xmessage->dropSiteEnter.flags);
        cb->dropSiteStatus = (unsigned char)GET_SITE_STATUS(xmessage->dropSiteEnter.flags);
      }
      break;
    case XmCR_DROP_SITE_LEAVE:
      /*
    ...
    case XmCR_DROP_START:
       ...
       */
      {
        ...
        cb->dropSiteStatus = (unsigned char)GET_SITE_STATUS(xmessage->drop.flags);
...
#endif /* DEBUG */
    return (type == GetMessageData(msgEv->display, xmessage, callback)) ? True : False;
  }
 ...
 ***********************************************************************/
CARD16
_XmReadDragBuffer(xmPropertyBuffer propBuf, BYTE which, BYTE *ptr, CARD32 size)
{
  xmByteBufRec *buf;
  CARD32 numCurr;
 ...
 ***********************************************************************/
CARD16
_XmWriteDragBuffer(xmPropertyBuffer propBuf, BYTE which, BYTE *ptr, CARD32 size)
{
  CARD16 returnVal;
  xmByteBufRec *buf;
  if (which == BUFFER_DATA) /* data */
 ...
 ***********************************************************************/
Boolean _XmGetDragReceiverInfo(Display *display,
                               Window window,
 ...
 *
 ***********************************************************************/
Boolean _XmReadDSFromStream(XmDropSiteManagerObject dsm,
                            XtPointer iccInfo,
 ...
 *
 ***********************************************************************/
void _XmWriteDSToStream(XmDropSiteManagerObject dsm,
                        XtPointer stream,
 ...
 *
 ***********************************************************************/
void _XmFreeDragReceiverInfo(XtPointer info)
{
 ...
 *
 ***********************************************************************/
void _XmInitByteOrderChar(void)
{
  _XmProcessLock();
...
#ifdef DEBUG
static void PrintMessage(char c, xmICCMessageStruct *xmessage)
{
...
#  endif /* DEBUG_DRAG_MOTION */
    ...
    case XmCR_OPERATION_CHANGED:
      printf("%c ", c);
      printf(
          "Op changed %x %x\n", xmessage->operationChanged.flags, xmessage->operationChanged.time);
      break;
    case XmCR_DROP_SITE_ENTER:
      printf("%c ", c);
    ...
    case XmCR_DROP_SITE_LEAVE:
      printf("%c ", c);
      printf("Leave %x %x\n", xmessage->dropSiteLeave.flags, xmessage->dropSiteLeave.time);
      break;
    case XmCR_DROP_START:
      printf("%c ", c);
