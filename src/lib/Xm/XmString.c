...
enum { XmSTRING_FIRST_SEG, XmSTRING_MIDDLE_SEG, XmSTRING_LAST_SEG, XmSTRING_SINGLE_SEG };
...
#define _XmEntryRendCountedBegins(entry, count) \
  (_XmEntryOptimized(entry) ? (((count) && _XmEntryRendIndex(entry) != REND_INDEX_UNSET) ? \
                                   &(_tag_cache[_XmEntryRendIndex(entry)]) : \
                                   NULL) : \
                              _XmUnoptSegRendBegins(entry))
#define _XmEntryRendBegins(entry) \
  (_XmEntryOptimized(entry) ? \
       ((_XmEntryRendBeginCountGet(entry) && _XmEntryRendIndex(entry) != REND_INDEX_UNSET) ? \
            &(_tag_cache[_XmEntryRendIndex(entry)]) : \
            NULL) : \
       _XmUnoptSegRendBegins(entry))
...
#define _XmEntryRendEnds(entry) \
  (_XmEntryOptimized(entry) ? \
       ((_XmEntryRendEndCountGet(entry) && _XmEntryRendIndex(entry) != REND_INDEX_UNSET) ? \
            &(_tag_cache[_XmEntryRendIndex(entry)]) : \
            NULL) : \
       _XmUnoptSegRendEnds(entry))
/*
...
#define MAXSHORTVALUE \
  127 /* maximum len to be used for short \
                     length form */
#define CSLONGLEN 3
#define CSSHORTLEN 1
...
static void LineMetrics(_XmStringEntry line,
                        ...
                        Dimension *ascender,
                        Dimension *descender);
static void SubStringPosition(Boolean one_byte,
                              XmRenderTable rt,
                              XmRendition entry,
                              _XmStringEntry seg,
...
static void recursive_layout(_XmString string,
                             ...
                             int depth);
static void last_direction(_XmStringEntry line, int *index, XmDirection *direction);
...
static void _calc_align_and_clip(Display *d,
                                 Window w,
                                 GC gc,
                                 Position *x,
                                 Position y,
...
static void ComputeMetrics(XmRendition rend,
                           ...
                           Dimension *descent,
                           Boolean utf8);
static Dimension ComputeWidth(unsigned char which, XCharStruct char_ret);
...
static Boolean match_pattern(XtPointer text,
                             ...
                             int char_len,
                             Boolean dir_change);
static void parse_unmatched(XmString *result, char **ptr, XmTextType text_type, int length);
...
static void end_context_rends(_XmStringContext context,
                              ...
                              int count);
static XFontStruct *GetFont(XmRenderTable rt, _XmStringEntry entry);
static _XmStringCache CacheGet(_XmStringEntry entry, int type, int create, XtPointer match_value);
...
XmString XmStringDirectionCreate(XmStringDirection direction)
{
  ...
  static XmConst XmStringDirection dir_index[] = {XmSTRING_DIRECTION_L_TO_R,
...
static Boolean RenditionsCompatible(_XmStringEntry seg1, _XmStringEntry seg2)
{
  ...
  if (_XmEntryOptimized(seg1) &&
      (((ecnt1 != 0) && (bcnt2 != 0)) || ((bcnt1 > 0) && (bcnt2 > 0)) ||
       ((ecnt1 > 0) && (ecnt2 > 0)) || ((bcnt1 > 0) && (ecnt2 > 0) && (begin1[0] != end2[0]))))
    return (FALSE);
...
static void MergeEnds(_XmStringEntry a, _XmStringEntry b)
{
  ...
  if (_XmEntryOptimized(a)) {
    if (a_count == 0 && b_count == 1) {
      _XmEntryRendIndex(a) = _XmEntryOptimized(b) ?
                                 _XmEntryRendIndex(b) :
...
static Boolean IsUnopt(_XmString str, int lines)
{
  ...
  if (lines > 0) {
    ...
    if ((_XmEntrySegmentCountGet(line) > 0) &&
        (_XmEntryType(_XmEntrySegmentGet(line)[0]) != XmSTRING_ENTRY_OPTIMIZED))
      return True;
  }
  return False;
}

static _XmStringEntry Unoptimize(_XmStringEntry entry, int free)
{
  ...
  if (_XmEntryType(entry) == XmSTRING_ENTRY_OPTIMIZED) {
    ...
    if (free) {
      ...
      for (j = 0; j < _XmEntrySegmentCount(entry); j++) {
        tmp_seg = _XmEntrySegment(entry)[j];
        if (_XmEntryType(tmp_seg) == XmSTRING_ENTRY_OPTIMIZED)
          _XmEntrySegment(new_entry)[j] = (_XmStringNREntry)EntryCvtToUnopt(
              (_XmStringEntry)tmp_seg);
        else
          _XmEntrySegment(new_entry)[j] = (_XmStringNREntry)_XmStringEntryCopy(
              (_XmStringEntry)tmp_seg);
      }
    }
  }
...
XmString XmStringConcatAndFree(XmString a, XmString b)
{
  ...
  if ((_XmStrOptimized(a) && _XmStrOptimized(b))) {
    ...
    a_tabs = _XmStrTabs(a);
    b_tabs = _XmStrTabs(b);
    if (((a_index == b_index) || (a_index == TAG_INDEX_UNSET) || (b_index == TAG_INDEX_UNSET)) &&
        RenditionsCompatible((_XmStringEntry)a, (_XmStringEntry)b) &&
        ...
        ((_XmStrText(a) && b_tabs == 0) || (a_tabs + b_tabs <= 3)))
    {
      ...
      _XmStrRendIndex((_XmString)opt_str) = (a_rend_index == REND_INDEX_UNSET) ? b_rend_index :
                                                                                 a_rend_index;
      /* Push begin and end state. */
      _XmStrRendBegin((_XmString)opt_str) = (_XmStrRendBegin(a) ? _XmStrRendBegin(a) :
                                                                  _XmStrRendBegin(b));
      _XmStrRendEnd((_XmString)opt_str) = (_XmStrRendEnd(b) ? _XmStrRendEnd(b) : _XmStrRendEnd(a));
      _XmStrDirection((_XmString)opt_str) = (_XmStrDirection(a) == XmSTRING_DIRECTION_UNSET) ?
                                                _XmStrDirection(b) :
                                                _XmStrDirection(a);
      ...
      if (b_len && (opt_str != b))
        memcpy((_XmStrText((_XmString)opt_str) + a_len), _XmStrText(b), b_len);
      if (opt_str != a)
        XmStringFree(a);
      if (opt_str != b)
        XmStringFree(b);
  ...
  if ((a_sc != 0) && (b_sc != 0)) {
    ...
    merged = 0;
    if (((a_tag == b_tag) || (a_tag == NULL) || (b_tag == NULL)) &&
        RenditionsCompatible((_XmStringEntry)a_last, (_XmStringEntry)b_seg) &&
        (a_type == b_type || a_type == XmNO_TEXT || b_type == XmNO_TEXT) &&
        ((last == _XmEntryDirectionGet((_XmStringEntry)b_seg)) ||
         (_XmEntryDirectionGet((_XmStringEntry)b_seg) == XmSTRING_DIRECTION_UNSET) ||
         ((last == XmSTRING_DIRECTION_UNSET) && (a_len == 0))) &&
        ...
        (((a_len != 0) && b_tabs == 0) || ((a_len == 0) && a_tabs + b_tabs <= 7)))
    {
      if (b_len) {
        if ((_XmEntryType(a_last) == XmSTRING_ENTRY_OPTIMIZED) && _XmEntryImm(a_last)) {
          ...
          _XmEntryTextSet(
              ...
              XtRealloc((char *)_XmEntryTextGet((_XmStringEntry)a_last), a_len + b_len));
        }
      }
      if (_XmEntryOptimized(a_last))
        _XmEntryTagIndex(a_last) = ((a_tag == NULL) ?
                                        (b_tag == NULL ?
                                             TAG_INDEX_UNSET :
      ...
      if (last == XmSTRING_DIRECTION_UNSET)
        _XmEntryDirectionSet((_XmStringEntry)a_last, _XmEntryDirectionGet((_XmStringEntry)b_seg));
  ...
  if (a_str && _XmStrEntryCount(a_str) > 0) {
    ...
    if (tmp_line && _XmEntrySegmentCountGet(tmp_line) > 0) {
      ...
      for (cache = _XmStringCacheGet(_XmEntryCacheGet((_XmStringEntry)tmp_seg), _XmSCANNING_CACHE);
           cache;
...
Boolean XmStringCompare(XmString a, XmString b)
{
  ...
  if (_XmStrOptimized(a) && _XmStrOptimized(b)) {
    if (!((_XmStrTagGet(a) == _XmStrTagGet(b)) || (_XmStrTagGet(a) == NULL) ||
          (_XmStrTagGet(b) == NULL) ||
      ...
      if (_XmEntryMultiple(entry_a[i]) && _XmEntryMultiple(entry_b[i])) {
        ...
        for (j = 0; j < _XmEntrySegmentCount(entry_a[i]); j++) {
          ...
          }
          {
            ...
            if ((a_dir != b_dir) &&
                (((a_dir == XmSTRING_DIRECTION_UNSET) && (b_dir != XmSTRING_DIRECTION_L_TO_R)) ||
                 ((b_dir == XmSTRING_DIRECTION_UNSET) && (a_dir != XmSTRING_DIRECTION_L_TO_R))))
            {
              if (a_unopt)
                XmStringFree(a_unopt);
              if (b_unopt)
        ...
        if (!((_XmEntryTag(entry_a[i]) == _XmEntryTag(entry_b[i])) ||
              ...
              ((strcmp(_XmEntryTag(entry_a[i]), XmFONTLIST_DEFAULT_TAG) == 0) &&
               _XmStringIsCurrentCharset(_XmEntryTag(entry_b[i]))) ||
              ((strcmp(_XmEntryTag(entry_b[i]), XmFONTLIST_DEFAULT_TAG) == 0) &&
        ...
        if ((_XmEntryDirectionGet(entry_a[i]) != _XmEntryDirectionGet(entry_b[i])) &&
            (((_XmEntryDirectionGet(entry_a[i]) == XmSTRING_DIRECTION_UNSET) &&
              (_XmEntryDirectionGet(entry_b[i]) != XmSTRING_DIRECTION_L_TO_R)) ||
             ((_XmEntryDirectionGet(entry_b[i]) == XmSTRING_DIRECTION_UNSET) &&
              (_XmEntryDirectionGet(entry_a[i]) != XmSTRING_DIRECTION_L_TO_R))))
        {
          if (a_unopt)
            XmStringFree(a_unopt);
          if (b_unopt)
 ...
 *                                                                      *
 ************************************************************************/
Boolean XmeStringIsValid(XmString string)
{
  if (string == NULL)
    return (FALSE);
  return (TRUE);
...
static int TabVal(Display *d, Screen **pscreen, Window w, XmTab tab)
{
  int fromType;
  int intValue;
                           ...
                           Dimension *descent)
{
  ...
  if (base_rend == NULL) {
    ...
    if (_XmStrRendBegin(opt)) {
      ...
      rend = _XmRenditionMerge(
          _XmRendDisplay(base_rend), rend_io, base_rend, r, _XmStrTagGet(opt), tags, 1, FALSE);
    }
    else {
      rend = _XmRenditionMerge(
          _XmRendDisplay(base_rend), rend_io, base_rend, r, _XmStrTagGet(opt), NULL, 0, FALSE);
    }
  }
  /* 3. Default rendition. */
  if ((rend == NULL) || (_XmRendFont(rend) == NULL && _XmRendXftFont(rend) == NULL)) {
    tags[0] = ((_XmStrTextType(opt) == XmCHARSET_TEXT) ? XmFONTLIST_DEFAULT_TAG :
                        ...
                        Dimension *descender)
{
  ...
  if (peek_seg != NULL)
    (void)SpecifiedSegmentExtents(
        (_XmStringEntry)seg, r, rend_io, base, XmSTRING_FIRST_SEG, &w, &h, &asc, &dsc);
  else
    (void)SpecifiedSegmentExtents(
        (_XmStringEntry)seg, r, rend_io, base, XmSTRING_SINGLE_SEG, &w, &h, &asc, &dsc);
  if (*rend_io != NULL)
  ...
  while (seg != NULL) {
    ...
    if ((tab != NULL) && (_XmEntryTabsGet((_XmStringEntry)seg) != 0) &&
        (tab_cnt < _XmTabLCount(tl)))
    {
      for (i = 0; (i < _XmEntryTabsGet((_XmStringEntry)seg)) && (tab_cnt < _XmTabLCount(tl));
    ...
    if (_XmEntryType(seg) != XmSTRING_ENTRY_OPTIMIZED) {
      ...
      seg_index++;
      seg = (seg_index < _XmEntrySegmentCountGet(line) ? _XmEntrySegmentGet(line)[seg_index] :
                                                         NULL);
      peek_seg = ((seg_index + 1) < _XmEntrySegmentCountGet(line) ?
                      _XmEntrySegmentGet(line)[seg_index + 1] :
                      NULL);
    ...
    if (seg != NULL) {
      ...
      if (peek_seg != NULL)
        (void)SpecifiedSegmentExtents(
            (_XmStringEntry)seg, r, rend_io, base, XmSTRING_MIDDLE_SEG, &w, &h, &asc, &dsc);
      else
        (void)SpecifiedSegmentExtents(
            (_XmStringEntry)seg, r, rend_io, base, XmSTRING_LAST_SEG, &w, &h, &asc, &dsc);
    }
  }
  *width = tab_w;
  if (max_h > 0)
...
unsigned char _XmStringCharacterCount(XtPointer text,
...
unsigned char _XmEntryCharCountGet(_XmStringEntry entry, XmRenderTable rt)
{
  unsigned int len;
  ...
  if (_XmEntryUnoptimized(entry)) {
    if (((_XmStringUnoptSeg)entry)->char_count == 0 && (len = _XmEntryByteCountGet(entry)) != 0) {
      ((_XmStringUnoptSeg)entry)->char_count = _XmStringCharacterCount(
          (char *)_XmEntryTextGet(entry),
          (XmTextType)_XmEntryTextTypeGet(entry),
          len,
          GetFont(rt, entry));
    }
...
_XmStringCache _XmStringCacheGet(_XmStringCache caches, int type)
{
...
void _XmStringCacheFree(_XmStringCache caches)
{
...
XtPointer _XmScanningCacheGet(_XmStringNREntry entry, XmDirection d, int field)
{
  _XmStringScanningCache cache;
  cache = (_XmStringScanningCache)CacheGet(
      (_XmStringEntry)entry, _XmSCANNING_CACHE, False, (XtPointer)(long)d);
  if (!cache) {
...
void _XmScanningCacheSet(_XmStringNREntry entry, XmDirection d, int field, XtPointer value)
{
  _XmStringScanningCache cache;
  cache = (_XmStringScanningCache)CacheGet(
      (_XmStringEntry)entry, _XmSCANNING_CACHE, True, (XtPointer)(long)d);
  if (!cache)
    return;
  switch (field) {
    case _XmCACHE_DIRTY:
...
XtPointer _XmRenderCacheGet(_XmStringEntry entry, XmRenderTable rt, int field)
{
...
void _XmRenderCacheSet(_XmStringEntry entry, XmRenderTable rt, int field, XtPointer value)
{
  _XmStringRenderingCache cache;
  ...
  switch (field) {
    ...
    case _XmCACHE_RENDER_DESCENT:
      ...
      break;
    case _XmCACHE_RENDER_RENDITION:
      if (cache->rendition != NULL)
        XmRenditionFree(cache->rendition);
 ...
 */
void XmStringExtent(XmRenderTable rendertable,
                    XmString string,
                    Dimension *width,
                    Dimension *height)
{
  ...
  *width = 0, *height = 0;
  ...
  if (_XmStrOptimized(string))
    ...
    rend = &tmp;
    /* Initialize for tabs. */
    d = (_XmRTDisplay(rendertable) == NULL) ? _XmGetDefaultDisplay() : _XmRTDisplay(rendertable);
    ...
    for (j = 0; j < _XmStrLineCountGet(string); j++) {
      ...
      LineMetrics(
          line, rendertable, &rend, NULL, XmLEFT_TO_RIGHT, &cur_width, &cur_height, &asc, &dsc);
...
static _XmStringEntry EntryCvtToOpt(_XmStringEntry entry)
{
  ...
  if (_XmEntryUnoptimized(entry)) {
    if (_XmEntryPushGet(entry) || _XmEntryPopGet(entry) || _XmUnoptSegRendBeginCount(entry) > 1 ||
        _XmUnoptSegRendEndCount(entry) > 1 || _XmEntryTabsGet(entry) > 7 ||
        _XmEntryByteCountGet(entry) > 255 ||
        (_XmUnoptSegRendBeginCount(entry) &&
         _XmStringIndexCacheTag(_XmEntryRendBeginGet(entry, 0), XmSTRING_TAG_STRLEN) >=
             REND_INDEX_MAX) ||
        (_XmUnoptSegRendEndCount(entry) &&
         _XmStringIndexCacheTag(_XmEntryRendEndGet(entry, 0), XmSTRING_TAG_STRLEN) >=
             REND_INDEX_MAX) ||
        ...
        (_XmUnoptSegTag(entry) &&
         _XmStringIndexCacheTag(_XmUnoptSegTag(entry), XmSTRING_TAG_STRLEN) >= TAG_INDEX_MAX))
      return NULL;
    ...
    _XmEntryTagIndex(
        new_entry) = (_XmUnoptSegTag(entry) ?
...
_XmString _XmStringOptToNonOpt(_XmStringOpt string)
{
  _XmString str;
  _XmStringUnoptSegRec seg;
  _XmStrCreate(str, XmSTRING_MULTIPLE_ENTRY, 0);
  ...
  _XmEntryDirectionSet((_XmStringEntry)&seg, _XmStrDirection((_XmString)string));
                              ...
                              Dimension *under_end)
{
  ...
  if (_XmRendFontType(entry) == XmFONT_IS_FONT || _XmRendFontType(entry) == XmFONT_IS_XFT) {
        ...
        if (!fail) { /* found it */
          if (begin == 0)
            ...
            *under_begin = x + abs(XTextWidth(font_struct, a, begin));
            ...
            XGlyphInfo ext;
            XftTextExtentsUtf8(
                _XmRendDisplay(entry), _XmRendXftFont(entry), (FcChar8 *)a, begin, &ext);
          ...
          if (width == 0) {
            if (_XmRendFontType(entry) == XmFONT_IS_FONT)
              ...
              XGlyphInfo ext;
              XftTextExtentsUtf8(
                  _XmRendDisplay(entry), _XmRendXftFont(entry), (FcChar8 *)b, under_seg_len, &ext);
              width = ext.xOff;
            }
#endif
        ...
        if (!fail) { /* found it */
          if (begin == 0)
            ...
            *under_begin = x + abs(XTextWidth16(font_struct, (XChar2b *)a, begin / 2));
            ...
            XGlyphInfo ext;
            XftTextExtents16(
                _XmRendDisplay(entry), _XmRendXftFont(entry), (FcChar16 *)a, begin, &ext);
            *under_begin = x + ext.xOff;
          }
#endif
          width = _XmEntryWidthGet((_XmStringEntry)under_seg, rt);
          if (width == 0) {
            if (_XmRendFontType(entry) == XmFONT_IS_FONT)
              ...
              XGlyphInfo ext;
              XftTextExtents16(_XmRendDisplay(entry),
                               _XmRendXftFont(entry),
                               (FcChar16 *)b,
                               under_seg_len,
                               &ext);
              width = ext.xOff;
    ...
    for (i = 0; i <= max; i += len_a) {
      ...
      if (!fail) { /* found it */
        ...
        if (begin == 0)
          ...
          *under_begin = x +
                         abs(XwcTextEscapement(font_set, (wchar_t *)a, begin / sizeof(wchar_t)));
        }
        else {
#if XM_UTF8
          if (utf8)
        ...
        if (width == 0) {
          ...
          width = (type == XmWIDECHAR_TEXT) ?
                      abs(XwcTextEscapement(
                          font_set, (wchar_t *)b, under_seg_len / sizeof(wchar_t))) :
                      (utf8 ? abs(Xutf8TextEscapement(font_set, b, under_seg_len)) :
                                ...
                                Boolean colors_set)
{
  ...
  if ((cur_style != old_style) &&
      ((old_style == LineSolid) || (old_style == LineOnOffDash) || (old_style == LineDoubleDash)))
  {
                                 ...
                                 Dimension descender)
{
  ...
  if (seg_len > 0) {
    multibyte = (((text_type == XmMULTIBYTE_TEXT) || (text_type == XmCHARSET_TEXT)) &&
    ...
    if (!multibyte && !widechar && _XmRendFontType(rend) != XmFONT_IS_XFT) {
      ...
      if (current_gcv.font != xgcv.font) /* not right one */ { /* change it */
    ...
    if (*underline != (_XmString)NULL) {
      ...
      if (_XmStrOptimized(*underline)) {
         ...
         */
        {
          ...
          _XmEntryTextTypeSet(&under_seg, (XmTextType)_XmStrTextType(*underline));
          _XmEntryTextSet((_XmStringEntry)&under_seg, (char *)_XmStrText(*underline));
          SubStringPosition((!text16),
                            rendertable,
                            rend,
                            (_XmStringEntry)seg,
                            (_XmStringEntry)&under_seg,
    ...
    _XmStringDrawLining(
        d, w, x, y, width, height, descender, rend, XmUNSPECIFIED_PIXEL, XmHIGHLIGHT_NORMAL, TRUE);
                     ...
                     Dimension opt_height)
{
  ...
  if (opt) {
    ...
    _XmEntryTextTypeSet(&segm, (XmTextType)_XmStrTextType(optline));
    ...
    if (_XmStrDirection(optline) != XmSTRING_DIRECTION_UNSET)
      ...
      _XmEntryDirectionSet((_XmStringEntry)&segm, XmDirectionToStringDirection(prim_dir));
    if (_XmStrRendBegin(optline))
      _XmEntryRendIndex(&segm) = _XmStrRendIndex(optline);
    if (*scr_rend != NULL)
      tl = _XmRendTabs(*scr_rend);
    tab = (tl == NULL) ? NULL : _XmTabLStart(tl);
    prev_val = x;
    tab_cnt = 0;
    if ((tab != NULL) && (_XmEntryTabsGet((_XmStringEntry)&segm) != 0) &&
        (tab_cnt < _XmTabLCount(tl)) &&
        _XmEntryDirectionGet((_XmStringEntry)&segm) != XmSTRING_DIRECTION_R_TO_L)
    {
      for (i = 0; (i < _XmEntryTabsGet((_XmStringEntry)&segm)) && (tab_cnt < _XmTabLCount(tl));
           i++, tab = _XmTabNext(tab), tab_cnt++)
      {
    ...
    _XmStringDrawSegment(d,
                         w,
                         x,
                         y,
                         opt_width,
                         opt_height,
                         (_XmStringNREntry)&segm,
                         *scr_rend,
                         rendertable,
                         image,
                         underline,
    ...
    if (XmDirectionMatch(lay_dir, XmLEFT_TO_RIGHT)) {
      ...
      ok = SpecifiedSegmentExtents((_XmStringEntry)seg,
                                   rendertable,
                                   scr_rend,
                                   base,
                                   XmSTRING_MIDDLE_SEG,
      ...
      while (seg != NULL) {
        ...
        if (ok)
          _XmStringDrawSegment(
              d, w, x, y, width, height, seg, *scr_rend, rendertable, image, underline, descender);
        x += width;
        if (set_direction) {
          _XmEntryDirectionSet((_XmStringEntry)seg, XmSTRING_DIRECTION_UNSET);
          set_direction = False;
                                 ...
                                 XmFontType font_type)
{
  ...
  if ((clip != NULL) && (!*restore))
    if (((*x) <= clip->x + clip->width) && (clip->x <= (*x) + line_width) &&
        (y - line_height + descender <= clip->y + clip->height) && (clip->y <= y + descender))
    {
      *restore = TRUE;
#if USE_XFT
                    ...
                    XRectangle *clip)
{
  ...
  if (_XmStrOptimized(string)) {
    OptLineMetrics(
        rendertable, string, &rend2, rend, &line_width, &line_height, &ascender, &descender);
    y += ascender;
    if (line_width != 0) {
      draw_x = base_x; /* most left position */
      _calc_align_and_clip(d,
                           w,
                           gc,
                           &draw_x,
                           y,
                           ...
                           descender,
                           &restore_clip,
                           _XmRendFontType(rend2));
      DrawLine(d,
               w,
               &screen,
               draw_x,
               y,
               (_XmStringEntry)string,
               &rend2,
               rend,
               rendertable,
               lay_dir,
  ...
  if (_XmRendTags(rend1) != NULL)
    XtFree((char *)_XmRendTags(rend1));
  if (_XmRendTags(rend2) != NULL)
    XtFree((char *)_XmRendTags(rend2));
}

void _XmStringRender(Display *d,
                     Drawable w,
                     XmRenderTable rendertable,
                     XmRendition rend,
                     _XmString string,
 ...
 */
void _XmStringSegmentNew(_XmString string, int line_index, _XmStringEntry value, int copy)
{
  _XmStringEntry line;
  _XmStringEntry seg;
  int sc;
...
XmString XmCvtByteStreamToXmString(unsigned char *property)
{
  ...
  while (continue_flag) {
    ...
    switch (*c_opt) {
      ...
      case XmSTRING_COMPONENT_LOCALE_TEXT:
        /* Check the tag. */
        tag_index = _XmStringIndexCacheTag((char *)XmFONTLIST_DEFAULT_TAG, XmSTRING_TAG_STRLEN);
        havetag = TRUE;
        if ((txt_seen) || (tag_index >= TAG_INDEX_MAX)) {
          optimized = FALSE;
          break;
  ...
  if (optimized)
    string = (_XmString)_XmStringOptCreate(c, end, txtlength, havetag, tag_index);
  else
...
_XmStringEntry _XmStringEntryCopy(_XmStringEntry entry)
{
  int i;
  int size;
  XtPointer text;
  ...
  switch (_XmEntryType(entry)) {
    ...
    case XmSTRING_ENTRY_UNOPTIMIZED: {
      ...
      if (_XmEntryPermGet(entry)) {
        ...
        text = (XtPointer)XtMalloc(entry_len);
        memcpy((char *)text, (char *)_XmEntryTextGet((_XmStringEntry)entry), entry_len);
        _XmEntryTextSet(new_entry, text);
      }
      else
        _XmEntryTextSet(new_entry, NULL);
...
XmDirection _XmEntryPushGet(_XmStringEntry entry)
{
  return (_XmEntryUnoptimized(entry) ? ((_XmStringEntry)(entry))->unopt_single.push_before :
                                       False);
}

unsigned int _XmEntryByteCountGet(_XmStringEntry entry)
{
  switch (_XmEntryType(entry)) {
    case XmSTRING_ENTRY_OPTIMIZED:
...
XtPointer _XmEntryTextGet(_XmStringEntry entry)
{
  return (_XmEntryOptimized(entry) ?
              (!_XmEntryImm(entry) ? ((_XmStringOptSeg)(entry))->data.text :
                                     (XtPointer)((_XmStringOptSeg)(entry))->data.chars) :
              (((_XmStringUnoptSeg)(entry))->data.text));
}

unsigned int _XmEntryDirectionGet(_XmStringEntry entry)
{
  return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.str_dir :
                                     ((_XmStringEntry)(entry))->unopt_single.str_dir);
}

void _XmEntryDirectionSet(_XmStringEntry entry, XmDirection val)
{
  (_XmEntryOptimized(entry) ? (((_XmStringEntry)entry)->single.str_dir = val) :
                              (((_XmStringEntry)entry)->unopt_single.str_dir = val));
}

unsigned int _XmEntryTextTypeGet(_XmStringEntry entry)
{
  return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.text_type :
                                     ((_XmStringEntry)(entry))->unopt_single.text_type);
}

_XmStringCache _XmEntryCacheGet(_XmStringEntry entry)
{
  return (_XmEntryUnoptimized(entry) ? ((_XmStringUnoptSeg)(entry))->cache : NULL);
}

unsigned char _XmEntryRendEndCountGet(_XmStringEntry entry)
{
  return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.rend_end :
                                     _XmUnoptSegRendEndCount(entry));
}

unsigned char _XmEntryRendBeginCountGet(_XmStringEntry entry)
{
  return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.rend_begin :
                                     _XmUnoptSegRendBeginCount(entry));
}

Boolean _XmEntryPopGet(_XmStringEntry entry)
{
  return (_XmEntryUnoptimized(entry) ? ((_XmStringEntry)(entry))->unopt_single.pop_after : False);
}

XmStringTag _XmEntryRendEndGet(_XmStringEntry entry, int n)
{
  return ((_XmEntryRendEndCountGet(entry) > n) ?
              (_XmEntryOptimized(entry) ? (_XmEntryRendIndex(entry) != REND_INDEX_UNSET ?
                                               _XmStringIndexGetTag(_XmEntryRendIndex(entry)) :
                                               NULL) :
                                          (((_XmStringUnoptSeg)(entry))->rend_end_tags)[n]) :
              NULL);
}

XmStringTag _XmEntryRendBeginGet(_XmStringEntry entry, int n)
{
...
void _XmEntryRendEndSet(_XmStringEntry entry, XmStringTag tag, int n)
{
  int i;
  if (_XmEntryOptimized(entry)) {
    if (tag == NULL) {
      ...
      _XmEntryRendIndex(entry) = _XmStringIndexCacheTag(tag, XmSTRING_TAG_STRLEN);
    }
...
void _XmEntryRendBeginSet(_XmStringEntry entry, XmStringTag tag, int n)
{
  int i;
  if (_XmEntryOptimized(entry)) {
    if (tag == NULL) {
...
unsigned char _XmEntryTabsGet(_XmStringEntry entry)
{
  return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.tabs_before :
                                     ((_XmStringEntry)(entry))->unopt_single.tabs_before);
}

/** End macros converted to functions. **/
void _XmStringEntryFree(_XmStringEntry entry)
{
  int i;
  if (!entry)
    return;
  switch (_XmEntryType(entry)) {
    ...
    case XmSTRING_ENTRY_ARRAY:
      ...
      if (_XmEntrySegment(entry))
        XtFree((char *)_XmEntrySegment(entry));
      XtFree((char *)entry);
      break;
    case XmSTRING_ENTRY_UNOPTIMIZED:
                           ...
                           Boolean utf8)
{
  ...
  switch (_XmRendFontType(rend)) {
      ...
      if (two_byte_font(font_struct)) {
        if (byte_count >= 2 || utf8) {
          if (utf8) {
            ...
            XTextExtents16(
                font_struct, (XChar2b *)text, Half(byte_count), &dir, &asc, &desc, &char_ret);
    ...
    case XmFONT_IS_XFT:
      ...
      {
        XGlyphInfo info;
        XftTextExtentsUtf8(_XmRendDisplay(rend), _XmRendXftFont(rend), text, byte_count, &info);
        wid = info.xOff;
      }
#endif
  }
 ...
 *
 ****************************************************************/
Boolean _XmStringSegmentExtents(_XmStringEntry entry,
                                XmRenderTable rendertable,
                                XmRendition *rend_in_out,
                                XmRendition base,
                                Dimension *width,
                                ...
                                Dimension *descent)
{
  return (SpecifiedSegmentExtents(
      entry, rendertable, rend_in_out, base, XmSTRING_MIDDLE_SEG, width, height, ascent, descent));
}
                                       ...
                                       Dimension *descent)
{
  ...
  render_cache = (_XmStringRenderingCache)CacheGet(
      entry, _XmRENDERING_CACHE, False, (XtPointer)rendertable);
  ...
  if (cached_rend == NULL) /* Update *rend_in_out. */ {
    ...
    if ((_XmRendTag(*rend_in_out) != entry_tag) || (count != 0) || _XmRendHadEnds(*rend_in_out)) {
      *rend_in_out = _XmRenditionMerge(
          d, rend_in_out, base, rendertable, entry_tag, tags, tag_count, (render_cache != NULL));
      _XmRendTag(*rend_in_out) = entry_tag;
    }
    /* 3. Default rendition. */
    if (_XmRendFont(*rend_in_out) == NULL && _XmRendXftFont(*rend_in_out) == NULL) {
      def_tag = ((_XmEntryTextTypeGet(entry) == XmCHARSET_TEXT) ? XmFONTLIST_DEFAULT_TAG :
                                                                  _MOTIF_DEFAULT_LOCALE);
      rend = _XmRenditionMerge(
          d, rend_in_out, base, rendertable, def_tag, NULL, 0, (render_cache != NULL));
      if ((rend != NULL) && (_XmRendFont(rend) == NULL) && (_XmRendXftFont(rend) == NULL) &&
          ((def_rend = _XmRenderTableFindRendition(
                rendertable, def_tag, TRUE, FALSE, FALSE, NULL)) != NULL))
      /* Call noFontCallback. */
      {
        XmDisplay dsp;
        XmDisplayCallbackStruct cb;
      ...
      if ((rend == NULL) &&
        ...
        rend = _XmRenditionMerge(
            d, rend_in_out, base, rendertable, NULL, NULL, 0, (render_cache != NULL));
      if ((rend != NULL) && (_XmRendFont(rend) == NULL) && (_XmRendXftFont(rend) == NULL))
      /* Call noFontCallback. */
      {
        XmDisplay dsp;
...
unsigned int XmCvtXmStringToByteStream(XmString string, unsigned char **prop_return)
{
  ...
  *prop_return = ext;
  ...
  _XmStringContextReInit(&stack_context, string);
  while ((tag = XmeStringGetComponent(&stack_context, TRUE, FALSE, &length, &value)) !=
         XmSTRING_COMPONENT_END)
                  ...
                  XRectangle *clip)
{
  ...
  if (string)
    _draw(
        d, w, rendertable, (_XmString)string, gc, x, y, width, align, lay_dir, clip, FALSE, NULL);
  _XmAppUnlock(app);
}

void XmStringDrawImage(Display *d,
                       Window w,
                       XmRenderTable rendertable,
                       XmString string,
                       GC gc,
...
void XmStringDrawUnderline(Display *d,
                           Window w,
...
static char *type_image(XmTextType type)
{
  switch (type) {
    case XmCHARSET_TEXT:
...
static char *entry_type_image(int type)
{
  switch (type) {
    case XmSTRING_ENTRY_OPTIMIZED:
...
void _Xm_dump_internal(_XmString string)
{
  int i, j, k;
  if (_XmStrOptimized(string)) {
    ...
    printf("\t    rend_index    = %4d <%s>\n",
           ...
           (_XmStrRendTagGet(string) ? _XmStrRendTagGet(string) : "(unset)"));
    ...
    for (i = 0; i < line_count; i++) {
      ...
      for (j = 0; j < seg_count; j++) {
        ...
        for (k = 0; k < _XmEntryRendBeginCountGet(seg); k++)
          printf("\t\t  %4d <%s>\n",
                 _XmStringIndexCacheTag(_XmEntryRendBeginGet(seg, k), XmSTRING_TAG_STRLEN),
        ...
        for (k = 0; k < _XmEntryRendEndCountGet(seg); k++)
          printf("\t\t  %4d <%s>\n",
                 _XmStringIndexCacheTag(_XmEntryRendEndGet(seg, k), XmSTRING_TAG_STRLEN),
                 _XmEntryRendEndGet(seg, k));
        printf("\t\tpop after       = %d \n", _XmEntryPopGet(seg));
      }
    }
  }
...
Boolean _XmStringSingleSegment(XmString str, char **pTextOut, XmStringTag *pTagOut)
{
  ...
  *pTagOut = NULL;
  if (str) {
    ...
    while ((type = XmeStringGetComponent(&stack_context, FALSE, FALSE, &len, &val)) !=
           XmSTRING_COMPONENT_END)
    {
      switch (type) {
        case XmSTRING_COMPONENT_TAG:
...
void XmeSetWMShellTitle(XmString xmstr, Widget shell)
{
  char *text = (char *)NULL;
 ...
 */
XmIncludeStatus XmeGetDirection(XtPointer *in_out,
                                ...
                                XtPointer call_data) /* unused */
{
  ...
  *str_include = XmStringComponentCreate(
      XmSTRING_COMPONENT_DIRECTION, sizeof(dir), (XtPointer)&dir);
                             ...
                             Boolean *terminate)
{
  ...
  if (action == XmINVOKE) {
    /* Resolve parse procs. */
    if (pat->parse_proc)
      action = (pat->parse_proc)(
          (XtPointer *)ptr, text_end, type, tag, pat, length, &insertion, call_data);
        ...
        case XmSTRING_COMPONENT_WIDECHAR_TEXT:
          if (!tag)
            *next_text_match = True;
                   ...
                   (!_XmStrContTag(&n_context) || (tag == _XmStrContTag(&n_context)) ||
  ...
  for (n_pat = 0; !match && (n_pat < parse_count); n_pat++) {
    ...
    if (unparse_is_plausible(pat)) {
      ...
      for (n_comp = 0; match; n_comp++) {
        ...
        if ((p_ctype == XmSTRING_COMPONENT_TEXT) || (p_ctype == XmSTRING_COMPONENT_LOCALE_TEXT) ||
        ...
        else if ((m_ctype != p_ctype) || (m_length != p_length) ||
                 ((m_value != p_value) && memcmp(m_value, p_value, m_length)))
          match = False;
      }
                          ...
                          XmParseModel parse_model)
{
  ...
  if ((tag_type == XmCHARSET_TEXT) && tag &&
      ((tag == XmSTRING_DEFAULT_CHARSET) || (strcmp(tag, XmSTRING_DEFAULT_CHARSET) == 0)))
  ...
  if (!done) {
    _XmStringContextReInit(&stack_context, string);
    check_unparse_models(&stack_context,
                         tag,
                         tag_type,
                         parse_model,
                         &prev_text_match,
  ...
  while (!done) {
    ...
    switch (c_type) {
      ...
      default:
        ...
        if (non_text_match)
          unparse_components(
              &result, &length, output_type, &stack_context, parse_table, parse_count);
        /* Advance to the next component. */
        if (!done)
          (void)XmeStringGetComponent(&stack_context, True, False, &c_length, &c_value);
        break;
 ...
 */
XmStringComponentType XmeStringGetComponent(_XmStringContext context,
                                            Boolean update_context,
                                            Boolean copy_data,
                                            unsigned int *length,
                                            XtPointer *value)
  ...
  switch (state) {
    ...
    case TAG_STATE:
      ...
      if ((tag == XmSTRING_DEFAULT_CHARSET) ||
        ...
        tag = _XmStringGetCurrentCharset();
      if ((text_type != XmNO_TEXT && text_type != _XmStrContTagType(context)) ||
           ...
           (!_XmStrContTag(context) || strcmp(tag, _XmStrContTag(context)))))
      {
        ...
        if (!skip) {
          ...
          *length = strlen(tag);
          ...
          _XmProcessUnlock();
          return ((text_type == XmCHARSET_TEXT) ? XmSTRING_COMPONENT_TAG :
 ...
 */
void _XmStringContextCopy(_XmStringContext target, _XmStringContext source)
{
  int size;
 ...
 */
void _XmStringContextFree(_XmStringContext context)
{
  assert(context);
...
XmString XmStringGenerate(XtPointer text, XmStringTag tag, XmTextType type, XmStringTag rendition)
{
  ...
  if (!_XmStrOptimized(result)) {
    ...
    while (--n_line >= 0) {
      ...
      if (_XmEntrySegmentCountGet(line) > 0) {
        ...
        if (_XmStrImplicitLine(result))
          seg = (_XmStringEntry)_XmEntrySegmentGet(line)[_XmEntrySegmentCountGet(line) - 1];
        else
          seg = line;
        if (_XmEntryOptimized(seg)) {
          unsigned int rend_index;
  ...
  /* As a last resort merge the rendition components normally. */
  {
    ...
    tmp_1 = XmStringComponentCreate(
        XmSTRING_COMPONENT_RENDITION_BEGIN, strlen(rendition), rendition);
    tmp_2 = result;
    result = XmStringConcatAndFree(tmp_1, tmp_2);
    /* Append the rendition end. */
    tmp_1 = result;
    tmp_2 = XmStringComponentCreate(
        XmSTRING_COMPONENT_RENDITION_END, strlen(rendition), rendition);
    result = XmStringConcatAndFree(tmp_1, tmp_2);
  }
  _XmProcessUnlock();
  return result;
...
XtPointer _XmStringUngenerate(XmString string,
                              XmStringTag tag,
                              XmTextType tag_type,
                              XmTextType output_type)
{
