...
externaldef(xmdrawingareaclassrec) XmDrawingAreaClassRec xmDrawingAreaClassRec = {
    {
        /* core_class fields      */
...
externaldef(xmdrawingareawidgetclass)
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if (new_w->drawing_area.resize_policy != XmRESIZE_SWINDOW &&
      !XmRepTypeValidValue(XmRID_RESIZE_POLICY, new_w->drawing_area.resize_policy, (Widget)new_w))
  {
 ...
 ****************/
/*ARGSUSED*/
void _XmDrawingAreaInput(Widget wid,
                         XEvent *event,
                         ...
                         Cardinal *num_params) /* unused */
{
  ...
  if (!input_on_gadget && (!da->manager.active_child || button_event)) {
    ...
    cb.event = event;
    cb.window = XtWindow(da);
    XtCallCallbackList((Widget)da, da->drawing_area.input_callback, &cb);
  }
}
                                        ...
                                        XtWidgetGeometry *reply)
{
  ...
  return (_XmGMHandleGeometryManager((Widget)da,
                                     w,
                                     request,
                                     reply,
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (XtIsRealized((Widget)new_w) &&
      (((new_w->drawing_area.margin_width != current->drawing_area.margin_width) ||
        (new_w->drawing_area.margin_height != current->drawing_area.margin_height))))
  {
    ...
    while (idx < ((XmManagerWidget)wid)->composite.num_children) {
      if (_XmGetNavigability(children[idx]) &&
          ((((XmManagerWidget)wid)->manager.initial_focus == NULL) ||
           (((XmManagerWidget)wid)->manager.initial_focus == children[idx])))
      {
...
Widget XmVaCreateDrawingArea(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmDrawingAreaWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedDrawingArea(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmDrawingAreaWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
