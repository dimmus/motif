...
#define EVENTS \
  ((unsigned int)(ButtonPressMask | ButtonReleaseMask | EnterWindowMask | LeaveWindowMask))
    ...
    static void
    MenuTraverse(Widget w, XEvent *event, XmTraversalDirection direction);
static void GadgetCleanup(XmRowColumnWidget rc, XmGadget oldActiveChild);
static Boolean WrapRight(XmRowColumnWidget rc);
static Boolean WrapLeft(XmRowColumnWidget rc);
static void LocateChild(XmRowColumnWidget rc, Widget wid, XmTraversalDirection direction);
static void MoveDownInMenuBar(XmRowColumnWidget rc, Widget pw);
static void MoveLeftInMenuBar(XmRowColumnWidget rc, Widget pw);
static void MoveRightInMenuBar(XmRowColumnWidget rc, Widget pw);
static void FindNextMenuBarItem(XmRowColumnWidget menubar);
static void FindPrevMenuBarItem(XmRowColumnWidget menubar);
static Boolean ValidateMenuBarItem(Widget oldActiveChild, Widget newActiveChild);
...
static Boolean ValidateMenuBarCascade(Widget oldActiveChild, Widget newMenuChild);

/********    End Static Function Declarations    ********/
Boolean _XmIsActiveTearOff(Widget widget)
{
 ...
 */
int _XmGrabPointer(Widget widget,
                   Bool owner_events,
...
void _XmLeafPaneFocusOut(Widget wid)
{
  ...
  while (RC_PopupPosted(rc))
    rc = (XmRowColumnWidget)((XmMenuShellWidget)RC_PopupPosted(rc))->composite.children[0];
                 ...
                 Cardinal *num_params) /* unused */
{
  ...
  if (!_XmGetInDragMode((Widget)rc)) {
    ...
    if ((gadget = (XmGadget)XmObjectAtPoint((Widget)rc, event->xkey.x, event->xkey.y)) != NULL)
      _XmDispatchGadgetInput((Widget)gadget, event, XmHELP_EVENT);
    else {
      _XmSocorro((Widget)rc, event, NULL, NULL);
      _XmMenuPopDown((Widget)rc, event, NULL);
...
static void MenuTraverse(Widget w, XEvent *event, XmTraversalDirection direction)
{
  Widget parent;
  /*
  ...
  {
    _XmRecordEvent(event);
    (*(((XmRowColumnWidgetClass)XtClass(parent))->row_column_class.traversalHandler))(
        (Widget)parent, (Widget)w, direction);
  }
}

void _XmMenuTraverseLeft(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  if (_XmIsEventUnique(event)) {
...
void _XmMenuTraverseRight(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  if (_XmIsEventUnique(event)) {
...
void _XmMenuTraverseUp(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  if (_XmIsEventUnique(event)) {
...
void _XmMenuTraverseDown(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  if (_XmIsEventUnique(event)) {
...
void _XmMenuEscape(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
   ...
   */
  if ((XmIsCascadeButton(w) || XmIsCascadeButtonGadget(w)) && XmIsRowColumn(parent) &&
      (RC_Type(parent) == XmMENU_BAR) && !RC_PopupPosted(parent))
  {
    (*(((XmRowColumnClassRec *)XtClass(parent))->row_column_class.menuProcedures))(
...
void _XmRC_GadgetTraverseDown(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
...
void _XmRC_GadgetTraverseUp(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
  XmGadget gadget = (XmGadget)rc->manager.active_child;
  if (gadget && XmIsGadget(gadget))
...
void _XmRC_GadgetTraverseLeft(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
  XmGadget gadget = (XmGadget)rc->manager.active_child;
  /*
...
void _XmRC_GadgetTraverseRight(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
  XmGadget gadget = (XmGadget)rc->manager.active_child;
  /*
...
static Boolean WrapLeft(XmRowColumnWidget rc)
{
   ...
   * submenu.
   */
  if (XmIsMenuShell(XtParent(rc)) && (RC_Type(rc) != XmMENU_POPUP) && RC_CascadeBtn(rc) &&
      (RC_Type(XtParent(RC_CascadeBtn(rc))) == XmMENU_BAR) &&
      (FindPrevMenuBarCascade((XmRowColumnWidget)XtParent(RC_CascadeBtn(rc)))))
  {
    GadgetCleanup(rc, (XmGadget)oldActiveChild);
    done = True;
  }
   ...
   */
           ...
           (RC_Type(XtParent(RC_CascadeBtn(rc))) != XmMENU_OPTION) && XmIsMenuShell(XtParent(rc)))
  {
    (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownOne))(
        XtParent(rc), NULL, NULL, NULL);
    done = True;
  }
  return (done);
}
...
static void LocateChild(XmRowColumnWidget rc, Widget wid, XmTraversalDirection direction)
{
  ...
  if (XmIsRowColumn(wid) && ((XmManagerWidget)wid)->manager.active_child == 0) {
...
void _XmMenuTraversalHandler(Widget w, Widget pw, XmTraversalDirection direction)
{
  ...
  if (RC_Type(rc) != XmMENU_BAR) {
    ...
    if (direction == XmTRAVERSE_RIGHT && XmIsCascadeButtonGadget(pw) && CBG_Submenu(pw)) {
      (*(((XmGadgetClassRec *)XtClass(pw))->gadget_class.arm_and_activate))(
          (Widget)pw, NULL, NULL, NULL);
    }
    else if (direction == XmTRAVERSE_RIGHT && XmIsCascadeButton(pw) && CB_Submenu(pw)) {
      (*(((XmPrimitiveClassRec *)XtClass(pw))->primitive_class.arm_and_activate))(
          (Widget)pw, NULL, NULL, NULL);
    }
    else
...
static void MoveDownInMenuBar(XmRowColumnWidget rc, Widget pw)
{
  ...
  if (XmIsPrimitive(pw)) {
    ...
    (*(prim->primitive_class.arm_and_activate))((Widget)pw, NULL, NULL, NULL);
    ...
    (*(gad->gadget_class.arm_and_activate))((Widget)pw, NULL, NULL, NULL);
...
static void MoveLeftInMenuBar(XmRowColumnWidget rc, Widget pw)
{
  ...
  if ((mst->MU_CurrentMenuChild != NULL) && (RC_PopupPosted(rc) != NULL) &&
...
{
  XmMenuState mst = _XmGetMenuState((Widget)rc);
  if ((rc->manager.active_child == NULL) && ((XmIsCascadeButtonGadget(pw) && !CBG_Submenu(pw)) ||
                                             (XmIsCascadeButton(pw) && !CB_Submenu(pw))))
  {
  ...
  if (XmIsTraversable(newActiveChild)) {
    ...
    if (XmIsPrimitive(newActiveChild)) {
      ...
      if (!mst->MU_InPMMode && CB_Submenu(newActiveChild))
        (*(prim->primitive_class.arm_and_activate))(newActiveChild, NULL, NULL, NULL);
    }
      ...
      if (!mst->MU_InPMMode && CBG_Submenu(newActiveChild))
        (*(gadget->gadget_class.arm_and_activate))(newActiveChild, NULL, NULL, NULL);
    }
    return True;
  }
  else
...
static Boolean FindNextMenuBarCascade(XmRowColumnWidget menubar)
{
  ...
  if (shell != NULL)
    active_child = mst->MU_CurrentMenuChild = RC_CascadeBtn(shell->composite.children[0]);
  /* Find the index of the currently active item */
  for (i = 0; i < upper_limit; i++) {
...
static Boolean ValidateMenuBarCascade(Widget oldActiveChild, /* unused */
                                      Widget newMenuChild)
{
  ...
  if (XmIsTraversable(newMenuChild)) {
    if (XmIsCascadeButtonGadget(newMenuChild)) {
      ...
      if (RC_PopupPosted(menubar) && !CBG_Submenu(newMenuChild)) {
        ...
        (*(gadget->gadget_class.arm_and_activate))(newMenuChild, NULL, NULL, NULL);
      }
      return True;
    }
      ...
      if (RC_PopupPosted(menubar) && !CB_Submenu(newMenuChild)) {
        ...
        (*(prim->primitive_class.arm_and_activate))(newMenuChild, NULL, NULL, NULL);
      }
      return True;
    }
  }
