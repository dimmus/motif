...
#include <string.h>
     ...
     ************************************************************************/
    XmGadget
    _XmInputForGadget(Widget wid, int x, int y)
{
  Widget widget;
    ...
    if (XmIsGadget(child) && XtIsManaged(child)) {
      if (region == NULL) {
            ...
            child->core.y + child->core.height > event->xexpose.y)
        {
          ...
          _XmProcessUnlock();
          if (expose)
            (*(expose))(child, event, region);
        }
      }
      else {
        if (XRectInRegion(
                region, child->core.x, child->core.y, child->core.width, child->core.height))
        {
          _XmProcessLock();
          expose = child->core.widget_class->core_class.expose;
          _XmProcessUnlock();
 ...
 *
 ************************************************************************/
void _XmDispatchGadgetInput(Widget wid, XEvent *event, Mask mask)
{
  XmGadget g = (XmGadget)wid;
  if ((g->gadget.event_mask & mask) && XtIsSensitive((Widget)g) && XtIsManaged((Widget)g)) {
    if (event != NULL) {
      XEvent synth_event;
#define CopyEvent(source, dest, type) source.type = dest->type
      switch (mask) {
        case XmENTER_EVENT:
        ...
        case XmACTIVATE_EVENT:
          CopyEvent(synth_event, event, xkey);
          if (event->type != ButtonRelease && event->type != KeyPress) {
        ...
        case XmHELP_EVENT:
          ...
          break;
        default:
          memcpy((char *)&synth_event, (char *)event, (size_t)sizeof(synth_event));
          break;
      }
      (*(((XmGadgetClass)(g->object.widget_class))->gadget_class.input_dispatch))(
          (Widget)g, (XEvent *)&synth_event, mask);
    }
    else {
      (*(((XmGadgetClass)(g->object.widget_class))->gadget_class.input_dispatch))(
          (Widget)g, (XEvent *)event, mask);
    }
  }
}
