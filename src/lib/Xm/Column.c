...
static Boolean CvtStringToDistribution(
    Display *, XrmValue *, Cardinal *, XrmValue *, XrmValue *, XtPointer);
static void Layout(XmColumnWidget, Widget, XtWidgetGeometry *, int, int);
static void HorizontalLayout(XmColumnWidget, Widget, XtWidgetGeometry *, int, int);
static void VerticalLayout(XmColumnWidget, Widget, XtWidgetGeometry *, int, int);
static void VerifyResources(XmColumnWidget, XmColumnWidget, XmColumnWidget);
static void VerifyConstraints(Widget, Widget, Widget);
static void CalcSize(
    XmColumnWidget, Widget, XtWidgetGeometry *, Boolean, Dimension *, Dimension *);
static Boolean CompareGeometry(XtWidgetGeometry *, XtWidgetGeometry *);
static Boolean CompareGeometryToWidget(XtWidgetGeometry *, Widget);
...
#define XiValidChild(c) \
  (((c)) != NULL && XtIsManaged((c)) && !(c)->core.being_destroyed && XiC(c)->label_widget != NULL)
#define XmColumn(c) ((XmColumnWidget)XtParent((c)))
#define XiAlignment(c) \
...
static XtResource resources[] = {{"pri.vate",
                                  ...
                                  XmRBoolean,
                                  sizeof(Boolean),
                                  XtOffsetOf(XmColumnRec, column.check_set_render_table),
                                  XmRImmediate,
                                  (XtPointer)False},
                                 {XmNdefaultEntryLabelFontList,
                                  ...
                                  XmRFontList,
                                  sizeof(XmFontList),
                                  XtOffsetOf(XmBulletinBoardRec, bulletin_board.label_font_list),
                                  XmRCallProc,
                                  (XtPointer)CheckSetDefaultEntryLabelRenderTable},
...
#if 0 /* POSITION HANDLING */
  {
    ...
    XmRImmediate, (XtPointer) DEFAULT_POSITION
                                 ...
                                 {XmNdefaultFillStyle,
                                  ...
                                  XmRFillStyle,
                                  sizeof(unsigned char),
                                  XtOffsetOf(XmColumnRec, column.default_fill_style),
                                  XmRImmediate,
                                  (XtPointer)DEFAULT_FILL_STYLE},
                                 {XmNitemSpacing,
                                  XmCItemSpacing,
                                 ...
                                 {XmNorientation,
                                  ...
                                  XmROrientation,
                                  sizeof(unsigned char),
                                  XtOffsetOf(XmColumnRec, column.orientation),
                                  XmRImmediate,
                                  (XtPointer)DEFAULT_ORIENTATION},
                                 {XmNdistribution,
                                  XmCDistribution,
                                  XmRDistribution,
                                  sizeof(unsigned char),
                                  XtOffsetOf(XmColumnRec, column.distribution),
                                  XmRImmediate,
                                  (XtPointer)XmDISTRIBUTE_TIGHT}};
static XmSyntheticResource get_resources[] = {{XmNlabelSpacing,
                                               sizeof(Dimension),
                                               XtOffsetOf(XmColumnRec, column.label_spacing),
...
static XtResource constraint_resources[] = {
    {"pri.vate",
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmColumnConstraintRec, column.check_set_render_table),
     XmRImmediate,
     (XtPointer)False},
    {XmNentryLabelFontList,
     ...
     XmRFontList,
     sizeof(XmFontList),
     XtOffsetOf(XmColumnConstraintRec, column.label_font_list),
     XmRCallProc,
     (XtPointer)CheckSetEntryLabelRenderTable},
...
#if 0 /* POSITION HANDLING */
  {
    ...
    {XmNfillStyle,
     ...
     XmRFillStyle,
     sizeof(unsigned char),
     XtOffsetOf(XmColumnConstraintRec, column.fill_style),
     XmRImmediate,
     (XtPointer)XmFILL_UNSPECIFIED},
    {XmNentryLabelType,
     XmCLabelType,
     XmRLabelType,
     sizeof(unsigned char),
     XtOffsetOf(XmColumnConstraintRec, column.label_type),
     XmRImmediate,
     (XtPointer)XmSTRING},
    {XmNentryLabelString,
     XmCLabelString,
    ...
    {XmNentryLabelPixmap,
     ...
     sizeof(Pixmap),
     XtOffsetOf(XmColumnConstraintRec, column.label_pixmap),
     XmRImmediate,
    ...
    {XmNshowEntryLabel,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmColumnConstraintRec, column.show_label),
     XmRImmediate,
     (XtPointer)True},
    {XmNstretchable,
     XmCStretchable,
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmColumnConstraintRec, column.stretchable),
     XmRImmediate,
     (XtPointer)False}};
/*
...
static XmSyntheticResource cont_get_resources[]
    __attribute__((unused)) = {{XmNentryLabelString,
                                sizeof(XmString),
                                XtOffsetOf(XmColumnConstraintRec, column.label_string),
                                Get_entryLabelString,
                                (XmImportProc)NULL}};
...
static void ClassInitialize(void)
{
  ...
  XtSetTypeConverter(XmRString,
                     ...
                     NULL);
  XtSetTypeConverter(
      XmRString, XmRFillStyle, (XtTypeConverter)CvtStringToFillStyle, NULL, 0, XtCacheAll, NULL);
  XtSetTypeConverter(XmRString,
                     XmRDistribution,
...
{
  XmColumnWidget rc = (XmColumnWidget)request, sc = (XmColumnWidget)set;
...
{
  XmColumnWidget cc = (XmColumnWidget)current, cs = (XmColumnWidget)set;
  Boolean request_size = False, relayout = False;
  ...
  for (i = 0, kid = kids; i < kidCnt; ++i, ++kid) {
    if ((*kid) == NULL || (*kid)->core.being_destroyed || XiC(*kid)->label_widget == NULL)
      continue;
  ...
  if (BBPart(cc)->label_font_list != BBPart(cs)->label_font_list) {
    for (i = 0, kid = kids; i < kidCnt; ++i, ++kid) {
      ...
      if (XiC(*kid)->label_font_list == NULL) {
        XtVaSetValues(XiC(*kid)->label_widget, XmNrenderTable, BBPart(cs)->label_font_list, NULL);
      }
    }
  }
  if (XmColumn_default_label_alignment(cc) != XmColumn_default_label_alignment(cs)) {
    for (i = 0, kid = kids; i < kidCnt; ++i, ++kid) {
      ...
      if (XiC(*kid)->label_alignment == XmALIGNMENT_UNSPECIFIED) {
        XtVaSetValues(
            XiC(*kid)->label_widget, XmNalignment, XmColumn_default_label_alignment(cs), NULL);
      }
    }
  }
  if (request_size) {
...
static void ChangeManaged(Widget widget)
{
  ...
  for (i = 0; i < cw->composite.num_children; ++i, ++kid) {
    if (!XiValidChild(*kid)) {
      if ((*kid) != NULL && !XtIsManaged(*kid)) { /* CR03731 */
...
static void ConstraintInitialize(Widget request, Widget new_w, ArgList arg_list, Cardinal *arg_cnt)
{
  ...
  if (label_widget) {
    ...
    nargs++;
    XtSetArg(args[nargs], XmNlabelString, XiC(new_w)->label_string);
    nargs++;
    XtSetArg(args[nargs], XmNlabelPixmap, XiC(new_w)->label_pixmap);
    nargs++;
    ...
    nargs++;
    XtSetArg(args[nargs], XmNbackground, cw->core.background_pixel);
    nargs++;
...
static Boolean ConstraintSetValues(
    Widget current, Widget request, Widget new_w, ArgList arg_list, Cardinal *arg_cnt)
{
  XmColumnWidget cw = (XmColumnWidget)XtParent(new_w);
  XmColumnConstraintPart *cc = XiC(current), *sc = XiC(new_w);
  Boolean relayout = False;
  Arg args[10];
  Cardinal i = 0;
  if (XiC(new_w)->label_widget == NULL)
    return (False);
  ...
  if (i > 0)
    XtSetValues(sc->label_widget, args, i);
  XmColumn_resize_done(cw) = False;
...
static void ConstraintDestroy(Widget widget)
{
  ...
  if (XiC(widget)->label_widget != NULL) {
    XtRemoveCallback(XiC(widget)->label_widget,
                     XmNdestroyCallback,
                     XmColumnLabelDestroyedCallback,
                     (XtPointer)widget);
 ...
 */
static void ConstraintGetValues(Widget w, ArgList args, Cardinal *num_args)
{
  XrmQuark quark;
  int i;
 ...
 */
static Boolean CvtStringToXiAlignment(
    Display *dpy, XrmValue *args, Cardinal *arg_cnt, XrmValue *from, XrmValue *to, XtPointer data)
{
 ...
 *	None.
 */
static void VerifyResources(XmColumnWidget request, XmColumnWidget current, XmColumnWidget new_w)
{
  Boolean reset;
  ...
  switch (XmColumn_default_label_alignment(new_w)) {
    ...
    case XmALIGNMENT_UNSPECIFIED:
      XmeWarning((Widget)new_w,
                 "The illegal resource value \"XmALIGNMENT_UNSPECIFIED\" was assigned to the "
                 "resource XmNdefaultEntryLabelAlignment");
      reset = True;
      break;
    default:
      XmeWarning(
  ...
  if (reset) {
    XmColumn_default_label_alignment(
        new_w) = (current != NULL ? XmColumn_default_label_alignment(current) : DEFAULT_ALIGNMENT);
  }
  reset = False;
  ...
  if (reset) {
    XmColumn_orientation(new_w) = (current != NULL ? XmColumn_orientation(current) :
                                                     DEFAULT_ORIENTATION);
  }
}
 ...
 */
static void Layout(
    XmColumnWidget cw, Widget child, XtWidgetGeometry *child_size, int col_width, int col_height)
{
    ...
    XmColumnWidget cw, Widget child, XtWidgetGeometry *child_size, int col_width, int col_height)
{
  ...
  int kidSpace;
  if (col_width < 0)
    col_width = XtWidth(cw);
  if (col_height < 0)
    col_height = XtHeight(cw);
  fillHeight = col_height - 2 * (cw->manager.shadow_thickness + BBPart(cw)->margin_height);
  for (i = 0, kid = kids, lWidth = 0, cbWidth = 0, valid = 0, space = 0, kidSpace = 0; i < kidCnt;
       ++i, ++kid)
  {
  ...
  if (valid > 0 && (x + lWidth + space + cbWidth + ispace * (valid - 1) + x > col_width)) {
     ...
     */
    int have = (col_width - 2 * x), want = lWidth + space + ispace * (valid - 1) + cbWidth,
        diff = want - have;
    if (space - diff > 1) {
      space -= diff;
    }
    ...
    XmColumnWidget cw, Widget child, XtWidgetGeometry *child_size, int col_width, int col_height)
{
  ...
  Position x, y;
  int cWidth, cMaxWidth, cMinWidth, cHeight, cBorder, cbWidth, cbHeight, lWidth, lHeight, cnt,
      hExtra, hEach = 0, hLeft = 0, valid, space;
  Dimension width, height;
  ...
  if (col_height < 0)
    col_height = XtHeight(cw);
  CalcSize(cw, NULL, NULL, False, &width, &height);
  for (i = 0, kid = kids, lWidth = 0, cMinWidth = 0, cMaxWidth = 0, cnt = 0, valid = 0, space = 0;
       i < kidCnt;
       ++i, ++kid)
  {
    XiC(*kid)->position.x = XiC(*kid)->position.y = 0;
  ...
  if (valid > 0 && (x + lWidth + space + cMaxWidth + x > col_width)) {
    int have = (col_width - 2 * x), want = lWidth + space + cMaxWidth, diff = want - have;
  ...
  for (kid = kids, i = 0; i < kidCnt; ++i, ++kid) {
    ...
    cbWidth = col_width - ((int)cw->manager.shadow_thickness + (int)BBPart(cw)->margin_width + x +
                           lWidth + space);
    if (cbWidth < 1)
      cbWidth = 1;
...
static void VerifyConstraints(Widget request, Widget current, Widget set)
{
  ...
  if (reset) {
    XiC(set)->label_alignment = (current != NULL ? XiC(current)->label_alignment :
  ...
  switch (XiC(set)->fill_style) {
    ...
    case XmFILL_RAGGED:
      break;
    default:
      XmeWarning(set, "An illegal resource value was assigned to the resource XmNfillStyle");
      reset = True;
      break;
  }
  if (reset) {
    XiC(set)->fill_style = (current != NULL ? XiC(current)->fill_style : XmFILL_UNSPECIFIED);
  }
}
 ...
 *	None.
 */
static void CalcSize(XmColumnWidget cw,
                     Widget child,
                     ...
                     Dimension *height)
{
  ...
  for (i = 0, kid = kids; i < kidCnt; ++i, ++kid) {
    ...
    if (XmColumn_orientation(cw) == XmVERTICAL) {
      ...
      if (_height < (int)cHeight)
        _height = cHeight;
      if (_height < (int)lHeight)
        _height = lHeight;
  ...
  if (XmColumn_orientation(cw) == XmVERTICAL) {
    ...
    _height += (cnt * XmColumn_item_spacing(cw) +
                2 * (cw->manager.shadow_thickness + BBPart(cw)->margin_height));
  }
  else /* XmHORIZONTAL Layout */ {
    _width += (hSumSpace + 2 * (cw->manager.shadow_thickness + BBPart(cw)->margin_width)) +
              cnt * XmColumn_item_spacing(cw);
 ...
 */
static Boolean CompareGeometry(XtWidgetGeometry *geom1, XtWidgetGeometry *geom2)
{
  Boolean result;
  result = (geom1 == NULL || geom2 == NULL) || (geom1->request_mode != geom2->request_mode) ||
 ...
 */
static Boolean CompareGeometryToWidget(XtWidgetGeometry *geom, Widget widget)
{
  Boolean result;
  result = (geom == NULL || widget == NULL) || (geom->request_mode == 0) ||
           ...
           (geom->request_mode & CWBorderWidth && geom->border_width != XtBorderWidth(widget));
  return (!result);
}
 ...
 *	None.
 */
static void XmColumnLabelDestroyedCallback(Widget widget, XtPointer client, XtPointer cbdata)
{
 ...
 */
Widget XmCreateColumn(Widget parent, String name, ArgList arg_list, Cardinal arg_cnt)
{
...
Widget XmVaCreateColumn(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmColumnWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedColumn(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmColumnWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
