...
#include <Xm/XmosP.h>
    ...
    InsertSelection(Widget w,
                    ...
                    int *format,
                    XtPointer tid);
static void HandleInsertTargets(Widget w,
                                XtPointer closure,
                                Atom *seltype,
                                Atom *type,
                                XtPointer value,
                                ...
                                int *format,
                                XtPointer tid);
static void HandleDrop(Widget w, XmDropProcCallbackStruct *cb, XmDestinationCallbackStruct *ds);
static void HandleTargets(Widget w, XtPointer ignore, XmSelectionCallbackStruct *ds);
static void DoStuff(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds);
                                ...
                                XmTransferDoneCallbackStruct *ts) /* unused */
{
  ...
  if (prim_select->num_chars > 0) {
    ...
    (*tw->text.source->SetSelection)(
        tw->text.source, data->anchor, tw->text.cursor_position, prim_select->time);
  }
...
/*ARGSUSED*/
static void CleanPrimarySelection(Widget w,
                                  XtEnum op,                        /* unused */
                                  XmTransferDoneCallbackStruct *ts) /* unused */
{
  _XmProcessLock();
...
static void TextSecondaryWrapper(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
  Atom XA_TARGETS = XInternAtom(XtDisplay(w), XmSTARGETS, False);
  if (ds->target == XA_TARGETS)
    HandleInsertTargets(w,
                        closure,
                        &(ds->selection),
                        &(ds->type),
                        ds->value,
                            ...
                            XtPointer tid)
{
  ...
  if (_XmTextModifyVerify(tw,
                          ...
                          &freeBlock))
  {
    if ((*tw->text.source->Replace)(
            tw, (XEvent *)_insert_select->event, &left, &right, &newblock, False) != EditDone)
    {
      ...
      if (!tw->text.add_mode)
        tw->text.input->data->anchor = left;
      ...
      _XmTextSetDestinationSelection(
          w, tw->text.cursor_position, False, _insert_select->event->time);
                                ...
                                XtPointer tid)
{
  ...
  static char *atom_names[] = {XmSTEXT,
                               XmSCOMPOUND_TEXT,
#if XM_UTF8
                               XmSUTF8_STRING
#endif
  ...
  if (supports_text && supports_encoding_data)
    ...
    target = XA_STRING;
  XmTransferValue(
      tid, target, (XtCallbackProc)TextSecondaryWrapper, closure, _insert_select->event->time);
}

/* ARGSUSED */
Boolean _XmTextConvert(Widget w,
                       Atom *selection,
                       Atom *target,
                       Atom *type,
                       XtPointer *value,
  ...
  if (*target == atoms[XmATARGETS]) {
    ...
    *format = 8;
    ...
    status = XmbTextListToTextProperty(
        XtDisplay(tw), &tmp_value, 1, (XICCEncodingStyle)XStringStyle, &tmp_prop);
    XtFree(tmp_value);
    ...
    *format = 8;
    ...
    tmp_value = _XmStringSourceGetString(tw, left, right, False);
    status = XmbTextListToTextProperty(
        XtDisplay(tw), &tmp_value, 1, (XICCEncodingStyle)XCompoundTextStyle, &tmp_prop);
    XtFree(tmp_value);
    if (status == Success || status > 0) {
      /* NOTE: casting tmp_prop.nitems could result in a truncated long. */
      *value = (XtPointer)XtMalloc((unsigned)tmp_prop.nitems);
    ...
    *format = 8;
    ...
    tmp_value = _XmStringSourceGetString(tw, left, right, False);
    status = XmbTextListToTextProperty(
        XtDisplay(tw), &tmp_value, 1, (XICCEncodingStyle)XUTF8StringStyle, &tmp_prop);
    XtFree(tmp_value);
    if (status == Success || status > 0) {
      /* NOTE: casting tmp_prop.nitems could result in a truncated long. */
      *value = (XtPointer)XtMalloc((unsigned)tmp_prop.nitems);
      ...
      if ((*tw->text.source->Replace)(tw, event, &left, &right, &newblock, False) != EditDone) {
        ...
        if (is_drop) {
          ...
          if ((*selection == atoms[XmACLIPBOARD]) ||
              (req_event != NULL && req_event->requestor != XtWindow((Widget)tw)))
            _XmTextSetCursorPosition(w, cursorPos);
        }
    ...
    (*tw->text.source->SetSelection)(
        tw->text.source, tw->text.cursor_position, tw->text.cursor_position, _time);
    *type = atoms[XmANULL];
    *value = NULL;
    *length = 0;
    *format = 8;
...
/* ARGSUSED */
void _XmTextLoseSelection(Widget w, Atom *selection)
{
  XmTextWidget tw = (XmTextWidget)w;
...
static void HandleDrop(Widget w, XmDropProcCallbackStruct *cb, XmDestinationCallbackStruct *ds)
{
  ...
  if (cb->operation & XmDROP_MOVE && w == initiator &&
      ((*tw->text.source->GetSelection)(tw->text.source, &left, &right) && left != right &&
       insert_pos >= left && insert_pos <= right))
  {
...
#if XM_UTF8
      if (exportTargets[n] == atoms[XmAUTF8_STRING])
        utf8_string_found = True;
#endif
      if (exportTargets[n] == atoms[XmACOMPOUND_TEXT])
        c_text_found = True;
      if (exportTargets[n] == XA_STRING)
        string_found = True;
      if (exportTargets[n] == atoms[XmATEXT])
        text_found = True;
    }
    n = 0;
    if (encoding_found || c_text_found || string_found || text_found || utf8_string_found) {
      if (!encoding_found) {
        if (c_text_found)
  ...
  if (doTransfer) {
    ...
    XmTransferValue(
        tid, desiredTarget, (XtCallbackProc)DropTransferProc, (XtPointer)transfer_rec, 0);
  }
}
 ...
 * successful exchange.
 */
static void HandleTargets(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
  enum {
    XmACOMPOUND_TEXT,
    XmACLIPBOARD,
  ...
  if (ds->selection != atoms[XmACLIPBOARD]) {
    if ((*tw->text.source->GetSelection)(tw->text.source, &left, &right) && left != right &&
        select_pos > left && select_pos < right)
    {
      XtFree((char *)ds->value);
      ds->value = NULL;
      return;
...
static void DoStuff(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
  ...
  static char *atom_names[] = {XmSNULL,
                               XmSCLIPBOARD,
                               XmSTEXT,
                               XmSCOMPOUND_TEXT,
#if XM_UTF8
  ...
  if (ds->type == atoms[XmANULL]) {
    ...
    if (_prim_select->num_chars > 0 && data->selectionMove) {
      ...
      (*tw->text.source->SetSelection)(
          tw->text.source, data->anchor, tw->text.cursor_position, _prim_select->time);
    }
    _XmProcessUnlock();
  }
  else {
    ...
    if (ds->type == atoms[XmACOMPOUND_TEXT] ||
        ...
        ds->type == XA_STRING)
    {
      if ((total_value = _XmTextToLocaleText(
               w, ds->value, ds->type, ds->format, ds->length, NULL)) != NULL)
      {
        block.ptr = total_value;
        block.length = strlen(block.ptr);
      }
    ...
    if (_XmTextModifyVerify(
            tw, ds->event, &replace_from, &replace_to, &cursorPos, &block, &newblock, &freeBlock))
    {
      _XmProcessLock();
      _prim_select->num_chars = _XmTextCountCharacters(newblock.ptr, newblock.length);
      _XmProcessUnlock();
      if ((*tw->text.source->Replace)(
              tw, ds->event, &replace_from, &replace_to, &newblock, False) != EditDone)
      {
        ...
        if ((*tw->text.source->GetSelection)(tw->text.source, &left, &right)) {
          if (ds->selection == atoms[XmACLIPBOARD]) {
            ...
            if (left != right && (!dest_disjoint || !tw->text.add_mode))
              (*source->SetSelection)(
                  source, tw->text.cursor_position, tw->text.cursor_position, _prim_select->time);
          }
          else {
...
static void DropDestroyCB(Widget w, XtPointer clientData, XtPointer callData)
{
  ...
  DeleteDropContext(w);
  if (ts->client_data != NULL)
    XtFree((char *)ts->client_data);
}

/* ARGSUSED */
static void DropTransferProc(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
  enum {
    XmACOMPOUND_TEXT,
    XmANULL,
  ...
  if (ds->type == atoms[XmANULL]) {
    if (transfer_rec->num_chars > 0 && transfer_rec->move) {
      ...
      _XmTextSetCursorPosition((Widget)tw, cursorPos);
      _XmTextSetDestinationSelection(
          (Widget)tw, tw->text.cursor_position, False, XtLastTimestampProcessed(XtDisplay(w)));
      (*tw->text.source->SetSelection)(tw->text.source,
                                       data->anchor,
                                       tw->text.cursor_position,
  ...
  block.format = XmFMT_8_BIT;
  if (data->pendingdelete && (*tw->text.source->GetSelection)(tw->text.source, &left, &right) &&
      (left != right))
  {
    if (insertPosLeft > left && insertPosLeft < right)
      insertPosLeft = left;
    if (insertPosRight < right && insertPosRight > left)
  ...
  {
    if ((*tw->text.source->Replace)(
            tw, ds->event, &insertPosLeft, &insertPosRight, &newblock, False) != EditDone)
    {
      if (tw->text.verify_bell)
      ...
      transfer_rec->num_chars = _XmTextCountCharacters(newblock.ptr, newblock.length);
      if (transfer_rec->num_chars > 0 && !transfer_rec->move) {
        _XmTextSetCursorPosition((Widget)tw, cursorPos);
        _XmTextSetDestinationSelection(
            (Widget)tw, tw->text.cursor_position, False, transfer_rec->timestamp);
      }
      if ((*tw->text.source->GetSelection)(tw->text.source, &left, &right)) {
...
Widget _XmTextGetDropReciever(Widget w)
{
  Display *display = XtDisplay(w);
  Screen *screen = XtScreen(w);
  Widget widget;
 ...
 ********************************************/
/*ARGSUSED*/
static void TextConvertCallback(Widget w,
                                ...
                                XmConvertCallbackStruct *cs)
{
  ...
  static char *atom_names[] = {XmSDELETE,
  ...
  if (cs->selection == atoms[XmACLIPBOARD] && cs->parm == (XtPointer)XmLINK &&
      (cs->target == atoms[XmA_MOTIF_CLIPBOARD_TARGETS] || cs->target == atoms[XmATARGETS]))
    return;
  if (!_XmTextConvert(w,
                      &cs->selection,
                      &cs->target,
 ...
 ************************************************/
/*ARGSUSED*/
static void FreeLocationData(Widget w,  /* unused */
                             XtEnum op, /* unused */
...
/*ARGSUSED*/
static void TextDestinationCallback(Widget w,
