...
static void FixWidget(XmRowColumnWidget m, Widget w);
...
static Boolean DoEntryStuff(XmRowColumnWidget old, XmRowColumnWidget new_w);
static void DoSize(XmRowColumnWidget old, XmRowColumnWidget new_w);
static Boolean set_values_non_popup(XmRowColumnWidget old, XmRowColumnWidget new_w);
static Boolean set_values_popup(XmRowColumnWidget old, XmRowColumnWidget new_w);
static void set_values_passive_grab(XmRowColumnWidget old, XmRowColumnWidget new_w);
...
static char *GetRealKey(XmRowColumnWidget rc, char *str);
static void MenuBarInitialize(XmRowColumnWidget bar);
static void PreparePostFromList(XmRowColumnWidget rowcol);
static void PopupInitialize(XmRowColumnWidget popup);
static void PulldownInitialize(XmRowColumnWidget pulldown);
static void OptionInitialize(XmRowColumnWidget option);
static void WorkAreaInitialize(XmRowColumnWidget work);
...
static Boolean ConstraintSetValues(
    Widget old, Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static Widget create(
    Widget p, char *name, ArgList old_al, Cardinal old_ac, int type, int is_radio);
static void ClassInitialize(void);
static void ClassPartInitialize(WidgetClass rcc);
...
static Boolean TraversalChildren(Widget wid, Widget **childList, Cardinal *numChildren);
static void FixEventBindings(XmRowColumnWidget m, Widget w);
static Widget FindFirstManagedChild(CompositeWidget m, Boolean first_button);
static void Resize(Widget wid);
static void Redisplay(Widget w, XEvent *event, Region region);
static void FixVisual(XmRowColumnWidget m, Widget w);
static void FixCallback(XmRowColumnWidget m, Widget w);
static void ActionNoop(Widget wid, XEvent *event, String *param, Cardinal *num_param);
static void EventNoop(Widget reportingWidget, XtPointer data, XEvent *event, Boolean *cont);
static void MenuFocusIn(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void MenuFocusOut(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void MenuUnmap(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void MenuEnter(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void GadgetEscape(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void GetMnemonicCharSet(Widget wid, int resource, XtArgVal *value);
static void GetMenuAccelerator(Widget wid, int resource, XtArgVal *value);
static void GetMenuPost(Widget wid, int resource, XtArgVal *value);
static void GetLabelString(Widget wid, int resource_offset, XtArgVal *value);
static void GetTearOffTitle(Widget wid, int resource_offset, XtArgVal *value);
 ...
 */
static XtActionsRec actions[] = {{"Help", _XmManagerHelp},
...
static XmManagerClassExtRec managerClassExtRec = {NULL,
                                                  NULLQUARK,
 ...
 */
externaldef(xmrowcolumnwidgetclass)
...
static void Destroy(Widget w)
{
  ...
  if (RC_TornOff(m)) {
    if (!XmIsMenuShell(XtParent(m))) {
       ...
       */
      XtRemoveAllCallbacks(m->row_column.tear_off_lastSelectToplevel, XmNdestroyCallback);
      ...
      if (XmIsMenuShell(RC_ParentShell(m))) {
        ...
        _XmProcessLock();
        delete_child = ((CompositeWidgetClass)RC_ParentShell(m)->core.widget_class)
                           ->composite_class.delete_child;
        _XmProcessUnlock();
        (*delete_child)((Widget)m);
      }
    }
    else if (RC_ParentShell(m)) {
  ...
  if (IsPopup(m)) {
    ...
    for (i = 0; i < m->row_column.postFromCount; i++) {
      if (!m->row_column.postFromList[i]->core.being_destroyed) {
        XtRemoveCallback(m->row_column.postFromList[i],
                         XtNdestroyCallback,
    ...
    XtRemoveEventHandler(
        topManager, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, m);
    /* Remove it from us */
    XtRemoveEventHandler(
        (Widget)m, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)m);
  }
  /*
  ...
  if ((IsPopup(m) && RC_PopupEnabled(m)) || (IsBar(m) && RC_MenuAccelerator(m)) ||
  ...
  if (!IsPulldown(m) && RC_MenuPost(m))
    XtFree(RC_MenuPost(m));
  /* Free accelerator string */
  if ((IsPopup(m) && RC_MenuAccelerator(m)) || (IsBar(m) && RC_MenuAccelerator(m)))
      ...
      (RC_EntryClass(m) != XtClass(w)))
  {
    ...
    if (!((_XmIsFastSubclass(RC_EntryClass(m), XmCASCADE_BUTTON_BIT) &&
          ...
          (_XmIsFastSubclass(RC_EntryClass(m), XmTOGGLE_BUTTON_GADGET_BIT) &&
           XmIsToggleButton(w)) ||
          (_XmIsFastSubclass(RC_EntryClass(m), XmPUSH_BUTTON_BIT) && XmIsPushButtonGadget(w)) ||
          (_XmIsFastSubclass(RC_EntryClass(m), XmPUSH_BUTTON_GADGET_BIT) && XmIsPushButton(w))))
    {
 ...
 * utilized.
 */
static void ResetMatchingOptionMemWidget(XmRowColumnWidget menu, Widget child)
{
  int i;
  if (IsPulldown(menu)) {
    for (i = 0; i < menu->row_column.postFromCount; i++) {
      ResetMatchingOptionMemWidget((XmRowColumnWidget)XtParent(menu->row_column.postFromList[i]),
                                   child);
    }
  }
    ...
    if (RC_OptionSubMenu(menu) && RC_MemWidget(RC_OptionSubMenu(menu))) {
      ...
      RC_MemWidget(menu) = FindFirstManagedChild((CompositeWidget)RC_OptionSubMenu(menu), True);
...
static void DeleteChild(Widget child)
{
   ...
   */
  if (XtIsWidget(child) && ((IsPopup(m) || IsBar(m) || IsPulldown(m)) && XmIsLabel(child) &&
                            (child->core.widget_class != xmLabelWidgetClass)))
  {
    XtRemoveEventHandler(
        child, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, m);
  }
  /*
   ...
   */
  {
    XtWidgetProc delete_child;
    _XmProcessLock();
    delete_child = ((CompositeWidgetClass)compositeWidgetClass)->composite_class.delete_child;
    _XmProcessUnlock();
    (*delete_child)(child);
  }
  /*
...
static void ChangeManaged(Widget wid)
{
  ...
  if ((PackColumn(m) && (IsVertical(m) || IsHorizontal(m))) || (PackTight(m) && IsHorizontal(m))) {
  ...
  if (m->row_column.old_shadow_thickness)
    _XmClearShadowType((Widget)m,
                       m->row_column.old_width,
                       m->row_column.old_height,
                       m->row_column.old_shadow_thickness,
                       0);
  /* and redraw it for shrinking size */
  if (XtIsRealized((Widget)m) && m->manager.shadow_thickness)
    XmeDrawShadows(XtDisplay(m),
                   XtWindow(m),
...
static void Realize(Widget wid, XtValueMask *window_mask, XSetWindowAttributes *window_attributes)
{
  ...
  if (!IsWorkArea(m)) {
    ...
    XtAddEventHandler((Widget)m, OwnerGrabButtonMask, False, EventNoop, NULL);
  }
  /*
  ...
  window_attributes->do_not_propagate_mask = ButtonPressMask | ButtonReleaseMask | KeyPressMask |
                                             KeyReleaseMask | PointerMotionMask;
  XtCreateWindow((Widget)m, InputOutput, CopyFromParent, *window_mask, window_attributes);
  /*
...
static Boolean DoEntryStuff(XmRowColumnWidget old, XmRowColumnWidget new_w)
{
  ...
  if ((RC_EntryAlignment(old) != RC_EntryAlignment(new_w)) && (IsAligned(new_w)) &&
      (!IsOption(new_w)))
  {
    Widget *p;
    Arg al[2];
  ...
  if ((RC_EntryVerticalAlignment(old) != RC_EntryVerticalAlignment(new_w)) && (!IsOption(new_w)))
    need_expose = TRUE;
  return (need_expose);
}
...
static void DoSize(XmRowColumnWidget old, XmRowColumnWidget new_w)
{
  ...
  Dimension w;
  Dimension h;
  if (orient) /* flip all the separator */ { /* widgets too */
    Arg al[2];
    int ac = 0;
    XtSetArg(al[ac], XmNorientation, (IsVertical(new_w) ? XmHORIZONTAL : XmVERTICAL));
    ForAllChildren(new_w, i, p)
    {
  ...
  if ((!XtWidth(new_w)) || (XtWidth(new_w) != XtWidth(old)) || (!XtHeight(new_w)) ||
      (XtHeight(new_w) != XtHeight(old)) ||
      (orient ||
       ((IsPopup(new_w) || IsPulldown(new_w) || IsBar(new_w)) &&
        (MGR_ShadowThickness(new_w) != MGR_ShadowThickness(old))) ||
       (RC_EntryBorder(old) != RC_EntryBorder(new_w)) || (RC_MarginW(old) != RC_MarginW(new_w)) ||
       (RC_MarginH(old) != RC_MarginH(new_w)) || (RC_Spacing(old) != RC_Spacing(new_w)) ||
       (RC_Packing(old) != RC_Packing(new_w)) || (RC_NCol(old) != RC_NCol(new_w)) ||
       (RC_AdjLast(old) != RC_AdjLast(new_w)) || (RC_AdjMargin(old) != RC_AdjMargin(new_w)) ||
  ...
  if (IsOption(new_w)) {
    if (RC_OptionSubMenu(new_w) != RC_OptionSubMenu(old)) {
      ...
      if (!RC_MemWidget(new_w) || (RC_MemWidget(old) == RC_MemWidget(new_w))) {
        if ((child = FindFirstManagedChild((CompositeWidget)RC_OptionSubMenu(new_w),
                                           FIRST_BUTTON)) != NULL)
        {
    ...
    if (RC_MemWidget(old) != RC_MemWidget(new_w)) {
      ...
      _XmRC_UpdateOptionMenuCBG(XmOptionButtonGadget((Widget)new_w), RC_MemWidget(new_w));
    }
    n = 0;
    if (RC_OptionLabel(new_w) != RC_OptionLabel(old)) {
      XtSetArg(args[n], XmNlabelString, RC_OptionLabel(new_w));
      n++;
      XtSetArg(args[n], XmNlabelType, XmSTRING);
      n++;
    }
    if (RC_MnemonicCharSet(new_w) != RC_MnemonicCharSet(old)) {
      XtSetArg(args[n], XmNmnemonicCharSet, RC_MnemonicCharSet(new_w));
      n++;
    }
...
static Boolean set_values_popup(XmRowColumnWidget old, XmRowColumnWidget new_w)
{
  ...
  if (RC_TearOffModel(old) != RC_TearOffModel(new_w)) {
    if ((RC_TearOffModel(new_w) != XmTEAR_OFF_DISABLED) && !RC_TearOffControl(new_w)) {
      XmTearOffButtonWidget tw;
      /* prevent RowColumn: InsertChild() from inserting tear off button
       * into child list.
       */
      RC_SetFromInit(new_w, TRUE);
      tw = (XmTearOffButtonWidget)XtCreateWidget(
          TEAROFF_CONTROL, xmTearOffButtonWidgetClass, (Widget)new_w, args, n);
    ...
    else if ((RC_TearOffModel(new_w) == XmTEAR_OFF_DISABLED) && RC_TearOffControl(new_w)) {
      XtUnmanageChild(RC_TearOffControl(new_w)); /* otherwise ChangeManaged is too clever and
                                                    doesn't catch change if tear-off goes away */
  ...
  if ((old->core.background_pixel != new_w->core.background_pixel) && RC_TearOffControl(new_w)) {
  ...
  if ((RC_TearOffTitle(new_w) != RC_TearOffTitle(old)) && RC_TornOff(old)) {
    XmeSetWMShellTitle(RC_TearOffTitle(new_w), (Widget)XtParent(new_w));
  }
  return (need_expose);
}
  ...
  if (IsPopup(old)) {
    ...
    if (RC_PopupEnabled(old)) {
      ...
      for (i = 0; i < old->row_column.postFromCount; i++) {
        ...
        XtUngrabButton(old->row_column.postFromList[i], RC_PostButton(old), RC_PostModifiers(old));
      }
      if (RC_PopupEnabled(new_w)) {
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (!XmRepTypeValidValue(XmRID_ORIENTATION, RC_Orientation(new_w), (Widget)new_w)) {
    RC_Orientation(new_w) = RC_Orientation(old);
  }
  if (!XmRepTypeValidValue(XmRID_PACKING, RC_Packing(new_w), (Widget)new_w)) {
  ...
  if (!XmRepTypeValidValue(XmRID_ALIGNMENT, RC_EntryAlignment(new_w), (Widget)new_w)) {
    RC_EntryAlignment(new_w) = RC_EntryAlignment(old);
  }
  if (!XmRepTypeValidValue(
          XmRID_VERTICAL_ALIGNMENT, RC_EntryVerticalAlignment(new_w), (Widget)new_w))
  {
  ...
  if (RC_MenuPost(new_w) != RC_MenuPost(old)) {
    if (IsPulldown(new_w)) {
      ...
      if (_XmMapBtnEvent(RC_MenuPost(new_w),
                         &RC_PostEventType(new_w),
                         &RC_PostButton(new_w),
                         &RC_PostModifiers(new_w)) == FALSE)
      {
  ...
  if (IsOption(new_w) && (RC_IsHomogeneous(req) != RC_IsHomogeneous(old))) {
  ...
  if (IsPopup(new_w) || IsPulldown(new_w)) {
        ...
        (new_w->row_column.postFromCount != old->row_column.postFromCount))
    {
      ...
      if (old->row_column.postFromList) {
        ...
        for (i = 0; i < cnt; i++) {
          _XmRC_RemoveHandlersFromPostFromWidget((Widget)new_w, old->row_column.postFromList[i]);
        }
                                        ...
                                        XtWidgetGeometry *allowed)
{
   ...
   */
  RC_Boxes(m) = (XmRCKidGeometry)_XmRCGetKidGeo((Widget)m,
                                                instigator,
                                                desired,
                                                RC_EntryBorder(m),
                                                RC_EntryBorder(m),
  ...
  if (IsOption(m)) {
    ...
    if (kg->kid && (instigator == omcb)) {
       ...
       * Grow only
       */
      if ((desired->request_mode & CWWidth) && (desired->width < BWidth(button_box))) {
        allowed->width = BWidth(button_box);
        allowed->height = BHeight(button_box);
        allowed->request_mode = (CWHeight | CWWidth);
        result = XtGeometryAlmost;
      }
      if ((desired->request_mode & CWHeight) && (desired->height < BHeight(button_box))) {
        allowed->width = BWidth(button_box);
        allowed->height = BHeight(button_box);
        allowed->request_mode = (CWHeight | CWWidth);
        result = XtGeometryAlmost;
    ...
    if (kg->kid && (instigator == oml)) {
       ...
       * Can't get shorter
       */
      if ((desired->request_mode & CWHeight) && (desired->height < kg->box.height)) {
...
static void MenuBarInitialize(XmRowColumnWidget bar)
{
  ...
  XtAddEventHandler((Widget)bar,
                    KeyPressMask | KeyReleaseMask,
                    False,
                    _XmRC_KeyboardInputHandler,
                    (XtPointer)bar);
  XtAddEventHandler((Widget)topManager,
                    KeyPressMask | KeyReleaseMask,
                    False,
                    _XmRC_KeyboardInputHandler,
                    (XtPointer)bar);
...
static void PopupInitialize(XmRowColumnWidget popup)
{
  ...
  if (RC_TearOffModel(popup) != XmTEAR_OFF_DISABLED) {
    ...
    RC_TearOffControl(popup) = XtCreateWidget(
        TEAROFF_CONTROL, xmTearOffButtonWidgetClass, (Widget)popup, args, n);
...
static void PulldownInitialize(XmRowColumnWidget pulldown)
{
  ...
  if (RC_TearOffModel(pulldown) != XmTEAR_OFF_DISABLED) {
    ...
    RC_TearOffControl(pulldown) = XtCreateWidget(
        TEAROFF_CONTROL, xmTearOffButtonWidgetClass, (Widget)pulldown, args, n);
...
static void OptionInitialize(XmRowColumnWidget option)
{
    ...
    RC_Spacing(option) = 3;
  {
    ...
    _XmProcessLock();
    translations =
        (XtTranslations)((XmManagerClassRec *)XtClass(option))->manager_class.translations;
    _XmProcessUnlock();
  ...
  if (RC_OptionLabel(option)) {
    XtSetArg(args[n], XmNlabelString, RC_OptionLabel(option));
    n++;
  }
  else {
    ...
    XtSetArg(args[n], XmNlabelString, empty_string);
    n++;
  }
  if (RC_MnemonicCharSet(option)) {
    XtSetArg(args[n], XmNmnemonicCharSet, RC_MnemonicCharSet(option));
    n++;
  }
  child = XmCreateLabelGadget((Widget)option, OPTION_LABEL, args, n);
  XtManageChild(child);
  if (empty_string != NULL)
    XmStringFree(empty_string);
  /* end fix for 5235 */
  /* Create the cascade button widget portion of the option menu */
  n = 0;
  XtSetArg(args[n], XmNsubMenuId, RC_OptionSubMenu(option));
  n++;
  ...
  XtAddEventHandler((Widget)option,
                    KeyPressMask | KeyReleaseMask,
                    False,
                    _XmRC_KeyboardInputHandler,
                    (XtPointer)option);
...
static void WorkAreaInitialize(XmRowColumnWidget work)
{
    ...
    RC_Spacing(work) = 3;
  {
    ...
    _XmProcessLock();
    translations =
        (XtTranslations)((XmManagerClassRec *)XtClass(work))->manager_class.translations;
    _XmProcessUnlock();
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if ((RC_Orientation(nw) != XmNO_ORIENTATION) &&
      !XmRepTypeValidValue(XmRID_ORIENTATION, RC_Orientation(nw), (Widget)nw))
  {
    RC_Orientation(nw) = XmNO_ORIENTATION;
  }
  if ((RC_Packing(nw) != XmNO_PACKING) &&
      !XmRepTypeValidValue(XmRID_PACKING, RC_Packing(nw), (Widget)nw))
  {
    RC_Packing(nw) = XmNO_PACKING;
  }
  if (!XmRepTypeValidValue(XmRID_ROW_COLUMN_TYPE, RC_Type(nw), (Widget)nw)) {
    RC_Type(nw) = XmWORK_AREA;
  }
  else if ((RC_Type(req) == XmMENU_POPUP) || (RC_Type(req) == XmMENU_PULLDOWN)) {
    if (!XmIsMenuShell(XtParent(req)) || !XtParent(XtParent(req))) {
      XmeWarning((Widget)m, BadTypeParentMsg);
      RC_Type(m) = XmWORK_AREA;
    }
  }
  if (!XmRepTypeValidValue(XmRID_ALIGNMENT, RC_EntryAlignment(nw), (Widget)nw)) {
    RC_EntryAlignment(nw) = XmALIGNMENT_BEGINNING;
  }
  if (!XmRepTypeValidValue(XmRID_VERTICAL_ALIGNMENT, RC_EntryVerticalAlignment(nw), (Widget)nw)) {
  ...
  if ((RC_MenuPost(m) != NULL) && !IsPulldown(m)) {
    if (_XmMapBtnEvent(
            RC_MenuPost(m), &RC_PostEventType(m), &RC_PostButton(m), &RC_PostModifiers(m)) ==
        FALSE)
    {
  ...
  if (!IsWorkArea(m) && (XmIsManager(XtParent(m)))) {
     ...
     */
    m->manager.accelerator_widget = ((XmManagerWidget)XtParent(m))->manager.accelerator_widget;
                                   ...
                                   Cardinal *num_args) /* unused */
{
  ...
  if (XmIsLabelGadget(old)) {
    margins_changed = ((LabG_MarginTop(old) != LabG_MarginTop(new_w)) ||
    ...
    margins_changed = ((Lab_MarginTop(old) != Lab_MarginTop(new_w)) ||
  ...
  if (RCIndex(old) != RCIndex(new_w)) {
    ...
    if ((RCIndex(new_w) < 0) || (RCIndex(new_w) >= rc->composite.num_children)) {
      ...
      if ((current.x != XtX(new_w)) || (current.width != XtWidth(new_w)) ||
          (current.height != XtHeight(new_w)) || (current.border_width != XtBorderWidth(new_w)))
 ...
 * list
 */
static Widget create(Widget p, /* parent widget */
                     char *name,
                     ArgList old_al,
                     Cardinal old_ac,
                     ...
                     int is_radio) /* the radio flag */
{
  ...
  Widget m;
  int i, ac = 0;
  if (is_radio) /* get ours in ahead of the */ { /* caller's, so his override */
    XtSetArg(al[ac], XmNpacking, XmPACK_COLUMN);
    ac++;
    XtSetArg(al[ac], XmNradioBehavior, is_radio);
    ac++;
    XtSetArg(al[ac], XmNisHomogeneous, TRUE);
    ac++;
...
Widget XmVaCreateRowColumn(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmRowColumnWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedRowColumn(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmRowColumnWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
static void ClassInitialize(void)
{
  ...
  /* Trait records */
  XmeTraitSet(
      (XtPointer)xmRowColumnWidgetClass, XmQTmenuSystem, (XtPointer)&_XmRC_menuSystemRecord);
}
...
static void ClassPartInitialize(WidgetClass rcc)
{
  ...
  if ((WidgetClass)rcc != xmRowColumnWidgetClass) {
    ...
    if (this_class->row_column_class.menuProcedures == XmInheritMenuProceduresProc)
      this_class->row_column_class.menuProcedures = super->row_column_class.menuProcedures;
    if (this_class->row_column_class.armAndActivate == XmInheritArmAndActivateProc)
      this_class->row_column_class.armAndActivate = super->row_column_class.armAndActivate;
    if (this_class->row_column_class.traversalHandler == XmInheritMenuTraversalProc)
      this_class->row_column_class.traversalHandler = super->row_column_class.traversalHandler;
  }
}
 ...
 *
 ************************************************************/
static void InitializePrehook(Widget req, /* unused */
                              Widget new_w,
                              ArgList args,       /* unused */
                              Cardinal *num_args) /* unused */
{
  ...
  if ((RC_Type(new_w) == XmMENU_PULLDOWN) || (RC_Type(new_w) == XmMENU_POPUP)) {
    ...
    new_w->core.widget_class->core_class.tm_table = xmManagerClassRec.core_class.tm_table;
  _XmProcessUnlock();
}
 ...
 *
 ************************************************************/
static void InitializePosthook(Widget req, /* unused */
                               Widget new_w,
                               ArgList args,       /* unused */
                               Cardinal *num_args) /* unused */
{
 ...
 */
static void FixEventBindings(XmRowColumnWidget m, /* row column (parent) widget */
                             Widget w)            /* subwidget */
{
  if (XtIsWidget(w) && ((IsPopup(m) || IsBar(m) || IsPulldown(m)) && XmIsLabel(w) &&
                        (w->core.widget_class != xmLabelWidgetClass)))
  {
    XtAddEventHandler(w, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, m);
  }
...
static Widget FindFirstManagedChild(CompositeWidget m, Boolean first_button)
{
  ...
  while ((i < n) && ((*kid)->core.being_destroyed ||
                     (!XtIsManaged(*kid) || (first_button && !(XmIsTraversable(*kid))))))
    kid++, i++;
  if (i >= n)
    return (NULL);
  else
...
static void Resize(Widget wid)
{
  ...
  if ((m->row_column.old_width != m->core.width) || (m->row_column.old_height != m->core.height) ||
      (m->row_column.old_shadow_thickness != m->manager.shadow_thickness))
   ...
   */
  if (!IsOption(m) &&
      (m->row_column.old_height > m->core.height || m->row_column.old_width > m->core.width))
    draw_shadow = True;
  m->row_column.old_width = m->core.width;
  m->row_column.old_height = m->core.height;
  m->row_column.old_shadow_thickness = m->manager.shadow_thickness;
...
static void Redisplay(Widget w, XEvent *event, Region region)
{
  ...
  if ((IsPopup(m) || IsPulldown(m)) && !((XmMenuShellWidget)XtParent(m))->shell.popped_up) {
    RC_SetExpose(m, TRUE);
    return;
  }
  if (RC_DoExpose(m)) /* a one-shot set on popup */ { /* so we ignore next expose */
...
static void FixVisual(XmRowColumnWidget m, Widget w)
{
  ...
  if (RC_EntryBorder(m)) {
    ...
    if (XtIsRealized(w))
      XmeConfigureObject(
          w, w->core.x, w->core.y, w->core.width, w->core.height, RC_EntryBorder(m));
    else
  ...
  if (XmIsLabelGadget(w)) {
    if (IsAligned(m)) {
      if (IsWorkArea(m) || ((w->core.widget_class != xmLabelWidgetClass) &&
    ...
    if (IsAligned(m)) {
      if ((w->core.widget_class != xmLabelWidgetClass) || IsWorkArea(m)) {
        ac = 0;
        XtSetArg(al[ac], XmNalignment, RC_EntryAlignment(m));
        ac++;
        XtSetValues(w, al, ac);
...
static void FixCallback(XmRowColumnWidget m, Widget w)
{
  ...
  char *c;
  menuSavvyRec = (XmMenuSavvyTrait)XmeTraitGet((XtPointer)XtClass(w), XmQTmenuSavvy);
  if (menuSavvyRec == (XmMenuSavvyTrait)NULL || menuSavvyRec->getActivateCBName == NULL)
    return;
...
static void MenuFocusIn(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  else if ((((XmMenuShellWidget)XtParent(rc))->shell.popped_up) && !_XmGetInDragMode((Widget)rc))
...
static void MenuEnter(Widget wid,
                      XEvent *event,
 ...
 * menu system.
 */
static void GadgetEscape(Widget wid,
                         XEvent *event,
                         String *params,       /* unused */
                         Cardinal *num_params) /* unused */
{
  ...
  if (IsBar(rc)) {
     ...
     */
    if (RC_IsArmed(rc))
      (*(((XmRowColumnClassRec *)(rc->core.widget_class))->row_column_class.armAndActivate))(
 ...
 */
static void GetMenuAccelerator(Widget wid,
                               int resource, /* unused */
                               XtArgVal *value)
{
  String data;
 ...
 */
static void GetMenuPost(Widget wid,
                        int resource, /* unused */
                        XtArgVal *value)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
 ...
 */
static void GetTearOffTitle(Widget wid,
                            int resource_offset, /* unused */
                            XtArgVal *value)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
