...
#  define SB_WithChild(w) (((XmSpinBoxRec *)(w))->spinBox.textw != NULL)
#  define SB_ChildCount(w) (((XmSpinBoxRec *)(w))->composite.num_children)
#  define SB_GetArrowOrientation(w) (((XmSpinBoxRec *)(w))->spinBox.arrow_orientation)
...
#  define SB_ChildPositionTypeIsValue(wc) \
    (((XmSpinBoxConstraint)(wc))->position_type == (unsigned char)XmPOSITION_VALUE)
#  define SB_ChildMaximumPositionValue(wc) \
    ((((wc) == (XmSpinBoxConstraint)NULL) ? \
          0 : \
          ((SB_ChildIsNumeric(wc)) ? (wc)->maximum_value : \
                                     (((wc)->num_values > 0) ? ((wc)->num_values - 1) : 0))))
#  define SB_ChildMinimumPositionValue(wc) \
    ((((wc) == (XmSpinBoxConstraint)NULL) ? 0 : \
                                            ((SB_ChildIsNumeric(wc)) ? (wc)->minimum_value : 0)))
#  ifdef __cplusplus
