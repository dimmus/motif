 ...
 *********************************************************************/
/*ARGSUSED*/
void _XmDirectionDefault(Widget widget,
                         int offset, /* unused */
                         XrmValue *value)
{
  ...
  if (XmIsManager(widget) &&
      (((XmManagerWidget)widget)->manager.string_direction != XmSTRING_DIRECTION_DEFAULT))
    direction = XmStringDirectionToDirection(((XmManagerWidget)widget)->manager.string_direction);
  else
    direction = _XmGetLayoutDirection(XtParent(widget));
}

/*ARGSUSED*/
void _XmFromLayoutDirection(Widget wid,
                            ...
                            XtArgVal *value)
{
  if (XmIsManager(wid))
    ...
    *value = (XtArgVal)XmDirectionToStringDirection(_XmGetLayoutDirection(wid));
}

/*ARGSUSED*/
XmImportOperator _XmToLayoutDirection(Widget wid,
                                      int resource_offset, /* unused */
                                      XtArgVal *value)
{
  if (XmIsManager(wid)) {
    *value = (XtArgVal)XmStringDirectionToDirection((XmStringDirection)*value);
    ...
    XmPrim_layout_direction(((XmPrimitiveWidget)wid)) = XmStringDirectionToDirection(
        (XmStringDirection)*value);
    return XmSYNTHETIC_NONE;
  }
  else if (XmIsLabelGadget(wid)) {
    ((XmGadget)wid)->gadget.layout_direction = XmStringDirectionToDirection(
        (XmStringDirection)*value);
    return XmSYNTHETIC_NONE;
  }
  return XmSYNTHETIC_LOAD;
...
XmDirection XmStringDirectionToDirection(XmStringDirection dir)
{
  switch (dir) {
...
XmDirection _XmGetLayoutDirection(Widget w)
{
  ...
  while (w && !(layoutT = (XmSpecifyLayoutDirectionTrait)XmeTraitGet((XtPointer)XtClass(w),
...
Boolean XmDirectionMatch(XmDirection d1, XmDirection d2)
{
  d1 = Fixdir(d1);
  d2 = Fixdir(d2);
...
Boolean XmDirectionMatchPartial(XmDirection d1, XmDirection d2, XmDirection dmask)
{
