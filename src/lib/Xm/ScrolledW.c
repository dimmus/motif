...
static void VisualPolicyDefault(Widget widget, int offset, XrmValue *value);
static void SliderMove(Widget w, XtPointer closure, XtPointer cd);
static void MoveWindow(XmScrolledWindowWidget sw, int value, unsigned char direction);
static void LeftEdge(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void RightEdge(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void TopEdge(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void BottomEdge(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void PageLeft(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void PageRight(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void PageUp(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void PageDown(Widget wid, XEvent *event, String *params, Cardinal *num_params);
...
static void ComputeLocations(XmScrolledWindowWidget sw,
                             Dimension HSBht,
                             Dimension VSBht,
                             Boolean HasHSB,
                             Boolean HasVSB,
                             ...
                             Position *vsbX,
                             Position *vsbY);
static void VariableLayout(XmScrolledWindowWidget sw);
static void ConstantLayout(XmScrolledWindowWidget sw);
static void Resize(Widget wid);
static void Destroy(Widget wid);
static void GetVariableSize(XmScrolledWindowWidget sw, Dimension *pwidth, Dimension *pheight);
...
static void ConstraintInitialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static void GetHorRects(Widget sw, XRectangle **hrect, Cardinal *num_hrect);
static void GetVertRects(Widget sw, XRectangle **vrect, Cardinal *num_vrect);
static void HandleDrop(Widget w, XtPointer client_data, XtPointer call_data);
static void HandleDrag(Widget w, XtPointer client_data, XtPointer call_data);
static void ScrollFrameInit(Widget sf, XtCallbackProc moveCB, Widget scrollable);
static Boolean GetInfo(Widget sf, Cardinal *dimension, Widget **nav_list, Cardinal *num_nav_list);
static void AddNavigator(Widget sf, Widget nav, Mask dimMask);
static void RemoveNavigator(Widget sf, Widget nav);
static void UpdateOrigGeom(Widget sf, Widget child, XtWidgetGeometry *geom);
static void CheckKids(XmScrolledWindowWidget sw);
...
static XtResource resources[] = {
    ...
    {XmNscrollingPolicy,
     ...
     XmRImmediate,
     (XtPointer)XmAPPLICATION_DEFINED},
    {/* Obsolete resource */
     XmNvisualPolicy,
     XmCVisualPolicy,
     XmRVisualPolicy,
    ...
    {XmNscrollBarPlacement,
     ...
     XmRCallProc,
     (XtPointer)ScrollBarPlacementDefault},
 ...
 *
 ****************************************************************/
externaldef(xmscrolledwindowclassrec) XmScrolledWindowClassRec xmScrolledWindowClassRec = {
    {
        /* core_class fields      */
...
externaldef(xmscrolledwindowwidgetclass)
 ...
 *
 *********************************************************************/
static void ScrollBarPlacementDefault(Widget widget,
                                      int offset, /* unused */
                                      XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void VisualPolicyDefault(Widget widget,
                                int offset, /* unused */
                                XrmValue *value)
{
      ...
      if (XtIsManaged(child) && !child->core.being_destroyed) {
        ...
        if ((swc->child_type == XmWORK_AREA) || (swc->child_type == XmSCROLL_HOR)) {
          ...
          if (LayoutIsRtoL((Widget)sw))
            real_orig_x = clip->core.width - swc->orig_x - child->core.width;
          else
 ...
 *									*
 ************************************************************************/
static void MoveWindow(XmScrolledWindowWidget sw, int value, unsigned char orientation)
{
  Widget focus;
  XtCallbackList tmp;
  XmNavigatorDataRec nav_data;
...
static void RightEdge(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (!sw->swindow.hScrollBar)
    return;
  MoveWindow(
      sw, sw->swindow.hmax - sw->swindow.hExtent, sw->swindow.hScrollBar->scrollBar.orientation);
}
...
static void BottomEdge(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (!sw->swindow.vScrollBar)
    return;
  MoveWindow(
      sw, sw->swindow.vmax - sw->swindow.vExtent, sw->swindow.vScrollBar->scrollBar.orientation);
}
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
   ...
   *
   ****************/
  if (!XmRepTypeValidValue(XmRID_SCROLLING_POLICY, new_w->swindow.ScrollPolicy, (Widget)new_w)) {
  ...
  if (!XmRepTypeValidValue(XmRID_VISUAL_POLICY, new_w->swindow.VisualPolicy, (Widget)new_w)) {
  ...
  if (!XmRepTypeValidValue(
          XmRID_SCROLL_BAR_DISPLAY_POLICY, new_w->swindow.ScrollBarPolicy, (Widget)new_w))
  {
    if (new_w->swindow.ScrollPolicy == XmAUTOMATIC)
      new_w->swindow.ScrollBarPolicy = XmAS_NEEDED;
    else
  ...
  if (!XmRepTypeValidValue(XmRID_SCROLL_BAR_PLACEMENT, new_w->swindow.Placement, (Widget)new_w)) {
  ...
  XtAugmentTranslations(
      (Widget)new_w,
      (XtTranslations)((XmManagerClassRec *)XtClass(new_w))->manager_class.translations);
  /***********************
  ...
  if (new_w->swindow.ScrollPolicy == XmAPPLICATION_DEFINED) {
    ...
    if (future_width > new_w->manager.shadow_thickness * 2)
      new_w->swindow.AreaWidth = future_width - (new_w->manager.shadow_thickness * 2);
    else
      new_w->swindow.AreaWidth = 2;
    if (future_height > new_w->manager.shadow_thickness * 2)
      new_w->swindow.AreaHeight = future_height - (new_w->manager.shadow_thickness * 2);
    else
  ...
  if (new_w->core.width == 0)
    new_w->core.width = DEFAULT_SIZE;
  if (new_w->core.height == 0)
    new_w->core.height = DEFAULT_SIZE;
  /* default Area fields too, see above comments */
  if (new_w->core.width > new_w->manager.shadow_thickness * 2)
    new_w->swindow.AreaWidth = new_w->core.width - (new_w->manager.shadow_thickness * 2);
  else
    new_w->swindow.AreaWidth = 2;
  if (new_w->core.height > new_w->manager.shadow_thickness * 2)
    new_w->swindow.AreaHeight = new_w->core.height - (new_w->manager.shadow_thickness * 2);
  else
    new_w->swindow.AreaHeight = 2;
  /****************
   *
  ...
  new_w->swindow.ClipWindow = (XmDrawingAreaWidget)XtCreateManagedWidget(
      "ClipWindow", xmClipWindowWidgetClass, (Widget)new_w, loc_args, n);
  /***********************
      Initialize the scrollFrame with move callback for Automatic */
  ((XmScrollFrameTrait)XmeTraitGet((XtPointer)XtClass(nw), XmQTscrollFrame))
  ...
  n++;
  new_w->swindow.vScrollBar = (XmScrollBarWidget)XtCreateManagedWidget(
      "VertScrollBar", xmScrollBarWidgetClass, (Widget)new_w, loc_args, n);
  /************************
   * horizontal scrollbar */
  n = 0;
  XtSetArg(loc_args[n], XmNorientation, XmHORIZONTAL);
  n++;
  new_w->swindow.hScrollBar = (XmScrollBarWidget)XtCreateManagedWidget(
      "HorScrollBar", xmScrollBarWidgetClass, (Widget)new_w, loc_args, n);
...
static void Redisplay(Widget wid, XEvent *event, Region region)
{
  ...
  Dimension st, bw;
  if (!XtIsRealized(wid))
    return; /* get rid of that when GM redone*/
  st = sw->manager.shadow_thickness;
  if (sw->swindow.ScrollPolicy == XmAUTOMATIC) {
    XmeDrawShadows(XtDisplay(sw),
                   XtWindow(sw),
...
static void InsertChild(Widget w)
{
  ...
  if (swc->child_type == (unsigned char)RESOURCE_DEFAULT) {
    if (XmIsScrollBar(w)) {
      if (((XmScrollBarWidget)w)->scrollBar.orientation == XmHORIZONTAL) {
  ...
  if (swc->child_type == XmHOR_SCROLLBAR)
    ((XmScrollFrameTrait)XmeTraitGet((XtPointer)XtClass((Widget)(sw)), XmQTscrollFrame))
  ...
  if (sw->swindow.InInit) {
     ...
     ****************/
      ...
      if ((swc->child_type == XmWORK_AREA) || (swc->child_type == XmSCROLL_HOR) ||
          (swc->child_type == XmSCROLL_VERT) || (swc->child_type == XmNO_SCROLL))
      {
...
static void ComputeLocations(XmScrolledWindowWidget sw,
                             ...
                             Position *vsbY)
{
  Dimension shad = sw->manager.shadow_thickness, pad = sw->swindow.pad, vsb_width, hsb_height;
...
static void VariableLayout(XmScrolledWindowWidget sw)
{
  ...
  Dimension shad = sw->manager.shadow_thickness, pad = sw->swindow.pad, bw;
  int tmp;
   ...
   *
   ****************/
  ComputeLocations(sw, HSBht, VSBht, HasHSB, HasVSB, &newx, &newy, &hsbX, &hsbY, &vsbX, &vsbY);
  /* reset the child sizes */
  childWidth = (HasVSB) ? MyWidth - (VSBwidth + (2 * VSBht) + SB_BORDER_WIDTH +
                                     ((HSBht + bw + shad) * 2) + pad) :
                          MyWidth - ((HSBht + shad + bw) * 2);
  childHeight = (HasHSB) ? MyHeight - (HSBheight + (2 * HSBht) + SB_BORDER_WIDTH +
                                       ((VSBht + shad + bw) * 2) + pad) :
...
static void ConstantLayout(XmScrolledWindowWidget sw)
{
  ...
  Dimension HSBht, VSBht, shad = sw->manager.shadow_thickness, pad = sw->swindow.pad;
    ...
    if (ExistManaged(child)) {
      ...
      if ((swc->child_type == XmWORK_AREA) || (swc->child_type == XmSCROLL_VERT)) {
        ...
        sw->swindow.vmax = MAX(sw->swindow.vmax,
                               swc->orig_y + child->core.height + 2 * child->core.border_width);
      }
      /* only regular work_area and scroll_hor kids
      ...
      if ((swc->child_type == XmWORK_AREA) || (swc->child_type == XmSCROLL_HOR)) {
        ...
        sw->swindow.hmax = MAX(sw->swindow.hmax,
                               swc->orig_x + child->core.width + 2 * child->core.border_width);
      }
  ...
  if (((MyHeight - (shad * 2)) >= sw->swindow.vmax) &&
      ((MyWidth - (shad * 2)) >= sw->swindow.hmax) && (sw->swindow.ScrollBarPolicy == XmAS_NEEDED))
  {
    ...
    clipHeight = (HSBExists) ? MyHeight - (sw->swindow.hScrollBar->core.height + SB_BORDER_WIDTH +
                                           ((HSBht + shad) * 2 + pad)) :
                               MyHeight - ((VSBht + shad) * 2);
    clipWidth = (VSBExists) ? MyWidth - (sw->swindow.vScrollBar->core.width + SB_BORDER_WIDTH +
                                         ((VSBht + shad) * 2 + pad)) :
   ...
   *
   ****************/
  ComputeLocations(sw, HSBht, VSBht, HasHSB, HasVSB, &clipX, &clipY, &hsbX, &hsbY, &vsbX, &vsbY);
  /****************
   *
 ...
 *									*
 ************************************************************************/
static void HandleDrop(Widget w,              /* unused */
...
static void TimerEvent(XtPointer closure, XtIntervalId *id) /* unused */
{
  ...
  if (!XmIsScrollBar(auto_clos->widget))
    return;
  ...
  if (!hit) {
    ...
    sw->swindow.auto_drag_timer = XtAppAddTimeOut(
        XtWidgetToApplicationContext((Widget)scrb), repeat_delay, TimerEvent, closure);
  }
  else {
                       ...
                       XtPointer call_data)
{
  ...
  if (dragProc->reason == XmCR_DROP_SITE_ENTER_MESSAGE) {
    ...
    if (sw->swindow.auto_drag_timer == 0) {
      sw->swindow.auto_drag_closure = auto_clos = (AutoDragClosure *)XtMalloc(
          sizeof(AutoDragClosure));
      auto_clos->widget = w;
      ...
      sw->swindow.auto_drag_timer = XtAppAddTimeOut(
          XtWidgetToApplicationContext(w), initial_delay, TimerEvent, (XtPointer)auto_clos);
    }
  }
...
static void GetHorRects(Widget sw, XRectangle **hrect, Cardinal *num_hrect)
{
  ...
  *hrect = (XRectangle *)XtMalloc(sizeof(XRectangle) * (*num_hrect));
  ...
  (*hrect)[1].x = w->core.x + w->core.width - scw->swindow.hScrollBar->core.x;
  (*hrect)[1].y = (*hrect)[0].y;
...
static void GetVertRects(Widget sw, XRectangle **vrect, Cardinal *num_vrect)
{
  ...
  *vrect = (XRectangle *)XtMalloc(sizeof(XRectangle) * (*num_vrect));
  ...
  (*vrect)[1].x = (*vrect)[0].x;
  (*vrect)[1].y = w->core.y + w->core.height - scw->swindow.vScrollBar->core.y;
 ...
 *									*
 ************************************************************************/
static void UpdateAutoDrag(XmScrolledWindowWidget sw)
{
  Widget hsb = (Widget)sw->swindow.hScrollBar, vsb = (Widget)sw->swindow.vScrollBar;
  Arg args[10];
  Cardinal n;
...
static void GetVariableSize(XmScrolledWindowWidget sw, Dimension *pwidth, Dimension *pheight)
{
  XmScrollBarWidget hsb = sw->swindow.hScrollBar, vsb = sw->swindow.vScrollBar;
  Widget w = sw->swindow.WorkWindow;
  Dimension hsheight, vswidth;
  Dimension hsbht = 0, vsbht = 0, shads = sw->manager.shadow_thickness * 2;
  if (ExistManaged(w)) {
    ...
    if (ExistManaged((Widget)hsb)) {
      ...
      hsheight = 0;
    if (!*pwidth)
      *pwidth = w->core.width + (2 * w->core.border_width) + vswidth + shads + hsbht +
                sw->swindow.XOffset + sw->swindow.WidthPad;
    if (!*pheight)
      *pheight = w->core.height + (2 * w->core.border_width) + hsheight + shads + vsbht +
                                        ...
                                        XtWidgetGeometry *reply)
{
  ...
  if (sw->swindow.VisualPolicy == XmVARIABLE) {
    ...
    if (request->request_mode & CWWidth)
      newWidth = request->width + vswidth + ht + bw + hsbht + sw->swindow.XOffset +
                 sw->swindow.WidthPad;
    else
      newWidth = w->core.width + vswidth + ht + bw + hsbht + sw->swindow.XOffset +
                 sw->swindow.WidthPad;
    if (request->request_mode & CWHeight)
      newHeight = request->height + hsheight + ht + bw + vsbht + sw->swindow.YOffset +
                  sw->swindow.HeightPad;
    else
      newHeight = w->core.height + hsheight + ht + bw + vsbht + sw->swindow.YOffset +
                  sw->swindow.HeightPad;
    parent_request.request_mode = CWWidth | CWHeight;
    parent_request.width = newWidth;
    parent_request.height = newHeight;
 ...
 *									*
 ************************************************************************/
void _XmSWNotifyGeoChange(Widget sw, Widget child, XtWidgetGeometry *request)
{
 ...
 *									*
 ************************************************************************/
static XtGeometryResult QueryGeometry(Widget wid, XtWidgetGeometry *request, XtWidgetGeometry *ret)
{
  XmScrolledWindowWidget sw = (XmScrolledWindowWidget)wid;
  ...
  if (request->request_mode == 0) {
    ...
    if (ExistManaged((Widget)hsb)) {
      ...
      hsheight = 0;
    if (ExistManaged(w)) {
      ret->width = KidWidth + (2 * w->core.border_width) + vswidth + ht + hsbht +
                   sw->swindow.XOffset + sw->swindow.WidthPad;
      ret->height = KidHeight + (2 * w->core.border_width) + hsheight + ht + vsbht +
                    sw->swindow.YOffset + sw->swindow.HeightPad;
    }
    else {
      ret->width = sw->core.width;
  ...
  {
    ret->height = w->core.height + (2 * w->core.border_width) + ht + sw->swindow.YOffset +
                  sw->swindow.HeightPad;
    ret->width = w->core.width + (2 * w->core.border_width) + ht + sw->swindow.XOffset +
                 sw->swindow.WidthPad;
    ret->request_mode |= (CWWidth | CWHeight);
    return (XtGeometryAlmost);
  }
  if (request->request_mode & CWHeight) {
    ...
    ret->request_mode |= CWWidth;
    ret->width = w->core.width + (2 * w->core.border_width) + vswidth + ht + vsbht +
                 sw->swindow.XOffset + sw->swindow.WidthPad;
    retval = XtGeometryAlmost;
  }
  if (request->request_mode & CWWidth) {
    ...
    ret->request_mode |= CWHeight;
    ret->height = w->core.height + (2 * w->core.border_width) + hsheight + ht + hsbht +
                  sw->swindow.YOffset + sw->swindow.HeightPad;
    retval = XtGeometryAlmost;
  }
  return (retval);
  ...
  if (new_w->swindow.auto_drag_model != current->swindow.auto_drag_model) {
    if (new_w->swindow.auto_drag_model == XmAUTO_DRAG_ENABLED) {
      ...
      if (new_w->swindow.hScrollBar)
        XmDropSiteRegister((Widget)new_w->swindow.hScrollBar, s_args, n);
      if (new_w->swindow.vScrollBar)
        XmDropSiteRegister((Widget)new_w->swindow.vScrollBar, s_args, n);
    }
    else {
 ...
 *
 *************************************<->***********************************/
static void ConstraintInitialize(Widget rw, /* unused */
                                 Widget nw,
                                 ArgList args,       /* unused */
                                 Cardinal *num_args) /* unused */
{
  ...
  if ((swc->child_type != (unsigned char)RESOURCE_DEFAULT) &&
      (!XmRepTypeValidValue(XmRID_SCROLLED_WINDOW_CHILD_TYPE, swc->child_type, nw)))
  {
...
static void ScrollFrameInit(Widget sf, XtCallbackProc moveCB, Widget scrollable)
{
  ...
  sw->swindow.scroll_frame_data = (XmScrollFrameData)XtMalloc(sizeof(XmScrollFrameDataRec));
...
static Boolean GetInfo(Widget sf, Cardinal *dimension, Widget **nav_list, Cardinal *num_nav_list)
{
  XmScrolledWindowWidget sw = (XmScrolledWindowWidget)sf;
  if (dimension)
    *dimension = 2; /* two dimensional frame */
  if (sw->swindow.scroll_frame_inited) {
    if (nav_list)
      *nav_list = sw->swindow.scroll_frame_data->nav_list;
    if (num_nav_list)
...
static void AddNavigator(Widget sf, Widget nav, Mask dimMask)
{
  ...
  if (!sw->swindow.scroll_frame_inited) {
    XmScrollFrameTrait sf_trait = (XmScrollFrameTrait)XmeTraitGet((XtPointer)XtClass(sf),
                                                                  XmQTscrollFrame);
 ...
 *
 ************************************************************************/
Boolean _XmSWGetClipArea(Widget widget, XRectangle *rect)
{
...
Widget XmVaCreateScrolledWindow(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmScrolledWindowWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedScrolledWindow(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmScrolledWindowWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
