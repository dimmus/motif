...
static void CommandCallback(Widget w, XtPointer cl_data, XtPointer call_data);
static Boolean CommandParentProcess(Widget wid, XmParentProcessData event);
 ...
 */
externaldef(xmcommandwidgetclass)
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if (new_w->selection_box.selection_label_string == (XmString)XmUNSPECIFIED) {
    ...
    argc = 0;
    XtSetArg(argv[argc], XmNlabelString, local_xmstring);
    ++argc;
  ...
  argc = 0;
  XtSetArg(argv[argc], XmNitemCount, &max);
  ++argc;
  ...
  XtAddCallback(
      new_w->selection_box.text, XmNvalueChangedCallback, CommandCallback, (XtPointer)new_w);
 ...
 ****************/
static void ListCallback(Widget w, /* unused */
                         XtPointer client_data,
                         XtPointer call_data)
{
  XmListCallbackStruct *listCB = (XmListCallbackStruct *)call_data;
...
****************/
  argc = 0;
  XtSetArg(argv[argc], XmNitemCount, &count);
  ++argc;
  ...
  if (cmdWid->command.error) {
    if ((listCB->item_position == (count - 1)) ||
        ((listCB->item_position == count) && (listCB->reason != XmCR_DEFAULT_ACTION)))
    {
      ...
      if (cmdWid->selection_box.list_selected_item_position) {
        XmListSelectPos(SB_List(cmdWid), cmdWid->selection_box.list_selected_item_position, FALSE);
      }
      return;
    }
    ...
    if (count <= 2) { /* List only contained error message and blank line.
                       */
      cmdWid->selection_box.list_selected_item_position = 0;
      return;
    }
    count -= 2;
    if ((listCB->item_position > count) && (listCB->reason == XmCR_DEFAULT_ACTION)) {
  ...
  {    /* Update the text widget to relect the latest list selection.
        */
    cmdWid->selection_box.list_selected_item_position = listCB->item_position;
 ...
 ****************/
static void CommandCallback(Widget w, /* unused */
                            XtPointer cl_data,
                            XtPointer call_data)
{
  ...
  cb.value = XmStringGenerate(str, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
  XtFree(str);
...
static Boolean CommandParentProcess(Widget wid, XmParentProcessData event)
{
  ...
  if ((event->any.process_type == XmINPUT_ACTION) &&
      ((event->input_action.action == XmPARENT_ACTIVATE) ||
       ((event->input_action.action == XmPARENT_CANCEL) && BB_CancelButton(cmd))))
  {
    if (event->input_action.action == XmPARENT_ACTIVATE) {
      _XmCommandReturn((Widget)cmd,
 ...
 ****************/
void _XmCommandReturn(Widget wid,
                      XEvent *event,
                      ...
                      Cardinal *numParams) /* unused */
{
  ...
  tmpXmString = XmStringGenerate(tmpString, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
...
/****************************************************************/
void _XmCommandUpOrDown(Widget wid,
                        XEvent *event, /* unused */
                        String *argv,
                        Cardinal *argc) /* unused */
{
  ...
  ++ac;
  XtSetArg(av[ac], XmNselectedItemCount, &selected_count);
  ac++;
  XtGetValues((Widget)list, av, ac);
  if (!count || (cmd->command.error && (count <= 2))) {
    return;
  }
  /*
...
static void Resize(Widget wid)
{
  ...
  ac = 0;
  XtSetArg(av[ac], XmNvisibleItemCount, &(cw->selection_box.list_visible_item_count));
  ++ac;
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (new_w->command.history_max_items < old->command.history_max_items) {
    argc = 0;
    XtSetArg(argv[argc], XmNitemCount, &max);
    ++argc;
    XtGetValues(SB_List(new_w), argv, argc);
...
Widget XmVaCreateCommand(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmCommandWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedCommand(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmCommandWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
void XmCommandAppendValue(Widget widget, XmString value)
{
  ...
  XmTextFieldReplace(w->selection_box.text, endPosition, endPosition, strNew);
 ...
 ****************/
Widget XmCreateCommandDialog(Widget ds_p, String name, ArgList com_args, Cardinal com_n)
{
  return XmeCreateClassDialog(xmCommandWidgetClass, ds_p, name, com_args, com_n);
}
