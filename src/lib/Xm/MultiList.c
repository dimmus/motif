...
static XtResource resources[] = {
    ...
    {XmNtitle,
     ...
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(XmMultiListRec, ext_list.title),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNtitleString,
     XmCTitleString,
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(XmMultiListRec, ext_list.title_string),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNdoubleClickCallback,
     ...
     XtOffsetOf(XmMultiListRec, ext_list.double_click),
     XmRImmediate,
     (XtPointer)NULL},
    {"pri.vate",
     "Pri.vate",
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmMultiListRec, ext_list.check_set_select_callback),
     XmRImmediate,
     (XtPointer)False},
    {XmNselectCallback,
     XmCCallback,
    ...
    {XmNshowFind,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmMultiListRec, ext_list.show_find),
     XmRImmediate,
     (XtPointer)True},
    {XmNfindLabel,
     XmCFindLabel,
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(XmMultiListRec, ext_list.find_label),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNitemNotFoundCallback,
     ...
     XmRCallback,
     sizeof(XtCallbackList),
     XtOffsetOf(XmMultiListRec, ext_list.not_found),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNitemFoundCallback,
     ...
     XmRCallback,
     sizeof(XtCallbackList),
     XtOffsetOf(XmMultiListRec, ext_list.item_found),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNvisibleItemCount,
     XmCVisibleItemCount,
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmMultiListRec, ext_list.visible_rows),
     XmRImmediate,
...
static XmSyntheticResource syn_resources[] = {{XmNwidth,
                                               sizeof(Dimension),
                                               XtOffsetOf(CoreRec, core.width),
...
static void Initialize(Widget req, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  if (XtHeight(req) == 0) {
    ...
    if (XmMultiList_title(elist) || XmMultiList_title_string(elist))
      title_height = XmMultiList_title_wid(elist)->core.height +
                     2 * (XmMultiList_title_wid(elist)->core.border_width + V_MARGIN);
    if (XmMultiList_show_find(elist))
      XtHeight(set) = XmMultiList_h_bar(elist)->core.height +
                      ...
                      XmMultiList_find(elist)->core.height +
                      2 * (V_MARGIN + elist->core.border_width +
      ...
      XtHeight(set) = XmMultiList_title_wid(elist)->core.height +
                      ...
                      XmMultiList_frame(elist)->core.height +
                      2 * (V_MARGIN + elist->core.border_width +
                           XmMultiList_h_bar(elist)->core.border_width +
                           XmMultiList_frame(elist)->core.border_width);
  }
  if (XtWidth(req) == 0) {
    XtWidth(set) = XmMultiList_ilist(elist)->core.width + XmMultiList_v_bar(elist)->core.width +
                   (5 * H_MARGIN) + (2 * elist->core.border_width);
  }
...
static void GetValuesHook(Widget w, ArgList args, Cardinal *num_args)
{
   ...
   */
  for (i = 0; i < j; i++) {
    if (streq(args[i].name, XmNnumColumns) || streq(args[i].name, XmNnumRows) ||
        streq(args[i].name, XmNselectedColumn) || streq(args[i].name, XmNcolumnTitles) ||
        streq(args[i].name, XmNentryData) || streq(args[i].name, XmNentryBackground) ||
        streq(args[i].name, XmNfirstColumnPixmaps) || streq(args[i].name, XmNfontList) ||
        streq(args[i].name, XmNrenderTable) || streq(args[i].name, XmNverticalScrollBar) ||
        streq(args[i].name, XmNhorizontalScrollBar) || streq(args[i].name, XmNfirstRow) ||
        streq(args[i].name, XmNfirstColumn) || streq(args[i].name, XmNsortFunctions) ||
 ...
 ************************************************************/
/*ARGSUSED*/
static void ActivateTextSearch(Widget w, XtPointer elist_ptr, XtPointer client)
{
...
static void LayoutChildren(Widget w)
{
   ...
   */
  _XmConfigureWidget(hbar,
                     scroll_left,
                     scroll_bottom - (Position)h_thick,
                     width,
                     hbar->core.height,
  ...
  if ((Position)temp_val >= scroll_bottom)
    ...
    height = scroll_bottom - temp_val;
  _XmConfigureWidget(
      vbar, scroll_right - v_thick, scroll_top, vbar->core.width, height, vbar->core.border_width);
}
...
static void PositionFindAndFindText(Widget w)
{
   ...
   */
  {
    ...
    _XmConfigureWidget(
        find_text, x, y, text_width, find_text->core.height, find_text->core.border_width);
  }
}
 ...
 */
static Widget CreateTitle(Widget parent, XmString label, ArgList args, Cardinal num_args)
{
  Widget w;
 ...
 *	Returns:       none
 */
static Widget CreateScrollbar(Widget parent, Boolean is_vert, ArgList args, Cardinal num_args)
{
  Widget w;
  Arg *new_args, largs[5];
...
static Widget CreateFindButton(Widget parent, ArgList args, Cardinal num_args)
{
  Widget w;
  Arg *new_args, largs[5];
  Cardinal num_largs = 0;
 ...
 */
static Widget CreateFindText(Widget parent, ArgList args, Cardinal num_args)
{
  Widget w;
  XmMultiListWidget elist = (XmMultiListWidget)parent;
 ...
 *	Returns:       none
 */
static Widget CreateFrame(Widget parent, ArgList args, Cardinal num_args)
{
  XmMultiListWidget elist = (XmMultiListWidget)parent;
  Widget w;
  ...
  num_largs++;
  new_args = XtMergeArgLists(args, num_args, largs, num_largs);
  XmMultiList_ilist(elist) = XtCreateManagedWidget(
      "list", xmI18ListWidgetClass, w, new_args, num_args + num_largs);
  XtFree((XtPointer)new_args);
  /* PWC - Note...
   *
 ...
 */
Widget XmCreateMultiList(Widget parent, String name, ArgList args, Cardinal num_args)
{
...
Widget XmVaCreateMultiList(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmMultiListWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedMultiList(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmMultiListWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
 ...
 *  Returns:       none
 */
void XmMultiListUnselectAllItems(Widget w)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
 ...
 *  Returns:       none
 */
void XmMultiListUnselectItem(Widget w, XmMultiListRowInfo *row_info)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
 ...
 *  Returns:       none
 */
void XmMultiListToggleRow(Widget w, short row)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
 ...
 *  Returns:       none
 */
void XmMultiListSelectItems(Widget w, XmString item, int column, Boolean notify)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
  XmI18ListSelectItems((XmI18ListWidget)XmMultiList_ilist(elist), item, column, notify);
  _XmAppUnlock(app);
}
 ...
 *  Returns:       none
 */
void XmMultiListDeselectItems(Widget w, XmString item, int column)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
  XmI18ListDeselectItems((XmI18ListWidget)XmMultiList_ilist(elist), item, column);
  _XmAppUnlock(app);
}
 ...
 *  Returns:       none
 */
void XmMultiListSelectRow(Widget w, int row, Boolean notify)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
 ...
 *  Returns:       none
 */
void XmMultiListDeselectRow(Widget w, int row)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
 ...
 */
int *XmMultiListGetSelectedRowArray(Widget w, int *num_rows)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  int *ret_val;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
  ret_val = XmI18ListGetSelectedRowArray((XmI18ListWidget)XmMultiList_ilist(elist), num_rows);
  _XmAppUnlock(app);
  return ret_val;
}
 ...
 *  Returns:       none
 */
void XmMultiListMakeRowVisible(Widget w, int row)
{
  XmMultiListWidget elist = (XmMultiListWidget)w;
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
