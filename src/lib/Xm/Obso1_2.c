...
Boolean _XmTestTraversability(Widget widget, XRectangle *visRect) /* unused */
{
  return XmIsTraversable(widget);
}

void _XmClearTabGroup(Widget w) /* unused */
{
  return;
}
...
void _XmClearKbdFocus(Widget tabGroup) /* unused */
{
  return;
}
...
Boolean _XmWidgetIsTraversable(Widget widget,
 ...
 *
 ************************************************************************/
void _XmEraseShadow(Display *display, Drawable d, int size, int x, int y, int width, int height)
{
  if (width > 0 && size > 0) {
  ...
  if (size > 0 && height - (2 * size) > 0) {
    ...
    XClearArea(display, d, x + width - size, y + size, size, height - (2 * size), FALSE);
  }
}
 ...
 *
 ************************************************************************/
void _XmGetArrowDrawRects(int highlight_thickness,
                          ...
                          XRectangle **bot)
{
  ...
  if (core_width > core_height) {
    size = core_height - 2 - 2 * (highlight_thickness + shadow_thickness);
    xOffset = (core_width - core_height) / 2;
  }
  else {
    size = core_width - 2 - 2 * (highlight_thickness + shadow_thickness);
  ...
  *bot = (XRectangle *)XtMalloc(sizeof(XRectangle) * (size / 2 + 6));
  ...
  while (width > 0) {
    if (width == 1) {
      ...
      if (size == 2 || (direction == XmARROW_UP || direction == XmARROW_LEFT)) {
        (*top)[t].x = start;
        (*top)[t].y = y;
        (*top)[t].width = 2;
        (*top)[t].height = 1;
      ...
      if (start == highlight_thickness + shadow_thickness + 1 + xOffset) {
 ...
 *
 ************************************************************************/
void _XmOffsetArrow(int diff_x,
                    int diff_y,
                    XRectangle *top,
                    XRectangle *cent,
                    XRectangle *bot,
...
void _XmDrawSquareButton(
    Widget w, int x, int y, int size, GC topGC, GC bottomGC, GC centerGC, Boolean fill)
{
  _XmDrawShadow(XtDisplay(w), XtWindow(w), topGC, bottomGC, 2, x, y, size, size);
  if (fill)
    if (size > 6)
...
void _XmDrawDiamondButton(
    Widget tw, int x, int y, int size, GC topGC, GC bottomGC, GC centerGC, Boolean fill)
{
  XSegment seg[12];
  XPoint pt[5];
  int midX, midY;
  ...
  XDrawSegments(XtDisplay((Widget)tw), XtWindow((Widget)tw), topGC, &seg[3], 3);
  XDrawSegments(XtDisplay((Widget)tw), XtWindow((Widget)tw), bottomGC, &seg[6], 6);
  XDrawSegments(XtDisplay((Widget)tw), XtWindow((Widget)tw), topGC, &seg[0], 3);
  /* For Fill */
  if (fill) {
    pt[0].x = x + 3;
    pt[0].y = midY - 1;
  ...
  XFillPolygon(
      XtDisplay((Widget)tw), XtWindow((Widget)tw), centerGC, pt, 4, Convex, CoordModeOrigin);
}
...
static void get_rects(int max_i,
                      register int offset,
                      register int x,
                      register int y,
                      register int width,
...
static void XmDrawEtchedShadow(Display *display,
                               Drawable d,
                               GC top_GC,
                               GC bottom_GC,
                               register int size,
                               ...
                               register int height)
{
  ...
  if (size <= 0)
    return;
  if (size == 1) {
    _XmDrawShadow(display, d, top_GC, bottom_GC, size, x, y, width, height);
    return;
  }
  if (size > width / 2)
    size = width / 2;
  ...
  get_rects(half_size, 0, x, y, width, height, pos_top, pos_left, pos_bottom, pos_right);
  pos_top = size3;
 ...
 *****************************************************************/
void _XmDrawShadowType(Widget w,
                       ...
                       GC bottom_shadow_GC)
{
  ...
  switch (shadow_type) {
    ...
    case XmSHADOW_ETCHED_OUT:
      XmDrawEtchedShadow(XtDisplay(w),
                         XtWindow(w),
                         (shadow_type == XmSHADOW_ETCHED_IN) ? bottom_shadow_GC : top_shadow_GC,
                         (shadow_type == XmSHADOW_ETCHED_IN) ? top_shadow_GC : bottom_shadow_GC,
                         shadow_thickness,
                         highlight_thickness,
                         highlight_thickness,
                         core_width - 2 * highlight_thickness,
...
****************************************************************/
void _XmFileSelectionBoxCreateFilterLabel(XmFileSelectionBoxWidget fsb)
{
  FS_FilterLabel(fsb) = _XmBB_CreateLabelG(
      (Widget)fsb, FS_FilterLabelString(fsb), "FilterLabel", XmFilterStringLoc);
}

void _XmFileSelectionBoxCreateDirListLabel(XmFileSelectionBoxWidget fsb)
{
...
/*****************************************************************/
void _XmFileSelectionBoxCreateDirList(XmFileSelectionBoxWidget fsb)
{
  Arg al[20];
  register int ac = 0;
...
/*****************************************************************/
void _XmFileSelectionBoxCreateFilterText(XmFileSelectionBoxWidget fs)
{
  Arg arglist[10];
  int argCount;
  char *stext_value;
...
/****************************************************************/
void _XmFileSelectionBoxGetDirectory(Widget fs,
                                     int resource, /* unused */
                                     XtArgVal *value)
{
  XmString data;
...
/****************************************************************/
void _XmFileSelectionBoxGetNoMatchString(Widget fs,
                                         int resource, /* unused */
                                         XtArgVal *value)
{
  XmString data;
...
/****************************************************************/
void _XmFileSelectionBoxGetPattern(Widget fs,
                                   int resource, /* unused */
                                   XtArgVal *value)
{
  XmString data;
...
/*****************************************************************/
void _XmFileSelectionBoxGetFilterLabelString(Widget fs,
...
/*****************************************************************/
void _XmFileSelectionBoxGetDirListLabelString(Widget fs,
                                              int resource_offset, /* unused */
                                              XtArgVal *value)
{
  XmString data;
...
/*****************************************************************/
void _XmFileSelectionBoxGetDirListItems(Widget fs,
                                        int resource_offset, /* unused */
                                        XtArgVal *value)
{
  XmString data;
...
/*****************************************************************/
void _XmFileSelectionBoxGetDirListItemCount(Widget fs,
                                            int resource_offset, /* unused */
                                            XtArgVal *value)
{
  XmString data;
...
/*****************************************************************/
void _XmFileSelectionBoxGetListItems(Widget fs,
                                     int resource_offset, /* unused */
                                     XtArgVal *value)
{
  XmString data;
...
/*****************************************************************/
void _XmFileSelectionBoxGetListItemCount(Widget fs,
                                         int resource_offset, /* unused */
                                         XtArgVal *value)
{
  XmString data;
...
/*****************************************************************/
void _XmFileSelectionBoxGetDirMask(Widget fs,
                                   int resource_offset, /* unused */
                                   XtArgVal *value)
{
  String filterText;
...
/****************************************************************/
void _XmFileSelectionBoxUpOrDown(Widget wid,
                                 XEvent *event,
                                 String *argv,
...
/****************************************************************/
void _XmFileSelectionBoxRestore(Widget wid, XEvent *event, String *argv, Cardinal *argc)
{
  XmFileSelectionBoxWidget fsb = (XmFileSelectionBoxWidget)wid;
  String itemString;
  String dir;
  ...
  else /* activeChild == FS_FilterText( fsb) */ { /* Should do this stuff entirely with XmStrings
                                                   * when the text widget supports it.
                                                   */
...
/*****************************************************************/
XmGeoMatrix _XmFileSBGeoMatrixCreate(Widget wid, Widget instigator, XtWidgetGeometry *desired)
{
  return (*(xmFileSelectionBoxClassRec.bulletin_board_class.geo_matrix_create))(
      wid, instigator, desired);
}

/****************************************************************/
Boolean _XmFileSelectionBoxNoGeoRequest(XmGeoMatrix geoSpec)
{
...
/****************************************************************/
void _XmFileSelectionBoxFocusMoved(Widget wid, XtPointer client_data, XtPointer data)
{
  (*(xmFileSelectionBoxClassRec.bulletin_board_class.focus_moved_proc))(wid, client_data, data);
}

XmHeap _XmHeapCreate(int segment_size)
{
  XmHeap heap;
...
char *_XmHeapAlloc(XmHeap heap, Cardinal bytes)
{
...
void _XmGetWidgetNavigPtrs(Widget widget, WidgetNavigPtrs np)
{
...
typedef enum { HereOnly, AboveOnly, BelowOnly, AboveAndBelow } XmNavigTestType;

Boolean _XmPathIsTraversable(Widget widget,
...
void _XmBB_GetDialogTitle(Widget bb,
                          int resource, /* unused */
                          XtArgVal *value)
{
  XmString data = XmStringCopy((XmString)((XmBulletinBoardWidget)bb)->bulletin_board.dialog_title);
  *value = (XtArgVal)data;
}
...
Widget _XmGetDisplayObject(Widget shell,
                           ArgList args,       /* unused */
...
Widget _XmGetScreenObject(Widget shell,
                          ArgList args,       /* unused */
                          Cardinal *num_args) /* unused */
{
...
XmWrapperData _XmGetWrapperData(WidgetClass w_class) /* unused */
{
...
void _XmLowerCase(register char *source, register char *dest)
{
  register char ch;
  int i;
...
void _XmCreateMenuCursor(Widget m) /* unused */
{
  /* ... or this one.
   */
}
...
Boolean _XmGetTransientFlag(Widget w) /* unused */
{
  return simplistic_transient_flag;
}

void _XmSetTransientFlag(Widget w, /* unused */
                         Boolean value)
{
...
Boolean _XmQueryPixmapCache(Screen *screen,   /* unused */
...
void _XmRC_GetLabelString(XmRowColumnWidget rc,
...
void _XmRC_GetMenuAccelerator(XmRowColumnWidget rc,
                              XrmQuark resource, /* unused */
                              XtArgVal *value)
{
  String data;
...
void _XmRC_GetMnemonicCharSet(XmRowColumnWidget rc,
                              XrmQuark resource, /* unused */
                              XtArgVal *value)
{
  ...
  if (label) {
    ...
    Arg al[1];
    String data;
    XtSetArg(al[n], XmNmnemonicCharSet, &data);
    n++;
...
void _XmScaleGetTitleString(Widget wid,
                            int resource, /* unused */
                            XtArgVal *value)
{
...
int _XmTextGetBaseLine(Widget widget)
{
  return XmTextGetBaseline(widget);
}

void _XmTextOutLoadGCsAndRecolorCursors(XmTextWidget old_tw, /* unused */
 ...
 ************************************************************************/
void _XmDoGadgetTraversal(XmManagerWidget mw,
                          XEvent *event, /* unused */
                          int direction)
{
 ...
 *
 **********************************************************************/
void _XmBuildManagerResources(WidgetClass c)
{
...
void _XmGetHighlightColor(Widget w,
                          int offset, /* unused */
                          XtArgVal *value)
{
...
void _XmGetTopShadowColor(Widget w,
                          int offset, /* unused */
                          XtArgVal *value)
{
  XmManagerWidget mw = (XmManagerWidget)XtParent(w);
...
void _XmGetBottomShadowColor(Widget w,
                             int offset, /* unused */
                             XtArgVal *value)
{
  XmManagerWidget mw = (XmManagerWidget)XtParent(w);
 ...
 *
 ************************************************************************/
void _XmBuildPrimitiveResources(WidgetClass c)
{
  ...
  if (sc == (XmPrimitiveWidgetClass)widgetClass)
    return;
