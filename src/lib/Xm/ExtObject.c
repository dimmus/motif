...
static Boolean SetValuesPrehook(
    Widget req, Widget curr, Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesPrehook(Widget new_w, ArgList args, Cardinal *num_args);
static Boolean SetValues(Widget old, Widget ref, Widget new_w, ArgList args, Cardinal *num_args);
...
static XtResource extResources[] = {{XmNlogicalParent,
                                     XmCLogicalParent,
                                     XmRWidget,
                                     sizeof(Widget),
...
externaldef(xmextobjectclass) WidgetClass xmExtObjectClass = (WidgetClass)(&xmExtClassRec);

/*ARGSUSED*/
static void UseParent(Widget w,
                      int offset, /* unused */
                      XrmValue *value)
{
...
static void Initialize(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (!(*wcePtr)->use_sub_resources) {
    if (resParent) {
      ...
      _XmProcessLock();
      extData->reqWidget = (Widget)_XmExtObjAlloc(XtClass(new_w)->core_class.widget_size);
      memcpy((char *)extData->reqWidget, (char *)req, XtClass(new_w)->core_class.widget_size);
      _XmProcessUnlock();
...
/*ARGSUSED*/
static Boolean SetValuesPrehook(Widget req,  /* unused */
                                Widget curr, /* unused */
                                Widget new_w,
                                ArgList args,
                                Cardinal *num_args)
{
  ...
  if ((*wcePtr)->use_sub_resources) {
    _XmProcessLock();
    XtSetSubvalues(
        (XtPointer)new_w, (*wcePtr)->ext_resources, (*wcePtr)->num_ext_resources, args, *num_args);
    _XmProcessUnlock();
  }
  return False;
}
 ...
 *
 **********************************************************************/
void _XmBuildExtResources(WidgetClass c)
{
  XmExtObjectClass wc = (XmExtObjectClass)c;
  XmExtObjectClass sc;
  _XmProcessLock();
  _XmInitializeSyntheticResources(wc->ext_class.syn_resources, wc->ext_class.num_syn_resources);
  if (wc != (XmExtObjectClass)xmExtObjectClass) {
