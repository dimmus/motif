...
static void CalcChildSize(XmButtonBoxWidget,
                          Widget,
                          Dimension,
                          Dimension,
                          Dimension,
...
static XtResource resources[] = {{XmNequalSize,
                                  ...
                                  XmRBoolean,
                                  sizeof(Boolean),
                                  XtOffsetOf(XmButtonBoxRec, button_box.equal_size),
                                  XmRImmediate,
                                  (XtPointer)False},
                                 {XmNfillOption,
                                  XmCFillOption,
                                  XmRXmFillOption,
                                  sizeof(XmFillOption),
                                  XtOffsetOf(XmButtonBoxRec, button_box.fill_option),
                                  XmRImmediate,
                                  (XtPointer)XmFillNone},
                                 {XmNmarginHeight,
                                  XmCMargin,
                                 ...
                                 {XmNorientation,
                                  ...
                                  XtOffsetOf(XmButtonBoxRec, button_box.orientation),
                                  XmRImmediate,
                                 ...
                                 {XmNdefaultButton,
                                  ...
                                  XmRWidget,
                                  sizeof(Widget),
                                  XtOffsetOf(XmButtonBoxRec, button_box.default_button),
                                  XmRImmediate,
                                  (XtPointer)NULL}};
...
static XtResource constraints[] = {{"pri.vate1",
                                    "Pri.vate1",
                                    XmRDimension,
                                    sizeof(Dimension),
 ...
 */
/* ARGSUSED */
static void ConstraintInitialize(Widget request, Widget set, ArgList args, Cardinal *num_args)
{
                                        ...
                                        XtWidgetGeometry *reply)
{
  ...
  if (request->request_mode & (CWWidth | CWHeight)) {
    ...
    CalcChildSize(
        bbox, w, max_major, max_minor, child_major_total, num_managed, &child_major, &child_minor);
    if (IsHorizontal(bbox)) {
    ...
    if (((child_width == XmButtonBoxC_pref_width(w)) || !(request->request_mode & CWWidth)) &&
        ((child_height == XmButtonBoxC_pref_height(w)) || !(request->request_mode & CWHeight)))
    {
...
static void ChangeManaged(Widget w)
{
  ...
  if (w->core.height == 1)
    mask |= CWHeight;
  if (mask)
    (void)TryNewLayout(w, &mask, False);
  LayoutChildren(w, NULL);
  /*
    ...
    Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  if (set->core.width == 0) { /* CR03420 */
    set->core.width = 1;      /* setting to 1 to prevent zero width */
    mask |= CWWidth;
  }
  if (set->core.height == 0) { /* CR03420 */
    set->core.height = 1;      /* setting to 1 to prevent zero height */
    mask |= CWHeight;
  }
  if (mask)
    TryNewLayout((Widget)b_set, &mask, False);
  if (newLayout == True)
    LayoutChildren((Widget)b_set, NULL);
  return (False);
}
 ...
 */
/* ARGSUSED */
static Boolean CvtStringToFillOption(
 ...
 */
/* ARGSUSED */
static XtGeometryResult TryNewLayout(Widget parent, Mask *mask, Boolean queryOnly)
{
...
static void LayoutChildren(Widget w, Widget special_child)
{
  ...
  ForAllChildren(bbox, childp)
  {
    ...
    if (*childp == special_child) {
      ...
      _XmConfigureWidget(
          *childp, pos_x, pos_y, child_width, child_height, (*childp)->core.border_width);
    }
 ...
 */
/* ARGSUSED */
static Cardinal CalcChildrenPrefSizes(XmButtonBoxWidget bbox,
                                      Dimension *max_major,
                                      Dimension *max_minor,
                                      Dimension *total)
{
 ...
 */
/* ARGSUSED */
static void CalcChildSize(XmButtonBoxWidget bbox,
                          Widget w,
                          Dimension max_major,
                          Dimension max_minor,
                          Dimension child_major_total,
                          Cardinal num_managed,
                          Dimension *child_major,
 ...
 */
Widget XmCreateButtonBox(Widget parent, String name, ArgList args, Cardinal num_args)
{
...
Widget XmVaCreateButtonBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmButtonBoxWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedButtonBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmButtonBoxWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
