...
externaldef(xmprintshellclass) WidgetClass
   ...
   = (WidgetClass)(&xmPrintShellClassRec);
   ...
   Boolean  *continue_to_dispatch)
{
   ...
   if (!print_shell->print.start_job_callback && !print_shell->print.page_setup_callback && !print_shell->print.end_job_callback) return;
   pr_cbs.event   = event;
...
static void
ResourcesUpdate(Widget w, XPContext pcontext, Dimension *pwidth, Dimension *pheight)
{
   XmPrintShellWidget print_shell = (XmPrintShellWidget)w;
   ...
   if (!print_shell->print.print_resolution)
      ...
      print_shell->print.print_resolution = (254 * WidthOfScreen(XtScreen(w)) + 5 * WidthMMOfScreen(XtScreen(w))) / (10 * WidthMMOfScreen(XtScreen(w)));
...
******************************/
static void
GetFromTable(Display *pdpy, XPContext context, Widget *widget)
{
   XFindContext(pdpy, (XID)context, PrintContextToWidget, (XPointer *)widget);
}

static void
DeleteFromTable(XPContext context, Widget widget)
{
   XDeleteContext(XtDisplay(widget), (XID)context, PrintContextToWidget);
}

static void
AddToTable(XPContext context, Widget widget)
{
   ...
   Cardinal *num_args)
{
   ...
   XSaveContext(XtDisplay(new_widget), (XID)XtScreen(new_widget), _XmPrintScreenToShellContext, (XPointer)new_widget);
   ...
   Cardinal num_args)
{
   ...
   /* we want to be able to specify things like:
       ...
       named "print".
       */
   pappshell = XtVaAppCreateShell(video_name, video_class, applicationShellWidgetClass, print_display, XmNscreen, print_screen, NULL);
...
static void
FilePipeCB(XtPointer client_data, int *source, XtInputId *id)
{
...
static void
ChildPrintToFile(String    display_name,
                 XPContext pcontext,
                 FILE     *file,
                 char     *file_name,
                 int       pipe,
                 ...
                 String    application_class)
{
   ...
   if ((display = XtOpenDisplay(app_context, display_name, application_name, application_class, NULL, 0, &argc, argv)) == NULL)
   {
      _exit(1);
   }
              ...
              XPointer     client_data)
{
   ...
   if (pid == 0)
   {
      ChildPrintToFile(display_name, pcontext, file, file_name, filedes[1], application_name, application_class);
   }
   else if (pid < 0)
   {
      fclose(file);
                 ...
                 Boolean  *cont)
{
   ...
   static char               *atom_names[] = { XmIPDM_REPLY, XmIPDM_EXIT_OK, XmIPDM_EXIT_ERROR, XmIPDM_EXIT_CANCEL };
   XmPrintShellWidget         print_shell  = (XmPrintShellWidget)w;
   XmPrintShellCallbackStruct pr_cbs;
                 ...
                 int           *format)
{
   ...
   Atom                       atoms[XtNumber(atom_names)];
   assert(XtNumber(atom_names) == NUM_ATOMS);
   XInternAtoms(XtDisplay(pdm_select_data->print_shell), atom_names, XtNumber(atom_names), False, atoms);
                ...
                Widget transient_for_video_shell)
{
   ...
   if (display_used == XtDisplay(print_shell))
      ...
      return XmPDM_NOTIFY_FAIL;
   XtSetSelectionParameters(widget_for_selection, pdm_selection, type, (XtPointer)value, length, format);
   XFree(value);
...
#define REALLY_LONG_TIMEOUT (2 * 60 * 1000)
   ...
   pdm_select_data->transient_for_input_only_window = XCreateWindow(XtDisplay(transient_for_video_shell),
                                                                    XtWindow(transient_for_video_shell),
                                                                    0,
                                                                    0,
