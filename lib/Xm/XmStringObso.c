   ...
   {             /* looking for \n */
      ...
      if ((start != end) || (start == text))
         string = XmStringConcatAndFree(string,
                                        _XmStringNCreate(start, tag, end - start));
   ...
   char      **text)
{
   ...
   *text = NULL; /* pre-condition result */
   ...
   while (!done)
   {
      ...
      {
         if (c && ((d == XmSTRING_DIRECTION_L_TO_R) || (d == XmSTRING_DIRECTION_UNSET)) && (((is_local || is_default) && ((c == XmFONTLIST_DEFAULT_TAG) || (strcmp(c, XmFONTLIST_DEFAULT_TAG) == 0) || (strcmp(c, _XmStringGetCurrentCharset()) == 0))) || (curtag && (strcmp(c, curtag) == 0))))
         {
   ...
   Boolean           *separator)
{
   ...
   result = _XmStringGetSegment(context, True, True, (XtPointer *)text, tag, &type, &rendition_tags, &tag_count, direction, separator, &tabs, char_count, &push_before, &pop_after);
   if (result)
   {
      if (rendition_tags)
      {
