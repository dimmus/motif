...
static XmTextPosition NullScan(XmTextSource source,
                               ...
                               int n,
                               Boolean include);
static Boolean NullGetSelection(XmTextSource source, XmTextPosition *start, XmTextPosition *end);
...
static void AddRedraw(XmTextWidget tw, XmTextPosition left, XmTextPosition right);
static _XmHighlightRec *FindHighlight(XmTextWidget tw,
                                      XmTextPosition position,
                                      XmTextScanDirection dir);
static void DisplayText(XmTextWidget tw, XmTextPosition updateFrom, XmTextPosition updateTo);
...
static void RemoveLines(XmTextWidget tw, int num_lines, unsigned int cur_index);
...
static void GetValuesHook(Widget w, ArgList args, Cardinal *num_args_ptr);
static Boolean SetValues(Widget oldw, Widget reqw, Widget new_w, ArgList args, Cardinal *num_args);
static XtGeometryResult QueryGeometry(Widget w,
                                      XtWidgetGeometry *intended,
                                      XtWidgetGeometry *reply);
static void _XmTextSetString(Widget widget, char *value);
static XtPointer TextGetValue(Widget w, int format);
static void TextSetValue(Widget w, XtPointer s, int format);
...
static void _XmCreateCutBuffers(Widget w)
{
  ...
  if (XFindContext(dpy, (Window)screen, local_context, &tmp)) {
    ...
    XtAddCallback(xm_display,
                  ...
                  (XtPointer)ctx_data);
    XChangeProperty(
        dpy, RootWindowOfScreen(screen), XA_CUT_BUFFER0, XA_STRING, 8, PropModeAppend, NULL, 0);
    XChangeProperty(
        dpy, RootWindowOfScreen(screen), XA_CUT_BUFFER1, XA_STRING, 8, PropModeAppend, NULL, 0);
    XChangeProperty(
        dpy, RootWindowOfScreen(screen), XA_CUT_BUFFER2, XA_STRING, 8, PropModeAppend, NULL, 0);
    XChangeProperty(
        dpy, RootWindowOfScreen(screen), XA_CUT_BUFFER3, XA_STRING, 8, PropModeAppend, NULL, 0);
    XChangeProperty(
        dpy, RootWindowOfScreen(screen), XA_CUT_BUFFER4, XA_STRING, 8, PropModeAppend, NULL, 0);
    XChangeProperty(
        dpy, RootWindowOfScreen(screen), XA_CUT_BUFFER5, XA_STRING, 8, PropModeAppend, NULL, 0);
    XChangeProperty(
        dpy, RootWindowOfScreen(screen), XA_CUT_BUFFER6, XA_STRING, 8, PropModeAppend, NULL, 0);
    XChangeProperty(
        dpy, RootWindowOfScreen(screen), XA_CUT_BUFFER7, XA_STRING, 8, PropModeAppend, NULL, 0);
...
static void DisplayText(XmTextWidget tw, XmTextPosition updateFrom, XmTextPosition updateTo)
{
   ...
   */
  for (i = _XmTextPosToLine(tw, updateFrom); updateFrom <= updateTo && i < tw->text.number_lines;
       i++)
  {
...
static void RedrawChanges(XmTextWidget tw)
{
  ...
  if (tw->text.first_position == tw->text.last_position) {
    (*tw->text.output->Draw)(
        tw, (LineNum)0, tw->text.first_position, tw->text.last_position, XmHIGHLIGHT_NORMAL);
  }
...
static void RefigureLines(XmTextWidget tw)
{
  ...
  for (i = 0; i == 0 || !line[i - 1].past_end; i++) {
    ...
    if (line[i].start >= tw->text.forget_past || line[i].start != oldline[j].start ||
        oldline[j].changed || oldline[j + 1].changed)
    {
      line[i].past_end = !(*tw->text.output->MeasureLine)(
          tw, i, line[i].start, &line[i + 1].start, &line[i].extra);
      line[i + 1].extra = NULL;
    ...
    if (!line[i].past_end) {
      if (line[i].start != oldline[j].start || line[i + 1].start != oldline[j + 1].start ||
                             ...
                             register XmTextPosition cur_end)
{
  ...
  cur_index++;
  line_end = (*tw->text.source->Scan)(
      tw->text.source, cur_start, XmSELECT_LINE, XmsdRight, 1, TRUE);
  while (next_start < cur_end) {
    ...
    if (next_start == PASTENDPOS || next_start == cur_end)
      break;
    ...
    if (line_end == next_start) {
      ...
      line_end = (*tw->text.source->Scan)(
          tw->text.source, next_start, XmSELECT_LINE, XmsdRight, 1, TRUE);
    }
    else
    ...
    Widget widget, XmTextPosition start, XmTextPosition end, XmTextBlock block, Boolean update)
{
  ...
  if ((start == end && length == 0) ||
      (word_wrap && !XtIsRealized(widget) && XmIsScrolledWindow(XtParent(widget)) &&
...
static void Redisplay(XmTextWidget tw)
{
  ...
  if (tw->text.in_redisplay || tw->core.being_destroyed || tw->text.disable_depth != 0 ||
 ...
 * Mark the given range of text to be redrawn.
 */
void _XmTextMarkRedraw(XmTextWidget tw, XmTextPosition left, XmTextPosition right)
{
  if (left < right) {
 ...
 */
LineNum _XmTextNumLines(XmTextWidget tw)
{
  if (tw->text.needs_refigure_lines)
    RefigureLines(tw);
...
void _XmTextLineInfo(XmTextWidget tw,
                     LineNum line,
                     ...
                     LineTableExtra *extra)
{
  ...
  if (tw->text.number_lines >= line) {
    if (startpos)
      *startpos = tw->text.line[line].start;
    if (extra)
      *extra = tw->text.line[line].extra;
  }
  else {
    if (startpos) {
 ...
 */
LineNum _XmTextPosToLine(XmTextWidget tw, XmTextPosition position)
{
  int i;
  if (tw->text.needs_refigure_lines)
    RefigureLines(tw);
 ...
 *
 ****************************************************************/
void _XmTextInvalidate(XmTextWidget tw, XmTextPosition position, XmTextPosition topos, long delta)
{
  LineNum l;
  int i;
  ...
  int shift_start = 0;
#define ladjust(p) \
  if ((p > position && p != PASTENDPOS) || (p == position && delta < 0)) { \
    p += delta; \
    if (p < tw->text.first_position) \
      p = tw->text.first_position; \
    if (p > tw->text.last_position) \
      p = tw->text.last_position; \
  }
#define radjust(p) \
  if ((p > position && p != PASTENDPOS) || (p == position && delta > 0)) { \
    p += delta; \
  ...
  tw->text.first_position = (*tw->text.source->Scan)(
      tw->text.source, 0, XmSELECT_ALL, XmsdLeft, 1, FALSE);
  tw->text.last_position = (*tw->text.source->Scan)(
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if (req->core.width == 0)
    newtw->core.width = req->core.width;
  if (req->core.height == 0)
    newtw->core.height = req->core.height;
  ...
  if (!XmRepTypeValidValue(XmRID_EDIT_MODE, newtw->text.edit_mode, nw)) {
  ...
  newtw->text.onthespot->start = newtw->text.onthespot->end = newtw->text.onthespot->cursor =
      newtw->text.onthespot->over_len = newtw->text.onthespot->over_maxlen = 0;
...
static void InitializeHook(Widget wid, ArgList args, Cardinal *num_args_ptr)
{
  ...
  tw->text.highlight.list = (_XmHighlightRec *)XtMalloc(sizeof(_XmHighlightRec));
  ...
  tw->text.old_highlight.list = (_XmHighlightRec *)XtMalloc(sizeof(_XmHighlightRec));
  tw->text.highlight_changed = FALSE;
  ...
  if (tw->text.edit_mode == XmMULTI_LINE_EDIT)
    top_character = (*tw->text.source->Scan)(
        tw->text.source, tw->text.top_character, XmSELECT_LINE, XmsdLeft, 1, FALSE);
  else
...
void _XmTextSetTopCharacter(Widget widget, XmTextPosition top_character)
{
  XmTextWidget tw = (XmTextWidget)widget;
  LineNum line_num;
  if (tw->text.edit_mode != XmSINGLE_LINE_EDIT) {
...
static Boolean SetValues(Widget oldw, Widget reqw, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (!XmRepTypeValidValue(XmRID_EDIT_MODE, newtw->text.edit_mode, new_w)) {
      ...
      (newtw->text.top_character != newtw->text.new_top))
  {
    ...
    if (newtw->text.output->data->resizeheight &&
        !(newtw->text.output->data->scrollvertical && XmIsScrolledWindow(XtParent((Widget)newtw))))
      new_top = 0;
    else
  ...
  if (old->text.cursor_position != newtw->text.cursor_position) {
    ...
    if (cursorPos == -1)
      cursorPos = (*newtw->text.source->Scan)(
          newtw->text.source, newtw->text.cursor_position, XmSELECT_ALL, XmsdLeft, 1, TRUE);
  ...
  (*newtw->text.output->PosToXY)(
      newtw, newtw->text.cursor_position, &(newtw->text.cursor_position_x), &dummy);
  if (o_redisplay)
  ...
  if (newtw->core.width == 0)
    newtw->core.width = old->core.width;
  if (newtw->core.height == 0)
    newtw->core.height = old->core.height;
                                      ...
                                      XtWidgetGeometry *reply)
{
  ...
  if ((GMode(intended) != GMode(reply)) || (reply->width != intended->width) ||
...
void _XmTextSetCursorPosition(Widget widget, XmTextPosition position)
{
  ...
  if (!tw->text.add_mode && tw->text.pendingoff && _XmStringSourceHasSelection(source))
    (*source->SetSelection)(
        source, position, position, XtLastTimestampProcessed(XtDisplay(widget)));
...
void _XmTextSetEditable(Widget widget, Boolean editable)
{
  Arg args[20];
  XIMCallback xim_cb[4];
  Cardinal n = 0;
...
void _XmTextSetHighlight(Widget w, XmTextPosition left, XmTextPosition right, XmHighlightMode mode)
{
  XmTextWidget tw = (XmTextWidget)w;
  _XmHighlightRec *l;
...
void _XmTextShowPosition(Widget widget, XmTextPosition position)
{
  XmTextWidget tw = (XmTextWidget)widget;
  _XmWidgetToAppContext(widget);
  _XmAppLock(app);
  if (!tw->text.needs_refigure_lines && (position < 0 || (position >= tw->text.top_character &&
...
int _XmTextGetTotalLines(Widget widget)
{
  return (((XmTextWidget)widget)->text.total_lines);
}

/* Why is this here? It's never used */
XmTextLineTable _XmTextGetLineTable(Widget widget, int *total_lines)
{
  XmTextWidget tw = (XmTextWidget)widget;
  XmTextLineTable line_table;
...
static int TextPreferredValue(Widget w) /* unused */
{
 ...
 */
static void _XmTextPreeditSetCursorPosition(Widget widget, XmTextPosition position)
{
  XmTextWidget tw = (XmTextWidget)widget;
  Position dummy;
  if (position < 0) {
...
static int PreeditStart(XIC xic, XPointer client_data, XPointer call_data)
{
  ...
  if (tw->text.input->data->overstrike) {
    lastPos = (*(tw->text.source->Scan))(
        tw->text.source, PreCursorTW(tw), XmSELECT_LINE, XmsdRight, 1, TRUE);
...
static void PreeditDraw(XIC xic, XPointer client_data, XIMPreeditDrawCallbackStruct *call_data)
{
  ...
  if (!call_data->caret && !call_data->chg_first && !call_data->chg_length && !call_data->text)
    return;
  ...
  /* turn cursor off */
  (*tw->text.output->DrawInsertionPoint)(tw, tw->text.cursor_position, off);
  XmTextSetHighlight(w,
                     PreStartTW(tw) + call_data->chg_first,
  ...
  if (data->overstrike) {
    ...
    if (rest_len) {
      over_mb = _XmStringSourceGetString(
          tw,
 ...
 */
void _XmTextResetIC(Widget widget)
{
...
char *XmTextGetString(Widget widget)
{
  ...
  if (XmIsTextField(widget)) {
    ...
    textT = (XmAccessTextualTrait)XmeTraitGet((XtPointer)XtClass(widget), XmQTaccessTextual);
    if (textT)
...
void XmTextSetTopCharacter(Widget widget, XmTextPosition top_character)
{
  ...
  if (tw->text.output->data->resizeheight &&
      !(tw->text.output->data->scrollvertical && XmIsScrolledWindow(XtParent(widget))))
  {
                     ...
                     XmTextPosition cursor_position)
{
  ...
  if (tw->text.edit_mode == XmMULTI_LINE_EDIT)
    top_character = (*tw->text.source->Scan)(
        tw->text.source, top_character, XmSELECT_LINE, XmsdLeft, 1, FALSE);
  tw->text.new_top = top_character;
  tw->text.top_character = 0;
...
Widget XmVaCreateText(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmTextWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedText(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmTextWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
