   ...
   Pixel  background)
{
   ...
   */
   ...
   XmGetColors(windowed_ancestor->core.screen, windowed_ancestor->core.colormap, use_background, &foreground_ret, &topshadow_ret, &bottomshadow_ret, NULL);
   if ((XmIsManager(w)) || (XmIsPrimitive(w)) || (XmIsGadget(w)))
   {
      ...
      {
         if ((XmIsScrollBar(w)) || (XmIsPushButton(w)) || (XmIsPushButtonGadget(w)) || (XmIsToggleButton(w)) || (XmIsToggleButtonGadget(w)))
         {
