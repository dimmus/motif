...
externaldef(dragContextclass) WidgetClass
   xmDragContextClass
   = (WidgetClass)&xmDragContextClassRec;
   ...
   Cardinal *numArgs)
{
   ...
   if (oldDC->drag.operationCursorIcon != newDC->drag.operationCursorIcon || oldDC->drag.sourceCursorIcon != newDC->drag.sourceCursorIcon || oldDC->drag.sourcePixmapIcon != newDC->drag.sourcePixmapIcon || oldDC->drag.stateCursorIcon != newDC->drag.stateCursorIcon || oldDC->drag.cursorBackground != newDC->drag.cursorBackground || oldDC->drag.cursorForeground != newDC->drag.cursorForeground || oldDC->drag.noneCursorForeground != newDC->drag.noneCursorForeground || oldDC->drag.invalidCursorForeground != newDC->drag.invalidCursorForeground || oldDC->drag.validCursorForeground != newDC->drag.validCursorForeground)
   {
   ...
   Atom     atom)
{
   ...
   Window         inf  = 0;
   XGetWindowProperty(dpy, win, atom, 0, 0, False, AnyPropertyType, &type, &format, &nitems, &after, &data);
   if (data)
      XFree(data);
   if (type)
      return win;
   ...
   Window        win)
{
   ...
   /*
     ...
     */
   if ((dc->drag.trackingMode == XmDRAG_TRACK_MOTION) && (XtWindow(dc->drag.srcShell) == win) && (!currReceiverInfo || (currReceiverInfo->frame == currReceiverInfo->window)))
   {
...
static int
drag_error(Display *display, XErrorEvent *e)
{
   (void)display;
   (void)e;
   if (!current_dc)
   ...
   unsigned char messageType)
{
   ...
   if (dc->drag.activeProtocolStyle == XmDRAG_NONE || (dc->drag.activeProtocolStyle == XmDRAG_DROP_ONLY && reason != XmCR_DROP_START))
      return;
   ...
   /*
     ...
     * to the wire
     */
   if ((!dc->drag.currReceiverInfo->shell) && (!dc->drag.sourceIsExternal /* sanity check */) && (dc->drag.activeProtocolStyle == XmDRAG_DYNAMIC || dc->drag.activeProtocolStyle == XmDRAG_XDND || reason == XmCR_DROP_START))
   {
      XFlush(XtDisplayOfObject((Widget)dc));
      XSync(XtDisplayOfObject((Widget)dc), False);
      current_dc  = dc;
      old_handler = XSetErrorHandler(drag_error);
      _XmSendICCCallback(XtDisplayOfObject((Widget)dc), destination, dc->drag.srcWindow, dc->drag.activeProtocolStyle, &callbackRec, XmICC_INITIATOR_EVENT);
      XFlush(XtDisplayOfObject((Widget)dc));
      XSync(XtDisplayOfObject((Widget)dc), False);
   ...
   else
   {
      ...
      if ((reason == XmCR_TOP_LEVEL_ENTER) || (reason == XmCR_TOP_LEVEL_LEAVE) || (reason == XmCR_DROP_START))
      {
   ...
   XmDragContext dc)
{
   ...
   XtRemoveEventHandler(dc->drag.srcShell, FocusChangeMask, True, InitiatorMsgHandler, (XtPointer)dc);
   ...
   XtRequestId   *requestID)
{
   ...
   if ((success = (*target == atoms[XmATRANSFER_SUCCESS])) || (*target == atoms[XmATRANSFER_FAILURE]))
   {
      if (success)
   ...
   int           *formatRtn)
{
   ...
   {
      Atom motifDrop = atoms[XmA_MOTIF_DROP];
      returnVal      = (Boolean)((*(dc->drag.convertProc.sel))((Widget)dc,
                                                          &motifDrop,
                                                          target,
                                                          typeRtn,
                                                          valueRtn,
   ...
   Window        newRoot)
{
   ...
   */
   dc->drag.curDragOver = (XmDragOverShellWidget)
      XtCreatePopupShell("dragOver", xmDragOverShellWidgetClass, (Widget)dc, args, i);
   ...
   XtPointer call)
{
   ...
   if (dc->drag.activeProtocolStyle == XmDRAG_NONE && dc->drag.currReceiverInfo->dragProtocolStyle == XmDRAG_XDND)
   ...
   switch (cb->reason)
   {
	 ...
	 */
         ...
         current_dc  = dc;
         old_handler = XSetErrorHandler(drag_error);
         _XmSendICCCallback(XtDisplayOfObject((Widget)dc), dc->drag.srcWindow, dc->drag.srcWindow, dc->drag.activeProtocolStyle, (XmICCCallback)cb, XmICC_RECEIVER_EVENT);
         XFlush(XtDisplayOfObject((Widget)dc));
         XSync(XtDisplayOfObject((Widget)dc), False);
         (void)XSetErrorHandler(old_handler);
         break;
   ...
   Boolean  *dontSwallow)
{
   ...
   XmICCCallbackStruct callbackRec;
   (void)dontSwallow;
   if (!dc || !_XmICCEventToICCCallback((XClientMessageEvent *)event, &callbackRec, XmICC_RECEIVER_EVENT))
      return;
   if (dc->drag.dragStartTime > callbackRec.any.timeStamp)
      return;
   if (dc->drag.activeProtocolStyle != XmDRAG_XDND && dc->drag.crossingTime > callbackRec.any.timeStamp)
      return;
   ...
   XtPointer call)
{
   ...
   {
      XtCallCallbackList((Widget)dc, dc->drag.siteLeaveCallback, (XmDropSiteLeaveCallbackStruct *)cb);
   }
   CalculateDragOperation(dc);
   /*
   ...
   XtPointer call)
{
   ...
   dc->drag.dragTimerId = XtAppAddTimeOut(appContext,
   ...
   XEvent       *event)
{
   ...
   { /* XmDRAG_TRACK_WM_QUERY */
      dc->drag.trackingMode = XmDRAG_TRACK_WM_QUERY_PENDING;
      confineWindow         = XtWindow(dc->drag.srcShell);
   }
   if (dc->drag.trackingMode == XmDRAG_TRACK_WM_QUERY_PENDING && activeProtocolStyle == XmDRAG_PREREGISTER)
   {
      dc->drag.blendModel = XmBLEND_NONE;
   }
   NewScreen(dc, RootWindowOfScreen(XtScreen(dc)));
   XtInsertEventHandler(dc->drag.srcShell, FocusChangeMask, True, InitiatorMsgHandler, (XtPointer)dc, XtListHead);
   mask = _XmDRAG_EVENT_MASK(dc);
   /*
   ...
   Window        subWindow)
{
   ...
   */
   if ((root != dc->drag.currWmRoot) || ((((dc->drag.trackingMode == XmDRAG_TRACK_MOTION) && (dc->drag.currReceiverInfo->frame != subWindow))) || ((dc->drag.trackingMode == XmDRAG_TRACK_WM_QUERY) && (dc->drag.currReceiverInfo->window != subWindow))))
   {
      if (dc->drag.currReceiverInfo->window)
      {
         if ((dc->drag.activeProtocolStyle != XmDRAG_NONE) && (dc->drag.activeProtocolStyle != XmDRAG_DROP_ONLY))
         {
            /*
	       ...
	       */
            if ((dc->drag.activeProtocolStyle == XmDRAG_DYNAMIC) && (!dc->drag.currReceiverInfo->shell) && (dc->drag.inDropSite))
            {
   ...
   MotionBuffer  mb)
{
   ...
   if (protoWindow == XtWindow(dc->drag.curDragOver))
   {
      Window currRoot = dc->drag.currWmRoot;
      int    dummyx, dummyy;
      (void)XTranslateCoordinates(XtDisplay(dc), currRoot, currRoot, dc->core.x, dc->core.y, &dummyx, &dummyy, &protoWindow);
   }
   /*
   ...
   Cardinal *numParams)
{
   ...
   { /* Update on modifier key changes */
      dx = 0;
      dy = 0;
   ...
   XEvent       *ev)
{
   ...
   if (dc->drag.currReceiverInfo)
   {
      if (dc->drag.currReceiverInfo->window)
      {
         if ((dc->drag.activeProtocolStyle != XmDRAG_NONE) && ((dc->drag.dragCompletionStatus == XmDROP) || (dc->drag.dragCompletionStatus == XmDROP_HELP)))
         {
            XtAppContext appContext = XtWidgetToApplicationContext((Widget)dc);
            /*
   ...
   unsigned int  state)
{
   ...
   {
      if ((dc->drag.currReceiverInfo->window) && (dc->drag.activeProtocolStyle != XmDRAG_NONE) && (dc->drag.activeProtocolStyle != XmDRAG_DROP_ONLY))
      {
         SendDragMessage(dc,
                         dc->drag.currReceiverInfo->window,
                         XmOPERATION_CHANGED);
   ...
   XtIntervalId *id)
{
   ...
   while ((*activeDC) && (XtAppGetExitFlag(appContext) == False))
   {
...
#ifdef XTHREADS
      XtInputMask mask;
      while (!(mask = XtAppPending(appContext))); /* busy wait */
      if (mask & XtIMXEvent)
      {
#endif
	 ...
	 */
         ...
         if ((event.type == MotionNotify || event.type == LeaveNotify || event.type == EnterNotify) && event.xmotion.state == dc->drag.lastEventState)
   ...
   Cardinal numArgs)
{
   ...
   if (event->type != ButtonPress && event->type != ButtonRelease && event->type != KeyRelease && event->type != KeyPress && event->type != MotionNotify)
   {
   ...
   dc = (XmDragContext)
      XtCreateWidget("dragContext", xmDragContextClass, (Widget)dd, mergedArgs, numArgs + 1);
...
CalculateDragOperation(XmDragContext dc)
{
   ...
   */
   ...
   {
      dc->drag.operations = dc->drag.operation = (unsigned char)(XmDROP_LINK & dc->drag.dragOperations);
   }
   else if (dc->drag.lastEventState & ShiftMask)
   {
      dc->drag.operations = dc->drag.operation = (unsigned char)(XmDROP_MOVE & dc->drag.dragOperations);
   }
   else if (dc->drag.lastEventState & ControlMask)
   {
      dc->drag.operations = dc->drag.operation = (unsigned char)(XmDROP_COPY & dc->drag.dragOperations);
   }
