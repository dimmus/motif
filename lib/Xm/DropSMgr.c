...
static XtResource resources[] = {
   { XmNnotifyProc,     XmCNotifyProc,     XmRCallbackProc, sizeof(XtCallbackProc), XtOffsetOf(struct _XmDropSiteManagerRec, dropManager.notifyProc),     XmRImmediate, NULL },
   { XmNtreeUpdateProc, XmCTreeUpdateProc, XmRCallbackProc, sizeof(XtCallbackProc), XtOffsetOf(struct _XmDropSiteManagerRec, dropManager.treeUpdateProc), XmRImmediate, NULL },
   { XmNclientData,     XmCClientData,     XmRPointer,      sizeof(XtPointer),      XtOffsetOf(struct _XmDropSiteManagerRec, dropManager.client_data),    XmRImmediate, NULL },
};
...
externaldef(xmdropsitemanagerobjectclass) WidgetClass
   ...
   = (WidgetClass)&xmDropSiteManagerClassRec;
   ...
   XtPointer               call_data)
{
   ...
   if (parentInfo == NULL)
   {
      ...
      AddDSChild(parentInfo, childInfo, GetDSNumChildren(parentInfo));
      if ((dsm->dropManager.treeUpdateProc) && (!XtIsRealized(parent) || (_XmGetDragProtocolStyle(parent) == XmDRAG_DYNAMIC)))
      {
         /*
   ...
   XmRegion r)
{
   ...
   return ((!_XmRegionIsEmpty(r)) && (IntersectWithDSInfoAncestors((XmDSInfo)GetDSParent(parent), r)));
}
   ...
   XmDragProcCallbackStruct *callback)
{
   ...
   num_import = _XmIndexToTargets(shell,
                                  GetDSImportTargetsID(info),
                                  &import_targets);
   if ((callback->operation != XmDROP_NOOP) && (XmTargetsAreCompatible(XtDisplay(dsm), export_targets, num_export, import_targets, num_import)))
   ...
   XtPointer               cb)
{
   ...
   if (dsm->dropManager.notifyProc)
   {
      (*(dsm->dropManager.notifyProc))((Widget)dsm, dsm->dropManager.client_data, (XtPointer)callback);
   }
   ...
   if (XmIsPrimitive(w))
   {
      ...
      switch (iccInfo.header.animationStyle)
      {
         ...
         case XmDRAG_UNDER_SHADOW_OUT:
         {
            ...
            info->animation_data.topShadowPixmap = info->animation_data.bottomShadowPixmap = XmUNSPECIFIED_PIXMAP;
            if (!GetDSHasRegion(dsInfo))
               info->animation_data.borderWidth = pw->core.border_width;
            info->animation_data.highlightThickness = pw->primitive.highlight_thickness;
   ...
   unsigned char          *type)
{
   ...
   switch (GetDSAnimationStyle(info))
   {
      case XmDRAG_UNDER_HIGHLIGHT:
      {
         XmDSRemoteHighlightStyle hs = (XmDSRemoteHighlightStyle)
   ...
   Cardinal                argCount)
{
   ...
   XtGetSubresources(widget, &fullInfoRec, NULL, NULL, _XmDSResources, _XmNumDSResources, args, argCount);
   /* Handle ignore first. */
   ...
   if ((fullInfoRec.animation_style == XmDRAG_UNDER_PIXMAP) && (fullInfoRec.animation_pixmap != XmUNSPECIFIED_PIXMAP) && (fullInfoRec.animation_pixmap_depth == 0))
   {
      /*
	     * They didn't tell us the depth of the pixmaps.  ask for it. */
      XmeGetPixmapData(XtScreen(widget), fullInfoRec.animation_pixmap, NULL, (int *)&(fullInfoRec.animation_pixmap_depth), NULL, NULL, NULL, NULL, NULL, NULL);
   }
   if ((fullInfoRec.type == XmDROP_SITE_COMPOSITE) && ((fullInfoRec.rectangles != NULL) || (fullInfoRec.num_rectangles != 1)))
   {
   ...
   fullInfoRec.import_targets_ID = _XmTargetsToIndex(shell,
                                                     fullInfoRec.import_targets,
                                                     fullInfoRec.num_import_targets);
   ...
   XmDSFullInfo            full_info)
{
   ...
   if (GetDSRemote(variant))
   {
      switch (GetDSAnimationStyle(variant))
      {
         ...
         case XmDRAG_UNDER_SHADOW_OUT:
         {
            XmDSRemoteShadowStyle ss = (XmDSRemoteShadowStyle)
               GetDSRemoteAnimationPart(variant);
         ...
         case XmDRAG_UNDER_PIXMAP:
         {
            XmDSRemotePixmapStyle ps = (XmDSRemotePixmapStyle)
               GetDSRemoteAnimationPart(variant);
   ...
   else
   {
      switch (GetDSAnimationStyle(variant))
      {
         ...
         case XmDRAG_UNDER_PIXMAP:
         {
            XmDSLocalPixmapStyle ps = (XmDSLocalPixmapStyle)
   ...
   Cardinal                argCount)
{
   ...
   /* Update the info */
   {
      ...
      if ((full_info->num_import_targets != old_num_import_targets) || (full_info->import_targets != old_import_targets))
      {
         Widget shell = widget;
         while (!(XtIsShell(shell))) shell = XtParent(shell);
         full_info->import_targets_ID = _XmTargetsToIndex(shell,
                                                          full_info->import_targets,
                                                          full_info->num_import_targets);
      }
   }
   ...
   if ((full_info->animation_style == XmDRAG_UNDER_PIXMAP) && (full_info->animation_pixmap_depth == 0))
   {
      XmeGetPixmapData(XtScreen(GetDSWidget(info)), full_info->animation_pixmap, NULL, (int *)&(full_info->animation_pixmap_depth), NULL, NULL, NULL, NULL, NULL, NULL);
   }
   /*
   ...
   Widget                  refWidget)
{
   ...
   if (clean)
   {
      dsm->dropManager.updateTimeOutId = XtAppAddTimeOut(XtWidgetToApplicationContext(shell), 0, _XmIEndUpdate, dsm);
   }
}
   ...
   Widget                  widget)
{
   ...
   if (info == (XmDSInfo)(dsm->dropManager.curInfo))
   {
      ...
      cdRec.dragOver = (Widget)(((XmDragContext)(dsm->dropManager.curDragContext))
                                   ->drag.curDragOver);
      HandleLeave(dsm, &cdRec, &cbRec, (XmDSInfo)dsm->dropManager.curInfo, style, False);
      dsm->dropManager.curInfo = NULL;
   }
   ...
   Cardinal stack_mode)
{
   ...
   if (sibling != NULL)
   {
      ...
      int      i;
      if ((sib == NULL) || (((XmDSInfo)GetDSParent(sib)) != parent) || (XtParent(widget) != XtParent(sibling)))
      {
         _XmAppUnlock(app);
         return;
      }
