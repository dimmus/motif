               ...
               Boolean        is_wchar)
{
   ...
   {
      if ((selleft > frompos && selleft < topos) || (selright > frompos && selright < topos) || (selleft <= frompos && selright >= topos))
      {
         (*source->SetSelection)(tw->text.source, tw->text.cursor_position, tw->text.cursor_position, XtLastTimestampProcessed(XtDisplay(widget)));
   ...
   { /* value is really a wchar_t ptr cast to char* */
      if (value == NULL)
      {
         block.length = 0;
      }
   ...
   if (tw->text.input->data->has_destination)
      _XmTextSetDestinationSelection(widget, tw->text.cursor_position, False, XtLastTimestampProcessed(XtDisplay(widget)));
          ...
          Time   cut_time)
{
   ...
   if (XmTextGetEditable(widget) && XmTextGetSelectionPosition(widget, &left, &right) && (right != left))
      /* start copy to clipboard */
                    ...
                    Boolean        want_wchar)
{
   ...
   for (pos = start; pos < end;)
   {
      ...
      { /* Need number of characters for buffer comparison */
