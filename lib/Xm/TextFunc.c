...
void _XmTextReplace(
    Widget widget, XmTextPosition frompos, XmTextPosition topos, char *value, Boolean is_wchar)
{
  ...
  if ((*source->GetSelection)(tw->text.source, &selleft, &selright)) {
    if ((selleft > frompos && selleft < topos) || (selright > frompos && selright < topos) ||
        (selleft <= frompos && selright >= topos))
    {
      (*source->SetSelection)(tw->text.source,
  ...
  if (!is_wchar) {
    ...
    if (value == NULL) {
      ...
      block.length = wcstombs(block.ptr, tmp_wc, (num_chars + 1) * (int)tw->text.char_size);
      if (block.length == -1) {
  ...
  if (_XmTextModifyVerify(tw, NULL, &frompos, &topos, &cursorPos, &block, &newblock, &freeBlock)) {
    ...
    if (UnderVerifyPreedit(tw))
      if (newblock.length != block.length ||
          (newblock.ptr && block.ptr && strncmp(newblock.ptr, block.ptr, block.length) != 0))
      {
        VerifyCommitNeeded(tw) = True;
        PreEndTW(tw) += (newblock.ptr ? _XmTextCountCharacters(newblock.ptr, newblock.length) :
                                        0) -
  ...
  if (tw->text.input->data->has_destination)
    _XmTextSetDestinationSelection(
        widget, tw->text.cursor_position, False, XtLastTimestampProcessed(XtDisplay(widget)));
...
Boolean XmTextRemove(Widget widget)
{
  if (XmIsTextField(widget))
    ...
    _XmTextResetIC(widget);
    if (!(*source->GetSelection)(source, &left, &right) || left == right) {
...
Boolean XmTextCopyLink(Widget widget, Time copy_time)
{
  Boolean result = False;
  XmTextPosition left, right;
  _XmWidgetToAppContext(widget);
...
Boolean XmTextCut(Widget widget, Time cut_time)
{
  ...
  if (XmTextGetEditable(widget) && XmTextGetSelectionPosition(widget, &left, &right) &&
      (right != left))
    /* start copy to clipboard */
...
char *XmTextGetSelection(Widget widget)
{
  if (XmIsTextField(widget))
    ...
    ret_val = _XmStringSourceGetString((XmTextWidget)widget, left, right, False);
    _XmAppUnlock(app);
    return ret_val;
  }
}
...
wchar_t *XmTextGetSelectionWcs(Widget widget)
{
  if (XmIsTextField(widget))
    ...
    ret_val = (wchar_t *)_XmStringSourceGetString((XmTextWidget)widget, left, right, True);
    _XmAppUnlock(app);
    return ret_val;
  }
}
...
void XmTextSetSelection(Widget widget, XmTextPosition first, XmTextPosition last, Time set_time)
{
  if (XmIsTextField(widget))
    ...
    _XmTextSetDestinationSelection(widget, tw->text.cursor_position, False, set_time);
    _XmAppUnlock(app);
  }
}
...
static int _XmTextGetSubstring(Widget widget,
                               ...
                               Boolean want_wchar)
{
  ...
  for (pos = start; pos < end;) {
    pos = (*tw->text.source->ReadSource)(tw->text.source, pos, end, &block);
    if (block.length == 0) {
      if (!want_wchar)
        buffer[destpos] = '\0';
      else
    ...
    if (!want_wchar) {
      (void)memcpy((void *)&buffer[destpos], (void *)block.ptr, block.length);
...
{
  if (XmIsTextField(widget))
    return (XmTextFieldGetSubstring(widget, start, num_chars, buf_size, buffer));
  else {
    int ret_val;
    _XmWidgetToAppContext(widget);
    _XmAppLock(app);
    ret_val = _XmTextGetSubstring(widget, start, num_chars, buf_size, buffer, False);
    _XmAppUnlock(app);
    return ret_val;
  }
}
...
{
  if (XmIsTextField(widget))
    return (XmTextFieldGetSubstringWcs(widget, start, num_chars, buf_size, buffer));
  else {
    int ret_val;
    _XmWidgetToAppContext(widget);
    _XmAppLock(app);
    ret_val = _XmTextGetSubstring(widget, start, num_chars, buf_size, (char *)buffer, True);
    _XmAppUnlock(app);
    return ret_val;
  }
}
