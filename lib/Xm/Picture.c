...
static XmPictureNode *_XmPictureCopySubGraph(XmPictureRec *,
                                             int,
                                             XmPictureNode *,
                                             XmPictureNode *);
...
XmPicture XmParsePicture(char *input)
{
  ...
  picture->nodes = (XmPictureNode **)XtMalloc(NODE_START_COUNT * sizeof(XmPictureNode *));
 ...
 * user with XtFree()
 */
XmPictureState XmGetNewPictureState(XmPicture picture)
{
  int i;
 ...
 */
char *XmPictureProcessCharacter(XmPictureState state, char in, Boolean *is_finished)
{
  int i;
  char status;
  unsigned char *temp;
...
void XmPictureDelete(XmPicture p)
{
  int i;
...
void XmPictureDeleteState(XmPictureState s)
{
...
char *XmPictureGetCurrentString(XmPictureState s)
{
  return s->current_string;
}

char *XmPictureDoAutoFill(XmPictureState state)
{
  XmAutoFill fill;
  int i;
  ...
  while ((inc = *((*in_string)++))) {
    switch (inc) {
      ...
      case NUMDIGIT:
        ...
        newtrans = _XiGetNewTransition(NumericDigit, current_node, newnode);
        current_node = newnode;
        break;
      case HEXDIGIT:
        newnode = _XiGetNewNode(picture);
        newtrans = _XiGetNewTransition(HexDigit, current_node, newnode);
        current_node = newnode;
        break;
      case OCTALDIGIT:
        newnode = _XiGetNewNode(picture);
        newtrans = _XiGetNewTransition(OctalDigit, current_node, newnode);
        current_node = newnode;
        break;
      case NONCASELETTER:
        newnode = _XiGetNewNode(picture);
        newtrans = _XiGetNewTransition(AnyLetter, current_node, newnode);
        current_node = newnode;
        break;
      case UPCASELETTER:
        newnode = _XiGetNewNode(picture);
        newtrans = _XiGetNewTransition(UpCaseLetter, current_node, newnode);
        current_node = newnode;
        break;
      case NONCASECHAR:
        newnode = _XiGetNewNode(picture);
        newtrans = _XiGetNewTransition(AnyCharacter, current_node, newnode);
        current_node = newnode;
        break;
      case UPCASECHAR:
        newnode = _XiGetNewNode(picture);
        newtrans = _XiGetNewTransition(UpCaseCharacter, current_node, newnode);
        current_node = newnode;
        break;
        /*
         * The weird stuff.
      ...
      case CLOSURE: /* CR03602 picture should formatted to *nx;
        ...
        if (count)
          *in_string = endcount;
        _XmPictureParseNode(picture, in_string, &start_node_2, &current_node_2, True);
        if (count) {
          newtrans = _XiGetNewTransition(NullTransition, current_node, start_node_2);
          current_node = _XmPictureCopySubGraph(picture, count - 1, start_node_2, current_node_2);
        }
        else {
          newtrans = _XiGetNewTransition(NullTransition, current_node_2, start_node_2);
          newtrans = _XiGetNewTransition(NullTransition, current_node, current_node_2);
      ...
      case ALTERNATIVE: /* CR03656 CR03359 a major overhaul */
        ...
        if (current_node->index)
          picture->nodes[current_node->index - 1]->transitions->next = start_node_2->transitions;
        current_node = current_node_2;
        break;
        /*
         ...
         */
      case LBRACKET:
        _XmPictureParseNode(picture, in_string, &start_node_2, &current_node_2, False);
        newtrans = _XiGetNewTransition(NullTransition, current_node, current_node_2);
        newtrans = _XiGetNewTransition(NullTransition, current_node, start_node_2);
        current_node = current_node_2;
        break;
...
static XmPictureTransition *_XiGetNewTransition(XmTransType type,
                                                XmPictureNode *src,
                                                XmPictureNode *dest)
{
 ...
 */
static XmPictureNode *_XiGetNewNode(XmPictureRec *picture)
{
...
static void _XmPictureSetState(unsigned char *v, int i)
{
  int base = i / 8;
  int offset = i % 8;
  v[base] |= 1 << offset;
...
*/
static char _XmPictureGetState(unsigned char *v, int i)
{
  int base = i / 8;
  int offset = i % 8;
 ...
 */
static void _XmPictureFollowTransitions(XmPictureStateRec *state, char inc, XmPictureNode *node)
{
  ...
  while (curr) {
    ...
    switch (curr->type) {
      ...
      case NumericDigit:
        ...
        break;
      case HexDigit:
        if (isdigit(inc) || (inc >= 'a' && inc <= 'f') || (inc >= 'A' && inc <= 'F')) {
          found = True;
        }
        break;
      case OctalDigit:
    ...
    if (found) {
      ...
      _XmPictureFollowTransitions(state, follow_c, state->picture->nodes[curr->destination]);
    }
    found = False;
    curr = curr->next;
  }
 ...
 * a non-null value.
 */
static void _XmPictureTagNodes(XmPictureRec *picture, XmPictureNode **table, int start)
{
 ...
 */
static XmPictureNode *_XmPictureCopySubGraph(XmPictureRec *picture,
                                             int n,
                                             XmPictureNode *start,
                                             XmPictureNode *end)
{
    ...
    for (j = 0; j < tablesize; j++) {
      if (table[j]) {
        ...
        while (trans) {
          ...
          newtrans = _XiGetNewTransition(trans->type, table[j], table[trans->destination]);
          newtrans->c = trans->c;
          trans = trans->next;
        }
      }
...
static void _XmPictureFillTraverse(XmPictureRec *picture, int start, XmAutoFill *fill)
{
  XmPictureTransition *trans = picture->nodes[start]->transitions;
  while (trans) {
    switch (trans->type) {
...
#ifdef DEBUG
static void _XiPrintStateVector(XmPictureStateRec *state)
{
  int i;
  char c;
  for (i = 0; i < state->picture->num_nodes; i++) {
