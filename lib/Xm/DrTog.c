...
/*ARGSUSED*/
void
XmeDrawDiamond(Display *display, Drawable d, GC top_gc, GC bottom_gc, GC center_gc, Position x, Position y, Dimension width, Dimension height, /* unused */
               ...
               Dimension margin)
{
   ...
   { /* NORMAL SIZED ToggleButtonS : initial width >= 5 */
      midX = x + (width + 1) / 2;
      midY = y + (width + 1) / 2;
              ...
              Dimension margin)
{
   ...
   if (shadow_thick > 0)
   {
      ...
      if (center_gc != NULL)
      {
         ...
         XFillArc(display, d, center_gc, x + delta, y + delta, MAX(width - 2 * delta, 1), MAX(height - 2 * delta, 1), 0, 360 * 64);
      }
      XDrawArc(display, d, top_gc, x + line_width / 2, y + line_width / 2, MAX(width - line_width, 1), MAX(height - line_width, 1), 45 * 64, 180 * 64);
      XDrawArc(display, d, bottom_gc, x + line_width / 2, y + line_width / 2, MAX(width - line_width, 1), MAX(height - line_width, 1), 45 * 64, -180 * 64);
