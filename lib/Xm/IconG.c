...
#define PIXMAP_VALID(pix) (pix != XmUNSPECIFIED_PIXMAP && pix != None)
#define SHOW_DETAIL(wid, container_data) \
  (IG_Detail(wid) && IG_DetailCount(wid) && (container_data)->detail_order_count)
#define INVALID_DIMENSION ((Dimension) - 1)
/********    Static Function Declarations    ********/
/* XmRCallProcs */
static void CheckSetRenderTable(Widget wid, int offset, XrmValue *value);
/* Converters */
static Boolean CvtStringToIconPixmap(Display *dpy,
                                     XrmValue *args,
                                     Cardinal *numArgs,
                                     XrmValue *fromVal,
                                     XrmValue *toVal,
...
static Boolean GetBaselines(Widget wid, Dimension **baselines, int *line_count);
static void MarginsProc(Widget w, XmBaselineMargins *margins_rec);
static Boolean GetDisplayRect(Widget wid, XRectangle *displayrect);
/* XmIconGadget methods */
/* Action procs */
/* Internal functions */
static void GetLabelXY(Widget wid, Position *x_ret, Position *y_ret);
static Position GetLargeIconX(Widget wid);
static Position GetSmallIconY(Widget wid);
static Dimension GetIconLabelWidth(Widget wid);
static Dimension GetIconLabelHeight(Widget wid);
static Cardinal GetShapeInfo(Widget wid,
                             ...
                             Dimension ht,
                             XPoint *points);
static void GetContainerData(Widget wid, XmContainerData container_data);
static void ChangeHighlightGC(Widget wid, XtEnum selection_mode, int line_width);
static void UpdateSelectGCs(Widget wid, Pixel select_color);
static void UpdateGCs(Widget wid);
static XmStringTable GetStringTableReOrdered(XmStringTable st,
                                             Cardinal st_count,
                                             Cardinal *order,
                                             Cardinal order_count);
static void GetStringTableExtent(Screen *screen,
                                 XmStringTable st,
                                 Cardinal st_count,
...
static void GetSize(Widget wid, Dimension *ideal_width, Dimension *ideal_height);
/* Trait methods */
...
static Boolean PointIn(Widget widget, Position x, Position y);
...
static XtResource cache_resources[] = {
    ...
    {XmNhighlightColor,
     ...
     XmRCallProc,
     (XtPointer)_XmHighlightColorDefault},
    {XmNbackgroundPixmap,
     XmCBackgroundPixmap,
     XmRPixmap,
     sizeof(Pixmap),
    ...
    {XmNhighlightPixmap,
     ...
     XmRCallProc,
     (XtPointer)_XmHighlightPixmapDefault},
    {XmNalignment,
     XmCAlignment,
     XmRAlignment,
     sizeof(unsigned char),
...
externaldef(xmicongcacheobjclassrec) XmIconGCacheObjClassRec xmIconGCacheObjClassRec = {
    {
...
static XmConst XmAccessColorsTraitRec iconACT = {0, /* version */
                                                 GetColors};
...
static XtConvertArgRec largeIconArgs[] = {{XtBaseOffset, (XtPointer)0, sizeof(int)},
                                          {XtAddress, (XtPointer)(int)XmLARGE_ICON, 0}};
static XtConvertArgRec smallIconArgs[] = {{XtBaseOffset, (XtPointer)0, sizeof(int)},
 ...
 *
 ************************************************************************/
static void FetchPixmap(Widget widget, String image_name, unsigned char res_type, Pixmap *pixmap)
{
  int depth;
  XtPointer mask_addr;
  ...
  /* always called when the cache is valid and the colors
      can be returned */
  access_colors_trait = (XmAccessColorsTrait)XmeTraitGet((XtPointer)XtClass(widget),
                                                         XmQTaccessColors);
  access_colors_trait->getColors(widget, &acc_color_rec);
  *pixmap = _XmGetScaledPixmap(
      XtScreen(widget), widget, image_name, &acc_color_rec, depth, FALSE, 0, 0, 0);
 ...
 * GetLabelString
 ************************************************************************/
static void GetLabelString(Widget wid,
                           int offset, /* unused */
                           XtArgVal *value)
{
...
static void InitializePosthook(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
   ...
   */
  _XmProcessLock();
  IG_Cache(sw) = (XmIconGCacheObjPart *)_XmCachePart(
      IG_ClassCachePart(sw), (XtPointer)IG_Cache(sw), sizeof(XmIconGCacheObjPart));
  /*
    ...
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  /* assign if changed! */
  _XmProcessLock();
  if (!IconGCacheCompare((XtPointer)IG_Cache(new_w), (XtPointer)IG_Cache(current))) {
    /* delete the old one */
    _XmCacheDelete((XtPointer)IG_Cache(current));
    IG_Cache(new_w) = (XmIconGCacheObjPart *)_XmCachePart(
        IG_ClassCachePart(new_w), (XtPointer)IG_Cache(new_w), sizeof(XmIconGCacheObjPart));
  }
  else
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  /* XmNvisualEmphasis */
  if (!XmRepTypeValidValue(XmRID_VISUAL_EMPHASIS, IG_VisualEmphasis(nw), nw))
  ...
  if (!IG_LabelString(nw)) {
    IG_LabelString(nw) = XmeGetLocalizedString(
        (char *)NULL, nw, XmNlabelString, XrmQuarkToString(new_ig->object.xrm_name));
  }
  else
  ...
  if (!XmStringEmpty(IG_LabelString(nw)))
    XmStringExtent(IG_RenderTable(nw),
                   IG_LabelString(nw),
  ...
  if (IG_LargeIconPixmap(nw) == XmDELAYED_PIXMAP) {
    ...
    if (IG_LargeIconPixmap(nw) == XmUNSPECIFIED_PIXMAP) {
      XtDisplayStringConversionWarning(
          XtDisplay(nw), IG_LargePixmapName(nw), "Large/SmallIconPixmap");
    }
  }
  if (IG_SmallIconPixmap(nw) == XmDELAYED_PIXMAP) {
    ...
    if (IG_SmallIconPixmap(nw) == XmUNSPECIFIED_PIXMAP) {
      XtDisplayStringConversionWarning(
          XtDisplay(nw), IG_SmallPixmapName(nw), "Large/SmallIconPixmap");
    }
  }
  /* get the large icon size */
  if (PIXMAP_VALID(IG_LargeIconPixmap(nw)))
    XmeGetPixmapData(
        XtScreen(nw), IG_LargeIconPixmap(nw), NULL, NULL, NULL, NULL, NULL, NULL, &w, &h);
  else {
    w = h = 0;
  }
  ...
  if (PIXMAP_VALID(IG_SmallIconPixmap(nw)))
    XmeGetPixmapData(
        XtScreen(nw), IG_SmallIconPixmap(nw), NULL, NULL, NULL, NULL, NULL, NULL, &w, &h);
  else {
    w = h = 0;
  }
  ...
  UpdateGCs(nw);
  new_ig->gadget.event_mask = XmHELP_EVENT | XmFOCUS_IN_EVENT | XmFOCUS_OUT_EVENT | XmENTER_EVENT |
                              XmLEAVE_EVENT;
}
...
{
  Cardinal i;
  if (IG_RenderTable(wid) != NULL)
    XmRenderTableFree(IG_RenderTable(wid));
  if (IG_LabelString(wid) != NULL)
    XmStringFree(IG_LabelString(wid));
...
#define HAS_PIXMAP(wid) \
  (((IG_ViewType(wid) == XmLARGE_ICON) && (PIXMAP_VALID(IG_LargeIconPixmap(wid)))) || \
   ((IG_ViewType(wid) == XmSMALL_ICON) && (PIXMAP_VALID(IG_SmallIconPixmap(wid)))))
#define SPACING(wid) (HAS_PIXMAP(wid) ? IG_Spacing(wid) : 0)
#define HAS_MASK(wid) \
  (((IG_ViewType(wid) == XmSMALL_ICON) && (PIXMAP_VALID(IG_SmallIconMask(wid)))) || \
   ((IG_ViewType(wid) == XmLARGE_ICON) && (PIXMAP_VALID(IG_LargeIconMask(wid)))))
  ...
  if (LayoutIsRtoLG(wid)) {
    label_x = XtWidth(wid) - label_x - IG_LabelRectWidth(wid) - IG_HLThickness(wid);
  }
  else {
  ...
  if (x_ret)
    *x_ret = label_x;
  if (y_ret)
    *y_ret = label_y + IG_HLThickness(wid) + IG_MarginHeight(wid);
}
...
#define GetLargeIconY(wid) \
  (IG_HLThickness(wid) + IG_MarginHeight(wid) + (HAS_MASK(wid) ? 0 : IG_ShadowThickness(wid)))
/************************************************************************
 * GetSmallIconX
 ************************************************************************/
/* the shadow thickness is only incorporated when there is no mask */
#define GetSmallIconX(wid) \
  (IG_HLThickness(wid) + IG_MarginWidth(wid) + (HAS_MASK(wid) ? 0 : IG_ShadowThickness(wid)))
...
static Dimension GetIconLabelWidth(Widget wid)
{
  ...
  if (IG_ViewType(wid) == XmLARGE_ICON) {
    ...
    width += IG_SmallIconRectWidth(wid) + IG_LabelRectWidth(wid) + 2 * IG_ShadowThickness(wid) +
             SPACING(wid);
  }
  return width;
}
...
static Dimension GetIconLabelHeight(Widget wid)
{
  ...
  if (IG_ViewType(wid) == XmLARGE_ICON) {
    height += IG_LargeIconRectHeight(wid) + IG_LabelRectHeight(wid) + 2 * IG_ShadowThickness(wid) +
              SPACING(wid);
  }
  else {
    if (!HAS_MASK(wid))
      height += MAX(IG_SmallIconRectHeight(wid), IG_LabelRectHeight(wid)) +
                2 * IG_ShadowThickness(wid);
    else
                             ...
                             XPoint *points)
{
  ...
  if (n == 8 && IG_ViewType(wid) == XmLARGE_ICON) {
    ...
    if ((!HAS_MASK(wid) && (IG_LargeIconRectWidth(wid) > IG_LabelRectWidth(wid))) ||
        (HAS_MASK(wid) && (IG_LargeIconRectWidth(wid) > IG_LabelRectWidth(wid) + 2 * ist)))
    {
    ...
    if (!highlight_case && HAS_MASK(wid) && ist)
      points[5].x += 1;
    points[5].y = points[0].y;
    points[6].x = points[5].x;
    points[6].y = IG_LargeIconRectHeight(wid) + IG_LabelRectHeight(wid) + 2 * ist + 2 * rht - ht +
                  2 * rmh - mh + SPACING(wid) - 1;
    if (!highlight_case && HAS_MASK(wid) && ist)
      points[6].y += 1;
    ...
    if ((!HAS_MASK(wid) && (IG_SmallIconRectHeight(wid) > IG_LabelRectHeight(wid))) ||
        (HAS_MASK(wid) && (IG_SmallIconRectHeight(wid) > IG_LabelRectHeight(wid) + 2 * ist)))
    {
    ...
    points[3].x = IG_SmallIconRectWidth(wid) + SPACING(wid) + IG_LabelRectWidth(wid) + 2 * ist +
                  2 * rht - ht + 2 * rmw - mw - 1;
    if (highlight_case && HAS_MASK(wid))
      points[3].x -= 1;
    points[3].y = points[2].y;
...
static void GetContainerData(Widget wid, XmContainerData container_data)
{
  ...
  container_data->first_column_width = IG_HLThickness(wid) + GetIconLabelWidth(wid) -
                                       IG_MarginWidth(wid);
  ...
  container_id = (igc->icong_class.get_container_parent) ?
                     (*(igc->icong_class.get_container_parent))(wid) :
                     XtParent(wid);
  /* then get the trait info */
  container_trait = (XmContainerTrait)XmeTraitGet((XtPointer)XtClass(container_id), XmQTcontainer);
  if (container_trait)
    ...
    if (LayoutIsRtoLG(wid)) {
      if (XtWidth(XtParent(wid))) {
        if ((Position)container_data->first_column_width >
            (XtWidth(XtParent(wid)) - ig->rectangle.width - ig->rectangle.x))
        {
          container_data->first_column_width -= (XtWidth(XtParent(wid)) - ig->rectangle.width -
                                                 ig->rectangle.x);
        }
        else {
      ...
      /* here we have to worry of special cases where
               ...
               are extreme */
      if (((Position)container_data->first_column_width > ig->rectangle.x) && ig->rectangle.x >= 0)
      {
        container_data->first_column_width -= ig->rectangle.x;
                      ...
                      Region region) /* unused */
{
  ...
  Position large_icon_x = 0, small_icon_y = 0, small_icon_x, label_x = 0, label_y = 0;
  ...
  if ((IG_ViewType(wid) == XmLARGE_ICON) && (PIXMAP_VALID(IG_LargeIconPixmap(wid)))) {
    ...
    XmeGetPixmapData(
        XtScreen(wid), IG_LargeIconPixmap(wid), NULL, &depth, NULL, NULL, NULL, NULL, NULL, NULL);
    pm_width = IG_LargeIconRectWidth(wid);
    if (large_icon_x + pm_width > MIN(ig->rectangle.width, container_data.first_column_width)) {
      pm_width = MIN(ig->rectangle.width - 2 * ht, container_data.first_column_width - ht);
      pm_width -= MIN(pm_width, (Dimension)large_icon_x);
      if (LayoutIsRtoLG(wid))
        large_icon_x = ig->rectangle.width -
                       MIN(ig->rectangle.width - 2 * ht, container_data.first_column_width - ht);
    }
    else {
      if (LayoutIsRtoLG(wid))
  ...
  if ((IG_ViewType(wid) == XmSMALL_ICON) && (PIXMAP_VALID(IG_SmallIconPixmap(wid)))) {
    ...
    XmeGetPixmapData(
        XtScreen(wid), IG_SmallIconPixmap(wid), NULL, &depth, NULL, NULL, NULL, NULL, NULL, NULL);
    pm_width = IG_SmallIconRectWidth(wid);
    small_icon_x = GetSmallIconX(wid);
    if (small_icon_x + pm_width >
        MIN(ig->rectangle.width - 2 * ht, container_data.first_column_width - 2 * ht))
    {
      pm_width = MIN(ig->rectangle.width, container_data.first_column_width) - 2 * ht;
  ...
  if (container_data.first_column_width) {
    ...
    if (n == 2) {
      ...
      XmeDrawPolygonShadow(XtDisplay(wid),
                           XtWindow(wid),
                           IG_TopShadowGC(wid),
                           IG_BottomShadowGC(wid),
                           shad_points,
  ...
  if (SHOW_DETAIL(wid, &container_data) && container_data.first_column_width >= ht + mw) {
    ...
    new_detail = GetStringTableReOrdered(IG_Detail(wid),
                                         IG_DetailCount(wid),
      ...
      if (container_data.detail_tablist)
        next_tab_x = container_data.first_column_width +
                     _XmTabListGetPosition(
                         XtScreen(wid), container_data.detail_tablist, XmPIXELS, i);
      if (new_detail[i]) {
        ...
        if (i < tab_count) {
          ...
          if (LayoutIsRtoLG(wid))
            clip_rect.x = ig->rectangle.x + ig->rectangle.width - detail_x - clip_rect.width;
          clip_rect.y = ig->rectangle.y + ht + mh;
          clip_rect.height = ig->rectangle.height - 2 * ht - 2 * mh;
          XSetClipRectangles(XtDisplay(wid), gc, 0, 0, &clip_rect, 1, Unsorted);
        }
        ...
        detail_baseline = XmStringBaseline(IG_RenderTable(wid), new_detail[i]);
        ...
        XmStringDraw(XtDisplay(wid),
                     ...
                     new_detail[i],
                     gc,
                     (LayoutIsRtoLG(wid)) ? (ig->rectangle.x + ig->rectangle.width - detail_x -
  ...
  if (ig->gadget.highlighted) {
    if (((XmGadgetClass)XtClass(wid))->gadget_class.border_highlight) {
      (*(((XmGadgetClass)XtClass(wid))->gadget_class.border_highlight))(wid);
    }
  }
}
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if ((IG_Background(nw) != IG_Background(cw)) || (IG_Foreground(nw) != IG_Foreground(cw)) ||
  ...
  if ((Relayout) || (IG_Spacing(nw) != IG_Spacing(cw)) ||
      (IG_MarginWidth(nw) != IG_MarginWidth(cw)) || (IG_MarginHeight(nw) != IG_MarginHeight(cw)) ||
...
static void HighlightBorder(Widget w)
{
  ...
  if (ig->rectangle.width == 0 || ig->rectangle.height == 0 || ig->gadget.highlight_thickness == 0)
    return;
  ...
  if (SHOW_DETAIL(w, &container_data)) {
                     ...
                     points) == 2)
    {
      ...
      XmeDrawPolygonShadow(XtDisplay(w),
                           XtWindow(w),
                           IG_HighlightGC(w),
                           IG_HighlightGC(w),
                           points,
...
static void UnhighlightBorder(Widget w)
{
  ...
  if (XmIsManager(XtParent(w))) {
    background_gc = ((XmManagerWidget)XtParent(w))->manager.background_GC;
  }
  else {
  ...
  if (SHOW_DETAIL(w, &container_data)) {
                     ...
                     points) == 2)
    {
      ...
      XmeDrawPolygonShadow(
          XtDisplay(w), XtWindow(w), background_gc, background_gc, points, 8, ht, XmSHADOW_OUT);
    }
  }
}
...
static int IconGCacheCompare(XtPointer A, XtPointer B)
{
  ...
  if ((icon_inst->render_table == icon_cache_inst->render_table) &&
      ...
      (icon_inst->bottom_shadow_pixmap == icon_cache_inst->bottom_shadow_pixmap) &&
 ...
 */
static Cardinal GetIconGClassSecResData(WidgetClass w_class, /* unused */
                                        XmSecondaryResourceData **data_rtn)
{
  ...
  arrayCount = _XmSecondaryResourceData(
      bcePtr, data_rtn, client_data, resource_name, resource_class, GetIconGClassSecResBase);
  return (arrayCount);
}
 ...
 */
static XtPointer GetIconGClassSecResBase(Widget widget, XtPointer client_data) /* unused */
{
...
static Boolean GetBaselines(Widget wid, Dimension **baselines, int *line_count)
{
  ...
  *line_count = 1;
  ...
  if (IG_LabelString(wid) == NULL) {
    ...
    base_array[0] = IG_HLThickness(wid) + label_y + DEFAULT_LABEL_MARGIN_HEIGHT +
...
static void ChangeHighlightGC(Widget wid, XtEnum selection_mode, int line_width)
{
  ...
  values.line_style = (selection_mode == XmADD_MODE) ? LineDoubleDash : LineSolid;
...
static void UpdateSelectGCs(Widget wid, Pixel select_color)
{
  ...
  if (select_color != XmREVERSED_GROUND_COLORS) {
    ...
    XtVaGetValues(XtParent(wid), XmNbackground, &(values.foreground), NULL);
    IG_InverseGC(wid) = XtAllocateGC(
        XtParent(wid), XtParent(wid)->core.depth, valueMask, &values, modifyMask, 0);
    /* get the foreground for the inversed selected background */
    values.background = IG_Background(wid);
    XtVaGetValues(XtParent(wid), XmNforeground, &(values.foreground), NULL);
  }
  IG_SelectedGC(wid) = XtAllocateGC(
      XtParent(wid), XtParent(wid)->core.depth, valueMask, &values, modifyMask, 0);
}
...
static void UpdateGCs(Widget wid)
{
  ...
  if (PIXMAP_VALID(IG_BackgroundPixmap(wid))) {
    int depth;
    XmeGetPixmapData(
        XtScreen(wid), IG_BackgroundPixmap(wid), NULL, &depth, NULL, NULL, NULL, NULL, NULL, NULL);
    if (depth == 1) {
  ...
  IG_BackgroundGC(wid) = XtAllocateGC(
      XtParent(wid), XtParent(wid)->core.depth, valueMask, &values, modifyMask, 0);
  /** selected gcs **/
  ...
  IG_HighlightGC(wid) = XtAllocateGC(
      XtParent(wid), XtParent(wid)->core.depth, valueMask, &values, modifyMask, 0);
  /** topshadow, bottomshadow gc */
  IG_TopShadowGC(wid) = _XmGetPixmapBasedGC(
      wid, IG_TopShadowColor(wid), IG_Background(wid), IG_TopShadowPixmap(wid));
  IG_BottomShadowGC(wid) = _XmGetPixmapBasedGC(
      wid, IG_BottomShadowColor(wid), IG_Background(wid), IG_BottomShadowPixmap(wid));
}
                                 ...
                                 Position *baseline_ret)
{
  ...
  *height_ret = 0;
  if (tab_list)
    tab_count = XmTabListTabCount(tab_list);
  if (st == NULL || !st_count)
    return;
  /* the extra tabs are ignored */
  ...
  if (tab_count)
    *width_ret = _XmTabListGetPosition(screen, tab_list, XmPIXELS, tab_count - 1);
 ...
 * GetSize
 ************************************************************************/
static void GetSize(Widget wid, Dimension *ret_width, Dimension *ret_height)
{
  XmContainerDataRec container_data;
  ...
  if (SHOW_DETAIL(wid, &container_data)) {
    ...
    GetStringTableExtent(XtScreen(wid),
                         new_detail,
                         MIN(container_data.detail_order_count, IG_DetailCount(wid)),
                         IG_RenderTable(wid),
    ...
    ideal_height = MAX(ideal_height,
                       label_y - ht + DEFAULT_LABEL_MARGIN_HEIGHT + lab_baseline - mh -
                           detail_baseline + detail_height);
  }
  /* do not set non null dimensions */
  if (*ret_width == 0)
    *ret_width = ideal_width + 2 * ht;
  if (*ret_height == 0)
    *ret_height = ideal_height + 2 * ht;
}
                            ...
                            Mask visual_flag)
{
  ...
  if (visual_flag & VisualSelectColor) {
    ...
    _XmProcessLock();
    IG_Cache(ig) = (XmIconGCacheObjPart *)_XmCachePart(
        IG_ClassCachePart(ig), (XtPointer)IG_Cache(ig), sizeof(XmIconGCacheObjPart));
    _XmProcessUnlock();
  }
  return redraw;
}
...
static void GetColors(Widget w, XmAccessColorData color_data)
{
  ...
  if (IG_Cache(w)) { /* mean it's valid */
    color_data->valueMask = AccessForeground | AccessBackgroundPixel | AccessHighlightColor |
                            AccessTopShadowColor | AccessBottomShadowColor | AccessSelectColor;
    ...
    /*** get the Container select color using the trait */
    {
      ...
      XmContainerTrait container_trait;
      container_id = (igc->icong_class.get_container_parent) ?
                         (*(igc->icong_class.get_container_parent))(w) :
                         XtParent(w);
      container_trait = (XmContainerTrait)XmeTraitGet((XtPointer)XtClass(container_id),
                                                      XmQTcontainer);
...
static Boolean PointIn(Widget wid, Position x, Position y)
{
  XmContainerDataRec container_data;
  /* x, y in parent coordinates system */
  /* first check that point is in bbox */
  if (!(x >= wid->core.x && y >= wid->core.y && x < wid->core.x + wid->core.width &&
 ...
 *
 ************************************************************************/
Widget XmCreateIconGadget(Widget parent, char *name, ArgList arglist, Cardinal argcount)
{
...
Widget XmVaCreateIconGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmIconGadgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedIconGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmIconGadgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
