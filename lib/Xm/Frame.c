...
#define GetFrameConstraint(w) (&((XmFrameConstraintPtr)(w)->core.constraints)->frame)
    /********    Static Function Declarations    ********/
    static void
    CheckSetChildType(Widget wid, int offset, XrmValue *value);
static void DrawShadow(XmFrameWidget fw);
static void ClearShadow(XmFrameWidget fw);
static void ConfigureChildren(XmFrameWidget fw, Widget instigator, XtWidgetGeometry *inst_desired);
...
static void CalcFrameSize(XmFrameWidget fw,
                          ...
                          Dimension *fwWidth,
                          Dimension *fwHeight);
static void CalcWorkAreaSize(XmFrameWidget fw,
                             Dimension *workWidth,
                             Dimension *workHeight,
                             Dimension workBorder,
                             Dimension fwWidth,
                             Dimension fwHeight);
static void CalcTitleExtent(XmFrameWidget fw,
                            Dimension titleHeight,
                            Dimension titleBorder,
...
static XtResource resources[] = {{XmNmarginWidth,
                                  XmCMarginWidth,
 ...
 *
 ************************************************************************/
static void CheckSetChildType(Widget wid, int offset, XrmValue *value)
{
 ...
 *
 ************************************************************************/
static void ConfigureChildren(XmFrameWidget fw, Widget instigator, XtWidgetGeometry *inst_geometry)
{
  Widget child;
  ...
  if (fw->frame.title_area && XtIsManaged(fw->frame.title_area)) {
    ...
    if (child == instigator) {
      ...
      childBW = (inst_geometry->request_mode & CWBorderWidth) ? inst_geometry->border_width :
                                                                childBW;
    }
...
static void Redisplay(Widget wid, XEvent *event, Region region)
{
  ...
  if (title_area && XmIsGadget(title_area) && XtIsManaged(title_area)) {
    XClearArea(XtDisplay(fw),
               XtWindow(fw),
               title_area->core.x,
               title_area->core.y,
               title_area->core.width,
               title_area->core.height,
               False);
    if (region && !XRectInRegion(region,
                                 title_area->core.x,
                                 title_area->core.y,
                                 title_area->core.width,
                                 title_area->core.height))
                                        ...
                                        XtWidgetGeometry *reply)
{
  ...
  switch (XtMakeGeometryRequest((Widget)fw, &parent_request, &parent_reply)) {
    case XtGeometryYes:
      if (!almost) {
        if (!query_only) {
          ClearShadow(fw);
    ...
    case XtGeometryAlmost:
      if (w == title_area) {
        ...
        CalcWorkAreaSize(
            fw, &almost_width, &almost_height, req_bw, parent_reply.width, parent_reply.height);
      }
      break;
    default:
      return (XtGeometryNo);
...
{
  XmFrameWidget fw = (XmFrameWidget)wid;
  Widget title_area = (fw->frame.title_area && XtIsManaged(fw->frame.title_area)) ?
                          fw->frame.title_area :
                          (Widget)NULL;
  Dimension t_w = 0;
  Dimension t_h = 0;
  Dimension t_bw = (title_area) ? title_area->core.border_width : 0;
  Widget work_area = (fw->frame.work_area && XtIsManaged(fw->frame.work_area)) ?
                         fw->frame.work_area :
                         (Widget)NULL;
  ...
  if (XtIsRealized((Widget)fw) || (XtWidth(fw) == 0) || (XtHeight(fw) == 0)) {
    ...
    while (XtMakeResizeRequest((Widget)fw, fwWidth, fwHeight, &fwWidth, &fwHeight) ==
           XtGeometryAlmost)
      /*EMPTY*/;
    ClearShadow(fw);
  }
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  Widget title_area = (new_w->frame.title_area && XtIsManaged(new_w->frame.title_area)) ?
                          new_w->frame.title_area :
                          NULL;
  Dimension t_w = (title_area) ? title_area->core.width : 0;
  Dimension t_h = (title_area) ? title_area->core.height : 0;
  Dimension t_bw = (title_area) ? title_area->core.border_width : 0;
  Widget work_area = (new_w->frame.work_area && XtIsManaged(new_w->frame.work_area)) ?
                         new_w->frame.work_area :
                         (Widget)NULL;
  Dimension w_w = (work_area) ? work_area->core.width : 0;
  Dimension w_h = (work_area) ? work_area->core.height : 0;
  Dimension w_bw = (work_area) ? work_area->core.border_width : 0;
                          ...
                          Dimension *fwHeight)
{
  ...
  *fwWidth = MAX(workMax, titleMax);
  if (*fwWidth == 0)
    *fwWidth = 1;
  *fwHeight = workHeight + 2 * (workBorder + fw->frame.margin_height) + shadowThickness +
              titleExtent;
  if (*fwHeight == 0)
    *fwHeight = 1;
}
                             ...
                             Dimension fwHeight)
{
  ...
  if (title && XtIsManaged(title))
    CalcTitleExtent(
        fw, title->core.height, title->core.border_width, &titleExtent, NULL, NULL, NULL);
  ...
  temp = (int)fwHeight -
         (int)(2 * (workBorder + fw->frame.margin_height) + shadowThickness + titleExtent);
  if (temp <= 0)
    *workHeight = 1;
  else
                            ...
                            Position *shadowY)
{
  ...
  switch (fc->child_v_alignment) {
    ...
    case (XmALIGNMENT_BASELINE_BOTTOM):
      ...
      ty = (base + titleBorder > shadowThickness / 2) ? 0 :
                                                        shadowThickness / 2 - (base + titleBorder);
      extent = ty + MAX(total, titleBorder + base + shadowThickness / 2);
      sy = (base + titleBorder > shadowThickness / 2) ? base + titleBorder - shadowThickness / 2 :
                                                        0;
                                 ...
                                 Cardinal *num_args) /* unused */
{
  ...
  if (!XmRepTypeValidValue(XmRID_CHILD_HORIZONTAL_ALIGNMENT, nc->child_h_alignment, nw)) {
    nc->child_h_alignment = XmALIGNMENT_BEGINNING;
  }
  if (!XmRepTypeValidValue(XmRID_CHILD_VERTICAL_ALIGNMENT, nc->child_v_alignment, nw)) {
 ...
 *
 ************************************************************************/
static Boolean ConstraintSetValues(Widget cw,
                                   Widget rw, /* unused */
                                   Widget nw,
                                   ArgList args,       /* unused */
                                   Cardinal *num_args) /* unused */
{
  ...
  if (nc->child_h_alignment != cc->child_h_alignment &&
      !XmRepTypeValidValue(XmRID_CHILD_HORIZONTAL_ALIGNMENT, nc->child_h_alignment, nw))
  {
    nc->child_h_alignment = cc->child_h_alignment;
  }
  if (nc->child_v_alignment != cc->child_v_alignment &&
      !XmRepTypeValidValue(XmRID_CHILD_VERTICAL_ALIGNMENT, nc->child_v_alignment, nw))
  {
    nc->child_v_alignment = cc->child_v_alignment;
  }
  if (nc->child_type == XmFRAME_TITLE_CHILD && (nc->child_h_alignment != cc->child_h_alignment ||
...
Widget XmVaCreateFrame(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmFrameWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedFrame(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmFrameWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
