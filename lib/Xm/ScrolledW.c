...
static XtResource resources[] = {
   { XmNhorizontalScrollBar,        XmCHorizontalScrollBar,        XmRWidget,                 sizeof(Widget),         XtOffsetOf(XmScrolledWindowRec, swindow.hScrollBar),               XmRImmediate, NULL                                 },
   { XmNverticalScrollBar,          XmCVerticalScrollBar,          XmRWidget,                 sizeof(Widget),         XtOffsetOf(XmScrolledWindowRec, swindow.vScrollBar),               XmRImmediate, NULL                                 },
   { XmNworkWindow,                 XmCWorkWindow,                 XmRWidget,                 sizeof(Widget),         XtOffsetOf(XmScrolledWindowRec, swindow.WorkWindow),               XmRImmediate, NULL                                 },
   { XmNclipWindow,                 XmCClipWindow,                 XmRWidget,                 sizeof(Widget),         XtOffsetOf(XmScrolledWindowRec, swindow.ClipWindow),               XmRImmediate, NULL                                 },
   { XmNscrollingPolicy,            XmCScrollingPolicy,            XmRScrollingPolicy,        sizeof(unsigned char),  XtOffsetOf(XmScrolledWindowRec, swindow.ScrollPolicy),             XmRImmediate, (XtPointer)XmAPPLICATION_DEFINED     },
   { /* Obsolete resource */
     XmNvisualPolicy,
    XmCVisualPolicy,
    XmRVisualPolicy,
    ...
    XmRCallProc,
    (XtPointer)VisualPolicyDefault                                                                                                                                                                                                          },
   { XmNscrollBarDisplayPolicy,     XmCScrollBarDisplayPolicy,     XmRScrollBarDisplayPolicy, sizeof(char),           XtOffsetOf(XmScrolledWindowRec, swindow.ScrollBarPolicy),          XmRImmediate, (XtPointer)RESOURCE_DEFAULT          },
   { XmNscrollBarPlacement,         XmCScrollBarPlacement,         XmRScrollBarPlacement,     sizeof(unsigned char),  XtOffsetOf(XmScrolledWindowRec, swindow.Placement),                XmRCallProc,  (XtPointer)ScrollBarPlacementDefault },
   { XmNscrolledWindowMarginWidth,  XmCScrolledWindowMarginWidth,  XmRHorizontalDimension,    sizeof(Dimension),      XtOffsetOf(XmScrolledWindowRec, swindow.WidthPad),                 XmRImmediate, (XtPointer)0                         },
   { XmNscrolledWindowMarginHeight, XmCScrolledWindowMarginHeight, XmRVerticalDimension,      sizeof(Dimension),      XtOffsetOf(XmScrolledWindowRec, swindow.HeightPad),                XmRImmediate, (XtPointer)0                         },
   { XmNspacing,                    XmCSpacing,                    XmRHorizontalDimension,    sizeof(Dimension),      XtOffsetOf(XmScrolledWindowRec, swindow.pad),                      XmRImmediate, (XtPointer)RESOURCE_DEFAULT          },
   { XmNshadowThickness,            XmCShadowThickness,            XmRHorizontalDimension,    sizeof(Dimension),      XtOffsetOf(XmScrolledWindowRec, manager.shadow_thickness),         XmRImmediate, (XtPointer)RESOURCE_DEFAULT          },
   { XmNtraverseObscuredCallback,   XmCCallback,                   XmRCallback,               sizeof(XtCallbackList), XtOffsetOf(XmScrolledWindowRec, swindow.traverseObscuredCallback), XmRImmediate, NULL                                 },
   { XmNautoDragModel,              XmCAutoDragModel,              XmRAutoDragModel,          sizeof(XtEnum),         XtOffsetOf(XmScrolledWindowRec, swindow.auto_drag_model),          XmRImmediate, (XtPointer)XmAUTO_DRAG_ENABLED       }
};
static XtResource constraints[] = {
   { XmNscrolledWindowChildType, XmCScrolledWindowChildType, XmRScrolledWindowChildType, sizeof(unsigned char), XtOffsetOf(XmScrolledWindowConstraintRec, swindow.child_type), XmRImmediate, (XtPointer)RESOURCE_DEFAULT },
};
/****************
 *
   ...
   if (nav_data.dimMask & NavigDimensionX)
   {
      ...
      for (i = 0; i < clip->composite.num_children; i++)
      {
         ...
         if (XtIsManaged(child) && !child->core.being_destroyed)
         {
            ...
            if ((swc->child_type == XmWORK_AREA) || (swc->child_type == XmSCROLL_HOR))
            {
      ...
      for (i = 0; i < clip->composite.num_children; i++)
      {
         ...
         if (XtIsManaged(child) && !child->core.being_destroyed)
         {
            ...
            /* only regular work_area and scroll_vert kids
                   can move in this direction */
            if ((swc->child_type == XmWORK_AREA) || (swc->child_type == XmSCROLL_VERT))
            {
               XmeConfigureObject(child, child->core.x, swc->orig_y - nav_data.value.y, child->core.width, child->core.height, child->core.border_width);
            }
         }
      }
   }
   ...
   Cardinal *num_params)
{
   ...
   MoveWindow(sw, sw->swindow.hmin, sw->swindow.hScrollBar->scrollBar.orientation);
}
   ...
   Cardinal *num_params)
{
   XmScrolledWindowWidget sw = (XmScrolledWindowWidget)wid;
   if (!sw->swindow.hScrollBar) return;
   MoveWindow(sw, sw->swindow.hmax - sw->swindow.hExtent, sw->swindow.hScrollBar->scrollBar.orientation);
}
   ...
   Cardinal *num_params)
{
   ...
   MoveWindow(sw, sw->swindow.vmin, sw->swindow.vScrollBar->scrollBar.orientation);
}
   ...
   Cardinal *num_params)
{
   XmScrolledWindowWidget sw = (XmScrolledWindowWidget)wid;
   if (!sw->swindow.vScrollBar) return;
   MoveWindow(sw, sw->swindow.vmax - sw->swindow.vExtent, sw->swindow.vScrollBar->scrollBar.orientation);
}
   ...
   Cardinal *num_params)
{
   ...
   if (sw->swindow.hScrollBar)
      value = sw->swindow.hOrigin - (sw->swindow.hScrollBar)->scrollBar.page_increment;
   else
   {
   ...
   Cardinal *num_params)
{
   ...
   MoveWindow(sw, MIN(value, (sw->swindow.hmax - sw->swindow.hExtent)), XmHORIZONTAL);
}
   ...
   Cardinal *num_params)
{
   ...
   if (sw->swindow.vScrollBar)
      value = sw->swindow.vOrigin - (sw->swindow.vScrollBar)->scrollBar.page_increment;
   else
   {
      if (sw->swindow.WorkWindow)
   ...
   Cardinal *num_params)
{
   ...
   MoveWindow(sw, MIN(value, (sw->swindow.vmax - sw->swindow.vExtent)), XmVERTICAL);
}
   ...
   WidgetClass wc)
{
   ...
   XmScrolledWindowWidgetClass super = (XmScrolledWindowWidgetClass)wc->core_class.superclass;
   ...
   Cardinal *num_args)
{
   ...
   if ((new_w->swindow.ScrollPolicy == XmAPPLICATION_DEFINED) && (new_w->swindow.VisualPolicy != XmVARIABLE))
   {
   ...
   if ((new_w->swindow.VisualPolicy == XmVARIABLE) && (request->swindow.ScrollBarPolicy == XmAS_NEEDED))
   {
      XmeWarning((Widget)new_w, SWMessage8);
      new_w->swindow.ScrollBarPolicy = XmSTATIC;
   }
   if (!XmRepTypeValidValue(XmRID_SCROLL_BAR_PLACEMENT,
                            new_w->swindow.Placement,
                            (Widget)new_w))
   {
   ...
   /****************
     ...
     *
     ****************/
   XtAugmentTranslations((Widget)new_w, (XtTranslations)((XmManagerClassRec *)XtClass(new_w))->manager_class.translations);
   /***********************
   ...
   if (new_w->swindow.ScrollPolicy == XmAPPLICATION_DEFINED)
   {
      ...
      if (new_w->core.width == 0)
         future_width = DEFAULT_SIZE;
      else
         future_width = new_w->core.width;
      if (new_w->core.height == 0)
         future_height = DEFAULT_SIZE;
      else
         future_height = new_w->core.height;
   ...
   /****************
     ...
     ****************/
   n = 0;
   XtSetArg(loc_args[n], XmNscrolledWindowChildType, XmCLIP_WINDOW);
   n++;
   ...
   XtSetArg(loc_args[n], XmNwidth, new_w->swindow.AreaWidth);
   n++;
   XtSetArg(loc_args[n], XmNheight, new_w->swindow.AreaHeight);
   n++;
   new_w->swindow.ClipWindow = (XmDrawingAreaWidget)
      XtCreateManagedWidget("ClipWindow", xmClipWindowWidgetClass, (Widget)new_w, loc_args, n);
   /***********************
   ...
   /**********************
     * vertical scrollbar */
   n = 0;
   XtSetArg(loc_args[n], XmNorientation, XmVERTICAL);
   n++;
   new_w->swindow.vScrollBar = (XmScrollBarWidget)
      XtCreateManagedWidget("VertScrollBar", xmScrollBarWidgetClass, (Widget)new_w, loc_args, n);
   /************************
     * horizontal scrollbar */
   n = 0;
   XtSetArg(loc_args[n], XmNorientation, XmHORIZONTAL);
   n++;
   new_w->swindow.hScrollBar = (XmScrollBarWidget)
      XtCreateManagedWidget("HorScrollBar", xmScrollBarWidgetClass, (Widget)new_w, loc_args, n);
   ...
   Region  region)
{
   ...
   if (sw->swindow.ScrollPolicy == XmAUTOMATIC)
   {
      XmeDrawShadows(XtDisplay(sw), XtWindow(sw), sw->manager.bottom_shadow_GC, sw->manager.top_shadow_GC, sw->swindow.ClipWindow->core.x - st, sw->swindow.ClipWindow->core.y - st, sw->swindow.AreaWidth + (st * 2), sw->swindow.AreaHeight + (st * 2), sw->manager.shadow_thickness, XmSHADOW_OUT);
   }
   else
   {
      if (sw->swindow.WorkWindow)
      {
         ...
         XmeDrawShadows(XtDisplay(sw), XtWindow(sw), sw->manager.bottom_shadow_GC, sw->manager.top_shadow_GC, sw->swindow.WorkWindow->core.x - st, sw->swindow.WorkWindow->core.y - st, sw->swindow.AreaWidth + ((bw + st) * 2), sw->swindow.AreaHeight + ((bw + st) * 2), sw->manager.shadow_thickness, XmSHADOW_OUT);
      }
      else
         XmeDrawShadows(XtDisplay(sw), XtWindow(sw), sw->manager.bottom_shadow_GC, sw->manager.top_shadow_GC, 0, 0, sw->swindow.AreaWidth + (st * 2), sw->swindow.AreaHeight + (st * 2), sw->manager.shadow_thickness, XmSHADOW_OUT);
   }
...
InsertChild(
   Widget w)
{
   XmManagerWidgetClass       superclass = (XmManagerWidgetClass)xmManagerWidgetClass;
   ...
   if ((swc->child_type == XmHOR_SCROLLBAR) || (swc->child_type == XmVERT_SCROLLBAR))
   {
      /************************/
      ...
      if (sw->swindow.auto_drag_model == XmAUTO_DRAG_ENABLED)
      {
         ...
         n++;
         XtSetArg(args[n], XmNdragProc, HandleDrag);
         n++;
         XtSetArg(args[n], XmNnumImportTargets, 1);
         n++;
         XtSetArg(args[n], XmNimportTargets, w);
         n++;
   ...
   else if (swc->child_type == XmVERT_SCROLLBAR)
      ((XmScrollFrameTrait)
          XmeTraitGet((XtPointer)XtClass((Widget)(sw)), XmQTscrollFrame))
   ...
   else
   {
      ...
      if (sw->swindow.ScrollPolicy == XmAPPLICATION_DEFINED)
      {
         ...
         else if (swc->child_type == XmWORK_AREA)
         {
      ...
      else
      {
         ...
         if ((swc->child_type == XmWORK_AREA) || (swc->child_type == XmSCROLL_HOR) || (swc->child_type == XmSCROLL_VERT) || (swc->child_type == XmNO_SCROLL))
         {
   ...
   Widget child)
{
   ...
   _XmProcessLock();
   delete_child = ((CompositeWidgetClass)xmScrolledWindowClassRec.core_class.superclass)
   ...
   Position              *vsbY)
{
   ...
   *vsbX = (HasVSB) ? (sw->core.width - sw->swindow.WidthPad - vsb_width) : sw->core.width;
   *hsbY = (HasHSB) ? (sw->core.height - sw->swindow.HeightPad - hsb_height) : sw->core.height;
   /****************
     *
   ...
   {
      case XmTOP_LEFT:
         *newx = (HasVSB) ? (vsb_width + sw->swindow.XOffset + pad + shad + HSBht)
                          : (sw->swindow.XOffset + shad + HSBht);
         *newy = (HasHSB) ? (hsb_height + sw->swindow.YOffset + pad + shad + VSBht)
...
{
   /* do a sanity check */
   if (sw->swindow.WorkWindow != NULL && sw->swindow.WorkWindow->core.being_destroyed)
   {
      sw->swindow.WorkWindow = NULL;
   }
   if (sw->swindow.hScrollBar != NULL && sw->swindow.hScrollBar->core.being_destroyed)
   {
      sw->swindow.hScrollBar = NULL;
   }
   if (sw->swindow.vScrollBar != NULL && sw->swindow.vScrollBar->core.being_destroyed)
   {
      sw->swindow.vScrollBar = NULL;
   }
}
   ...
   XmScrolledWindowWidget sw)
{
   ...
   /****************
     ...
     *
     ****************/
   if (ExistManaged(sw->swindow.WorkWindow) && XmIsPrimitive(sw->swindow.WorkWindow))
   {
   ...
   childWidth  = (HasVSB) ? MyWidth - (VSBwidth + (2 * VSBht) + SB_BORDER_WIDTH + ((HSBht + bw + shad) * 2) + pad)
                          : MyWidth - ((HSBht + shad + bw) * 2);
   childHeight = (HasHSB) ? MyHeight - (HSBheight + (2 * HSBht) + SB_BORDER_WIDTH + ((VSBht + shad + bw) * 2) + pad)
                          : MyHeight - ((VSBht + shad + bw) * 2);
   if (childWidth > MyWidth) childWidth = 2;
   if (childHeight > MyHeight) childHeight = 2;
   XmeConfigureObject(sw->swindow.WorkWindow, newx, newy, childWidth, childHeight, bw);
   /* not needed here, because never auto, but we leave it for now */
   if (sw->swindow.ClipWindow)
      XmeConfigureObject((Widget)sw->swindow.ClipWindow, newx, newy, childWidth, childHeight, 0);
   ...
   if (HasVSB)
      XmeConfigureObject((Widget)sw->swindow.vScrollBar, vsbX, vsbY, VSBwidth, sw->swindow.vsbHeight, SB_BORDER_WIDTH);
   if (HasHSB)
      XmeConfigureObject((Widget)sw->swindow.hScrollBar, hsbX, hsbY, sw->swindow.hsbWidth, HSBheight, SB_BORDER_WIDTH);
}
   ...
   XmScrolledWindowWidget sw)
{
   ...
   if (i == clip->composite.num_children)
   {
      ...
      XmeConfigureObject((Widget)clip,
                         ...
                         sw->swindow.AreaWidth,
                         sw->swindow.AreaHeight,
                         0);
      return;
   }
   ...
   /****************
     ...
     *
     ****************/
   if (((MyHeight - (shad * 2)) >= sw->swindow.vmax) && ((MyWidth - (shad * 2)) >= sw->swindow.hmax) && (sw->swindow.ScrollBarPolicy == XmAS_NEEDED))
   {
   ...
   {
      clipHeight = (HSBExists) ? MyHeight - (sw->swindow.hScrollBar->core.height + SB_BORDER_WIDTH + ((HSBht + shad) * 2 + pad))
                               : MyHeight - ((VSBht + shad) * 2);
      clipWidth  = (VSBExists) ? MyWidth - (sw->swindow.vScrollBar->core.width + SB_BORDER_WIDTH + ((VSBht + shad) * 2 + pad))
                               : MyWidth - ((HSBht + shad) * 2);
      if ((((MyHeight - (shad * 2)) < sw->swindow.vmax) && ((MyWidth - (shad * 2)) < sw->swindow.hmax)) || (sw->swindow.ScrollBarPolicy == XmSTATIC))
      {
         HasVSB = HasHSB = TRUE;
      }
      else
   ...
   if (HasVSB)
      ...
      sw->swindow.vsbHeight = MyHeight;
   HSBht = (HasHSB)
            ? sw->swindow.hScrollBar->primitive.highlight_thickness
            : 0;
   VSBht = (HasVSB)
            ? sw->swindow.vScrollBar->primitive.highlight_thickness
            : 0;
   /****************
     *
     * Get the position for scrollbars and workarea
   ...
   if (HasVSB)
      XmeConfigureObject((Widget)sw->swindow.vScrollBar, vsbX, vsbY, sw->swindow.vScrollBar->core.width, sw->swindow.vsbHeight, SB_BORDER_WIDTH);
   else if (VSBExists)
   {
      XmeConfigureObject((Widget)sw->swindow.vScrollBar, sw->core.width, sw->swindow.YOffset, sw->swindow.vScrollBar->core.width, sw->swindow.vsbHeight, SB_BORDER_WIDTH);
      if (_XmFocusIsHere((Widget)sw->swindow.vScrollBar))
         XmProcessTraversal((Widget)sw, XmTRAVERSE_NEXT_TAB_GROUP);
   }
   if (HasHSB)
      XmeConfigureObject((Widget)sw->swindow.hScrollBar, hsbX, hsbY, sw->swindow.hsbWidth, sw->swindow.hScrollBar->core.height, SB_BORDER_WIDTH);
   else if (HSBExists)
   {
      XmeConfigureObject((Widget)sw->swindow.hScrollBar,
                         sw->swindow.XOffset,
   ...
   XtIntervalId *id) /* unused */
{
   ...
   {
      if ((scrb->scrollBar.value + scrb->scrollBar.increment) <= (scrb->scrollBar.maximum - scrb->scrollBar.slider_size))
   ...
   {
      if ((scrb->scrollBar.value - scrb->scrollBar.increment) >= scrb->scrollBar.minimum)
         value = scrb->scrollBar.value - scrb->scrollBar.increment;
      else
      {
   ...
   XtPointer call_data)
{
   ...
   if (dragProc->reason == XmCR_DROP_SITE_ENTER_MESSAGE)
   {
      ...
      {
         sw->swindow.auto_drag_closure = auto_clos = (AutoDragClosure *)XtMalloc(sizeof(AutoDragClosure));
         auto_clos->widget                         = w;
         ...
         if (POINT_IN_RECT(dragProc->x, dragProc->y, rects->up) || POINT_IN_RECT(dragProc->x, dragProc->y, rects->left))
            auto_clos->direction = BACKWARD;
         else
            auto_clos->direction = FORWARD;
         sw->swindow.auto_drag_timer = XtAppAddTimeOut(XtWidgetToApplicationContext(w),
                                                       initial_delay,
                                                       TimerEvent,
                                                       (XtPointer)auto_clos);
      }
   }
   else if (dragProc->reason == XmCR_DROP_SITE_LEAVE_MESSAGE)
   {
   ...
   XmScrolledWindowWidget sw)
{
   ...
   if (hsb)
   {
      ...
      n = 0;
      XtSetArg(args[n], XmNdropRectangles, hrect);
      n++;
      XtSetArg(args[n], XmNnumDropRectangles, num_hrect);
      n++;
   ...
   if (vsb)
   {
      ...
      n = 0;
      XtSetArg(args[n], XmNdropRectangles, vrect);
      n++;
      XtSetArg(args[n], XmNnumDropRectangles, num_vrect);
      n++;
   ...
   Widget wid)
{
   ...
   if (XtIsRealized(wid))
   {
      XClearArea(XtDisplay(wid), XtWindow(wid), 0, 0, XtWidth(wid), XtHeight(wid), True);
   }
   if (sw->swindow.auto_drag_model == XmAUTO_DRAG_ENABLED)
   {
      UpdateAutoDrag(sw);
   ...
   Dimension             *pheight)
{
   ...
   if (ExistManaged(w))
   {
      ...
      if (ExistManaged((Widget)hsb))
      {
         ...
         hsheight = 0;
      if (!*pwidth)
         *pwidth = w->core.width + (2 * w->core.border_width) + vswidth + shads + hsbht + sw->swindow.XOffset + sw->swindow.WidthPad;
      if (!*pheight)
         *pheight = w->core.height + (2 * w->core.border_width) + hsheight + shads + vsbht + sw->swindow.YOffset + sw->swindow.HeightPad;
   }
   else
   {
   ...
   XtWidgetGeometry *reply)
{
   ...
   /****************
     ...
     ****************/
   if ((swc->child_type == XmGENERIC_CHILD) || ((w == (Widget)vsb) && (request->request_mode & CWY)) || ((w == (Widget)vsb) && (request->request_mode & CWHeight)) || ((w == (Widget)hsb) && (request->request_mode & CWX)) || ((w == (Widget)hsb) && (request->request_mode & CWWidth)))
   {
   ...
   {
      vsbht = 2 * vsb->primitive.highlight_thickness;
      if (ExistManaged(sw->swindow.WorkWindow) && XmIsPrimitive(sw->swindow.WorkWindow) && !(XmIsTraversable((Widget)sw->swindow.vScrollBar)))
         vsbht = 0;
      vswidth = vsb->core.width + sw->swindow.pad + vsbht;
   }
   else
   ...
   {
      hsbht = 2 * hsb->primitive.highlight_thickness;
      if (ExistManaged(sw->swindow.WorkWindow) && XmIsPrimitive(sw->swindow.WorkWindow) && !(XmIsTraversable((Widget)sw->swindow.hScrollBar)))
         hsbht = 0;
      hsheight = hsb->core.height + sw->swindow.pad + hsbht;
   }
   else
   ...
   if (((w == (Widget)hsb) || (w == (Widget)vsb)) && (request->request_mode & CWBorderWidth))
      return (XtGeometryNo); /* border apply to both dimensions */
   if (((w == (Widget)hsb) && ((Dimension)w->core.y < sw->core.height)) || ((w == (Widget)vsb) && ((Dimension)w->core.x < sw->core.width)))
   {
   ...
   XtWidgetGeometry *ret)
{
   ...
   {
      if ((sw->swindow.VisualPolicy == XmCONSTANT) || (!sw->swindow.WorkWindow))
      {
      ...
      if (ExistManaged(w))
      {
         ret->width  = KidWidth + (2 * w->core.border_width) + vswidth + ht + hsbht + sw->swindow.XOffset + sw->swindow.WidthPad;
         ret->height = KidHeight + (2 * w->core.border_width) + hsheight + ht + vsbht + sw->swindow.YOffset + sw->swindow.HeightPad;
      }
      else
      {
         ret->width  = sw->core.width;
   ...
   /****************
     ...
     *
     ****************/
   if ((sw->swindow.ScrollPolicy == XmAPPLICATION_DEFINED) || (!ExistManaged(w)))
   {
      if (!(request->request_mode & CWWidth))
      {
   ...
   hsb           = sw->swindow.hScrollBar;
   vsb           = sw->swindow.vScrollBar;
   if ((request->request_mode & CWWidth) && (request->request_mode & CWHeight) && (sw->swindow.ScrollBarPolicy == XmAS_NEEDED))
   {
      ret->height        = w->core.height + (2 * w->core.border_width) + ht + sw->swindow.YOffset + sw->swindow.HeightPad;
      ret->width         = w->core.width + (2 * w->core.border_width) + ht + sw->swindow.XOffset + sw->swindow.WidthPad;
      ret->request_mode |= (CWWidth | CWHeight);
      return (XtGeometryAlmost);
   }
   if (request->request_mode & CWHeight)
   {
      MyHeight = request->height - sw->swindow.YOffset - sw->swindow.HeightPad - ht;
      if (((w->core.height + (2 * w->core.border_width)) > MyHeight) || (sw->swindow.ScrollBarPolicy == XmSTATIC))
      {
         vsbht   = 2 * vsb->primitive.highlight_thickness;
      ...
      ret->request_mode |= CWWidth;
      ret->width         = w->core.width + (2 * w->core.border_width) + vswidth + ht + vsbht + sw->swindow.XOffset + sw->swindow.WidthPad;
      retval             = XtGeometryAlmost;
   }
   if (request->request_mode & CWWidth)
   {
      MyWidth = request->width - sw->swindow.XOffset - sw->swindow.WidthPad - ht;
      if (((w->core.width + (2 * w->core.border_width)) > MyWidth) || (sw->swindow.ScrollBarPolicy == XmSTATIC))
      {
         hsbht    = 2 * hsb->primitive.highlight_thickness;
      ...
      ret->request_mode |= CWHeight;
      ret->height        = w->core.height + (2 * w->core.border_width) + hsheight + ht + hsbht + sw->swindow.YOffset + sw->swindow.HeightPad;
      retval             = XtGeometryAlmost;
   }
   return (retval);
}
   ...
   Cardinal *num_args) /* unused */
{
   ...
   /*******************
     ...
     ********************/
   if ((new_w->swindow.WidthPad != current->swindow.WidthPad) || (new_w->swindow.HeightPad != current->swindow.HeightPad) || (new_w->manager.shadow_thickness != current->manager.shadow_thickness) || (new_w->swindow.pad != current->swindow.pad) || (new_w->swindow.Placement != current->swindow.Placement) || (new_w->swindow.ScrollBarPolicy != current->swindow.ScrollBarPolicy))
   {
      XtWidgetProc resize;
      new_w->swindow.XOffset = new_w->swindow.WidthPad;
      new_w->swindow.YOffset = new_w->swindow.HeightPad;
   ...
   if ((new_w->swindow.hScrollBar != current->swindow.hScrollBar) || (new_w->swindow.vScrollBar != current->swindow.vScrollBar) || (new_w->swindow.WorkWindow != current->swindow.WorkWindow))
   {
   ...
   Cardinal *num_args) /* unused */
{
   ...
   if ((swc->child_type != (unsigned char)RESOURCE_DEFAULT) && (!XmRepTypeValidValue(XmRID_SCROLLED_WINDOW_CHILD_TYPE, swc->child_type, nw)))
   {
                 ...
                 XRectangle *rect)
{
   ...
   for (i = 0; i < clip->composite.num_children; i++)
   {
      ...
      if (ExistManaged(child))
      {
         ...
         else if (swc->child_type == XmSCROLL_HOR)
         {
            /* same thing for x */
            if (child->core.x == 0)
            {
   ...
   XtTranslateCoords((Widget)clip, rect->x, rect->y, &root_x, &root_y);
   rect->x = root_x;
   rect->y = root_y;
   return True;
}
   ...
   Widget wregion)
{
   ...
   if (hscroll)
   {
      XtSetArg(args[n], XmNhorizontalScrollBar, hscroll);
      n++;
   }
   if (vscroll)
   {
      XtSetArg(args[n], XmNverticalScrollBar, vscroll);
      n++;
   }
   if (wregion)
   {
      XtSetArg(args[n], XmNworkWindow, wregion);
      n++;
   }
