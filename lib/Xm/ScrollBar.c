...
#define INVERSED_VALUE(sbw) \
  (sbw->scrollBar.maximum + sbw->scrollBar.minimum - sbw->scrollBar.value - \
   sbw->scrollBar.slider_size)
    /********    Static Function Declarations    ********/
    static void
    ClassPartInitialize(WidgetClass wc);
static void ProcessingDirectionDefault(XmScrollBarWidget widget, int offset, XrmValue *value);
static void BackgroundPixelDefault(XmScrollBarWidget widget, int offset, XrmValue *value);
static void TraversalDefault(XmScrollBarWidget widget, int offset, XrmValue *value);
static void HighlightDefault(XmScrollBarWidget widget, int offset, XrmValue *value);
static void SliderVisualDefault(XmScrollBarWidget widget, int offset, XrmValue *value);
static void SliderMarkDefault(XmScrollBarWidget widget, int offset, XrmValue *value);
static void EditableDefault(XmScrollBarWidget widget, int offset, XrmValue *value);
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static void GetForegroundGC(XmScrollBarWidget sbw);
static void GetUnavailableGC(XmScrollBarWidget sbw);
static void GetFlatSliderGC(XmScrollBarWidget sbw);
static void GetSliderPixmap(XmScrollBarWidget sbw);
static void CalcSliderRect(XmScrollBarWidget sbw,
                           short *slider_x,
                           short *slider_y,
                           short *slider_width,
                           short *slider_height);
...
static void Realize(Widget sbw, XtValueMask *window_mask, XSetWindowAttributes *window_attributes);
static void Destroy(Widget wid);
static Boolean ValidateInputs(XmScrollBarWidget current,
...
static int CalcSliderVal(XmScrollBarWidget sbw, int x, int y);
static void Select(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void Release(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void Moved(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void TopOrBottom(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void IncrementUpOrLeft(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void IncrementDownOrRight(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void PageUpOrLeft(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void PageDownOrRight(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void CancelDrag(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void MoveSlider(XmScrollBarWidget sbw, int currentX, int currentY);
...
static XmImportOperator ImportScrollBarValue(Widget wid, int offset, XtArgVal *value);
static void NavigChangeMoveCB(Widget nav,
                              XtCallbackProc moveProc,
                              XtPointer closure,
                              Boolean setunset);
static void NavigSetValue(Widget nav, XmNavigatorData nav_data, Boolean notify);
static void NavigGetValue(Widget nav, XmNavigatorData nav_data);
...
static XtResource resources[] = {
    ...
    {XmNbackground,
     ...
     XmRCallProc,
     (XtPointer)BackgroundPixelDefault},
    {XmNtroughColor,
     XmCTroughColor,
     XmRPixel,
     sizeof(Pixel),
    ...
    {XmNsnapBackMultiple,
     ...
     XmRImmediate,
     (XtPointer)MAXDIMENSION},
    {XmNslidingMode,
     XmCSlidingMode,
     XmRSlidingMode,
     sizeof(XtEnum),
     XtOffsetOf(XmScrollBarRec, scrollBar.sliding_mode),
     XmRImmediate,
     (XtPointer)XmSLIDER},
    {XmNeditable,
     XmCEditable,
     XmRBoolean,
     sizeof(XtEnum),
     XtOffsetOf(XmScrollBarRec, scrollBar.editable),
     XmRCallProc,
     (XtPointer)EditableDefault},
    {XmNsliderVisual,
     XmCSliderVisual,
     XmRSliderVisual,
     sizeof(XtEnum),
...
externaldef(xmscrollbarwidgetclass)
 ...
 *
 *********************************************************************/
static void ExportScrollBarValue(Widget wid,
                                 int offset, /* unused */
                                 XtArgVal *value)
{
 ...
 *
 *********************************************************************/
static XmImportOperator ImportScrollBarValue(Widget wid,
                                             int offset, /* unused */
                                             XtArgVal *value)
{
  XmScrollBarWidget sbw = (XmScrollBarWidget)wid;
 ...
 *
 *********************************************************************/
static void ProcessingDirectionDefault(XmScrollBarWidget widget,
                                       int offset, /* unused */
                                       XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void TraversalDefault(XmScrollBarWidget widget,
                             int offset, /* unused */
                             XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void SliderVisualDefault(XmScrollBarWidget widget,
                                int offset, /* unused */
                                XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void SliderMarkDefault(XmScrollBarWidget widget,
                              int offset, /* unused */
                              XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void EditableDefault(XmScrollBarWidget widget,
                            int offset, /* unused */
                            XrmValue *value)
{
 ...
 *
 *********************************************************************/
static void HighlightDefault(XmScrollBarWidget widget,
                             int offset, /* unused */
                             XrmValue *value)
{
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if (!XmRepTypeValidValue(XmRID_SHOW_ARROWS, new_w->scrollBar.show_arrows, (Widget)new_w)) {
    new_w->scrollBar.show_arrows = XmEACH_SIDE;
  }
  if (!XmRepTypeValidValue(XmRID_SLIDER_VISUAL, new_w->scrollBar.slider_visual, (Widget)new_w)) {
    new_w->scrollBar.slider_visual = XmSHADOWED_BACKGROUND;
  }
  if (!XmRepTypeValidValue(XmRID_SLIDER_MARK, new_w->scrollBar.slider_mark, (Widget)new_w)) {
    new_w->scrollBar.slider_mark = XmNONE;
  }
  if (!XmRepTypeValidValue(XmRID_SLIDING_MODE, new_w->scrollBar.sliding_mode, (Widget)new_w)) {
  ...
  if (new_w->scrollBar.slider_size == XmINVALID_DIMENSION) {
    new_w->scrollBar.slider_size = (new_w->scrollBar.maximum - new_w->scrollBar.minimum) / 10;
  ...
  if (new_w->scrollBar.slider_size > (new_w->scrollBar.maximum - new_w->scrollBar.minimum)) {
    new_w->scrollBar.slider_size = new_w->scrollBar.maximum - new_w->scrollBar.minimum;
  ...
  if (!XmRepTypeValidValue(XmRID_ORIENTATION, new_w->scrollBar.orientation, (Widget)new_w)) {
...
static void GetFlatSliderGC(XmScrollBarWidget sbw)
{
  ...
  values.graphics_exposures = False;
  sbw->scrollBar.flat_slider_GC = XtAllocateGC(
      (Widget)sbw, 0, valueMask, &values, GCClipMask, unusedMask);
}
...
static void GetUnavailableGC(XmScrollBarWidget sbw)
{
  ...
  sbw->scrollBar.unavailable_GC = XtAllocateGC(
      (Widget)sbw, 0, valueMask, &values, GCClipMask, unusedMask);
}
...
static void GetSliderPixmap(XmScrollBarWidget sbw)
{
  ...
  sbw->scrollBar.pixmap = XCreatePixmap(XtDisplay(sbw),
                                        RootWindowOfScreen(XtScreen(sbw)),
...
static void DrawSliderPixmap(XmScrollBarWidget sbw)
{
      ...
      (sbw->scrollBar.slider_visual == XmTROUGH_COLOR))
  {
    ...
    /* The trough area itself has been set, as the window background,
              ...
              pixel */
    XSetClipMask(XtDisplay((Widget)sbw), sbw->scrollBar.flat_slider_GC, None);
  ...
  else if ((sbw->scrollBar.slider_visual == XmBACKGROUND_COLOR) ||
  ...
  if (sbw->scrollBar.slider_mark == XmETCHED_LINE) {
    ...
    if (sbw->scrollBar.orientation == XmHORIZONTAL) {
      XmeDrawSeparator(XtDisplay(sbw),
                       slider,
                       sbw->primitive.top_shadow_GC,
                       sbw->primitive.bottom_shadow_GC,
                       NULL,
 ...
 *
 ************************************************************************/
static void CopySliderInWindow(XmScrollBarWidget sbw)
{
                           ...
                           short *slider_height)
{
  ...
  /* NOTE SIDE EFFECT */
#define MAX_SCROLLBAR_DIMENSION(val, min) ((val) > (min)) ? (val) : (hitTheWall = min)
  /* Don't let the slider get too small */
  if (sbw->scrollBar.orientation == XmHORIZONTAL) {
    *slider_width = MAX_SCROLLBAR_DIMENSION((int)(slideSize + 0.5), minSliderWidth);
    *slider_height = MAX(sbw->scrollBar.slider_area_height, minSliderHeight);
  }
  else /* orientation == XmVERTICAL */ {
    *slider_width = MAX(sbw->scrollBar.slider_area_width, minSliderWidth);
  ...
  if (sbw->scrollBar.orientation == XmHORIZONTAL) {
    ...
    if (sbw->scrollBar.sliding_mode == XmTHERMOMETER) {
      if (PROCESS_DIR_INVERSED(sbw)) {
        *slider_x = sbw->scrollBar.slider_area_x + sbw->scrollBar.slider_area_width -
                    *slider_width;
      }
      else {
      ...
      *slider_x = ((int)(((((float)sbw->scrollBar.value) - ((float)sbw->scrollBar.minimum)) *
                          factor) +
                         0.5)) +
    ...
    *slider_x = sbw->scrollBar.slider_area_x;
    if (sbw->scrollBar.sliding_mode == XmTHERMOMETER) {
      if (PROCESS_DIR_INVERSED(sbw)) {
        *slider_y = sbw->scrollBar.slider_area_y + sbw->scrollBar.slider_area_height -
                    *slider_height;
      }
      else {
        *slider_y = sbw->scrollBar.slider_area_y;
      ...
      *slider_y = ((int)(((((float)sbw->scrollBar.value) - ((float)sbw->scrollBar.minimum)) *
                          factor) +
                         0.5)) +
...
static void Redisplay(Widget wid, XEvent *event, Region region)
{
  ...
  if (sbw->scrollBar.show_arrows) {
    DRAWARROW(sbw,
              ((sbw->scrollBar.arrow1_selected) ? sbw->primitive.bottom_shadow_GC :
                                                  sbw->primitive.top_shadow_GC),
              ((sbw->scrollBar.arrow1_selected) ? sbw->primitive.top_shadow_GC :
                                                  sbw->primitive.bottom_shadow_GC),
              sbw->scrollBar.arrow1_x,
              sbw->scrollBar.arrow1_y,
              sbw->scrollBar.arrow1_orientation);
    DRAWARROW(sbw,
              ((sbw->scrollBar.arrow2_selected) ? sbw->primitive.bottom_shadow_GC :
                                                  sbw->primitive.top_shadow_GC),
              ((sbw->scrollBar.arrow2_selected) ? sbw->primitive.top_shadow_GC :
                                                  sbw->primitive.bottom_shadow_GC),
  ...
  if (!(XtIsSensitive(wid))) {
    ...
    XFillRectangle(
        ...
        sbw->primitive.highlight_thickness + sbw->primitive.shadow_thickness,
        sbw->primitive.highlight_thickness + sbw->primitive.shadow_thickness,
        XtWidth(sbw) -
            (2 * (sbw->primitive.highlight_thickness + sbw->primitive.shadow_thickness)),
        XtHeight(sbw) -
            (2 * (sbw->primitive.highlight_thickness + sbw->primitive.shadow_thickness)));
  }
...
#define BOTH_ARROWS_NEAR_SIDE(sbw) \
  (((sbw->scrollBar.show_arrows == XmMIN_SIDE) && !PROCESS_DIR_INVERSED(sbw)) || \
   ((sbw->scrollBar.show_arrows == XmMAX_SIDE) && PROCESS_DIR_INVERSED(sbw)))
#define BOTH_ARROWS_FAR_SIDE(sbw) \
  (((sbw->scrollBar.show_arrows == XmMIN_SIDE) && PROCESS_DIR_INVERSED(sbw)) || \
   ((sbw->scrollBar.show_arrows == XmMAX_SIDE) && !PROCESS_DIR_INVERSED(sbw)))
#define ARROW1_NEAR_SIDE(sbw) \
  ((sbw->scrollBar.show_arrows == XmEACH_SIDE) || BOTH_ARROWS_NEAR_SIDE(sbw))
#define ARROW2_FAR_SIDE(sbw) \
  ((sbw->scrollBar.show_arrows == XmEACH_SIDE) || BOTH_ARROWS_FAR_SIDE(sbw))
  ...
  if (sbw->scrollBar.show_arrows) {
    if (sbw->scrollBar.orientation == XmHORIZONTAL) {
      ...
      sbw->scrollBar.arrow_width = sbw->scrollBar.arrow_height = sbw->core.height - 2 * (ht + st);
      if (ARROW1_NEAR_SIDE(sbw)) {
        sbw->scrollBar.arrow1_x = ht + st;
      }
      else {
        sbw->scrollBar.arrow1_x = sbw->core.width - ht - st - 2 * sbw->scrollBar.arrow_width;
      }
      if (sbw->core.width < 2 * (sbw->scrollBar.arrow_width + ht + st) + MIN_SLIDER_LENGTH + 2)
        sbw->scrollBar.arrow_width = (sbw->core.width - (MIN_SLIDER_LENGTH + 2 + 2 * (ht + st))) /
                                     2;
      ...
      if (ARROW2_FAR_SIDE(sbw)) {
        ...
        sbw->scrollBar.arrow2_x = ht + st + sbw->scrollBar.arrow_width;
      }
      ...
      if (ARROW1_NEAR_SIDE(sbw)) {
        ...
        sbw->scrollBar.arrow1_y = sbw->core.height - ht - st - 2 * sbw->scrollBar.arrow_height;
      }
      if (sbw->core.height < 2 * (sbw->scrollBar.arrow_height + ht + st) + MIN_SLIDER_LENGTH + 2)
      ...
      if (ARROW2_FAR_SIDE(sbw)) {
        ...
        sbw->scrollBar.arrow2_y = ht + st + sbw->scrollBar.arrow_height;
      }
 ...
 *
 ********************************************************************/
static void Realize(Widget wid, XtValueMask *window_mask, XSetWindowAttributes *window_attributes)
{
  XmScrollBarWidget sbw = (XmScrollBarWidget)wid;
  ...
  if (new_w->scrollBar.orientation == XmHORIZONTAL) {
    if (new_w->scrollBar.processing_direction == XmMAX_ON_LEFT &&
        ...
        ((new_w->scrollBar.slider_size != current->scrollBar.slider_size) ||
         ...
         (new_w->scrollBar.minimum != current->scrollBar.minimum)))
    {
      new_w->scrollBar.value = (new_w->scrollBar.maximum + new_w->scrollBar.minimum -
      ...
      (new_w->scrollBar.processing_direction == current->scrollBar.processing_direction))
  {
    if ((new_w->scrollBar.orientation == XmHORIZONTAL) &&
      ...
      new_w->scrollBar.processing_direction = XmMAX_ON_LEFT;
             ...
             (current->scrollBar.processing_direction == XmMAX_ON_BOTTOM))
      new_w->scrollBar.processing_direction = XmMAX_ON_RIGHT;
...
window background already, so we need to undo that */
    ...
    if (change_to != XmUNSPECIFIED_PIXEL) {
      ...
      XtReleaseGC((Widget)new_w, new_w->scrollBar.flat_slider_GC);
      GetFlatSliderGC(new_w);
      XSetWindowBackground(XtDisplay((Widget)new_w), XtWindow((Widget)new_w), change_to);
    }
  }
  /*
  ...
  if ((new_w->scrollBar.orientation != current->scrollBar.orientation) ||
      ...
      (new_w->scrollBar.show_arrows != current->scrollBar.show_arrows))
  {
  ...
  if ((new_w->scrollBar.slider_size != current->scrollBar.slider_size) ||
      (new_w->scrollBar.minimum != current->scrollBar.minimum) ||
      (new_w->scrollBar.maximum != current->scrollBar.maximum) ||
      (new_w->scrollBar.processing_direction != current->scrollBar.processing_direction))
  {
...
static int CalcSliderVal(XmScrollBarWidget sbw, int x, int y)
{
  ...
  range = sbw->scrollBar.maximum - sbw->scrollBar.minimum - sbw->scrollBar.slider_size;
  /* Now scale the proportion in pixels to user units */
  proportion = (proportion * range) + ((float)sbw->scrollBar.minimum);
  ...
  else if (int_proportion > (sbw->scrollBar.maximum - sbw->scrollBar.slider_size))
    int_proportion = sbw->scrollBar.maximum - sbw->scrollBar.slider_size;
...
static void Select(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if ((sbw->scrollBar.orientation == XmHORIZONTAL) && (buttonEvent->y >= slider_y) &&
      (buttonEvent->y <= slider_y + slider_height) && (buttonEvent->button == Button1) &&
      (sbw->scrollBar.sliding_mode == XmTHERMOMETER) &&
      (((PROCESS_DIR_INVERSED(sbw)) && (buttonEvent->x >= slider_x) &&
        (buttonEvent->x <= slider_x + THERMO_MARK_OFFSET)) ||
       (!PROCESS_DIR_INVERSED(sbw) && (buttonEvent->x <= slider_x + slider_width) &&
        (buttonEvent->x >= slider_x + slider_width - THERMO_MARK_OFFSET))))
    /* hack */
    buttonEvent->button = Button2;
  if ((sbw->scrollBar.orientation == XmVERTICAL) && (buttonEvent->x >= slider_x) &&
      (buttonEvent->x <= slider_x + slider_width) && (buttonEvent->button == Button1) &&
      (sbw->scrollBar.sliding_mode == XmTHERMOMETER) &&
      (((PROCESS_DIR_INVERSED(sbw)) && (buttonEvent->y >= slider_y) &&
  ...
  if ((buttonEvent->x >= slider_x) && (buttonEvent->x <= slider_x + slider_width) &&
      (buttonEvent->y >= slider_y) && (buttonEvent->y <= slider_y + slider_height) &&
      ((buttonEvent->button != Button1) || (sbw->scrollBar.sliding_mode != XmTHERMOMETER)))
  {
  ...
  /* ... in the trough (i.e. slider area)... */
           ...
           (buttonEvent->x <= sbw->scrollBar.slider_area_x + sbw->scrollBar.slider_area_width) &&
           (buttonEvent->y <= sbw->scrollBar.slider_area_y + sbw->scrollBar.slider_area_height))
  {
    sbw->scrollBar.arrow1_selected = FALSE;
    sbw->scrollBar.arrow2_selected = FALSE;
    sbw->scrollBar.saved_value = sbw->scrollBar.value;
  ...
  /* ... in arrow 1 */
           ...
           (buttonEvent->x <= sbw->scrollBar.arrow1_x + sbw->scrollBar.arrow_width) &&
           (buttonEvent->y <= sbw->scrollBar.arrow1_y + sbw->scrollBar.arrow_height))
  {
  ...
  /* ... in arrow 2 */
           ...
           (buttonEvent->x <= sbw->scrollBar.arrow2_x + sbw->scrollBar.arrow_width) &&
           (buttonEvent->y <= sbw->scrollBar.arrow2_y + sbw->scrollBar.arrow_height))
  {
 ...
 *
 ************************************************************************/
static void Release(Widget wid,
                    XEvent *event,
                    ...
                    Cardinal *num_params) /* unused */
{
      ...
      (sbw->scrollBar.value > sbw->scrollBar.minimum))
  {
    ...
    sbw->scrollBar.flags &= ~ARROW1_AVAILABLE;
  if ((!(sbw->scrollBar.flags & ARROW2_AVAILABLE)) &&
      (sbw->scrollBar.value < (sbw->scrollBar.maximum - sbw->scrollBar.slider_size)))
  {
    XClearArea(XtDisplay(sbw),
               XtWindow(sbw),
               sbw->scrollBar.arrow2_x,
  ...
  if (sbw->scrollBar.sliding_on == True) {
    ...
    ScrollCallback(
        sbw, XmCR_VALUE_CHANGED, sbw->scrollBar.value, event->xbutton.x, event->xbutton.y, event);
  }
#ifdef FUNKY_INSENSITIVE_VISUAL
  XSetClipMask(XtDisplay(sbw), sbw->scrollBar.unavailable_GC, None);
  if (!(sbw->scrollBar.flags & ARROW1_AVAILABLE)) {
                  ...
                  Cardinal *num_params) /* unused */
{
  ...
  int real_height_limit = (sbw->scrollBar.snap_back_multiple + (buttonEvent->y > 0)) *
                          XtHeight(wid);
  if (!sbw->scrollBar.editable)
    return;
  if (!(sbw->scrollBar.flags & SLIDER_AVAILABLE))
  ...
  if (sbw->scrollBar.flags & OPERATION_CANCELLED)
    return;
  if (!sbw->scrollBar.sliding_on)
    return;
  /* Only deal with snap_back if operation was started by a
       click in the slider. */
  if (((sbw->scrollBar.orientation == XmVERTICAL) &&
       ((buttonEvent->x > real_width_limit) || (-buttonEvent->x > real_width_limit))) ||
      ((sbw->scrollBar.orientation == XmHORIZONTAL) &&
       ((buttonEvent->y > real_height_limit) || (-buttonEvent->y > real_height_limit))))
  {
    ...
    if (!(sbw->scrollBar.add_flags & SNAPPED_OUT)) {
      ...
      ScrollCallback(
          sbw, XmCR_VALUE_CHANGED, sbw->scrollBar.value, savedX, savedY, (XEvent *)buttonEvent);
  ...
  if (button_x > sbw->scrollBar.slider_area_x + sbw->scrollBar.slider_area_width)
    button_x = sbw->scrollBar.slider_area_x + sbw->scrollBar.slider_area_width;
  if (button_y < sbw->scrollBar.slider_area_y)
    button_y = sbw->scrollBar.slider_area_y;
  if (button_y > sbw->scrollBar.slider_area_y + sbw->scrollBar.slider_area_height)
    button_y = sbw->scrollBar.slider_area_y + sbw->scrollBar.slider_area_height;
  /*
  ...
  if (sbw->scrollBar.orientation == XmHORIZONTAL) {
    ...
    if ((newX + sbw->scrollBar.slider_width >
         sbw->scrollBar.slider_area_x + sbw->scrollBar.slider_area_width) &&
        (sbw->scrollBar.sliding_mode != XmTHERMOMETER))
      newX = sbw->scrollBar.slider_area_x + sbw->scrollBar.slider_area_width -
             sbw->scrollBar.slider_width;
  }
  else {
    ...
    if ((newY + sbw->scrollBar.slider_height >
         sbw->scrollBar.slider_area_y + sbw->scrollBar.slider_area_height) &&
        (sbw->scrollBar.sliding_mode != XmTHERMOMETER))
      newY = sbw->scrollBar.slider_area_y + sbw->scrollBar.slider_area_height -
             sbw->scrollBar.slider_height;
  }
      ...
      ((sbw->scrollBar.orientation == XmVERTICAL) && (realY != sbw->scrollBar.initial_y)))
  {
    ...
    if (slideVal != sbw->scrollBar.value) {
      ...
      if (slideVal >= (sbw->scrollBar.maximum - sbw->scrollBar.slider_size))
        slideVal = sbw->scrollBar.maximum - sbw->scrollBar.slider_size;
 ...
 *
 *********************************************************************/
static void TopOrBottom(Widget wid,
                        XEvent *event,
                        String *params,       /* unused */
                        Cardinal *num_params) /* unused */
{
  ...
  if (event->type == KeyPress) {
    ...
    if (key_sym == osfXK_BeginLine) {
      if (sbp->orientation == XmVERTICAL) {
        if (sbp->processing_direction == XmMAX_ON_BOTTOM)
          ...
          MoveSlider(sbw,
                     sbp->slider_x,
                     sbp->slider_area_y + sbp->slider_area_height - sbp->slider_height);
      }
      else {
        if (sbp->processing_direction == XmMAX_ON_RIGHT)
          ...
          MoveSlider(
              sbw, sbp->slider_area_x + sbp->slider_area_width - sbp->slider_width, sbp->slider_y);
      }
      /*
      ...
      if (!PROCESS_DIR_INVERSED(sbw)) {
        ...
        if (sbp->sliding_mode == XmTHERMOMETER)
          RedrawSliderWindow(sbw);
        ScrollCallback(
            sbw, XmCR_TO_BOTTOM, sbp->value, keyEvent->x, keyEvent->y, (XEvent *)keyEvent);
      }
    }
    ...
    XmDropSiteStartUpdate(wid);
    if /* In arrow1... */
        ((buttonEvent->x >= sbp->arrow1_x) && (buttonEvent->y >= sbp->arrow1_y) &&
         ...
         (buttonEvent->y <= sbp->arrow1_y + sbp->arrow_height))
    {
      ...
      if (sbp->sliding_mode == XmTHERMOMETER)
        RedrawSliderWindow(sbw);
      ScrollCallback(
          sbw, XmCR_TO_TOP, sbp->value, buttonEvent->x, buttonEvent->y, (XEvent *)buttonEvent);
    }
    else if /* In arrow2... */
        ((buttonEvent->x >= sbp->arrow2_x) && (buttonEvent->y >= sbp->arrow2_y) &&
         (buttonEvent->x <= sbp->arrow2_x + sbp->arrow_width) &&
         (buttonEvent->y <= sbp->arrow2_y + sbp->arrow_height))
    {
      ...
      if (sbp->orientation == XmVERTICAL)
        MoveSlider(
            sbw, sbp->slider_x, sbp->slider_area_y + sbp->slider_area_height - sbp->slider_height);
      else
        MoveSlider(
            sbw, sbp->slider_area_x + sbp->slider_area_width - sbp->slider_width, sbp->slider_y);
      sbp->value = sbp->maximum - sbp->slider_size;
      if (sbp->sliding_mode == XmTHERMOMETER)
        RedrawSliderWindow(sbw);
      ScrollCallback(
          sbw, XmCR_TO_BOTTOM, sbp->value, buttonEvent->x, buttonEvent->y, (XEvent *)buttonEvent);
    }
    else if /* in the trough between arrow2 and the slider... */
        (((sbp->sliding_mode != XmTHERMOMETER) &&
          (((sbp->orientation == XmHORIZONTAL) && (buttonEvent->x >= sbp->slider_area_x) &&
            (buttonEvent->x < sbp->slider_x) && (buttonEvent->y >= sbp->slider_area_y) &&
            (buttonEvent->y <= sbp->slider_area_y + sbp->slider_area_height)) ||
           ((sbp->orientation == XmVERTICAL) && (buttonEvent->y >= sbp->slider_area_y) &&
            (buttonEvent->y < sbp->slider_y) && (buttonEvent->x >= sbp->slider_area_x) &&
            (buttonEvent->x < sbp->slider_area_x + sbp->slider_area_width)))) ||
         /* only partial treatment, processing direction
                  has to be handled here */
         ((sbp->sliding_mode == XmTHERMOMETER) &&
          (((sbp->orientation == XmHORIZONTAL) && (buttonEvent->x >= sbp->slider_area_x) &&
            (buttonEvent->x < sbp->slider_width) && (buttonEvent->y >= sbp->slider_area_y) &&
            (buttonEvent->y <= sbp->slider_area_y + sbp->slider_area_height)) ||
           ((sbp->orientation == XmVERTICAL) &&
            (buttonEvent->y < sbp->slider_area_height - sbp->slider_height) &&
            ...
            (buttonEvent->x < sbp->slider_area_x + sbp->slider_area_width)))))
    {
      ...
      ScrollCallback(
        ...
        (((sbp->orientation == XmHORIZONTAL) &&
          ...
          (buttonEvent->y <= sbp->slider_area_y + sbp->slider_area_height)) ||
         ((sbp->orientation == XmVERTICAL) &&
          (buttonEvent->y > sbp->slider_y + sbp->slider_height) &&
          (buttonEvent->y <= sbp->slider_area_y + sbp->slider_area_height) &&
          (buttonEvent->x >= sbp->slider_area_x) &&
          (buttonEvent->x <= sbp->slider_area_x + sbp->slider_area_width)) ||
         (sbp->sliding_mode == XmTHERMOMETER))
...
static void IncrementUpOrLeft(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (new_value != sbw->scrollBar.value) {
...
#ifdef FUNKY_INSENSITIVE_VISUAL
    if ((sbw->scrollBar.value = new_value) == sbw->scrollBar.minimum) {
      XSetClipMask(XtDisplay(sbw), sbw->scrollBar.unavailable_GC, None);
      XFillRectangle(XtDisplay(sbw),
                     XtWindow(sbw),
                     sbw->scrollBar.unavailable_GC,
    ...
    RedrawSliderWindow(sbw);
    ScrollCallback(
        sbw, XmCR_DECREMENT, sbw->scrollBar.value, event->xbutton.x, event->xbutton.y, event);
  }
}
...
static void IncrementDownOrRight(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (new_value != sbw->scrollBar.value) {
    ...
    RedrawSliderWindow(sbw);
    ScrollCallback(
        sbw, XmCR_INCREMENT, sbw->scrollBar.value, event->xbutton.x, event->xbutton.y, event);
  }
}
...
static void PageUpOrLeft(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (new_value != sbw->scrollBar.value) {
    ...
    RedrawSliderWindow(sbw);
    ScrollCallback(
        sbw, XmCR_PAGE_DECREMENT, sbw->scrollBar.value, event->xbutton.x, event->xbutton.y, event);
  }
}
...
static void PageDownOrRight(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (new_value != sbw->scrollBar.value) {
    ...
    RedrawSliderWindow(sbw);
    ScrollCallback(
        sbw, XmCR_PAGE_INCREMENT, sbw->scrollBar.value, event->xbutton.x, event->xbutton.y, event);
  }
}
...
static Boolean ChangeScrollBarValue(XmScrollBarWidget sbw)
{
  ...
  if (sbw->scrollBar.value >= (sbw->scrollBar.maximum - sbw->scrollBar.slider_size))
    sbw->scrollBar.value = sbw->scrollBar.maximum - sbw->scrollBar.slider_size;
 ...
 *
 *********************************************************************/
static void TimerEvent(XtPointer closure, XtIntervalId *id) /* unused */
{
    ...
    XmScrollBarWidget sbw, int reason, int value, int xpixel, int ypixel, XEvent *event)
{
  ...
  if (PROCESS_DIR_INVERSED(sbw)) {
    ...
    call_value.value = sbw->scrollBar.maximum + sbw->scrollBar.minimum - value -
                       sbw->scrollBar.slider_size;
  }
  else
...
static void NavigSetValue(Widget nav, XmNavigatorData nav_data, Boolean notify)
{
  ...
  if (!(sbw->scrollBar.dimMask & nav_data->dimMask))
    return;
  ...
  if (nav_data->valueMask & NavValue) {
    if ((PROCESS_DIR_INVERSED(sbw) ? INVERSED_VALUE(sbw) : sbw->scrollBar.value) !=
  ...
  if ((nav_data->valueMask & NavMinimum) &&
      (sbw->scrollBar.minimum != ACCESS_DIM(sbw->scrollBar.dimMask, nav_data->minimum)))
  {
    XtSetArg(arglist[n], XmNminimum, ACCESS_DIM(sbw->scrollBar.dimMask, nav_data->minimum));
    n++;
  }
  if ((nav_data->valueMask & NavMaximum) &&
      (sbw->scrollBar.maximum != ACCESS_DIM(sbw->scrollBar.dimMask, nav_data->maximum)))
  {
  ...
  if (sbw->scrollBar.sliding_mode != XmTHERMOMETER) {
    ...
    {
      XtSetArg(
          arglist[n], XmNsliderSize, ACCESS_DIM(sbw->scrollBar.dimMask, nav_data->slider_size));
      n++;
    }
  }
  if ((nav_data->valueMask & NavIncrement) &&
      (sbw->scrollBar.increment != ACCESS_DIM(sbw->scrollBar.dimMask, nav_data->increment)) &&
...
Widget XmVaCreateScrollBar(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmScrollBarWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedScrollBar(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmScrollBarWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
