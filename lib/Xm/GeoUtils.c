   ...
   XmGeoCreateProc   createMatrix)
{
   ...
   else
   {
      ...
      if ((resize_policy == XmRESIZE_GROW) && ((width < XtWidth(widget)) || (height < XtHeight(widget))))
      {
         desired->width  = XtWidth(widget);
         desired->height = XtHeight(widget);
      }
   ...
   int           amtOffset)
{
   ...
   if (boxWidth >= numBoxes)
   {
      ...
      while (rowPtr->kid)
      {
         deltaW = (amtOffset * (int)(rowPtr->box.width + (rowPtr->box.border_width << 1))) / ((int)boxWidth);
   ...
   Dimension      betweenSpace)
{
   ...
   switch (layoutPtr->fill_mode)
   {
      case XmGEO_CENTER:
      {
         ...
         _XmGeoCalcFill(totalFill, marginW, numBoxes, layoutPtr->space_end, layoutPtr->space_between, &endSpace, &betweenSpace);
         break;
      }
      case XmGEO_PACK:
   ...
   Dimension      marginW)
{
   ...
   while (boxPtr->kid)
   {
      ...
      { /* Wrap the line.  Also adjust preceding segment according to
         ...
         SegmentFill(rowBegin, numBoxes, layoutPtr, x, width, marginW, endX, maxX, endSpace, betweenSpace);
         numBoxes = 0;
         rowX     = x + endSpace;
         y       += rowH;
         rowBegin = boxPtr;
   ...
   Dimension      marginH) /* unused */
{
   ...
   { /* Wrapping is required, so fill routines and other policy decisions
        ...
        *   done.
        */
      y = _XmGeoLayoutWrap(rowBoxes, layoutPtr, x, y, endsOfBoxes, betweenBoxes, maxX, width, marginW);
   }
   else
   {
      if (sumW > width)
            ...
            switch (layoutPtr->fill_mode)
            {
               case XmGEO_CENTER:
               {
                  ...
                  totalFill = (marginW << 1) + layoutPtr->fill_width
                            + width - sumW;
                  _XmGeoCalcFill(totalFill, marginW, numBoxes, layoutPtr->space_end, layoutPtr->space_between, &endsOfBoxes, &betweenBoxes);
                  break;
               }
               case XmGEO_PACK:
               { /* For a packed layout, just layout with extra space
   ...
   XWindowChanges *old)
{
   ...
   XClearArea(XtDisplay(parent), XtWindow(parent), old->x, old->y, old->width + bw2, old->height + bw2, TRUE);
   bw2 = r->rectangle.border_width << 1;
   XClearArea(XtDisplay(parent), XtWindow(parent), (int)r->rectangle.x, (int)r->rectangle.y, (unsigned int)(r->rectangle.width + bw2), (unsigned int)(r->rectangle.height + bw2), TRUE);
   return;
}
