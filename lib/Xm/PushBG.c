...
static XtResource resources[] = {
   { XmNactivateCallback,   XmCCallback,           XmRCallback,            sizeof(XtCallbackList), XtOffsetOf(struct _XmPushButtonGadgetRec, pushbutton.activate_callback), XmRPointer,   (XtPointer)NULL            },
   { XmNarmCallback,        XmCCallback,           XmRCallback,            sizeof(XtCallbackList), XtOffsetOf(struct _XmPushButtonGadgetRec, pushbutton.arm_callback),      XmRPointer,   (XtPointer)NULL            },
   { XmNdisarmCallback,     XmCCallback,           XmRCallback,            sizeof(XtCallbackList), XtOffsetOf(struct _XmPushButtonGadgetRec, pushbutton.disarm_callback),   XmRPointer,   (XtPointer)NULL            },
   { XmNshadowThickness,    XmCShadowThickness,    XmRHorizontalDimension, sizeof(Dimension),      XtOffsetOf(struct _XmPushButtonGadgetRec, gadget.shadow_thickness),      XmRCallProc,  (XtPointer)_XmSetThickness },
   ...
   { XmNshowAsDefault,      XmCShowAsDefault,      XmRBooleanDimension,    sizeof(Dimension),      XtOffsetOf(struct _XmPushButtonGadgetRec, pushbutton.show_as_default),   XmRImmediate, (XtPointer)0               },
};
...
static XtResource cache_resources[] = {
   { XmNmultiClick,                   XmCMultiClick,                   XmRMultiClick,          sizeof(unsigned char), XtOffsetOf(struct _XmPushButtonGCacheObjRec, pushbutton_cache.multiClick),                      XmRImmediate, (XtPointer)XmINVALID_MULTICLICK },
   { XmNdefaultButtonShadowThickness, XmCDefaultButtonShadowThickness, XmRHorizontalDimension, sizeof(Dimension),     XtOffsetOf(struct _XmPushButtonGCacheObjRec, pushbutton_cache.default_button_shadow_thickness), XmRImmediate, (XtPointer)0                    },
   { XmNfillOnArm,                    XmCFillOnArm,                    XmRBoolean,             sizeof(Boolean),       XtOffsetOf(struct _XmPushButtonGCacheObjRec, pushbutton_cache.fill_on_arm),                     XmRImmediate, (XtPointer)True                 },
   { XmNarmColor,                     XmCArmColor,                     XmRPixel,               sizeof(Pixel),         XtOffsetOf(struct _XmPushButtonGCacheObjRec, pushbutton_cache.arm_color),                       XmRImmediate, (XtPointer)INVALID_PIXEL        },
   { XmNarmPixmap,                    XmCArmPixmap,                    XmRDynamicPixmap,       sizeof(Pixmap),        XtOffsetOf(struct _XmPushButtonGCacheObjRec, pushbutton_cache.arm_pixmap),                      XmRImmediate, (XtPointer)XmUNSPECIFIED_PIXMAP },
};
/*  Definition for resources that need special processing in get values  */
static XmSyntheticResource cache_syn_resources[] = {
   { XmNdefaultButtonShadowThickness, sizeof(Dimension), XtOffsetOf(struct _XmPushButtonGCacheObjRec, pushbutton_cache.default_button_shadow_thickness), XmeFromHorizontalPixels, XmeToHorizontalPixels },
};
/*************************************<->*************************************
 *
 *
...
externaldef(xmpushbuttongadgetclass) WidgetClass xmPushButtonGadgetClass = (WidgetClass)&xmPushButtonGadgetClassRec;
   ...
   Mask    event_mask)
{
   ...
   if ((event_mask & XmARM_EVENT) || ((PBG_MultiClick(pb) == XmMULTICLICK_KEEP) && (event_mask & XmMULTI_ARM_EVENT)))
   {
   ...
   XEvent            *event)
{
   ...
   PBG_Armed(pb) = TRUE;
   _XmProcessLock();
   expose = ((XmPushButtonGadgetClassRec *)(pb->object.widget_class))->rect_class.expose;
   _XmProcessUnlock();
   ...
   XEvent            *event)
{
   ...
   if ((event->xany.type == ButtonPress || event->xany.type == ButtonRelease) && _XmGetPointVisibility((Widget)pb, event->xbutton.x_root, event->xbutton.y_root))
   {
   ...
   Cardinal *num_params) /* unused */
{
   ...
   if (is_menupane && menuSTrait != NULL)
   {
      ...
      if (torn_has_focus)
      {
         ...
         /* Set the focus here. */
         XmProcessTraversal((Widget)pb, XmTRAVERSE_CURRENT);
         if ((pb->rectangle.width > 2 * pb->gadget.highlight_thickness) && (pb->rectangle.height > 2 * pb->gadget.highlight_thickness))
            XmeDrawShadows(XtDisplay(pb), XtWindow(pb), LabG_BottomShadowGC(pb), LabG_TopShadowGC(pb), pb->rectangle.x + pb->gadget.highlight_thickness, pb->rectangle.y + pb->gadget.highlight_thickness, pb->rectangle.width - 2 * pb->gadget.highlight_thickness, pb->rectangle.height - 2 * pb->gadget.highlight_thickness, pb->gadget.shadow_thickness, etched_in ? XmSHADOW_IN : XmSHADOW_OUT);
      }
   }
   else
   {
   ...
   */
   if (!is_menupane || torn_has_focus)
   {
      ...
      {
         PBG_Timer(pb)                                              = XtAppAddTimeOut(XtWidgetToApplicationContext((Widget)pb),
   ...
   if (XtIsRealized((Widget)pb) && XtIsManaged((Widget)pb))
   {
	   ...
	   */
         if (XmeFocusIsInShell((Widget)pb) && (XmGetFocusWidget((Widget)pb) == (Widget)pb))
         {
            ...
            if ((pb->rectangle.width > 2 * pb->gadget.highlight_thickness) && (pb->rectangle.height > 2 * pb->gadget.highlight_thickness))
               XmeDrawShadows(XtDisplay(pb), XtWindow(pb), LabG_TopShadowGC(pb), LabG_BottomShadowGC(pb), pb->rectangle.x + pb->gadget.highlight_thickness, pb->rectangle.y + pb->gadget.highlight_thickness, pb->rectangle.width - 2 * pb->gadget.highlight_thickness, pb->rectangle.height - 2 * pb->gadget.highlight_thickness, pb->gadget.shadow_thickness, etched_in ? XmSHADOW_IN : XmSHADOW_OUT);
         }
      }
      else
      {
   ...
   XEvent            *event)
{
   ...
   */
   ...
   else if ((pb->rectangle.width > 2 * pb->gadget.highlight_thickness) && (pb->rectangle.height > 2 * pb->gadget.highlight_thickness))
      XmeDrawShadows(
         XtDisplay(pb),
         XtWindow(pb),
         LabG_TopShadowGC(pb),
   ...
   XEvent *event)
{
   ...
   if (LabG_IsMenupane(pb))
   {
      if (_XmGetInDragMode((Widget)pb) && PBG_Armed(pb))
      {
         ...
         XmeDrawHighlight(XtDisplay(pb), XtWindow(pb), LabG_BackgroundGC(pb), pb->rectangle.x + pb->gadget.highlight_thickness, pb->rectangle.y + pb->gadget.highlight_thickness, pb->rectangle.width - 2 * pb->gadget.highlight_thickness, pb->rectangle.height - 2 * pb->gadget.highlight_thickness, pb->gadget.shadow_thickness);
         if (PBG_DisarmCallback(pb))
         {
            XFlush(XtDisplay(pb));
            call_value.reason = XmCR_DISARM;
   ...
   Widget wid)
{
   ...
   if (highlight_width > 0)
   {
      ...
      switch (default_button_emphasis)
      {
         ...
         case XmINTERNAL_HIGHLIGHT:
            if (PBG_DefaultButtonShadowThickness(pb))
               delta = Xm3D_ENHANCE_PIXEL + 2 * (PBG_Compatible(pb) ? PBG_ShowAsDefault(pb) : PBG_DefaultButtonShadowThickness(pb));
            else
               delta = 0;
            break;
         default:
      ...
      XmeDrawHighlight(XtDisplay(pb), XtWindow(pb), LabG_HighlightGC(pb), x, y, width, height, highlight_width);
   }
}
   ...
   Widget wid)
{
   ...
   if (LabG_IsMenupane(pb))
   {
      ...
      XmeClearBorder(XtDisplay(pb), XtWindow(pb), pb->rectangle.x + pb->gadget.highlight_thickness, pb->rectangle.y + pb->gadget.highlight_thickness, pb->rectangle.width - 2 * pb->gadget.highlight_thickness, pb->rectangle.height - 2 * pb->gadget.highlight_thickness, pb->gadget.shadow_thickness);
      if (PBG_DisarmCallback(pb))
      {
         XFlush(XtDisplay(pb));
         call_value.reason = XmCR_DISARM;
      ...
      switch (default_button_emphasis)
      {
         case XmINTERNAL_HIGHLIGHT:
            if (PBG_DefaultButtonShadowThickness(pb) && (border > 0))
            {
               ...
               width                      = pb->rectangle.width - 2 * delta;
               height                     = pb->rectangle.height - 2 * delta;
               XmeClearBorder(XtDisplay(pb), XtWindow(pb), x, y, width, height, border);
               break;
            }
   ...
   Cardinal *num_args)
{
   ...
   */
   ...
   _XmPushWidgetExtData(new_w, extData, ((XmPushButtonGCacheObject)newSec)->ext.extensionType);
   ...
   Cardinal *num_args)
{
   ...
   */
   ...
   if ((LabG_Pixmap(new_w) == XmUNSPECIFIED_PIXMAP) && (PBG_ArmPixmap(new_w) != XmUNSPECIFIED_PIXMAP))
   {
      XtWidgetProc resize;
   ...
   if ((LabG_LabelType(new_w) == XmPIXMAP) && (PBG_ArmPixmap(new_w) != XmUNSPECIFIED_PIXMAP))
   {
      if (request->rectangle.width == 0)
         new_w->rectangle.width = 0;
      if (request->rectangle.height == 0)
   ...
   if (PBG_DefaultButtonShadowThickness(new_w))
   {
      ...
      increase                       = 2 * PBG_DefaultButtonShadowThickness(new_w) + new_w->gadget.shadow_thickness + adjustment;
   ...
   new_w->gadget.event_mask = XmARM_EVENT | XmACTIVATE_EVENT | XmHELP_EVENT | XmFOCUS_IN_EVENT | XmFOCUS_OUT_EVENT | XmENTER_EVENT | XmLEAVE_EVENT
   ...
   Cardinal *num_args)
{
   ...
   */
   ...
   PBG_Cache(newParent)  = &(((XmPushButtonGCacheObject)newSec)->pushbutton_cache);
   PBG_Cache(refParent)  = &(((XmPushButtonGCacheObject)extData->reqWidget)->pushbutton_cache);
   ...
   Cardinal *num_args) /* unused */
{
   ...
   *  - Sankar 2/1/90.
   */
   if (PBG_DefaultButtonShadowThickness(new_w) != PBG_DefaultButtonShadowThickness(current))
      PBG_Compatible(new_w) = False;
   if (PBG_Compatible(new_w))
      PBG_DefaultButtonShadowThickness(new_w) = PBG_ShowAsDefault(new_w);
   ...
   if (PBG_DefaultButtonShadowThickness(new_w) != PBG_DefaultButtonShadowThickness(current))
   {
      if (PBG_DefaultButtonShadowThickness(new_w) > PBG_DefaultButtonShadowThickness(current))
      {
         if (PBG_DefaultButtonShadowThickness(current) > 0)
            increase = (2 * PBG_DefaultButtonShadowThickness(new_w) + new_w->gadget.shadow_thickness) - (2 * PBG_DefaultButtonShadowThickness(current) + current->gadget.shadow_thickness);
         else
            increase = (2 * PBG_DefaultButtonShadowThickness(new_w) + new_w->gadget.shadow_thickness);
      }
      else
      {
         if (PBG_DefaultButtonShadowThickness(new_w) > 0)
            increase = -((2 * PBG_DefaultButtonShadowThickness(current) + current->gadget.shadow_thickness) - (2 * PBG_DefaultButtonShadowThickness(new_w) + new_w->gadget.shadow_thickness));
         else
            increase = -(2 * PBG_DefaultButtonShadowThickness(current) + current->gadget.shadow_thickness);
      }
      increase += adjustment;
   ...
   if ((PBG_ArmPixmap(new_w) != PBG_ArmPixmap(current)) && (LabG_LabelType(new_w) == XmPIXMAP) && (PBG_Armed(new_w)))
      flag = TRUE;
   /* No unarm_pixmap but do have an arm_pixmap, use that. */
   if ((LabG_Pixmap(new_w) == XmUNSPECIFIED_PIXMAP) && (PBG_ArmPixmap(new_w) != XmUNSPECIFIED_PIXMAP))
   {
      LabG_Pixmap(new_w) = PBG_ArmPixmap(new_w);
      if (LabG_RecomputeSize(new_w) && request->rectangle.width == current->rectangle.width)
         new_w->rectangle.width = 0;
      if (LabG_RecomputeSize(new_w) && request->rectangle.height == current->rectangle.height)
         new_w->rectangle.width = 0;
      _XmCalcLabelGDimensions((Widget)new_w);
      {
         XtWidgetProc resize;
   ...
   if ((LabG_LabelType(new_w) == XmPIXMAP) && (PBG_ArmPixmap(new_w) != PBG_ArmPixmap(current)))
   {
   ...
   if ((PBG_FillOnArm(new_w) != PBG_FillOnArm(current)) && (PBG_Armed(new_w) == TRUE))
      flag = TRUE;
   if (!LabG_IsMenupane(new_w) || etched_in)
   {
   ...
   if ((flag == False) && XtIsRealized((Widget)new_w))
   {
      ...
      if ((PBG_ShowAsDefault(current) != 0) && (PBG_ShowAsDefault(new_w) == 0))
   ...
   Widget wid)
{
   ...
   XmDisplay          dpy       = (XmDisplay)XmGetXmDisplay(XtDisplay(wid));
   Boolean            etched_in = dpy->display.enable_etched_in_menu;
   if (PBG_Timer(pb) && ((XmPushButtonGadget)(pb))->pushbutton.cache->timer_widget == wid)
   {
   ...
   Mask               event_mask)
{
   ...
   else
   {
      if (event->type == ButtonRelease)
      {
         ...
         (*(((XmPushButtonGadgetClassRec *)(pb->object.widget_class))->gadget_class.arm_and_activate))((Widget)pb, event, NULL, NULL);
   }
}
   ...
   XmSecondaryResourceData **data_rtn)
{
   ...
   arrayCount = _XmSecondaryResourceData(bcePtr, data_rtn, NULL, NULL, NULL, GetPushBGClassSecResBase);
   _XmProcessUnlock();
   return (arrayCount);
}
   ...
   XmPushButtonGadget pb)
{
   ...
   dx     = pb->rectangle.x + pb->rectangle.width - (LabG_MarginRight(pb) + LabG_MarginWidth(pb) + pb->gadget.highlight_thickness + pb->gadget.shadow_thickness);
   dy     = pb->rectangle.y + pb->gadget.highlight_thickness + pb->gadget.shadow_thickness + LabG_MarginTop(pb) + LabG_MarginHeight(pb) + (MAX(LabG_TextRect(pb).height, LabG_AccTextRect(pb).height) - def_pixmap_height) / 2;
   ...
   XCopyPlane(XtDisplay(pb), def_pixmap, XtWindow(XtParent(pb)), LabG_NormalGC(pb), 0, 0, width, height, dx, dy, 1);
}
   ...
   XmPushButtonGadget pb)
{
   ...
   dx     = pb->rectangle.x + pb->rectangle.width - (LabG_MarginRight(pb) + LabG_MarginWidth(pb) + pb->gadget.highlight_thickness + pb->gadget.shadow_thickness);
   dy     = pb->rectangle.y + pb->gadget.highlight_thickness + pb->gadget.shadow_thickness + LabG_MarginTop(pb) + LabG_MarginHeight(pb);
   width  = LabG_MarginRight(pb);
   height = MAX(LabG_TextRect(pb).height, LabG_AccTextRect(pb).height);
   XClearArea(XtDisplay(pb), XtWindow(XtParent(pb)), dx, dy, width, height, False);
}
#endif /* DEFAULT_GLYPH_PIXMAP */
/*
   ...
   Region             region)
{
   ...
   XmDisplay dpy                    = (XmDisplay)XmGetXmDisplay(XtDisplay(pb));
   Boolean   etched_in              = dpy->display.enable_etched_in_menu;
   if (PBG_Armed(pb) && ((!LabG_IsMenupane(pb) && PBG_FillOnArm(pb)) || (LabG_IsMenupane(pb) && etched_in)))
   {
      if ((LabG_LabelType(pb) == XmSTRING || LabG_LabelType(pb) == XmPIXMAP_AND_STRING) && (PBG_ArmColor(pb) == LabG_Foreground(pb)))
      {
   ...
   Region             region)
{
   ...
   if (LabG_LabelType(pb) == XmPIXMAP || LabG_LabelType(pb) == XmPIXMAP_AND_STRING)
   {
      if (PBG_Armed(pb))
      {
   ...
   XmPushButtonGadget pb)
{
   ...
   if ((shadow_thickness > 0) && (top_gc) && (bottom_gc))
   {
      ...
      {
         adjust = (adjust << 1);
         dx     = pb->gadget.highlight_thickness + adjust + pb->gadget.shadow_thickness;
      }
      else
         dx = pb->gadget.highlight_thickness;
      if ((pb->rectangle.width > 2 * dx) && (pb->rectangle.height > 2 * dx))
      {
         XmeDrawShadows(XtDisplay(pb), XtWindow(pb), top_gc, bottom_gc, dx + pb->rectangle.x, dx + pb->rectangle.y, pb->rectangle.width - 2 * dx, pb->rectangle.height - 2 * dx, shadow_thickness, XmSHADOW_OUT);
      }
   }
}
   ...
   XmPushButtonGadget pb)
{
   ...
   if (default_button_shadow > 0)
   {
      ...
      height = pb->rectangle.height - 2 * delta;
      if ((width > 0) && (height > 0))
         XmeClearBorder(XtDisplay(pb), XtWindow(XtParent(pb)), x, y, width, height, default_button_shadow);
   }
#ifdef DEFAULT_GLYPH_PIXMAP
   else if (_XmGetDefaultGlyphPixmap(XtScreen((Widget)(pb)), NULL, NULL) != XmUNSPECIFIED_PIXMAP)
      EraseDefaultGlyphPixmap(pb);
#endif
}
   ...
   XmPushButtonGadget pb)
{
   ...
   */
   if ((default_button_shadow_thickness > 0) && (pb->rectangle.width > 2 * pb->gadget.highlight_thickness) && (pb->rectangle.height > 2 * pb->gadget.highlight_thickness))
   {
      int       x, y, width, height, delta;
      XmDisplay xm_dpy                  = (XmDisplay)XmGetXmDisplay(XtDisplay(pb));
      XtEnum    default_button_emphasis = xm_dpy->display.default_button_emphasis;
   ...
   LRectangle        *box)
{
   ...
   int     dx, adjust;
   short   fill = 0;
   if ((PBG_ArmColor(pb) == LabG_TopShadowColor(pb)) || (PBG_ArmColor(pb) == LabG_BottomShadowColor(pb)))
      fill = 1;
   if (pb == NULL)
      result = False;
   else
...
ExportHighlightThickness(
   ...
   XtArgVal *value)
{
   if (PBG_DefaultButtonShadowThickness(widget) || PBG_ShowAsDefault(widget))
   {
   ...
   XmPushButtonGadget newpb)
{
   ...
   {
      XmeGetPixmapData(XtScreen(newpb), LabG_Pixmap(newpb), NULL, NULL, NULL, NULL, NULL, NULL, &onW, &onH);
      XmeGetPixmapData(XtScreen(newpb), PBG_ArmPixmap(newpb), NULL, NULL, NULL, NULL, NULL, NULL, &onW2, &onH2);
