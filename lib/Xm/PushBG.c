...
static void Arm(XmPushButtonGadget pb, XEvent *event);
static void Activate(XmPushButtonGadget pb, XEvent *event);
...
static void Disarm(XmPushButtonGadget pb, XEvent *event);
static void BtnDown(XmPushButtonGadget pb, XEvent *event);
static void BtnUp(Widget wid, XEvent *event);
static void Enter(Widget wid, XEvent *event);
static void Leave(Widget wid, XEvent *event);
static void BorderHighlight(Widget wid);
static void DrawBorderHighlight(Widget wid);
static void BorderUnhighlight(Widget wid);
static void KeySelect(Widget wid, XEvent *event);
...
static void GetFillGC(XmPushButtonGadget pb);
...
static void Help(XmPushButtonGadget pb, XEvent *event);
static void Destroy(Widget wid);
static void Resize(Widget wid);
static void ActivateCommonG(XmPushButtonGadget pb, XEvent *event, Mask event_mask);
static Cardinal GetPushBGClassSecResData(WidgetClass w_class, XmSecondaryResourceData **data_rtn);
static XtPointer GetPushBGClassSecResBase(Widget widget, XtPointer client_data);
static void EraseDefaultButtonShadow(XmPushButtonGadget pb);
static void DrawDefaultButtonShadow(XmPushButtonGadget pb);
static int AdjustHighLightThickness(XmPushButtonGadget new_w, XmPushButtonGadget current);
static void Redisplay(Widget wid, XEvent *event, Region region);
static void DrawPushButtonLabelGadget(XmPushButtonGadget pb, XEvent *event, Region region);
static void DrawLabelGadget(XmPushButtonGadget pb, XEvent *event, Region region);
static void DrawPushButtonGadgetShadows(XmPushButtonGadget pb);
static void DrawPBGadgetShadows(XmPushButtonGadget pb);
static void EraseDefaultButtonShadows(XmPushButtonGadget pb);
static void DrawDefaultButtonShadows(XmPushButtonGadget pb);
static XmImportOperator ShowAsDef_ToHorizPix(Widget widget, int offset, XtArgVal *value);
static Boolean ComputePBLabelArea(XmPushButtonGadget pb, LRectangle *box);
static void ExportHighlightThickness(Widget widget, int offset, XtArgVal *value);
static void SetPushButtonSize(XmPushButtonGadget newpb);
...
externaldef(xmpushbuttongadgetclassrec) XmPushButtonGadgetClassRec xmPushButtonGadgetClassRec = {
    {
...
externaldef(xmpushbuttongadgetclass)
 ...
 *
 *******************************************************************/
static int _XmPushBCacheCompare(XtPointer A, XtPointer B)
{
  XmPushButtonGCacheObjPart *a_inst = (XmPushButtonGCacheObjPart *)A;
  XmPushButtonGCacheObjPart *b_inst = (XmPushButtonGCacheObjPart *)B;
  if ((a_inst->fill_on_arm == b_inst->fill_on_arm) && (a_inst->arm_color == b_inst->arm_color) &&
      (a_inst->arm_pixmap == b_inst->arm_pixmap) &&
      (a_inst->unarm_pixmap == b_inst->unarm_pixmap) && (a_inst->fill_gc == b_inst->fill_gc) &&
      (a_inst->background_gc == b_inst->background_gc) &&
      (a_inst->multiClick == b_inst->multiClick) &&
      (a_inst->default_button_shadow_thickness == b_inst->default_button_shadow_thickness) &&
      (a_inst->timer == b_inst->timer) && (a_inst->timer_widget == b_inst->timer_widget))
    return 1;
  else
    return 0;
}
...
static void InputDispatch(Widget wid, XEvent *event, Mask event_mask)
{
  ...
  if ((event_mask & XmARM_EVENT) ||
      ((PBG_MultiClick(pb) == XmMULTICLICK_KEEP) && (event_mask & XmMULTI_ARM_EVENT)))
  {
...
static void Activate(XmPushButtonGadget pb, XEvent *event)
{
  ...
  XtExposeProc expose;
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(XtParent(pb)), XmQTmenuSystem);
  PBG_Armed(pb) = FALSE;
  _XmProcessLock();
  expose = ((XmPushButtonGadgetClassRec *)(pb->object.widget_class))->rect_class.expose;
  _XmProcessUnlock();
...
static void BtnDown(XmPushButtonGadget pb, XEvent *event)
{
  ...
  else if ((pb->rectangle.width > 2 * pb->gadget.highlight_thickness) &&
           (pb->rectangle.height > 2 * pb->gadget.highlight_thickness))
    XmeDrawShadows(XtDisplay(pb),
                   XtWindow(pb),
                   LabG_TopShadowGC(pb),
...
static void DrawBorderHighlight(Widget wid)
{
  ...
  if (highlight_width > 0) {
    ...
    switch (default_button_emphasis) {
      ...
      case XmINTERNAL_HIGHLIGHT:
        if (PBG_DefaultButtonShadowThickness(pb))
          delta = Xm3D_ENHANCE_PIXEL + 2 * (PBG_Compatible(pb) ?
    ...
    XmeDrawHighlight(
        XtDisplay(pb), XtWindow(pb), LabG_HighlightGC(pb), x, y, width, height, highlight_width);
  }
}
...
static void InitializePosthook(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
   ...
   */
  _XmProcessLock();
  LabG_Cache(pbw) = (XmLabelGCacheObjPart *)_XmCachePart(
      LabG_ClassCachePart(pbw), (XtPointer)LabG_Cache(pbw), sizeof(XmLabelGCacheObjPart));
  PBG_Cache(pbw) = (XmPushButtonGCacheObjPart *)_XmCachePart(
      PBG_ClassCachePart(pbw), (XtPointer)PBG_Cache(pbw), sizeof(XmPushButtonGCacheObjPart));
  /*
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  new_w->gadget.event_mask = XmARM_EVENT | XmACTIVATE_EVENT | XmHELP_EVENT | XmFOCUS_IN_EVENT |
    ...
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  /* assign if changed! */
  if (!_XmPushBCacheCompare((XtPointer)PBG_Cache(new_w), (XtPointer)PBG_Cache(current))) {
    _XmCacheDelete((XtPointer)PBG_Cache(current));
    PBG_Cache(new_w) = (XmPushButtonGCacheObjPart *)_XmCachePart(
        PBG_ClassCachePart(new_w), (XtPointer)PBG_Cache(new_w), sizeof(XmPushButtonGCacheObjPart));
  }
  else
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (PBG_DefaultButtonShadowThickness(new_w) != PBG_DefaultButtonShadowThickness(current)) {
    if (PBG_DefaultButtonShadowThickness(new_w) > PBG_DefaultButtonShadowThickness(current)) {
      ...
      if (PBG_DefaultButtonShadowThickness(new_w) > 0)
        increase = -(
            (2 * PBG_DefaultButtonShadowThickness(current) + current->gadget.shadow_thickness) -
            (2 * PBG_DefaultButtonShadowThickness(new_w) + new_w->gadget.shadow_thickness));
      else
  ...
  if (LabG_IsMenupane(pb)) {
    ...
    if (event->type == ButtonRelease) {
      ...
      (*(((XmPushButtonGadgetClassRec *)(pb->object.widget_class))
             ->gadget_class.arm_and_activate))((Widget)pb, event, NULL, NULL);
  }
}
...
static Cardinal GetPushBGClassSecResData(WidgetClass w_class, XmSecondaryResourceData **data_rtn)
{
  ...
  arrayCount = _XmSecondaryResourceData(
      bcePtr, data_rtn, NULL, NULL, NULL, GetPushBGClassSecResBase);
  _XmProcessUnlock();
  return (arrayCount);
}
 ...
 */
/*ARGSUSED*/
static XtPointer GetPushBGClassSecResBase(Widget widget, XtPointer client_data) /* unused */
{
  ...
  char *cp;
  widgetSecdataPtr = (XtPointer)XtMalloc(labg_cache_size + pushbg_cache_size + 1);
  _XmProcessLock();
  if (widgetSecdataPtr) {
 ...
 */
static void DrawDefaultGlyphPixmap(XmPushButtonGadget pb)
{
  int dx, dy, width, height;
  Pixmap def_pixmap;
  unsigned int def_pixmap_width, def_pixmap_height;
  def_pixmap = _XmGetDefaultGlyphPixmap(
      XtScreen((Widget)(pb)), &def_pixmap_width, &def_pixmap_height);
  /* We draw in the margin right area here. */
  dx = pb->rectangle.x + pb->rectangle.width -
       (LabG_MarginRight(pb) + LabG_MarginWidth(pb) + pb->gadget.highlight_thickness +
        pb->gadget.shadow_thickness);
  dy = pb->rectangle.y + pb->gadget.highlight_thickness + pb->gadget.shadow_thickness +
       LabG_MarginTop(pb) + LabG_MarginHeight(pb) +
       (MAX(LabG_TextRect(pb).height, LabG_AccTextRect(pb).height) - def_pixmap_height) / 2;
  width = MIN(def_pixmap_width, LabG_MarginRight(pb));
  height = MIN(def_pixmap_height, MAX(LabG_TextRect(pb).height, LabG_AccTextRect(pb).height));
  XCopyPlane(XtDisplay(pb),
             def_pixmap,
 ...
 */
static void EraseDefaultGlyphPixmap(XmPushButtonGadget pb)
{
  int dx, dy, width, height;
...
static void DrawPushButtonLabelGadget(XmPushButtonGadget pb, XEvent *event, Region region)
{
  ...
  Boolean etched_in = dpy->display.enable_etched_in_menu;
  if (PBG_Armed(pb) &&
      ((!LabG_IsMenupane(pb) && PBG_FillOnArm(pb)) || (LabG_IsMenupane(pb) && etched_in)))
  {
    if ((LabG_LabelType(pb) == XmSTRING || LabG_LabelType(pb) == XmPIXMAP_AND_STRING) &&
...
static void EraseDefaultButtonShadows(XmPushButtonGadget pb)
{
  ...
  if (default_button_shadow > 0) {
    ...
    if ((width > 0) && (height > 0))
      XmeClearBorder(
          XtDisplay(pb), XtWindow(XtParent(pb)), x, y, width, height, default_button_shadow);
  }
#ifdef DEFAULT_GLYPH_PIXMAP
...
static void SetPushButtonSize(XmPushButtonGadget newpb)
{
  ...
  if (PBG_ArmPixmap(newpb) != XmUNSPECIFIED_PIXMAP) {
    XmeGetPixmapData(
        XtScreen(newpb), LabG_Pixmap(newpb), NULL, NULL, NULL, NULL, NULL, NULL, &onW, &onH);
    XmeGetPixmapData(
        XtScreen(newpb), PBG_ArmPixmap(newpb), NULL, NULL, NULL, NULL, NULL, NULL, &onW2, &onH2);
...
static void ShowAsDefault(Widget w, XtEnum state)
{
  ...
  switch (state) {
    case XmDEFAULT_READY: {
      ...
      if (_XmGetDefaultGlyphPixmap(XtScreen((Widget)(pb)), &def_pixmap_width, NULL) !=
...
Widget XmVaCreatePushButtonGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmPushButtonGadgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedPushButtonGadget(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmPushButtonGadgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
