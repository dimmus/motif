...
/****************************XmeDrawArrow**********************************/
void
XmeDrawArrow(Display *display, Drawable d, GC top_gc, GC bot_gc, GC cent_gc, Position x, Position y, Dimension width, Dimension height, Dimension shadow_thick, unsigned char direction)
{
...
#define SWAP(x, y) temp = x ; x = y; y = temp
   ...
   if (shadow_thick)
   { /* 1 or 2 shadow thickness: always draw
   ...
   if (shadow_thick == 1)
   {
	  ...
	  flat look */
      XmeDrawArrow(display, d, top_gc, bot_gc, cent_gc, x + 1, y + 1, width - 2, height - 2, 0, direction);
   }
   else if (cent_gc)
      XFillRectangles(display, d, cent_gc, cent, c);
   _XmProcessUnlock();
   _XmAppUnlock(app);
}
