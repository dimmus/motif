...
#define DELAY_DEFAULT 100
    ...
    static void
    Arm(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void MultiArm(Widget wid, XEvent *event, String *params, Cardinal *num_params);
static void Activate(Widget wid, XEvent *buttonEvent, String *params, Cardinal *num_params);
static void MultiActivate(Widget wid, XEvent *buttonEvent, String *params, Cardinal *num_params);
static void ActivateCommon(Widget wid, XEvent *event, String *params, Cardinal *num_params);
...
static void KeySelect(Widget wid, XEvent *event, String *params, Cardinal *num_params);
...
static void GetFillGC(XmPushButtonWidget pb);
static void GetBackgroundGC(XmPushButtonWidget pb);
static Boolean SetValuesPrehook(Widget cw, Widget rw, Widget nw, ArgList args, Cardinal *num_args);
...
static void EraseDefaultButtonShadow(XmPushButtonWidget pb);
static void Redisplay(Widget wid, XEvent *event, Region region);
static void DrawPushButtonBackground(XmPushButtonWidget pb);
static void DrawPushButtonLabel(XmPushButtonWidget pb, XEvent *event, Region region);
static void DrawPushButtonShadows(XmPushButtonWidget pb);
static void ComputePBLabelArea(XmPushButtonWidget pb, XRectangle *box);
static void DrawPBPrimitiveShadows(XmPushButtonWidget pb);
static void DrawDefaultButtonShadows(XmPushButtonWidget pb);
static XmImportOperator ShowAsDef_ToHorizPix(Widget widget, int offset, XtArgVal *value);
static int AdjustHighLightThickness(XmPushButtonWidget new_w, XmPushButtonWidget current);
static void ExportHighlightThickness(Widget widget, int offset, XtArgVal *value);
static void FillBorderWithParentColor(
    XmPushButtonWidget pb, int borderwidth, int dx, int dy, int rectwidth, int rectheight);
static void SetPushButtonSize(XmPushButtonWidget newtb);
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  if ((new_w->label.label_type == XmPIXMAP || new_w->label.label_type == XmPIXMAP_AND_STRING) &&
      (new_w->pushbutton.arm_pixmap != XmUNSPECIFIED_PIXMAP))
  {
    if (request->core.width == 0)
      new_w->core.width = 0;
 ...
 */
static void DrawDefaultGlyphPixmap(XmPushButtonWidget pb)
{
  ...
  dx = pb->core.width - (Lab_MarginRight(pb) + Lab_MarginWidth(pb) +
                         pb->primitive.highlight_thickness + pb->primitive.shadow_thickness);
  dy = pb->primitive.highlight_thickness + pb->primitive.shadow_thickness + Lab_MarginTop(pb) +
       Lab_MarginHeight(pb) +
       (MAX(Lab_TextRect_height(pb), pb->label.acc_TextRect.height) - def_pixmap_height) / 2;
  width = MIN(def_pixmap_width, Lab_MarginRight(pb));
  height = MIN(def_pixmap_height, MAX(Lab_TextRect_height(pb), pb->label.acc_TextRect.height));
 ...
 */
static void EraseDefaultGlyphPixmap(XmPushButtonWidget pb)
{
  int dx, dy, width, height;
  ...
  if (XtIsRealized((Widget)pb)) {
    if (Lab_IsMenupane(pb)) {
      ...
      if (pb->pushbutton.armed)
        (*(((XmPushButtonWidgetClass)XtClass(pb))->primitive_class.border_highlight))((Widget)pb);
    }
    else {
...
static void DrawPushButtonLabel(XmPushButtonWidget pb, XEvent *event, Region region)
{
  ...
  Boolean etched_in = dpy->display.enable_etched_in_menu;
  if (pb->pushbutton.armed &&
      ((!Lab_IsMenupane(pb) && pb->pushbutton.fill_on_arm) || (Lab_IsMenupane(pb) && etched_in)))
  {
    XSetWindowBackground(XtDisplay(pb), XtWindow(pb), pb->pushbutton.arm_color);
    replaceBg = True;
    if ((pb->label.label_type == XmSTRING || pb->label.label_type == XmPIXMAP_AND_STRING) &&
  ...
  switch (default_button_emphasis) {
    case XmEXTERNAL_HIGHLIGHT:
      adjust = (pb->primitive.highlight_thickness -
                (pb->pushbutton.default_button_shadow_thickness ? Xm3D_ENHANCE_PIXEL : 0));
      break;
    case XmINTERNAL_HIGHLIGHT:
      adjust = 0;
      break;
    ...
    switch (default_button_emphasis) {
      case XmINTERNAL_HIGHLIGHT:
        ...
        if (pb->primitive.highlight_drawn)
          (*(((XmPushButtonWidgetClass)XtClass(pb))->primitive_class.border_highlight))(
              (Widget)pb);
        break;
      default:
        break;
...
static void DrawBorderHighlight(Widget wid)
{
  ...
  if (highlight_width > 0) {
    ...
    switch (default_button_emphasis) {
      ...
      case XmINTERNAL_HIGHLIGHT:
        if (pb->pushbutton.default_button_shadow_thickness)
          delta = Xm3D_ENHANCE_PIXEL + 2 * (pb->pushbutton.compatible ?
 ...
 *
 ************************************************************************/
static void SetPushButtonSize(XmPushButtonWidget newpb)
{
  ...
  if (newpb->pushbutton.arm_pixmap != XmUNSPECIFIED_PIXMAP) {
    if (newpb->label.pixmap != XmUNSPECIFIED_PIXMAP)
      XmeGetPixmapData(
          XtScreen(newpb), newpb->label.pixmap, NULL, NULL, NULL, NULL, NULL, NULL, &onW, &onH);
    XmeGetPixmapData(XtScreen(newpb),
...
/*ARGSUSED*/
static void MultiArm(Widget wid,
                     XEvent *event,
                     String *params,       /* unused */
                     Cardinal *num_params) /* unused */
{
...
/*ARGSUSED*/
static void ActivateCommon(Widget wid,
                           XEvent *event,
                           String *params,       /* unused */
                           Cardinal *num_params) /* unused */
{
                           ...
                           Cardinal *num_params) /* unused */
{
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass((Widget)XtParent(pb)),
                                              XmQTmenuSystem);
 ...
 ************************************************************************/
/*ARGSUSED*/
static void BtnDown(Widget wid,
                    XEvent *event,
                    String *params,       /* unused */
                    Cardinal *num_params) /* unused */
{
 ...
 *************************************<->***********************************/
/*ARGSUSED*/
static void KeySelect(Widget wid,
                      XEvent *event,
                      String *params,       /* unused */
                      Cardinal *num_params) /* unused */
{
...
Widget XmVaCreatePushButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmPushButtonWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedPushButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmPushButtonWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
