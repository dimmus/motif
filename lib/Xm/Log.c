...
static unsigned int XmLogPidGet(void)
{
  return (unsigned int)getpid();
}

static const char *XmLogDomainStrGet(const char *name, const char *color)
{
  if (color) {
...
// Domain creation
static XmLogDomain *XmLogDomainNew(XmLogDomain *d, const char *name, const char *color)
{
...
// Domain cleanup
static void XmLogDomainFree(XmLogDomain *d)
{
...
// Domain registration
static int XmLogDomainRegisterUnlocked(const char *name, const char *color)
{
  unsigned int i;
...
static inline void XmLogPrintUnlocked(int domain,
                                      XmLogLevel level,
                                      const char *file,
                                      const char *fnc,
                                      int line,
                                      ...
                                      va_list args)
{
  ...
  if ((unsigned int)domain >= _log_domains_count || domain < 0) {
    fprintf(stderr,
            "CRI<%u>:XmLog %s:%d %s() unknown log domain %d\n",
            XmLogPidGet(),
            file,
            line,
            fnc,
  ...
  if (d->deleted) {
    fprintf(stderr,
            "ERR<%u>:XmLog %s:%d %s() log domain %d was deleted\n",
            XmLogPidGet(),
            file,
            line,
            fnc,
                        ...
                        va_list args)
{
  ...
  fprintf(stderr, "<%u>:%s %s:%d %s() ", XmLogPidGet(), d->name, file, line, fnc);
  if (domain_color) {
    fputs(reset_color, stderr);
  }
                        ...
                        va_list args)
{
  ...
  fprintf(stdout, "<%u>:%s %s:%d %s() ", XmLogPidGet(), d->name, file, line, fnc);
  if (domain_color)
    fputs(reset_color, stdout);
                      ...
                      va_list args)
{
  ...
  fprintf(f, "<%u>:%s %s:%d %s() ", XmLogPidGet(), d->name, file, line, fnc);
  if (domain_color)
    fputs(reset_color, f);
