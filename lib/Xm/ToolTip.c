...
static void
ToolTipLabelDestroyCallback(Widget w, XtPointer client_data, XtPointer call_data)
{
            ...
            XtIntervalId *id)
{
   ...
   XtVaSetValues(XtParent(ttp->label),
                 ...
                 1,
                 NULL);
   ttp->slider = XtVaCreateWidget("ToolTipSlider", xmSlideContextWidgetClass, XmGetXmDisplay(XtDisplay(w)), XmNslideWidget, XtParent(ttp->label), XmNslideDestX, destX, XmNslideDestY, destY, XmNslideDestWidth, geo.width, XmNslideDestHeight, geo.height, NULL);
                ...
                Cardinal *num_params)
{
   ...
   if ((XmIsPrimitive(wid) || XmIsGadget(wid)) && XmGetToolTipString(wid))
   {
      if (ttp->enable && !ttp->timer)
      {
         ...
         ttp->timer = XtAppAddTimeOut(XtWidgetToApplicationContext(wid), delay, (XtTimerCallbackProc)ToolTipPost, wid);
      }
   }
}
