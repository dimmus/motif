...
#endif
#define XmTextFieldGetString(t) (XmIsTextField(t) ? XmTextFieldGetString(t) : XmTextGetString(t))
...
#define XmTextFieldSetInsertionPosition(t, p) \
  (XmIsTextField(t) ? XmTextFieldSetInsertionPosition(t, p) : XmTextSetInsertionPosition(t, p))
/* From GrabShell.c */
...
static void FindDesiredSize(Widget,
                            Widget,
                            Dimension *,
                            Dimension *,
                            XtWidgetGeometry *,
                            XtWidgetGeometry *,
                            XtWidgetGeometry *);
static void GetTextAndLabelWidth(
    Widget, Dimension, Dimension, Dimension, Dimension, Dimension *, Dimension *);
...
static XtResource resources[] = {
    ...
    {XmNverify,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmDropDownRec, combo.verify),
     XmRImmediate,
     (XtPointer)True},
    {XmNeditable,
     XmCEditable,
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmDropDownRec, combo.editable),
     XmRImmediate,
     (XtPointer)True},
    {XmNcustomizedCombinationBox,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmDropDownRec, combo.customized_combo_box),
     XmRImmediate,
     (XtPointer)False},
    {XmNverifyTextCallback,
     ...
     XtOffsetOf(XmDropDownRec, combo.verify_text_callback),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNupdateTextCallback,
     ...
     XmRCallback,
     sizeof(XtCallbackList),
     XtOffsetOf(XmDropDownRec, combo.update_text_callback),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNupdateShellCallback,
     ...
     XmRCallback,
     sizeof(XtCallbackList),
     XtOffsetOf(XmDropDownRec, combo.update_shell_callback),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNpopupShellWidget,
     XmCWidget,
     XmRWidget,
     sizeof(Widget),
     XtOffsetOf(XmDropDownRec, combo.popup_shell),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNshowLabel,
     XmCBoolean,
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmDropDownRec, combo.show_label),
     XmRImmediate,
     (XtPointer)True},
    {XmNpopupOffset,
     XmCPopupOffset,
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmDropDownRec, combo.popup_offset),
     XmRImmediate,
     (XtPointer)15},
    {XmNpopupCursor,
     XmCCursor,
     XmRCursor,
     sizeof(Cursor),
     XtOffsetOf(XmDropDownRec, combo.popup_cursor),
     XmRString,
     (XtPointer) "left_ptr"},
    {XmNuseTextField,
     XmCUseTextField,
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmDropDownRec, combo.use_text_field),
     XmRImmediate,
     (XtPointer)True},
    {XmNcomboTranslations,
     ...
     XtOffsetOf(XmDropDownRec, combo.translations),
     XmRString,
     (XtPointer)combo_translations},
    {XmNvisibleItemCount,
     XmCVisibleItemCount,
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmDropDownRec, combo.visible_items),
     XmRImmediate,
     (XtPointer)5},
    {XmNnewVisualStyle,
     XmCNewVisualStyle,
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmDropDownRec, combo.new_visual_style),
     XmRImmediate,
     (XtPointer)True},
    {XmNshadowThickness,
     XmCShadowThickness,
    ...
    {XmNautoTraversal,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmDropDownRec, combo.autoTraversal),
     XmRImmediate,
     (XtPointer)True},
#endif
    ...
    {XmNactivateOnFill,
     ...
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmDropDownRec, combo.activateOnFill),
     XmRImmediate,
     (XtPointer)0},
#endif
};
...
#undef offset
XmDropDownClassRec xmDropDownClassRec = {{/* core fields */
 ...
 */
/*ARGSUSED*/
static void Initialize(Widget req, Widget set, ArgList args, Cardinal *num_args)
{
...
static Boolean SetValues(
    Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
  Boolean place_children = False;
  XmDropDownWidget old_cbw = (XmDropDownWidget)current, set_cbw = (XmDropDownWidget)set;
  ArgList f_args;
  Cardinal f_num_args;
  ...
  if (XmDropDown_editable(set_cbw) != XmDropDown_editable(old_cbw)) {
    if (!XmDropDown_editable(set_cbw))
      ...
      XtRemoveEventHandler(
          XmDropDown_text(set_cbw), ButtonPressMask, False, TextButtonPress, NULL);
  }
 ...
 */
static void ExposeMethod(Widget wid, XEvent *event, Region r)
{
  Dimension x, y, w, h;
  ...
  w = text->core.width + arrow->core.width +
      2 * (text->core.border_width + arrow->core.border_width) + 2 * cbw->manager.shadow_thickness;
  h = text->core.height + 2 * text->core.border_width + 2 * cbw->manager.shadow_thickness;
  XmeDrawShadows(XtDisplay(cbw),
                 XtWindow(cbw),
 ...
 *	Returns:       none.
 */
static void CheckUnpost(Widget w, XtPointer combo_ptr, XtPointer info_ptr)
{
 ...
 *	Returns:       none.
 */
static void ArrowClicked(Widget w, XtPointer combo_ptr, XtPointer info_ptr)
{
  XmDropDownWidget cbw = (XmDropDownWidget)combo_ptr;
  Arg args[10];
  Cardinal num_args;
  ...
  if (is_unposted) {
    ...
    if ((success = PopupList((Widget)cbw)) == True) {
      ...
      if (!XmDropDown_customized_combo_box(cbw) && !SetListFromText((Widget)cbw, False) &&
 ...
 *	Returns:       none.
 */
static void PopdownDone(Widget w, XtPointer combo_ptr, XtPointer info_ptr)
{
  XmDropDownWidget cbw = (XmDropDownWidget)combo_ptr;
 ...
 */
static XmDropDownClassPartExtension *CheckExtensions(XmDropDownWidgetClass combo)
{
 ...
 */
/*ARGSUSED*/
static void VerifyTextField(Widget w, XtPointer combo_ptr, XtPointer info_ptr)
{
  XmDropDownWidget cbw = (XmDropDownWidget)combo_ptr;
  ...
  if (!XmDropDown_customized_combo_box(cbw)) {
    if (XmDropDown_verify(cbw) && !IsTextOK(cbw)) {
      ...
      XtCallCallbackList(
          (Widget)cbw, XmDropDown_verify_text_failed_callback(cbw), (XtPointer)&cbdata);
    ...
    /* else blindly accept the value */
      ...
      XmDropDownClassPartExtension *addition = CheckExtensions(
          (XmDropDownWidgetClass)XtClass(cbw));
      if (addition && addition->update) {
        char *text = XmTextFieldGetString(XmDropDown_text(cbw));
        (void)(*addition->update)((Widget)cbw, text);
        XtFree(text);
  ...
  if (w != NULL) {
    ...
    cbdata.event = (field == NULL ? NULL : field->event);
    XtCallCallbackList((Widget)cbw, XmDropDown_verify_text_callback(cbw), (XtPointer)&cbdata);
  }
 ...
 */
/*ARGSUSED*/
static void ModifyVerifyTextField(Widget w, XtPointer combo_ptr, XtPointer info_ptr)
{
  XmDropDownWidget cbw = (XmDropDownWidget)combo_ptr;
  XmTextVerifyCallbackStruct *field = (XmTextVerifyCallbackStruct *)info_ptr;
  if (XmDropDown_activateOnFill(cbw) <= 0)
    return;
  if (field == NULL || field->event == NULL || field->event->type != KeyPress)
    return;
  ...
  if (field->currInsert != field->endPos)
    return;
  if (field->currInsert != field->startPos)
    return;
 ...
 */
/*ARGSUSED*/
static void ValueChangedTextField(Widget w, XtPointer combo_ptr, XtPointer info_ptr)
{
  XmDropDownWidget cbw = (XmDropDownWidget)combo_ptr;
  if (XmDropDown_inValueChanged(cbw))
    return;
 ...
 */
/* ARGSUSED */
static void ListSelected(Widget w, XtPointer cbw_ptr, XtPointer list_data_ptr)
{
  ...
  if ((list_data->reason == XmCR_BROWSE_SELECT) &&
      ((list_data->event == NULL) || ((list_data->event->xany.type != ButtonPress) &&
 ...
 */
/* ARGSUSED */
static void ShellButtonEvent(Widget w, XtPointer cbw_ptr, XEvent *event, Boolean *dispatch)
{
  XmDropDownWidget cbw = (XmDropDownWidget)cbw_ptr;
  Widget event_widget;
  switch (event->type) {
    ...
    case ButtonPress:
      ...
      if ((cbw->combo.vsb && XtIsRealized(cbw->combo.vsb) &&
           (event->xbutton.window == XtWindow(cbw->combo.vsb))) ||
          (cbw->combo.hsb && XtIsRealized(cbw->combo.hsb) &&
 ...
 */
/*ARGSUSED*/
static void ComboUnpost(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
 ...
 */
/*ARGSUSED*/
static void ComboPost(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
  XmDropDownWidget cbw = FindComboBox(w);
  if (cbw == NULL)
    return;
 ...
 */
/*ARGSUSED*/
static void ComboCancel(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
  Arg args[10];
  Cardinal num_args;
  XmDropDownWidget cbw = FindComboBox(w);
 ...
 */
static XmDropDownWidget FindComboBox(Widget w)
{
 ...
 *	Returns: none.
 */
static void RegisterShellHandler(Widget w)
{
  XmDropDownWidget cbw = (XmDropDownWidget)w;
 ...
 *	Returns:       none
 */
static void PlaceChildren(Widget w, Boolean allow_resize, Widget child)
{
  XmDropDownWidget cbw = (XmDropDownWidget)w;
  ...
  if (width != rwidth) {
    ...
    GetTextAndLabelWidth(
        w, rwidth, arrow_width, label_bw, text_geom.border_width, &text_width, &label_width);
  }
  child_height = rheight - 2 * XmDropDown_v_space(cbw);
  if (LayoutIsRtoLM(cbw)) {
    arrow_x = XmDropDown_h_space(cbw);
    label_x = (rwidth - (label_width + 2 * label_geom.border_width) - XmDropDown_h_space(cbw));
  ...
  _XmConfigureWidget(text,
                     text_x,
                     XmDropDown_v_space(cbw) + shadow,
                     text_width,
                     child_height - 2 * text_geom.border_width,
                     text_geom.border_width);
  _XmConfigureWidget(arrow,
                     arrow_x,
                     XmDropDown_v_space(cbw) + shadow,
                     arrow_geom.width,
 ...
 *	Returns:       none
 */
static void GetTextAndLabelWidth(Widget w,
                                 Dimension combo_width,
 ...
 *	Returns:       none
 */
static void FindDesiredSize(Widget w,
                            Widget child,
                            Dimension *width_ret,
                            Dimension *height_ret,
                            XtWidgetGeometry *label,
                            XtWidgetGeometry *text,
                            XtWidgetGeometry *arrow)
{
  ...
  *width_ret = (Dimension)(label_width + text_width + arrow_width + h_space);
  if (XmDropDown_show_label(cbw)) {
 ...
 */
static void CreateChildren(Widget w, ArgList args, Cardinal num_args)
{
  XmDropDownWidget cbw = (XmDropDownWidget)w;
  Arg largs[10];
  Cardinal num_largs;
  ...
  if (XmDropDown_use_text_field(cbw)) {
    ...
    XmDropDown_text(cbw) = XtCreateManagedWidget(
        "text", xmTextFieldWidgetClass, w, merge, num_args + tn);
    ...
    merge = XtMergeArgLists(args, num_args, targs, tn);
    XmDropDown_text(cbw) = XtCreateManagedWidget(
        "text", xmTextWidgetClass, w, merge, num_args + tn);
    XtFree((XtPointer)merge);
  }
  ...
  XtAddCallback(
      XmDropDown_text(cbw), XmNmodifyVerifyCallback, ModifyVerifyTextField, (XtPointer)cbw);
  XtAddCallback(
      XmDropDown_text(cbw), XmNvalueChangedCallback, ValueChangedTextField, (XtPointer)cbw);
  XtOverrideTranslations(XmDropDown_text(cbw), XmDropDown_translations(cbw));
  if (XmDropDown_new_visual_style(cbw)) {
    tn = 0;
    XtSetArg(targs[tn], XmNhighlightThickness, 0);
    tn++;
    ...
    tn++;
    merge = XtMergeArgLists(args, num_args, targs, tn);
    XmDropDown_arrow(cbw) = XtCreateManagedWidget(
        "arrow", xmArrowButtonWidgetClass, w, merge, num_args + tn);
    XtFree((XtPointer)merge);
  }
  else {
    tn = 0;
    ...
    tn++;
    XmDropDown_arrow(cbw) = XtCreateManagedWidget(
        "arrow", xmArrowButtonWidgetClass, w, args, num_args);
  }
  num_largs = 0;
  XtSetArg(largs[num_largs], XmNarrowDirection, XmARROW_DOWN);
  num_largs++;
  XtSetValues(XmDropDown_arrow(cbw), largs, num_largs);
  XtOverrideTranslations(XmDropDown_arrow(cbw), XmDropDown_translations(cbw));
 ...
 *	Returns:       none.
 */
static void CreatePopup(Widget w, ArgList args, Cardinal num_args)
{
  XmDropDownWidget cbw = (XmDropDownWidget)w;
  ...
  XmDropDown_popup_shell(cbw) = XtCreatePopupShell(
      "popupShell", xmGrabShellWidgetClass, w, new_list, num_largs + num_args);
  ...
  num_largs++;
  new_list = XtMergeArgLists(args, num_args, largs, num_largs);
  XmDropDown_list(cbw) = XmCreateScrolledList(
      XmDropDown_popup_shell(cbw), "list", new_list, num_args + num_largs);
  XtFree((char *)new_list);
  ...
  sb = cbw->combo.vsb;
  if (sb != (Widget)NULL) {
    XtInsertEventHandler(sb,
                         ButtonPressMask,
                         False,
                         SBBtnDownEH,
                         (XtPointer)XmDropDown_popup_shell(cbw),
                         XtListHead);
    XtInsertEventHandler(sb,
                         ButtonReleaseMask,
                         False,
...
static void TextButtonPress(Widget w, XtPointer client, XEvent *event, Boolean *go_on)
{
  XmDropDownWidget cbw = FindComboBox(w);
  if (cbw == NULL)
    return;
...
static Boolean PopupList(Widget w)
{
  ...
  if (!XmIsGrabShell(shell))
    XGetInputFocus(
        XtDisplay(shell), &(XmDropDown_focus_owner(cbw)), &(XmDropDown_focus_state(cbw)));
  /*
 ...
 *                     empty.
 */
static Boolean SetListFromText(Widget w, Boolean no_action)
{
  Arg args[10];
  Cardinal num_args;
 ...
 *                     empty.
 */
static Boolean SetTextFromList(Widget w)
{
  XmDropDownWidget cbw = (XmDropDownWidget)w;
  Arg args[10];
  Cardinal num_args;
 ...
 */
Widget XmCreateDropDown(Widget parent, String name, ArgList args, Cardinal num_args)
{
...
Widget XmVaCreateDropDown(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmDropDownWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedDropDown(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmDropDownWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
