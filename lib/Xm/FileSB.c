   ...
   Cardinal *num_args) /* unused */
{
   ...
   XtAddCallback(SB_ApplyButton(new_w), XmNactivateCallback, FileSelectionPB, (XtPointer)XmDIALOG_APPLY_BUTTON);
   XtAddCallback(SB_OkButton(new_w), XmNactivateCallback, FileSelectionPB, (XtPointer)XmDIALOG_OK_BUTTON);
   XtAddCallback(SB_CancelButton(new_w), XmNactivateCallback, FileSelectionPB, (XtPointer)XmDIALOG_CANCEL_BUTTON);
   XtAddCallback(SB_HelpButton(new_w), XmNactivateCallback, FileSelectionPB, (XtPointer)XmDIALOG_HELP_BUTTON);
   if (FS_NoMatchString(new_w) == (XmString)XmUNSPECIFIED)
   {
      FS_NoMatchString(new_w) = XmStringConcatAndFree(XmStringDirectionCreate(XmSTRING_DIRECTION_L_TO_R),
   ...
   Widget w)
{
   ...
   _XmProcessLock();
   delete_child = ((XmSelectionBoxWidgetClass)xmSelectionBoxWidgetClass)->composite_class.delete_child;
   _XmProcessUnlock();
   (*delete_child)(w);
   return;
}
         ...
         else
         {
            ...
            if (extension->filter_label
                ...
                && (fileListGeo->box.height >= ((extension->filter_label->box.height
                                                ...
                                                << 1)))
            {
               dirListGeo->box.height = (fileListGeo->box.height -= (extension->filter_label->box.height
                                                                     + extension->filter_text->box.height
      ...
      case XmGEO_PRE_SET:
      {
         if (FS_PathMode(geoSpec->composite) == XmPATH_MODE_FULL)
         {
            ...
            if (extension->file_list_label)
            {
               ...
               else if (LayoutIsRtoLM(geoSpec->composite) && extension->file_list_label->box.width > fileListGeo->box.width)
               {
                  extension->file_list_label->box.width
                     = fileListGeo->box.width;
               }
      ...
      while (Index < numFiles)
      {
             ...
             || ((fileList[Index])[dirLen] != '.'))
         {
            if (FS_PathMode(fs) == XmPATH_MODE_FULL)
               XmStringFileList[numItems++] = XmStringGenerate(fileList[Index],
                                                               XmFONTLIST_DEFAULT_TAG,
                                                               XmCHARSET_TEXT,
                                                               NULL);
            else
               XmStringFileList[numItems++] = XmStringGenerate(&(fileList[Index])[dirLen],
                                                               XmFONTLIST_DEFAULT_TAG,
                                                               XmCHARSET_TEXT,
                                                               NULL);
         }
   ...
   XtPointer qsd)
{
   ...
   else
   {
      ...
      qualifiedSearchData->value = XmStringGenerate(valueString, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
   }
   qualifiedSearchData->length      = XmStringLength(qualifiedSearchData->value);
   qualifiedSearchData->mask        = XmStringGenerate(qualifiedMask, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
   qualifiedSearchData->mask_length = XmStringLength(
      qualifiedSearchData->mask);
   qualifiedSearchData->dir        = XmStringGenerate(qualifiedDir, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
   qualifiedSearchData->dir_length = XmStringLength(
      qualifiedSearchData->dir);
   qualifiedSearchData->pattern        = XmStringGenerate(qualifiedPattern, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
   ...
   XmFileSelectionBoxCallbackStruct *searchData)
{
   ...
   (*FS_QualifySearchDataProc(fs))((Widget)fs, (XtPointer)searchData, (XtPointer)&qualifiedSearchData);
   ...
   XtPointer sd)
{
       ...
       || !XmStringCompare(searchData->dir, FS_Directory(fs)))
   {
      ...
      /* Directory is different than current, so update dir list.
        */
      dirList = NULL;
      _XmOSGetDirEntries(qualifiedDir, "*", XmFILE_DIRECTORY, FALSE, TRUE, &dirList, &numDirs, &numAlloc);
      if (!numDirs)
      {
      ...
      while (Index < numDirs)
      {
	     ...
	     */
         ...
         {
            if (FS_PathMode(fs) == XmPATH_MODE_FULL)
               XmStringDirList[numItems++] = XmStringGenerate(dirList[Index],
                                                              XmFONTLIST_DEFAULT_TAG,
                                                              XmCHARSET_TEXT,
                                                              NULL);
            else
               XmStringDirList[numItems++] = XmStringGenerate(&(dirList[Index])[dirLen],
                                                              XmFONTLIST_DEFAULT_TAG,
                                                              XmCHARSET_TEXT,
                                                              NULL);
         }
   ...
   switch (callback->reason)
   {
      ...
      case XmCR_SINGLE_SELECT:
      {
         if (wid == FS_DirList(fsb))
         {
            ...
            change_data.mask        = XmStringGenerate(textValue, XmFONTLIST_DEFAULT_TAG, XmCHARSET_TEXT, NULL);
            ...
            if (FS_PathMode(fsb) == XmPATH_MODE_FULL)
            {
               ...
               {
                  if ((patternString = _XmStringGetTextConcat(qualified_change_data.pattern))
                      != NULL)
                  {
                     len        = strlen(dirString);
      ...
      case XmCR_DEFAULT_ACTION:
      {
		...
		*/
               (*(gadget_class->gadget_class.arm_and_activate))((Widget)dbutton, callback->event, NULL, NULL);
            }
         }
         break;
      }
   ...
   XtPointer call_data)
{
   ...
   switch ((long)which_button)
   {
      case XmDIALOG_OK_BUTTON:
      {
         ...
         if (!match)
         {
            ...
            XtCallCallbackList(((Widget)fs),
                               fs->selection_box.no_match_callback,
                               &qualifiedSearchData);
         }
         else
         {
      ...
      case XmDIALOG_APPLY_BUTTON:
      {
         ...
         XtCallCallbackList(((Widget)fs),
                            fs->selection_box.apply_callback,
                            &qualifiedSearchData);
         break;
      }
      case XmDIALOG_CANCEL_BUTTON:
      {
         ...
         XtCallCallbackList(((Widget)fs),
                            fs->selection_box.cancel_callback,
                            &qualifiedSearchData);
         allowUnmanage = TRUE;
         break;
      }
 ...
 ****************/
Widget
XmFileSelectionBoxGetChild(Widget fs, unsigned char which)
{
   Widget child;
