...
_XmGetNavigability(Widget wid)
{
   ...
   {
      XmBaseClassExt *er;
      if ((er = _XmGetBaseClassExtPtr(XtClass(wid), XmQmotif)) && (*er) && ((*er)->version >= XmBaseClassExtVersion) && (*er)->widgetNavigable)
      {
      ...
      else
      {
         ...
         if (XmIsPrimitive(wid))
            ...
            wc = NULL;
         if (wc && (er = _XmGetBaseClassExtPtr(wc, XmQmotif)) && (*er) && ((*er)->version >= XmBaseClassExtVersion) && (*er)->widgetNavigable)
         {
            return (*((*er)->widgetNavigable))(wid);
         }
      }
                          ...
                          XRectangle *visRect)
{
   ...
   if (wid && XmIsClipWindow(wid) && ((sw = (XmScrolledWindowWidget)XtParent(wid)) != NULL) && XmIsScrolledWindow(sw) && ((Widget)sw->swindow.ClipWindow == wid) && (!scrollable || sw->swindow.traverseObscuredCallback))
   {
      if (visRect)
      {
   ...
   */
        ...
        prev = wid)
   {
      ...
      else
      {
         ...
         {
            if (!_XmIntersectionOf(visRect, &widRect, &tmpRect) || (visRect->width != tmpRect.width) || (visRect->height != tmpRect.height))
            {
               _XmClearRect(visRect);
               return FALSE;
            }
...
{
   return (node && node->any.widget && (node->any.type != XmTAB_GRAPH_NODE) && (node->any.type != XmCONTROL_GRAPH_NODE) && XmIsTraversable(node->any.widget));
}
   ...
   */
   ...
   if ((list->current->any.widget != wid) && (list->current->any.type == XmTAB_GRAPH_NODE))
   {
      ...
      if ((nearest_node = GetNextNearestNode((XmGraphNode)list->current,
                                             &wid_rect,
                                             GetLayout(list->current->any.widget)))
          != NULL)
      {
   ...
   if ((list->current->any.widget == wid) || !NodeIsTraversable(list->current))
   {
       ...
       */
      if (((list->current->any.type != XmCONTROL_NODE) && (list->current->any.type != XmCONTROL_GRAPH_NODE)) || !(rtnNode = TraverseControl(list->current, XmTRAVERSE_RIGHT)))
      {
            ...
            Widget                reference_wid)
{
   ...
   */
   ...
   switch (action)
   {
      ...
      case XmTRAVERSE_GLOBALLY_BACKWARD:
         ...
         if (traverse_control)
            ...
            *local_dir = (action == XmTRAVERSE_GLOBALLY_FORWARD ? XmTRAVERSE_NEXT_TAB_GROUP : XmTRAVERSE_PREV_TAB_GROUP);
         break;
      default:
   ...
   else
   {
      ...
      if (!rtnNode && ((action == XmTRAVERSE_GLOBALLY_FORWARD) || (action == XmTRAVERSE_GLOBALLY_BACKWARD)))
      {
             ...
             Cardinal *num_in_list)
{
   ...
   if (XmIsManager(composite) && (mext = (XmManagerClassExt *)_XmGetClassExtensionPtr((XmGenericClassExt *)&(((XmManagerWidgetClass)XtClass(composite))->manager_class.extension), NULLQUARK)) && *mext && (*mext)->traversal_children)
   {
   ...
   */
   ...
   else /* XmDESCENDANTS_NAVIGABLE or XmDESCENDANTS_TAB_NAVIGABLE. */
   {
      ...
      for (i = 0; i < num_trav_children; i++)
         GetNodeList(trav_children[i], list_entry_rect, trav_list, list_entry_offset, controls_graph_offset);
      if (free_child_list)
             ...
             Boolean         horizontal)
{
   ...
   if (horizontal)
   {
      ...
      if (((nodeA->any.rect.y + nodeA->any.rect.height) < Bcent) && (Acent < (Dimension)nodeB->any.rect.y))
         return False;
      else if (((nodeB->any.rect.y + nodeB->any.rect.height) < Acent) && (Bcent < (Dimension)nodeA->any.rect.y))
         return False;
   }
   else
   {
      ...
      if (((nodeA->any.rect.x + nodeA->any.rect.width) < Bcent) && (Acent < (Dimension)nodeB->any.rect.x))
         return False;
      else if (((nodeB->any.rect.x + nodeB->any.rect.width) < Acent) && (Bcent < (Dimension)nodeA->any.rect.x))
         return False;
   }
   return True;
}
              ...
              XmDirection     layout)
{
   if (horizontal)
   {
      if (XmDirectionMatchPartial(layout, XmTOP_TO_BOTTOM, XmVERTICAL_MASK))
         ...
         return ((node_1->any.rect.y + node_1->any.rect.height) > (node_2->any.rect.y + node_2->any.rect.height));
   }
   else
   {
      if (XmDirectionMatchPartial(layout, XmLEFT_TO_RIGHT, XmHORIZONTAL_MASK))
         ...
         return ((node_1->any.rect.x + node_1->any.rect.width) > (node_2->any.rect.x + node_2->any.rect.width));
   }
}
     ...
     XmDirection      layout)
{
   ...
   while (++first_free < n_mem)
   {
      ...
      while (!done && (row < num_rows))
      {
         ...
         if (new_row)
         {
            ...
            if (horizontal)
            {
               might_overlap = (rows[row].max_hint > node->any.rect.y) && (rows[row].min_hint < (node->any.rect.y + node->any.rect.height));
            }
            else
            {
               might_overlap = (rows[row].max_hint > node->any.rect.x) && (rows[row].min_hint < (node->any.rect.x + node->any.rect.width));
            }
         }
         ...
         else if ((row_len > 1) && might_overlap && NodeDominates(node, rows[row].items[row_len - 1], horizontal, layout))
            row_len--;
         /* Try the next row. */
         else
         {
   ...
   first_free = 0;
   if ((horizontal && XmDirectionMatchPartial(layout, XmLEFT_TO_RIGHT, XmHORIZONTAL_MASK)) || (!horizontal && XmDirectionMatchPartial(layout, XmTOP_TO_BOTTOM, XmVERTICAL_MASK)))
   {
                    ...
                    XmConst void *B)
{
   ...
   if ((nodeA->any.rect.x + nodeA->any.rect.width) != (nodeB->any.rect.x + nodeB->any.rect.width))
      return ((nodeA->any.rect.x + nodeA->any.rect.width) > (nodeB->any.rect.x + nodeB->any.rect.width)) ? -1 : 1;
   if (nodeA->any.rect.y != nodeB->any.rect.y)
      return (nodeA->any.rect.y < nodeB->any.rect.y) ? -1 : 1;
   if (nodeA->any.rect.height != nodeB->any.rect.height)
      return (nodeA->any.rect.height < nodeB->any.rect.height) ? -1 : 1;
                    ...
                    XmConst void *B)
{
   ...
   if ((nodeA->any.rect.y + nodeA->any.rect.height) != (nodeB->any.rect.y + nodeB->any.rect.height))
      return ((nodeA->any.rect.y + nodeA->any.rect.height) > (nodeB->any.rect.y + nodeB->any.rect.height)) ? -1 : 1;
   if (nodeA->any.rect.height != nodeB->any.rect.height)
      return (nodeA->any.rect.height < nodeB->any.rect.height) ? -1 : 1;
   if (nodeA->any.rect.width != nodeB->any.rect.width)
      return (nodeA->any.rect.width < nodeB->any.rect.width) ? -1 : 1;
         ...
         else
         {
	       ...
	       */
            ...
            horizontal = XmDirectionMatchPartial(layout,
                                                 ...
                                                 XmPRECEDENCE_MASK);
            reverse    = (horizontal ? !XmDirectionMatchPartial(layout, XmLEFT_TO_RIGHT, XmHORIZONTAL_MASK) : !XmDirectionMatchPartial(layout, XmTOP_TO_BOTTOM, XmVERTICAL_MASK));
                        ...
                        Widget      init_focus)
{
   ...
   if (tab_node && ((tab_node->any.type == XmTAB_GRAPH_NODE) || (tab_node->any.type == XmCONTROL_GRAPH_NODE)))
   {
...
SetInitialWidgets(XmTravGraph trav_list)
{
   ...
   while (cnt < trav_list->num_entries)
   {
      if (((ptr->any.type == XmTAB_GRAPH_NODE) || (ptr->any.type == XmCONTROL_GRAPH_NODE)) && ptr->graph.sub_head)
      {
         if (ptr->any.widget && XmIsManager(ptr->any.widget) && (init_focus = ((XmManagerWidget)ptr->any.widget)->manager.initial_focus) && (init_node = GetNodeFromGraph((XmGraphNode)ptr, init_focus)))
         {
