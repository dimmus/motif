...
#define MESSAGE1 _XmMMsgGadget_0000
    ...
    static void
    GetHighlightColor(Widget w, int offset, XtArgVal *value);
...
static void ClassPartInit(WidgetClass g);
...
static XtResource resources[] = {
    {XmNx,
     ...
     sizeof(Position),
     XtOffsetOf(struct _WidgetRec, core.x),
     XmRImmediate,
     (XtPointer)0},
    {XmNy,
     XmCPosition,
     XmRVerticalPosition,
     sizeof(Position),
     XtOffsetOf(struct _WidgetRec, core.y),
     XmRImmediate,
     (XtPointer)0},
    {XmNwidth,
     XmCDimension,
     ...
     sizeof(Dimension),
     XtOffsetOf(struct _WidgetRec, core.width),
     XmRImmediate,
     (XtPointer)0},
    {XmNheight,
     XmCDimension,
     ...
     sizeof(Dimension),
     XtOffsetOf(struct _WidgetRec, core.height),
     XmRImmediate,
     (XtPointer)0},
    {XmNborderWidth,
     XmCBorderWidth,
     XmRHorizontalDimension,
     sizeof(Dimension),
     XtOffsetOf(struct _WidgetRec, core.border_width),
     XmRImmediate,
     (XtPointer)0},
    {XmNtraversalOn,
     XmCTraversalOn,
...
static XmConst XmAccessColorsTraitRec gadACT = {0, /* version */
                                                GetColors};
/* Unit Type Trait record for Gadget */
static XmConst XmSpecUnitTypeTraitRec gadUTT = {0, /* version */
                                                GetUnitType};
static XmConst XmSpecifyLayoutDirectionTraitRec gadLDT = {0, /* version */
                                                          GetDirection};
 ...
 ************************************************************************/
/*ARGSUSED*/
static void GetHighlightColor(Widget w,
                              int offset, /* unused */
                              XtArgVal *value)
{
...
/*ARGSUSED*/
static void GetTopShadowColor(Widget w,
                              int offset, /* unused */
                              XtArgVal *value)
{
  XmManagerWidget mw = (XmManagerWidget)XtParent(w);
...
/*ARGSUSED*/
static void GetBottomShadowColor(Widget w,
                                 int offset, /* unused */
                                 XtArgVal *value)
{
  XmManagerWidget mw = (XmManagerWidget)XtParent(w);
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if ((gw->gadget.navigation_type != XmDYNAMIC_DEFAULT_TAB_GROUP) &&
      !XmRepTypeValidValue(XmRID_NAVIGATION_TYPE, gw->gadget.navigation_type, (Widget)gw))
  {
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if (cur->gadget.navigation_type != new_w->gadget.navigation_type) {
    if (!XmRepTypeValidValue(XmRID_NAVIGATION_TYPE, new_w->gadget.navigation_type, (Widget)new_w))
    {
  ...
  if (new_w->gadget.highlight_drawn &&
      (!XtIsSensitive((Widget)new_w) ||
       (cur->gadget.highlight_on_enter && !(new_w->gadget.highlight_on_enter) &&
        (_XmGetFocusPolicy((Widget)new_w) == XmPOINTER))))
  {
    if (((XmGadgetClass)XtClass(new_w))->gadget_class.border_unhighlight) {
      (*(((XmGadgetClass)XtClass(new_w))->gadget_class.border_unhighlight))((Widget)new_w);
    }
  }
...
void _XmBuildGadgetResources(WidgetClass c)
{
  ...
  if (!secObjSuperClass->core_class.class_inited) {
    _XmInitializeSyntheticResources(secondaryObjClass->ext_class.syn_resources,
  ...
  if (secObjSuperClass != (WidgetClass)xmExtObjectClass) {
    _XmBuildResources(&(secondaryObjClass->ext_class.syn_resources),
                      &(secondaryObjClass->ext_class.num_syn_resources),
                      ((XmExtClassRec *)secObjSuperClass)->ext_class.syn_resources,
                      ((XmExtClassRec *)secObjSuperClass)->ext_class.num_syn_resources);
  }
  _XmProcessUnlock();
}
...
static void BorderHighlight(Widget w)
{
  ...
  if (g->rectangle.width == 0 || g->rectangle.height == 0 || g->gadget.highlight_thickness == 0) {
    return;
  }
  XmeDrawHighlight(XtDisplay((Widget)g),
                   XtWindow((Widget)g),
...
static void BorderUnhighlight(Widget w)
{
  ...
  if (XmIsManager(g->object.parent) &&
      ((UnhighlightT = (XmSpecifyUnhighlightTrait)XmeTraitGet((XtPointer)XtClass(g->object.parent),
                                                              XmQTspecifyUnhighlight)) != NULL) &&
...
static void FocusChange(Widget wid, XmFocusChange change)
{
  ...
  switch (change) {
    ...
    case XmFOCUS_OUT:
      ...
      if (((XmGadgetClass)XtClass(wid))->gadget_class.border_unhighlight) {
        (*(((XmGadgetClass)XtClass(wid))->gadget_class.border_unhighlight))(wid);
      }
      break;
  }
  return;
...
static XmNavigability WidgetNavigable(Widget wid)
{
  if (XtIsSensitive(wid) && ((XmGadget)wid)->gadget.traversal_on) {
