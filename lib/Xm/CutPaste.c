...
static void    AssertClipboardSelection(
      ...
      Time            time);
static Boolean ClipboardConvertProc(Widget, Atom *, Atom *, Atom *, XtPointer *, unsigned long *, int *);
...
static int ClipboardWindowExists(
   Display *display,
   Window   window);
static void    ClipboardReceiveData(Widget, XtPointer, Atom *, Atom *, XtPointer, unsigned long *, int *);
static int     ClipboardRetrieve(Display *display, Window window, char *format, XtPointer buffer, unsigned long length, unsigned long *outlength, long *private_id, Atom *outtype);
   ...
   Display *dpy)
{
   ...
   XChangeProperty(dpy, RootWindow(dpy, 0), XmA_MOTIF_CLIP_TIME, XmA_MOTIF_CLIP_TIME, 8, PropModeAppend, NULL, 0);
   ...
   long     itemid)
{
   ...
   *(long *)int_ptr = nextItem;
   ClipboardReplaceItem(display, XM_NEXT_ID, int_ptr, length, PropModeReplace, 32, True, XA_INTEGER);
}
...
static Boolean
ClipboardGetByNameItem(Display *dpy, Window win, ClipboardHeader header, char *format)
{
   short               dataok;
   ...
   matchformat = ClipboardFindFormat(dpy, header, format, (itemId)NULL, 0, &maxname, &count, &matchformatlength);
   if (matchformat != 0)
   {
      dataok = 1;
   ...
   *format = 8;
      ...
      *ptr = atoms[XmA_TIMESTAMP];
      ...
      while (nextitem != NULL && n < count)
      {
         ...
         *ptr = nextitem->formatNameAtom;
         ...
         nextitem = ClipboardFindFormat(display, header, 0, (itemId)NULL, n + 1, &dummy, &ret_count, &dummy);
   ...
   else
   {
      ...
      *value = XtMalloc((int)*size);
      if (ClipboardRetrieve(display, window, format_name, (XtPointer)*value, *size, &outlength, &private_id, type) != ClipboardSuccess)
      {
         rval = False;
         goto done;
      }
   ...
   Boolean  *cont)
{
   ...
   if (reason == XmCR_CLIPBOARD_DATA_REQUEST)
   {
      ...
      header->recopyId = 0;
      ClipboardReplaceItem(display, XM_HEADER_ID, header, hlength, PropModeReplace, 32, True, XA_INTEGER);
   }
   return;
}
   ...
   unsigned long  discard) /* ignore old data */
{
   ...
   XtPointer        pointer;
   /* retrieve the item from the root */
   ret_value = ClipboardFindItem(display, itemid, &pointer, &loclength, &loctype, &loc_format, rec_type);
   ...
   unsigned long  *matchlength)    /* receives length of matching format */
{
   ...
   *maxnamelength = 0;
   ...
   /* get the query item */
   if (ClipboardFindItem(display, queryitemid, (XtPointer *)&queryitem, &reclength, &rectype, 0, XM_DATA_ITEM_RECORD_TYPE) == ClipboardFail)
      return 0;
   if (queryitem == 0)
   {
      CleanupHeader(display);
   ...
   *matchlength = 0;
   ...
   for (i = 0; i < queryitem->formatCount; i++)
   {
      ...
      free_flag = 1;
      /* get the next format */
      ClipboardFindItem(display, currformatid, (XtPointer *)&currformat, &reclength, &rectype, 0, XM_FORMAT_HEADER_TYPE);
      if (currformat == 0)
      {
         CleanupHeader(display);
         ClipboardError(CLIPBOARD_CORRUPT, CORRUPT_DATA_STRUCTURE);
   ...
   unsigned long   state)
{
   ...
   ClipboardReplaceItem(display, dataitemid, (XtPointer)itemheader, itemlength, PropModeReplace, 32, True, XA_INTEGER);
}
   ...
   int                 messagetype)
{
   ...
   switch (messagetype)
   {
      case XM_DATA_REQUEST_MESSAGE:
         ...
         /* replace the clipboard header */
         ClipboardReplaceItem(display, XM_HEADER_ID, (XtPointer)root_clipboard_header, headerlength, PropModeReplace, 32, True, XA_INTEGER);
                      ...
                      int           *format)
{
   ...
   XmA_CLIPBOARD = XInternAtom(display, XmSCLIPBOARD, False);
   XtGetSelectionValue(dest, XmA_CLIPBOARD, target, ClipboardReceiveData, &info, XtLastTimestampProcessed(display));
#ifdef XTHREADS
...
#else
      XtInputMask mask;
      while (!(mask = XtAppPending(app_context))); /* busy waiting - don't lose lock */
      if (mask & XtIMXEvent)
      {
         XtAppNextEvent(app_context, &event_return);
         dataisready = ClipboardDataIsReady(display, &event_return, (char *)&cutbynameinfo);
   ...
   Display *display)
{
   ...
   ClipboardReplaceItem(display, XM_NEXT_ID, propertynumber, length, PropModeReplace, 32, True, XA_INTEGER);
   return loc_id;
}
   ...
   Window   window)
{
   ...
   if (take_lock == True)
   {
      ...
      {
         XSetSelectionOwner(display, atoms[XmA_MOTIF_CLIP_LOCK], window, ClipboardGetCurrentTime(display));
   ...
   ClipboardReplaceItem(display, XM_LOCK_ID, (XtPointer)lockptr, sizeof(ClipboardLockRec), PropModeReplace, 32, False, XA_INTEGER);
   ...
   Boolean  all_levels)
{
   ...
   ClipboardReplaceItem(display, XM_LOCK_ID, (XtPointer)lockptr, length, PropModeReplace, 32, False, XA_INTEGER);
   XtFree((char *)lockptr);
...
{
   return (XmClipboardStartCopy(display, window, label, CurrentTime, widget, (XmCutPasteProc)callback, itemid));
}
   ...
   long         *dataid)             /* Data id returned by clipboard */
{
   ...
   formatptr = ClipboardFindFormat(display, header, format, (itemId)itemid, 0, &maxname, &count, &formatlength);
   ...
   if (formatptr == 0)
   {
      ...
      *idptr = formatid;
      ...
      /* return the property on the root window for the item */
      ClipboardReplaceItem(display, itemid, (XtPointer)itemheader, itemlength, PropModeReplace, 32, True, XA_INTEGER);
   ...
   long          *private_id)         /* Private id provide by application */
{
   ...
   _XmDisplayToAppContext(display);
   _XmAppLock(app);
   ret_val = ClipboardRetrieve(display, window, format, buffer, length, outlength, private_id, &ignoretype);
   _XmAppUnlock(app);
   return ret_val;
}

static int
ClipboardRetrieve(Display *display, Window window, char *format, XtPointer buffer, unsigned long length, unsigned long *outlength, long *private_id, Atom *outtype)
{
   ClipboardHeader     header;
   ClipboardFormatItem matchformat;
   char               *formatdata;
   ...
   if (WeOwnSelection(display, header))
   {
      ...
      /* find the matching format for the next paste item */
      matchformat = ClipboardFindFormat(display, header, format, (itemId)NULL, 0, &maxname, &count, &matchformatlength);
      if (matchformat != 0)
      {
         dataok  = 1;
   ...
   else
   {
      ...
      if (ClipboardGetSelection(display, window, XInternAtom(display, format, False), (XtPointer *)&formatdata, outtype, &loc_outlength, &outformatsize))
      {
         /* we're okay */
         dataok = 1;
         ...
         if (header->incrementalCopyFrom)
         {
            ...
            {
               header->foreignCopiedLength = header->foreignCopiedLength
                                           + loc_outlength;
            }
         }
      }
   ...
   unsigned long *maxlength) /* receives max length of format names */
{
   ...
   if (WeOwnSelection(display, header))
   {
      /* yes, find the next paste item, only looking for
	   maxlength and count */
      alloc_to_free = (char *)ClipboardFindFormat(display, header, 0, (itemId)NULL, 0, &loc_maxlength, &loc_count, &loc_matchlength);
   }
   else
   {
      /* we don't own the selection, get the data from selection owner */
      if (!ClipboardGetSelection(display, window, XInternAtom(display, XmSTARGETS, False), (XtPointer *)&alloc_to_free, &ignoretype, &loc_count_len, &ignoreformat))
      {
         _XmAppUnlock(app);
   ...
   unsigned long *outlength)    /* Receives length copied to name buffer */
{
   ...
   if (WeOwnSelection(display, header))
   {
      ...
      matchformat = ClipboardFindFormat(display, header, 0, (itemId)NULL, n, &maxname, &count, &loc_matchlength);
      if (matchformat != 0)
      {
   ...
   {
      /* we don't own the selection, get the data from selection owner */
      if (!ClipboardGetSelection(display, window, XInternAtom(display, XmSTARGETS, False), (XtPointer *)&alloc_to_free, &ignoretype, &loc_matchlength, &ignoreformat))
      {
         *outlength = 0;
         _XmAppUnlock(app);
         return ClipboardNoData;
   ...
   unsigned long *length) /* Receives length of the data in that format */
{
   ...
   if (WeOwnSelection(display, header))
   {
      ...
      matchformat = ClipboardFindFormat(display, header, format, (itemId)NULL, 0, &maxname, &count, &loc_matchlength);
      /* return the length */
      if (matchformat != 0)
      {
   ...
   {
      /* we don't own the selection, get the data from selection owner */
      if (!ClipboardGetSelection(display, window, XInternAtom(display, format, False), (XtPointer *)&alloc_to_free, &ignoretype, &loc_length, &ignoreformat))
      {
         _XmAppUnlock(app);
         return ClipboardNoData;
      }
   ...
   unsigned long          *count) /* Number of items in returned list */
{
   ...
   *list     = 0;
   ...
   itemlist    = (XmClipboardPendingList)XtMalloc((size_t)(header->currItems * sizeof(XmClipboardPendingRec)));
   ...
   for (i = 0; i < header->currItems; i++)
   {
      ...
      else
      {
         ...
         matchformat = ClipboardFindFormat(display, header, format, *id_ptr, 0, &maxname, &dummy, &loc_matchlength);
      }
      if (matchformat != 0)
      {
...
GetTypeFromTarget(Display *display, Atom target)
{
   ...
   assert(XtNumber(atom_names) == NUM_ATOMS);
   XInternAtoms(display, atom_names, XtNumber(atom_names), False, atoms);
   if (target == atoms[XmATARGETS] || target == atoms[XmA_MOTIF_EXPORT_TARGETS] || target == atoms[XmA_MOTIF_CLIPBOARD_TARGETS] || target == atoms[XmA_MOTIF_DEFERRED_CLIPBOARD_TARGETS])
      return (XA_ATOM);
   if (target == atoms[XmAMULTIPLE])
      return (atoms[XmAATOM_PAIR]);
   if (target == atoms[XmATIMESTAMP] || target == atoms[XmALIST_LENGTH] || target == atoms[XmAPROCESS] || target == atoms[XmATASK] || target == atoms[XmALENGTH])
      return (XA_INTEGER);
   if (target == atoms[XmATEXT] || target == atoms[XmAODIF] || target == atoms[XmAOWNER_OS] || target == atoms[XmAFILE_NAME] || target == atoms[XmAFILE] || target == atoms[XmAHOST_NAME] || target == atoms[XmAUSER] || target == atoms[XmAPROCEDURE] || target == atoms[XmAMODULE] || target == atoms[XmACLASS] || target == atoms[XmANAME])
   {
      int           ret_status = 0;
      ...
      Atom          encoding;
      tmp_prop.value = NULL;
      ret_status     = XmbTextListToTextProperty(display, &tmp_string, 1, (XICCEncodingStyle)XTextStyle, &tmp_prop);
   ...
   if (target == atoms[XmACHARACTER_POSITION] || target == atoms[XmALINE_NUMBER] || target == atoms[XmACOLUMN_NUMBER])
      return (atoms[XmASPAN]);
   if (target == atoms[XmACLIENT_WINDOW])
      return (XA_WINDOW);
   if (target == atoms[XmADELETE] || target == atoms[XmAINSERT_SELECTION] || target == atoms[XmALINK_SELECTION] || target == atoms[XmAINSERT_PROPERTY])
