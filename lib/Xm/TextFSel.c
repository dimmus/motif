                    ...
                    XmTransferDoneCallbackStruct *ts) /* unused */
{
   ...
   if (prim_select->num_chars > 0)
   {
      ...
      _XmTextFieldStartSelection(tf, tf->text.prim_anchor, cursorPos, prim_select->time);
...
static void
TextFieldSecondaryWrapper(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
   Atom XA_TARGETS = XInternAtom(XtDisplay(w), XmSTARGETS, False);
   if (ds->target == XA_TARGETS)
      HandleInsertTargets(w, closure, &(ds->selection), &(ds->type), ds->value, &(ds->length), &(ds->format), ds->transfer_id);
   else
      InsertSelection(w, closure, &(ds->selection), &(ds->type), ds->value, &(ds->length), &(ds->format), ds->transfer_id);
}
   ...
   XtPointer      tid)
{
   ...
   else if (_insert_select->select_type == XmDEST_SELECT)
   {
      ...
      {
         if (TextF_CursorPosition(tf) < left || TextF_CursorPosition(tf) > right || !tf->text.pending_delete)
         {
   ...
   {
      total_value = _XmTextToLocaleText(w, value, *type, *format, *length, NULL);
      if (total_value)
      {
         ...
         { /* must convert to wchar_t before passing to Replace */
   ...
   { /* it must be either TEXT or codeset of the locale */
      if (tf->text.max_char_size == 1)
      {
   ...
   else
   {
      ...
      (void)_XmTextFieldSetDestination(w, TextF_CursorPosition(tf), _insert_select->event->time);
      if (_insert_select->select_type == XmDEST_SELECT)
      {
         if (left != right)
         {
            ...
            {
               _XmTextFieldStartSelection(tf, TextF_CursorPosition(tf), TextF_CursorPosition(tf), _insert_select->event->time);
            }
         }
      }
   ...
   XtPointer      tid)
{
   ...
   if (supports_text && supports_encoding_data)
      ...
      target = XA_STRING;
   XmTransferValue(tid, target, (XtCallbackProc)TextFieldSecondaryWrapper, closure, _insert_select->event->time);
}
   ...
   */
   if (*target == atoms[XmATARGETS])
   {
      ...
      *value      = (XtPointer)targs;
      ...
      {
         targs[target_count] = CS_OF_ENCODING;
         target_count++;
      }
      if (is_primary || is_destination)
      {
         targs[target_count] = atoms[XmAINSERT_SELECTION];
         target_count++;
      }
      ...
      {
         targs[target_count] = atoms[XmADELETE];
         target_count++;
      }
      *type   = XA_ATOM;
      *length = target_count;
      ...
      *format = 8;
      ...
      if (tf->text.max_char_size != 1)
      {
         ...
         *length   = _XmTextFieldCountBytes(tf, TextF_WcValue(tf) + left, (int)(right - left));
         ...
         stat      = wcstombs(tmp_value, TextF_WcValue(tf) + left, (unsigned)*length); /* NOTE: casting *length could
      ...
      else
      {
         *length   = right - left;
         ...
         (void)memcpy((void *)tmp_value, (void *)(TextF_Value(tf) + left), (size_t)*length); /* NOTE: casting *length could result
					  in a truncated long. */
      }
      tmp_value[*length] = '\0';
      tmp_prop.value     = NULL;
      /* convert tmp_value to 8859.1 */
      ret_status = XmbTextListToTextProperty(XtDisplay(w), &tmp_value, 1, (XICCEncodingStyle)XStringStyle, &tmp_prop);
      XtFree(tmp_value);
      ...
      *format = 8;
      ...
      if (tf->text.max_char_size != 1)
      {
         ...
         *value  = XtMalloc((unsigned)*length + 1);
         stat    = wcstombs((char *)*value, TextF_WcValue(tf) + left, (unsigned)*length); /* NOTE: casting *length could
					     result in a truncated long */
      ...
      else
      {
         ...
         *value  = XtMalloc((unsigned)*length + 1);
         /* get the selection value */
         (void)memcpy((void *)*value, (void *)(TextF_Value(tf) + left), (size_t)*length); /* NOTE: casting *length could result
					  in a truncated long. */
      }
      (*(char **)value)[*length] = '\0';
   }
   else if (*target == atoms[XmACOMPOUND_TEXT])
   {
      ...
      *format = 8;
      ...
      { /* malloc the space and copy the data to be converted */
         *length   = right - left;
         tmp_value = XtMalloc((unsigned)*length + 1);
         /* get the selection value */
         (void)memcpy((void *)tmp_value, (void *)(TextF_Value(tf) + left), (size_t)*length); /* NOTE: casting *length could result
      ...
      ret_status = XmbTextListToTextProperty(XtDisplay(w), &tmp_value, 1, (XICCEncodingStyle)XCompoundTextStyle, &tmp_prop);
      XtFree(tmp_value);
      if (ret_status == Success || ret_status > 0)
      {
         *length = tmp_prop.nitems;
   ...
   else if (*target == atoms[XmAUTF8_STRING])
   {
      ...
      *format = 8;
      ...
      tmp_prop.value     = NULL;
      /* Convert to compound text */
      ret_status = XmbTextListToTextProperty(XtDisplay(w), &tmp_value, 1, (XICCEncodingStyle)XUTF8StringStyle, &tmp_prop);
      XtFree(tmp_value);
      if (ret_status == Success || ret_status > 0)
      {
         *length = tmp_prop.nitems;
   ...
   else if (*target == atoms[XmADELETE])
   {
      ...
      {
         if (req_event != NULL && req_event->requestor == XtWindow((Widget)tf))
            move_cursor = False;
      }
      if (!_XmTextFieldReplaceText(tf, (XEvent *)req_event, left, right, NULL, 0, move_cursor))
      {
         tf->text.has_primary = True;
         return False;
      }
      _XmTextFieldStartSelection(tf, tf->text.prim_anchor, tf->text.prim_anchor, _time);
      cb.reason = XmCR_VALUE_CHANGED;
   ...
   Atom  *selection)
{
   ...
   {
      _XmTextFieldSetSel2(w, 0, 0, True, XtLastTimestampProcessed(XtDisplay(w)));
   }
}
...
_XmTextFieldGetDropReciever(Widget w)
{
   ...
   if (!XFindContext(XtDisplay(w), (Window)XtScreen(w), loc_context, (char **)&widget))
   {
      return widget;
   }
   return NULL;
...
static void
DropTransferProc(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
   enum
   {
      XmACOMPOUND_TEXT,
   ...
   if (ds->type == atoms[XmANULL])
   {
      if (transfer_rec->num_chars > 0 && transfer_rec->move)
      {
         ...
         _XmTextFieldStartSelection(tf, tf->text.prim_anchor, TextF_CursorPosition(tf), XtLastTimestampProcessed(XtDisplay(w)));
         tf->text.pending_off = False;
         _XmTextFieldSetCursorPosition(tf, NULL, TextF_CursorPosition(tf), True, True);
      }
      if (ds->value)
      {
   ...
   if (!(ds->value) || (ds->type != CS_OF_ENCODING && ds->type != atoms[XmACOMPOUND_TEXT] &&
#if XM_UTF8
   ...
   {
      if ((total_value = _XmTextToLocaleText(w, ds->value, ds->type, 8, ds->length, NULL)) != NULL)
   ...
   if (TextF_PendingDelete(tf) && tf->text.has_primary && tf->text.prim_pos_left != tf->text.prim_pos_right)
   {
   ...
   transfer_rec->num_chars = _XmTextFieldCountCharacters(tf, total_value, total_length);
   _XmTextFieldDrawInsertionPoint(tf, False);
   ...
   if (tf->text.max_char_size == 1)
   {
      replace = _XmTextFieldReplaceText(tf, ds->event, insertPosLeft, insertPosRight, (char *)total_value, (int)total_length, False);
   }
   else
   {
      ...
      if (wc_total_length > 0)
         replace = _XmTextFieldReplaceText(tf, ds->event, insertPosLeft, insertPosRight, (char *)wc_total_value, wc_total_length, False);
   ...
   if (replace)
   {
      ...
      if (transfer_rec->num_chars > 0 && !transfer_rec->move)
      {
         cursorPos = transfer_rec->insert_pos + transfer_rec->num_chars;
         _XmTextFieldSetCursorPosition(tf, NULL, cursorPos, True, True);
         _XmTextFieldSetDestination((Widget)tf, TextF_CursorPosition(tf), transfer_rec->timestamp);
      }
      left  = tf->text.prim_pos_left;
      right = tf->text.prim_pos_right;
      ...
      {
         if (!transfer_rec->move && !tf->text.add_mode && transfer_rec->num_chars != 0)
        ...
        XmSelectionCallbackStruct *ds)
{
   ...
   if (ds->selection != atoms[XmACLIPBOARD] && ds->length == 0 && ds->type != atoms[XmANULL])
   {
      ...
      if (_prim_select->target == atoms[XmATEXT])
      {
         ...
         XmTransferValue(ds->transfer_id, XA_STRING, (XtCallbackProc)DoStuff, (XtPointer)_prim_select, _prim_select->time);
      }
      _XmProcessUnlock();
      XtFree((char *)ds->value);
      ds->value = NULL;
   ...
   */
   if (ds->type == atoms[XmANULL])
   {
      ...
      if (_prim_select->num_chars > 0 && tf->text.selection_move)
      {
         ...
         _XmTextFieldStartSelection(tf, _prim_select->position, _prim_select->position + prim_char_length, _prim_select->time);
         tf->text.pending_off = False;
         _XmTextFieldSetCursorPosition(tf, NULL, _prim_select->position + prim_char_length, True, True);
   ...
   else
   {
      ...
      if (ds->selection == atoms[XmACLIPBOARD])
      {
         if (tf->text.has_primary)
         {
            left  = tf->text.prim_pos_left;
            right = tf->text.prim_pos_right;
            if (tf->text.pending_delete && replace_from >= left && replace_to <= right)
            {
               replace_from  = left;
               replace_to    = right;
               dest_disjoint = False;
      ...
      {
         char *total_value;
         if ((total_value = _XmTextToLocaleText(w, ds->value, ds->type, ds->format, ds->length, NULL))
             != NULL)
         {
            if (tf->text.max_char_size == 1)
            {
               ...
               replace_res             = _XmTextFieldReplaceText(tf, ds->event, replace_from, replace_to, total_value, _prim_select->num_chars, ds->selection == atoms[XmACLIPBOARD]);
               _XmProcessUnlock();
               XtFree(total_value);
            }
            else
            {
               ...
               if (_prim_select->num_chars < 0)
                  ...
                  replace_res = _XmTextFieldReplaceText(tf, ds->event, replace_from, replace_to, (char *)wc_value, _prim_select->num_chars, ds->selection == atoms[XmACLIPBOARD]);
               _XmProcessUnlock();
               XtFree((char *)wc_value);
               XtFree(total_value);
            }
         ...
         { /* initialize _prim_select values for possible delete oper */
            _XmProcessLock();
            _prim_select->num_chars = 0;
            _XmProcessUnlock();
         }
      ...
      else
      {
         if (tf->text.max_char_size == 1)
         {
            ...
            replace_res             = _XmTextFieldReplaceText(tf, ds->event, replace_from, replace_to, (char *)ds->value, _prim_select->num_chars, ds->selection == atoms[XmACLIPBOARD]);
            _XmProcessUnlock();
         }
         else
         {
      ...
      if (replace_res)
      {
         ...
         {
            _XmProcessLock();
            (void)_XmTextFieldSetDestination(w, TextF_CursorPosition(tf), _prim_select->time);
            _XmProcessUnlock();
         }
         left  = tf->text.prim_pos_left;
         right = tf->text.prim_pos_right;
         if (tf->text.has_primary)
         {
            if (ds->selection == atoms[XmACLIPBOARD])
            {
               if (left != right && (!dest_disjoint || !tf->text.add_mode))
                  _XmProcessLock();
               _XmTextFieldStartSelection(tf, TextF_CursorPosition(tf), TextF_CursorPosition(tf), _prim_select->time);
               _XmProcessUnlock();
            }
            else
            {
         ...
         else
         {
            ...
            else if (!tf->text.selection_move && !tf->text.add_mode && _prim_select->num_chars != 0)
               tf->text.prim_anchor = _prim_select->position;
            _XmProcessUnlock();
         }
         cb.reason = XmCR_VALUE_CHANGED;
...
static void
HandleTargets(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
   enum
   {
      XmACOMPOUND_TEXT,
   ...
   */
   ...
   if (ds->selection != atoms[XmACLIPBOARD])
   {
      left  = tf->text.prim_pos_left;
      right = tf->text.prim_pos_right;
      if (tf->text.has_primary && left != right && select_pos > left && select_pos < right)
      {
         XtFree((char *)ds->value);
         ds->value = NULL;
         return;
   ...
   XmTransferValue(ds->transfer_id, targets[0], (XtCallbackProc)DoStuff, (XtPointer)prim_select, prim_select->time);
   _XmProcessUnlock();
   XtFree((char *)ds->value);
   ds->value = NULL;
}
           ...
           XmDestinationCallbackStruct *ds)
{
   ...
   if (cb->operation & XmDROP_MOVE && w == initiator && ((XmTextFieldWidget)w)->text.has_primary && left != right && insert_pos >= left && insert_pos <= right)
   {
      /*EMPTY*/
   }
   else
   ...
   if (doTransfer)
   {
      ...
      XmTransferValue(tid, desiredTarget, (XtCallbackProc)DropTransferProc, (XtPointer)transfer_rec, 0);
   }
}
                         ...
                         XmConvertCallbackStruct *cs)
{
   ...
   static char *atom_names[] = { XmSDELETE, XmS_MOTIF_LOSE_SELECTION, XmS_MOTIF_EXPORT_TARGETS, XmS_MOTIF_CLIPBOARD_TARGETS, XmSCOMPOUND_TEXT, XmSTEXT, XmSTARGETS, XmSCLIPBOARD,
#if XM_UTF8
                                 XmSUTF8_STRING
#endif
   };
   ...
   if (cs->target == atoms[XmADELETE] && cs->selection == XA_SECONDARY)
   {
   ...
   if (cs->selection == atoms[XmACLIPBOARD] && cs->parm == (XtPointer)XmLINK && (cs->target == atoms[XmA_MOTIF_CLIPBOARD_TARGETS] || cs->target == atoms[XmATARGETS])) return;
   if (!_XmTextFieldConvert(w, &cs->selection, &cs->target, &type, &value, &size, &format, (Widget)cs->source_data, cs->event))
   {
      value  = NULL;
      type   = XA_INTEGER;
      size   = 0;
                             ...
                             XmDestinationCallbackStruct *ds)
{
   ...
   */
   ...
   {
      XmDropProcCallbackStruct *cb = (XmDropProcCallbackStruct *)ds->destination_data;
      DropPoint.x                  = cb->x;
      DropPoint.y                  = cb->y;
   ...
   else if (ds->selection == XA_SECONDARY)
   {
       ...
       */
         XmTransferValue(ds->transfer_id, atoms[XmATARGETS], (XtCallbackProc)TextFieldSecondaryWrapper, (XtPointer)&insert_select, ds->time);
      }
      else
      {
         /*
       ...
       */
         XmTransferValue(ds->transfer_id, ((Atom)ds->location_data), (XtCallbackProc)TextFieldSecondaryWrapper, (XtPointer)&insert_select, ds->time);
      }
      _XmProcessUnlock();
   }
   else
      /* CLIPBOARD or PRIMARY */
      XmTransferValue(ds->transfer_id, atoms[XmATARGETS], (XtCallbackProc)HandleTargets, ds->location_data, ds->time);
}

void
_XmTextFieldInstallTransferTrait(void)
{
   XmeTraitSet((XtPointer)xmTextFieldWidgetClass, XmQTtransfer, (XtPointer)&textFieldTT);
}
