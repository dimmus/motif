...
static XtGeometryResult QueryGeometry(Widget w, XtWidgetGeometry *req, XtWidgetGeometry *rep);
static XtGeometryResult GeometryManager(Widget w, XtWidgetGeometry *req, XtWidgetGeometry *rep);
...
static void SpinChildFocusChange(Widget focusWidget,
...
static void SpinBDisarm(Widget disarmWidget,
                        XEvent *disarmEvent,
                        String *disarmParams,
                        Cardinal *disarmCount);
static void SpinBFirst(Widget firstWidget,
                       XEvent *firstEvent,
                       String *firstParams,
                       Cardinal *firstCount);
static void SpinBLast(Widget lastWidget,
                      XEvent *lastEvent,
                      String *lastParams,
                      Cardinal *lastCount);
static void SpinBLeft(Widget leftWidget,
                      XEvent *leftEvent,
                      String *leftParams,
                      Cardinal *leftCount);
static void SpinBNext(Widget nextWidget,
                      XEvent *nextEvent,
                      String *nextParams,
                      Cardinal *nextCount);
static void SpinBPrior(Widget priorWidget,
                       XEvent *priorEvent,
                       String *priorParams,
                       Cardinal *priorCount);
static void SpinBRight(Widget rightWidget,
                       XEvent *rightEvent,
                       String *rightParams,
                       Cardinal *rightCount);
...
static void FireCallbacks(XmSpinBoxCallbackStruct *spinBoxCallData,
                          ...
                          XEvent *arrowEvent,
                          int arrowReason);
static void ArrowCallback(Widget arrowWidget, XEvent *arrowEvent, int arrowReason);
static Boolean ArrowVerify(Widget arrowWidget, XEvent *arrowEvent, int arrowReason);
...
static void SpinNChangeMoveCB(Widget nav,
...
#define SB_ArrowsAreStacked(w) \
  ((SB_ArrowLayout(w) == XmARROWS_END) || (SB_ArrowLayout(w) == XmARROWS_BEGINNING))
...
static XtResource resources[] = {{XmNarrowLayout,
                                  XmCArrowLayout,
                                  XmRArrowLayout,
                                  sizeof(unsigned char),
                                  Offset(arrow_layout),
                                  XmRImmediate,
                                  (XtPointer)XmARROWS_END},
                                 {XmNarrowOrientation,
                                  XmCArrowOrientation,
                                  XmRArrowOrientation,
                                  sizeof(unsigned char),
                                  Offset(arrow_orientation),
                                  XmRImmediate,
                                  (XtPointer)XmARROWS_VERTICAL},
                                 {XmNarrowSize,
                                  XmCArrowSize,
                                 ...
                                 {XmNdefaultArrowSensitivity,
                                  ...
                                  XmRArrowSensitivity,
                                  sizeof(unsigned char),
                                  Offset(default_arrow_sensitivity),
                                  XmRImmediate,
                                  (XtPointer)XmARROWS_SENSITIVE},
                                 {XmNmodifyVerifyCallback,
                                  XmCCallback,
                                  XmRCallback,
                                  sizeof(XtCallbackList),
                                  Offset(modify_verify_cb),
                                  XmRPointer,
                                  NULL},
                                 {
...
static XtResource constraints[] = {{XmNspinBoxChildType,
                                    XmCSpinBoxChildType,
                                    XmRSpinBoxChildType,
                                    sizeof(unsigned char),
                                    ConstraintOffset(sb_child_type),
                                    XmRImmediate,
                                    (XtPointer)XmSTRING},
                                   {XmNpositionType,
                                    XmCPositionType,
                                    XmRPositionType,
                                    sizeof(unsigned char),
                                    ConstraintOffset(position_type),
                                    XmRImmediate,
                                    (XtPointer)XmPOSITION_VALUE},
                                   {XmNnumValues,
                                    XmCNumValues,
                                    XmRInt,
                                    sizeof(int),
                                    ConstraintOffset(num_values),
                                    XmRImmediate,
                                    (XtPointer)0},
                                   {XmNvalues,
                                    XmCValues,
                                   ...
                                   {XmNminimumValue,
                                    ...
                                    XmRInt,
                                    sizeof(int),
                                    ConstraintOffset(minimum_value),
                                    XmRImmediate,
                                    (XtPointer)0},
                                   {XmNmaximumValue,
                                    XmCMaximumValue,
                                    XmRInt,
                                    sizeof(int),
                                    ConstraintOffset(maximum_value),
                                    XmRImmediate,
                                    (XtPointer)10},
                                   {XmNincrementValue,
                                    XmCIncrementValue,
                                    XmRInt,
                                    sizeof(int),
                                    ConstraintOffset(increment_value),
                                    XmRImmediate,
                                    (XtPointer)1},
                                   {XmNdecimalPoints,
                                    XmCDecimalPoints,
                                    XmRShort,
                                    sizeof(short),
                                    ConstraintOffset(decimal_points),
                                    XmRImmediate,
                                    (XtPointer)0},
                                   {XmNarrowSensitivity,
                                    XmCArrowSensitivity,
                                    XmRArrowSensitivity,
                                    sizeof(unsigned char),
                                    ConstraintOffset(arrow_sensitivity),
                                    XmRImmediate,
                                    (XtPointer)XmARROWS_DEFAULT_SENSITIVITY},
                                   {XmNwrap,
                                    XmCWrap,
                                    XmRBoolean,
                                    sizeof(Boolean),
                                    ConstraintOffset(wrap),
                                    XmRImmediate,
                                    (XtPointer)True},
                                   {XmNposition,
                                    XmCPosition,
                       ...
                       Cardinal *num_args) /* unused */
{
  ...
  spinW->spinBox.insensitive_gc = XtAllocateGC(
      new_w, 0, GCmask, &GCvalues, GCClipMask, unusedMask);
}
                      ...
                      Region region) /* unused */
{
  ...
  if (XtIsRealized(w)) {
    ...
    if (spinW->manager.shadow_thickness > 0) {
      int width, height;
      width = (spinW->spinBox.ideal_width < XtWidth(spinW)) ? spinW->spinBox.ideal_width :
                                                              XtWidth(spinW);
      height = (spinW->spinBox.ideal_height < XtHeight(spinW)) ? spinW->spinBox.ideal_height :
                                                                 XtHeight(spinW);
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (XtIsRealized(new_w) && (reqW->spinBox.arrow_layout != oldW->spinBox.arrow_layout ||
  ...
  if (reqW->spinBox.default_arrow_sensitivity != oldW->spinBox.default_arrow_sensitivity ||
      reqW->spinBox.detail_shadow_thickness != oldW->spinBox.detail_shadow_thickness)
    displayFlag = True;
  return (displayFlag);
}
...
static void InsertChild(Widget newChild)
{
  ...
  if (XmeTraitGet((XtPointer)XtClass(newChild), XmQTaccessTextual) != NULL) {
    ...
    XtInsertEventHandler(
        newChild, FocusChangeMask, False, SpinChildFocusChange, (XtPointer)spinW, XtListHead);
    /* To handle implicit mode,  we also call the focus change
         if a button is clicked.  The work is finished in the event
         handler */
    XtInsertEventHandler(
        newChild, ButtonPressMask, False, SpinChildFocusChange, (XtPointer)spinW, XtListHead);
  }
                                 ...
                                 Cardinal *num_args) /* unused */
{
  ...
  if (SB_ChildIsNumeric(newC)) {
    ...
    if ((newC->minimum_value < newC->maximum_value && newC->increment_value < 0) ||
        (newC->minimum_value > newC->maximum_value && newC->increment_value > 0))
    {
                                   ...
                                   Cardinal *num_args) /* unused */
{
  ...
  if (SB_ChildIsNumeric(newC)) {
    ...
    if (newC->minimum_value != oldC->minimum_value || newC->maximum_value != oldC->maximum_value ||
        newC->increment_value != oldC->increment_value ||
        newC->decimal_points != oldC->decimal_points || newC->position != oldC->position)
    {
      redisplayText = True;
    ...
    if (focusEvent->type == ButtonPress && focusEvent->xbutton.button == Button1) {
      if (spinW->spinBox.textw != (Widget)NULL) {
        ...
        if (XmIsPrimitive(child)) {
          (*(((XmPrimitiveWidgetClass)wc)->primitive_class.border_unhighlight))(child);
        }
      }
      spinW->spinBox.textw = focusWidget;
      if (spinW->spinBox.textw != (Widget)NULL) {
        ...
        WidgetClass wc = XtClass(child);
        if (XmIsPrimitive(child)) {
          (*(((XmPrimitiveWidgetClass)wc)->primitive_class.border_highlight))(child);
        }
      }
    }
  }
 ...
 *****************************************************************************/
static void SpinBEnter(Widget widget, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (_XmGetFocusPolicy(widget) != XmPOINTER)
    return;
...
static void SpinBLeave(Widget widget, XEvent *event, String *params, Cardinal *num_params)
{
  XmSpinBoxWidget spinW = (XmSpinBoxWidget)widget;
  Widget child = (Widget)spinW->spinBox.textw;
  /* We only perform this action for POINTER mode */
 ...
 *****************************************************************************/
static void SpinBArm(Widget armWidget,
                     XEvent *armEvent,
...
static void LayoutSpinBox(Widget w, XtWidgetGeometry *spinG, Widget child) /* unused */
{
  XmSpinBoxWidget spinW = (XmSpinBoxWidget)w;
  int arrowLayout;
  int arrowSize;
   ...
   */
  spinW->spinBox.up_arrow_rect.width = spinW->spinBox.up_arrow_rect.height =
      spinW->spinBox.down_arrow_rect.width = spinW->spinBox.down_arrow_rect.height = arrowSize;
  /*
...
static void DrawSpinArrow(Widget arrowWidget, int arrowFlag)
{
  ...
  if (XtIsRealized((Widget)spinW)) {
    ...
    XmeDrawArrow(XtDisplay(arrowWidget),
...
static void SpinTimeOut(Widget w, int spinDelay)
{
  ...
  if (spinW->spinBox.initial_delay > 0 && spinW->spinBox.repeat_delay > 0)
    spinW->spinBox.spin_timer = XtAppAddTimeOut(
        XtWidgetToApplicationContext(w), spinDelay, SpinBArrow, (XtPointer)w);
}
...
static void UpdateChildText(Widget textW)
{
  ...
  if (textW) {
    ...
    if (SB_ChildIsNumeric(textC)) {
      ...
      textT->setValue(textW, (XtPointer)textC->values[textC->position], XmFORMAT_XmSTRING);
    }
  }
}
...
static Boolean ArrowWasHit(Widget arrowW, int arrowType, XEvent *arrowEvent)
{
  ...
  if (arrowEvent->type == ButtonPress) {
    ...
    if (hitX < 0 || hitX > arrowArea.width || hitY < 0 || hitY > arrowArea.height)
      arrowHit = False;
    else
      arrowHit = True;
  }
 ...
 *****************************************************************************/
static void SpinBArrow(XtPointer spinData, XtIntervalId *spinInterval) /* unused */
{
...
static void SpinBAction(Widget actionWidget, short arrowHit)
{
  ...
  if ((upHit && UpArrowSensitive(spinW)) || (downHit && DownArrowSensitive(spinW))) {
                          ...
                          int arrowReason)
{
  ...
  spinBoxCallData->widget = (spinWC->spinBox_class.get_callback_widget) ?
                                (Widget)(*(spinWC->spinBox_class.get_callback_widget))(
                                    (Widget)spinW) :
  ...
  if (SB_ChildCount(spinW) && SB_WithChild(spinW)) {
    ...
    if (arrowReason == XmCR_SPIN_NEXT || arrowReason == XmCR_SPIN_PRIOR)
...
static void ArrowCallback(Widget arrowWidget, XEvent *arrowEvent, int arrowReason)
{
  ...
  FireCallbacks(
      &spinBoxCallData, spinW->spinBox.value_changed_cb, arrowWidget, arrowEvent, arrowReason);
}
...
static Boolean ArrowVerify(Widget arrowWidget, XEvent *arrowEvent, int arrowReason)
{
  ...
  XmSpinBoxCallbackStruct spinBoxCallData;
  FireCallbacks(
      &spinBoxCallData, spinW->spinBox.modify_verify_cb, arrowWidget, arrowEvent, arrowReason);
...
static void SpinNSetValue(Widget nav, XmNavigatorData nav_data, Boolean notify)
{
  ...
  if (!(numericCount = NumericChildCount(spinW)))
    return;
  ...
  for (numericCount = 0, i = 0; i < SB_ChildCount(spinW) && numericCount < 2 && mask; i++) {
    ...
    if (SB_ChildIsNumeric(spinC)) {
      ...
      if (argCount)
        XtSetValues(spinW->composite.children[i], arglist, argCount);
 ...
 *****************************************************************************/
static int __attribute__((unused)) GetMaximumPositionValue(XmSpinBoxConstraint sc)
{
  int max;
 ...
 *****************************************************************************/
static int __attribute__((unused)) GetMinimumPositionValue(XmSpinBoxConstraint sc)
{
  int min;
  if (sc == (XmSpinBoxConstraint)NULL)
    min = 0;
...
static Boolean CvtStringToPositionValue(Display *display,
 ...
 ************************************************************************/
Widget XmCreateSpinBox(Widget parent, String name, ArgList arglist, Cardinal argcount)
{
...
Widget XmVaCreateSpinBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmSpinBoxWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedSpinBox(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmSpinBoxWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
int XmSpinBoxValidatePosition(Widget text_field, int *position)
{
  ...
  _XmAppLock(app);
  textT = (XmAccessTextualTrait)XmeTraitGet((XtPointer)XtClass(text_field), XmQTaccessTextual);
  if (textT == NULL) {
    _XmAppUnlock(app);
    return (XmCURRENT_VALUE);
  }
