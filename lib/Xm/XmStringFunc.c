   ...
   XmString substring)
{
   ...
   */
   ...
   if (((entry = _XmStrEntryGet(substring)) != NULL) && _XmEntrySegmentCountGet(entry[0]) > 1)
   {
      _XmProcessUnlock();
      return (FALSE);
   }
                     ...
                     XmParseModel  parse_model)
{
   ...
   for (i = 0; i < count; i++)
      strs[i] = XmStringUnparse(table[i], tag, tag_type, output_type, parse, parse_count, parse_model);
   _XmProcessUnlock();
   return (strs);
}
...
ContextsMatch(XmStringContext a,
              XmStringContext b)
{
   if ((_XmStrContCurrLine(a) == _XmStrContCurrLine(b)) && (_XmStrContCurrSeg(a) == _XmStrContCurrSeg(b)) && (_XmStrContState(a) == _XmStrContState(b)))
        ...
        XmStringContext end)
{
   ...
   while ((_XmStrContCurrLine(start) < (_XmStrContCurrLine(end) - 1)) || ((_XmStrContCurrLine(start) == _XmStrContCurrLine(end)) && (_XmStrContCurrSeg(start) < _XmStrContCurrSeg(end))) || !LastSeg(start))
   {
                        ...
                        XmStringTable *table)
{
   ...
   if (break_component)
   {
      ...
      b_type = XmeStringGetComponent(&stack_context, TRUE, FALSE, &b_len, &b_val);
      _XmStringContextFree(&stack_context);
   }
   else
                            ...
                            XmOffsetModel offset_model)
{
   ...
   n                    = 0;
   XtSetArg(args[n], XmNrenderTable, &rt);
   n++;
   XtGetValues(widget, args, n);
   /* Work around weird bug with XtGetValues. */
   n = 0;
   XtSetArg(args[n], XmNunitType, &units);
   n++;
   XtGetValues(widget, args, n);
   ...
   for (i = 0; i < num_strings; i++)
   {
      ...
      j = 0;
      while ((ret_val = _XmStringGetNextTabWidth(&stack_ctx, widget, units, rt, &width, &rend)) != XmTAB_EOS)
      {
      ...
      if (_XmStrContCurrLine(ctx) < line_count)
      {
         ...
         while (_XmStrContCurrSeg(ctx) < seg_count)
         {
            ...
            if (_XmStrContTabCount(ctx) < _XmEntryTabsGet(seg))
            {
               ...
               *width = (XmConvertUnits(widget, XmHORIZONTAL, XmPIXELS, w_sum, toType) / divisor);
               return (XmTAB_NEXT);
            }
            (void)_XmStringSegmentExtents(seg, rt, rend, NULL, &w_cur, NULL, NULL, NULL);
            w_sum += w_cur;
