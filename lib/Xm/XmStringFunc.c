...
XmString XmStringTableToXmString(XmStringTable table, Cardinal count, XmString break_comp)
{
...
void XmParseMappingFree(XmParseMapping mapping)
{
  _XmProcessLock();
  if (mapping != NULL) {
    /* Free copied data. */
 ...
 */
XmIncludeStatus XmeGetNextCharacter(XtPointer *in_out,
                                    XtPointer text_end,
                                    XmTextType type,
                                    XmStringTag tag,
...
Cardinal XmStringToXmStringTable(XmString string, XmString break_component, XmStringTable *table)
{
  /* Note: this is a very expensive way to do this.  Fix for Beta */
                                      ...
                                      XmOffsetModel offset_model)
{
  ...
  for (i = 0; i < num_strings; i++) {
    ...
    j = 0;
    while ((ret_val = _XmStringGetNextTabWidth(&stack_ctx, widget, units, rt, &width, &rend)) !=
           XmTAB_EOS)
    {
 ...
 */
NextTabResult _XmStringGetNextTabWidth(XmStringContext ctx,
                                       Widget widget,
                                       unsigned char units,
                                       XmRenderTable rt,
                                       float *width,
