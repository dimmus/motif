...
static XtResource resources[] = {
   { XmNborderWidth,    XmCBorderWidth,    XmRHorizontalDimension, sizeof(Dimension),      XtOffsetOf(struct _XmSashRec,      core.border_width),         XmRImmediate, (XtPointer)0                  },
   { XmNcallback,       XmCCallback,       XmRCallback,            sizeof(XtCallbackList), XtOffsetOf(struct _XmSashRec,      sash.sash_action),          XmRPointer,   NULL                          },
   { XmNnavigationType, XmCNavigationType, XmRNavigationType,      sizeof(unsigned char),  XtOffsetOf(struct _XmPrimitiveRec, primitive.navigation_type), XmRImmediate, (XtPointer)XmSTICKY_TAB_GROUP },
};
...
externaldef(xmsashwidgetclass) WidgetClass xmSashWidgetClass = (WidgetClass)&xmSashClassRec;
   ...
   Widget sash)
{
   ...
   XFillRectangle(XtDisplay(sash), XtWindow(sash), ((XmSashWidget)sash)->primitive.highlight_GC, x, y, sash->core.width - (2 * x), sash->core.height - (2 * y));
}
   ...
   Widget sash)
{
   int x, y;
   x = y = ((XmSashWidget)sash)->primitive.shadow_thickness;
   XClearArea(XtDisplay(sash), XtWindow(sash), x, y, sash->core.width - (2 * x), sash->core.height - (2 * y), FALSE);
}
   ...
   Cardinal *num_params)
{
   ...
   if (_XmGetFocusPolicy((Widget)sash) == XmEXPLICIT)
      HighlightSash(w);
   XmeDrawShadows(XtDisplay(w), XtWindow(w), sash->primitive.top_shadow_GC, sash->primitive.bottom_shadow_GC, 0, 0, w->core.width, w->core.height, sash->primitive.shadow_thickness, XmSHADOW_OUT);
   ...
   XSetWindowAttributes *attributes)
{
   ...
   if (0L == SashCursor)
   {
      /* create some data shared among all instances on this
		...
		*/
      SashCursor = ((XmDisplayInfo *)(dd->display.displayInfo))->SashCursor = XCreateFontCursor(XtDisplay(w), XC_crosshair);
      XtAddCallback((Widget)dd, XtNdestroyCallback, SashDisplayDestroyCallback, (XtPointer)NULL);
   }
...
/*ARGSUSED*/
static void
SashDisplayDestroyCallback(Widget    w,
                           ...
                           XtPointer call_data)   /* unused */
{
   ...
   {
      SashCursor = ((XmDisplayInfo *)(dd->display.displayInfo))->SashCursor;
      if (0L != SashCursor)
      {
