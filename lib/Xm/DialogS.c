...
static Widget GetRectObjKid(CompositeWidget p);
...
static void GetDefaultPosition(Widget child, Widget parent, Position *xRtn, Position *yRtn);
...
****************************************************************************/
  ...
  xmDialogShellExtClassRec.object_class.num_resources = wc_unique_res + sc_num_res;
...
static Widget GetRectObjKid(CompositeWidget p)
{
  Cardinal i;
  Widget *currKid;
  for (i = 0, currKid = p->composite.children; i < p->composite.num_children; i++, currKid++) {
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (!current->core.mapped_when_managed && new_w->core.mapped_when_managed) {
    if ((child = GetRectObjKid((CompositeWidget)new_w)) && !child->core.being_destroyed) {
      if ((trait = (XmDialogSavvyTrait)XmeTraitGet((XtPointer)XtClass(child),
...
static void ChangeManaged(Widget wid)
{
  ...
  if (((child = GetRectObjKid((CompositeWidget)shell)) == NULL) || (child->core.being_destroyed))
    return;
  trait = (XmDialogSavvyTrait)XmeTraitGet((XtPointer)XtClass(child), XmQTdialogShellSavvy);
  /* MANAGED Case first ********/
  if (child->core.managed) {
    ...
    if (trait) {
      if (defaultPosition) {
        GetDefaultPosition(child, XtParent(shell), &request.x, &request.y);
        if (request.x != kidX)
                                        ...
                                        XtWidgetGeometry *reply) /* unused */
{
  ...
  if (XtMakeGeometryRequest((Widget)shell, &my_request, NULL) == XtGeometryYes) {
    if (!(request->request_mode & XtCWQueryOnly)) {
    ...
    WidgetClass w_class, Widget ds_p, String name, ArgList bb_args, Cardinal bb_n)
{
   ...
   */
  XtAddCallback(bb, XmNdestroyCallback, _XmDestroyParentCallback, (XtPointer)NULL);
  /*	Return child.
   */
  return (bb);
}
