                    ...
                    Cardinal *acPtrIn) /* unused */
{
  ...
  if (!tmpColorObj->color_obj.colorIsRunning)
    return;
  ac = 0;
  ...
  if (rcType == XmMENU_BAR) /* set to secondary, rather than primary */ {
    ...
    {
      if (mono)
        ditherPix = XmGetPixmapByDepth(screen,
                                       XmS50_foreground,
      ...
      { /* simulate solid white (will happen for B_W case only)*/
        solidPix = XmGetPixmapByDepth(
            screen, "background", WhitePixelOfScreen(screen), WhitePixelOfScreen(screen), depth);
                                ...
                                &tmpColorObj->color_obj.myColors[color]))
    {
      ...
      { /* simulate solid black (will happen for B_W case only)*/
        solidPix = XmGetPixmapByDepth(
            screen, "background", BlackPixelOfScreen(screen), BlackPixelOfScreen(screen), depth);
