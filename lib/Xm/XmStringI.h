...
/* Convenience macros */
#define _XmStringCvtNonOpt(str) (_XmStrOptimized(str) ? _XmStringOptToNonOpt(str) : (str))
/*
 * Macros for string internal context block data structure access
 */
#define _XmStrContString(cont) ((_XmStringContextRec *)(cont))->string
#define _XmStrContCurrLine(cont) ((_XmStringContextRec *)(cont))->current_line
#define _XmStrContCurrSeg(cont) ((_XmStringContextRec *)(cont))->current_seg
#define _XmStrContOpt(cont) ((_XmStringContextRec *)(cont))->optimized
...
#define _XmStrRefCountGet(str) \
  (_XmStrMultiple(str) ? (str)->multi_str.refcount : \
                         (_XmStrOptimized(str) ? (str)->opt_str.refcount : 1))
#define _XmStrRefCountSet(str, val) \
  (_XmStrMultiple(str) ? ((str)->multi_str.refcount = (val)) : \
                         (_XmStrOptimized(str) ? ((str)->opt_str.refcount = (val)) : 0))
...
#define _XmStrTagGet(str) \
  (_XmStrTagIndex(str) == TAG_INDEX_UNSET ? NULL : _XmStringIndexGetTag(_XmStrTagIndex(str)))
#define _XmStrByteCount(str) ((str)->opt_str.byte_count)
#define _XmStrCharCount(str) \
  _XmStringCharacterCount((str)->opt_str.text, _XmStrTextType(str), _XmStrByteCount(str), NULL)
#define _XmStrRendBegin(str) ((str)->opt_str.rend_begin)
#define _XmStrRendIndex(str) ((str)->opt_str.rend_index)
#define _XmStrRendTagGet(str) \
  (_XmStrRendIndex(str) == REND_INDEX_UNSET ? NULL : _XmStringIndexGetTag(_XmStrRendIndex(str)))
...
#define _XmStrEntryGet(str) (_XmStrMultiple(str) ? _XmStrEntry(str) : (_XmStringEntry *)NULL)
...
#define _XmEntryOptimized(entry) (_XmEntryType(entry) == XmSTRING_ENTRY_OPTIMIZED)
#define _XmEntryMultiple(entry) (_XmEntryType(entry) == XmSTRING_ENTRY_ARRAY)
#define _XmEntryUnoptimized(entry) (_XmEntryType(entry) == XmSTRING_ENTRY_UNOPTIMIZED)
/* Non-array entry macros */
#define _XmEntryTextTypeSet(entry, val) \
  (_XmEntryOptimized(entry) ? (((_XmStringEntry)(entry))->single.text_type = (val)) : \
                              (((_XmStringEntry)(entry))->unopt_single.text_type = (val)))
#define _XmEntryTagIndex(entry) (((_XmStringEntry)(entry))->single.tag_index)
#define _XmUnoptSegTag(entry) ((_XmStringUnoptSeg)(entry))->tag
#define _XmUnoptSegByteCount(entry) ((_XmStringUnoptSeg)(entry))->byte_count
#define _XmEntryByteCountSet(entry, val) \
  (_XmEntryOptimized(entry) ? (((_XmStringEntry)(entry))->single.byte_count = (val)) : \
                              (_XmUnoptSegByteCount(entry) = (val)))
#define _XmEntryCharCountSet(entry, val) \
  (_XmEntryUnoptimized(entry) ? (((_XmStringUnoptSeg)(entry))->char_count = (val)) : 0)
#define _XmEntryRendIndex(entry) (((_XmStringEntry)(entry))->single.rend_index)
#define _XmUnoptSegRendBeginCount(entry) ((_XmStringUnoptSeg)(entry))->begin_count
#define _XmEntryRendBeginCountSet(entry, val) \
  (_XmEntryOptimized(entry) ? (((_XmStringEntry)(entry))->single.rend_begin = (val)) : \
                              (_XmUnoptSegRendBeginCount(entry) = (val)))
#define _XmUnoptSegRendBegins(entry) ((_XmStringUnoptSeg)(entry))->rend_begin_tags
#define _XmUnoptSegRendEndCount(entry) ((_XmStringUnoptSeg)(entry))->end_count
#define _XmEntryRendEndCountSet(entry, val) \
  (_XmEntryOptimized(entry) ? (((_XmStringEntry)(entry))->single.rend_end = (val)) : \
                              (_XmUnoptSegRendEndCount(entry) = (val)))
#define _XmUnoptSegRendEnds(entry) ((_XmStringUnoptSeg)(entry))->rend_end_tags
#define _XmEntryTabsSet(entry, val) \
  (_XmEntryOptimized(entry) ? (((_XmStringEntry)(entry))->single.tabs_before = (val)) : \
                              (((_XmStringEntry)(entry))->unopt_single.tabs_before = (val)))
#define _XmEntryFlippedGet(entry) \
  (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.flipped : \
                              ((_XmStringEntry)(entry))->unopt_single.flipped)
#define _XmEntryFlippedSet(entry, val) \
  (_XmEntryOptimized(entry) ? (((_XmStringEntry)(entry))->single.flipped = (val)) : \
                              (((_XmStringEntry)(entry))->unopt_single.flipped = (val)))
#define _XmEntryPermGet(entry) \
  (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.permanent : \
                              ((_XmStringEntry)(entry))->unopt_single.permanent)
#define _XmEntryPermSet(entry, val) \
  (_XmEntryOptimized(entry) ? (((_XmStringEntry)(entry))->single.permanent = (val)) : \
...
#define _XmEntrySoftNewlineGet(entry) \
  (_XmEntryOptimized(entry) ? \
       ...
       (_XmEntryUnoptimized(entry) ? ((_XmStringEntry)(entry))->unopt_single.soft_line_break : \
...
#define _XmEntryPopSet(entry, val) \
  (_XmEntryUnoptimized(entry) ? (((_XmStringEntry)(entry))->unopt_single.pop_after = (val)) : 0)
#define _XmEntryMBText(entry) (((_XmStringOptSeg)(entry))->data.chars)
#define _XmEntryWCText(entry) (((_XmStringOptSeg)(entry))->data.wchars)
#define _XmEntryCacheSet(entry, val) \
  (_XmEntryUnoptimized(entry) ? (((_XmStringUnoptSeg)(entry))->cache = (val)) : NULL)
...
#define _XmEntryDirtyGet(entry, type, data) \
  (((type) == _XmSCANNING_CACHE) ? \
       (Boolean)(long)_XmScanningCacheGet( \
           (_XmStringNREntry)entry, (XmDirection)(long)data, _XmCACHE_DIRTY) : \
       (((type) == _XmRENDERING_CACHE) ? \
            (Boolean)(long)_XmRenderCacheGet( \
                (_XmStringEntry)entry, (XmRenderTable)(long)data, _XmCACHE_DIRTY) : \
            True))
#define _XmEntryDirtySet(entry, type, data, val) \
  (((type) == _XmSCANNING_CACHE) ? \
       _XmScanningCacheSet((_XmStringNREntry)entry, \
                           ...
                           _XmCACHE_DIRTY, \
                           (XtPointer)(long)val) : \
       (((type) == _XmRENDERING_CACHE) ? _XmRenderCacheSet((_XmStringEntry)entry, \
...
#define _XmEntryXGet(entry, rt) (long)_XmRenderCacheGet(entry, rt, _XmCACHE_RENDER_X)
#define _XmEntryYGet(entry, rt) (long)_XmRenderCacheGet(entry, rt, _XmCACHE_RENDER_Y)
#define _XmEntryWidthGet(entry, rt) (long)_XmRenderCacheGet(entry, rt, _XmCACHE_RENDER_WIDTH)
#define _XmEntryHeightGet(entry, rt) (long)_XmRenderCacheGet(entry, rt, _XmCACHE_RENDER_HEIGHT)
#define _XmEntryBaselineGet(entry, rt) (long)_XmRenderCacheGet(entry, rt, _XmCACHE_RENDER_BASELINE)
#define _XmEntryAscentGet(entry, rt) (long)_XmRenderCacheGet(entry, rt, _XmCACHE_RENDER_ASCENT)
#define _XmEntryDescentGet(entry, rt) (long)_XmRenderCacheGet(entry, rt, _XmCACHE_RENDER_DESCENT)
...
#define _XmStrLineSegment(line) ((_XmStringSegment *)_XmEntrySegment(line))
#define _XmStrLineSegmentSet(line, val) _XmEntrySegment(line) = (_XmStringNREntry *)(val)
/*
...
#define _XmStrLineLineSet(str, val) _XmStrEntry(str) = (_XmStringEntry *)(val)
/****************************************************************
 *
 * Function headers
 *
...
extern XFontStruct *_XmGetFirstFont(XmFontListEntry entry);
extern Boolean _XmFontListSearch(XmFontList fontlist,
...
extern Dimension _XmStringWidth(XmFontList fontlist, _XmString string);
extern Dimension _XmStringHeight(XmFontList fontlist, _XmString string);
extern void _XmStringExtent(XmFontList fontlist,
                            _XmString string,
                            Dimension *width,
                            Dimension *height);
extern Boolean _XmStringEmpty(_XmString string);
extern void _XmStringDraw(Display *d,
                          Window w,
                          XmFontList fontlist,
                          _XmString string,
                          GC gc,
                          ...
                          XRectangle *clip);
extern void _XmStringDrawImage(Display *d,
                               Window w,
                               XmFontList fontlist,
                               _XmString string,
                               GC gc,
                               ...
                               unsigned char lay_dir,
                               XRectangle *clip);
extern void _XmStringDrawUnderline(Display *d,
                                   Window w,
                                   XmFontList f,
                                   _XmString s,
                                   GC gc,
                                   ...
                                   XRectangle *clip,
                                   _XmString u);
extern void _XmStringDrawMnemonic(Display *d,
                                  Window w,
                                  XmFontList fontlist,
                                  _XmString string,
                                  GC gc,
...
extern char *_XmCharsetCanonicalize(String charset);
...
extern Dimension _XmStringBaseline(XmFontList fontlist, _XmString string);
...
extern Boolean _XmStringIsCurrentCharset(XmStringCharSet c);
extern Boolean _XmStringSingleSegment(XmString str, char **pTextOut, XmStringCharSet *pCharsetOut);
...
extern Boolean _XmStringSegmentExtents(_XmStringEntry entry,
                                       ...
                                       Dimension *ascent,
                                       Dimension *descent);
extern void _XmStringLayout(_XmString string, XmDirection direction);
...
extern XmString _XmStringMakeXmString(_XmStringEntry **entries, int count);
...
extern unsigned char _XmStringCharacterCount(XtPointer text,
                                             ...
                                             int byte_count,
                                             XFontStruct *font);
extern unsigned char _XmEntryCharCountGet(_XmStringEntry entry, XmRenderTable rt);
extern _XmStringCache _XmStringCacheGet(_XmStringCache caches, int type);
extern void _XmStringCacheFree(_XmStringCache caches);
extern XtPointer _XmScanningCacheGet(_XmStringNREntry entry, XmDirection d, int field);
extern void _XmScanningCacheSet(_XmStringNREntry entry, XmDirection d, int field, XtPointer value);
/* Rendering cache */
extern XtPointer _XmRenderCacheGet(_XmStringEntry entry, XmRenderTable rt, int field);
extern void _XmRenderCacheSet(_XmStringEntry entry, XmRenderTable rt, int field, XtPointer value);
...
extern XmStringTag _XmEntryRendEndGet(_XmStringEntry entry, int n);
extern XmStringTag _XmEntryRendBeginGet(_XmStringEntry entry, int n);
extern void _XmEntryRendEndSet(_XmStringEntry entry, XmStringTag tag, int n);
extern void _XmEntryRendBeginSet(_XmStringEntry entry, XmStringTag tag, int n);
