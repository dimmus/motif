   ...
   Widget wid)
{
   ...
   Boolean               popdown;
   if (Lab_IsMenupane(cb) && (((XmManagerWidget)XtParent(cb))->manager.active_child == wid) && CB_Submenu(cb))
   {
      XmMenuShellWidget mshell = (XmMenuShellWidget)XtParent(CB_Submenu(cb));
      if ((mshell->composite.children[0] == CB_Submenu(cb)) && (XmIsMenuShell(mshell)) && (mshell->shell.popped_up))
      {
         popdown = True;
      }
      else
   ...
   register XmCascadeButtonWidget cb)
{
   ...
   if (CB_IsArmed(cb))
   {
      ...
      {
         XmeDrawShadows(XtDisplay(cb), XtWindow(cb), cb->primitive.top_shadow_GC, cb->primitive.bottom_shadow_GC, cb->primitive.highlight_thickness, cb->primitive.highlight_thickness, cb->core.width - 2 * cb->primitive.highlight_thickness, cb->core.height - 2 * cb->primitive.highlight_thickness, cb->primitive.shadow_thickness, etched_in ? XmSHADOW_IN : XmSHADOW_OUT);
      }
   }
}
   ...
   register XmCascadeButtonWidget cb)
{
   if ((CB_HasCascade(cb)) && (CB_Cascade_width(cb) != 0))
   {
      Pixmap pixmap;
      int    depth;
      pixmap = CB_IsArmed(cb) && (CB_ArmedPixmap(cb) != XmUNSPECIFIED_PIXMAP) ? CB_ArmedPixmap(cb) : CB_CascadePixmap(cb);
   ...
   Region          region)
{
   ...
   if (XtIsRealized(cb))
   {
      ...
      {
         if (CB_IsArmed(cb))
            XFillRectangle(XtDisplay(cb), XtWindow(cb), CB_ArmGC(cb), 0, 0, cb->core.width, cb->core.height);
         else
            XClearArea(XtDisplay(cb), XtWindow(cb), 0, 0, cb->core.width, cb->core.height, False);
#if USE_XFT
      }
      else if (Lab_MenuType(cb) != XmWORK_AREA)
      { /* adeed with XFT support */
         XClearArea(XtDisplay(cb), XtWindow(cb), 0, 0, cb->core.width, cb->core.height, False);
#endif
      }
      if (etched_in && CB_IsArmed(cb))
      {
         ...
         XmGetColors(XtScreen(cb), cb->core.colormap, cb->core.background_pixel, &junk, &junk, &junk, &select_pix);
         if (select_pix == ((XmCascadeButtonWidget)cb)->primitive.foreground)
         {
            replaceGC                                    = True;
            tmpGC                                        = ((XmCascadeButtonWidget)cb)->label.normal_GC;
            ((XmCascadeButtonWidget)cb)->label.normal_GC = CB_BackgroundGC(cb);
         }
         ...
         if (cb->core.background_pixmap != XmUNSPECIFIED_PIXMAP)
            XSetWindowBackgroundPixmap(XtDisplay(cb), XtWindow(cb), cb->core.background_pixmap);
         if (replaceGC)
   ...
   switch (Lab_MenuType(cb))
   {
      case XmMENU_BAR:
      {
         ...
         if (myShell && XmIsMenuShell(myShell) && /* not torn ?! */
             (myShell->shell.popped_up) && (myShell->composite.children[0] == CB_Submenu(cb)) && (cb == (XmCascadeButtonWidget)RC_CascadeBtn(CB_Submenu(cb))))
         {
      ...
      if ((!RC_PoppingDown(rowcol)) || RC_TornOff(rowcol))
      {
         if (XtIsRealized((Widget)cb))
         {
            ...
            if (etched_in)
               ...
               XmeClearBorder(XtDisplay(cb), XtWindow(cb), cb->primitive.highlight_thickness, cb->primitive.highlight_thickness, cb->core.width - 2 * cb->primitive.highlight_thickness, cb->core.height - 2 * cb->primitive.highlight_thickness, cb->primitive.shadow_thickness);
         }
      }
      DrawCascade(cb);
   }
...
{
   XmCascadeButtonWidget cb = (XmCascadeButtonWidget)wid;
   if ((!CB_IsArmed(cb)) && (((XmMenuShellWidget)XtParent(XtParent(cb)))->shell.popped_up) && _XmGetInDragMode((Widget)cb))
   {
      ...
      else
      {
	    ...
	    the menu wouldn't post. */
         Arm(cb);
         cb->cascade_button.timer = XtAppAddTimeOut(XtWidgetToApplicationContext((Widget)cb),
   ...
   Cardinal *num_param)
{
   ...
   if (_XmGetInDragMode((Widget)cb) && (/* !ActiveTearOff || */ event->xcrossing.mode == NotifyNormal))
   {
      if ((CB_IsArmed(cb)) && (CB_Submenu(cb)))
      {
         ...
         if (submenushell->shell.popped_up)
         {
            ...
            /*
              ...
              */
            if ((entEvent->mode == NotifyGrab) && !XmIsMenuShell(XtParent(XtParent(cb))))
               return;
         }
      }
      Disarm(cb, TRUE);
   ...
   Cardinal *num_param)
{
   ...
   if (CB_Submenu(cb) && RC_IsArmed((XmRowColumnWidget)CB_Submenu(cb)))
   {
    ...
    */
   if (Lab_IsMenupane(cb) && (!((XmMenuShellWidget)XtParent(parent))->shell.popped_up))
   {
      /* To support menu replay, keep the pointer in sync mode */
      XAllowEvents(XtDisplay(cb), SyncPointer, CurrentTime);
      return;
   ...
   Boolean                        doCascade)
{
   ...
   if (CB_Submenu(cb) == NULL)
   {
      ...
      if ((!cb->label.skipCallback) && (cb->cascade_button.activate_callback))
      {
   ...
   Cardinal *num_param)
{
   ...
   /* To support menu replay, keep the pointer in sync mode */
   XAllowEvents(XtDisplay(cb), SyncPointer, CurrentTime);
   if (event && event->type == ButtonRelease && event->xbutton.button == 2)
      return;
   if (!CB_IsArmed(cb))
      return;
    ...
    * that can occur with rapid pressing of the mouse button
    */
   if (Lab_IsMenupane(cb) && (!((XmMenuShellWidget)XtParent(XtParent(cb)))->shell.popped_up))
   {
      return;
   }
...
{
   XmCascadeButtonWidget cb = (XmCascadeButtonWidget)wid;
   if (!_XmGetInDragMode((Widget)cb) && (RC_IsArmed(XtParent(cb)) || (RC_Type(XtParent(cb)) != XmMENU_BAR && !XmIsMenuShell(XtParent(XtParent(cb))))))
      (*(((XmCascadeButtonClassRec *)(cb->core.widget_class))->primitive_class.arm_and_activate))((Widget)cb, event, NULL, NULL);
}
   ...
   else
   {
      ...
      if (validButton)
      {
	     ...
	     */
            ...
            {
               _XmGrabPointer(XtParent(cb), True, EVENTS, GrabModeAsync, GrabModeAsync, None, _XmGetMenuCursorByScreen(XtScreen(cb)), _time);
   ...
   Cardinal *num_param)
{
   ...
   if (RC_IsArmed(parent))
   {
      (*((XmRowColumnWidgetClass)XtClass(parent))->row_column_class.armAndActivate)((Widget)parent,
                                                                                    (XEvent *)NULL,
                                                                                    (String *)NULL,
                                                                                    (Cardinal *)NULL);
      _XmRecordEvent(event);
   ...
   Cardinal *num_params)
{
   ...
   else if ((RC_Type(parent) == XmMENU_PULLDOWN) || (RC_Type(parent) == XmMENU_POPUP))
   {
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownDone))(XtParent(parent), event, params, num_params);
   }
   if (XmIsGadget(w))
   ...
   XmRowColumnWidget rowcol)
{
   ...
   if ((menuShell = (CompositeWidget)
       ...
       != NULL)
   {
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))((Widget)menuShell, NULL, NULL, NULL);
   }
}
   ...
   XEvent *event)
{
   ...
   if (XmIsCascadeButton(w))
   {
      ...
      if (submenu)
      {
         ...
         if (XmIsMenuShell(ms) && ms->shell.popped_up && ms->composite.children[0] == (Widget)submenu && submenu->row_column.cascadeBtn == (Widget)cb)
         {
            return;
         }
      }
   ...
   Boolean doCascade)
{
   ...
   {
      excPP->num_panes = 1;
      if (RC_TornOff(*(excPP->pane)) && !XmIsMenuShell(XtParent(*(excPP->pane))))
      {
 	  ...
 	  */
         ...
         {
            XmRowColumnWidget postedPane = (XmRowColumnWidget)((CompositeWidget)RC_PopupPosted(XtParent(cb)))->composite.children[0];
   ...
   XEvent *event)
{
   ...
   if (submenu && (shell = (XmMenuShellWidget)XtParent(submenu)) && XmIsMenuShell(shell) && (popped_up = shell->shell.popped_up))
   {
      ...
      if ((XmRowColumnWidget)shell->composite.children[0] == submenu)
      {
         if (cb == RC_CascadeBtn(submenu))
         {
            if (RC_PopupPosted(submenu))
               (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(RC_PopupPosted(submenu), NULL, NULL, NULL);
            if (submenu->manager.active_child)
            {
               /* update visible focus/highlighting */
               if (XmIsPrimitive(submenu->manager.active_child))
               {
                  (*(((XmPrimitiveClassRec *)XtClass(submenu->manager.active_child))->primitive_class.border_unhighlight))(submenu->manager.active_child);
               }
               else if (XmIsGadget(submenu->manager.active_child))
               {
                  (*(((XmGadgetClassRec *)XtClass(submenu->manager.active_child))->gadget_class.border_unhighlight))(submenu->manager.active_child);
               }
   ...
   if (XtIsManaged((Widget)parent))
   {
      ...
      if (oldActiveChild && (oldActiveChild != (Widget)cb) && XmIsCascadeButtonGadget(oldActiveChild) && CBG_Submenu(oldActiveChild) && (((XmMenuShellWidget)XtParent(CBG_Submenu(oldActiveChild)))->shell.popped_up == False))
      {
         parent->manager.active_child = NULL;
         _XmDispatchGadgetInput((Widget)oldActiveChild, NULL, XmFOCUS_OUT_EVENT);
         ((XmGadget)oldActiveChild)->gadget.have_traversal = False;
      }
      else
         /*
	   ...
	   */
         if (!submenu || !popped_up || (RC_PopupPosted(parent) != (Widget)shell) || (submenu && RC_CascadeBtn(submenu) && (RC_CascadeBtn(submenu) != cb) && ((Widget)parent == XtParent(RC_CascadeBtn(submenu)))))
         {
	    ...
	    */
            ...
            {
               (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(RC_PopupPosted(parent), NULL, NULL, NULL);
            }
            /* Focus is not being handled perfectly for tear offs whose parent
	    * is a top level shell.  So force cascade unhighlighting here.
	    */
            if (NULL != oldActiveChild && oldActiveChild != cb && ((parent->row_column.type == XmMENU_PULLDOWN) || (parent->row_column.type == XmMENU_POPUP)) && !XmIsMenuShell(XtParent(parent)))
      ...
      *(excPP->pane)   = NULL;
      ...
      if (submenu)
      {
         ...
         else
         {
	       ...
	       */
            (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popupSharedMenupane))(cb,
                                                                                                       (Widget)submenu,
                                                                                                       event);
         }
   ...
   XmCascadeButtonWidget cascadebtn)
{
   ...
   {
      XGetGeometry(XtDisplay(cascadebtn), CB_CascadePixmap(cascadebtn), &rootwin, &x, &y, &width, &height, &border, &depth);
   ...
   XmCascadeButtonWidget cascadebtn)
{
   ...
   if (CB_HasCascade(cascadebtn))
   {
      if (LayoutIsRtoLP(cascadebtn))
         CB_Cascade_x(cascadebtn) = cascadebtn->primitive.highlight_thickness + cascadebtn->primitive.shadow_thickness + Lab_MarginWidth(cascadebtn);
      else
         CB_Cascade_x(cascadebtn) = XtWidth(cascadebtn) - cascadebtn->primitive.highlight_thickness - cascadebtn->primitive.shadow_thickness - Lab_MarginWidth(cascadebtn) - CB_Cascade_width(cascadebtn);
      buffer                   = cascadebtn->primitive.highlight_thickness + cascadebtn->primitive.shadow_thickness + Lab_MarginHeight(cascadebtn);
      CB_Cascade_y(cascadebtn) = buffer + ((XtHeight(cascadebtn) - 2 * buffer) - CB_Cascade_height(cascadebtn)) / 2;
   }
   else
   {
   ...
   Boolean               adjustHeight)
{
   ...
   if (CB_HasCascade(cascadebtn))
   {
      ...
      if (LayoutIsRtoLP(cascadebtn))
      {
         if ((CB_Cascade_width(cascadebtn) + CASCADE_PIX_SPACE) > Lab_MarginLeft(cascadebtn))
         {
            delta                       = CB_Cascade_width(cascadebtn) + CASCADE_PIX_SPACE - Lab_MarginLeft(cascadebtn);
      ...
      {
         if ((CB_Cascade_width(cascadebtn) + CASCADE_PIX_SPACE) > Lab_MarginRight(cascadebtn))
         {
            delta                        = CB_Cascade_width(cascadebtn) + CASCADE_PIX_SPACE - Lab_MarginRight(cascadebtn);
       ...
       */
      delta = CB_Cascade_height(cascadebtn) + 2 * (Lab_MarginHeight(cascadebtn) + cascadebtn->primitive.shadow_thickness + cascadebtn->primitive.highlight_thickness);
   ...
   Cardinal *num_args) /* unused */
{
   ...
   if ((CB_Submenu(new_w)) && ((!XmIsRowColumn(CB_Submenu(new_w))) || (RC_Type(CB_Submenu(new_w)) != XmMENU_PULLDOWN)))
   {
   ...
   else if (Lab_IsMenupane(new_w))
   {
      ...
      else if (((CB_CascadePixmap(new_w) == XmUNSPECIFIED_PIXMAP) && (!CB_Submenu(old) && CB_Submenu(new_w))) || ((CB_ArmedPixmap(old) != XmUNSPECIFIED_PIXMAP) && ((Lab_TextRect_height(old) != Lab_TextRect_height(new_w)) || (old->primitive.foreground != new_w->primitive.foreground) || (old->core.background_pixel != new_w->core.background_pixel))))
      {
         _XmProcessLock();
         _XmArrowPixmapCacheDelete((XtPointer)CB_ArmedPixmap(old));
         _XmArrowPixmapCacheDelete((XtPointer)CB_CascadePixmap(old));
	...
	*/
      if ((CB_CascadePixmap(old) != CB_CascadePixmap(new_w)) || (old->label.label_type != new_w->label.label_type) || (old->cascade_button.submenu != new_w->cascade_button.submenu))
      {
      ...
      {
         if ((new_w->primitive.highlight_thickness != old->primitive.highlight_thickness) || (new_w->primitive.shadow_thickness != old->primitive.shadow_thickness) || (Lab_MarginRight(new_w) != Lab_MarginRight(old)) || (Lab_MarginHeight(new_w) != Lab_MarginHeight(old)) || (Lab_MarginTop(new_w) != Lab_MarginTop(old)) || (Lab_MarginBottom(new_w) != Lab_MarginBottom(old)))
         {
            setup_cascade(new_w, adjustWidth, adjustHeight);
            flag = TRUE;
         }
         else if ((Lab_MarginWidth(new_w) != Lab_MarginWidth(old)) || (new_w->core.width != old->core.width) || (new_w->core.height != old->core.height))
         {
   ...
   XmCascadeButtonWidget cb)
{
   ...
   XmGetColors(XtScreen(cb), cb->core.colormap, cb->core.background_pixel, &junk, &junk, &junk, &select_pixel);
   ...
   Cardinal *num_args) /* unused */
{
   ...
   menuSTrait                    = (XmMenuSystemTrait)
      XmeTraitGet((XtPointer)XtClass((Widget)parent), XmQTmenuSystem);
   if ((!XmIsRowColumn(parent)) && ((Lab_MenuType(new_w) == XmMENU_PULLDOWN) || (Lab_MenuType(new_w) == XmMENU_POPUP) || (Lab_MenuType(new_w) == XmMENU_BAR)))
   {
