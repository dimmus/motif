 ...
 */
   ...
   _XmPushWidgetExtData(new_w, extData, ((XmCascadeButtonGCacheObject)newSec)->ext.extensionType);
   ...
   XtPointer B)
{
   ...
   if ((cascadeB_inst->cascade_pixmap == cascadeB_cache_inst->cascade_pixmap) && (cascadeB_inst->map_delay == cascadeB_cache_inst->map_delay) && (cascadeB_inst->armed_pixmap == cascadeB_cache_inst->armed_pixmap) && (cascadeB_inst->arm_gc == cascadeB_cache_inst->arm_gc) && (cascadeB_inst->background_gc == cascadeB_cache_inst->background_gc))
      return 1;
   else
      return 0;
}
   ...
   XtPointer B)
{
   ...
   if ((arrowpix_rec->height == arrowpix_cache_rec->height) && (arrowpix_rec->width == arrowpix_cache_rec->width) && (arrowpix_rec->screen == arrowpix_cache_rec->screen) && (arrowpix_rec->depth == arrowpix_cache_rec->depth) && (arrowpix_rec->direction == arrowpix_cache_rec->direction) && (arrowpix_rec->top_shadow_color == arrowpix_cache_rec->top_shadow_color) && (arrowpix_rec->bottom_shadow_color == arrowpix_cache_rec->bottom_shadow_color) && (arrowpix_rec->foreground_color == arrowpix_cache_rec->foreground_color))
      return 1;
   else
      return 0;
}
   ...
   Widget wid)
{
   ...
   if (LabG_MenuType(cb) == XmMENU_OPTION)
      (*((XmGadgetClass)xmGadgetClass)->gadget_class.border_highlight)((Widget)cb);
   else
      Arm(cb);
}
   ...
   Widget wid)
{
   XmCascadeButtonGadget cb = (XmCascadeButtonGadget)wid;
   if (LabG_MenuType(cb) == XmMENU_OPTION)
      (*((XmGadgetClass)xmGadgetClass)->gadget_class.border_unhighlight)((Widget)cb);
   else
      Disarm(cb, False);
}
   ...
   register XmCascadeButtonGadget cb)
{
   ...
   {
      /* draw the casacade */
      if ((LabG_MenuType(cb) == XmMENU_OPTION) && (CBG_CascadePixmap(cb) == XmUNSPECIFIED_PIXMAP))
      {
         ...
         if (LayoutIsRtoLG(cb))
         {
            ...
            XFillRectangle(XtDisplay(cb), XtWindow(XtParent(cb)), LabG_BackgroundGC(cb), cb->rectangle.x + CBG_Cascade_x(cb) + offset_x, cb->rectangle.y + CBG_Cascade_y(cb) + offset_y, width, height);
            XmeDrawShadows(XtDisplay(cb), XtWindow(XtParent(cb)), LabG_TopShadowGC(cb), LabG_BottomShadowGC(cb), cb->rectangle.x + CBG_Cascade_x(cb) + offset_x - G_ShadowThickness(cb), cb->rectangle.y + CBG_Cascade_y(cb) + offset_y - G_ShadowThickness(cb), width + (2 * G_ShadowThickness(cb)), height + (2 * G_ShadowThickness(cb)), G_ShadowThickness(cb), XmSHADOW_OUT);
         }
         else
         {
            XFillRectangle(XtDisplay(cb), XtWindow(XtParent(cb)), LabG_BackgroundGC(cb), cb->rectangle.x + CBG_Cascade_x(cb) + G_ShadowThickness(cb), cb->rectangle.y + CBG_Cascade_y(cb) + offset_y, width, height);
            XmeDrawShadows(XtDisplay(cb), XtWindow(XtParent(cb)), LabG_TopShadowGC(cb), LabG_BottomShadowGC(cb), cb->rectangle.x + CBG_Cascade_x(cb), cb->rectangle.y + CBG_Cascade_y(cb) + offset_y - G_ShadowThickness(cb), width + (2 * G_ShadowThickness(cb)), height + (2 * G_ShadowThickness(cb)), G_ShadowThickness(cb), XmSHADOW_OUT);
         }
      }
      else
      {
         Pixmap pixmap;
         int    depth;
         pixmap = CBG_IsArmed(cb) && (CBG_ArmedPixmap(cb) != XmUNSPECIFIED_PIXMAP) ? CBG_ArmedPixmap(cb) : CBG_CascadePixmap(cb);
         ...
         if (depth == XtParent(cb)->core.depth)
            XCopyArea(XtDisplay(cb), pixmap, XtWindow(XtParent(cb)), LabG_NormalGC(cb), 0, 0, CBG_Cascade_width(cb), CBG_Cascade_height(cb), cb->rectangle.x + CBG_Cascade_x(cb), cb->rectangle.y + CBG_Cascade_y(cb));
         else if (depth == 1)
            XCopyPlane(XtDisplay(cb), pixmap, XtWindow(XtParent(cb)), LabG_NormalGC(cb), 0, 0, CBG_Cascade_width(cb), CBG_Cascade_height(cb), cb->rectangle.x + CBG_Cascade_x(cb), cb->rectangle.y + CBG_Cascade_y(cb), 1);
      }
   }
}
   ...
   XmCascadeButtonGadget cascadebtn)
{
   ...
   if (CBG_HasCascade(cascadebtn))
   {
      if (LayoutIsRtoLG(cascadebtn))
         CBG_Cascade_x(cascadebtn) = cascadebtn->gadget.highlight_thickness + cascadebtn->gadget.shadow_thickness + LabG_MarginWidth(cascadebtn);
      else
         CBG_Cascade_x(cascadebtn) = cascadebtn->rectangle.width - cascadebtn->gadget.highlight_thickness - cascadebtn->gadget.shadow_thickness - LabG_MarginWidth(cascadebtn) - CBG_Cascade_width(cascadebtn);
      buffer                    = cascadebtn->gadget.highlight_thickness + cascadebtn->gadget.shadow_thickness + LabG_MarginHeight(cascadebtn);
      CBG_Cascade_y(cascadebtn) = buffer + ((cascadebtn->rectangle.height - 2 * buffer) - CBG_Cascade_height(cascadebtn)) / 2;
   }
   else
   {
   ...
   Region  region)
{
	...
	*/
      ...
      if (etched_in)
      {
         XFillRectangle(XtDisplay(cb), XtWindow(XtParent(cb)), CBG_IsArmed(cb) ? CBG_ArmGC(cb) : CBG_BackgroundGC(cb), cb->rectangle.x, cb->rectangle.y, cb->rectangle.width, cb->rectangle.height);
      }
      if (etched_in && CBG_IsArmed(cb))
      {
         ...
         XGCValues       values;
         GC              tmp_bgc = NULL;
         XmGetColors(XtScreen(mw), mw->core.colormap, mw->core.background_pixel, &junk, &junk, &junk, &select_pix);
   ...
   Mask    event_mask)
{
   ...
   {
      if ((LabG_MenuType(cb) == XmMENU_PULLDOWN) || (LabG_MenuType(cb) == XmMENU_POPUP) || (LabG_MenuType(cb) == XmMENU_BAR))
      {
   ...
   else if (event_mask & XmLEAVE_EVENT)
   {
      ...
      (*(((XmCascadeButtonGadgetClassRec *)(cb->object.widget_class))->gadget_class.border_highlight))((Widget)cb);
   else if (event_mask & XmFOCUS_OUT_EVENT)
   {
      if (LabG_IsMenupane(cb) && (((XmManagerWidget)XtParent(cb))->manager.active_child == wid) && CBG_Submenu(cb))
      {
         XmMenuShellWidget mshell = (XmMenuShellWidget)XtParent(CBG_Submenu(cb));
         if ((mshell->composite.children[0] == CBG_Submenu(cb)) && (XmIsMenuShell(mshell)) && (mshell->shell.popped_up))
         {
            return;
         }
      }
      (*(((XmCascadeButtonGadgetClassRec *)(cb->object.widget_class))->gadget_class.border_unhighlight))((Widget)cb);
   }
...
Arm(
   XmCascadeButtonGadget cb)
{
   if ((LabG_MenuType(cb) != XmMENU_OPTION) && (!CBG_IsArmed(cb)))
   {
      XmDisplay dpy       = (XmDisplay)XmGetXmDisplay(XtDisplay(cb));
      Boolean   etched_in = dpy->display.enable_etched_in_menu;
   ...
   XEvent               *event)
{
   ...
   if (!CBG_IsArmed(cb))
   {
      if ((LabG_MenuType(cb) == XmMENU_OPTION) && (XtParent(cb) == mst->RC_ReplayInfo.toplevel_menu) && (event->xbutton.time == mst->RC_ReplayInfo.time))
         return;
   ...
   Cardinal *num_params) /* unused */
{
   ...
   switch (LabG_MenuType(cb))
   {
      ...
      case XmMENU_BAR:
      {
         ...
         if (myShell && XmIsMenuShell(myShell) && /* not torn ?! */
             (myShell->shell.popped_up) && (myShell->composite.children[0] == CBG_Submenu(cb)) && (cb == (XmCascadeButtonGadget)RC_CascadeBtn(CBG_Submenu(cb))))
         {
   ...
   Boolean               unpost)
{
   ...
   if (CBG_IsArmed(cb))
   {
      ...
      {
         (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(RC_PopupPosted(rowcol), NULL, NULL, NULL);
      }
         ...
         if (XtIsRealized(rowcol))
         {
            ...
            if (etched_in)
               ...
               XmeDrawHighlight(XtDisplay(cb),
                                ...
                                cb->rectangle.width - 2 * cb->gadget.highlight_thickness,
                                cb->rectangle.height - 2 * cb->gadget.highlight_thickness,
...
DelayedArm(
   ...
   XEvent               *event)
{
   if ((!CBG_IsArmed(cb)) && (((XmMenuShellWidget)XtParent(XtParent(cb)))->shell.popped_up) && _XmGetInDragMode((Widget)cb))
   {
	  ...
	  *
	  */
         CBG_Timer(cb) = XtAppAddTimeOut(XtWidgetToApplicationContext((Widget)cb),
   ...
   XEvent               *event)
{
   ...
   {
      if ((CBG_IsArmed(cb)) && (CBG_Submenu(cb)))
      {
         submenushell = (XmMenuShellWidget)XtParent(CBG_Submenu(cb));
         if (submenushell->shell.popped_up)
         {
            if ((entEvent->x_root >= submenushell->core.x) && (entEvent->x_root < submenushell->core.x + submenushell->core.width + (submenushell->core.border_width << 1)) && (entEvent->y_root >= submenushell->core.y) && (entEvent->y_root < submenushell->core.y + submenushell->core.height + (submenushell->core.border_width << 1)))
   ...
   XEvent               *event)
{
   ...
   CBG_SetWasPosted(cb, FALSE);
   if (CBG_Submenu(cb) && RC_IsArmed((XmRowColumnWidget)CBG_Submenu(cb)))
   {
      CBG_SetWasPosted(cb, TRUE);
   }
   if (LabG_IsMenupane(cb) && !mshell->shell.popped_up)
   {
      return;
   }
   /* In case the tear off is active but not armed or grabbed */
   ...
   Boolean               doCascade)
{
   ...
   if (CBG_Submenu(cb) == NULL)
   {
      ...
      if ((!LabG_SkipCallback(cb)) && (CBG_ActivateCall(cb)))
      {
   ...
   XEvent               *event)
{
   ...
   if ((LabG_MenuType(cb) == XmMENU_BAR) && !RC_IsArmed(XtParent(cb)))
      return;
   /*
    * make sure the shell is popped up, this takes care of a corner case
    * that can occur with rapid pressing of the mouse button
    */
   if (LabG_IsMenupane(cb) && (!((XmMenuShellWidget)XtParent(XtParent(cb)))->shell.popped_up))
   {
      return;
   }
      ...
      if (validButton)
      {
	     ...
	     */
            if (RC_BeingArmed(XtParent(cb)))
            {
               ...
               _XmGrabPointer(XtParent(cb), True, EVENTS, GrabModeAsync, GrabModeAsync, None, cursor, _time);
   ...
   XEvent *event)
{
       ...
       && _XmGetInDragMode((Widget)cb))
   {
      if (!CBG_Submenu(cb))
      {
         _XmMenuFocus((Widget)rc, XmMENU_MIDDLE, _XmGetDefaultTime(wid, event));
      }
      _XmCascadingPopup((Widget)cb, event, TRUE);
   ...
   Widget wid)
{
   ...
   if (XmIsGadget(cb))
   {
      if ((CBG_CascadePixmap(cb) != XmUNSPECIFIED_PIXMAP) && (CBG_ArmedPixmap(cb) != XmUNSPECIFIED_PIXMAP))
         return;
   ...
   {
      Pixel select_pixel;
      if ((CB_CascadePixmap(cb) != XmUNSPECIFIED_PIXMAP) && (CB_ArmedPixmap(cb) != XmUNSPECIFIED_PIXMAP))
         return;
   ...
   armed_arrow               = (XmArrowPixmap *)
      _XmCachePart(&ArrowPixmapCache, (XtPointer)&cpart, sizeof(XmArrowPixmap));
   _XmProcessUnlock();
   if ((unarmed_arrow->pixmap == XmUNSPECIFIED_PIXMAP) || (armed_arrow->pixmap == XmUNSPECIFIED_PIXMAP))
   {
      ...
      {
         pixmap              = XCreatePixmap(XtDisplay(cb), RootWindowOfScreen(screen), side, side, depth);
         armed_arrow->pixmap = pixmap;
         XFillRectangle(XtDisplay(cb), pixmap, etched_in ? armGC : gc, 0, 0, side, side);
         XmeDrawArrow(XtDisplay((Widget)cb), pixmap, bsGC, tsGC, gc, ht + st - 1, ht + st - 1, side - 2 * (ht + st - 1), side - 2 * (ht + st - 1), st, arrow_direction);
      }
      ...
      if (unarmed_arrow->pixmap == XmUNSPECIFIED_PIXMAP)
      {
         ...
         XmeDrawArrow(XtDisplay((Widget)cb), pixmap, tsGC, bsGC, gc, ht + st - 1, ht + st - 1, side - 2 * (ht + st - 1), side - 2 * (ht + st - 1), st, arrow_direction);
      }
   ...
   XmCascadeButtonGadget cascadebtn)
{
   ...
   {
      XGetGeometry(XtDisplay(cascadebtn), CBG_CascadePixmap(cascadebtn), &rootwin, &x, &y, &width, &height, &border, &depth);
   ...
   else
   {
      ...
      {
         CBG_Cascade_width(cascadebtn) = CBG_Cascade_height(cascadebtn) = MAX(LabG_TextRect(cascadebtn).height,
   ...
   Boolean               adjustHeight)
{
   ...
   if (CBG_HasCascade(cascadebtn))
   {
      ...
      if (LayoutIsRtoLG(cascadebtn))
      {
         if ((CBG_Cascade_width(cascadebtn) + CASCADE_PIX_SPACE) > LabG_MarginLeft(cascadebtn))
         {
            delta                        = CBG_Cascade_width(cascadebtn) + CASCADE_PIX_SPACE - LabG_MarginLeft(cascadebtn);
      ...
      {
         if ((CBG_Cascade_width(cascadebtn) + CASCADE_PIX_SPACE) > LabG_MarginRight(cascadebtn))
         {
            delta                        = CBG_Cascade_width(cascadebtn) + CASCADE_PIX_SPACE - LabG_MarginRight(cascadebtn);
            LabG_MarginRight(cascadebtn) = LabG_MarginRight(cascadebtn) + delta;
            if (adjustWidth)
               cascadebtn->rectangle.width += delta;
            else
            {
       ...
       */
      delta = CBG_Cascade_height(cascadebtn) + 2 * (LabG_MarginHeight(cascadebtn) + cascadebtn->gadget.shadow_thickness + cascadebtn->gadget.highlight_thickness);
      if (delta > cascadebtn->rectangle.height)
      {
         delta                        -= cascadebtn->rectangle.height;
         LabG_MarginTop(cascadebtn)    = LabG_MarginTop(cascadebtn) + (delta / 2);
         LabG_TextRect_y(cascadebtn)  += delta / 2;
         LabG_MarginBottom(cascadebtn) = LabG_MarginBottom(cascadebtn) + delta - (delta / 2);
         if (adjustHeight)
   ...
   Cardinal *num_args)
{
   ...
   CBG_Cache(newParent)  = &(((XmCascadeButtonGCacheObject)newSec)->cascade_button_cache);
   CBG_Cache(refParent)  = &(((XmCascadeButtonGCacheObject)extData->reqWidget)->cascade_button_cache);
   ...
   Cardinal *num_args) /* unused */
{
   ...
   if ((CBG_Submenu(new_w)) && ((!XmIsRowColumn(CBG_Submenu(new_w))) || (RC_Type(CBG_Submenu(new_w)) != XmMENU_PULLDOWN)))
   {
   ...
   else if (LabG_MenuType(new_w) == XmMENU_PULLDOWN || LabG_MenuType(new_w) == XmMENU_POPUP || LabG_MenuType(new_w) == XmMENU_OPTION)
   {
      ...
      else if ((LabG_MenuType(new_w) != XmMENU_OPTION) && (((CBG_CascadePixmap(new_w) == XmUNSPECIFIED_PIXMAP) && (!CBG_Submenu(old) && CBG_Submenu(new_w))) || ((CBG_ArmedPixmap(old) != XmUNSPECIFIED_PIXMAP) && (LabG_TextRect_height(old) != LabG_TextRect_height(new_w)))))
      {
         _XmProcessLock();
         _XmArrowPixmapCacheDelete((XtPointer)CBG_ArmedPixmap(old));
         _XmArrowPixmapCacheDelete((XtPointer)CBG_CascadePixmap(old));
	...
	*/
      if ((CBG_CascadePixmap(old) != CBG_CascadePixmap(new_w)) || (LabG_LabelType(old) != LabG_LabelType(new_w)) || (CBG_Submenu(old) != CBG_Submenu(new_w)))
      {
      ...
      {
         if ((new_w->gadget.highlight_thickness != old->gadget.highlight_thickness) || (new_w->gadget.shadow_thickness != old->gadget.shadow_thickness) || (LabG_MarginRight(new_w) != LabG_MarginRight(old)) || (LabG_MarginHeight(new_w) != LabG_MarginHeight(old)) || (LabG_MarginTop(new_w) != LabG_MarginTop(old)) || (LabG_MarginBottom(new_w) != LabG_MarginBottom(old)))
         {
            setup_cascade(new_w, adjustWidth, adjustHeight);
            flag = TRUE;
         }
         else if ((LabG_MarginWidth(new_w) != LabG_MarginWidth(old)) || (new_w->rectangle.width != old->rectangle.width) || (new_w->rectangle.height != old->rectangle.height))
         {
   ...
   new_w->gadget.event_mask = XmARM_EVENT | XmACTIVATE_EVENT | XmFOCUS_IN_EVENT | XmFOCUS_OUT_EVENT | XmENTER_EVENT | XmLEAVE_EVENT | XmHELP_EVENT | XmBDRAG_EVENT;
   return (flag);
}
   ...
   XmCascadeButtonGadget cb)
{
   ...
   XmManagerWidget mw = (XmManagerWidget)XtParent(cb);
   XmGetColors(XtScreen(mw), mw->core.colormap, mw->core.background_pixel, &junk, &junk, &junk, &select_pixel);
   ...
   Cardinal *num_args)
{
   ...
   if (!(LabG_MenuType(new_w) == XmMENU_BAR || LabG_MenuType(new_w) == XmMENU_PULLDOWN || LabG_MenuType(new_w) == XmMENU_POPUP || LabG_MenuType(new_w) == XmMENU_OPTION))
   {
   ...
   if (LabG_MenuType(new_w) == XmMENU_BAR)
   {
      ...
      XtGetSubresources(XtParent(new_w), &requestedMarginWidth, XtName((Widget)new_w), new_w->object.widget_class->core_class.class_name, &request_resources, 1, args, *num_args);
   ...
   CBG_Timer(new_w) = 0;
   if ((submenu) && ((!XmIsRowColumn(submenu)) || (RC_Type(submenu) != XmMENU_PULLDOWN)))
   {
      submenu = NULL;
      XmeWarning((Widget)new_w, WRONGSUBMENU);
   }
   ...
   if (LabG_MenuType(new_w) == XmMENU_PULLDOWN || LabG_MenuType(new_w) == XmMENU_POPUP || LabG_MenuType(new_w) == XmMENU_OPTION)
   {
   ...
   if (LabG_MenuType(new_w) == XmMENU_BAR || LabG_MenuType(new_w) == XmMENU_PULLDOWN || LabG_MenuType(new_w) == XmMENU_POPUP)
   {
      new_w->gadget.traversal_on = TRUE;
   }
   /*
    ...
    */
   new_w->gadget.event_mask = XmARM_EVENT | XmACTIVATE_EVENT | XmFOCUS_IN_EVENT | XmFOCUS_OUT_EVENT | XmENTER_EVENT | XmLEAVE_EVENT | XmHELP_EVENT;
}
   ...
   XmSecondaryResourceData **data_rtn)
{
   ...
   resource_class = NULL;
   resource_name  = NULL;
   arrayCount     = _XmSecondaryResourceData(bcePtr, data_rtn, client_data, resource_name, resource_class, GetCascadeBGClassSecResBase);
   _XmProcessUnlock();
   return (arrayCount);
}
   ...
   XtPointer client_data) /* unused */
{
   ...
   char     *cp;
   _XmProcessLock();
   widgetSecdataPtr = (XtPointer)(XtMalloc(labg_cache_size + cascadebg_cache_size + 1));
   if (widgetSecdataPtr)
   {
