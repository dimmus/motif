  ...
  switch ((int)tw->text.char_size) {
    case 1: {
      ...
      if (!return_val && (data->ptr + start) > (data->gap_start - search_length)) {
        ...
        for (match = True;
             ptr >= data->ptr &&
             (data->gap_start - ptr) + (end_of_data - data->gap_end) >= search_length;
             ptr--, match = True)
        {
    ...
    case 2: {
      ...
      if (!return_val && (bits16_ptr + start) > (bits16_gap_start - search_length)) {
        ...
        for (match = True;
             bits16_ptr >= (BITS16 *)data->ptr &&
             (bits16_gap_start - bits16_ptr) + (bits16_end_of_data - bits16_gap_end) >=
                 search_length;
    ...
    default: {
      ...
      if (!return_val && (wchar_t_ptr + start) > (wchar_t_gap_start - search_length)) {
        ...
        for (match = True;
             wchar_t_ptr >= (wchar_t *)data->ptr &&
             (wchar_t_gap_start - wchar_t_ptr) + (wchar_t_end_of_data - wchar_t_gap_end) >=
                 search_length;
    ...
    case 2: {
      ...
      if (!return_val && (bits16_ptr + start) < bits16_gap_start) {
        ...
        for (match = True;
             ...
             bits16_ptr++, match = True)
        {
          if (*bits16_ptr == *bits16_search_string) { /* have a potential winner*/
            for (i = 1; i < search_length && match == True; i++) {
              if (bits16_ptr + i >= bits16_gap_start) {
                bits16_tmp_ptr = bits16_ptr + (bits16_gap_end - bits16_gap_start) + i;
                if (*bits16_tmp_ptr != bits16_search_string[i]) {
