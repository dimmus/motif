   ...
   */
   switch ((int)tw->text.char_size)
   {
      case 1:
      {
         ...
         end_of_data = data->ptr + data->length + (data->gap_end - data->gap_start);
         ...
         if (end_of_data - search_length >= data->gap_end)
            ...
            end_ptr = data->gap_start - (search_length - (end_of_data - data->gap_end));
         ...
         while (!return_val && ptr >= data->gap_end)
         {
            ...
            { /* potential winner! */
               ...
               if (i == search_length)
               { /* we have a winner! */
                  *position  = ptr - data->ptr - (data->gap_end - data->gap_start);
                  return_val = True;
               }
            }
     ...
     */
         ...
         /* Set the base for the search. */
         if (!return_val && (data->ptr + start) > (data->gap_start - search_length))
         {
            if (data->ptr < data->gap_start)
               ptr = data->gap_start - 1;
            /* else, we're done... gap_start is at data->ptr and still no match*/
            for (match = True;
                 ptr >= data->ptr && (data->gap_start - ptr) + (end_of_data - data->gap_end) >= search_length;
                 ptr--, match = True)
            {
               if (*ptr == *search_string)
     ...
     */
      ...
      case 2:
      {
         ...
         bits16_search_string = (BITS16 *)XtMalloc((unsigned)(search_length + 1) * (int)tw->text.char_size);
         ...
         bits16_end_of_data = bits16_ptr + data->length + (bits16_gap_end - bits16_gap_start);
         ...
         if (bits16_end_of_data - search_length >= bits16_gap_end)
            ...
            bits16_end_ptr = bits16_gap_start - (search_length - (bits16_end_of_data - bits16_gap_end));
         /* Phase one: search from start back to gap_end */
         bits16_ptr = (BITS16 *)data->ptr;
         if (bits16_ptr + start > bits16_gap_start) /* backside of gap */
            bits16_ptr = (BITS16 *)data->ptr + (bits16_gap_end - bits16_gap_start) + start;
         else /* we're starting before the gap */
         ...
         while (!return_val && bits16_ptr >= bits16_gap_end)
         {
            ...
            { /* potential winner! */
               ...
               if (i == search_length)
               { /* we have a winner! */
                  *position  = bits16_ptr - (BITS16 *)data->ptr - (bits16_gap_end - bits16_gap_start);
                  return_val = True;
               }
            }
     ...
     */
         ...
         /* Set the base for the search. */
         bits16_ptr = (BITS16 *)data->ptr;
         if (!return_val && (bits16_ptr + start) > (bits16_gap_start - search_length))
         {
            if (bits16_ptr < bits16_gap_start)
               bits16_ptr = bits16_gap_start - 1;
            /* else, we're done... gap_start is at data->ptr and still no match*/
            for (match = True;
                 bits16_ptr >= (BITS16 *)data->ptr && (bits16_gap_start - bits16_ptr) + (bits16_end_of_data - bits16_gap_end) >= search_length;
                 bits16_ptr--, match = True)
            {
               ...
               { /* potential winner */
                  for (i = 1; i < search_length && match == True; i++)
                  {
                     ...
                     {
                        bits16_tmp_ptr = bits16_ptr + (bits16_gap_end - bits16_gap_start) + i;
     ...
     */
      ...
      default:
      {
         ...
         wchar_t_end_of_data = wchar_t_ptr + data->length + (wchar_t_gap_end - wchar_t_gap_start);
         /* only need to search up to end - search_length position */
         if (wchar_t_end_of_data - search_length >= wchar_t_gap_end)
            wchar_t_end_ptr = wchar_t_end_of_data - search_length;
         else
            wchar_t_end_ptr = wchar_t_gap_start - (search_length - (wchar_t_end_of_data - wchar_t_gap_end));
         /* Phase one: search from start back to gap_end */
         wchar_t_ptr = (wchar_t *)data->ptr;
         if (wchar_t_ptr + start > wchar_t_gap_start) /* backside of gap */
            wchar_t_ptr = (wchar_t *)data->ptr + (wchar_t_gap_end - wchar_t_gap_start) + start;
         else /* we're starting before the gap */
         ...
         while (!return_val && wchar_t_ptr >= wchar_t_gap_end)
         {
            ...
            { /* potential winner! */
               ...
               if (i == search_length)
               { /* we have a winner! */
                  *position  = wchar_t_ptr - (wchar_t *)data->ptr - (wchar_t_gap_end - wchar_t_gap_start);
                  return_val = True;
               }
            }
     ...
     */
         ...
         /* Set the base for the search. */
         wchar_t_ptr = (wchar_t *)data->ptr;
         if (!return_val && (wchar_t_ptr + start) > (wchar_t_gap_start - search_length))
         {
            if (wchar_t_ptr < wchar_t_gap_start)
               wchar_t_ptr = wchar_t_gap_start - 1;
            /*else, we're done... gap_start is at data->ptr and still no match*/
            for (match = True;
                 wchar_t_ptr >= (wchar_t *)data->ptr && (wchar_t_gap_start - wchar_t_ptr) + (wchar_t_end_of_data - wchar_t_gap_end) >= search_length;
                 wchar_t_ptr--, match = True)
            {
               ...
               { /* potential winner */
                  for (i = 1; i < search_length && match == True; i++)
                  {
                     ...
                     {
                        wchar_t_tmp_ptr = wchar_t_ptr + (wchar_t_gap_end - wchar_t_gap_start) + i;
     ...
     */
         ...
         if (!return_val && (data->ptr + start) < data->gap_start)
         {
            if (data->ptr + start < data->gap_start - search_length)
               ...
               ptr = data->ptr + start;
            for (match = True;
                 ptr < data->gap_start && (data->gap_start - ptr) + (end_of_data - data->gap_end) >= search_length;
                 ptr++, match = True)
            {
               if (*ptr == *search_string)
               { /* we have a potential winner */
     ...
     */
         ...
         if (!return_val && (bits16_ptr + start) < bits16_gap_start)
         {
            if (bits16_ptr + start < bits16_gap_start - search_length)
               ...
               bits16_ptr = (BITS16 *)data->ptr + start;
            for (match = True;
                 bits16_ptr < bits16_gap_start && (bits16_gap_start - bits16_ptr) + (bits16_end_of_data - bits16_gap_end) >= search_length;
                 bits16_ptr++, match = True)
            {
               if (*bits16_ptr == *bits16_search_string)
     ...
     */
         ...
         if (!return_val && (wchar_t_ptr + start) < wchar_t_gap_start)
         {
            if (wchar_t_ptr + start < wchar_t_gap_start - search_length)
               ...
               wchar_t_ptr = (wchar_t *)data->ptr + start;
            for (match = True;
                 wchar_t_ptr < wchar_t_gap_start && (wchar_t_gap_start - wchar_t_ptr) + (wchar_t_end_of_data - wchar_t_gap_end) >= search_length;
                 wchar_t_ptr++, match = True)
            {
               if (*wchar_t_ptr == *wchar_t_search_string)
               {
