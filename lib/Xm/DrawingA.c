   ...
   Cardinal *num_params) /* unused */
{
   ...
   if (!input_on_gadget && (!da->manager.active_child || button_event))
   {
      ...
      XtCallCallbackList((Widget)da,
                         da->drawing_area.input_callback,
                         &cb);
   }
}
   ...
   XtWidgetGeometry *reply)
{
   ...
   return (_XmGMHandleGeometryManager((Widget)da, w, request, reply, da->drawing_area.margin_width, da->drawing_area.margin_height, da->drawing_area.resize_policy, True)); /* no overlap checking */
}
   ...
   Cardinal *num_args) /* unused */
{
   ...
   if (XtIsRealized((Widget)new_w) && (((new_w->drawing_area.margin_width != current->drawing_area.margin_width) || (new_w->drawing_area.margin_height != current->drawing_area.margin_height))))
   {
   ...
   XtWidgetGeometry *desired)
{
   XmDrawingAreaWidget da = (XmDrawingAreaWidget)wid;
   /* function shared with Bulletin Board */
   return (_XmGMHandleQueryGeometry(wid, intended, desired, da->drawing_area.margin_width, da->drawing_area.margin_height, da->drawing_area.resize_policy));
}
   ...
   Widget wid)
{
       ...
       && ((XmManagerWidget)wid)->manager.traversal_on)
   {
	 ...
	 is either NULL or point to this child, allow navigation to it.
      ...
      {
         if (_XmGetNavigability(children[idx]) && ((((XmManagerWidget)wid)->manager.initial_focus == NULL) || (((XmManagerWidget)wid)->manager.initial_focus == children[idx])))
         {
