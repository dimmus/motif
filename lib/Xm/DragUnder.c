   ...
   XmDragProcCallbackStruct *dpcb) /* unused */
{
   ...
   /*
     ...
     */
   if (!XGetGeometry(aSaveData->display, aSaveData->window, &junkWin, &junkInt, &junkInt, &junkUInt, &junkUInt, &junkUInt, &(aSaveData->windowDepth)))
   {
   ...
   if (aSaveData->animationStyle == XmDRAG_UNDER_PIXMAP && aSaveData->animationPixmap != None && aSaveData->animationPixmap != XmUNSPECIFIED_PIXMAP && aSaveData->animationPixmapDepth != 1 && aSaveData->animationPixmapDepth != aSaveData->windowDepth)
   {
   ...
   Dimension           height)
{
   ...
   pData->width  = width;
   pData->height = height;
   pData->pixmap = _XmAllocScratchPixmap(aSaveData->xmScreen,
                                         (Cardinal)aSaveData->windowDepth,
                                         pData->width,
                                         pData->height);
   XCopyArea(aSaveData->display, aSaveData->window, pData->pixmap, aSaveData->drawGC, pData->x, pData->y, pData->width, pData->height, 0, 0);
   return (True);
}
   ...
   XmAnimationSaveData aSaveData)
{
   ...
   if (_XmRegionGetNumRectangles(aSaveData->dropSiteRegion) == 1L)
   {
      ...
      {
         XmeDrawHighlight(aSaveData->display, aSaveData->window, aSaveData->highlightGC, x, y, width, height, aSaveData->highlightThickness);
      }
   }
   else
   {
      ...
      {
         _XmRegionDrawShadow(aSaveData->display, aSaveData->window, aSaveData->highlightGC, aSaveData->highlightGC, aSaveData->dropSiteRegion, offset, aSaveData->highlightThickness, XmSHADOW_OUT);
      }
   }
}
   ...
   XmAnimationSaveData aSaveData)
{
   ...
   v.subwindow_mode     = IncludeInferiors;
   vmask                = GCGraphicsExposures | GCSubwindowMode | GCForeground | GCBackground;
   if (aSaveData->bottomShadowPixmap != None && aSaveData->bottomShadowPixmap != XmUNSPECIFIED_PIXMAP)
   {
      int depth;
      XmeGetPixmapData(XtScreen(aSaveData->xmScreen),
   ...
   if (_XmRegionGetNumRectangles(aSaveData->dropSiteRegion) == 1L)
   {
      ...
      {
         XmeDrawShadows(aSaveData->display, aSaveData->window, aSaveData->topShadowGC, aSaveData->bottomShadowGC, x, y, width, height, aSaveData->shadowThickness, (aSaveData->animationStyle == XmDRAG_UNDER_SHADOW_IN) ? XmSHADOW_IN : XmSHADOW_OUT);
      }
   }
   else
   {
      if (SaveAll(aSaveData, extents.x, extents.y, extents.width, extents.height))
      {
         _XmRegionDrawShadow(aSaveData->display, aSaveData->window, aSaveData->topShadowGC, aSaveData->bottomShadowGC, aSaveData->dropSiteRegion, offset, aSaveData->shadowThickness, (aSaveData->animationStyle == XmDRAG_UNDER_SHADOW_IN) ? XmSHADOW_IN : XmSHADOW_OUT);
      }
   }
}
   ...
   XmDragProcCallbackStruct *dpcb)
{
   ...
   *((XtPointer *)aData->saveAddr) = (XtPointer)aSaveData;
   ...
   if (aSaveData->activeMode == XmDRAG_WINDOW)
   {
      ...
      XtInsertEventHandler(hwidget, ExposureMask, False, (XtEventHandler)AnimateExpose, (XtPointer)aSaveData, XtListTail);
   }
}
   ...
   XmDragProcCallbackStruct *dpcb) /* unused */
{
   ...
   if (aSaveData->activeMode == XmDRAG_WINDOW)
   {
      ...
      if (XmIsGadget(hwidget))
         hwidget = XtParent(hwidget);
      XtRemoveEventHandler(hwidget, ExposureMask, False, (XtEventHandler)AnimateExpose, (XtPointer)aSaveData);
   }
   /*
