...
#define MESSAGE2 _XmMMsgDragUnder_0001
    ...
    CreateAnimationSaveData(
        XmDragContext dc, XmAnimationData aData, XmDragProcCallbackStruct *dpcb);
static void FreeAnimationData(XmAnimationSaveData aSaveData);
static Boolean SaveAll(
    XmAnimationSaveData aSaveData, Position x, Position y, Dimension width, Dimension height);
static Boolean SaveSegments(XmAnimationSaveData aSaveData,
                            Position x,
                            Position y,
                            Dimension width,
                            Dimension height,
                            Dimension *thickness);
static void DrawHighlight(XmAnimationSaveData aSaveData);
static void DrawShadow(XmAnimationSaveData aSaveData);
static void DrawPixmap(XmAnimationSaveData aSaveData);
static void AnimateExpose(Widget w, XmAnimationSaveData aSaveData, XEvent *event, Boolean *cont);
static void AnimateEnter(XmDropSiteManagerObject dsm,
                         XmAnimationData aData,
                         XmDragProcCallbackStruct *dpcb);
static void AnimateLeave(XmDropSiteManagerObject dsm,
                         XmAnimationData aData,
                         XmDragProcCallbackStruct *dpcb);
 ...
 ***************************************************************************/
static XmAnimationSaveData CreateAnimationSaveData(XmDragContext dc,
                                                   XmAnimationData aData,
                                                   XmDragProcCallbackStruct *dpcb) /* unused */
{
  ...
  aSaveData = (XmAnimationSaveData)XtMalloc(sizeof(XmAnimationSaveDataRec));
   ...
   */
  if (!XGetGeometry(aSaveData->display,
                    aSaveData->window,
                    &junkWin,
                    &junkInt,
                    &junkInt,
  ...
  XtFree((char *)dsv);
  ac = 0;
  XtSetArg(al[ac], XmNanimationStyle, &(aSaveData->animationStyle));
  ac++;
  XtSetArg(al[ac], XmNanimationMask, &(aSaveData->animationMask));
  ac++;
  XtSetArg(al[ac], XmNanimationPixmap, &(aSaveData->animationPixmap));
  ac++;
  XtSetArg(al[ac], XmNanimationPixmapDepth, &(aSaveData->animationPixmapDepth));
  ac++;
  XmDropSiteRetrieve((Widget)dc, al, ac);
  if (aSaveData->animationStyle == XmDRAG_UNDER_PIXMAP && aSaveData->animationPixmap != None &&
      aSaveData->animationPixmap != XmUNSPECIFIED_PIXMAP && aSaveData->animationPixmapDepth != 1 &&
  ...
  aSaveData->drawGC = XCreateGC(aSaveData->display, aSaveData->window, vmask, &v);
...
static void FreeAnimationData(XmAnimationSaveData aSaveData)
{
  ...
  if (aSaveData->numSavedPixmaps) {
    for (i = 0; i < aSaveData->numSavedPixmaps; i++) {
      _XmFreeScratchPixmap(aSaveData->xmScreen, aSaveData->savedPixmaps[i].pixmap);
    }
    ...
    XmAnimationSaveData aSaveData, Position x, Position y, Dimension width, Dimension height)
{
  ...
  aSaveData->savedPixmaps = pData = (DragPixmapData *)XtMalloc(sizeof(DragPixmapData));
  if (!pData) {
    return (False);
  }
  pData->x = x;
  ...
  pData->width = width;
  pData->height = height;
  pData->pixmap = _XmAllocScratchPixmap(
      aSaveData->xmScreen, (Cardinal)aSaveData->windowDepth, pData->width, pData->height);
                            ...
                            Dimension *thickness)
{
  ...
  aSaveData->savedPixmaps = pData = (DragPixmapData *)XtMalloc(sizeof(DragPixmapData) * 4);
  if (!pData) {
    return (False);
  }
  pData->x = x;
...
static void DrawHighlight(XmAnimationSaveData aSaveData)
{
  ...
  v.subwindow_mode = IncludeInferiors;
  vmask = GCGraphicsExposures | GCSubwindowMode | GCForeground | GCBackground;
  if (aSaveData->highlightPixmap != None && aSaveData->highlightPixmap != XmUNSPECIFIED_PIXMAP) {
    int depth;
  ...
  aSaveData->highlightGC = XCreateGC(aSaveData->display, aSaveData->window, vmask, &v);
  _XmRegionSetGCRegion(aSaveData->display, aSaveData->highlightGC, 0, 0, aSaveData->clipRegion);
  /* draw highlight */
  ...
  if (_XmRegionGetNumRectangles(aSaveData->dropSiteRegion) == 1L) {
    ...
    if (SaveSegments(aSaveData, x, y, width, height, &aSaveData->highlightThickness)) {
      XmeDrawHighlight(aSaveData->display,
                       aSaveData->window,
    ...
    if (SaveAll(aSaveData, extents.x, extents.y, extents.width, extents.height)) {
      _XmRegionDrawShadow(aSaveData->display,
                          aSaveData->window,
                          aSaveData->highlightGC,
                          aSaveData->highlightGC,
...
static void DrawShadow(XmAnimationSaveData aSaveData)
{
  ...
  v.subwindow_mode = IncludeInferiors;
  vmask = GCGraphicsExposures | GCSubwindowMode | GCForeground | GCBackground;
  if (aSaveData->topShadowPixmap != None && aSaveData->topShadowPixmap != XmUNSPECIFIED_PIXMAP) {
    int depth;
    XmeGetPixmapData(XtScreen(aSaveData->xmScreen),
  ...
  aSaveData->topShadowGC = XCreateGC(aSaveData->display, aSaveData->window, vmask, &v);
  _XmRegionSetGCRegion(aSaveData->display, aSaveData->topShadowGC, 0, 0, aSaveData->clipRegion);
  /*
  ...
  aSaveData->bottomShadowGC = XCreateGC(aSaveData->display, aSaveData->window, vmask, &v);
  _XmRegionSetGCRegion(aSaveData->display, aSaveData->bottomShadowGC, 0, 0, aSaveData->clipRegion);
  /*
   *  Draw the shadows.
   */
  _XmRegionGetExtents(aSaveData->dropSiteRegion, &extents);
  ...
  if (_XmRegionGetNumRectangles(aSaveData->dropSiteRegion) == 1L) {
    ...
    width = extents.width - (offset << 1);
    height = extents.height - (offset << 1);
    if (SaveSegments(aSaveData, x, y, width, height, &aSaveData->shadowThickness)) {
      XmeDrawShadows(aSaveData->display,
                     aSaveData->window,
                     ...
                     (aSaveData->animationStyle == XmDRAG_UNDER_SHADOW_IN) ? XmSHADOW_IN :
                                                                             XmSHADOW_OUT);
    }
  }
  else {
...
static void DrawPixmap(XmAnimationSaveData aSaveData)
{
  ...
  GC maskGC = NULL;
  if (aSaveData->animationPixmap == None || aSaveData->animationPixmap == XmUNSPECIFIED_PIXMAP) {
    return;
  }
  /*
  ...
  if (SaveAll(aSaveData, x, y, width, height)) {
    if (aSaveData->animationMask != None && aSaveData->animationMask != XmUNSPECIFIED_PIXMAP) {
      /*
       ...
       */
      mask = _XmAllocScratchPixmap(aSaveData->xmScreen, 1, width, height);
      v.background = 0;
      v.foreground = 1;
      v.function = GXclear;
      v.graphics_exposures = False;
      v.subwindow_mode = IncludeInferiors;
      vmask = GCGraphicsExposures | GCSubwindowMode | GCBackground | GCForeground | GCFunction;
      maskGC = XCreateGC(aSaveData->display, mask, vmask, &v);
      XFillRectangle(aSaveData->display, mask, maskGC, 0, 0, width, height);
      XSetFunction(aSaveData->display, maskGC, GXor);
      _XmRegionSetGCRegion(aSaveData->display, maskGC, -x, -y, aSaveData->clipRegion);
      XCopyArea(
          aSaveData->display, aSaveData->animationMask, mask, maskGC, 0, 0, width, height, 0, 0);
      ...
      _XmRegionSetGCRegion(aSaveData->display, aSaveData->drawGC, 0, 0, aSaveData->clipRegion);
    }
    /*
 ...
 *
 ***************************************************************************/
static void AnimateExpose(Widget w, /* unused */
                          XmAnimationSaveData aSaveData,
                          XEvent *event, /* unused */
                          Boolean *cont) /* unused */
{
  ...
  if (aSaveData->dragOver && aSaveData->activeMode != XmDRAG_WINDOW) {
    _XmDragOverHide(
        aSaveData->dragOver, aSaveData->windowX, aSaveData->windowY, aSaveData->clipRegion);
  }
  /* Draw the visuals. */
  switch (aSaveData->animationStyle) {
    default:
 ...
 *
 ***************************************************************************/
static void AnimateEnter(XmDropSiteManagerObject dsm, /* unused */
                         XmAnimationData aData,
                         XmDragProcCallbackStruct *dpcb)
{
 ...
 *
 ***************************************************************************/
static void AnimateLeave(XmDropSiteManagerObject dsm, /* unused */
                         XmAnimationData aData,
                         XmDragProcCallbackStruct *dpcb) /* unused */
{
  Cardinal i;
  DragPixmapData *pData;
  XmAnimationSaveData aSaveData = (XmAnimationSaveData) * ((XtPointer *)aData->saveAddr);
  (void)dpcb;
  if (!aSaveData)
    return;
  /**
  ...
  if (aSaveData->activeMode == XmDRAG_WINDOW) {
    ...
    XtRemoveEventHandler(
        hwidget, ExposureMask, False, (XtEventHandler)AnimateExpose, (XtPointer)aSaveData);
  }
  /*
   ...
   */
  _XmRegionSetGCRegion(aSaveData->display, aSaveData->drawGC, 0, 0, aSaveData->clipRegion);
  for (pData = aSaveData->savedPixmaps, i = aSaveData->numSavedPixmaps; i; pData++, i--) {
    XCopyArea(aSaveData->display,
              pData->pixmap,
              aSaveData->window,
              aSaveData->drawGC,
