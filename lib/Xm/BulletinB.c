...
static void Initialize(Widget wid_req, Widget wid_new, ArgList args, Cardinal *numArgs);
static void Destroy(Widget wid);
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args, Cardinal *num_args);
static Boolean SetValuesHook(Widget wid, ArgList args, Cardinal *num_args);
static void HandleChangeManaged(XmBulletinBoardWidget bbWid, XmGeoCreateProc geoMatrixCreate);
static void HandleResize(XmBulletinBoardWidget bbWid, XmGeoCreateProc geoMatrixCreate);
...
static Boolean BulletinBoardParentProcess(Widget wid, XmParentProcessData event);
static void GetDialogTitle(Widget bb, int resource, XtArgVal *value);
static Widget GetBBWithDB(Widget wid);
static XmFontList GetTable(Widget wid, XtEnum type);
static void CallMapUnmap(Widget wid, Boolean map_unmap);
static void BulletinBoardSetDefaultShadow(Widget button);
static void BBUpdateDynDefaultButton(Widget bb);
static void CheckSetRenderTables(Widget wid, int offset, XrmValue *value);
 ...
 *
 ****************************************************************/
externaldef(xmbulletinboardclassrec) XmBulletinBoardClassRec xmBulletinBoardClassRec = {
    {
        /* core_class fields      */
...
externaldef(xmbulletinboardwidgetclass)
 ...
 */
/*ARGSUSED*/
static void GetDialogTitle(Widget bb,
                           int resource, /* unused */
                           XtArgVal *value)
{
  XmString data;
  /****************/
  data = XmStringCopy((XmString)((XmBulletinBoardWidget)bb)->bulletin_board.dialog_title);
  *value = (XtArgVal)data;
  return;
}
...
{
  XmBulletinBoardWidgetClass bbClass = (XmBulletinBoardWidgetClass)w_class;
  XmBulletinBoardWidgetClass bbSuper = (XmBulletinBoardWidgetClass)w_class->core_class.superclass;
  _XmFastSubclassInit(w_class, XmBULLETIN_BOARD_BIT);
  if (bbClass->bulletin_board_class.geo_matrix_create == XmInheritGeoMatrixCreate) {
    bbClass->bulletin_board_class.geo_matrix_create =
        bbSuper->bulletin_board_class.geo_matrix_create;
  }
  if (bbClass->bulletin_board_class.focus_moved_proc == XmInheritFocusMovedProc) {
  ...
  XmeTraitSet((XtPointer)w_class, XmQTspecifyRenderTable, (XtPointer)&bulletinbSRT);
  /* Install the dialogShellsavyy trait for all subclasses */
  XmeTraitSet((XtPointer)w_class, XmQTdialogShellSavvy, (XtPointer)&bulletinbDST);
  return;
}
 ...
 ****************/
/*ARGSUSED*/
static void Initialize(Widget wid_req,
                       Widget wid_new,
                       ArgList args,      /* unused */
                       Cardinal *numArgs) /* unused */
{
  ...
  if ((request->manager.shadow_thickness == XmINVALID_DIMENSION) && XtIsShell(XtParent(request))) {
  ...
  if (request->bulletin_board.dialog_title) {
    new_w->bulletin_board.dialog_title = XmStringCopy(request->bulletin_board.dialog_title);
    XmeSetWMShellTitle(new_w->bulletin_board.dialog_title, XtParent(new_w));
  }
  ...
  if (XmIsDialogShell(XtParent(request))) {
    ...
    XtSetArg(al[ac], XmNmwmInputMode, mwmStyle);
    ac++;
  ...
  XtFree(text_value);
  if (!XmRepTypeValidValue(XmRID_SHADOW_TYPE, new_w->bulletin_board.shadow_type, (Widget)new_w)) {
    new_w->bulletin_board.shadow_type = XmSHADOW_OUT;
  }
  if (!XmRepTypeValidValue(
          XmRID_RESIZE_POLICY, new_w->bulletin_board.resize_policy, (Widget)new_w))
  {
    new_w->bulletin_board.resize_policy = XmRESIZE_ANY;
  }
  if (new_w->bulletin_board.shell) {
    if (!XmRepTypeValidValue(
            XmRID_DIALOG_STYLE, new_w->bulletin_board.dialog_style, (Widget)new_w))
    {
  ...
  if (bbClass->bulletin_board_class.focus_moved_proc) {
    ...
    if (ancestor && (extData = _XmGetWidgetExtData(ancestor, XmSHELL_EXTENSION)) &&
        (extData->widget))
    {
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (new_w->bulletin_board.shadow_type != current->bulletin_board.shadow_type) {
    if (XmRepTypeValidValue(XmRID_SHADOW_TYPE, new_w->bulletin_board.shadow_type, (Widget)new_w))
      flag = True;
    else
      new_w->bulletin_board.shadow_type = current->bulletin_board.shadow_type;
  }
  if ((new_w->bulletin_board.resize_policy != current->bulletin_board.resize_policy) &&
      !XmRepTypeValidValue(
          XmRID_RESIZE_POLICY, new_w->bulletin_board.resize_policy, (Widget)new_w))
  {
    new_w->bulletin_board.resize_policy = current->bulletin_board.resize_policy;
  }
  if (new_w->bulletin_board.dialog_style != current->bulletin_board.dialog_style) {
    if (new_w->bulletin_board.shell) {
      if (!XmRepTypeValidValue(
              XmRID_DIALOG_STYLE, new_w->bulletin_board.dialog_style, (Widget)new_w))
      {
        new_w->bulletin_board.dialog_style = current->bulletin_board.dialog_style;
      }
    }
    else {
      if (new_w->bulletin_board.dialog_style != XmDIALOG_MODELESS) {
   ...
   */
  if (new_w->bulletin_board.dialog_title != current->bulletin_board.dialog_title)
   ...
   */
  if (request->bulletin_board.button_font_list != current->bulletin_board.button_font_list) {
  ...
  if (request->bulletin_board.label_font_list != current->bulletin_board.label_font_list) {
  ...
  if (request->bulletin_board.text_font_list != current->bulletin_board.text_font_list) {
  ...
  if (new_w->manager.shadow_thickness != current->manager.shadow_thickness) {
    flag = True;
  ...
  if (XtClass(new_w) == xmBulletinBoardWidgetClass) {
    ...
    if (((new_w->bulletin_board.margin_width != current->bulletin_board.margin_width) ||
         (new_w->bulletin_board.margin_height != current->bulletin_board.margin_height)))
    {
      /*
...
static void HandleChangeManaged(XmBulletinBoardWidget bbWid, XmGeoCreateProc geoMatrixCreate)
{
  ...
  _XmGeoArrangeBoxes(geoSpec, (Position)0, (Position)0, &desired_w, &desired_h);
   ...
   */
  if ((desired_h != XtHeight(bbWid)) || (desired_w != XtWidth(bbWid))) {
    switch (XtMakeResizeRequest((Widget)bbWid, desired_w, desired_h, &allowed_w, &allowed_h)) {
      case XtGeometryYes: {
        break;
      }
      case XtGeometryAlmost: {
        XtMakeResizeRequest((Widget)bbWid, allowed_w, allowed_h, NULL, NULL);
        _XmGeoArrangeBoxes(geoSpec, (Position)0, (Position)0, &allowed_w, &allowed_h);
        break;
      }
      case XtGeometryNo:
      default: {
  ...
  if (bbWid->manager.shadow_thickness && (XtWidth(bbWid) <= bbWid->bulletin_board.old_width) &&
      (XtHeight(bbWid) <= bbWid->bulletin_board.old_height))
  {
    ...
    bbWid->bulletin_board.old_shadow_thickness = bbWid->manager.shadow_thickness;
  }
                                              ...
                                              XmGeoCreateProc geoMatrixCreate)
{
  ...
  /*	Clear shadow if necessary.
   */
  if (bbWid->bulletin_board.old_shadow_thickness && (BB_ResizePolicy(bbWid) != XmRESIZE_NONE)) {
    _XmClearShadowType((Widget)bbWid,
                       bbWid->bulletin_board.old_width,
                       bbWid->bulletin_board.old_height,
                       bbWid->bulletin_board.old_shadow_thickness,
  ...
  result = _XmHandleGeometryManager((Widget)bbWid,
                                    ...
                                    desired,
                                    allowed,
                                    BB_ResizePolicy(bbWid),
                                    &bbWid->bulletin_board.geo_cache,
                                    geoMatrixCreate);
  if (!bbWid->bulletin_board.in_set_values && bbWid->manager.shadow_thickness &&
...
void _XmBulletinBoardSizeUpdate(Widget wid)
{
  ...
  XmBulletinBoardWidgetClass classPtr = (XmBulletinBoardWidgetClass)bbWid->core.widget_class;
  /****************/
  if (!XtIsRealized((Widget)bbWid)) {
    return;
  }
  if (classPtr->bulletin_board_class.geo_matrix_create) {
    ...
    _XmHandleSizeUpdate(
        (Widget)bbWid, BB_ResizePolicy(bbWid), classPtr->bulletin_board_class.geo_matrix_create);
    if (bbWid->manager.shadow_thickness && (XtWidth(bbWid) <= bbWid->bulletin_board.old_width) &&
        (XtHeight(bbWid) <= bbWid->bulletin_board.old_height))
    {
      XmeDrawShadows(XtDisplay((Widget)bbWid),
...
{
  XmBulletinBoardWidget bb = (XmBulletinBoardWidget)wid;
  XmBulletinBoardWidgetClass classPtr = (XmBulletinBoardWidgetClass)bb->core.widget_class;
  /****************/
  if (classPtr->bulletin_board_class.geo_matrix_create) {
    HandleChangeManaged(bb, classPtr->bulletin_board_class.geo_matrix_create);
    return;
  }
  ...
  if (bb->bulletin_board.old_shadow_thickness) {
    _XmClearShadowType((Widget)bb,
                       bb->bulletin_board.old_width,
  ...
  if (bb->manager.shadow_thickness && (XtWidth(bb) <= bb->bulletin_board.old_width) &&
      (XtHeight(bb) <= bb->bulletin_board.old_height))
  {
    ...
    bb->bulletin_board.old_shadow_thickness = bb->manager.shadow_thickness;
  }
 ...
 ****************/
/*ARGSUSED*/
static void UnmanageCallback(Widget w, /* unused */
...
static void InsertChild(Widget child)
{
  ...
  if (is_button && bb->bulletin_board.shell && bb->bulletin_board.auto_unmanage) {
    XtAddCallback(child, XmNactivateCallback, UnmanageCallback, (XtPointer)bb);
  }
  if (XmIsText(child) || XmIsTextField(child)) {
    if (bb->bulletin_board.text_translations) {
      XtOverrideTranslations(child, bb->bulletin_board.text_translations);
    }
  }
  return;
}
                                        ...
                                        XtWidgetGeometry *reply)
{
  ...
  if (classPtr->bulletin_board_class.geo_matrix_create) {
    return (HandleGeometryManager(
        w, request, reply, classPtr->bulletin_board_class.geo_matrix_create));
  }
  /* function shared with DrawingArea */
  return (_XmGMHandleGeometryManager((Widget)bb,
                                     w,
                                     request,
                                     reply,
                                     bb->bulletin_board.margin_width,
                                      ...
                                      XtWidgetGeometry *desired)
{
  ...
  /* function shared with DrawingArea */
  return (_XmGMHandleQueryGeometry(wid,
                                   intended,
                                   desired,
                                   bb->bulletin_board.margin_width,
                                   bb->bulletin_board.margin_height,
...
/****************************************************************/
/*ARGSUSED*/
void _XmBulletinBoardFocusMoved(Widget wid, /* unused */
                                XtPointer client_data,
                                XtPointer data)
{
  ...
  if (BBHasFocus) {
    ...
    if (!dbutton) {
      ...
      if (XmeTraitGet((XtPointer)XtClass(call_data->new_focus), XmQTtakesDefault)) {
        /* Any defaultable which gets the focus
         *   gets the default visuals and behavior.
         */
        _XmBulletinBoardSetDynDefaultButton((Widget)bb, call_data->new_focus);
      }
      else /* call_data->new_focus is not a push button */ {
        if ((call_data->focus_policy == XmEXPLICIT) || !XmIsManager(call_data->new_focus) ||
            !call_data->old_focus ||
            (!XmeTraitGet((XtPointer)XtClass(call_data->old_focus), XmQTtakesDefault)))
        {
    ...
    if (!BBHadFocus) {
      ...
      cb.event = NULL;
      XtCallCallbackList((Widget)bb, bb->bulletin_board.focus_callback, &cb);
    }
  }
...
static Boolean BulletinBoardParentProcess(Widget wid, XmParentProcessData event)
{
  ...
  if ((event->any.process_type == XmINPUT_ACTION) &&
      (((event->input_action.action == XmPARENT_ACTIVATE) && BB_DynamicDefaultButton(bb)) ||
       ((event->input_action.action == XmPARENT_CANCEL) && BB_DynamicCancelButton(bb))))
  {
    if (event->input_action.action == XmPARENT_ACTIVATE) {
      _XmBulletinBoardReturn((Widget)bb,
                             event->input_action.event,
                             event->input_action.params,
                             event->input_action.num_params);
    }
    else {
      _XmBulletinBoardCancel((Widget)bb,
                             event->input_action.event,
                             event->input_action.params,
                             event->input_action.num_params);
    }
 ...
 ****************/
void _XmBulletinBoardReturn(Widget wid, XEvent *event, String *params, Cardinal *numParams)
{
  XmBulletinBoardWidget bb = (XmBulletinBoardWidget)wid;
  ...
  if (!dbutton) {
    ...
    if (gadget_class->gadget_class.arm_and_activate && XtIsSensitive(dbutton)) {
      (*(gadget_class->gadget_class.arm_and_activate))(dbutton, event, params, numParams);
    }
  }
  else if (XmIsPrimitive(dbutton) && XtIsManaged(dbutton)) {
    primitive_class = (XmPrimitiveWidgetClass)XtClass(dbutton);
    if (primitive_class->primitive_class.arm_and_activate && XtIsSensitive(dbutton)) {
      (*(primitive_class->primitive_class.arm_and_activate))(dbutton, event, params, numParams);
    }
  }
  else {
 ...
 ****************/
void _XmBulletinBoardCancel(Widget wid, XEvent *event, String *params, Cardinal *numParams)
{
  XmBulletinBoardWidget bb = (XmBulletinBoardWidget)wid;
  XmGadgetClass gadget_class;
  XmPrimitiveWidgetClass primitive_class;
  ...
  if (!cbutton) {
    ...
    if (gadget_class->gadget_class.arm_and_activate && XtIsSensitive(cbutton)) {
      (*(gadget_class->gadget_class.arm_and_activate))(cbutton, event, params, numParams);
    }
  }
  else if (XmIsPrimitive(cbutton) && XtIsManaged(cbutton)) {
    primitive_class = (XmPrimitiveWidgetClass)XtClass(cbutton);
    if (primitive_class->primitive_class.arm_and_activate && XtIsSensitive(cbutton)) {
      (*(primitive_class->primitive_class.arm_and_activate))(cbutton, event, params, numParams);
    }
  }
  else {
                         ...
                         Cardinal *numParams) /* unused */
{
  if (BB_DefaultButton(wid)) {
    ...
    while (focus_hier && !XtIsShell(focus_hier)) {
      if (focus_hier == wid) {
        _XmBulletinBoardSetDynDefaultButton(wid, BB_DefaultButton(wid));
        break;
      }
      if (XmIsBulletinBoard(focus_hier) && BB_DefaultButton(focus_hier)) {
        break;
      }
...
static void BulletinBoardSetDefaultShadow(Widget button)
{
  ...
  trait_default = (XmTakesDefaultTrait)XmeTraitGet((XtPointer)XtClass(button), XmQTtakesDefault);
  if (trait_default)
 ...
 ****************/
void _XmBulletinBoardSetDynDefaultButton(Widget wid, Widget newDefaultButton)
{
  XmBulletinBoardWidget bbWid = (XmBulletinBoardWidget)wid;
  XmTakesDefaultTrait trait_default;
  if (newDefaultButton != BB_DynamicDefaultButton(bbWid)) {
    if (BB_DynamicDefaultButton(bbWid)) {
      trait_default = (XmTakesDefaultTrait)XmeTraitGet(
          (XtPointer)XtClass(BB_DynamicDefaultButton(bbWid)), XmQTtakesDefault);
      if (trait_default)
        trait_default->showAsDefault((Widget)BB_DynamicDefaultButton(bbWid), XmDEFAULT_OFF);
    }
    ...
    if (newDefaultButton) {
      trait_default = (XmTakesDefaultTrait)XmeTraitGet(
          (XtPointer)XtClass(BB_DynamicDefaultButton(bbWid)), XmQTtakesDefault);
      if (trait_default)
        trait_default->showAsDefault((Widget)BB_DynamicDefaultButton(bbWid), XmDEFAULT_ON);
    }
  }
  return;
}
...
static void BBUpdateDynDefaultButton(Widget bb)
{
  ...
  if (bbwdb == NULL) {
    if (((XmBulletinBoardWidget)bb)->bulletin_board.dynamic_default_button) {
...
static Widget GetBBWithDB(Widget wid)
{
  ...
  if ((_XmGetFocusPolicy(wid) == XmEXPLICIT) &&
      ((focus = XmGetFocusWidget(wid)) || (focus = _XmGetFirstFocus(wid))))
  {
    while (focus && !XtIsShell(focus)) {
      if (XmIsBulletinBoard(focus) && BB_DefaultButton(focus)) {
        return focus;
      }
...
#define IS_SET_TFL(state) (state & 0x04)
  ...
  if (((char *)bbw + offset) == (char *)&(bbw->bulletin_board.button_font_list)) {
  ...
  else if (((char *)bbw + offset) == (char *)&(bbw->bulletin_board.label_font_list)) {
  ...
  else if (((char *)bbw + offset) == (char *)&(bbw->bulletin_board.text_font_list)) {
...
static void CallMapUnmap(Widget wid, Boolean map_unmap)
{
  ...
  if (map_unmap) {
    XtCallCallbackList(wid, bb->bulletin_board.map_callback, &call_data);
  }
  else {
    XtCallCallbackList(wid, bb->bulletin_board.unmap_callback, &call_data);
  }
}
...
Widget XmVaCreateBulletinBoard(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmBulletinBoardWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedBulletinBoard(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmBulletinBoardWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
