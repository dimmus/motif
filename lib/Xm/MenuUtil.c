   ...
   Time         time)
{
   ...
   {
      if ((status = XtGrabPointer(widget, owner_events, event_mask, pointer_mode, keyboard_mode, confine_to, cursor, time)) == GrabSuccess)
         break;
      XmeMicroSleep(1000);
   }
   ...
   Time   time)
{
   ...
   for (retry = 0; retry < 5; retry++)
   {
      if ((status = XtGrabKeyboard(widget, owner_events, pointer_mode, keyboard_mode, time)) == GrabSuccess)
         break;
      XmeMicroSleep(1000);
   }
   if (status != GrabSuccess)
   ...
   Widget wid)
{
   ...
   {
      if (widget && XmIsPrimitive(widget) && (((XmPrimitiveWidgetClass)(widget->core.widget_class))->primitive_class.border_unhighlight != (XtWidgetProc)NULL))
         (*(((XmPrimitiveWidgetClass)(widget->core.widget_class))->primitive_class.border_unhighlight))((Widget)widget);
      else
   ...
   Cardinal *num_params) /* unused */
{
   XmRowColumnWidget rc = (XmRowColumnWidget)wid;
   XmGadget          gadget;
   if (!_XmIsEventUnique(event) || (!RC_IsArmed(rc) && !((RC_Type(rc) == XmMENU_OPTION) || (RC_Type(rc) == XmMENU_PULLDOWN))))
      return;
   ...
   {
      if ((gadget = (XmGadget)
              XmObjectAtPoint((Widget)rc, event->xkey.x, event->xkey.y))
          != NULL)
         _XmDispatchGadgetInput((Widget)gadget, event, XmHELP_EVENT);
      else
      {
   ...
   XmTraversalDirection direction)
{
   ...
   if (XmIsRowColumn(w))
      ...
      return;
   if ((RC_Type(parent) == XmMENU_POPUP) || (RC_Type(parent) == XmMENU_PULLDOWN) || (RC_Type(parent) == XmMENU_BAR))
   {
      _XmRecordEvent(event);
      (*(((XmRowColumnWidgetClass)XtClass(parent))->row_column_class.traversalHandler))((Widget)parent, (Widget)w, direction);
   }
}
   ...
   Cardinal *num_params)
{
    ...
    */
   if ((XmIsCascadeButton(w) || XmIsCascadeButtonGadget(w)) && XmIsRowColumn(parent) && (RC_Type(parent) == XmMENU_BAR) && !RC_PopupPosted(parent))
   {
      (*(((XmRowColumnClassRec *)XtClass(parent))->row_column_class.menuProcedures))(XmMENU_POPDOWN, parent, NULL, event, NULL);
   }
   else
      /* Let the menushell widget clean things up */
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownOne))(w, event, NULL, NULL);
}
   ...
   XmRowColumnWidget rc)
{
   ...
   if (XmIsMenuShell(XtParent(rc)) && (RC_Type(topLevel) == XmMENU_BAR) && (FindNextMenuBarCascade((XmRowColumnWidget)topLevel)))
   {
   ...
   XmRowColumnWidget rc)
{
    ...
    * submenu.
    */
   if (XmIsMenuShell(XtParent(rc)) && (RC_Type(rc) != XmMENU_POPUP) && RC_CascadeBtn(rc) && (RC_Type(XtParent(RC_CascadeBtn(rc))) == XmMENU_BAR) && (FindPrevMenuBarCascade((XmRowColumnWidget)XtParent(RC_CascadeBtn(rc)))))
   {
      GadgetCleanup(rc, (XmGadget)oldActiveChild);
      done = True;
   }
    ...
    */
   else if ((RC_Type(rc) == XmMENU_PULLDOWN) && RC_CascadeBtn(rc) && (RC_Type(XtParent(RC_CascadeBtn(rc))) != XmMENU_OPTION) && XmIsMenuShell(XtParent(rc)))
   {
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownOne))(XtParent(rc), NULL, NULL, NULL);
      done = True;
   }
   return (done);
}
   ...
   XmTraversalDirection direction)
{
   ...
   else
   {
      ...
      {
         /* watch for left wrap */
         if ((wid->core.x <= nextWidget->core.x) || (nextWidget->core.y + nextWidget->core.height <= wid->core.y) || (nextWidget->core.y >= wid->core.y + wid->core.height))
            done = WrapLeft(rc);
      }
      else if (direction == XmTRAVERSE_RIGHT)
      {
         /* watch for right wrap */
         if ((wid->core.x >= nextWidget->core.x) || (wid->core.y + wid->core.height <= nextWidget->core.y) || (wid->core.y >= nextWidget->core.y + nextWidget->core.height))
            done = WrapRight(rc);
      }
      if (!done)
   ...
   XmTraversalDirection direction)
{
   ...
   if (RC_Type(rc) != XmMENU_BAR)
   {
      ...
      {
         (*(((XmGadgetClassRec *)XtClass(pw))->gadget_class.arm_and_activate))((Widget)pw, NULL, NULL, NULL);
      }
      else if (direction == XmTRAVERSE_RIGHT && XmIsCascadeButton(pw) && CB_Submenu(pw))
      {
         (*(((XmPrimitiveClassRec *)XtClass(pw))->primitive_class.arm_and_activate))((Widget)pw, NULL, NULL, NULL);
      }
      else
   ...
   Widget            pw)
{
   ...
   if ((mst->MU_CurrentMenuChild != NULL) && (RC_PopupPosted(rc) != NULL) && ((XmIsCascadeButtonGadget(pw) && !CBG_Submenu(pw)) || (XmIsCascadeButton(pw) && !CB_Submenu(pw))))
   {
...
{
   XmMenuState mst = _XmGetMenuState((Widget)rc);
   if ((rc->manager.active_child == NULL) && ((XmIsCascadeButtonGadget(pw) && !CBG_Submenu(pw)) || (XmIsCascadeButton(pw) && !CB_Submenu(pw))))
   {
                       ...
                       Widget newMenuChild)
{
   ...
   if (XmIsTraversable(newMenuChild))
   {
      if (XmIsCascadeButtonGadget(newMenuChild))
      {
         ...
         {
            (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(RC_PopupPosted(menubar), NULL, NULL, NULL);
...
_XmMenuGrabKeyboardAndPointer(
   ...
   Time   time)
{
   register int status = (_XmGrabKeyboard(widget,
                                          True,
                                          GrabModeSync,
                                          GrabModeAsync,
                                          time)
   ...
   if (status)
      return (status);
   status = _XmGrabPointer(widget, True, EVENTS, GrabModeSync, GrabModeAsync, None, _XmGetMenuCursorByScreen(XtScreen(widget)), time) != GrabSuccess;
   if (status)
