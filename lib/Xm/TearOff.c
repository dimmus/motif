...
#define IsPopup(m) (((XmRowColumnWidget)(m))->row_column.type == XmMENU_POPUP)
#define IsPulldown(m) (((XmRowColumnWidget)(m))->row_column.type == XmMENU_PULLDOWN)
#define IsOption(m) (((XmRowColumnWidget)(m))->row_column.type == XmMENU_OPTION)
#define IsBar(m) (((XmRowColumnWidget)(m))->row_column.type == XmMENU_BAR)
...
#define SEGS_PER_DRAW (4 * OUTLINE_WIDTH)
    ...
    static GC
    InitXmTearOffXorGC(Widget wid);
static void SetupOutline(
    Widget wid, GC gc, XSegment pOutline[], XEvent *event, Dimension delta_x, Dimension delta_y);
static void EraseOutline(Widget wid, GC gc, XSegment *pOutline);
static void MoveOutline(
    Widget wid, GC gc, XSegment *pOutline, XEvent *event, Dimension delta_x, Dimension delta_y);
static void PullExposureEvents(Widget wid);
static void MoveOpaque(Widget wid, XEvent *event, Dimension delta_x, Dimension delta_y);
static void GetConfigEvent(Display *display, Window window, unsigned long mask, XEvent *event);
static Cursor GetTearOffCursor(Widget wid);
static Boolean DoPlacement(Widget wid, XEvent *event);
static void CallTearOffMenuActivateCallback(Widget wid, XEvent *event, unsigned short origin);
static void CallTearOffMenuDeactivateCallback(Widget wid, XEvent *event, unsigned short origin);
static void RemoveTearOffEventHandlers(Widget wid);
static void DismissOnPostedFromDestroy(Widget w, XtPointer clientData, XtPointer callData);
static void DisplayDestroyCallback(Widget w, XtPointer client_data, XtPointer call_data);

/********    End Static Function Declarations    ********/
static GC InitXmTearOffXorGC(Widget wid)
{
  XGCValues gcv;
  XtGCMask mask;
  ...
  return (XCreateGC(XtDisplay(wid), wid->core.screen->root, mask, &gcv));
}
    ...
    Widget wid, GC gc, XSegment pOutline[], XEvent *event, Dimension delta_x, Dimension delta_y)
{
  ...
  XDrawSegments(XtDisplay(wid), wid->core.screen->root, gc, pOutline, SEGS_PER_DRAW);
}
...
#define CONFIG_GRAB_MASK \
  (ButtonPressMask | ButtonReleaseMask | PointerMotionMask | PointerMotionHintMask | KeyPress | \
   KeyRelease)
#define POINTER_GRAB_MASK \
...
static void GetConfigEvent(Display *display, Window window, unsigned long mask, XEvent *event)
{
  ...
  if (event->type == MotionNotify && event->xmotion.is_hint == NotifyHint) {
    /*
     * "Ack" the motion notify hint
     */
    if ((XQueryPointer(
            display, window, &root_ret, &child_ret, &root_x, &root_y, &win_x, &win_y, &mask_ret)))
    {
      /*
...
static Cursor GetTearOffCursor(Widget wid)
{
  ...
  Cursor TearOffCursor = ((XmDisplayInfo *)(dd->display.displayInfo))->TearOffCursor;
  if (0L == TearOffCursor) {
     ...
     */
    TearOffCursor = XCreateFontCursor(XtDisplay(wid), XC_fleur);
    if (0L == TearOffCursor)
      TearOffCursor = _XmGetMenuCursorByScreen(XtScreen(wid));
    else
      XtAddCallback(
          (Widget)dd, XtNdestroyCallback, DisplayDestroyCallback, (XtPointer)TearOffCursor);
    ((XmDisplayInfo *)(dd->display.displayInfo))->TearOffCursor = TearOffCursor;
  }
...
static Boolean DoPlacement(Widget wid, XEvent *event)
{
  ...
  XGrabPointer(XtDisplay(rc),
               ...
               CurrentTime);
  XGrabKeyboard(
      XtDisplay(rc), rc->core.screen->root, FALSE, GrabModeAsync, GrabModeAsync, CurrentTime);
    ...
    switch (event->type) {
      ...
      case MotionNotify:
        if (moveOpaque)
          ...
          MoveOutline((Widget)rc, tearoffGC, outline, event, delta_x, delta_y);
        break;
      case KeyPress:
...
static void CallTearOffMenuActivateCallback(Widget wid, XEvent *event, unsigned short origin)
{
  ...
  XtCallCallbackList((Widget)rc, rc->row_column.tear_off_activated_callback, &callback);
}
...
static void CallTearOffMenuDeactivateCallback(Widget wid, XEvent *event, unsigned short origin)
{
  ...
  callback.data = (char *)(unsigned long)origin;
  callback.callbackstruct = NULL;
  XtCallCallbackList((Widget)rc, rc->row_column.tear_off_deactivated_callback, &callback);
}
                                   ...
                                   Boolean *cont)
{
  ...
  if (reportingWidget) {
    ...
    if ((XmIsRowColumn(parent = XtParent(reportingWidget))) && (RC_Type(parent) != XmWORK_AREA)) {
...
void _XmAddTearOffEventHandlers(Widget wid)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
  Widget child;
  int i;
  ...
  for (i = 0; i < rc->composite.num_children; i++) {
    ...
    if (!XmIsGadget(child) &&
        (mtrait == (XmMenuSavvyTrait)NULL ||
         mtrait->getActivateCBName == (XmMenuSavvyGetActivateCBNameProc)NULL))
    {
      XtAddEventHandler(child, ButtonPressMask, False, _XmTearOffBtnDownEventHandler, NULL);
      XtAddEventHandler(child, ButtonReleaseMask, False, _XmTearOffBtnUpEventHandler, NULL);
    }
    if (XtIsWidget(child))
      XtGrabButton(child,
                   (int)AnyButton,
...
static void RemoveTearOffEventHandlers(Widget wid)
{
  ...
  for (i = 0; i < rc->composite.num_children; i++) {
    ...
    {
      XtRemoveEventHandler(child, ButtonPressMask, False, _XmTearOffBtnDownEventHandler, NULL);
      XtRemoveEventHandler(child, ButtonReleaseMask, False, _XmTearOffBtnUpEventHandler, NULL);
    }
...
void _XmDestroyTearOffShell(Widget wid)
{
...
void _XmDismissTearOff(Widget shell, XtPointer closure, XtPointer call_data) /* unused */
{
  XmRowColumnWidget submenu = NULL;
  /* The first time a pane is torn, there's no tear off shell to destroy.
   */
  if (!shell || !(((ShellWidget)shell)->composite.num_children) ||
      !(submenu = (XmRowColumnWidget)(((ShellWidget)shell)->composite.children[0])) ||
      !RC_TornOff(submenu))
    return;
  ...
  if (submenu->manager.active_child) {
    ...
    if (XmIsPrimitive(submenu->manager.active_child)) {
      (*(((XmPrimitiveClassRec *)XtClass(submenu->manager.active_child))
             ->primitive_class.border_unhighlight))(submenu->manager.active_child);
    }
    else if (XmIsGadget(submenu->manager.active_child)) {
      (*(((XmGadgetClassRec *)XtClass(submenu->manager.active_child))
             ->gadget_class.border_unhighlight))(submenu->manager.active_child);
    }
  ...
  if (XmIsMenuShell(shell)) {
    ...
    XtRemoveCallback(submenu->row_column.tear_off_lastSelectToplevel,
                     XtNdestroyCallback,
                     (XtCallbackProc)DismissOnPostedFromDestroy,
    ...
    if (submenu) {
      ...
      XReparentWindow(XtDisplay(submenu),
                      XtWindow(submenu),
                      XtWindow(XtParent(submenu)),
                      XtX(submenu),
                      XtY(submenu));
    ...
    CallTearOffMenuDeactivateCallback((Widget)submenu, (XEvent *)closure, DESTROY_TEAR_OFF);
...
static void DismissOnPostedFromDestroy(Widget w, /* unused */
...
void _XmTearOffInitiate(Widget wid, XEvent *event)
{
  ...
  if ((event->xbutton.type == ButtonPress) && (event->xbutton.button == 2 /*BDrag*/)) {
    if (!DoPlacement((Widget)submenu, &newEvent)) /* Cancelled! */ {
      ...
      _XmGrabPointer(
          ...
          CurrentTime);
      _XmGrabKeyboard((Widget)rc, True, GrabModeSync, GrabModeSync, CurrentTime);
  ...
  if (mst->RC_LastSelectToplevel) {
    submenu->row_column.tear_off_lastSelectToplevel = mst->RC_LastSelectToplevel;
  }
  else if (RC_TornOff(rc) && RC_TearOffActive(rc))
    submenu->row_column.tear_off_lastSelectToplevel = rc->row_column.tear_off_lastSelectToplevel;
  else {
    /* Fix CR 7983 - Assigning NULL RC_CascadeBtn causes core dump */
    if (IsPopup(submenu) && RC_CascadeBtn(submenu))
      submenu->row_column.tear_off_lastSelectToplevel = RC_CascadeBtn(submenu);
    else
  ...
  if (!XmIsMenuShell(XtParent(rc))) /* MenuBar or TearOff */
    (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(
        RC_PopupPosted(rc), event, NULL, NULL);
  else
    (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(
   ...
   */
  (*(((XmRowColumnClassRec *)XtClass(rc))->row_column_class.menuProcedures))(XmMENU_DISARM,
                                                                             (Widget)rc);
  ...
  /* system & title */
  XtSetArg(args[1], XmNmwmDecorations, MWM_DECOR_BORDER | MWM_DECOR_TITLE | MWM_DECOR_MENU);
      ...
      IsPopup(submenu->row_column.tear_off_lastSelectToplevel))
  {
    XtSetArg(args[4],
             XmNtransientFor,
             _XmFindTopMostShell(RC_CascadeBtn(submenu->row_column.tear_off_lastSelectToplevel)));
  }
  else
    XtSetArg(args[4],
             XmNtransientFor,
   ...
   */
  XtSetArg(args[6], XmNvisual, ((XmMenuShellWidget)(XtParent(wid)))->shell.visual);
  XtSetArg(args[7], XmNcolormap, ((XmMenuShellWidget)(XtParent(wid)))->core.colormap);
  XtSetArg(args[8], XmNdepth, ((XmMenuShellWidget)(XtParent(wid)))->core.depth);
  /* End Fix CR 5459 */
  to_shell = (ShellWidget)XtCreatePopupShell(
      DEFAULT_TEAR_OFF_TITLE, transientShellWidgetClass, toplevel, args, 9);
  ...
  XInternAtoms(XtDisplay(to_shell), atom_names, XtNumber(atom_names), FALSE, atoms);
  XmAddWMProtocolCallback((Widget)to_shell, atoms[XmAWM_DELETE_WINDOW], _XmDismissTearOff, NULL);
  ...
  CallTearOffMenuActivateCallback((Widget)submenu, event, CREATE_TEAR_OFF);
  ...
  _XmProcessLock();
  proc = ((TransientShellWidgetClass)transientShellWidgetClass)->composite_class.insert_child;
  _XmProcessUnlock();
  (*proc)((Widget)submenu);
  ...
  XmeConfigureObject((Widget)to_shell,
                     newEvent.xbutton.x_root,
                     newEvent.xbutton.y_root,
                     XtWidth(submenu),
                     XtHeight(submenu),
                     XtBorderWidth(to_shell));
  /* Call change_managed routine to set up focus info */
  _XmProcessLock();
  proc = ((TransientShellWidgetClass)transientShellWidgetClass)->composite_class.change_managed;
  _XmProcessUnlock();
...
Boolean _XmIsTearOffShellDescendant(Widget wid)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
  Widget cb;
...
void _XmLowerTearOffObscuringPoppingDownPanes(Widget ancestor, Widget tearOff)
{
...
void _XmRestoreExcludedTearOffToToplevelShell(Widget wid, XEvent *event)
{
  int i;
  Widget pane;
  XmDisplay dd = (XmDisplay)XmGetXmDisplay(XtDisplay(wid));
 ...
 */
void _XmRestoreTearOffToToplevelShell(Widget wid, XEvent *event)
{
  ...
  if (RC_TornOff(rowcol) && !RC_TearOffActive(rowcol)) {
    ...
    /* Sync up the server */
    XReparentWindow(XtDisplay(shell), XtWindow(rowcol), XtWindow(XtParent(rowcol)), 0, 0);
    XFlush(XtDisplay(shell));
    if (XtParent(rowcol)->core.background_pixmap != XtUnspecifiedPixmap) {
      XFreePixmap(XtDisplay(XtParent(rowcol)), XtParent(rowcol)->core.background_pixmap);
    ...
    if (IsPulldown(rowcol))
      rowcol->row_column.lastSelectToplevel = rowcol->row_column.tear_off_lastSelectToplevel;
    else /* IsPopup */
      RC_CascadeBtn(rowcol) = rowcol->row_column.tear_off_lastSelectToplevel;
    CallTearOffMenuActivateCallback((Widget)rowcol, event, RESTORE_TEAR_OFF_TO_TOPLEVEL_SHELL);
     ...
     */
    answer = XtMakeResizeRequest(
        XtParent(rowcol), XtWidth(rowcol), XtHeight(rowcol), &almostWidth, &almostHeight);
    if (answer == XtGeometryAlmost)
      answer = XtMakeResizeRequest(XtParent(rowcol), almostWidth, almostHeight, NULL, NULL);
    ...
    XmProcessTraversal(rowcol->row_column.tear_off_focus_item, XmTRAVERSE_CURRENT);
  }
}

void _XmRestoreTearOffToMenuShell(Widget wid, XEvent *event)
{
  XmRowColumnWidget submenu = (XmRowColumnWidget)wid;
  XmMenuState mst = _XmGetMenuState((Widget)wid);
  XtExposeProc expose;
  ...
  if (RC_TornOff(submenu) && RC_TearOffActive(submenu)) {
        ...
        XmeFocusIsInShell((Widget)submenu))
    {
      ...
      if ((child = submenu->manager.active_child) != NULL) {
        if (XtIsWidget(child))
          (*(((XmPrimitiveClassRec *)XtClass(child))->primitive_class.border_unhighlight))(child);
        else
          (*(((XmGadgetClassRec *)XtClass(child))->gadget_class.border_unhighlight))(child);
      }
     ...
     */
    submenu->row_column.tear_off_focus_item = XmGetFocusWidget((Widget)submenu);
    _XmClearFocusPath((Widget)submenu);
    /* Get a pixmap holder first! */
    values.graphics_exposures = False;
    values.subwindow_mode = IncludeInferiors;
    gc = XtGetGC((Widget)shell, GCGraphicsExposures | GCSubwindowMode, &values);
    ...
    submenu->core.managed = False;
    /* Sync up the server */
    XSetWindowBackgroundPixmap(XtDisplay(shell), XtWindow(shell), shell->core.background_pixmap);
    XReparentWindow(XtDisplay(shell),
                    XtWindow(submenu),
                    XtWindow(XtParent(submenu)),
                    XtX(submenu),
                    XtY(submenu));
    ...
    CallTearOffMenuDeactivateCallback((Widget)submenu, event, RESTORE_TEAR_OFF_TO_MENUSHELL);
    RemoveTearOffEventHandlers((Widget)submenu);
  }
}
