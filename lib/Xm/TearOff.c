...
{
   /* Move the MenuShell */
   XMoveWindow(XtDisplay(wid), XtWindow(XtParent(wid)), event->xbutton.x_root - delta_x, event->xbutton.y_root - delta_y);
   ...
   XEvent       *event)
{
   ...
   if (event->type == MotionNotify && event->xmotion.is_hint == NotifyHint)
   {
      /*
        ...
        */
      if ((XQueryPointer(display, window, &root_ret, &child_ret, &root_x, &root_y, &win_x, &win_y, &mask_ret)))
      {
         /*
   ...
   XEvent *event)
{
    ...
    */
   XGrabPointer(XtDisplay(rc), rc->core.screen->root, FALSE, (unsigned int)POINTER_GRAB_MASK, GrabModeAsync, GrabModeAsync, rc->core.screen->root, GetTearOffCursor(wid), CurrentTime);
   XGrabKeyboard(XtDisplay(rc), rc->core.screen->root, FALSE, GrabModeAsync, GrabModeAsync, CurrentTime);
   ...
   while (!placementDone)
   {
      GetConfigEvent(XtDisplay(rc), rc->core.screen->root, CONFIG_GRAB_MASK, event); /* ok to overwrite event? */
      switch (event->type)
      {
         case ButtonRelease:
   ...
   unsigned short origin)
{
   ...
   XtCallCallbackList((Widget)rc,
                      rc->row_column.tear_off_deactivated_callback,
                      &callback);
}
   ...
   Widget wid)
{
   ...
   for (i = 0; i < rc->composite.num_children; i++)
   {
	 ...
	 *
	 */
      if (!XmIsGadget(child) && (mtrait == (XmMenuSavvyTrait)NULL || mtrait->getActivateCBName == (XmMenuSavvyGetActivateCBNameProc)NULL))
      {
      ...
      if (XtIsWidget(child))
         XtGrabButton(child, (int)AnyButton, AnyModifier, TRUE, (unsigned int)ButtonPressMask, GrabModeAsync, GrabModeAsync, None, cursor);
   }
}
   ...
   Widget wid)
{
   ...
   for (i = 0; i < rc->composite.num_children; i++)
   {
	 ...
	 */
      if ((XtClass(child) == xmLabelWidgetClass) || _XmIsFastSubclass(XtClass(child), XmSEPARATOR_BIT))
      {
   ...
   XtPointer call_data) /* unused */
{
    ...
    */
   if (!shell || !(((ShellWidget)shell)->composite.num_children) || !(submenu = (XmRowColumnWidget)(((ShellWidget)shell)->composite.children[0])) || !RC_TornOff(submenu))
      return;
   ...
   else /* toplevel shell! */
   {
      ...
      if (submenu)
      {
         ...
         XReparentWindow(XtDisplay(submenu), XtWindow(submenu), XtWindow(XtParent(submenu)), XtX(submenu), XtY(submenu));
   ...
   XEvent *event)
{
   ...
   if (!(XmIsMenuShell(XtParent(submenu)) && ((XmMenuShellWidget)XtParent(submenu))->shell.popped_up))
      return;
   ...
   if ((event->xbutton.type == ButtonPress) && (event->xbutton.button == 2 /*BDrag*/))
   {
      if (!DoPlacement((Widget)submenu, &newEvent)) /* Cancelled! */
      {
         ...
         _XmGrabPointer((Widget)rc, True, (unsigned int)(ButtonPressMask | ButtonReleaseMask | EnterWindowMask | LeaveWindowMask), GrabModeSync, GrabModeAsync, None, _XmGetMenuCursorByScreen(XtScreen(rc)), CurrentTime);
   ...
   if (!XmIsMenuShell(XtParent(rc))) /* MenuBar or TearOff */
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(RC_PopupPosted(rc), event, NULL, NULL);
   else
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(XtParent(rc), event, NULL, NULL);
    ...
    */
   (*(((XmRowColumnClassRec *)XtClass(rc))->row_column_class.menuProcedures))(XmMENU_DISARM, (Widget)rc);
   ...
   if (XmIsRowColumn(submenu->row_column.tear_off_lastSelectToplevel) && IsPopup(submenu->row_column.tear_off_lastSelectToplevel))
   {
      XtSetArg(args[4], XmNtransientFor, _XmFindTopMostShell(RC_CascadeBtn(submenu->row_column.tear_off_lastSelectToplevel)));
   }
   else
      XtSetArg(args[4], XmNtransientFor, _XmFindTopMostShell(submenu->row_column.tear_off_lastSelectToplevel));
   ...
   XGetWindowProperty(XtDisplay(to_shell), XtWindow(to_shell), atoms[XmA_MOTIF_WM_HINTS], 0, PROP_MWM_HINTS_ELEMENTS, False, atoms[XmA_MOTIF_WM_HINTS], &actual_type, &actual_format, &num_items, &bytes_after, (unsigned char **)&rprop);
   if ((actual_type != atoms[XmA_MOTIF_WM_HINTS]) || (actual_format != 32) || (num_items < PROP_MOTIF_WM_INFO_ELEMENTS))
   {
   ...
   else
   {
	 ...
	 size of block from get property */
      ...
      XChangeProperty(XtDisplay(to_shell), XtWindow(to_shell), atoms[XmA_MOTIF_WM_HINTS], atoms[XmA_MOTIF_WM_HINTS], 32, PropModeReplace, (unsigned char *)&sprop, PROP_MWM_HINTS_ELEMENTS);
   }
   ...
   XEvent *event)
{
   ...
   if (RC_TornOff(rowcol) && !RC_TearOffActive(rowcol))
   {
       ...
       */
      answer = XtMakeResizeRequest(XtParent(rowcol), XtWidth(rowcol), XtHeight(rowcol), &almostWidth, &almostHeight);
   ...
   XEvent *event)
{
   ...
   if (RC_TornOff(submenu) && RC_TearOffActive(submenu))
   {
       ...
       *   to this submenu.
       */
      if (RC_TearOffDirty(submenu) || (event && (event->type == ButtonPress) && (event->xbutton.time == mst->RC_ReplayInfo.time) && (mst->RC_ReplayInfo.toplevel_menu == (Widget)submenu)) || XmeFocusIsInShell((Widget)submenu))
      {
         RC_SetTearOffDirty(submenu, False);
         wasDirty = True;
	  ...
	  */
      ...
      shell->core.background_pixmap = XCreatePixmap(XtDisplay(shell),
                                                    ...
                                                    shell->core.depth);
      /* End of Fix #4855 */
      XCopyArea(XtDisplay(shell), XtWindow(submenu), shell->core.background_pixmap, gc, 0, 0, shell->core.width, shell->core.height, 0, 0);
      ...
      XReparentWindow(XtDisplay(shell), XtWindow(submenu), XtWindow(XtParent(submenu)), XtX(submenu), XtY(submenu));
      XtManageChild(RC_TearOffControl(submenu));
