 ...
 */
int
_XmPngGetImage(FILE *fp, XColor *bg, XImage **ximage)
{
   long          pos;
   XImage       *img = NULL;
   ...
   if (!ximage || !fp || fread(header, 1, sizeof header, fp) != sizeof header || png_sig_cmp(header, 0, sizeof header))
      return 1;
   *ximage = NULL;
   if (!(pngp = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL)) || !(infop = png_create_info_struct(pngp)))
   {
   ...
   png_read_png(pngp, infop, PNG_TRANSFORM_SWAP_ALPHA | PNG_TRANSFORM_GRAY_TO_RGB | PNG_TRANSFORM_SCALE_16 | PNG_TRANSFORM_EXPAND, NULL);
   png_destroy_read_struct(&pngp, &infop, NULL);
   free(rows);
   /* Create our XImage */
