	  ...
	  */
      ...
      else
      {
         ...
         if (h > subtrahend)
         {
            ...
            if (m_top && (XmIsText(kg[start_i].kid) || XmIsTextField(kg[start_i].kid) || XmIsCSText(kg[start_i].kid)))
            {
   ...
   XmRowColumnWidget menu)
{
   ...
   if ((cb = XmOptionButtonGadget((Widget)menu)) != NULL)
   {
      if (RC_OptionSubMenu(menu))
      {
         ...
         if (LayoutIsRtoLG(cb))
            width += Double(G_HighlightThickness(cb)) + G_ShadowThickness(cb) + LabG_MarginLeft(cb) + Double(MGR_ShadowThickness(RC_OptionSubMenu(menu))) - 2;
         else
            width += Double(G_HighlightThickness(cb)) + G_ShadowThickness(cb) + LabG_MarginRight(cb) + Double(MGR_ShadowThickness(RC_OptionSubMenu(menu))) - 2;
      ...
      if (XmIsPrimitive(kg[start_i].kid) || XmIsGadget(kg[start_i].kid))
      {
         ...
         if (label_type == XmSTRING)
         {
            if (kg[start_i].baseline < baseline)
            {
               ...
               {
                  kg[start_i].margin_bottom += h - (kg[start_i].box.height + (baseline - kg[start_i].baseline));
                  kg[start_i].box.height     = h;
               }
            }
            else
   ...
   *toc_b = *b = Double(RC_EntryBorder(m));
   if (RC_TearOffControl(m) && XtIsManaged(RC_TearOffControl(m)))
   {
      ...
      *toc_height = *toc_height >> r; /* r is 1 or 2 depending on the
      ...
      ASSIGN_MAX(*toc_height, 2 + *toc_b + 2 * ((XmPrimitiveWidget)kg[0].kid)->primitive.shadow_thickness);
      /* Sync up the kid geo */
      /* Fix CR# 4778 */
   ...
   Dimension        *m_height)       /* if 0 then caller's asking */
{
   ...
   GetMaxValues(m, &border, &w, &h, &items_per_column, &baseline, &shadow, &highlight, &margin_top, &margin_height, &text_height);
   ...
   else if (AlignmentTop(m) || AlignmentBottom(m))
   {
      ...
      while (kg[kid_i].kid != NULL)
         kid_i++;
      TopOrBottomAlignment(m, h, shadow, highlight, baseline, margin_top, margin_height, text_height, &new_height, 0, kid_i);
   }
   ...
   Dimension        *m_height)       /* if 0 then caller's asking */
{
   ...
   for (; kg[child_i].kid != NULL; child_i++)
   {
      ...
      if (((y + h) > *m_height) && (!Asking(*m_height)) && (child_i))
      { /* start new column */
   ...
   Dimension        *m_height)       /* if 0 then caller's asking */
{
   ...
   for (; kg[child_i].kid != NULL; child_i++)
   {
      ...
      if (((x + w) > *m_width) && (!Asking(*m_width)) && (child_i))
      { /* start a new row */
         if (AlignmentBaselineTop(m) || AlignmentBaselineBottom(m))
            BaselineAlignment(m, h, shadow, highlight, baseline, &new_height, start_i, child_i);
         else if (AlignmentTop(m) || AlignmentBottom(m))
            TopOrBottomAlignment(m, h, shadow, highlight, baseline, margin_top, margin_height, text_height, &new_height, start_i, child_i);
         else if (AlignmentCenter(m))
   ...
   {
      bx = &(kg[child_i].box);
      ASSIGN_MIN(BWidth(bx), *m_width - Double(MGR_ShadowThickness(m)) - Double(RC_MarginW(m)) - Double(BBorder(bx)));
      if (IsHelp(m, ((Widget)kg[child_i].kid)))
   ...
   Boolean                    calcMenuDimension)
{
   ...
   if (p)
   {
      ...
      if (LayoutIsRtoLG(cb))
         c_width += Double(G_HighlightThickness(cb)) + G_ShadowThickness(cb) + LabG_MarginLeft(cb) + Double(MGR_ShadowThickness(p)) -
                    /* magic value */ 2;
      else
         c_width += Double(G_HighlightThickness(cb)) + G_ShadowThickness(cb) + LabG_MarginRight(cb) + Double(MGR_ShadowThickness(p)) -
                    /* magic value */ 2;
   ...
   Dimension        *h)
{
   ...
   int        line_count;
   Dimension  y;
   if ((!IsOption(m)) && ((PackColumn(m) && (IsVertical(m) || IsHorizontal(m))) || (PackTight(m) && IsHorizontal(m))))
   {
    ...
    */
      RC_Boxes(m) = (XmRCKidGeometry)_XmRCGetKidGeo((Widget)m, NULL, NULL, RC_EntryBorder(m), RC_EntryBorder(m), (IsVertical(m) && RC_DoMarginAdjust(m)), (IsHorizontal(m) && RC_DoMarginAdjust(m)), RC_HelpPb(m), RC_TearOffControl(m), XmGET_PREFERRED_SIZE);
   ...
   XtWidgetGeometry *request)
{
    ...
    */
   RC_Boxes(m) = (XmRCKidGeometry)_XmRCGetKidGeo((Widget)m, instigator, request, RC_EntryBorder(m), RC_EntryBorder(m), (IsVertical(m) && RC_DoMarginAdjust(m)), (IsHorizontal(m) && RC_DoMarginAdjust(m)), RC_HelpPb(m), RC_TearOffControl(m), XmGET_PREFERRED_SIZE);
   if ((!IsOption(m)) && ((PackColumn(m) && (IsVertical(m) || IsHorizontal(m))) || (PackTight(m) && IsHorizontal(m))))
   {
      for (i = 0; RC_Boxes(m)[i].kid != NULL; i++)
      {
         if (XmIsGadget(RC_Boxes(m)[i].kid) || XmIsPrimitive(RC_Boxes(m)[i].kid))
         {
            ...
            {
               XmeConfigureObject((Widget)ro, ro->rectangle.x, ro->rectangle.y, ro->rectangle.width, RC_Boxes(m)[i].box.height, ro->rectangle.border_width);
            }
   ...
   ** time to figure out the real size. This is not a frequent case.
    ...
    */
   if (!IsOption(m) || !instigator || !((instigator == RC_Boxes(m)[0].kid) || (instigator == RC_Boxes(m)[1].kid)))
   {
   ...
   if ((instigator) && (instigator_w == XtWidth(instigator)) && (instigator_h == XtHeight(instigator)) && (XmIsLabel(instigator) || XmIsLabelGadget(instigator)))
   {
   ...
   int               geo_type)                             /* Actual or preferred. */
{
   ...
   geo      = (XmRCKidGeometry)XtMalloc((_XmGeoCount_kids(c) + 1 + tocFound) * sizeof(XmRCKidGeometryRec));
   i        = 0;
   if (tocFound)
   {
      geo[j].kid = toc;
   ...
   for (; i < c->composite.num_children; i++)
   {
      ...
      if (XtIsManaged(kidWid))
      {
		...
		will take the separator out of the width consideration. */
            ...
            if ((XmIsSeparator(kidWid) || XmIsSeparatorGadget(kidWid)) && (RC_Packing(c) != XmPACK_NONE))
            {
   ...
   ForManagedChildren(m, i, p)
   {
      if (XmIsLabelGadget(*p))
      {
         ...
         if (IsVertical(m))
         {
            ...
            {
               XmeConfigureObject((Widget)q, q->rectangle.x, q->rectangle.y, w, q->rectangle.height, q->rectangle.border_width);
            }
         }
         if (!IsVertical(m) || PackColumn(m))
         {
            ...
            {
               XmeConfigureObject((Widget)q, q->rectangle.x, q->rectangle.y, q->rectangle.width, h, q->rectangle.border_width);
            }
      ...
      else if (XmIsLabel(*p))
      {
         ...
         {                  /* be uniform */
            ...
            if (XtWidth(lw) != w)
            {
               XmeConfigureObject((Widget)lw, lw->core.x, lw->core.y, w, lw->core.height, lw->core.border_width);
            }
         }
         if (!IsVertical(m) || PackColumn(m)) /* change vert margins */
         {
            ...
            if (XtHeight(lw) != h)
            {
               XmeConfigureObject((Widget)lw, lw->core.x, lw->core.y, lw->core.width, h, lw->core.border_width);
            }
