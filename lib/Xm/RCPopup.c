...
static void
PopupMenuEventHandler(Widget wid, XtPointer poster, XEvent *e, Boolean *cont)
{
   Boolean                      seen;
   Widget                       postWidget;
   Widget                       popup;
   ...
   if (e->type != ButtonPress && e->type != ButtonRelease && e->type != KeyPress && e->type != KeyRelease)
      return;
   ...
   {
      mst->RC_ButtonEventStatus.verified = _XmMatchBtnEvent(e, RC_PostEventType(popupW), RC_PostButton(popupW), RC_PostModifiers(popupW));
   }
   ...
   if (mst->RC_ButtonEventStatus.verified)
   {
      if (!popupW->core.being_destroyed && !popupW->row_column.popup_workproc)
         popupW->row_column.popup_workproc = XtAppAddWorkProc(XtWidgetToApplicationContext((Widget)popupW),
                                                              _XmRC_PostTimeOut,
                                                              (XtPointer)popupW);
...
MenuMatches(Widget menu, int level, XEvent *event)
{
   ...
   if (XtIsShell(menu) && ((CompositeRec *)menu)->composite.num_children == 1)
   {
      ...
      if (XmIsRowColumn(menu) && IsPopup(menu) && (((level == 0) && (RC_PopupEnabled(menu) == XmPOPUP_AUTOMATIC)) || (RC_PopupEnabled(menu) == XmPOPUP_AUTOMATIC_RECURSIVE)))
	 ...
	 an appropriate popup enabled value */
      {
         ...
         {
            found = _XmMatchBtnEvent(event, RC_PostEventType(menu), RC_PostButton(menu), RC_PostModifiers(menu));
         }
      }
   }
   if (found)
   ...
   Boolean  *cont)
{
    ...
    */
   if ((mst->RC_ButtonEventStatus.time == xbutton_event->time) && (mst->RC_ButtonEventStatus.verified == True))
      /*
      ...
      */
      if (!RC_TornOff(popup) || !_XmMatchBtnEvent(event, RC_PostEventType(popup), RC_PostButton(popup), RC_PostModifiers(popup)))
         return;
   mst->RC_ButtonEventStatus.time     = xbutton_event->time;
   mst->RC_ButtonEventStatus.verified = _XmMatchBtnEvent(event,
                                                         RC_PostEventType(popup),
                                                         RC_PostButton(popup),
   ...
   if (mst->RC_ButtonEventStatus.verified)
   {
      ...
      if (!popup->core.being_destroyed && !popup->row_column.popup_workproc)
         popup->row_column.popup_workproc = XtAppAddWorkProc(XtWidgetToApplicationContext((Widget)popup),
                                                             _XmRC_PostTimeOut,
                                                             (XtPointer)popup);
   ...
   Widget widget)
{
   ...
   if (RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC || RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC_RECURSIVE)
      XtInsertEventHandler(widget, ButtonPressMask | ButtonReleaseMask, False, PopupMenuEventHandler, (XtPointer)popup, XtListHead);
   else
      XtInsertEventHandler(widget, ButtonPressMask | ButtonReleaseMask, False, ButtonEventHandler, (XtPointer)popup, XtListHead);
   if ((RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC || RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC_RECURSIVE))
      XtAddEventHandler(widget, KeyPressMask | KeyReleaseMask, False, PopupMenuEventHandler, (XtPointer)popup);
   else
      XtAddEventHandler(widget, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)popup);
   /*
    ...
    */
   XtGrabButton(widget, RC_PostButton(popup), RC_PostModifiers(popup), TRUE, (unsigned int)ButtonReleaseMask, GrabModeAsync, GrabModeSync, None, cursor);
}
...
{
   if (RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC || RC_PopupEnabled(popup) == XmPOPUP_AUTOMATIC_RECURSIVE)
      XtRemoveEventHandler(widget, ButtonPressMask | ButtonReleaseMask, False, PopupMenuEventHandler, (XtPointer)popup);
   else
      XtRemoveEventHandler(widget, ButtonPressMask | ButtonReleaseMask, False, ButtonEventHandler, (XtPointer)popup);
   XtRemoveEventHandler(widget, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)popup);
   ...
   XmRowColumnWidget pane)
{
   ...
   XtAddEventHandler((Widget)pane, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)pane);
   /* Add to Our shell parent */
   XtAddEventHandler(XtParent(pane), KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, pane);
   ...
   XmRowColumnWidget pane)
{
   int i;
   /* Remove it from us */
   XtRemoveEventHandler((Widget)pane, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)pane);
   /* Remove it from our shell parent */
   XtRemoveEventHandler(XtParent(pane), KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)pane);
...
_XmAllowAcceleratedInsensitiveUnmanagedMenuItems(
   ...
   Boolean allowed)
{
   _XmGetMenuState((Widget)wid)->RC_allowAcceleratedInsensitiveUnmanagedMenuItems = (Boolean)allowed;
}
   ...
   Boolean  *cont) /* unused */
{
   ...
   else if ((IsPopup(topLevel) || IsPulldown(topLevel)) && !XmIsMenuShell(topLevelShell) && XmeFocusIsInShell((Widget)topLevel))
         ...
         if (XtIsSensitive(child) && XtIsManaged(child))
         {
            if (XmIsCascadeButtonGadget(child))
            {
               if (CBG_Submenu(child))
               {
                  ...
                  found                             = ProcessKey((XmRowColumnWidget)((XmCascadeButtonGadget)child)->cascade_button.submenu,
                                     event);
		    ...
		    */
            ...
            else if (XmIsCascadeButton(child))
            {
               if (CB_Submenu(child))
               {
                  ...
                  found                            = ProcessKey((XmRowColumnWidget)((XmCascadeButtonWidget)child)->cascade_button.submenu,
                                     event);
                  if (!found)
   ...
   XEvent           *event)
{
   ...
   while ((menu_index = MatchInKeyboardList(rowcol, (XKeyEvent *)event, menu_index)) != -1)
   {
	  ...
	  */
         if (!XtIsSensitive(entry->component) || ((RC_Type(entry->component) != XmMENU_POPUP) && (RC_Type(entry->component) != XmMENU_PULLDOWN) && (!XtIsManaged(entry->component))))
         {
            menu_index++;
            continue;
         }
      }
      else if (((XmIsMenuShell(XtParent(rowcol)) && ((XmMenuShellWidget)XtParent(rowcol))->shell.popped_up) || !_XmGetMenuState((Widget)rowcol)->RC_allowAcceleratedInsensitiveUnmanagedMenuItems) && (!XtIsSensitive(entry->component) || !XtIsManaged(entry->component)))
      {
      ...
      if (entry->isMnemonic)
      {
         if (XmeTraitGet((XtPointer)XtClass(entry->component),
             ...
             != NULL)
         {
            if (IsBar(XtParent(entry->component)) && !RC_PopupPosted(XtParent(entry->component)) && ((XmManagerWidget)XtParent(entry->component))->manager.active_child == NULL)
            {
               menu_index++;
               continue;
            }
		...
		*/
               if ((!shell->shell.popped_up) || (shell->composite.children[0] != XtParent(entry->component)))
               {
                  menu_index++;
                  continue;
               }
   ...
   int               startIndex)
{
   ...
   for (i = startIndex; i < count; i++)
   {
      ...
      {
         if (_XmMatchKeyEvent((XEvent *)event, klist[i].eventType, klist[i].key, klist[i].isMnemonic ? klist[i].modifiers | (event->state & (ShiftMask | LockMask)) : klist[i].modifiers))
         {
            return (i);
         }
      }
