...
static XtResource     output_resources[] = {
   { XmNfontList,              XmCFontList,              XmRFontList,    sizeof(XmFontList),    XtOffsetOf(OutputDataRec,         fontlist),                XmRImmediate, (XtPointer)NULL                },
   { XmNrenderTable,           XmCRenderTable,           XmRRenderTable, sizeof(XmRenderTable), XtOffsetOf(OutputDataRec,         rendertable),             XmRImmediate, (XtPointer)NULL                },
   { XmNwordWrap,              XmCWordWrap,              XmRBoolean,     sizeof(Boolean),       XtOffsetOf(struct _OutputDataRec, wordwrap),                XmRImmediate, (XtPointer)False               },
   { XmNblinkRate,             XmCBlinkRate,             XmRInt,         sizeof(int),           XtOffsetOf(struct _OutputDataRec, blinkrate),               XmRImmediate, (XtPointer)500                 },
   { XmNcolumns,               XmCColumns,               XmRShort,       sizeof(short),         XtOffsetOf(struct _OutputDataRec, columns),                 XmRImmediate, (XtPointer)0                   },
   { XmNrows,                  XmCRows,                  XmRShort,       sizeof(short),         XtOffsetOf(struct _OutputDataRec, rows),                    XmRImmediate, (XtPointer)0                   },
   { XmNresizeWidth,           XmCResizeWidth,           XmRBoolean,     sizeof(Boolean),       XtOffsetOf(struct _OutputDataRec, resizewidth),             XmRImmediate, (XtPointer)False               },
   { XmNresizeHeight,          XmCResizeHeight,          XmRBoolean,     sizeof(Boolean),       XtOffsetOf(struct _OutputDataRec, resizeheight),            XmRImmediate, (XtPointer)False               },
   { XmNscrollVertical,        XmCScroll,                XmRBoolean,     sizeof(Boolean),       XtOffsetOf(struct _OutputDataRec, scrollvertical),          XmRImmediate, (XtPointer)True                },
   { XmNscrollHorizontal,      XmCScroll,                XmRBoolean,     sizeof(Boolean),       XtOffsetOf(struct _OutputDataRec, scrollhorizontal),        XmRImmediate, (XtPointer)True                },
   { XmNscrollLeftSide,        XmCScrollSide,            XmRBoolean,     sizeof(Boolean),       XtOffsetOf(struct _OutputDataRec, scrollleftside),          XmRImmediate, (XtPointer)XmDYNAMIC_BOOL      },
   { XmNscrollTopSide,         XmCScrollSide,            XmRBoolean,     sizeof(Boolean),       XtOffsetOf(struct _OutputDataRec, scrolltopside),           XmRImmediate, (XtPointer)False               },
   { XmNcursorPositionVisible, XmCCursorPositionVisible, XmRBoolean,     sizeof(Boolean),       XtOffsetOf(struct _OutputDataRec, cursor_position_visible), XmRCallProc,  (XtPointer)CursorPosVisDefault },
};
          ...
          GC           gc)
{
   ...
   ClipRect.width  = tw->core.width - (2 * (tw->primitive.highlight_thickness + tw->primitive.shadow_thickness));
   ClipRect.height = tw->core.height - (2 * (tw->primitive.highlight_thickness + tw->primitive.shadow_thickness));
...
GetRect(XmTextWidget tw,
        XRectangle  *rect)
{
   Dimension margin_width  = tw->text.margin_width + tw->primitive.shadow_thickness + tw->primitive.highlight_thickness;
   Dimension margin_height = tw->text.margin_height + tw->primitive.shadow_thickness + tw->primitive.highlight_thickness;
   ...
   if (tw->text.char_size != 1)
   {
      ...
      for (i = from, ptr = block->ptr + from; i < to; i += csize, ptr += csize)
      {
         ...
         if (csize == 1)
         {
            ...
            {
               if (font->per_char && (c >= font->min_char_or_byte2 && c <= font->max_char_or_byte2))
         ...
         else
         {
            if (_XmIsISO10646(XtDisplay(tw), data->font))
            {
               ...
               XTextExtents16(data->font, ucsstr, ucsstr_len, &dummy, &dummy, &dummy, &overall);
               XFree(ucsstr);
            }
            else
         ...
         else
         {
            if (font->per_char)
            {
               if (c >= font->min_char_or_byte2 && c <= font->max_char_or_byte2)
                  result += font->per_char[c - font->min_char_or_byte2].width;
               else if (font->default_char >= font->min_char_or_byte2 && font->default_char <= font->max_char_or_byte2)
          ...
          int          to)            /* How many bytes in to stop measuring */
{
   ...
   if (tw->text.char_size != 1)
   {
      for (i = from, ptr = block->ptr + from; i < to; i += csize, ptr += csize)
      {
         ...
         {
            XGlyphInfo ext;
            XftTextExtentsUtf8(XtDisplay(tw), ((XftFont *)data->font), (FcChar8 *)ptr, csize, &ext);
            result += ext.xOff;
         }
         else
#endif
   ...
   { /* no need to pay for mblen if we know all chars are 1 byte */
      for (i = from, ptr = block->ptr + from; i < to; i++, ptr++)
      {
         ...
         {
            XGlyphInfo ext;
            XftTextExtentsUtf8(XtDisplay(tw), ((XftFont *)data->font), (FcChar8 *)ptr, 1, &ext);
            result += ext.xOff;
         }
         else
#endif
                      ...
                      int          to)            /* How many bytes in to stop measuring */
{
   ...
   if (tw->text.char_size != 1)
   {
      for (i = from, ptr = block->ptr + from; i < to; i += csize, ptr += csize)
      {
         ...
         {
            result += (data->tabheight - ((y + result - data->topmargin) % data->tabheight));
         }
         else
         {
        ...
        Position     y)
{
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      ...
      if (start == PASTENDPOS)
         return (*tw->text.source->Scan)(tw->text.source, 0, XmSELECT_ALL, XmsdRight, 1, False);
      _XmTextLineInfo(tw, line + 1, &end, &extra);
      end    = (*tw->text.source->Scan)(tw->text.source, end, XmSELECT_POSITION, XmsdLeft, 1, True);
      height = lastheight = data->topmargin;
      if (start >= end && !delta) return start;
      /* if original y was negative, we need to find new laststart */
      if (delta && start > 0)
      {
         end   = (*tw->text.source->Scan)(tw->text.source, start, XmSELECT_POSITION, XmsdLeft, 1, True);
         start = _XmTextFindScroll(tw, start, delta);
      }
      do
      {
         laststart = start;
         start     = (*tw->text.source->ReadSource)(tw->text.source, start, end, &block);
         length    = block.length;
         if ((int)tw->text.char_size > 1)
         {
            for (i = num_chars = 0, num_bytes = mblen(block.ptr, (int)tw->text.char_size);
                 i < length && height < y && num_bytes >= 0;
                 i += num_bytes, num_chars++, num_bytes = mblen(&block.ptr[i], (int)tw->text.char_size))
            {
               lastheight = height;
   ...
   return (*tw->text.source->Scan)(tw->text.source, laststart, XmSELECT_POSITION, (i < 0) ? XmsdLeft : XmsdRight, abs(i), True);
}
        ...
        Position      *y)
{
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      ...
      local_x = tw->text.inner_widget->core.width - (data->rightmargin + line * data->linewidth + data->linewidth * 0.5);
                   ...
                   LineTableExtra *extra)
{
   ...
   int            num_bytes = 0;
   lastChar                 = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_LINE, XmsdRight, 1, False);
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      ...
      {
         nextLeft  = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_WHITESPACE, XmsdRight, 1, False);
         nextBreak = (*tw->text.source->Scan)(tw->text.source, nextLeft, XmSELECT_WHITESPACE, XmsdRight, 1, True);
         while (position < nextLeft)
         {
            ...
            if (y > goalheight)
            {
               if (lastBreak > startpos)
               {
                  ...
                  {
                     oldpos   = position;
                     position = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_POSITION, XmsdLeft, 1, True);
                     (void)(*tw->text.source->ReadSource)(tw->text.source, position, oldpos, &block);
   ...
   else
   {
      ...
      while (position < lastChar)
      {
         nextLeft  = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_WHITESPACE, XmsdRight, 1, False);
         nextBreak = (*tw->text.source->Scan)(tw->text.source, nextLeft, XmSELECT_WHITESPACE, XmsdRight, 1, True);
         while ((position < nextLeft) || (nextBreak == lastChar && position < nextBreak))
         {
   ...
   lineEnd = (*tw->text.source->Scan)(tw->text.source, lastChar, XmSELECT_LINE, XmsdRight, 1, True);
   if (lineEnd != lastChar)
      return lineEnd;
   else
      return PASTENDPOS;
          ...
          Dimension    height)
{
   ...
   if (result == XtGeometryAlmost)
   {
      ...
      {
         result = XtGeometryNo;
         if (((request.request_mode & CWWidth) && reply.width != origwidth) || ((request.request_mode & CWHeight) && reply.height != origheight))
   ...
   if (result == XtGeometryYes)
   {
      ...
      if (((request.request_mode & CWWidth) && tw->text.inner_widget->core.width != width) || ((request.request_mode & CWHeight) && tw->text.inner_widget->core.height != height) || ((request.request_mode == (CWWidth & CWHeight)) && (tw->text.inner_widget->core.width == origwidth && tw->text.inner_widget->core.height == origheight)))
         result = XtGeometryNo;
   }
   return result;
}
...
_XmRedisplayHBar(XmTextWidget tw)
{
   ...
   if (!(data->scrollhorizontal && XmIsScrolledWindow(XtParent(tw))) || data->suspend_hoffset || tw->text.disable_depth != 0 || tw->core.being_destroyed || data->hbar == NULL)
      return;
   ...
   if (nav_trait)
   {
      ...
      return;
   if ((maximum != data->scrollwidth || value != data->hoffset || sliderSize != new_sliderSize) && !(sliderSize == maximum && new_sliderSize == data->scrollwidth))
   {
...
_XmRedisplayVBar(XmTextWidget tw)
{
   ...
   XmNavigatorDataRec nav_data;
   XmNavigatorTrait   nav_trait;
   if (!(data->scrollvertical && XmIsScrolledWindow(XtParent(tw))) || data->suspend_voffset || tw->text.disable_depth != 0 || tw->core.being_destroyed || data->vbar == NULL)
      return;
   ...
   if (nav_trait)
   {
      ...
      return;
   if ((maximum != data->scrollheight || value != data->voffset || sliderSize != new_sliderSize) && !(sliderSize == maximum && new_sliderSize == data->scrollheight))
   {
      data->ignorehbar          = True;
           ...
           XmTextPosition end)
{
   ...
   {
      while (t_index <= max_index && line_table[t_index].start_pos < (unsigned int)start) t_index++;
   }
   else
   {
      /* look backward to find the current record */
      while (t_index && line_table[t_index].start_pos > (unsigned int)start) t_index--;
   }
                 ...
                 Dimension   *widthRtn)
{
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      ...
      LineTableExtra extra;
      newwidth = (int)(tw->text.total_lines * data->linewidth) + data->leftmargin + data->rightmargin;
      _XmTextLineInfo(tw, (LineNum)0, &start, &extra);
      if (start > 0)
      {
         first_position = (*tw->text.source->Scan)(tw->text.source, start, XmSELECT_ALL, XmsdLeft, 1, True);
   ...
   else
   {
      if (data->resizeheight && tw->text.total_lines > data->number_lines)
      {
              ...
              i++)
         {
            ...
            while (linestart < position)
            {
               linestart   = (*tw->text.source->ReadSource)(tw->text.source, linestart, position, &block);
                  ...
                  Dimension     *heightRtn)
{
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      if (data->resizeheight && tw->text.total_lines > data->number_lines)
      {
              ...
              i++)
         {
            ...
            while (linestart < pos)
            {
               linestart    = (*tw->text.source->ReadSource)(tw->text.source, linestart, pos, &block);
   ...
   {
      *heightRtn = tw->text.total_lines * data->lineheight + data->topmargin + data->bottommargin;
      _XmTextLineInfo(tw, (LineNum)0, &start, &extra);
      if (start > 0)
      {
         first_position = (*tw->text.source->Scan)(tw->text.source, start, XmSELECT_ALL, XmsdLeft, 1, True);
                ...
                XmTextPosition position)
{
   ...
   {
      if (data->scrollvertical && XmIsScrolledWindow(XtParent(tw)) && !tw->text.vsbar_scrolling)
         _XmChangeVSB(tw);
   }
   if (tw->text.in_resize || tw->text.in_expose)
   {
                           ...
                           XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
      {
         if (data->scrollvertical && XmIsScrolledWindow(XtParent(tw)))
         {
            ...
            if (newheight != data->scrollheight && !data->suspend_voffset)
            {
               if (newheight)
      ...
      else
      {
         if (data->scrollhorizontal && XmIsScrolledWindow(XtParent(tw)))
         {
            ...
            if (newwidth != data->scrollwidth && !data->suspend_hoffset)
            {
               if (newwidth)
   ...
   else
   {
      ...
      {
         if (data->resizewidth || (data->scrollhorizontal && XmIsScrolledWindow(XtParent(tw))))
         {
            TextFindNewWidth(tw, &newwidth);
            if (data->scrollhorizontal && XmIsScrolledWindow(XtParent(tw)))
            {
         ...
         if (data->resizeheight
             && !(data->scrollvertical && XmIsScrolledWindow(XtParent((Widget)tw))))
         {
            TextFindNewHeight(tw, position, &newheight);
            if (newheight < data->minheight) newheight = data->minheight;
         }
      }
      if ((newwidth != tw->text.inner_widget->core.width || newheight != tw->text.inner_widget->core.height))
      {
...
_XmTextOutputGetSecResData(XmSecondaryResourceData *secResDataRtn)
{
   ...
   _XmTransformSubResources(output_resources, XtNumber(output_resources), &(secResData->resources), &(secResData->num_resources));
            ...
            LineTableExtra *extra)
{
   ...
   {
      if (data->resizewidth || data->resizeheight || ((data->scrollvertical || data->scrollhorizontal) && XmIsScrolledWindow(XtParent(tw))))
      {
         ...
         else
         {
            ...
            *nextpos      = (*tw->text.source->Scan)(tw->text.source,
            ...
            if (XmDirectionMatch(XmPrim_layout_direction(tw),
                                 XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
            {
               if (extra && (data->resizeheight || (data->scrollvertical && XmIsScrolledWindow(XtParent(tw)))))
               {
                  (*extra) = (LineTableExtra)
                     XtMalloc((unsigned)sizeof(LineTableExtraRec));
                  ...
                  while (temp < last_position)
                  {
                     ...
                     height += FindHeight(tw, (Position)width, &block, 0, block.length);
                  }
            ...
            {
               if (extra && (data->resizewidth || (data->scrollhorizontal && XmIsScrolledWindow(XtParent(tw)))))
               {
                  (*extra) = (LineTableExtra)
                     XtMalloc((unsigned)sizeof(LineTableExtraRec));
                  (*extra)->wrappedbychar = False;
   ...
   else
   {
      ...
      else
      {
         if (font->per_char)
         {
            ...
            {
               overall->lbearing = font->per_char[font->default_char - font->min_char_or_byte2].lbearing;
               overall->rbearing = font->per_char[font->default_char - font->min_char_or_byte2].rbearing;
               overall->width    = font->per_char[font->default_char - font->min_char_or_byte2].width;
            }
            else
            {
               overall->lbearing = font->min_bounds.lbearing;
     ...
     XmHighlightMode highlight)
{
   ...
   int             num_bytes = 0;
   int             text_border;
   int             rightedge = (((int)tw->text.inner_widget->core.width) - data->rightmargin) + data->hoffset;
   ...
   int             bottomedge   = (((int)tw->text.inner_widget->core.height) - data->bottommargin) + data->voffset;
   int             newy         = 0;
   ...
   {
      nextlinestart = (*tw->text.source->Scan)(tw->text.source, 0, XmSELECT_ALL, XmsdRight, 1, False);
   ...
   else
   {
      ...
      if (cleartoend && (!extra || !extra->wrappedbychar))
         end = (*tw->text.source->Scan)(tw->text.source, nextlinestart, XmSELECT_POSITION, XmsdLeft, 1, True);
   }
   if (XmDirectionMatch(XmPrim_layout_direction(tw),
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      y = data->topmargin;
      x = tw->text.inner_widget->core.width - (data->rightmargin + line * data->linewidth + (int)(data->linewidth * 0.5));
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      ...
      while (start < end && y <= bottomedge)
      {
         ...
         while (block.length > 0)
         {
            ...
            {
               newy = y;
               while (block.length > 0 && num_bytes == 1 && newy - data->voffset < data->topmargin)
               {
               ...
               if (block.length <= 0 || num_bytes != 1 || block.ptr[0] != '\t') break;
               ...
               if (((y - data->voffset) + height) > (int)(tw->text.inner_widget->core.height - data->bottommargin))
                  rec_height = (tw->text.inner_widget->core.height - data->bottommargin) - (y - data->voffset);
               else
                  rec_height = height;
               if (x - (int)(data->linewidth * 0.5) < data->leftmargin)
                  rec_width = (tw->text.inner_widget->core.width - data->rightmargin) - x;
               else
                  rec_width = data->linewidth;
               XFillRectangle(XtDisplay(tw),
                              XtWindow(tw->text.inner_widget),
                              data->gc,
                              x - (data->linewidth * 0.5),
                              y - data->voffset,
                              rec_width,
                              rec_height);
               SetMarginGC(tw, data->gc);
            ...
            if (newy > bottomedge)
            {
               ...
               else
               {
                  for (i = 0, num_bytes = mblen(block.ptr, (int)tw->text.char_size);
                       i < length && (newy - data->voffset) <= bottomedge && num_bytes > 0;
                       i += num_bytes, num_bytes = mblen(&block.ptr[i], (int)tw->text.char_size))
            ...
            if (highlight == XmHIGHLIGHT_SELECTED)
            {
               ...
               SetNormGC(tw, data->gc, False, False);
               SetFullGC(tw, data->gc);
               if (((y - data->voffset) + (newy - y)) > (int)tw->text.inner_widget->core.height - data->bottommargin)
                  rec_height = tw->text.inner_widget->core.height - (y - data->voffset) - data->bottommargin;
               else
                  rec_height = newy - y;
               if (x + (int)(data->linewidth * 0.5) < data->leftmargin)
                  rec_width = x + (tw->text.inner_widget->core.width - data->rightmargin);
               else
                  rec_width = data->linewidth;
               XFillRectangle(XtDisplay(tw),
                              XtWindow(tw->text.inner_widget),
                              data->gc,
                              x - (int)(data->linewidth * 0.5),
                              y - data->voffset,
                              rec_width,
                              rec_height);
               SetInvGC(tw, data->gc);
               ...
               {
                  _XmXftDrawString2(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, (XftFont *)data->font, 1, x, y - data->voffset, block.ptr, length);
#endif
               }
               else
               {
                       ...
                       len -= csize, p += csize)
                  {
                     ...
                     orig_x  = wx - (int)((overall.rbearing - overall.lbearing) >> 1) - overall.lbearing;
                     orig_y += overall.ascent;
                     if (_XmIsISO10646(XtDisplay(tw), data->font))
                     {
                        size_t   ucsstr_len = 0;
                        XChar2b *ucsstr     = _XmUtf8ToUcs2(p, csize, &ucsstr_len);
                        XDrawString16(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, orig_x, orig_y, ucsstr, ucsstr_len);
                        XFree(ucsstr);
                     }
                     else
            ...
            else
            {
               ...
               else if (data->use_xft)
               {
                  if (stipple)
                  {
                     /*Draw shadow for insensitive text*/
                     SetShadowGC(tw, data->gc);
                     _XmXftDrawString2(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, (XftFont *)data->font, 1, x - data->hoffset + 1, y - data->voffset + 1, block.ptr, length);
                     SetNormGC(tw, data->gc, True, stipple);
                  }
                  _XmXftDrawString2(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, (XftFont *)data->font, 1, x - data->hoffset, y - data->voffset, block.ptr, length);
#endif
               }
               else
               {
                       ...
                       len -= csize, p += csize)
                  {
                     ...
                     if (_XmIsISO10646(XtDisplay(tw), data->font))
                     {
                        ...
                        if (stipple)
                        {
                           /*Draw shadow for insensitive text*/
                           SetShadowGC(tw, data->gc);
                           XDrawString16(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, orig_x + 1, orig_y + 1, ucsstr, ucsstr_len);
                           SetNormGC(tw, data->gc, True, stipple);
                        }
                        XDrawString16(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, orig_x, orig_y, ucsstr, ucsstr_len);
                        XFree(ucsstr);
            ...
            if (highlight == XmHIGHLIGHT_SECONDARY_SELECTED)
               XDrawLine(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, x + (int)(data->linewidth * 0.5) - 1, y - data->voffset, x + (int)(data->linewidth * 0.5) - 1, (newy - data->voffset) - 1);
            y             = newy;
            block.length -= length;
            block.ptr    += length;
            if ((int)tw->text.char_size != 1)
   ...
   else
   {
      ...
      while (start < end && x <= rightedge)
      {
         ...
         while (block.length > 0)
         {
            ...
            {
               newx = x;
               while (block.length > 0 && num_bytes == 1 && newx - data->hoffset < data->leftmargin)
               {
               ...
               if (highlight == XmHIGHLIGHT_SELECTED)
                  ...
                  SetInvGC(tw, data->gc);
               SetFullGC(tw, data->gc);
               if ((int)((x - data->hoffset) + width) > (int)(tw->text.inner_widget->core.width - data->rightmargin))
                  rec_width = (tw->text.inner_widget->core.width - data->rightmargin) - (x - data->hoffset);
               else
                  rec_width = width;
               if ((int)(y + data->font_descent) > (int)(tw->text.inner_widget->core.height - data->bottommargin))
                  rec_height = (tw->text.inner_widget->core.height - data->bottommargin) - y;
               else
            ...
            if (highlight == XmHIGHLIGHT_SELECTED)
            {
               ...
               SetNormGC(tw, data->gc, False, False);
               SetFullGC(tw, data->gc);
               if ((int)((x - data->hoffset) + (newx - x)) > (int)(tw->text.inner_widget->core.width - data->rightmargin))
                  rec_width = tw->text.inner_widget->core.width - (x - data->hoffset) - data->rightmargin;
               else
                  rec_width = newx - x;
               if ((int)(y + data->font_descent) > (int)(tw->text.inner_widget->core.height - data->bottommargin))
                  rec_height = (tw->text.inner_widget->core.height - data->bottommargin) - (y - data->font_ascent);
               else
                  rec_height = data->font_ascent + data->font_descent;
               XFillRectangle(XtDisplay(tw),
                              XtWindow(tw->text.inner_widget),
               ...
               {
                  _XmXftDrawString2(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, (XftFont *)data->font, 1, x - data->hoffset, y, block.ptr, length);
#endif
               }
               else
               {
                  if (_XmIsISO10646(XtDisplay(tw), data->font))
                  {
                     ...
                     XDrawString16(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, x - data->hoffset, y, ucsstr, ucsstr_len);
                     XFree(ucsstr);
                  }
                  else
                     XDrawString(XtDisplay(tw),
            ...
            else
            {
               ...
               else if (data->use_xft)
               {
                  if (stipple)
                  {
                     /*Draw shadow for insensitive text*/
                     SetShadowGC(tw, data->gc);
                     _XmXftDrawString2(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, (XftFont *)data->font, 1, x - data->hoffset + 1, y + 1, block.ptr, length);
                     SetNormGC(tw, data->gc, True, stipple);
                  }
                  _XmXftDrawString2(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, (XftFont *)data->font, 1, x - data->hoffset, y, block.ptr, length);
#endif
               ...
               else
               {
                  if (_XmIsISO10646(XtDisplay(tw), data->font))
                  {
                     ...
                     if (stipple)
                     {
                        /*Draw shadow for insensitive text*/
                        SetShadowGC(tw, data->gc);
                        XDrawString16(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, x - data->hoffset + 1, y + 1, ucsstr, ucsstr_len);
                        SetNormGC(tw, data->gc, True, stipple);
                     }
                     XDrawString16(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, x - data->hoffset, y, ucsstr, ucsstr_len);
                     XFree(ucsstr);
            ...
            if (highlight == XmHIGHLIGHT_SECONDARY_SELECTED)
               XDrawLine(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->gc, x - data->hoffset, y, (newx - data->hoffset) - 1, y);
            x             = newx;
            block.length -= length;
            block.ptr    += length;
            if ((int)tw->text.char_size != 1)
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      ...
      if (data->topmargin - text_border > 0 && x < rightedge + text_border)
         XClearArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), x - (int)(data->linewidth * 0.5) - 1, text_border, rightedge - x + (int)(data->linewidth * 0.5), data->topmargin - text_border, FALSE);
      if (cleartoend)
      {
         y -= data->voffset;
         ...
         if (y < data->topmargin)
            y = data->topmargin;
         height = ((int)tw->text.inner_widget->core.height) - y - data->bottommargin;
      ...
      if (cleartobottom)
      {
         ...
         height = tw->text.inner_widget->core.height - (data->topmargin + data->bottommargin);
         if (width > 0 && height > 0)
            XClearArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), data->leftmargin, y, width, height, False);
      }
   }
   else
   {
      ...
      if (data->leftmargin - text_border > 0 && (int)(y + data->font_descent) > 0)
         XClearArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), text_border, text_border, data->leftmargin - text_border, y + data->font_descent - text_border, False);
      if (cleartoend)
      {
         x -= data->hoffset;
         ...
         if (x < data->leftmargin)
            x = data->leftmargin;
         width = ((int)tw->text.inner_widget->core.width) - x - data->rightmargin;
      ...
      {
         x      = data->leftmargin;
         width  = tw->text.inner_widget->core.width - (data->rightmargin + data->leftmargin);
         height = tw->text.inner_widget->core.height - ((y + data->font_descent) + data->bottommargin);
         if (width > 0 && height > 0)
            XClearArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), x, y + data->font_descent, width, height, False);
      }
   }
...
PaintCursor(XmTextWidget tw)
{
   ...
   * the I Beam. */
   if ((tw->text.top_character <= tw->text.cursor_position) && (tw->text.cursor_position <= tw->text.bottom_position))
   {
      ...
      { /* get area under IBeam first */
         ...
         XCopyArea(XtDisplay((Widget)tw), XtWindow((Widget)tw), data->ibeam_off, data->save_gc, x, y, data->cursorwidth, data->cursorheight, 0, 0);
    ...
    */
      ...
      {
         if (x + data->cursorwidth > tw->text.inner_widget->core.width - tw->primitive.shadow_thickness - tw->primitive.highlight_thickness)
            cursor_width = (tw->text.inner_widget->core.width - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness)) - x;
         if (cursor_width > 0 && cursor_height > 0)
         {
            if (!XtIsSensitive((Widget)tw))
            {
               SetShadowGC(tw, data->imagegc);
               XFillRectangle(XtDisplay((Widget)tw), XtWindow((Widget)tw), data->imagegc, x + 1, y + 1, (unsigned int)cursor_width, (unsigned int)cursor_height);
            }
            _XmTextToggleCursorGC((Widget)tw);
            XFillRectangle(XtDisplay((Widget)tw), XtWindow((Widget)tw), data->imagegc, x, y, (unsigned int)cursor_width, (unsigned int)cursor_height);
         }
      }
      else
      {
         ...
         {
            cursor_width = data->cursorwidth - (tw->primitive.highlight_thickness + tw->primitive.shadow_thickness - x);
            src_x        = data->cursorwidth - cursor_width;
            x            = tw->primitive.highlight_thickness + tw->primitive.shadow_thickness;
         }
         if (y + data->cursorheight > tw->text.inner_widget->core.height - (tw->primitive.highlight_thickness + tw->primitive.shadow_thickness))
         {
            cursor_height = data->cursorheight - ((y + data->cursorheight) - (tw->text.inner_widget->core.height - (tw->primitive.highlight_thickness + tw->primitive.shadow_thickness)));
         }
         if (cursor_width > 0 && cursor_height > 0)
         {
            XCopyArea(XtDisplay((Widget)tw), data->ibeam_off, XtWindow((Widget)tw), data->save_gc, src_x, 0, (unsigned int)cursor_width, (unsigned int)cursor_height, x, y);
         }
      }
   }
}
              ...
              Boolean      redisplay_hbar)
{
   ...
   if ((data->scrollhorizontal && XmIsScrolledWindow(XtParent(tw))) && data->scrollwidth - innerwidth < newhoffset)
   ...
   if (XtIsRealized((Widget)tw))
   {
      ...
      if (delta < 0)
      {
         ...
         {
            XCopyArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), XtWindow(tw->text.inner_widget), data->gc, data->leftmargin, data->topmargin, width, innerheight, data->leftmargin - delta, data->topmargin);
            /* clear left margin + delta change */
            if ((int)(data->leftmargin - delta - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness)) < innerwidth)
               XClearArea(XtDisplay(tw), XtWindow(tw), tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, data->topmargin, data->leftmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness) - delta, innerheight, False);
            /* clear right margin */
            if ((int)(data->rightmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness)) > 0)
               XClearArea(XtDisplay(tw), XtWindow(tw), data->leftmargin + innerwidth, data->topmargin, data->rightmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), innerheight, False);
            data->exposehscroll++;
         }
      ...
      else
      {
         ...
         {
            XCopyArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), XtWindow(tw->text.inner_widget), data->gc, data->leftmargin + delta, data->topmargin, innerwidth - delta, innerheight, data->leftmargin, data->topmargin);
            /* clear right margin + delta change */
            XClearArea(XtDisplay(tw), XtWindow(tw), data->leftmargin + innerwidth - delta, data->topmargin, delta + data->rightmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), innerheight, False);
            /* clear left margin */
            if (data->leftmargin - (int)(tw->primitive.shadow_thickness + tw->primitive.highlight_thickness)
                > 0)
               XClearArea(XtDisplay(tw), XtWindow(tw), tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, data->topmargin, data->leftmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), innerheight, False);
            data->exposehscroll++;
         }
         else
         {
            /* clear all text */
            XClearArea(XtDisplay(tw), XtWindow(tw), tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, data->topmargin, width - 2 * (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), innerheight, False);
            data->exposehscroll++;
         }
              ...
              Boolean      redisplay_vbar)
{
   ...
   if ((data->scrollvertical && XmIsScrolledWindow(XtParent(tw))) && data->scrollheight - innerheight < newvoffset)
   ...
   if (XtIsRealized((Widget)tw))
   {
      ...
      if (delta < 0)
      {
         ...
         {
            XCopyArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), XtWindow(tw->text.inner_widget), data->gc, data->leftmargin, data->topmargin, innerwidth, height, data->leftmargin, data->topmargin - delta);
            /* clear top margin + delta change */
            if ((int)(data->topmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness) - delta) < innerheight)
               XClearArea(XtDisplay(tw), XtWindow(tw), data->leftmargin, tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, innerwidth, data->topmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness) - delta, False);
            /* clear right margin */
            if ((int)data->topmargin - (int)(tw->primitive.shadow_thickness + tw->primitive.highlight_thickness) > 0)
               XClearArea(XtDisplay(tw), XtWindow(tw), data->leftmargin, data->topmargin + innerheight, innerwidth, data->bottommargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), False);
            data->exposevscroll++;
         }
      ...
      else
      {
         ...
         {
            XCopyArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), XtWindow(tw->text.inner_widget), data->gc, data->leftmargin, data->topmargin + delta, innerwidth, innerheight - delta, data->leftmargin, data->topmargin);
            /* clear bottom margin + delta change */
            XClearArea(XtDisplay(tw), XtWindow(tw), data->leftmargin, data->topmargin + innerheight - delta, innerwidth, delta + data->bottommargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), False);
            /* clear top margin */
            if (data->topmargin - (int)(tw->primitive.shadow_thickness + tw->primitive.highlight_thickness)
                > 0)
               XClearArea(XtDisplay(tw), XtWindow(tw), data->leftmargin, tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, innerwidth, data->topmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), False);
            data->exposevscroll++;
         }
         else
         {
            /* clear all text */
            XClearArea(XtDisplay(tw), XtWindow(tw), data->leftmargin, tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, innerwidth, height - 2 * (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), False);
            data->exposevscroll++;
         }
                           ...
                           XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
      {
         if (y <= data->topmargin)
         {
            ...
            else
            {
               ...
               if (position == tw->text.bottom_position || (line_num < tw->text.total_lines && position == tw->text.line_table[line_num + 1].start_pos - 1))
         ...
         else if (y - data->voffset > ((Position)(tw->text.inner_widget->core.height - data->bottommargin)))
         {
            ChangeVOffset(tw, (int)(y) - (int)(tw->text.inner_widget->core.height) + (int)(data->bottommargin), True);
         }
      }
      else
      {
...
{
   OutputData data = tw->text.output->data;
   if ((data->cursor_on >= 0) && data->blinkstate == CurrentCursorState(tw) && XtIsRealized((Widget)tw))
   {
      /* Toggle blink state */
          ...
          LineNum      destline)
{
   ...
   if (XmDirectionMatch(XmPrim_layout_direction(tw),
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      XCopyArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), XtWindow(tw->text.inner_widget), data->gc, tw->text.inner_widget->core.width - data->rightmargin - (Position)data->linewidth * (toline + 1), tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, (Position)data->linewidth * (toline - fromline + 1), tw->text.inner_widget->core.height - 2 * (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), tw->text.inner_widget->core.width - data->rightmargin - (Position)data->linewidth * (destline + toline - fromline + 1), tw->primitive.shadow_thickness + tw->primitive.highlight_thickness);
   }
   else
   {
      XCopyArea(XtDisplay(tw), XtWindow(tw->text.inner_widget), XtWindow(tw->text.inner_widget), data->gc, tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, (Position)data->lineheight * fromline + data->topmargin, tw->text.inner_widget->core.width - 2 * (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness), data->lineheight * (toline - fromline + 1), tw->primitive.shadow_thickness + tw->primitive.highlight_thickness, (Position)data->lineheight * destline + data->topmargin);
   }
   SetMarginGC(tw, data->gc);
   if (XmDirectionMatch(XmPrim_layout_direction(tw),
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
                 ...
                 Dimension   *height)
{
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      ...
      *height = (Dimension)((data->rows_set * (data->font_ascent + data->font_descent)) + data->topmargin + data->bottommargin);
   ...
   {
      *width = (Dimension)((data->columns_set * data->averagecharwidth) + data->leftmargin + data->rightmargin);
        ...
        Pixel        foreground)
{
   ...
   */
   ...
   data->save_gc = XtAllocateGC((Widget)tw, tw->core.depth, valueMask, &values, dynamicMask, unusedMask);
   /*
   ...
   */
   ...
   data->gc     = XtAllocateGC((Widget)tw, tw->core.depth, valueMask, &values, dynamicMask, 0);
   ...
   data->imagegc = XtAllocateGC((Widget)tw, tw->core.depth, valueMask, &values, dynamicMask, 0);
}
                 ...
                 Dimension    height)
{
   ...
   * be painted */
   data->ibeam_off         = XCreatePixmap(dpy, RootWindowOfScreen(screen), width, height, tw->core.depth);
   data->refresh_ibeam_off = True;
}
                 ...
                 int          line_width)
{
   ...
   if (data->cursor == XmUNSPECIFIED_PIXMAP)
   {
      ...
      data->cursor      = XCreatePixmap(dpy, XtWindow(tw), data->cursorwidth, data->cursorheight, 1);
      values.foreground = 0;
      values.line_width = 0;
      values.fill_style = FillSolid;
      values.function   = GXcopy;
      ...
      XFillRectangle(dpy, data->cursor, data->cursor_gc, 0, 0, data->cursorwidth, data->cursorheight);
      ...
      (void)_XmCachePixmap(data->cursor, screen, pixmap_name, 1, 0, 1, data->cursorwidth, data->cursorheight);
   }
   valueMask = (GCForeground | GCBackground | GCStipple | GCFillStyle);
   if (tw->text.input->data->overstrike)
   {
      values.background = values.foreground = tw->core.background_pixel ^ tw->primitive.foreground;
   }
   else
   {
      values.foreground = tw->primitive.foreground;
                  ...
                  int          line_width)
{
   ...
   if (data->add_mode_cursor == XmUNSPECIFIED_PIXMAP)
   {
      ...
      data->add_mode_cursor = XCreatePixmap(dpy, XtWindow((Widget)tw), data->cursorwidth, data->cursorheight, 1);
      values.function       = GXcopy;
      valueMask             = GCFunction;
      XChangeGC(dpy, data->cursor_gc, valueMask, &values);
      XCopyArea(dpy, data->cursor, data->add_mode_cursor, data->cursor_gc, 0, 0, data->cursorwidth, data->cursorheight, 0, 0);
      valueMask          = (GCTile | GCFillStyle | GCForeground | GCBackground | GCFunction | GCTileStipXOrigin);
      ...
      XFillRectangle(dpy, data->add_mode_cursor, data->cursor_gc, 0, 0, data->cursorwidth, data->cursorheight);
      /* Install the pixmap for pixmap caching */
      _XmCachePixmap(data->add_mode_cursor, screen, pixmap_name, 1, 0, 1, data->cursorwidth, data->cursorheight);
   }
}
...
MakeCursors(XmTextWidget tw)
{
   ...
   if (data->cursor == XmUNSPECIFIED_PIXMAP || data->add_mode_cursor == XmUNSPECIFIED_PIXMAP || data->ibeam_off == XmUNSPECIFIED_PIXMAP || oldheight != data->cursorheight || oldwidth != data->cursorwidth)
   {
      ...
      MakeIBeamOffArea(tw, MAX((int)data->cursorwidth, (int)data->cursorheight >> 1), data->cursorheight);
                ...
                Cardinal num_args)
{
   ...
   XtGetSubvalues((XtPointer)tw->text.output->data, output_resources, XtNumber(output_resources), args, num_args);
}
...
#define CP(fld) (data->fld = newdata->fld)
   ...
   Cardinal      n = 0;
   *newdata        = *data;
   XtSetSubvalues((XtPointer)newdata, output_resources, XtNumber(output_resources), args, *num_args);
   ...
   if (data->fontlist != oldtw->text.output->data->fontlist || newtw->core.background_pixel != oldtw->core.background_pixel || newtw->primitive.foreground != oldtw->primitive.foreground)
   {
      XtSetArg(im_args[n], XmNbackground, newtw->core.background_pixel);
      n++;
      XtSetArg(im_args[n], XmNforeground, newtw->primitive.foreground);
      n++;
      XtSetArg(im_args[n], XmNfontList, data->fontlist);
      n++;
   }
   ...
   /*  what is called margin, in this code, is composed of margin, shadow, and
      ...
      code works.
      */
   if (newtw->text.margin_width != oldtw->text.margin_width || newtw->text.margin_height != oldtw->text.margin_height || newtw->primitive.shadow_thickness != oldtw->primitive.shadow_thickness || newtw->primitive.highlight_thickness != oldtw->primitive.highlight_thickness)
   {
      data->leftmargin = data->rightmargin = newtw->text.margin_width + newtw->primitive.shadow_thickness + newtw->primitive.highlight_thickness;
      data->topmargin = data->bottommargin = newtw->text.margin_height + newtw->primitive.shadow_thickness + newtw->primitive.highlight_thickness;
      o_redisplay                          = True;
      newsize                              = True;
   }
   if (CK(wordwrap))
   {
                           ...
                           XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
      {
         ...
         if (data->vbar)
         {
            if (newdata->wordwrap)
            {
               ...
               nav_data.page_increment.y = 0;
               nav_data.dimMask          = NavigDimensionY;
               nav_data.valueMask        = NavValue | NavMinimum | NavMaximum | NavSliderSize;
               _XmSFUpdateNavigatorsValue(XtParent(new_w), &nav_data, True);
               data->ignorevbar = False;
               data->voffset    = 0;
            }
      ...
      if (!oldtw->text.output->data->wordwrap)
      {
         ...
         {
            newtw->text.top_character = (*newtw->text.source->Scan)(newtw->text.source, newtw->text.top_character, XmSELECT_LINE, XmsdLeft, 1, False);
      ...
      if (CK(columns) || CK(rows) || newsize)
      {
                              ...
                              XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
         {
            ...
            if (new_height == oldtw->core.height)
            {
               ...
               data->minheight    = (Dimension)((data->rows_set * (data->font_ascent + data->font_descent)) + data->topmargin + data->bottommargin);
            }
            else
            {
         ...
         else
         {
            ...
            {
               newtw->core.width = width;
               data->minwidth    = (Dimension)((data->columns_set * data->averagecharwidth) + data->leftmargin + data->rightmargin);
            }
            else
            {
   ...
   XtSetArg(im_args[n], XmNbackgroundPixmap, newtw->core.background_pixmap);
   n++;
   XtSetArg(im_args[n], XmNspotLocation, &xmim_point);
   n++;
   XtSetArg(im_args[n], XmNarea, &xmim_area);
   n++;
   XtSetArg(im_args[n], XmNlineSpace, newdata->lineheight);
   n++;
              ...
              Boolean o_create)
{
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      ...
      if (tw->text.top_character)
      {
         ...
         if (tw->text.total_lines >= data->number_lines && (tw->text.total_lines - tw->text.top_line) < data->number_lines)
   ...
   {
      data->number_lines = tw->text.inner_widget->core.height - data->topmargin - data->bottommargin;
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      if (tw->text.edit_mode == XmSINGLE_LINE_EDIT)
      {
         ...
         position        = (*tw->text.source->Scan)(tw->text.source, linestart, XmSELECT_LINE, XmsdRight, 1, False);
         while (linestart < position)
         {
            linestart    = (*tw->text.source->ReadSource)(tw->text.source,
                                                       linestart,
                  ...
                  Boolean  *cont)
{
   ...
   switch (event->type)
   {
      ...
      case FocusOut:
         ...
         break;
      case EnterNotify:
         if ((_XmGetFocusPolicy(w) != XmEXPLICIT) && !(newhasfocus) && event->xcrossing.focus && (event->xcrossing.detail != NotifyInferior))
         {
            cb.reason = XmCR_FOCUS;
            cb.event  = event;
            XtCallCallbackList(w, tw->text.focus_callback, (Opaque)&cb);
             ...
             int          height)
{
                        ...
                        XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
   {
      for (i = x; i < (int)(x + width + data->linewidth); i += data->linewidth)
      {
         ...
         first = (*tw->text.source->Scan)(tw->text.source, first, XmSELECT_POSITION, XmsdLeft, 1, True);
         last  = (*tw->text.source->Scan)(tw->text.source, last, XmSELECT_POSITION, XmsdRight, 1, True);
             ...
             Region  region)
{
   ...
   if (xe->x == 0 && xe->y == 0 && xe->width == tw->core.width && xe->height == tw->core.height)
   ...
   _XmTextInvalidate(tw, (XmTextPosition)tw->text.top_character, (XmTextPosition)tw->text.top_character, NODELTA);
                    ...
                    Cardinal num_args)
{
   ...
   XtGetSubresources(wid, (XtPointer)data, NULL, NULL, output_resources, XtNumber(output_resources), args, num_args);
   ...
   */
   ...
   if (!LoadFontMetrics(tw))
   {
      ...
      data->fontlist = XmRenderTableCopy(
         XmeGetDefaultRenderTable(wid, XmTEXT_FONTLIST),
         NULL,
         0);
   ...
   data->leftmargin = data->rightmargin = tw->text.margin_width + tw->primitive.shadow_thickness + tw->primitive.highlight_thickness;
   data->topmargin = data->bottommargin = tw->text.margin_height + tw->primitive.shadow_thickness + tw->primitive.highlight_thickness;
   /* Don't word wrap, have multiple row or have vertical scrollbars
     if editMode is single_line */
   if (tw->text.edit_mode == XmSINGLE_LINE_EDIT)
   {
   ...
   */
   ...
   if (scrollFrameTrait != NULL && !(scrollFrameTrait->getInfo(wid->core.parent, NULL, NULL, NULL)))
   {
      int n;
      Arg arglist[30];
      Arg swarglist[1];
      ...
      if (data->scrollhorizontal)
      {
         ...
         n++;
         XtSetArg(arglist[n], XmNshadowThickness, tw->primitive.shadow_thickness);
         n++;
         XtSetArg(arglist[n], XmNorientation, XmHORIZONTAL);
         n++;
         XtSetArg(arglist[n], XmNtraversalOn, False);
         n++;
                              ...
                              XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
         {
            ...
            XtAddEventHandler(data->hbar, (EventMask)ButtonReleaseMask, False, HandleHBarButtonRelease, (Opaque)tw);
         }
         else
            XtManageChild(data->hbar);
      }
      ...
      if (data->scrollvertical)
      {
         ...
         else
         {
            if (tw->text.edit_mode != XmSINGLE_LINE_EDIT)
               XtManageChild(data->vbar);
            XtAddEventHandler(data->vbar, (EventMask)ButtonReleaseMask, False, HandleVBarButtonRelease, (Opaque)tw);
         }
      }
      else
         data->vbar = NULL;
   ...
   else
   {
      data->vbar = NULL;
      data->hbar = NULL;
      if (XmIsScrolledWindow(XtParent(tw)) && (((XmScrolledWindowWidget)tw->core.parent)->swindow.VisualPolicy == XmCONSTANT))
      {
                   ...
                   XmBaselineMargins *margins_rec)
{
   ...
   {
      data->topmargin = margins_rec->margin_top + tw->primitive.shadow_thickness + tw->primitive.highlight_thickness;
      _XmProcessLock();
      posToXYCachedWidget = NULL;
      _XmProcessUnlock();
   }
   ...
   {
      margins_rec->margin_top    = data->topmargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness);
      margins_rec->margin_bottom = data->bottommargin - (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness);
...
_XmTextToggleCursorGC(Widget w)
{
   ...
   else
   {
      if (i_data->overstrike)
      {
         ...
         if (XtIsSensitive(w) && !tw->text.add_mode && (data->hasfocus || _XmTextHasDestination(w)))
            values.fill_style = FillSolid;
         else
         {
            valueMask        |= GCStipple;
         ...
         values.foreground = values.background = tw->primitive.foreground ^ tw->core.background_pixel;
         values.function                       = GXxor;
      }
      else
      {
         ...
         else if (stipple != data->add_mode_cursor)
         {
