...
static void SetFullGC(XmTextWidget tw, GC gc);
static void GetRect(XmTextWidget tw, XRectangle *rect);
static void SetMarginGC(XmTextWidget tw, GC gc);
static void SetNormGC(XmTextWidget tw, GC gc, Boolean change_stipple, Boolean stipple);
static void SetShadowGC(XmTextWidget tw, GC gc);
static void InvertImageGC(XmTextWidget tw);
static void SetInvGC(XmTextWidget tw, GC gc);
static int _FontStructFindWidth(XmTextWidget tw, int x, XmTextBlock block, int from, int to);
static int FindWidth(XmTextWidget tw, int x, XmTextBlock block, int from, int to);
static XmTextPosition XYToPos(XmTextWidget tw, Position x, Position y);
static Boolean PosToXY(XmTextWidget tw, XmTextPosition position, Position *x, Position *y);
static XtGeometryResult TryResize(XmTextWidget tw, Dimension width, Dimension height);
static int CountLines(XmTextWidget tw, XmTextPosition start, XmTextPosition end);
static void TextFindNewWidth(XmTextWidget tw, Dimension *widthRtn);
static void TextFindNewHeight(XmTextWidget tw, XmTextPosition position, Dimension *heightRtn);
static void CheckForNewSize(XmTextWidget tw, XmTextPosition position);
static XtPointer OutputBaseProc(Widget tw, XtPointer client_data);
...
static void ChangeHOffset(XmTextWidget tw, int newhoffset, Boolean redisplay_hbar);
static void ChangeVOffset(XmTextWidget tw, int newvoffset, Boolean redisplay_vbar);
static void DrawInsertionPoint(XmTextWidget tw, XmTextPosition position, OnOrOff onoroff);
static void MakePositionVisible(XmTextWidget tw, XmTextPosition position);
static void BlinkInsertionPoint(XmTextWidget tw);
static Boolean MoveLines(XmTextWidget tw, LineNum fromline, LineNum toline, LineNum destline);
...
static void MakeIBeamStencil(XmTextWidget tw, int line_width);
static void MakeAddModeCursor(XmTextWidget tw, int line_width);
static void MakeCursors(XmTextWidget tw);
static void OutputGetValues(Widget wid, ArgList args, Cardinal num_args);
...
static void NotifyResized(Widget w, Boolean o_create);
static void HandleTimer(XtPointer closure, XtIntervalId *id);
static void HandleFocusEvents(Widget w, XtPointer closure, XEvent *event, Boolean *cont);
static void HandleGraphicsExposure(Widget w, XtPointer closure, XEvent *event, Boolean *cont);
static void OutputRealize(Widget w, XtValueMask *valueMask, XSetWindowAttributes *attributes);
static void OutputDestroy(Widget w);
static void RedrawRegion(XmTextWidget tw, int x, int y, int width, int height);
static void OutputExpose(Widget w, XEvent *event, Region region);
static void GetPreferredSize(Widget widget, Dimension *width, Dimension *height);
static void HandleVBarButtonRelease(Widget w, XtPointer closure, XEvent *event, Boolean *cont);
static void HandleHBarButtonRelease(Widget w, XtPointer closure, XEvent *event, Boolean *cont);
static int _FontStructFindHeight(XmTextWidget tw, int x, XmTextBlock block, int from, int to);
static int FindHeight(XmTextWidget tw, int x, XmTextBlock block, int from, int to);
static Boolean _FontStructPerCharExtents(XmTextWidget tw,
                                         char *str,
                                         int length,
                                         XCharStruct *overall);
static Boolean SetXOCOrientation(XmTextWidget tw, XOC om, XOrientation orientation);
...
void _XmTextFreeContextData(Widget w, /* unused */
...
static void _XmTextDrawShadow(XmTextWidget tw)
{
  if (XtIsRealized((Widget)tw)) {
    ...
    if (tw->primitive.highlighted) {
      (*(((XmPrimitiveWidgetClass)XtClass((Widget)tw))->primitive_class.border_highlight))(
          (Widget)tw);
    }
    else {
      (*(((XmPrimitiveWidgetClass)XtClass((Widget)tw))->primitive_class.border_unhighlight))(
          (Widget)tw);
    }
  }
}

void _XmTextResetClipOrigin(XmTextWidget tw, XmTextPosition position, Boolean clip_mask_reset)
{
...
static void SetFullGC(XmTextWidget tw, GC gc)
{
  ...
  ClipRect.width = tw->core.width -
                   (2 * (tw->primitive.highlight_thickness + tw->primitive.shadow_thickness));
...
static void SetShadowGC(XmTextWidget tf, GC gc)
{
  unsigned long valueMask = (GCForeground | GCBackground);
  XGCValues values;
...
#if USE_XFT
static int _XftFindHeight(XmTextWidget tw,
                          int y, /* Starting position (needed for tabs) */
                          XmTextBlock block,
                          int from, /* How many bytes in to start measuring */
                          int to)   /* How many bytes in to stop measuring */
...
static XmTextPosition XYToPos(XmTextWidget tw, Position x, Position y)
{
  ...
  return (*tw->text.source->Scan)(
      tw->text.source, laststart, XmSELECT_POSITION, (i < 0) ? XmsdLeft : XmsdRight, abs(i), True);
}

/*****************************************************************************
 * To make TextOut a true "Object" this function should be a class function. *
 *****************************************************************************/
Boolean _XmTextShouldWordWrap(XmTextWidget tw)
{
  OutputData data = tw->text.output->data;
...
static Boolean PosToXY(XmTextWidget tw, XmTextPosition position, Position *x, Position *y)
{
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    ...
    local_x = tw->text.inner_widget->core.width -
              (data->rightmargin + line * data->linewidth + data->linewidth * 0.5);
 ...
 * To make TextOut a true "Object" this function should be a class function. *
 *****************************************************************************/
XmTextPosition _XmTextFindLineEnd(XmTextWidget tw, XmTextPosition position, LineTableExtra *extra)
{
  OutputData data = tw->text.output->data;
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    ...
    while (position < lastChar) {
      nextLeft = (*tw->text.source->Scan)(
          tw->text.source, position, XmSELECT_WHITESPACE, XmsdRight, 1, False);
      nextBreak = (*tw->text.source->Scan)(
          tw->text.source, nextLeft, XmSELECT_WHITESPACE, XmsdRight, 1, True);
      while (position < nextLeft) {
        position = (*tw->text.source->ReadSource)(tw->text.source, position, nextLeft, &block);
        length = block.length;
        ...
        if (y > goalheight) {
          if (lastBreak > startpos) {
            ...
            if (extra) {
              *extra = (LineTableExtra)XtMalloc((unsigned)sizeof(LineTableExtraRec));
      ...
      while (position < nextBreak) {
        position = (*tw->text.source->ReadSource)(tw->text.source, position, nextBreak, &block);
        length = block.length;
        y += FindHeight(tw, y, &block, 0, block.length);
      }
      lastBreak = nextBreak;
...
static XtGeometryResult TryResize(XmTextWidget tw, Dimension width, Dimension height)
{
  ...
  if (result == XtGeometryYes) {
    ...
    if (((request.request_mode & CWWidth) && tw->text.inner_widget->core.width != width) ||
        ((request.request_mode & CWHeight) && tw->text.inner_widget->core.height != height) ||
...
void _XmRedisplayHBar(XmTextWidget tw)
{
  OutputData data = tw->text.output->data;
  int value, sliderSize, maximum, new_sliderSize;
  XmNavigatorDataRec nav_data;
  XmNavigatorTrait nav_trait;
  if (!(data->scrollhorizontal && XmIsScrolledWindow(XtParent(tw))) || data->suspend_hoffset ||
      tw->text.disable_depth != 0 || tw->core.being_destroyed || data->hbar == NULL)
    return;
  ...
  if (nav_trait) {
    ...
    return;
  if ((maximum != data->scrollwidth || value != data->hoffset || sliderSize != new_sliderSize) &&
...
void _XmRedisplayVBar(XmTextWidget tw)
{
  OutputData data = tw->text.output->data;
  int value = 0, sliderSize = 0, maximum = 0, new_sliderSize = 0;
  XmNavigatorDataRec nav_data;
  XmNavigatorTrait nav_trait;
  if (!(data->scrollvertical && XmIsScrolledWindow(XtParent(tw))) || data->suspend_voffset ||
      tw->text.disable_depth != 0 || tw->core.being_destroyed || data->vbar == NULL)
    return;
  ChangeVOffset(tw, data->voffset, False); /* Makes sure that voffset is
                                                  still reasonable. */
  new_sliderSize = tw->text.inner_widget->core.height - (data->topmargin + data->bottommargin);
  if (new_sliderSize < 1)
    new_sliderSize = 1;
  ...
  if (nav_trait) {
    ...
    return;
  if ((maximum != data->scrollheight || value != data->voffset || sliderSize != new_sliderSize) &&
...
static int CountLines(XmTextWidget tw, XmTextPosition start, XmTextPosition end)
{
  ...
  if (line_table[t_index].start_pos < (unsigned int)start) {
    while (t_index <= max_index && line_table[t_index].start_pos < (unsigned int)start)
      t_index++;
  }
  else {
    /* look backward to find the current record */
    while (t_index && line_table[t_index].start_pos > (unsigned int)start)
      t_index--;
  }
...
void _XmChangeVSB(XmTextWidget tw)
{
  OutputData data = tw->text.output->data;
  int local_total;
  int new_size;
...
void _XmChangeHSB(XmTextWidget tw)
{
  OutputData data = tw->text.output->data;
  int local_total = 0;
  int new_size = 0;
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    ...
    if (start > 0) {
      first_position = (*tw->text.source->Scan)(
          tw->text.source, start, XmSELECT_ALL, XmsdLeft, 1, True);
    ...
    if (data->resizeheight && tw->text.total_lines > data->number_lines) {
      ...
      for (linestart = tw->text.top_character; i + 1 < tw->text.total_lines; i++) {
...
static XtPointer OutputBaseProc(Widget widget, XtPointer client_data)
{
...
void _XmTextOutputGetSecResData(XmSecondaryResourceData *secResDataRtn)
{
  ...
  _XmTransformSubResources(output_resources,
 ...
 * To make TextOut a true "Object" this function should be a class function. *
 *****************************************************************************/
int _XmTextGetNumberLines(XmTextWidget tw)
{
  OutputData data = tw->text.output->data;
 ...
 */
void _XmTextMovingCursorPosition(XmTextWidget tw, XmTextPosition position)
{
  OutputData data = tw->text.output->data;
  ...
  if (nextpos) {
    if (position == PASTENDPOS) {
      *nextpos = last_position = PASTENDPOS;
      ...
      if (ShouldWordWrap(data, tw)) {
        ...
        *nextpos = (*tw->text.source->Scan)(
        ...
        if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
          ...
          if (extra &&
              (data->resizewidth || (data->scrollhorizontal && XmIsScrolledWindow(XtParent(tw)))))
          {
            (*extra) = (LineTableExtra)XtMalloc((unsigned)sizeof(LineTableExtraRec));
            (*extra)->wrappedbychar = False;
      ...
      if (*nextpos == position)
        *nextpos = (*tw->text.source->Scan)(
            tw->text.source, position, XmSELECT_POSITION, XmsdRight, 1, True);
    }
  }
  return (True);
}
  ...
  if (tw->text.char_size != 1) {
    ...
    if (c == '\t') {
      ...
      if (font->per_char) {
        ...
        {
          overall->lbearing =
              font->per_char[font->default_char - font->min_char_or_byte2].lbearing;
          overall->rbearing =
              font->per_char[font->default_char - font->min_char_or_byte2].rbearing;
                 ...
                 XmHighlightMode highlight)
{
  ...
  int bottomedge = (((int)tw->text.inner_widget->core.height) - data->bottommargin) +
                   data->voffset;
  int newy = 0;
  ...
  if (linestart == PASTENDPOS) {
    ...
    nextlinestart = (*tw->text.source->Scan)(
        tw->text.source, 0, XmSELECT_ALL, XmsdRight, 1, False);
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    ...
    while (linestart < start && x <= rightedge) {
      linestart = (*tw->text.source->ReadSource)(tw->text.source, linestart, start, &block);
      y += FindHeight(tw, y, &block, 0, block.length);
    }
  }
  else {
    ...
    while (start < end && y <= bottomedge) {
      ...
      while (block.length > 0) {
        while (num_bytes == 1 && block.ptr[0] == '\t') {
          ...
          if (block.length <= 0 || num_bytes != 1 || block.ptr[0] != '\t')
            break;
          ...
          if (((y - data->voffset) + height) >
            ...
            rec_height = (tw->text.inner_widget->core.height - data->bottommargin) -
                         (y - data->voffset);
          else
            rec_height = height;
        ...
        if (highlight == XmHIGHLIGHT_SELECTED) {
          ...
          if (((y - data->voffset) + (newy - y)) >
              (int)tw->text.inner_widget->core.height - data->bottommargin)
            rec_height = tw->text.inner_widget->core.height - (y - data->voffset) -
                         data->bottommargin;
          else
            rec_height = newy - y;
          if (x + (int)(data->linewidth * 0.5) < data->leftmargin)
          ...
          if (data->use_fontset) {
            ...
            _XmXftDrawString2(XtDisplay(tw),
                              XtWindow(tw->text.inner_widget),
                              data->gc,
                              (XftFont *)data->font,
                              1,
            ...
            for (len = length, p = block.ptr; len > 0 && p; len -= csize, p += csize) {
      ...
      while (block.length > 0) {
        while (num_bytes == 1 && block.ptr[0] == '\t') {
          ...
          if ((int)((x - data->hoffset) + width) >
            ...
            rec_width = (tw->text.inner_widget->core.width - data->rightmargin) -
                        (x - data->hoffset);
          else
            rec_width = width;
        ...
        if (highlight == XmHIGHLIGHT_SELECTED) {
          ...
          if ((int)((x - data->hoffset) + (newx - x)) >
              (int)(tw->text.inner_widget->core.width - data->rightmargin))
            rec_width = tw->text.inner_widget->core.width - (x - data->hoffset) -
                        data->rightmargin;
          else
            rec_width = newx - x;
          if ((int)(y + data->font_descent) >
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    ...
    if (data->topmargin - text_border > 0 && x < rightedge + text_border)
      XClearArea(XtDisplay(tw),
                 XtWindow(tw->text.inner_widget),
                 x - (int)(data->linewidth * 0.5) - 1,
                 text_border,
                 rightedge - x + (int)(data->linewidth * 0.5),
                 data->topmargin - text_border,
                 FALSE);
    if (cleartoend) {
      y -= data->voffset;
    ...
    if (data->leftmargin - text_border > 0 && (int)(y + data->font_descent) > 0)
      XClearArea(XtDisplay(tw),
                 ...
                 y + data->font_descent - text_border,
                 False);
    if (cleartoend) {
      x -= data->hoffset;
    ...
    if ((data->cursor_on >= 0) && (data->blinkstate == on)) {
                                      ...
                                      tw->primitive.highlight_thickness)
      {
        ...
        cursor_width = data->cursorwidth -
                       (tw->primitive.highlight_thickness + tw->primitive.shadow_thickness - x);
        src_x = data->cursorwidth - cursor_width;
        x = tw->primitive.highlight_thickness + tw->primitive.shadow_thickness;
      }
      if (y + data->cursorheight >
          tw->text.inner_widget->core.height -
              (tw->primitive.highlight_thickness + tw->primitive.shadow_thickness))
      {
    ...
    if (delta < 0) {
      if (width > 0 && innerheight > 0) {
        ...
        if ((int)(data->rightmargin -
                  (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness)) > 0)
          XClearArea(XtDisplay(tw),
                     XtWindow(tw),
                     data->leftmargin + innerwidth,
                     data->topmargin,
                     data->rightmargin -
                         (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness),
                     innerheight,
      ...
      if (innerwidth - delta > 0 && innerheight > 0) {
        ...
        XClearArea(XtDisplay(tw),
                   XtWindow(tw),
                   data->leftmargin + innerwidth - delta,
                   data->topmargin,
        ...
        /* clear left margin */
        if (data->leftmargin -
                (int)(tw->primitive.shadow_thickness + tw->primitive.highlight_thickness) >
            0)
          XClearArea(XtDisplay(tw),
                     XtWindow(tw),
                     tw->primitive.shadow_thickness + tw->primitive.highlight_thickness,
        ...
        XClearArea(XtDisplay(tw),
                   ...
                   data->topmargin,
                   width -
                       2 * (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness),
                   innerheight,
                   False);
        data->exposehscroll++;
      }
    ...
    if (delta < 0) {
      if (height > 0 && innerwidth > 0) {
        XCopyArea(XtDisplay(tw),
                  ...
                  height,
                  data->leftmargin,
                  data->topmargin - delta);
        /* clear top margin + delta change */
        if ((int)(data->topmargin -
                  (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness) - delta) <
            innerheight)
          XClearArea(XtDisplay(tw),
                     XtWindow(tw),
                     data->leftmargin,
    ...
    if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
      ...
      if (y - data->voffset < data->topmargin) {
        ChangeVOffset(tw, y - data->topmargin, True);
               ...
               ((Position)(tw->text.inner_widget->core.height - data->bottommargin)))
      {
        ChangeVOffset(tw,
                      (int)(y) - (int)(tw->text.inner_widget->core.height) +
      ...
      if (x - data->hoffset < data->leftmargin) {
        ChangeHOffset(tw, x - data->leftmargin, True);
               ...
               ((Position)(tw->text.inner_widget->core.width - data->rightmargin)))
      {
        ChangeHOffset(tw,
                      (int)(x) - (int)(tw->text.inner_widget->core.width) +
...
{
  OutputData data = tw->text.output->data;
  if ((data->cursor_on >= 0) && data->blinkstate == CurrentCursorState(tw) &&
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    XCopyArea(XtDisplay(tw),
              ...
              tw->text.inner_widget->core.height -
                  2 * (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness),
              tw->text.inner_widget->core.width - data->rightmargin -
                  (Position)data->linewidth * (destline + toline - fromline + 1),
              tw->primitive.shadow_thickness + tw->primitive.highlight_thickness);
  }
  else {
    XCopyArea(XtDisplay(tw),
...
static void SizeFromRowsCols(XmTextWidget tw, Dimension *width, Dimension *height)
{
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    ...
    *width = (Dimension)((data->columns_set * data->averagecharwidth) + data->leftmargin +
                         data->rightmargin);
...
static void LoadGCs(XmTextWidget tw, Pixel background, Pixel foreground)
{
  ...
  data->save_gc = XtAllocateGC(
      (Widget)tw, tw->core.depth, valueMask, &values, dynamicMask, unusedMask);
  /*
...
static void MakeIBeamStencil(XmTextWidget tw, int line_width)
{
  ...
  if (data->cursor == XmUNSPECIFIED_PIXMAP) {
    ...
    XFillRectangle(
        dpy, data->cursor, data->cursor_gc, 0, 0, data->cursorwidth, data->cursorheight);
    ...
    (void)_XmCachePixmap(
        data->cursor, screen, pixmap_name, 1, 0, 1, data->cursorwidth, data->cursorheight);
  }
...
static void MakeAddModeCursor(XmTextWidget tw, int line_width)
{
  ...
  if (data->add_mode_cursor == XmUNSPECIFIED_PIXMAP) {
    ...
    data->add_mode_cursor = XCreatePixmap(
        dpy, XtWindow((Widget)tw), data->cursorwidth, data->cursorheight, 1);
    values.function = GXcopy;
    valueMask = GCFunction;
    XChangeGC(dpy, data->cursor_gc, valueMask, &values);
    XCopyArea(dpy,
              data->cursor,
              data->add_mode_cursor,
              data->cursor_gc,
              0,
              0,
              data->cursorwidth,
    ...
    XFillRectangle(
        dpy, data->add_mode_cursor, data->cursor_gc, 0, 0, data->cursorwidth, data->cursorheight);
    /* Install the pixmap for pixmap caching */
    _XmCachePixmap(data->add_mode_cursor,
                   screen,
                   pixmap_name,
                   1,
                   0,
...
static void MakeCursors(XmTextWidget tw)
{
  ...
  if (data->cursor == XmUNSPECIFIED_PIXMAP || data->add_mode_cursor == XmUNSPECIFIED_PIXMAP ||
      data->ibeam_off == XmUNSPECIFIED_PIXMAP || oldheight != data->cursorheight ||
      oldwidth != data->cursorwidth)
  {
    ...
    MakeIBeamOffArea(
        tw, MAX((int)data->cursorwidth, (int)data->cursorheight >> 1), data->cursorheight);
...
static void OutputGetValues(Widget w, ArgList args, Cardinal num_args)
{
  XmTextWidget tw = (XmTextWidget)w;
  RefigureDependentInfo(tw);
  XtGetSubvalues((XtPointer)tw->text.output->data,
                 output_resources,
                 XtNumber(output_resources),
                 args,
                 num_args);
...
#define CP(fld) (data->fld = newdata->fld)
  ...
  *newdata = *data;
  XtSetSubvalues(
      (XtPointer)newdata, output_resources, XtNumber(output_resources), args, *num_args);
    ...
    if (!oldtw->text.output->data->wordwrap) {
      if (data->resizeheight)
        ...
        newtw->text.top_character = (*newtw->text.source->Scan)(
            newtw->text.source, newtw->text.top_character, XmSELECT_LINE, XmsdLeft, 1, False);
...
static void NotifyResized(Widget w, Boolean o_create)
{
  ...
  if (data->vbar) {
    ...
    if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
      ...
      if (new_size < 1)
        new_size = 1;
      if (new_size > data->scrollheight)
        new_size = data->scrollheight;
  ...
  if (data->hbar) {
    ...
    if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
      ...
      if (new_size < 1)
        new_size = 1;
      if (new_size > data->scrollwidth)
        new_size = data->scrollwidth;
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    if (tw->text.edit_mode == XmSINGLE_LINE_EDIT) {
      ...
      position = (*tw->text.source->Scan)(
          tw->text.source, linestart, XmSELECT_LINE, XmsdRight, 1, False);
      while (linestart < position) {
        linestart = (*tw->text.source->ReadSource)(tw->text.source, linestart, position, &block);
        text_height += FindHeight(tw, text_height, &block, 0, block.length);
      }
 ...
 * To make TextOut a True "Object" this function should be a class function. *
 *****************************************************************************/
void _XmTextChangeBlinkBehavior(XmTextWidget tw, Boolean newvalue)
{
  OutputData data = tw->text.output->data;
  if (newvalue) {
...
static void HandleFocusEvents(Widget w, XtPointer closure, XEvent *event, Boolean *cont)
{
  ...
  switch (event->type) {
    ...
    case FocusOut:
      ...
      break;
    case EnterNotify:
      if ((_XmGetFocusPolicy(w) != XmEXPLICIT) && !(newhasfocus) && event->xcrossing.focus &&
...
static void OutputDestroy(Widget w)
{
  ...
  XtRemoveEventHandler(
      (Widget)tw->text.inner_widget, (EventMask)0, True, HandleGraphicsExposure, NULL);
...
static void RedrawRegion(XmTextWidget tw, int x, int y, int width, int height)
{
  ...
  if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
    for (i = x; i < (int)(x + width + data->linewidth); i += data->linewidth) {
      ...
      first = (*tw->text.source->Scan)(
          tw->text.source, first, XmSELECT_POSITION, XmsdLeft, 1, True);
      last = (*tw->text.source->Scan)(
          tw->text.source, last, XmSELECT_POSITION, XmsdRight, 1, True);
...
static void OutputExpose(Widget w, XEvent *event, Region region)
{
  ...
  _XmTextInvalidate(
      tw, (XmTextPosition)tw->text.top_character, (XmTextPosition)tw->text.top_character, NODELTA);
...
static void GetPreferredSize(Widget w, Dimension *width, Dimension *height)
{
  XmTextWidget tw = (XmTextWidget)w;
  OutputData data = tw->text.output->data;
 ...
 *
 ************************************************************************/
static void SliderMove(Widget w, XtPointer closure, XtPointer cd)
{
  /* w is a navigator tw */
  XmTextWidget tw = (XmTextWidget)closure;
  XmNavigatorDataRec nav_data;
...
void _XmTextOutputCreate(Widget wid, ArgList args, Cardinal num_args)
{
  ...
  data->leftmargin = data->rightmargin = tw->text.margin_width + tw->primitive.shadow_thickness +
                                         tw->primitive.highlight_thickness;
  data->topmargin = data->bottommargin = tw->text.margin_height + tw->primitive.shadow_thickness +
                                         tw->primitive.highlight_thickness;
  /* Don't word wrap, have multiple row or have vertical scrollbars
     if editMode is single_line */
  if (tw->text.edit_mode == XmSINGLE_LINE_EDIT) {
    ...
    if (data->scrollhorizontal) {
      ...
      if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
        if (tw->text.edit_mode != XmSINGLE_LINE_EDIT)
          XtManageChild(data->hbar);
        XtAddEventHandler(
            data->hbar, (EventMask)ButtonReleaseMask, False, HandleHBarButtonRelease, (Opaque)tw);
      }
      else
        XtManageChild(data->hbar);
    }
    ...
    if (data->scrollvertical) {
      ...
      if (XmDirectionMatch(XmPrim_layout_direction(tw), XmTOP_TO_BOTTOM_RIGHT_TO_LEFT)) {
        ...
        if (tw->text.edit_mode != XmSINGLE_LINE_EDIT)
          XtManageChild(data->vbar);
        XtAddEventHandler(
            data->vbar, (EventMask)ButtonReleaseMask, False, HandleVBarButtonRelease, (Opaque)tw);
      }
    }
    else
      data->vbar = NULL;
  ...
  XtAddEventHandler((Widget)tw->text.inner_widget,
                    ...
                    (Opaque)NULL);
  XtAddEventHandler(
      (Widget)tw->text.inner_widget, (EventMask)0, True, HandleGraphicsExposure, (Opaque)NULL);
}

/*****************************************************************************
 * To make TextOut a True "Object" this function should be a class function. *
 *****************************************************************************/
Boolean _XmTextGetBaselines(Widget w, Dimension **baselines, int *line_count)
{
  XmTextWidget tw = (XmTextWidget)w;
  OutputData data = tw->text.output->data;
...
void _XmTextMarginsProc(Widget w, XmBaselineMargins *margins_rec)
{
  ...
  if (margins_rec->get_or_set == XmBASELINE_SET) {
    data->topmargin = margins_rec->margin_top + tw->primitive.shadow_thickness +
                      tw->primitive.highlight_thickness;
    _XmProcessLock();
    posToXYCachedWidget = NULL;
    _XmProcessUnlock();
    ...
    margins_rec->margin_top = data->topmargin -
                              (tw->primitive.shadow_thickness + tw->primitive.highlight_thickness);
    margins_rec->margin_bottom = data->bottommargin - (tw->primitive.shadow_thickness +
                                                       tw->primitive.highlight_thickness);
 ...
 * To make TextOut a True "Object" this function should be a class function. *
 *****************************************************************************/
void _XmTextChangeHOffset(XmTextWidget tw, int length)
{
  OutputData data = tw->text.output->data;
  Dimension margin_width = tw->text.margin_width + tw->primitive.shadow_thickness +
                           tw->primitive.highlight_thickness;
 ...
 * To make TextOut a True "Object" this function should be a class function. *
 *****************************************************************************/
void _XmTextChangeVOffset(XmTextWidget tw, int length)
{
  OutputData data = tw->text.output->data;
  Dimension margin_height = tw->text.margin_height + tw->primitive.shadow_thickness +
                            tw->primitive.highlight_thickness;
 ...
 * To make TextOut a true "Object" this function should be a class function. *
 *****************************************************************************/
void _XmTextToggleCursorGC(Widget w)
{
  XmTextWidget tw = (XmTextWidget)w;
  OutputData data = tw->text.output->data;
  ...
  if (!XtIsSensitive((Widget)tw)) {
    ...
    if (i_data->overstrike) {
      if (data->stipple_tile == XmUNSPECIFIED_PIXMAP)
        return;
      values.stipple = data->stipple_tile;
    ...
    if (i_data->overstrike) {
      ...
      if (tw->text.input->data->overstrike) {
        values.background = values.foreground = tw->core.background_pixel ^
