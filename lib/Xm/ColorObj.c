...
externaldef(xmcolorobjclass) WidgetClass
   xmColorObjClass
   = (WidgetClass)&xmColorObjClassRec;
...
DisplayDestroy(Widget wid, XtPointer clientData, XtPointer callData)
{
   ...
   if (_XmColorObjCacheDisplay)
      if (XFindContext(_XmColorObjCacheDisplay, (XID)XtDisplay(wid), context, (XPointer *)&tmpColorObj) == 0)
      {
         if (tmpColorObj)
         {
   ...
   Cardinal *numArgs) /* unused */
{
   ...
   XSaveContext(_XmColorObjCacheDisplay, (XID)new_obj->color_obj.display, _XmColorObjCache, (XPointer)new_obj);
   _XmProcessUnlock();
...
#define MAX_ATOM_LENGTH		24
         ...
         {
            names[i] = ((nscreens <= MAX_SCREENS) ? &buf[i * MAX_ATOM_LENGTH] : XtMalloc(MAX_ATOM_LENGTH));
            ...
            if (!result)
            {
		       ...
		       */
               ...
               if (init_first_time == False)
               {
                  ...
                  savetimeout = XtAppGetSelectionTimeout(
               ...
               while (new_obj->color_obj.done == FALSE)
                  XtAppProcessEvent(XtWidgetToApplicationContext((Widget)new_obj),
                                    XtIMAll);
...
FetchPixelData(Widget w, char *value, int screen)
{
   ...
   for (i = 0; i < XmCO_NUM_COLORS; i++)
   {
      ...
      sscanf(&(value[count]), "%lx_%lx_%lx_%lx_%lx_", &(colors[i].bg), &(colors[i].fg), &(colors[i].ts), &(colors[i].bs), &(colors[i].sc));
      sprintf(tmp, "%lx_%lx_%lx_%lx_%lx_", colors[i].bg, colors[i].fg, colors[i].ts, colors[i].bs, colors[i].sc);
      count += strlen(tmp);
   }
   ...
   int      screen)
{
   ...
   result = XGetWindowProperty(dpy, SelOwner, pixel_set_atom, 0L, 1000000, False, (Atom)AnyPropertyType, &target, &format, &length, &bytesafter, (unsigned char **)&value);
 ...
 */
   ...
   else if (XmCO_DitherTopShadow(tmpColorObj->color_obj.display, screen, &(colors[tmpColorObj->color_obj.primary])))
   {
   ...
   else if (XmCO_DitherBottomShadow(tmpColorObj->color_obj.display, screen, &(colors[tmpColorObj->color_obj.primary])))
   {
   ...
   {
      if (XmCO_DitherTopShadow(tmpColorObj->color_obj.display, screen, &(colors[tmpColorObj->color_obj.primary])))
      {
      ...
      if (XmCO_DitherTopShadow(tmpColorObj->color_obj.display, screen, &(colors[tmpColorObj->color_obj.secondary])))
      {
   ...
   {
      if (XmCO_DitherBottomShadow(tmpColorObj->color_obj.display, screen, &(colors[tmpColorObj->color_obj.primary])))
      {
      ...
      if (XmCO_DitherBottomShadow(tmpColorObj->color_obj.display, screen, &(colors[tmpColorObj->color_obj.secondary])))
      {
                   ...
                   short         *text_id)
{
   ...
   if ((!_XmColorObjCacheDisplay) || (XFindContext(_XmColorObjCacheDisplay, (XID)XDisplayOfScreen(screen), _XmColorObjCache, (XPointer *)&tmpColorObj) != 0))
   {
   ...
   /* return False if color srv is not running, or color obj not used */
   _XmProcessLock();
   if (!tmpColorObj || !tmpColorObj->color_obj.useColorObj || !tmpColorObj->color_obj.colorIsRunning)
   {
      _XmProcessUnlock();
      return False;
   }
...
static Boolean
NotBW(Screen *screen, Pixel pixel)
{
...
static Boolean
DupPixel(Pixel pixel, XColor *colors, int up_to)
{
   int i;
   for (i = 0; i < up_to; i++)
...
static int
trap_XCloseDisplay(Display *disp, XExtCodes *codes)
{
