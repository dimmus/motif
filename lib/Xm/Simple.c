...
#include <stdio.h>
    ...
    static void
    EvaluateConvenienceStructure(Widget wid, XmSimpleMenu sm);
...
static XtResource SimpleMenuResources[] = {
    ...
    {XmNsimpleCallback,
     ...
     XtOffsetOf(struct _XmSimpleMenuRec, callback),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNbuttons,
     XmCButtons,
     ...
     XtOffsetOf(struct _XmSimpleMenuRec, label_string),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNbuttonAccelerators,
     ...
     XmRStringTable,
     sizeof(String *),
     XtOffsetOf(struct _XmSimpleMenuRec, accelerator),
     XmRImmediate,
     (XtPointer)NULL},
    ...
    {XmNbuttonMnemonics,
     XmCButtonMnemonics,
     XmRKeySymTable,
     sizeof(XmKeySymTable),
     XtOffsetOf(struct _XmSimpleMenuRec, mnemonic),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNbuttonMnemonicCharSets,
     XmCButtonMnemonicCharSets,
     XmRCharSetTable,
     sizeof(XmStringCharSetTable),
    ...
    {XmNbuttonType,
     XmCButtonType,
     XmRButtonType,
     sizeof(XmButtonTypeTable),
     XtOffsetOf(struct _XmSimpleMenuRec, button_type),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNbuttonSet,
     XmCButtonSet,
     XmRInt,
     sizeof(int),
     XtOffsetOf(struct _XmSimpleMenuRec, button_set),
     XmRImmediate,
     (XtPointer)-1},
    {XmNoptionLabel,
     XmCOptionLabel,
     XmRXmString,
     sizeof(XmString),
     XtOffsetOf(struct _XmSimpleMenuRec, option_label),
     XmRImmediate,
     (XtPointer)NULL},
    {XmNoptionMnemonic,
     XmCOptionMnemonic,
     XmRKeySym,
     sizeof(KeySym),
     XtOffsetOf(struct _XmSimpleMenuRec, option_mnemonic),
     XmRImmediate,
     (XtPointer)NULL},
};

static void EvaluateConvenienceStructure(Widget wid, XmSimpleMenu sm)
{
  ...
  for (i = 0; i < sm->count; i++) {
    ...
    switch (btype) {
      ...
      case XmCHECKBUTTON:
        ...
        child = XtCreateManagedWidget(name_buf, xmToggleButtonGadgetClass, (Widget)rc, args, n);
        if (sm->callback)
          XtAddCallback(child,
...
Widget XmCreateSimpleRadioBox(Widget parent, String name, ArgList args, Cardinal arg_count)
{
  ...
  for (i = 0; i < mr.count; i++) {
    ...
    child = XtCreateManagedWidget(name_buf, xmToggleButtonGadgetClass, (Widget)rc, local_args, n);
    if (mr.callback)
