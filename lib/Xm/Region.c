...
#define MESSAGE1 _XmMMsgRegion_0000
    typedef void(*XmOverlapProc)(
        XmRegion, XmRegionBox *, XmRegionBox *, XmRegionBox *, XmRegionBox *, short, short);
typedef void (*XmNonOverlapProc)(XmRegion, XmRegionBox *, XmRegionBox *, short, short);
...
static void CreateRightShadow(
    XmRegionBox *here, unsigned long mask, XSegment **segml, int *segmc, int *segmi);
static void CreateTopShadow(Position start_x,
                            Position end_x,
                            XmRegionBox *here,
                            unsigned long mask,
                            XSegment **segml,
                            int *segmc,
                            int *segmi);
static void CreateBottomShadow(Position start_x,
                               Position end_x,
                               XmRegionBox *here,
                               unsigned long mask,
                               XSegment **segml,
                               int *segmc,
                               int *segmi);
static void ShrinkRegion(XmRegion r, XmRegion s, XmRegion t, int dx, int dy);
...
Boolean _XmRegionEqual(register XmRegion r1, register XmRegion r2)
{
   ...
   *  Check the extents.
   */
  else if (r1->extents.x1 != r2->extents.x1 || r1->extents.x2 != r2->extents.x2 ||
           r1->extents.y1 != r2->extents.y1 || r1->extents.y2 != r2->extents.y2)
  {
    return (False);
  }
  /*
   ...
   */
    ...
    for (i = 0; i < r1->numRects; i++) {
      if (r1->rects[i].x1 != r2->rects[i].x1 || r1->rects[i].x2 != r2->rects[i].x2 ||
          r1->rects[i].y1 != r2->rects[i].y1 || r1->rects[i].y2 != r2->rects[i].y2)
      {
        return (False);
      }
    }
 ...
 ***********************************************************************/
void _XmRegionSetGCRegion(Display *dpy, GC gc, int x_origin, int y_origin, XmRegion r)
{
  XRectangle *rects;
  long nrects;
  _XmRegionGetRectangles(r, &rects, &nrects);
  if (rects || !nrects) { /* not if XtMalloc failed */
    XSetClipRectangles(dpy, gc, x_origin, y_origin, rects, (int)nrects, YXBanded);
  }
  XtFree((char *)rects);
}
...
static void miRegionCopy(register XmRegion dstrgn, register XmRegion rgn)
{
  if (dstrgn != rgn) /*  don't want to copy to itself */ {
    ...
    memmove((char *)dstrgn->rects, (char *)rgn->rects, (int)(rgn->numRects * sizeof(XmRegionBox)));
  }
}
  ...
  if (r1 != r1End) {
    if (nonOverlap1Func) {
      do {
        ...
        (*nonOverlap1Func)(newReg, r1, r1BandEnd, MAX(r1->y1, ybot), r1->y2);
        r1 = r1BandEnd;
      } while (r1 != r1End);
    }
  }
    ...
    do {
      ...
      (*nonOverlap2Func)(newReg, r2, r2BandEnd, MAX(r2->y1, ybot), r2->y2);
      r2 = r2BandEnd;
    } while (r2 != r2End);
  }
  if (newReg->numRects != curBand) {
 ...
 ***********************************************************************/
void _XmRegionIntersectRectWithRegion(XRectangle *rect, XmRegion source, XmRegion dest)
{
  XmRegionRec region;
...
void _XmRegionUnion(XmRegion reg1, XmRegion reg2, XmRegion newReg)
{
  ...
  if ((reg1->numRects == 1) && (reg1->extents.x1 <= reg2->extents.x1) &&
      (reg1->extents.y1 <= reg2->extents.y1) && (reg1->extents.x2 >= reg2->extents.x2) &&
  ...
  if ((reg2->numRects == 1) && (reg2->extents.x1 <= reg1->extents.x1) &&
      (reg2->extents.y1 <= reg1->extents.y1) && (reg2->extents.x2 >= reg1->extents.x2) &&
 ...
 *  region.  The new region is overwritten.
 ***********************************************************************/
void _XmRegionUnionRectWithRegion(XRectangle *rect, XmRegion source, XmRegion dest)
{
  XmRegionRec region;
  XmRegionBox box[1];
  /*
...
{
  /* check for trivial reject */
  if (ISEMPTY(regM) || ISEMPTY(regS) || (!EXTENTCHECK(&regM->extents, &regS->extents))) {
  ...
  while ((nrects = workReg->numRects) && shadow_thick) {
    ...
    while (end_here != end_all) {
      ...
      while (here < end_here) {
        ...
        CreateLeftShadow(here, mask, &topSegms, &topSegmCount, &curTopSeg);
        /*
        ...
        while (above < end_above && above->x2 < x2) {
          if (draw) {
            CreateTopShadow(
                start_x, above->x1, here, mask | TR_OPEN, &topSegms, &topSegmCount, &curTopSeg);
          }
          mask &= ~TL_MATCH;
          mask |= TL_OPEN;
          draw = True;
        ...
        if (above < end_above && above->x2 == x2) {
          ...
          if (draw) {
            CreateTopShadow(start_x, end_x, here, mask, &topSegms, &topSegmCount, &curTopSeg);
          }
        }
        /*
        ...
        while (below < end_below && below->x2 < x2) {
          if (draw) {
            CreateBottomShadow(
                start_x, below->x1, here, mask | BR_OPEN, &botSegms, &botSegmCount, &curBotSeg);
          }
          mask &= ~BL_MATCH;
          mask |= BL_OPEN;
          draw = True;
        ...
        if (below < end_below && below->x2 == x2) {
          ...
          if (draw) {
            CreateBottomShadow(start_x, end_x, here, mask, &botSegms, &botSegmCount, &curBotSeg);
          }
        }
        /*
         *  Determine the right shadow.
         */
        CreateRightShadow(here, mask, &botSegms, &botSegmCount, &curBotSeg);
        here++;
      }
    }
