...
#define MESSAGE1	_XmMMsgRegion_0000
   typedef void(*XmOverlapProc)(XmRegion, XmRegionBox *, XmRegionBox *, XmRegionBox *, XmRegionBox *, short, short);
   ...
   register XmRegion r2)
{
   ...
   /*
     ...
     *  Check the extents.
     */
   else if (r1->extents.x1 != r2->extents.x1 || r1->extents.x2 != r2->extents.x2 || r1->extents.y1 != r2->extents.y1 || r1->extents.y2 != r2->extents.y2)
   {
      return (False);
   }
   /*
   ...
   else
   {
      ...
      {
         if (r1->rects[i].x1 != r2->rects[i].x1 || r1->rects[i].x2 != r2->rects[i].x2 || r1->rects[i].y1 != r2->rects[i].y1 || r1->rects[i].y2 != r2->rects[i].y2)
         {
            return (False);
         }
      }
   ...
   XmRegion newReg)
{
   ...
   /*
     ...
     */
   if ((reg1->numRects == 1) && (reg1->extents.x1 <= reg2->extents.x1) && (reg1->extents.y1 <= reg2->extents.y1) && (reg1->extents.x2 >= reg2->extents.x2) && (reg1->extents.y2 >= reg2->extents.y2))
   {
      miRegionCopy(newReg, reg1);
      return;
   }
   /*
     ...
     */
   if ((reg2->numRects == 1) && (reg2->extents.x1 <= reg1->extents.x1) && (reg2->extents.y1 <= reg1->extents.y1) && (reg2->extents.x2 >= reg1->extents.x2) && (reg2->extents.y2 >= reg1->extents.y2))
   {
      miRegionCopy(newReg, reg2);
      return;
   }
	 ...
	 */
      while (end_here != end_all)
      {
	     ...
	     */
         ...
         if ((end_here == end_all) || (end_here->y1 != y))
         { /* gap */
            below = end_below;
         }
         /*
	     ...
	     */
         while (here < end_here)
         {
            ...
            while (above < end_above && above->x2 < x2)
            {
               if (draw)
               {
                  CreateTopShadow(start_x, above->x1, here, mask | TR_OPEN, &topSegms, &topSegmCount, &curTopSeg);
               }
               mask   &= ~TL_MATCH;
               mask   |= TL_OPEN;
               draw    = True;
            ...
            while (below < end_below && below->x2 < x2)
            {
               if (draw)
               {
                  CreateBottomShadow(start_x, below->x1, here, mask | BR_OPEN, &botSegms, &botSegmCount, &curBotSeg);
               }
               mask   &= ~BL_MATCH;
               mask   |= BL_OPEN;
               draw    = True;
