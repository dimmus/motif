...
static void GetUnitFromFont(Display *display, XFontStruct *fst, int *ph_unit, int *pv_unit);
static void Initialize(Widget requested_widget,
                       Widget new_widget,
                       ArgList args,
                       Cardinal *num_args);
static Boolean SetValues(
    Widget current, Widget requested, Widget new_w, ArgList args, Cardinal *num_args);
...
static XtResource resources[] = {
    ...
    },
    {
        ...
        XmRInt,
        sizeof(int),
        Offset(screen.foregroundThreshold),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRWidget,
        sizeof(Widget),
        Offset(screen.defaultNoneCursorIcon),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRWidget,
        sizeof(Widget),
        Offset(screen.defaultValidCursorIcon),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRWidget,
        sizeof(Widget),
        Offset(screen.defaultInvalidCursorIcon),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRWidget,
        sizeof(Widget),
        Offset(screen.defaultMoveCursorIcon),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRWidget,
        sizeof(Widget),
        Offset(screen.defaultLinkCursorIcon),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRWidget,
        sizeof(Widget),
        Offset(screen.defaultCopyCursorIcon),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        XmRWidget,
        sizeof(Widget),
        Offset(screen.defaultSourceCursorIcon),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
    ...
    },
    {
        ...
        Offset(screen.unpostBehavior),
        XmRImmediate,
        (XtPointer)XmUNPOST_AND_REPLAY,
    },
    {
        XmNfont,
        XmCFont,
    ...
    },
    {
        ...
        XmRProc,
        sizeof(XtProc),
        Offset(screen.color_alloc_proc),
        XmRImmediate,
        (XtPointer)NULL,
    },
    {
        ...
        sizeof(XtEnum),
        Offset(screen.bitmap_conversion_model),
        XmRImmediate,
        (XtPointer)XmMATCH_DEPTH,
    },
    {XmNuserData,
     XmCUserData,
     XmRPointer,
     sizeof(XtPointer),
     Offset(screen.user_data),
     XmRPointer,
     (XtPointer)NULL},
    ...
    {XmNdefaultGlyphPixmap,
     ...
     XmRBitmap,
     sizeof(Pixmap),
     Offset(screen.default_glyph_pixmap),
     XmRImmediate,
...
static XmBaseClassExtRec baseClassExtRec = {
    NULL,  NULLQUARK, XmBaseClassExtVersion, sizeof(XmBaseClassExtRec),
...
externaldef(xmscreenclass) WidgetClass xmScreenClass = (WidgetClass)(&xmScreenClassRec);
...
static void GetUnitFromFont(Display *display, XFontStruct *fst, int *ph_unit, int *pv_unit)
{
  ...
  static char *atom_names[] = {XmIAVERAGE_WIDTH, XmIPIXEL_SIZE, XmIRESOLUTION_Y};
  ...
  if (fst) {
    ...
    if (ph_unit) {
      if (atoms[XmAAVERAGE_WIDTH] && XGetFontProperty(fst, atoms[XmAAVERAGE_WIDTH], &avg_w)) {
        ...
        *ph_unit = (int)((fst->min_bounds.width + fst->max_bounds.width) / 2.3) + 0.5;
      }
    }
    /* Vertical units */
    if (pv_unit) {
      if (XGetFontProperty(fst, atoms[XmAPIXEL_SIZE], &pixel_s)) {
        *pv_unit = (int)(((float)pixel_s) / 1.8) + 0.5;
               ...
               (XGetFontProperty(fst, atoms[XmARESOLUTION_Y], &resolution_y)))
      {
        float ps, ry, tmp;
        ps = point_s;
        ry = resolution_y;
        ...
        *pv_unit = (int)((float)(fst->max_bounds.ascent + fst->max_bounds.descent) / 2.2) + 0.5;
      }
    }
  }
  else { /* no X fontstruct */
    if (ph_unit)
      *ph_unit = DEFAULT_QUAD_WIDTH;
    if (pv_unit)
      *pv_unit = DEFAULT_QUAD_WIDTH;
  }
}
                       ...
                       Cardinal *num_args)
{
  ...
  xmScreen->screen.scratchPixmaps = (XtPointer)_XmAllocHashTable(20, MatchPixmap, HashPixmap);
  xmScreen->screen.inUsePixmaps = (XtPointer)_XmAllocHashTable(20, NULL, NULL);
  ...
  if (!XmRepTypeValidValue(XmRID_UNPOST_BEHAVIOR, xmScreen->screen.unpostBehavior, new_widget)) {
  ...
  if (xmScreen->screen.h_unit == RESOURCE_DEFAULT)
    GetUnitFromFont(display, xmScreen->screen.font_struct, &xmScreen->screen.h_unit, NULL);
  if (xmScreen->screen.v_unit == RESOURCE_DEFAULT)
    GetUnitFromFont(display, xmScreen->screen.font_struct, NULL, &xmScreen->screen.v_unit);
  xmScreen->screen.screenInfo = (XtPointer)XtNew(XmScreenInfo);
  ((XmScreenInfo *)(xmScreen->screen.screenInfo))->menu_state = (XtPointer)NULL;
  ((XmScreenInfo *)(xmScreen->screen.screenInfo))->destroyCallbackAdded = False;
#ifdef DEFAULT_GLYPH_PIXMAP
    ...
    Widget current, Widget requested, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (!XmRepTypeValidValue(XmRID_UNPOST_BEHAVIOR, newScr->screen.unpostBehavior, new_w)) {
  ...
  if (newScr->screen.defaultNoneCursorIcon != oldScr->screen.defaultNoneCursorIcon &&
      newScr->screen.defaultNoneCursorIcon != NULL &&
      XtScreenOfObject(XtParent(newScr->screen.defaultNoneCursorIcon)) != XtScreen(newScr))
  {
    XmeWarning((Widget)new_w, MESSAGE1);
    newScr->screen.defaultNoneCursorIcon = oldScr->screen.defaultNoneCursorIcon;
  }
  /* defaultValidCursorIcon */
  if (newScr->screen.defaultValidCursorIcon != oldScr->screen.defaultValidCursorIcon &&
      newScr->screen.defaultValidCursorIcon != NULL &&
      XtScreenOfObject(XtParent(newScr->screen.defaultValidCursorIcon)) != XtScreen(newScr))
  {
    XmeWarning((Widget)new_w, MESSAGE1);
    newScr->screen.defaultValidCursorIcon = oldScr->screen.defaultValidCursorIcon;
  }
  /* defaultInvalidCursorIcon */
  if (newScr->screen.defaultInvalidCursorIcon != oldScr->screen.defaultInvalidCursorIcon &&
      newScr->screen.defaultInvalidCursorIcon != NULL &&
      XtScreenOfObject(XtParent(newScr->screen.defaultInvalidCursorIcon)) != XtScreen(newScr))
  {
    XmeWarning((Widget)new_w, MESSAGE1);
    newScr->screen.defaultInvalidCursorIcon = oldScr->screen.defaultInvalidCursorIcon;
  }
  /* defaultMoveCursorIcon */
  if (newScr->screen.defaultMoveCursorIcon != oldScr->screen.defaultMoveCursorIcon &&
      newScr->screen.defaultMoveCursorIcon != NULL &&
      XtScreenOfObject(XtParent(newScr->screen.defaultMoveCursorIcon)) != XtScreen(newScr))
  {
    XmeWarning((Widget)new_w, MESSAGE1);
    newScr->screen.defaultMoveCursorIcon = oldScr->screen.defaultMoveCursorIcon;
  }
  /* defaultCopyCursorIcon */
  if (newScr->screen.defaultCopyCursorIcon != oldScr->screen.defaultCopyCursorIcon &&
      newScr->screen.defaultCopyCursorIcon != NULL &&
      XtScreenOfObject(XtParent(newScr->screen.defaultCopyCursorIcon)) != XtScreen(newScr))
  {
    XmeWarning((Widget)new_w, MESSAGE1);
    newScr->screen.defaultCopyCursorIcon = oldScr->screen.defaultCopyCursorIcon;
  }
  /* defaultLinkCursorIcon */
  if (newScr->screen.defaultLinkCursorIcon != oldScr->screen.defaultLinkCursorIcon &&
      newScr->screen.defaultLinkCursorIcon != NULL &&
      XtScreenOfObject(XtParent(newScr->screen.defaultLinkCursorIcon)) != XtScreen(newScr))
  {
    XmeWarning((Widget)new_w, MESSAGE1);
    newScr->screen.defaultLinkCursorIcon = oldScr->screen.defaultLinkCursorIcon;
  }
  /* defaultSourceCursorIcon */
  if (newScr->screen.defaultSourceCursorIcon != oldScr->screen.defaultSourceCursorIcon &&
      newScr->screen.defaultSourceCursorIcon != NULL &&
      XtScreenOfObject(XtParent(newScr->screen.defaultSourceCursorIcon)) != XtScreen(newScr))
  {
    XmeWarning((Widget)new_w, MESSAGE1);
    newScr->screen.defaultSourceCursorIcon = oldScr->screen.defaultSourceCursorIcon;
  }
  ...
  if (newScr->screen.font_struct->fid != oldScr->screen.font_struct->fid) {
    if (newScr->screen.h_unit == oldScr->screen.h_unit) {
      GetUnitFromFont(display, newScr->screen.font_struct, &newScr->screen.h_unit, NULL);
    }
    if (newScr->screen.v_unit == oldScr->screen.v_unit) {
      GetUnitFromFont(display, newScr->screen.font_struct, NULL, &newScr->screen.v_unit);
    }
  }
  return FALSE;
}
...
void _XmScreenRemoveFromCursorCache(XmDragIconObject icon)
{
  ...
  while (ptr) {
    next = ptr->next;
    if ((ptr->sourceIcon == icon) || (ptr->stateIcon == icon) || (ptr->opIcon == icon)) {
      if (ptr->cursor)
...
XmDragIconObject _XmScreenGetOperationIcon(Widget w, unsigned char operation)
{
  ...
  if (*ptr == NULL) {
    if (*pt2 == NULL) {
      *pt2 = (XmDragIconObject)XmCreateDragIcon(
          (Widget)xmScreen, XrmQuarkToString(nameQuark), NULL, 0);
    }
    *ptr = *pt2;
  }
  return *ptr;
...
XmDragIconObject _XmScreenGetStateIcon(Widget w, unsigned char state)
{
  ...
  if (icon == NULL) {
     ...
     */
    if (xmScreen->screen.xmStateCursorIcon == NULL) {
      xmScreen->screen.xmStateCursorIcon = (XmDragIconObject)XmCreateDragIcon(
          (Widget)xmScreen, XrmQuarkToString(nameQuark), NULL, 0);
    }
...
XmDragIconObject _XmScreenGetSourceIcon(Widget w)
{
  ...
  if (xmScreen->screen.defaultSourceCursorIcon == NULL) {
    if (xmScreen->screen.xmSourceCursorIcon == NULL) {
      xmScreen->screen.xmSourceCursorIcon = (XmDragIconObject)XmCreateDragIcon(
          (Widget)xmScreen, XrmQuarkToString(_XmDefaultDragIconQuark), NULL, 0);
    }
    xmScreen->screen.defaultSourceCursorIcon = xmScreen->screen.xmSourceCursorIcon;
  }
 ...
 *
 ************************************************************************/
static Boolean MatchPixmap(XmHashKey k1, XmHashKey k2)
{
  XmScratchPixmapKey key1 = (XmScratchPixmapKey)k1;
  XmScratchPixmapKey key2 = (XmScratchPixmapKey)k2;
  return (key1->height == key2->height && key1->width == key2->width &&
          key1->depth == key2->depth);
}

static XmHashValue HashPixmap(XmHashKey k)
{
  XmScratchPixmapKey key = (XmScratchPixmapKey)k;
  return (key->height + key->width + key->depth);
}

Pixmap _XmAllocScratchPixmap(XmScreen xmScreen, Cardinal depth, Dimension width, Dimension height)
{
  ...
  if (scratchPixmap != None) {
    ...
    returned_key = (XmScratchPixmapKey)_XmRemoveHashEntry(scratchTable, &key);
  }
  else {
    ...
    scratchPixmap = XCreatePixmap(
        XtDisplay(xmScreen), RootWindowOfScreen(XtScreen(xmScreen)), width, height, depth);
  }
 ...
 *
 ************************************************************************/
void _XmFreeScratchPixmap(XmScreen xmScreen, Pixmap pixmap)
{
  XmHashTable scratchTable = (XmHashTable)xmScreen->screen.scratchPixmaps;
  XmHashTable inUseTable = (XmHashTable)xmScreen->screen.inUsePixmaps;
  XmScratchPixmapKey returned_key;
  _XmProcessLock();
  returned_key = (XmScratchPixmapKey)_XmGetHashEntry(inUseTable, (XmHashKey)pixmap);
 ...
 *
 ************************************************************************/
XmDragCursorCache *_XmGetDragCursorCachePtr(XmScreen xmScreen)
{
...
Cursor XmeGetNullCursor(Widget w)
{
  ...
  if (xmScreen->screen.nullCursor == None) {
    foreground.pixel = background.pixel = 0;
    pixmap = XCreatePixmapFromBitmapData(XtDisplayOfObject(w),
    ...
    cursor = XCreatePixmapCursor(
        XtDisplayOfObject(w), pixmap, pixmap, &foreground, &background, 0, 0);
...
/* a convenience for RowColumn */
unsigned char _XmGetUnpostBehavior(Widget wid)
{
 ...
 *
 **********************************************************************/
Pixmap _XmGetDefaultGlyphPixmap(Screen *screen, unsigned int *width, unsigned int *height)
{
  XmScreen xmScreen;
  xmScreen = (XmScreen)XmGetXmScreen(screen);
  if (width)
    *width = xmScreen->screen.default_glyph_pixmap_width;
  if (height)
    *height = xmScreen->screen.default_glyph_pixmap_height;
...
Widget XmGetXmScreen(Screen *screen)
{
  ...
  for (scr = ScreenOfDisplay(XtDisplay(xmDisplay), i); i < ScreenCount(XtDisplay(xmDisplay));
  ...
  i++;
  widget = XtCreateWidget(name, xmScreenClass, (Widget)xmDisplay, args, i);
  _XmAppUnlock(app);
  return widget;
}
