...
#define _XmDragCancel(dc) ((*((XmDragContextClass)XtClass(dc))->drag_class.cancel)(dc))
#define _XmDCtoDD(dc) ((XmDisplay)XtParent(dc))
#define _XmDRAG_MASK_BASE (ButtonPressMask | ButtonReleaseMask | ButtonMotionMask)
#ifdef DRAG_USE_MOTION_HINTS
#  define _XmDRAG_GRAB_MASK (_XmDRAG_MASK_BASE PointerMotionHintMask)
#else
...
enum { XmCR_DROP_SITE_TREE_ADD = _XmNUMBER_DND_CB_REASONS, XmCR_DROP_SITE_TREE_REMOVE };
...
#define XmMakeCanonicalString(s) (XmCanonicalString) XrmQuarkToString(XrmStringToQuark(s))
#define _XmAllocAndCopy(data, len) memcpy((XtPointer)XtMalloc(len), (XtPointer)(data), (len))
