 ...
 */
int
XmTabbedStackListInsert(XmTabbedStackList tab_list, int position, XtValueMask mask, XmTabAttributes attributes)
{
   ...
   /*
     ...
     */
   if (tab_list == NULL || position < XmTAB_LAST_POSITION || (position > 0 && position >= tab_list->used)) return XmTAB_NOT_FOUND;
   /*
 ...
 */
int
XmTabbedStackListAppend(XmTabbedStackList tab_list, XtValueMask mask, XmTabAttributes attributes)
{
   /*
 ...
 */
void
XmTabbedStackListModify(XmTabbedStackList tab_list, int position, XtValueMask mask, XmTabAttributes attributes)
{
   XmTabAttributes tab;
   /*
   ...
   /*
     ...
     * change.
     */
   if (mask & XmTAB_VALUE_MODE && tab->value_mode != attributes->value_mode && attributes->value_mode == XmTAB_VALUE_COPY)
   {
      /*
...
XmTabbedStackListCompare(XmTabbedStackList list1, XmTabbedStackList list2)
{
   ...
   int         i;
   if ((list1 == NULL && list2 != NULL) || (list1 != NULL && list2 == NULL) || list1->used != list2->used) return (XmTAB_CMP_SIZE);
   for (i = 0; i < list1->used; ++i)
   {
      if (list1->tabs[i].label_pixmap != list2->tabs[i].label_pixmap || !XiXmStringCompare(list1->tabs[i].label_string, list2->tabs[i].label_string) || list1->tabs[i].pixmap_placement != list2->tabs[i].pixmap_placement)
      {
         return (XmTAB_CMP_SIZE);
      }
      if (list1->tabs[i].string_direction != list2->tabs[i].string_direction || list1->tabs[i].label_alignment != list2->tabs[i].label_alignment || list1->tabs[i].foreground != list2->tabs[i].foreground || list1->tabs[i].background != list2->tabs[i].background || list1->tabs[i].sensitive != list2->tabs[i].sensitive || list1->tabs[i].background_pixmap != list2->tabs[i].background_pixmap)
      {
