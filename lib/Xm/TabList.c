...
#define XiXmStringCompare(s1, s2) \
  (((s1) == (s2)) ? True : ((s1) != NULL && (s2) != NULL) ? XmStringCompare(s1, s2) : False)
#define XiXmStringFree(s) \
  if ((s) != (XmString)NULL) \
    XmStringFree(s); \
  else \
    (s) = NULL
...
XmTabbedStackList XmTabbedStackListCopy(XmTabbedStackList tab_list)
{
  ...
  if (newList->used == 0) {
    ...
    for (i = 0; i < newList->used; ++i) {
      ...
      newList->tabs[i].string_direction = tab_list->tabs[i].string_direction;
      newList->tabs[i].pixmap_placement = tab_list->tabs[i].pixmap_placement;
      newList->tabs[i].label_alignment = tab_list->tabs[i].label_alignment;
      newList->tabs[i].foreground = tab_list->tabs[i].foreground;
      newList->tabs[i].background = tab_list->tabs[i].background;
      newList->tabs[i].background_pixmap = tab_list->tabs[i].background_pixmap;
...
void XmTabbedStackListFree(XmTabbedStackList tab_list)
{
  ...
  if (tab_list->used != 0) {
    for (i = 0; i < tab_list->used; ++i) {
      if (tab_list->tabs[i].value_mode == XmTAB_VALUE_SHARE)
        continue;
...
void XmTabbedStackListRemove(XmTabbedStackList tab_list, int position)
{
  ...
  int count;
  if (tab_list == NULL || position < 0 || (position > 0 && position >= tab_list->used)) {
    return;
  }
  ...
  if (position != tab_list->used) {
    ...
    memmove((XtPointer)to, (XtPointer)from, sizeof(XmTabAttributeRec) * count);
  }
}
 ...
 */
int XmTabbedStackListInsert(XmTabbedStackList tab_list,
                            int position,
                            XtValueMask mask,
 ...
 */
int XmTabbedStackListAppend(XmTabbedStackList tab_list,
                            XtValueMask mask,
                            XmTabAttributes attributes)
{
  /*
 ...
 *	None.
 */
void XmTabbedStackListModify(XmTabbedStackList tab_list,
                             int position,
                             XtValueMask mask,
                             XmTabAttributes attributes)
{
 ...
 *	None.
 */
void XmTabbedStackListQuery(XmTabbedStackList tab_list, int position, XmTabAttributes attributes)
{
  XmTabAttributes tab;
  /*
 ...
 */
int XmTabbedStackListFind(XmTabbedStackList tab_list, XmString label_string)
{
  int i;
  if (tab_list == NULL)
    return (XmTAB_NOT_FOUND);
 ...
 *	None.
 */
void XmTabbedStackListSimpleRemove(XmTabbedStackList tab_list, XmString label_string)
{
  int position = XmTabbedStackListFind(tab_list, label_string);
  if (position != XmTAB_NOT_FOUND)
    XmTabbedStackListRemove(tab_list, position);
}
 ...
 *	int - the position of the new tab
 */
int XmTabbedStackListSimpleInsert(XmTabbedStackList tab_list, int position, XmString label_string)
{
  XmTabAttributeRec attributes;
  attributes.label_string = label_string;
  return (XmTabbedStackListInsert(tab_list, position, XmTAB_LABEL_STRING, &attributes));
}
 ...
 *	int - the position of the new tab
 */
int XmTabbedStackListSimpleAppend(XmTabbedStackList tab_list, XmString label_string)
{
  XmTabAttributeRec attributes;
  attributes.label_string = label_string;
  return (XmTabbedStackListAppend(tab_list, XmTAB_LABEL_STRING, &attributes));
}
 ...
 *	None.
 */
void XmTabAttributesFree(XmTabAttributes attributes)
{
  if (attributes->value_mode == XmTAB_VALUE_COPY) {
 ...
 */
int _XmTabbedStackListCount(XmTabbedStackList tab_list)
{
  if (tab_list == NULL)
    return (0);
 ...
 */
XmTabAttributes _XmTabbedStackListGet(XmTabbedStackList tab_list, int position)
{
  XmTabAttributes tab;
  /*
   * Make sure that we have valid data and if not "asta la vesta, baby"
...
XmTabResult XmTabbedStackListCompare(XmTabbedStackList list1, XmTabbedStackList list2)
{
  ...
  if ((list1 == NULL && list2 != NULL) || (list1 != NULL && list2 == NULL) ||
      list1->used != list2->used)
    return (XmTAB_CMP_SIZE);
  for (i = 0; i < list1->used; ++i) {
    if (list1->tabs[i].label_pixmap != list2->tabs[i].label_pixmap ||
        !XiXmStringCompare(list1->tabs[i].label_string, list2->tabs[i].label_string) ||
        list1->tabs[i].pixmap_placement != list2->tabs[i].pixmap_placement)
    {
      return (XmTAB_CMP_SIZE);
    }
    if (list1->tabs[i].string_direction != list2->tabs[i].string_direction ||
        ...
        list1->tabs[i].background_pixmap != list2->tabs[i].background_pixmap)
    {
