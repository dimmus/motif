...
static void SetTargetsTable(Display *display, xmTargetsTable targetsTable);
static xmAtomsTable GetAtomsTable(Display *display);
static void SetAtomsTable(Display *display, xmAtomsTable atomsTable);
static Window ReadMotifWindow(Display *display);
static Window CreateMotifWindow(Display *display);
static void WriteMotifWindow(Display *display, Window *motifWindow);
...
static Window GetMotifWindow(Display *display)
{
  ...
  _XmProcessUnlock();
  if (XFindContext(display, DefaultRootWindow(display), loc_context, (char **)&motifWindow)) {
    motifWindow = None;
  }
  return (motifWindow);
}
...
static void SetMotifWindow(Display *display, Window motifWindow)
{
   ...
   */
  if (XFindContext(display, DefaultRootWindow(display), loc_context, (char **)&oldMotifWindow)) {
    XSaveContext(display, DefaultRootWindow(display), loc_context, (char *)motifWindow);
  }
  else if (oldMotifWindow != motifWindow) {
    XDeleteContext(display, DefaultRootWindow(display), loc_context);
    XSaveContext(display, DefaultRootWindow(display), loc_context, (char *)motifWindow);
  }
}
...
static xmTargetsTable GetTargetsTable(Display *display)
{
  ...
  _XmProcessUnlock();
  if (XFindContext(display, DefaultRootWindow(display), loc_context, (char **)&targetsTable)) {
    targetsTable = NULL;
  }
  return (targetsTable);
}
...
static void SetTargetsTable(Display *display, xmTargetsTable targetsTable)
{
   ...
   * Delete old context if one exists.
   */
  if (XFindContext(display, DefaultRootWindow(display), loc_context, (char **)&oldTargetsTable)) {
    if (targetsTable)
      XSaveContext(display, DefaultRootWindow(display), loc_context, (char *)targetsTable);
  }
...
static xmAtomsTable GetAtomsTable(Display *display)
{
  ...
  _XmProcessUnlock();
  if (XFindContext(display, DefaultRootWindow(display), loc_context, (XPointer *)&atomsTable)) {
    atomsTable = NULL;
  }
  return (atomsTable);
}
...
static void SetAtomsTable(Display *display, xmAtomsTable atomsTable)
{
   ...
   * Delete old context if one exists.
   */
  if (XFindContext(display, DefaultRootWindow(display), loc_context, (char **)&oldAtomsTable)) {
    if (atomsTable)
      XSaveContext(display, DefaultRootWindow(display), loc_context, (char *)atomsTable);
  }
...
static Boolean ReadAtomsTable(Display *display, xmAtomsTable atomsTable)
{
  ...
  _XmProcessUnlock();
  if (ret) {
    if (propertyRecPtr->info.protocol_version != _MOTIF_DRAG_PROTOCOL_VERSION) {
...
static xmTargetsTable CreateDefaultTargetsTable(Display *display)
{
  ...
  targetsTable->entries = (xmTargetsTableEntry)XtMalloc(sizeof(xmTargetsTableEntryRec) * 2);
...
static xmAtomsTable CreateDefaultAtomsTable(Display *display)
{
  ...
  atomsTable->entries = (xmAtomsTableEntry)XtMalloc(sizeof(xmAtomsTableEntryRec));
  atomsTable->entries[0].atom = XInternAtom(display, XmS_MOTIF_ATOM_0, False);
...
Cardinal _XmTargetsToIndex(Widget shell, Atom *targets, Cardinal numTargets)
{
  ...
  qsort((void *)newTargets, (size_t)numTargets, (size_t)sizeof(Atom), AtomCompare);
  /*
 ...
 *
 ***************************************************************************/
void _XmDestroyMotifWindow(Display *display)
{
  Window motifWindow;
  Atom motifWindowAtom;
  if ((motifWindow = ReadMotifWindow(display)) != None) {
    motifWindowAtom = XInternAtom(display, XmI_MOTIF_DRAG_WINDOW, False);
    XDeleteProperty(display, DefaultRootWindow(display), motifWindowAtom);
