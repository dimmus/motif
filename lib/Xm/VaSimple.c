...
#define MESSAGE3 _XmMMsgVaSimple_0002
    ...
    static XmButtonType
    _XmVaBType_to_XmBType(String symbol);
static void _XmCountNestedList(XtTypedArgList avlist, int *total_count, int *typed_count);
static int _XmTypedArgToArg(Widget widget,
                            XtTypedArgList typed_arg,
                            ArgList arg_return,
                            XtResourceList resources,
                            Cardinal num_resources);
static int _XmNestedArgtoArg(Widget widget,
                             XtTypedArgList avlist,
                             ArgList args,
                             XtResourceList resources,
                             Cardinal num_resources);
static int _XmNestedArgtoTypedArg(XtTypedArgList args, XtTypedArgList avlist);
static void _XmVaProcessEverything(Widget widget,
                                   va_list var,
...
static XmButtonType _XmVaBType_to_XmBType(String symbol)
{
...
static void _XmCountNestedList(XtTypedArgList avlist, int *total_count, int *typed_count)
{
  for (; avlist->name != NULL; avlist++) {
    if (strcmp(avlist->name, XtVaNestedList) == 0) {
      _XmCountNestedList((XtTypedArgList)avlist->value, total_count, typed_count);
    }
    else {
      if (avlist->type != NULL) {
        ++(*typed_count);
 ...
 *
 */
int XmeCountVaListSimple(va_list al)
{
  int d1, d2, d3, count;
 ...
 */
void _XmCountVaList(
    va_list var, int *button_count, int *args_count, int *typed_count, int *total_count)
{
  String attr;
  int i;
  *button_count = 0;
  ...
  *total_count = 0;
  for (attr = va_arg(var, String); attr != NULL; attr = va_arg(var, String)) {
    ...
    if (strcmp(attr, XtVaTypedArg) == 0) {
      ...
      _XmCountNestedList(va_arg(var, XtTypedArgList), total_count, typed_count);
    }
    else {
      ...
      else if ((strcmp(attr, XmVaSEPARATOR) == 0) || (strcmp(attr, XmVaDOUBLE_SEPARATOR) == 0)) {
        ++(*total_count);
        ++(*button_count);
      }
                             ...
                             Cardinal num_resources)
{
  ...
  for (; avlist->name != NULL; avlist++) {
    if (avlist->type != NULL) {
      ...
      if (widget != NULL) {
        /* this is a typed arg */
        count += _XmTypedArgToArg(widget, avlist, (args + count), resources, num_resources);
      }
    }
    else if (strcmp(avlist->name, XtVaNestedList) == 0) {
      count += _XmNestedArgtoArg(
          widget, (XtTypedArgList)avlist->value, (args + count), resources, num_resources);
    }
    else {
...
static int _XmNestedArgtoTypedArg(XtTypedArgList args, XtTypedArgList avlist)
{
  ...
  for (; avlist->name != NULL; avlist++) {
    if (avlist->type != NULL) {
      ...
      count += _XmNestedArgtoTypedArg((args + count), (XtTypedArgList)avlist->value);
    }
    else {
      (args + count)->name = avlist->name;
                         ...
                         Cardinal *num_args_return)
{
  ...
  for (attr = va_arg(var, String), count = 0; attr != NULL; attr = va_arg(var, String)) {
    if (strcmp(attr, XtVaTypedArg) == 0) {
      ...
      count += _XmNestedArgtoTypedArg(&args[count], va_arg(var, XtTypedArgList));
    }
    else {
  ...
  *buttonAcceleratorText = (XmStringTable)XtMalloc((unsigned)(button_count * sizeof(XmString *)));
  ...
  for (attr = va_arg(var, String), count = 0; attr != NULL; attr = va_arg(var, String)) {
    if (strcmp(attr, XtVaTypedArg) == 0) {
      ...
      count += _XmTypedArgToArg(widget, &typed_args, &(*args)[count], NULL, 0);
    }
    else if (strcmp(attr, XtVaNestedList) == 0) {
      count += _XmNestedArgtoArg(widget, va_arg(var, XtTypedArgList), &(*args)[count], NULL, 0);
    }
    else if ((strcmp(attr, XmVaSEPARATOR) == 0) || (strcmp(attr, XmVaDOUBLE_SEPARATOR) == 0)) {
...
#define MB_EXTRA_ARGS 1
  ...
  _XmVaProcessEverything(parent,
                         var,
                         &menuBarButtonTypes,
                         &menuBarStrings,
                         &menuBarMnemonics,
                         &menuBarAccelerators,
                         &menuBarAcceleratorText,
                         button_count,
                         &args,
                         num_args);
  n = args_count;
  XtSetArg(args[n], XmNbuttonCount, button_count);
  n++;
  XtSetArg(args[n], XmNbuttonType, menuBarButtonTypes);
  n++;
  XtSetArg(args[n], XmNbuttons, menuBarStrings);
  n++;
...
#define PD_EXTRA_ARGS 3
  ...
  _XmVaProcessEverything(parent,
                         var,
                         &pulldownMenuButtonTypes,
                         &pulldownMenuStrings,
                         &pulldownMenuMnemonics,
                         &pulldownMenuAccelerators,
                         &pulldownMenuAcceleratorText,
                         button_count,
                         &args,
                         num_args);
  n = args_count;
  XtSetArg(args[n], XmNsimpleCallback, callback);
  n++;
  XtSetArg(args[n], XmNpostFromButton, post_from_button);
  n++;
  XtSetArg(args[n], XmNbuttonCount, button_count);
  n++;
  XtSetArg(args[n], XmNbuttonType, pulldownMenuButtonTypes);
  n++;
  XtSetArg(args[n], XmNbuttons, pulldownMenuStrings);
  n++;
  XtSetArg(args[n], XmNbuttonMnemonics, pulldownMenuMnemonics);
  n++;
  XtSetArg(args[n], XmNbuttonAccelerators, pulldownMenuAccelerators);
  n++;
...
#define PU_EXTRA_ARGS 2
  ...
  _XmVaProcessEverything(parent,
                         var,
                         &popupMenuButtonTypes,
                         &popupMenuStrings,
                         &popupMenuMnemonics,
                         &popupMenuAccelerators,
  ...
  XtSetArg(args[n], XmNbuttonCount, button_count);
  n++;
  XtSetArg(args[n], XmNbuttonType, popupMenuButtonTypes);
  n++;
  XtSetArg(args[n], XmNbuttons, popupMenuStrings);
  n++;
  XtSetArg(args[n], XmNbuttonMnemonics, popupMenuMnemonics);
  n++;
  XtSetArg(args[n], XmNbuttonAccelerators, popupMenuAccelerators);
  n++;
...
#define OM_EXTRA_ARGS 5
  ...
  _XmVaProcessEverything(parent,
                         var,
                         &optionMenuButtonTypes,
                         &optionMenuStrings,
                         &optionMenuMnemonics,
                         &optionMenuAccelerators,
                         &optionMenuAcceleratorText,
                         button_count,
                         &args,
                         num_args);
  n = args_count;
  XtSetArg(args[n], XmNsimpleCallback, callback);
  n++;
  XtSetArg(args[n], XmNoptionLabel, option_label);
  n++;
  XtSetArg(args[n], XmNoptionMnemonic, option_mnemonic);
  n++;
  XtSetArg(args[n], XmNbuttonSet, button_set);
  n++;
  XtSetArg(args[n], XmNbuttonCount, button_count);
  n++;
  XtSetArg(args[n], XmNbuttonType, optionMenuButtonTypes);
  n++;
  XtSetArg(args[n], XmNbuttons, optionMenuStrings);
  n++;
  XtSetArg(args[n], XmNbuttonMnemonics, optionMenuMnemonics);
  n++;
  XtSetArg(args[n], XmNbuttonAccelerators, optionMenuAccelerators);
  n++;
...
#define RB_EXTRA_ARGS 3
  ...
  _XmVaProcessEverything(parent,
                         ...
                         &radioBoxStrings,
                         &radioBoxMnemonics,
                         &radioBoxAccelerators,
                         &radioBoxAcceleratorText,
                         button_count,
                         &args,
                         num_args);
  n = args_count;
  ...
  XtSetArg(args[n], XmNbuttonCount, button_count);
  n++;
  XtSetArg(args[n], XmNbuttonType, radioBoxButtonTypes);
  n++;
  XtSetArg(args[n], XmNbuttons, radioBoxStrings);
  n++;
  XtSetArg(args[n], XmNbuttonMnemonics, radioBoxMnemonics);
  n++;
  XtSetArg(args[n], XmNbuttonAccelerators, radioBoxAccelerators);
  n++;
...
#define CB_EXTRA_ARGS 2
  ...
  _XmVaProcessEverything(parent,
                         ...
                         &checkBoxStrings,
                         &checkBoxMnemonics,
                         &checkBoxAccelerators,
                         &checkBoxAcceleratorText,
                         button_count,
                         &args,
                         num_args);
  n = args_count;
  ...
  XtSetArg(args[n], XmNbuttonCount, button_count);
  n++;
  XtSetArg(args[n], XmNbuttonType, checkBoxButtonTypes);
  n++;
  XtSetArg(args[n], XmNbuttons, checkBoxStrings);
  n++;
  XtSetArg(args[n], XmNbuttonMnemonics, checkBoxMnemonics);
  n++;
  XtSetArg(args[n], XmNbuttonAccelerators, checkBoxAccelerators);
  n++;
    ...
    char *name, WidgetClass wc, Widget parent, Boolean managed, va_list al, int count)
{
   ...
   */
  for (attr = va_arg(al, String), n = 0; attr != NULL; attr = va_arg(al, String), n++) {
    args[n].name = attr;
  ...
  if (managed)
    w = XtCreateManagedWidget(name, wc, parent, args, n);
  else
    w = XtCreateWidget(name, wc, parent, args, n);
  /*
