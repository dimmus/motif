 ...
 */
externaldef(xmrowcolumnwidgetclass) WidgetClass xmRowColumnWidgetClass = (WidgetClass)&xmRowColumnClassRec;
   ...
   Widget w)
{
   ...
   if (IsPopup(m))
   {
      ...
      for (i = 0; i < m->row_column.postFromCount; i++)
      {
         ...
         {
            XtRemoveCallback(m->row_column.postFromList[i], XtNdestroyCallback, (XtCallbackProc)_XmRC_RemoveFromPostFromListOnDestroyCB, (XtPointer)m);
         }
      }
   }
   else if (IsOption(m) || IsBar(m))
   {
      ...
      XtRemoveEventHandler(topManager, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, m);
      /* Remove it from us */
      XtRemoveEventHandler((Widget)m, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)m);
   }
   /*
   ...
   if ((IsPopup(m) && RC_PopupEnabled(m)) || (IsBar(m) && RC_MenuAccelerator(m)) || (IsOption(m) && (RC_Mnemonic(m) != XK_VoidSymbol)))
   {
   ...
   Widget w)
{
   ...
   if (!IsWorkArea(m) /* it's a menu */ && !(XmeTraitGet((XtPointer)XtClass(w), XmQTmenuSavvy) != NULL))
   ...
   /*
     ...
     */
   if (XtIsRectObj(w) && RC_IsHomogeneous(m) && RC_EntryClass(m) && (RC_EntryClass(m) != XtClass(w)))
   {
   ...
   Widget            child)
{
   ...
   else if (IsOption(menu) && (child == RC_MemWidget(menu)))
   {
      ...
      {
         RC_MemWidget(menu) = FindFirstManagedChild((CompositeWidget)RC_OptionSubMenu(menu),
                                                    True);
   ...
   Widget child)
{
   ...
   /*
     ...
     */
   if (XtIsWidget(child) && ((IsPopup(m) || IsBar(m) || IsPulldown(m)) && XmIsLabel(child) && (child->core.widget_class != xmLabelWidgetClass)))
   {
      XtRemoveEventHandler(child, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, m);
   }
   /*
   ...
   Widget wid)
{
   ...
   */
   ...
   if (m->row_column.old_shadow_thickness)
      _XmClearShadowType((Widget)m, m->row_column.old_width, m->row_column.old_height, m->row_column.old_shadow_thickness, 0);
   /* and redraw it for shrinking size */
   if (XtIsRealized((Widget)m) && m->manager.shadow_thickness)
      XmeDrawShadows(XtDisplay(m), XtWindow(m), m->manager.top_shadow_GC, m->manager.bottom_shadow_GC, 0, 0, m->core.width, m->core.height, m->manager.shadow_thickness, XmSHADOW_OUT);
   ...
   XSetWindowAttributes *window_attributes)
{
   ...
   window_attributes->do_not_propagate_mask = ButtonPressMask | ButtonReleaseMask | KeyPressMask | KeyReleaseMask | PointerMotionMask;
   ...
   XmRowColumnWidget new_w)
{
   ...
   if (RC_EntryBorder(old) != RC_EntryBorder(new_w))
   {
      ...
      ForAllChildren(new_w, i, p)
      {
         ...
         if (XtIsRealized(*p))
            XmeConfigureObject(*p, (*p)->core.x, (*p)->core.y, (*p)->core.width, (*p)->core.height, desired.border_width);
         else
            (*p)->core.border_width = desired.border_width;
      }
      need_expose = TRUE;
   }
   if ((RC_EntryAlignment(old) != RC_EntryAlignment(new_w)) && (IsAligned(new_w)) && (!IsOption(new_w)))
   {
      Widget *p;
      Arg     al[2];
      int     i;
      ...
      if (RC_OptionSubMenu(new_w) != RC_OptionSubMenu(old))
      {
         ...
         if (!RC_MemWidget(new_w) || (RC_MemWidget(old) == RC_MemWidget(new_w)))
         {
            if ((child = FindFirstManagedChild(
                    (CompositeWidget)RC_OptionSubMenu(new_w),
                    FIRST_BUTTON))
                != NULL)
            {
   ...
   Cardinal *num_args) /* unused */
{
   ...
   if (!XmRepTypeValidValue(XmRID_VERTICAL_ALIGNMENT, RC_EntryVerticalAlignment(new_w), (Widget)new_w))
   {
   ...
   if (RC_MenuPost(new_w) != RC_MenuPost(old))
   {
      ...
      {
         if (_XmMapBtnEvent(RC_MenuPost(new_w), &RC_PostEventType(new_w), &RC_PostButton(new_w), &RC_PostModifiers(new_w)) == FALSE)
         {
   ...
   {
      if ((new_w->row_column.postFromList != old->row_column.postFromList) || (new_w->row_column.postFromCount != old->row_column.postFromCount))
      {
   ...
   XtWidgetGeometry *allowed)
{
   ...
   RC_Boxes(m) = (XmRCKidGeometry)
      _XmRCGetKidGeo((Widget)m, instigator, desired, RC_EntryBorder(m), RC_EntryBorder(m), (IsVertical(m) && RC_DoMarginAdjust(m)), (IsHorizontal(m) && RC_DoMarginAdjust(m)), RC_HelpPb(m), RC_TearOffControl(m), XmGET_PREFERRED_SIZE);
   ...
   if (!(desired->request_mode & XtCWQueryOnly))
   {
      ...
      /* Clear shadow if necessary. */
      if (m->row_column.old_shadow_thickness && ((m->row_column.old_width != m->core.width) || (m->row_column.old_height != m->core.height) || (m->row_column.old_shadow_thickness != m->manager.shadow_thickness)))
         _XmClearShadowType((Widget)m, m->row_column.old_width, m->row_column.old_height, m->row_column.old_shadow_thickness, 0);
      m->row_column.old_width            = m->core.width;
      m->row_column.old_height           = m->core.height;
      m->row_column.old_shadow_thickness = m->manager.shadow_thickness;
   ...
   XmRowColumnWidget bar)
{
   ...
   XtAddEventHandler((Widget)bar, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)bar);
   XtAddEventHandler((Widget)topManager, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)bar);
   if (RC_MenuAccelerator(bar))
   ...
   XmRowColumnWidget popup)
{
   ...
   if (RC_TearOffModel(popup) != XmTEAR_OFF_DISABLED)
   {
      ...
      RC_TearOffControl(popup) = XtCreateWidget(TEAROFF_CONTROL, xmTearOffButtonWidgetClass, (Widget)popup, args, n);
   ...
   XmRowColumnWidget pulldown)
{
   ...
   XtAddEventHandler((Widget)pulldown, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)pulldown);
	...
	*/
      ...
      RC_TearOffControl(pulldown) = XtCreateWidget(TEAROFF_CONTROL, xmTearOffButtonWidgetClass, (Widget)pulldown, args, n);
   ...
   XmRowColumnWidget option)
{
      ...
      RC_Spacing(option) = 3;
   {
      ...
      _XmProcessLock();
      translations = (XtTranslations)((XmManagerClassRec *)XtClass(option))->manager_class.translations;
      _XmProcessUnlock();
   ...
   XtAddEventHandler((Widget)option, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)option);
   XtAddEventHandler((Widget)topManager, KeyPressMask | KeyReleaseMask, False, _XmRC_KeyboardInputHandler, (XtPointer)option);
   ...
   XmRowColumnWidget work)
{
      ...
      RC_Spacing(work) = 3;
   {
      XtTranslations translations;
      _XmProcessLock();
      translations = (XtTranslations)((XmManagerClassRec *)XtClass(work))->manager_class.translations;
      _XmProcessUnlock();
   ...
   Cardinal *num_args)
{
   ...
   if ((RC_Orientation(nw) != XmNO_ORIENTATION) && !XmRepTypeValidValue(XmRID_ORIENTATION, RC_Orientation(nw), (Widget)nw))
   {
      RC_Orientation(nw) = XmNO_ORIENTATION;
   }
   if ((RC_Packing(nw) != XmNO_PACKING) && !XmRepTypeValidValue(XmRID_PACKING, RC_Packing(nw), (Widget)nw))
   {
   ...
   {
      if (_XmMapBtnEvent(RC_MenuPost(m), &RC_PostEventType(m), &RC_PostButton(m), &RC_PostModifiers(m)) == FALSE)
      {
   ...
   Cardinal *num_args)
{
   ...
   {
      ((XmManagerWidget)XtParent(XtParent(new_w)))->manager.accelerator_widget = ((XmManagerWidget)XtParent(new_w))->manager.accelerator_widget;
   ...
   Cardinal *num_args) /* unused */
{
   ...
   {
      margins_changed = ((LabG_MarginTop(old) != LabG_MarginTop(new_w)) || (LabG_MarginBottom(old) != LabG_MarginBottom(new_w)) || (LabG_MarginLeft(old) != LabG_MarginLeft(new_w)) || (LabG_MarginRight(old) != LabG_MarginRight(new_w)));
   }
   else if (XmIsLabel(old))
   {
      margins_changed = ((Lab_MarginTop(old) != Lab_MarginTop(new_w)) || (Lab_MarginBottom(old) != Lab_MarginBottom(new_w)) || (Lab_MarginLeft(old) != Lab_MarginLeft(new_w)) || (Lab_MarginRight(old) != Lab_MarginRight(new_w)));
   }
   else
   ...
   if (RCIndex(old) != RCIndex(new_w))
   {
      ...
      else
      {
         ...
         if ((current.x != XtX(new_w)) || (current.width != XtWidth(new_w)) || (current.height != XtHeight(new_w)) || (current.border_width != XtBorderWidth(new_w)))
   ...
   int      is_radio)  /* the radio flag */
{
   ...
   if ((type == XmMENU_PULLDOWN) || (type == XmMENU_POPUP))
   {
      ...
      if (XmIsRowColumn(p) && (IsBar(p) || IsPopup(p) || IsPulldown(p)))
      {
         ...
         {
            if ((XmIsMenuShell(pw->core.popup_list[i])) && (((XmMenuShellWidget)pw->core.popup_list[i])->menu_shell.private_shell) && (!(pw->core.popup_list[i])->core.being_destroyed))
            {
...
ClassInitialize(void)
{
   ...
   /* Trait records */
   XmeTraitSet((XtPointer)xmRowColumnWidgetClass, XmQTmenuSystem, (XtPointer)&_XmRC_menuSystemRecord);
}
...
FixEventBindings(
   ...
   Widget            w)            /* subwidget */
{
   if (XtIsWidget(w) && ((IsPopup(m) || IsBar(m) || IsPulldown(m)) && XmIsLabel(w) && (w->core.widget_class != xmLabelWidgetClass)))
   {
   ...
   Boolean         first_button)
{
   ...
   while ((i < n) && ((*kid)->core.being_destroyed || (!XtIsManaged(*kid) || (first_button && !(XmIsTraversable(*kid))))))
      kid++, i++;
   if (i >= n)
      return (NULL);
   else
   ...
   Widget wid)
{
    ...
    */
   if (!IsOption(m) && (m->row_column.old_height > m->core.height || m->row_column.old_width > m->core.width))
      draw_shadow = True;
   m->row_column.old_width            = m->core.width;
   m->row_column.old_height           = m->core.height;
   m->row_column.old_shadow_thickness = m->manager.shadow_thickness;
   ...
   Widget            w)
{
   ...
   if (RC_EntryBorder(m))
   {
      /* fix for 7660, setting entryborder before realize time
	       ...
	       be a behavior compatibility issue */
      if (XtIsRealized(w))
         XmeConfigureObject(w, w->core.x, w->core.y, w->core.width, w->core.height, RC_EntryBorder(m));
      else
   ...
   if (XmIsLabelGadget(w))
   {
      if (IsAligned(m))
      {
         if (IsWorkArea(m) || ((w->core.widget_class != xmLabelWidgetClass) && (w->core.widget_class != xmLabelGadgetClass)))
         {
            ac = 0;
   ...
   Cardinal *num_params) /* unused */
{
   ...
   if (IsBar(rc))
   {
      /*
         ...
         */
      if (RC_IsArmed(rc))
         (*(((XmRowColumnClassRec *)(rc->core.widget_class))->row_column_class.armAndActivate))((Widget)rc, event, NULL, NULL);
   }
   else
   {
      /* Let the menushell widget clean things up */
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownOne))(XtParent(rc), event, NULL, NULL);
   }
