                 ...
                 int         len)
{
   ...
   if ((tag_index < TAG_INDEX_UNSET) && (t_length < (1 << BYTE_COUNT_BITS)))
                       ...
                       int         length)
{
   ...
   {
      if (((tag == _tag_cache[i]) || ((length != XmSTRING_TAG_STRLEN) && (strncmp(tag, _tag_cache[i], length) == 0)) || ((length == XmSTRING_TAG_STRLEN) && (strcmp(tag, _tag_cache[i]) == 0))) && ((length == XmSTRING_TAG_STRLEN) || (_tag_cache[i][length] == '\0')))
      {
         _XmProcessUnlock();
         return (i);
      }
                     ...
                     _XmStringEntry seg2)
{
   ...
   if (_XmEntryOptimized(seg1) && (((ecnt1 != 0) && (bcnt2 != 0)) || ((bcnt1 > 0) && (bcnt2 > 0)) || ((ecnt1 > 0) && (ecnt2 > 0)) || ((bcnt1 > 0) && (ecnt2 > 0) && (begin1[0] != end2[0]))))
      return (FALSE);
          ...
          _XmStringEntry b)
{
   ...
   if (_XmEntryOptimized(a))
   {
      ...
      {
         _XmEntryRendIndex(a) = _XmEntryOptimized(b) ? _XmEntryRendIndex(b) : _XmStringIndexCacheTag(_XmEntryRendEndGet(b, 0), XmSTRING_TAG_STRLEN);
            ...
            _XmStringEntry b)
{
   ...
   if (_XmEntryOptimized(a))
   {
      ...
      {
         _XmEntryRendIndex(a) = _XmEntryOptimized(b) ? _XmEntryRendIndex(b) : _XmStringIndexCacheTag(b_begin[0], XmSTRING_TAG_STRLEN);
...
IsUnopt(_XmString str, int lines)
{
   ...
   if (lines > 0)
   {
      ...
      if ((_XmEntrySegmentCountGet(line) > 0) && (_XmEntryType(_XmEntrySegmentGet(line)[0]) != XmSTRING_ENTRY_OPTIMIZED))
         return True;
   }
   return False;
}
   ...
   else if (_XmEntryMultiple(entry))
   {
      if (free)
      {
         for (j = 0; j < _XmEntrySegmentCount(entry); j++)
         {
            ...
            {
               _XmEntrySegment(entry)[j] = (_XmStringNREntry)EntryCvtToUnopt((_XmStringEntry)tmp_seg);
      ...
      else
      {
         ...
         _XmEntrySegment(new_entry) = (_XmStringNREntry *)XtMalloc(_XmEntrySegmentCount(entry) * sizeof(_XmStringNREntry));
         for (j = 0; j < _XmEntrySegmentCount(entry); j++)
         {
            tmp_seg = _XmEntrySegment(entry)[j];
            if (_XmEntryType(tmp_seg) == XmSTRING_ENTRY_OPTIMIZED)
               _XmEntrySegment(new_entry)[j] = (_XmStringNREntry)EntryCvtToUnopt((_XmStringEntry)tmp_seg);
            else
               _XmEntrySegment(new_entry)[j] = (_XmStringNREntry)_XmStringEntryCopy((_XmStringEntry)tmp_seg);
         }
      }
   }
   else
                      ...
                      XmString b)
{
   ...
   if ((_XmStrOptimized(a) && _XmStrOptimized(b)))
   {
      ...
      a_tabs       = _XmStrTabs(a);
      b_tabs       = _XmStrTabs(b);
      if (((a_index == b_index) || (a_index == TAG_INDEX_UNSET) || (b_index == TAG_INDEX_UNSET)) && RenditionsCompatible((_XmStringEntry)a, (_XmStringEntry)b) && ((_XmStrDirection(a) == _XmStrDirection(b)) || (_XmStrDirection(b) == XmSTRING_DIRECTION_UNSET) || ((_XmStrDirection(a) == XmSTRING_DIRECTION_UNSET) && (a_len == 0))) && (a_type == b_type || a_type == XmNO_TEXT || b_type == XmNO_TEXT) && ((a_len + b_len) < (1 << BYTE_COUNT_BITS)) && ((_XmStrText(a) && b_tabs == 0) || (a_tabs + b_tabs <= 3)))
      {
         ...
         _XmStrRendIndex((_XmString)opt_str) = (a_rend_index == REND_INDEX_UNSET) ? b_rend_index : a_rend_index;
         /* Push begin and end state. */
         _XmStrRendBegin((_XmString)opt_str) = (_XmStrRendBegin(a) ? _XmStrRendBegin(a) : _XmStrRendBegin(b));
         _XmStrRendEnd((_XmString)opt_str)   = (_XmStrRendEnd(b) ? _XmStrRendEnd(b) : _XmStrRendEnd(a));
         _XmStrDirection((_XmString)opt_str) = (_XmStrDirection(a) == XmSTRING_DIRECTION_UNSET) ? _XmStrDirection(b) : _XmStrDirection(a);
   ...
   _XmStrImplicitLine(a_str) = _XmStrImplicitLine(a_str) || _XmStrImplicitLine(b_str);
   ...
   if ((a_sc != 0) && (b_sc != 0))
   {
      ...
      if (((a_tag == b_tag) || (a_tag == NULL) || (b_tag == NULL)) && RenditionsCompatible((_XmStringEntry)a_last, (_XmStringEntry)b_seg) && (a_type == b_type || a_type == XmNO_TEXT || b_type == XmNO_TEXT) && ((last == _XmEntryDirectionGet((_XmStringEntry)b_seg)) || (_XmEntryDirectionGet((_XmStringEntry)b_seg) == XmSTRING_DIRECTION_UNSET) || ((last == XmSTRING_DIRECTION_UNSET) && (a_len == 0))) && !_XmEntryPopGet((_XmStringEntry)a_last) && !_XmEntryPushGet((_XmStringEntry)b_seg) && (((a_len != 0) && b_tabs == 0) || ((a_len == 0) && a_tabs + b_tabs <= 7)))
      {
         if (b_len)
         {
            if ((_XmEntryType(a_last) == XmSTRING_ENTRY_OPTIMIZED) && _XmEntryImm(a_last))
            {
               ...
               if (a_line == (_XmStringEntry)a_last)
               {
                  ...
                  _XmEntrySegmentGet(a_line)[a_sc - 1] = a_last = (_XmStringNREntry)XtRealloc((char *)a_last, size);
            }
            else
            {
         ...
         if (_XmEntryOptimized(a_last))
            _XmEntryTagIndex(a_last) = ((a_tag == NULL) ? (b_tag == NULL ? TAG_INDEX_UNSET : _XmStringIndexCacheTag(b_tag, XmSTRING_TAG_STRLEN)) : _XmEntryTagIndex(a_last));
         else
         ...
         {
            if (_XmEntryOptimized(a_last) && _XmEntryRendEndCountGet((_XmStringEntry)b_seg) > 0)
            {
               b_tag                     = _XmEntryRendEndGet((_XmStringEntry)b_seg, 0);
               _XmEntryRendIndex(a_last) = _XmStringIndexCacheTag(b_tag, XmSTRING_TAG_STRLEN);
            }
            if (!_XmEntryOptimized(a_last) && (_XmEntryRendEndCountGet((_XmStringEntry)b_seg) != 0))
            {
               if (_XmEntryOptimized(b_seg))
               {
                  _XmUnoptSegRendEnds(a_last)    = (XmStringTag *)XtMalloc(sizeof(XmStringTag));
                  _XmUnoptSegRendEnds(a_last)[0] = _XmEntryRendEndGet((_XmStringEntry)b_seg, 0);
               }
               else if (modify_b)
               {
            ...
            _XmEntryRendEndCountSet(a_last, _XmEntryRendEndCountGet((_XmStringEntry)b_seg));
         }
         else
         ...
         if (modify_b)
         {
            ...
            {
               if (_XmEntryOptimized(a_last) || (_XmUnoptSegRendBegins(a_last) != _XmUnoptSegRendBegins(b_seg)))
                  XtFree((char *)_XmUnoptSegRendBegins(b_seg));
               if (_XmEntryOptimized(a_last) || (_XmUnoptSegRendEnds(a_last) != _XmUnoptSegRendEnds(b_seg)))
                  XtFree((char *)_XmUnoptSegRendEnds(b_seg));
            }
            if (!((_XmEntryType(b_seg) == XmSTRING_ENTRY_OPTIMIZED) && _XmEntryImm(b_seg)))
   ...
   {
      Boolean free_b_line = (modify_b && _XmEntryMultiple(b_line) && ((_XmStringEntry)b_seg != b_line));
      if (_XmEntryMultiple(a_line))
      {
         _XmEntrySegment(a_line)      = (_XmStringNREntry *)XtRealloc((char *)_XmEntrySegment(a_line),
                                                                 sizeof(_XmStringNREntry) * (a_sc + b_sc - merged));
      ...
      if (free_b_line)
      {
         if (_XmEntrySegment(b_line) != (_XmStringNREntry *)&b && _XmEntrySegment(b_line) != (_XmStringNREntry *)_XmStrEntry(b_str))
   ...
   XmString b)
{
   ...
   if (_XmStrOptimized(a) && _XmStrOptimized(b))
   {
      if (!((_XmStrTagGet(a) == _XmStrTagGet(b)) || (_XmStrTagGet(a) == NULL) || (_XmStrTagGet(b) == NULL) || ((strcmp(_XmStringIndexGetTag(_XmStrTagIndex(a)), XmFONTLIST_DEFAULT_TAG) == 0) && _XmStringIsCurrentCharset(_XmStrTagGet(b))) || ((strcmp(_XmStringIndexGetTag(_XmStrTagIndex(b)), XmFONTLIST_DEFAULT_TAG) == 0) && _XmStringIsCurrentCharset(_XmStrTagGet(a)))))
      {
         _XmProcessUnlock();
         return (FALSE);
      }
      ...
      if ((_XmStrDirection(a) != _XmStrDirection(b)) && (((_XmStrDirection(a) == XmSTRING_DIRECTION_UNSET) && (_XmStrDirection(b) != XmSTRING_DIRECTION_L_TO_R)) || ((_XmStrDirection(b) == XmSTRING_DIRECTION_UNSET) && (_XmStrDirection(a) != XmSTRING_DIRECTION_L_TO_R))))
      {
         _XmProcessUnlock();
         return (FALSE);
      }
      ...
      for (i = 0; i < _XmStrEntryCountGet(a); i++)
      {
         if (_XmEntryMultiple(entry_a[i]) && _XmEntryMultiple(entry_b[i]))
         {
            ...
            for (j = 0; j < _XmEntrySegmentCount(entry_a[i]); j++)
            {
               ...
               if (!((a_tag == b_tag) || (a_tag == NULL) || (b_tag == NULL) || ((strcmp(a_tag, XmFONTLIST_DEFAULT_TAG) == 0) && _XmStringIsCurrentCharset(b_tag)) || ((strcmp(b_tag, XmFONTLIST_DEFAULT_TAG) == 0) && _XmStringIsCurrentCharset(a_tag))))
               {
                  if (a_unopt) XmStringFree(a_unopt);
                  if (b_unopt) XmStringFree(b_unopt);
                  _XmProcessUnlock();
               ...
               }
               {
                  ...
                  if ((a_dir != b_dir) && (((a_dir == XmSTRING_DIRECTION_UNSET) && (b_dir != XmSTRING_DIRECTION_L_TO_R)) || ((b_dir == XmSTRING_DIRECTION_UNSET) && (a_dir != XmSTRING_DIRECTION_L_TO_R))))
                  {
                     if (a_unopt) XmStringFree(a_unopt);
                     if (b_unopt) XmStringFree(b_unopt);
                     _XmProcessUnlock();
         ...
         {
            unsigned int len;
            if (!((_XmEntryTag(entry_a[i]) == _XmEntryTag(entry_b[i])) || (_XmEntryTag(entry_a[i]) == NULL) || (_XmEntryTag(entry_b[i]) == NULL) || ((strcmp(_XmEntryTag(entry_a[i]), XmFONTLIST_DEFAULT_TAG) == 0) && _XmStringIsCurrentCharset(_XmEntryTag(entry_b[i]))) || ((strcmp(_XmEntryTag(entry_b[i]), XmFONTLIST_DEFAULT_TAG) == 0) && _XmStringIsCurrentCharset(_XmEntryTag(entry_a[i])))))
            {
               if (a_unopt) XmStringFree(a_unopt);
               if (b_unopt) XmStringFree(b_unopt);
               _XmProcessUnlock();
            ...
            if ((_XmEntryDirectionGet(entry_a[i]) != _XmEntryDirectionGet(entry_b[i])) && (((_XmEntryDirectionGet(entry_a[i]) == XmSTRING_DIRECTION_UNSET) && (_XmEntryDirectionGet(entry_b[i]) != XmSTRING_DIRECTION_L_TO_R)) || ((_XmEntryDirectionGet(entry_b[i]) == XmSTRING_DIRECTION_UNSET) && (_XmEntryDirectionGet(entry_a[i]) != XmSTRING_DIRECTION_L_TO_R))))
            {
               if (a_unopt) XmStringFree(a_unopt);
               if (b_unopt) XmStringFree(b_unopt);
               _XmProcessUnlock();
               ...
               Dimension    *descent)
{
   ...
   if (base_rend == NULL)
   {
      if (_XmStrRendBegin(opt))
         rend = _XmRenderTableFindRendition(r, _XmStrRendTagGet(opt), TRUE, FALSE, TRUE, &rend_index);
      if ((rend == NULL) || (_XmRendFont(rend) == NULL && _XmRendXftFont(rend) == NULL))
         rend = _XmRenderTableFindRendition(r, _XmStrTagGet(opt), TRUE, FALSE, TRUE, &rend_index);
   }
   else
   {
      if (_XmStrRendBegin(opt))
      {
         ...
         rend    = _XmRenditionMerge(_XmRendDisplay(base_rend), rend_io, base_rend, r, _XmStrTagGet(opt), tags, 1, FALSE);
      }
      else
      {
         rend = _XmRenditionMerge(_XmRendDisplay(base_rend), rend_io, base_rend, r, _XmStrTagGet(opt), NULL, 0, FALSE);
      }
   }
   /* 3. Default rendition. */
   if ((rend == NULL) || (_XmRendFont(rend) == NULL && _XmRendXftFont(rend) == NULL))
   {
      tags[0] = ((_XmStrTextType(opt) == XmCHARSET_TEXT) ? XmFONTLIST_DEFAULT_TAG : _MOTIF_DEFAULT_LOCALE);
      rend    = _XmRenderTableFindRendition(r, tags[0], TRUE, FALSE, FALSE, NULL);
      if ((rend != NULL) && (_XmRendFont(rend) == NULL) && (_XmRendXftFont(rend) == NULL) && (((base_rend != NULL) && (_XmRendDisplay(base_rend) != NULL)) || (_XmRendDisplay(rend) != NULL)))
      /* Call noFontCallback. */
      {
         XmDisplay               dsp;
         ...
         if ((base_rend != NULL) && (_XmRendDisplay(base_rend) != NULL))
            ...
            dsp = (XmDisplay)XmGetXmDisplay(_XmRendDisplay(rend));
      ...
      if ((rend == NULL) && ((_XmStrTextType(opt) == XmCHARSET_TEXT) || ((_XmStrTextType(opt) == XmMULTIBYTE_TEXT) && (_XmStrTagGet(opt) == XmFONTLIST_DEFAULT_TAG))) && (r != NULL) && (_XmRTCount(r) > 0))
      ...
      if ((rend == NULL) || (_XmRendFont(rend) == NULL && _XmRendXftFont(rend) == NULL))
      {
         ...
         if (_XmStrRendBegin(opt) || (_XmStrTagGet(opt) != NULL))
   ...
   if (rend != NULL)
      ComputeMetrics(rend,
                     ...
                     descent,
#if XM_UTF8
                     (_XmStrTextType(opt) == XmCHARSET_TEXT || _XmStrTextType(opt) == XmMULTIBYTE_TEXT) && ((_XmStrTagGet(opt) == XmFONTLIST_DEFAULT_TAG && _XmStringIsCurrentCharset("UTF-8")) || (_XmStrTagGet(opt) && strcmp(_XmStringIndexGetTag(_XmStrTagIndex(opt)), "UTF-8") == 0))
#else
                     False
#endif
      );
            ...
            Dimension     *descender)
{
   ...
   {
      peek_seg = ((seg_index + 1) < _XmEntrySegmentCountGet(line) ? _XmEntrySegmentGet(line)[seg_index + 1] : NULL);
   }
   ...
   if (peek_seg != NULL)
      (void)SpecifiedSegmentExtents((_XmStringEntry)seg, r, rend_io, base, XmSTRING_FIRST_SEG, &w, &h, &asc, &dsc);
   else
      (void)SpecifiedSegmentExtents((_XmStringEntry)seg, r, rend_io, base, XmSTRING_SINGLE_SEG, &w, &h, &asc, &dsc);
   ...
   while (seg != NULL)
   {
      ...
      if ((tab != NULL) && (_XmEntryTabsGet((_XmStringEntry)seg) != 0) && (tab_cnt < _XmTabLCount(tl)))
      {
         for (i = 0;
              (i < _XmEntryTabsGet((_XmStringEntry)seg)) && (tab_cnt < _XmTabLCount(tl));
      ...
      {
         seg_index++;
         seg      = (seg_index < _XmEntrySegmentCountGet(line) ? _XmEntrySegmentGet(line)[seg_index] : NULL);
         peek_seg = ((seg_index + 1) < _XmEntrySegmentCountGet(line) ? _XmEntrySegmentGet(line)[seg_index + 1] : NULL);
      }
      if (seg != NULL)
      {
         ...
         if (peek_seg != NULL)
            (void)SpecifiedSegmentExtents((_XmStringEntry)seg, r, rend_io, base, XmSTRING_MIDDLE_SEG, &w, &h, &asc, &dsc);
         else
            (void)SpecifiedSegmentExtents((_XmStringEntry)seg, r, rend_io, base, XmSTRING_LAST_SEG, &w, &h, &asc, &dsc);
      }
   }
   *width = tab_w;
        ...
        _XmStringEntry entry)
{
   ...
   {
      n = 0;
      XtSetArg(args[n], XmNfontType, &type);
      n++;
      XtSetArg(args[n], XmNfont, &font);
      n++;
                     ...
                     XmRenderTable  rt)
{
   ...
   {
      if (((_XmStringUnoptSeg)entry)->char_count == 0 && (len = _XmEntryByteCountGet(entry)) != 0)
      {
         ((_XmStringUnoptSeg)entry)->char_count = _XmStringCharacterCount((char *)_XmEntryTextGet(entry),
                                                                          (XmTextType)_XmEntryTextTypeGet(entry),
                                                                          len,
                                                                          GetFont(rt, entry));
      }
...
_XmStringCacheFree(_XmStringCache caches)
{
   ...
   while (current)
   {
      ...
      if (prev)
      {
         if (prev->cache_type == _XmRENDERING_CACHE && ((_XmStringRenderingCache)prev)->rendition != NULL)
   ...
   switch (type)
   {
      case _XmSCANNING_CACHE:
      {
         ...
         if (d)
         {
            ...
            while (cache && !(cache->cache_type == type && (XmDirectionMatch(((_XmStringScanningCache)cache)->prim_dir, d))))
               cache = cache->next;
            if (!cache && create)
            {
      ...
      case _XmRENDERING_CACHE:
      {
         ...
         {
            cache = _XmEntryCacheGet(entry);
            while (cache && !(cache->cache_type == type && ((_XmStringRenderingCache)cache)->rt == rt))
               cache = cache->next;
            if (!cache && create)
            {
                  ...
                  int            field)
{
   ...
   cache = (_XmStringRenderingCache)CacheGet(entry, _XmRENDERING_CACHE, False, (XtPointer)rt);
   if (!cache)
   {
      if (entry && _XmEntryUnoptimized(entry) && (field == _XmCACHE_DIRTY))
         return (XtPointer)True;
...
{
   _XmStringRenderingCache cache;
   cache = (_XmStringRenderingCache)CacheGet(entry, _XmRENDERING_CACHE, True, (XtPointer)rt);
   if (!cache)
      return;
   switch (field)
   {
   ...
   Dimension    *height)
{
   ...
   *width = 0, *height = 0;
   ...
   else
   {
      ...
      rend = &tmp;
      /* Initialize for tabs. */
      d                    = (_XmRTDisplay(rendertable) == NULL) ? _XmGetDefaultDisplay()
      ...
      for (j = 0; j < _XmStrLineCountGet(string); j++)
      {
         ...
         LineMetrics(line, rendertable, &rend, NULL, XmLEFT_TO_RIGHT, &cur_width, &cur_height, &asc, &dsc);
...
XmStringIsVoid(XmString string)
{
   ...
   while ((type = XmeStringGetComponent(&stack_context, TRUE, FALSE, &len, &val)) != XmSTRING_COMPONENT_END)
   {
      switch (type)
      {
...
EntryCvtToOpt(_XmStringEntry entry)
{
   ...
   if (_XmEntryUnoptimized(entry))
   {
      if (_XmEntryPushGet(entry) || _XmEntryPopGet(entry) || _XmUnoptSegRendBeginCount(entry) > 1 || _XmUnoptSegRendEndCount(entry) > 1 || _XmEntryTabsGet(entry) > 7 || _XmEntryByteCountGet(entry) > 255 || (_XmUnoptSegRendBeginCount(entry) && _XmStringIndexCacheTag(_XmEntryRendBeginGet(entry, 0), XmSTRING_TAG_STRLEN) >= REND_INDEX_MAX) || (_XmUnoptSegRendEndCount(entry) && _XmStringIndexCacheTag(_XmEntryRendEndGet(entry, 0), XmSTRING_TAG_STRLEN) >= REND_INDEX_MAX) || (_XmUnoptSegRendBeginCount(entry) && _XmUnoptSegRendEndCount(entry) && _XmEntryRendEndGet(entry, 0) != _XmEntryRendBeginGet(entry, 0)) || (_XmUnoptSegTag(entry) && _XmStringIndexCacheTag(_XmUnoptSegTag(entry), XmSTRING_TAG_STRLEN) >= TAG_INDEX_MAX))
         return NULL;
      ...
      _XmEntryTagIndex(new_entry) = (_XmUnoptSegTag(entry) ? _XmStringIndexCacheTag(_XmUnoptSegTag(entry), XmSTRING_TAG_STRLEN)
                                                           : TAG_INDEX_UNSET);
      _XmEntryByteCountSet(new_entry, _XmUnoptSegByteCount(entry));
      _XmEntryRendIndex(new_entry) = (_XmUnoptSegRendBeginCount(entry) ? _XmStringIndexCacheTag(_XmEntryRendBeginGet(entry, 0),
                                                                                                XmSTRING_TAG_STRLEN)
                                                                       : (_XmUnoptSegRendEndCount(entry) ? _XmStringIndexCacheTag(_XmEntryRendEndGet(entry, 0),
                                                                                                                                  XmSTRING_TAG_STRLEN)
                                                                                                         : REND_INDEX_UNSET));
...
EntryCvtToUnopt(_XmStringEntry entry)
{
   ...
   {
      _XmUnoptSegRendBegins(new_entry)    = (XmStringTag *)XtMalloc(sizeof(XmStringTag));
      _XmUnoptSegRendBegins(new_entry)[0] = _XmEntryRendBeginGet(entry, 0);
   }
   if (_XmEntryRendEndCountGet(entry))
   {
      _XmUnoptSegRendEnds(new_entry)    = (XmStringTag *)XtMalloc(sizeof(XmStringTag));
   ...
   Dimension     *under_end)
{
   ...
   * it as if it was set to XmFONTLIST_DEFAULT_TAG. */
   ...
   if (!((seg_tag == _XmEntryTag(under_seg)) || ((strcmp(seg_tag, XmFONTLIST_DEFAULT_TAG) == 0) && _XmStringIsCurrentCharset(_XmEntryTag(under_seg))) || ((strcmp(_XmEntryTag(under_seg), XmFONTLIST_DEFAULT_TAG) == 0) && _XmStringIsCurrentCharset(seg_tag))))
      return;
      ...
      if (one_byte)
      {
         for (i = 0; i <= max; i++)
         {
            ...
            { /* found it */
               if (begin == 0)
                  *under_begin = x;
               else if (_XmRendFontType(entry) == XmFONT_IS_FONT)
                  *under_begin = x + abs(XTextWidth(font_struct, a, begin));
#if USE_XFT
               else
               {
                  XGlyphInfo ext;
                  XftTextExtentsUtf8(_XmRendDisplay(entry), _XmRendXftFont(entry), (FcChar8 *)a, begin, &ext);
               ...
               if (width == 0)
               {
                  ...
                  {
                     XGlyphInfo ext;
                     XftTextExtentsUtf8(_XmRendDisplay(entry), _XmRendXftFont(entry), (FcChar8 *)b, under_seg_len, &ext);
                     width = ext.xOff;
                  }
#endif
         ...
         for (i = 0; i <= max; i += 2)
         {
            ...
            { /* found it */
               if (begin == 0)
                  ...
                  *under_begin = x + abs(XTextWidth16(font_struct, (XChar2b *)a, begin / 2));
#if USE_XFT
               else
               {
                  XGlyphInfo ext;
                  XftTextExtents16(_XmRendDisplay(entry), _XmRendXftFont(entry), (FcChar16 *)a, begin, &ext);
                  *under_begin = x + ext.xOff;
               }
#endif
               width = _XmEntryWidthGet((_XmStringEntry)under_seg, rt);
               ...
               {
                  if (_XmRendFontType(entry) == XmFONT_IS_FONT)
                     width = abs(XTextWidth16(font_struct, (XChar2b *)b, under_seg_len / 2));
#if USE_XFT
                  else
                  {
                     XGlyphInfo ext;
                     XftTextExtents16(_XmRendDisplay(entry), _XmRendXftFont(entry), (FcChar16 *)b, under_seg_len, &ext);
                     width = ext.xOff;
                  }
#endif
                  _XmEntryWidthSet((_XmStringEntry)under_seg, rt, width);
   ...
   else
   {
      ...
      for (i = 0; i <= max; i += len_a)
      {
         ...
         { /* found it */
#if XM_UTF8
            Boolean utf8 = ((_XmEntryTextTypeGet(seg) == XmCHARSET_TEXT) && (((_XmEntryTag((_XmStringEntry)seg) == XmFONTLIST_DEFAULT_TAG) && _XmStringIsCurrentCharset("UTF-8")) || (strcmp(seg_tag, "UTF-8") == 0)));
#else
            Boolean utf8 = False;
#endif
            if (begin == 0)
            ...
            {
               *under_begin = x + abs(XwcTextEscapement(font_set, (wchar_t *)a, begin / sizeof(wchar_t)));
            }
            else
            {
#if XM_UTF8
...
#endif
                  *under_begin = x + abs(XmbTextEscapement(font_set, a, begin));
            }
            width = _XmEntryWidthGet((_XmStringEntry)under_seg, rt);
            if (width == 0)
            {
               ...
               width = (type == XmWIDECHAR_TEXT)
                        ? abs(XwcTextEscapement(font_set, (wchar_t *)b, under_seg_len / sizeof(wchar_t)))
                        : (utf8
                    ...
                    Boolean         colors_set)
{
   ...
   if (mode == XmHIGHLIGHT_SECONDARY_SELECTED)
   {
      ...
      XDrawLine(d, w, gc, x, y + SINGLE_OFFSET, x + width - 1, y + SINGLE_OFFSET);
   }
   else
   {
      XGetGCValues(d, gc, GCLineStyle, &current_gcv);
      ...
      {
         if ((under == XmSINGLE_DASHED_LINE) || (under == XmDOUBLE_DASHED_LINE))
         ...
         if ((under == XmSINGLE_LINE) || (under == XmSINGLE_DASHED_LINE))
         {
            XDrawLine(d, w, gc, x, y + SINGLE_OFFSET, (x + width - 1), y + SINGLE_OFFSET);
         }
         else if ((under == XmDOUBLE_LINE) || (under == XmDOUBLE_DASHED_LINE))
         {
            XSegment segs[2];
      ...
      {
         if ((thru == XmSINGLE_DASHED_LINE) || (thru == XmDOUBLE_DASHED_LINE))
            style = LineDoubleDash;
         else
            style = LineSolid;
         if (cur_style != style)
         ...
         if ((thru == XmSINGLE_LINE) || (thru == XmSINGLE_DASHED_LINE))
         {
            XDrawLine(d, w, gc, x, (y + descender - height / 2 - 1), (x + width - 1), (y + descender - height / 2 - 1));
         }
         else if ((thru == XmDOUBLE_LINE) || (thru == XmDOUBLE_DASHED_LINE))
         {
            XSegment segs[2];
            segs[0].x1 = segs[1].x1 = x;
            segs[0].x2 = segs[1].x2 = x + width - 1;
   ...
   if ((cur_style != old_style) && ((old_style == LineSolid) || (old_style == LineOnOffDash) || (old_style == LineDoubleDash)))
   {
                     ...
                     Dimension        descender)
{
   ...
   if (seg_len > 0)
   {
      multibyte = (((text_type == XmMULTIBYTE_TEXT) || (text_type == XmCHARSET_TEXT)) && (font_type == XmFONT_IS_FONTSET));
      widechar  = ((text_type == XmWIDECHAR_TEXT) && (font_type == XmFONT_IS_FONTSET));
#if XM_UTF8
      utf8 = ((text_type == XmMULTIBYTE_TEXT || text_type == XmCHARSET_TEXT) && (font_type == XmFONT_IS_FONTSET || font_type == XmFONT_IS_XFT || (font_type == XmFONT_IS_FONT && _XmIsISO10646(d, _XmRendFont(rend)))) && (((_XmEntryTag((_XmStringEntry)seg) == XmFONTLIST_DEFAULT_TAG && (_XmStringIsCurrentCharset("UTF-8"))) || ((_XmEntryTagIndex(seg) != TAG_INDEX_UNSET && strcmp(_XmEntryTag((_XmStringEntry)seg), "UTF-8") == 0)))));
#else
      utf8 = False;
#endif
      gc = _XmRendGC(rend);
		 ...
		 */
            {
               ...
               SubStringPosition((!text16), rendertable, rend, (_XmStringEntry)seg, (_XmStringEntry)&under_seg, x, &under_begin, &under_end);
            }
            else
            {
               imm                                     = _XmEntryImm((_XmStringEntry)*underline);
               _XmEntryImm((_XmStringEntry)*underline) = TRUE;
               SubStringPosition((!text16), rendertable, rend, (_XmStringEntry)seg, (_XmStringEntry)*underline, x, &under_begin, &under_end);
         ...
         else
         {
            ...
            if ((_XmStrEntryCount(*underline) > 0) && (_XmEntrySegmentCountGet(line) > 0))
            {
...
#endif
      {
		 ...
		 * twice - for getting extents and drawing */
               ...
               Xutf8DrawImageString(d, w, (XFontSet)_XmRendFont(rend), gc, x, y, draw_text, seg_len);
#endif
            else if (multibyte)
               XmbDrawImageString(d, w, (XFontSet)_XmRendFont(rend), gc, x, y, draw_text, seg_len);
            else if (widechar)
               XwcDrawImageString(d, w, (XFontSet)_XmRendFont(rend), gc, x, y, (wchar_t *)draw_text, (seg_len / sizeof(wchar_t)));
            else
		 ...
		 * twice - for getting extents and drawing */
               ...
               Xutf8DrawString(d, w, (XFontSet)_XmRendFont(rend), gc, x, y, draw_text, seg_len);
#endif
            else if (multibyte)
               XmbDrawString(d, w, (XFontSet)_XmRendFont(rend), gc, x, y, draw_text, seg_len);
            else if (widechar)
               XwcDrawString(d, w, (XFontSet)_XmRendFont(rend), gc, x, y, (wchar_t *)draw_text, (seg_len / sizeof(wchar_t)));
            else
      ...
      if ((*underline != NULL) && (under_begin != under_end))
      {
         *underline = (_XmString)NULL; /* only once */
         XDrawLine(d, w, gc, under_begin, (y + descender), under_end, (y + descender));
      }
      _XmStringDrawLining(d, w, x, y, width, height, descender, rend, XmUNSPECIFIED_PIXEL, XmHIGHLIGHT_NORMAL, TRUE);
                 ...
                 int         depth)
{
   ...
   while (*line_index < nline)
   {
      ...
      while (*seg_index < nseg)
      {
         ...
         if (_XmEntryPushGet((_XmStringEntry)seg) && !_XmEntryPopGet((_XmStringEntry)seg))
         {
            push_line = *line_index;
            (*seg_index)++;
            _XmEntryLayoutSet(seg, p_direction, (long)_XmEntryPushGet((_XmStringEntry)seg));
            _XmEntryLayoutDepthSet(seg, p_direction, (long)++depth);
            recursive_layout(string, line_index, seg_index, _XmEntryPushGet((_XmStringEntry)seg), p_direction, depth);
	   ...
	   */
         ...
         else if (!_XmEntryPushGet((_XmStringEntry)seg) && _XmEntryPopGet((_XmStringEntry)seg))
         {
                ...
                XmDirection direction)
{
   ...
   while (line_index < _XmStrLineCountGet(string) && seg_index < _XmEntrySegmentCountGet(_XmStrEntry(string)[line_index]))
   {
      line = _XmStrEntry(string)[line_index];
      seg  = _XmEntrySegmentGet(line)[seg_index];
   ...
   Dimension      opt_height)
{
   ...
   if (opt)
   {
      ...
      prev_val = x;
      tab_cnt  = 0;
      if ((tab != NULL) && (_XmEntryTabsGet((_XmStringEntry)&segm) != 0) && (tab_cnt < _XmTabLCount(tl)) && _XmEntryDirectionGet((_XmStringEntry)&segm) != XmSTRING_DIRECTION_R_TO_L)
      {
         for (i = 0;
              (i < _XmEntryTabsGet((_XmStringEntry)&segm)) && (tab_cnt < _XmTabLCount(tl));
              i++, tab = _XmTabNext(tab), tab_cnt++)
         {
      ...
      _XmStringDrawSegment(d, w, x, y, opt_width, opt_height, (_XmStringNREntry)&segm, *scr_rend, rendertable, image, underline, descender);
   }
   else
   {
      _XmStringNREntry seg;
      ...
      if (XmDirectionMatch(lay_dir, XmLEFT_TO_RIGHT))
      {
         ...
         ok = SpecifiedSegmentExtents((_XmStringEntry)seg, rendertable, scr_rend, base, XmSTRING_MIDDLE_SEG, &width, &height, NULL, NULL);
         if (*scr_rend != NULL) tl = _XmRendTabs(*scr_rend);
         tab      = (tl == NULL) ? NULL : _XmTabLStart(tl);
         prev_val = x;
         tab_cnt  = 0;
         while (seg != NULL)
         {
            ...
            if ((tab != NULL) && (_XmEntryTabsGet((_XmStringEntry)seg) != 0) && (tab_cnt < _XmTabLCount(tl)))
            {
               ...
               _XmStringDrawLining(d, w, start_x, y, (x - start_x), height, descender, *scr_rend, XmUNSPECIFIED_PIXEL, XmHIGHLIGHT_NORMAL, FALSE);
            }
            if (ok)
               _XmStringDrawSegment(d, w, x, y, width, height, seg, *scr_rend, rendertable, image, underline, descender);
            x += width;
            if (set_direction)
            {
               _XmEntryDirectionSet((_XmStringEntry)seg,
      ...
      else
      {
         ...
         while (seg != NULL)
         {
            ...
            if ((tab != NULL) && (_XmEntryTabsGet((_XmStringEntry)seg) != 0) && (tab_cnt < _XmTabLCount(tl)))
            {
               ...
               _XmStringDrawLining(d, w, x, y, (start_x - x), height, descender, *scr_rend, XmUNSPECIFIED_PIXEL, XmHIGHLIGHT_NORMAL, FALSE);
            }
            x -= width;
            if (ok)
               _XmStringDrawSegment(d, w, x, y, width, height, seg, *scr_rend, rendertable, image, underline, descender);
   ...
   XmFontType    font_type)
{
   ...
   if ((clip != NULL) && (!*restore))
      if (((*x) <= clip->x + clip->width) && (clip->x <= (*x) + line_width) && (y - line_height + descender <= clip->y + clip->height) && (clip->y <= y + descender))
      {
         *restore = TRUE;
#if USE_XFT
   ...
   _XmString     underline)
{
   ...
   _render(d, w, rendertable, rend, string, x, y, width, align, lay_dir, image, underline, clip);
   _XmProcessUnlock();
}
        ...
        XRectangle   *clip)
{
   ...
   if (_XmStrOptimized(string))
   {
      OptLineMetrics(rendertable, string, &rend2, rend, &line_width, &line_height, &ascender, &descender);
      y += ascender;
      if (line_width != 0)
      {
         draw_x = base_x; /* most left position */
         _calc_align_and_clip(d, w, gc, &draw_x, y, width, line_width, line_height, lay_dir, clip, align, descender, &restore_clip, _XmRendFontType(rend2));
         DrawLine(d, w, &screen, draw_x, y, (_XmStringEntry)string, &rend2, rend, rendertable, lay_dir, image, &underline, descender, TRUE, line_width, line_height);
      }
   ...
   else
   {
      ...
      for (i = 0; i < _XmStrLineCountGet(string); i++)
      {
         ...
         LineMetrics(line, rendertable, &rend1, rend, lay_dir, &line_width, &line_height, &ascender, &descender);
         y += ascender;
         if (line_width != 0)
         {
            draw_x = base_x; /* most left position */
            _calc_align_and_clip(d, w, gc, &draw_x, y, width, line_width, line_height, direction, clip, align, descender, &restore_clip, _XmRendFontType(rend1));
            DrawLine(d, w, &screen, draw_x, y, line, &rend1, rend, rendertable, lay_dir, image, &underline, descender, FALSE, line_width, line_height);
            val = 0;
...
_XmStringRender(Display      *d,
                ...
                unsigned char lay_dir)
{
   _render(d, w, rendertable, rend, string, x, y, width, align, lay_dir, FALSE, NULL, NULL);
}
   ...
   unsigned int   tag_index)
{
   ...
   else
   {
      ...
      _XmStrTagIndex((_XmString)string) = _XmStringIndexCacheTag((char *)tag, XmSTRING_TAG_STRLEN);
   }
   while (c < end)
   {
               ...
               XmStringDirection dir)
{
   ...
   if (!*unopt && (opt_seg = EntryCvtToOpt((_XmStringEntry)seg)))
      _XmStringSegmentNew(str, _XmStrImplicitLine(str) ? *lc : *sc, opt_seg, False);
   else
   ...
   Boolean        havetag)
{
   ...
   {
      tag                  = XmFONTLIST_DEFAULT_TAG;
      _XmUnoptSegTag(&seg) = _XmStringCacheTag((char *)(tag), XmSTRING_TAG_STRLEN);
   }
   ...
   while (c < end)
   {
      ...
      switch (*c)
      {
         ...
         case XmSTRING_COMPONENT_RENDITION_BEGIN:
            ...
            _XmUnoptSegRendBegins(&seg)[rend_cnt - 1] = _XmStringCacheTag((char *)(c + _asn1_size(length)),
                                                                          (int)length);
            break;
         case XmSTRING_COMPONENT_LOCALE:
            if (txt_seen)
         ...
         case XmSTRING_COMPONENT_RENDITION_END:
            ...
            _XmUnoptSegRendEnds(&seg)[rend_cnt - 1] = _XmStringCacheTag((char *)(c + _asn1_size(length)),
                                                                        (int)length);
            break;
...
XmCvtByteStreamToXmString(unsigned char *property)
{
   ...
   */
   ...
   while (continue_flag)
   {
      ...
      switch (*c_opt)
      {
         ...
         case XmSTRING_COMPONENT_RENDITION_BEGIN:
            ...
            {
               begin_seen  = TRUE;
               begin_index = _XmStringIndexCacheTag((char *)(c_opt + _asn1_size(length)),
                                                    (int)length);
         ...
         case XmSTRING_COMPONENT_TEXT:
            if (txt_seen || (((c_opt + length + _asn1_size(length)) < end) || (length >= (1 << BYTE_COUNT_BITS))))
               optimized = FALSE;
            txtlength = length;
            txt_seen  = TRUE;
            break;
         case XmSTRING_COMPONENT_RENDITION_END:
            ...
            else
            {
               end_seen  = TRUE;
               txt_seen  = TRUE;
               end_index = _XmStringIndexCacheTag((char *)(c_opt + _asn1_size(length)),
                                                  (int)length);
   ...
   if (optimized)
      ...
      string = _XmStringNonOptCreate(c, end, havetag);
   _XmProcessUnlock();
   return (string);
}
...
_XmStringEntryCopy(_XmStringEntry entry)
{
   ...
   switch (_XmEntryType(entry))
   {
      ...
      case XmSTRING_ENTRY_UNOPTIMIZED:
      {
         ...
         {
            _XmUnoptSegRendBegins(new_entry) = (XmStringTag *)XtMalloc(_XmUnoptSegRendBeginCount(entry) * sizeof(XmStringTag));
         ...
         {
            _XmUnoptSegRendEnds(new_entry) = (XmStringTag *)XtMalloc(_XmUnoptSegRendEndCount(entry) * sizeof(XmStringTag));
...
_XmEntryTag(_XmStringEntry entry)
{
   ...
   return rettag;
#else
   return (_XmEntryOptimized(entry) ? (_XmEntryTagIndex(entry) != TAG_INDEX_UNSET ? _XmStringIndexGetTag(_XmEntryTagIndex(entry)) : NULL) : _XmUnoptSegTag(entry));
#endif
}
...
_XmEntryTagSet(_XmStringEntry entry, XmStringTag tag)
{
   if (_XmEntryOptimized(entry))
   {
      if (tag == NULL)
         ...
         _XmEntryTagIndex(entry) = _XmStringIndexCacheTag(tag, XmSTRING_TAG_STRLEN);
   }
   else
   {
...
{
   return (_XmEntryUnoptimized(entry) ? ((_XmStringEntry)(entry))->unopt_single.push_before : False);
}
...
{
   return (_XmEntryOptimized(entry) ? (!_XmEntryImm(entry) ? ((_XmStringOptSeg)(entry))->data.text : (XtPointer)((_XmStringOptSeg)(entry))->data.chars) : (((_XmStringUnoptSeg)(entry))->data.text));
}

unsigned int
_XmEntryDirectionGet(_XmStringEntry entry)
{
   return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.str_dir : ((_XmStringEntry)(entry))->unopt_single.str_dir);
}
...
_XmEntryDirectionSet(_XmStringEntry entry,
                     XmDirection    val)
{
   (_XmEntryOptimized(entry) ? (((_XmStringEntry)entry)->single.str_dir = val) : (((_XmStringEntry)entry)->unopt_single.str_dir = val));
}

unsigned int
_XmEntryTextTypeGet(_XmStringEntry entry)
{
   return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.text_type : ((_XmStringEntry)(entry))->unopt_single.text_type);
}

_XmStringCache
_XmEntryCacheGet(_XmStringEntry entry)
{
   return (_XmEntryUnoptimized(entry) ? ((_XmStringUnoptSeg)(entry))->cache : NULL);
}

unsigned char
_XmEntryRendEndCountGet(_XmStringEntry entry)
{
   return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.rend_end : _XmUnoptSegRendEndCount(entry));
}

unsigned char
_XmEntryRendBeginCountGet(_XmStringEntry entry)
{
   return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.rend_begin : _XmUnoptSegRendBeginCount(entry));
}

Boolean
_XmEntryPopGet(_XmStringEntry entry)
{
   return (_XmEntryUnoptimized(entry) ? ((_XmStringEntry)(entry))->unopt_single.pop_after : False);
}
...
_XmEntryRendEndGet(_XmStringEntry entry,
                   int            n)
{
   return ((_XmEntryRendEndCountGet(entry) > n) ? (_XmEntryOptimized(entry) ? (_XmEntryRendIndex(entry) != REND_INDEX_UNSET ? _XmStringIndexGetTag(_XmEntryRendIndex(entry)) : NULL) : (((_XmStringUnoptSeg)(entry))->rend_end_tags)[n]) : NULL);
}
...
_XmEntryRendBeginGet(_XmStringEntry entry,
                     int            n)
{
   return ((_XmEntryRendBeginCountGet(entry) > n) ? (_XmEntryOptimized(entry) ? (_XmEntryRendIndex(entry) != REND_INDEX_UNSET ? _XmStringIndexGetTag(_XmEntryRendIndex(entry)) : NULL) : (((_XmStringUnoptSeg)(entry))->rend_begin_tags)[n]) : NULL);
}
      ...
      else
      {
         if (n >= _XmUnoptSegRendEndCount(entry))
         {
            ...
            _XmUnoptSegRendEnds(entry) = (XmStringTag *)XtRealloc((char *)_XmUnoptSegRendEnds(entry),
                                                                  _XmUnoptSegRendEndCount(entry) * sizeof(XmStringTag));
         }
   ...
   else
   {
      if (tag == NULL)
      {
         if (_XmEntryRendBeginCountGet(entry) > n)
         {
            for (i = n; i < _XmEntryRendBeginCountGet(entry) - 1; i++)
               _XmUnoptSegRendBegins(entry)[i] = _XmUnoptSegRendBegins(entry)[i + 1];
            _XmUnoptSegRendBeginCount(entry)--;
            _XmUnoptSegRendBegins(entry)[_XmEntryRendBeginCountGet(entry)] = NULL;
            if (_XmEntryRendBeginCountGet(entry) == 0)
            {
      ...
      else
      {
         if (n >= _XmUnoptSegRendBeginCount(entry))
         {
            ...
            _XmUnoptSegRendBeginCount(entry)++;
            _XmUnoptSegRendBegins(entry) = (XmStringTag *)XtRealloc((char *)_XmUnoptSegRendBegins(entry),
...
{
   return (_XmEntryOptimized(entry) ? ((_XmStringEntry)(entry))->single.tabs_before : ((_XmStringEntry)(entry))->unopt_single.tabs_before);
}
   ...
   switch (_XmRendFontType(rend))
   {
      case XmFONT_IS_FONT:
      {
         ...
         else
         {
            if (byte_count >= 1)
            {
               XTextExtents(font_struct, (char *)text, byte_count, &dir, &asc, &desc, &char_ret);
               wid = ComputeWidth(which_seg, char_ret);
               /* pir 2967 */
               if (wid == 0)
      ...
      case XmFONT_IS_FONTSET:
      {
         if (byte_count >= 1)
         {
            ...
            if (type == XmWIDECHAR_TEXT)
               XwcTextExtents(font_set, (wchar_t *)text, byte_count / sizeof(wchar_t), &ink, &logical);
            else
            {
#if XM_UTF8
               if (utf8)
...
#endif
                  XmbTextExtents(font_set, (char *)text, byte_count, &ink, &logical);
            }
            if (logical.height == 0)
            {
             ...
             XCharStruct   char_ret)
{
   ...
   switch (which)
   {
      ...
      case XmSTRING_LAST_SEG:
         wid = ((char_ret.width > char_ret.rbearing) ? char_ret.width : char_ret.rbearing);
         break;
...
_XmStringSegmentExtents(_XmStringEntry entry,
                        ...
                        Dimension     *descent)
{
   return (SpecifiedSegmentExtents(entry, rendertable, rend_in_out, base, XmSTRING_MIDDLE_SEG, width, height, ascent, descent));
}
                        ...
                        Dimension     *descent)
{
   ...
   */
   render_cache = (_XmStringRenderingCache)CacheGet(entry, _XmRENDERING_CACHE, False, (XtPointer)rendertable);
   ...
   if (cached_rend == NULL) /* Update *rend_in_out. */
   {
      ...
      /* 1. Find font from rendition tags. */
      /* 2. Find font from locale/charset tag. */
      if ((_XmRendTag(*rend_in_out) != entry_tag) || (count != 0) || _XmRendHadEnds(*rend_in_out))
      {
         *rend_in_out             = _XmRenditionMerge(d, rend_in_out, base, rendertable, entry_tag, tags, tag_count, (render_cache != NULL));
         _XmRendTag(*rend_in_out) = entry_tag;
      }
      /* 3. Default rendition. */
      if (_XmRendFont(*rend_in_out) == NULL && _XmRendXftFont(*rend_in_out) == NULL)
      {
         def_tag = ((_XmEntryTextTypeGet(entry) == XmCHARSET_TEXT) ? XmFONTLIST_DEFAULT_TAG : _MOTIF_DEFAULT_LOCALE);
         rend    = _XmRenditionMerge(d, rend_in_out, base, rendertable, def_tag, NULL, 0, (render_cache != NULL));
         if ((rend != NULL) && (_XmRendFont(rend) == NULL) && (_XmRendXftFont(rend) == NULL) && ((def_rend = _XmRenderTableFindRendition(rendertable, def_tag, TRUE, FALSE, FALSE, NULL)) != NULL))
         /* Call noFontCallback. */
         {
            XmDisplay               dsp;
            XmDisplayCallbackStruct cb;
         ...
         /* 4a. Take the first one */
         if ((rend == NULL) && ((_XmEntryTextTypeGet(entry) == XmCHARSET_TEXT) || ((_XmEntryTextTypeGet(entry) == XmMULTIBYTE_TEXT) && (entry_tag == XmFONTLIST_DEFAULT_TAG))) && (rendertable != NULL) && (_XmRTCount(rendertable) > 0))
            rend = _XmRenditionMerge(d, rend_in_out, base, rendertable, NULL, NULL, 0, (render_cache != NULL));
         if ((rend != NULL) && (_XmRendFont(rend) == NULL) && (_XmRendXftFont(rend) == NULL))
         /* Call noFontCallback. */
         {
            XmDisplay               dsp;
   ...
   if (can_do)
   {
      ...
      ComputeMetrics(*rend_in_out, _XmEntryTextGet(entry), _XmEntryByteCountGet(entry), (XmTextType)_XmEntryTextTypeGet(entry), which_seg, &w, &h, &asc, &dsc,
#if XM_UTF8
                     _XmEntryType(entry) == XmCHARSET_TEXT && (_XmEntryTag(entry) == XmFONTLIST_DEFAULT_TAG && (_XmStringIsCurrentCharset("UTF-8") || (_XmEntryTagIndex(entry) != TAG_INDEX_UNSET && strcmp(_XmEntryTag(entry), "UTF-8") == 0)))
#else
                     False
#endif
      );
...
static void
_parse_locale(char *str, int *indx, int *len)
{
   char *temp;
   int   start;
   int   end;
      ...
      int      lines)
{
   ...
   if (_XmStrOptimized(string))
   {
      _XmStringOpt n_o_string = (_XmStringOpt)_XmStrMalloc(sizeof(_XmStringOptRec) + _XmStrByteCount(string) - TEXT_BYTES_IN_STRUCT);
      memcpy(n_o_string, string, sizeof(_XmStringOptRec) + _XmStrByteCount(string) - TEXT_BYTES_IN_STRUCT);
   ...
   *line_count = 0;
   ...
   else if (*line_count > 1)
   {
      ...
      *baselines = (Dimension *)XtMalloc(*line_count * sizeof(Dimension));
      ...
      bzero((char *)&scratch, sizeof(_XmRenditionRec));
      _XmRendDisplay(rend) = ((_XmRTDisplay(rendertable) == NULL) ? _XmGetDefaultDisplay() : _XmRTDisplay(rendertable));
      _XmStringLayout(string, XmLEFT_TO_RIGHT);
   ...
   XRectangle   *clip)
{
   ...
   _XmAppLock(app);
   if (string)
      _draw(d, w, rendertable, (_XmString)string, gc, x, y, width, align, lay_dir, clip, FALSE, NULL);
   _XmAppUnlock(app);
}
   ...
   XRectangle   *clip)
{
   ...
   _XmAppLock(app);
   if (string)
      _draw(d, w, rendertable, (_XmString)string, gc, x, y, width, align, lay_dir, clip, TRUE, NULL);
   _XmAppUnlock(app);
}
   ...
   XmString      under)
{
   ...
   _XmAppLock(app);
   if (str)
      _draw(d, w, fntlst, (_XmString)str, gc, x, y, width, align, lay_dir, clip, FALSE, (_XmString)under);
   _XmAppUnlock(app);
}
      ...
      switch (*c)
      {
         ...
         case XmSTRING_COMPONENT_LOCALE_TEXT:
            if (*c == XmSTRING_COMPONENT_TEXT)
               ...
               printf("\tLocalized text component\n");
            printf("\tlength = %d\n", length);
            printf("\tvalue  = <");
            for (k = 0; k < length; k++)
               printf("%c", *(c + _asn1_size(length) + k));
   ...
   _XmString string)
{
   ...
   if (_XmStrOptimized(string))
   {
      ...
      printf("\t    rend_index    = %4d <%s>\n", _XmStrRendIndex(string), (_XmStrRendTagGet(string) ? _XmStrRendTagGet(string) : "(unset)"));
   ...
   XmStringTag *pTagOut)
{
   ...
   *pTagOut  = NULL;
   if (str)
   {
      ...
      while ((type = XmeStringGetComponent(&stack_context, FALSE, FALSE, &len, &val)) != XmSTRING_COMPONENT_END)
      {
         switch (type)
         {
            case XmSTRING_COMPONENT_TAG:
   ...
   Widget   shell)
{
   ...
   if (XtIsWMShell(shell))
   {
      ...
      if (_XmStringSingleSegment(xmstr, &text, &tag))
      {
         ...
         else if ((tag != NULL) && (strcmp(XmFONTLIST_DEFAULT_TAG, tag) == 0))
         {
      ...
      if (!text)
      {
         ...
         if (XmCvtXmStringToText(XtDisplay(shell), NULL, NULL, &from, &to, NULL))
         {
            text     = to.addr;
      ...
      if (text)
      {
         XtSetArg(al[ac], XtNtitle, text);
         ++ac;
         XtSetArg(al[ac], XtNtitleEncoding, encoding);
         ++ac;
         XtSetArg(al[ac], XtNiconName, text);
         ++ac;
         XtSetArg(al[ac], XtNiconNameEncoding, encoding);
         ++ac;
              ...
              Boolean        dir_change)
{
   ...
   else if ((pattern->pattern_type == XmWIDECHAR_TEXT) && (type == XmWIDECHAR_TEXT))
   {
              ...
              Boolean       *terminate)
{
   ...
   if (action == XmINVOKE)
   {
      /* Resolve parse procs. */
      if (pat->parse_proc)
         action = (pat->parse_proc)((XtPointer *)ptr, text_end, type, tag, pat, length, &insertion, call_data);
                  ...
                  XtPointer    call_data)
{
   ...
   while (!halt && (wide_char ? *((wchar_t *)ptr) : *ptr))
   {
      ...
      {
         XmDirection xm_dir;
         if ((*init_char_proc)((XtPointer)ptr, type, tag, &index, &xm_dir) == Success)
            dir_ptr = ptr + index;
      }
      ...
      if (!has_dir_pattern && (ptr == dir_ptr))
      {
         ...
         advanced = parse_pattern(&result, &ptr, end_ptr, tag, type, default_dir_pattern, len, call_data, &halt);
      }
      ...
      for (index = 0; !advanced && !halt && (index < parse_count); index++)
      {
         ...
         {
            parse_unmatched(&result, &prev_ptr, type, ptr - prev_ptr);
            advanced = parse_pattern(&result, &ptr, end_ptr, tag, type, pat, len, call_data, &halt);
      ...
      while (!done)
      {
         ...
         switch (n_ctype)
         {
            ...
            case XmSTRING_COMPONENT_WIDECHAR_TEXT:
               ...
               else if ((tag_type == _XmStrContTagType(&n_context)) && (!_XmStrContTag(&n_context) || (tag == _XmStrContTag(&n_context)) || (strcmp(tag, _XmStrContTag(&n_context)) == 0)))
                  *next_text_match = True;
               else
   ...
   for (n_pat = 0; !match && (n_pat < parse_count); n_pat++)
   {
      ...
      if (unparse_is_plausible(pat))
      {
         ...
         for (n_comp = 0; match; n_comp++)
         {
            ...
            m_ctype = XmeStringGetComponent(&m_context, True, False, &m_length, &m_value);
            p_ctype = XmeStringGetComponent(&p_context, True, False, &p_length, &p_value);
            /* It's a match! */
            if (p_ctype == XmSTRING_COMPONENT_END)
               break;
            /* Comparison of text components always fails. */
            if ((p_ctype == XmSTRING_COMPONENT_TEXT) || (p_ctype == XmSTRING_COMPONENT_LOCALE_TEXT) || (p_ctype == XmSTRING_COMPONENT_WIDECHAR_TEXT))
            {
               pat->internal_flags = XmSTRING_UNPARSE_IMPLAUSIBLE;
               match               = False;
            }
            /* Bit-compare components. */
            else if ((m_ctype != p_ctype) || (m_length != p_length) || ((m_value != p_value) && memcmp(m_value, p_value, m_length)))
               match = False;
         }
         ...
         if (match)
         {
            ...
            if (pat->pattern_type == XmWIDECHAR_TEXT)
               unparse_text(result, length, output_type, XmSTRING_COMPONENT_WIDECHAR_TEXT, sizeof(wchar_t), pat->pattern);
            else
               unparse_text(result, length, output_type, XmSTRING_COMPONENT_TEXT, mblen((char *)pat->pattern, MB_CUR_MAX), pat->pattern);
            /* Skip all but the last matched component. */
            while (--n_comp > 0)
            {
               m_ctype = XmeStringGetComponent(context, True, False, &m_length, &m_value);
                ...
                XmParseModel parse_model)
{
   ...
   if ((tag_type == XmCHARSET_TEXT) && tag && ((tag == XmSTRING_DEFAULT_CHARSET) || (strcmp(tag, XmSTRING_DEFAULT_CHARSET) == 0)))
   ...
   {
      _XmStringContextReInit(&stack_context, string);
      check_unparse_models(&stack_context, tag, tag_type, parse_model, &prev_text_match, &next_text_match, &non_text_match);
   }
   while (!done)
   {
      /* Peek at the next component. */
      c_type = XmeStringGetComponent(&stack_context, False, False, &c_length, &c_value);
      switch (c_type)
      {
         case XmSTRING_COMPONENT_TEXT:
         case XmSTRING_COMPONENT_LOCALE_TEXT:
         case XmSTRING_COMPONENT_WIDECHAR_TEXT:
            ...
            if (next_text_match)
               unparse_text(&result, &length, output_type, c_type, c_length, c_value);
            /* Advance to the next component. */
            (void)XmeStringGetComponent(&stack_context, True, False, &c_length, &c_value);
         ...
         default:
            ...
            if (non_text_match)
               unparse_components(&result, &length, output_type, &stack_context, parse_table, parse_count);
            /* Advance to the next component. */
            if (!done)
               (void)XmeStringGetComponent(&stack_context, True, False, &c_length, &c_value);
            break;
   ...
   switch (output_type)
   {
      ...
      {
         wchar_t zero = 0;
         unparse_text(&result, &length, output_type, XmSTRING_COMPONENT_WIDECHAR_TEXT, sizeof(wchar_t), (XtPointer)&zero);
      }
      break;
      case XmCHARSET_TEXT:
      case XmMULTIBYTE_TEXT:
      case XmNO_TEXT:
         unparse_text(&result, &length, output_type, XmSTRING_COMPONENT_TEXT, 1, (XtPointer) "");
         break;
   }
   _XmProcessUnlock();
                        ...
                        XtPointer             value)
{
   ...
   switch (c_type)
   {
      case XmSTRING_COMPONENT_TAG:
         ...
         if ((value == XmSTRING_DEFAULT_CHARSET) || (strcmp((char *)value, XmSTRING_DEFAULT_CHARSET) == 0))
         {
   ...
   if (optimized)
   {
      ...
      str = (_XmString)_XmStrMalloc(sizeof(_XmStringOptRec) + (_XmStrByteCount((_XmString)&opt) ? (_XmStrByteCount((_XmString)&opt) - TEXT_BYTES_IN_STRUCT) : 0));
                      ...
                      XtPointer       *value)
{
   ...
   else
   {
      ...
      else
      {
         ...
         if (_XmEntryMultiple(line))
            seg = (_XmStringEntry)_XmEntrySegment(line)[_XmStrContCurrSeg(context)];
         else
            seg = line;
         /* Only lookup pop_dir when we need it. */
         /* Only lookup push_dir when we need it. */
   ...
   switch (state)
   {
      ...
      case BEGIN_REND_STATE:
         tmp_index = ((_XmStrContState(context) == BEGIN_REND_STATE) ? _XmStrContRendIndex(context) : 0);
         if (tmp_index < begin_count)
         {
            ...
            *length = strlen((char *)*value);
            if (update_context)
            {
               ...
               begin_context_rends(context, update_context, begin_rends + tmp_index, 1);
               _XmStrContState(context)     = BEGIN_REND_STATE;
      ...
      case TAG_STATE:
         ...
         if ((tag == XmSTRING_DEFAULT_CHARSET) || (tag && !strcmp((char *)tag, XmSTRING_DEFAULT_CHARSET)))
            tag = _XmStringGetCurrentCharset();
         if ((text_type != XmNO_TEXT && text_type != _XmStrContTagType(context)) || (tag && (tag != _XmStrContTag(context)) && (!_XmStrContTag(context) || strcmp(tag, _XmStrContTag(context)))))
         {
            skip = (tag == NULL);
	   ...
	   * context if necessary for GetNextSegment. */
            if ((tag == XmFONTLIST_DEFAULT_TAG) && (text_type == XmMULTIBYTE_TEXT))
            {
               skip                   = TRUE;
            ...
            if (!skip)
            {
               ...
               *length = strlen(tag);
               ...
               _XmProcessUnlock();
               return ((text_type == XmCHARSET_TEXT) ? XmSTRING_COMPONENT_TAG : XmSTRING_COMPONENT_LOCALE);
            }
         }
      ...
      case END_REND_STATE:
         tmp_index = ((_XmStrContState(context) == END_REND_STATE) ? _XmStrContRendIndex(context) : 0);
         if (tmp_index < end_count)
         {
            ...
            *length = strlen((char *)*value);
            if (update_context)
            {
               ...
               end_context_rends(context, update_context, end_rends + tmp_index, 1);
               _XmStrContState(context)     = END_REND_STATE;
               _XmStrContRendIndex(context) = tmp_index + 1;
            }
            _XmProcessUnlock();
      ...
      while (--n_tag >= 0)
         if (_XmStrContRendTags(context)[n_tag] == rends[n_rend])
         {
            ...
            for (i = n_tag; i < (_XmStrContRendCount(context) - 1); i++)
               _XmStrContRendTags(context)[i] = _XmStrContRendTags(context)[i + 1];
                 ...
                 XmStringTag rendition)
{
  ...
  */
   ...
   if (!_XmStrOptimized(result))
   {
      ...
      XmStringTag    cached_rend = _XmStringCacheTag(rendition, XmSTRING_TAG_STRLEN);
      int            n_line;
      _XmStringEntry line;
      _XmStringEntry seg;
      ...
      while (--n_line >= 0)
      {
         ...
         if (_XmEntrySegmentCountGet(line) > 0)
         {
            ...
            if (_XmEntryOptimized(seg))
            {
               ...
               if (((_XmEntryRendIndex(seg) == REND_INDEX_UNSET) || (_XmEntryRendIndex(seg) == rend_index)) && (rend_index < REND_INDEX_MAX))
               {
                  _XmEntryRendIndex(seg) = rend_index;
            ...
            else
            {
               ...
               _XmUnoptSegRendEnds(seg) = (XmStringTag *)
                  XtRealloc((char *)_XmUnoptSegRendEnds(seg),
                            (_XmUnoptSegRendEndCount(seg) + 1) * sizeof(XmStringTag));
                    ...
                    XmTextType  output_type)
{
  ...
  */
   ...
   result = XmStringUnparse(string, tag, tag_type, output_type, gen_table, table_size, XmOUTPUT_ALL);
   /*
                        ...
                        Cardinal       arg_count)
{
   ...
   for (i = 0; i < arg_count; i++)
   {
      ...
      if ((arg_name == XmNpattern) || (strcmp(arg_name, XmNpattern) == 0))
         mapping->pattern = (XtPointer)arg_list[i].value;
      else if ((arg_name == XmNpatternType) || (strcmp(arg_name, XmNpatternType) == 0))
         mapping->pattern_type = (XmTextType)arg_list[i].value;
      else if ((arg_name == XmNsubstitute) || (strcmp(arg_name, XmNsubstitute) == 0))
         mapping->substitute = XmStringCopy((XmString)arg_list[i].value);
      else if ((arg_name == XmNinvokeParseProc) || (strcmp(arg_name, XmNinvokeParseProc) == 0))
         mapping->parse_proc = (XmParseProc)arg_list[i].value;
      else if ((arg_name == XmNclientData) || (strcmp(arg_name, XmNclientData) == 0))
         mapping->client_data = (XtPointer)arg_list[i].value;
      else if ((arg_name == XmNincludeStatus) || (strcmp(arg_name, XmNincludeStatus) == 0))
