           ...
           unsigned long  length)
{
   ...
   seg_length = (data->gap_start - data->ptr) / (tw->text.char_size < 3 ? (int)tw->text.char_size : sizeof(wchar_t));
           ...
           XmTextBlock    block)            /* RETURN: text read in */
{
   ...
   if (block->length > 0)
   {
      ...
      {
         data->value      = XtRealloc(data->value, (unsigned)((block->length + 1) * (int)tw->text.char_size));
         data->old_length = block->length;
      }
      ...
      else
      {
         ...
         num_bytes     = _XmTextCharactersToBytes(data->value, block->ptr, block->length / char_size, (int)tw->text.char_size);
   ...
   if (initiator->text.wcs_modify_verify_callback)
   {
       ...
       * data there */
         ...
         *freeBlock = False;
         ...
         {
            newblock->ptr                                = (char *)XtMalloc((unsigned)(1 + wcs_tvcb.text->length) * (int)initiator->text.char_size);
            *freeBlock                                   = True;
         ...
         if ((delta > 0 && data->length + delta > data->maxallowed && (!UnderVerifyPreedit(initiator))) || newblock->length < 0)
         {
            if (newblock->ptr && newblock->ptr != block->ptr)
               XtFree(newblock->ptr);
            *freeBlock = False;
        ...
        Boolean         call_callbacks) /* unused */
{
   ...
   int                   gap_size;
   int                   old_maxlength;
   int                   char_size = (initiator->text.char_size < 3 ? (int)initiator->text.char_size : sizeof(wchar_t));
   if (*start == *end && block->length == 0) return EditReject;
   _XmTextValidate(start, end, data->length);
   block_num_chars = _XmTextCountCharacters(block->ptr, block->length);
   delta           = block_num_chars - (*end - *start);
   if (!data->editable || (delta > 0 && data->length + delta > data->maxallowed && (!UnderVerifyPreedit(initiator))))
      return EditError;
   /**********************************************************************/
   ...
   if (data->length + delta >= data->maxlength)
   {
      ...
      if (gap_end_offset != (old_maxlength * char_size))
         memmove(data->gap_end, data->ptr + gap_end_offset, (char_size * old_maxlength) - gap_end_offset);
   ...
   {
      data->gap_start += char_size * _XmTextBytesToCharacters(data->gap_start, &block->ptr[0], block_num_chars, False, (int)initiator->text.char_size);
   }
   ...
   initiator->text.output->DrawInsertionPoint(initiator,
                                              initiator->text.cursor_position,
                                              on);
   if (data->maxlength != TEXT_INITIAL_INCREM && ((data->maxlength > TEXT_INCREMENT && data->length <= data->maxlength - TEXT_INCREMENT) || data->length <= data->maxlength >> 1))
   {
              ...
              XmTextPosition     *last_char)
{
   ...
   while (position >= 0 && position <= data->length)
   {
      ...
      if (((dir == XmsdRight) && (position == data->length)) || ((dir == XmsdLeft) && (position == 0)))
         break;
      Increment(data, position, dir);
   }
             ...
             Time           set_time)
{
   ...
   if (!XtIsRealized((Widget)data->widgets[0]) || (left > right && !data->hasselection)) return;
                    ...
                    XEvent      *event)
{
   ...
   if (initiator->text.value_changed_callback)
      XtCallCallbackList((Widget)initiator,
                         initiator->text.value_changed_callback,
                         (XtPointer)&cb);
}
                      ...
                      Boolean is_wchar)
{
   ...
   if (is_wchar)
   {
      ...
      if (ret_value < 0)
         ...
         data->length = _XmTextBytesToCharacters(data->ptr, tmp_value, num_chars, False, max_char_size);
      XtFree(tmp_value);
   }
   else
   {
