...
#define MELT_TIME 50000L
    ...
    static void
    DoZapEffect(XtPointer clientData, XtIntervalId *id);
static void DoMeltEffect(XtPointer clientData, XtIntervalId *id);
static void GetIconPosition(XmDragOverShellWidget dos,
...
static void BlendIcon(XmDragOverShellWidget dos,
                      ...
                      GC maskGC,
                      GC pixmapGC);
static void MixedIconSize(XmDragOverShellWidget dos,
                          XmDragIconObject sourceIcon,
                          XmDragIconObject stateIcon,
                          XmDragIconObject opIcon,
                          Dimension *width,
                          Dimension *height);
static void DestroyMixedIcon(XmDragOverShellWidget dos, XmDragIconObject mixedIcon);
static void MixIcons(XmDragOverShellWidget dos,
                     XmDragIconObject sourceIcon,
                     XmDragIconObject stateIcon,
                     XmDragIconObject opIcon,
                     XmDragOverBlendRec *blendPtr,
                     Boolean clip);
static Boolean FitsInCursor(XmDragOverShellWidget dos,
                            XmDragIconObject sourceIcon,
                            XmDragIconObject stateIcon,
                            XmDragIconObject opIcon);
static Boolean GetDragIconColors(XmDragOverShellWidget dos);
static Cursor GetDragIconCursor(XmDragOverShellWidget dos,
                                XmDragIconObject sourceIcon,
                                XmDragIconObject stateIcon,
                                XmDragIconObject opIcon,
                                Boolean clip,
                                Boolean dirty);
static void Initialize(Widget req, Widget new_w, ArgList args, Cardinal *numArgs);
static Boolean SetValues(
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void DrawIcon(
    XmDragOverShellWidget dos, XmDragIconObject icon, Window window, Position x, Position y);
static void Redisplay(Widget wid, XEvent *event, Region region);
static void Destroy(Widget w);
static void ChangeActiveMode(XmDragOverShellWidget dos, unsigned char newActiveMode);
static void ChangeDragWindow(XmDragOverShellWidget dos);
...
externaldef(xmDragOvershellwidgetclass)
...
static Boolean CacheMixedIcon(XmDragOverShellWidget dos,
                              Cardinal depth,
                              Dimension width,
                              Dimension height,
                              XmDragIconObject sourceIcon,
...
static XmDragIconObject GetMixedIcon(XmDragOverShellWidget dos,
                                     Cardinal depth,
                                     Dimension width,
                                     Dimension height,
                                     XmDragIconObject sourceIcon,
                                     ...
                                     Position opY)
{
  ...
  for (cache_ptr = mixed_cache; cache_ptr; cache_ptr = cache_ptr->next) {
    if (cache_ptr->depth == depth && cache_ptr->width == width && cache_ptr->height == height &&
        ...
        cache_ptr->cursorBackground == dos->drag.cursorBackground &&
        cache_ptr->sourcePixmap == sourceIcon->drag.pixmap &&
        cache_ptr->sourceMask == sourceIcon->drag.mask && cache_ptr->sourceX == sourceX &&
        cache_ptr->sourceY == sourceY &&
        ((cache_ptr->statePixmap == 0 && !stateIcon) ||
         (stateIcon && cache_ptr->statePixmap == stateIcon->drag.pixmap &&
          cache_ptr->stateMask == stateIcon->drag.mask && cache_ptr->stateX == stateX &&
          cache_ptr->stateY == stateY)) &&
 ...
 *
 ***********************************************************************/
static void DoZapEffect(XtPointer clientData, XtIntervalId *id) /* unused */
{
  ...
  for (;; i++) {
    ...
    if (dos->drag.activeMode == XmDRAG_WINDOW) {
      ...
      XCopyArea(display,
                ...
                0);
      DrawIcon(dos,
               (dos->drag.rootBlend.mixedIcon ? dos->drag.rootBlend.mixedIcon :
 ...
 ***********************************************************************/
static void DoMeltEffect(XtPointer clientData, XtIntervalId *id) /* unused */
{
  XmDragOverShellWidget dos = (XmDragOverShellWidget)clientData;
  int iterations;
  int i = 0;
  ...
  if (dos->drag.activeMode == XmDRAG_WINDOW) {
    ...
    for (i = 0; i < iterations; i++) {
      XShapeCombineRectangles(
          XtDisplay(dos), XtWindow(dos), ShapeBounding, 0, 0, &rect, 1, ShapeSet, YXSorted);
    ...
    for (i = 0; i < iterations; i++) {
      XSetClipRectangles(XtDisplay((Widget)dos), draw_gc, 0, 0, rects, 4, Unsorted);
  ...
  switch ((int)icon->drag.attachment) {
    ...
    case XmATTACH_NORTH_WEST:
      ...
      *iconY = icon->drag.offset_y;
      break;
    case XmATTACH_NORTH:
      *iconX = ((Position)sourceIcon->drag.width / 2) + icon->drag.offset_x;
      *iconY = icon->drag.offset_y;
      break;
    case XmATTACH_NORTH_EAST:
      *iconX = ((Position)sourceIcon->drag.width) + icon->drag.offset_x;
      *iconY = icon->drag.offset_y;
      break;
    case XmATTACH_EAST:
      *iconX = ((Position)sourceIcon->drag.width) + icon->drag.offset_x;
      *iconY = ((Position)sourceIcon->drag.height / 2) + icon->drag.offset_y;
      break;
    case XmATTACH_SOUTH_EAST:
      *iconX = ((Position)sourceIcon->drag.width) + icon->drag.offset_x;
      *iconY = ((Position)sourceIcon->drag.height) + icon->drag.offset_y;
      break;
    case XmATTACH_SOUTH:
      *iconX = ((Position)sourceIcon->drag.width / 2) + icon->drag.offset_x;
      *iconY = ((Position)sourceIcon->drag.height) + icon->drag.offset_y;
    ...
    case XmATTACH_HOT: {
      ...
      if (ref->drag.rootBlend.mixedIcon) {
        *iconX = ref->drag.rootBlend.mixedIcon->drag.hot_x - ref->drag.rootBlend.sourceX -
                 icon->drag.hot_x;
        *iconY = ref->drag.rootBlend.mixedIcon->drag.hot_y - ref->drag.rootBlend.sourceY -
                 icon->drag.hot_y;
      }
      else if (ref->drag.cursorBlend.mixedIcon) {
        *iconX = ref->drag.cursorBlend.mixedIcon->drag.hot_x - ref->drag.cursorBlend.sourceX -
                 icon->drag.hot_x;
        *iconY = ref->drag.cursorBlend.mixedIcon->drag.hot_y - ref->drag.cursorBlend.sourceY -
                 icon->drag.hot_y;
      }
      else {
        Widget sourceWidget;
                     ...
                     Boolean clip)
{
  ...
  if (mixedIcon == NULL) {
    ...
    ac++;
    mixedIcon = blendPtr->mixedIcon = (XmDragIconObject)XmCreateDragIcon(
        (Widget)xmScreen, "mixedIcon", al, ac);
    if (need_mask) {
  ...
  if (blendPtr->gc == NULL) {
    ...
    vmask = GCBackground | GCForeground | GCFunction | GCClipXOrigin | GCClipYOrigin | GCClipMask |
  ...
  XFillRectangle(
      display, pixmap, blendPtr->gc, 0, 0, mixedIcon->drag.width, mixedIcon->drag.height);
  if (mask != XmUNSPECIFIED_PIXMAP) {
    if (cursorBlend->gc == NULL) {
      ...
      v.clip_x_origin = 0; /* pedantic */
      v.clip_y_origin = 0; /* pedantic */
      vmask = GCBackground | GCForeground | GCFunction | GCClipXOrigin | GCClipYOrigin |
              GCClipMask | GCGraphicsExposures | GCSubwindowMode;
  ...
  if (mixedIcon->drag.region != NULL) {
    ...
    XSubtractRegion(
        mixedIcon->drag.restore_region, mixedIcon->drag.region, mixedIcon->drag.restore_region);
  }
  if (do_cache) {
    CacheMixedIcon(dos,
                   depth,
...
static Boolean GetDragIconColors(XmDragOverShellWidget dos)
{
  ...
  if (dos->drag.rootBlend.gc == NULL) {
    ...
    v.clip_x_origin = 0; /* pedantic */
    v.clip_y_origin = 0; /* pedantic */
    vmask = GCBackground | GCForeground | GCClipXOrigin | GCClipYOrigin | GCClipMask |
            GCGraphicsExposures | GCSubwindowMode;
    dos->drag.rootBlend.gc = XtAllocateGC(
        (Widget)dos, DefaultDepthOfScreen(screen), vmask, &v, vmask, 0L);
  }
                                ...
                                Boolean dirty)
{
  ...
  if (stateIcon && stateIcon->drag.attachment == XmATTACH_HOT) {
    ...
    cursorCache = *cursorCachePtr;
    while (cursorCache) {
      if ((cursorCache->stateIcon == stateIcon) && (cursorCache->opIcon == opIcon) &&
  ...
  cursor = XCreatePixmapCursor(
      ...
      ((dos->drag.cursorBlend.mixedIcon->drag.mask == XmUNSPECIFIED_PIXMAP) ?
           None :
           dos->drag.cursorBlend.mixedIcon->drag.mask),
      &colors[0], /* foreground_color */
      &colors[1], /* background_color */
                       ...
                       Cardinal *numArgs) /* unused */
{
  ...
  dos->drag.cursorBlend.sourceIcon = dos->drag.cursorBlend.mixedIcon =
      dos->drag.rootBlend.sourceIcon = dos->drag.rootBlend.mixedIcon = NULL;
                         ...
                         Cardinal *num_args) /* unused */
{
  ...
  if (oldDos->drag.hotX != dos->drag.hotX || oldDos->drag.hotY != dos->drag.hotY ||
      oldDos->core.width != dos->core.width || oldDos->core.height != dos->core.height)
    dos->drag.holePunched = FALSE;
  /*
  ...
  if (oldDos->drag.mode != dos->drag.mode && dc->drag.blendModel != XmBLEND_NONE) {
    if ((dos->drag.mode == XmPIXMAP || dos->drag.mode == XmDRAG_WINDOW) &&
 ...
 *
 ***********************************************************************/
void _XmDragOverHide(Widget w, Position clipOriginX, Position clipOriginY, XmRegion clipRegion)
{
  XmDragOverShellWidget dos = (XmDragOverShellWidget)w;
  XmDragContext dc = (XmDragContext)XtParent(dos);
  Boolean clipped = False;
  if (dos->drag.isVisible && dc->drag.blendModel != XmBLEND_NONE &&
      dos->drag.activeMode != XmCURSOR)
  {
    ...
    if (dos->drag.activeMode != XmWINDOW) {
      if (clipRegion != None) {
        ...
        _XmRegionSetGCRegion(
            XtDisplay(w), dos->drag.rootBlend.gc, clipOriginX, clipOriginY, clipRegion);
      }
      else {
        XSetClipMask(XtDisplay(w), dos->drag.rootBlend.gc, None);
      }
 ...
 *
 ***********************************************************************/
void _XmDragOverShow(Widget w, Position clipOriginX, Position clipOriginY, XmRegion clipRegion)
{
  XmDragOverShellWidget dos = (XmDragOverShellWidget)w;
...
static void ChangeActiveMode(XmDragOverShellWidget dos, unsigned char newActiveMode)
{
  ...
  if ((dos->drag.activeMode = newActiveMode) == XmCURSOR) {
    ...
    if (dos->drag.activeMode == XmPIXMAP) {
      if (BackingPixmap(dos) == XmUNSPECIFIED_PIXMAP) {
        BackingPixmap(dos) = _XmAllocScratchPixmap(
            xmScreen, dos->core.depth, dos->core.width, dos->core.height);
      }
      XSetClipMask(display, draw_gc, None);
      XCopyArea(display,
                RootWindowOfScreen(XtScreen(dos)),
...
static void ChangeDragWindow(XmDragOverShellWidget dos)
{
  ...
  BlendIcon(
      dos, sourceIcon, mixedIcon, blend->sourceX, blend->sourceY, cursorBlend->gc, blend->gc);
  /*
...
void _XmDragOverMove(Widget w, Position x, Position y)
{
   ...
   */
  if (dos->drag.tmpPix == XmUNSPECIFIED_PIXMAP) {
    dos->drag.tmpPix = _XmAllocScratchPixmap(
        xmScreen, dos->core.depth, dos->core.width, dos->core.height);
  }
      ...
      y < BackingY(dos) + ((Position)dos->core.height))
  {
    ...
    if (mixedIcon->drag.restore_region) {
      ...
      XCopyArea(
          display, new_backing, root, draw_gc, 0, 0, dos->core.width, dos->core.height, x, y);
      XSetClipMask(display, draw_gc, None);
    }
  }
  else {
...
void _XmDragOverChange(Widget w, unsigned char dropSiteStatus)
{
  ...
  if (sourceIcon == NULL || XtScreenOfObject(XtParent(sourceIcon)) != XtScreen(w) ||
      ((sourceIcon->drag.depth != dos->core.depth) && (sourceIcon->drag.depth != 1)))
  {
  ...
  switch ((int)dc->drag.blendModel) {
    ...
    case XmBLEND_ALL:
      ...
      {
        opIcon = _XmScreenGetOperationIcon(w, dc->drag.operation);
    ...
    case XmBLEND_STATE_SOURCE:
      ...
      {
        stateIcon = _XmScreenGetStateIcon(w, dropSiteStatus);
   ...
   */
  dirty = (_XmDragIconIsDirty(sourceIcon) || (opIcon && _XmDragIconIsDirty(opIcon)) ||
           (stateIcon && _XmDragIconIsDirty(stateIcon)));
  doChange = GetDragIconColors(dos) || dos->drag.opIcon != opIcon ||
             dos->drag.stateIcon != stateIcon || dos->drag.rootBlend.sourceIcon != sourceIcon ||
             dirty;
  /*
  ...
  if (!usedSrcPixIcon &&
      (dos->drag.activeCursor = GetDragIconCursor(
           dos, sourceIcon, stateIcon, opIcon, False /* no clip */, dirty)) != None)
  {
    /*
  ...
  if (dos->drag.mode == XmCURSOR) {
     ...
     */
    dos->drag.activeCursor = GetDragIconCursor(
        dos, sourceIcon, stateIcon, opIcon, True /* clip */, dirty);
    _XmDragIconClean(sourceIcon, stateIcon, opIcon);
    if (dos->drag.activeMode != XmCURSOR) {
      _XmDragOverHide(w, 0, 0, None);
      dos->drag.activeMode = XmCURSOR;
 ...
 *
 ***********************************************************************/
void _XmDragOverSetInitialPosition(Widget w, Position initialX, Position initialY)
{
  XmDragOverShellWidget dos = (XmDragOverShellWidget)w;
...
static void InstallColormap(XmDragOverShellWidget dw)
{
  ...
  if (dw->drag.colormapShell) {
    ...
    status = XGetWMColormapWindows(
        XtDisplay(dw), XtWindow(dw->drag.colormapShell), &windowsReturn, &countReturn);
    /* if no property, just create one */
    if (!status) {
      Window windows[2];
      windows[0] = XtWindow(dw);
      windows[1] = XtWindow(dw->drag.colormapShell);
      XSetWMColormapWindows(XtDisplay(dw), XtWindow(dw->drag.colormapShell), windows, 2);
    }
    /* there was a property, add myself to the beginning */
      ...
      XSetWMColormapWindows(
          XtDisplay(dw), XtWindow(dw->drag.colormapShell), windows, countReturn + 1);
      XtFree((char *)windows);
    ...
    if (dw->drag.colormapOverride) {
      dw->drag.savedColormaps = XListInstalledColormaps(
          XtDisplay(dw), XtWindow(dw), &dw->drag.numSavedColormaps);
 ...
 * colormap
 */
static void UninstallColormap(XmDragOverShellWidget dos)
{
  Status status;
  Window *windowsReturn;
  int countReturn;
  ...
  if (dos->drag.colormapShell) {
    /* check to see if there is a property */
    status = XGetWMColormapWindows(
        XtDisplay(dos), XtWindow(dos->drag.colormapShell), &windowsReturn, &countReturn);
    ...
    if (status) {
      ...
      for (i = 0; i < countReturn; i++) {
        if (windowsReturn[i] == XtWindow(dos)) {
          ...
          XSetWMColormapWindows(
              XtDisplay(dos), XtWindow(dos->drag.colormapShell), windowsReturn, countReturn - 1);
    ...
    if (dos->drag.colormapOverride) {
      for (i = 0; i < dos->drag.numSavedColormaps; i++)
        XInstallColormap(XtDisplay(dos), dos->drag.savedColormaps[i]);
...
static void DragOverShellPunchHole(Widget w)
{
  ...
  XmDragOverShellWidget dos = (XmDragOverShellWidget)w;
  XmDragIconObject icon = (dos->drag.rootBlend.mixedIcon ? dos->drag.rootBlend.mixedIcon :
                                                           dos->drag.cursorBlend.mixedIcon);
  ...
  if (icon && icon->drag.mask) {
    if (icon->drag.mask)
      XShapeCombineMask(
          XtDisplay(dos), XtWindow(dos), ShapeBounding, 0, 0, icon->drag.mask, ShapeSet);
    /* punch a hole in the window */
    XShapeCombineRectangles(XtDisplay(dos),
                            XtWindow(dos),
                            ShapeBounding,
                            icon->drag.hot_x,
                            icon->drag.hot_y,
                            (XRectangle *)&pixelPunch,
                            1,
                            ShapeSubtract,
                            YXBanded);
