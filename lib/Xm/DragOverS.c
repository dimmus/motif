...
static XtResource resources[] = {
    ...
    },
   {
    ...
    XmCDragOverActiveMode,
    XtRUnsignedChar,
    sizeof(unsigned char),
    Offset(drag.activeMode),
    XmRImmediate,
    (XtPointer)XmCURSOR,
    },
   {
...
externaldef(xmDragOvershellwidgetclass) WidgetClass xmDragOverShellWidgetClass = (WidgetClass)(&xmDragOverShellClassRec);
   ...
   Position              opY)
{
   ...
   {
      if (cache_ptr->depth == depth && cache_ptr->width == width && cache_ptr->height == height && cache_ptr->cursorForeground == dos->drag.cursorForeground && cache_ptr->cursorBackground == dos->drag.cursorBackground && cache_ptr->sourcePixmap == sourceIcon->drag.pixmap && cache_ptr->sourceMask == sourceIcon->drag.mask && cache_ptr->sourceX == sourceX && cache_ptr->sourceY == sourceY && ((cache_ptr->statePixmap == 0 && !stateIcon) || (stateIcon && cache_ptr->statePixmap == stateIcon->drag.pixmap && cache_ptr->stateMask == stateIcon->drag.mask && cache_ptr->stateX == stateX && cache_ptr->stateY == stateY)) && ((cache_ptr->opPixmap == 0 && !opIcon) || (opIcon && cache_ptr->opPixmap == opIcon->drag.pixmap && cache_ptr->opMask == opIcon->drag.mask && cache_ptr->opX == opX && cache_ptr->opY == opY)))
      {
   ...
   */
   ...
   for (;; i++)
   {
      ...
      if (dos->drag.activeMode != XmDRAG_WINDOW)
      {
         ...
         XChangeGC(display, draw_gc, vmask, &v);
         XCopyArea(display, BackingPixmap(dos), root, draw_gc, 0, 0, dos->core.width, dos->core.height, segments[0].x2, segments[0].y2);
      }
      ...
      {
         XCopyArea(display, root, BackingPixmap(dos), draw_gc, segments[0].x2, segments[0].y2, dos->core.width, dos->core.height, 0, 0);
         DrawIcon(dos,
                  (dos->drag.rootBlend.mixedIcon ? dos->drag.rootBlend.mixedIcon : dos->drag.cursorBlend.mixedIcon),
                  root,
                  segments[0].x2,
                  segments[0].y2);
      }
   ...
   XtIntervalId *id) /* unused */
{
   ...
   if (dos->drag.activeMode == XmDRAG_WINDOW)
   {
     ...
     */
      ...
      {
         XShapeCombineRectangles(XtDisplay(dos), XtWindow(dos), ShapeBounding, 0, 0, &rect, 1, ShapeSet, YXSorted);
   ...
   else
   {
     ...
     */
      ...
      rects[1].x      = rects[0].x + blend->sourceX + sourceIcon->drag.width - xClipOffset;
      ...
      rects[2].x      = rects[0].x;
      rects[2].y      = rects[0].y + blend->sourceY + sourceIcon->drag.height - yClipOffset;
   ...
   switch ((int)icon->drag.attachment)
   {
      ...
      case XmATTACH_HOT:
      {
         ...
         /*
		     ...
		     */
         ref = (dc->drag.origDragOver != NULL) ? dc->drag.origDragOver : dos;
         ...
         else
         {
            ...
            ac           = 0;
            XtSetArg(al[ac], XmNborderWidth, &borderW);
            ac++;
            XtSetArg(al[ac], XmNhighlightThickness, &highlightT);
            ac++;
            XtSetArg(al[ac], XmNshadowThickness, &shadowT);
            ac++;
            ...
            XQueryPointer(XtDisplay(dos),
                          ...
                          &winY,
                          &modMask);
            *iconX = winX - icon->drag.hot_x - borderW - highlightT - shadowT;
            *iconY = winY - icon->drag.hot_y - borderW - highlightT - shadowT;
         }
      }
      break;
   }
   ...
   GC                    pixmapGC)
{
   ...
   if (icon->drag.pixmap != XmUNSPECIFIED_PIXMAP)
   {
      ...
      if (icon->drag.mask != XmUNSPECIFIED_PIXMAP)
      {
         ...
         XCopyArea(display, icon->drag.mask, mixedIcon->drag.mask, maskGC, sourceX, sourceY, mixedIcon->drag.width, mixedIcon->drag.height, destX, destY);
	 ...
	 has no mask */
         if (mixedIcon->drag.mask != XmUNSPECIFIED_PIXMAP)
         {
            ...
            vmask     |= GCFunction;
            XChangeGC(display, maskGC, vmask, &v);
            XFillRectangle(display, mixedIcon->drag.mask, maskGC, destX, destY, destWidth, destHeight);
         }
      }
      ...
      if (icon->drag.region != NULL && mixedIcon->drag.region != NULL)
      {
         if (icon->drag.x_offset || icon->drag.y_offset)
            XOffsetRegion(icon->drag.region, -icon->drag.x_offset, -icon->drag.y_offset);
         XOffsetRegion(icon->drag.region, destX, destY);
         icon->drag.x_offset = destX;
         icon->drag.y_offset = destY;
         XUnionRegion(mixedIcon->drag.region, icon->drag.region, mixedIcon->drag.region);
      }
      else
      {
     ...
     */
      ...
      {
         XCopyPlane(display, icon->drag.pixmap, mixedIcon->drag.pixmap, pixmapGC, sourceX, sourceY, icon->drag.width, icon->drag.height, destX, destY, 1L);
      }
      else if (icon->drag.depth == mixedIcon->drag.depth)
      {
         XCopyArea(display, icon->drag.pixmap, mixedIcon->drag.pixmap, pixmapGC, sourceX, sourceY, icon->drag.width, icon->drag.height, destX, destY);
      }
      else
      {
   ...
   Boolean               clip)
{
   ...
   mixedIcon = GetMixedIcon(dos, depth, width, height, sourceIcon, stateIcon, opIcon, sourceX, sourceY, stateX, stateY, opX, opY);
   /*
   ...
   /*
     ...
     */
   if ((sourceIcon->drag.mask == XmUNSPECIFIED_PIXMAP) && (!stateIcon || stateIcon->drag.mask == XmUNSPECIFIED_PIXMAP) && (!opIcon || opIcon->drag.mask == XmUNSPECIFIED_PIXMAP))
   {
      XRectangle rect;
   ...
   if (mixedIcon == NULL)
   {
      ...
      ac     = 0;
      XtSetArg(al[ac], XmNpixmap, pixmap);
      ac++;
      XtSetArg(al[ac], XmNmask, mask);
      ac++;
      XtSetArg(al[ac], XmNdepth, depth);
      ac++;
      XtSetArg(al[ac], XmNwidth, width);
      ac++;
      XtSetArg(al[ac], XmNheight, height);
      ac++;
      XtSetArg(al[ac], XmNhotX, hotX);
      ac++;
      XtSetArg(al[ac], XmNhotY, hotY);
      ac++;
      mixedIcon = blendPtr->mixedIcon = (XmDragIconObject)
         XmCreateDragIcon((Widget)xmScreen, "mixedIcon", al, ac);
      if (need_mask)
      {
         mask = mixedIcon->drag.mask = _XmAllocScratchPixmap(xmScreen, 1, width, height);
      }
   }
   else
   {
      ...
      {
         mixedIcon->drag.mask = _XmAllocScratchPixmap(xmScreen, 1, width, height);
      }
   ...
   if (blendPtr->gc == NULL)
   {
      ...
      vmask                = GCBackground | GCForeground | GCFunction | GCClipXOrigin | GCClipYOrigin | GCClipMask | GCGraphicsExposures | GCSubwindowMode;
      blendPtr->gc         = XtAllocateGC((Widget)dos, mixedIcon->drag.depth, vmask, &v, vmask, 0L);
   }
   else
   {
      v.clip_mask = None;
   ...
   XFillRectangle(display, pixmap, blendPtr->gc, 0, 0, mixedIcon->drag.width, mixedIcon->drag.height);
   if (mask != XmUNSPECIFIED_PIXMAP)
   {
      if (cursorBlend->gc == NULL)
      {
         ...
         v.clip_y_origin      = 0; /* pedantic */
         vmask                = GCBackground | GCForeground | GCFunction | GCClipXOrigin | GCClipYOrigin | GCClipMask | GCGraphicsExposures | GCSubwindowMode;
         cursorBlend->gc      = XtAllocateGC((Widget)dos, 1, vmask, &v, vmask, 0L);
      }
      else
      {
         v.function = GXclear;
      ...
      XFillRectangle(display, mixedIcon->drag.mask, cursorBlend->gc, 0, 0, mixedIcon->drag.width, mixedIcon->drag.height);
   }
   /*
     *  Blend the icons into mixedIcon.
     */
   BlendIcon(dos, sourceIcon, mixedIcon, sourceX, sourceY, cursorBlend->gc, blendPtr->gc);
   blendPtr->sourceX = sourceX;
   blendPtr->sourceY = sourceY;
   if (stateIcon)
   {
      BlendIcon(dos, stateIcon, mixedIcon, stateX, stateY, cursorBlend->gc, blendPtr->gc);
   }
   if (opIcon)
   {
      BlendIcon(dos, opIcon, mixedIcon, opX, opY, cursorBlend->gc, blendPtr->gc);
   }
   if (mixedIcon->drag.region != NULL)
   {
      XRectangle rect;
      ...
      XUnionRectWithRegion(&rect, mixedIcon->drag.restore_region, mixedIcon->drag.restore_region);
      XSubtractRegion(mixedIcon->drag.restore_region, mixedIcon->drag.region, mixedIcon->drag.restore_region);
   }
   if (do_cache)
   {
      CacheMixedIcon(dos, depth, width, height, sourceIcon, stateIcon, opIcon, sourceX, sourceY, stateX, stateY, opX, opY, mixedIcon);
   } /* do_cache */
}
   ...
   XmDragIconObject      opIcon)
{
   Dimension maxWidth, maxHeight;
   Dimension width, height;
   if (((sourceIcon->drag.depth != 1) || (sourceIcon->drag.pixmap == XmUNSPECIFIED_PIXMAP)))
   {
      return False;
   }
   ...
   XmDragOverShellWidget dos)
{
   ...
   if (XAllocColor(display, DefaultColormapOfScreen(screen), &colors[0]) && XAllocColor(display, DefaultColormapOfScreen(screen), &colors[1]))
   {
      fg = colors[0].pixel;
      bg = colors[1].pixel;
      if (fg == bg)
   ...
   if (dos->drag.rootBlend.gc == NULL)
   {
      ...
      v.clip_x_origin                           = 0; /* pedantic */
      v.clip_y_origin                           = 0; /* pedantic */
      vmask                                     = GCBackground | GCForeground | GCClipXOrigin | GCClipYOrigin | GCClipMask | GCGraphicsExposures | GCSubwindowMode;
   ...
   else if (dos->drag.cursorBackground != bg || dos->drag.cursorForeground != fg)
   {
      doChange     = True;
      v.background = dos->drag.cursorBackground = bg;
      v.foreground = dos->drag.cursorForeground = fg;
   ...
   Boolean               dirty)
{
   ...
   if (dirty)
   {
      ...
      while (cursorCache)
      {
         if ((dirtystateIcon && (cursorCache->stateIcon == dirtystateIcon)) || (dirtysourceIcon && (cursorCache->sourceIcon == dirtysourceIcon)) || (dirtyopIcon && (cursorCache->opIcon == dirtyopIcon)))
   ...
   else
   {
      ...
      while (cursorCache)
      {
         if ((cursorCache->stateIcon == stateIcon) && (cursorCache->opIcon == opIcon) && (cursorCache->sourceIcon == sourceIcon))
         {
            /*
		 ...
		 */
            ...
            /* recolor the cursor */
            XRecolorCursor(display, cursorCache->cursor, &colors[0], /* foreground_color */
   ...
   /*
     ...
     */
   MixIcons(dos, sourceIcon, stateIcon, opIcon, &dos->drag.cursorBlend, clip);
   cursor = XCreatePixmapCursor(display,
                                dos->drag.cursorBlend.mixedIcon->drag.pixmap,
                                ((dos->drag.cursorBlend.mixedIcon->drag.mask == XmUNSPECIFIED_PIXMAP) ? None : dos->drag.cursorBlend.mixedIcon->drag.mask),
   ...
   Cardinal *numArgs) /* unused */
{
   ...
   dos->drag.cursorBlend.gc = dos->drag.rootBlend.gc = NULL;
   dos->drag.cursorBlend.sourceIcon = dos->drag.cursorBlend.mixedIcon = dos->drag.rootBlend.sourceIcon = dos->drag.rootBlend.mixedIcon = NULL;
   dos->drag.backing.pixmap = dos->drag.tmpPix = dos->drag.tmpBit = XmUNSPECIFIED_PIXMAP;
   ...
   Cardinal *num_args) /* unused */
{
   ...
   if (oldDos->drag.hotX != dos->drag.hotX || oldDos->drag.hotY != dos->drag.hotY || oldDos->core.width != dos->core.width || oldDos->core.height != dos->core.height)
      dos->drag.holePunched = FALSE;
   /*
   ...
   {
      if ((dos->drag.mode == XmPIXMAP || dos->drag.mode == XmDRAG_WINDOW) && (dos->drag.rootBlend.sourceIcon == NULL))
      {
   ...
   else if ((dos->drag.hotX != oldDos->drag.hotX) || (dos->drag.hotY != oldDos->drag.hotY))
   {
   ...
   Position              y)
{
   ...
   v.function            = GXcopy;
   vmask                 = GCFunction;
   if (icon->drag.region == NULL && icon->drag.mask != XmUNSPECIFIED_PIXMAP)
   {
      v.clip_mask     = icon->drag.mask;
      v.clip_x_origin = x;
      v.clip_y_origin = y;
   ...
   XmRegion clipRegion)
{
   ...
   XmDragContext         dc      = (XmDragContext)XtParent(dos);
   Boolean               clipped = False;
   if (dos->drag.isVisible && dc->drag.blendModel != XmBLEND_NONE && dos->drag.activeMode != XmCURSOR)
   {
      if (dos->drag.activeMode == XmWINDOW || dos->drag.activeMode == XmDRAG_WINDOW)
      {
         XtPopdown(w);
   ...
   XmRegion clipRegion)
{
   ...
   if (!dos->drag.isVisible && dc->drag.blendModel != XmBLEND_NONE && dos->drag.activeMode != XmCURSOR)
   {
      ...
      {
         clipped = True;
         _XmRegionSetGCRegion(display, dos->drag.rootBlend.gc, clipOriginX - BackingX(dos), clipOriginY - BackingY(dos), clipRegion);
      }
      else
      {
      ...
      {
         XCopyArea(display, RootWindowOfScreen(XtScreen(w)), BackingPixmap(dos), dos->drag.rootBlend.gc, BackingX(dos), BackingY(dos), dos->core.width, dos->core.height, 0, 0);
      }
      if (clipped)
      {
         XSetClipMask(display, dos->drag.rootBlend.gc, None);
	   ...
	   */
         if (dos->drag.activeMode == XmDRAG_WINDOW)
         {
            ...
            if (dos->drag.installColormap)
               InstallColormap(dos);
            XtSetArg(args[0], XmNbackgroundPixmap, dos->drag.rootBlend.mixedIcon->drag.pixmap);
   ...
   */
   ...
   else
   {
     ...
     */
      ...
      if (BackingPixmap(dos) != XmUNSPECIFIED_PIXMAP)
      {
         XCopyArea(display, BackingPixmap(dos), RootWindowOfScreen(XtScreen(dos)), draw_gc, 0, 0, dos->core.width, dos->core.height, BackingX(dos), BackingY(dos));
      }
   }
   dos->drag.isVisible = False;
   /*
   ...
   */
   ...
   else
   {
     ...
     */
      ...
      MixIcons(dos, sourceIcon, dos->drag.stateIcon, dos->drag.opIcon, blend, False);
      /*
     *  Compute the new location and handle an icon size change.
     */
      BackingX(dos) = dos->core.x = dos->drag.hotX - blend->mixedIcon->drag.hot_x;
      BackingY(dos) = dos->core.y = dos->drag.hotY - blend->mixedIcon->drag.hot_y;
      if (dos->core.width != blend->mixedIcon->drag.width || dos->core.height != blend->mixedIcon->drag.height)
      {
      ...
      if (dos->drag.activeMode == XmDRAG_WINDOW && BackingPixmap(dos) != XmUNSPECIFIED_PIXMAP)
      {
         _XmFreeScratchPixmap(xmScreen, BackingPixmap(dos));
         BackingPixmap(dos) = XmUNSPECIFIED_PIXMAP;
      }
     ...
     */
      if (dos->drag.activeMode == XmPIXMAP)
      {
         if (BackingPixmap(dos) == XmUNSPECIFIED_PIXMAP)
         {
            BackingPixmap(dos) = _XmAllocScratchPixmap(xmScreen, dos->core.depth, dos->core.width, dos->core.height);
         }
         XSetClipMask(display, draw_gc, None);
         XCopyArea(display, RootWindowOfScreen(XtScreen(dos)), BackingPixmap(dos), draw_gc, BackingX(dos), BackingY(dos), dos->core.width, dos->core.height, 0, 0);
         DrawIcon(dos, blend->mixedIcon, RootWindowOfScreen(XtScreen(dos)), dos->core.x, dos->core.y);
      }
      else if (dos->drag.activeMode == XmWINDOW)
      {
         ...
         XChangeWindowAttributes(display, XtWindow(dos), CWCursor | CWDontPropagate, &xswa);
         /*
       * don't call thru class record since VendorS bug may be
       * causing override
       */
      ...
      { /* XmDRAG_WINDOW */
         ...
         if (XtWindow(dos) != None)
            XMoveWindow(XtDisplay(dos), XtWindow(dos), dos->core.x, dos->core.y);
         XtPopup((Widget)dos, XtGrabNone);
         if (dos->drag.activeMode == XmDRAG_WINDOW)
         {
            Arg args[1];
...
ChangeDragWindow(XmDragOverShellWidget dos)
{
   ...
   XFillRectangle(display, mixedIcon->drag.pixmap, blend->gc, 0, 0, mixedIcon->drag.width, mixedIcon->drag.height);
   if (mixedIcon->drag.mask != XmUNSPECIFIED_PIXMAP)
   {
      if (cursorBlend->gc == NULL)
      {
   ...
   BlendIcon(dos, sourceIcon, mixedIcon, blend->sourceX, blend->sourceY, cursorBlend->gc, blend->gc);
   /*
   ...
   */
   XMoveResizeWindow(display, win, dos->core.x, dos->core.y, dos->core.width, dos->core.height);
   ...
   Position y)
{
   ...
   if (dos->drag.tmpPix == XmUNSPECIFIED_PIXMAP)
   {
      dos->drag.tmpPix = _XmAllocScratchPixmap(xmScreen, dos->core.depth, dos->core.width, dos->core.height);
   }
   ...
   vmask       = GCClipMask | GCFunction;
   XChangeGC(display, draw_gc, vmask, &v);
   XCopyArea(display, root, new_backing, draw_gc, x, y, dos->core.width, dos->core.height, 0, 0);
   if (x + ((Position)dos->core.width) > BackingX(dos) && x < BackingX(dos) + ((Position)dos->core.width) && y + ((Position)dos->core.height) > BackingY(dos) && y < BackingY(dos) + ((Position)dos->core.height))
   {
	 ...
	 */
      if (mixedIcon->drag.region == NULL && mixedIcon->drag.mask != XmUNSPECIFIED_PIXMAP)
      {
         Pixmap root_mask;
         GC     mask_gc = dos->drag.cursorBlend.gc;
         if (dos->drag.tmpBit == XmUNSPECIFIED_PIXMAP)
         {
            dos->drag.tmpBit = _XmAllocScratchPixmap(xmScreen, 1, dos->core.width, dos->core.height);
         }
         ...
         XFillRectangle(display, root_mask, mask_gc, 0, 0, dos->core.width, dos->core.height);
         XSetFunction(display, mask_gc, GXandInverted);
         XCopyArea(display, mixedIcon->drag.mask, root_mask, mask_gc, 0, 0, mixedIcon->drag.width, mixedIcon->drag.height, x - BackingX(dos), y - BackingY(dos));
         /*
	     ...
	     */
         ...
         vmask           = GCClipMask | GCClipXOrigin | GCClipYOrigin;
         XChangeGC(display, draw_gc, vmask, &v);
         XCopyArea(display, old_backing, root, draw_gc, 0, 0, dos->core.width, dos->core.height, BackingX(dos), BackingY(dos));
         XSetClipMask(display, draw_gc, None);
      }
      else
      {
	     ...
	     */
         ...
         if (rect1.width > 0)
         {
            XCopyArea(display, old_backing, root, draw_gc, rect1.x, rect1.y, rect1.width, rect1.height, pt.x, pt.y);
         }
         /*
	     ...
	     */
         ...
         if (rect2.height > 0)
         {
            XCopyArea(display, old_backing, root, draw_gc, rect2.x, rect2.y, rect2.width, rect2.height, pt.x, pt.y);
         }
      }
      /*
	 ...
	 */
      ...
      XCopyArea(display, old_backing, new_backing, draw_gc, rect.x, rect.y, rect.width, rect.height, pt.x, pt.y);
      if (mixedIcon->drag.restore_region)
      {
         ...
         XCopyArea(display, new_backing, root, draw_gc, 0, 0, dos->core.width, dos->core.height, x, y);
         XSetClipMask(display, draw_gc, None);
      }
   }
   else
   ...
   unsigned char dropSiteStatus)
{
   ...
   /*
     ...
     */
   if (dos->drag.mode == XmPIXMAP || dos->drag.mode == XmDRAG_WINDOW || dos->drag.mode == XmWINDOW)
   {
      sourceIcon = dc->drag.sourcePixmapIcon;
   }
   if (sourceIcon == NULL || XtScreenOfObject(XtParent(sourceIcon)) != XtScreen(w) || ((sourceIcon->drag.depth != dos->core.depth) && (sourceIcon->drag.depth != 1)))
   {
      usedSrcPixIcon = False;
      sourceIcon     = dc->drag.sourceCursorIcon;
      if (sourceIcon == NULL || XtScreenOfObject(XtParent(sourceIcon)) != XtScreen(w) || sourceIcon->drag.depth != 1)
      {
   ...
   switch ((int)dc->drag.blendModel)
   {
      ...
      case XmBLEND_ALL:
	     ...
	     */
         ...
         if (opIcon == NULL || opIcon->drag.depth != 1 || XtScreenOfObject(XtParent(opIcon)) != XtScreen(w))
         {
      ...
      case XmBLEND_STATE_SOURCE:
	     ...
	     */
         ...
         if (stateIcon == NULL || stateIcon->drag.depth != 1 || XtScreenOfObject(XtParent(stateIcon)) != XtScreen(w))
         {
   ...
   /*
     ...
     */
   dirty    = (_XmDragIconIsDirty(sourceIcon) || (opIcon && _XmDragIconIsDirty(opIcon)) || (stateIcon && _XmDragIconIsDirty(stateIcon)));
   doChange = GetDragIconColors(dos) || dos->drag.opIcon != opIcon || dos->drag.stateIcon != stateIcon || dos->drag.rootBlend.sourceIcon != sourceIcon || dirty;
   /*
   ...
   if (!usedSrcPixIcon && (dos->drag.activeCursor = GetDragIconCursor(dos, sourceIcon, stateIcon, opIcon, False /* no clip */, dirty)) != None)
   {
      /*
   ...
   if (dos->drag.mode == XmCURSOR)
   {
	 ...
	 */
      dos->drag.activeCursor = GetDragIconCursor(dos, sourceIcon, stateIcon, opIcon, True /* clip */, dirty);
      _XmDragIconClean(sourceIcon, stateIcon, opIcon);
      if (dos->drag.activeMode != XmCURSOR)
      {
         _XmDragOverHide(w, 0, 0, None);
...
DragOverShellPunchHole(Widget w)
{
   ...
   XmDragOverShellWidget     dos        = (XmDragOverShellWidget)w;
   XmDragIconObject          icon       = (dos->drag.rootBlend.mixedIcon ? dos->drag.rootBlend.mixedIcon : dos->drag.cursorBlend.mixedIcon);
   ...
   XShapeCombineMask(XtDisplay(dos), XtWindow(dos), ShapeBounding, 0, 0, None, ShapeSet);
   /* shape the outside of the window */
   if (icon && icon->drag.mask)
   {
      if (icon->drag.mask)
         XShapeCombineMask(XtDisplay(dos), XtWindow(dos), ShapeBounding, 0, 0, icon->drag.mask, ShapeSet);
      /* punch a hole in the window */
      XShapeCombineRectangles(XtDisplay(dos), XtWindow(dos), ShapeBounding, icon->drag.hot_x, icon->drag.hot_y, (XRectangle *)&pixelPunch, 1, ShapeSubtract, YXBanded);
