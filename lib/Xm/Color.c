...
static XColor *
GetDefaultBackgroundColor(Screen *screen, Colormap color_map)
{
   XrmName           names[2];
   XrmClass          classes[2];
   ...
   if (XrmQGetResource(XtScreenDatabase(screen), names, classes, &rep, &db_value))
   {
      ...
      {
         if (!XParseColor(DisplayOfScreen(screen), color_map, db_value.addr, &color)) return NULL;
      }
   ...
   {
      if (!XParseColor(DisplayOfScreen(screen), color_map, XmDEFAULT_BACKGROUND, &color)) return NULL;
   }
   return &color;
}
   ...
   XmColorData **ret)
{
   ...
   for (i = 0; i < Set_Count; i++)
   {
      if ((!(which & XmLOOK_AT_SCREEN) || ((Color_Set + i)->screen == values->screen))
          ...
          && (!(which & XmLOOK_AT_BACKGROUND) || (((Color_Set + i)->allocated & XmBACKGROUND) && ((Color_Set + i)->background.pixel == values->background.pixel)))
          && (!(which & XmLOOK_AT_FOREGROUND) || (((Color_Set + i)->allocated & XmFOREGROUND) && ((Color_Set + i)->foreground.pixel == values->foreground.pixel)))
          && (!(which & XmLOOK_AT_TOP_SHADOW) || (((Color_Set + i)->allocated & XmTOP_SHADOW) && ((Color_Set + i)->top_shadow.pixel == values->top_shadow.pixel)))
          && (!(which & XmLOOK_AT_BOTTOM_SHADOW) || (((Color_Set + i)->allocated & XmBOTTOM_SHADOW) && ((Color_Set + i)->bottom_shadow.pixel == values->bottom_shadow.pixel)))
          && (!(which & XmLOOK_AT_SELECT) || (((Color_Set + i)->allocated & XmSELECT) && ((Color_Set + i)->select.pixel == values->select.pixel))))
      {
         *ret = (Color_Set + i);
         _XmProcessUnlock();
         return (TRUE);
   ...
   unsigned char which)
{
   ...
   {
      case XmBACKGROUND:
         if (!(cd->allocated & which) && ((*aproc)(cd->screen->display, cd->color_map, &(cd->background)) == 0))
         {
         ...
         break;
      case XmFOREGROUND:
         if (!(cd->allocated & which) && ((*aproc)(cd->screen->display, cd->color_map, &(cd->foreground)) == 0))
         {
            if (Brightness(&(cd->background))
                < XmFOREGROUND_THRESHOLD)
         ...
         break;
      case XmTOP_SHADOW:
         if (!(cd->allocated & which) && ((*aproc)(cd->screen->display, cd->color_map, &(cd->top_shadow)) == 0))
         {
            if (Brightness(&(cd->background))
      ...
      case XmBOTTOM_SHADOW:
         if (!(cd->allocated & which) && ((*aproc)(cd->screen->display, cd->color_map, &(cd->bottom_shadow)) == 0))
         {
            if (Brightness(&(cd->background))
         ...
         break;
      case XmSELECT:
         if (!(cd->allocated & which) && ((*aproc)(cd->screen->display, cd->color_map, &(cd->select)) == 0))
         {
            if (Brightness(&(cd->background))
                < XmFOREGROUND_THRESHOLD)
   ...
   XrmValue *value)
{
   ...
   if (!XtIsWidget(widget))
   {
	 ...
	 */
      ...
      {
         if ((XmIsLabelGadget(widget)) || (XmIsArrowButtonGadget(widget)) || (XmIsSeparatorGadget(widget)))
