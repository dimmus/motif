...
static XtResource resources[] = {
   { XmNslideFinishCallback, XtCCallback,        XtRCallback,  sizeof(XtCallbackList), Offset(slideFinishCallback), XtRCallback,  NULL                              },
   { XmNslideMotionCallback, XtCCallback,        XtRCallback,  sizeof(XtCallbackList), Offset(slideMotionCallback), XtRCallback,  NULL                              },
   { XmNslideWidget,         XmCSlideWidget,     XtRWidget,    sizeof(Widget),         Offset(slide_widget),        XtRImmediate, NULL                              },
   { XmNslideInterval,       XmCSlideInterval,   XtRInt,       sizeof(int),            Offset(interval),            XtRImmediate, (XtPointer)5                      },
   { XmNslideDestWidth,      XmCSlideDestWidth,  XtRDimension, sizeof(Dimension),      Offset(dest_width),          XtRImmediate, (XtPointer)XmUNSPECIFIED          },
   { XmNslideDestHeight,     XmCSlideDestHeight, XtRDimension, sizeof(Dimension),      Offset(dest_height),         XtRImmediate, (XtPointer)XmUNSPECIFIED          },
   { XmNslideDestX,          XmCSlideDestX,      XtRPosition,  sizeof(Position),       Offset(dest_x),              XtRImmediate, (XtPointer)XmUNSPECIFIED_POSITION },
   { XmNslideDestY,          XmCSlideDestY,      XtRPosition,  sizeof(Position),       Offset(dest_y),              XtRImmediate, (XtPointer)XmUNSPECIFIED_POSITION },
};
#undef Offset
/*
   Widget class record
...
_XmSlideProc(Widget w)
{
   ...
   XtVaSetValues(Slide_Widget(w),
                 ...
                 height,
                 NULL);
   if (Slide_DestX(w) == XtX(Slide_Widget(w)) && Slide_DestY(w) == XtY(Slide_Widget(w)) && Slide_DestWidth(w) == XtWidth(Slide_Widget(w)) && Slide_DestHeight(w) == XtHeight(Slide_Widget(w)))
   {
