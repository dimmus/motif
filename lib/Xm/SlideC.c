...
static XtResource resources[] = {
    ...
    {XmNslideDestX,
     ...
     Offset(dest_x),
     XtRImmediate,
     (XtPointer)XmUNSPECIFIED_POSITION},
    {XmNslideDestY,
     XmCSlideDestY,
     XtRPosition,
     sizeof(Position),
...
static void initialize(Widget request, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (Slide_Widget(new_w) == NULL) {
    _XmWarningMsg(
        new_w, "Invalid parameter", XmNslideWidget " must be specified during creation", NULL, 0);
  }
  else {
...
static void _XmSlideProc(Widget w)
{
  ...
  /*
    ...
    */
  XtVaSetValues(Slide_Widget(w), XmNx, x, XmNy, y, XmNwidth, width, XmNheight, height, NULL);
  if (Slide_DestX(w) == XtX(Slide_Widget(w)) && Slide_DestY(w) == XtY(Slide_Widget(w)) &&
      ...
      Slide_DestHeight(w) == XtHeight(Slide_Widget(w)))
  {
    ...
    Slide_Id(w) = XtAppAddTimeOut(
        XtWidgetToApplicationContext(w), Slide_Interval(w), (XtTimerCallbackProc)_XmSlideProc, w);
  }
}
