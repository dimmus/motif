   ...
   Cardinal *num_args) /* unused */
{
    ...
    */
   if ((current->message_box.dialog_type != XmDIALOG_TEMPLATE) && (current->bulletin_board.cancel_button != new_w->bulletin_board.cancel_button))
   {
   ...
   { /* If symbol pixmap is unchanged and a new dialog type is specified,
        ...
        */
      if (new_w->message_box.dialog_type != current->message_box.dialog_type || new_w->message_box.baseline != current->message_box.baseline)
      {
         newPixmap = TRUE;
   ...
   XmMessageBoxWidget w)
{
   ...
   if (!(w->message_box.dialog_type == XmDIALOG_TEMPLATE && w->message_box.symbol_pixmap == XmUNSPECIFIED_PIXMAP))
      SetUpSymbol(w);
   ...
   if (!(w->message_box.dialog_type == XmDIALOG_TEMPLATE && w->message_box.ok_label_string == NULL && w->message_box.ok_callback == NULL))
   {
      w->message_box.ok_button       = _XmBB_CreateButtonG((Widget)w,
                                                     w->message_box.ok_label_string,
                                                     "OK",
                                                     XmOkStringLoc);
   ...
   if (!(w->message_box.dialog_type == XmDIALOG_TEMPLATE && w->message_box.cancel_label_string == NULL && w->message_box.cancel_callback == NULL))
   {
      w->bulletin_board.cancel_button    = _XmBB_CreateButtonG((Widget)w,
                                                            w->message_box.cancel_label_string,
                                                            "Cancel",
                                                            XmCancelStringLoc);
      w->message_box.cancel_label_string = NULL;
      XtAddCallback(w->bulletin_board.cancel_button, XmNactivateCallback, MessageCallback, (XtPointer)w);
   }
   /* "Help" button... if no label, use default localized "Help" string from
       message catalog, or "Help" if no message catalog, for label */
   if (!(w->message_box.dialog_type == XmDIALOG_TEMPLATE && w->message_box.help_label_string == NULL && w->manager.help_callback == NULL))
   {
      w->message_box.help_button       = _XmBB_CreateButtonG((Widget)w,
                                                       w->message_box.help_label_string,
                                                       "Help",
                                                       XmHelpStringLoc);
   ...
   XmMessageBoxWidget mBox)
{
   ...
   if (fileName)
   {
      ...
      XmStringFree(s);
      tmpPix = _XmGetScaledPixmap(mBox->core.screen, (Widget)mBox, fileName, &acc_color_rec, depth, False, 0, 0, mBox->message_box.baseline * 2.5);
      if (tmpPix == XmUNSPECIFIED_PIXMAP)
      {
         tmpPix = _XmGetScaledPixmap(mBox->core.screen, (Widget)mBox, defaultName, &acc_color_rec, depth, False, 0, 0, mBox->message_box.baseline * 2.5);
      }
   }
