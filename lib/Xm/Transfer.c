...
static void           SelectionCallbackWrapper(Widget, XtPointer, Atom *, Atom *, XtPointer, unsigned long *, int *);
static Boolean        DragConvertHandler(Widget, Atom *, Atom *, Atom *, XtPointer *, unsigned long *, int *);
static void           SecondaryConvertHandler(Widget, XtPointer, XmConvertCallbackStruct *);
static void           ReleaseSecondaryLock(Widget, XtEnum, XmTransferDoneCallbackStruct *);
static void           DropDestinationHandler(Widget, XtPointer, XmDropProcCallbackStruct *);
static TransferBlock  AddTransferBlock(TransferContext tc);
static void           SecondaryDone(Widget, XtPointer, Atom *, Atom *, XtPointer, unsigned long *, int *);
static void           TransferWarning(Widget w, char *name, char *type, char *message);
static void           DeleteDropCBStruct(Widget w, XtEnum ignored_status, XmTransferDoneCallbackStruct *cs);
...
/****************************************************************/
Boolean
_XmConvertHandler(Widget wid, Atom *selection, Atom *target, Atom *type, XtPointer *value, unsigned long *size, int *fmt)
{
   enum
   {
      XmA_MOTIF_DESTINATION,
      XmAINSERT_SELECTION,
      XmALINK_SELECTION,
      ...
      XmA_MOTIF_DEFERRED_CLIPBOARD_TARGETS,
      NUM_ATOMS
   };

   static char            *atom_names[] = { XmS_MOTIF_DESTINATION, XmSINSERT_SELECTION, XmSLINK_SELECTION, XmS_MOTIF_LOSE_SELECTION, XmS_MOTIF_DROP, XmSCLIPBOARD, XmS_MOTIF_CLIPBOARD_TARGETS, XmS_MOTIF_DEFERRED_CLIPBOARD_TARGETS };
   ...
   if (my_local_convert_flag == 0)
   {
      ...
      {
         cbstruct.event = (XEvent *)XtGetSelectionRequest(wid, *selection, NULL);
         req_widget     = wid;
      }
       ...
       selection atom here as well */
      {
         Atom sel_atom = (real_selection_atom != None)
                          ? real_selection_atom
                          : *selection;
         XtGetSelectionParameters(req_widget, sel_atom, NULL, &cbstruct.parm_type, &cbstruct.parm, &cbstruct.parm_length, &cbstruct.parm_format);
      }
   }
   else
   {
      ...
      {
         if (*target == atoms[XmA_MOTIF_CLIPBOARD_TARGETS] || *target == atoms[XmA_MOTIF_DEFERRED_CLIPBOARD_TARGETS])
         {
   ...
   _XmProcessUnlock();
   if (cbstruct.event != NULL && ((XSelectionRequestEvent *)cbstruct.event)->requestor == ((XSelectionRequestEvent *)cbstruct.event)->owner)
   {
   ...
   if (*selection != atoms[XmA_MOTIF_DESTINATION] || *target == atoms[XmA_MOTIF_LOSE_SELECTION])
   {
      ...
      /* Now lookup the trait on this widget and call the
       internal routine */
      if (cbstruct.status == XmCONVERT_DEFAULT || cbstruct.status == XmCONVERT_MERGE)
      {
   ...
   if (cbstruct.status == XmCONVERT_DEFAULT && (*target == atoms[XmAINSERT_SELECTION] || *target == atoms[XmALINK_SELECTION]))
...
/****************************************************************/
static Boolean
DragConvertHandler(Widget drag_context, Atom *selection, Atom *target, Atom *type, XtPointer *value, unsigned long *size, int *fmt)
{
   ConvertContext cc;
                        ...
                        XmConvertCallbackStruct *cs)
{
   ...
   if (cs->target == atoms[XmAINSERT_SELECTION])
      ...
      operation = XmOTHER;
   if (_XmDestinationHandler(w, pair->selection, operation, ReleaseSecondaryLock, (XtPointer)pair->target, req_event->time, req_event) != True)
   {
      cs->status = XmCONVERT_REFUSE;
      return;
   }
...
#else
      while (!(mask = XtAppPending(app))); /* Busy waiting - so that we don't lose our lock */
      if (mask & XtIMXEvent)
      { /* We have an XEvent */
...
ClipboardLoseProc(Widget w, Atom *selection)
{
   ...
   _XmConvertHandler(w, selection, &DELETE, &type, &value, &size, &fmt);
...
LoseProc(Widget w, Atom *selection)
{
   ...
   _XmConvertHandlerSetLocal();
   _XmConvertHandler(w, selection, &MOTIF_LOSE, &type, &value, &size, &fmt);
   XtFree((char *)value);
   XtRemoveCallback(w, XmNdestroyCallback, DisownCallback, (XtPointer)*selection);
}
...
/****************************************************************/
void
XmeConvertMerge(XtPointer data, Atom type, int format, unsigned long size, XmConvertCallbackStruct *cs)
{
   _XmProcessLock();
...
XmeNamedSource(Widget w, Atom sel, Time time)
{
   ...
   status = XtOwnSelection(w, sel, time, _XmConvertHandler, LoseProc, NULL);
   if (status) XtAddCallback(w, XmNdestroyCallback, DisownCallback, (XtPointer)sel);
   _XmAppUnlock(app);
   return (status);
}
...
XmeSecondaryTransfer(Widget w, Atom target, XtEnum op, Time time)
{
   ...
   static char      *atom_names[] = { XmS_MOTIF_DESTINATION, XmSINSERT_SELECTION, XmSLINK_SELECTION, XmIATOM_PAIR };
   ConvertContext    cc;
   _XmTextInsertPair pair;
   ...
   pair.target    = target;
   XtSetSelectionParameters(w, atoms[XmA_MOTIF_DESTINATION], atoms[XmAATOM_PAIR], (XtPointer)&pair, 2, 32);
   XtGetSelectionValue(w, atoms[XmA_MOTIF_DESTINATION], transfer_target, SecondaryDone, NULL, time);
   _XmAppUnlock(app);
}
...
XmeClipboardSource(Widget w, XtEnum op, Time time)
{
   ...
   if (clipboard_owner != None)
   {
	 ...
	 _XmConvertHandler to delete the selection */
         status = XtOwnSelection(w, atoms[XmACLIPBOARD], time, _XmConvertHandler, ClipboardLoseProc, NULL);
      }
      else
      {
         status = XtOwnSelection(w, atoms[XmACLIPBOARD], time, _XmConvertHandler, LoseProc, NULL);
      }
      if (status)
      {
         XtAddCallback(w, XmNdestroyCallback, DisownCallback, (XtPointer)atoms[XmACLIPBOARD]);
      }
      else
      {
         _XmAppUnlock(app);
   ...
   /* Use Motif clipboard */
   status = XmClipboardStartCopy(display, XtWindow(w), NULL, time, w, ClipboardCallback, &itemid);
   ...
   /* Call the converter to get the targets for the clipboard
     ...
     TARGETS target */
   _XmConvertHandlerSetLocal();
   if (_XmConvertHandler(w, &atoms[XmACLIPBOARD], &atoms[XmA_MOTIF_CLIPBOARD_TARGETS], &type, &value, &size, &format) == True && size != 0 && type == XA_ATOM)
   {
      targets = (Atom *)value;
      count   = size;
      ...
      for (i = 0; i < count; i++)
      {
         ...
         _XmConvertHandlerSetLocal();
         if (_XmConvertHandler(w, &atoms[XmACLIPBOARD], &targets[i], &type2, &value, &size2, &format2) == True && !(cc->flags & XmCONVERTING_PARTIAL))
         {
   ...
   /* Call the converter to get the deferred targets for the
     clipboard */
   _XmConvertHandlerSetLocal();
   if (_XmConvertHandler(w, &atoms[XmACLIPBOARD], &atoms[XmA_MOTIF_DEFERRED_CLIPBOARD_TARGETS], &type, &value, &size, &format) == True && size != 0 && type == XA_ATOM)
   {
      _XmProcessLock();
	 ...
	 functions */
      ...
      /* If there are deferred targets then the snapshot target
       ...
       item */
      _XmConvertHandlerSetLocal();
      if (_XmConvertHandler(w, &atoms[XmACLIPBOARD], &atoms[XmA_MOTIF_SNAPSHOT], &type2, &value, &size2, &format2) == True)
      {
         long            data_id;
         SnapshotRequest req;
         if (count != 0)
         ...
         for (i = 0; i < count; i++)
         {
            ...
            _XmProcessLock();
            _XmClipboardPassType(type2);
            XmClipboardCopy(display, XtWindow(w), itemid, name, NULL, 0, targets[i], &data_id);
            _XmProcessUnlock();
            /* End Critical section for MT */
   ...
   {
      _XmConvertHandlerSetLocal();
      _XmConvertHandler(w, &atoms[XmACLIPBOARD], &atoms[XmADELETE], &type, &value, &size, &format);
      XtFree((char *)value);
   }
   if (transferred != 0)
   {
...
ClipboardCallback(Widget w, long *data_id, long *target, int *reason)
{
   ...
   {
      _XmConvertHandlerSetLocal();
      if (_XmConvertHandler(w, &req->distinguisher, (Atom *)target, &type, &value, &size, &format) == True && !(cc->flags & XmCONVERTING_PARTIAL))
      {
         char    *name;
         FreeType howFree;
         ...
         XmClipboardCopyByName(display, XtWindow(w), *data_id, value, size, 0L);
         _XmProcessUnlock();
         XtFree((char *)value);
      }
      else
         XmClipboardCopyByName(display, XtWindow(w), *data_id, NULL, 0, 0L);
   }
   if (req->outstanding == 0)
   {
      ...
      _XmConvertHandlerSetLocal();
      _XmConvertHandler(w, &req->distinguisher, (Atom *)&done, &type, &value, &size, &format);
      XtFree((char *)value);
      XtFree((char *)req);
   }
}
...
/****************************************************************/
Widget
XmeDragSource(Widget w, XtPointer location_data, XEvent *event, ArgList in_args, Cardinal in_arg_count)
{
   enum
   {
      XmA_MOTIF_DROP,
   ...
   arg_count++;
   _XmConvertHandlerSetLocal();
   if (_XmConvertHandler(w, &atoms[XmA_MOTIF_DROP], &atoms[XmA_MOTIF_EXPORT_TARGETS], &type, &targets, &size, &format) == True)
   {
      XtSetArg(args[arg_count], XmNexportTargets, targets);
      arg_count++;
      XtSetArg(args[arg_count], XmNnumExportTargets, size);
      arg_count++;
   }
   else
   {
   ...
   XtSetArg(args[arg_count], XmNclientData, location_data);
   arg_count++;
...
Boolean
_XmDestinationHandler(Widget wid, Atom selection, XtEnum op, XmSelectionFinishedProc done_proc, XtPointer location_data, Time time, XSelectionRequestEvent *event)
{
   ...
   if (tc->selection == MOTIF_DROP)
   {
      ...
      i                = 0;
      XtSetArg(args[i], XmNiccHandle, &tc->real_selection);
      i++;
   ...
   if (ttrait != NULL && tc->status == XmTRANSFER_DONE_DEFAULT && ((tc->count == 0) || (tc->outstanding == 0 && !(TC_CALLED_WIDGET))))
   {
      _XmProcessLock();
      TB_internal = 1;
      _XmProcessUnlock();
                       ...
                       XmDropProcCallbackStruct *ds)
{
   ...
   XtEnum op;
   if (ds->dropAction == XmDROP_HELP || ds->operation == XmDROP_NOOP)
      op = XmOTHER;
   else
      op = ds->operation;
   (void)_XmDestinationHandler(w, MOTIF_DROP, op, NULL, (XtPointer)ds, ds->timeStamp, NULL);
}
...
XmePrimarySink(Widget w, XtEnum op, XtPointer location_data, Time time)
{
   ...
   _XmWidgetToAppContext(w);
   _XmAppLock(app);
   ret_val = _XmDestinationHandler(w, XA_PRIMARY, op, NULL, location_data, time, NULL);
   _XmAppUnlock(app);
   return ret_val;
}
...
XmeNamedSink(Widget w, Atom sel, XtEnum op, XtPointer location_data, Time time)
{
   ...
   _XmWidgetToAppContext(w);
   _XmAppLock(app);
   ret_val = _XmDestinationHandler(w, sel, op, NULL, location_data, time, NULL);
   _XmAppUnlock(app);
   return ret_val;
}
...
XmeSecondarySink(Widget w, Time time)
{
   ...
   status = XtOwnSelection(w, MOTIF_DESTINATION, time, _XmConvertHandler, LoseProc, NULL);
   if (status) XtAddCallback(w, XmNdestroyCallback, DisownCallback, (XtPointer)MOTIF_DESTINATION);
   _XmAppUnlock(app);
   return (status);
}
...
XmeClipboardSink(Widget w, XtEnum op, XtPointer location_data)
{
   ...
   _XmWidgetToAppContext(w);
   _XmAppLock(app);
   ret_val = _XmDestinationHandler(w, CLIPBOARD, op, NULL, location_data, 0, NULL);
   _XmAppUnlock(app);
   return ret_val;
}
...
XmTransferDone(XtPointer transfer_id, XmTransferStatus status)
{
   ...
   if (tc->flags & TC_IN_MULTIPLE)
   {
      ...
      XtSendSelectionRequest(tc->widget, tc->selection, XtLastTimestampProcessed(XtDisplay(tc->widget)));
   }
   if (status == XmTRANSFER_DONE_SUCCEED || status == XmTRANSFER_DONE_FAIL || status == XmTRANSFER_DONE_CONTINUE)
   {
      tc->flags |= TC_FLUSHED;
      if (status == XmTRANSFER_DONE_FAIL && tc->selection == MOTIF_DROP)
      {
         Arg args[2];
         XtSetArg(args[0], XmNtransferStatus, XmTRANSFER_FAILURE);
         XtSetArg(args[1], XmNnumDropTransfers, 0);
	   ...
	   data is normally freed, won't be called */
         if (tc->count == 0 && tc->flags & TC_EXITED_DH)
         {
                        ...
                        Atom          parm_type)
{
   ...
   if (parm != NULL)
      XtSetSelectionParameters(tc->widget, tc->real_selection, parm_type, parm, parm_length, parm_fmt);
   _XmAppUnlock(app);
}
                ...
                Time           time)
{
   ...
   assert(XtNumber(atom_names) == NUM_ATOMS);
   XInternAtoms(XtDisplay(tc->widget), atom_names, XtNumber(atom_names), False, atoms);
   if (time == 0)
   ...
   {
      XtGetSelectionValue(tc->widget, tc->real_selection, target, SelectionCallbackWrapper, (XtPointer)tc, time);
   }
   else
   {
      ...
      if (tc->drop_context == NULL)
      {
         ...
         tc->drop_context = (Widget)XmDropTransferStart(tc->drag_context, args, 3);
      }
      else
...
XmTransferStartRequest(XtPointer transfer_id)
{
   ...
   if (tc->flags & TC_IN_MULTIPLE)
   {
      ...
      TransferWarning(tc->widget, START_MULTIPLE, sel, ERROR_MULTIPLE_IN_PROGRESS);
...
XmTransferSendRequest(XtPointer transfer_id, Time time)
{
   ...
   if (!(tc->flags & TC_IN_MULTIPLE))
   {
      ...
      TransferWarning(tc->widget, END_MULTIPLE, sel, ERROR_MULTIPLE_NOT_IN_PROGRESS);
      FreeSafeAtomName(sel, howFree);
      _XmAppUnlock(app);
      return;
   }
...
/************************************************************************/
static void
SelectionCallbackWrapper(Widget wid, XtPointer client_data, Atom *selection, Atom *type, XtPointer value, unsigned long *length, int *format)
{
   enum
   {
      XmA_MOTIF_DROP,
   ...
   if (tc->outstanding == 0 && tc->status == XmTRANSFER_DONE_DEFAULT && tc->flags & TC_CALLED_CALLBACKS && !(tc->flags & TC_CALLED_WIDGET))
   {
      XmTransferTrait ttrait;
      tc->flags |= TC_CALLED_WIDGET;
      ttrait     = (XmTransferTrait)
   ...
   if (tc->selection == XA_PRIMARY && tc->outstanding == 0 && tc->count != 0 && (tc->status == XmTRANSFER_DONE_SUCCEED || tc->status == XmTRANSFER_DONE_DEFAULT) && tc->op == XmMOVE && !(tc->flags & TC_DID_DELETE))
   {
      tc->flags |= TC_DID_DELETE;
      XmTransferValue((XtPointer)tc, atoms[XmADELETE], NULL, NULL, XtLastTimestampProcessed(XtDisplay(wid)));
   }
   ...
   if (tc->outstanding == 0 && tc->flags & TC_EXITED_DH)
   {
      FinishTransfer(wid, tc);
   }
}
...
CCMatch(XtPointer x, XtPointer y)
{
   ...
   a = (_XmCCKey)x;
   b = (_XmCCKey)y;
   return (a->display == b->display && a->selection == b->selection);
}
...
XmeStandardTargets(Widget w, int count, int *tcount)
{
   ...
   assert(XtNumber(atom_names) == NUM_ATOMS);
   XInternAtoms(XtDisplay(w), atom_names, XtNumber(atom_names), False, atoms);
   targets[i] = atoms[XmATARGETS];
   i++;
   targets[i] = atoms[XmATIMESTAMP];
   i++;
   targets[i] = atoms[XmAFOREGROUND];
   i++;
   targets[i] = atoms[XmABACKGROUND];
   i++;
   targets[i] = XA_COLORMAP;
   i++;
   targets[i] = atoms[XmACLASS];
   i++;
   targets[i] = atoms[XmANAME];
   i++;
   targets[i] = atoms[XmACLIENT_WINDOW];
   i++;
   targets[i] = atoms[XmA_MOTIF_RENDER_TABLE];
   i++;
                   ...
                   XmConvertCallbackStruct *cs)
{
   ...
   else if (atoms[XmACLASS] == cs->target)
   {
           ...
           current = XtParent(current))
      {
         ...
         {
            XGetWindowProperty(XtDisplay(current), XtWindow(current), XA_WM_CLASS, 0L, 100000L, False, (Atom)AnyPropertyType, &cs->type, &cs->format, &cs->length, &bytesAfter, (unsigned char **)&cs->value);
   ...
   else if (atoms[XmANAME] == cs->target)
   {
           ...
           current = XtParent(current))
      {
         if (XtIsShell(current))
         {
            XGetWindowProperty(XtDisplay(current), XtWindow(current), XA_WM_NAME, 0L, 100000L, False, (Atom)AnyPropertyType, &type, &format, &length, &bytesAfter, &value);
            if (value != NULL) break;
         }
      }
...
XmeGetEncodingAtom(Widget w)
{
   ...
   ret_status     = XmbTextListToTextProperty(XtDisplay(w), &tmp_string, 1, (XICCEncodingStyle)XTextStyle, &tmp_prop);
                    ...
                    Boolean      *success)
{
   ...
   )
   {
      ...
      status             = XmbTextPropertyToTextList(XtDisplay(w), &text_prop, &values, &num_values);
      if (success != NULL)
      {
...
void
_XmConvertComplete(Widget wid, XtPointer value, unsigned long size, int format, Atom type, XmConvertCallbackStruct *cs)
{
