...
#define FreeSafeAtomName(val, how) \
  if (1) { \
    if (DoXFree == how) \
      XFree(val); \
    else \
...
static void SelectionCallbackWrapper(
    Widget, XtPointer, Atom *, Atom *, XtPointer, unsigned long *, int *);
static Boolean DragConvertHandler(
...
/****************************************************************/
Boolean _XmConvertHandler(Widget wid,
                          Atom *selection,
                          Atom *target,
                          Atom *type,
                          XtPointer *value,
                          ...
                          int *fmt)
{
  ...
  static char *atom_names[] = {XmS_MOTIF_DESTINATION,
                               XmSINSERT_SELECTION,
                               XmSLINK_SELECTION,
  ...
  if (my_local_convert_flag == 0) {
    ...
    if (*selection == atoms[XmA_MOTIF_DROP]) {
      ...
      cbstruct.event = (XEvent *)XtGetSelectionRequest(
          cc->drag_context, real_selection_atom, NULL);
    ...
    {
      Atom sel_atom = (real_selection_atom != None) ? real_selection_atom : *selection;
      XtGetSelectionParameters(req_widget,
                               sel_atom,
                               NULL,
                               &cbstruct.parm_type,
                               &cbstruct.parm,
                               &cbstruct.parm_length,
                               &cbstruct.parm_format);
    }
  }
  else {
  ...
  _XmProcessUnlock();
  if (cbstruct.event != NULL && ((XSelectionRequestEvent *)cbstruct.event)->requestor ==
  ...
  if (*selection != atoms[XmA_MOTIF_DESTINATION] || *target == atoms[XmA_MOTIF_LOSE_SELECTION]) {
    ...
    if (cbstruct.status == XmCONVERT_DEFAULT || cbstruct.status == XmCONVERT_MERGE) {
      ttrait = (XmTransferTrait)XmeTraitGet((XtPointer)XtClass(wid), XmQTtransfer);
      if (ttrait != NULL)
  ...
  if (cbstruct.status == XmCONVERT_DEFAULT &&
      (*target == atoms[XmAINSERT_SELECTION] || *target == atoms[XmALINK_SELECTION]))
...
/****************************************************************/
static Boolean DragConvertHandler(Widget drag_context,
                                  Atom *selection,
                                  Atom *target,
                                  Atom *type,
                                  XtPointer *value,
                                  ...
                                  int *fmt)
{
  ...
  return (_XmConvertHandler((Widget)cc->client_data, selection, target, type, value, size, fmt));
}
...
/********************************************************************/
static void SecondaryConvertHandler(Widget w,
...
/********************************************************************/
static void DisownCallback(Widget w,
...
void XmeConvertMerge(
    XtPointer data, Atom type, int format, unsigned long size, XmConvertCallbackStruct *cs)
{
  _XmProcessLock();
...
Boolean XmeNamedSource(Widget w, Atom sel, Time time)
{
  ...
  if (status)
    XtAddCallback(w, XmNdestroyCallback, DisownCallback, (XtPointer)sel);
  _XmAppUnlock(app);
  return (status);
}
...
{
  enum { XmA_MOTIF_DESTINATION, XmAINSERT_SELECTION, XmALINK_SELECTION, XmAATOM_PAIR, NUM_ATOMS };

  static char *atom_names[] = {
      XmS_MOTIF_DESTINATION, XmSINSERT_SELECTION, XmSLINK_SELECTION, XmIATOM_PAIR};
  ConvertContext cc;
  _XmTextInsertPair pair;
  ...
  XtSetSelectionParameters(
      w, atoms[XmA_MOTIF_DESTINATION], atoms[XmAATOM_PAIR], (XtPointer)&pair, 2, 32);
...
/*****************************************************************/
static void SecondaryDone(Widget wid,
...
Boolean XmeClipboardSource(Widget w, XtEnum op, Time time)
{
  ...
  if (clipboard_owner != None) {
    if (op == XmMOVE) {
      ...
      status = XtOwnSelection(
          w, atoms[XmACLIPBOARD], time, _XmConvertHandler, ClipboardLoseProc, NULL);
    }
    else {
      ...
      size != 0 && type == XA_ATOM)
  {
    ...
    for (i = 0; i < count; i++) {
      ...
      _XmConvertHandlerSetLocal();
      if (_XmConvertHandler(
              w, &atoms[XmACLIPBOARD], &targets[i], &type2, &value, &size2, &format2) == True &&
...
static void ClipboardCallback(Widget w, long *data_id, long *target, int *reason)
{
  XtPointer value;
  Atom type;
  unsigned long size;
  ...
  _XmProcessLock();
  req = (SnapshotRequest)_XmGetHashEntry(DataIdDictionary, (XmHashKey)*data_id);
  ...
  if (*reason != XmCR_CLIPBOARD_DATA_DELETE) {
    _XmConvertHandlerSetLocal();
    if (_XmConvertHandler(w, &req->distinguisher, (Atom *)target, &type, &value, &size, &format) ==
            True &&
        !(cc->flags & XmCONVERTING_PARTIAL))
    {
      char *name;
...
Widget XmeDragSource(
    Widget w, XtPointer location_data, XEvent *event, ArgList in_args, Cardinal in_arg_count)
{
                              ...
                              XSelectionRequestEvent *event)
{
  ...
  cbstruct = (XmDestinationCallbackStruct *)XtMalloc(sizeof(XmDestinationCallbackStruct));
  cbstruct->reason = XmCR_OK;
...
/* in the drop							*/
/****************************************************************/
static void DeleteDropCBStruct(Widget w,              /* unused */
...
static void DropDestinationHandler(Widget w,
                                   XtPointer client_data, /* unused */
...
Boolean XmeSecondarySink(Widget w, Time time)
{
  ...
  if (status)
    XtAddCallback(w, XmNdestroyCallback, DisownCallback, (XtPointer)MOTIF_DESTINATION);
  _XmAppUnlock(app);
  return (status);
}
...
void XmTransferDone(XtPointer transfer_id, XmTransferStatus status)
{
  ...
  if (tc->flags & TC_IN_MULTIPLE) {
    ...
    XtSendSelectionRequest(
        tc->widget, tc->selection, XtLastTimestampProcessed(XtDisplay(tc->widget)));
  }
  if (status == XmTRANSFER_DONE_SUCCEED || status == XmTRANSFER_DONE_FAIL ||
      status == XmTRANSFER_DONE_CONTINUE)
  {
    ...
    for (tb = tc->requests; tb != NULL; tb = (TransferBlock)tb->next) {
...
void XmTransferSetParameters(
    XtPointer transfer_id, XtPointer parm, int parm_fmt, unsigned long parm_length, Atom parm_type)
{
  TransferContext tc = (TransferContext)transfer_id;
  _XmWidgetToAppContext(tc->widget);
  _XmAppLock(app);
  ...
  if (parm != NULL)
    XtSetSelectionParameters(
        tc->widget, tc->real_selection, parm_type, parm, parm_length, parm_fmt);
  _XmAppUnlock(app);
}
...
void XmTransferValue(
    XtPointer transfer_id, Atom target, XtCallbackProc proc, XtPointer client_data, Time time)
{
  ...
  if (tc->selection == atoms[XmACLIPBOARD]) {
    ...
    XmClipboardInquireLength(XtDisplay(tc->widget), XtWindow(tc->widget), XmSTARGETS, &length);
  }
  if (tc->selection != atoms[XmA_MOTIF_DROP]) {
    XtGetSelectionValue(
        tc->widget, tc->real_selection, target, SelectionCallbackWrapper, (XtPointer)tc, time);
  }
  else {
...
/************************************************************************/
static void SelectionCallbackWrapper(Widget wid,
                                     XtPointer client_data,
                                     Atom *selection,
                                     Atom *type,
                                     XtPointer value,
                                     ...
                                     int *format)
{
  ...
  if (tc->outstanding == 0 && tc->status == XmTRANSFER_DONE_DEFAULT &&
      tc->flags & TC_CALLED_CALLBACKS && !(tc->flags & TC_CALLED_WIDGET))
  {
    XmTransferTrait ttrait;
    tc->flags |= TC_CALLED_WIDGET;
    ttrait = (XmTransferTrait)XmeTraitGet((XtPointer)XtClass(wid), XmQTtransfer);
  ...
  if (tc->selection == XA_PRIMARY && tc->outstanding == 0 && tc->count != 0 &&
      (tc->status == XmTRANSFER_DONE_SUCCEED || tc->status == XmTRANSFER_DONE_DEFAULT) &&
      tc->op == XmMOVE && !(tc->flags & TC_DID_DELETE))
  {
    tc->flags |= TC_DID_DELETE;
    XmTransferValue(
        (XtPointer)tc, atoms[XmADELETE], NULL, NULL, XtLastTimestampProcessed(XtDisplay(wid)));
  }
...
static Boolean CCMatch(XtPointer x, XtPointer y)
{
  _XmCCKey a, b;
  a = (_XmCCKey)x;
  b = (_XmCCKey)y;
...
static XmHashValue CCHash(XtPointer x)
{
  _XmCCKey a;
  a = (_XmCCKey)x;
...
static ConvertContext LookupContextBlock(Display *d, Atom a)
{
  ConvertContext cc;
  _XmCCKeyRec x;
  x.display = d;
...
static void ClearContextBlock(Display *d, Atom a)
{
  ConvertContext cc;
...
static void FreeTransferID(XtPointer id)
{
  TransferContext tid = (TransferContext)id;
...
static void CallDoneProcs(Widget wid, XtPointer id, XmTransferDoneCallbackStruct *ts)
{
  int i;
  TransferContext tid = (TransferContext)id;
 ...
 */
void XmeStandardConvert(Widget w,
                        XtPointer ignore, /* unused */
                        XmConvertCallbackStruct *cs)
{
  enum {
...
char *_XmTextToLocaleText(
    Widget w, XtPointer value, Atom type, int format, unsigned long length, Boolean *success)
{
...
void _XmConvertComplete(Widget wid,
                        XtPointer value,
                        unsigned long size,
                        int format,
                        Atom type,
...
XmDestinationCallbackStruct *_XmTransferGetDestinationCBStruct(XtPointer tid)
{
