...
externaldef(xmnotebookwidgetclass) WidgetClass xmNotebookWidgetClass = (WidgetClass)&xmNotebookClassRec;
   ...
   Widget w)
{
       ...
       && XtHasCallbacks(w, XmNpageChangedCallback) == XtCallbackHasSome)
   {
      ...
      cbs.page_widget      = GetChildWidget(nb,
                                       nb->notebook.current_page_number,
                                       XmPAGE);
      ...
      case XmMINOR_TAB:
         /* Install activiation callback */
         {
            ...
            if ((activeT = NB_ACTIVATABLE(child)) != NULL
               ...
               activeT->changeCB(child, TabPressed, (XtPointer)(unsigned long)(NB_IS_CHILD_MAJOR(nc->child_type) ? XmCR_MAJOR_TAB : XmCR_MINOR_TAB), True);
         }
   ...
   XrmValue *value)
{
   ...
   XmGetColors(XtScreen(w), nb->core.colormap, nb->notebook.frame_background, NULL, NULL, NULL, &pixel);
}
   ...
   XmNotebookWidget nb)
{
   ...
   ((XmActivatableTrait)XmeTraitGet((XtPointer)
                                       XtClass((Widget)nb->notebook.next_major),
                                    XmQTactivatable))
   ...
   nb->notebook.prev_major = XtVaCreateManagedWidget(MAJOR_TAB_PREV_NAME,
                                                     ...
                                                     NULL);
   ((XmActivatableTrait)XmeTraitGet((XtPointer)
                                       XtClass((Widget)nb->notebook.prev_major),
                                    XmQTactivatable))
   ...
   nb->notebook.next_minor = XtVaCreateManagedWidget(MINOR_TAB_NEXT_NAME,
                                                     ...
                                                     NULL);
   ((XmActivatableTrait)XmeTraitGet((XtPointer)
                                       XtClass((Widget)nb->notebook.next_minor),
                                    XmQTactivatable))
   ...
   nb->notebook.prev_minor = XtVaCreateManagedWidget(MINOR_TAB_PREV_NAME,
                                                     ...
                                                     NULL);
   ((XmActivatableTrait)XmeTraitGet((XtPointer)
                                       XtClass((Widget)nb->notebook.prev_minor),
                                    XmQTactivatable))
   ...
   Widget           instigator)
{
   ...
   if (nb->notebook.major_pos == LEFT)
      x += NB_MAJOR_MAX(nb, nb->notebook.major_width, nb->notebook.major_scroller_width) + nb->notebook.back_page_size;
   else if (nb->notebook.major_pos == TOP)
      y += NB_MAJOR_MAX(nb, nb->notebook.major_height, nb->notebook.major_scroller_height) + nb->notebook.back_page_size;
   if (nb->notebook.minor_pos == LEFT)
      x += NB_MINOR_MAX(nb, nb->notebook.minor_width, nb->notebook.minor_scroller_width) + nb->notebook.back_page_size;
   else if (nb->notebook.minor_pos == TOP)
      y += NB_MINOR_MAX(nb, nb->notebook.minor_height, nb->notebook.minor_scroller_height) + nb->notebook.back_page_size;
          ...
          || NB_IS_CHILD_STATUS(nc->child_type))
      {
         if (nc->active && nc->page_number == nb->notebook.current_page_number)
         {
            if (NB_IS_CHILD_PAGE(nc->child_type))
               ...
               ShowChild(child, instigator, x1, y1, nb->notebook.status_width, nb->notebook.status_height);
         }
         else
         {
   ...
   if (nb->notebook.scroller != NULL)
   {
      ...
      ShowChild(child, instigator, x2, y1, nb->notebook.scroller_width, nb->notebook.scroller_height);
   }
}
   ...
   Widget           instigator)
{
   ...
   /*
     * Initialize
     */
   first_major = nb->notebook.first_major ? NotebookConstraint(nb->notebook.first_major)->page_number : nb->notebook.first_page_number - 1;
   top_major   = nb->notebook.top_major ? NotebookConstraint(nb->notebook.top_major)->page_number : nb->notebook.first_page_number - 1;
   ...
   if (nb->notebook.back_page_pos == XmBOTTOM_RIGHT && nb->notebook.orientation == XmHORIZONTAL)
   {
      ...
      if (NB_IS_CHILD_MAJOR_SCROLLER(nb->notebook.need_scroller))
      {
         ...
         limit     = nb->notebook.frame_height - MAX(0, (int)(nb->notebook.major_scroller_height - nb->notebook.back_page_size)) - nb->notebook.major_height - nb->notebook.shadow_thickness;
   ...
   else if (nb->notebook.back_page_pos == XmBOTTOM_RIGHT && nb->notebook.orientation == XmVERTICAL)
   {
      ...
      if (NB_IS_CHILD_MAJOR_SCROLLER(nb->notebook.need_scroller))
      {
         ...
         limit     = nb->notebook.frame_width - MAX(0, (int)(nb->notebook.major_scroller_width - nb->notebook.back_page_size)) - nb->notebook.major_width - nb->notebook.shadow_thickness;
   ...
   else if (nb->notebook.back_page_pos == XmBOTTOM_LEFT && nb->notebook.orientation == XmVERTICAL)
   {
      ...
      if (NB_IS_CHILD_MAJOR_SCROLLER(nb->notebook.need_scroller))
      {
         ...
         sx2       = margin_w;
         limit     = margin_w + MAX(nb->notebook.back_page_size, nb->notebook.major_scroller_width) + nb->notebook.shadow_thickness;
         x        -= nb->notebook.major_scroller_width + spacing;
      }
      else
         x -= spacing;
   ...
   else if (nb->notebook.back_page_pos == XmTOP_RIGHT && nb->notebook.orientation == XmHORIZONTAL)
   {
      ...
      if (NB_IS_CHILD_MAJOR_SCROLLER(nb->notebook.need_scroller))
      {
         ...
         sy2       = margin_h;
         limit     = margin_h + MAX(nb->notebook.back_page_size, nb->notebook.major_scroller_height) + nb->notebook.shadow_thickness;
         y        -= nb->notebook.major_scroller_height + spacing;
      }
      else
         y -= spacing;
   ...
   else if (nb->notebook.back_page_pos == XmTOP_RIGHT && nb->notebook.orientation == XmVERTICAL)
   {
      ...
      if (NB_IS_CHILD_MAJOR_SCROLLER(nb->notebook.need_scroller))
      {
         ...
         sx1       = x;
         sx2       = nb->notebook.frame_width + nb->notebook.back_page_size - nb->notebook.major_scroller_width;
         limit     = nb->notebook.frame_width - MAX(0, (int)(nb->notebook.major_scroller_width - nb->notebook.back_page_size)) - nb->notebook.major_width - nb->notebook.shadow_thickness;
         x        += nb->notebook.major_scroller_width + spacing;
      }
      else
   ...
   for (i = 0; i < nb->composite.num_children; i++)
   {
      ...
      if (NB_IS_CHILD_MAJOR(nc->child_type))
      {
         ...
         else
         {
            ...
            /* Make tab visible */
            ShowChild(child, instigator, tmpx, tmpy, nb->notebook.major_width, nb->notebook.major_height);
            tab_count++;
   ...
   if (NB_IS_CHILD_MAJOR_SCROLLER(nb->notebook.need_scroller) && tab_count)
   {
      ...
      /* show the scrollers */
      ShowChild(nb->notebook.prev_major, instigator, sx1, sy1, nb->notebook.major_scroller_width, nb->notebook.major_scroller_height);
      ShowChild(nb->notebook.next_major, instigator, sx2, sy2, nb->notebook.major_scroller_width, nb->notebook.major_scroller_height);
   }
   else
   {
   ...
   Widget           instigator)
{
       ...
       && nb->notebook.orientation == XmHORIZONTAL)
   {
      ...
      limit = nb->notebook.real_binding_width + nb->notebook.frame_width - nb->notebook.minor_width - nb->notebook.shadow_thickness;
      if (NB_IS_CHILD_MINOR_SCROLLER(nb->notebook.need_scroller))
      {
         sy1 = sy2 = y + nb->notebook.back_page_size;
         sx1       = x;
         sx2       = nb->notebook.real_binding_width + nb->notebook.frame_width + nb->notebook.back_page_size - nb->notebook.minor_scroller_width;
         limit     = nb->notebook.real_binding_width + nb->notebook.frame_width - nb->notebook.minor_width - nb->notebook.shadow_thickness - MAX(0, (int)(nb->notebook.minor_scroller_width - nb->notebook.back_page_size));
            ...
            && nb->notebook.orientation == XmVERTICAL)
   {
      ...
      limit = nb->notebook.real_binding_width + nb->notebook.frame_height - nb->notebook.minor_height - nb->notebook.shadow_thickness;
      if (NB_IS_CHILD_MINOR_SCROLLER(nb->notebook.need_scroller))
      {
         sx1 = sx2 = x + nb->notebook.back_page_size;
         sy1       = y;
         sy2       = nb->notebook.real_binding_width + nb->notebook.frame_height + nb->notebook.back_page_size - nb->notebook.minor_scroller_height;
         limit     = nb->notebook.real_binding_width + nb->notebook.frame_height - MAX(0, (int)(nb->notebook.minor_scroller_height - nb->notebook.back_page_size)) - nb->notebook.minor_height - nb->notebook.shadow_thickness;
            ...
            && nb->notebook.orientation == XmHORIZONTAL)
   {
      ...
      x        = margin_w + nb->notebook.back_page_size + nb->notebook.frame_width - MAX(0, (int)(nb->notebook.back_page_size - nb->notebook.real_binding_width)) - nb->notebook.minor_width - offset;
      y        = nb->notebook.frame_height;
      fixed    = y + NB_MINOR_TAB_STEP(nb->notebook.back_page_size);
      limit    = margin_w + nb->notebook.back_page_size + nb->notebook.shadow_thickness;
      if (NB_IS_CHILD_MINOR_SCROLLER(nb->notebook.need_scroller))
      {
         sy1 = sy2 = y + nb->notebook.back_page_size;
         sx1       = margin_w + nb->notebook.back_page_size + nb->notebook.frame_width - MAX(0, (int)(nb->notebook.back_page_size - nb->notebook.real_binding_width)) - nb->notebook.minor_scroller_width;
         sx2       = margin_w;
         limit     = margin_w + MAX(nb->notebook.minor_scroller_width, nb->notebook.back_page_size) + nb->notebook.shadow_thickness;
         x        -= nb->notebook.minor_scroller_width + spacing;
      }
      else
         x -= nb->notebook.shadow_thickness;
            ...
            && nb->notebook.orientation == XmHORIZONTAL)
   {
      ...
      if (NB_IS_CHILD_MINOR_SCROLLER(nb->notebook.need_scroller))
      {
         ...
         sx1       = x;
         sx2       = nb->notebook.real_binding_width + nb->notebook.frame_width + nb->notebook.back_page_size - nb->notebook.minor_scroller_width;
         limit     = nb->notebook.real_binding_width + nb->notebook.frame_width - MAX(0, (int)(nb->notebook.minor_scroller_width - nb->notebook.back_page_size)) - nb->notebook.minor_width - nb->notebook.shadow_thickness;
         x        += nb->notebook.minor_scroller_width + spacing;
      }
      else
         x += nb->notebook.shadow_thickness;
            ...
            && nb->notebook.orientation == XmVERTICAL)
   {
      ...
      x        = nb->notebook.frame_width;
      y        = margin_h + nb->notebook.back_page_size + nb->notebook.frame_height - nb->notebook.minor_height - backpage_h - offset;
      fixed    = x + NB_MINOR_TAB_STEP(nb->notebook.back_page_size);
      limit    = margin_h + nb->notebook.back_page_size + nb->notebook.shadow_thickness;
      if (NB_IS_CHILD_MINOR_SCROLLER(nb->notebook.need_scroller))
      {
         sx1 = sx2 = x + nb->notebook.back_page_size;
         sy1       = margin_h + nb->notebook.back_page_size + nb->notebook.frame_height - nb->notebook.minor_scroller_height - backpage_h;
         sy2       = margin_h;
         limit     = margin_h + MAX(nb->notebook.back_page_size, nb->notebook.minor_scroller_height) + nb->notebook.shadow_thickness;
         y        -= (nb->notebook.minor_scroller_height + spacing);
      }
      else
         y -= nb->notebook.shadow_thickness;
	    ...
	    && nb->notebook.orientation == XmVERTICAL) */
   {
      ...
      margin_h = NB_MAJOR_MAX(nb, nb->notebook.major_height, nb->notebook.major_scroller_height);
      x        = margin_w + nb->notebook.back_page_size - nb->notebook.minor_width;
      y        = margin_h + nb->notebook.back_page_size + nb->notebook.frame_height - nb->notebook.minor_height - MAX(0, (int)(nb->notebook.back_page_size - nb->notebook.real_binding_width)) - offset;
      fixed    = x - NB_MINOR_TAB_STEP(nb->notebook.back_page_size);
      limit    = margin_h + nb->notebook.back_page_size + nb->notebook.shadow_thickness;
      if (NB_IS_CHILD_MINOR_SCROLLER(nb->notebook.need_scroller))
      {
         sx1 = sx2 = margin_w - nb->notebook.minor_scroller_width;
         sy1       = margin_h + nb->notebook.back_page_size + nb->notebook.frame_height - MAX(0, (int)(nb->notebook.back_page_size - nb->notebook.real_binding_width)) - nb->notebook.minor_scroller_height;
         sy2       = margin_h;
         limit     = margin_h + MAX(nb->notebook.back_page_size, nb->notebook.minor_scroller_height) + nb->notebook.shadow_thickness;
         y        -= nb->notebook.minor_scroller_height + spacing;
      }
   ...
   for (i = 0; i < nb->composite.num_children; i++)
   {
      ...
      if (NB_IS_CHILD_MINOR(nc->child_type))
      {
         ...
         else
         {
            ...
            /* Make tab visible */
            ShowChild(child, instigator, tmpx, tmpy, nb->notebook.minor_width, nb->notebook.minor_height);
            tab_count++;
            /* Calculate the next position */
            if (nb->notebook.binding_pos == LEFT)
   ...
   if (NB_IS_CHILD_MINOR_SCROLLER(nb->notebook.need_scroller) && tab_count)
   {
      ...
      /* show the scrollers */
      ShowChild(nb->notebook.prev_minor, instigator, sx1, sy1, nb->notebook.minor_scroller_width, nb->notebook.minor_scroller_height);
      ShowChild(nb->notebook.next_minor, instigator, sx2, sy2, nb->notebook.minor_scroller_width, nb->notebook.minor_scroller_height);
   }
   else
   {
   ...
   int              scroll)
{
   ...
   spacing           = MAX(nb->notebook.major_spacing, nb->notebook.shadow_thickness);
   num_visible_major = (int)(frame_dim - nb->notebook.back_page_size - nb->notebook.shadow_thickness) / (int)(tab_dim + spacing);
   if (num_visible_major < num_major)
   {
      nb->notebook.need_scroller = XmMAJOR_TAB_SCROLLER;
      num_visible_major          = (int)(frame_dim - nb->notebook.back_page_size - scroller_dim - MAX(0, (int)(scroller_dim - nb->notebook.back_page_size)) - nb->notebook.shadow_thickness) / (int)(tab_dim + spacing);
   }
   /* Minor tab */
   if (nb->notebook.orientation == XmHORIZONTAL)
   {
   ...
   spacing           = MAX(nb->notebook.minor_spacing, nb->notebook.shadow_thickness);
   num_visible_minor = (int)(frame_dim - MAX(0, (int)(nb->notebook.back_page_size - nb->notebook.real_binding_width)) - nb->notebook.shadow_thickness) / (int)(tab_dim + spacing);
   if (num_visible_minor < num_minor)
   {
      ...
      num_visible_minor = (int)(frame_dim - MAX(0, (int)(nb->notebook.back_page_size - nb->notebook.real_binding_width)) - scroller_dim - MAX(0, (int)(scroller_dim - nb->notebook.back_page_size)) - nb->notebook.shadow_thickness) / (int)(tab_dim + spacing);
   }
   /*
   ...
   Dimension        height)
{
   ...
   pixmap = nb->notebook.spiral_pixmap = XCreatePixmap(XtDisplay(nb), XtWindow(nb), pw, ph, nb->core.depth);
   /*
   ...
   XDrawArc(XtDisplay(nb), pixmap, nb->manager.bottom_shadow_GC, hx, hy, hd, hd, 225 * 64, -180 * 64);
   ...
   for (i = 1; i < sd; i++)
      XDrawArc(XtDisplay(nb), pixmap, nb->notebook.frame_gc, sx + i, sy + i, sw - i, sh - i, a1 * 64, a2 * 64);
   XSetLineAttributes(XtDisplay(nb), nb->notebook.frame_gc, MAX(0, sd - 2), LineSolid, CapRound, JoinMiter);
   XDrawArc(XtDisplay(nb), pixmap, nb->notebook.frame_gc, sx + (sd / 2), sy + (sd / 2), sw, sh, a3 * 64, a4 * 64);
   XDrawArc(XtDisplay(nb), pixmap, nb->manager.top_shadow_GC, sx, sy, sw, sh, a1 * 64, a2 * 64);
   XDrawArc(XtDisplay(nb), pixmap, nb->manager.bottom_shadow_GC, sx + sd, sy + sd, sw - sd, sh - sd, a1 * 64, a2 * 64);
}
   ...
   Region           region)
{
   ...
   x = y = 0;
   if (nb->notebook.major_pos == LEFT)
      x += NB_MAJOR_MAX(nb, nb->notebook.major_width, nb->notebook.major_scroller_width) + nb->notebook.back_page_size + nb->notebook.frame_width;
   else if (nb->notebook.major_pos == TOP)
      y += NB_MAJOR_MAX(nb, nb->notebook.major_height, nb->notebook.major_scroller_height) + nb->notebook.back_page_size + nb->notebook.frame_height;
   if (nb->notebook.minor_pos == LEFT)
      x += NB_MINOR_MAX(nb, nb->notebook.minor_width, nb->notebook.minor_scroller_width) + nb->notebook.back_page_size;
   else if (nb->notebook.minor_pos == TOP)
      y += NB_MINOR_MAX(nb, nb->notebook.minor_height, nb->notebook.minor_scroller_height) + nb->notebook.back_page_size;
   ...
   Pixmap           pixmap)
{
   ...
   XFillRectangle(XtDisplay(nb), XtWindow(nb), nb->notebook.binding_gc, x, y, width - 1, height - 1);
}
   ...
   Region           region)          /* unused */
{
   ...
   if (nb->notebook.shadow_thickness)
   {
      ...
      rect.width  = width;
      rect.height = height;
      XFillRectangle(XtDisplay(nb), XtWindow(nb), nb->notebook.frame_gc, x, y, width + 1, height + 1);
      ...
      /* draw the shadow */
      _XmRegionDrawShadow(XtDisplay(nb), XtWindow(nb), nb->manager.top_shadow_GC, nb->manager.bottom_shadow_GC, (XmRegion)shadow_region, nb->core.border_width, nb->notebook.shadow_thickness, XmSHADOW_OUT);
   ...
   else
   {
          ...
          && !NB_IS_CHILD_JOINSIDE(nb->notebook.top_major))
      {
		...
		to allow negative y values */
         ...
         XDrawRectangle(XtDisplay(nb), XtWindow(nb), nb->notebook.frame_gc, rect.x, rect.y, rect.width, rect.height);
      }
      /* adding the top minor tab area to the shadow region */
      if (nb->notebook.top_minor
          && NB_IS_VISIBLE(nb->notebook.top_minor)
   ...
   Region           region)
{
   ...
   {
      p[1].x = NB_MAJOR_MAX(nb, nb->notebook.major_width, nb->notebook.major_scroller_width) + nb->notebook.back_page_size + nb->notebook.frame_width + binding_width - 1;
      p[1].y = nb->notebook.frame_height;
   ...
   {
      p[1].x = nb->notebook.back_page_size + NB_MINOR_MAX(nb, nb->notebook.minor_width, nb->notebook.minor_scroller_width) - 1;
      p[1].y = nb->notebook.real_binding_width - binding_width;
      p[2].x = 0;
      p[2].y = binding_width + nb->notebook.frame_height;
      p[3].x = nb->notebook.frame_width;
   ...
   {
      p[1].x = nb->notebook.real_binding_width - binding_width;
      p[1].y = nb->notebook.back_page_size + NB_MINOR_MAX(nb, nb->notebook.minor_height, nb->notebook.minor_scroller_height) - 1;
      p[2].x = binding_width + nb->notebook.frame_width;
      p[2].y = 0;
      p[3].x = 0;
      p[3].y = nb->notebook.frame_height;
   ...
   {
      p[1].x = nb->notebook.frame_width;
      p[1].y = nb->notebook.back_page_size + nb->notebook.frame_height + binding_width + NB_MAJOR_MAX(nb, nb->notebook.major_height, nb->notebook.major_scroller_height) - 1;
      p[2].x = 0;
      p[2].y = -(binding_width + nb->notebook.frame_height);
      p[3].x = -nb->notebook.frame_width;
      p[3].y = 0;
   ...
   {
      p[1].x = nb->notebook.back_page_size + nb->notebook.frame_width + binding_width + NB_MAJOR_MAX(nb, nb->notebook.major_width, nb->notebook.major_scroller_width) - 1;
      p[1].y = nb->notebook.back_page_size + NB_MINOR_MAX(nb, nb->notebook.minor_height, nb->notebook.minor_scroller_height) - 1;
      p[2].x = -(binding_width + nb->notebook.frame_width);
      p[2].y = 0;
      p[3].x = 0;
   ...
   if (nb->notebook.binding_type == XmSPIRAL || nb->notebook.binding_type == XmPIXMAP || nb->notebook.binding_type == XmSOLID || nb->notebook.binding_type == XmNONE || nb->notebook.binding_type == XmPIXMAP_OVERLAP_ONLY)
   {
   ...
   if (nb->notebook.binding_type == XmSOLID || nb->notebook.binding_type == XmPIXMAP || nb->notebook.binding_type == XmPIXMAP_OVERLAP_ONLY)
   {
      ...
      while (limit > 0)
      {
         ...
         XDrawLines(XtDisplay(nb), XtWindow(nb), nb->notebook.foreground_gc, &(p[1]), 4, CoordModePrevious);
         ...
         if (delta >= 2 && limit > delta - 2)
         {
            ...
            while (back > 0)
            {
               XDrawLines(XtDisplay(nb), XtWindow(nb), nb->notebook.background_gc, q, 3, CoordModePrevious);
               q[0].y += NB_ENFORCE_SIGN(dy, 1);
               q[0].x += NB_ENFORCE_SIGN(dx, 1);
      ...
      p[1].x = mx;
      p[1].y = my;
      XSetLineAttributes(XtDisplay(nb), nb->notebook.foreground_gc, 2, LineSolid, CapButt, JoinMiter);
   ...
   Boolean           adjust)
{
   ...
   /* adjust page's width */
   page_width = MAX((int)page_width, (int)(status_width + nb->notebook.margin_width + scroller_width));
   ...
   switch (nb->notebook.binding_type)
   {
      ...
      case XmPIXMAP:
         ...
         {
            w = h = 0;
            XmeGetPixmapData(XtScreen(nb), nb->notebook.binding_pixmap, NULL, NULL, NULL, NULL, NULL, NULL, &w, &h);
            if (nb->notebook.orientation == XmHORIZONTAL)
            {
   ...
   frame_width  = page_width + (nb->notebook.shadow_thickness * 2) + (nb->notebook.margin_width * 2) + 1;
   frame_height = page_height + status_height + (nb->notebook.shadow_thickness * 2) + (nb->notebook.margin_height * 3) + 1;
   /*
   ...
   *preferred_height = frame_height + nb->notebook.back_page_size;
   ...
   else
   {
      ...
      *preferred_height += real_binding_width + NB_MAJOR_MAX(nb, major_height, major_scroller_height);
   }
   /*
   ...
   XtWidgetGeometry *desired)
{
   ...
   /* Calculate status w/h based on page,margin,& scroller w/h */
   nb->notebook.status_width  = MAX(0, (int)(nb->notebook.page_width - nb->notebook.margin_width - nb->notebook.scroller_width));
   nb->notebook.status_height = nb->notebook.scroller_height = MAX(nb->notebook.status_height, nb->notebook.scroller_height);
   /* Calculate the notebook frame size based on page size,shadows,& margins*/
   nb->notebook.frame_width  = nb->notebook.page_width + (nb->notebook.shadow_thickness * 2) + (nb->notebook.margin_width * 2) + 1;
   nb->notebook.frame_height = nb->notebook.page_height + nb->notebook.status_height + (nb->notebook.shadow_thickness * 2) + (nb->notebook.margin_height * 3) + 1;
}
   ...
   for (i = 0; i < nb->composite.num_children; i++)
   {
      ...
      if (XtIsManaged(child))
      {
         ...
         if (nc->page_number == XmUNSPECIFIED_PAGE_NUMBER)
         {
            ...
            else if (NB_IS_CHILD_MAJOR(nc->child_type) || NB_IS_CHILD_MINOR(nc->child_type) || NB_IS_CHILD_STATUS(nc->child_type))
            {
   ...
   Cardinal *num_params)
{
   ...
   else if (traverse_to == _PREVIOUS)
   {
      ...
      if ((prev) && NB_IS_HIDDEN(prev))
      {
         if (NB_IS_CHILD_MAJOR(nc->child_type))
            FlipTabs(
               (MaxIsRightUp(nb, nc->child_type)) ? nb->notebook.prev_major : nb->notebook.next_major,
               NULL,
               NULL);
         else
            FlipTabs(
               (MaxIsRightUp(nb, nc->child_type)) ? nb->notebook.prev_minor : nb->notebook.next_minor,
               NULL,
               NULL);
      }
      if (prev)
   ...
   else if (traverse_to == _NEXT)
   {
      ...
      if ((next) && NB_IS_HIDDEN(next))
      {
         if (NB_IS_CHILD_MAJOR(nc->child_type))
            FlipTabs(
               (MaxIsRightUp(nb, nc->child_type)) ? nb->notebook.next_major : nb->notebook.prev_major,
               NULL,
               NULL);
         else
            FlipTabs(
               (MaxIsRightUp(nb, nc->child_type)) ? nb->notebook.next_minor : nb->notebook.prev_minor,
               NULL,
               NULL);
      }
      if (next)
   ...
   unsigned char    direction)
{
   ...
   {
      top_major_page = nb->notebook.top_major ? NotebookConstraint(nb->notebook.top_major)->page_number : nb->notebook.first_page_number - 1;
      while ((!target_found) && (i < nb->composite.num_children))
      {
         child = nb->composite.children[i];
         nc    = NotebookConstraint(child);
   ...
   XmNotebookWidget nb)
{
   ...
   if (nb->notebook.scroller_status == DEFAULT_USED)
   {
      if ((nb->notebook.current_page_number == nb->notebook.last_page_number)
         ...
         XtVaSetValues(nb->notebook.scroller_child, XmNarrowSensitivity, XmARROWS_DECREMENT_SENSITIVE, NULL);
      else if (nb->notebook.current_page_number == nb->notebook.first_page_number)
         XtVaSetValues(nb->notebook.scroller_child, XmNarrowSensitivity, XmARROWS_INCREMENT_SENSITIVE, NULL);
      else
