...
static void DropDestroyCB(Widget    w,
                          ...
                          XtPointer callData);
static void DropTransferProc(Widget w, XtPointer ignore, XmSelectionCallbackStruct *ds);
static void SetDropContext(Widget w);
static void DeleteDropContext(Widget w);
static void TextSecondaryWrapper(Widget, XtPointer, XmSelectionCallbackStruct *);
static void TextConvertCallback(Widget, XtPointer, XmConvertCallbackStruct *);
static void TextDestinationCallback(Widget, XtPointer, XmDestinationCallbackStruct *);
                    ...
                    XmTransferDoneCallbackStruct *ts) /* unused */
{
   ...
   if (prim_select->num_chars > 0)
   {
      ...
      _XmTextSetDestinationSelection(w, tw->text.cursor_position, False, prim_select->time);
      (*tw->text.source->SetSelection)(tw->text.source, data->anchor, tw->text.cursor_position, prim_select->time);
   }
...
static void
TextSecondaryWrapper(Widget w, XtPointer closure, XmSelectionCallbackStruct *ds)
{
   ...
   XtPointer      tid)
{
   ...
   else if (_insert_select->select_type == XmDEST_SELECT)
   {
      if ((*tw->text.source->GetSelection)(tw->text.source, &left, &right) && left != right)
      {
         if (tw->text.cursor_position < left || tw->text.cursor_position > right || !tw->text.input->data->pendingdelete)
         {
   ...
   {
      if ((total_value = _XmTextToLocaleText(w, value, *type, *format, *length, NULL)) != NULL)
      {
   ...
   { /* it must be either CS_OF_ENCODING or TEXT */
      block.ptr = (char *)value;
      /* NOTE: casting *length could result in a truncated long. */
      block.length = (unsigned)*length;
      block.format = XmFMT_8_BIT;
   }
   if (_XmTextModifyVerify(tw, (XEvent *)_insert_select->event, &left, &right, &cursorPos, &block, &newblock, &freeBlock))
   {
      if ((*tw->text.source->Replace)(tw, (XEvent *)_insert_select->event, &left, &right, &newblock, False) != EditDone)
      {
      ...
      else
      {
         ...
         _XmTextSetCursorPosition(w, cursorPos);
         _XmTextSetDestinationSelection(w, tw->text.cursor_position, False, _insert_select->event->time);
   ...
   XtPointer      tid)
{
   ...
   if (supports_text && supports_encoding_data)
      ...
      target = XA_STRING;
   XmTransferValue(tid, target, (XtCallbackProc)TextSecondaryWrapper, closure, _insert_select->event->time);
}
   ...
   */
   ...
   else if (*target == XA_STRING)
   {
      ...
      *format = 8;
      ...
      status         = XmbTextListToTextProperty(XtDisplay(tw), &tmp_value, 1, (XICCEncodingStyle)XStringStyle, &tmp_prop);
      XtFree(tmp_value);
   ...
   else if (*target == atoms[XmACOMPOUND_TEXT])
   {
      ...
      *format = 8;
      ...
      tmp_prop.value = NULL;
      tmp_value      = _XmStringSourceGetString(tw, left, right, False);
      status         = XmbTextListToTextProperty(XtDisplay(tw), &tmp_value, 1, (XICCEncodingStyle)XCompoundTextStyle, &tmp_prop);
      XtFree(tmp_value);
      if (status == Success || status > 0)
      {
         /* NOTE: casting tmp_prop.nitems could result in a truncated long. */
   ...
   else if (*target == atoms[XmAUTF8_STRING])
   {
      ...
      *format = 8;
      ...
      tmp_prop.value = NULL;
      tmp_value      = _XmStringSourceGetString(tw, left, right, False);
      status         = XmbTextListToTextProperty(XtDisplay(tw), &tmp_value, 1, (XICCEncodingStyle)XUTF8StringStyle, &tmp_prop);
      XtFree(tmp_value);
      if (status == Success || status > 0)
      {
         /* NOTE: casting tmp_prop.nitems could result in a truncated long. */
   ...
   else if (*target == atoms[XmADELETE])
   {
      ...
      block.length       = 0;
      block.format       = XmFMT_8_BIT;
      if (_XmTextModifyVerify(tw, event, &left, &right, &cursorPos, &block, &newblock, &freeBlock))
      {
         if ((*tw->text.source->Replace)(tw, event, &left, &right, &newblock, False) != EditDone)
         {
            if (freeBlock && newblock.ptr) XtFree(newblock.ptr);
            return False;
         }
         else
         {
            ...
            {
               if ((*selection == atoms[XmACLIPBOARD]) || (req_event != NULL && req_event->requestor != XtWindow((Widget)tw)))
                  _XmTextSetCursorPosition(w, cursorPos);
            }
   ...
   Atom  *selection)
{
   ...
   if (*selection == XA_PRIMARY && _XmStringSourceHasSelection(source))
   {
      ...
      (*source->SetSelection)(source, 1, -999, XtLastTimestampProcessed(XtDisplay(w)));
           ...
           XmDestinationCallbackStruct *ds)
{
   ...
   if (cb->operation & XmDROP_MOVE && w == initiator && ((*tw->text.source->GetSelection)(tw->text.source, &left, &right) && left != right && insert_pos >= left && insert_pos <= right))
   {
              ...
              XmSelectionCallbackStruct *ds)
{
   ...
   */
   ...
   {
      select_pos = (*tw->text.output->XYToPos)(tw, (Position)point->x, (Position)point->y);
   }
   else
   {
   ...
   {
      if ((*tw->text.source->GetSelection)(tw->text.source, &left, &right) && left != right && select_pos > left && select_pos < right)
      {
         XtFree((char *)ds->value);
         ds->value = NULL;
         return;
        ...
        XmSelectionCallbackStruct *ds)
{
   ...
   if (ds->selection != atoms[XmACLIPBOARD] && !(ds->length) && ds->type != atoms[XmANULL])
   {
   ...
   */
   if (ds->type == atoms[XmANULL])
   {
      ...
      if (_prim_select->num_chars > 0 && data->selectionMove)
      {
         ...
         _XmTextSetCursorPosition(w, cursorPos);
         _XmTextSetDestinationSelection(w, tw->text.cursor_position, False, _prim_select->time);
         (*tw->text.source->SetSelection)(tw->text.source, data->anchor, tw->text.cursor_position, _prim_select->time);
      }
      _XmProcessUnlock();
   }
   else
   {
      ...
      if (ds->type == atoms[XmACOMPOUND_TEXT] ||
          ...
          ds->type == XA_STRING)
      {
         if ((total_value = _XmTextToLocaleText(w, ds->value, ds->type, ds->format, ds->length, NULL)) != NULL)
         {
            block.ptr    = total_value;
            block.length = strlen(block.ptr);
         }
      ...
      if (ds->selection == atoms[XmACLIPBOARD])
      {
         ...
         {
            if (tw->text.input->data->pendingdelete && replace_from >= left && replace_to <= right)
            {
               replace_from  = left;
               replace_to    = right;
               dest_disjoint = False;
	 ...
	 cursor shifting during the Replace operation */
      ...
      if (_XmTextModifyVerify(tw, ds->event, &replace_from, &replace_to, &cursorPos, &block, &newblock, &freeBlock))
      {
         _XmProcessLock();
         _prim_select->num_chars = _XmTextCountCharacters(newblock.ptr,
                                                          newblock.length);
         _XmProcessUnlock();
         if ((*tw->text.source->Replace)(tw, ds->event, &replace_from, &replace_to, &newblock, False) != EditDone)
         {
         ...
         else
         {
            ...
            if ((*tw->text.source->GetSelection)(tw->text.source, &left, &right))
            {
               if (ds->selection == atoms[XmACLIPBOARD])
               {
                  ...
                  if (left != right && (!dest_disjoint || !tw->text.add_mode))
                     (*source->SetSelection)(source, tw->text.cursor_position, tw->text.cursor_position, _prim_select->time);
               }
               else
               {
                  if (data->selectionMove)
                  {
                     ...
                     {
                        _prim_select->position = replace_from - _prim_select->num_chars;
                     }
                     else
                     {
            ...
            else
            {
               ...
               if (ds->selection == atoms[XmACLIPBOARD])
                  data->anchor = replace_from;
               else if (!data->selectionMove && !tw->text.add_mode && _prim_select->num_chars != 0)
                  data->anchor = _prim_select->position;
               _XmProcessUnlock();
            }
                 ...
                 XmSelectionCallbackStruct *ds)
{
   ...
   if (ds->type == atoms[XmANULL])
   {
      if (transfer_rec->num_chars > 0 && transfer_rec->move)
      {
         ...
         _XmTextSetCursorPosition((Widget)tw, cursorPos);
         _XmTextSetDestinationSelection((Widget)tw, tw->text.cursor_position, False, XtLastTimestampProcessed(XtDisplay(w)));
         (*tw->text.source->SetSelection)(tw->text.source, data->anchor, tw->text.cursor_position, XtLastTimestampProcessed(XtDisplay(w)));
         if (ds->value)
         {
            XtFree((char *)ds->value);
            ds->value = NULL;
   ...
   if (!ds->value || (ds->type != atoms[XmACOMPOUND_TEXT] &&
#if XM_UTF8
                      ds->type != atoms[XmAUTF8_STRING] &&
#endif
                      ds->type != CS_OF_ENCODING && ds->type != XA_STRING))
   {
   ...
   block.format = XmFMT_8_BIT;
   if (data->pendingdelete && (*tw->text.source->GetSelection)(tw->text.source, &left, &right) && (left != right))
   {
   ...
   tw->text.pendingoff = FALSE;
   if (_XmTextModifyVerify(tw, ds->event, &insertPosLeft, &insertPosRight, &cursorPos, &block, &newblock, &freeBlock))
   {
      if ((*tw->text.source->Replace)(tw, ds->event, &insertPosLeft, &insertPosRight, &newblock, False) != EditDone)
      {
      ...
      else
      {
         ...
         if ((*tw->text.source->GetSelection)(tw->text.source, &left, &right))
         {
            if (transfer_rec->move && left < insertPosLeft)
               transfer_rec->insert_pos = insertPosLeft - transfer_rec->num_chars;
            if (cursorPos < left || cursorPos > right)
               tw->text.pendingoff = TRUE;
         }
         else
         {
            if (!transfer_rec->move && !tw->text.add_mode && transfer_rec->num_chars != 0)
...
_XmTextGetDropReciever(Widget w)
{
   ...
   if (!XFindContext(display, (Window)screen, loc_context, (char **)&widget))
   {
      return widget;
   }
   return NULL;
                    ...
                    XmConvertCallbackStruct *cs)
{
   ...
   static char *atom_names[] = { XmSDELETE, XmS_MOTIF_LOSE_SELECTION, XmS_MOTIF_EXPORT_TARGETS, XmSTEXT, XmSCOMPOUND_TEXT, XmSTARGETS, XmS_MOTIF_CLIPBOARD_TARGETS, XmSCLIPBOARD,
#if XM_UTF8
                                 XmSUTF8_STRING
#endif
   };
   ...
   if (!_XmTextConvert(w, &cs->selection, &cs->target, &type, &value, &size, &format, (Widget)cs->source_data, cs->event))
   {
      value  = NULL;
      type   = XA_INTEGER;
      size   = 0;
                        ...
                        XmDestinationCallbackStruct *ds)
{
   ...
   */
   ...
   else if (ds->selection == XA_SECONDARY)
   {
       ...
       */
         XmTransferValue(ds->transfer_id, atoms[XmATARGETS], (XtCallbackProc)TextSecondaryWrapper, (XtPointer)&insert_select, ds->time);
      }
      else
      {
         /*
       ...
       */
         XmTransferValue(ds->transfer_id, ((Atom)ds->location_data), (XtCallbackProc)TextSecondaryWrapper, (XtPointer)&insert_select, ds->time);
      }
      _XmProcessUnlock();
   }
   else
...
{
   XmeTraitSet((XtPointer)xmTextWidgetClass, XmQTtransfer, (XtPointer)&TextTransfer);
}
