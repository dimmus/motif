...
#define BIGSIZE ((Dimension)32767)
    ...
    static Boolean
    CvtStringToHorizontalInt(Display * dpy,
                             XrmValue *args,
                             Cardinal *num_args,
                             XrmValue *from_val,
                             XrmValue *toVal,
                             XtPointer *data);
static Boolean CvtStringToVerticalInt(Display *dpy,
                                      XrmValue *args,
                                      Cardinal *num_args,
                                      XrmValue *from_val,
                                      XrmValue *toVal,
...
static void DeleteWindowHandler(Widget wid, XtPointer closure, XtPointer call_data);
static void OffsetHandler(Widget shell, XtPointer clientData, XtPointer cd);
static void InitializePrehook(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void Destroy(Widget wid);
static void GetMWMFunctionsFromProperty(Widget wid, int resource_offset, XtArgVal *value);
...
static XtResource extResources[] = {
    ...
    },
    {
        ...
        XmCInputPolicy,
        XmRInputPolicy,
        sizeof(XmInputPolicy),
        Offset(input_policy),
        XmRImmediate,
        (XtPointer)XmPER_SHELL,
    },
    {
    ...
    },
    {
        ...
        XmRString,
        sizeof(String),
        Offset(preedit_type_string),
        XmRImmediate,
        "OverTheSpot,OffTheSpot,Root,OnTheSpot",
    },
    {
        XmNlightThreshold,
        XmCLightThreshold,
...
#define ParentOffset(x) (XtOffsetOf(VendorShellRec, x) | XmLOGICAL_PARENT_RESOURCE)
...
externaldef(xmVendorShellExtobjectclass)
    WidgetClass xmVendorShellExtObjectClass = (WidgetClass)(&xmVendorShellExtClassRec);

static Boolean CvtStringToHorizontalInt(Display *display,
                                        XrmValue *args,
                                        ...
                                        XtPointer *converter_data)
{
  ...
  int tmpPix;
  Boolean parseError;
  tmpPix = (int)_XmConvertStringToUnits(
...
static Boolean CvtStringToVerticalInt(Display *display,
                                      XrmValue *args,
                                      Cardinal *num_args, /* unused */
                                      XrmValue *from,
                                      XrmValue *to,
...
static void ClassPartInitialize(WidgetClass w)
{
  ...
  XmVendorShellExtObjectClass sc = (XmVendorShellExtObjectClass)wc->object_class.superclass;
  ...
  if (wc->vendor_class.delete_window_handler == XmInheritProtocolHandler)
    wc->vendor_class.delete_window_handler = sc->vendor_class.delete_window_handler;
  if (wc->vendor_class.offset_handler == XmInheritProtocolHandler)
    wc->vendor_class.offset_handler = sc->vendor_class.offset_handler;
}
 ...
 *
 ************************************************************************/
static void DeleteWindowHandler(Widget wid, XtPointer closure, XtPointer call_data) /* unused */
{
  ...
  switch (ve->vendor.delete_response) {
    ...
    case XmDESTROY:
      if (XtIsApplicationShell((Widget)w)) {
        XtDestroyApplicationContext(XtWidgetToApplicationContext((Widget)w));
        exit(0);
      }
      else
 ...
 *
 ************************************************************************/
static void OffsetHandler(Widget shell, /* unused */
...
static void InitializePrehook(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if ((*wcePtr)->use_sub_resources) {
    ...
    if ((*wcePtr)->ext_resources == NULL) {
      ...
      XtGetResourceList(
          (WidgetClass)ec, &((*wcePtr)->ext_resources), &((*wcePtr)->num_ext_resources));
    }
...
/****************************************************************/
static void GetMWMFunctionsFromProperty(Widget wid,
...
/******* XmeFunctions ********/
void XmeAddFocusChangeCallback(Widget w, XtCallbackProc proc, XtPointer data)
{
  XmWidgetExtData extData;
  XmVendorShellExtObject vendorExt;
  ...
  vendorExt = (XmVendorShellExtObject)extData->widget;
  _XmAddCallback((InternalCallbackList *)&(vendorExt->vendor.focus_moved_callback), proc, data);
  _XmAppUnlock(app);
}

void XmeRemoveFocusChangeCallback(Widget w, XtCallbackProc proc, XtPointer data)
{
  XmWidgetExtData extData;
  XmVendorShellExtObject vendorExt;
  _XmWidgetToAppContext(w);
  ...
  vendorExt = (XmVendorShellExtObject)extData->widget;
  _XmRemoveCallback((InternalCallbackList *)&(vendorExt->vendor.focus_moved_callback), proc, data);
  _XmAppUnlock(app);
}
