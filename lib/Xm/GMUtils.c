              ...
              Boolean         queryonly)
{
   ...
   /* the current manager sizes are bigger than the new ones and
       ...
       it already fits */
   if ((resize_policy == XmRESIZE_GROW || resize_policy == XmRESIZE_NONE) && ((XtWidth(manager) >= request.width) && (XtHeight(manager) >= request.height)))
      return (True);
      ...
      if (XtIsManaged(child))
      {
         ...
         if (do_move)
         {
            if (setvalue)
            {
               ...
               XmeConfigureObject(child, newx, newy, child->core.width, child->core.height, child->core.border_width);
         }
      }
   }
}
             ...
             Widget          w)
{
   ...
   for (i = 0; i < manager->composite.num_children; i++)
   {
      ...
      if (w != kid && (((left1 >= left2) && ((Dimension)left1 <= right2)) || ((left2 >= left1) && ((Dimension)left2 <= right1))) && (((top1 >= top2) && ((Dimension)top1 <= bottom2)) || ((top2 >= top1) && ((Dimension)top2 <= bottom1))))
      {
         return (True);
      }
   }
 ...
 ****************/
XtGeometryResult
_XmGMHandleGeometryManager(Widget parent, Widget w, XtWidgetGeometry *request, XtWidgetGeometry *reply, Dimension margin_width, Dimension margin_height, int resize_policy, int allow_overlap)
{
   /* Policy: Yes
   ...
   else
   {
	 ...
	 go for it */
      if (!geoFlag && ((localReply.x + localReply.width + (localReply.border_width << 1)) <= (XtWidth(manager) - margin_width))
