 ...
 * Internal Routines.
 *********************/
static Boolean CvtStringToConnectStyle(
    Display *, XrmValuePtr, Cardinal *, XrmValuePtr, XrmValuePtr);
static Boolean CvtStringToCompressStyle(
    Display *, XrmValuePtr, Cardinal *, XrmValuePtr, XrmValuePtr);
...
static void _DrawLine(
    Widget, XRectangle *, TreeConstraints, TreeConstraints, LadderPoint, LadderPoint *);
...
static void MoveNode(
    XmTreeWidget, TreeConstraints, Position, Position, Position, Position, Boolean);
 ...
 *	STATIC DECLARATIONS
 *************************************************************/
static XtResource resources[] = {{XmNconnectStyle,
                                  XmCConnectStyle,
                                  XmRXmConnectStyle,
                                  sizeof(XmTreeConnectStyle),
                                  XtOffsetOf(XmTreeRec, tree.connect_style),
                                  XmRImmediate,
                                  (XtPointer)XmTreeDirect},
                                 {XmNorientation,
                                  XmCOrientation,
                                  XmROrientation,
                                  sizeof(unsigned char),
                                  XtOffsetOf(XmTreeRec, tree.orientation),
                                  XmRImmediate,
                                  (XtPointer)XmHORIZONTAL},
                                 {XmNcompressStyle,
                                  XmCCompressStyle,
                                  XmRXmCompressStyle,
                                  sizeof(XmTreeCompressStyle),
                                  XtOffsetOf(XmTreeRec, tree.compress_style),
                                  XmRImmediate,
                                  (XtPointer)XmTreeCompressLeaves},
                                 {XmNverticalDelta,
                                  XmCVerticalDelta,
                                 ...
                                 {XmNhorizontalNodeSpace,
                                  ...
                                  XmRCallProc,
                                  (XtPointer)HorizontalNodeSpaceDefault},
                                 {XmNverticalNodeSpace,
                                  ...
                                  XmRCallProc,
                                  (XtPointer)VerticalNodeSpaceDefault}};
static XmSyntheticResource get_resources[] = {{XmNhorizontalNodeSpace,
                                               sizeof(Dimension),
                                               XtOffsetOf(XmTreeRec, tree.h_node_space),
                                              ...
                                              {XmNhorizontalDelta,
                                               ...
                                               XmeFromHorizontalPixels,
                                               (XmImportProc)XmeToHorizontalPixels}};
static XtResource constraints[] = {{XmNopenClosePadding,
                                    XmCOpenClosePadding,
                                    XmRInt,
                                    sizeof(int),
                                   ...
                                   {XmNlineColor,
                                    ...
                                    XmRPixel,
                                    sizeof(Pixel),
                                    XtOffsetOf(XmTreeConstraintRec, tree.color),
                                    XmRCallProc,
                                    (XtPointer)LineColorDefault},
                                   {XmNlineBackgroundColor,
                                    XmCBackground,
                                    XmRPixel,
                                    sizeof(Pixel),
                                    XtOffsetOf(XmTreeConstraintRec, tree.background_color),
                                    XmRCallProc,
                                    (XtPointer)LineBackgroundColorDefault},
                                   {XmNlineWidth,
                                    XmCLineWidth,
                                    XmRInt,
                                    sizeof(int),
                                    XtOffsetOf(XmTreeConstraintRec, tree.line_width),
                                    XmRImmediate,
                                    (XtPointer)0},
                                   {XmNlineStyle,
                                    XmCLineStyle,
                                    XmRXmLineStyle,
                                    sizeof(int),
                                    XtOffsetOf(XmTreeConstraintRec, tree.line_style),
                                    XmRImmediate,
                                    (XtPointer)LineSolid}};
...
static void Initialize(Widget req, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  top_node = ((TreeConstraints)XtRealloc((XtPointer)XmHierarchy_top_node(tw),
 ...
 */
static Bool CheckExpose(Display *disp, XEvent *event, char *info_ptr)
{
 ...
 *	Returns:       none.
 */
static void ConstraintInitialize(Widget req, Widget set, ArgList args, Cardinal *num_args)
{
 ...
 *	Returns: none
 */
static void GetNodeGCs(Widget w)
{
  XtGCMask mask;
  XGCValues values;
  TreeConstraints node = GetNodeInfo(w);
 ...
 *	Returns: none
 */
static void ReleaseNodeGCs(Widget w)
{
  TreeConstraints node = GetNodeInfo(w);
    ...
    Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  if ((XmHierarchyC_parent(set_node) != XmHierarchyC_parent(old_node)) ||
      (XmHierarchyC_state(set_node) != XmHierarchyC_state(old_node)) || (insert_change) ||
      (XmTreeC_open_close_padding(set_node) != XmTreeC_open_close_padding(old_node)))
  {
    /*
 ...
 *	Returns:       none.
 */
static void ToggleNodeState(Widget w, XtPointer node_ptr, XtPointer call_data)
{
  Widget tw = XtParent(w);
 ...
 */
static Boolean CvtStringToConnectStyle(
    Display *dpy, XrmValuePtr args, Cardinal *num_args, XrmValuePtr fromVal, XrmValuePtr toVal)
{
    ...
    Display *dpy, XrmValuePtr args, Cardinal *num_args, XrmValuePtr fromVal, XrmValuePtr toVal)
{
  ...
  else if ((q == XtQECompressLeaves) || streq(lowerName, "leaves") ||
           streq(lowerName, "treecompressleaves"))
    compress = XmTreeCompressLeaves;
  else if ((q == XtQECompressAll) || streq(lowerName, "all") ||
           streq(lowerName, "treecompressall"))
...
static Boolean CvtStringToLineStyle(
    Display *dpy, XrmValuePtr args, Cardinal *num_args, XrmValuePtr fromVal, XrmValuePtr toVal)
{
 ...
 *	Returns: none.
 */
static void LineColorDefault(Widget widget, int offset, XrmValue *value)
{
  XmTreeWidget tw = (XmTreeWidget)XtParent(widget);
  value->addr = (XtPointer) & (tw->manager.foreground);
}

static void LineBackgroundColorDefault(Widget widget, int offset, XrmValue *value)
{
  XmTreeWidget tw = (XmTreeWidget)XtParent(widget);
 ...
 *	Returns: none
 */
static void DrawExtraLadderLines(
    Widget w, GC gc, LadderPoint first_kid, LadderPoint last_kid, LadderPoint parent_point)
{
  XmTreeWidget tw = (XmTreeWidget)w;
 ...
 *	Returns: none
 */
static void RedrawTreeLines(Widget w, XRectangle *rect)
{
  XmTreeWidget tw = (XmTreeWidget)w;
  if (XtIsRealized(w))
...
static void _CalcNodeMidPoint(TreeConstraints node, Widget w, LadderPoint *ret_point)
{
  ...
  if (XmTree_orientation(tw) == XmHORIZONTAL) {
    ret_point->x = (XmTreeC_box_x(node) + XmTreeC_widget_offset(node) +
                    (XmHierarchyC_widget(node))->core.width + XmHierarchy_h_margin(tw));
    ...
    ret_point->y = (XmTreeC_box_y(node) + XmTreeC_widget_offset(node) +
                    (XmHierarchyC_widget(node))->core.height + XmHierarchy_v_margin(tw));
 ...
 *	Returns: none
 */
static void _DrawLine(Widget w,
                      XRectangle *rect,
                      TreeConstraints parent,
                      TreeConstraints child,
                      LadderPoint from_ladder_point,
                      LadderPoint *to_ladder_point)
{
  ...
  if (child != (TreeConstraints)XmHierarchyC_children(parent)[0] &&
      (!(XmHierarchyC_status(child) & IS_MAPPED) || (XmHierarchyC_status(child) & IS_COMPRESSED)))
  {
    return;
  }
  gc = XmTreeC_gc(child);
 ...
 *	Returns:       none.
 */
static void CalcLocations(Widget w, Boolean resize_it)
{
  ...
  (void)GetNodeHeightAndWidth(
      w, (TreeConstraints)XmHierarchy_top_node(tw), &(XmHierarchy_num_nodes(tw)), 0);
  current_index = 0;
  {
 ...
 *	Returns:       none.
 */
static void GetDesiredSize(Widget w, Dimension *width, Dimension *height, Boolean recalc)
{
  XmTreeWidget tw = (XmTreeWidget)w;
  if (recalc)
 ...
 *	Returns:       none.
 */
static void LayoutChildren(Widget w, Widget assign_child)
{
  XmTreeWidget tw = (XmTreeWidget)w;
  XmTreeWidgetClass tc = (XmTreeWidgetClass)XtClass(w);
  ...
  if (register_workproc) {
    XmHierarchy_work_proc_id(tw) = XtAppAddWorkProc(
        XtWidgetToApplicationContext(w), MoveNodesTimer, (XtPointer)w);
  }
  XmDropSiteEndUpdate(w);
}
 ...
 *	Returns: none.
 */
static void FindNodeLocations(Widget w)
{
  XmTreeWidget tw = (XmTreeWidget)w;
  TreeConstraints *node;
...
static void _PlaceNode(Widget w, TreeConstraints node)
{
  ...
  if (XmTree_orientation(tw) == XmHORIZONTAL) {
    ...
    if (XmHierarchyC_state(parent) != XmHidden)
      x_loc += (pw->core.width + XmTreeC_widget_offset(parent) + 2 * pw->core.border_width +
    ...
    for (i = 0; i < num; i++, child++) {
      ...
      if ((XmHierarchyC_widget(*child)) && (XtIsManaged(XmHierarchyC_widget(*child)))) {
        if (prev_child == NULL)
          ...
          XmTreeC_box_y(*child) = (XmTreeC_box_y(prev_child) + XmTreeC_bb_height(prev_child) +
    ...
    for (i = 0; i < num; child++, i++) {
      if ((child != NULL) && (XtIsManaged(XmHierarchyC_widget(*child)))) {
        ...
        if ((XmTreeC_is_compressed(*child)) || (prev_child && XmTreeC_is_compressed(prev_child)))
    ...
    if (XmHierarchyC_state(parent) != XmHidden)
      y_loc += (pw->core.height + XmTreeC_widget_offset(parent) + 2 * pw->core.border_width +
                XmTree_v_node_space(tw));
    for (i = 0; i < num; i++, child++) {
      XmTreeC_placed(*child) = TRUE;
      ...
      if ((XmHierarchyC_widget(*child)) && (XtIsManaged(XmHierarchyC_widget(*child)))) {
        if (prev_child == NULL)
          ...
          XmTreeC_box_x(*child) = (XmTreeC_box_x(prev_child) + XmTreeC_bb_width(prev_child) +
                                   XmTree_h_node_space(tw));
        /*
 ...
 *	Returns:       none
 */
static Boolean GetNodeHeightAndWidth(Widget w,
                                     TreeConstraints node,
                                     Cardinal *num,
                                     Cardinal sib_index)
{
  ...
  if (XmTree_orientation(tw) == XmHORIZONTAL) {
    ...
    if ((XmHierarchyC_state(node) != XmClosed) && (num_kids > 0)) {
      ...
      for (i = 0; i < num_kids; i++) {
         ...
         */
        if (XmTreeC_is_compressed(*child)) {
          if ((int)l_height < (int)XmTreeC_bb_height(*child) + (int)XmTree_vertical_delta(tw))
    ...
    if ((int)XmTreeC_bb_width(node) < (int)l_width)
      XmTreeC_bb_width(node) = l_width;
    if ((((XmTree_compress_style(tw) == XmTreeCompressAll) && ((sib_index % 2) == 1)) &&
         (XmHierarchyC_parent(node) != NULL)) ||
        (((XmTree_compress_style(tw) == XmTreeCompressLeaves) && (num_kids == 0) &&
          (XmHierarchyC_parent(node) != NULL) && ((sib_index % 2) == 1))))
    {
 ...
 */
static TreeConstraints GetNodeInfo(Widget w)
{
 ...
 *	Returns: none.
 */
static void MoveNode(XmTreeWidget tw,
                     TreeConstraints node,
                     Position x,
                     Position y,
                     Position oc_x,
 ...
 *	Returns: none.
 */
static void ProcessChildQueue(XmTreeWidget tw, XRectangle *vis)
{
  XmListElem *elem, *next;
  elem = XmListFirst(XmTree_child_op_list(tw));
  while (elem != NULL) {
 ...
 */
static Boolean CheckWidget(XRectangle *visible, TreeConstraints node)
{
  if ((((XmHierarchyC_status(node) & IS_MAPPED) || XmTreeC_map(node)) &&
      ...
      LocInRect(visible, XmHierarchyC_widget(node), XmTreeC_new_x(node), XmTreeC_new_y(node)))
  {
    ProcessNode(node);
    return (True);
  }
 ...
 *	Returns: none.
 */
static void ProcessNode(TreeConstraints node)
{
  ...
  if (XmTreeC_move(node)) {
    _XmMoveWidget(XmHierarchyC_widget(node), XmTreeC_new_x(node), XmTreeC_new_y(node));
    if (XmHierarchyC_open_close_button(node) != NULL)
      _XmMoveWidget(
          XmHierarchyC_open_close_button(node), XmTreeC_oc_new_x(node), XmTreeC_oc_new_y(node));
    XmTreeC_move(node) = False;
  }
  if (XmTreeC_map(node)) {
 ...
 */
static Boolean WidgetInRect(XRectangle *rect, Widget w)
{
  if (w == NULL)
    return (False);
 ...
 *	Returns: none
 */
static Boolean MoveNodesTimer(XtPointer data)
{
 ...
 */
Widget XmCreateTree(Widget parent, String name, ArgList args, Cardinal num_args)
{
