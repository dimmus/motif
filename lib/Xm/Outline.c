...
static void GetDesiredSize(
    Widget, Dimension *, Dimension *, Dimension *, Dimension *, Boolean, Boolean);
...
static void MoveNode(
    XmOutlineWidget, OutlineConstraints, Position, Position, Position, Position, Boolean);
...
static void _OutlineDrawLine(Widget w,
                             XRectangle *rect,
                             OutlineConstraints parent,
                             OutlineConstraints child,
 ...
 *	STATIC DECLARATIONS
 *************************************************************/
static XtResource resources[] = {{XmNindentSpace,
                                  XmCIndentSpace,
                                  XmRHorizontalDimension,
                                  sizeof(Dimension),
                                  XtOffsetOf(XmOutlineRec, outline.indent_space),
                                  XmRImmediate,
                                  (XtPointer)DEF_INDENT_SPACE},
                                 {XmNconstrainWidth,
                                  XmCConstrainWidth,
                                  XmRBoolean,
                                  sizeof(Boolean),
                                  XtOffsetOf(XmOutlineRec, outline.constrain_width),
                                  XmRImmediate,
                                  (XtPointer)False},
                                 {XmNconnectNodes,
                                  XmCBoolean,
                                  XmRBoolean,
                                  sizeof(Boolean),
                                  XtOffsetOf(XmOutlineRec, outline.connect_nodes),
                                  XmRImmediate,
                                  (XtPointer)False}};
static XmSyntheticResource get_resources[] = {{XmNindentSpace,
                                               sizeof(Dimension),
                                               XtOffsetOf(XmOutlineRec, outline.indent_space),
...
static void Initialize(Widget req, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  XmHierarchy_top_node(ow) = ((HierarchyConstraints)XtRealloc((XtPointer)XmHierarchy_top_node(ow),
                                      ...
                                      XtWidgetGeometry *preferred)
{
  ...
  GetDesiredSize(
      w, width_intended, height_intended, &(preferred->width), &(preferred->height), True, False);
    ...
    Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  if (XmOutline_constrain_width(c_outline) != XmOutline_constrain_width(outline)) {
    layout = True;
  }
                                        ...
                                        XtWidgetGeometry *result)
{
  ...
  if (!(request->request_mode & (CWWidth | CWHeight | CWBorderWidth)))
    return (XtGeometryNo);
  if ((XmOutline_constrain_width(ow)) && (request->request_mode & CWWidth) &&
...
{
  XmOutlineC_widget_x(set->core.constraints) = XmOutlineC_open_close_x(set->core.constraints) = 0;
}
 ...
 *	Returns:       none.
 */
static void CalcLocations(Widget w, Boolean allow_resize)
{
...
/*	Function Name:	GetNodeDimensions
        ...
        Returns:	none
 */
static void GetNodeDimensions(Widget w, OutlineConstraints node, Cardinal depth, Cardinal *num)
{
  XmOutlineWidget ow = (XmOutlineWidget)w;
 ...
 *	Returns:       none.
 */
static void RequestNewSize(Widget w)
{
 ...
 *	Returns:       none.
 */
static void GetDesiredSize(Widget w,
                           Dimension *width,
                           Dimension *height,
                           Dimension *width_ret,
...
static void LayoutChildren(Widget w, Widget assign_child)
{
  ...
  while (((int)cur_node < (int)num_nodes) &&
  ...
  if (register_workproc) {
    XmHierarchy_work_proc_id(ow) = XtAppAddWorkProc(
 ...
 *	Returns:       none
 */
static void GetNodeHeightAndWidth(
    Widget w, OutlineConstraints node, Cardinal outline_depth, Boolean recurse, Cardinal *num)
{
  ...
  if (XmHierarchyC_state(node) != XmHidden) {
    ...
    if (XmHierarchyC_open_close_button(node) == NULL)
      ...
      XmOutlineC_widget_x(node) = (XmOutlineC_open_close_x(node) + width +
                                   XmHierarchy_h_margin(ow));
 ...
 */
static OutlineConstraints GetNodeInfo(Widget w)
{
 ...
 */
static int CalcMaxWidth(Widget w)
{
 ...
 *	Returns: none.
 */
static void MoveNode(XmOutlineWidget ow,
                     OutlineConstraints node,
                     Position x,
                     Position y,
                     Position oc_x,
 ...
 *	Returns: none.
 */
static void ProcessChildQueue(XmOutlineWidget ow, XRectangle *vis)
{
  XmListElem *elem, *next;
  elem = XmListFirst(XmOutline_child_op_list(ow));
  while (elem != NULL) {
 ...
 */
static Boolean CheckWidget(XRectangle *visible, OutlineConstraints node)
{
  if ((((XmHierarchyC_status(node) & IS_MAPPED) || XmOutlineC_map(node)) &&
      ...
      LocInRect(
          visible, XmHierarchyC_widget(node), XmOutlineC_new_x(node), XmOutlineC_new_y(node)))
  {
    ProcessNode(node);
    return (True);
  }
 ...
 *	Returns: none.
 */
static void ProcessNode(OutlineConstraints node)
{
  ...
  if (XmOutlineC_move(node)) {
    _XmMoveWidget(XmHierarchyC_widget(node), XmOutlineC_new_x(node), XmOutlineC_new_y(node));
    if (XmHierarchyC_open_close_button(node) != NULL)
...
static void NegotiateNodeWidth(Widget w, OutlineConstraints node)
{
  ...
  result = XtQueryGeometry(XmHierarchyC_widget(node), &geom_asked, &geom_agreed);
  if (geom_agreed.width < 1)
    geom_agreed.width = 1;
  if (geom_agreed.height < 1)
    geom_agreed.height = 1;
  ...
  if (result == XtGeometryYes) {
    _XmResizeWidget(
        XmHierarchyC_widget(node), width_avail, geom_agreed.height, geom_agreed.border_width);
    return;
  }
...
*/
static void RedrawOutlineLines(Widget w, XRectangle *rect)
{
  XmOutlineWidget ow = (XmOutlineWidget)w;
...
static void DrawOutlineLine(Widget w, XRectangle *rect, OutlineConstraints node)
{
  ...
  }
  {
     ...
     */
    if (num_kids > 0 && anyKidManaged) { /* CR03730 Support Case 22066 anyKidManaged added to
                                            prevent draw needless (x & y are not initialized if no
                                            kid managed) line */
      XDrawLine(XtDisplay(w),
                XtWindow(w),
                XmOutline_draw_gc(ow),
                from_node_point.x,
                from_node_point.y,
                from_node_point.x,
                last_kid_point.y);
 ...
 */
Widget XmCreateOutline(Widget parent, String name, ArgList args, Cardinal num_args)
{
