...
static void               GetNodeDimensions(Widget, OutlineConstraints, Cardinal, Cardinal *);
static void               GetNodeHeightAndWidth(Widget, OutlineConstraints, Cardinal, Boolean, Cardinal *);
static void               GetDesiredSize(Widget, Dimension *, Dimension *, Dimension *, Dimension *, Boolean, Boolean);
...
static void               MoveNode(XmOutlineWidget, OutlineConstraints, Position, Position, Position, Position, Boolean);
...
static void               _OutlineDrawLine(Widget w, XRectangle *rect, OutlineConstraints parent, OutlineConstraints child, LadderPoint from_ladder_point, LadderPoint *to_ladder_point);
static void               CreateGC(XmOutlineWidget ow);
/************************************************************
*	STATIC DECLARATIONS
*************************************************************/
static XtResource resources[] = {
   { XmNindentSpace,    XmCIndentSpace,    XmRHorizontalDimension, sizeof(Dimension), XtOffsetOf(XmOutlineRec, outline.indent_space),    XmRImmediate, (XtPointer)DEF_INDENT_SPACE },
   { XmNconstrainWidth, XmCConstrainWidth, XmRBoolean,             sizeof(Boolean),   XtOffsetOf(XmOutlineRec, outline.constrain_width), XmRImmediate, (XtPointer)False            },
   { XmNconnectNodes,   XmCBoolean,        XmRBoolean,             sizeof(Boolean),   XtOffsetOf(XmOutlineRec, outline.connect_nodes),   XmRImmediate, (XtPointer)False            }
};
static XmSyntheticResource get_resources[] = {
   { XmNindentSpace, sizeof(Dimension), XtOffsetOf(XmOutlineRec, outline.indent_space), XmeFromHorizontalPixels, (XmImportProc)XmeToHorizontalPixels }
};
...
QueryGeometry(Widget w, XtWidgetGeometry *intended, XtWidgetGeometry *preferred)
{
   ...
   GetDesiredSize(w, width_intended, height_intended, &(preferred->width), &(preferred->height), True, False);
...
SetValues(Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
   ...
   if ((XmHierarchy_v_margin(c_outline) != XmHierarchy_v_margin(outline)) || (XmHierarchy_h_margin(c_outline) != XmHierarchy_h_margin(outline)))
   {
      layout = True;
   }
   ...
   if (layout)
   {
      ...
      _XmProcessLock()
         calc_locations
         = oc->outline_class.calc_locations;
...
GeometryManager(Widget w, XtWidgetGeometry *request, XtWidgetGeometry *result)
{
   ...
   if (!(request->request_mode & (CWWidth | CWHeight | CWBorderWidth)))
      return (XtGeometryNo);
   if ((XmOutline_constrain_width(ow)) && (request->request_mode & CWWidth) && (request->width > w->core.width))
   {
...
ConstraintSetValues(Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
   ...
   if (!XtIsRealized(set))
      return (False);
   if ((XmHierarchyC_parent(set_node) != XmHierarchyC_parent(old_node)) || (XmHierarchyC_state(set_node) != XmHierarchyC_state(old_node)) || (insert_change))
   {
      /*
   ...
   if (XtIsRealized(ow) && redisplay && (XmHierarchy_refigure_mode((XmOutlineWidget)ow)))
   {
      XClearArea(XtDisplay(ow), XtWindow(ow), 0, 0, ow->core.width, ow->core.height, True);
   }
   return (False);
...
ToggleNodeState(Widget w, XtPointer node_ptr, XtPointer call_data)
{
   ...
   XmOutlineWidgetClass oc = (XmOutlineWidgetClass)XtClass(ow);
   {
      ...
      _XmProcessLock()
         toggle_node_state
         = SUPERCLASS->hierarchy_class.toggle_node_state;
...
CalcLocations(Widget w, Boolean allow_resize)
{
   ...
   num_nodes     = 0;
   outline_depth = 0;
   GetNodeDimensions(w, (OutlineConstraints)XmHierarchy_top_node(ow), outline_depth, &num_nodes);
   {
      XmOutlineMaxWidthProc calc_max_width;
      _XmProcessLock();
      calc_max_width = oc->outline_class.calc_max_width;
      _XmProcessUnlock()
         XmOutline_max_width(ow)
         = (*calc_max_width)(w);
   }
   ...
   if (num_nodes != 0)
   {
      XmOutline_top_node_of_display(ow) = (OutlineConstraints)XmHierarchy_node_table(ow)[0];
   }
   else
      XmOutline_top_node_of_display(ow) = NULL;
   if (allow_resize)
 ...
 */
static void
GetNodeDimensions(Widget w, OutlineConstraints node, Cardinal depth, Cardinal *num)
{
   XmOutlineWidget ow             = (XmOutlineWidget)w;
...
RequestNewSize(Widget w)
{
   ...
   while (ret_val == XtGeometryAlmost)
   {
      ...
      {
         GetDesiredSize(w, &rwidth, &rheight, &fwidth, &fheight, False, True);
         ret_val = XtMakeResizeRequest(w, fwidth, fheight, &rwidth, &rheight);
      }
      else
      {
 ...
 */
static void
GetDesiredSize(Widget w, Dimension *width, Dimension *height, Dimension *width_ret, Dimension *height_ret, Boolean recalc, Boolean allow_resize)
{
   XmOutlineWidget      ow = (XmOutlineWidget)w;
...
LayoutChildren(Widget w, Widget assign_child)
{
      ...
      && (XmOutline_connect_nodes(ow) || ((int)cur_y < (int)ow->core.height)))
   {
      ...
      if (XmHierarchyC_open_close_button(node) != NULL)
      {
         ...
         offset = (XmOutlineC_height(node) - (_w->core.height + 2 * _w->core.border_width));
   ...
   {
      XmHierarchy_work_proc_id(ow) = XtAppAddWorkProc(XtWidgetToApplicationContext(w),
                                                      MoveNodesTimer,
                                                      (XtPointer)w);
   }
   XmDropSiteEndUpdate(w);
 ...
 */
static void
GetNodeHeightAndWidth(Widget w, OutlineConstraints node, Cardinal outline_depth, Boolean recurse, Cardinal *num)
{
   XmOutlineWidget  ow = (XmOutlineWidget)w;
   register int     i;
   ...
   if (XmHierarchyC_state(node) != XmHidden)
   {
      ...
      if (XmHierarchyC_open_close_button(node) != NULL)
      {
         ...
         num_args++;
         XtSetArg(args[num_args], XmNheight, &open_height);
         num_args++;
         XtSetArg(args[num_args], XmNborderWidth, &border_width);
         num_args++;
	 ...
	 */
      XmOutlineC_open_close_x(node) = ((outline_depth * XmOutline_indent_space(ow)) + XmHierarchy_h_margin(ow));
      if (XmHierarchyC_open_close_button(node) == NULL)
         XmOutlineC_widget_x(node) = XmOutlineC_open_close_x(node);
      else
         XmOutlineC_widget_x(node) = (XmOutlineC_open_close_x(node) + width + XmHierarchy_h_margin(ow));
	 ...
	 */
      if (XmOutline_constrain_width(w))
      {
         ...
         num_args = 0;
         XtSetArg(args[num_args], XmNheight, &node_height);
         num_args++;
         XtGetValues(XmHierarchyC_widget(node), args, num_args);
      }
 ...
 */
static void
MoveNode(XmOutlineWidget ow, OutlineConstraints node, Position x, Position y, Position oc_x, Position oc_y, Boolean map)
{
...
{
   if ((((XmHierarchyC_status(node) & IS_MAPPED) || XmOutlineC_map(node)) && (WidgetInRect(visible, XmHierarchyC_open_close_button(node)) || WidgetInRect(visible, XmHierarchyC_parent(node)) || WidgetInRect(visible, XmHierarchyC_widget(node)))) || LocInRect(visible, XmHierarchyC_open_close_button(node), XmOutlineC_oc_new_x(node), XmOutlineC_oc_new_y(node)) || LocInRect(visible, XmHierarchyC_widget(node), XmOutlineC_new_x(node), XmOutlineC_new_y(node)))
   {
      ProcessNode(node);
      return (True);
   }
...
ProcessNode(OutlineConstraints node)
{
   ...
   if (XmOutlineC_move(node))
   {
      _XmMoveWidget(XmHierarchyC_widget(node),
                    XmOutlineC_new_x(node),
                    XmOutlineC_new_y(node));
      if (XmHierarchyC_open_close_button(node) != NULL)
         _XmMoveWidget(XmHierarchyC_open_close_button(node),
                       XmOutlineC_oc_new_x(node),
                       XmOutlineC_oc_new_y(node));
      XmOutlineC_move(node) = False;
   }
...
NegotiateNodeWidth(Widget w, OutlineConstraints node)
{
   ...
   if (geom_agreed.width <= width_avail)
   {
	 ...
	 */
      if (curr_width == geom_agreed.width && curr_height == geom_agreed.height)
      {
         return;
      }
      else
   ...
   }
   {
     ...
     */
      ...
      { /* CR03730 Support Case 22066 anyKidManaged added to prevent draw needless
         ...
         XDrawLine(XtDisplay(w), XtWindow(w), XmOutline_draw_gc(ow), from_node_point.x, from_node_point.y, from_node_point.x, last_kid_point.y);
      }
   }
}
...
static void
_OutlineDrawLine(Widget w, XRectangle *rect, OutlineConstraints parent, OutlineConstraints child, LadderPoint from_ladder_point, LadderPoint *to_ladder_point)
{
   GC              gc;
   XmOutlineWidget ow = (XmOutlineWidget)w;
   register int    x2, y2;
   ...
   /*
     ...
     * of the ladder.
     */
   if (child != (OutlineConstraints)XmHierarchyC_children(parent)[0] && (!(XmHierarchyC_status(child) & IS_MAPPED) || (XmHierarchyC_status(child) & IS_COMPRESSED)))
   {
      return;
   }
 ...
 */
Widget
XmCreateOutline(Widget parent, String name, ArgList args, Cardinal num_args)
{
   return (XtCreateWidget(name, xmOutlineWidgetClass, parent, args, num_args));
}
