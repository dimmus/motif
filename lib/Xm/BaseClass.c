...
#define IsBaseClass(wc) \
  ((wc == xmGadgetClass) || (wc == xmManagerWidgetClass) || (wc == xmPrimitiveWidgetClass) || \
   (wc == vendorShellWidgetClass) || (wc == xmDisplayClass) || (wc == xmScreenClass) || \
   (wc == xmExtObjectClass) || (_XmIsFastSubclass(wc, XmMENU_SHELL_BIT)))
#define isWrappedXtClass(wc) ((wc == rectObjClass) || (wc == compositeWidgetClass))
    /*
...
#define GetRefW(dpy, context, w) \
  if (XFindContext(dpy, None, context, (XPointer *)&w)) \
  w = NULL
#define SetRefW(dpy, context, w) XSaveContext(dpy, None, context, (char *)w)
...
static void RealizeWrapper(Widget w, Mask *vmask, XSetWindowAttributes *attr, Cardinal depth);
...
static void InitializeLeafWrapper(
    Widget req, Widget new_w, ArgList args, Cardinal *num_args, int depth);
...
static Boolean SetValuesLeafWrapper(
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args, int depth);
static Boolean CSetValuesLeafWrapper(
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args, int depth);
static void GetValuesRootWrapper(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper(Widget new_w, ArgList args, Cardinal *num_args, int depth);
...
static void InitializeLeafWrapper1(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void InitializeLeafWrapper2(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void InitializeLeafWrapper3(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void InitializeLeafWrapper4(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void InitializeLeafWrapper5(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void InitializeLeafWrapper6(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void InitializeLeafWrapper7(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void InitializeLeafWrapper8(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void InitializeLeafWrapper9(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper0(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper1(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper2(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper3(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper4(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper5(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper6(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper7(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper8(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void CInitializeLeafWrapper9(Widget req, Widget new_w, ArgList args, Cardinal *num_args);
...
static Boolean CSetValuesLeafWrapper9(
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper0(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper1(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper2(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper3(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper4(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper5(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper6(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper7(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper8(Widget new_w, ArgList args, Cardinal *num_args);
static void GetValuesLeafWrapper9(Widget new_w, ArgList args, Cardinal *num_args);

/********    End Static Function Declarations    ********/
XmGenericClassExt *_XmGetClassExtensionPtr(XmGenericClassExt *listHeadPtr, XrmQuark owner)
{
...
static XmWrapperData GetWrapperData(WidgetClass w_class)
{
  ...
  if (!((*wcePtr)->wrapperData))
    (*wcePtr)->wrapperData = (XmWrapperData)XtCalloc(1, sizeof(XmWrapperDataRec));
...
static XtRealizeProc realizeWrappers[] = {RealizeWrapper0,
                                          RealizeWrapper1,
                                          RealizeWrapper2,
                                          RealizeWrapper3,
                                          RealizeWrapper4,
...
/* Used to be XmConst, but this was not linking on Solaris */
static XtWidgetProc resizeWrappers[] = {ResizeWrapper0,
                                        ResizeWrapper1,
                                        ResizeWrapper2,
                                        ResizeWrapper3,
                                        ResizeWrapper4,
...
static XtGeometryHandler geometryHandlerWrappers[] = {GeometryHandlerWrapper0,
...
static XmBaseClassExt *BaseClassPartInitialize(WidgetClass wc)
{
  ...
  if (!isBaseClass && !isWrappedXtClass(wc) && (!scePtr || !(*scePtr)))
    return NULL;
  ...
  if ((wc == vendorShellWidgetClass) || _XmIsSubclassOf(wc, vendorShellWidgetClass)) {
    /* Wrap Realize */
    /*
  ...
  if ((wc == rectObjClass) || _XmIsSubclassOf(wc, rectObjClass)) {
    /* Wrap resize */
    /*
     * check if this widget was using XtInherit and got the wrapper
     * from the superclass
  ...
  if ((wc == compositeWidgetClass) || _XmIsSubclassOf(wc, compositeWidgetClass)) {
    /* Wrap GeometryManager */
    /*
     * check if this widget was using XtInherit and got the wrapper
     * from the superclass
     */
    if (((CompositeWidgetClass)wc)->composite_class.geometry_manager == XtInheritGeometryManager) {
     ...
     */
      ...
      wcData->geometry_manager = ((CompositeWidgetClass)wc)->composite_class.geometry_manager;
    }
    ((CompositeWidgetClass)wc)->composite_class.geometry_manager = (XtGeometryHandler)
...
static void ClassPartInitRootWrapper(WidgetClass wc)
{
  ...
  if (wcePtr && *wcePtr) {
    ...
    if ((*wcePtr)->classPartInitPosthook) {
      XmWrapperData wrapperData;
      wrapperData = GetWrapperData(wc);
      leafFuncPtr = (XtWidgetClassProc *)&(wc->core_class.class_part_initialize);
 ...
 */
void _XmTransformSubResources(XtResourceList comp_resources,
                              ...
                              Cardinal *num_resources)
{
  ...
  if (((int)comp_resources[0].resource_offset) >= 0) {
    ...
    *num_resources = num_comp_resources;
    ...
    shadowObjectClassRec.constraint_class.resources = (XtResourceList)CreateIndirectionTable(
        comp_resources, num_comp_resources);
    shadowObjectClassRec.constraint_class.num_resources = num_comp_resources;
    XtGetConstraintResourceList((WidgetClass)&shadowObjectClassRec, resources, num_resources);
...
static XtInitProc InitializeLeafWrappers[] = {InitializeLeafWrapper0,
...
static XtInitProc CInitializeLeafWrappers[] = {CInitializeLeafWrapper0,
    ...
    if ((*wcePtr)->initializePosthook) {
      ...
      if (!XtIsShell(new_w) && XtParent(new_w) && XtIsConstraint(XtParent(new_w))) {
        ...
        if (wrapperData->constraintInitializeLeafCount == 0) {
          wrapperData->constraintInitializeLeaf = cwc->constraint_class.initialize;
          cwc->constraint_class.initialize = CInitializeLeafWrappers[GetDepth((WidgetClass)cwc)];
        }
    ...
    Widget req, Widget new_w, ArgList args, Cardinal *num_args, int depth)
{
  ...
  if (leafDepth == depth) { /* Correct depth */
    ...
    for (; depthDiff; depthDiff--, wc = wc->core_class.superclass) {
    };
    wrapperData = GetWrapperData(wc);
    init_proc = wrapperData->initializeLeaf;
  }
...
{
  WidgetClass wc = XtClass(new_w);
  ConstraintWidgetClass cwc = (ConstraintWidgetClass)XtClass(XtParent(new_w));
  XtInitProc init_proc = NULL;
  XtInitProc post_proc = NULL;
  ...
  if (leafDepth == depth) {
    ...
    if ((post_proc = (*wcePtr)->initializePosthook)) {
      if ((--(wrapperData->constraintInitializeLeafCount)) == 0)
        cwc->constraint_class.initialize = wrapperData->constraintInitializeLeaf;
    }
  }
  else {
    int depthDiff = leafDepth - depth;
    for (; depthDiff; depthDiff--, cwc = (ConstraintWidgetClass)cwc->core_class.superclass) {
    };
    wrapperData = GetWrapperData((WidgetClass)cwc);
    init_proc = wrapperData->constraintInitializeLeaf;
  }
...
static XtSetValuesFunc SetValuesLeafWrappers[] = {SetValuesLeafWrapper0,
...
static XtSetValuesFunc CSetValuesLeafWrappers[] = {CSetValuesLeafWrapper0,
    ...
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (wcePtr && *wcePtr) {
    if ((*wcePtr)->setValuesPrehook)
      returnVal |= (*((*wcePtr)->setValuesPrehook))(current, req, new_w, args, num_args);
    if ((*wcePtr)->setValuesPosthook) {
      ...
      if (!XtIsShell(new_w) && XtParent(new_w) && XtIsConstraint(XtParent(new_w))) {
        ...
        if (wrapperData->constraintSetValuesLeafCount == 0) {
          wrapperData->constraintSetValuesLeaf = cwc->constraint_class.set_values;
          cwc->constraint_class.set_values = CSetValuesLeafWrappers[GetDepth((WidgetClass)cwc)];
        }
        ...
        if (wrapperData->setValuesLeafCount == 0) {
          wrapperData->setValuesLeaf = wc->core_class.set_values;
          wc->core_class.set_values = SetValuesLeafWrappers[GetDepth(wc)];
        }
  ...
  if (objectClassWrapper.setValues)
    returnVal |= (*objectClassWrapper.setValues)(current, req, new_w, args, num_args);
  return returnVal;
}
    ...
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args, int depth)
{
  ...
  if (leafDepth == depth) { /* Correct depth */
    ...
    if (!XtIsShell(new_w) && XtParent(new_w) && XtIsConstraint(XtParent(new_w))) {
      ...
      if (!(post_proc = (*wcePtr)->setValuesPosthook)) {
        if ((--(wrapperData->setValuesLeafCount)) == 0)
          wc->core_class.set_values = wrapperData->setValuesLeaf;
      }
    }
  }
  else {
  ...
  _XmProcessUnlock();
  if (setvalues_proc)
    returnVal |= (*setvalues_proc)(current, req, new_w, args, num_args);
  if (post_proc)
    returnVal |= (*post_proc)(current, req, new_w, args, num_args);
  return returnVal;
}
    ...
    Widget current, Widget req, Widget new_w, ArgList args, Cardinal *num_args, int depth)
{
  ...
  if (leafDepth == depth) {
    ...
    if ((post_proc = (*wcePtr)->setValuesPosthook)) {
      if ((--(wrapperData->constraintSetValuesLeafCount)) == 0)
        cwc->constraint_class.set_values = wrapperData->constraintSetValuesLeaf;
    }
  }
  else {
    int depthDiff = leafDepth - depth;
...
static XtArgsProc GetValuesLeafWrappers[] = {GetValuesLeafWrapper0,
  ...
  if (wcePtr && *wcePtr) {
    ...
    if ((*wcePtr)->getValuesPosthook) {
      ...
      if (wrapperData->getValuesLeafCount == 0) {
        wrapperData->getValuesLeaf = wc->core_class.get_values_hook;
        wc->core_class.get_values_hook = GetValuesLeafWrappers[GetDepth(wc)];
      }
...
static void GetValuesLeafWrapper(Widget new_w, ArgList args, Cardinal *num_args, int depth)
{
  ...
  if (leafDepth == depth) { /* Correct depth */
    ...
    if ((post_proc = ((*wcePtr)->getValuesPosthook))) {
      if ((--(wrapperData->getValuesLeafCount)) == 0)
        wc->core_class.get_values_hook = wrapperData->getValuesLeaf;
    }
  }
  else {
    int depthDiff = leafDepth - depth;
...
{
  int i;
  for (i = 0; wc && wc != rectObjClass; i++, wc = wc->core_class.superclass) {
  };
  if (wc)
    return i;
  else
