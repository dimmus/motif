   ...
   Cardinal              depth)
{
   if (XmIsVendorShell(w))
   {
       ...
       */
      if ((extData = _XmGetWidgetExtData(w, XmSHELL_EXTENSION)) && (extData->widget) && strcmp(wc->core_class.class_name, "XmDialogShell"))
#endif
      {
   ...
   int       depth)
{
   ...
   else
   {
      int depthDiff = leafDepth - depth;
      for (; depthDiff;
           depthDiff--, cwc = (ConstraintWidgetClass)cwc->core_class.superclass)
      {
      };
      wrapperData = GetWrapperData((WidgetClass)cwc);
      init_proc   = wrapperData->constraintInitializeLeaf;
