...
#include "MessagesI.h"
#include "RepTypeI.h"
   typedef enum { FirstPane = 'U',
                  LastPane  = 'L' } Direction;
...
static XtResource resources[] = {
   { XmNmarginWidth, XmCMarginWidth, XmRHorizontalDimension, sizeof(Dimension), Offset(margin_width), XmRImmediate, (XtPointer)3 },
   { XmNmarginHeight, XmCMarginHeight, XmRVerticalDimension, sizeof(Dimension), Offset(margin_height), XmRImmediate, (XtPointer)3 },
   /* need to change the XmR to something dynamic: XmRPanedWMajorDimension,
   that checks eh orientation first */
   { XmNspacing, XmCSpacing, XmRVerticalDimension, sizeof(Dimension), Offset(spacing), XmRImmediate, (XtPointer)8 },
   { XmNrefigureMode, XmCBoolean, XmRBoolean, sizeof(Boolean), Offset(refiguremode), XmRImmediate, (XtPointer)TRUE },
   { XmNseparatorOn, XmCSeparatorOn, XmRBoolean, sizeof(Boolean), Offset(separator_on), XmRImmediate, (XtPointer)TRUE },
   /* need to change the XmR to something dynamic XmRPanedWMinorDimension */
   { XmNsashIndent, XmCSashIndent, XmRHorizontalPosition, sizeof(Position), Offset(sash_indent), XmRCallProc, (XtPointer)SashIndentDefault },
   { XmNsashWidth, XmCSashWidth, XmRHorizontalDimension, sizeof(Dimension), Offset(sash_width), XmRImmediate, (XtPointer)10 },
   { XmNsashHeight, XmCSashHeight, XmRVerticalDimension, sizeof(Dimension), Offset(sash_height), XmRImmediate, (XtPointer)10 },
   { XmNsashShadowThickness, XmCShadowThickness, XmRHorizontalDimension, sizeof(Dimension), Offset(sash_shadow_thickness), XmRCallProc, (XtPointer)_XmSetThickness },
   { XtNinsertPosition, XtCInsertPosition, XtRFunction, sizeof(XtOrderProc), XtOffsetOf(XmPanedWindowRec, composite.insert_position), XtRImmediate, (XtPointer)InsertOrder },
   { XmNorientation, XmCOrientation, XmROrientation, sizeof(unsigned char), Offset(orientation), XmRImmediate, (XtPointer)XmVERTICAL },
};
...
static XtResource constraint_resources[] = {
   { XmNallowResize,   XmCBoolean,       XmRBoolean,           sizeof(Boolean),   Offset(allow_resize),   XmRImmediate, (XtPointer)FALSE           },
   /* need to change the XmR to XmRPanedWMajorDimension */
   { XmNpaneMinimum,   XmCPaneMinimum,   XmRVerticalDimension, sizeof(Dimension), Offset(min),            XmRImmediate, (XtPointer)1               },
   { XmNpaneMaximum,   XmCPaneMaximum,   XmRVerticalDimension, sizeof(Dimension), Offset(max),            XmRImmediate, (XtPointer)1000            },
   { XmNskipAdjust,    XmCBoolean,       XmRBoolean,           sizeof(Boolean),   Offset(skip_adjust),    XmRImmediate, (XtPointer)FALSE           },
   { XmNpositionIndex, XmCPositionIndex, XmRShort,             sizeof(short),     Offset(position_index), XmRImmediate, (XtPointer)XmLAST_POSITION },
};
...
externaldef(xmpanedwindowwidgetclass) WidgetClass xmPanedWindowWidgetClass = (WidgetClass)&xmPanedWindowClassRec;
   ...
   Widget wid)
{
   ...
   RefigureLocationsAndCommit(pw, pw->paned_window.pane_count - 1, LastPane, True);
} /* Resize */
   ...
   XmPanedWindowWidget pw)
{
   ...
   pw->paned_window.flipgc = XtAllocateGC((Widget)pw, 0, valuemask, &values, dynamicMask, 0);
}
   ...
   register XmPanedWindowWidget pw)
{
   ...
   {
      needed += PaneDMajor(pw->paned_window.managed_children[i]) + 2 * pw->paned_window.managed_children[i]->core.border_width + pw->paned_window.spacing;
   }
   /*
   ...
   Boolean                      sflag)
{
   ...
   for (childP = children, i = 0; i < num_panes; childP++, i++)
   {
      ...
      if (PaneDMajor(*childP) < pane->min)
         PaneDMajor(*childP) = pane->min;
      else if (PaneDMajor(*childP) > pane->max)
         PaneDMajor(*childP) = pane->max;
   ...
   for (pass = 0; sizeused != MajorSize(pw) && pass < (9 * num_panes); pass++)
   {
      pane = &(PaneInfo(*childP)->panedw);
      if ((!pane->skip_adjust || sflag) || cdir != _dir)
      {
         ...
         else if (sizeused - MajorSize(pw) < PaneDMajor(*childP) && PaneDMajor(*childP) - (sizeused - MajorSize(pw)) > 1)
  ...
  */
      while ((childP < children) || ((childP - children) >= num_panes))
      {
         ...
         if (cdir == _dir)
         {
            ...
            {
               PaneDPos(*childP) = pos;
               pos              += PaneDMajor(*childP) + spacing + 2 * (*childP)->core.border_width;
            }
            ...
            if (!rflag)
            {
               if (pos > MajorSize(pw))
               {
                  ...
                  if (PaneDMajor(*childP) > (pos - MajorSize(pw)))
                     PaneDMajor(*childP) = (PaneDMajor(*childP) - (pos - MajorSize(pw)));
                  else
                     PaneDMajor(*childP) = 1;
               }
               else
   ...
   Widget              instigator)
{
   ...
   for (childP = children, i = 0; i < num_panes; childP++, i++)
   {
      ...
      if (sash) /* IF THIS IS NOT NULL */
      {
         ...
         if (separator)
         {
            sepPos = MajorChildPos(pw, *childP) + MajorChildSize(pw, *childP) + 2 * (*childP)->core.border_width + pw->paned_window.spacing / 2 - MajorChildSize(pw, separator) / 2 - separator->core.border_width;
         ...
         if ((minor_dim > (MinorSize(pw) - MinorChildSize(pw, sash))) || (minor_dim < 0))
            minor_dim = 0;
         major_dim = MajorChildPos(pw, *childP) + MajorChildSize(pw, *childP) + 2 * (*childP)->core.border_width + pw->paned_window.spacing / 2 - MajorChildSize(pw, sash) / 2 - sash->core.border_width;
	    ...
	    * also insuring the sash is Raised in the same request */
         ...
         if (XtIsRealized(pane->sash))
            XConfigureWindow(XtDisplay(pane->sash), XtWindow(pane->sash), CWX | CWY | CWStackMode, &changes);
      }
      else
      {
         if (*childP != instigator)
         {
            int tmp = MinorSize(pw) - 2 * (pw->core.border_width + MinorMargin(pw));
            if (tmp <= 0) tmp = 1;
            XmeConfigureObject(*childP,
                               Major(pw, PaneDPos(*childP), offset),
                               Major(pw, offset, PaneDPos(*childP)),
   ...
   XmPanedWindowWidget pw)
{
   ...
   for (childP = children + 1; childP - children < num_panes; childP++)
   {
      ...
      {
         offset = PaneOldDPos(*childP) - (pw->paned_window.spacing + sep_size) / 2;
         XDrawLine(XtDisplay(pw), XtWindow(pw), pw->paned_window.flipgc, Major(pw, offset, 0), Major(pw, 0, offset), Major(pw, offset, pw->core.width), Major(pw, pw->core.height, offset));
         offset = PaneDPos(*childP) - (pw->paned_window.spacing + sep_size) / 2;
         XDrawLine(XtDisplay(pw), XtWindow(pw), pw->paned_window.flipgc, Major(pw, offset, 0), Major(pw, 0, offset), Major(pw, offset, pw->core.width), Major(pw, pw->core.height, offset));
...
*******************************************************************/
      ...
      RefigureLocationsAndCommit(pw, PaneIndex(PaneInfo(w)->panedw.sash), FirstPane, False);
   }
   else if (PaneLast(pw))
   {
      ...
      RefigureLocationsAndCommit(pw, PaneIndex(PaneInfo(w)->panedw.sash), LastPane, False);
   }
   pw->paned_window.increment_count = 0;
}
   ...
   XtPointer callData)
{
   ...
   switch (call_data->event->xany.type)
   {
      case ButtonPress:
      case ButtonRelease:
         pos = Major(pw, call_data->event->xbutton.x_root, call_data->event->xbutton.y_root);
         break;
      case KeyRelease:
         return;
      case KeyPress:
         ...
         if (action_param == _KEY)
         {
            ...
            if (!pw->paned_window.increment_count)
            {
               ...
               pw->paned_window.timer           = XtAppAddTimeOut(XtWidgetToApplicationContext((Widget)pw),
         ...
         return;
      case MotionNotify:
         pos = Major(pw, call_data->event->xmotion.x_root, call_data->event->xmotion.y_root);
         break;
      default:
         pos = PaneStartPos(pw);
   }
   switch (action_param)
   {
      ...
      case _MOVE:
		  ...
		  *****************************************************/
            ...
            RefigureLocations(pw, PaneIndex(PaneInfo(w)->panedw.sash), FirstPane, False, True);
         }
         else if (PaneLast(pw))
         {
            if (diff >= (int)PaneLastDMajor(pw))
            ...
            RefigureLocations(pw, PaneIndex(PaneInfo(w)->panedw.sash), LastPane, False, True);
         }
         DrawTrackLines(pw);
         break;
      case _COMMIT:
   ...
   XtWidgetGeometry *reply)
{
   ...
   if ((mask & Minor(pw, CWWidth, CWHeight)))
   {
       ...
       */
      childP = children;
      while ((num_panes < pw->paned_window.num_managed_children) && XtIsRectObj(*childP) && IsPane(*childP))
      {
         childP++;
         num_panes++;
      }
      ...
      for (childP = children, i = 0; i < num_panes; childP++, i++)
      {
         if ((MinorChildSize(pw, *childP) + (*childP)->core.border_width) > childMinor + childBorderWidth)
         {
      ...
      {
         MinorAssign(pw, geo_desired, MinorReq(pw, request) + 2 * (childBorderWidth + MinorMargin(pw)));
         geo_desired.request_mode |= Minor(pw, CWWidth, CWHeight);
         switch (XtMakeGeometryRequest((Widget)pw, &geo_desired, &geo_reply))
         {
            case XtGeometryYes:
               MinorAssign(pw, allowed, MinorReq(pw, request));
               break;
            case XtGeometryAlmost:
               if (MinorReq(pw, &geo_reply) > 2 * (childBorderWidth + MinorMargin(pw)))
                  tmp = MinorReq(pw, &geo_reply) - 2 * (childBorderWidth + MinorMargin(pw));
               else
                  tmp = 1;
            ...
            case XtGeometryNo:
            default:
               MinorAssign(pw, allowed, MinorSize(pw) - 2 * (childBorderWidth + MinorMargin(pw)));
               break;
         }
      }
      else
      ...
      if ((new_major = NeedsAdjusting(pw)) != 0)
      {
         ...
         switch (XtMakeGeometryRequest((Widget)pw, &geo_desired, &geo_reply))
         {
            case XtGeometryYes:
               ...
               break;
            case XtGeometryAlmost:
               if ((new_major > MajorReq(pw, &geo_reply)) && MajorReq(pw, request) > (new_major - MajorReq(pw, &geo_reply)))
                  tmp = MajorReq(pw, request) - (new_major - MajorReq(pw, &geo_reply));
               else
                  tmp = 1;
    ...
    */
   if ((!is_almost) && (((mask & CWWidth) && (allowed.width == request->width)) || (!(mask & CWWidth))) && (((mask & CWHeight) && (allowed.height == request->height)) || (!(mask & CWHeight))))
   {
   ...
   register Widget w)
{
   ...
   */
   ...
   pane->isPane = TRUE;
   n            = 0;
   XtSetArg(args[n], Minor(pw, XmNwidth, XmNheight), MinorSize(pw));
   n++;
   ...
   n++;
   XtSetArg(args[n], XmNorientation, Minor(pw, XmHORIZONTAL, XmVERTICAL));
   n++;
   XtSetArg(args[n], XmNnavigationType, (XtArgVal)XmNONE);
   n++;
   pw->paned_window.recursively_called         = True;
   pane->separator                             = XtCreateWidget("Separator", xmSeparatorGadgetClass, (Widget)pw, args, n);
   pw->paned_window.recursively_called         = False;
    ...
    */
   n = 0;
   XtSetArg(args[n], XmNwidth, pw->paned_window.sash_width);
   n++;
   XtSetArg(args[n], XmNheight, pw->paned_window.sash_height);
   n++;
   XtSetArg(args[n], XmNshadowThickness, pw->paned_window.sash_shadow_thickness);
   n++;
   XtSetArg(args[n], XmNunitType, (XtArgVal)XmPIXELS);
   n++;
   pw->paned_window.recursively_called = True;
   ...
   Widget w)
{
   ...
   */
        ...
        childP++, i++)
   {
      ...
      PaneDMajor(*childP) = MajorChildSize(pw, *childP);
      newMinor            = childMinor + 2 * (childBorderWidth - (*childP)->core.border_width);
      ...
      if ((XtIsManaged(*childP)) && (*childP != children[num_panes - 1]))
      {
         ...
         if (sash)
         {
            if (PaneInfo(*childP)->panedw.min != PaneInfo(*childP)->panedw.max)
            {
               if (!XtIsManaged(sash)) XtManageChild(sash);
               if (XtIsRealized(sash))
                  XRaiseWindow(XtDisplay(sash), XtWindow(sash));
            }
            else if (XtIsManaged(sash))
               XtUnmanageChild(sash);
         }
      }
      else
      {
   ...
   major_dim += pw->paned_window.spacing * ((num_panes) ? num_panes - 1 : 0) + 2 * MajorMargin(pw);
   while ((result = XtMakeResizeRequest(w,
      ...
      ;
   if (result == XtGeometryYes || result == XtGeometryAlmost || MajorChildSize(pw, w) == major_dim)
   {
   ...
   Cardinal *num_args) /* unused */
{
   ...
   {
      XtSetArg(sashargs[n], XmNwidth, newpw->paned_window.sash_width);
      n++;
   }
   ...
   {
      XtSetArg(sashargs[n], XmNheight, newpw->paned_window.sash_height);
      n++;
   }
   if (oldpw->paned_window.sash_shadow_thickness != newpw->paned_window.sash_shadow_thickness)
   {
      XtSetArg(sashargs[n], XmNshadowThickness, newpw->paned_window.sash_shadow_thickness);
      n++;
   }
   if (oldpw->paned_window.separator_on != newpw->paned_window.separator_on && num_children > 2)
   {
   ...
   if (!XmRepTypeValidValue(XmRID_ORIENTATION,
                            newpw->paned_window.orientation,
                            (Widget)newpw))
   {
         ...
         if (IsPane(*childP))
         {
            ...
            if (sash) /* IF THIS IS NOT NULL */
            {
               ...
               if (newpw->paned_window.sash_indent < 0)
                  minor_dim = MinorSize(newpw) + newpw->paned_window.sash_indent - MinorChildSize(newpw, sash) - sash->core.border_width * 2;
               else
                  minor_dim = newpw->paned_window.sash_indent;
               /* PREVENT SASH FROM DISAPPEARING FROM SCREEN */
               if ((minor_dim > (MinorSize(newpw) - MinorChildSize(newpw, sash))) || (minor_dim < 0))
                  minor_dim = 0;
               major_dim = MajorChildPos(newpw, *childP) + MajorChildSize(newpw, *childP) + 2 * (*childP)->core.border_width + newpw->paned_window.spacing / 2 - MajorChildSize(newpw, sash) / 2 - sash->core.border_width;
   ...
   {
      newpw->core.width = newpw->core.width + ((2 * newpw->paned_window.margin_width) - (2 * oldpw->paned_window.margin_width));
      returnFlag        = True;
   }
   if ((oldpw->paned_window.spacing != newpw->paned_window.spacing || oldpw->paned_window.margin_height != newpw->paned_window.margin_height || (requestpw->paned_window.refiguremode && !(oldpw->paned_window.refiguremode))) && XtIsRealized((Widget)newpw))
   {
      Dimension needed;
   ...
   Cardinal *num_args)
{
   ...
   if (PanePosIndex(old) != PanePosIndex(new_w))
   {
      ...
      else
      {
         ...
         if (PanePosIndex(new_w) < PanePosIndex(old))
            inc = -1;
         else
            inc = 1;
         ...
         if ((current.x != XtX(new_w)) || (current.y != XtY(new_w)) || (current.width != XtWidth(new_w)) || (current.height != XtHeight(new_w)) || (current.border_width != XtBorderWidth(new_w)))
   ...
   if (old_pane->min != new_pane->min || old_pane->max != new_pane->max)
   {
      ...
      if ((new_pane->min == new_pane->max) && (new_pane->sash != NULL))
      {
      ...
      else
      {
         ...
         if (new_pane->position != count)
         {
            ...
            if (new_pane->min != new_pane->max)
            {
               ...
               {
                  Cardinal nargs = 0;
                  XtSetArg(sashargs[nargs], XmNwidth, pw->paned_window.sash_width), nargs++;
                  XtSetArg(sashargs[nargs], XmNheight, pw->paned_window.sash_height), nargs++;
                  XtSetArg(sashargs[nargs], XmNshadowThickness, pw->paned_window.sash_shadow_thickness), nargs++;
                  XtSetArg(sashargs[nargs], XmNunitType, (XtArgVal)XmPIXELS), nargs++;
                  ...
                  new_pane->sash                      = XtCreateManagedWidget("sash",
                                                         ...
                                                         sashargs,
                                                         nargs);
                  XtAddCallback(new_pane->sash, XmNcallback, HandleSash, (XtPointer)new_w);
                  pw->paned_window.recursively_called = False;
