...
#define GRABPTRERROR _XmMMsgCascadeB_0003
    ...
    static Widget
    _XmInputInWidget(Widget w, Position x, Position y);

/********    End Static Function Declarations    ********/
/******************************************************/
/* copy from XmeGadgetAtPoint, buts works for widget too,
   only used in this module so far, so let it static */
static Widget _XmInputInWidget(Widget w, Position x, Position y)
{
  int i;
  Widget child;
...
Widget XmTrackingEvent(Widget widget, Cursor cursor, Boolean confineTo, XEvent *pev)
{
  ...
  while (True) {
    ...
    if (((pev->type == ButtonRelease) && (pev->xbutton.button & Button1)) ||
        ((pev->type == KeyRelease) && key_has_been_pressed))
    {
      if ((!confineTo) && (pev->xbutton.window == w)) {
        ...
        if ((pev->xbutton.x < 0) || (pev->xbutton.y < 0) ||
            (pev->xbutton.x > widget->core.width) || (pev->xbutton.y > widget->core.height))
        {
      ...
      target = XtWindowToWidget(pev->xbutton.display, pev->xbutton.window);
...
Widget XmTrackingLocate(Widget widget, Cursor cursor, Boolean confineTo)
{
  XEvent ev;
