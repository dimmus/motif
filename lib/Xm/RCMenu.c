...
{
   (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(w, e, NULL, NULL);
}
   ...
   XEvent               *event)
{
   ...
   {                   /* of option button (p) */
      ...
      if (LayoutIsRtoLG(p))
         x = RC_MemWidget(m) ? (XtWidth(p) - (XtWidth(RC_MemWidget(m)) + G_HighlightThickness(p) + 2 * MGR_ShadowThickness(m))) : XtWidth(p) - G_HighlightThickness(p);
      else
         x = RC_MemWidget(m) ? (G_HighlightThickness(p) + MGR_ShadowThickness(m) - XtX(RC_MemWidget(m))) : G_HighlightThickness(p);
      y = RC_MemWidget(m) ? (Half(XtHeight(p)) - (XtY(RC_MemWidget(m)) + Half(XtHeight(RC_MemWidget(m))))) : XtY(p);
   }
   else if (IsBar(root))
   {
   ...
   Boolean *popped_up)
{
   ...
   if (IsBar(rc))
   {
      ...
      {
         (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownDone))((Widget)RC_PopupPosted(rc), event, NULL, NULL);
      }
   ...
   else if (IsOption(toplevel_menu) && !XmIsRowColumn(w)
            && (w != RC_MemWidget(rc)))
   {
      _XmMenuFocus(XtParent(rc), XmMENU_END, _time);
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))((Widget)XtParent(rc), event, NULL, NULL);
      MenuDisarm((Widget)toplevel_menu);
      XtUngrabPointer(XtParent(rc), _time);
   }
   else
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownDone))((Widget)rc, event, NULL, NULL);
   if (IsPulldown(rc))
   ...
   Boolean add_handler)
{
   ...
   if (add_handler)
      XtInsertEventHandler(_XmFindTopMostShell(widget), eventMask, False, SwallowEventHandler, NULL, XtListHead);
   else
   ...
   switch (operation)
   {
      case XmMENU_END:
	      ...
	      */
		    ...
		    */
                  XGetWindowAttributes(XtDisplay(mst->RC_menuFocus.oldWidget), mst->RC_menuFocus.oldFocus, &xwa);
...
**/
                  {
                     SetInputFocus(XtDisplay(w), mst->RC_menuFocus.oldFocus, mst->RC_menuFocus.oldRevert, mst->RC_menuFocus.oldTime);
                  }
               }
            }
            /*
      ...
      case XmMENU_BEGIN:
         ...
         SetInputFocus(XtDisplay(w), XtWindow(w), mst->RC_menuFocus.oldRevert, mst->RC_menuFocus.oldTime);
         /*
	   ...
	   */
         ...
         if (mst->RC_menuFocus.oldWidget != NULL)
            XtAddCallback(mst->RC_menuFocus.oldWidget, XtNdestroyCallback, (XtCallbackProc)InvalidateOldFocus, (XtPointer)&mst->RC_menuFocus.oldFocus);
         XFlush(XtDisplay(w));
         break;
      case XmMENU_MIDDLE:
         SetInputFocus(XtDisplay(w), XtWindow(w), mst->RC_menuFocus.oldRevert, mst->RC_menuFocus.oldTime);
   ...
   Cardinal *num_parms)
{
   ...
   if (IsPopup(m))
   {
	  ...
	  */
      ...
      else
      {
         ...
         (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownDone))(XtParent(m), event, NULL, NULL);
      }
   }
   else if (IsOption(m))
   {
      ...
      (*(((XmGadgetClassRec *)XtClass(g))->gadget_class.arm_and_activate))((Widget)g, event, parms, num_parms);
   }
   else if (IsBar(m))
   {
      if (RC_IsArmed(m))
   ...
   {
      /* Let the menushell widget clean things up */
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownOne))(XtParent(m), event, NULL, NULL);
   }
}
   ...
   int    mode) /* Add, Replace or Delete */
{
   ...
   else if (XmeTraitGet((XtPointer)XtClass(w), XmQTmenuSavvy) != NULL && XtClass(w) != xmLabelWidgetClass && XtClass(w) != xmLabelGadgetClass)
   {
      if (mode == XmREPLACE)
      {
         /* Remove old one */
    ...
    */
   if (w && !(w->core.being_destroyed) && !((mode == XmDELETE) && InSharedMenupaneHierarchy((XmRowColumnWidget)w)))
   {
      ...
      if (XtIsManaged((child = w->composite.children[i])))
      {
         ...
         else if (XmIsCascadeButton(child))
         {
            ProcessMenuTree((XmRowColumnWidget)
                               CB_Submenu((XmCascadeButtonWidget)child),
                            mode);
         }
      }
   }
   ...
   int    mode)
{
   ...
   if (menuSavvy != NULL)
   {
      if (mode == XmADD)
      {
         ...
         {
            if ((XmIsCascadeButton(w) || XmIsCascadeButtonGadget(w)) && XmIsRowColumn(XtParent(w)) && IsBar(XtParent(w)))
            {
   ...
   for (i = 0; i < count; i++)
   {
      ...
      while (--num_keys >= 0)
         if (keys[num_keys].keysym != NoSymbol)
         {
            ...
            _AddToKeyboardList(w, eventTypes[i], keys[num_keys].keysym, modifiers[i] | keys[num_keys].modifiers, needGrab, isMnemonic);
            keysyms[i] = NoSymbol;
         }
      XtFree((char *)keys);
      ...
      for (i = 0; i < num_ks; i += syms_per)
      {
         ...
         if ((ks_tbl[i + 1] == NoSymbol) || (ks_tbl[i + 1] == uc))
         {
            if (keysym == lc || keysym == uc)
               AddKeycodeToKeyboardList(w, eventType, (KeyCode)(min_kc_rtn + (i / syms_per)), keysym, modifiers, needGrab, isMnemonic);
         }
         else
         {
            if (keysym == ks_tbl[i])
   ...
   Boolean      isMnemonic)
{
   ...
   if (MGR_NumKeyboardEntries(rowcol) >= MGR_SizeKeyboardList(rowcol))
   {
      ...
      MGR_KeyboardList(rowcol)      = (XmKeyboardData *)XtRealloc((char *)MGR_KeyboardList(rowcol),
	  ...
	  */
         if (klist[i].needGrab && (w->core.being_destroyed || notInSharedMenupaneHierarchy))
         {
	      ...
	      grab on the same key/modifier */
            for (k = 0; k < count; k++)
            {
               if (k != i && klist[k].needGrab && klist[k].key == klist[i].key && klist[k].modifiers == klist[i].modifiers)
               {
                  another = True;
                  break;
               }
   ...
   int       keyboard_mode)
{
   ...
   XtGrabKey(widget, keycode, modifiers | ScrollLockMask, owner_events, pointer_mode, keyboard_mode);
   XtGrabKey(widget, keycode, modifiers | NumLockMask, owner_events, pointer_mode, keyboard_mode);
   XtGrabKey(widget, keycode, modifiers | LockMask | ScrollLockMask, owner_events, pointer_mode, keyboard_mode);
   XtGrabKey(widget, keycode, modifiers | LockMask | NumLockMask, owner_events, pointer_mode, keyboard_mode);
   XtGrabKey(widget, keycode, modifiers | ScrollLockMask | NumLockMask, owner_events, pointer_mode, keyboard_mode);
   XtGrabKey(widget, keycode, modifiers | LockMask | ScrollLockMask | NumLockMask, owner_events, pointer_mode, keyboard_mode);
}
   ...
   unsigned int      modifiers)
{
   ...
   if (IsPopup(rowcol))
   {
      for (i = 0; i < rowcol->row_column.postFromCount; i++)
         GrabKeyWithLockMask(rowcol->row_column.postFromList[i], detail, modifiers, False, GrabModeAsync, GrabModeAsync);
   }
   ...
   else if (IsPulldown(rowcol))
   {
      for (i = 0; i < rowcol->row_column.postFromCount; i++)
         GrabKeyOnAssocWidgets((XmRowColumnWidget)
                                  XtParent(rowcol->row_column.postFromList[i]),
                               detail,
                               modifiers);
   }
}
   ...
   Widget            widget)
{
    ...
    */
   if (IsPopup(m))
      XtAddCallback(widget, XtNdestroyCallback, (XtCallbackProc)_XmRC_RemoveFromPostFromListOnDestroyCB, (XtPointer)m);
}
   ...
   Widget            widget)
{
   ...
   if (found)
   {
      ...
      if (IsPopup(m))
         XtRemoveCallback(widget, XtNdestroyCallback, (XtCallbackProc)_XmRC_RemoveFromPostFromListOnDestroyCB, (XtPointer)m);
   }
}
...
*/
static void
DismissTearOffSubMenu(XmRowColumnWidget menu)
{
   int i;
   ...
   XEvent *event)
{
   ...
   if (XmIsMenuShell(XtParent(wid)))
   {
      ...
      if ((IsPopup(rc) && !((XmMenuShellWidget)XtParent(rc))->shell.popped_up) || (!IsPopup(rc) && !RC_PopupPosted(rc)))
         return (False);
   }
   ...
   Position y_root)
{
    ...
    */
   if ((!gadget || !XtIsSensitive((Widget)gadget) || !(XmIsCascadeButtonGadget(gadget))) && /* !cbg kludge */
       (RC_PopupPosted(rc)))
   {
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(RC_PopupPosted(rc), NULL, NULL, NULL);
   }
   ...
   XEvent *event)
{
   ...
   if ((event->xbutton.x_root >= x_root) && (event->xbutton.x_root < x_root + wid->core.width) && (event->xbutton.y_root >= y_root) && (event->xbutton.y_root < y_root + wid->core.height))
   {
   ...
   Cardinal *num_params) /* unused */
{
   ...
   if (menuSTrait->verifyButton((Widget)rc, event) && (event->xbutton.x_root >= x_root) && (event->xbutton.x_root < x_root + rc->core.width) && (event->xbutton.y_root >= y_root) && (event->xbutton.y_root < y_root + rc->core.height))
   {
      if (!XmIsMenuShell(XtParent(rc)) && RC_Type(rc) != XmMENU_BAR && RC_Type(rc) != XmMENU_OPTION)
         XChangeActivePointerGrab(XtDisplay(rc),
                                  ((unsigned int)(ButtonReleaseMask | PointerMotionMask | PointerMotionHintMask | EnterWindowMask | LeaveWindowMask)),
   ...
   Cardinal *num_params) /* unused */
{
   ...
   if (!_XmIsEventUnique(event) || !menuSTrait->verifyButton(wid, event) || (IsBar(w) && !RC_IsArmed(w)))
      return;
   if (w->core.window == event->xbutton.window)
      gadget = (XmGadget)XmObjectAtPoint((Widget)w, event->xbutton.x, event->xbutton.y);
   else
      gadget = NULL;
   nonButton = ((IsPulldown(w) || IsPopup(w)) && !XmIsMenuShell(XtParent(w)) && (!gadget || (XtClass(gadget) == xmLabelGadgetClass) || (XtClass(gadget) == xmSeparatorGadgetClass)));
 ...
 ******************************************************************/
void
_XmMenuGadgetDrag(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
   ...
   Widget memWidget)
{
   ...
   if (cbg == (Widget)NULL || memWidget == (Widget)NULL) return;
   ...
   Widget widget)
{
    ...
    */
   if (XmIsCascadeButton(widget) && RC_Type(XtParent(widget)) != XmMENU_BAR && !XmIsMenuShell(XtParent(XtParent(widget))) && CB_Submenu(widget) && ((XmMenuShellWidget)XtParent(CB_Submenu(widget)))->shell.popped_up && XmIsMenuShell(XtParent(CB_Submenu(widget))))
   {
      return (False);
   }
   return (True);
...
MenuStatus(Widget wid)
{
   ...
   RC_SetBit(menu_status, XmMENU_TEAR_OFF_SHELL_DESCENDANT_BIT, _XmIsTearOffShellDescendant((Widget)rc));
   ...
   Boolean *popped_up)
{
    ...
    */
   for (pane = rc, depth = 0;
        XmIsRowColumn(pane) && ((IsPulldown(pane) || IsPopup(pane)) && XmIsMenuShell(XtParent(pane)));
        depth++)
   {
 	...
 	*/
	  ...
	  up the parents */
      if (pane == (XmRowColumnWidget)NULL || !XmIsRowColumn(pane)) break;
   }
           ...
           i++, q++)
      {
         if (XtIsManaged(*q))
         {
            if (XmIsCascadeButtonGadget(*q))
            {
               ...
               if (CBG_Submenu(p) && SearchMenu((XmRowColumnWidget)CBG_Submenu(p), (XmRowColumnWidget *)parent_m, (RectObj)child, w, setHistory))
               {
                  if (setHistory)
            ...
            else if (XmIsCascadeButton(*q))
            {
               ...
               if (CB_Submenu(p) && SearchMenu((XmRowColumnWidget)CB_Submenu(p), (XmRowColumnWidget *)parent_m, (RectObj)child, w, setHistory))
               {
                  if (setHistory)
                     RC_MemWidget(search_m) = (Widget)child;
                  return (TRUE);
   ...
   XEvent *event)
{
   ...
   if (IsPopup(w))
   {
      valid = event && (_XmMatchBtnEvent(event, XmIGNORE_EVENTTYPE, RC_PostButton(w), RC_PostModifiers(w)) || _XmMatchBSelectEvent(w, event));
   }
   else
   {
