...
#include <ctype.h>
#include <stdio.h>
static void SwallowEventHandler(Widget widget,
                                XtPointer client_data,
                                XEvent *event,
                                Boolean *continue_to_dispatch);
static void LocatePulldown(XmRowColumnWidget root,
                           XmCascadeButtonWidget p,
                           XmRowColumnWidget m,
                           XEvent *event);
static int OnPostFromList(XmRowColumnWidget menu, Widget widget);
static XmRowColumnWidget FindMenu(Widget w);
static void LotaMagic(XmRowColumnWidget m, RectObj child, XmRowColumnWidget *parent_m, Widget *w);
static int InMenu(XmRowColumnWidget search_m,
                  XmRowColumnWidget *parent_m,
                  RectObj child,
                  Widget *w);
static Boolean SearchMenu(XmRowColumnWidget search_m,
                          XmRowColumnWidget *parent_m,
                          RectObj child,
                          Widget *w,
                          Boolean setHistory);
static void PrepareToCascade(XmRowColumnWidget submenu, Widget cb, XEvent *event);
static void GetLastSelectToplevel(XmRowColumnWidget submenu);
...
static void TearOffArm(Widget w);
static void MenuBarCleanup(XmRowColumnWidget rc);
static void InvalidateOldFocus(Widget oldWidget, Window **poldFocus, XEvent *event);
static void ProcessMenuTree(XmRowColumnWidget w, int mode);
static void AddToKeyboardList(Widget w, char *kbdEventStr, Boolean needGrab, Boolean isMnemonic);
static void _AddToKeyboardList(Widget w,
                               unsigned int eventType,
                               KeySym keysym,
                               unsigned int modifiers,
                               Boolean needGrab,
                               Boolean isMnemonic);
static void AddKeycodeToKeyboardList(Widget w,
                                     unsigned int eventType,
                                     KeyCode keycode,
                                     KeySym keysym,
                                     unsigned int modifiers,
                                     Boolean needGrab,
                                     Boolean isMnemonic);
static void RemoveFromKeyboardList(Widget w);
static void GrabKeyOnAssocWidgets(XmRowColumnWidget rowcol,
                                  KeyCode detail,
                                  unsigned int modifiers);
static void UngrabKeyOnAssocWidgets(XmRowColumnWidget rowcol,
                                    KeyCode detail,
                                    unsigned int modifiers);
static Boolean InSharedMenupaneHierarchy(XmRowColumnWidget m);
static void SetCascadeField(XmRowColumnWidget m, Widget cascadeBtn, Boolean attach);
static void BtnDownInRowColumn(Widget rc, XEvent *event, Position x_root, Position y_root);
static void CheckUnpostAndReplay(Widget rc, XEvent *event);
static Boolean VerifyMenuButton(Widget w, XEvent *event);
static Boolean UpdateMenuHistory(XmRowColumnWidget menu,
                                 Widget child,
                                 Boolean updateOnMemWidgetMatch);
static void RadioBehaviorAndMenuHistory(XmRowColumnWidget m, Widget w);
 ...
 */
void _XmRCMenuProcedureEntry(int proc, Widget widget, ...)
{
  int flag;
  XtPointer data;
  XtPointer data2;
  ...
  switch (proc) {
    ...
    case XmMENU_CALLBACK:
      ...
      ChildsActivateCallback((XmRowColumnWidget)widget, (Widget)data, (XtPointer)data2);
      break;
    ...
    case XmMENU_SUBMENU: /* appears to be obsolete within Xm */
      flag = va_arg(ap, int);
      data = va_arg(ap, XtPointer);
      SetCascadeField((XmRowColumnWidget)widget, (Widget)data, (Boolean)flag);
      break;
...
static void MenuShellPopdown(Widget w, XEvent *e)
{
  (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(
      w, e, NULL, NULL);
}
  ...
  if (IsPopup(submenu)) {
    ...
    if (mst->RC_LastSelectToplevel)
      ...
      if (RC_TearOffActive(topLevel))
        topLevel = (XmRowColumnWidget)topLevel->row_column.tear_off_lastSelectToplevel;
    }
                           ...
                           XEvent *event)
{
  ...
  if (IsOption(root)) /* near hot spot */ { /* of option button (p) */
    if (!XtIsRealized((Widget)m))
      XtRealizeWidget((Widget)m);
    if (LayoutIsRtoLG(p))
      x = RC_MemWidget(m) ? (XtWidth(p) - (XtWidth(RC_MemWidget(m)) + G_HighlightThickness(p) +
                                           2 * MGR_ShadowThickness(m))) :
                            XtWidth(p) - G_HighlightThickness(p);
    else
      x = RC_MemWidget(m) ?
              (G_HighlightThickness(p) + MGR_ShadowThickness(m) - XtX(RC_MemWidget(m))) :
              G_HighlightThickness(p);
    y = RC_MemWidget(m) ?
            (Half(XtHeight(p)) - (XtY(RC_MemWidget(m)) + Half(XtHeight(RC_MemWidget(m))))) :
            XtY(p);
  }
  else if (IsBar(root)) {
...
void _XmMenuPopDown(Widget w, XEvent *event, Boolean *popped_up)
{
  ...
  if (IsBar(rc)) {
    if (RC_PopupPosted(rc)) {
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownDone))(
          (Widget)RC_PopupPosted(rc), event, NULL, NULL);
    }
  ...
  else if (!XmIsMenuShell(XtParent(rc))) /* torn! */ {
  ...
  else if (IsOption(toplevel_menu) && !XmIsRowColumn(w) && (w != RC_MemWidget(rc))) {
    _XmMenuFocus(XtParent(rc), XmMENU_END, _time);
    (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(
        (Widget)XtParent(rc), event, NULL, NULL);
    MenuDisarm((Widget)toplevel_menu);
    XtUngrabPointer(XtParent(rc), _time);
  }
  else
 ...
 * focus.
 */
static void SwallowEventHandler(Widget widget,         /* unused */
...
void _XmSetSwallowEventHandler(Widget widget, Boolean add_handler)
{
  ...
  if (add_handler)
    XtInsertEventHandler(
        _XmFindTopMostShell(widget), eventMask, False, SwallowEventHandler, NULL, XtListHead);
  else
    XtRemoveEventHandler(_XmFindTopMostShell(widget), eventMask, False, SwallowEventHandler, NULL);
}

/*
 * Action routines specific to traversal.
 */
void _XmMenuUnmap(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
...
void _XmMenuFocusOut(Widget cb, XEvent *event, String *param, Cardinal *num_param)
{
  /*
 ...
 */
void _XmMenuFocusIn(Widget wid, XEvent *event, String *param, Cardinal *num_param)
{
  XmCascadeButtonWidget cb = (XmCascadeButtonWidget)wid;
  /* Forward the event for normal processing */
...
**/
    ...
    case XmMENU_BEGIN:
      ...
      if (tmpWindow != XtWindow(w)) {
        ...
        if (tmpWindow != mst->RC_menuFocus.oldFocus) {
          ...
          mst->RC_menuFocus.oldWidget = XtWindowToWidget(XtDisplay(w), tmpWindow);
        }
      }
      if (mst->RC_menuFocus.oldWidget != NULL)
        XtAddCallback(mst->RC_menuFocus.oldWidget,
                      XtNdestroyCallback,
                      (XtCallbackProc)InvalidateOldFocus,
                      (XtPointer)&mst->RC_menuFocus.oldFocus);
      XFlush(XtDisplay(w));
    ...
    case XmMENU_MIDDLE:
      ...
      if ((tmpWindow != XtWindow(w)) && (_time > mst->RC_menuFocus.oldTime)) {
        ...
        mst->RC_menuFocus.oldTime = _time;
        if (tmpWindow != mst->RC_menuFocus.oldFocus) {
          if (mst->RC_menuFocus.oldFocus != (Window)NULL && mst->RC_menuFocus.oldWidget != NULL)
            XtRemoveCallback(mst->RC_menuFocus.oldWidget,
                             XtNdestroyCallback,
                             (XtCallbackProc)InvalidateOldFocus,
                             (XtPointer)&mst->RC_menuFocus.oldFocus);
 ...
 * an option menu.
 */
void _XmRCArmAndActivate(Widget w, XEvent *event, String *parms, Cardinal *num_parms)
{
  ...
  if (IsPopup(m)) {
    ...
    if (!XtIsManaged((Widget)m)) {
      ...
      (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownDone))(
          XtParent(m), event, NULL, NULL);
    }
  }
  else if (IsOption(m)) {
    XmGadget g = (XmGadget)XmOptionButtonGadget((Widget)m);
    /* Let the cascade button gadget do the work */
    (*(((XmGadgetClassRec *)XtClass(g))->gadget_class.arm_and_activate))(
        (Widget)g, event, parms, num_parms);
  }
  else if (IsBar(m)) {
    if (RC_IsArmed(m)) {
 ...
 */
void _XmRC_DoProcessMenuTree(Widget w, int mode) /* Add, Replace or Delete */
{
  /*
  ...
  if (XmIsCascadeButton(w) || XmIsCascadeButtonGadget(w)) {
    ...
    if (XmIsRowColumn(XtParent(w))) {
      if (IsOption(XtParent(w))) {
      ...
      else if (IsBar(XtParent(w)) || IsPopup(XtParent(w)) || IsPulldown(XtParent(w))) {
        if (mode == XmREPLACE) {
        ...
        if (XmIsCascadeButtonGadget(w))
          ...
          w = (Widget)CB_Submenu(w);
           ...
           XtClass(w) != xmLabelWidgetClass && XtClass(w) != xmLabelGadgetClass)
  {
    if (mode == XmREPLACE) {
      /* Remove old one */
      _XmRC_ProcessSingleWidget(w, XmDELETE);
  ...
  if (w && !(w->core.being_destroyed) &&
      !((mode == XmDELETE) && InSharedMenupaneHierarchy((XmRowColumnWidget)w)))
  {
  ...
  for (i = 0; i < w->composite.num_children; i++) {
    if (XtIsManaged((child = w->composite.children[i]))) {
      ...
      if (XmIsCascadeButtonGadget(child)) {
        ...
        ProcessMenuTree((XmRowColumnWidget)CB_Submenu((XmCascadeButtonWidget)child), mode);
      }
    }
  }
}
 ...
 */
void _XmRC_ProcessSingleWidget(Widget w, int mode)
{
  Arg args[2];
  Widget child;
  XmMenuSavvyTrait menuSavvy;
  menuSavvy = (XmMenuSavvyTrait)XmeTraitGet((XtPointer)XtClass(w), XmQTmenuSavvy);
  if (menuSavvy != NULL) {
    if (mode == XmADD) {
      ...
      if (mnemonic != XK_VoidSymbol && mnemonic != (KeySym)NULL) /* CR 5255 */ {
        if ((XmIsCascadeButton(w) || XmIsCascadeButtonGadget(w)) && XmIsRowColumn(XtParent(w)) &&
            IsBar(XtParent(w)))
        {
          /* Add Alt-mnemonic for cascade buttons in
                  menu bars only */
          _AddToKeyboardList(w, KeyRelease, mnemonic, Mod1Mask, True, False);
        }
        _AddToKeyboardList(w, KeyRelease, mnemonic, 0, False, True);
      }
    ...
    if (IsPopup(m) || IsBar(m)) {
      ...
      if (mode == XmADD) {
        if (RC_Mnemonic(m) != XK_VoidSymbol) {
          _AddToKeyboardList(w, KeyRelease, RC_Mnemonic(m), Mod1Mask, True, True);
          if (child) {
  ...
  for (i = 0; i < count; i++) {
    ...
    while (--num_keys >= 0)
      if (keys[num_keys].keysym != NoSymbol) {
        ...
        _AddToKeyboardList(w,
                           eventTypes[i],
                           keys[num_keys].keysym,
    ...
    if (keysyms[i] != NoSymbol)
      _AddToKeyboardList(w, eventTypes[i], keysyms[i], modifiers[i], needGrab, isMnemonic);
  }
                               ...
                               Boolean isMnemonic)
{
  ...
  if (!isMnemonic) {
    AddKeycodeToKeyboardList(w, eventType, keycode, keysym, modifiers, needGrab, isMnemonic);
  }
  else {
                                     ...
                                     Boolean isMnemonic)
{
  ...
  list[i].key = keycode;
  list[i].modifiers = isMnemonic ? (modifiers & ~(ShiftMask | LockMask)) : modifiers;
  list[i].component = w;
...
static void RemoveFromKeyboardList(Widget w)
{
  ...
  for (i = 0; i < count;) {
    if (klist[i].component == w) {
       ...
       */
      if (klist[i].needGrab && (w->core.being_destroyed || notInSharedMenupaneHierarchy)) {
        Boolean another = False;
        int k;
        /* CR 7259,  don't ungrab if another entry requires the
              grab on the same key/modifier */
        for (k = 0; k < count; k++) {
          if (k != i && klist[k].needGrab && klist[k].key == klist[i].key &&
 ...
 */
void _XmRCGetTopManager(Widget w, Widget *topManager)
{
...
static void GrabKeyWithLockMask(Widget widget,
                                KeyCode keycode,
                                Modifiers modifiers,
                                ...
                                int keyboard_mode)
{
  ...
  XtGrabKey(widget, keycode, modifiers | LockMask, owner_events, pointer_mode, keyboard_mode);
  XtGrabKey(
      widget, keycode, modifiers | ScrollLockMask, owner_events, pointer_mode, keyboard_mode);
  XtGrabKey(widget, keycode, modifiers | NumLockMask, owner_events, pointer_mode, keyboard_mode);
  XtGrabKey(widget,
            keycode,
            modifiers | LockMask | ScrollLockMask,
            owner_events,
            pointer_mode,
            keyboard_mode);
  XtGrabKey(widget,
            keycode,
            modifiers | LockMask | NumLockMask,
            owner_events,
            pointer_mode,
            keyboard_mode);
  XtGrabKey(widget,
            keycode,
            modifiers | ScrollLockMask | NumLockMask,
            owner_events,
            pointer_mode,
            keyboard_mode);
  XtGrabKey(widget,
            keycode,
            modifiers | LockMask | ScrollLockMask | NumLockMask,
            owner_events,
            pointer_mode,
            keyboard_mode);
}

static void UngrabKeyWithLockMask(Widget widget, KeyCode keycode, Modifiers modifiers)
{
 ...
 */
static void GrabKeyOnAssocWidgets(XmRowColumnWidget rowcol, KeyCode detail, unsigned int modifiers)
{
  Widget topManager;
  int i;
  if (IsPopup(rowcol)) {
    for (i = 0; i < rowcol->row_column.postFromCount; i++)
      GrabKeyWithLockMask(rowcol->row_column.postFromList[i],
                          detail,
                          modifiers,
                          False,
                          GrabModeAsync,
    ...
    GrabKeyWithLockMask(topManager, detail, modifiers, False, GrabModeAsync, GrabModeAsync);
  }
  else if (IsPulldown(rowcol)) {
    for (i = 0; i < rowcol->row_column.postFromCount; i++)
      GrabKeyOnAssocWidgets(
          (XmRowColumnWidget)XtParent(rowcol->row_column.postFromList[i]), detail, modifiers);
  }
}
...
void _XmRC_AddToPostFromList(XmRowColumnWidget m, Widget widget)
{
...
void _XmRC_RemoveFromPostFromListOnDestroyCB(Widget w,
...
void _XmRC_RemoveFromPostFromList(XmRowColumnWidget m, Widget widget)
{
  int i;
  Boolean found = False;
...
static void SetCascadeField(XmRowColumnWidget m, Widget cascadeBtn, Boolean attach)
{
  ...
  if (attach) {
    ...
    if (m->row_column.postFromCount && (RC_TearOffModel(m) == XmTEAR_OFF_ENABLED))
    ...
    /* CR 5550 fix begin */
    if ((m != NULL) && (RC_CascadeBtn(m) == (Widget)cascadeBtn)) {
...
Boolean _XmMatchBSelectEvent(Widget wid, XEvent *event)
{
  ...
  if (XmIsMenuShell(XtParent(wid))) {
    ...
    if ((IsPopup(rc) && !((XmMenuShellWidget)XtParent(rc))->shell.popped_up) ||
  ...
  if (!event)
    return (False);
  if (!_XmMatchBtnEvent(event,
                        XmIGNORE_EVENTTYPE,
                        /*BSelect.button, BSelect.modifiers*/ Button1,
                        AnyModifier))
    return (False);
  return (True);
}
...
static void BtnDownInRowColumn(Widget rc, XEvent *event, Position x_root, Position y_root)
{
  ...
  else if (!XmIsMenuShell(XtParent(rc))) {
  ...
  {
    (*(((XmMenuShellClassRec *)xmMenuShellWidgetClass)->menu_shell_class.popdownEveryone))(
        RC_PopupPosted(rc), NULL, NULL, NULL);
  }
...
void _XmHandleMenuButtonPress(Widget wid, XEvent *event)
{
  ...
  if ((event->xbutton.x_root >= x_root) && (event->xbutton.x_root < x_root + wid->core.width) &&
      (event->xbutton.y_root >= y_root) && (event->xbutton.y_root < y_root + wid->core.height))
  {
    ...
    _XmHandleMenuButtonPress(((CompositeWidget)RC_PopupPosted(wid))->composite.children[0], event);
  }
  else {
    /* nothing else posted */
 ...
 * Button Action Procs
 */
void _XmMenuBtnDown(Widget wid,
                    XEvent *event,
                    ...
                    Cardinal *num_params) /* unused */
{
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass(wid), XmQTmenuSystem);
  ...
  if (menuSTrait->verifyButton((Widget)rc, event) && (event->xbutton.x_root >= x_root) &&
      (event->xbutton.x_root < x_root + rc->core.width) && (event->xbutton.y_root >= y_root) &&
      (event->xbutton.y_root < y_root + rc->core.height))
  {
    if (!XmIsMenuShell(XtParent(rc)) && RC_Type(rc) != XmMENU_BAR && RC_Type(rc) != XmMENU_OPTION)
      XChangeActivePointerGrab(
          XtDisplay(rc),
          ((unsigned int)(ButtonReleaseMask | PointerMotionMask | PointerMotionHintMask |
    ...
    if ((Widget)rc == topLevel) {
      ...
      if (RC_PopupPosted(rc)) {
        topLevel = ((CompositeWidget)RC_PopupPosted(rc))->composite.children[0];
      }
      else {
        /* nothing else posted */
                  ...
                  Cardinal *num_params) /* unused */
{
  ...
  if (!_XmIsEventUnique(event) || !menuSTrait->verifyButton(wid, event) ||
      (IsBar(w) && !RC_IsArmed(w)))
    return;
  if (w->core.window == event->xbutton.window)
    gadget = (XmGadget)XmObjectAtPoint((Widget)w, event->xbutton.x, event->xbutton.y);
  else
    gadget = NULL;
  ...
  if (nonButton) {
    if (w->manager.active_child && XmIsGadget(w->manager.active_child)) {
      if (((XmGadgetClass)XtClass(w->manager.active_child))->gadget_class.border_unhighlight)
        (*(((XmGadgetClass)XtClass(w->manager.active_child))->gadget_class.border_unhighlight))(
            w->manager.active_child);
    }
 ...
 ******************************************************************/
void _XmMenuGadgetDrag(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
...
void _XmRC_UpdateOptionMenuCBG(Widget cbg, Widget memWidget)
{
  XmString xmstr = NULL;
  Pixmap pix, ipix;
  Arg al[5];
  ...
  if (cbg == (Widget)NULL || memWidget == (Widget)NULL)
    return;
  if (XmIsLabelGadget(memWidget)) {
    ...
    if (LabG_IsText(lg)) {
      ...
      if (LabG_Font(lg) != LabG_Font(cbg)) {
        XtSetArg(al[ac], XmNfontList, LabG_Font(lg));
        ac++;
      }
    }
      ...
      ac++;
      pix = LabG_Pixmap(lg);
      XtSetArg(al[ac], XmNlabelPixmap, pix);
      ac++;
      ipix = LabG_PixmapInsensitive(lg);
      XtSetArg(al[ac], XmNlabelInsensitivePixmap, ipix);
      ac++;
    }
    else {
      XtSetArg(al[ac], XmNlabelType, XmPIXMAP_AND_STRING);
      ac++;
      xmstr = XmStringCopy(LabG__label(lg));
      XtSetArg(al[ac], XmNlabelString, xmstr);
      ac++;
    ...
    if (Lab_IsText(lw)) {
      ...
      if (lw->label.font != LabG_Font(cbg)) {
        XtSetArg(al[ac], XmNfontList, lw->label.font);
        ac++;
      }
    }
...
static Boolean ShouldDispatchFocusOut(Widget widget)
{
   ...
   */
  if (XmIsCascadeButton(widget) && RC_Type(XtParent(widget)) != XmMENU_BAR &&
      !XmIsMenuShell(XtParent(XtParent(widget))) && CB_Submenu(widget) &&
...
void _XmMenuBarGadgetSelect(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  XmRowColumnWidget rc = (XmRowColumnWidget)wid;
  Widget child;
...
void _XmMenuGadgetTraverseCurrent(Widget wid, XEvent *event, String *params, Cardinal *num_params)
{
  Widget child;
  if (!_XmIsEventUnique(event))
    return;
...
void _XmMenuGadgetTraverseCurrentUp(Widget wid,
                                    XEvent *event,
                                    String *params,
                                    Cardinal *num_params)
{
...
static int MenuStatus(Widget wid)
{
  ...
  RC_SetBit(
      menu_status, XmMENU_TEAR_OFF_SHELL_DESCENDANT_BIT, _XmIsTearOffShellDescendant((Widget)rc));
...
static int MenuType(Widget wid)
{
  return (RC_Type(wid));
}
  ...
  switch (m->row_column.type) {
    ...
    case XmMENU_PULLDOWN:
      ...
      if (!XmIsRowColumn(root))
        root = NULL;
      LocatePulldown(root,             /* menu above */
...
static void ButtonMenuPopDown(Widget w, XEvent *event, Boolean *popped_up)
{
   ...
   */
  for (pane = rc, depth = 0; XmIsRowColumn(pane) && ((IsPulldown(pane) || IsPopup(pane)) &&
       ...
       depth++)
  {
    ...
    /* If the parent isn't a rowcolumn widget then stop walking
          up the parents */
    if (pane == (XmRowColumnWidget)NULL || !XmIsRowColumn(pane))
      break;
  }
    ...
    for (i = 0, q = search_m->composite.children; i < search_m->composite.num_children; i++, q++) {
      if (XtIsManaged(*q)) {
        if (XmIsCascadeButtonGadget(*q)) {
          ...
          if (CBG_Submenu(p) && SearchMenu((XmRowColumnWidget)CBG_Submenu(p),
                                           (XmRowColumnWidget *)parent_m,
                                           (RectObj)child,
                                           w,
                                           setHistory))
          ...
          XmCascadeButtonWidget p = (XmCascadeButtonWidget)*q;
          if (CB_Submenu(p) && SearchMenu((XmRowColumnWidget)CB_Submenu(p),
                                          (XmRowColumnWidget *)parent_m,
                                          (RectObj)child,
                                          w,
                                          setHistory))
...
static Boolean VerifyMenuButton(Widget w, XEvent *event)
{
  ...
  if (IsPopup(w)) {
    valid = event &&
            (_XmMatchBtnEvent(event, XmIGNORE_EVENTTYPE, RC_PostButton(w), RC_PostModifiers(w)) ||
             _XmMatchBSelectEvent(w, event));
  }
  else {
    /* CDE modification allows any button to activate a cascade
       button to show a menu */
    valid = event && (event->type == ButtonPress || event->type == ButtonRelease);
  }
  return (valid);
}
                                 ...
                                 Boolean updateOnMemWidgetMatch)
{
  ...
  if (IsOption(menu)) {
    ...
    for (i = 0; i < menu->row_column.postFromCount; i++) {
      ...
      if (UpdateMenuHistory((XmRowColumnWidget)parent_menu, child, updateOnMemWidgetMatch)) {
 ...
 */
void _XmRC_SetMenuHistory(XmRowColumnWidget m, RectObj child)
{
  XmRowColumnWidget parent_m;
  Widget w;
  if (IsNull(child))
 ...
 */
void _XmRC_SetOptionMenuHistory(XmRowColumnWidget m, RectObj child)
{
...
static void ChildsActivateCallback(XmRowColumnWidget rowcol, Widget child, XtPointer call_value)
{
  ...
  menuSavvyRec = (XmMenuSavvyTrait)XmeTraitGet((XtPointer)XtClass(child), XmQTmenuSavvy);
  if (menuSavvyRec != NULL && menuSavvyRec->getActivateCBName != NULL)
  ...
  if (rowcol->row_column.entry_callback != NULL) {
     ...
     */
    if ((callbacks == NULL) || (callbacks[0].callback == (XtCallbackProc)NULL)) /* CR 5256 */
...
static void MenuArm(Widget w)
{
  ...
  if (!RC_IsArmed(m)) {
    ...
    if (IsBar(m)) {
      ...
      if ((RC_OldFocusPolicy(m) = _XmGetFocusPolicy((Widget)m)) != XmEXPLICIT) {
        ...
        if (mst->RC_activeItem) {
          ...
          xcrossing.serial = LastKnownRequestProcessed(XtDisplay(mst->RC_activeItem));
          ...
          xcrossing.time = XtLastTimestampProcessed(XtDisplay(mst->RC_activeItem));
          xcrossing.mode = 1;
...
static void MenuDisarm(Widget w)
{
  ...
  if (RC_IsArmed(m)) {
     ...
     */
    if (IsBar(m) || IsPopup(m) || IsOption(m) || (IsPulldown(m) && !(XmIsMenuShell(XtParent(m)))))
    {
      XmDisplay disp = (XmDisplay)XmGetXmDisplay(XtDisplay(w));
    ...
    if (IsBar(m)) {
      ...
      if (RC_OldFocusPolicy(m) == XmEXPLICIT) {
         ...
         */
        if (mst->RC_activeItem && !mst->RC_activeItem->core.being_destroyed) {
...
static void TearOffArm(Widget w)
{
  ...
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass((Widget)m), XmQTmenuSystem);
   ...
   */
  if ((IsPulldown(m) || IsPopup(m)) && !XmIsMenuShell(XtParent(m)) && !RC_IsArmed(m)) {
    /* Don't post the menu if the menu cannot control grabs! */
    if (_XmMenuGrabKeyboardAndPointer((Widget)m, _time) != GrabSuccess) {
      return;
    }
...
static void InvalidateOldFocus(Widget oldWidget, /* unused */
                               Window **poldFocus,
                               XEvent *event) /* unused */
{
  *poldFocus = NULL;
...
Widget XmGetPostedFromWidget(Widget menu)
{
  ...
  if (menu && XmIsRowColumn(menu)) {
    toplevel = (XmRowColumnWidget)((XmRowColumnWidget)menu)->row_column.lastSelectToplevel;
