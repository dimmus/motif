...
static int GetOverrideColors(
   ...
   XmAccessColorData acc_color,
   XpmColorSymbol   *override_colors);
static XtEnum GetXpmImage(Screen *screen, char *image_name, char *file_name, XmAccessColorData acc_color, XImage **image, unsigned short *pixmap_resolution, Pixel **pixels, int *npixels);
static XtEnum GetImage(Screen *screen, char *image_name, XmAccessColorData acc_color, XImage **image, unsigned short *pixmap_resolution, Pixel **pixels, /* allocated pixels */
                       int *npixels);
static GC     GetGCForPutImage(Screen *screen,
                               ...
                               Pixel   foreground,
                               Pixel   background);
static int    GetCacheColor(Display *display, Colormap colormap, char *colorname, XColor *color, void *data);
static int    FreeCacheColors(Display *display, Colormap colormap, Pixel *pixels, int npixels, void *data);
   ...
   XpmColorSymbol *override_colors)
{
   ...
   if (i == XmCO_NUM_COLORS)
   {
	 ...
	 */
       ...
       */
      if (!result && _XmSearchColorCache((XmLOOK_AT_SCREEN | XmLOOK_AT_CMAP | XmLOOK_AT_BACKGROUND), &new_colors, &old_colors))
      {
         XmGetColors(screen,
   ...
   int              *npixels)
{
   ...
   if (acc_color && (num_override_colors = GetOverrideColors(screen, acc_color, override_colors)) != 0)
   {
 ...
 */
static XtEnum
LoadImage(Screen *screen, char *image_name, XmAccessColorData acc_color, XImage **image, unsigned short *pixmap_res, Pixel **pixels, int *npixels)
{
   FILE         *fp;
   char         *fname;
   XColor        bg;
   ...
   fname = XmGetIconFileName(screen, NULL, image_name, NULL, XmUNSPECIFIED_ICON_SIZE);
   if (!fname)
      return False;
   ...
   switch (header[0])
   {
      ...
      case 0x89: /* 0x89 'P' 'N' 'G' - PNG */
         ...
         break;
      default: /* Try XPM */
         ret = GetXpmImage(screen, image_name, fname, acc_color, image, pixmap_res, pixels, npixels);
   }
   fclose(fp);
   XtFree(fname);
   return ret;
   ...
   int              *npixels)
{
   ...
   return LoadImage(screen, image_name, acc_color, image, pixmap_resolution, pixels, npixels);
}
               ...
               XmHashKey key_2)
{
   ...
   return (data_1->screen == data_2->screen && data_1->pixmap == data_2->pixmap);
}
   ...
   Dimension height)
{
   ...
   if (!(width && height && depth))
      XGetGeometry(DisplayOfScreen(screen), pixmap, &root, &x, &y, &loc_width, &loc_height, &border_width, &loc_depth);
   else
   {
      loc_width  = width;
      loc_height = height;
...
/* no pixmap_resolution returned... */
{
   ...
   if ((pix_entry = (PixmapData *)
           _XmGetHashEntry(pixmap_set, (XmHashKey)&pix_data))
       != NULL)
   {
   ...
   int               desired_h)
{
   ...
   if (!scaling_ratio && pix_data.print_shell)
      pix_data.print_resolution = ((XmPrintShellWidget)pix_data.print_shell)->print.print_resolution;
#endif
   ...
   _XmProcessLock();
   if ((pix_entry = (PixmapData *)
           _XmGetHashEntry(pixmap_data_set, (XmHashKey)&pix_data))
       != NULL)
   {
   ...
   if (!(ret = GetImage(screen, image_name, acc_color, &image, &pixmap_resolution, &pixels, &npixels)))
      return (XmUNSPECIFIED_PIXMAP);
   ...
   if (!pix_data.scaling_ratio)
   {
      ...
      if (!pixmap_resolution)
      {
         ...
         if (pix_data.print_shell)
            pixmap_resolution = ((XmPrintShellWidget)pix_data.print_shell)
                                   ->print.default_pixmap_resolution;
         else
#endif /* XM_PRINTING */
            pixmap_resolution = 100;
      }
      pix_data.scaling_ratio = (double)pix_data.print_resolution / (double)pixmap_resolution;
   }
   ...
   pixmap                     = XCreatePixmap(display, RootWindowOfScreen(screen), pix_entry->width, pix_entry->height, depth);
   pix_entry->pixmap          = pixmap;
   pix_entry->reference_count = 1;
   ...
   gc = GetGCForPutImage(screen, pix_entry->print_shell, image, pixmap, depth, acc_color->foreground, acc_color->background);
   /* transfer and scale the image */
   _XmPutScaledImage(screen, display, pixmap, depth, gc, image, 0, 0, image->width, image->height, 0, 0, pix_entry->width, pix_entry->height);
...
_XmGetColoredPixmap(Screen           *screen,
                    ...
                    Boolean           only_if_exists)
{
   return _XmGetScaledPixmap(screen, NULL, image_name, acc_color, depth, only_if_exists, 1, 0, 0); /* no scaling */
}
 ...
 */
Pixmap
XmGetSizedPixmap(Widget widget, char *image_name, Pixel foreground, Pixel background, int depth, int width, int height)
{
   Pixmap               ret;
   ...
   ret                               = _XmGetScaledPixmap(XtScreen(widget), widget, image_name, &acc_color_rec, depth, False, 1, width, height);
   _XmProcessUnlock();
   _XmAppUnlock(app);
   return ret;
}
   ...
   double scaling_ratio)
{
   ...
   acc_color_rec.select_color        = XmUNSPECIFIED_PIXEL;
   acc_color_rec.highlight_color     = XmUNSPECIFIED_PIXEL;
   ret_val                           = _XmGetScaledPixmap(XtScreen(widget), widget, image_name, &acc_color_rec, depth, False, scaling_ratio, 0, 0);
   _XmProcessUnlock();
   _XmAppUnlock(app);
   return ret_val;
}
   ...
   int     depth)
{
   ...
   acc_color_rec.select_color        = XmUNSPECIFIED_PIXEL;
   acc_color_rec.highlight_color     = XmUNSPECIFIED_PIXEL;
   ret_val                           = _XmGetScaledPixmap(screen, NULL, image_name, &acc_color_rec, depth, False, 1, 0, 0);
   _XmProcessUnlock();
   _XmAppUnlock(app);
   return ret_val;
}
   ...
   Pixel   background)
{
   ...
   (void)app; /* may be unused in non-threaded builds */
   _XmAppLock(app);
   ret_val = XmGetPixmapByDepth(screen, image_name, foreground, background, DefaultDepthOfScreen(screen));
   _XmAppUnlock(app);
   return (ret_val);
}
               ...
               XmHashKey key_2)
{
   ...
   return ((data_1->screen == data_2->screen) && (data_1->print_shell == data_2->print_shell) && (data_1->depth == data_2->depth) && ((data_1->image_depth != 1) || ((data_1->image_depth == 1) && (data_1->foreground == data_2->foreground) && (data_1->background == data_2->background))));
}
...
HashGCData(XmHashKey key)
{
   ...
   return (hv + (long)data->screen + (long)data->print_shell + data->depth + data->image_depth);
}
                 ...
                 Pixel   background)
{
   ...
   _XmProcessLock();
   if ((gc_entry = (GCData *)
           _XmGetHashEntry(gc_set, (XmHashKey)&gc_data))
       != NULL)
   {
      _XmProcessUnlock();
      return gc_entry->gc;
   ...
   gc_entry->gc        = XCreateGC(DisplayOfScreen(screen), pixmap, GCForeground | GCBackground, &gcValues);
   return gc_entry->gc;
}
 ...
 */
static void
render_image(Screen *screen, Display *display, Drawable d, int depth, GC gc, XImage *src, int sx, int sy, int sw, int sh, int dx, int dy, int dw, int dh)
{
   char     *data;
   Visual   *vis;
   XImage   *dest_image;
      ...
      for (x = 0; x < dw; x++)
      {
         ...
         switch (src->depth)
         {
            ...
            default:
               ...
               {
                  if (src->depth > 16 && depth == 16 && ones(vis->red_mask) == 5)
                  { /* 5:5:5 */
                     r >>= 3;
                     g >>= 3;
                     b >>= 3;
                  ...
                  pixel = (((r << ctz(vis->red_mask)) & vis->red_mask) | ((g << ctz(vis->green_mask)) & vis->green_mask) | ((b << ctz(vis->blue_mask)) & vis->blue_mask));
               }
         }
...
void
_XmPutScaledImage(Screen *screen, Display *display, Drawable d, int depth, GC gc, XImage *src, int sx, int sy, int sw, int sh, int dx, int dy, int dw, int dh)
{
   int     free_src = 0;
   ...
   if (src->depth == depth && dw == sw && dh == sh && src->red_mask == vis->red_mask && src->green_mask == vis->green_mask && src->blue_mask == vis->blue_mask)
   {
...
GetCacheColorByName(Display *display, Colormap colormap, char *colorname, XColor *xcolor)
{
   ...
   for (i = 0, color = colorCacheList.cache; i < colorCacheList.numEntries;
        i++, color++)
   {
      if (color->colorname == colorname_q && color->colormap == colormap && color->display == display)
      {
...
GetCacheColorByRGB(Display *display, Colormap colormap, XColor *xcolor)
{
   ...
   for (i = 0, color = colorCacheList.cache; i < colorCacheList.numEntries;
        i++, color++)
   {
      if (color->colormap == colormap && color->display == display && color->red == xcolor->red && color->green == xcolor->green && color->blue == xcolor->blue)
      {
         xcolor->pixel = color->pixel;
         color->num_cached++;
         return True;
...
FreeCacheColor(Display *display, Colormap colormap, Pixel pixel)
{
   ...
   for (i = 0, color = colorCacheList.cache; i < colorCacheList.numEntries;
        i++, color++)
   {
      if (color->colormap == colormap && color->display == display && color->pixel == pixel)
      {
         color->num_cached--;
   ...
   if (numEntries == colorCacheList.maxEntries)
   {
      ...
      colorCacheList.cache       = (CachedColor *)
         XtRealloc((char *)colorCacheList.cache,
                   colorCacheList.maxEntries * sizeof(CachedColor));
   }
   colorCacheList.cache[numEntries].display    = display;
   colorCacheList.cache[numEntries].colormap   = colormap;
   colorCacheList.cache[numEntries].colorname  = colorname ? XrmStringToQuark(colorname) : NULLQUARK;
...
static int
GetCacheColor(Display *display, Colormap colormap, char *colorname, XColor *xcolor, void *closure)
{
   int status;
   if (colorname)
   {
