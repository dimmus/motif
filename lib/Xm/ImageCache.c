...
static GC GetGCForPutImage(Screen *screen,
                           ...
                           Pixel foreground,
                           Pixel background);
static int GetCacheColor(
    Display *display, Colormap colormap, char *colorname, XColor *color, void *data);
static int FreeCacheColors(
    Display *display, Colormap colormap, Pixel *pixels, int npixels, void *data);
...
static Boolean UninstallImageMapProc(XmHashKey key, /* unused */
                                     XtPointer value,
                                     XtPointer image)
{
...
static void CompleteUnspecColors(Screen *screen, XpmColorSymbol *override_colors)
{
  ...
  /* Ask the color obj */
  if (!(result = XmeGetColorObjData(
            screen, &colorUse, pixelSets, XmCO_NUM_COLORS, NULL, NULL, NULL, NULL, NULL)))
    i = XmCO_NUM_COLORS;
  else
    i = 0;
                          ...
                          int *npixels)
{
  ...
  attrib.valuemask = XpmCloseness | XpmBitmapFormat | XpmReturnColorTable | XpmAllocColor |
                     XpmFreeColors | XpmReturnAllocPixels;
  ...
  (void)XmeGetIconControlInfo(screen, &useMask, &useColor, &useIconFileCache); /* unused */
  if (!useColor) {
    attrib.depth = 1;
    attrib.valuemask |= XpmDepth;
  }
  *image = NULL;
  xpmStatus = XmeXpmReadFileToImage(display, file_name, image, &mask_image, &attrib);
  if (xpmStatus < 0)
    *image = NULL;
  else {
...
static Boolean ComparePixmapDatas(XmHashKey key_1, XmHashKey key_2)
{
  ...
  return ((data_2->pixmap == XmUNSPECIFIED_PIXMAP || data_1->pixmap == data_2->pixmap) &&
          data_1->image_name != NULL && data_2->image_name != NULL &&
 ...
 *
 ************************************************************************/
Boolean _XmCachePixmap(Pixmap pixmap,
                       Screen *screen,
                       char *image_name,
                       Pixel foreground,
                       Pixel background,
                          ...
                          int desired_h)
{
  ...
  if (!(ret = GetImage(
            screen, image_name, acc_color, &image, &pixmap_resolution, &pixels, &npixels)))
    return (XmUNSPECIFIED_PIXMAP);
  ...
  if (!pix_data.scaling_ratio) {
    ...
    if (!pixmap_resolution) {
      ...
      if (pix_data.print_shell)
        pixmap_resolution =
            ((XmPrintShellWidget)pix_data.print_shell)->print.default_pixmap_resolution;
      else
#endif /* XM_PRINTING */
  ...
  pixmap = XCreatePixmap(
      display, RootWindowOfScreen(screen), pix_entry->width, pix_entry->height, depth);
  pix_entry->pixmap = pixmap;
  pix_entry->reference_count = 1;
 ...
 *
 *******************************************************************/
Pixmap _XmGetColoredPixmap(Screen *screen,
                           char *image_name,
                           XmAccessColorData acc_color,
                           int depth,
                           Boolean only_if_exists)
{
  return _XmGetScaledPixmap(
      screen, NULL, image_name, acc_color, depth, only_if_exists, 1, 0, 0); /* no scaling */
}
                        ...
                        int height)
{
  ...
  ret = _XmGetScaledPixmap(
      XtScreen(widget), widget, image_name, &acc_color_rec, depth, False, 1, width, height);
  _XmProcessUnlock();
  _XmAppUnlock(app);
  return ret;
}
                         ...
                         double scaling_ratio)
{
  ...
  acc_color_rec.select_color = XmUNSPECIFIED_PIXEL;
  acc_color_rec.highlight_color = XmUNSPECIFIED_PIXEL;
  ret_val = _XmGetScaledPixmap(
      XtScreen(widget), widget, image_name, &acc_color_rec, depth, False, scaling_ratio, 0, 0);
  _XmProcessUnlock();
  _XmAppUnlock(app);
  return ret_val;
}
...
Pixmap XmGetPixmap(Screen *screen, char *image_name, Pixel foreground, Pixel background)
{
  ...
  (void)app; /* may be unused in non-threaded builds */
  _XmAppLock(app);
  ret_val = XmGetPixmapByDepth(
      screen, image_name, foreground, background, DefaultDepthOfScreen(screen));
  _XmAppUnlock(app);
  return (ret_val);
}
...
static Boolean CompareGCDatas(XmHashKey key_1, XmHashKey key_2)
{
  ...
  return ((data_1->screen == data_2->screen) && (data_1->print_shell == data_2->print_shell) &&
          (data_1->depth == data_2->depth) &&
          ((data_1->image_depth != 1) ||
           ((data_1->image_depth == 1) && (data_1->foreground == data_2->foreground) &&
...
/************
 ...
 respective cache .
*****/
static Boolean CleanGCMapProc(XmHashKey key, /* unused */
                              XtPointer value,
                              XtPointer data)
{
...
static Boolean CleanPixmapMapProc(XmHashKey key, /* unused */
                                  XtPointer value,
                                  XtPointer data)
{
...
#ifdef XP_DEBUG
static Boolean PrintPixmapMapProc(XmHashKey key, /* unused */
                                  XtPointer value,
                                  XtPointer data)
{
  PixmapData *entry = (PixmapData *)value;
...
static Boolean PrintGCMapProc(XmHashKey key, /* unused */
                              XtPointer value,
                              XtPointer data)
{
  GCData *entry = (GCData *)value;
...
*******/
void _XmCleanPixmapCache(Screen *screen, Widget shell)
{
  CleanKey ck;
  ck.screen = screen;
  ck.shell = shell;
...
#else
  static const unsigned int mod37[] = {-1, 0, 1,  26, 2,  23, 27, 0,  3,  16, 24, 30, 28,
 ...
 */
static void render_image(Screen *screen,
                         Display *display,
                         Drawable d,
                         int depth,
                         GC gc,
...
void _XmPutScaledImage(Screen *screen,
                       Display *display,
                       Drawable d,
                       int depth,
                       GC gc,
...
/*** COLOR CACHE NOW ***/
static Boolean GetCacheColorByName(Display *display,
                                   Colormap colormap,
                                   char *colorname,
                                   XColor *xcolor)
{
  ...
  for (i = 0, color = colorCacheList.cache; i < colorCacheList.numEntries; i++, color++) {
    if (color->colorname == colorname_q && color->colormap == colormap &&
...
static Boolean GetCacheColorByRGB(Display *display, Colormap colormap, XColor *xcolor)
{
  int i;
  CachedColor *color;
  for (i = 0, color = colorCacheList.cache; i < colorCacheList.numEntries; i++, color++) {
    if (color->colormap == colormap && color->display == display && color->red == xcolor->red &&
        color->green == xcolor->green && color->blue == xcolor->blue)
    {
      xcolor->pixel = color->pixel;
      color->num_cached++;
      return True;
...
static Boolean FreeCacheColor(Display *display, Colormap colormap, Pixel pixel)
{
  int i;
  CachedColor *color;
  for (i = 0, color = colorCacheList.cache; i < colorCacheList.numEntries; i++, color++) {
...
static void CacheColorPixel(Display *display, Colormap colormap, char *colorname, XColor *xcolor)
{
...
static int GetCacheColor(
    Display *display, Colormap colormap, char *colorname, XColor *xcolor, void *closure)
{
  int status;
  if (colorname) {
...
static int FreeCacheColors(
    Display *display, Colormap colormap, Pixel *pixels, int n, void *closure)
{
  int i;
