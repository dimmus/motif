                    ...
                    XrmValue *value)
{
   ...
   /* This is an ugly hack, but what to do when user sets stringDirection
     ...
     the same field. Yuck! */
   if (XmIsManager(widget) && (((XmManagerWidget)widget)->manager.string_direction != XmSTRING_DIRECTION_DEFAULT))
   ...
   XtArgVal *value)
{
   ...
   {
      XmPrim_layout_direction(((XmPrimitiveWidget)wid)) = XmStringDirectionToDirection((XmStringDirection)*value);
      return XmSYNTHETIC_NONE;
   }
   else if (XmIsLabelGadget(wid))
   {
      ((XmGadget)wid)->gadget.layout_direction = XmStringDirectionToDirection((XmStringDirection)*value);
      return XmSYNTHETIC_NONE;
   }
   return XmSYNTHETIC_LOAD;
}
...
_XmGetLayoutDirection(Widget w)
{
   ...
   while (w && !(layoutT = (XmSpecifyLayoutDirectionTrait)XmeTraitGet((XtPointer)XtClass(w), XmQTspecifyLayoutDirection)))
      w = XtParent(w);
                 ...
                 XmDirection d2)
{
   d1 = Fixdir(d1);
   d2 = Fixdir(d2);
   return (((XmDirection)(d1 & d2) == (XmDirection)d1) || ((XmDirection)(d1 & d2) == (XmDirection)d2));
}
