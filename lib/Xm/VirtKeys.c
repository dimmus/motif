      ...
      for (tmp = 0; tmp < count; tmp++)
      {
	     ...
	     */
         ...
         if (event.keycode > 0)
         {
		   ...
		   */
                        if ((keysyms[tmp] == XK_KP_Enter) && (j == 4) && (keysymTab[0] == XK_Return) && (strcmp("Sun Microsystems, Inc.", ServerVendor(dpy)) == 0))
                        {
                           fini = True;
                        }
                        else
         ...
         if (!fini)
         {
            ...
            XtTranslateKey(dpy, event.keycode, event.state, &used_mods, keysyms + tmp);
         }
      }
      /* Fail if insufficient storage was provided. */
      if ((toVal->addr != NULL) && (toVal->size < sizeof(XmKeyBindingRec) * count))
      {
   ...
   for (vk_num = 0; vk_num < XtNumber(virtualKeysyms); vk_num++)
   {
      ...
      if (XrmQGetResource(rdb, xrm_name, xrm_class, &rep_type, &value))
      {
         ...
         else if (rep_type == XmQString)
         {
            ...
            toVal.addr = NULL;
            toVal.size = 0;
            if (XtCallConverter(dpy, CvtStringToVirtualBinding, NULL, 0, &value, &toVal, NULL))
            {
...
_XmVirtKeysInitialize(Widget widget)
{
   ...
   XtSetTypeConverter(XmRString, XmRVirtualBinding, CvtStringToVirtualBinding, NULL, 0, XtCacheNone, (XtDestructor)NULL);
   keyDB = XrmGetStringDatabase(bindingsString);
   FillBindingsFromDB(XtDisplay(xmDisplay), keyDB, &xmDisplay->display.bindings, &xmDisplay->display.num_bindings);
            ...
            KeySym    *keysym_return)
{
   ...
   */
   ...
   for (i = 0; i < xmDisplay->display.num_bindings; i++)
   {
       ...
       */
      if (vks && (vks == *keysym_return) && ((currBinding->modifiers & StdModMask) == (modifiers & VirtualStdMods & StdModMask)))
      {
                    ...
                    int      ks_per_kc)
{
   ...
   */
   Modifiers esm_mask = (Mod5Mask | Mod4Mask | Mod3Mask | Mod2Mask | Mod1Mask | ControlMask | LockMask | ShiftMask);
   switch (ks_per_kc)
   {
                                ...
                                String  *binding)
{
   ...
   *binding = NULL;
   ...
   if (*binding != NULL)
   {
      ...
      XChangeProperty(display, RootWindow(display, 0), atoms[XmA_MOTIF_BINDINGS], XA_STRING, 8, PropModeReplace, (unsigned char *)*binding, strlen(*binding));
      return 0;
   }
   ...
   XChangeProperty(display, RootWindow(display, 0), atoms[XmA_MOTIF_DEFAULT_BINDINGS], XA_STRING, 8, PropModeReplace, (unsigned char *)*binding, strlen(*binding));
   return 0;
}
