...
void _XmVirtKeysInitialize(Widget widget)
{
  ...
  if (bindingsString == NULL) {
    ...
    if (GetBindingsProperty(XtDisplay(xmDisplay), XmS_MOTIF_BINDINGS, &bindingsString) == True) {
      needXFree = True;
    }
    else if (GetBindingsProperty(
                 XtDisplay(xmDisplay), XmS_MOTIF_DEFAULT_BINDINGS, &bindingsString) == True)
    {
      needXFree = True;
    }
    else {
  ...
  FillBindingsFromDB(
      XtDisplay(xmDisplay), keyDB, &xmDisplay->display.bindings, &xmDisplay->display.num_bindings);
 ...
 */
void _XmVirtKeysDestroy(Widget widget)
{
  XmDisplay xmDisplay = (XmDisplay)widget;
  ...
  *modifiers_return |= VirtualStdMods;
  ...
  for (i = 0; i < xmDisplay->display.num_bindings; i++) {
    ...
    if (vks && (vks == *keysym_return) &&
        ((currBinding->modifiers & StdModMask) == (modifiers & VirtualStdMods & StdModMask)))
    {
...
static Modifiers EffectiveStdModMask(Display *dpy, KeySym *kc_map, int ks_per_kc)
{
   ...
   */
  Modifiers esm_mask = (Mod5Mask | Mod4Mask | Mod3Mask | Mod2Mask | Mod1Mask | ControlMask |
                        LockMask | ShiftMask);
  switch (ks_per_kc) {
...
void XmTranslateKey(Display *dpy,
                    KeyCode keycode,
                    Modifiers modifiers,
                    Modifiers *modifiers_return,
                    KeySym *keysym_return)
...
int XmeVirtualToActualKeysyms(Display *dpy, KeySym virtKeysym, XmKeyBinding *actualKeyData)
{
  int matches;
  Cardinal index;
  XmDisplay xmDisplay = (XmDisplay)XmGetXmDisplay(dpy);
