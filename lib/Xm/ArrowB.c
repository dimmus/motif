...
static void GetArrowGC(XmArrowButtonWidget aw);
...
static void MultiArm(Widget aw, XEvent *event, String *params, Cardinal *num_params);
...
static void ActivateCommon(Widget wid, XEvent *buttonEvent);
...
static void DrawArrow(XmArrowButtonWidget aw, GC top_gc, GC bottom_gc, GC center_gc);
/********    End Static Function Declarations    ********/
/*  Default translation table and action list  */
#define defaultTranslations _XmArrowB_defaultTranslations
static XtActionsRec actionsList[] = {{"Activate", Activate},
...
externaldef(xmarrowbuttonwidgetclass)
...
static void GetArrowGC(XmArrowButtonWidget aw)
{
  ...
  aw->arrowbutton.arrow_GC = XtAllocateGC(
      (Widget)aw, 0, valueMask, &values, GCClipMask, unusedMask);
  valueMask |= GCFillStyle | GCStipple;
  values.fill_style = FillOpaqueStippled;
  values.stipple = _XmGetInsensitiveStippleBitmap((Widget)aw);
  aw->arrowbutton.insensitive_GC = XtAllocateGC(
      (Widget)aw, 0, valueMask, &values, GCClipMask, unusedMask);
}
...
static void ActivateCommon(Widget wid, XEvent *buttonEvent)
{
  ...
  if (((buttonEvent->xany.type == ButtonPress) || (buttonEvent->xany.type == ButtonRelease)) &&
      _XmGetPointVisibility(wid, buttonEvent->xbutton.x_root, buttonEvent->xbutton.y_root) &&
...
Widget XmVaCreateArrowButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmArrowButtonWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedArrowButton(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmArrowButtonWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
...
static void DrawArrow(XmArrowButtonWidget aw, GC top_gc, GC bottom_gc, GC center_gc)
{
  ...
  XmeDrawArrow(XtDisplay((Widget)aw),
               XtWindow((Widget)aw),
               top_gc,
               bottom_gc,
               center_gc,
