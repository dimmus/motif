...
static Boolean Redraw(Widget kid, Widget cur_parent, Widget new_parent, Mask visual_flag);
...
static XtResource resources[] = {
    {XmNunitType,
     ...
     XtOffsetOf(XmPrimitiveRec, primitive.unit_type),
     XmRCallProc,
    ...
    {XmNforeground,
     ...
     XmRPixel,
     sizeof(Pixel),
     XtOffsetOf(XmPrimitiveRec, primitive.foreground),
     XmRCallProc,
    ...
    {XmNhighlightPixmap,
     ...
     sizeof(Pixmap),
     XtOffsetOf(XmPrimitiveRec, primitive.highlight_pixmap),
     XmRCallProc,
...
static XmSyntheticResource syn_resources[] = {
    ...
    {XmNshadowThickness,
     ...
     XmeFromHorizontalPixels,
     XmeToHorizontalPixels},
    {XmNtoolTipString, 0, 0, GetToolTipString, SetToolTipString}};
/*******************************************/
...
externaldef(xmprimitivewidgetclass)
    WidgetClass xmPrimitiveWidgetClass = (WidgetClass)&xmPrimitiveClassRec;
static XmConst XmSpecifyLayoutDirectionTraitRec primLDT = {0, /* version */
                                                           GetDirection};
...
static XmConst XmAccessColorsTraitRec primACT = {0, /* version */
                                                 GetColors};
/* Unit Type Trait record for Primitive */
static XmConst XmSpecUnitTypeTraitRec primUTT = {0, /* version */
                                                 GetUnitType};
...
static void GetXFromShell(Widget wid, int resource_offset, XtArgVal *value)
{
  ...
  if (XtIsShell(parent)) {
    XtTranslateCoords((Widget)wid, (Position)0, (Position)0, &rootx, &rooty);
...
static void BuildPrimitiveResources(WidgetClass c)
{
  ...
  XmPrimitiveWidgetClass sc = (XmPrimitiveWidgetClass)wc->core_class.superclass;
...
static void ClassPartInitialize(WidgetClass w)
{
  ...
  if (wc->primitive_class.translations == XtInheritTranslations)
    ...
    wc->primitive_class.translations = (String)XtParseTranslationTable(
  ...
  if (*wcePtr == NULL) {
    *wcePtr = (XmPrimitiveClassExt)XtCalloc(1, sizeof(XmPrimitiveClassExtRec));
  ...
  if (first_time) {
    ...
    _XmReOrderResourceList(xmPrimitiveWidgetClass, XmNforeground, XmNbackground);
    first_time = FALSE;
  }
...
static void Initialize(Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if ((pw->primitive.navigation_type != XmDYNAMIC_DEFAULT_TAB_GROUP) &&
      !XmRepTypeValidValue(XmRID_NAVIGATION_TYPE, pw->primitive.navigation_type, (Widget)pw))
  {
    pw->primitive.navigation_type = XmNONE;
  }
  _XmNavigInitialize((Widget)request, (Widget)pw, args, num_args);
  if (!XmRepTypeValidValue(XmRID_UNIT_TYPE, pw->primitive.unit_type, (Widget)pw)) {
...
static void Redisplay(Widget wid, XEvent *event, Region region)
{
  ...
  if (pw->primitive.highlighted) {
    (*(((XmPrimitiveWidgetClass)XtClass(wid))->primitive_class.border_highlight))(wid);
  }
  else {
    (*(((XmPrimitiveWidgetClass)XtClass(wid))->primitive_class.border_unhighlight))(wid);
  }
}
    ...
    Widget current, Widget request, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (newpw->primitive.traversal_on &&
      ...
      ((XmPrimitiveClassRec *)XtClass(newpw))->primitive_class.translations && !XmIsLabel(newpw))
  {
    _XmProcessLock();
    XtOverrideTranslations(
        (Widget)newpw,
        (XtTranslations)((XmPrimitiveClassRec *)XtClass(newpw))->primitive_class.translations);
    _XmProcessUnlock();
  }
  if (curpw->primitive.navigation_type != newpw->primitive.navigation_type) {
    if (!XmRepTypeValidValue(
            XmRID_NAVIGATION_TYPE, newpw->primitive.navigation_type, (Widget)newpw))
    {
      newpw->primitive.navigation_type = curpw->primitive.navigation_type;
    }
  }
  returnFlag = _XmNavigSetValues(current, request, new_w, args, num_args);
  /*  Validate changed data.  */
  if (!XmRepTypeValidValue(XmRID_UNIT_TYPE, newpw->primitive.unit_type, (Widget)newpw)) {
  ...
  if (curpw->primitive.shadow_thickness != newpw->primitive.shadow_thickness ||
      curpw->primitive.highlight_thickness != newpw->primitive.highlight_thickness)
    returnFlag = True;
  ...
  if (newpw->primitive.highlight_drawn &&
      (!XtIsSensitive((Widget)newpw) ||
       ((curpw->primitive.highlight_on_enter) && !(newpw->primitive.highlight_on_enter) &&
        (_XmGetFocusPolicy((Widget)newpw) == XmPOINTER))))
  {
    if (((XmPrimitiveWidgetClass)XtClass(newpw))->primitive_class.border_unhighlight) {
      (*(((XmPrimitiveWidgetClass)XtClass(newpw))->primitive_class.border_unhighlight))(
          (Widget)newpw);
    }
  }
...
static void UnhighlightBorder(Widget w)
{
  ...
  pw->primitive.highlighted = False;
  pw->primitive.highlight_drawn = False;
  if (XtWidth(w) == 0 || XtHeight(w) == 0 || pw->primitive.highlight_thickness == 0) {
    return;
  }
  if (XmIsManager(pw->core.parent)) {
    /* If unhighlight trait in parent use specified GC */
    if (((UnhighlightT = (XmSpecifyUnhighlightTrait)XmeTraitGet(
              (XtPointer)XtClass(pw->core.parent), XmQTspecifyUnhighlight)) != NULL) &&
    ...
    /* otherwise, use parent's background GC */
      ...
      manager_background_GC = ((XmManagerWidget)(pw->core.parent))->manager.background_GC;
    }
    XmeDrawHighlight(XtDisplay(pw),
                     XtWindow(pw),
...
static XmNavigability WidgetNavigable(Widget wid)
{
  if (XtIsSensitive(wid) && ((XmPrimitiveWidget)wid)->primitive.traversal_on) {
    XmNavigationType nav_type = ((XmPrimitiveWidget)wid)->primitive.navigation_type;
...
static void FocusChange(Widget wid, XmFocusChange change)
{
  ...
  switch (change) {
    ...
    case XmFOCUS_IN:
      ...
      if (((XmPrimitiveWidgetClass)XtClass(wid))->primitive_class.border_highlight) {
        (*(((XmPrimitiveWidgetClass)XtClass(wid))->primitive_class.border_highlight))(wid);
      }
      break;
    case XmLEAVE:
    ...
    case XmFOCUS_OUT:
      ...
      if (((XmPrimitiveWidgetClass)XtClass(wid))->primitive_class.border_unhighlight) {
        (*(((XmPrimitiveWidgetClass)XtClass(wid))->primitive_class.border_unhighlight))(wid);
      }
      break;
  }
...
/*ARGSUSED*/
void _XmTraverseRight(Widget w,
                      XEvent *event,        /* unused */
                      String *params,       /* unused */
                      Cardinal *num_params) /* unused */
{
...
/*ARGSUSED*/
void _XmTraverseNextTabGroup(Widget w,
                             XEvent *event,        /* unused */
                             String *params,       /* unused */
                             Cardinal *num_params) /* unused */
{
...
/*ARGSUSED*/
void _XmTraversePrevTabGroup(Widget w,
                             XEvent *event,        /* unused */
                             String *params,       /* unused */
                             Cardinal *num_params) /* unused */
{
...
/*ARGSUSED*/
void _XmPrimitiveHelp(Widget wid,
                      XEvent *event,
                      String *params,       /* unused */
                      Cardinal *num_params) /* unused */
{
...
void _XmPrimitiveParentActivate(Widget pw, XEvent *event, String *params, Cardinal *num_params)
{
...
void _XmPrimitiveParentCancel(Widget pw, XEvent *event, String *params, Cardinal *num_params)
{
  XmParentInputActionRec pp_data;
  pp_data.process_type = XmINPUT_ACTION;
 ...
 *********************************************************************/
/*ARGSUSED*/
void _XmButtonTakeFocus(Widget wid,
                        XEvent *event,        /* unused */
                        String *params,       /* unused */
                        Cardinal *num_params) /* unused */
{
...
XmImportOperator SetToolTipString(Widget wid,
                                  int resource, /* unused */
                                  XtArgVal *value)
{
