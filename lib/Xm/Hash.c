...
XmHashTable
_XmAllocHashTable(Cardinal size_hint, XmHashCompareProc cproc, XmHashFunction hproc)
{
   XmHashTable table;
   int         i;
   ...
   while (size_table[i] != 0 && size_table[i] < size_hint) i++;
   if (size_table[i] == 0) i--;
...
_XmResizeHashTable(XmHashTable table, Cardinal new_size)
{
   ...
   i = 0;
   /* Search size_table for size which is bigger than size_hint */
   while (size_table[i] != 0 && size_table[i] < new_size) i++;
   if (size_table[i] == 0) i--;
