...
static DtCachedDir MakeCachedDirEntry(String dirName);
static int CheckDirCache(String path);
static Boolean TestIconFile(String path);
...
static DtCachedDir MakeCachedDirEntry(String dirName)
{
  ...
  if ((fileDesc = opendir(dirName)) == NULL) {
    ...
    if (cachedDirType == DtVALID_CACHED_DIR) {
       ...
       */
      for (p = stackBuf; p - stackBuf < oldBufLen; p = p + strlen(p) + 1) {
        numFiles++;
       ...
       * the last name
       */
      validDir = (DtValidCachedDir)XtMalloc((sizeof(DtValidCachedDirStruct)) +
  ...
  switch (cachedDirType) {
    ...
    case DtUNCACHED_DIR:
      cachedDir = (DtCachedDir)XtMalloc(sizeof(DtCommonCachedDirStruct));
...
static String find_slash(String str)
{
  int n;
  if (MB_CUR_MAX == 1) {
    return strchr(str, '/');
...
#define H_SUB 3
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (size != XmUNSPECIFIED_ICON_SIZE) {
      ...
      memmove(&ext_name[basenameLen], iconSubs[M_SUB].substitution, sizeLen);
    ...
    if (!fileName) {
      ...
      iNameEntry = (DtIconNameEntry)_XmGetHashEntry(iconNameCache, (XmHashKey)&iNameData);
      if (iNameEntry) {
        int dirLen, leafLen;
        dirLen = strlen(iNameEntry->dirName);
        leafLen = strlen(iNameEntry->leafName);
        fileName = XtMalloc(dirLen + leafLen + 2);
        memmove(&fileName[0], iNameEntry->dirName, dirLen);
        if (dirLen == 0) {
          ...
          fileName[dirLen] = '/';
          memmove(&fileName[dirLen + 1], iNameEntry->leafName, leafLen);
