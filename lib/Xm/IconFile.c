...
MakeCachedDirEntry(String dirName)
{
      ...
      if (cachedDirType == DtVALID_CACHED_DIR)
      {
	     ...
	     */
         validDir = (DtValidCachedDir)
            XtMalloc((sizeof(DtValidCachedDirStruct)) + (sizeof(validDir->nameOffsets[0]) * numFiles) + (nameHeapSize));
         validDir->dirNameLen = strlen(dirName);
         validDir->dirName    = dirName;
         validDir->numFiles   = numFiles;
         cachedDirType = validDir->cachedDirType = DtVALID_CACHED_DIR;
   ...
   switch (cachedDirType)
   {
      ...
      case DtUNCACHED_DIR:
         ...
         cachedDir->common.cachedDirType = cachedDirType;
...
XmeFlushIconFileCache(String path)
{
   ...
   for (i = 0; i < cacheList.numDirs; i++)
   {
      ...
      if (!path || ((currDir->dirNameLen == dirNameLen) && (strncmp(currDir->dirName, path, dirNameLen) == 0)))
      {
	 ...
	 */
      ...
      if ((currDir->dirNameLen == dirNameLen) && (strncmp(currDir->dirName, path, dirNameLen) == 0))
      {
         switch (currDir->cachedDirType)
         {
		     ...
		     */
                  currNameLen = (currDir->nameOffsets[j + 1] - currDir->nameOffsets[j]);
                 ...
                 XmHashKey key_2)
{
   ...
   return ((data_1->key_name == data_2->key_name) || (strcmp(data_1->key_name, data_2->key_name) == 0));
}
...
#define H_SUB	3
   ...
   for (i = 0; i < 2; i++)
   {
      if (names[i] == NULL)
         continue;
      if ((absolute = _XmOSAbsolutePathName(names[i], &names[i], stackString)) != FALSE)
      {
	...
	*/
	 ...
	 */
      ...
      {
         fileName = XtResolvePathname(display, "icons", NULL, ".svg", iPath, iconSubs, XtNumber(iconSubs), (XtFilePredicate)testFileFunc);
      }
      if (fileName) break;
      /**
	 ...
	 */
      ...
      {
         fileName = XtResolvePathname(display, "icons", NULL, ".xpm", iPath, iconSubs, XtNumber(iconSubs), (XtFilePredicate)testFileFunc);
      }
      if (fileName) break;
      fileName = XtResolvePathname(display, "icons", NULL, NULL, iPath, iconSubs, XtNumber(iconSubs), (XtFilePredicate)testFileFunc);
      if (fileName) break;
      fileName = XtResolvePathname(display, "bitmaps", NULL, NULL, bPath, iconSubs, XtNumber(iconSubs), (XtFilePredicate)testFileFunc);
      if (fileName) break;
   }
   _XmProcessUnlock();
   if (fileName && !absolute)
   {
      ...
      _XmProcessLock();
      _XmAddHashEntry(iconNameCache, (XmHashKey)iNameEntry, (XtPointer)iNameEntry);
      _XmProcessUnlock();
   }
   /*
