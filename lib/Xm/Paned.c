...
static XtGeometryResult AdjustPanedSize(
    XmPanedWidget, Dimension, Boolean, Boolean, Dimension *, Dimension *);
...
static XtResource resources[] = {{XmNspacing,
                                 ...
                                 {XmNrefigureMode,
                                  ...
                                  XmRBoolean,
                                  sizeof(Boolean),
                                  XtOffsetOf(XmPanedRec, paned.refiguremode),
                                  XmRImmediate,
                                  (XtPointer)TRUE},
                                 {XmNorientation,
                                  XmCOrientation,
                                  ...
                                  XtOffsetOf(XmPanedRec, paned.orientation),
                                  XmRImmediate,
                                 ...
                                 {XmNsashTranslations,
                                  ...
                                  XtOffsetOf(XmPanedRec, paned.sash_translations),
                                  XmRString,
                                  (XtPointer)defSashTranslations},
                                 {XmNsashWidth,
                                  XmCSashWidth,
                                  XmRHorizontalDimension,
                                  sizeof(Dimension),
                                 ...
                                 {XmNallowUnusedSpace,
                                  ...
                                  XmRBoolean,
                                  sizeof(Boolean),
                                  XtOffsetOf(XmPanedRec, paned.allow_unused_space),
                                  XmRImmediate,
                                  (XtPointer)TRUE}};
/* Definition for resources that need special processing in get values */
static XmSyntheticResource get_resources[] = {{XmNmarginWidth,
                                               sizeof(Dimension),
                                               XtOffsetOf(XmPanedRec, paned.margin_width),
                                              ...
                                              {XmNsashIndent,
                                               sizeof(Position),
                                               XtOffsetOf(XmPanedRec, paned.sash_indent),
                                               FromPanedOppositePixels,
                                               (XmImportProc)ToPanedOppositePixels},
                                              {XmNsashWidth,
                                               sizeof(Dimension),
                                               XtOffsetOf(XmPanedRec, paned.sash_width),
                                               XmeFromHorizontalPixels,
...
static XtResource subresources[] = {
    {XmNallowResize,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmPanedConstraintsRec, paned.allow_resize),
     XmRImmediate,
     (XtPointer)FALSE},
    {XmNpaneMinimum,
     XmCPaneMinimum,
    ...
    {XmNposition,
     ...
     XmRInt,
     sizeof(int),
     XtOffsetOf(XmPanedConstraintsRec, paned.position),
     XmRImmediate,
     (XtPointer)0},
#endif
    {XmNpreferredPaneSize,
     XmCPreferredPaneSize,
     XmRHorizontalDimension,
     sizeof(Dimension),
     XtOffsetOf(XmPanedConstraintsRec, paned.preferred_size),
     XmRImmediate,
     (XtPointer)XmPanedAskChild},
    /*
    ...
    {XmNskipAdjust,
     ...
     XmRBoolean,
     sizeof(Boolean),
     XtOffsetOf(XmPanedConstraintsRec, paned.skip_adjust),
     XmRImmediate,
     (XtPointer)FALSE},
    {XmNshowSash,
     XmCShowSash,
...
static XmSyntheticResource get_constraint_resources[] = {
    {XmNpaneMinimum,
     sizeof(Dimension),
     XtOffsetOf(XmPanedConstraintsRec, paned.min),
     FromPanedChildPixels,
     (XmImportProc)ToPanedChildPixels},
    {XmNpaneMaximum,
     sizeof(Dimension),
     XtOffsetOf(XmPanedConstraintsRec, paned.max),
     FromPanedChildPixels,
 ...
 */
static XtGeometryResult AdjustPanedSize(XmPanedWidget pw,
                                        Dimension off_size,
                                        Boolean always_compute_on_size,
                                        Boolean query_only,
 ...
 *	Returns: none
 */
void _XmFromPanedPixels(Widget widget, int offset, XtArgVal *value)
{
 ...
 *	Returns: none
 */
static void FromPanedChildPixels(Widget widget, int offset, XtArgVal *value)
{
 ...
 *	Returns: none
 */
XmImportOperator _XmToPanedPixels(Widget widget, int offset, XtArgVal *value)
{
  XmPanedWidget pw = (XmPanedWidget)widget;
  if (IsVert(pw))
 ...
 *	Returns: none
 */
static XmImportOperator ToPanedChildPixels(Widget widget, int offset, XtArgVal *value)
{
  XmPanedWidget pw = (XmPanedWidget)XtParent(widget);
  if (IsVert(pw))
    return (XmeToVerticalPixels(widget, offset, value));
 ...
 *	Returns: none
 */
static void FromPanedOppositePixels(Widget widget, int offset, XtArgVal *value)
{
  XmPanedWidget pw = (XmPanedWidget)widget;
  if (IsVert(pw))
    XmeFromHorizontalPixels(widget, offset, value);
 ...
 *	Returns: none
 */
static XmImportOperator ToPanedOppositePixels(Widget widget, int offset, XtArgVal *value)
{
  XmPanedWidget pw = (XmPanedWidget)widget;
  if (IsVert(pw))
    return (XmeToHorizontalPixels(widget, offset, value));
 ...
 */
static Pane ChoosePaneToResize(XmPanedWidget pw, int paneindex, Direction dir, Boolean shrink)
{
  Widget *childP;
  int rules = 3;
  Direction _dir = dir;
 ...
 *	Returns: none.
 */
static Boolean IsPane(Widget w)
{
 ...
 */
static Boolean SatisfiesRule1(Pane pane, Boolean shrink, Direction dir)
{
  /*
...
{
  return ((pane->size != pane->wp_size) && ((shrink && ((int)pane->wp_size <= pane->size)) ||
 ...
 *	Returns: none.
 */
static void LoopAndRefigureChildren(XmPanedWidget pw, int paneindex, int dir, Dimension *sizeused)
{
       ...
       childP++)
  {
    ...
    if (IsVert(pw)) {
      if (*childP == no_resize_child) {
        ...
        _XmConfigureWidget(*childP,
                           (Position)offset,
                           pane->delta,
                           pw->core.width - 2 * (offset + (*childP)->core.border_width),
                           (Dimension)pane->size,
      ...
      if (sash != NULL) { /* Move and Display the Sash */
        ...
        if (XmPaned_sash_indent(pw) < 0) {
          changes.x = (pw->core.width + XmPaned_sash_indent(pw) - sash->core.width -
        ...
        changes.y = ((*childP)->core.y + (*childP)->core.height +
                     2 * (*childP)->core.border_width + internal_space / 2 -
                     sash->core.height / 2 - sash->core.border_width);
      }
      if (separator != NULL) {
        sep.width = pw->core.width;
        sep.height = XmPaned_sash_shadow_thickness(pw);
        sep.x = 0;
        sep.y = ((*childP)->core.y + (*childP)->core.height - sep.height / 2 -
                 separator->core.border_width + internal_space / 2 +
      ...
      if (*childP == no_resize_child) {
        ...
        _XmConfigureWidget(*childP,
                           ...
                           (Position)offset,
                           (Dimension)pane->size,
                           pw->core.height - 2 * (offset + (*childP)->core.border_width),
                           (Dimension)(*childP)->core.border_width);
      }
      if (sash != NULL) { /* Move and Display the Sash */
        if (LayoutIsRtoLM(pw))
          changes.x = ((*childP)->core.x - internal_space / 2 - sash->core.width / 2 -
                       sash->core.border_width);
        else
          changes.x = ((*childP)->core.x + (*childP)->core.width +
                       2 * (*childP)->core.border_width + internal_space / 2 -
                       sash->core.width / 2 - sash->core.border_width);
        /* Motif style does - indent from right edge, pos from left */
        if (XmPaned_sash_indent(pw) < 0) {
          changes.y = pw->core.height + XmPaned_sash_indent(pw) - sash->core.height -
                      sash->core.border_width * 2;
        }
        else {
      ...
      if (separator != NULL) {
        ...
        sep.y = 0;
        if (LayoutIsRtoLM(pw))
          sep.x = ((*childP)->core.x - sep.width / 2 - separator->core.border_width -
                   internal_space / 2);
        else
          sep.x = ((*childP)->core.x + (*childP)->core.width - sep.width / 2 -
                   separator->core.border_width + internal_space / 2 +
                   2 * (*childP)->core.border_width);
      }
    }
    ...
    if (separator != NULL) {
      _XmConfigureWidget(
          separator, sep.x, sep.y, sep.width, sep.height, separator->core.border_width);
    }
    if (sash != NULL) {
...
static void _DrawRect(
    XmPanedWidget pw, GC gc, int on_loc, int off_loc, unsigned int on_size, unsigned int off_size)
{
  if (IsVert(pw))
 ...
 *	Returns: none.
 */
static void _DrawTrackLines(XmPanedWidget pw, Boolean erase)
{
  Widget *childP;
  Pane pane;
  int on_loc, off_loc;
       ...
       childP++)
  {
     ...
     */
    if ((((IsVert(pw) || !(LayoutIsRtoLM(pw))) && (XmPaned_managed_children(pw) != childP)) ||
         (!(IsVert(pw)) && LayoutIsRtoLM(pw) && !(IsLastPane(pw, childP)))) &&
 ...
 */
static int GetEventLocation(XmPanedWidget pw, XEvent *event)
{
 ...
 */
/* ARGSUSED */
static void StartSashAdjustment(XmPanedWidget pw, Widget sash)
{
  Widget *childP;
 ...
 *	Returns: none.
 */
static void MoveSashAdjustment(XmPanedWidget pw, Widget sash, int loc)
{
 ...
 *	Returns: none
 */
static void CommitSashAdjustment(XmPanedWidget pw, Widget sash)
{
  int i;
  /*
 ...
 */
/* ARGSUSED */
static void HandleSash(Widget sash, XtPointer junk, XtPointer callData)
{
  ...
  if (call_data->num_params == 0 || (action_type == 'C' && call_data->num_params != 1) ||
 ...
 *	Returns: none.
 */
static void ManageAndUnmanageSashs(XmPanedWidget pw)
{
  ...
  if (unmanagedP != unmanaged_sashs)
    XtUnmanageChildren(unmanaged_sashs, (Cardinal)(unmanagedP - unmanaged_sashs));
  ...
  if (unmanagedS != unmanaged_seps)
    XtUnmanageChildren(unmanaged_seps, (Cardinal)(unmanagedS - unmanaged_seps));
 ...
 *	Returns: none.
 */
static void CreateSeparator(Widget child)
{
  ...
  PaneInfo(child)->separator = XtCreateWidget(
      "separator", xmSeparatorWidgetClass, (Widget)pw, args, num_args);
}
 ...
 *	Returns: none.
 */
static void SetChildrenPrefSizes(XmPanedWidget pw,
                                 Dimension off_size,
                                 Boolean use_off_size,
                                 Boolean only_if_new)
{
  Widget *childP;
  Boolean vert = IsVert(pw);
 ...
 *	Returns: none.
 */
static void ResetSash(Widget set)
{
 ...
 *	Returns: none.
 */
static void PushPaneStack(XmPanedWidget pw, Pane pane)
{
 ...
 *	Returns: none.
 */
static void GetPaneStack(XmPanedWidget pw, Boolean shrink, Pane *pane, int *start_size)
{
 ...
 */
static Boolean PopPaneStack(XmPanedWidget pw)
{
 ...
 *	Returns: none
 */
static void ClearPaneStack(XmPanedWidget pw)
{
 ...
 *
 ********************************************************************/
static void ReManageChildren(XmPanedWidget pw)
{
  int i;
  XmPaned_num_panes(pw) = 0;
...
{
  XmPanedWidget pw = (XmPanedWidget)w;
  if (XmPaned_recursively_called(pw)++)
    return;
 ...
 *	Returns: none.
 */
static void ResetSize(XmPanedWidget pw, Boolean recalc_off_size)
{
...
static Boolean PaneSetValues(
    Widget old, Widget request, Widget new, ArgList args, Cardinal *num_args)
{
 ...
 */
int XmPanedGetPanes(Widget w, WidgetList *panes, int *num)
{
  XmPanedWidget pw = (XmPanedWidget)w;
 ...
 */
Widget XmCreatePaned(Widget parent, String name, ArgList args, Cardinal num_args)
{
