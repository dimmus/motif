...
static Boolean CvtStringToColorMode(
 ...
 *       STATIC DECLARATIONS
 ************************************************************/
static XtResource resources[] = {{XmNcolorMode,
                                  XmCColorMode,
                                  XmRXmColorMode,
                                  sizeof(XmColorMode),
                                  XtOffsetOf(XmColorSelectorRec, cs.color_mode),
                                  XmRImmediate,
                                  (XtPointer)XmScaleMode},
                                 {XmNcolorName,
                                  XmCString,
                                  XmRString,
                                  sizeof(String),
                                  XtOffsetOf(XmColorSelectorRec, cs.color_name),
                                  XmRString,
                                  "White"},
                                 {XmNrgbFile,
                                  XmCString,
                                  XmRString,
                                  sizeof(String),
                                  XtOffsetOf(XmColorSelectorRec, cs.rgb_file),
                                  XmRString,
...
static XmSyntheticResource get_resources[] = {
    ...
    {XmNredSliderLabel,
     sizeof(XmString),
     XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[0]),
     GetValues_XmNredSliderLabel,
     NULL},
    {XmNgreenSliderLabel,
     sizeof(XmString),
     XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[1]),
     GetValues_XmNgreenSliderLabel,
     NULL},
    {XmNblueSliderLabel,
     sizeof(XmString),
     XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[2]),
     GetValues_XmNblueSliderLabel,
     NULL},
    {XmNcolorListTogLabel,
     sizeof(XmString),
     XtOffsetOf(XmColorSelectorRec, cs.strings.tog_labels[0]),
     GetValues_XmNcolorListTogLabel,
     NULL},
    {XmNsliderTogLabel,
     sizeof(XmString),
     XtOffsetOf(XmColorSelectorRec, cs.strings.tog_labels[1]),
     GetValues_XmNsliderTogLabel,
     NULL},
    {XmNnoCellError,
     sizeof(XmString),
     XtOffsetOf(XmColorSelectorRec, cs.strings.no_cell_error),
     GetValues_XmNnoCellError,
     NULL},
    {XmNfileReadError,
     sizeof(XmString),
     XtOffsetOf(XmColorSelectorRec, cs.strings.file_read_error),
     GetValues_XmNfileReadError,
     NULL}};
...
static void Initialize(Widget request, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  XtFree((XtPointer)f_args);
  {
    int i;
    for (i = 0; i < 3; i++)
      XmColorS_strings(csw).slider_labels[i] = XmStringCopy(
          XmColorS_strings(csw).slider_labels[i]);
    ...
    Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
  ...
  if (XmColorS_color_mode(curr) != XmColorS_color_mode(csw)) {
    new_mode(csw, XmColorS_color_mode(csw));
    XmToggleButtonSetState(XmColorS_chose_mode(csw)[XmColorS_color_mode(csw)], True, True);
  }
  /*
  ...
  }
  {
    ...
    for (i = 0; i < 3; i++) {
      if (XmColorS_strings(curr).slider_labels[i] != XmColorS_strings(csw).slider_labels[i]) {
        ...
        XmColorS_strings(csw).slider_labels[i] = XmStringCopy(
            XmColorS_strings(csw).slider_labels[i]);
        XtVaSetValues(XmColorS_sliders(csw)[i],
                      XmNtitleString,
                      XmColorS_strings(csw).slider_labels[i],
                      NULL);
      }
    ...
    for (i = 0; i < XmColorSelector_NUM_TOGGLES; i++) {
      if (XmColorS_strings(curr).tog_labels[i] != XmColorS_strings(csw).tog_labels[i]) {
        ...
        XmColorS_strings(csw).tog_labels[i] = XmStringCopy(XmColorS_strings(csw).tog_labels[i]);
        XtVaSetValues(XmColorS_chose_mode(csw)[i],
                      XmNlabelString,
                      XmColorS_strings(csw).tog_labels[i],
                      NULL);
      }
    }
 ...
 *      Returns:       status.
 */
static XtGeometryResult GeometryHandler(Widget w,
...
{
  CalcPreferredSize((XmColorSelectorWidget)w, &(preferred->width), &(preferred->height));
 ...
 *      Returns:
 */
static Boolean CvtStringToColorMode(Display *dpy,
                                    XrmValuePtr args,
                                    Cardinal num_args,
                                    XrmValuePtr from,
                                    XrmValuePtr to,
 ...
 *	Returns: none.
 */
static void CalcPreferredSize(XmColorSelectorWidget csw, Dimension *width, Dimension *height)
{
 ...
 */
static Boolean color_name_changed(XmColorSelectorWidget csw, char *name)
{
 ...
 *	Returns: none.
 */
static void SelectColor(XmColorSelectorWidget csw)
{
  int color_num;
 ...
 *      Returns:       	none.
 */
static void slider_changed(Widget w, XtPointer csw_ptr, XtPointer scale_ptr)
{
 ...
 *	Returns: True if successful
 */
static Boolean UpdateColorWindow(XmColorSelectorWidget csw, Boolean use_name)
{
  int index;
  XColor color;
  Pixel foreground;
 ...
 *      Returns:       	none.
 */
static void list_selected(Widget w, XtPointer csw_ptr, XtPointer list_ptr)
{
  XmColorSelectorWidget csw = (XmColorSelectorWidget)csw_ptr;
  XmListCallbackStruct *list = (XmListCallbackStruct *)list_ptr;
  XtFree(XmColorS_color_name(csw));
  XmColorS_color_name(csw) = XmStringUnparse(
      list->item, NULL, XmCHARSET_TEXT, XmCHARSET_TEXT, NULL, 0, XmOUTPUT_ALL);
  /* deprecated
 ...
 *      Returns:       	none.
 */
static void change_mode(Widget w, XtPointer csw_ptr, XtPointer tp)
{
  XmColorSelectorWidget csw = (XmColorSelectorWidget)csw_ptr;
 ...
 *      Returns:       	none.
 */
static void new_mode(XmColorSelectorWidget csw, XmColorMode mode)
{
 ...
 *      Returns:       	none.
 */
static void compute_size(XmColorSelectorWidget csw)
{
  ...
  if (radio_geom.width < csw->core.width)
    ...
    x = XmColorS_margin_width(csw);
  _XmConfigureWidget(XmColorS_chose_radio(csw),
                     x,
                     y,
                     radio_geom.width,
                     radio_geom.height,
                     radio_geom.border_width);
  y += radio_geom.height + XmColorS_margin_height(csw);
  /*
   * Lastly, place the color window
   */
  _XmConfigureWidget(XtParent(XmColorS_color_window(csw)),
                     XmColorS_margin_width(csw),
                     y,
                     width,
                     color_geom.height,
                     color_geom.border_width);
}
 ...
 *      Returns:       	none.
 */
static void read_rgb_file(XmColorSelectorWidget csw,
                          ArgList cargs,
                          Cardinal cnum_args,
                          Boolean initial)
{
  ...
  if ((file = fopen(XmColorS_rgb_file(csw), "r")) != NULL) {
    ...
    while (fgets(buf, BUFSIZ, file)) {
      ...
      if (count >= alloc) {
        if (0 == alloc)
          alloc = 755; /* rather than stat the file and determine a good value to use, just use
                          enough for X11R5, X11R6, and OpenWindows3 */
        else {
          alloc += 20;
        }
      ...
      sscanf(buf,
             "%hu %hu %hu",
             &(color_info[count].red),
             &(color_info[count].green),
             &(color_info[count].blue));
 ...
 *	Returns: none
 */
static int CmpColors(const void *ptr_1, const void *ptr_2)
{
...
static void CreateColorSliders(XmColorSelectorWidget csw, ArgList cargs, Cardinal cnum_args)
{
  ...
  XmColorS_bb(csw) = XtCreateManagedWidget(
      "buttonBox", xmButtonBoxWidgetClass, (Widget)csw, margs, cnum_args + num_args);
  XtFree((XtPointer)margs);
  num_args = 0;
  ...
  for (i = 0; i < 3; i++) {
    ...
    XmColorS_sliders(csw)[i] = XtCreateManagedWidget(
        "scale", xmScaleWidgetClass, XmColorS_bb(csw), margs, num_args + cnum_args);
 ...
 *      Returns:       	none.
 */
static void CreateSelectorRadio(XmColorSelectorWidget csw, ArgList cargs, Cardinal cnum_args)
{
  Widget w;
  Cardinal i, label;
  Arg *margs, args[5];
  ...
  for (count = 0; count < XmColorSelector_NUM_TOGGLES; count++) {
    ...
    w = XtCreateManagedWidget(
        names[count], xmToggleButtonWidgetClass, XmColorS_chose_radio(csw), margs, i + cnum_args);
 ...
 *      Returns:       	none.
 */
static void CreateColorWindow(XmColorSelectorWidget csw, ArgList cargs, Cardinal cnum_args)
{
  Widget fr;
  Arg *margs, args[10];
  Cardinal n;
  ...
  XmColorS_color_window(csw) = XtCreateManagedWidget(
      "colorWindow", xmLabelWidgetClass, fr, margs, n + cnum_args);
  XtFree((XtPointer)margs);
}
 ...
 *      Returns:       	Visual id.
 */
static int GetVisual(XmColorSelectorWidget csw)
{
  Visual *vis;
  int visual;
 ...
 *      Returns:       	None.
 */
static void NoPrivateColormaps(XmColorSelectorWidget csw,
                               Pixel foreground,
                               XColor color,
                               char *str)
{
 ...
 *      Returns:       	None.
 */
static void PrivateColormaps(XmColorSelectorWidget csw, Pixel foreground, XColor color, char *str)
{
  Arg args[5];
  XmString xm_str;
  Cardinal num_args;
  ...
  if (!XmColorS_good_cell(csw)) {
    if (XAllocColorCells(
            XtDisplay(csw), csw->core.colormap, 0, 0, 0, &(XmColorS_color_pixel(csw)), 1))
    {
      XmColorS_good_cell(csw) = True;
    }
    else {
 ...
 *			False otherwise.
 */
static Boolean DefaultVisualDisplay(XmColorSelectorWidget csw,
                                    Pixel foreground,
                                    XColor color,
                                    char *str)
{
 ...
 */
Widget XmCreateColorSelector(Widget parent, String name, ArgList args, Cardinal num_args)
{
...
Widget XmVaCreateColorSelector(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmColorSelectorWidgetClass, parent, False, var, count);
  va_end(var);
  return w;
}
...
Widget XmVaCreateManagedColorSelector(Widget parent, char *name, ...)
{
  ...
  va_end(var);
  Va_start(var, name);
  w = XmeVLCreateWidget(name, xmColorSelectorWidgetClass, parent, True, var, count);
  va_end(var);
  return w;
}
