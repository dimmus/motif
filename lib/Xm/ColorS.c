...
static Boolean          CvtStringToColorMode(Display *, XrmValuePtr, Cardinal, XrmValuePtr, XrmValuePtr, XtPointer *);
...
static XtResource resources[] = {
   { XmNcolorMode,         XmCColorMode,     XmRXmColorMode,         sizeof(XmColorMode), XtOffsetOf(XmColorSelectorRec, cs.color_mode),               XmRImmediate, (XtPointer)XmScaleMode                     },
   { XmNcolorName,         XmCString,        XmRString,              sizeof(String),      XtOffsetOf(XmColorSelectorRec, cs.color_name),               XmRString,    "White"                                    },
   { XmNrgbFile,           XmCString,        XmRString,              sizeof(String),      XtOffsetOf(XmColorSelectorRec, cs.rgb_file),                 XmRString,    (XtPointer) "/usr/lib/X11/rgb.txt"         },
   { XmNmarginWidth,       XmCMargin,        XmRHorizontalDimension, sizeof(Dimension),   XtOffsetOf(XmColorSelectorRec, cs.margin_width),             XmRImmediate, (XtPointer)2                               },
   { XmNmarginHeight,      XmCMargin,        XmRVerticalDimension,   sizeof(Dimension),   XtOffsetOf(XmColorSelectorRec, cs.margin_height),            XmRImmediate, (XtPointer)2                               },
   { XmNredSliderLabel,    XmCSliderLabel,   XmRXmString,            sizeof(XmString),    XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[0]), XmRString,    (XtPointer) "Red"                          },
   { XmNgreenSliderLabel,  XmCSliderLabel,   XmRXmString,            sizeof(XmString),    XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[1]), XmRString,    (XtPointer) "Green"                        },
   { XmNblueSliderLabel,   XmCSliderLabel,   XmRXmString,            sizeof(XmString),    XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[2]), XmRString,    (XtPointer) "Blue"                         },
   { XmNcolorListTogLabel, XmCTogLabel,      XmRXmString,            sizeof(XmString),    XtOffsetOf(XmColorSelectorRec, cs.strings.tog_labels[0]),    XmRString,    (XtPointer) "Color List"                   },
   { XmNsliderTogLabel,    XmCTogLabel,      XmRXmString,            sizeof(XmString),    XtOffsetOf(XmColorSelectorRec, cs.strings.tog_labels[1]),    XmRString,    (XtPointer) "Color Sliders"                },
   { XmNnoCellError,       XmCNoCellError,   XmRXmString,            sizeof(XmString),    XtOffsetOf(XmColorSelectorRec, cs.strings.no_cell_error),    XmRString,    (XtPointer) "\n\nNo Color Cell Available!" },
   { XmNfileReadError,     XmCFileReadError, XmRXmString,            sizeof(XmString),    XtOffsetOf(XmColorSelectorRec, cs.strings.file_read_error),  XmRString,    (XtPointer) "Could not read rgb.txt file:" }
};
static XmSyntheticResource get_resources[] = {
   { XmNmarginWidth,       sizeof(Dimension), XtOffsetOf(XmColorSelectorRec, cs.margin_width),             XmeFromHorizontalPixels,        (XmImportProc)XmeToHorizontalPixels },
   { XmNmarginHeight,      sizeof(Dimension), XtOffsetOf(XmColorSelectorRec, cs.margin_height),            XmeFromVerticalPixels,          (XmImportProc)XmeToVerticalPixels   },
   { XmNredSliderLabel,    sizeof(XmString),  XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[0]), GetValues_XmNredSliderLabel,    NULL                                },
   { XmNgreenSliderLabel,  sizeof(XmString),  XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[1]), GetValues_XmNgreenSliderLabel,  NULL                                },
   { XmNblueSliderLabel,   sizeof(XmString),  XtOffsetOf(XmColorSelectorRec, cs.strings.slider_labels[2]), GetValues_XmNblueSliderLabel,   NULL                                },
   { XmNcolorListTogLabel, sizeof(XmString),  XtOffsetOf(XmColorSelectorRec, cs.strings.tog_labels[0]),    GetValues_XmNcolorListTogLabel, NULL                                },
   { XmNsliderTogLabel,    sizeof(XmString),  XtOffsetOf(XmColorSelectorRec, cs.strings.tog_labels[1]),    GetValues_XmNsliderTogLabel,    NULL                                },
   { XmNnoCellError,       sizeof(XmString),  XtOffsetOf(XmColorSelectorRec, cs.strings.no_cell_error),    GetValues_XmNnoCellError,       NULL                                },
   { XmNfileReadError,     sizeof(XmString),  XtOffsetOf(XmColorSelectorRec, cs.strings.file_read_error),  GetValues_XmNfileReadError,     NULL                                }
};
...
{
   XtSetTypeConverter(XmRString, XmRXmColorMode, (XtTypeConverter)CvtStringToColorMode, NULL, (Cardinal)0, XtCacheAll, NULL);
}
...
Destroy(Widget w)
{
   ...
   {
      XFreeColors(XtDisplay(csw), csw->core.colormap, &XmColorS_color_pixel(csw), 1, 0);
   }
...
SetValues(Widget current, Widget request, Widget set, ArgList args, Cardinal *num_args)
{
   ...
   if ((XmColorS_margin_height(curr) != XmColorS_margin_height(csw)) || (XmColorS_margin_width(curr) != XmColorS_margin_width(csw)))
   {
      compute_size(csw);
   }
 ...
 */
static Boolean
CvtStringToColorMode(Display *dpy, XrmValuePtr args, Cardinal num_args, XrmValuePtr from, XrmValuePtr to, XtPointer *junk)
{
      ...
      if ((ptr->red == red) && (ptr->green == green) && (ptr->blue == blue))
      {
         ...
         if (XmColorS_color_name(csw))
         {
            if (XmColorS_color_name(csw)[0] == '#')
               *color_num = i;
            if (streq(XmColorS_color_name(csw), ptr->name) || streq(XmColorS_color_name(csw), ptr->no_space_lower_name))
            {
               *color_num = i;
               return (True);
            }
...
UpdateColorWindow(XmColorSelectorWidget csw, Boolean use_name)
{
   ...
   if (!use_name) /* Update color names */
   {
      ...
      sprintf(buf, "#%02x%02x%02x", XmColorS_slider_red(csw), XmColorS_slider_green(csw), XmColorS_slider_blue(csw));
   ...
   {
      if (XParseColor(XtDisplay(csw), csw->core.colormap, XmColorS_color_name(csw), &color) == 0)
      {
         return (False);
      }
...
list_selected(Widget w, XtPointer csw_ptr, XtPointer list_ptr)
{
   ...
   XtFree(XmColorS_color_name(csw));
   XmColorS_color_name(csw) = XmStringUnparse(list->item,
                                              NULL,
                                              XmCHARSET_TEXT,
                                              XmCHARSET_TEXT,
                                              NULL,
...
change_mode(Widget w, XtPointer csw_ptr, XtPointer tp)
{
   ...
   */
   if (toggle->reason == XmCR_VALUE_CHANGED && toggle->set)
   {
     ...
     */
      if ((w == XmColorS_chose_mode(csw)[XmListMode]) && (XmColorS_color_mode(csw) != XmListMode))
      {
         new_mode(csw, XmListMode);
      }
      else if ((w == XmColorS_chose_mode(csw)[XmScaleMode]) && (XmColorS_color_mode(csw) != XmScaleMode))
      {
...
compute_size(XmColorSelectorWidget csw)
{
   ...
   height = (csw->core.height - 4 * XmColorS_margin_height(csw) - (radio_geom.height + 2 * radio_geom.border_width));
   /*
   ...
   if (radio_geom.width < csw->core.width)
      ...
      x = XmColorS_margin_width(csw);
   _XmConfigureWidget(XmColorS_chose_radio(csw), x, y, radio_geom.width, radio_geom.height, radio_geom.border_width);
   y += radio_geom.height + XmColorS_margin_height(csw);
   /*
     * Lastly, place the color window
     */
   _XmConfigureWidget(XtParent(XmColorS_color_window(csw)), XmColorS_margin_width(csw), y, width, color_geom.height, color_geom.border_width);
}
...
read_rgb_file(XmColorSelectorWidget csw, ArgList cargs, Cardinal cnum_args, Boolean initial)
{
   ...
   if (XmColorS_list(csw) == NULL)
   {
      ...
      i++;
      XtSetArg(args[i], XmNvisibleItemCount, 15);
      i++;
      margs              = XtMergeArgLists(args, i, cargs, cnum_args);
      XmColorS_list(csw) = XmCreateScrolledList((Widget)csw, "list", margs, i + cnum_args);
   ...
   if ((file = fopen(XmColorS_rgb_file(csw), "r")) != NULL)
   {
      ...
      while (fgets(buf, BUFSIZ, file))
      {
	     ...
	     */
         ...
         sscanf(buf, "%hu %hu %hu", &(color_info[count].red), &(color_info[count].green), &(color_info[count].blue));
                   ...
                   Cardinal              cnum_args)
{
   ...
   num_args++;
   XtSetArg(args[num_args], XmNfillOption, XmFillMinor);
   num_args++;
   margs            = XtMergeArgLists(args, num_args, cargs, cnum_args);
   XmColorS_bb(csw) = XtCreateManagedWidget("buttonBox", xmButtonBoxWidgetClass, (Widget)csw, margs, cnum_args + num_args);
   XtFree((XtPointer)margs);
   num_args = 0;
   XtSetArg(args[num_args], XmNmaximum, 255);
   num_args++;
   XtSetArg(args[num_args], XmNorientation, XmHORIZONTAL);
   num_args++;
   XtSetArg(args[num_args], XmNshowValue, True);
   num_args++;
   XtSetArg(args[num_args], XmNprocessingDirection, XmMAX_ON_RIGHT);
   num_args++;
   XtSetArg(args[num_args], XmNtitleString, NULL);
   title = num_args++;
   margs = XtMergeArgLists(args, num_args, cargs, cnum_args);
   for (i = 0; i < 3; i++)
   {
      ...
      XmColorS_sliders(csw)[i] = XtCreateManagedWidget("scale",
                                                       ...
                                                       margs,
                                                       num_args + cnum_args);
      XtAddCallback(XmColorS_sliders(csw)[i], XmNdragCallback, slider_changed, csw);
      XtAddCallback(XmColorS_sliders(csw)[i], XmNvalueChangedCallback, slider_changed, csw);
   }
   XtFree((XtPointer)margs);
}
                    ...
                    Cardinal              cnum_args)
{
   ...
   i                     = 0;
   XtSetArg(args[i], XmNradioBehavior, True);
   i++;
   XtSetArg(args[i], XmNpacking, XmPACK_COLUMN);
   i++;
   XtSetArg(args[i], XmNnumColumns, 2);
   i++;
   margs                     = XtMergeArgLists(args, i, cargs, cnum_args);
   w                         = XtCreateManagedWidget("radioBox", xmRowColumnWidgetClass, (Widget)csw, margs, i + cnum_args);
   XmColorS_chose_radio(csw) = w;
   XtFree((XtPointer)margs);
   i = 0;
   XtSetArg(args[i], XmNlabelString, NULL);
   label = i++;
   margs = XtMergeArgLists(args, i, cargs, cnum_args);
   for (count = 0; count < XmColorSelector_NUM_TOGGLES; count++)
   {
      margs[label].value              = (XtArgVal)XmColorS_strings(csw).tog_labels[count];
      w                               = XtCreateManagedWidget(names[count], xmToggleButtonWidgetClass, XmColorS_chose_radio(csw), margs, i + cnum_args);
...
CreateColorWindow(XmColorSelectorWidget csw, ArgList cargs, Cardinal cnum_args)
{
   ...
   Arg     *margs, args[10];
   Cardinal n;
   fr = XtCreateManagedWidget("colorFrame", xmFrameWidgetClass, (Widget)csw, cargs, cnum_args);
   n  = 0;
...
static void
GetValues_XmNredSliderLabel(Widget w, int n, XtArgVal *value)
{
   (*value) = (XtArgVal)XmStringCopy(XmColorS_strings(w).slider_labels[0]);
}

static void
GetValues_XmNgreenSliderLabel(Widget w, int n, XtArgVal *value)
{
   (*value) = (XtArgVal)XmStringCopy(XmColorS_strings(w).slider_labels[1]);
}

static void
GetValues_XmNblueSliderLabel(Widget w, int n, XtArgVal *value)
{
   (*value) = (XtArgVal)XmStringCopy(XmColorS_strings(w).slider_labels[2]);
}

static void
GetValues_XmNcolorListTogLabel(Widget w, int n, XtArgVal *value)
{
   (*value) = (XtArgVal)XmStringCopy(XmColorS_strings(w).tog_labels[0]);
}

static void
GetValues_XmNsliderTogLabel(Widget w, int n, XtArgVal *value)
{
   (*value) = (XtArgVal)XmStringCopy(XmColorS_strings(w).tog_labels[1]);
}

static void
GetValues_XmNnoCellError(Widget w, int n, XtArgVal *value)
{
   (*value) = (XtArgVal)XmStringCopy(XmColorS_strings(w).no_cell_error);
}

static void
GetValues_XmNfileReadError(Widget w, int n, XtArgVal *value)
{
 ...
 */
static void
NoPrivateColormaps(XmColorSelectorWidget csw, Pixel foreground, XColor color, char *str)
{
   Arg      args[5];
   XmString xm_str;
   Cardinal num_args;
...
PrivateColormaps(XmColorSelectorWidget csw, Pixel foreground, XColor color, char *str)
{
   ...
   if (!XmColorS_good_cell(csw))
   {
      if (XAllocColorCells(XtDisplay(csw), csw->core.colormap, 0, 0, 0, &(XmColorS_color_pixel(csw)), 1))
      {
         XmColorS_good_cell(csw) = True;
      }
      else
...
DefaultVisualDisplay(XmColorSelectorWidget csw, Pixel foreground, XColor color, char *str)
{
   ...
   /*
     ...
     */
   if (visual == StaticColor || visual == TrueColor || visual == StaticGray)
   {
      NoPrivateColormaps(csw, foreground, color, str);
      return True;
   }
   else if (visual == PseudoColor || visual == DirectColor || visual == GrayScale)
   {
 ...
 */
Widget
XmCreateColorSelector(Widget parent, String name, ArgList args, Cardinal num_args)
{
