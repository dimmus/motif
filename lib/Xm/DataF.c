...
static void df_ValidateAndMove(Widget w, XEvent *ev, String *args, Cardinal *nargs);
static int _XmDataFieldCountCharacters(XmDataFieldWidget tf, char *ptr, int n_bytes);
static void df_MakeCopy(Widget w, int n, XtArgVal *value);
static void df_WcsMakeCopy(Widget w, int n, XtArgVal *value);
static void df_FreeContextData(Widget w, XtPointer clientData, XtPointer callData);
static TextFDestData df_GetTextFDestData(Widget w);
static void df_SetDropContext(Widget w);
static void df_DeleteDropContext(Widget w);
static TextFGCData df_GetTextFGCData(Widget w);
static _XmHighlightRec *df_FindHighlight(XmDataFieldWidget w, XmTextPosition position);
static void df_InsertHighlight(XmDataFieldWidget w, XmTextPosition position, XmHighlightMode mode);
static void DataFieldSetHighlight(XmDataFieldWidget tf,
                                  XmTextPosition left,
                                  XmTextPosition right,
                                  XmHighlightMode mode);
static Boolean df_GetXYFromPos(XmDataFieldWidget tf,
                               XmTextPosition position,
                               Position *x,
                               Position *y);
static Boolean df_CurrentCursorState(XmDataFieldWidget tf);
static void df_PaintCursor(XmDataFieldWidget tf);
static void df_BlinkInsertionPoint(XmDataFieldWidget tf);
static void df_HandleTimer(XtPointer closure, XtIntervalId *id);
...
static void df_XmSetNormGC(XmDataFieldWidget tf, GC gc, Boolean change_stipple, Boolean stipple);
static void df_XmSetShadowGC(XmDataFieldWidget tf, GC gc);
static void df_XmSetInvGC(XmDataFieldWidget tf, GC gc);
static void df_DrawText(XmDataFieldWidget tf, GC gc, int x, int y, char *string, int length);
static int df_FindPixelLength(XmDataFieldWidget tf, char *string, int length);
static void df_DrawTextSegment(XmDataFieldWidget tf,
                               XmHighlightMode mode,
...
static Boolean df_AdjustText(XmDataFieldWidget tf, XmTextPosition position, Boolean flag);
static void df_AdjustSize(XmDataFieldWidget tf);
static Boolean df_ModifyVerify(XmDataFieldWidget tf,
                               XEvent *event,
...
static void df_SetCursorPosition(XmDataFieldWidget tf,
                                 XEvent *event,
                                 XmTextPosition position,
                                 Boolean adjust_flag,
                                 Boolean call_cb,
                                 Boolean set_dest);
static void df_VerifyBounds(XmDataFieldWidget tf, XmTextPosition *from, XmTextPosition *to);
...
static Boolean df_SetDestination(Widget w, XmTextPosition position, Boolean disown, Time set_time);
...
static Boolean _XmDataFieldIsWordBoundary(XmDataFieldWidget tf,
                                          XmTextPosition pos1,
                                          XmTextPosition pos2);
static void df_FindWord(XmDataFieldWidget tf,
                        XmTextPosition begin,
                        XmTextPosition *left,
                        XmTextPosition *right);
static void df_FindPrevWord(XmDataFieldWidget tf, XmTextPosition *left, XmTextPosition *right);
static void df_FindNextWord(XmDataFieldWidget tf, XmTextPosition *left, XmTextPosition *right);
...
static void df_CompleteNavigation(
    XmDataFieldWidget tf, XEvent *event, XmTextPosition position, Time time, Boolean extend);
static void df_SimpleMovement(Widget w,
                              XEvent *event,
                              String *params,
                              Cardinal *num_params,
...
static void df_SetSelection(XmDataFieldWidget tf,
                            XmTextPosition left,
                            XmTextPosition right,
                            Boolean redisplay);
static void df_ProcessHorizontalParams(Widget w,
                                       XEvent *event,
                                       char **params,
                                       Cardinal *num_params,
                                       XmTextPosition *left,
                                       XmTextPosition *right,
                                       XmTextPosition *position);
static void df_ProcessSelectParams(Widget w,
                                   XEvent *event,
                                   XmTextPosition *left,
                                   XmTextPosition *right,
                                   XmTextPosition *position);
...
static void df_RestorePrimaryHighlight(XmDataFieldWidget tf,
...
static void df_DoStuff(Widget w,
                       XtPointer closure,
                       Atom *seltype,
                       Atom *type,
                       XtPointer value,
...
static void df_SecondaryNotify(Widget w, XEvent *event, char **params, Cardinal *num_params);
static void df_HandleTargets(Widget w,
                             XtPointer closure,
                             Atom *seltype,
                             Atom *type,
                             XtPointer value,
                             unsigned long *length,
                             int *format);
static void df_ProcessBDragRelease(Widget w, XEvent *event, String *params, Cardinal *num_params);
...
static void df_DropTransferCallback(Widget w,
                                    XtPointer closure,
                                    Atom *seltype,
                                    Atom *type,
                                    XtPointer value,
...
static void df_Initialize(Widget request, Widget new_w, ArgList args, Cardinal *num_args);
static void df_Realize(Widget w, XtValueMask *valueMask, XSetWindowAttributes *attributes);
static void df_Destroy(Widget wid);
static void df_Resize(Widget w);
static XtGeometryResult df_QueryGeometry(Widget w,
                                         XtWidgetGeometry *intended,
                                         XtWidgetGeometry *reply);
static void DataFieldExpose(Widget w, XEvent *event, Region region);
static Boolean df_SetValues(
    Widget old, Widget request, Widget new_w, ArgList args, Cardinal *num_args);
static Boolean DataFieldGetBaselines(Widget w, Dimension **baselines, int *line_count);
...
static XmTextScanType df_sarray[] = {XmSELECT_POSITION, XmSELECT_WORD, XmSELECT_LINE};
...
static XtResource resources[] = {
    ...
    {XmNalignment,
     ...
     XtOffsetOf(XmDataFieldRec, data.alignment),
     XmRImmediate,
...
static XmSyntheticResource syn_resources[] = {
    ...
    {XmNmarginHeight,
     ...
     XmeToVerticalPixels},
    {XmNvalue, sizeof(char *), XtOffsetOf(XmDataFieldRec, text.value), df_MakeCopy, NULL},
    {XmNvalueWcs,
     sizeof(wchar_t *),
     XtOffsetOf(XmDataFieldRec, text.wc_value),
     df_WcsMakeCopy,
...
static void ClassInit(void)
{
  ...
  XmeTraitSet((XtPointer)xmDataFieldWidgetClass, XmQTtransfer, (XtPointer)&tt);
  XmeTraitSet((XtPointer)xmDataFieldWidgetClass, XmQTaccessTextual, (XtPointer)&dataFieldCS);
}
 ...
 */
int _XmDataFieldCountBytes(XmDataFieldWidget tf, wchar_t *wc_value, int num_chars)
{
  wchar_t *wc_ptr;
...
void _XmDataFToggleCursorGC(Widget widget)
{
  ...
  if (!XtIsSensitive(widget)) {
    ...
    if (XmTextF_overstrike(tf)) {
      if (XmTextF_stipple_tile(tf) == XmUNSPECIFIED_PIXMAP)
        return;
                               ...
                               Position *y)
{
  ...
  if (XmDataField_alignment(tf) == XmALIGNMENT_BEGINNING) {
    ...
    *y = 0;
    ...
    int length;
    *x = tf->core.width - (XmTextF_margin_width(tf) + tf->primitive.highlight_thickness +
...
static void df_PaintCursor(XmDataFieldWidget tf)
{
  Position x, y;
  XmTextPosition position;
  if (!XmTextF_cursor_position_visible(tf))
    return;
  if (!XmTextF_has_rect(tf))
    _XmDataFieldSetClipRect(tf);
...
void _XmDataFieldDrawInsertionPoint(XmDataFieldWidget tf, Boolean turn_on)
{
  if (turn_on == True) {
...
static void df_BlinkInsertionPoint(XmDataFieldWidget tf)
{
  if ((XmTextF_cursor_on(tf) >= 0) && XmTextF_blink_on(tf) == df_CurrentCursorState(tf) &&
...
static void df_GetRect(XmDataFieldWidget tf, XRectangle *rect)
{
  Dimension margin_width = XmTextF_margin_width(tf) + tf->primitive.shadow_thickness +
                           tf->primitive.highlight_thickness;
  Dimension margin_top = XmTextF_margin_top(tf) + tf->primitive.shadow_thickness +
                         tf->primitive.highlight_thickness;
  Dimension margin_bottom = XmTextF_margin_bottom(tf) + tf->primitive.shadow_thickness +
                            tf->primitive.highlight_thickness;
...
void _XmDataFieldSetClipRect(XmDataFieldWidget tf)
{
  ...
  if (XmTextF_gc(tf)) {
#if USE_XFT
    if (!XmTextF_have_fontset(tf) && !XmTextF_use_xft(tf) && (XmTextF_font(tf) != NULL)) {
#else
...
static void df_DrawText(XmDataFieldWidget tf, GC gc, int x, int y, char *string, int length)
{
  if (XmTextF_have_fontset(tf)) {
    if (XmTextF_max_char_size(tf) != 1)
      ...
      XmbDrawString(
          XtDisplay(tf), XtWindow(tf), (XFontSet)XmTextF_font(tf), gc, x, y, string, length);
#if USE_XFT
  }
    ...
    if (XmTextF_max_char_size(tf) != 1) { /* was passed a wchar_t*  */
      ...
      if (num_bytes >= 0)
        _XmXftDrawString2(
            XtDisplay(tf), XtWindow(tf), gc, XmTextF_xft_font(tf), 1, x, y, tmp, num_bytes);
      XmStackFree(tmp, stack_cache);
    }
    else /* one byte chars */
      _XmXftDrawString2(
          XtDisplay(tf), XtWindow(tf), gc, XmTextF_xft_font(tf), 1, x, y, string, length);
#endif
  }
...
static int df_FindPixelLength(XmDataFieldWidget tf, char *string, int length)
{
  if (XmTextF_have_fontset(tf)) {
    if (XmTextF_max_char_size(tf) != 1)
      return (XwcTextEscapement((XFontSet)XmTextF_font(tf), (wchar_t *)string, length));
    else /* one byte chars */
                               ...
                               int *x)
{
  ...
  if (XmTextF_max_char_size(tf) != 1) {
    *x += df_FindPixelLength(
        tf, (char *)(XmTextF_wc_value(tf) + prev_seg_start), (int)(seg_start - prev_seg_start));
    x_seg_len = df_FindPixelLength(
        tf, (char *)(XmTextF_wc_value(tf) + seg_start), (int)seg_end - (int)seg_start);
  }
  else {
    *x += df_FindPixelLength(
        tf, XmTextF_value(tf) + prev_seg_start, (int)(seg_start - prev_seg_start));
    x_seg_len = df_FindPixelLength(
        tf, XmTextF_value(tf) + seg_start, (int)seg_end - (int)seg_start);
  }
  if (mode == XmHIGHLIGHT_SELECTED) {
  ...
  if (stipple) {
    ...
    if (tf->text.max_char_size != 1) {
      df_DrawText(tf,
                  ...
                  *x + 1,
                  y + 1,
                  (char *)(XmTextF_wc_value(tf) + seg_start),
  ...
  if (XmTextF_max_char_size(tf) != 1) {
    ...
    df_DrawText(
        tf, XmTextF_gc(tf), *x, y, XmTextF_value(tf) + seg_start, (int)seg_end - (int)seg_start);
  }
  if (stipple)
    df_XmSetNormGC(tf, XmTextF_gc(tf), True, !stipple);
  ...
  if (XmTextF_max_char_size(tf) != 1)
    *x += df_FindPixelLength(
        tf, (char *)(XmTextF_wc_value(tf) + seg_start), (int)(next_seg - (int)seg_start));
  else
...
static void df_RedisplayText(XmDataFieldWidget tf, XmTextPosition start, XmTextPosition end)
{
  ...
  if (XmDataField_alignment(tf) == XmALIGNMENT_END) {
    ...
    if (XmTextF_max_char_size(tf) != 1)
      x -= df_FindPixelLength(tf,
                              (char *)(XmTextF_wc_value(tf) + l[0].position),
                              XmTextF_string_length(tf) - l[0].position);
    else
      x -= df_FindPixelLength(
          tf, XmTextF_value(tf) + l[0].position, XmTextF_string_length(tf) - l[0].position);
  ...
  for (i = 0; i + 1 < XmTextF_highlight(tf).number; i++) {
    ...
    if (l[i].position <= start && start < l[i + 1].position && l[i].position < end) {
      ...
      if (XmTextF_max_char_size(tf) != 1) {
        x += df_FindPixelLength(tf,
                                (char *)(XmTextF_wc_value(tf) + l[i].position),
                                (int)(l[i + 1].position - l[i].position));
      }
      else {
        x += df_FindPixelLength(
            tf, XmTextF_value(tf) + l[i].position, (int)(l[i + 1].position - l[i].position));
      }
    }
  } /* end for loop */
  if (l[i].position < end) {
    ...
    df_DrawTextSegment(
        tf, l[i].mode, l[i].position, start, end, XmTextF_string_length(tf), stipple, y, &x);
  }
  else {
    if (XmTextF_max_char_size(tf) != 1)
      x += df_FindPixelLength(tf,
      ...
      x += df_FindPixelLength(
          tf, XmTextF_value(tf) + l[i].position, XmTextF_string_length(tf) - (int)l[i].position);
  }
  if (x < (int)(rect.x + rect.width) && XmDataField_alignment(tf) == XmALIGNMENT_BEGINNING) {
    df_XmSetInvGC(tf, XmTextF_gc(tf));
    XFillRectangle(XtDisplay(tf),
                   XtWindow(tf),
                   XmTextF_gc(tf),
  ...
  else if (XmTextF_h_offset(tf) < startx && XmDataField_alignment(tf) == XmALIGNMENT_END) {
    df_XmSetInvGC(tf, XmTextF_gc(tf));
    XFillRectangle(XtDisplay(tf),
                   XtWindow(tf),
                   XmTextF_gc(tf),
...
static void df_ComputeSize(XmDataFieldWidget tf, Dimension *width, Dimension *height)
{
  ...
  if (XmTextF_resize_width(tf) && XmTextF_columns(tf) < XmTextF_string_length(tf)) {
    ...
    *width = tmp + (2 * (XmTextF_margin_width(tf) + tf->primitive.shadow_thickness +
                         tf->primitive.highlight_thickness));
  }
  else {
...
static Boolean df_AdjustText(XmDataFieldWidget tf, XmTextPosition position, Boolean flag)
{
  ...
  if (XmDataField_alignment(tf) == XmALIGNMENT_END) {
    if (XmTextF_max_char_size(tf) != 1) {
      left_edge = tf->core.width - margin_width + XmTextF_h_offset(tf) -
                  df_FindPixelLength(tf,
                                     (char *)(XmTextF_wc_value(tf) + position),
                                     XmTextF_string_length(tf) - (int)position);
    }
    else {
      left_edge = tf->core.width - margin_width + XmTextF_h_offset(tf) -
                  df_FindPixelLength(
                      tf, XmTextF_value(tf) + position, XmTextF_string_length(tf) - (int)position);
    }
  }
  else {
    if (XmTextF_max_char_size(tf) != 1) {
      left_edge = df_FindPixelLength(tf, (char *)XmTextF_wc_value(tf), (int)position) +
  ...
  if (flag)
    df_RedisplayText(tf, position, XmTextF_string_length(tf));
  return False;
}
...
static void df_AdjustSize(XmDataFieldWidget tf)
{
  ...
  Dimension margin_width = XmTextF_margin_width(tf) + tf->primitive.shadow_thickness +
                           tf->primitive.highlight_thickness;
  if (XmTextF_max_char_size(tf) != 1) {
    left_edge = df_FindPixelLength(tf, (char *)XmTextF_wc_value(tf), XmTextF_string_length(tf)) +
                margin_width;
  }
  else {
    left_edge = df_FindPixelLength(tf, XmTextF_value(tf), XmTextF_string_length(tf)) +
                margin_width;
  }
                               ...
                               int *free_insert)
{
  ...
  *free_insert = (int)False;
   ...
   */
  if (XmTextF_wcs_modify_verify_callback(tf)) { /* use wcs_vcb data */
...
static void df_ResetClipOrigin(XmDataFieldWidget tf, Boolean clip_mask_reset)
{
  unsigned long valuemask = (GCTileStipXOrigin | GCTileStipYOrigin | GCClipXOrigin |
                             GCClipYOrigin);
  XGCValues values;
  ...
  if (x < (int)(tf->primitive.highlight_thickness + tf->primitive.shadow_thickness +
                XmTextF_margin_width(tf)))
  {
    clip_mask_x = tf->primitive.highlight_thickness + tf->primitive.shadow_thickness +
                  (int)(XmTextF_margin_width(tf));
  }
  else
    clip_mask_x = x;
  if (clip_mask_reset) {
 ...
 */
void _XmDataFielddf_SetCursorPosition(XmDataFieldWidget tf,
                                      XEvent *event,
                                      XmTextPosition position,
                                      Boolean adjust_flag,
                                      Boolean call_cb)
                                 ...
                                 Boolean set_dest)
{
  ...
  if (position == hl_list[i].position)
    ...
    df_InvertImageGC(tf);
  if (adjust_flag)
    (void)df_AdjustText(tf, position, flag);
  df_ResetClipOrigin(tf, False);
  XmTextF_refresh_ibeam_off(tf) = True;
  _XmDataFieldDrawInsertionPoint(tf, True);
  ...
  if (set_dest)
    (void)df_SetDestination((Widget)tf,
 ...
 */
Boolean _XmDataFieldReplaceText(XmDataFieldWidget tf,
                                XEvent *event,
                                XmTextPosition replace_prev,
                                XmTextPosition replace_next,
                                char *insert,
                                ...
                                Boolean move_cursor)
{
  ...
  if (XmTextF_has_primary(tf) && XmTextF_prim_pos_left(tf) != XmTextF_prim_pos_right(tf))
    XmDataFieldSetHighlight(
        (Widget)tf, XmTextF_prim_pos_left(tf), XmTextF_prim_pos_right(tf), XmHIGHLIGHT_NORMAL);
  _XmDataFieldDrawInsertionPoint(tf, False);
  ...
  if (XmTextF_max_char_size(tf) == 1) {
     ...
     */
    if (insert_length != 0) {
      for (src = insert, dst = XmTextF_value(tf) + replace_prev, i = insert_length; i > 0;
           ++src, ++dst, --i)
        *dst = *src;
    }
  }
  ...
  if (XmTextF_has_primary(tf) && XmTextF_prim_pos_left(tf) != XmTextF_prim_pos_right(tf)) {
    XmDataFieldSetHighlight(
        (Widget)tf, XmTextF_prim_pos_left(tf), XmTextF_prim_pos_right(tf), XmHIGHLIGHT_SELECTED);
  }
...
void _XmDataFieldDeselectSelection(Widget w, Boolean disown, Time sel_time)
{
  ...
  if (tf != NULL) {
    ...
    DataFieldSetHighlight(
        tf, XmTextF_prim_pos_left(tf), XmTextF_prim_pos_right(tf), XmHIGHLIGHT_NORMAL);
    XmTextF_prim_pos_left(tf) = XmTextF_prim_pos_right(tf) = XmTextF_prim_anchor(
        tf) = XmTextF_cursor_position(tf);
...
static XmTextPosition df_GetPosFromX(XmDataFieldWidget tf, Position x)
{
  ...
  for (position = 0; temp_x + next_char_width / 2 < (int)x && position < XmTextF_string_length(tf);
       position++)
  {
...
{
  XmTextPosition position;
  int margin_width = XmTextF_margin_width(tf) + tf->primitive.highlight_thickness +
                     tf->primitive.shadow_thickness;
  int temp_x;
  int next_char_width = 0;
  position = XmTextF_string_length(tf);
  ...
  for (; x < (temp_x - next_char_width / 2) && (position > 0); position--) {
    ...
    if (position > 1) {
      if (XmTextF_max_char_size(tf) != 1) {
        next_char_width = df_FindPixelLength(tf, (char *)(XmTextF_wc_value(tf) + position - 2), 1);
      }
      else {
        next_char_width = df_FindPixelLength(tf, XmTextF_value(tf) + position - 2, 1);
      }
    }
  } /* for */
#if PWC_DEBUG
...
static Boolean df_SetDestination(Widget w, XmTextPosition position, Boolean disown, Time set_time)
{
  ...
  if (!disown) {
    if (!XmTextF_has_destination(tf)) {
      if (!set_time)
        set_time = df_GetServerTime(w);
      result = XtOwnSelection(w,
                              MOTIF_DESTINATION,
                              set_time,
                              _XmDataFieldConvert,
                                          ...
                                          XmTextPosition pos2)
{
  ...
  if (XmTextF_max_char_size(tf) == 1) { /* data is char* and one-byte per char */
    if (isspace(XmTextF_value(tf)[pos1]) || isspace(XmTextF_value(tf)[pos2]))
      return True;
  }
  else {
...
static void df_FindNextWord(XmDataFieldWidget tf, XmTextPosition *left, XmTextPosition *right)
{
  ...
  if (XmTextF_max_char_size(tf) == 1) {
    if (isspace(XmTextF_value(tf)[end])) {
      for (end = XmTextF_cursor_position(tf); end < XmTextF_string_length(tf); end++) {
        if (!isspace(XmTextF_value(tf)[end])) {
          break;
        }
      }
...
static void df_CheckDisjointSelection(Widget w, XmTextPosition position, Time sel_time)
{
  ...
  if (XmTextF_add_mode(tf) || (XmDataFieldGetSelectionPosition(w, &left, &right) &&
...
static Boolean df_NeedsPendingDelete(XmDataFieldWidget tf)
{
  return (XmTextF_add_mode(tf) ? (XmTextF_pending_delete(tf) && XmTextF_has_primary(tf) &&
                                  XmTextF_prim_pos_left(tf) != XmTextF_prim_pos_right(tf) &&
...
static Boolean df_NeedsPendingDeleteDisjoint(XmDataFieldWidget tf)
{
  return (XmTextF_pending_delete(tf) && XmTextF_has_primary(tf) &&
          XmTextF_prim_pos_left(tf) != XmTextF_prim_pos_right(tf) &&
          XmTextF_prim_pos_left(tf) <= XmTextF_cursor_position(tf) &&
          XmTextF_prim_pos_right(tf) >= XmTextF_cursor_position(tf));
}
...
static Boolean PrintableString(XmDataFieldWidget tf,
                               char *str,
                               int n,
...
static void df_InsertChar(Widget w, XEvent *event, char **params, Cardinal *num_params)
{
  ...
  if (insert_length > 0) {
    ...
    if (XmTextF_max_char_size(tf) == 1) {
      ...
      if (nextPos > XmTextF_string_length(tf))
        nextPos = XmTextF_string_length(tf);
    ...
    if (replace_res) {
      if (pending_delete) {
        XmDataFieldSetSelection(
            w, XmTextF_cursor_position(tf), XmTextF_cursor_position(tf), event->xkey.time);
      }
...
static void df_DeleteToEndOfLine(Widget w, XEvent *event, char **params, Cardinal *num_params)
{
  ...
  if (df_NeedsPendingDelete(tf))
    ...
    if (_XmDataFieldReplaceText(
            tf, event, XmTextF_cursor_position(tf), XmTextF_string_length(tf), NULL, 0, True))
    {
      df_CheckDisjointSelection(w, XmTextF_cursor_position(tf), event->xkey.time);
      _XmDataFielddf_SetCursorPosition(tf, event, XmTextF_cursor_position(tf), False, True);
      cb.reason = XmCR_VALUE_CHANGED;
    ...
    XmDataFieldWidget tf, XEvent *event, XmTextPosition position, Time time, Boolean extend)
{
  ...
  if ((XmTextF_add_mode(tf) && XmDataFieldGetSelectionPosition((Widget)tf, &left, &right) &&
                            ...
                            Boolean redisplay)
{
  ...
  DataFieldSetHighlight(
      tf, XmTextF_prim_pos_left(tf), XmTextF_prim_pos_right(tf), XmHIGHLIGHT_SELECTED);
  if (redisplay) {
    if (old_prim_left > XmTextF_prim_pos_left(tf)) {
      ...
      display_left = (old_prim_right > XmTextF_prim_pos_right(tf)) ? XmTextF_prim_pos_right(tf) :
                                                                     old_prim_right;
    if (old_prim_right < XmTextF_prim_pos_right(tf)) {
      ...
      display_right = (old_prim_left < XmTextF_prim_pos_left(tf)) ? XmTextF_prim_pos_left(tf) :
                                                                    old_prim_left;
 ...
 */
void _XmDataFieldStartSelection(XmDataFieldWidget tf,
                                XmTextPosition left,
                                XmTextPosition right,
                                Time sel_time)
{
...
static void df_TextFocusIn(Widget w, XEvent *event, char **params, Cardinal *num_params)
{
  ...
  if (event->xfocus.send_event && !(XmTextF_has_focus(tf))) {
    ...
    if (_XmGetFocusPolicy(w) == XmEXPLICIT) {
      ...
      if (!XmTextF_has_destination(tf)) {
        (void)df_SetDestination(
            w, XmTextF_cursor_position(tf), False, XtLastTimestampProcessed(XtDisplay(w)));
      }
    }
...
static void df_ExtendScanSelection(XmDataFieldWidget tf, XEvent *event)
{
  ...
  float bal_point;
  if (!XmDataFieldGetSelectionPosition((Widget)tf, &left, &right) || left == right) {
    XmTextF_orig_left(tf) = XmTextF_orig_right(tf) = XmTextF_prim_anchor(
        tf) = XmTextF_cursor_position(tf);
...
static void df_DoExtendedSelection(Widget w, Time time)
{
  ...
  if (!(XmDataFieldGetSelectionPosition(w, &left, &right)) || left == right) {
    ...
    bal_point = (float)(((float)(XmTextF_orig_right(tf) - XmTextF_orig_left(tf)) / 2.0) +
...
{
  XmDataFieldWidget tf = (XmDataFieldWidget)w;
  Dimension margin_size = XmTextF_margin_width(tf) + tf->primitive.shadow_thickness +
                          tf->primitive.highlight_thickness;
  Dimension top_margin = XmTextF_margin_height(tf) + tf->primitive.shadow_thickness +
                         tf->primitive.highlight_thickness;
  ...
  if ((event->xmotion.x > (int)margin_size) &&
      ...
      (event->xmotion.y < (int)(top_margin + XmTextF_font_ascent(tf) + XmTextF_font_descent(tf))))
  {
    if (XmTextF_select_id(tf)) {
      XtRemoveTimeOut(XmTextF_select_id(tf));
      XmTextF_select_id(tf) = 0;
    ...
    Widget w, XmTextPosition left, XmTextPosition right, Boolean disown, Time sel_time)
{
  ...
  DataFieldSetHighlight((XmDataFieldWidget)w,
                        XmTextF_sec_pos_left(tf),
...
static void df_ProcessBDrag(Widget w, XEvent *event, char **params, Cardinal *num_params)
{
  ...
  if (XmDataFieldGetSelectionPosition(w, &left, &right) && left != right) {
    if ((position > left && position < right) ||
        ...
        (position == left && df_GetXYFromPos(tf, left, &left_x, &dummy) &&
         event->xbutton.x > left_x) ||
        (position == right && df_GetXYFromPos(tf, right, &right_x, &dummy) &&
                       ...
                       int *format)
{
  ...
  if (!(*length) && *type != NULL_ATOM) {
    ...
    if (prim_select->target == XmInternAtom(XtDisplay(w), "TEXT", False)) {
      ...
      XtGetSelectionValue(
          w, XA_PRIMARY, XA_STRING, df_DoStuff, (XtPointer)prim_select, prim_select->time);
    }
    XtFree((char *)value);
    value = NULL;
    return;
  ...
  if (*type == NULL_ATOM) {
    if (prim_select->num_chars > 0 && XmTextF_selection_move(tf)) {
      ...
      XmDataFieldSetSelection(
          w, prim_select->position, prim_select->position + prim_char_length, prim_select->time);
...
static void df_HandleSelectionReplies(Widget w, XtPointer closure, XEvent *event, Boolean *cont)
{
  ...
  XtRemoveEventHandler(w,
                       ...
                       (XtPointer)XmInternAtom(XtDisplay(w), "_XM_TEXT_I_S_PROP", False));
  ...
  if (event->xselection.property == None) {
    ...
    if (dest_data->has_destination) {
      ...
      XmDataFieldSetHighlight(
          w, XmTextF_sec_pos_left(tf), XmTextF_sec_pos_right(tf), XmHIGHLIGHT_NORMAL);
                             ...
                             int *format)
{
  ...
  if (XmDataFieldGetSelectionPosition(w, &left, &right) && left != right && select_pos > left &&
      select_pos < right)
  {
    XtFree((char *)value);
    value = NULL;
...
static void df_PageRight(Widget w, XEvent *event, char **params, Cardinal *num_params)
{
  ...
  if (length - ((int)(tf->core.width - (2 * margin_width)) - XmTextF_h_offset(tf)) >
...
static void df_PageLeft(Widget w, XEvent *event, char **params, Cardinal *num_params)
{
  Position x, y;
  XmDataFieldWidget tf = (XmDataFieldWidget)w;
  int margin_width = (int)XmTextF_margin_width(tf) + tf->primitive.shadow_thickness +
                     tf->primitive.highlight_thickness;
  _XmDataFieldDrawInsertionPoint(tf, False);
  if (*num_params > 0 && !strcmp(*params, "extend"))
    df_SetAnchorBalancing(tf, XmTextF_cursor_position(tf));
...
static void df_SetAnchor(Widget w, XEvent *event, char **params, Cardinal *num_params)
{
  ...
  if (XmDataFieldGetSelectionPosition(w, &left, &right)) {
    _XmDataFieldStartSelection(
        tf, XmTextF_prim_anchor(tf), XmTextF_prim_anchor(tf), event->xkey.time);
    XmDataFieldSetAddMode(w, False);
  }
}
...
static void df_TextEnter(Widget w, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (_XmGetFocusPolicy(w) != XmEXPLICIT && !(XmTextF_has_focus(tf)) && event->xcrossing.focus &&
  ...
  if (!is_wchar) {
    ...
    *temp_str = '\0';
    ...
    if (XmTextF_max_char_size(tf) == 1) {
      ...
      XmTextF_value(tf) = memcpy(XtMalloc(XmTextF_string_length(tf) + 30),
                                 (void *)start_temp,
      ...
      XmTextF_string_length(tf) = mbstowcs(
          XmTextF_wc_value(tf), start_temp, XmTextF_string_length(tf) + 30);
...
static void df_InitializeTextStruct(XmDataFieldWidget tf)
{
  ...
  if (XmTextF_font_list(tf) == NULL) {
    XmTextF_font_list(tf) = XmeGetDefaultRenderTable((Widget)tf, (unsigned char)XmTEXT_FONTLIST);
  ...
  XmTextF_orig_left(tf) = XmTextF_orig_right(tf) = XmTextF_prim_pos_left(
      tf) = XmTextF_prim_pos_right(tf) = XmTextF_prim_anchor(tf) = XmTextF_cursor_position(tf);
  XmTextF_sec_pos_left(tf) = XmTextF_sec_pos_right(tf) = XmTextF_sec_anchor(
      tf) = XmTextF_cursor_position(tf);
...
static Pixmap df_GetClipMask(XmDataFieldWidget tf, char *pixmap_name)
{
  ...
  GC fillGC;
  Pixmap clip_mask;
  clip_mask = XCreatePixmap(
      dpy, RootWindowOfScreen(screen), XmTextF_cursor_width(tf), XmTextF_cursor_height(tf), 1);
  values.foreground = 1;
  values.background = 0;
  fillGC = XCreateGC(dpy, clip_mask, GCForeground | GCBackground, &values);
  XFillRectangle(
      dpy, clip_mask, fillGC, 0, 0, XmTextF_cursor_width(tf), XmTextF_cursor_height(tf));
...
static void df_LoadGCs(XmDataFieldWidget tf, Pixel background, Pixel foreground)
{
  ...
  XmTextF_stipple_tile(tf) = (Pixmap)XmGetPixmapByDepth(XtScreen(tf),
                                                        XmS50_foreground,
  ...
  XmTextF_save_gc(tf) = XtAllocateGC(
      (Widget)tf, tf->core.depth, value_mask, &values, dynamic_mask, 0);
  ...
  XmTextF_gc(tf) = XtAllocateGC((Widget)tf, tf->core.depth, value_mask, &values, dynamic_mask, 0);
  ...
  dynamic_mask = (GCForeground | GCBackground | GCStipple | GCFillStyle | GCTileStipXOrigin |
                  GCTileStipYOrigin | GCFunction | GCClipMask | GCClipXOrigin | GCClipYOrigin);
  XmTextF_image_gc(tf) = XtAllocateGC(
      (Widget)tf, tf->core.depth, value_mask, &values, dynamic_mask, 0);
}
...
static void df_MakeIBeamOffArea(XmDataFieldWidget tf, Dimension width, Dimension height)
{
   ...
   * be painted */
  XmTextF_ibeam_off(tf) = XCreatePixmap(
...
static void df_MakeIBeamStencil(XmDataFieldWidget tf, int line_width)
{
  ...
  if (XmTextF_cursor(tf) == XmUNSPECIFIED_PIXMAP) {
    ...
    XmTextF_cursor(tf) = XCreatePixmap(
        dpy, XtWindow(tf), XmTextF_cursor_width(tf), XmTextF_cursor_height(tf), 1);
  ...
  if (XmTextF_add_mode_cursor(tf) == XmUNSPECIFIED_PIXMAP) {
    ...
    if (pixmap != XmUNSPECIFIED_PIXMAP) {
      XGetGeometry(XtDisplay(tf),
                   ...
                   &unused);
      image = XGetImage(
          XtDisplay(tf), (Drawable)pixmap, 0, 0, pix_width, pix_height, AllPlanes, XYPixmap);
      stipple = XCreatePixmap(dpy, XtWindow(tf), image->width, image->height, 1);
      XmTextF_add_mode_cursor(tf) = XCreatePixmap(
          dpy, XtWindow(tf), XmTextF_cursor_width(tf), XmTextF_cursor_height(tf), 1);
      ...
      _XmCachePixmap(XmTextF_add_mode_cursor(tf), XtScreen(tf), pixmap_name, 1, 0, 0, 0, 0);
...
static void df_MakeCursors(XmDataFieldWidget tf)
{
  ...
  df_MakeIBeamOffArea(tf,
                      MAX(XmTextF_cursor_height(tf) >> 1, XmTextF_cursor_height(tf)),
                      XmTextF_cursor_height(tf));
                                    ...
                                    int *format)
{
  ...
  if (*type == XmInternAtom(XtDisplay(transfer_rec->widget), "NULL", False)) {
    if (transfer_rec->num_chars > 0 && transfer_rec->move) {
      ...
      (void)df_SetDestination(
          (Widget)tf, XmTextF_cursor_position(tf), False, transfer_rec->timestamp);
      XmDataFieldSetSelection((Widget)tf,
                              XmTextF_prim_anchor(tf),
                              XmTextF_cursor_position(tf),
  ...
  status = XmbTextListToTextProperty(
      XtDisplay(transfer_rec->widget), &tmp_string, 1, (XICCEncodingStyle)XTextStyle, &tmp_prop);
  if (status == Success)
    CS_OF_LOCALE = tmp_prop.encoding;
  else
    CS_OF_LOCALE = 99999; /* XmbTextList... should never fail for XPCS
  ...
  if (XmTextF_pending_delete(tf) && XmTextF_has_primary(tf) &&
      XmTextF_prim_pos_left(tf) != XmTextF_prim_pos_right(tf) &&
      insertPosLeft > XmTextF_prim_pos_left(tf) && insertPosRight < XmTextF_prim_pos_right(tf))
  {
...
static void df_HandleDrop(Widget w, XmDropProcCallbackStruct *cb)
{
  ...
  if (cb->operation & XmDROP_MOVE && w == initiator &&
      XmDataFieldGetSelectionPosition(w, &left, &right) && left != right && insert_pos >= left &&
      insert_pos <= right)
  {
    XtSetArg(args[n], XmNtransferStatus, XmTRANSFER_FAILURE);
    n++;
    ...
    int status;
    status = XmbTextListToTextProperty(
        display, &tmp_string, 1, (XICCEncodingStyle)XTextStyle, &tmp_prop);
    if (status == Success)
      CS_OF_LOCALE = tmp_prop.encoding;
    else
      CS_OF_LOCALE = 99999; /* XmbTextList... should never fail for XPCS
    ...
    transfer_rec = (_XmTextDropTransferRec *)XtMalloc(sizeof(_XmTextDropTransferRec));
    ...
    for (n = 0; n < numExportTargets; n++) {
      ...
      if (exportTargets[n] == COMPOUND_TEXT)
        c_text_found = True;
#if XM_UTF8
      if (exportTargets[n] == UTF8_STRING)
        utf8_string_found = True;
#endif
...
static void df_DragProcCallback(Widget w, XtPointer client, XtPointer call)
{
  ...
  switch (cb->reason) {
    case XmCR_DROP_SITE_ENTER_MESSAGE:
      if (XmTargetsAreCompatible(XtDisplay(drag_cont),
                                 exp_targets,
#if XM_UTF8
                                 num_exp_targets,
                                 targets,
...
static void df_Resize(Widget w)
{
  ...
  if (XmDataField_alignment(tf) == XmALIGNMENT_END)
    ...
    XmTextF_h_offset(tf) = XmTextF_margin_width(tf) + tf->primitive.shadow_thickness +
                           tf->primitive.highlight_thickness;
  XmTextF_refresh_ibeam_off(tf) = True;
                                         ...
                                         XtWidgetGeometry *desired)
{
  ...
  df_ComputeSize((XmDataFieldWidget)widget, &desired->width, &desired->height);
    ...
    Widget old, Widget request, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (XmDataField_alignment(old_tf) != XmDataField_alignment(new_tf)) {
    if (XmDataField_alignment(new_tf) == XmALIGNMENT_END)
      ...
      XmTextF_h_offset(new_tf) = XmTextF_new_h_offset(
          new_tf) = XmTextF_margin_width(new_tf) + new_tf->primitive.shadow_thickness +
  ...
  if (XmTextF_cursor_position(new_tf) != XmTextF_cursor_position(old_tf)) {
    ...
    (void)df_SetDestination(
        new_w, XmTextF_cursor_position(new_tf), False, XtLastTimestampProcessed(XtDisplay(new_w)));
    cursor_pos_set = True;
  }
  else {
    int ix;
  ...
  if (new_w->core.sensitive == False && XmTextF_has_destination(new_tf)) {
    (void)df_SetDestination(
        new_w, XmTextF_cursor_position(new_tf), True, XtLastTimestampProcessed(XtDisplay(new_w)));
  }
  ...
  if (diff_values) { /* old value != new value */
    ...
    if (XmTextF_modify_verify_callback(new_tf) || XmTextF_wcs_modify_verify_callback(new_tf)) {
      ...
      if (XmTextF_max_char_size(new_tf) == 1) {
        ...
        mod_ver_ret = df_ModifyVerify(new_tf,
                                      NULL,
                                      &fromPos,
                                      &toPos,
                                      &temp,
                                      &XmTextF_string_length(new_tf),
                                      &newInsert,
                                      &free_insert);
      }
      else {
      ...
      if (free_insert)
        XtFree(temp);
      if (!mod_ver_ret) {
        if (XmTextF_verify_bell(new_tf))
          XBell(XtDisplay(new_w), 0);
        if (XmTextF_max_char_size(new_tf) == 1) {
          XmTextF_value(new_tf) = memcpy(
              XtRealloc(XmTextF_value(new_tf), (unsigned)XmTextF_size_allocd(old_tf)),
          ...
          XmTextF_wc_value(new_tf) = memcpy(
              XtRealloc((void *)XmTextF_wc_value(new_tf), (unsigned)XmTextF_size_allocd(old_tf)),
    ...
    if (do_it) {
      ...
      if (XmTextF_max_char_size(new_tf) == 1)
        ...
        XtFree((char *)XmTextF_wc_value(old_tf));
      XmDataFieldSetHighlight(new_w,
                              XmTextF_prim_pos_left(new_tf),
      ...
      if (!cursor_pos_set || new_position > XmTextF_string_length(old_tf)) {
        ...
        if (XmTextF_has_destination(new_tf))
          (void)df_SetDestination(new_w,
                                  XmTextF_cursor_position(new_tf),
                                  False,
  ...
  if (XtIsSensitive((Widget)new_tf) != XtIsSensitive((Widget)old_tf)) {
    ...
    if (XmTextF_string_length(new_tf) > 0)
      redisplay = True;
  }
  ...
  if (new_font)
    XmFontListFree((XmFontList)XmTextF_font_list(old_tf));
  if (!redisplay)
    redisplay = XmTextF_redisplay(new_tf);
   ...
   * widget.  */
  if (redisplay)
    XmTextF_refresh_ibeam_off(new_tf) = True;
...
void XmDataFieldReplace(Widget w, XmTextPosition from_pos, XmTextPosition to_pos, char *value)
{
  ...
  if (XmTextF_has_primary(tf)) {
    if ((XmTextF_prim_pos_left(tf) > from_pos && XmTextF_prim_pos_left(tf) < to_pos) ||
                           ...
                           wchar_t *wc_value)
{
  ...
  if (XmTextF_max_char_size(tf) != 1) {
    rep_result = _XmDataFieldReplaceText(
        tf, NULL, from_pos, to_pos, wc_value ? (char *)wc_value : "", wc_length, False);
  }
...
wchar_t *XmDataFieldGetSelectionWcs(Widget w)
{
  ...
  if (XmTextF_max_char_size(tf) == 1) {
    ...
    memcpy(wc_value, (XmTextF_wc_value(tf) + XmTextF_prim_pos_left(tf)), length * sizeof(wchar_t));
  }
...
Boolean XmDataFieldCopy(Widget w, Time clip_time)
{
  ...
  if (selected_string != NULL) {
    clip_label = XmStringLtoRCreate("XM_TEXT_FIELD", XmFONTLIST_DEFAULT_TAG);
    /* start copy to clipboard */
    status = XmClipboardStartCopy(
        display, window, clip_label, clip_time, w, (XmCutPasteProc)NULL, &item_id);
    ...
    status = XmbTextListToTextProperty(
        display, &selected_string, 1, (XICCEncodingStyle)XStdICCTextStyle, &tmp_prop);
    ...
    status = XmClipboardCopy(display,
                             window,
                             item_id,
                             atom_name,
...
Boolean XmDataFieldPaste(Widget w)
{
  ...
  if (status == ClipboardNoData || length == 0) {
    status = XmClipboardInquireLength(display, window, "COMPOUND_TEXT", &length);
    if (status == ClipboardNoData || length == 0) {
#if XM_UTF8
  ...
  if (!get_ct && !get_us) {
    ...
    status = XmClipboardRetrieve(
        display, window, XmSUTF8_STRING, buffer, length, &outlength, &private_id);
  }
#endif
  else {
    status = XmClipboardRetrieve(
        display, window, "COMPOUND_TEXT", buffer, length, &outlength, &private_id);
  }
  if (status != ClipboardSuccess) {
  ...
  if (num_vals && (status == Success || status > 0)) {
    if (XmTextF_max_char_size(tf) == 1) {
      ...
      rep_status = _XmDataFieldReplaceText(
          tf, NULL, paste_pos_left, paste_pos_right, (char *)wc_value, num_chars, True);
      if (malloc_size)
        XtFree((char *)wc_value);
    }
  }
  if (rep_status) {
    ...
    (void)df_SetDestination(
        w, XmTextF_cursor_position(tf), False, XtLastTimestampProcessed(display));
...
int XmDataFieldGetBaseline(Widget w)
{
  ...
  _XmWidgetToAppContext(w);
  _XmAppLock(app);
  margin_top = XmTextF_margin_top(tf) + tf->primitive.shadow_thickness +
               tf->primitive.highlight_thickness;
...
{
  XmDataFieldWidget tf = (XmDataFieldWidget)w;
  Position margin_width = XmTextF_margin_width(tf) + tf->primitive.shadow_thickness +
                          tf->primitive.highlight_thickness;
  Position margin_top = XmTextF_margin_top(tf) + tf->primitive.shadow_thickness +
                        tf->primitive.highlight_thickness;
  Position margin_bottom = XmTextF_margin_bottom(tf) + tf->primitive.shadow_thickness +
                           tf->primitive.highlight_thickness;
