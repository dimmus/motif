   ...
   XmScrollFrameData scroll_frame_data)
{
       ...
       != NULL)
   {
      ...
      if (scroll_frame_data->move_cb)
         nav_trait->changeMoveCB(nav, scroll_frame_data->move_cb, (XtPointer)scroll_frame_data->scrollable, True);
      ...
      if (scroll_frame_data->num_nav_list == scroll_frame_data->num_nav_slots)
      {
         ...
         scroll_frame_data->nav_list       = (WidgetList)XtRealloc((char *)scroll_frame_data->nav_list,
   ...
   XmScrollFrameData scroll_frame_data)
{
       ...
       != NULL)
   {
      ...
      if (scroll_frame_data->move_cb)
         nav_trait->changeMoveCB(nav, scroll_frame_data->move_cb, (XtPointer)scroll_frame_data->scrollable, False);
   }
   else
   {
   ...
   Boolean         notify)
{
   ...
   for (i = 0; i < num_nav_list; i++)
   {
      ...
      XmNavigatorSetValueProc nav_setValue = ((XmNavigatorTrait)
                                                 XmeTraitGet((XtPointer)XtClass(nav), XmQTnavigator))
                                                ->setValue;
