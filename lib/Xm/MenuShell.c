...
externaldef(xmmenushellwidgetclass) WidgetClass xmMenuShellWidgetClass = (WidgetClass)&xmMenuShellClassRec;
   ...
   if (!shell_widget->shell.popped_up)
   {
      ...
      if (XmIsMenuShell(shell_widget) && ((XmMenuShellWidget)shell_widget)->menu_shell.animate)
      {
         if (XmIsRowColumn(shell_widget->composite.children[0]))
         {
            ...
            slider = XtVaCreateWidget("MenuSlider", xmSlideContextWidgetClass, XmGetXmDisplay(XtDisplay(widget)), XmNslideWidget, widget, XmNslideDestWidth, width, XmNslideDestHeight, height, NULL);
   ...
   Cardinal *num_args)
{
   ...
   /*
     ...
     */
   XtInsertEventHandler((Widget)ms, (EventMask)StructureNotifyMask, True, StructureNotifyHandler, (XtPointer)NULL, XtListHead);
}
...
*/
   {
      ...
      XmeConfigureObject(child, -ms->core.border_width, -ms->core.border_width, ms->core.width, ms->core.height, ms->core.border_width);
   }
}
   ...
   Widget widget)
{
   ...
   else if ((parent->composite.num_children == 0) && (parent->menu_shell.private_shell) && (!parent->core.being_destroyed))
   {
   ...
   Widget widget)
{
	...
	*/
      for (i = 0, num_real_children = 0;
           i < parent->composite.num_children;
           i++)
   ...
   register Position         *y)
{
    ...
    * off the button.
    */
   if (pulldown_button && XtParent(pulldown_button) && XmIsRowColumn(XtParent(pulldown_button)) && (RC_Type(XtParent(pulldown_button)) == XmMENU_OPTION))
   {
      Position old_x = *x;
   ...
   if (bottomEdgeOfMenu >= (Position)dispHeight)
   {
      if (pulldown_button && XtParent(pulldown_button) && XmIsRowColumn(XtParent(pulldown_button)) && (RC_Type(XtParent(pulldown_button)) == XmMENU_BAR))
      {
         Position y_temp = *y;
   ...
   XEvent *event)
{
   ...
   if (popped_up || (old_cascadebtn && RC_TornOff(XtParent(old_cascadebtn)) && !XmIsMenuShell(XtParent(XtParent(old_cascadebtn)))))
   {
   ...
   Widget w)
{
   ...
   if (XtIsManaged((Widget)rowcol))
   {
	  ...
	  */
      ...
      else if (RC_Type(rowcol) == XmMENU_POPUP)
      {
         ...
         if ((mst->RC_ButtonEventStatus.time == XtLastTimestampProcessed(XtDisplay(rowcol))) && !mst->RC_ButtonEventStatus.verified)
         {
	  ...
	  */
         ...
         case XmMENU_PULLDOWN:
            ...
            if (!_XmGetInDragMode((Widget)rowcol))
            {
               ...
               if ((parent_menu->manager.active_child != (Widget)cascadebtn) && ((RC_Type(parent_menu) == XmMENU_POPUP) || (RC_Type(parent_menu) == XmMENU_PULLDOWN)))
               {
   ...
   else
   {
      ...
      (*(((XmMenuShellClassRec *)(popup->core.widget_class))->menu_shell_class.popdownEveryone))((Widget)popup, NULL, NULL, NULL);
   ...
   Widget widget)
{
   ...
   if (shell_widget->shell.popped_up)
   {
      ...
      XWithdrawWindow(XtDisplay(shell_widget), XtWindow(shell_widget), XScreenNumberOfScreen(XtScreen(shell_widget)));
   ...
   Boolean  *cont)
{
   ...
   /*
     ...
     */
   if (XmIsCascadeButtonGadget(cascadebtn) && (cascadebtn == XmObjectAtPoint((Widget)rowcol, enterevent->x, enterevent->y)))
   {
      return;
   }
   else if (XmIsCascadeButton(cascadebtn))
   {
      ...
      else
      {
         ...
         if ((enterevent->x_root >= x) && (enterevent->x_root < (x + XtWidth(cascadebtn))) && (enterevent->y_root >= y) && (enterevent->y_root < (y + XtHeight(cascadebtn))))
         {
            return;
         }
      }
   ...
   Cardinal *num_params)
{
   ...
   else if ((RC_Type(rowcol) == XmMENU_POPUP) || ((RC_Type(rowcol) == XmMENU_PULLDOWN) && !XmIsMenuShell(XtParent(rowcol))))
   {
   ...
   if (ms == tms)
      (*(((XmMenuShellClassRec *)(ms->core.widget_class))->menu_shell_class.popdownDone))(widget, event, params, num_params);
   else
   {
      (*(((XmMenuShellClassRec *)(ms->core.widget_class))->menu_shell_class.popdownEveryone))((Widget)ms, event, params, num_params);
   }
   if (event)
   ...
   Cardinal *num_params) /* unused */
{
   ...
   /* Only reset the focus if the this is not a tear off or the cascaded
     ...
     */
   if ((RC_Type(rowcol) == XmMENU_PULLDOWN) && XmIsMenuShell(XtParent(rowcol)) && RC_CascadeBtn(rowcol) && XmIsMenuShell(XtParent(XtParent(RC_CascadeBtn(rowcol)))))
   {
   ...
   Cardinal *num_params)
{
   ...
   /* Take menus out of drag mode in either case of click into traversal or
     ...
     */
   if ((RC_Type(rowcol) == XmMENU_BAR) || (RC_Type(rowcol) == XmMENU_OPTION) || XmIsMenuShell(XtParent(rowcol)))
   {
   ...
   /* This is to enable Option and Popup Menus to post on BMenu/BSelect Click.
     ...
     */
   if (RC_popupMenuClick(rowcol) && event && ((event->type == ButtonPress) || (event->type == ButtonRelease)) && ((event->xbutton.time - mst->MS_LastManagedMenuTime) < XtGetMultiClickTime(XtDisplay(ms)))) /* or 150 ms? */
   {
   ...
   {
      (*(((XmMenuShellClassRec *)(RC_ParentShell(rowcol)->core.widget_class))->menu_shell_class.popdownEveryone))((Widget)ms, event, params, num_params);
   }
   ...
   XEvent           *event) /* unused */
{
   ...
   if (activeChild)
   {
      ...
      {
         (*((XmLabelWidgetClass)XtClass(activeChild))->primitive_class.border_unhighlight)(activeChild);
      }
   }
   /*
   ...
   Cardinal *num_params) /* unused */
{
   ...
   if (!_XmIsEventUnique(event))
      return;
   /* if a valid event */
   if ((ms->composite.num_children != 0) && (rowcol = (XmRowColumnWidget)ms->composite.children[0]) && (_XmMatchBtnEvent(event, RC_PostEventType(rowcol), RC_PostButton(rowcol), RC_PostModifiers(rowcol)) || (_XmMatchBSelectEvent((Widget)rowcol, event))))
   {
