...
static void StructureNotifyHandler(Widget wid,
                                   XtPointer closure,
                                   XEvent *event,
...
static void Popdown(XmMenuShellWidget menushell, XEvent *event);
static void PopdownKids(XmMenuShellWidget menushell, XEvent *event);
static int SkipPopdown(XmCascadeButtonWidget cascade);
static void PopdownOne(Widget widget, XEvent *event, String *params, Cardinal *num_params);
static void PopdownEveryone(Widget widget, XEvent *event, String *params, Cardinal *num_params);
static void PopdownDone(Widget widget, XEvent *event, String *params, Cardinal *num_params);
static void ClearTraversalInternal(XmMenuShellWidget menushell, XEvent *event);
...
static void _XmMenuPopdownAction(Widget widget,
                                 XEvent *event,
                                 String *params,
                                 Cardinal *num_params);
...
static XtActionsRec actionsList[] = {{"MenuShellPopdownOne", PopdownOne},
...
externaldef(xmmenushellwidgetclass)
...
static void slideCancel(Widget w, Widget slide)
{
  XtRemoveCallback(w, XmNunmapCallback, (XtCallbackProc)slideCancel, slide);
  XtDestroyWidget(slide);
}

static void slideFinish(Widget slide, Widget w)
{
  XtRemoveCallback(w, XmNunmapCallback, (XtCallbackProc)slideCancel, slide);
}
  ...
  if (!shell_widget->shell.popped_up) {
    ...
    if (XmIsMenuShell(shell_widget) && ((XmMenuShellWidget)shell_widget)->menu_shell.animate) {
      if (XmIsRowColumn(shell_widget->composite.children[0])) {
        ...
        slider = XtVaCreateWidget("MenuSlider",
        ...
        XtAddCallback(slider,
                      ...
                      (XtCallbackProc)slideFinish,
                      shell_widget->composite.children[0]);
        XtAddCallback(shell_widget->composite.children[0],
                      XmNunmapCallback,
                      (XtCallbackProc)slideCancel,
                      slider);
      }
    }
...
void _XmPopupSpringLoaded(Widget shell)
{
 ...
 */
static void PostMenuShell(XmMenuShellWidget menuShell, XtGrabKind grab_kind, Boolean spring_loaded)
{
  XEvent event;
  XEvent *lastEvent;
  ...
  if ((lastEvent = XtLastEventProcessed(XtDisplay((Widget)menuShell))) == (XEvent *)NULL) {
    event.xbutton.serial = XLastKnownRequestProcessed(XtDisplay((Widget)menuShell));
  }
  else {
    event.xbutton.serial = lastEvent->xany.serial;
  }
  event.xbutton.send_event = 0;
  event.xbutton.time = XtLastTimestampProcessed(XtDisplay((Widget)menuShell));
...
static void Initialize(Widget req, Widget new_w, ArgList args, Cardinal *num_args)
{
  ...
  if (!defaultFont) {
    ...
    defaultFont = ms->menu_shell.default_font_list;
    if (!defaultFont)
      defaultFont = XmeGetDefaultRenderTable((Widget)ms, XmLABEL_FONTLIST);
  }
  ms->menu_shell.label_font_list = XmFontListCopy(defaultFont);
  if (ms->menu_shell.default_font_list != NULL)
    ms->menu_shell.default_font_list = XmFontListCopy(ms->menu_shell.default_font_list);
   ...
   */
  XtInsertEventHandler((Widget)ms,
...
static Boolean SetValues(Widget cw, Widget rw, Widget nw, ArgList args, Cardinal *num_args)
{
  ...
  if (old_w->menu_shell.layout_direction != new_w->menu_shell.layout_direction) {
    XmeWarning(nw, MESSAGE9);
    new_w->menu_shell.layout_direction = old_w->menu_shell.layout_direction;
  }
  if (new_w->menu_shell.button_font_list != old_w->menu_shell.button_font_list) {
  ...
  if (new_w->menu_shell.label_font_list != old_w->menu_shell.label_font_list) {
...
static void Resize(Widget wid)
{
  ...
  if (((ms->composite.num_children == 1) && (XtIsManaged(ms->composite.children[0]))))
  /*
...
static void DeleteChild(Widget widget)
{
  ...
  (*(((CompositeWidgetClass)compositeWidgetClass)->composite_class.delete_child))(widget);
  /*
  ...
  if (parent->composite.num_children == 1) {
    ...
    if (!parent->shell.popped_up && !(XmIsRowColumn(baseChild) && RC_TearOffActive(baseChild)))
      baseChild->core.managed = False;
    XMapWindow(XtDisplay(parent), XtWindow(baseChild));
  }
  else if ((parent->composite.num_children == 0) && (parent->menu_shell.private_shell) &&
...
static void InsertChild(Widget widget)
{
  ...
  if (XmIsRowColumn(widget)) {
    if ((RC_Type(widget) == XmMENU_PULLDOWN) || (RC_Type(widget) == XmMENU_POPUP)) {
      XtAddEventHandler(widget, EnterWindowMask, FALSE, _XmEnterRowColumn, FALSE);
    }
    /*
     * add to composite list
     */
    (*((CompositeWidgetClass)compositeWidgetClass)->composite_class.insert_child)(widget);
    XtRealizeWidget(widget);
                                  ...
                                  register Position *y)
{
   ...
   * off the button.
   */
  if (pulldown_button && XtParent(pulldown_button) && XmIsRowColumn(XtParent(pulldown_button)) &&
  ...
  if (bottomEdgeOfMenu >= (Position)dispHeight) {
        ...
        (RC_Type(XtParent(pulldown_button)) == XmMENU_BAR))
    {
      ...
      y_temp -= (CascadeBorderWidth + pulldown_button->core.height + RowColBorderWidth +
...
static void PopupSharedMenuShell(Widget cbwid, Widget smwid, XEvent *event)
{
  ...
  else if (cascadebtn != (XmCascadeButtonWidget)RC_CascadeBtn(popup->composite.children[0])) {
  ...
  if (popped_up || (old_cascadebtn && RC_TornOff(XtParent(old_cascadebtn)) &&
  ...
  if (RC_WidgetHasMoved(submenu)) /* but make sure it didn't */ { /* just move to where it */
                                                                  /* is now.  Happens with */
                                                                  /* pulldowns a lot */
    x = XtX(submenu);
    y = XtY(submenu);
    ...
    if (RC_WindowHasMoved(submenu)) {
      XMoveWindow(XtDisplay(submenu), XtWindow(submenu), XtX(submenu), XtY(submenu));
  ...
  if (popped_up && !((old_rowcol == submenu) && (cascadebtn == old_cascadebtn)))
  ...
  if (popped_up)
    ...
    if (RC_Type(parent_menu) == XmMENU_OPTION) {
       ...
       */
      if (_XmMenuGrabKeyboardAndPointer((Widget)parent_menu, _time) != GrabSuccess) {
        return;
      }
      ...
      if ((RC_Type(parent_menu) == XmMENU_BAR) && (RC_BeingArmed(parent_menu))) {
  ...
  if (popped_up) {
    ...
    if (activeChild && XmIsGadget(activeChild)) {
      _XmDispatchGadgetInput((Widget)activeChild, NULL, XmFOCUS_OUT_EVENT);
    }
  }
}
                                        ...
                                        XtWidgetGeometry *reply)
{
  ...
  if (!XtIsRealized((Widget)shell)) {
    ...
    *reply = *request;
    ...
    if (request->request_mode & CWBorderWidth)
      wid->core.border_width = shell->core.border_width = request->border_width;
  ...
  if ((wid != shell->composite.children[0]) ||
      (XtMakeGeometryRequest((Widget)shell, &my_request, NULL) == XtGeometryYes))
  {
    if (request->request_mode & CWX) {
      wid->core.x = 0;
    }
...
static void ChangeManaged(Widget w)
{
  ...
  if (XtIsManaged((Widget)rowcol)) {
    if ((RC_Type(rowcol) == XmMENU_PULLDOWN) && (!RC_CascadeBtn(rowcol))) {
      /*
      ...
      if ((mst->RC_ButtonEventStatus.time == XtLastTimestampProcessed(XtDisplay(rowcol))) &&
     ...
     */
    _XmCallRowColumnMapCallback((Widget)rowcol, (XEvent *)&mst->RC_ButtonEventStatus.event);
    /*
     * get the shell widget in sync with the menu widget
     * We keep the menu at 0,0 and shell at the menu's position.
     * the menu's position is just used as a place to keep shell's loc
    ...
    if (RC_WidgetHasMoved(rowcol)) {
      ...
      if (RC_WindowHasMoved(rowcol)) {
        XMoveWindow(XtDisplay(rowcol), XtWindow(rowcol), XtX(rowcol), XtY(rowcol));
    ...
    menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass((Widget)rowcol),
                                                XmQTmenuSystem);
    ...
    switch (RC_Type(rowcol)) {
      case XmMENU_POPUP:
        ...
        if (RC_CascadeBtn(rowcol)) /* Doggone loophole! */ {
          if (_XmMenuGrabKeyboardAndPointer(RC_CascadeBtn(rowcol), _time) != GrabSuccess) {
      ...
      case XmMENU_PULLDOWN:
        ...
        if (!_XmGetInDragMode((Widget)rowcol)) {
          if (RC_MemWidget(rowcol) && (RC_Type(parent_menu) == XmMENU_OPTION)) {
            if (XtParent(RC_MemWidget(rowcol)) == (Widget)rowcol)
              _XmSetInitialOfTabGroup((Widget)rowcol, RC_MemWidget(rowcol));
            else {
    ...
    (*(((XmMenuShellClassRec *)(popup->core.widget_class))->menu_shell_class.popdownEveryone))(
...
void _XmPopdown(Widget widget)
{
  ...
  if (shell_widget->shell.popped_up) {
    ...
    XWithdrawWindow(XtDisplay(shell_widget),
...
static void Popdown(XmMenuShellWidget menushell, XEvent *event)
{
  XmRowColumnWidget rowcol = (XmRowColumnWidget)menushell->composite.children[0];
  XmMenuSystemTrait menuSTrait;
...
static void PopdownKids(XmMenuShellWidget menushell, XEvent *event)
{
  ...
  if ((subShell = (ShellWidget)RC_PopupPosted(menushell->composite.children[0])) != NULL) {
 ...
 */
void _XmEnterRowColumn(Widget widget, XtPointer closure, XEvent *event, Boolean *cont)
{
   ...
   */
  if (!_XmGetInDragMode(widget) || (!menushell->shell.popped_up) || (!RC_PopupPosted(rowcol)))
    return;
  cascade = (XmRowColumnWidget)((CompositeWidget)RC_PopupPosted(rowcol))->composite.children[0];
  ...
  if (XmIsCascadeButtonGadget(cascadebtn) &&
      (cascadebtn == XmObjectAtPoint((Widget)rowcol, enterevent->x, enterevent->y)))
  {
    return;
  }
    ...
    if (SkipPopdown((XmCascadeButtonWidget)cascadebtn))
      ...
      if ((enterevent->x_root >= x) && (enterevent->x_root < (x + XtWidth(cascadebtn))) &&
          (enterevent->y_root >= y) && (enterevent->y_root < (y + XtHeight(cascadebtn))))
      {
        return;
      }
    }
...
static void PopdownOne(Widget widget, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (RC_Type(rowcol) == XmMENU_BAR)
    tms = (XmMenuShellWidget)RC_PopupPosted(rowcol);
           ...
           ((RC_Type(rowcol) == XmMENU_PULLDOWN) && !XmIsMenuShell(XtParent(rowcol))))
  {
    ...
    if (!XmIsMenuShell(tms)) /* torn! */ {
      ...
      if (rowcol == (XmRowColumnWidget)XtParent(RC_CascadeBtn(ms->composite.children[0])))
        ms = tms = (XmMenuShellWidget)RC_ParentShell(rowcol);
    }
  }
  if (ms == tms)
    (*(((XmMenuShellClassRec *)(ms->core.widget_class))->menu_shell_class.popdownDone))(
        widget, event, params, num_params);
  else {
    (*(((XmMenuShellClassRec *)(ms->core.widget_class))->menu_shell_class.popdownEveryone))(
 ...
 */
static void PopdownEveryone(Widget widget,
                            XEvent *event,
                            ...
                            Cardinal *num_params) /* unused */
{
   ...
   */
  if ((RC_Type(rowcol) == XmMENU_PULLDOWN) && XmIsMenuShell(XtParent(rowcol)) &&
      RC_CascadeBtn(rowcol) && XmIsMenuShell(XtParent(XtParent(RC_CascadeBtn(rowcol)))))
  {
    _XmMenuFocus(XtParent(XtParent(RC_CascadeBtn(rowcol))),
                 XmMENU_MIDDLE,
                 _XmGetDefaultTime(widget, event));
  }
  /* mark this row colum as popping down */
  RC_SetPoppingDown(rowcol, True);
...
static void PopdownDone(Widget widget, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (ms == NULL)
    return;
  _XmGetActiveTopLevelMenu(ms->composite.children[0], (Widget *)&rowcol);
  menuSTrait = (XmMenuSystemTrait)XmeTraitGet((XtPointer)XtClass((Widget)rowcol), XmQTmenuSystem);
   ...
   */
  if ((RC_Type(rowcol) == XmMENU_BAR) || (RC_Type(rowcol) == XmMENU_OPTION) ||
       ...
       XtGetMultiClickTime(XtDisplay(ms)))) /* or 150 ms? */
  {
    ...
    else if (!rowcol->manager.highlighted_widget) {
  ...
  if (XmIsMenuShell(ms)) {
    ...
    (*(((XmMenuShellClassRec *)(RC_ParentShell(rowcol)->core.widget_class))
           ->menu_shell_class.popdownEveryone))((Widget)ms, event, params, num_params);
  }
...
static void ClearTraversalInternal(XmMenuShellWidget menushell, XEvent *event) /* unused */
{
  Widget activeChild;
  XmRowColumnWidget rowcol;
  ...
  if (activeChild) {
    if (XmIsGadget(activeChild)) {
      (*((XmLabelGadgetClass)XtClass(activeChild))->gadget_class.border_unhighlight)(activeChild);
    }
    else {
      (*((XmLabelWidgetClass)XtClass(activeChild))->primitive_class.border_unhighlight)(
          activeChild);
    }
  }
  /*
...
static void CheckSetRenderTables(Widget wid, int offset, XrmValue *value)
{
  ...
  if (check_set_save == msw) {
    if ((offset == check_set_offset1) || (offset == check_set_offset2)) {
      value->addr = NULL;
    }
...
static XmFontList GetTable(Widget wid, XtEnum type)
{
  ...
  switch (type) {
    case XmLABEL_RENDER_TABLE:
      return menu->menu_shell.label_font_list;
    case XmBUTTON_RENDER_TABLE:
      return menu->menu_shell.button_font_list;
    case XmTEXT_RENDER_TABLE:
      return menu->menu_shell.default_font_list;
  }
  return NULL;
}
                       ...
                       Cardinal *num_params) /* unused */
{
  ...
  if ((ms->composite.num_children != 0) &&
      ...
      (_XmMatchBtnEvent(
           event, RC_PostEventType(rowcol), RC_PostButton(rowcol), RC_PostModifiers(rowcol)) ||
       (_XmMatchBSelectEvent((Widget)rowcol, event))))
  {
     ...
     */
    if ((RC_Type(rowcol) == XmMENU_POPUP) && (mst->MS_LastManagedMenuTime == xbutton->time)) {
      XAllowEvents(XtDisplay(ms), SyncPointer, _XmGetDefaultTime(wid, event));
      return;
    }
...
static void _XmMenuPopupAction(Widget widget, XEvent *event, String *params, Cardinal *num_params)
{
  ...
  if (spring_loaded)
    _XmPopupI(popup_shell, XtGrabExclusive, TRUE);
  else
    _XmPopupI(popup_shell, XtGrabNonexclusive, FALSE);
}
