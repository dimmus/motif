...
static XtResource resources[]               = {
   ...
   { XmNattachment, XmCAttachment, XmRIconAttachment, sizeof(unsigned char), Offset(attachment), XmRImmediate, (XtPointer)XmATTACH_NORTH_WEST },
};
...
externaldef(dragIconobjectclass) WidgetClass
   ...
   = (WidgetClass)&xmDragIconClassRec;
   ...
   Cardinal *numArgs) /* unused */
{
   ...
   if (dragIcon->drag.pixmap == XmUNSPECIFIED_PIXMAP)
   {
	 ...
	 */
      ...
      if (cursorData)
      {
         ...
         _XmCreateImage(image, display, (char *)cursorData->data, dragIcon->drag.width, dragIcon->drag.height, LSBFirst);
         _XmInstallImage(image, cursorData->dataName, (int)dragIcon->drag.hot_x, (int)dragIcon->drag.hot_y);
         dragIcon->drag.pixmap = XmGetPixmapByDepth(screen, cursorData->dataName, 1, 0, 1);
         if (cursorData->maskData)
         {
            _XmCreateImage(image, display, (char *)cursorData->maskData, dragIcon->drag.width, dragIcon->drag.height, LSBFirst);
   ...
   else if (dragIcon->drag.pixmap != XmUNSPECIFIED_PIXMAP)
   {
      ...
      if (dragIcon->drag.mask != XmUNSPECIFIED_PIXMAP)
      {
         ...
         if (dragIcon->drag.width > 0 && dragIcon->drag.height > 0)
         {
            image                 = XGetImage(display, (Drawable)dragIcon->drag.mask, 0, 0, dragIcon->drag.width, dragIcon->drag.height, 1L, XYPixmap);
