...
extern void    _XmDataFielddf_SetCursorPosition(XmDataFieldWidget, XEvent *, XmTextPosition, Boolean, Boolean);
   ...
   int           *format)
{
   ...
   else if (insert_select->select_type == XmDEST_SELECT)
   {
      if (XmDataFieldGetSelectionPosition(w, &left, &right) && left != right)
      {
         if (TextF_CursorPosition(tf) < left || TextF_CursorPosition(tf) > right || !XmTextF_pending_delete(tf))
         {
   ...
   else
   {
      ...
      if (XmTextF_max_char_size(tf) == 1)
      {
         _XmDataFielddf_SetCursorPosition(tf, NULL, (XmTextPosition)(left + *length), False, True);
      }
      else
      {
         _XmDataFielddf_SetCursorPosition(tf, NULL, left + num_chars, False, True);
      }
      (void)_XmDataFielddf_SetDestination(w, TextF_CursorPosition(tf), insert_select->event->time);
      if (insert_select->select_type == XmDEST_SELECT)
      {
         if (left != right)
         {
            if (!dest_disjoint)
            {
               _XmDataFieldStartSelection(tf, TextF_CursorPosition(tf), TextF_CursorPosition(tf), insert_select->event->time);
            }
            else
            {
   ...
   int           *format)
{
   ...
   XtGetSelectionValue(w, *seltype, target, InsertSelection, (XtPointer)insert_select, insert_select->event->time);
}
   ...
   Atom           locale_atom)
{
   ...
   if (old_serial != req_event->serial)
      ...
      return False;
   if (XGetWindowProperty(req_event->display, req_event->requestor, req_event->property, 0L, 10000000, False, AnyPropertyType, &actual_type, &actual_format, &nitems, &bytes, &prop) != Success)
      return FALSE;
   ...
   if (pair->target != locale_atom)
   {
      ...
      */
      XtGetSelectionValue(w, pair->selection, TARGETS, HandleInsertTargets, (XtPointer)&insert_select, req_event->time);
   }
   else
   {
      /*
      ...
      */
      XtGetSelectionValue(w, pair->selection, pair->target, InsertSelection, (XtPointer)&insert_select, req_event->time);
   }
   /*
   ...
   int           *format)
{
      ...
      *format = 8;
      ...
      else
      {
         *length   = right - left;
         ...
         (void)memcpy((void *)tmp_value, (void *)(TextF_Value(tf) + left), (unsigned)*length); /* NOTE: casting *length could result
				             in a truncated long. */
      }
      tmp_value[*length] = '\0';
   ...
   else if (*target == TEXT || *target == CS_OF_LOCALE)
   {
      ...
      *format = 8;
      ...
      else
      {
         ...
         *value  = XtMalloc((unsigned)*length + 1);
         /* get the selection value */
         (void)memcpy((void *)*value, (void *)(TextF_Value(tf) + left), (unsigned)*length); /* NOTE: casting *length could result
				             in a truncated long. */
      }
      (*(char **)value)[*length] = '\0';
   }
   ...
   */
   ...
   else if (*target == INSERT_SELECTION)
   {
      ...
      return (ConvertInsertSelection((Widget)tf, selection, type, value, length, format, CS_OF_LOCALE));
   ...
   else if (*target == DELETE)
   {
      ...
      _XmDataFieldStartSelection(tf, XmTextF_prim_anchor(tf), XmTextF_prim_anchor(tf), req_event->time);
      if (!_XmDataFieldReplaceText(tf, (XEvent *)req_event, left, right, NULL, 0, move_cursor))
      {
   ...
   Atom  *selection)
{
   ...
   {
      _XmDataFieldSetSel2(w, 0, 0, True, XtLastTimestampProcessed(XtDisplay(w)));
   }
}
