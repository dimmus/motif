...
#include <Xm/XmosP.h>
    ...
    InsertSelection(Widget w,
                    ...
                    int *format);
static Boolean ConvertInsertSelection(Widget w,
                                      Atom *selection,
                                      Atom *type,
                                      XtPointer *value,
                                      unsigned long *length,
      ...
      if (left != right) {
        if (!dest_disjoint) {
          _XmDataFieldStartSelection(
              tf, TextF_CursorPosition(tf), TextF_CursorPosition(tf), insert_select->event->time);
        }
        else {
          if (!XmTextF_add_mode(tf)) {
            _XmDataFieldStartSelection(tf,
                            ...
                            int *format)
{
  ...
  Atom MOTIF_DESTINATION = XmInternAtom(XtDisplay(w), "MOTIF_DESTINATION", False);
  Atom INSERT_SELECTION = XmInternAtom(XtDisplay(w), "INSERT_SELECTION", False);
  ...
  if (*target == TARGETS) {
    ...
    *format = 8;
    ...
    if (is_secondary)
      return False;
    return (
        ConvertInsertSelection((Widget)tf, selection, type, value, length, format, CS_OF_LOCALE));
    ...
    _XmDataFieldStartSelection(
        tf, XmTextF_prim_anchor(tf), XmTextF_prim_anchor(tf), req_event->time);
