...
#define INVALID_PROTOCOL_VALUE 255
    ...
    static void
    DisplayClassPartInitialize(WidgetClass wc);
static void DisplayClassInitialize(void);
static void SetDragReceiverInfo(Widget w,
                                XtPointer client_data,
                                XEvent *event,
                                Boolean *dontSwallow);
static void TreeUpdateHandler(Widget w, XtPointer client, XtPointer call);
static void DisplayInitialize(Widget requested_widget,
                              Widget new_widget,
                              ArgList args,
                              Cardinal *num_args);
static void DisplayInsertChild(Widget w);
...
static void ReceiverShellExternalSourceHandler(Widget w,
                                               ...
                                               XEvent *event,
                                               Boolean *dontSwallow);
static Widget GetDisplay(Display *display);
...
static XtResource resources[] = {
    ...
    {XmNdefaultButtonEmphasis,
     ...
     XmRImmediate,
     (XtPointer)XmEXTERNAL_HIGHLIGHT},
...
externaldef(xmdisplayclass) WidgetClass xmDisplayClass = (WidgetClass)(&xmDisplayClassRec);

static void DisplayClassPartInitialize(WidgetClass wc)
{
...
static void TreeUpdateHandler(Widget w, XtPointer client, XtPointer call)
{
  ...
  switch (anyCB->reason) {
    case XmCR_DROP_SITE_TREE_ADD: {
       ...
       */
      XtAddEventHandler(
          cb->rootShell, NoEventMask, True, ReceiverShellExternalSourceHandler, (XtPointer)dd);
    } break;
...
static void DisplayInitialize(Widget requested_widget,
                              ...
                              Cardinal *num_args)
{
  static char *atom_names[] = {"Customize Data:0", /* XmSCUSTOMIZE_DATA + "0" */
                               XmIATOM_PAIR,
                               XmIAVERAGE_WIDTH,
                               XmIPIXEL_SIZE,
                               XmIRESOLUTION_Y,
  ...
  xmDisplay->display.displayHasShapeExtension = XShapeQueryExtension(
      XtDisplay(xmDisplay), &dummy1, &dummy2);
  ...
  if (!XFindContext(XtDisplay(xmDisplay), None, context, (char **)&xmDisplay)) {
    ...
    XSaveContext(XtDisplayOfObject((Widget)xmDisplay), None, context, (char *)xmDisplay);
  }
...
static void DisplayInsertChild(Widget w)
{
  if (XtIsRectObj(w)) {
    ...
    _XmProcessLock();
    insert_child = ((CompositeWidgetClass)compositeWidgetClass)->composite_class.insert_child;
    _XmProcessUnlock();
    (*insert_child)(w);
  }
}
...
static void DisplayDestroy(Widget w)
{
  ...
  if (((XmDisplayInfo *)(dd->display.displayInfo))->excParentPane.pane != (Widget *)NULL)
    XtFree((char *)((XmDisplayInfo *)(dd->display.displayInfo))->excParentPane.pane);
...
XmDropSiteManagerObject _XmGetDropSiteManagerObject(XmDisplay xmDisplay)
{
  ...
  if (xmDisplay->display.dsm == NULL) {
    ...
    xmDisplay->display.dsm = (XmDropSiteManagerObject)XtCreateWidget(
        "dsm", xmDisplay->display.dropSiteManagerClass, (Widget)xmDisplay, lclArgs, 1);
  }
  return (xmDisplay->display.dsm);
}

unsigned char _XmGetDragProtocolStyle(Widget w)
{
  XmDisplay xmDisplay;
  unsigned char style;
...
Widget XmGetDragContext(Widget w, Time time)
{
  ...
  for (i = 0; i < xmDisplay->composite.num_children; i++) {
    ...
    if ((XmIsDragContext((Widget)dc)) && (CHECK_TIME(dc, time)) &&
        ((!matchedDC) || (matchedDC->drag.dragStartTime < dc->drag.dragStartTime)) &&
...
Widget _XmGetDragContextFromHandle(Widget w, Atom iccHandle)
{
  XmDisplay xmDisplay;
  XmDragContext dc;
  Cardinal i;
  ...
  for (i = 0; i < xmDisplay->composite.num_children; i++) {
    dc = (XmDragContext)(xmDisplay->composite.children[i]);
    if ((XmIsDragContext((Widget)dc)) && (dc->drag.iccHandle == iccHandle) &&
        !dc->core.being_destroyed)
      return ((Widget)dc);
  }
  return (NULL);
...
static XmDragContext FindDC(XmDisplay xmDisplay, Time time, Boolean sourceIsExternal)
{
  ...
  for (i = 0; i < xmDisplay->composite.num_children; i++) {
    dc = (XmDragContext)(xmDisplay->composite.children[i]);
    if ((XmIsDragContext((Widget)dc)) && (CHECK_TIME(dc, time)) &&
        (dc->drag.sourceIsExternal == sourceIsExternal) && !dc->core.being_destroyed)
      return (dc);
  }
  return (NULL);
}
...
static int isMine(Display *dpy, register XEvent *event, char *arg)
{
  ...
  if (q->hasDropStart)
    return (False);
  if (!_XmICCEventToICCCallback((XClientMessageEvent *)event, cb, XmICC_INITIATOR_EVENT))
    return (False);
                                               ...
                                               Boolean *dontSwallow)
{
  ...
  if (q.hasEnter || q.hasMotion || q.hasDropStart || q.hasLeave) {
    ...
    if (q.hasLeave) {
      ...
      _XmDSMUpdate(dsm, (XtPointer)&topClientData, (XtPointer)callback);
    ...
    if (q.hasEnter || q.hasDropStart) {
      if (!q.dc) {
        ...
        dc = (XmDragContext)XtCreateWidget(
            "dragContext", dd->display.dragContextClass, (Widget)dd, args, i);
        /*
    ...
    if (q.hasDropStart) {
      ...
      _XmDSMUpdate(dsm, (XtPointer)&topClientData, (XtPointer)&dropStartCB);
    }
    /*
     * we only see enters if they're not matched with a leave
     */
    if (q.hasEnter) {
      _XmDSMUpdate(dsm, (XtPointer)&topClientData, (XtPointer)&enterCB);
    }
    if (q.hasMotion) {
