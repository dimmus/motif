...
static XtResource input_resources[] = {
   { XmNselectionArray,      XmCSelectionArray,      XmRPointer, sizeof(XtPointer), XtOffsetOf(struct _InputDataRec, sarray),        XmRImmediate, (XtPointer)sarray           },
   { XmNselectionArrayCount, XmCSelectionArrayCount, XmRInt,     sizeof(int),       XtOffsetOf(struct _InputDataRec, sarraycount),   XmRImmediate, (XtPointer)XtNumber(sarray) },
   { XmNpendingDelete,       XmCPendingDelete,       XmRBoolean, sizeof(Boolean),   XtOffsetOf(struct _InputDataRec, pendingdelete), XmRImmediate, (XtPointer)True             },
   { XmNselectThreshold,     XmCSelectThreshold,     XmRInt,     sizeof(int),       XtOffsetOf(struct _InputDataRec, threshold),     XmRImmediate, (XtPointer)5                },
};
...
GetTextDestData(Widget tw)
{
   ...
   if (XFindContext(display, (Window)screen, loc_context, (char **)&dest_data))
   {
      ...
      XtAddCallback(xm_display, XmNdestroyCallback, (XtCallbackProc)_XmTextFreeContextData, (XtPointer)ctx_data);
                             ...
                             int             check_add_mode)
{
   ...
   if (!(*tw->text.source->GetSelection)(tw->text.source, left, right))
   {
      *left = *right = tw->text.cursor_position;
      ...
      return (data->pendingdelete && *left != *right && *left <= tw->text.cursor_position && *right >= tw->text.cursor_position);
}
                ...
                int          buffer)
{
   ...
   block.format      = XmFMT_8_BIT;
   if (_XmTextModifyVerify(tw, event, &from_pos, &to_pos, &cursorPos, &block, &newblock, &freeBlock))
   {
                       ...
                       Time           sel_time)
{
   ...
   InputData      data = tw->text.input->data;
   left = right = position;
   if (tw->text.add_mode || ((*tw->text.source->GetSelection)(tw->text.source, &left, &right) && left != right && position >= left && position <= right))
           ...
           Cardinal *num_params)
{
   ...
   */
   n = XmImMbLookupString(w, (XKeyEvent *)event, str, TEXT_MAX_INSERT_SIZE, (KeySym *)NULL, &status_return);
   ...
   */
   ...
   if (n > 0)
   {
      ...
      block.length = n;
      block.format = XmFMT_8_BIT;
      if (_XmTextModifyVerify(tw, event, &beginPos, &nextPos, &cursorPos, &block, &newblock, &freeBlock))
      {
         if (pending_delete)
            (*tw->text.source->SetSelection)(tw->text.source, cursorPos, cursorPos, event_time);
             ...
             Cardinal *num_params)
{
   ...
   for (i = 0; i < *num_params; i++)
   {
      ...
      block.format = XmFMT_8_BIT;
      if (_XmTextModifyVerify(tw, event, &beginPos, &nextPos, &newCursorPos, &block, &newblock, &freeBlock))
      {
         if (pending_delete)
         {
            (*tw->text.source->SetSelection)(tw->text.source, cursorPos, cursorPos, event_time);
              ...
              int                 count)
{
   ...
   else
   {
      ...
      return (*source->Scan)(source, tw->text.line_table[line].start_pos, XmSELECT_ALL, XmsdRight, 1, TRUE);
   }
}
             ...
             Cardinal *num_params)
{
   ...
   if (left > right)
   { /* PIR1858: We are on other side of anchor */
      tempIndex = left;
      left      = right;
      right     = tempIndex;
   } /* end PIR1858 */
                   ...
                   Boolean        extend)
{
   XmTextPosition left, right;
   InputData      data = tw->text.input->data;
   if ((tw->text.add_mode && (*tw->text.source->GetSelection)(tw->text.source, &left, &right) && position >= left && position <= right) || extend)
      tw->text.pendingoff = FALSE;
   else
      tw->text.pendingoff = TRUE;
   if (extend)
                ...
                Cardinal *num_params)
{
   ...
   /* Add if We only want to select to the end of the word w/o spaces
     ...
     */
   position = (*tw->text.source->Scan)(tw->text.source, cursorPos, XmSELECT_WORD, XmsdRight, 1, TRUE);
                 ...
                 Cardinal *num_params)
{
   ...
   position = (*tw->text.source->Scan)(tw->text.source, cursorPos, XmSELECT_WORD, XmsdLeft, 1, FALSE);
   if (position == cursorPos)
   {
      position = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_WORD, XmsdLeft, 1, TRUE);
      position = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_WORD, XmsdLeft, 1, FALSE);
   }
   SetNavigationAnchor(tw, cursorPos, position, event_time, extend);
   CompleteNavigation(tw, position, event_time, extend);
   DisplayInsertionPoint(tw);
              ...
              Cardinal *num_params)
{
   ...
   else
   {
      ...
      if (position == PASTENDPOS)
         position = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_ALL, XmsdRight, 1, TRUE);
      else
              ...
              Boolean   pendingoff)
{
   ...
   if (start == PASTENDPOS)
   {
      newPos = (*tw->text.source->Scan)(tw->text.source, currentPos, XmSELECT_ALL, XmsdRight, 1, TRUE);
   ...
   if (line != NOLINE)
   {
      ...
      {
         newPos = (*tw->text.source->Scan)(tw->text.source, start, XmSELECT_LINE, XmsdRight, 1, FALSE);
         SetNavigationAnchor(tw, currentPos, newPos, event_time, extend);
         CompleteNavigation(tw, newPos, event_time, extend);
      }
   }
                  ...
                  Boolean   pendingoff)
{
   ...
   else
   {
      ...
      if (line == 0)
      {
         ...
         if (line == 0)
         {
            newPos  = (*tw->text.source->Scan)(tw->text.source, currentPos, XmSELECT_ALL, XmsdLeft, 1, TRUE);
            changed = True;
            goto done;
         }
     ...
     * GGHHII...   Should now be at beginning of CC */
      ...
      if (line != NOLINE)
      {
         ...
         if (start2 != start)
         {
            newPos = (*tw->text.source->Scan)(tw->text.source, origstart, XmSELECT_POSITION, XmsdLeft, 1, TRUE);
         }
      }
   }
done:
                       ...
                       Cardinal *num_params)
{
   ...
   if (left != (*tw->text.source->Scan)(tw->text.source, left, XmSELECT_WHITESPACE, XmsdRight, 1, FALSE))
   {
      AddNewLine(w, event, True);
   }
   else
   {
      right = (*tw->text.source->Scan)(tw->text.source, left, XmSELECT_WHITESPACE, XmsdRight, 1, TRUE);
      if (right > pos)
         right = pos;
      AddNewLine(w, event, True);
      ...
      while (left < right)
      {
         left = (*tw->text.source->ReadSource)(tw->text.source,
                                               ...
                                               right,
                                               &block);
         if (_XmTextModifyVerify(tw, event, &from_pos, &to_pos, &newCursorPos, &block, &newblock, &freeBlock))
         {
            if ((*tw->text.source->Replace)(tw, NULL, &from_pos, &to_pos, &newblock, False) != EditDone)
            {
               RingBell(w, event, params, num_params);
              ...
              Cardinal *num_params)
{
   ...
   DisplayInsertionPoint(tw);
   if (tw->text.add_mode && (!(*tw->text.source->GetSelection)(data->widget->text.source, &left, &right) || left == right))
   {
                  ...
                  Cardinal *num_params)
{
   ...
   _XmTextResetIC(w);
   if (event)
      _XmTextSetCursorPosition(w, (*tw->text.output->XYToPos)(tw, event->xbutton.x, event->xbutton.y));
}
                   ...
                   Boolean   kill)
{
   ...
   else
   {
      ...
      cursorPos = (*tw->text.source->Scan)(tw->text.source, nextPos, XmSELECT_POSITION, XmsdLeft, 1, TRUE);
                  ...
                  Boolean   kill)
{
   ...
   {
      cursorPos = tw->text.cursor_position;
      nextPos   = (*tw->text.source->Scan)(tw->text.source, cursorPos, XmSELECT_POSITION, XmsdRight, 1, TRUE);
      if (DeleteOrKill(tw, event, cursorPos, nextPos, kill, &newCursorPos))
      {
         _XmTextSetCursorPosition(w, newCursorPos);
         CheckDisjointSelection(w, tw->text.cursor_position, event_time);
   ...
   else
   {
      ...
      else
      {
         ...
         if (right == PASTENDPOS)
            ...
            right = (*tw->text.source->Scan)(tw->text.source, right, XmSELECT_POSITION, XmsdLeft, 1, TRUE);
      }
      if (left < right)
      {
         if (DeleteOrKill(tw, event, left, right, kill, &newCursorPos))
            ...
            int          set_empty_selection)
{
   ...
   else
   {
      ...
      newRight = (*tw->text.source->Scan)(tw->text.source, position, data->stype, XmsdRight, 1, data->stype == XmSELECT_LINE);
   }
   if (data->stype == XmSELECT_WORD && (int)tw->text.char_size > 1)
   {
      if (position == (*tw->text.source->Scan)(tw->text.source, newLeft, data->stype, XmsdRight, 1, FALSE))
         newLeft = position;
   }
             ...
             Cardinal *num_params)
{
   ...
   if (data->stype != XmSELECT_POSITION || ((*tw->text.source->GetSelection)(tw->text.source, &left, &right) && left != right))
          ...
          XtIntervalId *id) /* unused */
{
   ...
   indata->drag_id     = 0;
   StartDrag((Widget)tw, indata->transfer_action->event, indata->transfer_action->params, indata->transfer_action->num_params);
}
            ...
            XEvent *event)
{
   ...
   return ((*tw->text.source->GetSelection)(tw->text.source, &left, &right) && (right != left) && ((position > left && position < right) || (position == left && (*tw->text.output->PosToXY)(tw, left, &left_x, &left_y) && x > left_x) || (position == right && (*tw->text.output->PosToXY)(tw, right, &right_x, &right_y) && x < right_x)));
}
               ...
               Cardinal *num_params)
{
   ...
   else
   {
      ...
      {
         case ButtonPress:
            if (!InSelection(w, event) || (event_time > data->lasttime && event_time - data->lasttime < XtGetMultiClickTime(XtDisplay(w))))
            {
               if (*num_params > 0)
                  XtCallActionProc(w, params[0], event, NULL, 0);
            }
         ...
         case ButtonRelease:
            if (data->drag_id)
            {
               ...
               {
                  XtCallActionProc(w, data->transfer_action->params[0], data->transfer_action->event, NULL, 0);
               }
            }
            XtCallActionProc(w, params[0], event, NULL, 0);
            break;
                    ...
                    Time   ev_time)
{
   ...
   if ((data->extendDir == XmsdRight && position < data->anchor) || (data->extendDir == XmsdLeft && position > data->anchor))
   {
   ...
   if (data->extendDir == XmsdRight)
   {
      ...
      {
         right = cursorPos = (*tw->text.source->Scan)(tw->text.source, position, data->stype, XmsdRight, 1, data->stype == XmSELECT_LINE);
      }
      left = data->anchor;
   }
   else
   {
      ...
      if (data->stype == XmSELECT_WORD && (int)tw->text.char_size > 1)
      {
         if (position == (*tw->text.source->Scan)(tw->text.source, left, data->stype, XmsdRight, 1, FALSE))
                  ...
                  Time   ev_time)
{
   ...
   /* check for change in extend direction */
   if ((data->Sel2ExtendDir == XmsdRight && position < data->Sel2OrigLeft) || (data->Sel2ExtendDir == XmsdLeft && position > data->Sel2OrigRight))
   {
   ...
   if (data->Sel2ExtendDir == XmsdRight)
      right = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_POSITION, XmsdRight, 1, FALSE);
   else
      left = (*tw->text.source->Scan)(tw->text.source, position, XmSELECT_POSITION, XmsdLeft, 1, FALSE);
                    ...
                    XEvent *event)
{
   ...
   if ((event->xmotion.x > (int)o_data->leftmargin) && (event->xmotion.x < (int)(tw->core.width - o_data->rightmargin)) && (event->xmotion.y > (int)o_data->topmargin) && (event->xmotion.y < (int)(o_data->topmargin + (o_data->lineheight * o_data->number_lines))))
   {
      if (data->select_id)
      {
   ...
   else
   {
                           ...
                           XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
      {
         ...
         if (event->xmotion.y <= (int)o_data->topmargin)
            data->select_pos_y = (Position)(o_data->topmargin - (o_data->font_ascent + o_data->font_descent + 1));
         /* to the below of the text */
         else if (event->xmotion.y >= (int)(tw->core.height - o_data->bottommargin))
            data->select_pos_y = (Position)((tw->core.height - o_data->bottommargin) + (o_data->font_ascent + o_data->font_descent + 1));
         /* right the text */
         ...
         else if (event->xmotion.x <= (int)((tw->core.width - o_data->rightmargin) - (o_data->linewidth * (o_data->number_lines + 1))))
            data->select_pos_y = (tw->core.width - o_data->rightmargin) - (o_data->linewidth * (o_data->number_lines + 1));
         /* ask the vertical scroller its delay */
         if (o_data->hbar)
      ...
      else
      {
         ...
         /* to the right of the text */
            ...
            data->select_pos_x = (Position)((tw->core.width - o_data->rightmargin) + o_data->averagecharwidth + 1);
         /* above the text */
         if (event->xmotion.y <= (int)o_data->topmargin)
         {
         ...
         else if (event->xmotion.y >= (int)(o_data->topmargin + (o_data->lineheight * o_data->number_lines)))
            ...
            Cardinal *num_params)
{
   ...
   if (event)
   {
                           ...
                           XmTOP_TO_BOTTOM_RIGHT_TO_LEFT))
      {
         ...
         /* below the text */
         else if (event->xbutton.x <= (int)(tw->core.width - o_data->rightmargin - (o_data->linewidth * o_data->number_lines)))
            event->xbutton.x = ((int)(tw->core.width - o_data->rightmargin) - (o_data->linewidth * o_data->number_lines)) + 1;
      }
      else
      {
         /* to the left of the text */
         ...
         /* below the text */
         else if (event->xbutton.y >= (int)(o_data->topmargin + (o_data->lineheight * o_data->number_lines)))
                ...
                Cardinal *num_params)
{
   ...
   Time           event_time = event ? event->xbutton.time : XtLastTimestampProcessed(XtDisplay(w));
   _XmTextResetIC(w);
   if (data->selectionMove == TRUE && data->has_destination && tw->text.dest_position >= data->sel2Left && tw->text.dest_position <= data->sel2Right)
   {
                   ...
                   Cardinal *num_params)
{
   ...
   if (event)
      if (event->xbutton.x > (int)tw->core.width || event->xbutton.x < 0 || event->xbutton.y > (int)tw->core.height || event->xbutton.y < 0)
      {
          ...
          Cardinal *num_params)
{
   XmTextWidget   tw            = (XmTextWidget)w;
   InputData      data          = tw->text.input->data;
   XmTextPosition last_position = (*tw->text.source->Scan)(tw->text.source, 0, XmSELECT_ALL, XmsdRight, 1, TRUE);
   Time           event_time    = event ? event->xkey.time : XtLastTimestampProcessed(XtDisplay((Widget)tw));
   EraseInsertionPoint(tw);
             ...
             Cardinal *num_params)
{
   ...
   if (!event_time) event_time = _XmValidTimestamp(w);
   EraseInsertionPoint(tw);
   if (_XmStringSourceGetEditable(GetSrc(w)) && (*source->GetSelection)(source, &left, &right) && right != left)
            ...
            Cardinal *num_params)
{
   ...
   if (_XmGetFocusPolicy(w) == XmEXPLICIT && !_XmTextHasDestination(w) && !tw->text.input->data->sel_start)
      _XmTextSetDestinationSelection(w, tw->text.cursor_position, False, XtLastTimestampProcessed(XtDisplay(w)));
             ...
             Cardinal *num_params)
{
   ...
   if (event && event->xfocus.send_event && _XmGetFocusPolicy(w) == XmEXPLICIT && !tw->text.traversed)
   {
...
externaldef(nonvisible) Cardinal _XmdefaultTextActionsTableSize = XtNumber(ZdefaultTextActionsTable);
/* added <Key> event */
...
_XmTextInputGetSecResData(XmSecondaryResourceData *secResDataRtn)
{
   ...
   _XmTransformSubResources(input_resources, XtNumber(input_resources), &(secResData->resources), &(secResData->num_resources));
                   ...
                   Cardinal num_args)
{
   ...
   XtGetSubresources(wid, (XtPointer)data, NULL, NULL, input_resources, XtNumber(input_resources), args, num_args);
   data->widget = tw;
             ...
             LineNum      line)
{
   ...
   Position       x1 = 0, y1 = 0;
   XmTextPosition pos;
   pos = (*tw->text.output->XYToPos)(tw, tw->text.inner_widget->core.width - (line * data->linewidth) - data->rightmargin, y);
   (*tw->text.output->PosToXY)(tw, pos, &x1, &y1);
   if (pos > 0 && y1 > y)
      return pos - 1;
   else
