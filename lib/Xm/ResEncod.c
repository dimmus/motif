...
static Boolean processCharsetAndTextUtf8(XmStringCharSet tag,
                                         ...
                                         ct_Charset *prev);
#endif
static Boolean processESCHack(ct_context *ctx, Octet final);
...
static Boolean process94GL(ct_context *ctx, Octet final);
static Boolean process94GR(ct_context *ctx, Octet final);
static Boolean process96GR(ct_context *ctx, Octet final);
static Boolean cvtXmStringToText(XrmValue *from, XrmValue *to);
static OctetPtr ctextConcat(OctetPtr str1,
                            unsigned int str1len,
                            const_OctetPtr str2,
                            unsigned int str2len);
#if XM_UTF8
static Boolean cvtXmStringToUTF8String(XrmValue *from, XrmValue *to);
static char *Convert(const char *str,
                     unsigned int len,
                     const char *to_codeset,
                     const char *from_codeset);
static char *ConvertWithIconv(const char *str, unsigned int len, iconv_t converter);
#endif
 ...
 *
 ************************************************************************/
XtPointer _XmGetEncodingRegistryTarget(int *length)
{
 ...
 */
   ...
   */
  if ((ctx->octet[0] == ESC) && (ctx->octet[1] == 0x23) &&
      (_IsInColumn2(ctx->octet[2]) && ((ctx->octet[3] == 0x30) || ctx->octet[3] == 0x31)))
  {
    ...
    switch (*ctx->octet) { /* look at next octet in seq */
      case ESC:
        ...
        */
        ...
        /* scan for final char */
        while ((ctx->octet[0] != 0) && (_IsInColumn2(*ctx->octet))) {
          ctx->octet++;
          ctx->itemlen++; /* advance ptr to next char */
        }
        ...
        */
        if (ctx->flags.text) {
          ...
          if (((ctx->octet[1] == 0x31) && (ctx->octet[2] == 0x5d)) ||
              ((ctx->octet[1] == 0x32) && (ctx->octet[2] == 0x5d)) || (ctx->octet[1] == 0x5d))
        ...
        ctx->itemlen++; /* advance ptr to next char */
        /* scan for final char */
        while ((ctx->octet[0] != 0) && _IsInColumn3(*ctx->octet)) {
          ctx->octet++;
          ctx->itemlen++; /* advance ptr to next char */
        }
        while ((ctx->octet[0] != 0) && _IsInColumn2(*ctx->octet)) {
      ...
      case NL: /* new line */
        ...
        if (ctx->flags.text) {
          ...
          ctx->xmstring = XmStringConcatAndFree(ctx->xmstring, XmStringCopy(ctx->xmsep));
        }
      ...
      case HT:
        ...
        ctx->xmstring = XmStringConcatAndFree(ctx->xmstring, XmStringCopy(ctx->xmtab));
        ctx->octet++; /* advance ptr to next char */
        break;
      default:                  /* just 'normal' text */
        ...
        while (ctx->octet[0] != 0) {
          ...
          if ((_IsInC0Set(c) && (!_IsValidC0(ctx, c))) || (_IsInC1Set(c) && (!_IsValidC1(ctx, c))))
          {
            ok = False;
            break;
          }
  ...
  if (ctx->xmsep != NULL)
    XmStringFree(ctx->xmsep);
  if (ctx->xmtab != NULL)
    XmStringFree(ctx->xmtab);
...
static Boolean processExtendedSegmentsHack(ct_context *ctx, Octet final)
{
   ...
   */
  if ((ctx->itemlen == 4) && (ctx->item[2] == 0x2f) && (_IsInColumn3(final))) {
    if ((ctx->octet[0] < 0x80) || (ctx->octet[1] < 0x80)) {
      return (False);
    }
    /*
 ...
 *
 ************************************************************************/
Boolean XmCvtTextToXmString(Display *display,
 ...
 */
   ...
   **  where V is in the range 02/00 thru 02/15 and represents versions 1 thru 16
   */
  if ((from->size >= 4) && (ctx->octet[0] == ESC) && (ctx->octet[1] == 0x23) &&
      (_IsInColumn2(ctx->octet[2]) && ((ctx->octet[3] == 0x30) || ctx->octet[3] == 0x31)))
  {
    ctx->version = ctx->octet[2] - 0x1f; /* 0x20-0x2f => version 1-16 */
    if (ctx->octet[3] == 0x30)           /* 0x30 == can ignore extensions */
    ...
    switch (*ctx->octet) { /* look at next octet in seq */
      case ESC:
        ...
        */
        ...
        ctx->itemlen++; /* advance ptr to next char */
        /* scan for final char */
        while ((ctx->octet != ctx->lastoctet) && (_IsInColumn2(*ctx->octet))) {
          ctx->octet++;
          ctx->itemlen++; /* advance ptr to next char */
        }
      ...
      case CSI:
        ...
        */
        ...
        ctx->itemlen++; /* advance ptr to next char */
        /* scan for final char */
        while ((ctx->octet != ctx->lastoctet) && _IsInColumn3(*ctx->octet)) {
          ctx->octet++;
          ctx->itemlen++; /* advance ptr to next char */
        }
        while ((ctx->octet != ctx->lastoctet) && _IsInColumn2(*ctx->octet)) {
...
static char **cvtCTsegment(ct_context *ctx, OctetPtr item, unsigned int length)
{
  ...
  ret_val = XmbTextPropertyToTextList(_XmGetDefaultDisplay(), &tmp_prop, &strings, &count);
  if (ret_val > 0) {
...
static void outputXmString(ct_context *ctx, Boolean separator)
{
  ...
  if (((ctx->gl_charset == CS_ISO8859_1) &&
       ((ctx->gr_charset == CS_ISO8859_1) || (ctx->gr_charset == CS_ISO8859_2) ||
        (ctx->gr_charset == CS_ISO8859_3) || (ctx->gr_charset == CS_ISO8859_4) ||
        (ctx->gr_charset == CS_ISO8859_5) || (ctx->gr_charset == CS_ISO8859_6) ||
        (ctx->gr_charset == CS_ISO8859_7) || (ctx->gr_charset == CS_ISO8859_8) ||
...
static Boolean processExtendedSegments(ct_context *ctx, Octet final)
{
   ...
   */
  if ((ctx->itemlen == 4) && (ctx->item[2] == 0x2f) && (_IsInColumn3(final))) {
    if (((ctx->lastoctet - ctx->octet) < 2) || (ctx->octet[0] < 0x80) || (ctx->octet[1] < 0x80)) {
      return (False);
    }
    /*
     ** The most significant bit of M and L are always set to 1
 ...
 *                                                                       *
 ***************************************************************************/
char *_XmConvertCSToString(XmString cs) /* unused */
{
  return ((char *)NULL);
}
 ...
 * 									   *
 ***************************************************************************/
Boolean _XmCvtXmStringToUTF8String(XrmValue *from, XrmValue *to)
{
 ...
 *
 ************************************************************************/
Boolean XmCvtXmStringToText(Display *display,
                            XrmValuePtr args,   /* unused */
                            Cardinal *num_args, /* unused */
                            XrmValue *from_val,
                            XrmValue *to_val,
...
static Boolean cvtXmStringToUTF8String(XrmValue *from, XrmValue *to)
{
  ...
  while ((comp = XmeStringGetComponent(&stack_context, True, False, &len, &val)) !=
         XmSTRING_COMPONENT_END)
  {
    switch (comp) {
      ...
      case XmSTRING_COMPONENT_DIRECTION:
        ...
        if (*(XmStringDirection *)val == XmSTRING_DIRECTION_L_TO_R) {
          if (prev_direction != ct_Dir_LeftToRight) {
            outc = ctextConcat(
                outc, outlen, (unsigned char *)UTF8_L_TO_R, (unsigned int)UTF8_L_TO_R_LEN);
          ...
          if (prev_direction != ct_Dir_RightToLeft) {
            outc = ctextConcat(
                outc, outlen, (unsigned char *)UTF8_R_TO_L, (unsigned int)UTF8_R_TO_L_LEN);
      ...
      case XmSTRING_COMPONENT_TAB:
        outc = ctextConcat(
            outc, outlen, (unsigned char *)UTF8_TABSTRING, (unsigned int)UTF8_TABSTRING_LEN);
        outlen++;
        break;
      default:
        /* Just ignore it. */
    ...
    switch (comp) {
      ...
      case XmSTRING_COMPONENT_TEXT:
        ...
        if (ct_encoding != NULL) {
          ...
          /* No mapping.  Vendor dependent. */
          ok = _XmOSProcessUnmappedCharsetAndText(
              cset_save, tmp, FALSE, &outc, &outlen, &prev_charset);
        }
        /* Free our local copy of val. */
        XmStackFree((char *)tmp, tmp_buf);
        if (!ok) {
      ...
      case XmSTRING_COMPONENT_DIRECTION:
        ...
        if (*(XmStringDirection *)val == XmSTRING_DIRECTION_L_TO_R) {
          if (prev_direction != ct_Dir_LeftToRight) {
            outc = ctextConcat(
                outc, outlen, (unsigned char *)CTEXT_L_TO_R, (unsigned int)CTEXT_L_TO_R_LEN);
          ...
          if (prev_direction != ct_Dir_RightToLeft) {
            outc = ctextConcat(
                outc, outlen, (unsigned char *)CTEXT_R_TO_L, (unsigned int)CTEXT_R_TO_L_LEN);
      ...
      case XmSTRING_COMPONENT_SEPARATOR:
        if (ct_encoding != NULL) { /* We have a mapping. */
          ...
          /* No mapping.  Vendor dependent. */
          ok = _XmOSProcessUnmappedCharsetAndText(
              cset_save, NULL, TRUE, &outc, &outlen, &prev_charset);
        }
        if (!ok) {
          _XmStringContextFree(&stack_context);
          return (False);
                                         ...
                                         ct_Charset *prev)
{
  ...
  if (strcmp(tag, XmFONTLIST_DEFAULT_TAG) == 0) {
    if (_XmStringIsCurrentCharset("UTF-8")) {
      ...
      ret_val = XmbTextListToTextProperty(
          _XmGetDefaultDisplay(), (char **)&ctext, 1, XUTF8StringStyle, &prop_rtn);
      if (ret_val) {
        switch (ret_val) {
          case XNoMemory:
            msg = MSG9;
  ...
  if (separator) {
    *outc = ctextConcat(
        *outc, *outlen, (unsigned char *)UTF8_NEWLINESTRING, (unsigned int)UTF8_NEWLINESTRING_LEN);
    (*outlen)++;
  }
  return (True);
}
                                     ...
                                     ct_Charset *prev)
{
  ...
  if (strcmp(tag, XmFONTLIST_DEFAULT_TAG) == 0) {
    ...
    /* Call XmbTextListToTextProperty */
    ret_val = XmbTextListToTextProperty(
        _XmGetDefaultDisplay(), (char **)&ctext, 1, XCompoundTextStyle, &prop_rtn);
    if (ret_val) {
      switch (ret_val) {
        case XNoMemory:
          msg = MSG9;
    ...
    if (separator) {
      *outc = ctextConcat(
          *outc, *outlen, (unsigned char *)NEWLINESTRING, (unsigned int)NEWLINESTRING_LEN);
      (*outlen)++;
    };
    *prev = cs_none;
    return (True);
  ...
  if (strcmp(tag, CS_ISO8859_1) == 0) {
    ...
    if (*prev != cs_ir111) {
      *outc = ctextConcat(
          *outc, *outlen, (unsigned char *)CTEXT_SET_IR_111, (unsigned int)CTEXT_SET_IR_111_LEN);
...
#if XM_UTF8
char *Convert(const char *str, unsigned int len, const char *to_codeset, const char *from_codeset)
{
  char *res;
  iconv_t cd;
  ...
  if (cd == (iconv_t)-1) {
    char msg[255];
    snprintf(
        msg, sizeof(msg), "Could not open converter from '%s' to '%s'", from_codeset, to_codeset);
    XmeWarning(NULL, msg);
    return NULL;
  }
...
char *ConvertWithIconv(const char *str, unsigned int len, iconv_t converter)
{
  char *dest;
  char *outp;
  const char *p;
