...
static SegmentEncoding  _mit_ISO_IR_1111_registry     = { "ISO-IR-111", "ISO-IR-111", NULL };
static SegmentEncoding  _mit_UTF_8_registry           = { "UTF-8", "UTF-8", &_mit_ISO_IR_1111_registry };
static SegmentEncoding  _mit_KSC5601_1987_1_registry  = { "KSC5601.1987-1", "KSC5601.1987-1", &_mit_UTF_8_registry };
static SegmentEncoding  _mit_KSC5601_1987_0_registry  = { "KSC5601.1987-0", "KSC5601.1987-0", &_mit_KSC5601_1987_1_registry };
static SegmentEncoding  _mit_JISX0208_1983_1_registry = { "JISX0208.1983-1", "JISX0208.1983-1", &_mit_KSC5601_1987_0_registry };
static SegmentEncoding  _mit_JISX0208_1983_0_registry = { "JISX0208.1983-0", "JISX0208.1983-0", &_mit_JISX0208_1983_1_registry };
static SegmentEncoding  _mit_GB2312_1980_1_registry   = { "GB2312.1980-1", "GB2312.1980-1", &_mit_JISX0208_1983_0_registry };
...
static SegmentEncoding  _mit_ISO8859_9_registry       = { "ISO8859-9", "ISO8859-9", &_mit_JISX0201_1976_0_registry };
static SegmentEncoding  _mit_ISO8859_8_registry       = { "ISO8859-8", "ISO8859-8", &_mit_ISO8859_9_registry };
static SegmentEncoding  _mit_ISO8859_7_registry       = { "ISO8859-7", "ISO8859-7", &_mit_ISO8859_8_registry };
static SegmentEncoding  _mit_ISO8859_6_registry       = { "ISO8859-6", "ISO8859-6", &_mit_ISO8859_7_registry };
static SegmentEncoding  _mit_ISO8859_5_registry       = { "ISO8859-5", "ISO8859-5", &_mit_ISO8859_6_registry };
static SegmentEncoding  _mit_ISO8859_4_registry       = { "ISO8859-4", "ISO8859-4", &_mit_ISO8859_5_registry };
static SegmentEncoding  _mit_ISO8859_3_registry       = { "ISO8859-3", "ISO8859-3", &_mit_ISO8859_4_registry };
static SegmentEncoding  _mit_ISO8859_2_registry       = { "ISO8859-2", "ISO8859-2", &_mit_ISO8859_3_registry };
static SegmentEncoding  _mit_ISO8859_1_registry       = { "ISO8859-1", "ISO8859-1", &_mit_ISO8859_2_registry };
static SegmentEncoding  _loc_encoding_registry        = { _MOTIF_DEFAULT_LOCALE, XmFONTLIST_DEFAULT_TAG, &_mit_ISO8859_1_registry };
static SegmentEncoding  _encoding_registry            = { XmFONTLIST_DEFAULT_TAG, XmFONTLIST_DEFAULT_TAG, &_loc_encoding_registry };
...
_XmGetEncodingRegistryTarget(int *length)
{
   ...
   while (current != NULL)
   {
      total_size += strlen(EncodingRegistryTag(current)) + strlen(EncodingRegistryEncoding(current)) + 2;
      ...
      { /* look at next octet in seq */
	    ...
	    */
            if (ctx->flags.text)
            {
               ...
               if (((ctx->octet[1] == 0x31) && (ctx->octet[2] == 0x5d)) || ((ctx->octet[1] == 0x32) && (ctx->octet[2] == 0x5d)) || (ctx->octet[1] == 0x5d))
         ...
         case HT:
            ...
            if (ctx->xmtab == NULL)
            {
               ctx->xmtab = XmStringComponentCreate(XmSTRING_COMPONENT_TAB, 0, NULL);
            }
            ctx->xmstring = XmStringConcatAndFree(ctx->xmstring,
      ...
      switch (final)
      {
	     ...
	     * contains an integral number of characters. */
            ...
            tempxm1        = XmStringConcatAndFree(
               XmStringDirectionCreate((_CurDir(ctx) == ct_Dir_LeftToRight ? XmSTRING_DIRECTION_L_TO_R : (_CurDir(ctx) == ct_Dir_RightToLeft ? XmSTRING_DIRECTION_R_TO_L : XmSTRING_DIRECTION_UNSET))),
   ...
   Boolean     separator)
{
   ...
   */
   ...
   if (strings)
   {
      ctx->xmstring = concatStringToXmString(ctx->xmstring,
                                             strings[0],
                                             strlen(strings[0]),
                                             XmFONTLIST_DEFAULT_TAG,
                                             (XmStringDirection)((_CurDir(ctx) == ct_Dir_LeftToRight) ? XmSTRING_DIRECTION_L_TO_R : ((_CurDir(ctx) == ct_Dir_RightToLeft) ? XmSTRING_DIRECTION_R_TO_L : XmSTRING_DIRECTION_UNSET)),
                                             separator);
      XFreeStringList(strings);
      return;
   }
   ...
   */
   if (((ctx->gl_charset == CS_ISO8859_1)
        && ((ctx->gr_charset == CS_ISO8859_1) || (ctx->gr_charset == CS_ISO8859_2) || (ctx->gr_charset == CS_ISO8859_3) || (ctx->gr_charset == CS_ISO8859_4) || (ctx->gr_charset == CS_ISO8859_5) || (ctx->gr_charset == CS_ISO8859_6) || (ctx->gr_charset == CS_ISO8859_7) || (ctx->gr_charset == CS_ISO8859_8) || (ctx->gr_charset == CS_ISO8859_9)))
       || ((ctx->gl_charset == CS_GB2312_0) && (ctx->gr_charset == CS_GB2312_1))
       || ((ctx->gl_charset == CS_KSC5601_0) && (ctx->gr_charset == CS_KSC5601_1)))
   {
   ...
   else
   {
      ...
      curseg_is_gl = (ctx->item[0] >= 0 && ctx->item[0] <= 127);
      while (j < ctx->itemlen)
      {
         c = ctx->item[j];
         if ((c >= 0 && c <= 127))
...
{
   XmString tempxm1;
   tempxm1 = XmStringConcatAndFree(XmStringDirectionCreate(direction),
   ...
   XmString string)
{
   ...
   if (!ok)
   {
      XtWarningMsg("conversionError", "compoundText", "XtToolkitError", MSG8, NULL, NULL);
      return ((char *)NULL);
   }
   ...
   while ((comp = XmeStringGetComponent(&stack_context, True, False, &len, &val)) != XmSTRING_COMPONENT_END)
   {
      switch (comp)
      {
         ...
         case XmSTRING_COMPONENT_TEXT:
            ...
            {
               /* We have a mapping. */
               ok = processCharsetAndTextUtf8(ct_encoding, tmp, FALSE, &outc, &outlen, &prev_charset);
            }
            else
            {
               /* No mapping.  Vendor dependent. */
               ok = processCharsetAndTextUtf8(cset_save, tmp, FALSE, &outc, &outlen, &prev_charset);
            }
         ...
         case XmSTRING_COMPONENT_DIRECTION:
            ...
            if (*(XmStringDirection *)val == XmSTRING_DIRECTION_L_TO_R)
            {
               ...
               {
                  outc           = ctextConcat(outc, outlen, (unsigned char *)UTF8_L_TO_R, (unsigned int)UTF8_L_TO_R_LEN);
            ...
            else
            {
               ...
               {
                  outc           = ctextConcat(outc, outlen, (unsigned char *)UTF8_R_TO_L, (unsigned int)UTF8_R_TO_L_LEN);
         ...
         case XmSTRING_COMPONENT_SEPARATOR:
            if (ct_encoding != NULL)
            { /* We have a mapping. */
               ok = processCharsetAndTextUtf8(ct_encoding, NULL, TRUE, &outc, &outlen, &prev_charset);
            }
            else
            {
               /* No mapping.  Vendor dependent. */
               ok = processCharsetAndTextUtf8(cset_save, NULL, TRUE, &outc, &outlen, &prev_charset);
            }
            if (!ok)
            {
               _XmStringContextFree(&stack_context);
         ...
         case XmSTRING_COMPONENT_TAB:
            outc = ctextConcat(outc, outlen, (unsigned char *)UTF8_TABSTRING, (unsigned int)UTF8_TABSTRING_LEN);
            outlen++;
            break;
         default:
            /* Just ignore it. */
   ...
   while ((comp = XmeStringGetComponent(&stack_context, True, False, &len, &val)) != XmSTRING_COMPONENT_END)
   {
      switch (comp)
      {
         ...
         case XmSTRING_COMPONENT_TEXT:
            ...
            {
               /* We have a mapping. */
               ok = processCharsetAndText(ct_encoding, tmp, FALSE, &outc, &outlen, &prev_charset);
            }
            else
            {
               /* No mapping.  Vendor dependent. */
               ok = _XmOSProcessUnmappedCharsetAndText(cset_save, tmp, FALSE, &outc, &outlen, &prev_charset);
            }
            /* Free our local copy of val. */
            XmStackFree((char *)tmp, tmp_buf);
            if (!ok)
         ...
         case XmSTRING_COMPONENT_DIRECTION:
            ...
            if (*(XmStringDirection *)val == XmSTRING_DIRECTION_L_TO_R)
            {
               if (prev_direction != ct_Dir_LeftToRight)
               {
                  outc           = ctextConcat(outc, outlen, (unsigned char *)CTEXT_L_TO_R, (unsigned int)CTEXT_L_TO_R_LEN);
            ...
            else
            {
               if (prev_direction != ct_Dir_RightToLeft)
               {
                  outc           = ctextConcat(outc, outlen, (unsigned char *)CTEXT_R_TO_L, (unsigned int)CTEXT_R_TO_L_LEN);
         ...
         case XmSTRING_COMPONENT_SEPARATOR:
            if (ct_encoding != NULL)
            { /* We have a mapping. */
               ok = processCharsetAndText(ct_encoding, NULL, TRUE, &outc, &outlen, &prev_charset);
            }
            else
            {
               /* No mapping.  Vendor dependent. */
               ok = _XmOSProcessUnmappedCharsetAndText(cset_save, NULL, TRUE, &outc, &outlen, &prev_charset);
            }
            if (!ok)
            {
               _XmStringContextFree(&stack_context);
            ...
            break;
         case XmSTRING_COMPONENT_TAB:
            outc = ctextConcat(outc, outlen, (unsigned char *)TABSTRING, (unsigned int)TABSTRING_LEN);
            outlen++;
            break;
         default:
            /* Just ignore it. */
                          ...
                          ct_Charset     *prev)
{
   ...
   if (strcmp(tag, XmFONTLIST_DEFAULT_TAG) == 0)
   {
      ...
      else
      {
         ...
         ret_val = XmbTextListToTextProperty(_XmGetDefaultDisplay(),
                                             (char **)&ctext,
                                             1,
                                             XUTF8StringStyle,
                                             &prop_rtn);
         if (ret_val)
         {
            ...
            XtWarningMsg("conversionError", "textProperty", "XtToolkitError", msg, NULL, 0);
            return (False);
         }
      ...
      if (separator)
      {
         *outc = ctextConcat(*outc, *outlen, (unsigned char *)UTF8_NEWLINESTRING, (unsigned int)UTF8_NEWLINESTRING_LEN);
         (*outlen)++;
      };
      *prev = cs_none;
      return (True);
                      ...
                      ct_Charset     *prev)
{
   ...
   if (strcmp(tag, XmFONTLIST_DEFAULT_TAG) == 0)
   {
      ...
      String        msg;
      /* Call XmbTextListToTextProperty */
      ret_val = XmbTextListToTextProperty(_XmGetDefaultDisplay(), (char **)&ctext, 1, XCompoundTextStyle, &prop_rtn);
      if (ret_val)
      {
         switch (ret_val)
         {
      ...
      if (separator)
      {
         *outc = ctextConcat(*outc, *outlen, (unsigned char *)NEWLINESTRING, (unsigned int)NEWLINESTRING_LEN);
         (*outlen)++;
      };
      *prev = cs_none;
      return (True);
   ...
   if (strcmp(tag, CS_ISO8859_1) == 0)
   {
      if (*prev != cs_Latin1)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_1, (unsigned int)CTEXT_SET_ISO8859_1_LEN);
   ...
   else if (strcmp(tag, CS_ISO8859_2) == 0)
   {
      if (*prev != cs_Latin2)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_2, (unsigned int)CTEXT_SET_ISO8859_2_LEN);
   ...
   else if (strcmp(tag, CS_ISO8859_3) == 0)
   {
      if (*prev != cs_Latin3)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_3, (unsigned int)CTEXT_SET_ISO8859_3_LEN);
   ...
   else if (strcmp(tag, CS_ISO8859_4) == 0)
   {
      if (*prev != cs_Latin4)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_4, (unsigned int)CTEXT_SET_ISO8859_4_LEN);
   ...
   else if (strcmp(tag, CS_ISO8859_5) == 0)
   {
      ...
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_5, (unsigned int)CTEXT_SET_ISO8859_5_LEN);
   ...
   else if (strcmp(tag, CS_ISO8859_6) == 0)
   {
      ...
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_6, (unsigned int)CTEXT_SET_ISO8859_6_LEN);
   ...
   else if (strcmp(tag, CS_ISO8859_7) == 0)
   {
      ...
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_7, (unsigned int)CTEXT_SET_ISO8859_7_LEN);
   ...
   else if (strcmp(tag, CS_ISO8859_8) == 0)
   {
      ...
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_8, (unsigned int)CTEXT_SET_ISO8859_8_LEN);
         *outlen += CTEXT_SET_ISO8859_8_LEN;
         *prev    = cs_LatinHebrew;
      };
   }
   else if (strcmp(tag, CS_ISO8859_9) == 0)
   {
      if (*prev != cs_Latin5)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_ISO8859_9, (unsigned int)CTEXT_SET_ISO8859_9_LEN);
   ...
   else if (strcmp(tag, CS_JISX0201) == 0)
   {
      if (*prev != cs_Katakana)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_JISX0201, (unsigned int)CTEXT_SET_JISX0201_LEN);
         *outlen += CTEXT_SET_JISX0201_LEN;
         *prev    = cs_Katakana;
      };
   }
   else if ((strcmp(tag, CS_GB2312_0) == 0) || (strcmp(tag, CS_GB2312_1) == 0))
   {
      if (*prev != cs_Hanzi)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_GB2312_0, (unsigned int)CTEXT_SET_GB2312_0_LEN);
         *outlen += CTEXT_SET_GB2312_0_LEN;
         *prev    = cs_Hanzi;
      };
   }
   else if ((strcmp(tag, CS_JISX0208_0) == 0) || (strcmp(tag, CS_JISX0208_1) == 0))
   {
      if (*prev != cs_JapaneseGCS)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_JISX0208_0, (unsigned int)CTEXT_SET_JISX0208_0_LEN);
         *outlen += CTEXT_SET_JISX0208_0_LEN;
         *prev    = cs_JapaneseGCS;
      };
   }
   else if ((strcmp(tag, CS_KSC5601_0) == 0) || (strcmp(tag, CS_KSC5601_1) == 0))
   {
      if (*prev != cs_KoreanGCS)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_KSC5601_0, (unsigned int)CTEXT_SET_KSC5601_0_LEN);
   ...
   else if (strcmp(tag, CS_ISO_IR_111) == 0)
   {
      if (*prev != cs_ir111)
      {
         *outc    = ctextConcat(*outc, *outlen, (unsigned char *)CTEXT_SET_IR_111, (unsigned int)CTEXT_SET_IR_111_LEN);
        ...
        const char  *from_codeset)
{
   ...
   {
      char msg[255];
      snprintf(msg, sizeof(msg), "Could not open converter from '%s' to '%s'", from_codeset, to_codeset);
      XmeWarning(NULL, msg);
      return NULL;
   }
...
again:
   ...
   if (err == (size_t)-1)
   {
      switch (errno)
      {
         ...
         {
            char msg[255];
            snprintf(msg, sizeof(msg), "Error during conversion: %s", strerror(errno));
            XmeWarning(NULL, msg);
            have_error = TRUE;
            break;
         }
