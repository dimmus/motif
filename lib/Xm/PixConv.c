...
#define DEPTH(widget) \
  (XtIsWidget(widget)) ? ((widget)->core.depth) : ((XtParent(widget))->core.depth)
/********    Static Function Declarations    ********/
static Boolean GetColorInfo(Widget widget, XmAccessColorData acc_color);
static Pixmap GetPixmap(Widget widget,
                        unsigned char conv_type,
                        String image_name,
                        Boolean scaling);
static Boolean CvtStringToPixmap(Display *dpy,
                                 XrmValue *args,
                                 Cardinal *numArgs,
                                 XrmValue *fromVal,
                                 XrmValue *toVal,
...
static XtConvertArgRec bitmapNoScalingArgs[] = {{XtBaseOffset, (XtPointer)0, sizeof(int)},
                                                {XtAddress, (XtPointer)CONVERT_BITMAP, 0},
                                                {XtAddress, (XtPointer)False, 0}};
static XtConvertArgRec dynamicArgs[] = {{XtBaseOffset, (XtPointer)0, sizeof(int)},
                                        {XtAddress, (XtPointer)CONVERT_DYNAMIC, 0},
                                        {XtAddress, (XtPointer)True, 0}};
static XtConvertArgRec dynamicNoScalingArgs[] = {{XtBaseOffset, (XtPointer)0, sizeof(int)},
                                                 {XtAddress, (XtPointer)CONVERT_DYNAMIC, 0},
                                                 {XtAddress, (XtPointer)False, 0}};
static XtConvertArgRec pixmapArgs[] = {{XtBaseOffset, (XtPointer)0, sizeof(int)},
                        ...
                        Boolean scaling)
{
  ...
  pixmap = _XmGetScaledPixmap(
      screen, widget, image_name, &acc_color_rec, depth, FALSE, scaling_ratio, 0, 0); /* pass
                     scaling down to ImageCache */
  return pixmap;
}
...
static Boolean GetColorInfo(Widget widget, XmAccessColorData acc_color)
{
  ...
  if (access_colors_trait) {
    acc_color->valueMask = AccessForeground | AccessBackgroundPixel | AccessHighlightColor |
    ...
    acc_color->highlight_color = acc_color->top_shadow_color = acc_color->bottom_shadow_color =
        acc_color->select_color = XmUNSPECIFIED_PIXEL;
  }
  return True;
}
 ...
 *
 ************************************************************************/
void _XmTopShadowPixmapDefault(Widget widget,
                               int offset, /* unused */
                               XrmValue *value)
{
...
void _XmHighlightPixmapDefault(Widget widget,
                               int offset, /* unused */
                               XrmValue *value)
{
  static Pixmap pixmap;
