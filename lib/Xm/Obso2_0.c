   ...
   Widget w)
{
   ...
   {
      sw->swindow.vExtent = (sw->swindow.ClipWindow->core.height > 0) ? sw->swindow.ClipWindow->core.height : 1;
      sw->swindow.hExtent = (sw->swindow.ClipWindow->core.width > 0) ? sw->swindow.ClipWindow->core.width : 1;
   ...
   Widget parent)
{
   ...
   {
      if (w->core.background_pixel != parent->core.background_pixel || w->core.background_pixmap != parent->core.background_pixmap)
         return (True);
   }
   return (False);
}
   ...
   XmColorData *colors)
{
   ...
   if (background == BlackPixelOfScreen(screen))
   {
      ...
      colors->bottom_shadow.red = colors->bottom_shadow.green = colors->bottom_shadow.blue = XmMAX_SHORT;
   ...
   Colormap color_map)
{
   ...
   if (DefaultDepthOfScreen(screen) == 1)
   {
      ...
      if (!XtCallConverter(DisplayOfScreen(screen), XtCvtStringToPixel, args, num_args, &fromVal, &toVal, NULL))
         background = WhitePixelOfScreen(screen);
   }
   else
   {
      /*  Parse out a color for the default background  */
      if (XParseColor(DisplayOfScreen(screen), color_map, _XmGetDefaultBackgroundColorSpec(screen), &color_def))
      {
   ...
   unsigned char which)
{
   ...
   {
      case XmBACKGROUND:
         if (!(cd->allocated & which) && (XAllocColor(cd->screen->display, cd->color_map, &(cd->background)) == 0))
         {
         ...
         break;
      case XmFOREGROUND:
         if (!(cd->allocated & which) && (XAllocColor(cd->screen->display, cd->color_map, &(cd->foreground)) == 0))
         {
            if (_XmBrightness(&(cd->background))
                < XmFOREGROUND_THRESHOLD)
         ...
         break;
      case XmTOP_SHADOW:
         if (!(cd->allocated & which) && (XAllocColor(cd->screen->display, cd->color_map, &(cd->top_shadow)) == 0))
         {
            if (_XmBrightness(&(cd->background))
      ...
      case XmBOTTOM_SHADOW:
         if (!(cd->allocated & which) && (XAllocColor(cd->screen->display, cd->color_map, &(cd->bottom_shadow)) == 0))
         {
            if (_XmBrightness(&(cd->background))
         ...
         break;
      case XmSELECT:
         if (!(cd->allocated & which) && (XAllocColor(cd->screen->display, cd->color_map, &(cd->select)) == 0))
         {
            if (_XmBrightness(&(cd->background))
                < XmFOREGROUND_THRESHOLD)
...
void
_XmTextClearDestination(XmTextWidget tw, Boolean ignore_sens)
{
}
   ...
   XmStringTag   tag)
{
   ...
   XmStringDrawUnderline(d, w, rendertable, string, gc, x, y, width, align, lay_dir, clip, underline);
...
/* From BaseClass.c */
void
_XmFreeWidgetExtData(Widget widget) /* unused */
{
}

void
_XmBaseClassPartInitialize(WidgetClass wc) /* unused */
{
}

Boolean
_XmIsSlowSubclass(WidgetClass wc, unsigned int bit)
{
