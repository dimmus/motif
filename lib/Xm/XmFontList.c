   ...
   XtPointer  font)
{
   ...
   n++;
   ret_val = XmRenditionCreate(NULL,
   ...
   char      *tag)
{
   ...
   XtSetArg(args[n], XmNloadModel, XmLOAD_IMMEDIATE);
   n++;
   ret_val = _XmRenditionCreate(display, NULL, XmS, XmCFontList, _XmStringCacheTag(tag, XmSTRING_TAG_STRLEN), args, n, NULL);
   _XmAppUnlock(app);
   return ret_val;
}
   ...
   XmStringCharSet charset)
{
   ...
   XtSetArg(args[n], XmNloadModel, XmLOAD_IMMEDIATE);
   n++;
   rends[0] = XmRenditionCreate(NULL,
...
*/
XmFontList
XmStringCreateFontList_r(XFontStruct *font, XmStringCharSet charset, Widget wid)
{ /* deprecated */
   (void)wid;
   return XmFontListAppendEntry(NULL, XmFontListEntryCreate(charset, XmFONT_IS_FONT, font));
}
