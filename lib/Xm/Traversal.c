   ...
   if ((focusData = _XmGetFocusData(new_wid)) != NULL)
   {
          ...
          && (focusData->focus_policy == XmEXPLICIT))
      {
         ...
         else
         {
		   ...
		   */
               Widget new_focus = _XmTraverseAway(graph, new_wid, (focusData->active_tab_group != new_wid));
               if (!new_focus)
               {
   ...
   XmTraversalDirection dir)
{
        ...
        prev = ancestor)
   {
      ...
      if (!_XmIntersectionOf(&focus_rect, &clip_rect, &view_rect) || (view_rect.width != focus_rect.width) || (view_rect.height != focus_rect.height))
      {
   ...
   Widget w)
{
   ...
   */
   ...
   if (XtIsVendorShell(topmost_shell))
   {
      ...
      return (((XmVendorShellExtObject)(xwed)->widget)
                 ->vendor.focus_policy);
   }
   else
   {
...
_XmGetFocusData(Widget wid)
{
   ...
   if (wid && !(wid->core.being_destroyed))
   {
      ...
      else
      {
         ...
         {
            ((XmMenuShellWidget)wid)->menu_shell.focus_data->focus_policy = ((XmMenuShellWidget)wid)->menu_shell.focus_policy;
                         ...
                         Boolean     allow_scrolling)
{
   ...
   */
   ...
   if (allow_scrolling && w && XtParent(w) && ((sw = _XmIsScrollableClipWidget(w, True, rectPtr)) != NULL))
   {
      w = sw;
      if (!_XmIsViewable(w))
      {
                      ...
                      int    root_y)
{
   ...
   {
      return ((root_x >= rect.x) && (root_x < rect.x + (int)rect.width) && (root_y >= rect.y) && (root_y < rect.y + (int)rect.height));
   }
   return False;
}
   ...
   XmTraversalDirection direction)
{
       ...
       && (focus_data->focus_policy == XmEXPLICIT))
   {
      ...
      nav_wid               = RedirectTraversal(focus_data->focus_item, nav_wid, focus_data->focus_policy, local_dir, 0);
      if (trav_list->num_entries
          && (focus_data->focalPoint == XmUnrelated)
   ...
   Widget wid)
{
   ...
   children = NULL;
   if (!(parent_window = XtWindow(XtParent(wid))) || XQueryTree(XtDisplay(wid), parent_window, &rootwindow, &p_window, &children, &numchildren) == 0)
   {
