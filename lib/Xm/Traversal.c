...
#include <Xm/VirtKeysP.h>
    ...
    static Widget
    FindFirstManaged(Widget wid);
static Boolean CallTraverseObsured(Widget new_focus, XmTraversalDirection dir);
static Boolean IsTraversable(Widget wid, Boolean require_in_view);
static Widget FindFirstFocus(Widget wid);
...
/*ARGSUSED*/
void _XmNavigInitialize(Widget request, /* unused */
                        Widget new_wid,
                        ArgList args,       /* unused */
                        Cardinal *num_args) /* unused */
{
  ...
  if ((focusData = _XmGetFocusData(new_wid)) != NULL) {
    ...
    if (focusData->trav_graph.num_entries && _XmGetNavigability(new_wid)) {
...
/*ARGSUSED*/
Boolean _XmNavigSetValues(Widget current,
                          ...
                          Cardinal *num_args) /* unused */
{
  ...
  if ((focusData = _XmGetFocusData(new_wid)) != NULL) {
    ...
    if (curNavType != newNavType) {
      if ((curNavType == XmEXCLUSIVE_TAB_GROUP) || (newNavType == XmEXCLUSIVE_TAB_GROUP)) {
      ...
      if ((newNavType == XmEXCLUSIVE_TAB_GROUP) || (newNavType == XmSTICKY_TAB_GROUP)) {
        if ((curNavType != XmEXCLUSIVE_TAB_GROUP) && (curNavType != XmSTICKY_TAB_GROUP)) {
    ...
    if (XtIsRealized(new_wid) && (focusData->focus_policy == XmEXPLICIT)) {
      ...
      if (!(focusData->focus_item)) {
        ...
        if (XmIsTraversable(new_wid) && (shell = _XmFindTopMostShell(new_wid)) &&
        ...
        if ((focusData->focus_item == new_wid) && !IsTraversable(new_wid, TRUE)) {
          ...
          Widget new_focus = _XmTraverseAway(
              graph, new_wid, (focusData->active_tab_group != new_wid));
          if (!new_focus) {
...
void XmeNavigChangeManaged(Widget wid)
{
  ...
  _XmAppLock(app);
  if (XtIsRealized(wid) && (focus_data = _XmGetFocusData(wid)) &&
...
static Widget FindFirstManaged(Widget wid)
{
  if (XtIsShell(wid)) {
    unsigned i = 0;
    while (i < ((CompositeWidget)wid)->composite.num_children) {
      if (XtIsManaged(((CompositeWidget)wid)->composite.children[i])) {
...
void _XmNavigResize(Widget wid)
{
   ...
   */
  XmFocusData focus_data;
  if (XtIsRealized(wid) && !XtIsShell(wid) && (focus_data = _XmGetFocusData(wid))) {
    ...
    if ((focus_data->focus_policy == XmEXPLICIT) &&
        (!(focus_data->focus_item) || !((focus_data->focus_item)->core.being_destroyed)))
    {
      if (!(focus_data->focus_item)) {
        ...
        Widget firstManaged;
        if (parent && XtIsShell(parent) && (firstManaged = FindFirstManaged(parent))) {
          /* Set bootstrap trigger for hierarchy that
           * has no current focus.
           */
          XtSetKeyboardFocus(wid, firstManaged);
        ...
        if (!IsTraversable(focus_data->focus_item, TRUE)) {
          ...
          if (!IsTraversable(focus_data->focus_item, FALSE) ||
              !_XmMgrTraversal(focus_data->focus_item, XmTRAVERSE_CURRENT))
          {
            Widget new_focus = _XmTraverseAway(
                &(focus_data->trav_graph),
                focus_data->focus_item,
...
void _XmValidateFocus(Widget wid)
{
  ...
  if (focus_data && (focus_data->focus_policy == XmEXPLICIT) && (focus_data->focus_item != NULL) &&
      !IsTraversable(focus_data->focus_item, TRUE))
  {
  ...
  if (focusData) {
    ...
    if (focusData->focus_item == wid) {
      ...
      if ((focusData->focus_policy != XmEXPLICIT) ||
          (!(new_focus = _XmTraverseAway(
                 trav_list, focusData->focus_item, (focusData->active_tab_group != wid))) &&
  ...
  if (XmIsVendorShell(topShell)) {
    ...
    if ((vendorExt = (XmVendorShellExtObject)extData->widget) != NULL) {
      if ((callbacks = vendorExt->vendor.focus_moved_callback) != NULL) {
        ...
        _XmCallCallbackList((Widget)vendorExt, callbacks, (XtPointer)&callData);
...
#define traversal_in_progress ((XmDisplayInfo *)(dd->display.displayInfo))->traversal_in_progress
  if (traversal_in_progress || !(top_shell = _XmFindTopMostShell(wid)) ||
      top_shell->core.being_destroyed || !(focus_data = _XmGetFocusData(wid)) ||
      (focus_data->focus_policy != XmEXPLICIT))
  {
    return FALSE;
  }
  ...
  if ((old_focus == NULL) && (wid == top_shell) && focus_data->first_focus &&
  ...
  new_focus = RedirectTraversal(old_focus, new_focus, focus_data->focus_policy, local_dir, 0);
  if (new_focus && (new_focus == old_focus) && focus_data->old_focus_item) {
  ...
  else if (new_focus && (new_active_tab = XmGetTabGroup(new_focus)) &&
           ...
           CallTraverseObsured(new_focus, local_dir))
  {
    ...
    if ((new_active_tab != new_focus) && XmIsManager(new_active_tab)) {
     ...
     */
    if (!old_focus || !IsTraversable(old_focus, TRUE)) {
  ...
  if (trav_list->num_entries && (focus_data->focalPoint == XmUnrelated) &&
      (XmIsVendorShell(top_shell) || !XmeFocusIsInShell(top_shell)))
  {
...
static Boolean CallTraverseObsured(Widget new_focus, XmTraversalDirection dir)
{
  ...
  for (prev = ancestor; ((ancestor = XtParent(ancestor)) != NULL) && !XtIsShell(ancestor);
       prev = ancestor)
  {
    ...
    if (!_XmIntersectionOf(&focus_rect, &clip_rect, &view_rect) ||
        (view_rect.width != focus_rect.width) || (view_rect.height != focus_rect.height))
    {
      /* This ancestor clips somebody. */
      Widget sw = _XmIsScrollableClipWidget(prev, True, &focus_rect);
      if (sw != NULL) {
        XtCallbackList callbacks = ((XmScrolledWindowWidget)sw)->swindow.traverseObscuredCallback;
...
Boolean _XmFocusIsHere(Widget w)
{
  XmFocusData focus_data;
  Widget item;
  if ((focus_data = _XmGetFocusData(w)) && (item = focus_data->focus_item)) {
...
unsigned char _XmGetFocusPolicy(Widget w)
{
  ...
  if (XtIsVendorShell(topmost_shell)) {
    ...
    if (xwed == NULL)
      return (XmPOINTER);
    return (((XmVendorShellExtObject)(xwed)->widget)->vendor.focus_policy);
  }
  else {
    ...
    if (new_focus_policy == XmEXPLICIT) {
      ...
      if (new_item != NULL) {
        if (XmIsManager(new_item) &&
            (((XmManagerWidget)new_item)->manager.highlighted_widget != NULL))
        {
          new_item = ((XmManagerWidget)new_item)->manager.highlighted_widget;
        }
        _XmWidgetFocusChange(new_item, XmLEAVE);
      }
      if ((new_item == NULL) || !_XmMgrTraversal(new_item, XmTRAVERSE_CURRENT)) {
        _XmMgrTraversal(shell, XmTRAVERSE_CURRENT);
      }
    }
...
XmFocusData _XmGetFocusData(Widget wid)
{
  ...
  if (wid && !(wid->core.being_destroyed)) {
    if (XmIsVendorShell(wid)) {
      ...
      if (XmIsMenuShell(wid) && ((XmMenuShellWidget)wid)->menu_shell.focus_data) {
        ((XmMenuShellWidget)wid)->menu_shell.focus_data->focus_policy =
...
Boolean _XmComputeVisibilityRect(Widget w,
                                 XRectangle *rectPtr,
...
Boolean _XmGetPointVisibility(Widget w, int root_x, int root_y)
{
  ...
  if (_XmComputeVisibilityRect(w, &rect, TRUE, FALSE)) {
    return ((root_x >= rect.x) && (root_x < rect.x + (int)rect.width) && (root_y >= rect.y) &&
...
void _XmWidgetFocusChange(Widget wid, XmFocusChange change)
{
  ...
  if (XtIsRectObj(wid) && !wid->core.being_destroyed) {
    if ((er = _XmGetBaseClassExtPtr(XtClass(wid), XmQmotif)) && (*er) &&
        ((*er)->version >= XmBaseClassExtVersion) && (*er)->focusChange)
    {
      (*((*er)->focusChange))(wid, change);
    }
    else { /* From here on is compatibility code.
            */
      WidgetClass wc;
...
Widget _XmNavigate(Widget wid, XmTraversalDirection direction)
{
  ...
  Widget nav_wid = NULL;
  Widget shell = _XmFindTopMostShell(wid);
  if ((focus_data = _XmGetFocusData(shell)) && (focus_data->focus_policy == XmEXPLICIT)) {
    XmTravGraph trav_list = &(focus_data->trav_graph);
    nav_wid = _XmTraverse(trav_list, direction, &local_dir, wid);
    nav_wid = RedirectTraversal(
        focus_data->focus_item, nav_wid, focus_data->focus_policy, local_dir, 0);
    if (trav_list->num_entries && (focus_data->focalPoint == XmUnrelated) &&
        (XmIsVendorShell(shell) || !XmeFocusIsInShell(shell)))
    {
      _XmFreeTravGraph(trav_list);
    }
  }
...
void _XmSetInitialOfTabGroup(Widget tab_group, Widget init_focus)
{
  ...
  if ((focus_data = _XmGetFocusData(tab_group)) && focus_data->trav_graph.num_entries) {
    _XmSetInitialOfTabGraph(&(focus_data->trav_graph), tab_group, init_focus);
  }
}

static Boolean IsTraversable(Widget wid, Boolean require_in_view)
{
  if (wid && _XmIsNavigable(wid)) {
    if (require_in_view) {
...
Boolean XmeFocusIsInShell(Widget wid)
{
  ...
  _XmWidgetToAppContext(wid);
  _XmAppLock(app);
  if (XmIsVendorShell(shell_of_wid) && (focus_data = _XmGetFocusData(shell_of_wid))) {
...
{
  XmFocusData focusData = _XmGetFocusData(wid);
  if (focusData && focusData->trav_graph.exclusive) {
    return TRUE;
  }
  return FALSE;
}
...
Widget XmGetTabGroup(Widget wid)
{
  ...
  _XmWidgetToAppContext(wid);
  _XmAppLock(app);
  if (!wid || (_XmGetFocusPolicy(wid) != XmEXPLICIT) || !(focus_data = _XmGetFocusData(wid))) {
    _XmAppUnlock(app);
    return (NULL);
  }
  ...
  do {
    ...
    if ((nav_type == XmSTICKY_TAB_GROUP) || (nav_type == XmEXCLUSIVE_TAB_GROUP) ||
        ((nav_type == XmTAB_GROUP) && !exclusive))
    {
      _XmAppUnlock(app);
      return (wid);
    }
...
Widget XmGetFocusWidget(Widget wid)
{
  ...
  if (focus_data != NULL) {
    if (focus_data->focus_policy == XmEXPLICIT) {
      ...
      if ((focus_wid != NULL) && XmIsManager(focus_wid) &&
          (((XmManagerWidget)focus_wid)->manager.highlighted_widget != NULL))
      {
        focus_wid = ((XmManagerWidget)focus_wid)->manager.highlighted_widget;
      }
    }
  }
  _XmAppUnlock(app);
...
Boolean XmProcessTraversal(Widget w, XmTraversalDirection dir)
{
  ...
  if (w) {
    ...
    _XmAppLock(app);
    if ((focus_data = _XmGetFocusData(w)) && (focus_data->focus_policy == XmEXPLICIT)) {
                                ...
                                unsigned int pass)
{
  ...
  for (parent = old_focus; parent != NULL; parent = XtParent(parent)) {
    trav_trait = (XmTraversalControlTrait)XmeTraitGet((XtPointer)XtClass(parent),
                                                      XmQTtraversalControl);
    if (trav_trait && trav_trait->redirect) {
      proposal = trav_trait->redirect(old_focus, new_focus, focus_policy, direction, pass);
      if (proposal != new_focus)
        return RedirectTraversal(old_focus, proposal, focus_policy, direction, pass + 1);
    }
  }
