...
GetLocaleEncodingAtom(Display *dpy)
{
   ...
   ret_status     = XmbTextListToTextProperty(dpy, &tmp_string, 1, (XICCEncodingStyle)XTextStyle, &tmp_prop);
 ...
 ************************************************************************/
static int
TextPropertyToSingleTextItem(Display *display, XTextProperty *text_prop, char **text_item)
{
   int    result, count;
   char **textlist;
   result = XmbTextPropertyToTextList(display, text_prop, &textlist, &count);
               ...
               unsigned char   texttype)
{
   ...
   *buffer       = NULL;
   ...
   if (_XmStringGetSegment(xmcontext, TRUE, FALSE, &text, &tag, &type, &rendition_tags, &tag_count, &direction, &separator, &tabs, &char_count, &push_before, &pop_after))
   {
      switch (type)
      {
         ...
         case XmCHARSET_TEXT:
            ...
            if (encoding != NULL)
            {
               if (texttype == _LOCALE_TEXT && (strcmp(encoding, _MOTIF_DEFAULT_LOCALE) == 0 || strcmp(encoding, XmFONTLIST_DEFAULT_TAG) == 0))
		     ...
		     already be valid locale text. */
               ...
               else if (texttype == _STRING_TEXT && (strcmp(encoding, "ISO8859-1") == 0))
               {
      ...
      if ((return_status == _VALID_SEGMENT) && (separator == True || tabs != 0))
      {
         int   newlength;
         char *newstring;
 ...
 ************************************************************************/
static int
GetUseableText(Display *display, XmString xmstring, char **buffer, Boolean strict, unsigned char texttype)
{
   ...
   *buffer = NULL;
   ...
   while ((return_status = GetTextSegment(display, &stack_context, xmstring, &text, texttype)) == _VALID_SEGMENT)
   {
   ...
   if (return_status == _INVALID_SEGMENT)
   {
      ...
      text_prop_return.encoding       = XInternAtom(display, XmSCOMPOUND_TEXT, False);
      text_prop_return.format         = 8;
      XtFree(compound_text);
      /* then to a text list, which should result in text. */
      result = TextPropertyToSingleTextItem(display, &text_prop_return, &text_item);
                                 ...
                                 XTextProperty     *text_prop_return)
{
   ...
   switch (style)
   {
	 ...
	 to ASN1 strings, putting them directly into the buffer. */
         text_prop_return->value = ubufptr = (unsigned char *)XtMalloc(sizeof(unsigned char) * total_size);
         for (i = 0; i < count; ++i)
         {
            int size;
         ...
         for (i = 0; i < count; ++i)
         {
            ...
            if (result != Success)
            {
			 ...
			 and return whatever we got back. */
                     _XmAppUnlock(app);
                     return (XmCvtXmStringTableToTextProperty(display, string_table, count, XmSTYLE_COMPOUND_TEXT, text_prop_return));
                  }
                  else
                  {
                                 ...
                                 int           *count_return)
{
   ...
   {
      if (text_prop->value[i] == '\0')
         string_table[elements++] = XmStringGenerate((XtPointer) & (text_prop->value[i + 1]),
                                                     tag,
                                                     type,
                                                     NULL);
   }
