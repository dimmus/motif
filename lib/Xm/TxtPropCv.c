...
static Atom GetLocaleEncodingAtom(Display *dpy)
{
  ...
  ret_status = XmbTextListToTextProperty(
      dpy, &tmp_string, 1, (XICCEncodingStyle)XTextStyle, &tmp_prop);
 ...
 *
 ************************************************************************/
static int TextPropertyToSingleTextItem(Display *display,
...
static unsigned char GetTextSegment(Display *display, /* unused */
                                    ...
                                    unsigned char texttype)
{
  ...
  *buffer = NULL;
  ...
  if (_XmStringGetSegment(xmcontext,
                          TRUE,
                          FALSE,
                          &text,
                          &tag,
                          ...
                          &pop_after))
  {
    switch (type) {
      ...
      case XmCHARSET_TEXT:
        ...
        if (encoding != NULL) {
          if (texttype == _LOCALE_TEXT && (strcmp(encoding, _MOTIF_DEFAULT_LOCALE) == 0 ||
 ...
 *
 ************************************************************************/
static int GetUseableText(
    Display *display, XmString xmstring, char **buffer, Boolean strict, unsigned char texttype)
{
  ...
  *buffer = NULL;
  ...
  if (return_status == _INVALID_SEGMENT) {
    ...
    if ((compound_text = XmCvtXmStringToCT(xmstring)) == (char *)NULL) {
      _XmStringContextFree(&stack_context);
      return (XLocaleNotSupported);
    }
          ...
          if (strict) {
            ...
            if (result == XLocaleNotSupported) {
              ...
              return (XmCvtXmStringTableToTextProperty(
                  display, string_table, count, XmSTYLE_COMPOUND_TEXT, text_prop_return));
            }
            else {
