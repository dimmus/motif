...
static XmTab GetNthTab(XmTabList tl, int pos, XmTab cur_tab, int cur_pos)
{
  ...
  if ((cur_tab != NULL) && ((pos > cur_pos / 2) || (pos < (count + cur_pos) / 2))) {
    prev_tab = cur_tab;
    i = pos - cur_pos;
  }
  else {
 ...
 */
XmTabList XmTabListInsertTabs(XmTabList oldlist, XmTab *tabs, Cardinal tab_count, int position)
{
  XmTabList tl;
  int i;
...
Widget _XmCreateTabList(Widget parent,
                        String name,       /* unused */
...
XmTabList XmTabListCopy(XmTabList tablist, int offset, Cardinal count)
{
  ...
  if (count == 0)
    count = (_XmTabLCount(tablist) - abs(offset));
  if (count > _XmTabLCount(tablist))
    count = _XmTabLCount(tablist);
                                    ...
                                    Cardinal tab_count)
{
  ...
  XmTabList tl;
  _XmProcessLock();
  if ((oldlist == NULL) || (position_list == NULL) || (tabs == NULL) || (tab_count == 0)) {
    _XmProcessUnlock();
    return (oldlist);
  }
  tl = (XmTabList)XtMalloc(sizeof(_XmTabListRec));
  ...
  for (i = 0; i < tab_count; i++) {
    cur_tab = GetNthTab(tl, position_list[i], cur_tab, cur_pos);
 ...
 */
XmTabList XmTabListRemoveTabs(XmTabList oldlist, Cardinal *position_list, Cardinal position_count)
{
  XmTab cur_tab, tab, prev, next;
  int cur_pos, i;
  XmTabList tl;
  _XmProcessLock();
  if ((oldlist == NULL) || (position_list == NULL) || (position_count == 0)) {
    _XmProcessUnlock();
    return (oldlist);
  }
  ...
  /* Free marked tabs */
  for (tab = _XmTabNext(_XmTabLStart(oldlist)); tab != _XmTabLStart(oldlist); tab = next) {
    if (_XmTabMark(tab)) {
      prev = _XmTabPrev(tab);
      next = _XmTabNext(tab);
...
Widget _XmCreateTab(Widget parent,
                    String name, /* unused */
                    ArgList arglist,
                    Cardinal argcount)
{
                     ...
                     char **decimal)
{
  ...
  _XmProcessLock();
  if (units != NULL)
    *units = _XmTabUnits(tab);
  if (offset != NULL)
    *offset = _XmTabModel(tab);
  if (alignment != NULL)
    *alignment = _XmTabAlign(tab);
  if (decimal != NULL)
    *decimal = _XmTabDecimal(tab);
 ...
 **********/
Position _XmTabListGetPosition(Screen *screen,
                               XmTabList tab_list,
                               ...
                               Cardinal tab_position)
{
  ...
  if (tab) {
    xpos = (Position)XmTabGetValues(tab, &units, &offset, NULL, NULL);
    ...
    if ((offset == XmRELATIVE) && tab_position) {
      xpos += _XmTabListGetPosition(screen, tab_list, unit_type, tab_position - 1);
    }
    XmTabFree(tab);
  }
  return xpos;
...
static char *units_image(XtEnum units)
{
  static char buf[100];
  switch (units) {
    case XmPIXELS:
...
static char *model_image(XmOffsetModel model)
{
  static char buf[100];
  switch (model) {
    case XmABSOLUTE:
...
static char *alignment_image(XtEnum alignment)
{
  static char buf[100];
  switch (alignment) {
...
void _Xm_dump_tablist(XmTabList list)
{
  int i;
