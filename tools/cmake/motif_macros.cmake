...
macro(define_motif_subsystem _name)
  cmake_parse_arguments("_ds" "" "" "DEPS" ${ARGN})
  string(TOUPPER ${_name} _uname)
  if(NOT DEFINED WITH_${_uname})
    set(WITHL_${_uname} ON)
  endif()
  if(_ds_DEPS)
    cmake_dependent_option(WITH_${_uname} "Enable the ${_name} subsystem" "${WITH_${_uname}}" "${_ds_DEPS}" OFF)
  else()
    option(WITH_${_uname} "Enable the ${_name} subsystem" "${WITH_${_uname}}")
  endif()
  list(APPEND MOTIF_SUBSYSTEMS "${_name}")
endmacro()
...
function(motif_target_include_dirs_impl
  target
  system
  includes
  )
...
function(motif_target_include_dirs
  target
  )
  absolute_include_dirs(_ALL_INCS ${ARGN})
  motif_target_include_dirs_impl(${target} FALSE "${_ALL_INCS}")
endfunction()

function(motif_target_include_dirs_sys
  target
  )
  absolute_include_dirs(_ALL_INCS ${ARGN})
  motif_target_include_dirs_impl(${target} TRUE "${_ALL_INCS}")
endfunction()

# Enable unity build for the given target.
function(motif_set_target_unity_build target batch_size)
  if(WITH_UNITY_BUILD)
...
function(motif_user_header_search_paths
  name
  includes
  )
...
function(motif_add_lib__impl
  ...
  motif_target_include_dirs(${name} ${includes})
  motif_target_include_dirs_sys(${name} ${includes_sys})

  if(library_deps)
    motif_link_libraries(${name} "${library_deps}")
  endif()

  # works fine without having the includes
  # listed is helpful for IDE's (QtCreator/MSVC)
  motif_source_group("${name}" "${sources}")
  motif_user_header_search_paths("${name}" "${includes}")
...
function(motif_add_lib_nolist
  ...
  motif_add_lib__impl(${name} "${sources}" "${includes}" "${includes_sys}" "${library_deps}")
endfunction()

function(motif_add_lib
  name
  sources
  includes
  includes_sys
  ...
  set_property(GLOBAL APPEND PROPERTY MOTIF_LINK_LIBS ${name})
endfunction()
...
function(get_motif_version)
  # extracts header vars and defines them in the parent scope:
  #
  # - MOTIF_VERSION (major.minor)
  # - MOTIF_VERSION_MAJOR
  # - MOTIF_VERSION_MINOR
  # - MOTIF_VERSION_PATCH
  # - MOTIF_VERSION_CYCLE (alpha, beta, rc, release)

  # So CMAKE depends on `motif_version.h`, beware of infinite-loops!
  configure_file(
    ${CMAKE_SOURCE_DIR}/include/motif_version.h
    ${CMAKE_BINARY_DIR}/include/motif_version.h.done
  )

  file(
    STRINGS ${CMAKE_SOURCE_DIR}/include/motif_version.h
    _contents REGEX "^#define[ \t]+MOTIF_.*$"
  )

  string(
    REGEX REPLACE ".*#define[ \t]+MOTIF_VERSION[ \t]+([0-9]+).*" "\\1"
    _out_version "${_contents}"
  )
  string(
    REGEX REPLACE ".*#define[ \t]+MOTIF_VERSION_PATCH[ \t]+([0-9]+).*" "\\1"
    _out_version_patch "${_contents}"
  )
  string(
    REGEX REPLACE ".*#define[ \t]+MOTIF_VERSION_CYCLE[ \t]+([a-z]+).*" "\\1"
    _out_version_cycle "${_contents}"
  )

  if(NOT ${_out_version} MATCHES "[0-9]+")
    message(FATAL_ERROR "Version parsing failed for MOTIF_VERSION")
  endif()

  if(NOT ${_out_version_patch} MATCHES "[0-9]+")
    message(FATAL_ERROR "Version parsing failed for MOTIF_VERSION_PATCH")
  endif()

  if(NOT ${_out_version_cycle} MATCHES "[a-z]+")
    message(FATAL_ERROR "Version parsing failed for MOTIF_VERSION_CYCLE")
  endif()
  ...
  # output vars
  set(MOTIF_VERSION "${_out_version_major}.${_out_version_minor}" PARENT_SCOPE)
  set(MOTIF_VERSION_MAJOR "${_out_version_major}" PARENT_SCOPE)
  set(MOTIF_VERSION_MINOR "${_out_version_minor}" PARENT_SCOPE)
  set(MOTIF_VERSION_PATCH "${_out_version_patch}" PARENT_SCOPE)
  set(MOTIF_VERSION_CYCLE "${_out_version_cycle}" PARENT_SCOPE)
...
function(motif_print_summary)
  message(STATUS "")
  message(STATUS "Motif was configured with the following options:")
  message(STATUS "")
  message(STATUS "Platform: ${CMAKE_SYSTEM}")
  message(STATUS "64-bit:   ${ARCH_64}")
  message(STATUS "Compiler: ${CMAKE_C_COMPILER} (${CMAKE_C_COMPILER_ID} C${CMAKE_C_STANDARD})")
  message(STATUS "Revision: ${MOTIF_REVISION}")
  message(STATUS "Vendor:   ${MOTIF_VENDOR_INFO}")

  message(STATUS "")
  
  message(STATUS "Subsystems:")
  find_stringlength_longest_item(MOTIF_SUBSYSTEMS maxLength)
  foreach(_SUB IN LISTS MOTIF_SUBSYSTEMS)
    string(LENGTH ${_SUB} _SUBLEN)
    math(EXPR _PADLEN "(${maxLength} + 1) - ${_SUBLEN}")
    string(RANDOM LENGTH ${_PADLEN} ALPHABET " " _PADDING)
    string(TOUPPER ${_SUB} _OPT)
    message_bool_option(${_SUB} MOTIF_${_OPT} ${_PADDING})
  endforeach()
  ...
  message(STATUS " Build Shared Library: ${MOTIF_SHARED}")
  message(STATUS " Build Static Library: ${MOTIF_STATIC}")
  if(APPLE)
    message(STATUS " Build libraries as Apple Framework: ${MOTIF_FRAMEWORK}")
  endif()
...
#      if(NOT MOTIF_UNIX_CONSOLE_BUILD)
#        message(FATAL_ERROR
