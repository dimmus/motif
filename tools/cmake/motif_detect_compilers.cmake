...
function(motif_target_compile_option_all_languages TARGET OPTION)
...
function(MOTIF_AddCommonCompilerFlags TARGET)
  option(MOTIF_WERROR "Enable -Werror" OFF)
  ...
  if(USE_GCC OR USE_CLANG OR USE_INTELCC OR USE_QCC)
    ...
    elseif(HAVE_GCC_WALL)
      motif_target_compile_option_all_languages(${TARGET} "-Wall")
      if(HAIKU)
        motif_target_compile_option_all_languages(${TARGET} "-Wno-multichar")
      endif()
    endif()

    check_c_compiler_flag(-Wundef HAVE_GCC_WUNDEF)
    if(HAVE_GCC_WUNDEF)
      motif_target_compile_option_all_languages(${TARGET} "-Wundef")
    endif()

    check_c_compiler_flag(-Wfloat-conversion HAVE_GCC_WFLOAT_CONVERSION)
    if(HAVE_GCC_WFLOAT_CONVERSION)
      motif_target_compile_option_all_languages(${TARGET} "-Wfloat-conversion")
    endif()

    check_c_compiler_flag(-fno-strict-aliasing HAVE_GCC_NO_STRICT_ALIASING)
    if(HAVE_GCC_NO_STRICT_ALIASING)
      motif_target_compile_option_all_languages(${TARGET} "-fno-strict-aliasing")
    endif()
    ...
    if(HAVE_GCC_WDOCUMENTATION)
      if(MOTIF_WERROR)
        ...
        if(HAVE_GCC_WERROR_DOCUMENTATION)
          motif_target_compile_option_all_languages(${TARGET} "-Werror=documentation")
        endif()
      endif()
      motif_target_compile_option_all_languages(${TARGET} "-Wdocumentation")
    endif()
    ...
    if(HAVE_GCC_WDOCUMENTATION_UNKNOWN_COMMAND)
      if(MOTIF_WERROR)
        ...
        if(HAVE_GCC_WERROR_DOCUMENTATION_UNKNOWN_COMMAND)
          motif_target_compile_option_all_languages(${TARGET} "-Werror=documentation-unknown-command")
        endif()
      endif()
      motif_target_compile_option_all_languages(${TARGET} "-Wdocumentation-unknown-command")
    endif()
    ...
    if(HAVE_GCC_COMMENT_BLOCK_COMMANDS)
      motif_target_compile_option_all_languages(${TARGET} "-fcomment-block-commands=threadsafety")
    else()
      ...
      if(HAVE_CLANG_COMMENT_BLOCK_COMMANDS)
        motif_target_compile_option_all_languages(${TARGET} "/clang:-fcomment-block-commands=threadsafety")
      endif()
    endif()

    check_c_compiler_flag(-Wshadow HAVE_GCC_WSHADOW)
    if(HAVE_GCC_WSHADOW)
      motif_target_compile_option_all_languages(${TARGET} "-Wshadow")
    endif()

    check_c_compiler_flag(-Wunused-local-typedefs HAVE_GCC_WUNUSED_LOCAL_TYPEDEFS)
    if(HAVE_GCC_WUNUSED_LOCAL_TYPEDEFS)
      motif_target_compile_option_all_languages(${TARGET} "-Wno-unused-local-typedefs")
    endif()

    check_c_compiler_flag(-Wimplicit-fallthrough HAVE_GCC_WIMPLICIT_FALLTHROUGH)
    if(HAVE_GCC_WIMPLICIT_FALLTHROUGH)
      motif_target_compile_option_all_languages(${TARGET} "-Wimplicit-fallthrough")
    endif()
  endif()
  ...
  if(MOTIF_WERROR)
    ...
    elseif(USE_GCC OR USE_CLANG OR USE_INTELCC OR USE_QNX)
      ...
      if(HAVE_WERROR)
        motif_target_compile_option_all_languages(${TARGET} "-Werror")
      endif()
  ...
  if(USE_CLANG)
    ...
    if(COMPILER_SUPPORTS_FCOLOR_DIAGNOSTICS)
      motif_target_compile_option_all_languages(${TARGET} "-fcolor-diagnostics")
    endif()
  else()
    ...
    if(COMPILER_SUPPORTS_FDIAGNOSTICS_COLOR_ALWAYS)
      motif_target_compile_option_all_languages(${TARGET} "-fdiagnostics-color=always")
    endif()
  endif()
endfunction()
...
function(check_x86_source_compiles BODY VAR)
  ...
  if(APPLE_MULTIARCH AND (MOTIF_CPU_X86 OR MOTIF_CPU_X64))
...
function(check_arm_source_compiles BODY VAR)
  ...
  if(APPLE_MULTIARCH AND (MOTIF_CPU_ARM32 OR MOTIF_CPU_ARM64))
    set(test_conditional 1)
  else()
    set(test_conditional 0)
  endif()
