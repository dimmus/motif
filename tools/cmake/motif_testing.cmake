...
function(get_motif_test_install_dir VARIABLE_NAME)
...
function(motif_test_set_envvars testname envvar_list)
  ...
  if(WITH_COMPILER_CODE_COVERAGE AND CMAKE_C_COMPILER_ID MATCHES "Clang")
    list(APPEND envvar_list "LLVM_PROFILE_FILE=${COMPILER_CODE_COVERAGE_DATA_DIR}/raw/motif_%p.profraw")
  endif()
...
macro(motif_src_gtest_ex)
  if(WITH_GTESTS)
    ...
    motif_link_libraries(${TARGET_NAME} "${ARG_EXTRA_LIBS};${PLATFORM_LINKLIBS}")
    if(WITH_TBB)
    ...
    get_motif_test_install_dir(TEST_INSTALL_DIR)
...
function(motif_add_ctests)
  if(ARGC LESS 1)
    message(FATAL_ERROR "No arguments supplied to motif_add_ctests()")
  endif()
  ...
  motif_test_set_envvars("${ARGS_SUITE_NAME}" "")

  unset(_test_release_dir)
endfunction()

# Add tests for a Blender library, to be called in tandem with motif_add_lib().
#
# If WITH_TESTS_SINGLE_BINARY is enabled, tests will be put into the motif_test
...
# motif_add_test_suite_executable.
#
...
function(motif_add_test_suite_lib
  name
  sources
  includes
  includes_sys
  ...
  if(WITH_TESTS_SINGLE_BINARY)
    ...
    # This duplicates logic that's also in motif_src_gtest_ex.
    ...
    motif_add_lib__impl(${name}_tests
...
function(motif_add_test_executable_impl
  name
  sources
  includes
  includes_sys
  ...
  motif_target_include_dirs(${name}_test ${includes})
  motif_target_include_dirs_sys(${name}_test ${includes_sys})
  motif_source_group("${name}_test" "${sources}")
endfunction()
...
# very large executable, motif_add_test_suite_lib() should be used instead.
#
# The function accepts an optional argument which denotes list of sources which
# is to be compiled-in with the suit sources for each fo the suites when the
# WITH_TESTS_SINGLE_BINARY configuration is set to OFF.
function(motif_add_test_suite_executable
  name
  sources
  includes
  includes_sys
  ...
  if(WITH_TESTS_SINGLE_BINARY)
    motif_add_test_executable_impl(
      "${name}"
...
# Add test for a Blender library, to be called in tandem with motif_add_lib().
# Source files will be compiled into a single ${name}_test executable.
#
# To be used for smaller isolated libraries, that do not have many dependencies.
# For libraries that do drag in many other Blender libraries and would create a
# very large executable, motif_add_test_lib() should be used instead.
function(motif_add_test_executable
  name
  sources
  includes
  includes_sys
...
# Add performance test. This is like motif_add_test_executable, but no ctest
# is generated and the binary should be run manually.
function(motif_add_test_performance_executable
  name
  sources
  includes
  includes_sys
