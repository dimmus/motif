...
# -----------------------------------------------------------------------------
# YACC/LEX Tool Detection
# Equivalent to AC_CHECK_PROGS(YACC, byacc, yacc) and AC_PROG_LEX([yywrap])
# -----------------------------------------------------------------------------

macro(motif_detect_yacc_lex)
    # Check for YACC/Bison (prefer bison, fallback to yacc/byacc)
    find_program(BISON_EXECUTABLE bison)
    if(NOT BISON_EXECUTABLE)
        find_program(YACC_EXECUTABLE yacc)
        if(NOT YACC_EXECUTABLE)
            find_program(BYACC_EXECUTABLE byacc)
            if(BYACC_EXECUTABLE)
                set(BISON_EXECUTABLE ${BYACC_EXECUTABLE})
                set(USING_BYACC TRUE)
            else()
                message(FATAL_ERROR "Neither bison, yacc, nor byacc found. Please install one of them.")
            endif()
        else()
            set(BISON_EXECUTABLE ${YACC_EXECUTABLE})
            set(USING_YACC TRUE)
        endif()
    endif()

    # Check for LEX/Flex (prefer flex, fallback to lex)
    find_program(FLEX_EXECUTABLE flex)
    if(NOT FLEX_EXECUTABLE)
        find_program(LEX_EXECUTABLE lex)
        if(LEX_EXECUTABLE)
            set(FLEX_EXECUTABLE ${LEX_EXECUTABLE})
            set(USING_LEX TRUE)
        else()
            message(FATAL_ERROR "Neither flex nor lex found. Please install one of them.")
        endif()
    endif()

    # Check for flex library (libfl) - needed when using flex
    # Equivalent to LEXLIB logic from configure.ac
    if(NOT USING_LEX)
        find_library(FLEX_LIBRARY fl)
        if(NOT FLEX_LIBRARY)
            # Try alternative names
            find_library(FLEX_LIBRARY flex)
            if(NOT FLEX_LIBRARY)
                find_library(FLEX_LIBRARY fl_pic)
            endif()
        endif()
        if(FLEX_LIBRARY)
            message(STATUS "Found flex library: ${FLEX_LIBRARY}")
        else()
            message(WARNING "Flex library not found. Some programs may not link correctly.")
        endif()
    else()
        set(FLEX_LIBRARY "")
    endif()

    # Report which tools are being used
    if(USING_YACC)
        message(STATUS "Using yacc: ${BISON_EXECUTABLE}")
    elseif(USING_BYACC)
        message(STATUS "Using byacc: ${BISON_EXECUTABLE}")
    else()
        message(STATUS "Using bison: ${BISON_EXECUTABLE}")
    endif()

    if(USING_LEX)
        message(STATUS "Using lex: ${FLEX_EXECUTABLE}")
    else()
        message(STATUS "Using flex: ${FLEX_EXECUTABLE}")
    endif()

    # Make variables available globally
    set(MOTIF_BISON_EXECUTABLE ${BISON_EXECUTABLE} CACHE INTERNAL "Bison/YACC executable")
    set(MOTIF_FLEX_EXECUTABLE ${FLEX_EXECUTABLE} CACHE INTERNAL "Flex/LEX executable")
    set(MOTIF_FLEX_LIBRARY ${FLEX_LIBRARY} CACHE INTERNAL "Flex library")
    set(MOTIF_USING_YACC ${USING_YACC} CACHE INTERNAL "Using yacc")
    set(MOTIF_USING_BYACC ${USING_BYACC} CACHE INTERNAL "Using byacc")
    set(MOTIF_USING_LEX ${USING_LEX} CACHE INTERNAL "Using lex")
endmacro()
